...
install_command() {
	if [ "$installcommand" = '' ]; then
		echo 'Error detected: Empty install command!' >&2
		exit 1
	fi
	if [ "$manual_install" = "1" ]; then
		echo 'Proposed command to use:'
		echo
		echo "$installcommand"
...
Debian | Raspbian)
	...
	oldstable | 8 | 8.*)
		...
		installcommand="apt-get install $APT_OPTIONS build-essential cmake libcurl4-gnutls-dev libsdl2-dev libopenal-dev liblua5.2-dev libjpeg-dev libpng12-dev libfreetype6-dev libwxgtk3.0-dev libcppunit-dev libfribidi-dev libftgl-dev libogg-dev libvorbis-dev libminiupnpc-dev libircclient-dev -dev libxml2-dev libx11-dev libgl1-mesa-dev libglu1-mesa-dev librtmp-dev libkrb5-dev libldap2-dev libidn11-dev libgnutls28-dev"
		;;
	stable | 9 | 9.*)
...
LinuxMint | Linuxmint)
	...
	17 | 17.*)
		...
		installcommand="apt-get install $APT_OPTIONS build-essential cmake libsdl2-dev libalut-dev libgl1-mesa-dev libglu1-mesa-dev libvorbis-dev libwxgtk3.0-dev libx11-dev liblua5.1-0-dev libjpeg-dev libpng12-dev libcurl4-gnutls-dev libxml2-dev libircclient-dev libftgl-dev libfribidi-dev -dev core-dev libcppunit-dev"
		;;
	18 | 18.*)
		#LTS, based on Ubuntu 16.04, EoL April 2021
		installcommand="apt-get install $APT_OPTIONS build-essential cmake libcurl4-gnutls-dev libsdl2-dev libopenal-dev liblua5.3-dev libjpeg-dev libpng12-dev libfreetype6-dev libwxgtk3.0-dev libcppunit-dev libfribidi-dev libftgl-dev libogg-dev libvorbis-dev libminiupnpc-dev libircclient-dev -dev core-dev libxml2-dev libx11-dev libgl1-mesa-dev libglu1-mesa-dev librtmp-dev libkrb5-dev libldap2-dev libidn11-dev libgnutls28-dev libnghttp2-dev libssh2-1-dev"
		;;
	21 | 21.*)
...
Mageia)
	...
	case $release in
	*)
		installcommand="urpmi $URPMI_OPTIONS gcc gcc-c++ cmake make ${lib}curl-devel ${lib}sdl2.0-devel ${lib}openal-devel ${lib}lua${static}-devel ${lib}jpeg${static}-devel ${lib}png-devel ${lib}freetype6${static}-devel ${lib}wxgtku3.0-devel ${lib}cppunit-devel ${lib}fribidi${static}-devel ${lib}ftgl-devel-devel ${lib}ogg${static}-devel ${lib}vorbis-devel ${lib}miniupnpc-devel ${lib}ircclient-static-devel ${lib}xml2-devel ${lib}x11-devel ${lib}mesagl1-devel ${lib}mesaglu1-devel ${lib}openssl${static}-devel"
		unsupported_currently_this_OS="release"
		;;
	esac
	;;
...
Arch | archlinux | ManjaroLinux* | Manjarolinux*)
	...
	installcommand="pacman $PACMAN_OPTIONS -S --needed gcc-multilib cmake ${lib}libcurl-gnutls ${lib}sdl2 ${lib}openal lua54 ${lib}libjpeg-turbo ${lib}libpng ${lib}freetype2 wxwidgets-gtk3 cppunit fribidi ftgl ${lib}libogg ${lib}libvorbis miniupnpc libircclient ${lib}libxml2 ${lib}libx11 ${lib}mesa ${lib}glu"
	;;
...
if [ "$unsupported_currently_this_OS" = "release" ]; then
	unsupported_release
elif [ "$unsupported_currently_this_OS" = "distribution" ]; then
	unsupported_distribution

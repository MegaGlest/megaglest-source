...
while getopts "c:dfhl:q:mnx" option; do
   case "${option}" in
        c)
           CPU_COUNT=${OPTARG}
...
#           echo "${option} value: ${OPTARG}"
        ...
        h)
                echo "Usage: $0 <option>"
                echo "       where <option> can be: -c x, -d, -f, -m, -n, -h, -l x, -q x, -x"
		...
		echo "       -q x : Force using Qt version x - example: -q 5"
...
#           echo "${option} value: ${OPTARG} LUA_FORCED_VERSION [${LUA_FORCED_VERSION}]"
        ;;
        q)
           QT_FORCED_VERSION=${OPTARG}
#           echo "${option} value: ${OPTARG} QT_FORCED_VERSION [${QT_FORCED_VERSION}]"
        ;;
        m)
           CMAKE_ONLY=1
#           echo "${option} value: ${OPTARG}"
	...
	Fedora)
		case $release in
			*)
				echo 'Turning ON dynamic CURL ...'
				echo 'Using Qt 5 ...'
				EXTRA_CMAKE_OPTIONS="${EXTRA_CMAKE_OPTIONS} -DFORCE_CURL_DYNAMIC_LIBS=ON -DUSE_QT5=On"
				;;
		esac
		;;
	Ubuntu)
		case $release in
			13.10|14.04)
				echo 'Using Qt 5 ...'
				EXTRA_CMAKE_OPTIONS="${EXTRA_CMAKE_OPTIONS} -DUSE_QT5=On"
				;;
		esac
		;;
	Arch)
		...
		echo 'Using Qt 5 ...'
		EXTRA_CMAKE_OPTIONS="${EXTRA_CMAKE_OPTIONS} -DWANT_STATIC_LIBS=OFF -DUSE_QT5=On"
		;;
esac
...
if [ $QT_FORCED_VERSION != 0 ]; then
        if [ $QT_FORCED_VERSION = 5 ]; then
                EXTRA_CMAKE_OPTIONS="${EXTRA_CMAKE_OPTIONS} -DUSE_QT5=ON"
                echo "USER WANTS TO FORCE USE of Qt 5"
        elif [ $QT_FORCED_VERSION = 4 ]; then
                EXTRA_CMAKE_OPTIONS="${EXTRA_CMAKE_OPTIONS} -DUSE_QT5=OFF"
                echo "USER WANTS TO FORCE USE of Qt 4"
        fi
fi

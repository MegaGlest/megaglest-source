...
inline static void get_system_time(uuid_time_t *uuid_time) {
    ...
    *uuid_time = ((uint64_t)tp.tv_sec * 10000000) + ((uint64_t)tp.tv_usec * 10) + I64(0x01B21DD213814000);
}
...
inline void puid(uuid_t u) {
    ...
    for (i = 0; i < 6; i++) printf("%2.2x", u.node[i]);
    printf("\n");
}
...
inline void snpuid(char *str, size_t size, uuid_t u) {
    ...
    sprintf(tmp, "%8.8x-%4.4x-%4.4x-%2.2x%2.2x-", u.time_low, u.time_mid, u.time_hi_and_version, u.clock_seq_hi_and_reserved, u.clock_seq_low);
    tmp += 24;
    for (i = 0; i < 6; i++) {
...
/* format_uuid_v1 -- make a UUID from the timestamp, clockseq,
   and node ID */
inline static void format_uuid_v1(uuid_t *uuid, uint16_t clock_seq, uuid_time_t timestamp, uuid_node_t node) {

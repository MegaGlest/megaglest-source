...
template <typename T> class SingleFactory : public SingleFactoryBase {
  public:
...
template <typename T> class MultiFactory {
  private:
  ...
  public:
    ...
    template <typename R> void registerClass(string classId) { factories.insert(FactoryPair(classId, new SingleFactory<R>())); }
    ...
    bool isClassId(string classId) { return factories.find(classId) != factories.end(); }
};

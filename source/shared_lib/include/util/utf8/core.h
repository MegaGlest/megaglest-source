...
utf_error get_sequence_1(octet_iterator& it, octet_iterator end,
                         uint32_t* code_point) {
  if (it != end) {
    if (code_point) *code_point = mask8(*it);
    return UTF8_OK;
  }
...
utf_error validate_next(octet_iterator& it, octet_iterator end,
  ...
  if (length == 0) return INVALID_LEAD;

  // Now that we have a valid sequence length, get trail octets and calculate
  // the code point
...
inline bool is_bom(octet_iterator it) {
  return ((internal::mask8(*it++)) == bom[0] &&
...
/*
    ...
    inline bool starts_with_rlm (octet_iterator it, octet_iterator end)
    {
        ...
        while (result != end) {
            internal::utf_error err_code = internal::validate_next(result, end,
&code_point); if (err_code != internal::UTF8_OK) { return false;
            }
            else {
...
//                if(((result != end) && (internal::mask8(*result)) 	   ==
rlm[0]) &&
//                   ((result != end) && (internal::mask8(*result + 1))  ==
rlm[1]) &&
//                   ((result != end) && (internal::mask8(*result + 2))  ==
rlm[2])) {
//                	return true;
//                }

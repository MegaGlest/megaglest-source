...
// START - Special includes because the use a special new operator that we
// cannot override
#include "rapidxml.hpp"
#include <string>

#include <xercesc/dom/DOM.hpp>
// END - Special includes because the use a special new operator that we cannot
// override
...
public:
  ...
  inline AllocInfo()
      : ptr(0), file(""), line(-1), bytes(0), array(false), freetouse(false),
        inuse(false), stack("") {}

  inline AllocInfo(void *ptr, const char *file, int line, string stacktrace,
                   size_t bytes, bool array)
      : ptr(ptr), file(file), line(line), bytes(bytes), array(array),
        freetouse(false), inuse(true), stack(stacktrace) {}

// #if defined(__GNUC__) && !defined(__FreeBSD__) && !defined(BSD)
#if defined(HAS_GCC_BACKTRACE)
  inline static int getFileAndLine(char *function, void *address, char *file,
                                   size_t flen) {
    int line = -1;
    if (want_full_leak_stacktrace_line_numbers == true &&
        AllocInfo::get_application_binary() != "") {
      ...
      snprintf(buf, 8096, "addr2line -C -e %s -f -i %p",
               AllocInfo::get_application_binary().c_str(), address);
...
// !defined(BSD)
    ...
    if (want_full_leak_stacktrace == true) {
      ...
      // errMsg += "readelf --debug-dump=decodedline %s | egrep
      // 0xaddress-of-stack\n";
        ...
        for (size_t i = 1; i < stack_depth; i++) {
          ...
          if (begin && end) {
            ...
            *end = '\0';
            ...
            snprintf(szBuf, 8096, "%s:%s address [%p]", stack_strings[i],
                     function, lineAddress);
          } else {
            // didn't find the mangled name, just print the whole line
            // fprintf(out, "    %s\n", stack.strings[i]);
            snprintf(szBuf, 8096, "%s address [%p]", stack_strings[i],
                     lineAddress);
          }
...
public:
  ...
  inline void allocate(AllocInfo info) {
    ...
    // printf("ALLOCATE.\tfile: %s, line: %d, bytes: " MG_SIZE_T_SPECIFIER ",
    // array: %d inuse: %d\n", info.file, info.line, info.bytes, info.array,
    // info.inuse);
    ...
    printf("ALLOCATE NOT MONITORED.\tfile: %s, line: %d, ptr [%p], "
           "bytes: " MG_SIZE_T_SPECIFIER ", array: %d inuse: %d, \n%s\n",
           info.file, info.line, info.ptr, info.bytes, info.array, info.inuse,
           info.stack.c_str());
  ...
  inline void deallocate(void *ptr, bool array, const char *file, int line) {
    ...
    if (line > 0)
      printf("WARNING, possible error on pointer delete for ptr [%p] array = "
             "%d, file [%s] line: %d\n%s\n",
...
inline void *operator new(size_t bytes) {
  ...
  AllocRegistry::getInstance().allocate(
      AllocInfo(ptr, "unknown", 0, "", bytes, false));
  return ptr;
}
...
inline void *operator new[](size_t bytes) {
  void *ptr = malloc(bytes);
  AllocRegistry::getInstance().allocate(
      AllocInfo(ptr, "unknown", 0, "", bytes, true));
  return ptr;
}
...
inline void *operator new(size_t bytes, const char *file, int line,
  ...
  void *ptr = malloc(bytes);
  AllocRegistry::getInstance().allocate(
      AllocInfo(ptr, file, line, stack, bytes, false));
  return ptr;
}
...
inline void *operator new[](size_t bytes, const char *file, int line,
  ...
  void *ptr = malloc(bytes);
  AllocRegistry::getInstance().allocate(
      AllocInfo(ptr, file, line, stack, bytes, true));
  return ptr;
}

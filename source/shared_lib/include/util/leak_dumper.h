...
struct AllocInfo {
  ...
  public:
    ...
    inline AllocInfo() : ptr(0), file(""), line(-1), bytes(0), array(false), freetouse(false), inuse(false), stack("") {}
...
class AllocRegistry {
  ...
  public:
    ...
    inline void deallocate(void *ptr, bool array, const char *file, int line) {
        ...
        for (int i = 0; i < maxAllocs; ++i) {
            if (allocs[i].freetouse == false && allocs[i].inuse == true && allocs[i].ptr == ptr && allocs[i].array == array) {
                allocs[i].freetouse = true;
                allocs[i].inuse = false;
                nextFreeIndex = i;
        ...
        if (line > 0)
            printf("WARNING, possible error on pointer delete for ptr [%p] array = %d, "
                   "file [%s] line: %d\n%s\n",

...
template <typename T> inline T strToType(const std::string &s) {
  ...
  if (*endChar != '\0') {
    throw std::runtime_error("Error converting from string to type, found: [" +
                             s + "]");
  }
...
public:
  ...
  inline float length() const {
    ...
    float len = static_cast<float>(
        streflop::sqrt(static_cast<streflop::Simple>(x * x + y * y)));
#else
    float len =
        static_cast<float>(std::sqrt(static_cast<float>(x * x + y * y)));
...
public:
...
#ifdef USE_STREFLOP
    float len = static_cast<float>(
        streflop::sqrt(static_cast<streflop::Simple>(x * x + y * y + z * z)));
#else
  ...
  inline Vec3<T> cross(const Vec3<T> &v) const {
    return Vec3<T>(this->y * v.z - this->z * v.y, this->z * v.x - this->x * v.z,
  ...
  inline Vec3<T> normal(const Vec3<T> &p1, const Vec3<T> &p2, const Vec3<T> &p3,
                        const Vec3<T> &p4) const {
    Vec3<T> rv;

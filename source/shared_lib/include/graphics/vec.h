...
template <typename T> inline T truncateDecimal(const T &value, int precision = 6) {
    T precNum = 0;
    if (precision == 0) {
        precNum = 1;
    } else if (precision == 1) {
    ...
    //		//printf("=======================\nvalue = %.10f\nresultInt:
    //%d\nprecision: %d\nbecame:
    //%.10f\n----------\n",value,resultInt,precision,result); 		return
    // result;
    //	}
...
template <typename T> inline T strToType(const std::string &s) {
    char *endChar = NULL;
...
template <class T> void toEndianVecArray(T *vec, size_t size) {
...
template <class T> void fromEndianVecArray(T *vec, size_t size) {
    static bool bigEndianSystem = Shared::PlatformByteOrder::isBigEndian();
    if (bigEndianSystem == true) {
        for (size_t i = 0; i < size; ++i) {
            vec[i].fromEndian();
...
template <typename T> class Vec2 {
  ...
  public:
    ...
    template <typename S> explicit Vec2(Vec2<S> &v) {
        this->x = v.x;
        this->y = v.y;
    }
    ...
    inline const T *ptr() const { return reinterpret_cast<const T *>(this); }
    ...
    inline Vec2<T> operator+(const Vec2<T> &v) const { return Vec2(x + v.x, y + v.y); }

    inline Vec2<T> operator-(const Vec2<T> &v) const { return Vec2(x - v.x, y - v.y); }

    inline Vec2<T> operator-() const { return Vec2(-x, -y); }

    inline Vec2<T> operator*(const Vec2<T> &v) const { return Vec2(x * v.x, y * v.y); }

    inline Vec2<T> operator*(T s) const { return Vec2(x * s, y * s); }
    ...
    inline Vec2<T> lerp(T t, const Vec2<T> &v) const { return *this + (v - *this) * t; }
    ...
    inline Vec2<T> rotateAround(float rad, const Vec2<T> &pt) { return pt + (*this - pt).rotate(rad); }
...
template <typename T> inline std::ostream &operator<<(std::ostream &stream, const Vec2<T> &vec) {
...
template <typename T> class Vec3 {
  ...
  public:
    ...
    template <typename S> explicit Vec3(Vec3<S> &v) {
        this->x = v.x;
        this->y = v.y;
        this->z = v.z;
    }
    ...
    inline Vec3<T> operator*(T s) const { return Vec3(x * s, y * s, z * s); }
    ...
    inline bool operator<(const Vec3<T> &v) const { return x < v.x || (x == v.x && y < v.y) || (x == v.x && y == v.y && z < v.z); }

    inline Vec3<T> lerp(T t, const Vec3<T> &v) const { return *this + (v - *this) * t; }
...
template <typename T> class Vec4 {
  ...
  public:
    ...
    template <typename S> explicit Vec4(Vec4<S> &v) {
        this->x = v.x;
        this->y = v.y;
        this->z = v.z;
        this->w = v.w;
    ...
    inline Vec4<T> operator+(const Vec4<T> &v) const { return Vec4(x + v.x, y + v.y, z + v.z, w + v.w); }

    inline Vec4<T> operator-(const Vec4<T> &v) const { return Vec4(x - v.x, y - v.y, z - v.z, w - v.w); }

    inline Vec4<T> operator-() const { return Vec4(-x, -y, -z, -w); }

    inline Vec4<T> operator*(const Vec4<T> &v) const { return Vec4(x * v.x, y * v.y, z * v.z, w * v.w); }

    inline Vec4<T> operator*(T s) const { return Vec4(x * s, y * s, z * s, w * s); }
    ...
    inline Vec4<T> lerp(T t, const Vec4<T> &v) const { return *this + (v - *this) * t; }
    ...
    static inline Vec4<T> strToVec4(std::string value) {
        ...
        if (tokens.size() == 5) {
            ...
            if (tokens2.size() == 2 && tokens3.size() == 2 && tokens4.size() == 2 && tokens5.size() == 2) {
                result.x = (T)strToType<T>(tokens2[0]);
                result.y = (T)strToType<T>(tokens3[0]);
                result.z = (T)strToType<T>(tokens4[0]);

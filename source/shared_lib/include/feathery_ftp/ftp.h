...
typedef struct {
  ...
  int authenticated; ///< TRUE, if user is authenticated via USER and PASS
                     ///< commands
  int userId; ///< id of the dedicated user account, is 0 until execution of
              ///< USER command
  int rxBufWriteIdx;     ///< currect write index of receive buffer rxBuf
  char rxBuf[LEN_RXBUF]; ///< receive buffer for ftp commands
  char workingDir[MAX_PATH_LEN]; ///< current working directory (absolute path
                                 ///< which is relative to the root-path of the
                                 ///< user account)
  ip_t remoteIp;                 ///< IP of connected ftp client
  port_t
      remoteFTPServerPassivePort; ///< Port of the FTP Server from the clients
                                  ///< perspective related to Passive connection
  ...
  uint32_t timeLastCmd;  ///< timestamp of last ftp activity (used for timeout
                         ///< generation)
  socket_t ctrlSocket;   ///< socket for control connection
  socket_t passiveDataSocket; ///< listener socket for data connections in
                              ///< passive mode
  ip_t passiveIp; ///< IP of the FTP Server from the clients perspective related
                  ///< to Passive connection
  port_t passivePort; ///< Port of the FTP Server from the clients perspective
                      ///< related to Passive connection
  transmission_S activeTrans; ///< infos about a currently active
                              ///< file/directory-transmission
...
extern int ftpOpenSession(socket_t ctrlSocket, ip_t remoteIp,
                          port_t remotePort);
...
extern void ftpOpenTransmission(int id, operation_E op, void *fsHandle,
                                socket_t dataSocket, uint32_t fileSize);
...
extern int ftpWriteFile(const void *buffer, size_t size, size_t count,
                        void *stream);
...
extern socket_t ftpAcceptServerConnection(socket_t server, ip_t *remoteIP,
                                          port_t *remotePort);

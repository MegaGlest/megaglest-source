...
void printParameterHelp(const char *argv0, bool foundInvalidArgs) {
    ...
    printf("\n\n                     \tWhere x is an optional name of the saved "
           "game file to load.");
    printf("\n\n                     \tIf x is not specified we load the last game "
           "that was saved.");

    printf("\n\n%s=x,y,z  \tRun in auto test mode.", GAME_ARGS[GAME_ARG_AUTO_TEST]);
    printf("\n\n                     \tWhere x is an optional maximum # seconds to "
           "play.");
    printf("\n\n                     \tIf x is not specified the default is 1200 "
           "seconds (20 minutes).");
    printf("\n\n                     \tWhere y is an optional game settings file to "
           "play.");
    ...
    printf("\n\n                     \tWhere z is the word 'exit' indicating the "
           "game should exit");
    printf("\n\n                     \tafter the game is finished or the time runs "
           "out. If z is");
    printf("\n\n                     \tnot specified (or is empty) then auto test "
           "continues to cycle.");

    printf("\n\n%s=x:y  \t\tAuto connect to host server at IP or hostname x using", GAME_ARGS[GAME_ARG_CONNECT]);
    printf("\n\n                     \t    port y. Shortcut version of using %s and "
           "%s.",
           GAME_ARGS[GAME_ARG_CLIENT], GAME_ARGS[GAME_ARG_USE_PORTS]);
    printf("\n\n                     \t*NOTE: to automatically connect to the first "
           "LAN host you may");
    ...
    printf("\n\n                     \t'exit' - which quits the application after a "
           "game has no more");
    printf("\n\n                     \t    connected players.");
    printf("\n\n                     \t'vps' - which does NOT read commands from "
           "the local console");
    printf("\n\n                     \t    (for some vps's).");
    printf("\n\n                     \t'lan' - which does not broadcast the hosting "
           "server to the");
    ...
    printf("\n\n                     \tWhere y is the external port # on the "
           "router/proxy to");
    printf("\n\n                     \t    forward connection from to the internal "
           "port #.");
    ...
    printf("\n\n                     \tAuto Preview a map by map name. (tileset is "
           "optional)");
    ...
    printf("\n\n                     \tWhere 'purgeunused' is an optional parameter "
           "telling the");
    printf("\n\n                     \t    validation to delete extra files in the "
           "techtree that");
    printf("\n\n                     \t    are not used.");
    printf("\n\n                     \tWhere 'purgeduplicates' is an optional "
           "parameter telling");
    printf("\n\n                     \t    the validation to merge duplicate files "
           "in the techtree.");
    printf("\n\n                     \tWhere 'gitdelete' is an optional parameter "
           "telling the");
    printf("\n\n                     \t    validation to call 'git rm' on duplicate "
           "/ unused files");
    printf("\n\n                     \t    in the techtree.");
    printf("\n\n                     \tWhere 'hideduplicates' is an optional "
           "parameter telling the");
    printf("\n\n                     \t    validation to NOT SHOW duplicate files "
           "in the techtree.");
    ...
    printf("\n\n                     \tWhere 'purgeduplicates' is an optional "
           "parameter telling the");
    printf("\n\n                     \t    validation to merge duplicate files in "
           "the faction.");
    printf("\n\n                     \tWhere 'hideduplicates' is an optional "
           "parameter telling the");
    printf("\n\n                     \t    validation to NOT SHOW duplicate files "
    ...
    printf("\n\n                     \tConvert a model file or folder to the "
           "current g3d version");
    printf("\n\n                     \t    format.");
    printf("\n\n                     \tWhere x is a filename or folder containing "
           "the g3d model(s).");
    ...
    printf("\n\n                     \tWhere 'keepsmallest' is an optional flag "
           "indicating to keep");
    printf("\n\n                     \t    the original texture if its filesize is "
           "smaller than the");
    printf("\n\n                     \t    converted format.");
    printf("\n\n                     \texample:");
    ...
    printf("\n\n                     \t    by x. Where x is a language filename or "
           "ISO639-1 code.");
    ...
    printf("\n\n                     \tWhere x is a valid colorbits value supported "
           "by your video");
    printf("\n\n                     \t    driver.");
    ...
    printf("\n\n                     \tWhere x is a valid depthbits value supported "
           "by your video");
    printf("\n\n                     \t    driver.");
    ...
    printf("\n\n                     \tWhere x is the path and name of a font file "
           "supported by");
    printf("\n\n                     \t    freetype2.");
    printf("\n\n                     \texample:");
    printf("\n\n                     \t%s "
           "%s=$APPLICATIONDATAPATH/data/core/fonts/Vera.ttf",
           extractFileFromDirectoryPath(argv0).c_str(), GAME_ARGS[GAME_ARG_USE_FONT]);
    ...
    printf("\n\n                     \tCompress selected game data into archives "
           "for network sharing.");
    printf("\n\n                     \tWhere x is one of the following data items "
           "to compress:");
    printf("\n\n                     \t    techtrees, tilesets or all.");
    printf("\n\n                     \tWhere y = include_main to include main (non "
           "mod) data.");
...
// Ensure at runtime that the client has SSE
        ...
        if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] sse check got "
                   ...
                   __FILE__, __FUNCTION__, __LINE__, has_x64, has_MMX, has_SSE, has_SSE2, has_SSE3, has_SSSE3, has_SSE41, has_SSE42, has_SSE4a, has_AVX,
                   has_XOP, has_FMA3, has_FMA4);

        if (has_SSE == false && has_SSE2 == false && has_SSE3 == false && has_SSSE3 == false && has_SSE41 == false && has_SSE42 == false &&
            has_SSE4a == false) {
            char szBuf[8096] = "";
...
#define cpuid(func, ax, bx, cx, dx) __asm__ __volatile__("cpuid" : "=a"(ax), "=b"(bx), "=c"(cx), "=d"(dx) : "a"(func));
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_OPENGL_INFO]) == true || hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SDL_INFO]) == true ||
        ...
        hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_VALIDATE_TILESET]) == true || hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LIST_MAPS]) == true ||

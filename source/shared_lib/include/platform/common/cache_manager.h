...
class CacheManager {
  ...
  protected:
    ...
    template <typename T> static Mutex &manageCachedItemMutex(string cacheKey) {
    ...
    template <typename T> static T &manageCachedItem(string cacheKey, T *value, CacheAccessorType accessor) {
  ...
  public:
    ...
    static void cleanupMutexes() {
        MutexSafeWrapper safeMutex(&mutexMap);
        for (std::map<string, Mutex *>::iterator iterMap = itemCacheMutexList.begin(); iterMap != itemCacheMutexList.end(); iterMap++) {
    ...
    template <typename T> static void setCachedItem(string cacheKey, const T value) { manageCachedItem<T>(cacheKey, value, cacheItemSet); }
    template <typename T> static T &getCachedItem(string cacheKey) { return manageCachedItem<T>(cacheKey, NULL, cacheItemGet); }
    template <typename T> static void clearCachedItem(string cacheKey) { manageCachedItem<T>(cacheKey, NULL, cacheItemSet); }

    template <typename T> static Mutex &getMutexForItem(string cacheKey) { return manageCachedItemMutex<T>(cacheKey); }
};

  ...
  public:
    ...
    bool operator<(const ModeInfo &j) const {
        if (this->width < j.width) {
            ...
            return true;
        } else if (this->width == j.width && this->height == j.height && this->depth < j.depth) {
            return true;
        }
...
vector<string> getFolderTreeContentsListRecursively(const string &path, const string &filterFileExt, bool includeFolders = false,

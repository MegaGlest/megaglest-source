...
class ShellCommandOutputCallbackInterface {
 public:
  ...
  virtual void ShellCommandOutput_CallbackEvent(string cmd, char *output,
                                                void *userdata) = 0;
};
...
void findDirs(const vector<string> &paths, vector<string> &results,
              bool errorOnNotFound = false, bool keepDuplicates = false);
void findAll(const vector<string> &paths, const string &fileFilter,
             vector<string> &results, bool cutExtension = false,
             bool errorOnNotFound = true, bool keepDuplicates = false);
void findAll(const string &path, vector<string> &results,
             bool cutExtension = false, bool errorOnNotFound = true);
vector<string> getFolderTreeContentsListRecursively(
    const string &path, const string &filterFileExt,
    bool includeFolders = false, vector<string> *recursiveMap = NULL);
...
std::pair<string, string> getFolderTreeContentsCheckSumCacheKey(
...
std::pair<string, string> getFolderTreeContentsCheckSumListCacheKey(
    vector<string> paths, const string &pathSearchString,
    const string &filterFileExt);
...
void updatePathClimbingParts(string &path,
                             bool processPreviousDirTokenCheck = true);
...
string getFullFileArchiveExtractCommand(
    const string &fileArchiveExtractCommand,
    string fileArchiveExtractCommandParameters, const string &outputpath,
    const string &archivename);
string getFullFileArchiveCompressCommand(
    const string &fileArchiveCompressCommand,
    string fileArchiveCompressCommandParameters, const string &archivename,
    const string &archivefiles);

bool executeShellCommand(string cmd,
                         int expectedResult = IGNORE_CMD_RESULT_VALUE,
                         ShellCommandOutputCallbackInterface *cb = NULL);
string executable_path(const string &exeName,
                       bool includeExeNameInPath = false);
...
class ValueCheckerVault {
 ...
 public:
  ValueCheckerVault() { vaultList.clear(); }
};

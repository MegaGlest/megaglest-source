...
class FTPClientCallbackInterface {
 public:
  ...
  virtual void FTPClient_CallbackEvent(
      ...
      pair<FTP_Client_ResultType, string> result, void *userdata) = 0;
};

class FTPClientThread : public BaseThread,
                        public ShellCommandOutputCallbackInterface {
 protected:
  int portNumber;
  string serverUrl;
  ...
  pair<FTP_Client_ResultType, string> getMapFromServer(
  ...
  pair<FTP_Client_ResultType, string> getTilesetFromServer(
  ...
  pair<FTP_Client_ResultType, string> getTechtreeFromServer(
      pair<string, string> techtreeName, string ftpUser,
      string ftpUserPassword);

  void getScenarioFromServer(pair<string, string> fileName);
  pair<FTP_Client_ResultType, string> getScenarioInternalFromServer(
      pair<string, string> fileName);

  void getFileFromServer(pair<string, string> fileName);
  pair<FTP_Client_ResultType, string> getFileInternalFromServer(
      pair<string, string> fileName);

  void getTempFileFromServer(pair<string, string> fileName);
  pair<FTP_Client_ResultType, string> getTempFileInternalFromServer(
      pair<string, string> fileName);
  ...
  pair<FTP_Client_ResultType, string> getFileFromServer(
 ...
 public:
  FTPClientThread(int portNumber, string serverUrl,
                  ...
                  std::pair<string, string> scenariosPath,
                  FTPClientCallbackInterface *pCBObject,
                  string fileArchiveExtension, string fileArchiveExtractCommand,
                  string fileArchiveExtractCommandParameters,
                  int fileArchiveExtractCommandSuccessResult,
                  string tempFilesPath);

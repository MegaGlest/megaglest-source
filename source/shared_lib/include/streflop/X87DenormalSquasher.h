...
inline void X87DenormalSquashFunction(ValueType &value) {
  struct X {};
...
// Pro: branching may be costly, though the branching predictor may compensate
// when there are few denormals Con: cmov forces an unconditional writeback to
// the mem just after read, which may be worse than the branch
...
template <>
inline void X87DenormalSquashFunction<long double>(long double &value) {
...
/// In particular, the GNU libm may be compiled this way, though no effort was
/// made to optimize out the denormal parts (and squashing denormals is non-IEEE
/// compliant anyway)
...
template <typename ValueType> struct X87DenormalSquasher {
  ...
  inline X87DenormalSquasher &
  operator=(const X87DenormalSquasher<long double> &f) {
    value = f.value;
    X87DenormalSquashFunction<ValueType>(value);
    return *this;
  } // self-ref OK
...
#define STREFLOP_X87DENORMAL_NATIVE_FLOAT_OPS(native_type)                     \
  ...
  inline X87DenormalSquasher &operator+=(const native_type f) {                \
    return operator+=(X87DenormalSquasher(f));                                 \
  }                                                                            \
  inline X87DenormalSquasher &operator-=(const native_type f) {                \
    return operator-=(X87DenormalSquasher(f));                                 \
  }                                                                            \
  ...
  inline bool operator==(const native_type f) const {                          \
    return operator==(X87DenormalSquasher(f));                                 \
  }                                                                            \
  inline bool operator!=(const native_type f) const {                          \
    return operator!=(X87DenormalSquasher(f));                                 \
...
/// binary operators
template <typename ValueType>
inline X87DenormalSquasher<ValueType>
operator+(const X87DenormalSquasher<ValueType> &f1,
...
#define STREFLOP_X87DENORMAL_FLOAT_BINARY(native_type)                         \
  template <typename ValueType>                                                \
  inline X87DenormalSquasher<ValueType> operator+(                             \
      const X87DenormalSquasher<ValueType> &f1, const native_type f2) {        \
  ...
  template <typename ValueType>                                                \
  inline X87DenormalSquasher<ValueType> operator+(                             \
      const native_type f1, const X87DenormalSquasher<ValueType> &f2) {        \
  ...
  template <typename ValueType>                                                \
  inline bool operator==(const native_type value,                              \
                         const X87DenormalSquasher<ValueType> &f) {            \
...
template <typename ValueType>
inline X87DenormalSquasher<ValueType>
operator-(const X87DenormalSquasher<ValueType> &f) {
...
/// this function may not be reliable (they may depend on system rounding mode,
etc.)
/// Still useful for debug output and to report results with a few significant
digits template<typename ValueType> inline std::ostream&
operator<<(std::ostream& out, const X87DenormalSquasher<ValueType>& f) { out <<
f.value; return out;
}
template<typename ValueType> inline std::istream& operator>>(std::istream& in,
X87DenormalSquasher<ValueType>& f) { in >> f.value; return in;
}
*/

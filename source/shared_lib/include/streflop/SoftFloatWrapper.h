...
template <int Nbits> struct SoftFloatWrapper {
  ...
  /// Use dummy bool argument for construction from an already initialized
  /// holder
  ...
  /// Conversion between different types. Also unfortunately includes otherwise
  /// perfectly fine copy constructor
...
#define STREFLOP_SOFT_WRAPPER_MAKE_REAL_CLASS_OPS(N)                           \
  template <>                                                                  \
  bool SoftFloatWrapper<N>::operator<(const SoftFloatWrapper &f) const;        \
  template <>                                                                  \
  SoftFloatWrapper<N> &SoftFloatWrapper<N>::operator+=(                        \
      const SoftFloatWrapper<N> &f);                                           \
  template <>                                                                  \
  SoftFloatWrapper<N> &SoftFloatWrapper<N>::operator-=(                        \
      const SoftFloatWrapper<N> &f);                                           \
  template <>                                                                  \
  ...
  SoftFloatWrapper<N> &SoftFloatWrapper<N>::operator/=(                        \
      const SoftFloatWrapper<N> &f);                                           \
  template <>                                                                  \
  bool SoftFloatWrapper<N>::operator==(const SoftFloatWrapper<N> &f) const;    \
  template <>                                                                  \
  bool SoftFloatWrapper<N>::operator!=(const SoftFloatWrapper<N> &f) const;    \
  template <>                                                                  \
  bool SoftFloatWrapper<N>::operator<(const SoftFloatWrapper<N> &f) const;     \
...
#define STREFLOP_SOFT_WRAPPER_MAKE_REAL_NM_CONVERSION(Nbits)                   \
  template <>                                                                  \
  SoftFloatWrapper<Nbits>::SoftFloatWrapper(const SoftFloatWrapper<32> &f);    \
  template <>                                                                  \
  SoftFloatWrapper<Nbits> &SoftFloatWrapper<Nbits>::operator=(                 \
      const SoftFloatWrapper<32> &f);                                          \
  template <>                                                                  \
  SoftFloatWrapper<Nbits>::SoftFloatWrapper(const SoftFloatWrapper<64> &f);    \
  template <>                                                                  \
  SoftFloatWrapper<Nbits> &SoftFloatWrapper<Nbits>::operator=(                 \
      const SoftFloatWrapper<64> &f);                                          \
  template <>                                                                  \
  SoftFloatWrapper<Nbits>::SoftFloatWrapper(const SoftFloatWrapper<96> &f);    \
  template <>                                                                  \
  SoftFloatWrapper<Nbits> &SoftFloatWrapper<Nbits>::operator=(                 \
...
#define STREFLOP_SOFT_WRAPPER_MAKE_REAL_NATIVE_OPS(N, native_type)             \
  ...
  template <>                                                                  \
  SoftFloatWrapper<N> &SoftFloatWrapper<N>::operator=(const native_type f);    \
...
/// binary operators
template <int N>
SoftFloatWrapper<N> operator+(const SoftFloatWrapper<N> &f1,
                              const SoftFloatWrapper<N> &f2);
template <int N>
SoftFloatWrapper<N> operator-(const SoftFloatWrapper<N> &f1,
                              const SoftFloatWrapper<N> &f2);
template <int N>
...
#define STREFLOP_SOFT_WRAPPER_BINARY_OPS(native_type)                          \
  ...
  SoftFloatWrapper<N> operator/(const SoftFloatWrapper<N> &f1,                 \
                                const native_type f2);                         \
  template <int N>                                                             \
  SoftFloatWrapper<N> operator+(const native_type f1,                          \
                                const SoftFloatWrapper<N> &f2);                \
  template <int N>                                                             \
  SoftFloatWrapper<N> operator-(const native_type f1,                          \
                                const SoftFloatWrapper<N> &f2);                \
  ...
  SoftFloatWrapper<N> operator/(const native_type f1,                          \
                                const SoftFloatWrapper<N> &f2);                \
  template <int N>                                                             \
  bool operator==(const native_type value, const SoftFloatWrapper<N> &f);      \
  template <int N>                                                             \
  bool operator!=(const native_type value, const SoftFloatWrapper<N> &f);      \
  template <int N>                                                             \
  bool operator<(const native_type value, const SoftFloatWrapper<N> &f);       \

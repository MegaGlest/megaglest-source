...
// Stolen from math.h. All floating-point numbers can be put in one of these
// categories.
enum {
  STREFLOP_FP_NAN = 0,
...
// Threated as an integer, the bit pattern here may be incremented to get at
// least 2^20 different NaN custom numbers! Note that when switching the
// left-most bit to 1, you can get another bunch of negative NaNs, whatever this
// mean.
...
inline bool isunordered(Simple x, Simple y) {
  return (fpclassify(x) == STREFLOP_FP_NAN) ||
         (fpclassify(y) == STREFLOP_FP_NAN);
}
...
// Missing from libm: temporarily switch to Double and execute the Double
// version, then switch back to Extended and return the result

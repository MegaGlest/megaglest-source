...
// => this adds a level of protection, a function inadvertantly using a wrong
// precision function is detected
...
// <float.h> is not included, define only these constants that are needed to
// compile libm
...
// No template, only allow this for int types (could use traits, but it's
// simpler this way)
...
// eval method is 0 for x87, sse, soft, that is float_t has size float and
// double_t has size double
...
template <> struct ExtendedConverter<10> {
  ...
  // Mantissa
  static inline SizedUnsignedInteger<32>::Type *mPtr(Extended *e) {
    return reinterpret_cast<SizedUnsignedInteger<32>::Type *>(

...
union ieee754_float {
  ...
  inline const Simple &f() const {
    return CONST_SIMPLE_FROM_INT_PTR(&storage[0]);
  }
...
union ieee754_double {
  ...
  inline const Double &d() const {
    return CONST_DOUBLE_FROM_INT_PTR(&storage[0]);
  }
...
union ieee854_long_double {
  ...
  inline const Extended &d() const {
    return CONST_EXTENDED_FROM_INT_PTR(&storage[0]);
  }

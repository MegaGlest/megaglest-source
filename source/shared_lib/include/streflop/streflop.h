...
#error STREFLOP: Invalid combination or unknown FPU type.
  ...
  // MSVC will not compile without the long double variants defined, so we
  // either have to declare Extended:
//    typedef long double Extended;
//    #define Extended Extended
...
#define frexpl(a, b) frexp((double)a, b) The wrappers are possibly the better
  // choice for sync reasons.
#endif

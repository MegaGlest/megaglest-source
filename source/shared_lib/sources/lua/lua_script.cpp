...
void LuaScript::DumpGlobals() {
  ...
  while (lua_next(L, LUA_GLOBALSINDEX) != 0) {
    ...
    if (value_type != LUA_TNUMBER && value_type != LUA_TBOOLEAN &&
      ...
      lua_pop(
          L, 1); // again, pop the value before going to the next loop iteration
      continue;
    }
    ...
    case LUA_TNUMBER:
      ...
      // number to string happens, the top of the stack will be altered and the
      // table index will become invalid)
      lua_pushvalue(L, -1);
    ...
    // SaveLine(key_string + " = " + value_string);		// Pop the value so
    // the index remains on top of the stack for the next iteration
    ...
    case LUA_TTABLE: {
      ...
      for (lua_pushnil(L); lua_next(L, -2);) {
        ...
        if (skipTable == false) {
          ...
          if (LuaScript::debugModeEnabled == true)
            printf("***WARNING*** SKIPPING LUA TABLE because it has an "

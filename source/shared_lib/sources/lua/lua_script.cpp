...
void LuaScript::DumpGlobals() {
    ...
    while (lua_next(L, LUA_GLOBALSINDEX) != 0) {
        ...
        if (value_type != LUA_TNUMBER && value_type != LUA_TBOOLEAN && value_type != LUA_TSTRING) {
            lua_pop(L,
...
void LuaScript::saveGame(XmlNode *rootNode) {
    ...
    while (lua_next(L, LUA_GLOBALSINDEX) != 0) {
        ...
        // support only number, boolean and string values
        if (value_type != LUA_TNUMBER && value_type != LUA_TBOOLEAN && value_type != LUA_TSTRING && value_type != LUA_TTABLE) {
            lua_pop(L,
                    1); // again, pop the value before going to the next loop iteration
            continue;
        }
        ...
        case LUA_TTABLE: {
            ...
            if (LuaScript::debugModeEnabled == true)
                printf("================ LUA TABLE DETECTED - START "
                       "=================\n");

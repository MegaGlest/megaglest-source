...
void LuaScript::DumpGlobals() {
  ...
  while (lua_next(L, LUA_GLOBALSINDEX) != 0) {
    ...
    if (key_type != LUA_TSTRING) {
      lua_pop(L, 1);  // pop the value so that the top contains the key for the
                      // next iteration
      continue;
    }

    // support only number, boolean and string values
    if (value_type != LUA_TNUMBER && value_type != LUA_TBOOLEAN &&
        value_type != LUA_TSTRING) {
      lua_pop(
          L,
          1);  // again, pop the value before going to the next loop iteration
      continue;
    }

    // get the key as a string
    string key_string = lua_tostring(
        L, -2);  // no copy required - we already know this is a string

    // do not support variables that start with '_'
    // lua has some predefined values like _VERSION. They all start with
    // underscore
    ...
    switch (value_type) {
      ...
      case LUA_TNUMBER:
        ...
        // get the value as a string (this requires a copy because traversing
        // tables uses the top of the stack as an index. If conversion from a
        // number to string happens, the top of the stack will be altered and
        // the table index will become invalid)
        lua_pushvalue(L, -1);
    ...
    // SaveLine(key_string + " = " + value_string);		// Pop the value
    // so the index remains on top of the stack for the next iteration
...
void LuaScript::saveGame(XmlNode *rootNode) {
  ...
  while (lua_next(L, LUA_GLOBALSINDEX) != 0) {
    ...
    if (LuaScript::debugModeEnabled == true)
      printf("LUA save key_type = %d, value_type = %d for var [%s]\n", key_type,
             value_type, lua_tostring(L, -2));
    ...
    // support only number, boolean and string values
    if (value_type != LUA_TNUMBER && value_type != LUA_TBOOLEAN &&
        value_type != LUA_TSTRING && value_type != LUA_TTABLE) {
      lua_pop(
          L,
          1);  // again, pop the value before going to the next loop iteration
      continue;
    ...
    switch (value_type) {
      ...
      case LUA_TTABLE: {
        ...
        for (lua_pushnil(L); lua_next(L, -2);) {
          ...
          if (LuaScript::debugModeEnabled == true)
            printf("LUA TABLE loop item type [%s] key: %d value type: %d\n",
                   lua_typename(L, tableValueType), tableKeyType,
                   tableValueType);
          ...
          if (skipTable == false) {
            ...
            if (!lua_isnil(L, -1)) {
              if (LuaScript::debugModeEnabled == true)
                printf("LUA TABLE loop B\n");

              lua_pushvalue(L, -1);

              if (LuaScript::debugModeEnabled == true)
                printf("LUA TABLE loop C\n");
            ...
            // value_string += szBuf;
            // vector<pair<pair<int,string>, pair<int,string>> > tableList;
            tableList.push_back(make_pair(make_pair(tableKeyType, key),
                                          make_pair(tableValueType, value)));
          } else {
            if (LuaScript::debugModeEnabled == true)
              printf(
                  "***WARNING*** SKIPPING LUA TABLE because it has an "
                  "unsupported embedded type: %d [%s]\n",
        ...
        if (tableItemCount < 1) {
          if (LuaScript::debugModeEnabled == true)
            printf("Skipping lua table tableItemCount = %d\n", tableItemCount);

          skipTable = true;
        }
        if (LuaScript::debugModeEnabled == true)
          printf(
              "---------------------------- LUA TABLE DETECTED - END "
              "----------------------------\n");
      } break;
    }
    ...
    if (skipTable == true) {
      ...
      if (tableList.empty() == false) {
        ...
        luaScriptNode->addAttribute("value_type", intToStr(value_type),
                                    mapTagReplacements);
        ...
        for (unsigned int i = 0; i < tableList.size(); ++i) {
          ...
          luaScriptTableNode->addAttribute(
              "key_type", intToStr(item.first.first), mapTagReplacements);
          luaScriptTableNode->addAttribute("key", item.first.second,
                                           mapTagReplacements);
          luaScriptTableNode->addAttribute("value", item.second.second,
                                           mapTagReplacements);
          luaScriptTableNode->addAttribute(
              "value_type", intToStr(item.second.first), mapTagReplacements);
        }
      } else {
        // resulting line. Somehow save this and when you need to restore it,
        // just call luaL_dostring with that line.
        // SaveLine(key_string + " = " + value_string);		// Pop the value
        // so the index remains on top of the stack for the next iteration
        // printf("Found global LUA var: %s =
...
void LuaScript::loadGame(const XmlNode *rootNode) {
  if (LuaScript::debugModeEnabled)
    printf("START [%s::%s] Line: %d\n",
  ...
  if (LuaScript::debugModeEnabled)
    printf("luaScriptNodeList.size(): %d\n", (int)luaScriptNodeList.size());
  ...
  for (unsigned int i = 0; i < luaScriptNodeList.size(); ++i) {
    ...
    switch (value_type) {
      ...
      case LUA_TTABLE: {
        ...
        if (LuaScript::debugModeEnabled)
          printf("luaScriptTableNode.size(): %d\n",
        ...
        for (unsigned int j = 0; j < luaScriptTableNode.size(); ++j) {
          ...
          if (LuaScript::debugModeEnabled == true)
            printf("Table item key_type: %d [%s]\n", key_type,
                   lua_typename(luaState, key_type));

          switch (key_type) {
            case LUA_TSTRING:
              if (LuaScript::debugModeEnabled)
                printf("  table item key [%s]\n",
                       nodeTable->getAttribute("key")->getValue().c_str());
              lua_pushstring(
                  luaState, nodeTable->getAttribute("key")->getValue().c_str());
              break;
            case LUA_TNUMBER:
              if (LuaScript::debugModeEnabled)
                printf("  table item key [%s]\n",
              ...
              lua_pushnumber(luaState,
                             nodeTable->getAttribute("key")->getFloatValue());
              break;
            case LUA_TBOOLEAN:
              if (LuaScript::debugModeEnabled)
                printf("  table item key [%s]\n",
              ...
              lua_pushboolean(luaState,
                              nodeTable->getAttribute("key")->getBoolValue());
              break;
          }

          int value_type = nodeTable->getAttribute("value_type")->getIntValue();
          if (LuaScript::debugModeEnabled == true)
            printf("Table item value_type: %d [%s]\n", value_type,
                   lua_typename(luaState, value_type));

          switch (value_type) {
            case LUA_TSTRING:
              if (LuaScript::debugModeEnabled)
                printf("  table item value [%s]\n",
            ...
            case LUA_TNUMBER:
              if (LuaScript::debugModeEnabled)
                printf("  table item value [%s]\n",
                       nodeTable->getAttribute("value")->getValue().c_str());
              lua_pushnumber(luaState,
                             nodeTable->getAttribute("value")->getFloatValue());
              break;
            case LUA_TBOOLEAN:
              if (LuaScript::debugModeEnabled)
                printf("  table item value [%s]\n",
                       nodeTable->getAttribute("value")->getValue().c_str());
              lua_pushboolean(luaState,
                              nodeTable->getAttribute("value")->getBoolValue());
              break;
          }
...
void LuaScript::loadCode(string code, string name) {
  ...
  int errorCode =
      luaL_loadbuffer(luaState, code.c_str(), code.length(), name.c_str());
  if (errorCode != 0) {
    printf("=========================================================\n");
  ...
  if (errorCode != 0) {
    ...
    throw megaglest_runtime_error(
        "Error initializing lua: " + errorToString(errorCode), true);
  }
...
void LuaScript::setSandboxCode(string code) { sandboxCode = code; }
...
void LuaScript::beginCall(string functionName) {
  ...
  //		if(errorCode !=0 ) {
  //			throw megaglest_runtime_error("Error calling lua
  // function [" + currentLuaFunction + "] error: " + errorToString(errorCode));
  //		}
  ...
  // printf("currentLuaFunctionIsValid = %d functionName
...
void LuaScript::endCall() {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA,
                             "In [%s::%s Line: %d] currentLuaFunction [%s], "
                             "currentLuaFunctionIsValid = %d\n",
                             __FILE__, __FUNCTION__, __LINE__,
                             currentLuaFunction.c_str(),
                             currentLuaFunctionIsValid);

  if (currentLuaFunctionIsValid == true) {
    if (sandboxWrapperFunctionName != "" && sandboxCode != "") {
      // lua_pushstring(luaState, currentLuaFunction.c_str());   // push 1st
      // argument, the real lua function argumentCount = 1;

      string safeWrapper =
          sandboxWrapperFunctionName + " [[" + currentLuaFunction + "()]]";
      printf("Trying to execute [%s]\n", safeWrapper.c_str());
      int errorCode = runCode(safeWrapper);
      if (errorCode != 0) {
        throw megaglest_runtime_error(
            "Error calling lua function [" + currentLuaFunction +
                "] error: " + errorToString(errorCode),
            true);
      }
      ...
      //			int errorCode= lua_pcall(luaState,
      // argumentCount, 0, 0); 			if(errorCode !=0 ) {
      // throw megaglest_runtime_error("Error calling lua function [" +
      // currentLuaFunction + "] error: " + errorToString(errorCode));
      //			}
    } else {
...
Vec2f LuaArguments::getVec2f(int argumentIndex) const {
  ...
  if (!lua_istable(luaState, argumentIndex)) {
    throwLuaError(
        "Can not get vec2f from Lua state, value on the stack is not a table");
  }
...
Vec3f LuaArguments::getVec3f(int argumentIndex) const {
  ...
  if (!lua_istable(luaState, argumentIndex)) {
    throwLuaError(
        "Can not get vec3f from Lua state, value on the stack is not a table");
  }
...
Vec4f LuaArguments::getVec4f(int argumentIndex) const {
  ...
  if (!lua_istable(luaState, argumentIndex)) {
    throwLuaError(
        "Can not get vec4f from Lua state, value on the stack is not a table");
  }
...
Vec2i LuaArguments::getVec2i(int argumentIndex) const {
  ...
  if (!lua_istable(luaState, argumentIndex)) {
    throwLuaError(
        "Can not get vec2i from Lua state, value on the stack is not a table");
  }
...
Vec4i LuaArguments::getVec4i(int argumentIndex) const {
  ...
  if (!lua_istable(luaState, argumentIndex)) {
    throwLuaError(
        "Can not get vec4i from Lua state, value on the stack is not a table");
  }
...
string LuaArguments::getStackText() const {
  ...
  for (int i = 1; i <= stackSize; ++i) {
    ...
    if (lua_isnumber(luaState, -i)) {
      ...
      //				stackString+= "entry# " + intToStr(j) +
      //", value = " + lua_tostring(luaState, -i) + "\n";
      //			}

      //			int j = 1;
      //			lua_pushnil(luaState);  // The initial key for
      // the traversal.
      //
      //			printf("start loop\n");
      //
      ...
      //				printf("in loop j = %d\n",j);
      //
      //				const char* Param=lua_tostring(luaState,
      //-1);
      //
      //				printf("passed in loop j = %d Param
      //[%s]\n",j,(Param != NULL ? Param : "<nil>"));
      //
      //				if (Param!=NULL) {
      //					stackString+= "entry# " +
      // intToStr(j) + ", value = " + Param + "\n";
      //				}
      //
      //				// Remove the value, keep the key for
      // the next iteration. 				lua_pop(luaState, 1);
      // j++;
      //			}
      ...
      // luaState, -1 ); 				printf("D Table value =
      // %s\n",(value != NULL ? value :
      //"<nil>"));
      //
      //				//v.push_back( lua_tointeger( L, -1 ) );
      //				const char *value2 = lua_tostring(
      // luaState, -2 ); 				printf("E Table value2 =
      // %s\n",(value2 != NULL ? value2
      //: "<nil>"));
      //

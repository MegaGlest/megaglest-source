        ...
        for (int j = y - radius; j <= y + radius; j++) {
            ...
            if (inside(i, j) && dist < radius) {
                ...
                if (fabs(normIf) > fabs(normJf)) {
                    ...
                    usedGrad = gradient[normI[0]][normJ[0]] * fabs(normIf) + gradient[normI[1]][normJ[0]] * (1 - fabs(normIf));
                } else {
                    usedGrad = gradient[normI[0]][normJ[0]];
                }
#endif
                ...
                if (((newAlt - cells[i][j].height) > 0 && height > 0) || ((newAlt - cells[i][j].height) < 0 && height < 0) || height == 0) {
...
void MapPreview::changeObject(int x, int y, int object, int radius, bool overwrite) {
    ...
    for (i = x - radius + 1; i < x + radius; i++) {
        for (j = y - radius + 1; j < y + radius; j++) {
            if (inside(i, j)) {
                dist = get_dist(i - x, j - y);
                if (radius > dist && (overwrite || (cells[i][j].resource == 0 && cells[i][j].object == 0 && !isCliff(i, j) &&
...
void MapPreview::smoothSurface(bool limitHeight) {
    ...
    float *oldHeights = new float[w * h]; // dumb microsoft compiler cannot handle dynamic
    ...
    try {
        ...
        if (readBytes != 1) {
            ...
            if (errorOnInvalidMap == true) {
                ...
                snprintf(szBuf, 8096,
                         "In [%s::%s Line: %d]\nfile [%s]\nreadBytes != "
                         "sizeof(MapFileHeader) [" MG_SIZE_T_SPECIFIER "] [" MG_SIZE_T_SPECIFIER "]\n",
...
// static
string MapPreview::getMapPath(const vector<string> &pathList, const string &mapName, string scenarioDir, bool errorOnNotFound) {
    for (unsigned int idx = 0; idx < pathList.size(); idx++) {
        ...
        if ((EndsWith(original, ".mgm") == true || EndsWith(original, ".gbm") == true) && fileExists(original)) {
            return original;
...
vector<string> MapPreview::findAllValidMaps(const vector<string> &pathList, string scenarioDir, bool getUserDataOnly, bool cutExtension,

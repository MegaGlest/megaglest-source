...
MapPreview::MapPreview() {
  ...
  reset(DEFAULT_MAP_CELL_WIDTH, DEFAULT_MAP_CELL_LENGTH,
        (float)DEFAULT_MAP_CELL_HEIGHT, DEFAULT_MAP_CELL_SURFACE_TYPE);
...
float MapPreview::getHeight(int x, int y) const { return cells[x][y].height; }
    ...
    for (int l = -1; l <= 1; ++l) {
      ...
      if (xToCheck < 0 || yToCheck < 0 || xToCheck >= w || yToCheck >= h) {
        // ignore
                 ...
                 abs((int)(getHeight(x, y) - getHeight(xToCheck, yToCheck)))) {
        return true;
      }
    }
  }
...
int MapPreview::getObject(int x, int y) const { return cells[x][y].object; }

int MapPreview::getResource(int x, int y) const { return cells[x][y].resource; }
...
int MapPreview::get_dist(int delta_x, int delta_y) {
  ...
  return static_cast<int>(
      streflop::sqrtf(static_cast<streflop::Simple>(dx * dx + dy * dy)) +
      0.5); // round correctly
#else
    ...
    for (int j = y - radius + 1; j < y + radius; j++) {
      if (inside(i, j)) {
        ...
        if (radius > dist) {
          ...
          if ((height > 0 && newAlt > oldAlt) ||
              (height < 0 && newAlt < oldAlt) || height == 0) {
...
void MapPreview::pirateChangeHeight(int x, int y, int height, int radius) {
  ...
  // from the heights of the sides and corners of the brush to the center goal
  // height
  ...
  for (int i = x - radius; i <= x + radius; i += radius) {
    ...
    for (int j = y - radius; j <= y + radius; j += radius) {
      ...
      if (inside(ti, tj)) {
        gradient[indexI][indexJ] =
            (cells[ti][tj].height - (float)goalAlt) / (float)radius;
        //} else if (dist == 0) {
      ...
      // std::cout << "gradient[" << indexI << "][" << indexJ << "] = " <<
      // gradient[indexI][indexJ] << std::endl; std::cout << "derived from
      // height " << cells[ti][tj].height << " at " << ti << " " << tj <<
      // std::endl;
      indexJ++;
    }
    indexI++;
  ...
  //  // No becasue then "radius" 1==2
  // radius -= 1;
  for (int i = x - radius; i <= x + radius; i++) {
    for (int j = y - radius; j <= y + radius; j++) {
      int dist = get_dist(i - x, j - y);
      if (inside(i, j) && dist < radius) {
        // Normalize di and dj and round them to an int so they can be used as
        // indicies
...
#ifdef USE_STREFLOP
        if (streflop::fabs(static_cast<streflop::Simple>(normIf)) >
            streflop::fabs(static_cast<streflop::Simple>(normJf))) {

          usedGrad =
              gradient[normI[0]][normJ[0]] *
                  streflop::fabs(static_cast<streflop::Simple>(normJf)) +
              gradient[normI[0]][normJ[1]] *
                  (1 - streflop::fabs(static_cast<streflop::Simple>(normJf)));
        } else if (streflop::fabs(static_cast<streflop::Simple>(normIf)) <
                   streflop::fabs(static_cast<streflop::Simple>(normJf))) {
          usedGrad =
              gradient[normI[0]][normJ[0]] *
                  streflop::fabs(static_cast<streflop::Simple>(normIf)) +
              gradient[normI[1]][normJ[0]] *
                  (1 - streflop::fabs(static_cast<streflop::Simple>(normIf)));
        } else {
        ...
        if (fabs(normIf) > fabs(normJf)) {
          ...
          usedGrad = gradient[normI[0]][normJ[0]] * fabs(normJf) +
                     gradient[normI[0]][normJ[1]] * (1 - fabs(normJf));
        } else if (fabs(normIf) < fabs(normJf)) {
          usedGrad = gradient[normI[0]][normJ[0]] * fabs(normIf) +
        ...
        // if the change in height and what is supposed to be the change in
        // height are the same sign then we can change the height
        if (((newAlt - cells[i][j].height) > 0 && height > 0) ||
            ((newAlt - cells[i][j].height) < 0 && height < 0) || height == 0) {
...
void MapPreview::changeSurface(int x, int y, MapSurfaceType surface,
                               int radius) {
  int i = 0, j = 0;
  int dist = 0;
  ...
  for (i = x - radius + 1; i < x + radius; i++) {
    for (j = y - radius + 1; j < y + radius; j++) {
      if (inside(i, j)) {
        ...
        if (radius > dist &&
            (overwrite ||
             (cells[i][j].resource == 0 && cells[i][j].object == 0 &&
              !isCliff(i, j) &&
              cells[i][j].height >= waterLevel - 1.5f))) { // was >=
...
void MapPreview::reset(int w, int h, float alt, MapSurfaceType surf) {
  if (w < MIN_MAP_CELL_DIMENSION || h < MIN_MAP_CELL_DIMENSION) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "Size of map must be at least %dx%d",
             MIN_MAP_CELL_DIMENSION, MIN_MAP_CELL_DIMENSION);
  ...
  if (w > MAX_MAP_CELL_DIMENSION || h > MAX_MAP_CELL_DIMENSION) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "Size of map can be at most %dx%d",
             MAX_MAP_CELL_DIMENSION, MAX_MAP_CELL_DIMENSION);
    throw megaglest_runtime_error(szBuf);
  }

  if (alt < MIN_MAP_CELL_HEIGHT || alt > MAX_MAP_CELL_HEIGHT) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "Height must be in the range %d-%d",
             MIN_MAP_CELL_HEIGHT, MAX_MAP_CELL_HEIGHT);
    throw megaglest_runtime_error(szBuf);
  }

  if (surf < st_Grass || surf > st_Ground) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "Surface must be in the range %d-%d", st_Grass,
             st_Ground);
    throw megaglest_runtime_error(szBuf);
  }
...
void MapPreview::resetFactions(int maxPlayers) {
  if (maxPlayers < MIN_MAP_FACTIONCOUNT || maxPlayers > MAX_MAP_FACTIONCOUNT) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "Max Players must be in the range %d-%d",
             MIN_MAP_FACTIONCOUNT, MAX_MAP_FACTIONCOUNT);
    throw megaglest_runtime_error(szBuf);
  }
...
void MapPreview::randomizeHeights(bool withReset, int minimumHeight,
                                  int maximumHeight, int chanceDivider,
                                  int smoothRecursions) {
  if (withReset)
...
void MapPreview::importMapHeights(unsigned char *data) {
  for (int i = 0; i < w; ++i) {
    for (int j = 0; j < h; ++j) {
      cells[i][j].height = ((float)data[(i + j * w) * 3] *
                            (MAX_MAP_CELL_HEIGHT - MIN_MAP_CELL_HEIGHT) / 255) +
                           MIN_MAP_CELL_HEIGHT;
    }
  }
}
...
void MapPreview::randomizeFactions() {
  ...
  for (int i = 0; i < maxFactions; ++i) {
    ...
    sl.x = static_cast<int>(w * slNoiseFactor * ((i + slPlaceFactorX) % 2) +
                            w * (1.f - slNoiseFactor) / 2.f);
    sl.y =
        static_cast<int>(h * slNoiseFactor * (((i + slPlaceFactorY) / 2) % 2) +
                         h * (1.f - slNoiseFactor) / 2.f);
    startLocations[i] = sl;
  }
  hasChanged = true;
}
...
void MapPreview::smoothSurface(bool limitHeight) {
  ...
  float *oldHeights = new float[w * h]; // dumb microsoft compiler cannot handle
                                        // dynamic arrays, doing it manually
  for (int j = 0; j < h; ++j) {
    for (int i = 0; i < w; ++i) {
...
void MapPreview::switchSurfaces(MapSurfaceType surf1, MapSurfaceType surf2) {
  if (surf1 >= st_Grass && surf1 <= st_Ground && surf2 >= st_Grass &&
      surf2 <= st_Ground) {
    for (int i = 0; i < w; ++i) {
      for (int j = 0; j < h; ++j) {
...
void toEndianMapFileHeader(MapFileHeader &header) {
  ...
  header.maxFactions =
      Shared::PlatformByteOrder::toCommonEndian(header.maxFactions);
  ...
  header.heightFactor =
      Shared::PlatformByteOrder::toCommonEndian(header.heightFactor);
  header.waterLevel =
      Shared::PlatformByteOrder::toCommonEndian(header.waterLevel);
  for (unsigned int i = 0; i < (unsigned int)MAX_TITLE_LENGTH; ++i) {
    header.title[i] =
        Shared::PlatformByteOrder::toCommonEndian(header.title[i]);
  }
  for (unsigned int i = 0; i < (unsigned int)MAX_DESCRIPTION_LENGTH; ++i) {
    header.description[i] =
        Shared::PlatformByteOrder::toCommonEndian(header.description[i]);
  }
}
void fromEndianMapFileHeader(MapFileHeader &header) {
  header.version = Shared::PlatformByteOrder::fromCommonEndian(header.version);
  header.maxFactions =
      Shared::PlatformByteOrder::fromCommonEndian(header.maxFactions);
  ...
  header.heightFactor =
      Shared::PlatformByteOrder::fromCommonEndian(header.heightFactor);
  header.waterLevel =
      Shared::PlatformByteOrder::fromCommonEndian(header.waterLevel);
  for (unsigned int i = 0; i < (unsigned int)MAX_TITLE_LENGTH; ++i) {
    header.title[i] =
        Shared::PlatformByteOrder::fromCommonEndian(header.title[i]);
  }
  for (unsigned int i = 0; i < (unsigned int)MAX_DESCRIPTION_LENGTH; ++i) {
    header.description[i] =
        Shared::PlatformByteOrder::fromCommonEndian(header.description[i]);
  }
}

void MapPreview::loadFromFile(const string &path) {

  // "Could not open file, result: 3 - 2 No such file or directory [C:\Documents
  // and Settings\人間五\Application
  // Data\megaglest\maps\clearings_in_the_woods.gbm]
  ...
  if (f1 != NULL) {
    ...
    for (int i = 0; i < maxFactions; ++i) {
      ...
      startLocations[i].x =
          Shared::PlatformByteOrder::fromCommonEndian(startLocations[i].x);
      ...
      startLocations[i].y =
          Shared::PlatformByteOrder::fromCommonEndian(startLocations[i].y);
    }

    // read Heights
    reset(header.width, header.height, (float)DEFAULT_MAP_CELL_HEIGHT,
          DEFAULT_MAP_CELL_SURFACE_TYPE);
    for (int j = 0; j < h; ++j) {
      for (int i = 0; i < w; ++i) {
        ...
        cells[i][j].height =
            Shared::PlatformByteOrder::fromCommonEndian(cells[i][j].height);
      }
    }
    ...
    string strError = "[#5] Could not open file, result: " + intToStr(error) +
                      " - " + intToStr(fileErrno) + " " + strerror(fileErrno) +
                      " [" + path + "]";
    throw strError;
#else
...
void MapPreview::saveToFile(const string &path) {
  ...
  if (f1 != NULL) {
    ...
    strncpy(header.version2.short_desc, desc.c_str(),
            MAX_DESCRIPTION_LENGTH_VERSION2);
...
void MapPreview::realRandomize(int minimumHeight, int maximumHeight,
                               int _chanceDivider, int _smoothRecursions) {
  int moduloParam = abs(maximumHeight - minimumHeight);
  int chanceDivider = _chanceDivider;
  int smoothRecursions = _smoothRecursions;
  if (moduloParam < 2)
    moduloParam = 2;
  // printf("moduloParam=%d   minimumHeight=%d
  ...
  if (smoothRecursions < 0)
    smoothRecursions = 0;
  if (smoothRecursions > 1000)
    smoothRecursions = 1000;
...
void MapPreview::applyNewHeight(float newHeight, int x, int y, int strenght) {
  cells[x][y].height = static_cast<float>(
      ((cells[x][y].height * strenght) + newHeight) / (strenght + 1));
  hasChanged = true;
}

bool MapPreview::loadMapInfo(string file, MapInfo *mapInfo,
                             string i18nMaxMapPlayersTitle,
                             string i18nMapSizeTitle, bool errorOnInvalidMap) {
  bool validMap = false;
  FILE *f = NULL;
  try {
#ifdef WIN32
    ...
    if (readBytes != 1) {
      ...
      if (errorOnInvalidMap == true) {
        ...
        snprintf(szBuf, 8096,
                 "In [%s::%s Line: %d]\nfile [%s]\nreadBytes != "
                 "sizeof(MapFileHeader) [" MG_SIZE_T_SPECIFIER
                 "] [" MG_SIZE_T_SPECIFIER "]\n",
                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                 __LINE__, file.c_str(), readBytes, sizeof(MapFileHeader));
      ...
      if (header.version < mapver_1 || header.version >= mapver_MAX) {
        ...
        if (errorOnInvalidMap == true) {
          char szBuf[4096] = "";
          printf("In [%s::%s Line: %d]\file [%s]\nheader.version < mapver_1 || "
                 "header.version >= mapver_MAX [%d] [%d]\n",
                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                 __LINE__, file.c_str(), header.version, mapver_MAX);
          SystemFlags::OutputDebug(SystemFlags::debugError, "%s", szBuf);

          throw megaglest_runtime_error(szBuf);
        }
      } else if (header.maxFactions <= 0 ||
                 header.maxFactions > MAX_MAP_FACTIONCOUNT) {
        validMap = false;

        if (errorOnInvalidMap == true) {
          char szBuf[4096] = "";
          printf("In [%s::%s Line: %d]\file [%s]\nheader.maxFactions <= 0 || "
                 "header.maxFactions > MAX_MAP_FACTIONCOUNT [%d] [%d]\n",
                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                 __LINE__, file.c_str(), header.maxFactions,
                 MAX_MAP_FACTIONCOUNT);
          SystemFlags::OutputDebug(SystemFlags::debugError, "%s", szBuf);
        ...
        mapInfo->desc =
            i18nMaxMapPlayersTitle + ": " + intToStr(mapInfo->players) + "\n";
        mapInfo->desc += i18nMapSizeTitle + ": " + intToStr(mapInfo->size.x) +
                         " x " + intToStr(mapInfo->size.y) + "\n";
    ...
    throw megaglest_runtime_error(
        "Error loading map file: [" + file + "] msg: " + e.what() + " errno [" +
        intToStr(errno) + "] [" + strerror(errno) + "]");
  }
...
// static
string MapPreview::getMapPath(const vector<string> &pathList,
                              const string &mapName, string scenarioDir,
  ...
  if (errorOnNotFound == true) {
    throw megaglest_runtime_error(
        "Map [" + mapName + "] not found, scenarioDir [" + scenarioDir + "]");
  }
...
vector<string> MapPreview::findAllValidMaps(const vector<string> &pathList,
  ...
  if (getUserDataOnly == true) {
    if (pathList.size() > 1) {
      ...
      copy(results2.begin(), results2.end(),
           std::inserter(allMaps2, allMaps2.begin()));
  ...
  MapInfo mapInfo;
  for (unsigned int i = 0; i < mapFiles.size();
       i++) { // fetch info and put map in right list
    // loadMapInfo(string file, MapInfo *mapInfo, string
    // i18nMaxMapPlayersTitle,string i18nMapSizeTitle,bool
    // errorOnInvalidMap=true); printf("getMapPath [%s]\nmapFiles.at(i)
    // [%s]\nscenarioDir [%s] getUserDataOnly = %d cutExtension =
    // %d\n",getMapPath(pathList,mapFiles.at(i), scenarioDir,
    // false).c_str(),mapFiles.at(i).c_str(),scenarioDir.c_str(),
    // getUserDataOnly, cutExtension);

    if (loadMapInfo(getMapPath(pathList, mapFiles.at(i), scenarioDir, false),
                    &mapInfo, "", "", false) == true) {
      results.push_back(mapFiles.at(i));
    } else {
      if (invalidMapList != NULL) {
        invalidMapList->push_back(
            getMapPath(pathList, mapFiles.at(i), scenarioDir, false));
      }
    }
  }

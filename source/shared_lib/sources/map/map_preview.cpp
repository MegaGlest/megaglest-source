...
void MapPreview::smoothSurface(bool limitHeight) {
  ...
  float *oldHeights =
      new float[w * h];  // dumb microsoft compiler cannot handle dynamic
                         // arrays, doing it manually
  for (int j = 0; j < h; ++j) {
    for (int i = 0; i < w; ++i) {
      ...
      if (header.version < mapver_1 || header.version >= mapver_MAX) {
        ...
        if (errorOnInvalidMap == true) {
          ...
          printf(
              ...
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, file.c_str(), header.maxFactions, MAX_MAP_FACTIONCOUNT);
          SystemFlags::OutputDebug(SystemFlags::debugError, "%s", szBuf);

...
void SoundSource::unQueueBuffers() {
    ...
    //		ALuint buffer=0;
    //		alSourceUnqueueBuffers(source, 1, &buffer);
    //		SoundPlayerOpenAL::checkAlError("Problem unqueuing buffers
    //(alSourceUnqueueBuffers) in audio source: ");
    //	}
    //
    //	SoundPlayerOpenAL::checkAlError("Problem unqueuing buffers (alGetSourcei
    // AL_BUFFERS_QUEUED) in audio source: ");
}
...
void SoundSource::stop() {
    ...
    while (playing() == true) {
        // alSourceStop(source);
        // SoundPlayerOpenAL::checkAlError(string(__FILE__) + string(" ") +
        // string(__FUNCTION__) + string(" ") + intToStr(__LINE__));
        ALint state = AL_STOPPED;
        alGetSourcei(source, AL_SOURCE_STATE, &state);
        SoundPlayerOpenAL::checkAlError(string(__FILE__) + string(" ") + string(__FUNCTION__) + string(" ") + intToStr(__LINE__));
...
void StaticSoundSource::play(StaticSound *sound) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSound).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSound,
                                 "In [%s::%s Line: %d] filename [%s] format = %d, sound->getSamples() = "
                                 "%d, sound->getInfo()->getSize() = %d, "
                                 "sound->getInfo()->getSamplesPerSecond() = %d\n",
                                 __FILE__, __FUNCTION__, __LINE__, sound->getFileName().c_str(), format, sound->getSamples(), sound->getInfo()->getSize(),
                                 sound->getInfo()->getSamplesPerSecond());

    bufferAllocated = true;
    alBufferData(buffer, format, sound->getSamples(), static_cast<ALsizei>(sound->getInfo()->getSize()),
...
void StreamSoundSource::update() {
    ...
    while (processed > 0) {
        ...
        if (!fillBufferAndQueue(buffer)) break;
    }
    ...
    if (!playing()) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSound).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSound,
                                     "In [%s::%s %d] Restarting audio source because "
                                     "of buffer underrun.\n",
                                     __FILE__, __FUNCTION__, __LINE__);
    ...
    case FadingOn:
        if (chrono.getMillis() > fade) {
            ...
            alSourcef(source, AL_GAIN, sound->getVolume() * static_cast<float>(chrono.getMillis()) / fade);
            SoundPlayerOpenAL::checkAlError(string(__FILE__) + string(" ") + string(__FUNCTION__) + string(" ") + intToStr(__LINE__));
        }
        break;
    case FadingOff:
        if (chrono.getMillis() > fade) {
            ...
            alSourcef(source, AL_GAIN, sound->getVolume() * (1.0f - static_cast<float>(chrono.getMillis()) / fade));
            SoundPlayerOpenAL::checkAlError(string(__FILE__) + string(" ") + string(__FUNCTION__) + string(" ") + intToStr(__LINE__));
        }
        break;
    default:
...
bool StreamSoundSource::fillBufferAndQueue(ALuint buffer) {
    ...
    alBufferData(buffer, format, bufferdata, STREAMFRAGMENTSIZE, sound->getInfo()->getSamplesPerSecond());
    delete[] bufferdata;
...
bool SoundPlayerOpenAL::init(const SoundPlayerParams *params) {
    ...
    try {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSound).enabled) {
            ...
            os << "OpenAL default device: " << (const char *)alcGetString(NULL, ALC_DEFAULT_DEVICE_SPECIFIER) << std::endl;

            // Print current device name
            if (device) {
                os << "OpenAL current device: " << (const char *)alcGetString(device, ALC_DEVICE_SPECIFIER) << std::endl;
            }
...
void SoundPlayerOpenAL::end() {
    ...
    for (StaticSoundSources::iterator i = staticSources.begin(); i != staticSources.end(); ++i) {
    ...
    for (StreamSoundSources::iterator i = streamSources.begin(); i != streamSources.end(); ++i) {
...
StaticSoundSource *SoundPlayerOpenAL::findStaticSoundSource() {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSound).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSound,
                                 "In [%s::%s %d] playingCount = %d, staticSources.size() "
                                 "= " MG_SIZE_T_SPECIFIER ", params.staticBufferCount = %u\n",

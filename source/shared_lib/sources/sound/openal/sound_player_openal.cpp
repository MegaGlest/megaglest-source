...
void SoundSource::unQueueBuffers() {
  SoundPlayerOpenAL::checkAlError(
      "Problem unqueuing buffers START OF METHOD: ");
...
ALenum SoundSource::getFormat(Sound* sound) {
  if (sound->getInfo()->getChannels() == 2) {
    if (sound->getInfo()->getBitsPerSample() == 16) {
      ...
      throw std::runtime_error("[1] Sample format not supported in file: " +
                               sound->getFileName());
    }
    ...
    if (sound->getInfo()->getBitsPerSample() == 16) {
      ...
      throw std::runtime_error("[2] Sample format not supported in file: " +
                               sound->getFileName());
    }
  }

  throw std::runtime_error("[3] Sample format not supported in file: " +
                           sound->getFileName());
}
...
void StaticSoundSource::play(StaticSound* sound) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSound).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugSound,
        "In [%s::%s Line: %d] filename [%s] format = %d, sound->getSamples() = "
        "%d, sound->getInfo()->getSize() = %d, "
        "sound->getInfo()->getSamplesPerSecond() = %d\n",
        __FILE__, __FUNCTION__, __LINE__, sound->getFileName().c_str(), format,
        sound->getSamples(), sound->getInfo()->getSize(),
  ...
  switch (fadeState) {
    case FadingOn:
      if (chrono.getMillis() > fade) {
        ...
        alSourcef(
            source, AL_GAIN,
            sound->getVolume() * static_cast<float>(chrono.getMillis()) / fade);
        SoundPlayerOpenAL::checkAlError(string(__FILE__) + string(" ") +
                                        string(__FUNCTION__) + string(" ") +
                                        intToStr(__LINE__));
      }
...
bool StreamSoundSource::fillBufferAndQueue(ALuint buffer) {
  ...
  do {
    bytesread +=
        sound->read(bufferdata + bytesread, STREAMFRAGMENTSIZE - bytesread);
...
void SoundPlayerOpenAL::end() {
  ...
  if (context != 0) {
    alcMakeContextCurrent(NULL);
    SoundPlayerOpenAL::checkAlcError(string(__FILE__) + string(" ") +
                                     string(__FUNCTION__) + string(" ") +
                                     intToStr(__LINE__));
  }
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSound).enabled)
...
StaticSoundSource* SoundPlayerOpenAL::findStaticSoundSource() {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSound).enabled)
    SystemFlags::OutputDebug(
        ...
        __FILE__, __FUNCTION__, __LINE__, playingCount,
        (unsigned long)staticSources.size(), params.staticBufferCount);
...
void SoundPlayerOpenAL::checkAlcError(string message) {
  ...
  if (err != ALC_NO_ERROR) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "%s [%s]", message.c_str(),

...
void WavSoundFileLoader::open(const string &path, SoundInfo *soundInfo) {
  ...
  if (strcmp(chunkId, "RIFF") != 0) {
    throw megaglest_runtime_error(
        "Not a valid wav file (first four bytes are not RIFF):" + path, true);
  }
  ...
  if (strcmp(chunkId, "WAVE") != 0) {
    throw megaglest_runtime_error(
        "Not a valid wav file (wave data don't start by WAVE): " + path, true);
  }
  ...
  if (strcmp(chunkId, "fmt ") != 0) {
    throw megaglest_runtime_error(
        "Not a valid wav file (first sub-chunk Id is not fmt): " + path, true);
  }
  ...
  soundInfo->setBitRate(soundInfo->getSamplesPerSecond() *
                        soundInfo->getChannels() *
                        soundInfo->getBitsPerSample() / 8);
  ...
  bytesPerSecond = soundInfo->getBitsPerSample() * 8 *
                   soundInfo->getSamplesPerSecond() * soundInfo->getChannels();
...
void WavSoundFileLoader::close() { f.close(); }

void WavSoundFileLoader::restart() { f.seekg(dataOffset, ios_base::beg); }
...
void OggSoundFileLoader::open(const string &path, SoundInfo *soundInfo) {
  ...
  if (vi == NULL) {
    throw megaglest_runtime_error(
        "Can't read ogg header info for file: " + path, true);
  }
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSound).enabled)
    SystemFlags::OutputDebug(
        ...
        "In [%s::%s Line: %d] path = [%s] vi->version = %d, vi->channels = %d, "
        "vi->rate = %ld, vi->bitrate_upper = %ld, vi->bitrate_nominal = %ld, "
        "vi->bitrate_lower = %ld, vi->bitrate_window = %ld, samples "
        "= " MG_SIZE_T_SPECIFIER "\n",
        __FILE__, __FUNCTION__, __LINE__, path.c_str(), vi->version,
        vi->channels, vi->rate, vi->bitrate_upper, vi->bitrate_nominal,
        vi->bitrate_lower, vi->bitrate_window, samples);
...
uint32 OggSoundFileLoader::read(int8 *samples, uint32 size) {
  ...
  while (size > 0) {
    int bytesRead =
        ov_read(vf, reinterpret_cast<char *>(samples), size, 0, 2, 1, &section);
    if (bytesRead == 0) {
      break;
    }
    size -= bytesRead;
...
void OggSoundFileLoader::restart() { ov_raw_seek(vf, 0); }

...
char *ConvertToUTF8(const char *s) {
    if (NULL != s && *s != '\0') return ConvertToUTF8(s, (Shared::Platform::uint32)strlen(s));
    char *ret = new char[1];
    // LOG_ASSERT(NULL != ret);
    assert(NULL != ret);
    *ret = '\0';
...
char *ConvertFromUTF8(const char *str) {
    ...
    while (inpos < len) {
        if (in[inpos] < 0x80) {
            out[outpos++] = in[inpos];
            if (in[inpos] == 0 && len == -1) break;
            inpos++;
        } else if (in[inpos] < 0xE0) {
            // Shouldn't happen.
            if (in[inpos] < 0xC0 || (len != -1 && inpos + 1 >= len) || (in[inpos + 1] & 0xC0) != 0x80) {
                out[outpos++] = '?';
                inpos++;
                continue;
            }
            outc = ((((wchar_t)in[inpos]) & 0x1F) << 6) | (((wchar_t)in[inpos + 1]) & 0x3F);
            if (outc < 256)
            ...
            // Shouldn't happen.
            if ((len != -1 && inpos + 2 >= len) || (in[inpos + 1] & 0xC0) != 0x80 || (in[inpos + 2] & 0xC0) != 0x80) {
                out[outpos++] = '?';
                inpos++;
                continue;
            }
            ...
            // Shouldn't happen.
            if ((len != -1 && inpos + 3 >= len) || (in[inpos + 1] & 0xC0) != 0x80 || (in[inpos + 2] & 0xC0) != 0x80 || (in[inpos + 3] & 0xC0) != 0x80) {
                out[outpos++] = '?';
                inpos++;
                continue;
            }
...
void WString::fromUtf8(const char *str, size_t length) {
    ...
    for (unsigned int i = 0; i < length; i++) {
        ...
        if (((Shared::Platform::byte)str[i]) >= 0xC0) {
            ...
            while (((Shared::Platform::byte)str[i]) >= 0x80) c = (c << 6) | (((Shared::Platform::byte)str[i++]) - 0x80);
#else
        if (((byte)str[i]) >= 0xC0) {
            wchar_t c = ((byte)str[i++]) - 0xC0;
            while (((byte)str[i]) >= 0x80) c = (c << 6) | (((byte)str[i++]) - 0x80);
...
void strrev_utf8(std::string &p) {
    ...
    if (p.length() > 0) {
        ...
        // printf("In [%s::%s] Line: %d bufSize = %d p.size() = %d p
        // [%s]\n",__FILE__,__FUNCTION__,__LINE__,bufSize,p.length(),p.c_str());
        // fflush(stdout);

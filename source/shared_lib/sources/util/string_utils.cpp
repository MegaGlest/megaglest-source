...
char *ConvertToUTF8(const char *s, Shared::Platform::uint32 len,
                    Shared::Platform::uint32 &newSize) {
  if (NULL == s || '\0' == *s) {
    char *ret = new char[1];
    // LOG_ASSERT(NULL != ret);
    assert(NULL != ret);
...
char *ConvertFromUTF8(const char *str) {
  ...
  while (inpos < len) {
    if (in[inpos] < 0x80) {
      ...
      // Shouldn't happen.
      if ((len != -1 && inpos + 2 >= len) || (in[inpos + 1] & 0xC0) != 0x80 ||
      ...
      // Shouldn't happen.
      if ((len != -1 && inpos + 3 >= len) || (in[inpos + 1] & 0xC0) != 0x80 ||
          (in[inpos + 2] & 0xC0) != 0x80 || (in[inpos + 3] & 0xC0) != 0x80) {
        out[outpos++] = '?';
        inpos++;
        continue;
      }
...
WString::WString(const std::string &s) { fromUtf8(s.c_str(), s.size()); }

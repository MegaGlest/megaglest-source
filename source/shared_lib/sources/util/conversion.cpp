...
bool strToBool(const string &s) {
  ...
  throw megaglest_runtime_error(
      "Error converting string to bool, expected 0 or 1, found: [" + s + "]");
}
...
int strToInt(const string &s) {
  ...
  if (*endChar != '\0') {
    throw megaglest_runtime_error(
        "Error converting from string to int, found: [" + s + "]");
  }
...
uint32 strToUInt(const string &s) {
  ...
  if (*endChar != '\0') {
    throw megaglest_runtime_error(
        "Error converting from string to uint, found: [" + s + "]");
  }
...
float strToFloat(const string &s) {
  ...
  if (*endChar != '\0') {
    throw megaglest_runtime_error(
        "Error converting from string to float, found: [" + s + "]");
  }
...
string uIntToStr(const uint64 value) {
  char str[strSize] = "";
  //	static unsigned int MAX_UNSIGNED_INT_VALUE =
  // std::numeric_limits<unsigned int>::max(); 	if(value <=
  // MAX_UNSIGNED_INT_VALUE) { 		snprintf(str, strSize-1, "%u",
  // (int)value);
  //	}
  //	else {

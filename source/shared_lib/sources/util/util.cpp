...
CURL *SystemFlags::initHTTP() {
    if (SystemFlags::curl_global_init_called == false) {
        ...
        // printf("HTTP init\n");
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line %d] calling curl_global_init\n", extractFileFromDirectoryPath(__FILE__).c_str(),
        ...
        if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line %d] curl_global_init called and returned: result %d "
                   "[%s]\n",
                   extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, result, curl_easy_strerror(result));
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line %d] calling curl_easy_init\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__);
    if (SystemFlags::VERBOSE_MODE_ENABLED)
...
void SystemFlags::Close() {
    ...
    if (threadLogger != NULL) {
        ...
        for (; threadLogger->canShutdown(false) == false && difftime(time(NULL), elapsed) <= 15;) {
            // sleep(150);
        }
        if (SystemFlags::VERBOSE_MODE_ENABLED) printf("In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
        ...
        if (SystemFlags::lockfilename != "") {
            ...
            if (SystemFlags::VERBOSE_MODE_ENABLED)
                printf("In [%s::%s Line: %d] remove_result = %d for file [%s]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
                       remove_result, SystemFlags::lockfilename.c_str());
...
void SystemFlags::handleDebug(DebugType type, const char *fmt, ...) {
    ...
    if (currentDebugLog.debugLogFileName != "" && SystemFlags::ENABLE_THREADED_LOGGING && threadLogger != NULL && threadLogger->getRunningStatus() == true) {
...
void SystemFlags::logDebugEntry(DebugType type, string debugEntry, time_t debugTime) {
  ...
  */
    ...
    if (currentDebugLog.debugLogFileName != "") {
        if (currentDebugLog.fileStream == NULL || currentDebugLog.fileStream->is_open() == false) {
            ...
            if (SystemFlags::debugLogFileList != NULL) {
                for (std::map<SystemFlags::DebugType, SystemFlags::SystemFlagsType>::iterator iterMap = SystemFlags::debugLogFileList->begin();
                    ...
                    if (iterMap->first != type && currentDebugLog.debugLogFileName == currentDebugLog2.debugLogFileName &&
            ...
            if (SystemFlags::haveSpecialOutputCommandLineOption == false) {
                if (SystemFlags::VERBOSE_MODE_ENABLED)
                    printf("Opening logfile [%s] type = %d, currentDebugLog.fileStreamOwner "
    ...
    // output to console
    if (currentDebugLog.debugLogFileName == "" ||
        (currentDebugLog.debugLogFileName != "" && (currentDebugLog.fileStream == NULL || currentDebugLog.fileStream->is_open() == false))) {
...
string lastDir(const string &s) {
    ...
    if (pos == string::npos) {
        throw megaglest_runtime_error(string(extractFileFromDirectoryPath(__FILE__).c_str()) + " line: " + intToStr(__LINE__) + " pos == string::npos for [" +
                                      s + "]");
    }

    if (pos + 1 == s.length() && s.length() > 0 && (s[pos] == '/' || s[pos] == '\\')) {
    ...
    if (compatible == false) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                     ...
                                     "[%s]\n",
                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, clientVersionString.c_str(),
                                     serverVersionString.c_str());
        if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] clientVersionString [%s], serverVersionString "
                   "[%s]\n",
        ...
        for (unsigned int i = 0; i < tokensServer.size(); ++i) {
            ...
            if (tokensSpecialVersionTags.size() > 1) {
                if (StartsWith(tokensSpecialVersionTags[1], "dev") == true || StartsWith(tokensSpecialVersionTags[1], "gamma") == true ||
                    StartsWith(tokensSpecialVersionTags[1], "alpha") == true || StartsWith(tokensSpecialVersionTags[1], "beta") == true) {
                    if (SystemFlags::VERBOSE_MODE_ENABLED)
                    ...
                    if (tokensSpecialVersionTags.size() > 2) {
                        ...
                        if (SystemFlags::VERBOSE_MODE_ENABLED)
                            printf("Line Ref: %d NEW SERVER VERSION TAG [%s] OLD TAG [%s] delim "
                                   "[%s]\n",
            ...
            if (tokensSpecialVersionTags.size() > 1) {
                if (StartsWith(tokensSpecialVersionTags[1], "dev") == true || StartsWith(tokensSpecialVersionTags[1], "gamma") == true ||
                    ...
                    if (tokensSpecialVersionTags.size() > 2) {
                        ...
                        if (SystemFlags::VERBOSE_MODE_ENABLED)
                            printf("Line Ref: %d NEW CLIENT VERSION TAG [%s] OLD TAG [%s] delim "
                                   "[%s]\n",
        ...
        if (tokens.size() >= 2 && tokensServer.size() >= 2) {
            ...
            if (tokensServer[0] != "" && IsNumeric(tokensServer[0].c_str(), false) && strToInt(tokensServer[0]) >= 3 && tokensServer[1] != "" &&
                IsNumeric(tokensServer[1].c_str(), false) && strToInt(tokensServer[1]) >= 7 && tokens[0] != "" && IsNumeric(tokens[0].c_str(), false) &&
                strToInt(tokens[0]) >= 3 && tokens[1] != "" && IsNumeric(tokens[1].c_str(), false) && strToInt(tokens[1]) >= 7) {
                if (SystemFlags::VERBOSE_MODE_ENABLED) printf("In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
                ...
                if (tokens.size() >= 3 && tokensServer.size() >= 3) {
                    ...
                    if ((tokensServer[0] != "" && IsNumeric(tokensServer[0].c_str(), false) && strToInt(tokensServer[0]) >= 3 && tokensServer[1] != "" &&
                         IsNumeric(tokensServer[1].c_str(), false) && strToInt(tokensServer[1]) >= 7 && tokensServer[2] != "" &&
                         IsNumeric(tokensServer[2].c_str(), false) && strToInt(tokensServer[2]) > 0) &&
                        (tokens[0] != "" && IsNumeric(tokens[0].c_str(), false) && strToInt(tokens[0]) >= 3 && tokens[1] != "" &&
                         IsNumeric(tokens[1].c_str(), false) && strToInt(tokens[1]) >= 7 && tokens[2] != "" && IsNumeric(tokens[2].c_str(), false) &&
                         strToInt(tokens[2]) > 0)) {
                        if (SystemFlags::VERBOSE_MODE_ENABLED)
                            printf("In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
                    ...
                    if (strToInt(tokensServer[0]) == strToInt(tokens[0]) && strToInt(tokensServer[1]) == strToInt(tokens[1])) {
                        if (SystemFlags::VERBOSE_MODE_ENABLED)
                            printf("In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
                    ...
                    if ((tokensServer[0] != "" && IsNumeric(tokensServer[0].c_str(), false) && strToInt(tokensServer[0]) >= 3 && tokensServer[1] != "" &&
                         IsNumeric(tokensServer[1].c_str(), false) && strToInt(tokensServer[1]) > 7) &&
                        (tokens[0] != "" && IsNumeric(tokens[0].c_str(), false) && strToInt(tokens[0]) >= 3 && tokens[1] != "" &&
                         IsNumeric(tokens[1].c_str(), false) && strToInt(tokens[1]) > 7)) {
                        if (SystemFlags::VERBOSE_MODE_ENABLED)
                            printf("In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
                        ...
                        if (tokens[i] != tokensServer[i]) {
                            ...
                            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                                SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                                         ...
                                                         "[%s] compatible [%d]\n",
                                                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, tokens[i].c_str(),

...
// Init statics
std::map<SystemFlags::DebugType, SystemFlags::SystemFlagsType>
...
size_t SystemFlags::httpWriteMemoryCallback(void *ptr, size_t size,
...
std::string SystemFlags::getHTTP(std::string URL, CURL *handle, int timeOut,
  ...
  curl_easy_setopt(handle, CURLOPT_WRITEFUNCTION,
                   SystemFlags::httpWriteMemoryCallback);
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("CURL errbuf [%s]\n", errbuf);
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                             "In [%s::%s Line %d] return code [%d] [%s]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, result, errbuf);
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                             "In [%s::%s Line %d] serverResponse [%s]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, serverResponse.c_str());
...
CURL *SystemFlags::initHTTP() {
  if (SystemFlags::curl_global_init_called == false) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugSystem,
                               "In [%s::%s Line %d] calling curl_global_init\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__);
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line %d] calling curl_global_init\n",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__);
    CURLcode result = curl_global_init(CURL_GLOBAL_ALL);
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugSystem,
                               "In [%s::%s Line %d] curl_global_init called "
                               "and returned: result %d [%s]\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__, result,
                               curl_easy_strerror(result));
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line %d] curl_global_init called and returned: result "
             "%d [%s]\n",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__, result, curl_easy_strerror(result));
    // printf("In [%s::%s Line %d] curl_global_init called and returned: result
    // %d
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                             "In [%s::%s Line %d] calling curl_easy_init\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__);
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("In [%s::%s Line %d] calling curl_easy_init\n",
           extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
           __LINE__);
  ...
  if (handle == NULL) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugSystem,
                               "In [%s::%s Line %d] ERROR handle = NULL\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__);
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line %d] ERROR handle = NULL\n",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__);
  }
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("In [%s::%s Line %d] handle = %p\n",
           extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
           __LINE__, handle);
...
void SystemFlags::globalCleanupHTTP() {
  ...
  if (SystemFlags::curl_global_init_called == true) {
    ...
    // printf("HTTP cleanup\n");
    curl_global_cleanup();
    // printf("In [%s::%s Line %d] curl_global_cleanup
...
SystemFlags::SystemFlagsType *SystemFlags::setupRequiredMembers() {
  ...
  if (threadLogger == NULL && SystemFlags::SHUTDOWN_PROGRAM_MODE == true) {
    // throw megaglest_runtime_error("threadLogger == NULL &&
    // SystemFlags::SHUTDOWN_PROGRAM_MODE == true");
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d] ERROR threadLogger == NULL && "
             "SystemFlags::SHUTDOWN_PROGRAM_MODE == true\n",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__);
...
void SystemFlags::init(bool haveSpecialOutputCommandLineOption) {
  SystemFlags::haveSpecialOutputCommandLineOption =
      haveSpecialOutputCommandLineOption;
  if (SystemFlags::debugLogFileList == NULL) {
    ...
    SystemFlags::debugLogFileList =
        new std::map<SystemFlags::DebugType, SystemFlags::SystemFlagsType>();

    (*SystemFlags::debugLogFileList)[SystemFlags::debugSystem] =
        SystemFlags::SystemFlagsType(SystemFlags::debugSystem);
    (*SystemFlags::debugLogFileList)[SystemFlags::debugNetwork] =
        SystemFlags::SystemFlagsType(SystemFlags::debugNetwork);
    (*SystemFlags::debugLogFileList)[SystemFlags::debugPerformance] =
        SystemFlags::SystemFlagsType(SystemFlags::debugPerformance);
    (*SystemFlags::debugLogFileList)[SystemFlags::debugWorldSynch] =
        SystemFlags::SystemFlagsType(SystemFlags::debugWorldSynch);
    (*SystemFlags::debugLogFileList)[SystemFlags::debugUnitCommands] =
        SystemFlags::SystemFlagsType(SystemFlags::debugUnitCommands);
    (*SystemFlags::debugLogFileList)[SystemFlags::debugPathFinder] =
        SystemFlags::SystemFlagsType(SystemFlags::debugPathFinder);
    (*SystemFlags::debugLogFileList)[SystemFlags::debugLUA] =
        SystemFlags::SystemFlagsType(SystemFlags::debugLUA);
    (*SystemFlags::debugLogFileList)[SystemFlags::debugSound] =
        SystemFlags::SystemFlagsType(SystemFlags::debugSound);
    (*SystemFlags::debugLogFileList)[SystemFlags::debugError] =
        SystemFlags::SystemFlagsType(SystemFlags::debugError);
  }
  ...
  if (curl_handle == NULL) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugSystem,
                               "In [%s::%s Line %d] curl_handle = %p\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__, curl_handle);
  }
}
...
void SystemFlags::Close() {
  ...
  if (SystemFlags::debugLogFileList != NULL) {
    ...
    for (std::map<SystemFlags::DebugType,
                  SystemFlags::SystemFlagsType>::iterator iterMap =
             SystemFlags::debugLogFileList->begin();
    ...
    if (SystemFlags::lockfilename != "") {
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("In [%s::%s Line: %d] remove_result = %d for file [%s]\n",
               extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
               __LINE__, remove_result, SystemFlags::lockfilename.c_str());
...
void SystemFlags::handleDebug(DebugType type, const char *fmt, ...) {
  ...
  // SystemFlags::SystemFlagsType &currentDebugLog =
  // (*SystemFlags::debugLogFileList)[type];
  ...
  if (currentDebugLog.debugLogFileName != "" &&
      SystemFlags::ENABLE_THREADED_LOGGING && threadLogger != NULL &&
...
void SystemFlags::logDebugEntry(DebugType type, string debugEntry,
                                time_t debugTime) {
  if (SystemFlags::debugLogFileList == NULL) {
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d]\n",
  ...
  char szBuf2[100] = "";
  // if (type != debugPathFinder && type != debugError && type !=
  // debugWorldSynch) {
  ...
  if (currentDebugLog.debugLogFileName != "") {
    if (currentDebugLog.fileStream == NULL ||
        ...
        for (std::map<SystemFlags::DebugType,
          ...
          if (iterMap->first != type &&
              currentDebugLog.debugLogFileName ==
                  currentDebugLog2.debugLogFileName &&
      ...
      if (SystemFlags::lockFile == -1) {
        ...
        SystemFlags::lockFile =
            open(lockfile.c_str(), O_WRONLY | O_CREAT, S_IREAD | S_IWRITE);
#else
        SystemFlags::lockFile =
            _open(lockfile.c_str(), O_WRONLY | O_CREAT, S_IREAD | S_IWRITE);
#endif
        if (SystemFlags::lockFile < 0 ||
            acquire_file_lock(SystemFlags::lockFile) == false) {
          ...
          for (idx = 1; idx <= 100; ++idx) {
            ...
            // SystemFlags::lockFile = open(newlockfile.c_str(), O_WRONLY |
            // O_CREAT | O_EXCL, S_IRUSR|S_IWUSR);
#ifndef WIN32
            SystemFlags::lockFile = open(
                newlockfile.c_str(), O_WRONLY | O_CREAT, S_IREAD | S_IWRITE);
#else
            SystemFlags::lockFile = _open(
                newlockfile.c_str(), O_WRONLY | O_CREAT, S_IREAD | S_IWRITE);

#endif
            if (SystemFlags::lockFile >= 0 &&
                acquire_file_lock(SystemFlags::lockFile) == true) {
              break;
            }
          }
      ...
      if (currentDebugLog.fileStream == NULL) {
        ...
        currentDebugLog.fileStream =
            new std::ofstream(_wfopen(utf8_decode(debugLog).c_str(), L"w"));
#else
        currentDebugLog.fileStream = new std::ofstream();
        currentDebugLog.fileStream->open(debugLog.c_str(),
      ...
      if (SystemFlags::haveSpecialOutputCommandLineOption == false) {
        if (SystemFlags::VERBOSE_MODE_ENABLED)
          printf(
              "Opening logfile [%s] type = %d, currentDebugLog.fileStreamOwner "
              "= %d, file stream open = %d\n",
              debugLog.c_str(), type, currentDebugLog.fileStreamOwner,
              currentDebugLog.fileStream->is_open());
      }

      if (currentDebugLog.fileStream->is_open() == true) {
        MutexSafeWrapper safeMutex(
            currentDebugLog.mutex,
            string(extractFileFromDirectoryPath(__FILE__).c_str()) + "_" +
                intToStr(__LINE__));

        (*currentDebugLog.fileStream)
            << "Starting Mega-Glest logging for type: " << type << "\n";
    ...
    if (currentDebugLog.fileStream->is_open() == true) {
      static string mutexCodeLocation =
          string(extractFileFromDirectoryPath(__FILE__).c_str()) + "_" +
          intToStr(__LINE__);
      ...
      if (type != debugPathFinder && type != debugError &&
          type != debugWorldSynch) {
        (*currentDebugLog.fileStream)
            << "[" << szBuf2 << "] " << debugEntry.c_str();
      } else if (type == debugError) {
        (*currentDebugLog.fileStream)
            << "[" << szBuf2 << "] *ERROR* " << debugEntry.c_str();
      } else {
...
string lastDir(const string &s) {
  ...
  if (pos == string::npos) {
    throw megaglest_runtime_error(
        string(extractFileFromDirectoryPath(__FILE__).c_str()) + " line: " +
        intToStr(__LINE__) + " pos == string::npos for [" + s + "]");
  }
...
string lastFile(const string &s) { return lastDir(s); }
...
string cutLastFile(const string &s) {
  ...
  if (pos != string::npos) {
    ...
    // + " line: " + intToStr(__LINE__) + " pos == string::npos for [" + s +
    // "]");
    //}
...
string cutLastExt(const string &s) {
  ...
  if (i != string::npos) {
    // throw
    // megaglest_runtime_error(string(extractFileFromDirectoryPath(__FILE__).c_str())
    // + " line: " + intToStr(__LINE__) + " i==string::npos for [" + s + "]");
    //}
...
int compareMajorMinorVersion(const string &versionA, const string &versionB) {
  ...
  // printf("majorA:%d  minorA:%d majorB:%d
  ...
  if (compatible == false) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(
          ...
          "In [%s::%s Line: %d] clientVersionString [%s], serverVersionString "
          "[%s]\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, clientVersionString.c_str(), serverVersionString.c_str());
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d] clientVersionString [%s], "
             "serverVersionString [%s]\n",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__, clientVersionString.c_str(),
             serverVersionString.c_str());
    ...
    for (unsigned int i = 0; i < tokensServer.size(); ++i) {
      ...
      if (tokensSpecialVersionTags.size() > 1) {
        if (StartsWith(tokensSpecialVersionTags[1], "dev") == true ||
          ...
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("Line Ref: %d FOUND SPECIAL SERVER VERSION TAG [%s] [%s] "
                   "tokensSpecialVersionTags.size() = " MG_SIZE_T_SPECIFIER
                   "\n",
                   __LINE__, tokensSpecialVersionTags[0].c_str(),
                   tokensSpecialVersionTags[1].c_str(),
                   tokensSpecialVersionTags.size());
          ...
          if (tokensSpecialVersionTags.size() > 2) {
            ...
            string newVersionText =
                trim_at_delim(serverVersionString, trimRightDelim);

            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf("Line Ref: %d NEW SERVER VERSION TAG [%s] OLD TAG [%s] "
                     "delim [%s]\n",
                     __LINE__, newVersionText.c_str(),
                     serverVersionString.c_str(), trimRightDelim.c_str());
    ...
    for (unsigned int i = 0; i < tokens.size(); ++i) {
      ...
      if (tokensSpecialVersionTags.size() > 1) {
        if (StartsWith(tokensSpecialVersionTags[1], "dev") == true ||
          ...
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("Line Ref: %d FOUND SPECIAL CLIENT VERSION TAG [%s] [%s] "
                   "tokensSpecialVersionTags.size() = " MG_SIZE_T_SPECIFIER
                   "\n",
                   __LINE__, tokensSpecialVersionTags[0].c_str(),
                   tokensSpecialVersionTags[1].c_str(),
          ...
          if (tokensSpecialVersionTags.size() > 2) {
            string trimRightDelim = "-" + tokensSpecialVersionTags[2];
            string newVersionText =
                trim_at_delim(clientVersionString, trimRightDelim);

            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf("Line Ref: %d NEW CLIENT VERSION TAG [%s] OLD TAG [%s] "
                     "delim [%s]\n",
                     __LINE__, newVersionText.c_str(),
                     clientVersionString.c_str(), trimRightDelim.c_str());
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED) {
      ...
      for (unsigned int i = 0; i < tokensServer.size(); ++i) {
        printf(
            "Line Ref: %d Server version index = %u str [%s] IsNumeric = %d\n",
            __LINE__, i, tokensServer[i].c_str(),
            IsNumeric(tokensServer[i].c_str(), false));
      }
      for (unsigned int i = 0; i < tokens.size(); ++i) {
        printf(
            "Line Ref: %d Client version index = %u str [%s] IsNumeric = %d\n",
            __LINE__, i, tokens[i].c_str(),
            IsNumeric(tokens[i].c_str(), false));
      }
    }
    // **NOTE:
    ...
    if (tokens.size() >= 2 && tokensServer.size() >= 2) {
      ...
      if (tokensServer[0] != "" && IsNumeric(tokensServer[0].c_str(), false) &&
          strToInt(tokensServer[0]) >= 3 && tokensServer[1] != "" &&
          IsNumeric(tokensServer[1].c_str(), false) &&
          strToInt(tokensServer[1]) >= 7 && tokens[0] != "" &&
          IsNumeric(tokens[0].c_str(), false) && strToInt(tokens[0]) >= 3 &&
          tokens[1] != "" && IsNumeric(tokens[1].c_str(), false) &&
          strToInt(tokens[1]) >= 7) {
        ...
        if (tokens.size() >= 3 && tokensServer.size() >= 3) {
          ...
          if ((tokensServer[0] != "" &&
               IsNumeric(tokensServer[0].c_str(), false) &&
               strToInt(tokensServer[0]) >= 3 && tokensServer[1] != "" &&
               IsNumeric(tokensServer[1].c_str(), false) &&
               strToInt(tokensServer[1]) >= 7 && tokensServer[2] != "" &&
               IsNumeric(tokensServer[2].c_str(), false) &&
               strToInt(tokensServer[2]) > 0) &&
              (tokens[0] != "" && IsNumeric(tokens[0].c_str(), false) &&
               strToInt(tokens[0]) >= 3 && tokens[1] != "" &&
               IsNumeric(tokens[1].c_str(), false) &&
               strToInt(tokens[1]) >= 7 && tokens[2] != "" &&
               IsNumeric(tokens[2].c_str(), false) &&
               strToInt(tokens[2]) > 0)) {
          ...
          // If both are > 3.7 use new version checking of 2 digits since both
          // are higher than 3.7.0
          if ((tokensServer[0] != "" &&
               IsNumeric(tokensServer[0].c_str(), false) &&
               strToInt(tokensServer[0]) >= 3 && tokensServer[1] != "" &&
               IsNumeric(tokensServer[1].c_str(), false) &&
               strToInt(tokensServer[1]) > 7) &&
              (tokens[0] != "" && IsNumeric(tokens[0].c_str(), false) &&
               strToInt(tokens[0]) >= 3 && tokens[1] != "" &&
               IsNumeric(tokens[1].c_str(), false) &&
               strToInt(tokens[1]) > 7)) {
        ...
        if (compatiblePost3_7_0_Check == true) {
          ...
          // only check the first 2 sections with . to compare major versions
          // #'s
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugSystem,
                "In [%s::%s Line: %d] clientVersionString [%s], "
                "serverVersionString [%s] compatible [%d]\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, clientVersionString.c_str(),
                serverVersionString.c_str(), compatible);
          ...
          for (int i = 0; compatible == true && i < 2; ++i) {
            ...
            if (tokens[i] != tokensServer[i]) {
              if (SystemFlags::VERBOSE_MODE_ENABLED)
                printf("In [%s::%s Line: %d] tokens[i] = [%s] tokensServer[i] "
                       "= [%s]\n",
                       extractFileFromDirectoryPath(__FILE__).c_str(),
                       __FUNCTION__, __LINE__, tokens[i].c_str(),
                       tokensServer[i].c_str());
              ...
              if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem)
                ...
                SystemFlags::OutputDebug(
                    SystemFlags::debugSystem,
                    "In [%s::%s Line: %d] tokens[i] = [%s], tokensServer[i] = "
                    "[%s] compatible [%d]\n",
                    extractFileFromDirectoryPath(__FILE__).c_str(),
                    __FUNCTION__, __LINE__, tokens[i].c_str(),
                    tokensServer[i].c_str(), compatible);
            }
          }
        }
      }
        ...
        if (tokens[i] != tokensServer[i]) {
          ...
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] tokens[i] = [%s], tokensServer[i] = "
                   "[%s] compatible [%d]\n",
                   extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                   __LINE__, tokens[i].c_str(), tokensServer[i].c_str(),
                   compatible);
        }
      }
    }

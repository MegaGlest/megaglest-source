...
CURL *SystemFlags::initHTTP() {
  if (SystemFlags::curl_global_init_called == false) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "In [%s::%s Line %d] curl_global_init called and returned: result %d "
          "[%s]\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
...
SystemFlags::SystemFlagsType *SystemFlags::setupRequiredMembers() {
  ...
  if (threadLogger == NULL && SystemFlags::SHUTDOWN_PROGRAM_MODE == true) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "In [%s::%s Line: %d] ERROR threadLogger == NULL && "
          "SystemFlags::SHUTDOWN_PROGRAM_MODE == true\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__);
    ...
    for (unsigned int i = 0; i < tokensServer.size(); ++i) {
      ...
      if (tokensSpecialVersionTags.size() > 1) {
        if (StartsWith(tokensSpecialVersionTags[1], "dev") == true ||
          ...
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf(
                "Line Ref: %d FOUND SPECIAL SERVER VERSION TAG [%s] [%s] "
          ...
          if (tokensSpecialVersionTags.size() > 2) {
            ...
            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf(
                  "Line Ref: %d NEW SERVER VERSION TAG [%s] OLD TAG [%s] delim "
                  "[%s]\n",
                  __LINE__, newVersionText.c_str(), serverVersionString.c_str(),
                  trimRightDelim.c_str());
    ...
    for (unsigned int i = 0; i < tokens.size(); ++i) {
      ...
      if (tokensSpecialVersionTags.size() > 1) {
        if (StartsWith(tokensSpecialVersionTags[1], "dev") == true ||
          ...
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf(
                "Line Ref: %d FOUND SPECIAL CLIENT VERSION TAG [%s] [%s] "
                "tokensSpecialVersionTags.size() = " MG_SIZE_T_SPECIFIER "\n",
                __LINE__, tokensSpecialVersionTags[0].c_str(),
                tokensSpecialVersionTags[1].c_str(),
                tokensSpecialVersionTags.size());
          ...
          if (tokensSpecialVersionTags.size() > 2) {
            ...
            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf(
                  "Line Ref: %d NEW CLIENT VERSION TAG [%s] OLD TAG [%s] delim "
                  "[%s]\n",
                  __LINE__, newVersionText.c_str(), clientVersionString.c_str(),
                  trimRightDelim.c_str());
    ...
    if (compatiblePre3_7_0_1_Check == true) {
      ...
      for (int i = 0; compatible == true && i < 3; ++i) {
        if (tokens[i] != tokensServer[i]) {
          ...
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf(
                "In [%s::%s Line: %d] tokens[i] = [%s], tokensServer[i] = [%s] "
                "compatible [%d]\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,

...
void Checksum::addSum(uint32 value) { sum += value; }
  ...
  */
  ...
  /*
     ...
     if ((data = (int8 *)malloc(bytes_expected)) == NULL)
             throw megaglest_runtime_error("malloc failed, Can not open file: "
     + path);

     bytes_read = read(fd, data, bytes_expected);

     //if (bytes_read != bytes_expected)
     //   throw megaglest_runtime_error("read failed, Can not open file: " +
     path);
                              ...
                              for(int i = 0; i < bufSize && buf[i] != 0; i++) {
                                  ...
                                  if(isXMLFile == true) {
                                          if(inCommentTag == true) {
                                                  if(buf[i] == '>' && i >= 3 &&
  buf[i-1] == '-' && buf[i-2] == '-') { inCommentTag = false;
                                                          //printf("TURNING OFF
  comment TAG, i = %d [%c]",i,buf[i]);
                                                  }
                                                  else {
                                                          //printf("SKIPPING XML
  comment character, i = %d [%c]",i,buf[i]);
                                                  }
                                                  continue;
                                          }
                                          //else if(buf[i] == '-' && i >= 4 &&
  buf[i-1] == '-' && buf[i-2] == '!' && buf[i-3] == '<') { else if(buf[i] == '<'
  && i+4 < bufSize && buf[i+1] == '!' && buf[i+2] == '-' && buf[i+3] == '-') {
                                                  inCommentTag = true;
                                                  //printf("TURNING ON comment
  TAG, i = %d [%c]",i,buf[i]); continue;
                                          }
                                          else if(buf[i] == ' ' || buf[i] ==
  '\t' || buf[i] == '\n' || buf[i] == '\r') {
                                                  //printf("SKIPPING special
  character, i = %d [%c]",i,buf[i]); continue;
                                      }
                                  }
                                          addByte(buf[i]);
                              }
  ...
  if (ifs) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(
          ...
          "In [%s::%s Line: %d] buf.size() = %d, path [%s], isXMLFile = %d\n",
          __FILE__, __FUNCTION__, __LINE__, buf.size(), path.c_str(),
          isXMLFile);
    ...
    if (isXMLFile == true) {
      ...
      for (std::size_t i = 0; i < buf.size(); ++i) {
        ...
        // else if(buf[i] == '-' && i >= 4 && buf[i-1] == '-' && buf[i-2] == '!'
        // && buf[i-3] == '<') {
        else if (buf[i] == '<' && i + 4 < bufSize && buf[i + 1] == '!' &&
                 buf[i + 2] == '-' && buf[i + 3] == '-') {
          inCommentTag = true;
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
          SystemFlags::OutputDebug(
              SystemFlags::debugSystem,
              "In [%s::%s Line: %d] %d / %d, cipher = %u\n", __FILE__,
              __FUNCTION__, __LINE__, i, buf.size(), cipher);
      }
    } else {
      uint32 cipher = addBytes(&buf[0], buf.size());
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugSystem, "In [%s::%s Line: %d] %d, cipher = %u\n",
...

    {
      ...
      for (std::map<string, uint32>::iterator iterMap = fileList.begin();
        ...
        MutexSafeWrapper safeMutexSocketDestructorFlag(
            &Checksum::fileListCacheSynchAccessor,
            string(__FILE__) + "_" + intToStr(__LINE__));
        if (Checksum::fileListCache.find(iterMap->first) ==
            Checksum::fileListCache.end()) {
          Checksum fileResult;
          ...
          // printf("fileAddedOk = %d for file [%s] CRC
          // [%d]\n",fileAddedOk,iterMap->first.c_str(),Checksum::fileListCache[iterMap->first]);
        } else {
          // printf("Getting checksum from CACHE for file [%s] CRC
...
// uint32 Checksum::getFinalFileListSum() {
//	if(SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
// SystemFlags::OutputDebug(SystemFlags::debugSystem,"In [%s::%s Line: %d] sum =
...
uint32 Checksum::getFileCount() { return (uint32)fileList.size(); }

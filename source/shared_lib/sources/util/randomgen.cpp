...
void RandomGen::init(int seed) { lastNumber = seed % m; }
...
int RandomGen::randRange(int min, int max, string lastCaller) {
  if (min > max) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "In [%s::%s Line: %d] min > max, min = %d, max = %d",
             __FILE__, __FUNCTION__, __LINE__, min, max);
    throw megaglest_runtime_error(szBuf);
  }

  int diff = max - min;
  float numerator =
      static_cast<float>(diff + 1) * static_cast<float>(this->rand(lastCaller));
  int res = min + static_cast<int>(truncateDecimal<float>(
                      numerator / static_cast<float>(m), 6));
  if (res < min || res > max) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] res < min || res > max, min = %d, max = %d, "
             "res = %d",
...
float RandomGen::randRange(float min, float max, string lastCaller) {
  if (min > max) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "In [%s::%s Line: %d] min > max, min = %f, max = %f",
             __FILE__, __FUNCTION__, __LINE__, min, max);
    throw megaglest_runtime_error(szBuf);
  }
  ...
  if (res < min || res > max) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] res < min || res > max, min = %f, max = %f, "
             "res = %f",
             __FILE__, __FUNCTION__, __LINE__, min, max, res);
    throw megaglest_runtime_error(szBuf);
  }

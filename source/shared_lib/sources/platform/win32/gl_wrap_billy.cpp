...
void createGlFontBitmaps(uint32 &base, const string &type, int size, int width, int charCount, FontMetrics &metrics) {
    ...
    if (systemFontList.empty() == true) {
        ...
        ::EnumFontFamiliesEx(hDC, &lf, (FONTENUMPROC)EnumFontFamExProc, (LPARAM)&systemFontList, 0);
            ...
            if (_stricmp(useRealFontName.c_str(), fontName.c_str()) != 0) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] switch font name from [%s] to [%s]\n", __FILE__, __FUNCTION__,
                                             __LINE__, useRealFontName.c_str(), fontName.c_str());
    ...
    HFONT font = CreateFont(size, 0, 0, 0, width, FALSE, FALSE, FALSE, PlatformContextGl::charSet, OUT_TT_ONLY_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,

...
LONG WINAPI PlatformExceptionHandler::handler(LPEXCEPTION_POINTERS pointers) {
    ...
    HANDLE hFile = CreateFile(wstr, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
    if (wstr) delete[] wstr;
...
#if !defined(__GNUC__)
    MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), hFile, MiniDumpNormal, &lExceptionInformation, NULL, NULL);
...
string PlatformExceptionHandler::getStackTrace() {
    ...
    if (GetThreadContext(hThread, &context)) {
        ...
        do {
            ...
            char szBuf[8096] = "";
            snprintf(szBuf, 8096, "%08lx %08lx %08lx %08lx %s!%s %s(%lu) %+ld\n", stackframe.AddrReturn.Offset, stackframe.Params[0], stackframe.Params[1],
                     stackframe.Params[2], pSym->Name, module.ModuleName, line.FileName, line.LineNumber, dwDisplacement);
            result += szBuf;

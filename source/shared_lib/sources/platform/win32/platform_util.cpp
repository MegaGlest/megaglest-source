...
LPWSTR Ansi2WideString(LPCSTR lpaszString) {
  ...
  if (lpaszString != NULL) {
    ...
    if (::MultiByteToWideChar(CP_ACP, 0, lpaszString, nLen, lpwszString,
                              nLen) == 0) {
      // Conversation failed
      return lpwszString;
    }
...
std::wstring utf8_decode(const std::string &str) {
  ...
  int size_needed = MultiByteToWideChar(CP_UTF8, 0, &friendly_path[0],
                                        (int)friendly_path.size(), NULL, 0);
  std::wstring wstrTo(size_needed, 0);
  MultiByteToWideChar(CP_UTF8, 0, &friendly_path[0], (int)friendly_path.size(),
...
LONG WINAPI PlatformExceptionHandler::handler(LPEXCEPTION_POINTERS pointers) {
  ...
  HANDLE hFile = CreateFile(wstr, GENERIC_WRITE, FILE_SHARE_WRITE, NULL,
  ...
  MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), hFile,
                    MiniDumpNormal, &lExceptionInformation, NULL, NULL);
...
string PlatformExceptionHandler::getStackTrace() {
  ...
  if (GetThreadContext(hThread, &context)) {
    ...
    do {
      ...
      SymGetSymFromAddr(hProcess, stackframe.AddrPC.Offset, &dwDisplacement,
                        pSym);
      SymGetLineFromAddr(hProcess, stackframe.AddrPC.Offset, &dwDisplacement2,
                         &line);
      ...
      snprintf(szBuf, 8096, "%08lx %08lx %08lx %08lx %s!%s %s(%lu) %+ld\n",
               stackframe.AddrReturn.Offset, stackframe.Params[0],
               stackframe.Params[1], stackframe.Params[2], pSym->Name,
               module.ModuleName, line.FileName, line.LineNumber,
               dwDisplacement);
      result += szBuf;
...
megaglest_runtime_error::megaglest_runtime_error(const string &__arg,
                                                 bool noStackTrace)
    : std::runtime_error(noStackTrace == false
                             ? __arg + PlatformExceptionHandler::getStackTrace()
                             : __arg),
...
// void getFullscreenVideoInfo(int &colorBits,int &screenWidth,int
// &screenHeight) {
...
//
//		//printf("devMode.dmPelsWidth = %d, devMode.dmPelsHeight = %d,
...
// refreshFrequency){ 	DEVMODE devMode;
//
//	for (int i=0; EnumDisplaySettings(NULL, i, &devMode) ;i++){
...
void done_win32() { ::DestroyIcon(icon); }
...
void CheckPacketThrottling() {
  ...
  wstring subKey = L"SOFTWARE\\Microsoft\\Windows "
                   L"NT\\CurrentVersion\\Multimedia\\SystemProfile";
  wstring Key = L"NetworkThrottlingIndex";
  HKEY keyHandle;
  DWORD dwDesiredThrottle = 0xffffffff;
  // LONG reg_result = RegCreateKeyEx(HKEY_LOCAL_MACHINE,subKey.c_str(),0, NULL,
  // 0, KEY_ALL_ACCESS, NULL, &keyHandle, &dwDisposition); LONG reg_result =
  // RegOpenKeyEx(HKEY_LOCAL_MACHINE,subKey.c_str(),0, KEY_ALL_ACCESS,
  // &keyHandle);
  LONG reg_result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, subKey.c_str(), 0,
                                 KEY_QUERY_VALUE, &keyHandle);

  if (reg_result != ERROR_SUCCESS) {
    // printf("\nError opening network throttle registry hive:
    // %d\n",reg_result);
    return;
  }
  // Set the value.
  ...
  reg_result = RegQueryValueEx(keyHandle, Key.c_str(), 0, 0,
                               (LPBYTE)&disableThrottle, &len);
  if (reg_result != ERROR_SUCCESS) {
  ...
  if (disableThrottle != dwDesiredThrottle) {
    ...
    //		reg_result = RegSetValueEx(keyHandle, L"NetworkThrottlingIndex",
    //0, REG_DWORD, (LPBYTE) &disableThrottle, len); 		if(reg_result !=
    // ERROR_SUCCESS) { 			printf("Error setting network throttle registry key:
    // %d

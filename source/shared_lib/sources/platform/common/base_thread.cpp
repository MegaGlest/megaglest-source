...
BaseThread::BaseThread()
    : Thread(), mutexRunning(new Mutex(CODE_AT_LINE)),
      ...
      mutexStarted(new Mutex(CODE_AT_LINE)), ptr(NULL), genericData(NULL) {
...
BaseThread::~BaseThread() {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                             ...
                             __FUNCTION__, __LINE__, uniqueID.c_str());
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(
        ...
        "In [%s::%s Line: %d] uniqueID [%s] ret [%d] END\n", __FILE__,
        __FUNCTION__, __LINE__, uniqueID.c_str(), ret);
...
void BaseThread::signalQuit() { setQuitStatus(true); }
...
bool BaseThread::canShutdown(bool deleteSelfIfShutdownDelayed) { return true; }
...
bool BaseThread::shutdownAndWait() {
  ...
  if (pThread->getRunningStatus() == true) {
    ...
    for (time_t elapsed = time(NULL);
         difftime(time(NULL), elapsed) <= maxWaitSeconds;) {

...
int64 Chrono::queryCounter(int64 multiplier) {
    if (multiplier == lastMultiplier && stopped == lastStopped && lastStartCount == startCount) {
        if (stopped == true) {
            return lastResult;
        } else {
            Uint32 endCount = SDL_GetTicks();
...
string getFormattedCRCCacheFileName(std::pair<string, string> cacheKeys) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] cacheKeys.first = [%s] cacheKeys.second [%s]\n",
...
pair<bool, time_t> hasCachedFileCRCValue(string crcCacheFile, uint32 &value) {
    ...
    if (fileExists(crcCacheFile) == true) {
        ...
        if (fp != NULL) {
            ...
            int readbytes =
                fscanf(fp, "%20ld,%20u,%20ld\n%499s\n%499s\n%8095s", &refreshDate, &crcValue, &lastUpdateDate, &gameVer[0], &gitVer[0], &actualFilePath[0]);
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled) {
                SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s] Line: %d for Cache file [%s] readbytes = %d\n", __FILE__, __FUNCTION__,
                                         __LINE__, crcCacheFile.c_str(), readbytes);
            }
            ...
            if (readGameVer != "" && readGitVer != "" && refreshDate > 0 && refreshDate > tBadCRCDate && time(NULL) < refreshDate) {
                result.first = true;
                value = crcValue;
...
void writeCachedFileCRCValue(string crcCacheFile, uint32 &crcValue, string actualFileName) {
    ...
    if (fp != NULL) {
        ...
        fprintf(fp, "%20ld,%20u,%20ld", Shared::PlatformByteOrder::toCommonEndian(refreshDate), Shared::PlatformByteOrder::toCommonEndian(crcValue),
                Shared::PlatformByteOrder::toCommonEndian(now));

        fprintf(fp, "\n%s\n%s\n%s", writeGameVer.c_str(), writeGameGITVersion.c_str(), writeActualFileName.c_str());
...
uint32 getFolderTreeContentsCheckSumRecursively(vector<string> paths, string pathSearchString, const string &filterFileExt, Checksum *recursiveChecksum,
                                                bool forceNoCache) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                 "-------------- In [%s::%s Line: %d] Calculating "
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] Final CRC file count: %d\n", __FILE__, __FUNCTION__, __LINE__,
...
uint32 getFolderTreeContentsCheckSumRecursively(const string &path, const string &filterFileExt, Checksum *recursiveChecksum, bool forceNoCache) {
    ...
    for (int i = 0; i < (int)globbuf.gl_pathc; ++i) {
        ...
        if (S_ISDIR(statStruct.st_mode) == 0) continue;
#endif
        const char *p = globbuf.gl_pathv[i];
        // printf("Line: %d p [%s]\n",__LINE__,p);
    ...
    if (topLevelCaller == true) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] Final CRC file count for [%s]: %d\n", __FILE__, __FUNCTION__, __LINE__,
                                     path.c_str(), checksum.getFileCount());
...
void clearFolderTreeContentsCheckSumList(vector<string> paths, const string &pathSearchString, const string &filterFileExt) {
    ...
    std::map<string, vector<std::pair<string, uint32>>> &crcTreeCache =
        CacheManager::getCachedItem<std::map<string, vector<std::pair<string, uint32>>>>(cacheLookupId);
...
vector<std::pair<string, uint32>> getFolderTreeContentsCheckSumListRecursively(vector<string> paths, const string &pathSearchString,
                                                                               const string &filterFileExt, vector<std::pair<string, uint32>> *recursiveMap) {
    std::pair<string, string> cacheKeys = getFolderTreeContentsCheckSumListCacheKey(paths, pathSearchString, filterFileExt);
    string cacheLookupId = cacheKeys.first;
    std::map<string, vector<std::pair<string, uint32>>> &crcTreeCache =
        CacheManager::getCachedItem<std::map<string, vector<std::pair<string, uint32>>>>(cacheLookupId);
...
vector<string> getFolderTreeContentsListRecursively(const string &path, const string &filterFileExt, bool includeFolders, vector<string> *recursiveMap) {
   ...
   */
    ...
    for (int i = 0; i < (int)globbuf.gl_pathc; ++i) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "~~~~~~~~~~~ Glob file [%s] skipItem = %d\n", p, skipItem);
    ...
    for (int i = 0; i < (int)globbuf.gl_pathc; ++i) {
        ...
        bool skipItem = (EndsWith(p, ".") == true || EndsWith(p, "..") == true);
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "~~~~~~~~~~~ Glob folder [%s] skipItem = %d\n", p, skipItem);
...
// finds all filenames like path and gets the checksum of each file
vector<std::pair<string, uint32>> getFolderTreeContentsCheckSumListRecursively(const string &path, const string &filterFileExt,
   ...
   */
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s] scanning [%s] cacheKey [%s] checksumFiles.size() = %d\n", __FILE__, __FUNCTION__,
                                 path.c_str(), cacheKey.c_str(), checksumFiles.size());
...
vector<char> replaceAllBetweenTokens(vector<char> &context, const string &startToken, const string &endToken, const string &newText, bool removeTokens) {
...
string replaceAllBetweenTokens(string &context, const string &startToken, const string &endToken, const string &newText, bool removeTokens) {
    size_t lookHere = 0;
    size_t foundHere = 0;
...
string getFullFileArchiveExtractCommand(const string &fileArchiveExtractCommand, string fileArchiveExtractCommandParameters, const string &outputpath,
...
string getFullFileArchiveCompressCommand(const string &fileArchiveCompressCommand, string fileArchiveCompressCommandParameters, const string &archivename,
...
bool executeShellCommand(string cmd, int expectedResult, ShellCommandOutputCallbackInterface *cb) {
    bool result = false;
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem, "About to run [%s]", cmd.c_str());
    ...
    if (file != NULL) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "Process returned %d\n", cmdRet);
...
void ValueCheckerVault::checkItemInVault(const void *ptr, int value) const {
    ...
    if (iterFind == vaultList.end()) {
        ...
        // for(map<const void
        //*,string>::const_iterator iterFind = vaultList.begin();
        // iterFind != vaultList.end(); iterFind++) {
        // printf("In [%s::%s Line: %d] LIST-- check vault key [%p] value
    ...
    if (iterFind->second != checksum.addInt(value)) {
        ...
        // vaultList.begin(); 					iterFind !=
        // vaultList.end(); iterFind++) { 				printf("In

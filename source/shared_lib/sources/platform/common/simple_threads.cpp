...
FileCRCPreCacheThread::FileCRCPreCacheThread()
    : BaseThread(), mutexPauseForGame(new Mutex(CODE_AT_LINE)) {
...
FileCRCPreCacheThread::FileCRCPreCacheThread(
    vector<string> techDataPaths, vector<string> workerThreadTechPaths,
...
void FileCRCPreCacheThread::setPauseForGame(bool pauseForGame) {
  ...
  for (unsigned int index = 0; index < preCacheWorkerThreadList.size();
       ++index) {
...
void FileCRCPreCacheThread::execute() {
  {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(
          ...
          "FILE CRC PreCache thread is running threadControllerMode = %d\n",
          threadControllerMode);
    ...
    try {
      if (threadControllerMode == true) {
        ...
        if (techPaths.empty() == false) {
          ...
          vector<string>::iterator iterFindMegaPack = std::find(
              techPaths.begin(), techPaths.end(), megapackTechtreeName);
          if (iterFindMegaPack != techPaths.end()) {
            techPaths.erase(iterFindMegaPack);
            techPaths.insert(techPaths.begin(), megapackTechtreeName);

            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf("In [%s::%s Line: %d] Found megapack techtree and placing "
                     "it at the TOP of the list\n",
                     extractFileFromDirectoryPath(__FILE__).c_str(),
                     __FUNCTION__, __LINE__);
          }
          unsigned int techsPerWorker =
              ((unsigned int)techPaths.size() /
               (unsigned int)MAX_FileCRCPreCacheThread_WORKER_THREADS);
          ...
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] techsPerWorker = %u, "
                   "MAX_FileCRCPreCacheThread_WORKER_THREADS = %d, "
                   "techPaths.size() = %d\n",
                   extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                   __LINE__, techsPerWorker,
                   MAX_FileCRCPreCacheThread_WORKER_THREADS,
                   (int)techPaths.size());
          ...
          try {
            ...
            for (unsigned int workerIdx = 0;
              ...
              unsigned int currentWorkerMax =
                  ((unsigned int)techPaths.size() - consumedWorkers);
              if (currentWorkerMax > techsPerWorker) {
                currentWorkerMax = techsPerWorker;
              }

              vector<string> workerTechList;
              unsigned int endConsumerIndex =
                  consumedWorkers + currentWorkerMax;
              for (unsigned int idx = consumedWorkers; idx < endConsumerIndex;
                   idx++) {
                string techName = techPaths[idx];
                workerTechList.push_back(techName);

                if (SystemFlags::VERBOSE_MODE_ENABLED)
                  printf("In [%s::%s Line: %d] Spawning CRC thread for Tech "
                         "[%s] [%u of %lu]\n",
                         extractFileFromDirectoryPath(__FILE__).c_str(),
                         __FUNCTION__, __LINE__, techName.c_str(), idx + 1,
                         techPaths.size());
              }

              if (SystemFlags::VERBOSE_MODE_ENABLED)
                printf("In [%s::%s Line: %d] workerIdx = %u, currentWorkerMax "
                       "= %u, endConsumerIndex = %u\n",
                       extractFileFromDirectoryPath(__FILE__).c_str(),
                       __FUNCTION__, __LINE__, workerIdx, currentWorkerMax,
                       endConsumerIndex);
              ...
              while (getQuitStatus() == false &&
                     difftime(time(NULL), pauseTime) <=
                         PAUSE_SECONDS_BETWEEN_WORKERS) {
                sleep(25);
              }
              if (getQuitStatus() == true) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem)
              ...
              if (SystemFlags::VERBOSE_MODE_ENABLED)
                printf("Starting CRC for faction workers...\n");

              FileCRCPreCacheThread *workerThread = new FileCRCPreCacheThread(
                  techDataPaths, workerTechList, this->processTechCB);
              ...
              if (SystemFlags::VERBOSE_MODE_ENABLED)
                printf("In [%s::%s Line: %d] Spawning CRC thread, "
                       "preCacheWorkerThreadList.size() = %d\n",
                       extractFileFromDirectoryPath(__FILE__).c_str(),
                       __FUNCTION__, __LINE__,
                       (int)preCacheWorkerThreadList.size());
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugError, "In [%s::%s Line: %d] UNKNOWN Error\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__);
            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem)
                    .enabled)
          ...
          sleep(0);
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] Waiting for Spawned CRC threads to "
                   "complete, preCacheWorkerThreadList.size() = %d\n",
                   extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                   __LINE__, (int)preCacheWorkerThreadList.size());

          try {
            bool hasRunningWorkerThread = true;
            for (; hasRunningWorkerThread == true;) {
              hasRunningWorkerThread = false;
              for (unsigned int idx = 0; idx < preCacheWorkerThreadList.size();
                   idx++) {
                FileCRCPreCacheThread *workerThread =
                    preCacheWorkerThreadList[idx];
                ...
                if (workerThread != NULL) {
                  ...
                  if (workerThread->getRunningStatus() == true) {
                    ...
                    static string mutexOwnerId2 = CODE_AT_LINE;
                    MutexSafeWrapper safeMutexPause(mutexPauseForGame,
                                                    mutexOwnerId2);
              ...
              if (getQuitStatus() == false && hasRunningWorkerThread == true) {
                sleep(25);
              } else if (getQuitStatus() == true) {
                for (unsigned int idx = 0;
                     idx < preCacheWorkerThreadList.size(); idx++) {
                  FileCRCPreCacheThread *workerThread =
                      preCacheWorkerThreadList[idx];
                  if (workerThread != NULL &&
                      workerThread->getQuitStatus() == false) {
          ...
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("********************** CRC Controller thread took %.2f "
                   "seconds END **********************\n",
        ...
        try {
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf(
                "\tStarting CRC for faction worker thread for techs: %d...\n",
                (int)workerThreadTechPaths.size());

          for (unsigned int idx = 0; idx < workerThreadTechPaths.size();
               idx++) {
            ...
            // if(SystemFlags::VERBOSE_MODE_ENABLED)
            // printf("--------------------- CRC worker thread START for tech
            // [%s] ---------------------------\n",techName.c_str());
            // if(SystemFlags::VERBOSE_MODE_ENABLED) printf("In [%s::%s Line:
            // %d] caching CRC value for Tech
            // [%s]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,techName.c_str());
            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem)
                    .enabled)
              SystemFlags::OutputDebug(
                  SystemFlags::debugSystem,
                  "In [%s::%s Line: %d] caching CRC value for Tech [%s]\n",
                  extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                  __LINE__, techName.c_str());

            time_t elapsedTime = time(NULL);
            // Clear existing CRC to force a CRC refresh
            // string pathSearchString     = string("/") + techName +
            // string("/*"); const string filterFileExt  = ".xml";
            // clearFolderTreeContentsCheckSum(techDataPaths, pathSearchString,
            // filterFileExt);
            // clearFolderTreeContentsCheckSumList(techDataPaths,
            // pathSearchString, filterFileExt);
            ...
            // if(SystemFlags::VERBOSE_MODE_ENABLED)
            // printf("--------------------- CRC worker thread running for tech
            // [%s] ---------------------------\n",techName.c_str());
            if (getQuitStatus() == false) {
              if (SystemFlags::VERBOSE_MODE_ENABLED)
                printf("\t\tAbout to process CRC for techName [%s]\n",
                       techName.c_str());

              // Do not process CRC's while game in progress
              if (getPauseForGame() == true) {
                if (SystemFlags::VERBOSE_MODE_ENABLED)
                  printf("\t\tGame in progress so waiting to process CRC for "
                         "techName [%s]\n",
                         techName.c_str());
                for (; getPauseForGame() == true && getQuitStatus() == false;) {
                  sleep(25);
                }
                if (getQuitStatus() == true) {
                  break;
              ...
              uint32 techCRC = getFolderTreeContentsCheckSumRecursively(
                  techDataPaths, string("/") + techName + string("/*"), ".xml",
                  NULL, true);

              // if(SystemFlags::VERBOSE_MODE_ENABLED) printf("In [%s::%s Line:
              // %d] cached CRC value for Tech [%s] is [%d] took %.3f
              // seconds.\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,techName.c_str(),techCRC,difftime(time(NULL),elapsedTime));
              if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem)
                      .enabled)
                SystemFlags::OutputDebug(
                    SystemFlags::debugSystem,
                    "In [%s::%s Line: %d] cached CRC value for Tech [%s] is "
                    "[%u] took %.3f seconds.\n",
                    extractFileFromDirectoryPath(__FILE__).c_str(),
                    __FUNCTION__, __LINE__, techName.c_str(), techCRC,
                    difftime(time(NULL), elapsedTime));
              ...
              for (unsigned int idx = 0; idx < techDataPaths.size(); idx++) {
                ...
                findAll(techPath + techName + "/factions/*.", results, false,
                        false);
              ...
              if (SystemFlags::VERBOSE_MODE_ENABLED)
                printf("\t\tStarting CRC for faction worker thread for "
                       "results: %d...\n",
                       (int)results.size());
              if (results.empty() == true) {
                for (unsigned int factionIdx = 0; factionIdx < results.size();
                     ++factionIdx) {
                  string factionName = results[factionIdx];
                  if (SystemFlags::VERBOSE_MODE_ENABLED)
                    printf("\t\t\tAbout to process CRC for factionName [%s]\n",
                           factionName.c_str());

                  // Do not process CRC's while game in progress
                  if (getPauseForGame() == true) {
                    if (SystemFlags::VERBOSE_MODE_ENABLED)
                      printf("\t\t\tGame in progress so waiting to process CRC "
                             "for factionName [%s]\n",
                             factionName.c_str());
                  ...
                  if (SystemFlags::VERBOSE_MODE_ENABLED)
                    printf("\t\t\tStart Processing CRC for factionName [%s]\n",
                           factionName.c_str());

                  uint32 factionCRC = getFolderTreeContentsCheckSumRecursively(
                      techDataPaths,
                      "/" + techName + "/factions/" + factionName + "/*",
                      ".xml", NULL, true);

                  if (SystemFlags::VERBOSE_MODE_ENABLED)
                    printf("\t\t\tDone Processing CRC for factionName [%s] CRC "
                           "[%d]\n",
            ...
            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf("--------------------- CRC worker thread END for tech "
                     "[%s] ---------------------------\n",
                     techName.c_str());
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugSystem,
          "In [%s::%s Line: %d] FILE CRC PreCache thread is exiting...\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__);
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d] FILE CRC PreCache thread is exiting...\n",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__);
  }
  deleteSelfIfRequired();
...
SimpleTaskThread::SimpleTaskThread(
    ...
    bool needTaskSignal, void *userdata, bool wantSetupAndShutdown)
    : BaseThread(), simpleTaskInterface(NULL), overrideShutdownTask(NULL),
  ...
  try {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(
          ...
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, this->getUniqueID().c_str());
...
LogFileThread::LogFileThread()
    : BaseThread(), mutexLogList(new Mutex(CODE_AT_LINE)) {
...
void LogFileThread::execute() {
  ...
  bool mustDeleteSelf = false;
  {
    ...
    try {
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("In [%s::%s Line: %d] Error [%s]\n",
               extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
               __LINE__, ex.what());
    } catch (...) {
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("In [%s::%s Line: %d] UNKNOWN Error\n",
               extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
               __LINE__);
    }

    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d] LogFile thread is starting to exit\n",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__);

    mustDeleteSelf = getDeleteSelfOnExecutionDone();
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d] LogFile thread is exiting, mustDeleteSelf = "
             "%d\n",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__, mustDeleteSelf);
  }
  if (mustDeleteSelf == true) {
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d] LogFile thread is deleting self\n",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__);
    if (isThreadDeleted(ptr_cpy) == false) {
      this->setDeleteAfterExecute(true);
  ...
  if (logCount > 0) {
    ...
    if (logCount > 0) {
      for (unsigned int i = 0; i < logCount; ++i) {
        ...
        SystemFlags::logDebugEntry(entry.type, entry.entry,
                                   entry.entryDateTime);
      }
      ...
      if (logList.size() > 0) {
        if (logList.size() < logCount) {
          char szBuf[8096] = "";
          snprintf(szBuf, 8096, "logList.size() <= logCount [%lld][%lld]",
                   (long long int)logList.size(), (long long int)logCount);

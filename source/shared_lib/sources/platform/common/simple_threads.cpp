...
FileCRCPreCacheThread::FileCRCPreCacheThread() : BaseThread(), mutexPauseForGame(new Mutex(CODE_AT_LINE)) {
    techDataPaths.clear();
...
FileCRCPreCacheThread::FileCRCPreCacheThread(vector<string> techDataPaths, vector<string> workerThreadTechPaths,
...
void FileCRCPreCacheThread::execute() {
    {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "FILE CRC PreCache thread is running threadControllerMode = %d\n", threadControllerMode);
            ...
            if (threadControllerMode == true) {
                ...
                if (techPaths.empty() == false) {
                    ...
                    if (iterFindMegaPack != techPaths.end()) {
                        ...
                        if (SystemFlags::VERBOSE_MODE_ENABLED)
                            printf("In [%s::%s Line: %d] Found megapack techtree and placing it "
                    ...
                    try {
                        ...
                        for (unsigned int workerIdx = 0; workerIdx < (unsigned int)MAX_FileCRCPreCacheThread_WORKER_THREADS; ++workerIdx) {
                            ...
                            for (unsigned int idx = consumedWorkers; idx < endConsumerIndex; idx++) {
                                ...
                                if (SystemFlags::VERBOSE_MODE_ENABLED)
                                    printf("In [%s::%s Line: %d] Spawning CRC thread for Tech [%s] "
                                           "[%u of %lu]\n",
                            ...
                            while (getQuitStatus() == false && difftime(time(NULL), pauseTime) <= PAUSE_SECONDS_BETWEEN_WORKERS) {
                                sleep(25);
                            }
                            if (getQuitStatus() == true) {
                                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                            ...
                            FileCRCPreCacheThread *workerThread = new FileCRCPreCacheThread(techDataPaths, workerTechList, this->processTechCB);
                            ...
                            for (unsigned int idx = 0; idx < preCacheWorkerThreadList.size(); idx++) {
                                ...
                                if (workerThread != NULL) {
                                    if (workerThread->getRunningStatus() == true) {
                                        ...
                                        if (getQuitStatus() == true && workerThread->getQuitStatus() == false) {
                ...
                try {
                    if (SystemFlags::VERBOSE_MODE_ENABLED)
                        printf("\tStarting CRC for faction worker thread for techs: %d...\n", (int)workerThreadTechPaths.size());
                    ...
                    for (unsigned int idx = 0; idx < workerThreadTechPaths.size(); idx++) {
                        ...
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] caching CRC value for Tech [%s]\n",
                                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, techName.c_str());
                        ...
                        if (getQuitStatus() == false) {
                            ...
                            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                                SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                                         ...
                                                         "[%u] took %.3f seconds.\n",
                                                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, techName.c_str(), techCRC,
                                                         difftime(time(NULL), elapsedTime));
                            ...
                            if (SystemFlags::VERBOSE_MODE_ENABLED)
                                printf("\t\tStarting CRC for faction worker thread for results: "
                                       "%d...\n",
                                       (int)results.size());
                            if (results.empty() == true) {
                                for (unsigned int factionIdx = 0; factionIdx < results.size(); ++factionIdx) {
                                    ...
                                    uint32 factionCRC = getFolderTreeContentsCheckSumRecursively(
                                        techDataPaths, "/" + techName + "/factions/" + factionName + "/*", ".xml", NULL, true);
                        ...
                        if (SystemFlags::VERBOSE_MODE_ENABLED)
                            printf("--------------------- CRC worker thread END for tech [%s] "
                                   "---------------------------\n",
                                   techName.c_str());
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] FILE CRC PreCache thread is exiting...\n",
                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
        if (SystemFlags::VERBOSE_MODE_ENABLED)
...
SimpleTaskThread::SimpleTaskThread(SimpleTaskCallbackInterface *simpleTaskInterface, unsigned int executionCount, unsigned int millisecsBetweenExecutions,
                                   bool needTaskSignal, void *userdata, bool wantSetupAndShutdown)
    : BaseThread(), simpleTaskInterface(NULL), overrideShutdownTask(NULL), mutexSimpleTaskInterfaceValid(new Mutex(CODE_AT_LINE)),
      mutexTaskSignaller(new Mutex(CODE_AT_LINE)), mutexLastExecuteTimestamp(new Mutex(CODE_AT_LINE)) {
    ...
    try {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] uniqueID [%s]\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                     __FUNCTION__, __LINE__, this->getUniqueID().c_str());
    ...
    {
        {
            ...
            try {
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] uniqueID [%s] END\n",
                                             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, this->getUniqueID().c_str());
...
bool LogFileThread::checkSaveCurrentLogBufferToDisk() {
    bool ret = false;
    if (difftime(time(NULL), lastSaveToDisk) >= 5.0 || (difftime(time(NULL), lastSaveToDisk) >= 2.0 && LogFileThread::getLogEntryBufferCount() >= 1000000)) {
        lastSaveToDisk = time(NULL);
        ret = true;
    }
    return ret;
...
void LogFileThread::execute() {
    ...
    bool mustDeleteSelf = false;
    {
        ...
        if (SystemFlags::VERBOSE_MODE_ENABLED) printf("In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "LogFile thread is running\n");

        try {
            ExecutingTaskSafeWrapper safeExecutingTaskMutex(this);
            for (; this->getQuitStatus() == false;) {
                while (this->getQuitStatus() == false && checkSaveCurrentLogBufferToDisk() == true) {

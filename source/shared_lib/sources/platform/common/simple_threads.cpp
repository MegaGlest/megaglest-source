      ...
      if (threadControllerMode == true) {
        ...
        if (techPaths.empty() == false) {
          ...
          if (iterFindMegaPack != techPaths.end()) {
            ...
            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf(
                  "In [%s::%s Line: %d] Found megapack techtree and placing it "
          ...
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf(
                "In [%s::%s Line: %d] techsPerWorker = %u, "
          ...
          try {
            ...
            for (unsigned int workerIdx = 0;
              ...
              for (unsigned int idx = consumedWorkers; idx < endConsumerIndex;
                ...
                if (SystemFlags::VERBOSE_MODE_ENABLED)
                  printf(
                      "In [%s::%s Line: %d] Spawning CRC thread for Tech [%s] "
                      "[%u of %lu]\n",
                      extractFileFromDirectoryPath(__FILE__).c_str(),
              ...
              if (SystemFlags::VERBOSE_MODE_ENABLED)
                printf(
                    "In [%s::%s Line: %d] workerIdx = %u, currentWorkerMax = "
              ...
              if (SystemFlags::VERBOSE_MODE_ENABLED)
                printf(
                    "In [%s::%s Line: %d] Spawning CRC thread, "
          ...
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf(
                "In [%s::%s Line: %d] Waiting for Spawned CRC threads to "
          ...
          for (unsigned int idx = 0; idx < workerThreadTechPaths.size();
            ...
            if (getQuitStatus() == false) {
              ...
              if (getPauseForGame() == true) {
                if (SystemFlags::VERBOSE_MODE_ENABLED)
                  printf(
                      "\t\tGame in progress so waiting to process CRC for "
                      "techName [%s]\n",
                      techName.c_str());
              ...
              if (SystemFlags::VERBOSE_MODE_ENABLED)
                printf(
                    "\t\tStarting CRC for faction worker thread for results: "
                    "%d...\n",
                    (int)results.size());
              if (results.empty() == true) {
                for (unsigned int factionIdx = 0; factionIdx < results.size();
                  ...
                  if (getPauseForGame() == true) {
                    if (SystemFlags::VERBOSE_MODE_ENABLED)
                      printf(
                          "\t\t\tGame in progress so waiting to process CRC "
                          "for factionName [%s]\n",
                          factionName.c_str());
                  ...
                  if (SystemFlags::VERBOSE_MODE_ENABLED)
                    printf(
                        "\t\t\tDone Processing CRC for factionName [%s] CRC "
                        "[%d]\n",
            ...
            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf(
                  "--------------------- CRC worker thread END for tech [%s] "
                  "---------------------------\n",
                  techName.c_str());
...
void LogFileThread::execute() {
  ...
  bool mustDeleteSelf = false;
  {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "In [%s::%s Line: %d] LogFile thread is exiting, mustDeleteSelf = "
          "%d\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,

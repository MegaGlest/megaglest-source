...
void createGlFontBitmaps(uint32 &base, const string &type, int size, int width,
  ...
  if (fontInfo == NULL) {
    ...
    SystemFlags::OutputDebug(SystemFlags::debugError,
                             "Font not found [%s] trying to fallback to [%s]\n",
                             type.c_str(), default_font.c_str());
    // printf("Font not found [%s] trying to fallback to
    // [%s]\n",type.c_str(),default_font.c_str());
  ...
  // printf("Font height [%s] = %d\n",type.c_str(),(fontInfo->ascent +
  // fontInfo->descent) / 2);

  metrics.setHeight(static_cast<float>(fontInfo->ascent + fontInfo->descent) /
                    2);
  ...
  /*
          ...
          int pages = fontInfo->max_char_or_byte2 - fontInfo->min_char_or_byte2
     + 1; int byte1, byte2, index; int charIndex = 0; int charWidth, charHeight;
          XChar2b character;
          ...
          for (int i = first; count; i++, count--) {
                  ...
                  if (undefined) {
                    ...
                    if(skipToEnd == false) {
                            metrics.setWidth(charIndex, static_cast<float>
     (fontInfo->per_char[index].width)); charIndex++;
                    }
                  }
                  if(skipToEnd == false) {
  ...
  */
  ...
  // for(unsigned int i = 0; fontInfo->per_char != NULL && i <
  // static_cast<unsigned int> (charCount); ++i) {
  for (unsigned int i = 0;
       fontInfo->per_char != NULL && i < static_cast<unsigned int>(count);
       ++i) {
    if (i < fontInfo->min_char_or_byte2 || i > fontInfo->max_char_or_byte2) {
      ...
      metrics.setWidth(i, static_cast<float>(fontInfo->per_char[p].width));

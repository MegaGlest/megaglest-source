...
static UNSIGNED_INTEGER my_atoui(const char *s) {
...
MINIUPNP_LIBSPEC int UPNP_GetStatusInfo(const char *controlURL,
                                        const char *servicetype, char *status,
                                        unsigned int *uptime,
                                        char *lastconnerror) {
  struct NameValueParserData pdata;
  char *buffer;
  ...
  if (!(buffer = simpleUPnPcommand(-1, controlURL, servicetype, "GetStatusInfo",
                                   0, &bufsize))) {
    return UPNPCOMMAND_HTTP_ERROR;
  }
  ParseNameValue(buffer, bufsize, &pdata);
...
UPNP_AddPortMapping(const char *controlURL, const char *servicetype,
                    ...
                    const char *inClient, const char *desc, const char *proto,
                    const char *remoteHost, const char *leaseDuration) {
...
UPNP_AddAnyPortMapping(const char *controlURL, const char *servicetype,
  ...
  AddPortMappingArgs[7].val = leaseDuration ? leaseDuration : "0";
  if (!(buffer =
            simpleUPnPcommand(-1, controlURL, servicetype, "AddAnyPortMapping",
                              AddPortMappingArgs, &bufsize))) {
    free(AddPortMappingArgs);
    return UPNPCOMMAND_HTTP_ERROR;
  }
...
MINIUPNP_LIBSPEC int UPNP_DeletePortMapping(const char *controlURL,
  ...
  if (!(buffer =
            simpleUPnPcommand(-1, controlURL, servicetype, "DeletePortMapping",
...
MINIUPNP_LIBSPEC int UPNP_GetGenericPortMappingEntry(
    const char *controlURL, const char *servicetype, const char *index,
    char *extPort, char *intClient, char *intPort, char *protocol, char *desc,
...
MINIUPNP_LIBSPEC int UPNP_GetPortMappingNumberOfEntries(
    const char *controlURL, const char *servicetype, unsigned int *numEntries) {
  struct NameValueParserData pdata;
  char *buffer;
  int bufsize;
  char *p;
  ...
  if (!(buffer =
            simpleUPnPcommand(-1, controlURL, servicetype,
                              "GetPortMappingNumberOfEntries", 0, &bufsize))) {
    return UPNPCOMMAND_HTTP_ERROR;
  }
#ifdef DEBUG
  DisplayNameValueList(buffer, bufsize);
...
MINIUPNP_LIBSPEC int UPNP_GetSpecificPortMappingEntry(
    const char *controlURL, const char *servicetype, const char *extPort,
    const char *proto, const char *remoteHost, char *intClient, char *intPort,
    char *desc, char *enabled, char *leaseDuration) {
  struct NameValueParserData pdata;
  struct UPNParg *GetPortMappingArgs;
  char *buffer;
  int bufsize;
 ...
 */
MINIUPNP_LIBSPEC int
UPNP_GetListOfPortMappings(const char *controlURL, const char *servicetype,
                           const char *startPort, const char *endPort,
                           const char *protocol, const char *numberOfPorts,
...
MINIUPNP_LIBSPEC int UPNP_GetFirewallStatus(const char *controlURL,
  ...
  buffer = simpleUPnPcommand(-1, controlURL, servicetype, "GetFirewallStatus",
                             0, &bufsize);
  if (!buffer) {
    return UPNPCOMMAND_HTTP_ERROR;
  }
...
UPNP_GetOutboundPinholeTimeout(const char *controlURL, const char *servicetype,
                               ...
                               const char *intClient, const char *intPort,
                               const char *proto, int *opTimeout) {
...
UPNP_AddPinhole(const char *controlURL, const char *servicetype,
                const char *remoteHost, const char *remotePort,
                const char *intClient, const char *intPort, const char *proto,
                const char *leaseTime, char *uniqueID) {
  ...
  if (!intPort || !intClient || !proto || !remoteHost || !remotePort ||
      !leaseTime)
    return UPNPCOMMAND_INVALID_ARGS;
  ...
  buffer = simpleUPnPcommand(-1, controlURL, servicetype, "AddPinhole",
                             AddPinholeArgs, &bufsize);
  if (!buffer)
    return UPNPCOMMAND_HTTP_ERROR;
  ParseNameValue(buffer, bufsize, &pdata);
  free(buffer);
...
MINIUPNP_LIBSPEC int UPNP_UpdatePinhole(const char *controlURL,
  ...
  buffer = simpleUPnPcommand(-1, controlURL, servicetype, "UpdatePinhole",
                             UpdatePinholeArgs, &bufsize);
  if (!buffer)
    return UPNPCOMMAND_HTTP_ERROR;
  ParseNameValue(buffer, bufsize, &pdata);
  free(buffer);
...
MINIUPNP_LIBSPEC int UPNP_DeletePinhole(const char *controlURL,
  ...
  buffer = simpleUPnPcommand(-1, controlURL, servicetype, "DeletePinhole",
                             DeletePinholeArgs, &bufsize);
  if (!buffer)
    return UPNPCOMMAND_HTTP_ERROR;
  /*DisplayNameValueList(buffer, bufsize);*/
  ParseNameValue(buffer, bufsize, &pdata);
...
MINIUPNP_LIBSPEC int UPNP_CheckPinholeWorking(const char *controlURL,
  ...
  buffer = simpleUPnPcommand(-1, controlURL, servicetype, "CheckPinholeWorking",
...
MINIUPNP_LIBSPEC int UPNP_GetPinholePackets(const char *controlURL,
  ...
  buffer = simpleUPnPcommand(-1, controlURL, servicetype, "GetPinholePackets",
                             GetPinholePacketsArgs, &bufsize);
  if (!buffer)
    return UPNPCOMMAND_HTTP_ERROR;
  ParseNameValue(buffer, bufsize, &pdata);
  free(buffer);

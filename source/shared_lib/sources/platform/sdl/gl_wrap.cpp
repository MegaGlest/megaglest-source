...
void PlatformContextGl::init(int colorBits, int depthBits, int stencilBits,
  ...
  Window::setupGraphicsScreen(depthBits, stencilBits, hardware_acceleration,
                              fullscreen_anti_aliasing);
  ...
  // printf("In [%s::%s %d] PlatformCommon::Private::shouldBeFullscreen =
  // %d\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,PlatformCommon::Private::shouldBeFullscreen);

  // SDL_WINDOW_FULLSCREEN seems very broken when changing resolutions that
  // differ from the desktop resolution For now fullscreen will mean use desktop
  // resolution
  ...
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugSystem,
                               "In [%s::%s Line: %d] about to set resolution: "
                               "%d x %d, colorBits = %d.\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__, resW, resH, colorBits);

    if (Window::getIsFullScreen() && (flags & SDL_WINDOW_FULLSCREEN_DESKTOP) ==
    ...
    window = SDL_CreateWindow(windowTitleText.c_str(), windowX, windowY, resW,
                              resH, flags);
    if (window == 0) {
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("In [%s::%s %d]\n",
      ...
      msg << "Couldn't set video mode " << resW << "x" << resH << " ("
          << colorBits << "bpp " << stencilBits << " stencil " << depthBits
          << " depth-buffer). SDL Error is: " << SDL_GetError();
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugError).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugError,
                                 ...
                                 extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__, msg.str().c_str());
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                 "In [%s::%s Line: %d] error [%s]\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(),
      ...
      if (window == 0) {
        ...
        window = SDL_CreateWindow(windowTitleText.c_str(), windowX, windowY, 0,
                                  0, SDL_WINDOW_FULLSCREEN_DESKTOP | flags);
      }
      if (window == 0) {
        // try to revert to 640x480
        window = SDL_CreateWindow(windowTitleText.c_str(), windowX, windowY,
                                  650, 480, SDL_WINDOW_FULLSCREEN_DESKTOP);
      }
      if (window == 0) {
    ...
    //		windowDisplayID = SDL_GetWindowDisplayIndex( window );
    //		printf("!!! totalDisplays = %d, windowDisplayID =
    ...
    if ((w != resW || h != resH) && (flags & SDL_WINDOW_FULLSCREEN_DESKTOP) ==
                                        SDL_WINDOW_FULLSCREEN_DESKTOP) {
      ...
      //
      //			SDL_GetWindowSize(window, &w, &h);
      //			printf("#2 Change resolution mismatch get (%d x
      //%d) desired (%d x %d)\n",w,resW,h,resH);
    }
    glViewport(0, 0, w, h);

    // There seems to be a bug where if relative mouse mouse is enabled when you
    // create a new window, the window still reports that it has input & mouse
    // focus, but it doesn't send any events for mouse motion or mouse button
    // clicks. You can fix this by toggling relative mouse mode.
...
#ifndef NO_APPIMAGE
    string path = Properties::appendAppImagePath(
    ...
    } else if (mg_icon_file == "" &&
               fileExists("/usr/share/pixmaps/megaglest.png")) {
      mg_icon_file = "/usr/share/pixmaps/megaglest.png";
    } else if (mg_icon_file == "" &&
               fileExists("/usr/share/pixmaps/megaglest.bmp")) {
    ...
    if (mg_icon_file != "") {
      ...
      if (extractExtension(mg_icon_file) == "bmp") {
        ...
        std::pair<SDL_Surface *, unsigned char *> result =
            texture2D->CreateSDLSurface(true);
        icon = result.first;
        delete texture2D;
      ...
      // #if WORDS_BIGENDIAN
      //	SDL_Surface* icon=
      // SDL_CreateRGBSurfaceFrom((void*)logo,32,32,8,128,0xff000000,0x00ff0000,0x0000ff00,0);
      // #else
      //	SDL_Surface* icon=
      // SDL_CreateRGBSurfaceFrom((void*)logo,32,32,32,128,0x000000ff,0x0000ff00,0x00ff0000,0xff000000);
      // #endif

      // printf("In [%s::%s Line: %d] icon =
      ...
      if (icon != NULL) {
        ...
        if (SDL_SetColorKey(icon, SDL_TRUE,
...
const char *getPlatformExtensions(const PlatformContextGl *pcgl) { return ""; }

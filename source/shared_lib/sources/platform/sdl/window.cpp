  ...
  while (SDL_PollEvent(&event)) {
    try {
      ...
      case SDL_TEXTINPUT: {
        ...
        if (SystemFlags::VERBOSE_MODE_ENABLED)
          printf("In [%s::%s Line: %d] Raw SDL key [%d - %c] mod [%d] scancode "
                 "[%d] keyName [%s]\n",
                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
      ...
      case SDL_KEYUP: {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
          SystemFlags::OutputDebug(
              SystemFlags::debugSystem,
              "In [%s::%s Line: %d] KEY_UP, Raw SDL key [%d] mod [%d] scancode "
              "[%d]\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, event.key.keysym.sym, event.key.keysym.mod,
        ...
        //					string keyName =
        // SDL_GetKeyName(event.key.keysym.sym); 					if(
      ...
      case SDL_WINDOWEVENT: {
        ...
        //window focus 					if ((event.active.state
...
void Window::revertMousePos() { SDL_WarpMouseInWindow(sdlWindow, oldX, oldY); }
...
bool isKeyPressed(SDL_Keycode compareKey, SDL_KeyboardEvent input,
  ...
  // input.keysym.sym == SDLK_1 || 				 input.keysym.sym == SDLK_2 ||
  ...
  //SDLK_9)
  //|| 				(input.keysym.sym >= SDLK_KP_0 && input.keysym.sym <=
  //SDLK_KP_9)) {
  //				if(SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
  // SystemFlags::OutputDebug(SystemFlags::debugSystem,"In [%s::%s Line:
  //%d]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__);
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("In [%s::%s Line: %d] compareKey [%d - %s] pressed key [%d - %s] "
           "result = %d\n",
           extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
           __LINE__, compareKey, compareKeyName.c_str(), c,
           pressKeyName.c_str(), result);
...
bool Window::isKeyStateModPressed(int mod) {
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("isKeyStateModPressed mod = %d, keystate.mod = %d, keystate.mod & "
           "mod = %d\n",

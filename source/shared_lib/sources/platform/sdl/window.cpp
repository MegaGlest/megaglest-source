...
static HWND GetSDLWindow() {
  ...
  if (SDL_GetWindowWMInfo(Window::getSDLWindow(), &info) == -1) return NULL;
...
void Window::setSDLWindow(SDL_Window *window) { Window::sdlWindow = window; }
SDL_Window *Window::getSDLWindow() { return Window::sdlWindow; }
  ...
  while (SDL_PollEvent(&event)) {
    try {
      ...
      switch (event.type) {
        ...
        case SDL_MOUSEBUTTONUP: {
          ...
          if (global_window) {
            ...
            global_window->eventMouseUp(event.button.x, event.button.y,
        ...
        case SDL_MOUSEMOTION: {
          ...
          setMouseState(mbLeft, (currentMouseState & SDL_BUTTON_LMASK) ==
                                    SDL_BUTTON_LMASK);
          setMouseState(mbRight, (currentMouseState & SDL_BUTTON_RMASK) ==
                                     SDL_BUTTON_RMASK);
          setMouseState(mbCenter, (currentMouseState & SDL_BUTTON_MMASK) ==
                                      SDL_BUTTON_MMASK);

          if (global_window) {
            global_window->eventMouseMove(event.motion.x, event.motion.y,
                                          &getMouseState());  //&ms);
          }
        ...
        case SDL_TEXTINPUT: {
          ...
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf(
                "In [%s::%s Line: %d] Raw SDL key [%d - %c] mod [%d] scancode "
                "[%d] keyName [%s]\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, event.key.keysym.sym, event.key.keysym.sym,
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem)
            ...
            SystemFlags::OutputDebug(
                ...
                "In [%s::%s Line: %d] Raw SDL key [%d] mod [%d] scancode [%d] "
                "keyName [%s]\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, event.key.keysym.sym, event.key.keysym.mod,
                event.key.keysym.scancode, keyName.c_str());
        ...
        case SDL_KEYDOWN: {
          ...
          //					// Stop keys which would be
          // handled twice ( one time as text input, one time as key down )
          ...
          // printf("In SDL_KEYDOWN key [%d] keyName [%s] mod:
        ...
        case SDL_KEYUP: {
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugSystem,
                "In [%s::%s Line: %d] KEY_UP, Raw SDL key [%d] mod [%d] "
                "scancode [%d]\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, event.key.keysym.sym, event.key.keysym.mod,
                event.key.keysym.scancode);
          // printf("In [%s::%s Line: %d] KEY_UP, Raw SDL key [%d] mod [%d]
          // scancode
        ...
        case SDL_WINDOWEVENT: {
          ...
          //%d] SDL_ACTIVEEVENT event.active.state = %d event.active. =
          ...
          //
          //						bool willShowCursor =
          //(!Window::isActive || (Window::lastShowMouseState == SDL_ENABLE) ||
          // Window::getUseDefaultCursorOnly());
          // showCursor(willShowCursor);
          //					}

          // printf("In SDL_WINDOWEVENT, event.window.event:
          // %d\n",event.window.event);
      ...
      SystemFlags::OutputDebug(SystemFlags::debugError,
                               "In [%s::%s Line: %d] (a1) Couldn't process "
                               "event: [%s] codeLocation = %s\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__, e, codeLocation.c_str());
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      ...
      SystemFlags::OutputDebug(SystemFlags::debugError,
                               "In [%s::%s Line: %d] (a2) Couldn't process "
                               "event: [%s] codeLocation = %s\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
      ...
      SystemFlags::OutputDebug(SystemFlags::debugError,
                               "In [%s::%s Line: %d] (b) Couldn't process "
                               "event: [%s] codeLocation = %s\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__, e.what(),
                               codeLocation.c_str());
      ...
      SystemFlags::OutputDebug(SystemFlags::debugError,
                               "In [%s::%s Line: %d] (c) Couldn't process "
                               "event: [UNKNOWN ERROR] codeLocation = %s\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
...
Vec2i Window::getOldMousePos() { return Vec2i(oldX, oldY); }
...
void Window::minimize() { NOIMPL; }

void Window::setEnabled(bool enabled) { NOIMPL; }

void Window::setVisible(bool visible) { NOIMPL; }

void Window::setStyle(WindowStyle windowStyle) {
  if (windowStyle == wsFullscreen) return;
  // NOIMPL;
}
...
void Window::setupGraphicsScreen(int depthBits, int stencilBits,
  ...
  if (depthBits >= 0) newDepthBits = depthBits;
  if (stencilBits >= 0) newStencilBits = stencilBits;
  ...
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false) {
    ...
    // const float lodBias = std::max(std::min(
    // configHandler->Get("TextureLODBias", 0.0f) , 4.0f), -4.0f);
...
void Window::handleMouseWheel(SDL_Event event) {
  ...
  //	// windows implementation uses 120 for the resolution of a standard
  // mouse
  //	// wheel notch.  However, newer mice have finer resolutions.  I dunno if
  // SDL
  ...
  switch (sdlButton) {
    ...
    default:
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugSystem,
            "In [%s::%s Line: %d] Mouse Button [%d] not handled.\n",
            extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
            __LINE__, sdlButton);
...
void Window::clearAllowedKeys() { mapAllowedKeys.clear(); }
...
bool isKeyPressed(SDL_Keycode compareKey, SDL_KeyboardEvent input,
...
bool isKeyPressed(SDL_Keycode compareKey, SDL_KeyboardEvent input,
  ...
  // printf("START isKeyPressed input = %d compare = %d mod =
  // %d\n",c,compareKey,input.keysym.mod);

  //	if(compareKey == SDLK_QUESTION && (c == SDLK_SLASH && (input.keysym.mod
  //& (KMOD_SHIFT)))) { 		return true;
  //	}
  //	else if(compareKey == SDLK_SLASH && (c == SDLK_SLASH &&
  //(input.keysym.mod & (KMOD_SHIFT)))) { 		return false;
  //	}

  ////		string unicodeKeyName =
  /// SDL_GetKeyName((SDLKey)input.keysym.unicode);
  ////
  ////		if(SystemFlags::VERBOSE_MODE_ENABLED) printf("In [%s::%s Line:
  ///%d] input.keysym.unicode = %d input.keysym.mod = %d input.keysym.sym = %d
  /// unicodeKeyName
  ...
  /// if(SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
  /// SystemFlags::OutputDebug(SystemFlags::debugSystem,"In [%s::%s Line: %d]
  /// input.keysym.unicode = %d input.keysym.mod = %d input.keysym.sym = %d
  /// unicodeKeyName
  ///[%s]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,input.keysym.unicode,input.keysym.mod,input.keysym.sym,unicodeKeyName.c_str());
  //
  ...
  //				 // Need to allow Shift + # key for AZERTY style
  // keyboards 				 input.keysym.sym == SDLK_0 ||
  ...
  //%d]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__);
  //
  //			if( (input.keysym.sym >= SDLK_0 && input.keysym.sym <=
  // SDLK_9) || 				(input.keysym.sym >= SDLK_KP_0
  // && input.keysym.sym <= SDLK_KP_9)) {
  // if(SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
  // SystemFlags::OutputDebug(SystemFlags::debugSystem,"In [%s::%s Line:
  //%d]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__);
  ...
  //& 0xFF));
  //		if(SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
  // SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] #1
  //(c & 0xFF)
  //[%d]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,(c
  //& 0xFF));
  //	}
  ...
  if (result == true) {
    ...
    for (unsigned int i = 0; i < modifiersToCheck.size(); ++i) {
      if ((input.keysym.mod & modifiersToCheck[i])) {
        if (SystemFlags::VERBOSE_MODE_ENABLED)
          printf(
              "In [%s::%s Line: %d] result *WOULD HAVE BEEN TRUE* but is false "
              "due to: input.keysym.mod = %d modifiersToCheck[i] = %d\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, input.keysym.mod, modifiersToCheck[i]);
        result = false;
        break;
      }
    }
  ...
  // printf ("In [%s::%s Line: %d] compareKey [%d - %s] pressed key [%d - %s]
  // result =
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf(
        "In [%s::%s Line: %d] compareKey [%d - %s] pressed key [%d - %s] "
        "result = %d\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        compareKey, compareKeyName.c_str(), c, pressKeyName.c_str(), result);
  // printf ("ISPRESS compareKey [%d - %s] pressed key [%d - %s]
  // input.keysym.sym [%d] input.keysym.unicode [%d] mod = %d result = %d\n",
...
//		if(SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
// SystemFlags::OutputDebug(SystemFlags::debugSystem,"In [%s::%s Line: %d]
// input.keysym.sym = %d input.keysym.mod =
...
//& 0xFF),c);
//		if(SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
// SystemFlags::OutputDebug(SystemFlags::debugSystem,"In [%s::%s Line: %d] #1 (c
//& 0xFF) [%d] c =
//[%lc]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,(c
//& 0xFF),c);
//	}
...
//
//	if(SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
// SystemFlags::OutputDebug(SystemFlags::debugSystem,"In [%s::%s Line: %u] c =
//[%d][%lc]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,c);
//
//	//c = (SDLKey)(c & 0xFF);
...
//
//	//printf ("PRESS pressed key [%d - %s] input.keysym.sym [%d]
// input.keysym.unicode [%d] mod = %d\n",
//	//
...
SDL_Keycode extractKeyPressed(SDL_KeyboardEvent input) {
  ...
  // if(SystemFlags::VERBOSE_MODE_ENABLED) printf ("In [%s::%s Line: %d] #1 (c &
  // 0xFF)
  // [%d]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,(c
  // & 0xFF));
  // if(SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("In [%s::%s Line: %d] pressed key [%d - %s]\n",
           extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
           __LINE__, c, pressKeyName.c_str());
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                             "In [%s::%s Line: %d] pressed key [%d - %s]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, c, pressKeyName.c_str());
...
bool isAllowedInputTextKey(SDL_Keycode key) {
  ...
  bool result =
      (key != SDLK_DELETE && key != SDLK_BACKSPACE && key != SDLK_TAB &&
       key != SDLK_CLEAR && key != SDLK_RETURN && key != SDLK_PAUSE &&
       key != SDLK_UP && key != SDLK_DOWN && key != SDLK_RIGHT &&
       key != SDLK_LEFT && key != SDLK_INSERT && key != SDLK_HOME &&
       key != SDLK_END && key != SDLK_PAGEUP && key != SDLK_PAGEDOWN &&
       key != SDLK_F1 && key != SDLK_F2 && key != SDLK_F3 && key != SDLK_F4 &&
       key != SDLK_F5 && key != SDLK_F6 && key != SDLK_F7 && key != SDLK_F8 &&
       key != SDLK_F9 && key != SDLK_F10 && key != SDLK_F11 &&
       key != SDLK_F12 && key != SDLK_F13 && key != SDLK_F14 &&
       key != SDLK_F15 && key != SDLK_NUMLOCKCLEAR && key != SDLK_CAPSLOCK &&
       key != SDLK_SCROLLLOCK && key != SDLK_RSHIFT && key != SDLK_LSHIFT &&
       key != SDLK_RCTRL && key != SDLK_LCTRL && key != SDLK_RALT &&
       key != SDLK_LALT && key != SDLK_RGUI && key != SDLK_LGUI &&
       key != SDLK_MODE && key != SDLK_HELP && key != SDLK_PRINTSCREEN &&
       key != SDLK_SYSREQ && key != SDLK_MENU && key != SDLK_POWER);

  string inputKeyName = SDL_GetKeyName(key);
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("In [%s::%s Line: %d] pressed key [%d - %s] result = %d\n",
           extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
           __LINE__, key, inputKeyName.c_str(), result);
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugSystem,
        "In [%s::%s Line: %d] pressed key [%d - %s] result = %d\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        key, inputKeyName.c_str(), result);
...
bool Window::isKeyStateModPressed(int mod) {
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf(
        "isKeyStateModPressed mod = %d, keystate.mod = %d, keystate.mod & mod "
        "= %d\n",
...
wchar_t Window::extractLastKeyPressed() { return keystate.sym; }

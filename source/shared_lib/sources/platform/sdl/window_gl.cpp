...
int WindowGl::getScreenWidth() { return PlatformCommon::Private::ScreenWidth; }
...
void WindowGl::swapBuffersGl() { context.swapBuffers(); }
...
void WindowGl::eventToggleFullScreen(bool isFullscreen) {
  ...
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false) {
    ...
    if (getScreenWindow() != NULL) {
      if (getIsFullScreen()) {
        SDL_SetWindowFullscreen(getScreenWindow(),
    ...
    if (isFullscreen) {
      ...
      ChangeVideoMode(
          true, getScreenWidth(), getScreenHeight(), true,
          context.getColorBits(), context.getDepthBits(),
          context.getStencilBits(), context.getHardware_acceleration(),
          context.getFullscreen_anti_aliasing(), context.getGammaValue());

    } else {
      changeVideoModeFullScreen(false);
      ChangeVideoMode(
          true, getDesiredScreenWidth(), getDesiredScreenHeight(), false,
          context.getColorBits(), context.getDepthBits(),
          context.getStencilBits(), context.getHardware_acceleration(),
          context.getFullscreen_anti_aliasing(), context.getGammaValue());
    }
...
bool WindowGl::ChangeVideoMode(bool preserveContext, int resWidth,
                               int resHeight, bool fullscreenWindow,
  ...
  if (preserveContext == true) {
    ...
    if (SDL_GetWindowWMInfo(Window::getSDLWindow(), &info) == -1) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugError).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugError,
                                 "In [%s::%s %d] SDL_GetWMInfo #1 failed\n",
    ...
    if (tempRC == NULL) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugError).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugError,
                                 "In [%s::%s %d] wglCreateContext failed\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__);
      return false;
    }
  ...
  if (preserveContext == true) {
    ...
    if (SDL_GetWindowWMInfo(Window::getSDLWindow(), &info) == -1) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugError).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugError,
                                 "In [%s::%s %d] SDL_GetWMInfo #2 failed\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__);
      return false;
    }
    ...
    if (!wglDeleteContext(tempRC)) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugError).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugError,
                                 "In [%s::%s %d] wglDeleteContext failed\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__);
      return false;
    }

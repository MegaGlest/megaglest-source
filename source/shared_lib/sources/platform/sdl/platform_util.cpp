...
bool showMessage(std::string warning, const string &writepath) {
  ...
  if (dialogCommand) {
    ...
    if (!strcmp(dialogCommand, "kdialog")) {
      ...
      command +=
          "--text-info --center --wrap --width 640 --height 360 --filename=" +
          text_file;
    } else {
...
void message(const string &message, bool isNonGraphicalModeEnabled,
...
#if defined(HAS_GCC_BACKTRACE)
static int getFileAndLine(char *function, void *address, char *file,
                          size_t flen) {
  int line = -1;
...
#if __APPLE_CC__
    snprintf(buf, 8096, "xcrun atos -v -o %s %p",
             PlatformExceptionHandler::application_binary.c_str(), address);
    fprintf(stderr, "> %s\n", buf);
#else

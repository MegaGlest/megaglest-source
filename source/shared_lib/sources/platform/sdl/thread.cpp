  ...
  public:
    ...
    static void cleanupPendingThread(Thread *thread) {
        if (thread != NULL) {
            ...
            if (base_thread != NULL && (base_thread->getRunningStatus() == true || base_thread->getExecutingTask() == true)) {
                if (Thread::getEnableVerboseMode())
...
//          Threads
// =====================================
Thread::Thread() : thread(NULL), mutexthreadAccessor(new Mutex(CODE_AT_LINE)), deleteAfterExecute(false), currentState(thrsNew) {
    addThreadToList();
}
        ...
        if (iterFind == Thread::threadList.end()) {
            if (this != cleanupThread.get()) {
                ...
                snprintf(szBuf, 8095,
                         ...
                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, Thread::threadList.size());
                throw megaglest_runtime_error(szBuf);
            }
        } else {
...
void Thread::shutdownThreads() {
    ...
    if (cleanupThread.get() != 0) {
        ...
        for (; cleanupThread->getRunningStatus() == true && difftime((long int)time(NULL), elapsed) <= 5;) {
            sleep(100);
        }
        // printf("In Thread::shutdownThreads Line: %d\n",__LINE__);
        // sleep(100);
...
void Thread::queueAutoCleanThread() {
    if (this->deleteAfterExecute == true) {
        ...
        if (Thread::getEnableVerboseMode())
            printf("In Thread::shutdownThreads Line: %d thread = %p base_thread = %p "
                   "[%s]\n",
        ...
        for (time_t elapsed = time(NULL);
             cleanupThread->getRunningStatus() == false && cleanupThread->getQuitStatus() == false && difftime(time(NULL), elapsed) < 3;) {
            sleep(0);
        }
        ...
        if (cleanupThread->getQuitStatus() == true) {
            ...
            if (cleanupThread->getRunningStatus() == false) {
                ...
                for (time_t elapsed = time(NULL);
                     cleanupThread->getRunningStatus() == true && cleanupThread->getQuitStatus() == true && difftime(time(NULL), elapsed) < 3;) {
                    sleep(0);
                }
                sleep(0);
...
Mutex::~Mutex() {
    if (Mutex::mutexMutexList.get() && isStaticMutexListMutex == false) {
        ...
        if (iterFind == Mutex::mutexList.end()) {
            char szBuf[8096] = "";
            snprintf(szBuf, 8095, "In [%s::%s Line: %d] iterFind == Mutex::mutexList.end()", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                     __LINE__);
            throw megaglest_runtime_error(szBuf);
        }
    ...
    if (mutex == NULL) {
        ...
        snprintf(szBuf, 8095,
                 ...
                 "deleteownerId [%s]",
                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, refCount, ownerId.c_str(), deleteownerId.c_str());
        throw megaglest_runtime_error(szBuf);
        // printf("%s\n",szBuf);
    } else if (refCount >= 1) {
        char szBuf[8096] = "";
    ...
    if (this->slaveThreadList.empty() == false) {
        ...
        if (debugMasterSlaveThreadController)
            printf("In [%s::%s Line: %d] slaveTriggerCounter = %d slaveResult = %d\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
                   slaveTriggerCounter, slaveResult);

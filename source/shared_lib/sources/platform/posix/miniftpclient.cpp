...
static size_t my_fwrite(void *buffer, size_t size, size_t nmemb, void *stream) {
    ...
    if (out && out->stream == NULL) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "===> #2 FTP Client thread FAILED to open file for writing [%s]\n", fullFilePath.c_str());
...
int file_progress(struct FtpFile *out, double download_total, double download_now, double upload_total, double upload_now) {
    ...
    if (out != NULL && out->ftpServer != NULL && out->ftpServer->getCallBackObject() != NULL) {
...
FTPClientThread::FTPClientThread(int portNumber, string serverUrl, std::pair<string, string> mapsPath, std::pair<string, string> tilesetsPath,
                                 std::pair<string, string> techtreesPath, std::pair<string, string> scenariosPath, FTPClientCallbackInterface *pCBObject,
                                 string fileArchiveExtension, string fileArchiveExtractCommand, string fileArchiveExtractCommandParameters,
                                 int fileArchiveExtractCommandSuccessResult, string tempFilesPath)
    : BaseThread() {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line %d] Using FTP port #: %d, serverUrl [%s]\n", __FILE__, __FUNCTION__, __LINE__,
                                 portNumber, serverUrl.c_str());
}
...
pair<FTP_Client_ResultType, string> FTPClientThread::getMapFromServer(pair<string, string> mapFileName, string ftpUser, string ftpUserPassword) {
    ...
    struct FtpFile ftpfile = {NULL,  destFile.c_str(), /* name to store the file as if succesful */
                              NULL,  NULL,
                              this,  "",
                              false, ftp_cct_Map};
    ...
    if (curl) {
        ...
        if (mapFileName.second != "") {
            ...
            snprintf(szBuf, 8096, "ftp://%s:%s@%s:%d/%s%s", ftpUser.c_str(), ftpUserPassword.c_str(), serverUrl.c_str(), portNumber, mapFileName.first.c_str(),
                     destFileExt.c_str());
        }
...
pair<FTP_Client_ResultType, string> FTPClientThread::getTilesetFromServer(pair<string, string> tileSetName, string tileSetNameSubfolder, string ftpUser,
                                                                          string ftpUserPassword, bool findArchive) {
    ...
    pair<FTP_Client_ResultType, string> result =
        getFileFromServer(ftp_cct_Tileset, tileSetName, remotePath, destFileSaveAs, ftpUser, ftpUserPassword, pWantDirListOnly);
    ...
    if (result.first == ftp_crt_SUCCESS) {
        if (findArchive == true) {
            ...
            string extractCmd = getFullFileArchiveExtractCommand(this->fileArchiveExtractCommand, this->fileArchiveExtractCommandParameters,
                                                                 destRootArchiveFolder, destRootArchiveFolder + tileSetName.first + this->fileArchiveExtension);
            ...
            if (this->pCBObject != NULL) {
                ...
                this->pCBObject->FTPClient_CallbackEvent(tileSetName.first, ftp_cct_ExtractProgress, make_pair(ftp_crt_SUCCESS, "extracting"), NULL);
            }
            ...
            if (getFolderContents == true) {
                ...
                for (unsigned int i = 0; i < wantDirListOnly.size(); ++i) {
                    ...
                    if (fileFromList != "models" && fileFromList != "textures" && fileFromList != "sounds") {
                        result = getFileFromServer(ftp_cct_Tileset, tileSetName, remotePath + fileFromList, destFileSaveAsNewFile + fileFromList, ftpUser,
                                                   ftpUserPassword);
                        if (result.first != ftp_crt_SUCCESS) {
                            break;
                        }
                    } else {
                        result = getTilesetFromServer(tileSetName, fileFromList, ftpUser, ftpUserPassword, findArchive);
                        if (result.first != ftp_crt_SUCCESS) {
                            break;
                        }
                    }
...
pair<FTP_Client_ResultType, string> FTPClientThread::getTechtreeFromServer(pair<string, string> techtreeName, string ftpUser, string ftpUserPassword) {
    ...
    pair<FTP_Client_ResultType, string> result = getFileFromServer(ftp_cct_Techtree, techtreeName, remotePath, destFileSaveAs, ftpUser, ftpUserPassword);

    // Extract the archive
    if (result.first == ftp_crt_SUCCESS) {
        string extractCmd = getFullFileArchiveExtractCommand(this->fileArchiveExtractCommand, this->fileArchiveExtractCommandParameters, destRootArchiveFolder,
        ...
        if (this->pCBObject != NULL) {
            ...
            this->pCBObject->FTPClient_CallbackEvent(techtreeName.first, ftp_cct_ExtractProgress, make_pair(ftp_crt_SUCCESS, "extracting"), NULL);
        }
        safeMutex.ReleaseLock();
...
pair<FTP_Client_ResultType, string> FTPClientThread::getScenarioInternalFromServer(pair<string, string> fileName) {
    ...
    pair<FTP_Client_ResultType, string> result = getFileFromServer(ftp_cct_Scenario, fileName, remotePath, destFileSaveAs, "", "");
    ...
    if (result.first == ftp_crt_SUCCESS) {
        ...
        if (this->pCBObject != NULL) {
            ...
            this->pCBObject->FTPClient_CallbackEvent(fileName.first, ftp_cct_ExtractProgress, make_pair(ftp_crt_SUCCESS, "extracting"), NULL);
        }
        safeMutex.ReleaseLock();
...
pair<FTP_Client_ResultType, string> FTPClientThread::getFileInternalFromServer(pair<string, string> fileName) {
    ...
    pair<FTP_Client_ResultType, string> result = getFileFromServer(ftp_cct_File, fileName, remotePath, destFileSaveAs, "", "");
    ...
    if (result.first == ftp_crt_SUCCESS) {
        ...
        if (("." + ext) == fileArchiveExtension) {
            ...
            string extractCmd = getFullFileArchiveExtractCommand(this->fileArchiveExtractCommand, this->fileArchiveExtractCommandParameters,
                                                                 destRootArchiveFolder, destFileSaveAs);
...
pair<FTP_Client_ResultType, string> FTPClientThread::getFileFromServer(FTP_Client_CallbackType downloadType, pair<string, string> fileNameTitle,
                                                                       string remotePath, string destFileSaveAs, string ftpUser, string ftpUserPassword,
                                                                       vector<string> *wantDirListOnly) {
    pair<FTP_Client_ResultType, string> result = make_pair(ftp_crt_FAIL, "");
    if (wantDirListOnly) {
    ...
    struct FtpFile ftpfile = {fileNameTitle.first.c_str(),
    ...
    if (curl) {
        ...
        if (res != CURLE_OK) {
            ...
            printf("curl FAILED with: %d [%s] attempting to remove folder contents [%s] "
                   "szBuf [%s] ftpfile.isValidXfer = %d, pathCreated = %d\n",
...
void FTPClientThread::ShellCommandOutput_CallbackEvent(string cmd, char *output, void *userdata) {
    if (this->pCBObject != NULL) {
        ...
        this->pCBObject->FTPClient_CallbackEvent(itemName, ftp_cct_ExtractProgress, make_pair(ftp_crt_SUCCESS, "extracting"), output);
    }
}

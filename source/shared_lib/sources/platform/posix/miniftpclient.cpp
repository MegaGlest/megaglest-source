...
static size_t my_fwrite(void *buffer, size_t size, size_t nmemb, void *stream) {
  ...
  if (result != nmemb) {
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "===> FTP Client thread FAILED to write data chunk to file [%s] "
...
pair<FTP_Client_ResultType, string> FTPClientThread::getMapFromServer(
    pair<string, string> mapFileName, string ftpUser, string ftpUserPassword) {
...
pair<FTP_Client_ResultType, string> FTPClientThread::getTilesetFromServer(
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf(
        "FTPClientThread::getTilesetFromServer [%s] remotePath [%s] "
        "destFileSaveAs [%s] getFolderContents = %d findArchive = %d\n",
        tileSetName.first.c_str(), remotePath.c_str(), destFileSaveAs.c_str(),
        getFolderContents, findArchive);
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf(
        ...
        "findArchive = %d\n",
        tileSetName.first.c_str(), remotePath.c_str(), destFileSaveAs.c_str(),
        getFolderContents, result.first, result.second.c_str(), findArchive);
...
pair<FTP_Client_ResultType, string> FTPClientThread::getTechtreeFromServer(
    pair<string, string> techtreeName, string ftpUser, string ftpUserPassword) {
...
pair<FTP_Client_ResultType, string> FTPClientThread::getFileInternalFromServer(
...
pair<FTP_Client_ResultType, string> FTPClientThread::getFileFromServer(
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf(
        "===> FTP Client thread about to try to RETR into [%s] wantDirList = "
        "%d\n",
  ...
  if (curl) {
    ...
    if (res != CURLE_OK) {
      ...
      // we failed
      printf(
          "curl FAILED with: %d [%s] attempting to remove folder contents [%s] "

...
void event_channel(irc_session_t *session, const char *event,
  ...
  // dcc_file_recv_callback, &dccid); 		if(SystemFlags::VERBOSE_MODE_ENABLED
  // || IRCThread::debugEnabled) printf ("DCC send ID: %d\n", dccid);
  //	}
  //
...
void event_numeric(irc_session_t *session, unsigned int event,
  ...
  case LIBIRC_RFC_ERR_NICKCOLLISION:
    ...
    // if(ctx != NULL) {
    //			{
    //			//IRC: Event "433", origin: "leguin.freenode.net", params:
    //3
    //[*|softcoder|Nickname is already in use.] 			printf("In [%s::%s]
    //Line: %d\n",__FILE__,__FUNCTION__,__LINE__);
    //			//if(strstr (params[1], "Nickname is already in use") ==
    //params[1]) { 				IRCThread *ctx = (IRCThread
      ...
      if (count >= 4) {
        for (unsigned int i = 3; i < count && params[i]; ++i) {
          ...
          for (unsigned int j = 0; j < tokens.size(); ++j) {
            ...
            if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
              printf("===> IRC: LIBIRC_RFC_RPL_NAMREPLY user joined channel "
...
std::vector<string> IRCThread::GetIRCConnectedNickList(string target,
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
    printf("===> IRC: Line: %d\n", __LINE__);
  MutexSafeWrapper safeMutexSession(&mutexIRCSession, string(__FILE__) + "_" +
                                                          intToStr(__LINE__));
  bool validSession = (ircSession != NULL);

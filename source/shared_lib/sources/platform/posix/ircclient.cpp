...
std::vector<string> IRCThread::GetIRCConnectedNickList(string target, bool waitForCompletion) {
    ...
    if (validSession == true && hasJoinedChannel == true) {
        ...
        if (waitForCompletion == true) {
            ...
            for (time_t tElapsed = time(NULL); getEventDataDone() == false && this->getQuitStatus() == false && difftime(time(NULL), tElapsed) <= 5;) {
                sleep(50);
            }
...
void IRCThread::execute() {
    {
        ...
        try {
            ...
            safeMutex.Lock();
            if (irc_connect(ircSession, argv[0].c_str(), IRC_SERVER_PORT, 0, this->nick.c_str(), this->username.c_str(),
            ...
            for (int iAttempts = 1; this->getQuitStatus() == false && iAttempts <= 7; ++iAttempts) {
...
void normalizeNick(char *nick) {
    ...
    if (nick != NULL && strlen(nick) > 0) {
        ...
        for (unsigned int i = 0; i < strlen(nick); ++i) {
            if (nick[i] == '-' || nick[i] == '[' || nick[i] == ']' || nick[i] == '_' || nick[i] == '\\' || nick[i] == '`' || nick[i] == '^' || nick[i] == '{' ||
                nick[i] == '}' || (nick[i] >= '0' && nick[i] <= '9') || (nick[i] >= 'a' && nick[i] <= 'z') || (nick[i] >= 'A' && nick[i] <= 'Z')) {

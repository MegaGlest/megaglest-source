...
void addlog(const char *fmt, ...) {
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
    printf("===> IRC: %s\n", buf);
...
void get_nickname(const char *sourceNick, char *destNick,
                  size_t maxDestBufferSize) {
...
void event_notice(irc_session_t *session, const char *event, const char *origin,
                  const char **params, unsigned int count) {
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
    printf("NOTICE from '%s': %s", origin, params[1]);
  ...
  if (strstr(params[1], "This nick is not registered") == params[1]) {
    ...
    //				irc_cmd_msg(session, "nickserv",
    // ctx->getExecute_cmd_onconnect().c_str());
    //			}
    //		}
  ...
  if (ctx != NULL) {
    ...
    if (ctx->getHasJoinedChannel() == false) {
      ...
      for (unsigned int i = 0; i < nickList.size(); ++i) {
        if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
          printf("===> IRC: looking for match [%s] realNick [%s]\n",
...
void event_connect(irc_session_t *session, const char *event,
  ...
  // IRC: Event "433", origin: "leguin.freenode.net", params: 3
  // [*|softcoder|Nickname is already in use.]
...
void event_privmsg(irc_session_t *session, const char *event,
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
    printf("'%s' said me (%s): %s\n", origin ? origin : "someone", params[0],
           params[1]);
}
...
void event_channel(irc_session_t *session, const char *event,
  ...
  // [*|softcoder|Nickname is already in use.]
  if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
    printf("In [%s::%s] Line: %d count = %u origin = %s\n", __FILE__,
           __FUNCTION__, __LINE__, count, (origin ? origin : "null"));
  ...
  get_nickname(origin, realNick, 127);
  if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
    printf("===> IRC: event signalled realNick [%s] origin [%s]\n", realNick,
           origin);
  ...
  //		irc_cmd_msg (session, params[0], "quit, help, dcc chat, dcc
  // send, ctcp");
  //	}
  //
  ...
  //		irc_dcc_t dccid;
  //		irc_dcc_sendfile (session, 0, realNick, "irctest.c",
  // dcc_file_recv_callback, &dccid);
  // if(SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled) printf
  // ("DCC send ID: %d\n", dccid);
  //	}
  //
...
void event_leave(irc_session_t *session, const char *event, const char *origin,
  ...
  if (origin) {
    ...
    get_nickname(origin, realNick, 127);
    if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
      printf("===> IRC: user left channel realNick [%s] origin [%s]\n",
             realNick, origin);
...
void event_numeric(irc_session_t *session, unsigned int event,
  ...
  switch (event) {
    ...
    case LIBIRC_RFC_ERR_NICKCOLLISION:
      ...
      // if(ctx != NULL) {
      //			{
      //			//IRC: Event "433", origin:
      //"leguin.freenode.net", params: 3 [*|softcoder|Nickname is already in
      // use.] 			printf("In [%s::%s] Line:
      // %d\n",__FILE__,__FUNCTION__,__LINE__);
      //			//if(strstr (params[1], "Nickname is already in
      // use") == params[1]) { 				IRCThread *ctx =
      // (IRCThread
      //*)irc_get_ctx(session); 				if(ctx != NULL)
      //{ if(ctx->getExecute_cmd_onconnect()
      //!= "") { irc_cmd_msg(session, "nickserv",
      // ctx->getExecute_cmd_onconnect().c_str());
      //					}
    ...
    case LIBIRC_RFC_ERR_NOTREGISTERED:
      ...
      // if(ctx != NULL) {
      //			{
      //===> IRC: Event "451", origin: "leguin.freenode.net", params: 2 [*|You
      // have not registered]
      //			printf("In [%s::%s] Line:
      //%d\n",__FILE__,__FUNCTION__,__LINE__);
      //			//if(strstr (params[1], "Nickname is already in
      ...
      //!= "") {
      //						//irc_cmd_msg(session,
      //"nickserv",  ctx->getExecute_cmd_onconnect().c_str());
    ...
    case LIBIRC_RFC_RPL_NAMREPLY: {
      if (event == LIBIRC_RFC_RPL_NAMREPLY) {
        if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
          printf("===> IRC: LIBIRC_RFC_RPL_NAMREPLY count = %u\n", count);
        ...
        if (count >= 4) {
          for (unsigned int i = 3; i < count && params[i]; ++i) {
            ...
            for (unsigned int j = 0; j < tokens.size(); ++j) {
              ...
              if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
                printf(
                    "===> IRC: LIBIRC_RFC_RPL_NAMREPLY user joined channel "
                    "realNick [%s] tokens[j] [%s]\n",
...
bool IRCThread::getEventDataDone() {
  MutexSafeWrapper safeMutex(&mutexEventDataDone,
                             string(__FILE__) + "_" + intToStr(__LINE__));
...
IRCThread::IRCThread(const std::vector<string> &argv,
...
void IRCThread::disconnect() {
  ...
  MutexSafeWrapper safeMutex(&mutexIRCSession,
                             string(__FILE__) + "_" + intToStr(__LINE__));
  ...
  if (validSession == true) {
    ...
    MutexSafeWrapper safeMutex1(&mutexIRCSession,
                                string(__FILE__) + "_" + intToStr(__LINE__));
    if (ircSession != NULL) {
  ...
  if (validSession == true && hasJoinedChannel == true) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          ...
          "In [%s::%s Line: %d] sending IRC command to [%s] cmd [%s]\n",
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "In [%s::%s Line: %d] sending IRC command to "
                               "[%s] cmd [%s] ret = %d\n",
...
std::vector<string> IRCThread::GetIRCConnectedNickList(string target,
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
    printf("===> IRC: Line: %d\n", __LINE__);
  MutexSafeWrapper safeMutexSession(
      &mutexIRCSession, string(__FILE__) + "_" + intToStr(__LINE__));
  bool validSession = (ircSession != NULL);
  ...
  if (validSession == true && hasJoinedChannel == true) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] sending IRC nick list command to [%s]\n",
          __FILE__, __FUNCTION__, __LINE__, target.c_str());
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "In [%s::%s Line: %d] sending IRC nick list "
                               "command to [%s] ret = %d\n",
                               __FILE__, __FUNCTION__, __LINE__, target.c_str(),
                               ret);
    ...
    if (waitForCompletion == true) {
      ...
      for (time_t tElapsed = time(NULL); getEventDataDone() == false &&
...
bool IRCThread::isConnected(bool mutexLockRequired) {
  ...
  if (this->getQuitStatus() == false) {
    ...
    bool validSession = (lockStatus == SDL_MUTEX_TIMEDOUT ||
                         (lockStatus == 0 && ircSession != NULL));
    ...
    if (validSession == true) {
#if !defined(DISABLE_IRCCLIENT)
      MutexSafeWrapper safeMutex1(NULL,
                                  string(__FILE__) + "_" + intToStr(__LINE__));
      if (ircSession != NULL) {
        lockStatus = 0;
        if (mutexLockRequired == true) {
        ...
        ret = (lockStatus == SDL_MUTEX_TIMEDOUT ||
               (lockStatus == 0 && irc_is_connected(ircSession) != 0));
      }
      if (mutexLockRequired == true && lockStatus == 0) {
        safeMutex1.ReleaseLock();
      }
...
void IRCThread::execute() {
  {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "In [%s::%s Line: %d] argv.size() = %d\n",
                               __FILE__, __FUNCTION__, __LINE__, argv.size());
    if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
      printf("===> IRC: IRCThread::execute Line: %d\n", __LINE__);
    ...
    try {
      ...
      if (argv.size() != 5) {
        if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
          printf(
              "===> IRC Usage: <server> <nick> <channel> : got params [%ld]\n",
              (long int)argv.size());
        return;
      }
      ...
      //            if(argv.size() >= 5) {
      //            	this->execute_cmd_onconnect = argv[4]; // /msg NickServ
      //            identify <password>.
      //            }
      ...
      safeMutex.Lock();
      if (irc_connect(ircSession, argv[0].c_str(), IRC_SERVER_PORT, 0,
                      this->nick.c_str(), this->username.c_str(),
                      ("MegaGlest " + glestVersionString).c_str())) {
        safeMutex.ReleaseLock();
        if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
      ...
      for (int iAttempts = 1; this->getQuitStatus() == false && iAttempts <= 7;
           ++iAttempts) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "In [%s::%s Line: %d] IRC thread is exiting\n",
                               __FILE__, __FUNCTION__, __LINE__);
    if (SystemFlags::VERBOSE_MODE_ENABLED || IRCThread::debugEnabled)
      printf("===> IRC: Line: %d\n", __LINE__);
    // printf("In ~IRCThread Line: %d [%p]\n",__LINE__,this);
    ...
    // printf("In ~IRCThread Line: %d [%p]\n",__LINE__,this);
    //  Delete ourself when the thread is done (no other actions can happen
    //  after this such as the mutex which modifies the running status of this
    //  method
    MutexSafeWrapper safeMutex(&mutexIRCCB,
                               string(__FILE__) + "_" + intToStr(__LINE__));
...
IRCThread::~IRCThread() {
  ...
  if (IRCThread::globalCacheContainerName != NULL) {
    ...
    IRCThread *&ircClient = CacheManager::getCachedItem<IRCThread *>(
        IRCThread::globalCacheContainerName);
    ircClient = NULL;
  }
...
void normalizeNick(char *nick) {
  ...
  //	<target>     ::= <to> [ "," <target> ]
  //	   <to>         ::= <channel> | <user> '@' <servername> | <nick> |
  //<mask> 	   <channel>    ::= ('#' | '&') <chstring> 	   <servername>
  //::= <host> 	   <host>       ::= see RFC 952 [DNS:4] for details on allowed
  // hostnames 	   <nick>       ::= <letter> { <letter> | <number> | <special> }
  //	   <mask>       ::= ('#' | '$') <chstring>
...
void IRCThread::connectToHost() {
  ...
  if (connectRequired == false) {
    ...
    MutexSafeWrapper safeMutex1(&mutexIRCSession,
                                string(__FILE__) + "_" + intToStr(__LINE__));
    if (irc_connect(ircSession, argv[0].c_str(), IRC_SERVER_PORT, 0,
                    this->nick.c_str(), this->username.c_str(), "megaglest")) {
      safeMutex1.ReleaseLock();

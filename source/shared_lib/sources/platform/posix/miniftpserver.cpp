...
ip_t FindExternalFTPServerIp(ip_t clientIp) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        ...
        "===> FTP Server thread clientIp = %u, result = %u\n", clientIp,
        result);
...
int isValidClientType(ip_t clientIp) {
  ...
  if (FTPServerThread::getFtpValidationIntf() != NULL) {
    result =
        FTPServerThread::getFtpValidationIntf()->isValidClientType(clientIp);
  }
  return result;
}

int isClientAllowedToGetFile(ip_t clientIp, const char *username,
                             const char *filename) {
  int result = 1;

  // printf("In [%s::%s] Line: %d username [%s] file
  // [%s]\n",__FILE__,__FUNCTION__,__LINE__,username,filename);
  if (FTPServerThread::getFtpValidationIntf() != NULL) {
    result = FTPServerThread::getFtpValidationIntf()->isClientAllowedToGetFile(
        clientIp, username, filename);
    // printf("In [%s::%s] Line: %d username [%s] file [%s] result =
...
FTPServerThread::FTPServerThread(
    ...
    std::pair<string, string> techtreesPath, bool internetEnabledFlag,
    bool allowInternetTilesetFileTransfers,
    bool allowInternetTechtreeFileTransfers, int portNumber, int maxPlayers,
  ...
  ftpInit(&FindExternalFTPServerIp, &UPNP_Tools::AddUPNPPortForward,
          &UPNP_Tools::RemoveUPNPPortForward, &isValidClientType,
          &isClientAllowedToGetFile);
...
FTPServerThread::~FTPServerThread() {
  ...
  // printf("In [%s::%s] Line: %d ServerSocket::getFTPServerPort() =
...
void FTPServerThread::addClientToServerIPAddress(uint32 clientIp,
                                                 uint32 ServerIp) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "===> FTP Server thread clientIp = %u, ServerIp = %u\n", clientIp,
        ServerIp);
}
...
void FTPServerThread::setInternetEnabled(bool value, bool forceChange) {
  if (forceChange == true || this->internetEnabled != value) {
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("===> FTP Server thread, changing InternetEnabled = %d\n", value);
      ...
      if (this->allowInternetTilesetFileTransfers == true) {
        if (tilesetsPath.first != "") {
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] tilesetsPath #1 [%s]\n", __FILE__,
                __FUNCTION__, __LINE__, tilesetsPath.first.c_str());
          ftpCreateAccount(FTP_TILESETS_USERNAME, FTP_COMMON_PASSWORD,
                           tilesetsPath.first.c_str(),
                           FTP_ACC_RD | FTP_ACC_LS | FTP_ACC_DIR);
        }
        if (tilesetsPath.second != "") {
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] tilesetsPath #2 [%s]\n", __FILE__,
                __FUNCTION__, __LINE__, tilesetsPath.second.c_str());
          ftpCreateAccount(FTP_TILESETS_CUSTOM_USERNAME, FTP_COMMON_PASSWORD,
                           tilesetsPath.second.c_str(),
                           FTP_ACC_RD | FTP_ACC_LS | FTP_ACC_DIR);
        }
      ...
      if (this->allowInternetTechtreeFileTransfers == true) {
        ...
        if (techtreesPath.first != "") {
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] techtreesPath #1 [%s]\n", __FILE__,
                __FUNCTION__, __LINE__, techtreesPath.first.c_str());
          ftpCreateAccount(FTP_TECHTREES_USERNAME, FTP_COMMON_PASSWORD,
                           techtreesPath.first.c_str(),
                           FTP_ACC_RD | FTP_ACC_LS | FTP_ACC_DIR);
        }
        if (techtreesPath.second != "") {
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] techtreesPath #2 [%s]\n", __FILE__,
                __FUNCTION__, __LINE__, techtreesPath.second.c_str());
          ftpCreateAccount(FTP_TECHTREES_CUSTOM_USERNAME, FTP_COMMON_PASSWORD,
                           techtreesPath.second.c_str(),
                           FTP_ACC_RD | FTP_ACC_LS | FTP_ACC_DIR);
        }

        if (SystemFlags::VERBOSE_MODE_ENABLED)
          printf("===> FTP Server thread, techtrees users created\n");
      } else {
        ftpDeleteAccount(FTP_TECHTREES_USERNAME);
        ftpDeleteAccount(FTP_TECHTREES_CUSTOM_USERNAME);

        if (SystemFlags::VERBOSE_MODE_ENABLED)
          printf("===> FTP Server thread, techtrees users deleted\n");
      }
    } else {
      if (tilesetsPath.first != "") {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
...
void FTPServerThread::execute() {
  {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("===> FTP Server thread is running\n");
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "FTP Server thread is running\n");

    try {
      // ftpCreateAccount("anonymous", "", mapsPath.first.c_str(), FTP_ACC_RD |
      // FTP_ACC_LS | FTP_ACC_DIR);
      ...
      if (mapsPath.first != "") {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                .enabled)
          SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                   "In [%s::%s Line: %d] mapsPath #1 [%s]\n",
                                   __FILE__, __FUNCTION__, __LINE__,
                                   mapsPath.first.c_str());
        ftpCreateAccount(FTP_MAPS_USERNAME, FTP_COMMON_PASSWORD,
                         mapsPath.first.c_str(),
                         FTP_ACC_RD | FTP_ACC_LS | FTP_ACC_DIR);
      }
      if (mapsPath.second != "") {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                .enabled)
          SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                   "In [%s::%s Line: %d] mapsPath #2 [%s]\n",
                                   __FILE__, __FUNCTION__, __LINE__,
                                   mapsPath.second.c_str());
        ftpCreateAccount(FTP_MAPS_CUSTOM_USERNAME, FTP_COMMON_PASSWORD,
                         mapsPath.second.c_str(),
                         FTP_ACC_RD | FTP_ACC_LS | FTP_ACC_DIR);
      }
      ...
      if (tempFilesPath != "") {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                .enabled)
          SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                   "In [%s::%s Line: %d] tempFilesPath [%s]\n",
                                   __FILE__, __FUNCTION__, __LINE__,
                                   tempFilesPath.c_str());
        ftpCreateAccount(FTP_TEMPFILES_USERNAME, FTP_COMMON_PASSWORD,
                         tempFilesPath.c_str(),
                         FTP_ACC_RD | FTP_ACC_LS | FTP_ACC_DIR);
      }

      /*
                  ftpCreateAccount("anonymous", "", "./", FTP_ACC_RD |
         FTP_ACC_LS | FTP_ACC_DIR); ftpCreateAccount("nothing", "", "./", 0);
                  ftpCreateAccount("reader", "", "./", FTP_ACC_RD);
                  ftpCreateAccount("writer", "", "./", FTP_ACC_WR);
                  ftpCreateAccount("lister", "", "./", FTP_ACC_LS);
                  ftpCreateAccount("admin", "xxx", "./", FTP_ACC_RD | FTP_ACC_WR
         | FTP_ACC_LS | FTP_ACC_DIR);
      */
      if (getQuitStatus() == true) {
        return;
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] FTP Server thread is exiting\n", __FILE__,
          __FUNCTION__, __LINE__);
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d] FTP Server thread is exiting\n", __FILE__,
             __FUNCTION__, __LINE__);
  }
}

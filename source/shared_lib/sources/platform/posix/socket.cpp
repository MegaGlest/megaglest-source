...
} gaErrorList[] = {ErrorEntry(0, "No error"),
            ...
            if (ifaAddr > 0) {
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                             ...
                                             __FILE__, __FUNCTION__, __LINE__, ifaAddrStr, maskAddrStr, dstAddrStr, ipAddress.c_str(),
...
void Socket::getLocalIPAddressListForPlatform(std::vector<std::string> &ipList) {
    ...
    for (unsigned int intfIdx = 0; intfIdx < Socket::intfTypes.size(); intfIdx++) {
        ...
        for (int idx = 0; idx < 10; ++idx) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] Trying NIC named [%s]\n", __FILE__, __FUNCTION__, __LINE__, szBuf);
            ...
            if (result_ifaddrr >= 0) {
                ...
                if (pSockAddr != NULL) {
                    ...
                    if ((ifrA.ifr_flags & IFF_UP) == IFF_UP && (ifrA.ifr_flags & IFF_RUNNING) == IFF_RUNNING) {
                        if (strlen(myhostaddr) > 0 && strncmp(myhostaddr, "127.", 4) != 0 && strncmp(myhostaddr, "0.", 2) != 0) {
...
Socket::~Socket() {
    ...
    for (time_t elapsed = time(NULL);
         (dataSynchAccessorRead->getRefCount() > 0 || dataSynchAccessorWrite->getRefCount() > 0) && difftime((long int)time(NULL), elapsed) <= 2;) {
...
bool Socket::hasDataToRead(std::map<PLATFORM_SOCKET, bool> &socketTriggeredList) {
    ...
    if (socketTriggeredList.empty() == false) {
        ...
        for (std::map<PLATFORM_SOCKET, bool>::iterator itermap = socketTriggeredList.begin(); itermap != socketTriggeredList.end(); ++itermap) {
        ...
        if (retval < 0) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                         ...
                                         __FILE__, __FUNCTION__, __LINE__, retval, getLastSocketErrorFormattedText().c_str());
            printf("In [%s::%s] Line: %d, ERROR SELECTING SOCKET DATA retval = %d error "
                   "= %s\n",
                   __FILE__, __FUNCTION__, __LINE__, retval, getLastSocketErrorFormattedText().c_str());
        } else if (retval) {
            if (FD_ISSET(socket, &rfds)) {
            ...
            if (err < 0 && lastSocketError != PLATFORM_SOCKET_TRY_AGAIN) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] ERROR PEEKING SOCKET DATA, err = %d %s\n", __FILE__, __FUNCTION__,
                                             __LINE__, err, getLastSocketErrorFormattedText().c_str());
                break;
            } else if (err == 0) {
                if (isConnected() == false) {
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] ERROR PEEKING SOCKET DATA, err = %d %s\n", __FILE__,
                                                 __FUNCTION__, __LINE__, err, getLastSocketErrorFormattedText().c_str());
                    break;
                }
            }
...
int Socket::send(const void *data, int dataSize) {
    ...
    if (bytesSent < 0 && lastSocketError != PLATFORM_SOCKET_TRY_AGAIN) {
        ...
        while ((bytesSent < 0 && lastSocketError == PLATFORM_SOCKET_TRY_AGAIN) && (difftime((long int)time(NULL), tStartTimer) <= MAX_SEND_WAIT_SECONDS)) {
            attemptCount++;
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    ...
    if (isConnected() == true && bytesSent > 0 && bytesSent < dataSize) {
        ...
        time_t tStartTimer = time(NULL);
        while (((bytesSent > 0 && totalBytesSent < dataSize) || (bytesSent < 0 && lastSocketError == PLATFORM_SOCKET_TRY_AGAIN)) &&
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] attemptCount = %d, totalBytesSent = %d\n", __FILE__, __FUNCTION__,
                                         __LINE__, attemptCount, totalBytesSent);
...
int Socket::receive(void *data, int dataSize, bool tryReceiveUntilDataSizeMet) {
    ...
    if (bytesReceived < 0 && lastSocketError != PLATFORM_SOCKET_TRY_AGAIN) {
        ...
        time_t tStartTimer = time(NULL);
        while ((bytesReceived < 0 && lastSocketError == PLATFORM_SOCKET_TRY_AGAIN) && (difftime((long int)time(NULL), tStartTimer) <= MAX_RECV_WAIT_SECONDS)) {
            if (isConnected() == false) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugNetwork, "[%s::%s Line: %d] Socket is NOT connected!\n", __FILE__, __FUNCTION__, __LINE__);
    ...
    if (bytesReceived <= 0) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] bytesReceived = %d\n", __FILE__, __FUNCTION__, __LINE__, bytesReceived);
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                     ...
                                     __FILE__, __FUNCTION__, __LINE__, bytesReceived, getLastSocketErrorFormattedText(&iErr).c_str(), dataSize,
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] WARNING, additionalBytes = %d\n", __FILE__, __FUNCTION__, __LINE__,
                                     additionalBytes);
        if (SystemFlags::VERBOSE_MODE_ENABLED)
        ...
        if (additionalBytes > 0) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] additionalBytes == %d\n", __FILE__, __FUNCTION__, __LINE__,
                                         additionalBytes);
            if (SystemFlags::VERBOSE_MODE_ENABLED) printf("\nIn [%s::%s Line: %d] additionalBytes == %d\n", __FILE__, __FUNCTION__, __LINE__, additionalBytes);
            if (SystemFlags::getSystemSettingType(SystemFlags::debugError).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugError, "In [%s::%s Line: %d] additionalBytes == %d\n", __FILE__, __FUNCTION__, __LINE__,
                                         additionalBytes);
...
int Socket::peek(void *data, int dataSize, bool mustGetData, int *pLastSocketError) {
    ...
    if (isSocketValid() == true) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            if (chrono.getMillis() > 1)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] action running for msecs: %lld\n", __FILE__, __FUNCTION__, __LINE__,
                                         (long long int)chrono.getMillis());
    } else {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] SOCKET appears to be invalid [%d]\n", __FILE__, __FUNCTION__, __LINE__,
                                     sock);
    }
    ...
    if (err < 0 && lastSocketError != PLATFORM_SOCKET_TRY_AGAIN) {
        ...
        time_t tStartTimer = time(NULL);
        while ((err < 0 && lastSocketError == PLATFORM_SOCKET_TRY_AGAIN) && isSocketValid() == true &&
      ...
      */
            if (Socket::isReadable(true) == true || chronoElapsed.getMillis() % 100 == 0) {
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                    if (chrono.getMillis() > 1)
                        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] action running for msecs: %lld\n", __FILE__, __FUNCTION__,
                                                 __LINE__, (long long int)chrono.getMillis());
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                             "In [%s::%s Line: %d] #2 EAGAIN during "
    ...
    if (err < 0 || (err == 0 && dataSize != 0) ||
        ...
        if (mustGetData == true || lastSocketError != PLATFORM_SOCKET_TRY_AGAIN) {
            printf("** #2 Socket peek error for sock = %d err = %d lastSocketError = %d "
                   "mustGetData = %d\n",
            ...
            if (err == 0) {
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugNetwork, "** LAST CHANCE for disconnection check for sock = %d\n", sock);
                ...
                if (second_err == 0 || second_err < 0) {
                    ...
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "** Disconnecting sock = %d\n", sock);
    ...
    if (i < 0) {
        ...
        if (difftime(time(NULL), lastSocketError) > 1) {
            ...
            SystemFlags::OutputDebug(SystemFlags::debugError, "SOCKET WRITE TIMEOUT In [%s::%s Line: %d] i = %d sock = %d [%s]\n", __FILE__, __FUNCTION__,
                                     __LINE__, i, sock, ipAddress.c_str());
        }
    } else {
        result = true;
    }
...
void ClientSocket::connect(const Ip &ip, int port) {
    ...
    if (err < 0) {
        ...
        if (lastSocketError == PLATFORM_SOCKET_INPROGRESS || lastSocketError == PLATFORM_SOCKET_TRY_AGAIN) {
            ...
            do {
                ...
                if (err < 0 && lastSocketError != PLATFORM_SOCKET_INTERRUPTED) {
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] Error connecting %s\n", __FILE__, __FUNCTION__, __LINE__,
                                                 getLastSocketErrorFormattedText().c_str());
                    if (SystemFlags::VERBOSE_MODE_ENABLED)
                    ...
                    {
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] Error in getsockopt() %s\n", __FILE__, __FUNCTION__,
                                                     __LINE__, getLastSocketErrorFormattedText().c_str());
                        if (SystemFlags::VERBOSE_MODE_ENABLED)
                    ...
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] Timeout in select() - Cancelling!\n", __FILE__, __FUNCTION__,
                                                 __LINE__);
        ...
        if (err < 0) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] Before END sock = %d, err = %d, error = %s\n", __FILE__, __FUNCTION__,
                                         __LINE__, sock, err, getLastSocketErrorFormattedText().c_str());
            if (SystemFlags::VERBOSE_MODE_ENABLED)
...
void BroadCastClientSocketThread::execute() {
    ...
    if (bcfd <= 0) {
        ...
        if (::bind(bcfd, (struct sockaddr *)&bcaddr, sizeof(bcaddr)) < 0) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "bind failed: %s opt_result = %d\n", Socket::getLastSocketErrorFormattedText().c_str(),
                                         opt_result);
        } else {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] opt_result = %d\n", __FILE__, __FUNCTION__, __LINE__, opt_result);
            ...
            try {
                ...
                for (time_t elapsed = time(NULL); difftime((long int)time(NULL), elapsed) <= 5;) {
                    ...
                    if (gotData == false) {
                        ...
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "Client got broadcast from server: [%s] data [%s]\n", fromIP.c_str(), buff);
...
ServerSocket::ServerSocket(bool basicMode) : Socket() {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] basicMode = %d\n", __FILE__, __FUNCTION__, __LINE__, basicMode);
...
void ServerSocket::bind(int port) {
    ...
    if (err < 0) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 "In [%s::%s] Error binding socket sock = " PLATFORM_SOCKET_FORMAT_TYPE ", address [%s] port = %d err = %d, error = %s opt_result = %d\n",
        ...
        snprintf(szBuf, 8096, "Error binding socket sock = " PLATFORM_SOCKET_FORMAT_TYPE ", address [%s] port = %d err = %d, error = %s\n", sock,
                 this->bindSpecificAddress.c_str(), port, err, getLastSocketErrorFormattedText().c_str());
...
Socket *ServerSocket::accept(bool errorOnFail) {
    ...
    if (isSocketValid(&newSock) == false) {
        ...
        snprintf(szBuf, 8096,
                 ...
                 "= " PLATFORM_SOCKET_FORMAT_TYPE ", err = " PLATFORM_SOCKET_FORMAT_TYPE ", error = %s\n",
...
int UPNP_Tools::upnp_init(void *param) {
    ...
    if (UPNP_Tools::isUPNP == true) {
...
#endif
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "UPnP discover returned upnp_error = %d.\n", upnp_error);
...
#endif
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "UPnP device search finished devlist = %p.\n", devlist);
        ...
        if (devlist) {
            ...
            while (dev) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugNetwork, "UPnP discover deviceList [%s].\n", (dev->st ? dev->st : "null"));
...
#ifndef MINIUPNPC_VERSION_PRE1_6
        int r =
            UPNP_AddPortMapping(urls.controlURL, data.first.servicetype, ext_port_str, int_port_str, lanaddr, "MegaGlest - www.megaglest.org", "TCP", 0, NULL);
#else
        ...
        if (r != UPNPCOMMAND_SUCCESS) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] AddPortMapping(%s, %s, %s) failed\n", __FILE__, __FUNCTION__,
                                         __LINE__, ext_port_str, int_port_str, lanaddr);
            result = false;
        }
...
void BroadCastSocketThread::execute() {
    ...
    if (ipList.empty() == false) {
        for (unsigned int idx = 0; idx < (unsigned int)ipList.size() && idx < (unsigned int)MAX_NIC_COUNT; idx++) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "idx = %d broadCastAddress [%s]\n", idx, broadCastAddress.c_str());
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "NO Addresses found for broadCastAddress using INADDR_ANY\n");
    ...
    for (unsigned int idx = 0; idx < (unsigned int)ipSubnetMaskList.size(); idx++) {
        ...
        if (ipSubnetMaskList[idx] == "*") {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "UDP Socket broadcast using INADDR_ANY\n");
            bcLocal[idx].sin_addr.s_addr = INADDR_ANY;
        } else {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "UDP Socket broadcast using IP [%s]\n", ipSubnetMaskList[idx].c_str());
                ...
                try {
                    ...
                    if (difftime((long int)time(NULL), elapsed) >= 1 && getQuitStatus() == false) {
                        ...
                        if (pauseBroadCast == false) {
                            ...
                            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "Server is sending broadcast data [%s]\n", buff);
...
void ServerSocket::addIPAddressToBlockedList(string value) {
    if (isIPAddressBlocked(value) == false) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugError).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugError, "In [%s::%s Line: %d] Blocked IP Address [%s].\n", extractFileFromDirectoryPath(__FILE__).c_str(),

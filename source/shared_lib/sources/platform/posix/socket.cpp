...
SocketManager::~SocketManager() {
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("SocketManager calling WSACleanup...\n");
  WSACleanup();
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "Winsock cleanup complete.\n");
}
...
string Socket::getLastSocketErrorFormattedText(int *errNumber) {
  int errId = (errNumber != NULL ? *errNumber : getLastSocketError());
  string msg = "(Error: " + intToStr(errId) + " - [" +
               string(getLastSocketErrorText(&errId)) + "])";
  return msg;
}
...
Ip::Ip(unsigned char byte0, unsigned char byte1, unsigned char byte2,
       unsigned char byte3) {
  bytes[0] = byte0;
  bytes[1] = byte1;
  bytes[2] = byte2;
...
string Ip::getString() const {
  return intToStr(bytes[0]) + "." + intToStr(bytes[1]) + "." +
...
// ===============================================
    ...
    defined(__OpenBSD__) || defined(BSD) || defined(__APPLE__) ||              \
    defined(__linux__)
...
void Ip::Inet_NtoA(uint32 addr, char *ipbuf) {
  sprintf(ipbuf, "%d.%d.%d.%d", (addr >> 24) & 0xFF, (addr >> 16) & 0xFF,
   ...
   if (getifaddrs(&ifap) == 0)
   {
      ...
      while(p)
      {
         ...
         if (ifaAddr > 0)
         {
            ...
            printf("  Found interface:  name=[%s] desc=[%s] address=[%s]
netmask=[%s] broadcastAddr=[%s]\n", p->ifa_name, "unavailable", ifaAddrStr,
maskAddrStr, dstAddrStr);
         }
         p = p->ifa_next;
      }
      freeifaddrs(ifap);
...
http://msdn2.microsoft.com/en-us/library/aa365917.aspx
   // Now get Windows' IPv4 addresses table.  Once again, we gotta call
GetIpAddrTable()
...
IP
      // addresses we are returning.  Gotta call GetAdaptersInfo() up to 5 times
to handle
      // the potential race condition between the size-query call and the
get-data call.
      ...
      for (DWORD i=0; i<ipTable->dwNumEntries; i++)
      {
         ...
         // Now lookup the appropriate adaptor-name in the pAdaptorInfos, if we
can find it const char * name = NULL; const char * desc = NULL; if
(pAdapterInfo)
         {
         ...
         printf("  Found interface:  name=[%s] desc=[%s] address=[%s]
netmask=[%s] broadcastAddr=[%s]\n", name, desc?desc:"unavailable", ifaAddrStr,
maskAddrStr, dstAddrStr);
      }
...
string getNetworkInterfaceBroadcastAddress(string ipAddress) {
  ...
  if (getifaddrs(&ifap) == 0) {
    ...
    while (p) {
      ...
      if (ifaAddr > 0) {
        ...
        char ifaAddrStr[32];
        Ip::Inet_NtoA(ifaAddr, ifaAddrStr);
        char maskAddrStr[32];
        Ip::Inet_NtoA(maskAddr, maskAddrStr);
        char dstAddrStr[32];
        Ip::Inet_NtoA(dstAddr, dstAddrStr);
        // printf("  Found interface:  name=[%s] desc=[%s] address=[%s]
        // netmask=[%s] broadcastAddr=[%s]\n", p->ifa_name, "unavailable",
        // ifaAddrStr, maskAddrStr, dstAddrStr);
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              ...
              "In [%s::%s Line: %d] ifaAddrStr [%s], maskAddrStr [%s], "
              "dstAddrStr[%s], ipAddress [%s], broadCastAddress [%s]\n",
              __FILE__, __FUNCTION__, __LINE__, ifaAddrStr, maskAddrStr,
              dstAddrStr, ipAddress.c_str(), broadCastAddress.c_str());
      }
      p = p->ifa_next;
    }
    freeifaddrs(ifap);
  ...
  if (ipTable) {
    ...
    for (DWORD i = 0; i < ipTable->dwNumEntries; i++) {
      ...
      // Now lookup the appropriate adaptor-name in the pAdaptorInfos, if we can
      // find it
...
std::vector<std::string> Socket::getLocalIPAddressList() {
  ...
  if (myhostent) {
    ...
    for (int ipIdx = 0; myhostent->h_addr_list[ipIdx] != NULL; ++ipIdx) {
      Ip::Inet_NtoA(
          SockAddrToUint32((struct in_addr *)myhostent->h_addr_list[ipIdx]),
          myhostaddr);

      // printf("ipIdx = %d [%s]\n",ipIdx,myhostaddr);
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s Line: %d] myhostaddr = [%s]\n",
...
#ifndef WIN32
void Socket::getLocalIPAddressListForPlatform(
    std::vector<std::string> &ipList) {
  ...
  for (struct ifaddrs *ifa = ifap; ifa; ifa = ifa->ifa_next) {
    if (ifa->ifa_addr->sa_family == AF_INET) {
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
        ...
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "In [%s::%s Line: %d] Interface: [%s] address: [%s]\n", __FILE__,
            __FUNCTION__, __LINE__, ifa->ifa_name, addr);
      }
      if (strlen(addr) > 0 && strncmp(addr, "127.", 4) != 0 &&
          strncmp(addr, "0.", 2) != 0) {
  ...
  for (unsigned int intfIdx = 0; intfIdx < Socket::intfTypes.size();
       intfIdx++) {
    ...
    for (int idx = 0; idx < 10; ++idx) {
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s Line: %d] Trying NIC named [%s]\n",
                                 __FILE__, __FUNCTION__, __LINE__, szBuf);
      // printf("In [%s::%s Line: %d] Trying NIC named
      // [%s]\n",__FILE__,__FUNCTION__,__LINE__,szBuf);
      ...
      if (result_ifaddrr >= 0) {
        ...
        if (pSockAddr != NULL) {
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] szBuf [%s], myhostaddr = [%s], "
                "ifr.ifr_flags = %d, ifrA.ifr_flags = %d, ifr.ifr_name [%s]\n",
                __FILE__, __FUNCTION__, __LINE__, szBuf, myhostaddr,
                ifr.ifr_flags, ifrA.ifr_flags, ifr.ifr_name);
          ...
          if ((ifrA.ifr_flags & IFF_UP) == IFF_UP &&
            ...
            if (strlen(myhostaddr) > 0 && strncmp(myhostaddr, "127.", 4) != 0 &&
                strncmp(myhostaddr, "0.", 2) != 0) {
              if (std::find(ipList.begin(), ipList.end(), myhostaddr) ==
                  ipList.end()) {
                ipList.push_back(myhostaddr);
              }
            }
...
void Socket::getLocalIPAddressListForPlatform(
  ...
  GetAdaptersAddresses(AF_INET, GAA_FLAG_SKIP_ANYCAST, NULL, pAddresses,
                       &outBufLen);
  ...
  while (pCurrAddresses) {
    ...
    while (pUnicast) {
      ...
      if (addr->sa_family == AF_INET &&
          pCurrAddresses->IfType != MIB_IF_TYPE_LOOPBACK) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                .enabled) {
          SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                   "In [%s::%s Line: %d] strIP [%s]\n",
                                   __FILE__, __FUNCTION__, __LINE__, strIP);
        }
        if (strlen(strIP) > 0 && strncmp(strIP, "127.", 4) != 0 &&
            strncmp(strIP, "0.", 2) != 0) {
...
bool Socket::isSocketValid() const { return Socket::isSocketValid(&sock); }
...
Socket::Socket(PLATFORM_SOCKET sock) {
  ...
  MutexSafeWrapper safeMutexSocketDestructorFlag(
      inSocketDestructorSynchAccessor, CODE_AT_LINE);
...
Socket::~Socket() {
  ...
  if (this->inSocketDestructor == true) {
    SystemFlags::OutputDebug(
        ...
        "In [%s::%s Line: %d] this->inSocketDestructor == true\n", __FILE__,
        __FUNCTION__, __LINE__);
    return;
  }
  inSocketDestructorSynchAccessor->setOwnerId(CODE_AT_LINE);
...
void Socket::disconnectSocket() {
  ...
  if (isSocketValid() == true) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s] calling shutdown and close for socket = %d...\n",
          __FILE__, __FUNCTION__, sock);
...
bool Socket::hasDataToRead(
  ...
  if (socketTriggeredList.empty() == false) {
    ...
    for (std::map<PLATFORM_SOCKET, bool>::iterator itermap =
             socketTriggeredList.begin();
    ...
    if (imaxsocket > 0) {
      ...
      if (retval < 0) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "In [%s::%s] Line: %d, ERROR SELECTING SOCKET DATA retval = %d "
              "error = %s, socketDebugList [%s]\n",
              __FILE__, __FUNCTION__, __LINE__, retval,
              getLastSocketErrorFormattedText().c_str(),
              socketDebugList.c_str());
        printf("In [%s::%s] Line: %d, ERROR SELECTING SOCKET DATA retval = %d "
               "error = %s\n",
               __FILE__, __FUNCTION__, __LINE__, retval,
               getLastSocketErrorFormattedText().c_str());
      } else if (retval) {
...
bool Socket::hasDataToRead(PLATFORM_SOCKET socket) {
  ...
  if (Socket::isSocketValid(&socket) == true) {
    ...
    if (retval < 0) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s] Line: %d, ERROR SELECTING SOCKET "
                                 "DATA retval = %d error = %s\n",
                                 __FILE__, __FUNCTION__, __LINE__, retval,
                                 getLastSocketErrorFormattedText().c_str());
      printf("In [%s::%s] Line: %d, ERROR SELECTING SOCKET DATA retval = %d "
             "error = %s\n",
...
bool Socket::hasDataToReadWithWait(PLATFORM_SOCKET socket,
  ...
  // printf("hasdata waited [%d] milliseconds [%d], bResult =
...
int Socket::getDataToRead(bool wantImmediateReply) {
  ...
  if (isSocketValid() == true) {
    // int loopCount = 1;
    for (time_t elapsed = time(NULL);
         difftime((long int)time(NULL), elapsed) < 1;) {
      ...
      if (err < 0 && lastSocketError != PLATFORM_SOCKET_TRY_AGAIN) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "In [%s::%s Line: %d] ERROR PEEKING SOCKET DATA, err = %d %s\n",
      ...
      if (size > 0) {
        ...
        // %d] WARNING PEEKING SOCKET DATA, (hasDataToRead() == true) err = %d,
        // sock = %d, size = " MG_SIZE_T_SPECIFIER ", loopCount =
        // %d\n",__FILE__,__FUNCTION__,__LINE__,err,sock,size,loopCount);
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                .enabled)
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "In [%s::%s Line: %d] WARNING PEEKING SOCKET DATA, "
              "(hasDataToRead() == true) err = %d, sock = %d, size "
              "= " MG_SIZE_T_SPECIFIER "\n",
              __FILE__, __FUNCTION__, __LINE__, err, sock, size);
      } else {
        // if(SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        // SystemFlags::OutputDebug(SystemFlags::debugNetwork,"In [%s::%s Line:
        // %d] WARNING PEEKING SOCKET DATA, err = %d, sock = %d, size = "
        // MG_SIZE_T_SPECIFIER ", loopCount =
        // %d\n",__FILE__,__FUNCTION__,__LINE__,err,sock,size,loopCount);
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                .enabled)
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "In [%s::%s Line: %d] WARNING PEEKING SOCKET DATA, err = %d, "
              "sock = %d, size = " MG_SIZE_T_SPECIFIER "\n",
              __FILE__, __FUNCTION__, __LINE__, err, sock, size);
        break;
      }
...
int Socket::send(const void *data, int dataSize) {
  ...
  if (isSocketValid() == true) {
    ...
    //    	MutexSafeWrapper
    //    safeMutexSocketDestructorFlag(&inSocketDestructorSynchAccessor,CODE_AT_LINE);
    //    	if(this->inSocketDestructor == true) {
    //    		SystemFlags::OutputDebug(SystemFlags::debugError,"In
    //    [%s::%s Line: %d] this->inSocketDestructor ==
...
#else
      bytesSent = ::send(sock, (const char *)data, dataSize,
  ...
  if (bytesSent < 0 && lastSocketError != PLATFORM_SOCKET_TRY_AGAIN) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] ERROR WRITING SOCKET DATA, err = %d error = %s "
          "dataSize = %d\n",
          __FILE__, __FUNCTION__, __LINE__, bytesSent,
          getLastSocketErrorFormattedText(&lastSocketError).c_str(), dataSize);
  } else if (bytesSent < 0 && lastSocketError == PLATFORM_SOCKET_TRY_AGAIN &&
             isConnected() == true) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "In [%s::%s Line: %d] #1 EAGAIN during send, "
                               "trying again... dataSize = %d\n",
                               __FILE__, __FUNCTION__, __LINE__, dataSize);

    int attemptCount = 0;
    time_t tStartTimer = time(NULL);
    while ((bytesSent < 0 && lastSocketError == PLATFORM_SOCKET_TRY_AGAIN) &&
           (difftime((long int)time(NULL), tStartTimer) <=
            MAX_SEND_WAIT_SECONDS)) {
      attemptCount++;
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s Line: %d] attemptCount = %d\n",
      ...
      if (isConnected() == true) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "In [%s::%s Line: %d] attemptCount = %d, sock = %d, dataSize = "
              "%d, data = %p, canWrite = %d\n",
              __FILE__, __FUNCTION__, __LINE__, attemptCount, sock, dataSize,
              data, canWrite);
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                .enabled)
          SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                   "In [%s::%s Line: %d] #2 EAGAIN during "
                                   "send, trying again returned: %d\n",
                                   __FILE__, __FUNCTION__, __LINE__, bytesSent);
      } else {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "[%s::%s] DISCONNECTED SOCKET error while sending socket data, "
              "bytesSent = %d, error = %s\n",
              __FILE__, __FUNCTION__, bytesSent,
              getLastSocketErrorFormattedText(&iErr).c_str());
        break;
      }
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
  ...
  if (isConnected() == true && bytesSent > 0 && bytesSent < dataSize) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] need to send more data, trying again "
          "getLastSocketError() = %d, bytesSent = %d, dataSize = %d\n",
          __FILE__, __FUNCTION__, __LINE__, lastSocketError, bytesSent,
          dataSize);
    ...
    while (((bytesSent > 0 && totalBytesSent < dataSize) ||
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "In [%s::%s Line: %d] attemptCount = %d, totalBytesSent = %d\n",
      ...
      if (isConnected() == true) {
...
#ifdef __APPLE__
        bytesSent = ::send(sock, &sendBuf[totalBytesSent],
                           dataSize - totalBytesSent, SO_NOSIGPIPE);
#else
        bytesSent =
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "In [%s::%s Line: %d] retry send returned: %d\n", __FILE__,
              __FUNCTION__, __LINE__, bytesSent);
      } else {
        int iErr = getLastSocketError();
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "[%s::%s Line: %d] DISCONNECTED SOCKET error while sending "
              "socket data, bytesSent = %d, error = %s\n",
              __FILE__, __FUNCTION__, __LINE__, bytesSent,
              getLastSocketErrorFormattedText(&iErr).c_str());
        break;
      }
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] bytesSent = %d, totalBytesSent = %d\n",
  ...
  if (bytesSent <= 0 || isConnected() == false) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "In [%s::%s Line: %d] ERROR WRITING SOCKET "
                               "DATA, err = %d error = %s\n",
                               __FILE__, __FUNCTION__, __LINE__, bytesSent,
                               getLastSocketErrorFormattedText().c_str());
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "[%s::%s Line: %d] DISCONNECTED SOCKET error while sending socket "
          "data, bytesSent = %d, error = %s\n",
          __FILE__, __FUNCTION__, __LINE__, bytesSent,
          getLastSocketErrorFormattedText(&iErr).c_str());
  }
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s Line: %d] sock = %d, bytesSent = %d\n",
...
int Socket::receive(void *data, int dataSize, bool tryReceiveUntilDataSizeMet) {
  ...
  if (bytesReceived < 0 && lastSocketError != PLATFORM_SOCKET_TRY_AGAIN) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "[%s::%s Line: %d] ERROR READING SOCKET DATA error while sending "
          "socket data, bytesSent = %d, error = %s\n",
          __FILE__, __FUNCTION__, __LINE__, bytesReceived,
          getLastSocketErrorFormattedText(&lastSocketError).c_str());
  } else if (bytesReceived < 0 &&
             lastSocketError == PLATFORM_SOCKET_TRY_AGAIN) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] #1 EAGAIN during receive, trying again...\n",
          __FILE__, __FUNCTION__, __LINE__);
    ...
    time_t tStartTimer = time(NULL);
    while (
        (bytesReceived < 0 && lastSocketError == PLATFORM_SOCKET_TRY_AGAIN) &&
        (difftime((long int)time(NULL), tStartTimer) <=
         MAX_RECV_WAIT_SECONDS)) {
      if (isConnected() == false) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                .enabled)
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "[%s::%s Line: %d] Socket is NOT connected!\n", __FILE__,
              __FUNCTION__, __LINE__);
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "[%s::%s Line: %d] DISCONNECTED SOCKET error while receiving "
              "socket data, bytesSent = %d, error = %s\n",
              __FILE__, __FUNCTION__, __LINE__, bytesReceived,
              getLastSocketErrorFormattedText(&iErr).c_str());
        break;
      ...
      // else if(Socket::isReadable(true) == true) {
        ...
        if (Socket::isReadable(true) == true) {
          ...
          errno = 0;
          bytesReceived =
              recv(sock, reinterpret_cast<char *>(data), dataSize, 0);
          lastSocketError = getLastSocketError();
          // safeBlock.Restore();
          safeMutex.ReleaseLock();
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] #2 EAGAIN during receive, trying again "
                "returned: %d, lastSocketError = %d, dataSize = %d\n",
                __FILE__, __FUNCTION__, __LINE__, bytesReceived,
                lastSocketError, (int)dataSize);
          // printf("In [%s::%s Line: %d] #2 EAGAIN during receive, trying again
          // returned: %d, lastSocketError = %d, dataSize =
  ...
  if (bytesReceived <= 0) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "[%s::%s Line: %d] DISCONNECTED SOCKET error while receiving socket "
          "data, bytesReceived = %d, error = %s, dataSize = %d, "
          "tryReceiveUntilDataSizeMet = %d\n",
          __FILE__, __FUNCTION__, __LINE__, bytesReceived,
          getLastSocketErrorFormattedText(&iErr).c_str(), dataSize,
          tryReceiveUntilDataSizeMet);
  } else if (tryReceiveUntilDataSizeMet == true && bytesReceived < dataSize) {
    int newBufferSize = (dataSize - (int)bytesReceived);
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] WARNING, attempting to receive MORE data, "
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("\nIn [%s::%s Line: %d] WARNING, attempting to receive MORE data, "
             "bytesReceived = %d, dataSize = %d, newBufferSize = %d\n",
             __FILE__, __FUNCTION__, __LINE__, (int)bytesReceived, dataSize,
             newBufferSize);

    char *dataAsCharPointer = reinterpret_cast<char *>(data);
    int additionalBytes = receive(&dataAsCharPointer[bytesReceived],
                                  newBufferSize, tryReceiveUntilDataSizeMet);

    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] WARNING, additionalBytes = %d\n", __FILE__,
          __FUNCTION__, __LINE__, additionalBytes);
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("\nIn [%s::%s Line: %d] WARNING, additionalBytes = %d\n", __FILE__,
             __FUNCTION__, __LINE__, additionalBytes);
    ...
    if (additionalBytes > 0) {
      ...
      // throw megaglest_runtime_error("additionalBytes == " +
      // intToStr(additionalBytes));
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("\nIn [%s::%s Line: %d] additionalBytes == %d\n", __FILE__,
               __FUNCTION__, __LINE__, additionalBytes);
      if (SystemFlags::getSystemSettingType(SystemFlags::debugError).enabled)
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "[%s::%s Line: %d] DISCONNECTED SOCKET error while receiving "
            "socket data, bytesReceived = %d, error = %s, dataSize = %d, "
            "tryReceiveUntilDataSizeMet = %d\n",
            __FILE__, __FUNCTION__, __LINE__, bytesReceived,
            getLastSocketErrorFormattedText(&iErr).c_str(), dataSize,
            tryReceiveUntilDataSizeMet);
...
SafeSocketBlockToggleWrapper::SafeSocketBlockToggleWrapper(Socket *socket,
                                                           bool toggle) {
  this->socket = socket;
...
SafeSocketBlockToggleWrapper::~SafeSocketBlockToggleWrapper() { Restore(); }

int Socket::peek(void *data, int dataSize, bool mustGetData,
                 int *pLastSocketError) {
  Chrono chrono;
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    chrono.start();
  ...
  if (isSocketValid() == true) {
    ...
    // MutexSafeWrapper safeMutex(&dataSynchAccessor,CODE_AT_LINE + "_" +
    // intToStr(sock) + "_" + intToStr(dataSize));
    ...
    if (isSocketValid() == true) {
      ...
      //			if(recvTimer.getMillis() > 1000 || (err <= 0 &&
      // lastSocketError != 0 && lastSocketError != PLATFORM_SOCKET_TRY_AGAIN))
      // { 				printf("#1 PEEK err = %d lastSocketError = %d ms:
      //%lld\n",err,lastSocketError,(long long int)recvTimer.getMillis());

      // if(err != dataSize) {
      //	printf("#1 PEEK err = %d lastSocketError =
      //%d\n",err,lastSocketError);
      // }
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      if (chrono.getMillis() > 1)
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "In [%s::%s Line: %d] action running for msecs: %lld\n", __FILE__,
            __FUNCTION__, __LINE__, (long long int)chrono.getMillis());
  } else {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] SOCKET appears to be invalid [%d]\n", __FILE__,
  ...
  if (err < 0 && lastSocketError != PLATFORM_SOCKET_TRY_AGAIN) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "[%s::%s Line: %d] ERROR PEEKING SOCKET DATA error while sending "
          "socket data, err = %d, error = %s\n",
          __FILE__, __FUNCTION__, __LINE__, err,
          getLastSocketErrorFormattedText().c_str());
    disconnectSocket();
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] #1 ERROR EAGAIN during peek, trying again...\n",
          __FILE__, __FUNCTION__, __LINE__);
    ...
    while ((err < 0 && lastSocketError == PLATFORM_SOCKET_TRY_AGAIN) &&
      ...
      */
      if (Socket::isReadable(true) == true ||
        ...
        //                if(recvTimer.getMillis() > 1000 || (err <= 0 &&
        //                lastSocketError != 0 && lastSocketError !=
        //                PLATFORM_SOCKET_TRY_AGAIN)) {
        //    				printf("#2 PEEK err = %d lastSocketError = %d
        //    ms: %lld\n",err,lastSocketError,(long long
        //    int)recvTimer.getMillis());
        //    			}

        // printf("Socket peek delayed checking for sock = %d err =
        // %d\n",sock,err);
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                   "In [%s::%s Line: %d] #2 EAGAIN during "
                                   "peek, trying again returned: %d\n",
                                   __FILE__, __FUNCTION__, __LINE__, err);
      }
      // else {
      // printf("Socket peek delayed [NOT READABLE] checking for sock = %d err =
      // %d\n",sock,err);
      //}
    }
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] #2 SOCKET appears to be invalid [%d] "
          "lastSocketError [%d] err [%d] mustGetData [%d] dataSize [%d]\n",
          __FILE__, __FUNCTION__, __LINE__, sock, lastSocketError, err,
          mustGetData, dataSize);
  }
  ...
  if (err < 0 || (err == 0 && dataSize != 0) ||
      ((err == 0 || err == -1) && dataSize == 0 && lastSocketError != 0 &&
       lastSocketError != PLATFORM_SOCKET_TRY_AGAIN)) {
    // printf("** #1 Socket peek error for sock = %d err = %d lastSocketError =
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "[%s::%s Line: %d] DISCONNECTING SOCKET for socket [%d], err = %d, "
          "dataSize = %d, error = %s\n",
          __FILE__, __FUNCTION__, __LINE__, sock, err, dataSize,
          getLastSocketErrorFormattedText(&iErr).c_str());
    // printf("Peek #3 err = %d\n",err);
    // lastSocketError = getLastSocketError();
    if (mustGetData == true || lastSocketError != PLATFORM_SOCKET_TRY_AGAIN) {
      printf("** #2 Socket peek error for sock = %d err = %d lastSocketError = "
             "%d mustGetData = %d\n",
      ...
      if (err == 0) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "** LAST CHANCE for disconnection check for sock = %d\n", sock);

        MutexSafeWrapper safeMutex(dataSynchAccessorRead, CODE_AT_LINE);
        SafeSocketBlockToggleWrapper safeUnblock(this, false);
        errno = 0;
        int second_err =
            recv(sock, reinterpret_cast<char *>(data), dataSize, MSG_PEEK);
        safeUnblock.Restore();
        safeMutex.ReleaseLock();
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "[%s::%s Line: %d] DISCONNECTED SOCKET error while peeking socket "
            "data for socket [%d], err = %d, dataSize = %d, error = %s\n",
            __FILE__, __FUNCTION__, __LINE__, sock, err, dataSize,
            getLastSocketErrorFormattedText(&iErr).c_str());
    }
  }
...
inline bool Socket::isReadable(bool lockMutex) {
  ...
  if (i < 0) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "[%s::%s] error while selecting socket data, err = %d, error = %s\n",
  ...
  bool result = (i == 1);
  // if(result == false) {
  //	SystemFlags::OutputDebug(SystemFlags::debugError,"SOCKET DISCONNECTED In
  //[%s::%s Line: %d] i = %d sock =
...
inline bool Socket::isWritable(struct timeval *timeVal, bool lockMutex) {
  ...
  if (i < 0) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "[%s::%s Line: %d] error while selecting socket "
                               "data, err = %d, error = %s\n",
                               __FILE__, __FUNCTION__, __LINE__, i,
                               getLastSocketErrorFormattedText().c_str());
    ...
    SystemFlags::OutputDebug(
        SystemFlags::debugError,
        "SOCKET DISCONNECTED In [%s::%s Line: %d] error while selecting socket "
        "data, err = %d, error = %s\n",
        __FILE__, __FUNCTION__, __LINE__, i,
        getLastSocketErrorFormattedText().c_str());
  } else if (i == 0) {
    // SystemFlags::OutputDebug(SystemFlags::debugNetwork,"[%s::%s Line: %d]
    // TIMEOUT while selecting socket data, err = %d, error =
    ...
    if (difftime(time(NULL), lastSocketError) > 1) {
      ...
      SystemFlags::OutputDebug(
          SystemFlags::debugError,
          "SOCKET WRITE TIMEOUT In [%s::%s Line: %d] i = %d sock = %d [%s]\n",
...
bool Socket::isConnected() {
  ...
  // safeMutexSocketDestructorFlag(&inSocketDestructorSynchAccessor,CODE_AT_LINE);
  //	if(this->inSocketDestructor == true) {
  //		SystemFlags::OutputDebug(SystemFlags::debugError,"In [%s::%s
  //Line: %d] this->inSocketDestructor ==
  ...
  if (isWritable(NULL, true) == false) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "[%s::%s Line: %d] ERROR isWritable failed.\n",
                               __FILE__, __FUNCTION__, __LINE__);
    return false;
  }
  ...
  if (isReadable(true)) {
    ...
    // if(err <= 0 && err != PLATFORM_SOCKET_TRY_AGAIN) {
    // if(err <= 0 && lastSocketError != 0 && lastSocketError !=
    // PLATFORM_SOCKET_TRY_AGAIN) { if((err < 0 && lastSocketError !=
    // PLATFORM_SOCKET_TRY_AGAIN) || (err == 0 && peekDataBytes != 0) ||
    //	((err == 0 || err == -1) && peekDataBytes == 0 && lastSocketError != 0
    //&& lastSocketError != PLATFORM_SOCKET_TRY_AGAIN)) {
    if ((err < 0 && lastSocketError != PLATFORM_SOCKET_TRY_AGAIN) ||
        (err == 0 && peekDataBytes != 0)) {
      // printf("IsConnected socket has disconnected sock = %d err = %d
      // lastSocketError = %d\n",sock,err,lastSocketError);
      if (err == 0) {
        printf("IsConnected socket has disconnected sock = %d err = %d "
               "lastSocketError = %d\n",
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "[%s::%s Line: %d] ERROR Peek failed, err = %d for socket: %d, "
              "error = %s, lastSocketError = %d\n",
              __FILE__, __FUNCTION__, __LINE__, err, sock,
              getLastSocketErrorFormattedText().c_str(), lastSocketError);
      }
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        SystemFlags::OutputDebug(
            SystemFlags::debugError,
            "SOCKET DISCONNECTED In [%s::%s Line: %d] ERROR Peek failed, err = "
            "%d for socket: %d, error = %s, lastSocketError = %d\n",
            __FILE__, __FUNCTION__, __LINE__, err, sock,
            getLastSocketErrorFormattedText().c_str(), lastSocketError);
      return false;
    }
  ...
  if (Socket::host_name == "") {
    ...
    if (result == 0) {
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "In [%s::%s Line: %d] result = %d, error = %s\n", __FILE__,
            __FUNCTION__, __LINE__, result, getLastSocketErrorText());
    }
  }
...
string Socket::getIp() {
  ...
  return intToStr(address[0]) + "." + intToStr(address[1]) + "." +
         intToStr(address[2]) + "." + intToStr(address[3]);
}
  ...
  if (err < 0) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          ...
          __FILE__, __FUNCTION__, __LINE__, ip.getString().c_str(), port, err,
          getLastSocketErrorFormattedText().c_str());
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d] #2 Error connecting socket for IP: %s for "
             "Port: %d err = %d error = %s\n",
             __FILE__, __FUNCTION__, __LINE__, ip.getString().c_str(), port,
             err, getLastSocketErrorFormattedText().c_str());
    ...
    if (lastSocketError == PLATFORM_SOCKET_INPROGRESS ||
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "In [%s::%s Line: %d] PLATFORM_SOCKET_INPROGRESS in connect() - "
            "selecting\n",
            __FILE__, __FUNCTION__, __LINE__);
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("In [%s::%s Line: %d] PLATFORM_SOCKET_INPROGRESS in connect() - "
               "selecting\n",
               __FILE__, __FUNCTION__, __LINE__);
      ...
      do {
        ...
        if (err < 0 && lastSocketError != PLATFORM_SOCKET_INTERRUPTED) {
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] Error connecting %s\n", __FILE__,
                __FUNCTION__, __LINE__,
                getLastSocketErrorFormattedText().c_str());
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] Error connecting %s\n", __FILE__,
                   __FUNCTION__, __LINE__,
                   getLastSocketErrorFormattedText().c_str());
          ...
          if (getsockopt(sock, SOL_SOCKET, SO_ERROR, (void *)(&valopt), &lon) <
              0)
#else
          if (getsockopt(sock, SOL_SOCKET, SO_ERROR, (char *)(&valopt), &lon) <
              0)
#endif
          {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
              ...
              SystemFlags::OutputDebug(
                  SystemFlags::debugNetwork,
                  "In [%s::%s Line: %d] Error in getsockopt() %s\n", __FILE__,
                  __FUNCTION__, __LINE__,
                  getLastSocketErrorFormattedText().c_str());
            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf("In [%s::%s Line: %d] Error in getsockopt() %s\n",
                     __FILE__, __FUNCTION__, __LINE__,
                     getLastSocketErrorFormattedText().c_str());
            break;
          }
          ...
          if (valopt) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                    .enabled)
              SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                       "In [%s::%s Line: %d] Error in delayed "
                                       "connection() %d - [%s]\n",
                                       __FILE__, __FUNCTION__, __LINE__, valopt,
                                       strerror(valopt));
            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf("In [%s::%s Line: %d] Error in delayed connection() %d - "
                     "[%s]\n",
                     __FILE__, __FUNCTION__, __LINE__, valopt,
                     strerror(valopt));
            break;
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                  .enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                     "In [%s::%s Line: %d] Apparent recovery "
                                     "for connection sock = %d, err = %d\n",
                                     __FILE__, __FUNCTION__, __LINE__, sock,
                                     err);
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] Apparent recovery for connection sock "
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] Timeout in select() - Cancelling!\n",
                __FILE__, __FUNCTION__, __LINE__);
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] Timeout in select() - Cancelling!\n",
                   __FILE__, __FUNCTION__, __LINE__);
    ...
    if (err < 0) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "In [%s::%s Line: %d] Before END sock = %d, err = %d, error = %s\n",
            __FILE__, __FUNCTION__, __LINE__, sock, err,
            getLastSocketErrorFormattedText().c_str());
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("In [%s::%s Line: %d] Before END sock "
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s Line: %d] Valid recovery for "
                                 "connection sock = %d, err = %d, error = %s\n",
                                 __FILE__, __FUNCTION__, __LINE__, sock, err,
                                 getLastSocketErrorFormattedText().c_str());
      if (SystemFlags::VERBOSE_MODE_ENABLED)
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "Connected to host [%s] on port = %d sock = %d err = %d",
...
//					other servers
//
BroadCastClientSocketThread::BroadCastClientSocketThread(
    DiscoveredServersInterface *cb)
    : BaseThread() {
...
// Description:		Runs in its own thread to send out a broadcast to the
// local network
//					the current broadcast message is
//<myhostname:my.ip.address.dotted>
//
void BroadCastClientSocketThread::execute() {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "Broadcast Client thread is running\n");
  ...
  if (bcfd <= 0) {
    ...
    if (::bind(bcfd, (struct sockaddr *)&bcaddr, sizeof(bcaddr)) < 0) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork, "bind failed: %s opt_result = %d\n",
            Socket::getLastSocketErrorFormattedText().c_str(), opt_result);
    } else {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s Line: %d] opt_result = %d\n",
      ...
      try {
        ...
        for (time_t elapsed = time(NULL);
          ...
          bool gotData =
              (nb = recvfrom(bcfd, buff, 10024, 0, (struct sockaddr *)&bcSender,
                             &alen)) > 0;
          if (nb >= 0) {
            buff[nb] = 0;
          }

          // printf("Broadcasting client nb = %d buff [%s] gotData =
          // %d\n",nb,buff,gotData);
          ...
          if (gotData == false) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
              ...
              SystemFlags::OutputDebug(
                  SystemFlags::debugNetwork,
                  "broadcast message received: [%s] from: [%s]\n", buff,
                  fromIP.c_str());
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
              ...
              SystemFlags::OutputDebug(
                  SystemFlags::debugNetwork,
                  "Client got broadcast from server: [%s] data [%s]\n",
                  fromIP.c_str(), buff);
            ...
            if (std::find(foundServers.begin(), foundServers.end(), fromIP) ==
                foundServers.end()) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "Broadcast Client thread is exiting\n");
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s Line: %d]\n", __FILE__, __FUNCTION__,
                             __LINE__);
...
ServerSocket::ServerSocket(bool basicMode) : Socket() {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s Line: %d] basicMode = %d\n", __FILE__,
                             __FUNCTION__, __LINE__, basicMode);
...
ServerSocket::~ServerSocket() {
  ...
  // printf("In [%s::%s] Line: %d UPNP_Tools::enabledUPNP =
  // %d\n",__FILE__,__FUNCTION__,__LINE__,UPNP_Tools::enabledUPNP);

  stopBroadCastThread();

  if (this->basicMode == false) {
    MutexSafeWrapper safeMutexUPNP(&ServerSocket::mutexUpnpdiscoverThread,
                                   CODE_AT_LINE);
...
bool ServerSocket::isBroadCastThreadRunning() {
  ...
  bool isThreadRunning =
      (broadCastThread != NULL && broadCastThread->getRunningStatus() == true);

  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s Line: %d] isThreadRunning = %d\n",
...
void ServerSocket::bind(int port) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s] Line: %d port = %d, portBound = %d START\n", __FILE__,
        __FUNCTION__, __LINE__, port, portBound);
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s] Line: %d port = %d, portBound = %d END\n", __FILE__,
        __FUNCTION__, __LINE__, port, portBound);
}
...
void ServerSocket::listen(int connectionQueueSize) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s] Line: %d connectionQueueSize = %d\n",
                             __FILE__, __FUNCTION__, __LINE__,
                             connectionQueueSize);
  ...
  if (connectionQueueSize > 0) {
    ...
    if (err < 0) {
      char szBuf[8096] = "";
      snprintf(szBuf, 8096,
               "In [%s::%s] Error listening socket sock "
               "= " PLATFORM_SOCKET_FORMAT_TYPE ", err = %d, error = %s\n",
...
Socket *ServerSocket::accept(bool errorOnFail) {
  ...
  if (isSocketValid(&newSock) == false) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] Error accepting socket connection sock "
             "= " PLATFORM_SOCKET_FORMAT_TYPE
             ", err = " PLATFORM_SOCKET_FORMAT_TYPE ", error = %s\n",
             __FILE__, __FUNCTION__, __LINE__, sock, newSock,
             getLastSocketErrorFormattedText().c_str());
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "In [%s::%s Line: %d] %s\n", __FILE__,
  ...
  if (isIPAddressBlocked((client_host[0] != '\0' ? client_host : "")) == true) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "In [%s::%s Line: %d] BLOCKING connection, "
                               "newSock = %d client_host [%s]\n",
                               __FILE__, __FUNCTION__, __LINE__, newSock,
                               client_host);
...
void ServerSocket::NETdiscoverUPnPDevices() {
  ...
  // WATCH OUT! Because the thread takes void * as a parameter we MUST cast to
  // the pointer type used on the other side (inside the thread)
  // printf("STARTING UPNP Thread\n");
  ServerSocket::upnpdiscoverThread =
      SDL_CreateThread(&UPNP_Tools::upnp_init, "upnpdiscoverThread",
                       dynamic_cast<UPNPInitInterface *>(this));
  safeMutexUPNP.ReleaseLock();
...
void ServerSocket::UPNPInitStatus(bool result) {
  ...
  if (result == true) {
    // int ports[4] = { this->getExternalPort(), this->getBindPort(),
    // this->getFTPServerPort(), this->getFTPServerPort() };
    ...
    for (int clientIndex = 1; clientIndex <= ServerSocket::maxPlayerCount;
         ++clientIndex) {
...
void UPNP_Tools::AddUPNPPortForward(int internalPort, int externalPort) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s Line: %d] internalPort = %d, externalPort = %d\n", __FILE__,
        __FUNCTION__, __LINE__, internalPort, externalPort);
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s Line: %d] internalPort = %d, "
                             "externalPort = %d, addPorts = %d\n",
                             __FILE__, __FUNCTION__, __LINE__, internalPort,
                             externalPort, addPorts);
...
int UPNP_Tools::upnp_init(void *param) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s Line: %d] isUPNP = %d callback = %p\n",
                             __FILE__, __FUNCTION__, __LINE__,
                             UPNP_Tools::isUPNP, callback);

  if (UPNP_Tools::isUPNP == true) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "Searching for UPnP devices for automatic port forwarding...\n");
    ...
    devlist = upnpDiscover(upnp_delay, upnp_multicastif, upnp_minissdpdsock,
                           upnp_sameport, upnp_ipv6, &upnp_error);
#else
    // miniupnpc 1.9.20150730
    devlist = upnpDiscover(upnp_delay, upnp_multicastif, upnp_minissdpdsock,
                           upnp_sameport, upnp_ipv6, 2, &upnp_error);
#endif
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "UPnP discover returned upnp_error = %d.\n",
                               upnp_error);
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("UPnP discover returned upnp_error = %d.\n", upnp_error);
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "UPnP device search finished devlist = %p.\n",
                               devlist);
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("UPnP device search finished devlist = %p.\n", devlist);
    ...
    if (devlist) {
      ...
      while (dev) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                .enabled)
          SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                   "UPnP discover deviceList [%s].\n",
                                   (dev->st ? dev->st : "null"));
        if (SystemFlags::VERBOSE_MODE_ENABLED)
          printf("UPnP discover deviceList [%s].\n",
                 (dev->st ? dev->st : "null"));
      ...
      if (dev != NULL) {
        ...
        char *descXML = (char *)miniwget_getaddr(
            dev->descURL, &descXMLsize, lanaddr,
            (sizeof(lanaddr) / sizeof(lanaddr[0])), 0, NULL);
    ...
    (!defined(MINIUPNPC_VERSION_PRE1_7) && !defined(MINIUPNPC_VERSION_PRE1_6))
        char *descXML =
            (char *)miniwget_getaddr(dev->descURL, &descXMLsize, lanaddr,
                                     (sizeof(lanaddr) / sizeof(lanaddr[0])), 0);
#else
        char *descXML =
            (char *)miniwget_getaddr(dev->descURL, &descXMLsize, lanaddr,
                                     (sizeof(lanaddr) / sizeof(lanaddr[0])));
#endif
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                .enabled)
          SystemFlags::OutputDebug(SystemFlags::debugNetwork,
    ...
    (!defined(MINIUPNPC_VERSION_PRE1_7) && !defined(MINIUPNPC_VERSION_PRE1_6))
        ...
        snprintf(buf, 255, "UPnP device found: %s %s LAN address %s",
      ...
      if (!urls.controlURL || urls.controlURL[0] == '\0') {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                .enabled)
          SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                   "UPnP device found at: [%s] callback [%p]\n",
                                   externalIP, callback);
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "UPnP detection routine disabled by user.\n");
...
bool UPNP_Tools::upnp_add_redirect(int ports[2], bool mutexLock) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s Line: %d] upnp_add_redir(%d : %d)\n",
                             __FILE__, __FUNCTION__, __LINE__, ports[0],
                             ports[1]);
  ...
  if (!urls.controlURL || urls.controlURL[0] == '\0') {
    ...
    int r = UPNP_AddPortMapping(
        urls.controlURL, data.first.servicetype, ext_port_str, int_port_str,
        lanaddr, "MegaGlest - www.megaglest.org", "TCP", 0, NULL);
#else
    int r = UPNP_AddPortMapping(urls.controlURL, data.first.servicetype,
                                ext_port_str, int_port_str, lanaddr,
                                "MegaGlest - www.megaglest.org", "TCP", 0);
#endif
#else
    int r = UPNP_AddPortMapping(urls.controlURL, data.servicetype, ext_port_str,
                                int_port_str, lanaddr,
                                "MegaGlest - www.megaglest.org", "TCP", 0);
#endif
    if (r != UPNPCOMMAND_SUCCESS) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "In [%s::%s Line: %d] AddPortMapping(%s, %s, %s) failed\n",
            __FILE__, __FUNCTION__, __LINE__, ext_port_str, int_port_str,
            lanaddr);
      result = false;
    }
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] AddPortMapping(%s, %s, %s) success\n", __FILE__,
          __FUNCTION__, __LINE__, ext_port_str, int_port_str, lanaddr);
  }
...
void UPNP_Tools::upnp_rem_redirect(int ext_port) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s Line: %d] upnp_rem_redir(%d)\n",
  ...
  if (urls.controlURL && urls.controlURL[0] != '\0') {
    ...
    result = UPNP_DeletePortMapping(urls.controlURL, data.first.servicetype,
                                    ext_port_str, "TCP", 0);
#else
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("\n\n#1 DEBUGGING data.servicetype [%s]\n", data.servicetype);
    result = UPNP_DeletePortMapping(urls.controlURL, data.servicetype,
                                    ext_port_str, "TCP", 0);
#endif
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("\n\nresult = %d\n", result);
...
void UPNP_Tools::NETaddRedirects(std::vector<int> UPNPPortForwardList,
  ...
  if (UPNPPortForwardList.size() % 2 != 0) {
    // We need groups of 2 ports.. one external and one internal for opening
    // ports on UPNP router
    throw megaglest_runtime_error(
        "UPNPPortForwardList.size() MUST BE divisable by 2");
  }

  for (unsigned int clientIndex = 0; clientIndex < UPNPPortForwardList.size();
       clientIndex += 2) {
    int ports[2] = {UPNPPortForwardList[clientIndex],
                    UPNPPortForwardList[clientIndex + 1]};
...
// in		:		none
// return	:		none
// Description:		To be forked in its own thread to send out a broadcast
// to the local subnet
//					the current broadcast message is
//<myhostname:my.ip.address.dotted>
//
...
bool BroadCastSocketThread::getPauseBroadcast() {
  MutexSafeWrapper safeMutexSocketDestructorFlag(mutexPauseBroadcast,
                                                 CODE_AT_LINE);
...
void BroadCastSocketThread::execute() {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "Broadcast thread is running\n");

  const int MAX_NIC_COUNT = 10;
  short port = 0; // The port for the broadcast.
  struct sockaddr_in
      bcLocal[MAX_NIC_COUNT]; // local socket address for the broadcast.
  ...
  if (ipList.empty() == false) {
    for (unsigned int idx = 0;
         idx < (unsigned int)ipList.size() && idx < (unsigned int)MAX_NIC_COUNT;
         idx++) {
      string broadCastAddress =
          getNetworkInterfaceBroadcastAddress(ipList[idx]);
      // printf("idx = %d broadCastAddress
      // [%s]\n",idx,broadCastAddress.c_str());
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "idx = %d broadCastAddress [%s]\n", idx,
                                 broadCastAddress.c_str());

      // strcpy(subnetmask[idx], broadCastAddress.c_str());
      if (broadCastAddress != "" &&
          std::find(ipSubnetMaskList.begin(), ipSubnetMaskList.end(),
                    broadCastAddress) == ipSubnetMaskList.end()) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "NO Addresses found for broadCastAddress using INADDR_ANY\n");
  ...
  // for(unsigned int idx = 0; idx < ipList.size() && idx < MAX_NIC_COUNT;
  // idx++) {
  for (unsigned int idx = 0; idx < (unsigned int)ipSubnetMaskList.size();
       idx++) {
    ...
    if (ipSubnetMaskList[idx] == "*") {
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "UDP Socket broadcast using INADDR_ANY\n");
      bcLocal[idx].sin_addr.s_addr = INADDR_ANY;
    } else {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "UDP Socket broadcast using IP [%s]\n",
                                 ipSubnetMaskList[idx].c_str());
      bcLocal[idx].sin_addr.s_addr = inet_addr(
          ipSubnetMaskList[idx].c_str()); // htonl( INADDR_BROADCAST );
    }
    bcLocal[idx].sin_port = port; // We are letting the OS fill in the port
    ...
    if (bcfd[idx] <= 0) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "Unable to allocate broadcast socket [%s]: %s\n",
            ipSubnetMaskList[idx].c_str(),
            Socket::getLastSocketErrorFormattedText().c_str());
      // printf("Unable to allocate broadcast socket [%s]: %s\n",
      // ipSubnetMaskList[idx].c_str(),
      // Socket::getLastSocketErrorFormattedText().c_str()); exit(-1);
    }
    // Mark the socket for broadcast.
    else if (setsockopt(bcfd[idx], SOL_SOCKET, SO_BROADCAST, (const char *)&one,
                        sizeof(int)) < 0) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "Could not set socket to broadcast [%s]: %s\n",
            ipSubnetMaskList[idx].c_str(),
            Socket::getLastSocketErrorFormattedText().c_str());
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] setting up broadcast on address [%s]\n",
  ...
  for (pn = 1; getQuitStatus() == false; pn++) {
    for (unsigned int idx = 0;
         getQuitStatus() == false && idx < ipSubnetMaskList.size(); idx++) {
      if (Socket::isSocketValid(&bcfd[idx]) == true) {
        try {
          // Send this machine's host name and address in hostname:n.n.n.n
          // format
          ...
          for (unsigned int idx1 = 0; idx1 < ipList.size(); idx1++) {
            ...
            //						strcat(buff,":");
            //						string port_string =
            // intToStr(this->boundPort); #ifdef WIN32
            ...
            snprintf(buff, 1024, "%s:%s:%d", buffCopy.c_str(),
                     ipList[idx1].c_str(), this->boundPort);
          ...
          if (difftime((long int)time(NULL), elapsed) >= 1 &&
            ...
            if (pauseBroadCast == false) {
              ...
              // if( sendto( bcfd, buff, sizeof(buff) + 1, 0 , (struct sockaddr
              // *)&bcaddr, sizeof(struct sockaddr_in) ) != sizeof(buff) + 1 )

              if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                      .enabled)
                SystemFlags::OutputDebug(
                    SystemFlags::debugNetwork,
                    "Server is sending broadcast data [%s]\n", buff);
              // printf("Broadcasting index host info [%s]\n", buff);

              ssize_t send_res = sendto(bcfd[idx], buff, buffMaxSize, 0,
                                        (struct sockaddr *)&bcLocal[idx],
              ...
              if (send_res != buffMaxSize) {
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                  ...
                  SystemFlags::OutputDebug(
                      SystemFlags::debugNetwork,
                      "Broadcasting on port [%d] the message: [%s]\n",
                      Socket::getBroadCastPort(), buff);
              }
            }
            // printf("Broadcasting server send_res = %d buff [%s] ip [%s]
            // getPauseBroadcast() =
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s Line: %d] Broadcast thread is exiting\n", __FILE__,
        __FUNCTION__, __LINE__);
}
  ...
  /*
          ...
          #error "Your compiler needs to support popen!"
                  ...
                  if (ping != NULL){
                          ...
                          int bufferPos = 0;
                          for(;feof(ping) == false;) {
                                  char *data = fgets(&buf[bufferPos], 256,
  ping); bufferPos = strlen(buf);
                          }
  #ifdef WIN32
                          _pclose(ping);
  #elif defined(__GNUC__)
...
std::string Socket::getIpAddress() { return ipAddress; }
...
void ServerSocket::addIPAddressToBlockedList(string value) {
  if (isIPAddressBlocked(value) == false) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugError).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugError,
          "In [%s::%s Line: %d] Blocked IP Address [%s].\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, value.c_str());
  }
}
bool ServerSocket::isIPAddressBlocked(string value) const {
  bool result = (std::find(blockIPList.begin(), blockIPList.end(), value) !=
                 blockIPList.end());
  return result;
}

void ServerSocket::removeBlockedIPAddress(string value) {
  vector<string>::iterator iterFind =
      std::find(blockIPList.begin(), blockIPList.end(), value);
  if (iterFind != blockIPList.end()) {
    blockIPList.erase(iterFind);
  }
}

void ServerSocket::clearBlockedIPAddress() { blockIPList.clear(); }

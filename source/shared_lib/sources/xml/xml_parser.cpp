...
XmlIo::getRootDOMDocument(const string &path, DOMBuilder *parser,
                          bool noValidation) {
...
#else
XERCES_CPP_NAMESPACE_QUALIFIER DOMDocument *
XmlIo::getRootDOMDocument(const string &path, DOMLSParser *parser,
                          bool noValidation) {
  XERCES_CPP_NAMESPACE_QUALIFIER DOMDocument *document =
      parser->parseURI(path.c_str());
  return document;
...
const string
XmlAttribute::getRestrictedValue(string prefixValue,
                                 bool trimValueWithStartingSlash) const {
  if (skipRestrictionCheck == false && usesCommondata == false) {

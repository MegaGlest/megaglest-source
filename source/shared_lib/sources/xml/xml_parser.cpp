 ...
 public:
  virtual bool handleError(const DOMError &domError) {
    if (domError.getSeverity() == DOMError::DOM_SEVERITY_FATAL_ERROR) {
      ...
      XMLString::transcode(domError.getLocation()->getURI(), fileStr,
                           strSize - 1);
      Shared::Platform::uint64 lineNumber =
          domError.getLocation()->getLineNumber();
      throw megaglest_runtime_error(
          "Error parsing XML, file: " + string(fileStr) +
          ", line: " + intToStr(lineNumber) + ": " + string(msgStr));
    }
    return true;
  }
};

XmlIo::XmlIo() : parser(NULL) { init(); }

XmlIo::~XmlIo() { cleanup(); }

bool XmlIo::isInitialized() { return XmlIo::initialized; }
...
void XmlIo::init() {
  try {
    ...
    SystemFlags::OutputDebug(
        ...
        "In [%s::%s Line: %d] Error initializing XML system, msg %s\n",
  ...
  try {
    ...
    SystemFlags::OutputDebug(
        SystemFlags::debugError,
        "In [%s::%s Line: %d] Exception while creating XML parser, msg: %s\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        ex.getMessage());
...
XERCES_CPP_NAMESPACE_QUALIFIER DOMDocument *XmlIo::getRootDOMDocument(
    const string &path, DOMBuilder *parser, bool noValidation) {
...
#else
XERCES_CPP_NAMESPACE_QUALIFIER DOMDocument *XmlIo::getRootDOMDocument(
    const string &path, DOMLSParser *parser, bool noValidation) {
  XERCES_CPP_NAMESPACE_QUALIFIER DOMDocument *document =
      parser->parseURI(path.c_str());
  return document;
}
...
DOMNode *XmlIo::loadDOMNode(const string &path, bool noValidation) {
  ...
  try {
...
#if XERCES_VERSION_MAJOR < 3
    parser = (static_cast<DOMImplementationLS *>(implementation))
    ...
    // parser->parseURI(path.c_str());
    XERCES_CPP_NAMESPACE_QUALIFIER DOMDocument *document =
        getRootDOMDocument(path, parser, noValidation);
    ...
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] Exception while loading: [%s], msg:\n%s",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__, path.c_str(), XMLString::transcode(ex.msg));
...
XmlNode *XmlIo::load(const string &path,
                     ...
                     bool noValidation, bool skipStackTrace) {
  // printf("Load file using Xerces engine [%s]\n",path.c_str());
...
void XmlIo::save(const string &path, const XmlNode *node) {
  ...
  try {
    ...
    XERCES_CPP_NAMESPACE_QUALIFIER DOMDocument *document =
        implementation->createDocument(0, str, 0);
    ...
    for (unsigned int i = 0; i < node->getAttributeCount(); ++i) {
      ...
      XMLString::transcode(attr->getValue("", false).c_str(), strValue,
                           strSize - 1);
    ...
    SystemFlags::OutputDebug(
        SystemFlags::debugError,
        "In [%s::%s Line: %d] Exception while saving: [%s], %s\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        path.c_str(), XMLString::transcode(e.msg));
    throw megaglest_runtime_error("Exception while saving: " + path + ": " +
...
bool XmlIoRapid::isInitialized() { return XmlIoRapid::initialized; }
...
XmlIoRapid::~XmlIoRapid() { cleanup(); }

XmlNode *XmlIoRapid::load(
    const string &path, const std::map<string, string> &mapTagReplacementValues,
  ...
  Chrono chrono;
  chrono.start();
  if (SystemFlags::VERBOSE_MODE_ENABLED || showPerfStats)
  ...
  try {
    if (folderExists(path) == true) {
      throw megaglest_runtime_error(
          "Can not open file: [" + path + "] as it is a folder!", true);
    }
    ...
    if (showPerfStats)
      printf("In [%s::%s Line: %d] took msecs: " MG_I64_SPECIFIER "\n",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
    ...
    if (file_size <= 0) {
      throw megaglest_runtime_error("Invalid file size for file: [" + path +
                                    "] size = " + intToStr(file_size));
    }
    // printf("File size is: " MG_I64_SPECIFIER " for
    ...
    xml_document<> doc;
    doc.parse<parse_no_data_nodes | parse_validate_closing_tags>(
        &buffer.front());
  ...
  // if(SystemFlags::VERBOSE_MODE_ENABLED) printf("In [%s::%s Line: %d] took
  // msecs: %ld for file
  // [%s]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,chrono.getMillis(),path.c_str());
  // printf("In [%s::%s Line: %d] took msecs: %ld for file
  // [%s]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,chrono.getMillis(),path.c_str());
...
void XmlIoRapid::save(const string &path, const XmlNode *node) {
  try {
    ...
    decl->append_attribute(doc.allocate_attribute(
        doc.allocate_string("version"), doc.allocate_string("1.0")));
    decl->append_attribute(doc.allocate_attribute(
        doc.allocate_string("encoding"), doc.allocate_string("utf-8")));
    decl->append_attribute(doc.allocate_attribute(
        doc.allocate_string("standalone"), doc.allocate_string("no")));
    doc.append_node(decl);

    // root node
    xml_node<> *root = doc.allocate_node(
        node_element, doc.allocate_string(node->getName().c_str()));
    for (unsigned int i = 0; i < node->getAttributeCount(); ++i) {
      XmlAttribute *attr = node->getAttribute(i);
    ...
    //		// watch for name collisions here, print() is a very common
    // function name! 		print(std::back_inserter(xml_as_string), doc);
    //		// xml_as_string now contains the XML in string form, indented
    //		// (in all its angle bracket glory)
    //
    //		std::string xml_no_indent;
    //		// print_no_indenting is the only flag that print() knows about
    //		print(std::back_inserter(xml_no_indent), doc,
    // print_no_indenting);
...
XmlTree::XmlTree(xml_engine_parser_type engine_type) {
  ...
  switch (engine_type) {
    ...
    default:
      throw megaglest_runtime_error("Invalid XML parser engine: " +
                                    intToStr((int)engine_type));
  }
...
static string loadStackCacheName =
    string(__FILE__) + string("_loadStackCacheName");
...
void XmlTree::load(const string &path,
                   const std::map<string, string> &mapTagReplacementValues,
                   bool noValidation, bool skipStackCheck,
                   bool skipStackTrace) {
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("In [%s::%s Line: %d] about to load [%s] skipStackCheck = %d\n",
           extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
           __LINE__, path.c_str(), skipStackCheck);
  ...
  if (this->skipStackCheck == false) {
    ...
    LoadStack &loadStack =
        CacheManager::getCachedItem<LoadStack>(loadStackCacheName);
    Mutex &mutex = CacheManager::getMutexForItem<LoadStack>(loadStackCacheName);
    MutexSafeWrapper safeMutex(&mutex);

    for (LoadStack::iterator it = loadStack.begin(); it != loadStack.end();
         ++it) {
      if ((*it)->loadPath == path) {
  ...
  if (this->engine_type == XML_XERCES_ENGINE) {
    this->rootNode = XmlIo::getInstance().load(path, mapTagReplacementValues,
                                               noValidation, skipStackTrace);
  } else
#endif
  {
    this->rootNode = XmlIoRapid::getInstance().load(
        path, mapTagReplacementValues, noValidation, skipStackTrace,
        this->skipUpdatePathClimbingParts);
  }
...
std::shared_ptr<CallBack<void>> XmlTree::loadAsync(
    const string &path, const std::map<string, string> &mapTagReplacementValues,
    bool noValidation, bool skipStackCheck, bool skipStackTrace) {
  auto load = [this, path, mapTagReplacementValues, noValidation,
               skipStackCheck, skipStackTrace]() {
    this->load(path, mapTagReplacementValues, noValidation, skipStackCheck,
               skipStackTrace);
  };
    ...
    for (unsigned int i = 0; i < domAttributes->getLength(); ++i) {
      ...
      if (currentNode->getNodeType() == DOMNode::ATTRIBUTE_NODE) {
        XmlAttribute *xmlAttribute =
            new XmlAttribute(domAttributes->item(i), mapTagReplacementValues);
...
XmlNode::XmlNode(xml_node<> *node,
  ...
  // check children
  for (xml_node<> *currentNode = node->first_node(); currentNode;
  ...
  for (xml_attribute<> *attr = node->first_attribute(); attr;
    ...
    XmlAttribute *xmlAttribute =
        new XmlAttribute(attr, mapTagReplacementValues);
    attributes.push_back(xmlAttribute);
  }
  ...
  if (node->type() == node_element && children.size() == 0) {
    ...
    //			printf("\n----------------------\n** XML!! WILL REPLACE
    //[%s]\n",xmlText.c_str()); 			debugReplace = true;
    //		}
...
XmlNode::XmlNode(const string &name) : superNode(NULL) { this->name = name; }
...
XmlAttribute *XmlNode::getAttribute(unsigned int i) const {
  if (i >= attributes.size()) {
    throw megaglest_runtime_error(
        getName() + " node doesn't have " + uIntToStr(i) + " attributes", true);
  }
  return attributes[i];
}
...
XmlAttribute *XmlNode::getAttribute(const string &name, bool mustExist) const {
  ...
  if (mustExist == true) {
    throw megaglest_runtime_error(
        "\"" + getName() + "\" node doesn't have a attribute named \"" + name +
            "\"",
        true);
  }
...
XmlNode *XmlNode::getChild(unsigned int i) const {
  assert(!superNode);
  if (i >= children.size()) {
    throw megaglest_runtime_error("\"" + getName() + "\" node doesn't have " +
                                      uIntToStr(i + 1) + " children",
                                  true);
  }
  return children[i];
}
...
XmlNode *XmlNode::getChild(const string &childName, unsigned int i) const {
  ...
  if (i >= children.size()) {
    throw megaglest_runtime_error(
        "\"" + name + "\" node doesn't have " + uIntToStr(i + 1) +
            " children named \"" + childName + "\"\n\nTree: " + getTreeString(),
        true);
  }
  ...
  throw megaglest_runtime_error(
      "Node \"" + getName() + "\" doesn't have " + uIntToStr(i + 1) +
          " children named  \"" + childName + "\"\n\nTree: " + getTreeString(),
      true);
}
...
XmlNode *XmlNode::getChildWithAliases(vector<string> childNameList,
                                      unsigned int childIndex) const {
  for (int aliasIndex = 0; aliasIndex < (int)childNameList.size();
       ++aliasIndex) {
    ...
    if (childIndex >= children.size()) {
      throw megaglest_runtime_error("\"" + name + "\" node doesn't have " +
                                        intToStr(childIndex + 1) +
                                        " children named \"" + childName +
                                        "\"\n\nTree: " + getTreeString(),
                                    true);
    }
  ...
  throw megaglest_runtime_error(
      "Node \"" + getName() + "\" doesn't have " + intToStr(childIndex + 1) +
          " children named  \"" +
          (childNameList.empty() ? "???" : childNameList[0]) +
          "\"\n\nTree: " + getTreeString(),
      true);
}
...
bool XmlNode::hasChildAtIndex(const string &childName, int i) const {
  ...
  int count = 0;
  for (unsigned int j = 0; j < children.size(); ++j) {
    // printf("Looking for [%s] at index: %d found [%s] index =
...
bool XmlNode::hasChild(const string &childName) const {
  return hasChildNoSuper(childName) ||
         (superNode && superNode->hasChild(childName));
}
...
XmlAttribute *XmlNode::addAttribute(
...
DOMElement *XmlNode::buildElement(
    XERCES_CPP_NAMESPACE_QUALIFIER DOMDocument *document) const {
  XMLCh str[strSize];
...
xml_node<> *XmlNode::buildElement(xml_document<> *document) const {
  xml_node<> *node = document->allocate_node(
      node_element, document->allocate_string(name.c_str()));
...
XmlAttribute::XmlAttribute(
  ...
  value = str;
  usesCommondata = ((value.find("$COMMONDATAPATH") != string::npos) ||
                    (value.find("%%COMMONDATAPATH%%") != string::npos));
  skipRestrictionCheck =
      Properties::applyTagsToValue(this->value, &this->mapTagReplacementValues);
...
bool XmlAttribute::getBoolValue() const {
  if (value == "true") {
    ...
    throw megaglest_runtime_error(
        "Not a valid bool value (true or false): " + getName() + ": " + value,
        true);
  }
}

int XmlAttribute::getIntValue() const { return strToInt(value); }

uint32 XmlAttribute::getUIntValue() const { return strToUInt(value); }

int XmlAttribute::getIntValue(int min, int max) const {
  int i = strToInt(value);
  if (i < min || i > max) {
    throw megaglest_runtime_error(
        "Xml Attribute int out of range: " + getName() + ": " + value, true);
  }
  return i;
}

float XmlAttribute::getFloatValue() const { return strToFloat(value); }
...
float XmlAttribute::getFloatValue(float min, float max) const {
  ...
  if (f < min || f > max) {
    throw megaglest_runtime_error(
        "Xml attribute float out of range: " + getName() + ": " + value, true);
  }
  return f;
}

const string XmlAttribute::getValue(string prefixValue,
                                    bool trimValueWithStartingSlash) const {
  string result = value;
...
const string XmlAttribute::getRestrictedValue(
    string prefixValue, bool trimValueWithStartingSlash) const {
  if (skipRestrictionCheck == false && usesCommondata == false) {
      ...
      if (allowedCharacters.find(value[i]) == string::npos) {
        throw megaglest_runtime_error(
            string("The string \"" + value +
                ...
                value[i] +
                "\"\nFor portability reasons the only allowed characters in "
                "this field are: " +
                allowedCharacters,
            true);
      }
...
void XmlAttribute::setValue(string val) { value = val; }

    ...
    try {
        ...
        for (unsigned int i = 0; i < node->getAttributeCount(); ++i) {
            ...
            root->append_attribute(
                doc.allocate_attribute(doc.allocate_string(attr->getName().c_str()), doc.allocate_string(attr->getValue("", false).c_str())));
        }
...
std::shared_ptr<CallBack<void>> XmlTree::loadAsync(const string &path, const std::map<string, string> &mapTagReplacementValues, bool noValidation,
                                                   bool skipStackCheck, bool skipStackTrace) {
...
XmlNode::XmlNode(xml_node<> *node, const std::map<string, string> &mapTagReplacementValues, bool skipUpdatePathClimbingParts) : superNode(NULL) {
    ...
    // check children
    for (xml_node<> *currentNode = node->first_node(); currentNode; currentNode = currentNode->next_sibling()) {
...
XmlNode *XmlNode::getChild(const string &childName, unsigned int i) const {
    ...
    if (i >= children.size()) {
        throw megaglest_runtime_error(
            "\"" + name + "\" node doesn't have " + uIntToStr(i + 1) + " children named \"" + childName + "\"\n\nTree: " + getTreeString(), true);
    }
    ...
    throw megaglest_runtime_error(
        "Node \"" + getName() + "\" doesn't have " + uIntToStr(i + 1) + " children named  \"" + childName + "\"\n\nTree: " + getTreeString(), true);
}
...
XmlNode *XmlNode::getChildWithAliases(vector<string> childNameList, unsigned int childIndex) const {
    for (int aliasIndex = 0; aliasIndex < (int)childNameList.size(); ++aliasIndex) {
        ...
        if (childIndex >= children.size()) {
            throw megaglest_runtime_error(
                "\"" + name + "\" node doesn't have " + intToStr(childIndex + 1) + " children named \"" + childName + "\"\n\nTree: " + getTreeString(), true);
        }
    ...
    throw megaglest_runtime_error("Node \"" + getName() + "\" doesn't have " + intToStr(childIndex + 1) + " children named  \"" +
                                      (childNameList.empty() ? "???" : childNameList[0]) + "\"\n\nTree: " + getTreeString(),
                                  true);
}

bool XmlNode::hasChildAtIndex(const string &childName, int i) const {
    if (superNode && !hasChildNoSuper(childName)) return superNode->hasChildAtIndex(childName, i);
    int count = 0;
    for (unsigned int j = 0; j < children.size(); ++j) {
...
xml_node<> *XmlNode::buildElement(xml_document<> *document) const {
    ...
    for (unsigned int i = 0; i < attributes.size(); ++i) {
        node->append_attribute(document->allocate_attribute(document->allocate_string(attributes[i]->getName().c_str()),
    ...
    if (skipRestrictionCheck == false && usesCommondata == false) {
        ...
        for (unsigned int i = 0; i < value.size(); ++i) {
            if (allowedCharacters.find(value[i]) == string::npos) {
                throw megaglest_runtime_error(string("The string \"" + value + "\" contains a character that is not allowed: \"") + value[i] +

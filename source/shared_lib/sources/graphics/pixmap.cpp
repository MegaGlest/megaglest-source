...
void CalculatePixelsCRC(uint8 *pixels, std::size_t pixelByteCount,
                        Checksum &crc) {
  //	crc = Checksum();
...
PixmapIoTga::PixmapIoTga() { file = NULL; }
  ...
  try {
    ...
    if (bigEndianSystem == true) {
      fileHeader.bitsPerPixel =
          Shared::PlatformByteOrder::fromCommonEndian(fileHeader.bitsPerPixel);
      fileHeader.colourMapDepth = Shared::PlatformByteOrder::fromCommonEndian(
          fileHeader.colourMapDepth);
      fileHeader.colourMapLength = Shared::PlatformByteOrder::fromCommonEndian(
          fileHeader.colourMapDepth);
      fileHeader.colourMapOrigin = Shared::PlatformByteOrder::fromCommonEndian(
          fileHeader.colourMapOrigin);
      fileHeader.colourMapType =
          Shared::PlatformByteOrder::fromCommonEndian(fileHeader.colourMapType);
      fileHeader.dataTypeCode =
          Shared::PlatformByteOrder::fromCommonEndian(fileHeader.dataTypeCode);
      fileHeader.height =
          Shared::PlatformByteOrder::fromCommonEndian(fileHeader.height);
      fileHeader.idLength =
          Shared::PlatformByteOrder::fromCommonEndian(fileHeader.idLength);
      fileHeader.imageDescriptor = Shared::PlatformByteOrder::fromCommonEndian(
          fileHeader.imageDescriptor);
      fileHeader.width =
          Shared::PlatformByteOrder::fromCommonEndian(fileHeader.width);
    }
    ...
    if (fileHeader.dataTypeCode != tgaUncompressedRgb &&
        fileHeader.dataTypeCode != tgaUncompressedBw) {
      throw megaglest_runtime_error(
          path + ": only uncompressed BW and RGB targa images are supported",
          true);
    }

    // check bits per pixel
    if (fileHeader.bitsPerPixel != 8 && fileHeader.bitsPerPixel != 24 &&
        fileHeader.bitsPerPixel != 32) {
      throw megaglest_runtime_error(
          path + ": only 8, 24 and 32 bit targa images are supported", true);
    }
...
void PixmapIoTga::read(uint8 *pixels) { read(pixels, components); }
...
void PixmapIoTga::openWrite(const string &path, int w, int h, int components) {
  ...
  fileHeader.dataTypeCode =
      components == 1 ? tgaUncompressedBw : tgaUncompressedRgb;
  ...
  if (bigEndianSystem == true) {
    fileHeader.bitsPerPixel =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.bitsPerPixel);
    fileHeader.colourMapDepth =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.colourMapDepth);
    fileHeader.colourMapLength =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.colourMapDepth);
    fileHeader.colourMapOrigin =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.colourMapOrigin);
    fileHeader.colourMapType =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.colourMapType);
    fileHeader.dataTypeCode =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.dataTypeCode);
    fileHeader.height =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.height);
    fileHeader.idLength =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.idLength);
    fileHeader.imageDescriptor =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.imageDescriptor);
    fileHeader.width =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.width);
  }
...
void PixmapIoTga::write(uint8 *pixels) {
  if (components == 1) {
    ...
    static bool bigEndianSystem = Shared::PlatformByteOrder::isBigEndian();
    if (bigEndianSystem == true) {
      Shared::PlatformByteOrder::toEndianTypeArray<uint8>(pixels,
                                                          h * w * components);
    }
...
PixmapIoBmp::PixmapIoBmp() { file = NULL; }
...
void PixmapIoBmp::openRead(const string &path) {
  ...
  if (bigEndianSystem == true) {
    fileHeader.offsetBits =
        Shared::PlatformByteOrder::fromCommonEndian(fileHeader.offsetBits);
    fileHeader.reserved1 =
        Shared::PlatformByteOrder::fromCommonEndian(fileHeader.reserved1);
    fileHeader.reserved2 =
        Shared::PlatformByteOrder::fromCommonEndian(fileHeader.reserved2);
    fileHeader.size =
        Shared::PlatformByteOrder::fromCommonEndian(fileHeader.size);
    fileHeader.type1 =
        Shared::PlatformByteOrder::fromCommonEndian(fileHeader.type1);
    fileHeader.type2 =
        Shared::PlatformByteOrder::fromCommonEndian(fileHeader.type2);
  }
  ...
  if (bigEndianSystem == true) {
    infoHeader.bitCount =
        Shared::PlatformByteOrder::fromCommonEndian(infoHeader.bitCount);
    infoHeader.clrImportant =
        Shared::PlatformByteOrder::fromCommonEndian(infoHeader.clrImportant);
    infoHeader.clrUsed =
        Shared::PlatformByteOrder::fromCommonEndian(infoHeader.clrUsed);
    infoHeader.compression =
        Shared::PlatformByteOrder::fromCommonEndian(infoHeader.compression);
    infoHeader.height =
        Shared::PlatformByteOrder::fromCommonEndian(infoHeader.height);
    infoHeader.planes =
        Shared::PlatformByteOrder::fromCommonEndian(infoHeader.planes);
    infoHeader.size =
        Shared::PlatformByteOrder::fromCommonEndian(infoHeader.size);
    infoHeader.sizeImage =
        Shared::PlatformByteOrder::fromCommonEndian(infoHeader.sizeImage);
    infoHeader.width =
        Shared::PlatformByteOrder::fromCommonEndian(infoHeader.width);
    infoHeader.xPelsPerMeter =
        Shared::PlatformByteOrder::fromCommonEndian(infoHeader.xPelsPerMeter);
    infoHeader.yPelsPerMeter =
        Shared::PlatformByteOrder::fromCommonEndian(infoHeader.yPelsPerMeter);
  }
...
void PixmapIoBmp::read(uint8 *pixels) { read(pixels, 3); }
...
void PixmapIoBmp::openWrite(const string &path, int w, int h, int components) {
  ...
  fileHeader.size =
      sizeof(BitmapFileHeader) + sizeof(BitmapInfoHeader) + 3 * h * w;
  ...
  if (bigEndianSystem == true) {
    fileHeader.offsetBits =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.offsetBits);
    fileHeader.reserved1 =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.reserved1);
    fileHeader.reserved2 =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.reserved2);
    fileHeader.size =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.size);
    fileHeader.type1 =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.type1);
    fileHeader.type2 =
        Shared::PlatformByteOrder::toCommonEndian(fileHeader.type2);
  }
  ...
  if (bigEndianSystem == true) {
    infoHeader.bitCount =
        Shared::PlatformByteOrder::toCommonEndian(infoHeader.bitCount);
    infoHeader.clrImportant =
        Shared::PlatformByteOrder::toCommonEndian(infoHeader.clrImportant);
    infoHeader.clrUsed =
        Shared::PlatformByteOrder::toCommonEndian(infoHeader.clrUsed);
    infoHeader.compression =
        Shared::PlatformByteOrder::toCommonEndian(infoHeader.compression);
    infoHeader.height =
        Shared::PlatformByteOrder::toCommonEndian(infoHeader.height);
    infoHeader.planes =
        Shared::PlatformByteOrder::toCommonEndian(infoHeader.planes);
    infoHeader.size =
        Shared::PlatformByteOrder::toCommonEndian(infoHeader.size);
    infoHeader.sizeImage =
        Shared::PlatformByteOrder::toCommonEndian(infoHeader.sizeImage);
    infoHeader.width =
        Shared::PlatformByteOrder::toCommonEndian(infoHeader.width);
    infoHeader.xPelsPerMeter =
        Shared::PlatformByteOrder::toCommonEndian(infoHeader.xPelsPerMeter);
    infoHeader.yPelsPerMeter =
        Shared::PlatformByteOrder::toCommonEndian(infoHeader.yPelsPerMeter);
  }
...
PixmapIoPng::PixmapIoPng() { file = NULL; }
...
void PixmapIoPng::write(uint8 *pixels) {
  ...
  png_set_IHDR(imgp, infop, w, h, 8, color_type, PNG_INTERLACE_NONE,
...
PixmapIoJpg::PixmapIoJpg() { file = NULL; }
...
void PixmapIoJpg::write(uint8 *pixels) {
  ...
  if (pixels != NULL && flip != NULL) {
    ...
    while (cinfo.next_scanline < cinfo.image_height) {
      row_pointer[0] = &flip[cinfo.next_scanline * cinfo.image_width *
...
std::size_t Pixmap1D::getPixelByteCount() const { return (w * components); }
...
Pixmap1D::~Pixmap1D() { deletePixels(); }
...
void Pixmap2D::init(int w, int h, int components) {
  ...
  if (getPixelByteCount() <= 0 || (h <= 0 || w <= 0 || components <= 0)) {
    ...
    snprintf(
        szBuf, 8096,
        "Invalid pixmap dimensions for [%s], h = %d, w = %d, components = %d\n",
...
std::size_t Pixmap2D::getPixelByteCount() const { return (h * w * components); }
...
Pixmap2D::~Pixmap2D() { deletePixels(); }
...
void Pixmap2D::Scale(int format, int newW, int newH) {
  ...
  GLenum glErr = gluScaleImage(format, w, h, GL_UNSIGNED_BYTE, pixels, newW,
                               newH, GL_UNSIGNED_BYTE, newpixels);
...
Pixmap2D *Pixmap2D::loadPath(const string &path) {
  ...
  if (pixmap != NULL) {
    pixmap->path = path;
    CalculatePixelsCRC(pixmap->pixels, pixmap->getPixelByteCount(),
                       pixmap->crc);
  }
  return pixmap;
}
...
void Pixmap2D::getPixel(int x, int y, uint8 *value) const {
  for (int i = 0; i < components; ++i) {
    ...
    if (index >= getPixelByteCount()) {
      char szBuf[8096];
      snprintf(szBuf, 8096,
               "Invalid pixmap index: " MG_SIZE_T_SPECIFIER
               " for [%s], h = %d, w = %d, components = %d x = %d y = %d\n",
...
void Pixmap2D::setPixel(int x, int y, const uint8 *value, int arraySize) {
  if (arraySize > components) {
    char szBuf[8096];
    snprintf(szBuf, 8096,
             "Invalid pixmap arraySize: %d for [%s], h = %d, w = %d, "
             "components = %d x = %d y = %d\n",
...
void Pixmap2D::splat(const Pixmap2D *leftUp, const Pixmap2D *rightUp,
                     const Pixmap2D *leftDown, const Pixmap2D *rightDown) {

  RandomGen random;

  assert(components == 3 || components == 4);

  if (!doDimensionsAgree(leftUp) || !doDimensionsAgree(rightUp) ||
      !doDimensionsAgree(leftDown) || !doDimensionsAgree(rightDown)) {
    throw megaglest_runtime_error(
        "Pixmap2D::splat: pixmap dimensions don't agree");
  }
  ...
  for (int i = 0; i < w; ++i) {
    for (int j = 0; j < h; ++j) {
      ...
      float lu =
          distLu > avg ? 0 : ((avg - distLu)) * random.randRange(0.5f, 1.0f);
      float ru =
          distRu > avg ? 0 : ((avg - distRu)) * random.randRange(0.5f, 1.0f);
      float ld =
          distLd > avg ? 0 : ((avg - distLd)) * random.randRange(0.5f, 1.0f);
      float rd =
          distRd > avg ? 0 : ((avg - distRd)) * random.randRange(0.5f, 1.0f);

      float total = lu + ru + ld + rd;

      Vec4f pix =
          (leftUp->getPixel4f(i, j) * lu + rightUp->getPixel4f(i, j) * ru +
           leftDown->getPixel4f(i, j) * ld + rightDown->getPixel4f(i, j) * rd) *
          (1.0f / total);
...
void Pixmap2D::lerp(float t, const Pixmap2D *pixmap1, const Pixmap2D *pixmap2) {
  if (!doDimensionsAgree(pixmap1) || !doDimensionsAgree(pixmap2)) {
    throw megaglest_runtime_error(
        "Pixmap2D::lerp: pixmap dimensions don't agree");
  }

  for (int i = 0; i < w; ++i) {
    for (int j = 0; j < h; ++j) {
      setPixel(i, j,
               pixmap1->getPixel4f(i, j).lerp(t, pixmap2->getPixel4f(i, j)));
    }
  }
}
...
void Pixmap2D::copy(const Pixmap2D *sourcePixmap) {
  ...
  memcpy(pixels, sourcePixmap->getPixels(),
         w * h * sourcePixmap->getComponents());
...
Pixmap3D::~Pixmap3D() { deletePixels(); }
...
void Pixmap3D::loadSlicePng(const string &path, int slice) {
  ...
  // printf("Loading 3D pixmap PNG [%s] pixmap [%p] this

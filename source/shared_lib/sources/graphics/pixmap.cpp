...
void PixmapIoPng::write(uint8 *pixels) {
    ...
    png_set_IHDR(imgp, infop, w, h, 8, color_type, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
...
void Pixmap2D::Scale(int format, int newW, int newH) {
    ...
    GLenum glErr = gluScaleImage(format, w, h, GL_UNSIGNED_BYTE, pixels, newW, newH, GL_UNSIGNED_BYTE, newpixels);
...
void Pixmap2D::splat(const Pixmap2D *leftUp, const Pixmap2D *rightUp, const Pixmap2D *leftDown, const Pixmap2D *rightDown) {
    ...
    if (!doDimensionsAgree(leftUp) || !doDimensionsAgree(rightUp) || !doDimensionsAgree(leftDown) || !doDimensionsAgree(rightDown)) {
    ...
    for (int i = 0; i < w; ++i) {
        for (int j = 0; j < h; ++j) {
            ...
            Vec4f pix = (leftUp->getPixel4f(i, j) * lu + rightUp->getPixel4f(i, j) * ru + leftDown->getPixel4f(i, j) * ld + rightDown->getPixel4f(i, j) * rd) *
                        (1.0f / total);

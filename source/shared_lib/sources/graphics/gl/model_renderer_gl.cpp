...
void ModelRendererGl::begin(bool renderNormals, bool renderTextures,
                            bool renderColors, bool colorPickingMode,
...
void ModelRendererGl::render(Model *model, int renderMode) {
  ...
  // if(model->getIsStaticModel()) printf("In [%s::%s Line: %d] filename [%s] is
  // static about to
...
void ModelRendererGl::renderMesh(Mesh *mesh, int renderMode) {
  ...
  if (renderMode == rmSelection &&
      mesh->getNoSelect() == true) { // don't render this and do nothing
    return;
  }
  ...
  if (this->colorPickingMode == false) {
    ...
    // texture state
    const Texture2DGl *texture =
        static_cast<const Texture2DGl *>(mesh->getTexture(mtDiffuse));
    if (texture != NULL && renderTextures) {
      if (lastTexture != texture->getHandle()) {
        // assert(glIsTexture(texture->getHandle()));
        // throw megaglest_runtime_error("glIsTexture(texture->getHandle()) ==
        // false for texture: " + texture->getPath());
  ...
  if (getVBOSupported() == true && mesh->getFrameCount() == 1) {
    ...
    glVertexPointer(
        3, GL_FLOAT, 0,
        (char *)NULL); // Set The Vertex Pointer To The Vertex Buffer
    ...
    if (renderTextures && mesh->getTexture(mtDiffuse) != NULL) {
      if (duplicateTexCoords) {
        ...
        glTexCoordPointer(
            2, GL_FLOAT, 0,
            (char *)NULL); // Set The TexCoord Pointer To The TexCoord Buffer
                           // glBindBufferARB( GL_ARRAY_BUFFER_ARB, 0 );
      }
    ...
    glVertexPointer(3, GL_FLOAT, 0,
                    mesh->getInterpolationData()->getVertices());
  ...
  if (getVBOSupported() == true && mesh->getFrameCount() == 1) {
    ...
    glDrawRangeElements(GL_TRIANGLES, 0, vertexCount - 1, indexCount,
                        GL_UNSIGNED_INT, (char *)NULL);
    glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, 0);
    glBindBufferARB(GL_ARRAY_BUFFER_ARB, 0);

    // glDrawRangeElements(GL_TRIANGLES, 0, vertexCount-1, indexCount,
    // GL_UNSIGNED_INT, mesh->getIndices());
...
void ModelRendererGl::renderMeshNormals(Mesh *mesh) {
  if (getVBOSupported() == true && mesh->getFrameCount() == 1) {
    ...
    for (unsigned int i = 0; i < mesh->getIndexCount(); ++i) {
      const Vec3f &vertex =
          mesh->getInterpolationData()->getVertices()[mesh->getIndices()[i]];
      const Vec3f &normal =
          vertex +
          mesh->getInterpolationData()->getNormals()[mesh->getIndices()[i]];

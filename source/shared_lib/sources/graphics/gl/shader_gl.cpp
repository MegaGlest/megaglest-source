...
void ShaderProgramGl::attach(VertexShader *vertexShader,
...
bool ShaderProgramGl::link(string &messages) {
  ...
  for (unsigned int i = 0; i < attributes.size(); ++i) {
    ...
    glBindAttribLocationARB(handle, attributes[i].second,
                            attributes[i].first.c_str());
  }
...
void ShaderGl::load(const string &path) { source.load(path); }

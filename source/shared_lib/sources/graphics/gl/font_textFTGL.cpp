...
TextFTGL::TextFTGL(FontTextHandlerType type) : Text(type) {
    ...
    if (error != GL_NO_ERROR) {
        printf("\n[%s::%s] Line %d Error = %d [%s] for size = %u res = %d\n", __FILE__, __FUNCTION__, __LINE__, error, gluErrorString(error), defSize,
               TextFTGL::faceResolution);
        fflush(stdout);
    }
...
void TextFTGL::init(string fontName, string fontFamilyName, int fontSize) {
    ...
    GLenum error = glGetError();
    if (error != GL_NO_ERROR) {
        printf("\n[%s::%s] Line %d Error = %d [%s] for size = %d res = %d\n", __FILE__, __FUNCTION__, __LINE__, error, gluErrorString(error), fontSize,
               TextFTGL::faceResolution);
        fflush(stdout);
    }
    ...
    string preloadText = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ01234567890-=!@#$%^&"
                         "*()_+:\"{}[]/?.,<>\\';";

...
void BaseRenderer::renderMap(MapPreview *map, int x, int y, int clientW,
                             int clientH, int cellSize, bool grid,
                             bool heightMap, bool hideWater,
                             pair<int, int> *mouse_pos, int *radius) {
  float alt = 0;
  float showWater = 0;
  ...
  for (int j = 0; j < map->getH(); j++) {
    for (int i = 0; i < map->getW(); i++) {
      if (i * cellSize + x > -cellSize && i * cellSize + x < clientW &&
        ...
        if (map->getCliffLevel() >
            0) { // we maybe need to render cliff surfColor
          if (map->isCliff(i, j)) {
        ...
        if (!heightMap) {
          ...
          if (map->getObject(i, j) != 0 || isCliff) {
            ...
            glBegin(GL_POINTS);
            glVertex2i(i * cellSize + cellSize / 2,
                       clientH - j * cellSize - cellSize / 2);
            glEnd();
          }
          ...
          // left
          if (grid ||
              (i > 0 && map->getHeight(i - 1, j) > map->getHeight(i, j))) {
            glBegin(GL_LINES);
          ...
          // down
          if (grid ||
              (j > 0 && map->getHeight(i, j - 1) > map->getHeight(i, j))) {
            glBegin(GL_LINES);
            glVertex2i(i * cellSize, clientH - j * cellSize);
          ...
          if (map->getResource(i, j) != 0) {
            ...
            glVertex2i(i * cellSize + cellSize, clientH - j * cellSize);
            glVertex2i(i * cellSize, clientH - j * cellSize);
            glVertex2i(i * cellSize + cellSize,
  ...
  for (int i = 0; i < map->getMaxFactions(); i++) {
    ...
    glBegin(GL_LINES);
    glVertex2i((map->getStartLocationX(i) - 1) * cellSize,
               clientH - (map->getStartLocationY(i) - 1) * cellSize);
    glVertex2i((map->getStartLocationX(i) + 1) * cellSize + cellSize,
               clientH - (map->getStartLocationY(i) + 1) * cellSize - cellSize);
    glVertex2i((map->getStartLocationX(i) - 1) * cellSize,
               clientH - (map->getStartLocationY(i) + 1) * cellSize - cellSize);
    glVertex2i((map->getStartLocationX(i) + 1) * cellSize + cellSize,
               clientH - (map->getStartLocationY(i) - 1) * cellSize);

    ...
    case GL_COMPRESSED_RGBA_S3TC_DXT5_EXT:
        ...
        /*
                      ...
                      case GL_COMPRESSED_SRGB_ALPHA_BPTC_UNORM_ARB:
                              result =
         "GL_COMPRESSED_SRGB_ALPHA_BPTC_UNORM_ARB"; break;

                      case GL_COMPRESSED_RGB_BPTC_SIGNED_FLOAT_ARB:
                              result =
         "GL_COMPRESSED_RGB_BPTC_SIGNED_FLOAT_ARB"; break;

                      case GL_COMPRESSED_RGB_BPTC_UNSIGNED_FLOAT_ARB:
                              result =
         "GL_COMPRESSED_RGB_BPTC_UNSIGNED_FLOAT_ARB"; break;
      */
    ...
    case GL_COMPRESSED_RGBA_FXT1_3DFX:
        ...
        /*
                      ...
                      case GL_COMPRESSED_SIGNED_LUMINANCE_LATC1_EXT:
                              result =
         "GL_COMPRESSED_SIGNED_LUMINANCE_LATC1_EXT"; break;

                      case GL_COMPRESSED_LUMINANCE_ALPHA_LATC2_EXT:
                              result =
         "GL_COMPRESSED_LUMINANCE_ALPHA_LATC2_EXT"; break;

                      case GL_COMPRESSED_SIGNED_LUMINANCE_ALPHA_LATC2_EXT:
                              result =
         "GL_COMPRESSED_SIGNED_LUMINANCE_ALPHA_LATC2_EXT"; break;
                      ...
                      case GL_COMPRESSED_SIGNED_RED_GREEN_RGTC2_EXT:
                              result =
         "GL_COMPRESSED_SIGNED_RED_GREEN_RGTC2_EXT"; break;
      */
...
static std::vector<int> getSupportCompressedTextureFormats() {
    ...
    if (Texture::useTextureCompression == true) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                     "**** getSupportCompressedTextureFormats() "
                                     "result.size() = %d, count = %d\n",
                                     (int)result.size(), count);
        for (unsigned int i = 0; i < result.size(); ++i) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugSystem, "Texture Compression #i = %d, result[i] = %d [%s]\n", i, result[i],
                                         getSupportCompressedTextureFormatString(result[i]).c_str());
        }
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "------------------------------------------------\n");
    }
...
GLint toCompressionFormatGl(GLint format) {
    ...
    case GL_RGB:
    case GL_RGB8:
        if (std::find(supportedCompressionFormats.begin(), supportedCompressionFormats.end(), GL_COMPRESSED_RGB_S3TC_DXT1_EXT) !=
        ...
        // else
        // if(std::find(supportedCompressionFormats.begin(),supportedCompressionFormats.end(),GL_COMPRESSED_SRGB_S3TC_DXT1_EXT)
        // != supportedCompressionFormats.end()) { 	return
        // GL_COMPRESSED_SRGB_S3TC_DXT1_EXT;
        // }
        else if (std::find(supportedCompressionFormats.begin(), supportedCompressionFormats.end(), GL_COMPRESSED_RGB_FXT1_3DFX) !=
                 supportedCompressionFormats.end()) {
    ...
    case GL_RGBA8:
        ...
        if (std::find(supportedCompressionFormats.begin(), supportedCompressionFormats.end(), GL_COMPRESSED_RGBA_S3TC_DXT5_EXT) !=
            supportedCompressionFormats.end()) {
        ...
        // else
        // if(std::find(supportedCompressionFormats.begin(),supportedCompressionFormats.end(),GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT)
        // != supportedCompressionFormats.end()) { 	return
        ...
        // GL_COMPRESSED_SRGB_S3TC_DXT1_EXT;
        // }
        else if (std::find(supportedCompressionFormats.begin(), supportedCompressionFormats.end(), GL_COMPRESSED_RGBA_FXT1_3DFX) !=
                 supportedCompressionFormats.end()) {
...
void TextureGl::setup_FBO_RBO() {
    ...
    if (glGenFramebuffersEXT) {
        ...
        case GL_FRAMEBUFFER_COMPLETE_EXT:
            // SystemFlags::OutputDebug(SystemFlags::debugSystem,"FBO attachment
            // OK!\n");
            break;
        default:
            // SystemFlags::OutputDebug(SystemFlags::debugSystem,"FBO attachment
...
void Texture1DGl::init(Filter filter, int maxAnisotropy) {
    ...
    if (inited == false) {
        ...
        if (mipmap) {
            ...
            int error = gluBuild1DMipmaps(GL_TEXTURE_1D, glCompressionFormat, pixmap.getW(), glFormat, GL_UNSIGNED_BYTE, pixels);

            // Now try without compression if we tried compression
            if (error != 0 && glCompressionFormat != glInternalFormat) {
                int error2 = gluBuild1DMipmaps(GL_TEXTURE_1D, glInternalFormat, pixmap.getW(), glFormat, GL_UNSIGNED_BYTE, pixels);
            ...
            if (error != 0) {
                ...
                char szBuf[8096] = "";
                snprintf(szBuf, 8096,
                         "Error building texture 1D mipmaps, returned: %d [%s] for "
                         "[%s] w = %d, glCompressionFormat = %d",
                         error, errorString, pixmap.getPath().c_str(), pixmap.getW(), glCompressionFormat);
            ...
            glTexImage1D(GL_TEXTURE_1D, 0, glCompressionFormat, pixmap.getW(), 0, glFormat, GL_UNSIGNED_BYTE, pixels);

            GLint error = glGetError();

            // Now try without compression if we tried compression
            if (error != GL_NO_ERROR && glCompressionFormat != glInternalFormat) {
                glTexImage1D(GL_TEXTURE_1D, 0, glInternalFormat, pixmap.getW(), 0, glFormat, GL_UNSIGNED_BYTE, pixels);
            ...
            if (error != GL_NO_ERROR) {
                ...
                char szBuf[8096] = "";
                snprintf(szBuf, 8096,
                         "Error creating texture 1D, returned: %d [%s] (%X) [%s] w = "
                         "%d, glCompressionFormat = %d",
                         error, errorString, error, pixmap.getPath().c_str(), pixmap.getW(), glCompressionFormat);
                throw megaglest_runtime_error(szBuf);
            }
        }
        inited = true;
...
void Texture2DGl::init(Filter filter, int maxAnisotropy) {
    ...
    if (inited == false) {
      ...
      */
            ...
            if (!(count_bits_set(pixmap.getW()) == 1 && count_bits_set(pixmap.getH()) == 1) && TextureGl::enableATIHacks == true) {
                //	&& (!GLEW_ARB_texture_non_power_of_two ||
                //(globalRendering->atiHacks && nearest)) ) {
                if (SystemFlags::VERBOSE_MODE_ENABLED)
                    printf("\n\n\n**WARNING** Enabling ATI video card hacks, resizing "
                           "texture to power of two [%d x %d] to [%d x %d] components [%d] "
                           "path [%s]\n",
                           pixmap.getW(), pixmap.getH(), next_power_of_2(pixmap.getW()), next_power_of_2(pixmap.getH()), pixmap.getComponents(),
                           pixmap.getPath().c_str());

                pixmap.Scale(glFormat, next_power_of_2(pixmap.getW()), next_power_of_2(pixmap.getH()));
            }

            glTexImage2D(GL_TEXTURE_2D, 0, glCompressionFormat, pixmap.getW(), pixmap.getH(), 0, glFormat, GL_UNSIGNED_BYTE, pixels);

            GLint error = glGetError();

            // Now try without compression if we tried compression
            if (error != GL_NO_ERROR && glCompressionFormat != glInternalFormat) {
                glTexImage2D(GL_TEXTURE_2D, 0, glInternalFormat, pixmap.getW(), pixmap.getH(), 0, glFormat, GL_UNSIGNED_BYTE, pixels);
            ...
            if (error != GL_NO_ERROR) {
                int error3 = gluBuild2DMipmaps(GL_TEXTURE_2D, glCompressionFormat, pixmap.getW(), pixmap.getH(), glFormat, GL_UNSIGNED_BYTE, pixels);
                if (error3 == GL_NO_ERROR) {
                    error = GL_NO_ERROR;
                }
            }
            if (error != GL_NO_ERROR) {
                int error4 = gluBuild2DMipmaps(GL_TEXTURE_2D, glInternalFormat, pixmap.getW(), pixmap.getH(), glFormat, GL_UNSIGNED_BYTE, pixels);
            ...
            if (error != GL_NO_ERROR) {
                ...
                char szBuf[8096] = "";
                snprintf(szBuf, 8096,
                         "Error building texture 2D mipmaps [%s], returned: %d [%s] "
                         "for [%s] w = %d, h = %d, glCompressionFormat = %d",
                         this->path.c_str(), error, errorString, (pixmap.getPath() != "" ? pixmap.getPath().c_str() : this->path.c_str()), pixmap.getW(),
                         pixmap.getH(), glCompressionFormat);
                throw megaglest_runtime_error(szBuf);
            }
        } else {
            // build single texture
            ...
            if (error != GL_NO_ERROR && glCompressionFormat != glInternalFormat) {
                ...
                if (error2 == GL_NO_ERROR) {
                    ...
                    glTexImage2D(GL_TEXTURE_2D, 0, glInternalFormat, pot(pixmap.getW()), pot(pixmap.getH()), 0, glFormat, GL_UNSIGNED_BYTE, NULL);

                    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, pixmap.getW(), pixmap.getH(), glFormat, GL_UNSIGNED_BYTE, pixels);
            ...
            if (error != GL_NO_ERROR) {
                ...
                char szBuf[8096] = "";
                snprintf(szBuf, 8096,
                         "Error creating texture 2D path [%s], returned: %d [%s] (%X) "
                         "[%s] w = %d, h = %d, glInternalFormat = %d, glFormat = %d, "
                         "glCompressionFormat = %d",
                         this->path.c_str(), error, errorString, error, pixmap.getPath().c_str(), pixmap.getW(), pixmap.getH(), glInternalFormat, glFormat,
                         glCompressionFormat);
                throw megaglest_runtime_error(szBuf);
            }
        }
...
void Texture2DGl::end(bool deletePixelBuffer) {
    if (inited == true) {
        // printf("==> Deleting GL Texture [%s] handle =
...
void Texture3DGl::init(Filter filter, int maxAnisotropy) {
    ...
    if (inited == false) {
        ...
        glTexImage3D(GL_TEXTURE_3D, 0, glCompressionFormat, pixmap.getW(), pixmap.getH(), pixmap.getD(), 0, glFormat, GL_UNSIGNED_BYTE, pixels);

        GLint error = glGetError();

        // Now try without compression if we tried compression
        if (error != GL_NO_ERROR && glCompressionFormat != glInternalFormat) {
            glTexImage3D(GL_TEXTURE_3D, 0, glInternalFormat, pixmap.getW(), pixmap.getH(), pixmap.getD(), 0, glFormat, GL_UNSIGNED_BYTE, pixels);
            ...
            glTexImage3D(GL_TEXTURE_3D, 0, glInternalFormat, pot(pixmap.getW()), pot(pixmap.getH()), pot(pixmap.getD()), 0, glFormat, GL_UNSIGNED_BYTE, NULL);

            glTexSubImage3D(GL_TEXTURE_3D, 0, 0, 0, 0, pixmap.getW(), pixmap.getH(), pixmap.getD(), glFormat, GL_UNSIGNED_BYTE, pixels);
        ...
        if (error != GL_NO_ERROR) {
            ...
            char szBuf[8096] = "";
            snprintf(szBuf, 8096,
                     "Error creating texture 3D, returned: %d [%s] (%X) [%s] w = %d, "
                     "h = %d, d = %d, glCompressionFormat = %d",
                     error, errorString, error, pixmap.getPath().c_str(), pixmap.getW(), pixmap.getH(), pixmap.getD(), glCompressionFormat);
            throw megaglest_runtime_error(szBuf);
        }
        inited = true;
    ...
    if (inited == false) {
         ...
         *
                                        int error= gluBuild2DMipmaps(
                                                ...
                                                currentPixmap->getW(),
         currentPixmap->getH(), glFormat, GL_UNSIGNED_BYTE, pixels);

        */
                glTexParameteri(GL_TEXTURE_2D, GL_GENERATE_MIPMAP, GL_TRUE);

                if (!(count_bits_set(currentPixmap->getW()) == 1 && count_bits_set(currentPixmap->getH()) == 1) && TextureGl::enableATIHacks == true) {
                    //	&& (!GLEW_ARB_texture_non_power_of_two ||
                    //(globalRendering->atiHacks && nearest)) ) {
                    if (SystemFlags::VERBOSE_MODE_ENABLED)
                        printf("\n\n\n**WARNING** Enabling ATI video card hacks, resizing "
                               "texture to power of two [%d x %d] to [%d x %d] components "
                               "[%d] path [%s]\n",
                               currentPixmap->getW(), currentPixmap->getH(), next_power_of_2(currentPixmap->getW()), next_power_of_2(currentPixmap->getH()),
                               currentPixmap->getComponents(), currentPixmap->getPath().c_str());

                    currentPixmap->Scale(glFormat, next_power_of_2(currentPixmap->getW()), next_power_of_2(currentPixmap->getH()));
                }

                glTexImage2D(target, 0, glCompressionFormat, currentPixmap->getW(), currentPixmap->getH(), 0, glFormat, GL_UNSIGNED_BYTE, pixels);

                int error = glGetError();

                // Now try without compression if we tried compression
                if (error != GL_NO_ERROR && glCompressionFormat != glInternalFormat) {
                    glTexImage2D(target, 0, glInternalFormat, currentPixmap->getW(), currentPixmap->getH(), 0, glFormat, GL_UNSIGNED_BYTE, pixels);
                ...
                if (error != GL_NO_ERROR) {
                    int error3 =
                        gluBuild2DMipmaps(target, glCompressionFormat, currentPixmap->getW(), currentPixmap->getH(), glFormat, GL_UNSIGNED_BYTE, pixels);
                ...
                if (error != GL_NO_ERROR) {
                    int error4 = gluBuild2DMipmaps(target, glInternalFormat, currentPixmap->getW(), currentPixmap->getH(), glFormat, GL_UNSIGNED_BYTE, pixels);
                ...
                if (error != GL_NO_ERROR) {
                    // throw megaglest_runtime_error("Error building texture cube
                    // mipmaps");
                    const char *errorString = reinterpret_cast<const char *>(gluErrorString(error));
                    char szBuf[8096] = "";
                    snprintf(szBuf, 8096,
                             "Error building texture cube mipmaps, returned: %d [%s] for "
                             "[%s] w = %d, h = %d, glCompressionFormat = %d",
                             error, errorString, currentPixmap->getPath().c_str(), currentPixmap->getW(), currentPixmap->getH(), glCompressionFormat);
                    throw megaglest_runtime_error(szBuf);
                }
            } else {
                if (!(count_bits_set(currentPixmap->getW()) == 1 && count_bits_set(currentPixmap->getH()) == 1) && TextureGl::enableATIHacks == true) {
                ...
                if (error != GL_NO_ERROR && glCompressionFormat != glInternalFormat) {
                    ...
                    if (error2 == GL_NO_ERROR) {
                        ...
                        glTexImage2D(target, 0, glInternalFormat, pot(currentPixmap->getW()), pot(currentPixmap->getH()), 0, glFormat, GL_UNSIGNED_BYTE, NULL);

                        glTexSubImage2D(target, 0, 0, 0, currentPixmap->getW(), currentPixmap->getH(), glFormat, GL_UNSIGNED_BYTE, pixels);
                ...
                if (error != GL_NO_ERROR) {
                    ...
                    char szBuf[8096] = "";
                    snprintf(szBuf, 8096,
                             "Error creating texture cube, returned: %d [%s] (%X) [%s] w "
                             "= %d, h = %d, glCompressionFormat = %d",
                             error, errorString, error, currentPixmap->getPath().c_str(), currentPixmap->getW(), currentPixmap->getH(), glCompressionFormat);
                    throw megaglest_runtime_error(szBuf);
                }
            }
...
void TextureGl::OutputTextureDebugInfo(Texture::Format format, int components, const string path, std::size_t rawSize, GLenum texType) {
    if (Texture::useTextureCompression == true) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                     "**** Texture filename: [%s] format = %d components = %d, glFormat = "
                                     "%d, rawSize = %llu\n",
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                     "**** Texture image size in video RAM: %d "
                                     "[%.2f%%], error [%d] (%X)\n",

...
static void setupGLExtensionMethods() {
  ...
  if (setupExtensions == true) {
    ...
    if (isGlExtensionSupported("GL_EXT_framebuffer_object")) {
      ...
      glBindRenderbufferEXT = (PFNGLBINDRENDERBUFFEREXTPROC)wglGetProcAddress(
          "glBindRenderbufferEXT");
      glDeleteRenderbuffersEXT =
          (PFNGLDELETERENDERBUFFERSEXTPROC)wglGetProcAddress(
              "glDeleteRenderbuffersEXT");
      glGenRenderbuffersEXT = (PFNGLGENRENDERBUFFERSEXTPROC)wglGetProcAddress(
      ...
      glBindFramebufferEXT = (PFNGLBINDFRAMEBUFFEREXTPROC)wglGetProcAddress(
          "glBindFramebufferEXT");
      glDeleteFramebuffersEXT =
          (PFNGLDELETEFRAMEBUFFERSEXTPROC)wglGetProcAddress(
              "glDeleteFramebuffersEXT");
      glGenFramebuffersEXT = (PFNGLGENFRAMEBUFFERSEXTPROC)wglGetProcAddress(
      ...
      if (!glBindRenderbufferEXT || !glDeleteRenderbuffersEXT ||
  ...
  if (Texture::useTextureCompression == true) {
    ...
    for (unsigned int i = 0; i < result.size(); ++i) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugSystem,
            "Texture Compression #i = %d, result[i] = %d [%s]\n", i, result[i],
...
GLint toCompressionFormatGl(GLint format) {
  ...
  static std::vector<int> supportedCompressionFormats =
      getSupportCompressedTextureFormats();
  ...
  switch (format) {
    ...
    case GL_RGB:
    case GL_RGB8:
      if (std::find(supportedCompressionFormats.begin(),
                    supportedCompressionFormats.end(),
                    GL_COMPRESSED_RGB_S3TC_DXT1_EXT) !=
          supportedCompressionFormats.end()) {
        return GL_COMPRESSED_RGB_S3TC_DXT1_EXT;
      } else if (std::find(supportedCompressionFormats.begin(),
                           supportedCompressionFormats.end(),
                           GL_COMPRESSED_RGB) !=
                 supportedCompressionFormats.end()) {
...
bool TextureGl::supports_FBO_RBO() { return (glGenFramebuffersEXT != NULL); }

void TextureGl::setup_FBO_RBO() {
  if (getTextureWidth() < 0 || getTextureHeight() < 0) {
    throw megaglest_runtime_error(
        "getTextureWidth() < 0 || getTextureHeight() < 0");
  }
...
bool TextureGl::checkFrameBufferStatus() {
  ...
  if (glGenFramebuffersEXT) {
    ...
    if (status != GL_FRAMEBUFFER_COMPLETE_EXT) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                 "checkFrameBufferStatus() status = %d [%X]\n",
                                 status, status);
      return false;
    }
    return true;
...
void Texture1DGl::init(Filter filter, int maxAnisotropy) {
  ...
  if (inited == false) {
    ...
    if (forceCompressionDisabled == true ||
        (pixmap.getPixelByteCount() > 0 &&
         pixmap.getPixelByteCount() <= MIN_BYTES_TO_COMPRESS)) {
    ...
    if (isGlExtensionSupported("GL_EXT_texture_filter_anisotropic")) {
      glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_MAX_ANISOTROPY_EXT,
                      maxAnisotropy);
    }

    if (mipmap) {
      GLuint glFilter = filter == fTrilinear ? GL_LINEAR_MIPMAP_LINEAR
      ...
      int error =
          gluBuild1DMipmaps(GL_TEXTURE_1D, glCompressionFormat, pixmap.getW(),
                            glFormat, GL_UNSIGNED_BYTE, pixels);

      // Now try without compression if we tried compression
      if (error != 0 && glCompressionFormat != glInternalFormat) {
        int error2 =
            gluBuild1DMipmaps(GL_TEXTURE_1D, glInternalFormat, pixmap.getW(),
                              glFormat, GL_UNSIGNED_BYTE, pixels);
      ...
      if (error != 0) {
        ...
        snprintf(szBuf, 8096,
                 ...
                 error, errorString, pixmap.getPath().c_str(), pixmap.getW(),
                 glCompressionFormat);
      ...
      glTexImage1D(GL_TEXTURE_1D, 0, glCompressionFormat, pixmap.getW(), 0,
                   glFormat, GL_UNSIGNED_BYTE, pixels);
      ...
      if (error != GL_NO_ERROR && glCompressionFormat != glInternalFormat) {
        glTexImage1D(GL_TEXTURE_1D, 0, glInternalFormat, pixmap.getW(), 0,
                     glFormat, GL_UNSIGNED_BYTE, pixels);
      ...
      if (error != GL_NO_ERROR) {
        ...
        snprintf(szBuf, 8096,
                 ...
                 error, errorString, error, pixmap.getPath().c_str(),
                 pixmap.getW(), glCompressionFormat);
        throw megaglest_runtime_error(szBuf);
      }
    }
    inited = true;
    OutputTextureDebugInfo(format, pixmap.getComponents(), getPath(),
                           pixmap.getPixelByteCount(), GL_TEXTURE_1D);
  }
  ...
  if (inited == false) {
    ...
    if (mipmap) {
      ...
      */
      ...
      if (!(count_bits_set(pixmap.getW()) == 1 &&
            count_bits_set(pixmap.getH()) == 1) &&
        ...
        if (SystemFlags::VERBOSE_MODE_ENABLED)
          printf(
              ...
              "path [%s]\n",
              pixmap.getW(), pixmap.getH(), next_power_of_2(pixmap.getW()),
              next_power_of_2(pixmap.getH()), pixmap.getComponents(),
              pixmap.getPath().c_str());

        pixmap.Scale(glFormat, next_power_of_2(pixmap.getW()),
                     next_power_of_2(pixmap.getH()));
      }
      ...
      if (error != GL_NO_ERROR) {
        int error3 = gluBuild2DMipmaps(GL_TEXTURE_2D, glCompressionFormat,
                                       pixmap.getW(), pixmap.getH(), glFormat,
      ...
      if (error != GL_NO_ERROR) {
        int error4 = gluBuild2DMipmaps(GL_TEXTURE_2D, glInternalFormat,
                                       pixmap.getW(), pixmap.getH(), glFormat,
                                       GL_UNSIGNED_BYTE, pixels);
      ...
      if (error != GL_NO_ERROR) {
        ...
        snprintf(szBuf, 8096,
                 ...
                 this->path.c_str(), error, errorString,
                 (pixmap.getPath() != "" ? pixmap.getPath().c_str()
                                         : this->path.c_str()),
                 pixmap.getW(), pixmap.getH(), glCompressionFormat);
        throw megaglest_runtime_error(szBuf);
      }
    } else {
      // build single texture
      ...
      if (error != GL_NO_ERROR) {
        ...
        snprintf(szBuf, 8096,
                 ...
                 this->path.c_str(), error, errorString, error,
                 pixmap.getPath().c_str(), pixmap.getW(), pixmap.getH(),
                 glInternalFormat, glFormat, glCompressionFormat);
        throw megaglest_runtime_error(szBuf);
      }
    }
    inited = true;
    OutputTextureDebugInfo(format, pixmap.getComponents(), getPath(),
                           pixmap.getPixelByteCount(), GL_TEXTURE_2D);
  }
...
void Texture3DGl::init(Filter filter, int maxAnisotropy) {
  ...
  if (inited == false) {
    ...
    glTexImage3D(GL_TEXTURE_3D, 0, glCompressionFormat, pixmap.getW(),
                 pixmap.getH(), pixmap.getD(), 0, glFormat, GL_UNSIGNED_BYTE,
                 pixels);
    ...
    // Now try without compression if we tried compression
    if (error != GL_NO_ERROR && glCompressionFormat != glInternalFormat) {
      glTexImage3D(GL_TEXTURE_3D, 0, glInternalFormat, pixmap.getW(),
                   pixmap.getH(), pixmap.getD(), 0, glFormat, GL_UNSIGNED_BYTE,
                   pixels);
      ...
      glTexImage3D(GL_TEXTURE_3D, 0, glInternalFormat, pot(pixmap.getW()),
                   pot(pixmap.getH()), pot(pixmap.getD()), 0, glFormat,
                   GL_UNSIGNED_BYTE, NULL);

      glTexSubImage3D(GL_TEXTURE_3D, 0, 0, 0, 0, pixmap.getW(), pixmap.getH(),
                      pixmap.getD(), glFormat, GL_UNSIGNED_BYTE, pixels);
    ...
    OutputTextureDebugInfo(format, pixmap.getComponents(), getPath(),
                           pixmap.getPixelByteCount(), GL_TEXTURE_3D);
  }
...
TextureCubeGl::~TextureCubeGl() { end(); }
  ...
  if (inited == false) {
    ...
    for (int i = 0; i < 6; ++i) {
      ...
      GLint glInternalFormat =
          toInternalFormatGl(format, currentPixmap->getComponents());
      GLint glCompressionFormat = toCompressionFormatGl(glInternalFormat);
      if (forceCompressionDisabled == true ||
          (currentPixmap->getPixelByteCount() > 0 &&
           currentPixmap->getPixelByteCount() <= MIN_BYTES_TO_COMPRESS)) {
        glCompressionFormat = glInternalFormat;
      }
      ...
      if (mipmap) {
        ...
        */
        ...
        if (!(count_bits_set(currentPixmap->getW()) == 1 &&
              count_bits_set(currentPixmap->getH()) == 1) &&
            TextureGl::enableATIHacks == true) {
          //	&& (!GLEW_ARB_texture_non_power_of_two ||
          //(globalRendering->atiHacks && nearest)) ) {
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf(
                ...
                next_power_of_2(currentPixmap->getW()),
                next_power_of_2(currentPixmap->getH()),
                currentPixmap->getComponents(),
                currentPixmap->getPath().c_str());

          currentPixmap->Scale(glFormat, next_power_of_2(currentPixmap->getW()),
                               next_power_of_2(currentPixmap->getH()));
        }

        glTexImage2D(target, 0, glCompressionFormat, currentPixmap->getW(),
                     currentPixmap->getH(), 0, glFormat, GL_UNSIGNED_BYTE,
                     pixels);
        ...
        // Now try without compression if we tried compression
        if (error != GL_NO_ERROR && glCompressionFormat != glInternalFormat) {
          glTexImage2D(target, 0, glInternalFormat, currentPixmap->getW(),
                       currentPixmap->getH(), 0, glFormat, GL_UNSIGNED_BYTE,
                       pixels);
        ...
        if (error != GL_NO_ERROR) {
          int error3 = gluBuild2DMipmaps(
              target, glCompressionFormat, currentPixmap->getW(),
              currentPixmap->getH(), glFormat, GL_UNSIGNED_BYTE, pixels);
        ...
        if (error != GL_NO_ERROR) {
          int error4 = gluBuild2DMipmaps(
              target, glInternalFormat, currentPixmap->getW(),
              currentPixmap->getH(), glFormat, GL_UNSIGNED_BYTE, pixels);
        ...
        if (error != GL_NO_ERROR) {
          ...
          snprintf(szBuf, 8096,
                   ...
                   error, errorString, currentPixmap->getPath().c_str(),
                   currentPixmap->getW(), currentPixmap->getH(),
                   glCompressionFormat);
          throw megaglest_runtime_error(szBuf);
        }
        ...
        if (error != GL_NO_ERROR && glCompressionFormat != glInternalFormat) {
          ...
          if (error2 == GL_NO_ERROR) {
            ...
            glTexImage2D(target, 0, glInternalFormat,
                         pot(currentPixmap->getW()), pot(currentPixmap->getH()),
                         0, glFormat, GL_UNSIGNED_BYTE, NULL);

            glTexSubImage2D(target, 0, 0, 0, currentPixmap->getW(),
                            currentPixmap->getH(), glFormat, GL_UNSIGNED_BYTE,
                            pixels);
        ...
        if (error != GL_NO_ERROR) {
          ...
          snprintf(szBuf, 8096,
                   ...
                   error, errorString, error, currentPixmap->getPath().c_str(),
                   currentPixmap->getW(), currentPixmap->getH(),
                   glCompressionFormat);
          throw megaglest_runtime_error(szBuf);
        }
      ...
      OutputTextureDebugInfo(format, currentPixmap->getComponents(), getPath(),
                             currentPixmap->getPixelByteCount(), target);
    }
    inited = true;
  }
...
void TextureGl::OutputTextureDebugInfo(Texture::Format format, int components,
  ...
  if (Texture::useTextureCompression == true) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugSystem,
          "**** Texture compressed status: %d, error [%d] (%X)\n", compressed,
          error, error);

    bool isCompressed = (compressed == 1);
    compressed = 0;
    glGetTexLevelParameteriv(texType, 0, GL_TEXTURE_COMPRESSED_IMAGE_SIZE,
                             &compressed);
    error = glGetError();
    ...
    compressed = 0;
    glGetTexLevelParameteriv(texType, 0, GL_TEXTURE_INTERNAL_FORMAT,
                             &compressed);
    error = glGetError();
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugSystem,
          "**** Texture image compression format used: %d, error [%d] (%X)\n",

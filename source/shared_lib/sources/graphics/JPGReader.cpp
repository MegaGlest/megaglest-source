...
Pixmap2D* JPGReader::read(ifstream& is, const string& path,
                          Pixmap2D* ret) const {
  ...
  if (bigEndianSystem == true) {
    Shared::PlatformByteOrder::fromEndianTypeArray<uint8>(buffer,
                                                          (size_t)length);
  }
  if (length < 2) {
  ...
  //  Proper header check found from:
  //  http://www.fastgraph.com/help/jpeg_header_format.html
  if (buffer[0] != 0xFF || buffer[1] != 0xD8) {
    std::cout << "0 = [" << std::hex << (int)buffer[0] << "] 1 = [" << std::hex
  ...
  if (setjmp(error_buffer)) {  // Longjump was called --> an exception was
                               // thrown
    delete[] buffer;
  ...
  std::cout << "Image width and height: " << cinfo.image_width <<" pixels and "
  << cinfo.image_height <<" pixels." << std::endl;

  std::cout << "Color components per fixel: " << cinfo.num_components <<
  std::endl; std::cout << "Color space: " << cinfo.jpeg_color_space <<
  std::endl;*/
  int picComponents = (ret->getComponents() == -1) ? cinfo.num_components
                                                   : ret->getComponents();
  // std::cout << "JPG-Components: Pic: " << picComponents << " old: " <<
  // (ret->getComponents()) << " File: " << cinfo.num_components << std::endl;
  // picComponents = 4;
  ...
  // std::cout << "output width and height: " << cinfo.output_width <<" pixels
  // and " << cinfo.output_height <<" pixels." << std::endl;
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
    ...
    // Current pixel
    size_t location = (cinfo.output_height - cinfo.output_scanline) *
                      cinfo.output_width * picComponents;
    if (picComponents == cinfo.num_components) {
      memcpy(pixels + location, row_pointer[0],
             cinfo.output_width * cinfo.num_components);
    } else {
      int r, g, b, a, l;
      for (unsigned int xPic = 0, xFile = 0;
           xPic < cinfo.output_width * picComponents;
           xPic += picComponents, xFile += cinfo.num_components) {
  ...
  { if (i%39 == 0) std::cout << std::endl; int first = pixels[i]/16; if (first <
  10) std:: cout << first; else std::cout << (char)('A'+(first-10)); first =
  pixels[i]%16; if (first < 10) std:: cout << first; else std::cout <<
  (char)('A'+(first-10)); std::cout << " ";
  }*/

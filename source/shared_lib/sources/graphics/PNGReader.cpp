...
static void user_read_data(png_structp read_ptr, png_bytep data,
                           png_size_t length) {
...
// static void user_write_data(png_structp png_ptr, png_bytep data, png_size_t
// length) {}
...
Pixmap2D *PNGReader::read(ifstream &is, const string &path,
                          Pixmap2D *ret) const {
  ...
  if (png_sig_cmp(buffer, 0, 8) != 0) {
    delete[] buffer;
    // This is not a PNG file - could be used for fast checking whether file is
    // supported or not
  ...
  size_t picComponents =
      (ret->getComponents() == -1) ? fileComponents : ret->getComponents();
  // std::cout << "PNG-Components: Pic: " << picComponents << " old: " <<
  ...
  size_t location = 0;
  for (int y = height - 1; y >= 0; --y) { // you have to somehow invert the
                                          // lines
    if (picComponents == fileComponents) {
      memcpy(pixels + location, row_pointers[y], rowbytes);
    } else {
      int r, g, b, a, l;
      for (size_t xPic = 0, xFile = 0; xFile < rowbytes;
           xPic += picComponents, xFile += fileComponents) {
...
Pixmap3D *PNGReader3D::read(ifstream &is, const string &path,
                            Pixmap3D *ret) const {
  // Read file
  is.seekg(0, ios::end);
  // size_t length = is.tellg();
  ...
  // picComponents = 4;
  // Copy image
  // printf("pixmap3d loading path [%s] w = %d h = %d d = %d comp =

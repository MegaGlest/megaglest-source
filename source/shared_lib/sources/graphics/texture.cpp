...
std::pair<SDL_Surface *, unsigned char *> Texture2D::CreateSDLSurface(bool newPixelData) const {
    ...
    result.first = SDL_CreateRGBSurfaceFrom(surfData, pixmap.getW(), pixmap.getH(), 8 * pixmap.getComponents(), pixmap.getW() * pixmap.getComponents(),
                                            0x000000FF, 0x0000FF00, 0x00FF0000, (pixmap.getComponents() == 4) ? 0xFF000000 : 0);

...
std::pair<SDL_Surface *, unsigned char *>
Texture2D::CreateSDLSurface(bool newPixelData) const {
  ...
  if (newPixelData == true) {
    // copy pixel data
    surfData = new unsigned char[pixmap.getW() * pixmap.getH() *
                                 pixmap.getComponents()];
    memcpy(surfData, pixmap.getPixels(),
           pixmap.getW() * pixmap.getH() * pixmap.getComponents());
  } else {
  ...
  result.first = SDL_CreateRGBSurfaceFrom(
      surfData, pixmap.getW(), pixmap.getH(), 8 * pixmap.getComponents(),
      pixmap.getW() * pixmap.getComponents(), 0x000000FF, 0x0000FF00,
      0x00FF0000, (pixmap.getComponents() == 4) ? 0xFF000000 : 0);
  if ((result.first == NULL) && newPixelData == true) {
    // cleanup when we failed to the create surface
    delete[] surfData;
    result.second = NULL;
  } else {
    //	SDL_Surface *prepGLTexture(SDL_Surface *surface, GLfloat *texCoords =
    // NULL, const bool 	freeSource = false) {
    ...
    //	        if (texCoords != 0) {
    //	                texCoords[0] = 0.0f; /* Min 	X */ texCoords[1] =
    //0.0f; /*
    // Min 	Y */ 	                texCoords[2] = (GLfloat)surface->w / w; /* Max
    // X */ 	                texCoords[3] = (GLfloat)surface->h / h; /* Max Y
    //*/
    //	        }
    ...
    //	        Uint32 savedFlags =
    // result.first->flags&(SDL_SRCALPHA|SDL_RLEACCELOK); 	        Uint8
    // savedAlpha =
    // result.first->format->alpha; 	        if ( (savedFlags & SDL_SRCALPHA)
    // == SDL_SRCALPHA ) { 	                SDL_SetAlpha(result.first, 0,
    // 0);
    //	        }

...
std::pair<SDL_Surface *, unsigned char *> Texture2D::CreateSDLSurface(
  ...
  if ((result.first == NULL) && newPixelData == true) {
    ...
    // 0.0f; /* Min 	Y */ 	                texCoords[2] =
    // (GLfloat)surface->w / w; /* Max X */ 	                texCoords[3] =
    //(GLfloat)surface->h / h; /* Max Y */
    //	        }
    ...
    // savedAlpha = result.first->format->alpha; 	        if ( (savedFlags
    // & SDL_SRCALPHA) == SDL_SRCALPHA ) { SDL_SetAlpha(result.first, 0, 0);
    //	        }

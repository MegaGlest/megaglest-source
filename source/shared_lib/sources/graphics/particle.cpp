...
void ParticleSystem::loadGame(const XmlNode *rootNode) {
  ...
  // particleSystemNode->getChildList("Particle"); 	for(unsigned int i = 0; i
  // < particleNodeList.size(); ++i) { 		XmlNode *node =
  // particleNodeList[i];
  //
...
GameParticleSystem::GameParticleSystem(int particleCount)
    : ParticleSystem(particleCount), primitive(pQuad), model(NULL),
      modelCycle(0.0f), offset(0.0f), direction(0.0f, 1.0f, 0.0f), tween(0.0f) {
}
...
GameParticleSystem::Primitive
GameParticleSystem::strToPrimitive(const string &str) {
  if (str == "quad") {
    return pQuad;
  } else if (str == "line") {
    return pLine;
...
void UnitParticleSystem::updateParticle(Particle *p) {
  ...
  if (alternations > 0) {
    ...
    energyRatio = clamp(static_cast<float>(p->energy) /
                            static_cast<float>(maxParticleEnergy),
                        0.f, 1.f);
  }
...
ProjectileParticleSystem::Trajectory
ProjectileParticleSystem::strToTrajectory(const string &str) {
  if (str == "linear") {
    return tLinear;

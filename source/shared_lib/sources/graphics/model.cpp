...
void Mesh::loadV2(int meshIndex, const string &dir, FILE *f, TextureManager *textureManager, bool deletePixMapAfterLoad,
                  std::map<string, vector<pair<string, string>>> *loadedFileList, string sourceLoader, string modelFile) {
    ...
    if (meshHeader.normalFrameCount != meshHeader.vertexFrameCount) {
        ...
        snprintf(szBuf, 8096,
                 ...
                 meshHeader.vertexFrameCount, meshHeader.normalFrameCount, meshIndex, modelFile.c_str());
    ...
    if (meshHeader.hasTexture && textureManager != NULL) {
        ...
        if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] v2 model texture [%s] meshIndex = %d modelFile "
                   "[%s]\n",
...
void Mesh::loadV3(int meshIndex, const string &dir, FILE *f, TextureManager *textureManager, bool deletePixMapAfterLoad,
    ...
    if ((meshHeader.properties & mp3NoTexture) != mp3NoTexture && textureManager != NULL) {
        ...
        if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] v3 model texture [%s] meshIndex = %d modelFile "
                   "[%s]\n",
                   extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, texPath.c_str(), meshIndex, modelFile.c_str());
        ...
        if (textures[mtDiffuse] == NULL) {
            ...
            if (fileExists(texPath) == true) {
                ...
                SystemFlags::OutputDebug(SystemFlags::debugError,
                                         ...
                                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, texPath.c_str(), meshHeader.properties,
    ...
    if (textureFlags & (1 << mtDiffuse)) {
        for (unsigned int i = 0; i < meshHeader.texCoordFrameCount; ++i) {
            ...
            if (readBytes != 1 && vertexCount != 0) {
                char szBuf[8096] = "";
                snprintf(szBuf, 8096, "fread returned wrong size = " MG_SIZE_T_SPECIFIER " [%u][%u] on line: %d.", readBytes, frameCount, vertexCount,
                         __LINE__);
                throw megaglest_runtime_error(szBuf);
            }
            fromEndianVecArray<Vec2f>(texCoords, vertexCount);
...
Texture2D *Mesh::loadMeshTexture(int meshIndex, int textureIndex, TextureManager *textureManager, string textureFile, int textureChannelCount,
                                 bool &textureOwned, bool deletePixMapAfterLoad, std::map<string, vector<pair<string, string>>> *loadedFileList,
    ...
    if (texture == NULL) {
        ...
        if (fileExists(textureFile) == true) {
            ...
            SystemFlags::OutputDebug(SystemFlags::debugError,
                                     ...
                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, textureFile.c_str(), textureFlags, meshIndex,
                                     textureIndex, modelFile.c_str());
        }
    }
...
void Mesh::load(int meshIndex, const string &dir, FILE *f, TextureManager *textureManager, bool deletePixMapAfterLoad,
                std::map<string, vector<pair<string, string>>> *loadedFileList, string sourceLoader, string modelFile) {
    this->textureManager = textureManager;
    ...
    for (int i = 0; i < meshTextureCount; ++i) {
        if (meshHeader.textures & flag) {
            ...
            if (textureManager) {
                textures[i] = loadMeshTexture(meshIndex, i, textureManager, mapFullPath, meshTextureChannelCount[i], texturesOwned[i], deletePixMapAfterLoad,
...
void Mesh::save(int meshIndex, const string &dir, FILE *f, TextureManager *textureManager, string convertTextureToFormat,
                std::map<string, int> &textureDeleteList, bool keepsmallest, string modelFile) {
    MeshHeader meshHeader;
    ...
    for (int i = 0; i < meshTextureCount; ++i) {
        if ((meshHeader.textures & flag)) {
            ...
            if (texture != NULL) {
                ...
                if (toLower(convertTextureToFormat) != "" && EndsWith(file, "." + convertTextureToFormat) == false) {
                    ...
                    if (keepsmallest == false || newSize <= originalSize) {
                        texture = loadMeshTexture(meshIndex, i, textureManager, file, meshTextureChannelCount[i], texturesOwned[i], false, NULL, "", modelFile);
                    }
                }
...
void Mesh::computeTangents() {
    ...
    for (unsigned int i = 0; i < indexCount; i += 3) {
        for (int j = 0; j < 3; ++j) {
            ...
            tangents[i0] += ((p2 - p0) * (v1 - v0) - (p1 - p0) * (v2 - v0)) / ((u2 - u0) * (v1 - v0) - (u1 - u0) * (v2 - v0));
        }
    }
...
void Model::load(const string &path, bool deletePixMapAfterLoad, std::map<string, vector<pair<string, string>>> *loadedFileList, string *sourceLoader) {
...
void Model::save(const string &path, string convertTextureToFormat, bool keepsmallest) {
...
void Model::loadG3d(const string &path, bool deletePixMapAfterLoad, std::map<string, vector<pair<string, string>>> *loadedFileList, string sourceLoader) {
    try {
#ifdef WIN32
        ...
        if (fileHeader.version == 4) {
            ...
            for (uint32 i = 0; i < meshCount; ++i) {
                meshes[i].load(i, dir, f, textureManager, deletePixMapAfterLoad, loadedFileList, sourceLoader, path);
                meshes[i].buildInterpolationData();
            }
        }
        // version 3
            ...
            for (uint32 i = 0; i < meshCount; ++i) {
                meshes[i].loadV3(i, dir, f, textureManager, deletePixMapAfterLoad, loadedFileList, sourceLoader, path);
                meshes[i].buildInterpolationData();
            }
        }
        // version 2
            ...
            for (uint32 i = 0; i < meshCount; ++i) {
                meshes[i].loadV2(i, dir, f, textureManager, deletePixMapAfterLoad, loadedFileList, sourceLoader, path);
                meshes[i].buildInterpolationData();
            }
        } else {
...
void Model::saveG3d(const string &path, string convertTextureToFormat, bool keepsmallest) {
    ...
    if (fileHeader.version == 4 || fileHeader.version == 3 || fileHeader.version == 2) {
        ...
        for (uint32 i = 0; i < meshCount; ++i) {
            meshes[i].save(i, tempModelFilename, f, textureManager, convertTextureToFormat, textureDeleteList, keepsmallest, path);
        }

        removeFile(path);
        if (renameFile(tempModelFilename, path) == true) {
            // Now delete old textures since they were converted to a new format
            for (std::map<string, int>::iterator iterMap = textureDeleteList.begin(); iterMap != textureDeleteList.end(); ++iterMap) {
...
void Mesh::copyInto(Mesh *dest, bool ignoreInterpolationData, bool destinationOwnsTextures) {
...
void Model::autoJoinMeshFrames() {
  ...
  */
    ...
    for (uint32 index = 0; index < meshCount; ++index) {
        ...
        mesh_key += string("_") + intToStr(mesh.getFrameCount()) + string("_") + intToStr(mesh.getTwoSided()) + string("_") +
                    intToStr(mesh.getCustomTexture()) + string("_") + intToStr(mesh.getNoSelect()) + string("_") + floatToStr(mesh.getOpacity()) + string("_") +
                    floatToStr(mesh.getGlow()) + string("_") + mesh.getDiffuseColor().getString() + string("_") + mesh.getSpecularColor().getString() +
    ...
    if (haveJoinedMeshes == true) {
        ...
        for (std::map<std::string, MeshContainer>::iterator iterMap = joinedMeshes.begin(); iterMap != joinedMeshes.end(); ++iterMap) {
        ...
        int index = 0;
        for (std::map<int, std::vector<Mesh *>>::iterator iterMap = orderedMeshes.begin(); iterMap != orderedMeshes.end(); ++iterMap) {
            ...
            if (iterMap->second.size() > 1) {
                ...
                for (unsigned int joinIndex = 1; joinIndex < iterMap->second.size(); ++joinIndex) {
                    ...
                    uint32 newVertexCount = base->getVertexCount() + mesh->getVertexCount();
                    ...
                    for (unsigned int frameIndex = 0; frameIndex < base->getFrameCount(); ++frameIndex) {
                        ...
                        memcpy(&joined_vertices[join_index], &base->getVertices()[baseIndex], originalBaseVertexCount * sizeof(Vec3f));
                        memcpy(&joined_normals[join_index], &base->getNormals()[baseIndex], originalBaseVertexCount * sizeof(Vec3f));
                        join_index += originalBaseVertexCount;

                        // second joined mesh values get copied
                        memcpy(&joined_vertices[join_index], &mesh->getVertices()[meshIndex], mesh->getVertexCount() * sizeof(Vec3f));
                        memcpy(&joined_normals[join_index], &mesh->getNormals()[meshIndex], mesh->getVertexCount() * sizeof(Vec3f));
                    ...
                    if (base->getTextureFlags() & 1) {
                        ...
                        memcpy(&joined_texCoords[0], &base->getTexCoords()[0], originalBaseVertexCount * sizeof(Vec2f));
                        memcpy(&joined_texCoords[originalBaseVertexCount], &mesh->getTexCoords()[0], mesh->getVertexCount() * sizeof(Vec2f));
                    ...
                    memcpy(&joined_indexes[join_index_index], &base->getIndices()[0], base->getIndexCount() * sizeof(uint32));
                    join_index_index += base->getIndexCount();

                    for (unsigned int meshIndex = 0; meshIndex < mesh->getIndexCount(); ++meshIndex) {
...
Pixmap2D *PixelBufferWrapper::getPixelBufferFor(int x, int y, int w, int h, int colorComponents) {
    ...
    if (PixelBufferWrapper::isPBOEnabled == true) {
        ...
        try {
            ...
            SystemFlags::OutputDebug(SystemFlags::debugError, "In [%s::%s Line: %d] codeSection [%s] UNKNOWN Error!",
                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, codeSection.c_str());
...
void BaseColorPickEntity::assign_color_using_prime(unsigned char *assign_to) {
    ...
    const unsigned int r = (nextColorRGB >> 11) & ((1 << 5) - 1), g = (nextColorRGB >> 5) & ((1 << 6) - 1), b = nextColorRGB & ((1 << 5) - 1);
...
bool BaseColorPickEntity::isUniquePickingColor(unsigned char *pixel) const {
    bool result = false;
    if (uniqueColorID[0] == pixel[0] && uniqueColorID[1] == pixel[1] && uniqueColorID[2] == pixel[2]) {

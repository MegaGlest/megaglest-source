...
void Mesh::loadV2(
  ...
  if (meshHeader.hasTexture && textureManager != NULL) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d] v2 model texture [%s] meshIndex = %d "
             "modelFile [%s]\n",
...
void Mesh::loadV3(
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("Load v3, this = %p Found meshHeader.properties = %d, textureFlags "
  ...
  if ((meshHeader.properties & mp3NoTexture) != mp3NoTexture &&
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d] v3 model texture [%s] meshIndex = %d "
             "modelFile [%s]\n",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__, texPath.c_str(), meshIndex, modelFile.c_str());
...
void Mesh::load(int meshIndex, const string &dir, FILE *f,
  ...
  if (opacity == 0) {
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("found a mesh with opacity=0 in header, using opacity=1 to see it "
             "now \n");
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("file: %s\n", modelFile.c_str());
    opacity = 1.0f;
  ...
  for (int i = 0; i < meshTextureCount; ++i) {
    if (meshHeader.textures & flag) {
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("mapPath [%s] meshHeader.textures = %d flag = %d "
...
void Mesh::save(int meshIndex, const string &dir, FILE *f,
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("Save, this = %p, Found meshTextureCount = %d, meshHeader.textures "
           "= %d meshIndex = %d\n",
        ...
        if (toLower(convertTextureToFormat) != "" &&
          ...
          if (convertTextureToFormat == "tga") {
            ...
            if (keepsmallest == false || newSize <= originalSize) {
              ...
              printf("Texture will not be converted, keeping smallest texture "
                     "[%s]\n",
          ...
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("Save, load new texture [%s] originalSize [%ld] newSize "
                   "[%ld]\n",
...
void Model::autoJoinMeshFrames() {
  ...
  */
  ...
  for (uint32 index = 0; index < meshCount; ++index) {
    ...
    //meshTextureCount;
    //++meshTexIndex) { 				printf("Mesh texture index: %d
    //[%p]
...
bool BaseColorPickEntity::get_next_assign_color(unsigned char *assign_to) {
  ...
  if (BaseColorPickEntity::trackColorUse == true) {
    ...
    if (usedColorIDList.find(color_key) == usedColorIDList.end()) {
      ...
      isDuplicate = true;
      printf("Line ref: %d *WARNING* color [%s] used count: %d using_loop: %d "

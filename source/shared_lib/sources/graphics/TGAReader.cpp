...
Pixmap3D *TGAReader3D::read(ifstream &in, const string &path,
                            Pixmap3D *ret) const {
  // printf("In [%s] line: %d\n",__FILE__,__LINE__);
  //	try {
  ...
  if (bigEndianSystem == true) {
    ...
    fileHeader.colourMapDepth =
        Shared::PlatformByteOrder::fromCommonEndian(fileHeader.colourMapDepth);
    fileHeader.colourMapLength =
        Shared::PlatformByteOrder::fromCommonEndian(fileHeader.colourMapDepth);
    fileHeader.colourMapOrigin =
        Shared::PlatformByteOrder::fromCommonEndian(fileHeader.colourMapOrigin);
    ...
    fileHeader.imageDescriptor =
        Shared::PlatformByteOrder::fromCommonEndian(fileHeader.imageDescriptor);
    fileHeader.width =
  ...
  const int picComponents =
      (ret->getComponents() == -1) ? fileComponents : ret->getComponents();
  // std::cout << "TGA-Components: Pic: " << picComponents << " old: " <<
  // (ret->getComponents()) << " File: " << fileComponents << " slice:" <<
  // ret->getSlice() << std::endl;
  ...
  //	catch(exception& ex) {
  //		char szBuf[8096]="";
  //		snprintf(szBuf,8096,"Error in [%s] on line: %d msg:
...
Pixmap2D *TGAReader::read(ifstream &in, const string &path,
                          Pixmap2D *ret) const {
  // printf("In [%s] line: %d\n",__FILE__,__LINE__);
  // try {
  // read header
  ...
  const int picComponents =
      (ret->getComponents() == -1) ? fileComponents : ret->getComponents();
  // std::cout << "TGA-Components: Pic: " << picComponents << " old: " <<
  // (ret->getComponents()) << " File: " << fileComponents << std::endl;
  ...
  for (int i = 0; i < h * w * picComponents; i += picComponents) {
    ...
    if (fileComponents == 1) {
      ...
      //				if(bigEndianSystem == true) {
      //					b =
      // Shared::PlatformByteOrder::fromCommonEndian(b);
      //				}
      //
      //				in.read((char*)&g, 1);
      //				if(bigEndianSystem == true) {
      //					g =
      // Shared::PlatformByteOrder::fromCommonEndian(g);
      //				}
      //
      //				in.read((char*)&r, 1);
      //				if(bigEndianSystem == true) {
      //					r =
      // Shared::PlatformByteOrder::fromCommonEndian(r);
      //				}
      //
      //				if(fileComponents==4){
      //					in.read((char*)&a, 1);
      //					if(bigEndianSystem == true) {
      //						a =
      // Shared::PlatformByteOrder::fromCommonEndian(a);
      //					}
      //
      //				}
      //				else {

...
// Macro to select the correct version of a softfloat function according to user
// flags
#if N_SPECIALIZED == 96
...
template <>
SoftFloatWrapper<N_SPECIALIZED> &SoftFloatWrapper<N_SPECIALIZED>::operator+=(
    const SoftFloatWrapper<N_SPECIALIZED> &f) {
...
template <>
bool SoftFloatWrapper<N_SPECIALIZED>::operator==(
    const SoftFloatWrapper<N_SPECIALIZED> &f) const {
...
// => sizeof is useable
// Note: To avoid duplicate symbols, insert a third template argument
// corresponding to N_SPECIALIZED
...
template <typename T> struct IntConverter<N_SPECIALIZED, T, true> {
  static inline SF_TYPE convert_from_int(T an_int) {
...
#define STREFLOP_X87DENORMAL_NATIVE_OPS_INT(native_type)                       \
  template <>                                                                  \
  SoftFloatWrapper<N_SPECIALIZED>::SoftFloatWrapper(const native_type f) {     \
    value<SF_TYPE>() =                                                         \
        IntConverter<N_SPECIALIZED, native_type,                               \
                     (sizeof(native_type) > 4)>::convert_from_int(f);          \
  }                                                                            \
  template <>                                                                  \
  SoftFloatWrapper<N_SPECIALIZED> &SoftFloatWrapper<N_SPECIALIZED>::operator=( \
      const native_type f) {                                                   \
  ...
  template <> SoftFloatWrapper<N_SPECIALIZED>::operator native_type() const {  \
    return IntConverter<N_SPECIALIZED, native_type,                            \
                        (sizeof(native_type) >                                 \
                         4)>::convert_to_int(value<SF_TYPE>());                \
  }                                                                            \
  template <>                                                                  \
  SoftFloatWrapper<N_SPECIALIZED>                                              \
      &SoftFloatWrapper<N_SPECIALIZED>::operator+=(const native_type f) {      \
  ...
  template <>                                                                  \
  bool SoftFloatWrapper<N_SPECIALIZED>::operator==(const native_type f)        \
      const {                                                                  \
  ...
  template <>                                                                  \
  bool SoftFloatWrapper<N_SPECIALIZED>::operator<(const native_type f) const { \
...
// => consider the float as a memory zone, then pass that to the softfloat
// conversion routines
// => this way, conversion is done by softfloat, not by the FPU
// Use a sizeof trick:
// - Specialize for BOTH C type and the expected type size of C type for correct
// memory pattern
...
//   Ex: could specialize for <long double, 10>, <long double, 12> and <long
//   double, 16>
...
// Specialization for floatx80 when C long double type size is 8 (there is 16
// bit padding, endian dependent)
...
// Little endian OK: both address are the same
  ...
  static inline SF_TYPE convert_from_float(const long double a_float) {
    return SF_APPEND(floatx80_to_)(
        *reinterpret_cast<const floatx80 *>(&a_float));
  }
  static inline long double convert_to_float(SF_TYPE value) {
    // avoid invalid memory access: must return a 8-bytes value from a 10-byte
    // type do it this way, by declaring the 8-byte on the stack
    long double holder;
    // And use that space for the result using the softfloat memory bit pattern
...
#warning You are using a completely UNTESTED new architecture. Please check that the 8-byte long double containing a 10-byte float is properly aligned in memory so that softfloat may correctly read the bit pattern. If this works for you, remove this warning and please consider sending a patch!
  static inline SF_TYPE convert_from_float(const long double a_float) {
    return SF_APPEND(floatx80_to_)(*reinterpret_cast<const floatx80 *>(
        reinterpret_cast<const char *>(&a_float) - 2));
  }
  static inline long double convert_to_float(SF_TYPE value) {
    // avoid invalid memory access: must return a 8-bytes value from a 10-byte
    // type do it this way, by declaring the 8-byte on the stack
    ...
    // And use that space for the result using the softfloat memory bit pattern
    // equivalence property
    *reinterpret_cast<floatx80 *>(reinterpret_cast<const char *>(&holder) - 2) =
        SF_PREPEND(_to_floatx80)(value);
    return holder;
  }
#else
...
// Specialization for floatx80 when C long double type size is 12 (there is 16
// bit padding, endian dependent)
...
// Little endian OK: both address are the same
  ...
  static inline long double convert_to_float(SF_TYPE value) {
    // avoid invalid memory access: must return a 12-bytes value from a 10-byte
    // type do it this way, by declaring the 12-byte on the stack
    long double holder;
    // And use that space for the result using the softfloat memory bit pattern
    // equivalence property
    *reinterpret_cast<floatx80 *>(&holder) = SF_PREPEND(_to_floatx80)(value);
...
// Specialization for floatx80 when C long double type size is 16. This is the
// case for g++ using -m128bit-long-double, which is itself the default on
// x86_64
...
// Little endian OK: both address are the same
  ...
  static inline long double convert_to_float(SF_TYPE value) {
    // avoid invalid memory access: must return a 16-bytes value from a 10-byte
    // type do it this way, by declaring the 16-byte on the stack
    long double holder;
    // And use that space for the result using the softfloat memory bit pattern
    // equivalence property
    *reinterpret_cast<floatx80 *>(&holder) = SF_PREPEND(_to_floatx80)(value);
...
#warning You are using a completely UNTESTED new architecture. Please check that the 16-byte long double containing a 10-byte float is properly aligned in memory so that softfloat may correctly read the bit pattern. If this works for you, remove this warning and please consider sending a patch!
  static inline SF_TYPE convert_from_float(const long double a_float) {
    return SF_APPEND(floatx80_to_)(*reinterpret_cast<const floatx80 *>(
        reinterpret_cast<const char *>(&a_float) + 6));
  }
  static inline long double convert_to_float(SF_TYPE value) {
    // avoid invalid memory access: must return a 12-bytes value from a 10-byte
    // type do it this way, by declaring the 12-byte on the stack
    ...
    // And use that space for the result using the softfloat memory bit pattern
    // equivalence property
    *reinterpret_cast<floatx80 *>(reinterpret_cast<const char *>(&holder) + 6) =
        SF_PREPEND(_to_floatx80)(value);
    return holder;
  }
#else
...
#define STREFLOP_X87DENORMAL_NATIVE_OPS_FLOAT(native_type)                     \
  ...
  SoftFloatWrapper<N_SPECIALIZED>::SoftFloatWrapper(const native_type f) {     \
    value<SF_TYPE>() =                                                         \
        FloatConverter<N_SPECIALIZED, native_type,                             \
                       sizeof(native_type)>::convert_from_float(f);            \
  }                                                                            \
  template <>                                                                  \
  SoftFloatWrapper<N_SPECIALIZED> &SoftFloatWrapper<N_SPECIALIZED>::operator=( \
      const native_type f) {                                                   \
  ...
  template <> SoftFloatWrapper<N_SPECIALIZED>::operator native_type() const {  \
    return FloatConverter<N_SPECIALIZED, native_type, sizeof(native_type)>::   \
        convert_to_float(value<SF_TYPE>());                                    \
  }                                                                            \
  template <>                                                                  \
  SoftFloatWrapper<N_SPECIALIZED>                                              \
      &SoftFloatWrapper<N_SPECIALIZED>::operator+=(const native_type f) {      \
...
/// binary operators
/// use dummy argument factories to distinguish from integer conversion and
/// avoid creating temporary object
template <>
SoftFloatWrapper<N_SPECIALIZED>
operator+(const SoftFloatWrapper<N_SPECIALIZED> &f1,
          const SoftFloatWrapper<N_SPECIALIZED> &f2) {
  return SoftFloatWrapper<N_SPECIALIZED>(
      SF_PREPEND(_add)(f1.value<SF_TYPE>(), f2.value<SF_TYPE>()), true);
}
template <>
SoftFloatWrapper<N_SPECIALIZED>
operator-(const SoftFloatWrapper<N_SPECIALIZED> &f1,
          const SoftFloatWrapper<N_SPECIALIZED> &f2) {
  return SoftFloatWrapper<N_SPECIALIZED>(
      SF_PREPEND(_sub)(f1.value<SF_TYPE>(), f2.value<SF_TYPE>()), true);
}
template <>
SoftFloatWrapper<N_SPECIALIZED>
operator*(const SoftFloatWrapper<N_SPECIALIZED> &f1,
          const SoftFloatWrapper<N_SPECIALIZED> &f2) {
  return SoftFloatWrapper<N_SPECIALIZED>(
      SF_PREPEND(_mul)(f1.value<SF_TYPE>(), f2.value<SF_TYPE>()), true);
}
template <>
SoftFloatWrapper<N_SPECIALIZED>
operator/(const SoftFloatWrapper<N_SPECIALIZED> &f1,
          const SoftFloatWrapper<N_SPECIALIZED> &f2) {
  return SoftFloatWrapper<N_SPECIALIZED>(
      SF_PREPEND(_div)(f1.value<SF_TYPE>(), f2.value<SF_TYPE>()), true);
}

#define STREFLOP_X87DENORMAL_BINARY_OPS_INT(native_type)                       \
  template <>                                                                  \
  SoftFloatWrapper<N_SPECIALIZED> operator+(                                   \
      const SoftFloatWrapper<N_SPECIALIZED> &f1, const native_type f2) {       \
    return SoftFloatWrapper<N_SPECIALIZED>(                                    \
        SF_PREPEND(_add)(                                                      \
            f1.value<SF_TYPE>(),                                               \
            IntConverter<N_SPECIALIZED, native_type,                           \
                         (sizeof(native_type) > 4)>::convert_from_int(f2)),    \
        true);                                                                 \
  }                                                                            \
  template <>                                                                  \
  SoftFloatWrapper<N_SPECIALIZED> operator-(                                   \
  ...
  template <>                                                                  \
  SoftFloatWrapper<N_SPECIALIZED> operator+(                                   \
      const native_type f1, const SoftFloatWrapper<N_SPECIALIZED> &f2) {       \
    return SoftFloatWrapper<N_SPECIALIZED>(                                    \
        SF_PREPEND(_add)(                                                      \
            IntConverter<N_SPECIALIZED, native_type,                           \
                         (sizeof(native_type) > 4)>::convert_from_int(f1),     \
            f2.value<SF_TYPE>()),                                              \
        true);                                                                 \
  }                                                                            \
  template <>                                                                  \
  ...
  template <>                                                                  \
  bool operator==(const native_type value,                                     \
                  const SoftFloatWrapper<N_SPECIALIZED> &f) {                  \
    return SF_PREPEND(_eq)(                                                    \
        IntConverter<N_SPECIALIZED, native_type,                               \
                     (sizeof(native_type) > 4)>::convert_from_int(value),      \
        f.value<SF_TYPE>());                                                   \
  }                                                                            \
  template <>                                                                  \
  bool operator!=(const native_type value,                                     \
...
#define STREFLOP_X87DENORMAL_BINARY_OPS_FLOAT(native_type)                     \
  ...
  SoftFloatWrapper<N_SPECIALIZED> operator+(                                   \
    ...
    return SoftFloatWrapper<N_SPECIALIZED>(                                    \
        SF_PREPEND(_add)(                                                      \
            f1.value<SF_TYPE>(),                                               \
            FloatConverter<N_SPECIALIZED, native_type,                         \
                           sizeof(native_type)>::convert_from_float(f2)),      \
        true);                                                                 \
  }                                                                            \
  template <>                                                                  \
  SoftFloatWrapper<N_SPECIALIZED> operator-(                                   \
  ...
  SoftFloatWrapper<N_SPECIALIZED> operator+(                                   \
    ...
    return SoftFloatWrapper<N_SPECIALIZED>(                                    \
        SF_PREPEND(_add)(                                                      \
            FloatConverter<N_SPECIALIZED, native_type,                         \
                           sizeof(native_type)>::convert_from_float(f1),       \
            f2.value<SF_TYPE>()),                                              \
        true);                                                                 \
  }                                                                            \
  template <>                                                                  \
  ...
  bool operator==(const native_type value,                                     \
    ...
    return SF_PREPEND(_eq)(                                                    \
        FloatConverter<N_SPECIALIZED, native_type,                             \
                       sizeof(native_type)>::convert_from_float(value),        \
        f.value<SF_TYPE>());                                                   \
  }                                                                            \
  template <>                                                                  \
  bool operator!=(const native_type value,                                     \
...
template <>
SoftFloatWrapper<N_SPECIALIZED>
operator-(const SoftFloatWrapper<N_SPECIALIZED> &f) {
  // We could do it right here by flipping the bit sign
  // However, there is the exceptions handling and such, so...
  return SoftFloatWrapper<N_SPECIALIZED>(
      SF_PREPEND(_sub)(SF_APPEND(int32_to_)(0), f.value<SF_TYPE>()), true);
}
template <>
SoftFloatWrapper<N_SPECIALIZED>
operator+(const SoftFloatWrapper<N_SPECIALIZED> &f) {
...
template <>
SoftFloatWrapper<N_SPECIALIZED>::SoftFloatWrapper(
    const SoftFloatWrapper<32> &f) {
  value<SF_TYPE>() = SF_APPEND(float32_to_)(f.value<float32>());
}

template <>
SoftFloatWrapper<N_SPECIALIZED> &
SoftFloatWrapper<N_SPECIALIZED>::operator=(const SoftFloatWrapper<32> &f) {
  value<SF_TYPE>() = SF_APPEND(float32_to_)(f.value<float32>());
  return *this;
}
...
template <>
SoftFloatWrapper<N_SPECIALIZED>::SoftFloatWrapper(
    const SoftFloatWrapper<64> &f) {
  value<SF_TYPE>() = SF_APPEND(float64_to_)(f.value<float64>());
}

template <>
SoftFloatWrapper<N_SPECIALIZED> &
SoftFloatWrapper<N_SPECIALIZED>::operator=(const SoftFloatWrapper<64> &f) {
  value<SF_TYPE>() = SF_APPEND(float64_to_)(f.value<float64>());
  return *this;
}
...
template <>
SoftFloatWrapper<N_SPECIALIZED>::SoftFloatWrapper(
    const SoftFloatWrapper<96> &f) {
  value<SF_TYPE>() = SF_APPEND(floatx80_to_)(f.value<floatx80>());
}

template <>
SoftFloatWrapper<N_SPECIALIZED> &
SoftFloatWrapper<N_SPECIALIZED>::operator=(const SoftFloatWrapper<96> &f) {
  value<SF_TYPE>() = SF_APPEND(floatx80_to_)(f.value<floatx80>());
  return *this;
}

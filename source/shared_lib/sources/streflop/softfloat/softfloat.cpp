...
static long long roundAndPackInt64(flag zSign, bits64 absZ0, bits64 absZ1) {
  int8 roundingMode;
...
static float32 roundAndPackFloat32(flag zSign, int16 zExp, bits32 zSig) {
  ...
  if (0xFD <= (bits16)zExp) {
    if ((0xFD < zExp) ||
        ((zExp == 0xFD) && ((sbits32)(zSig + roundIncrement) < 0))) {
...
static floatx80 roundAndPackFloatx80(int8 roundingPrecision, flag zSign,
                                     int32 zExp, bits64 zSig0, bits64 zSig1) {
  int8 roundingMode;
...
precision80:
  ...
  if (0x7FFD <= (bits32)(zExp - 1)) {
    if ((0x7FFE < zExp) ||
        ((zExp == 0x7FFE) && (zSig0 == LIT64(0xFFFFFFFFFFFFFFFF)) &&
         increment)) {
      roundMask = 0;
    overflow:
      float_raise(float_flag_overflow | float_flag_inexact);
...
static void normalizeFloat128Subnormal(bits64 aSig0, bits64 aSig1,
                                       int32 *zExpPtr, bits64 *zSig0Ptr,
                                       bits64 *zSig1Ptr) {
  int8 shiftCount;
...
INLINE float128 packFloat128(flag zSign, int32 zExp, bits64 zSig0,
                             bits64 zSig1) {
  float128 z;
...
static float128 roundAndPackFloat128(flag zSign, int32 zExp, bits64 zSig0,
                                     bits64 zSig1, bits64 zSig2) {
  int8 roundingMode;
  flag roundNearestEven, increment, isTiny;
  ...
  if (0x7FFD <= (bits32)zExp) {
    if ((0x7FFD < zExp) || ((zExp == 0x7FFD) &&
                            eq128(LIT64(0x0001FFFFFFFFFFFF),
                                  LIT64(0xFFFFFFFFFFFFFFFF), zSig0, zSig1) &&
                            increment)) {
      float_raise(float_flag_overflow | float_flag_inexact);
      if ((roundingMode == float_round_to_zero) ||
          (zSign && (roundingMode == float_round_up)) ||
...
static float128 normalizeRoundAndPackFloat128(flag zSign, int32 zExp,
  ...
  if (0 <= shiftCount) {
    ...
    shift128ExtraRightJamming(zSig0, zSig1, 0, -shiftCount, &zSig0, &zSig1,
                              &zSig2);
  }
  zExp -= shiftCount;
...
float128 int64_to_float128(long long a) {
  flag zSign;
  uint64 absA;
  int8 shiftCount;
  int32 zExp;
...
shiftRight1:
  ...
  ++zExp;
roundAndPack:
  return roundAndPackFloatx80(floatx80_rounding_precision, zSign, zExp, zSig0,
                              zSig1);
}
...
aBigger:
  ...
  zExp = aExp;
normalizeRoundAndPack:
  return normalizeRoundAndPackFloatx80(floatx80_rounding_precision, zSign, zExp,
                                       zSig0, zSig1);
}
...
static float128 addFloat128Sigs(float128 a, float128 b, flag zSign) {
  ...
  if (0 < expDiff) {
    ...
    shift128ExtraRightJamming(aSig0, aSig1, 0, -expDiff, &aSig0, &aSig1,
                              &zSig2);
    zExp = bExp;
  } else {
    if (aExp == 0x7FFF) {

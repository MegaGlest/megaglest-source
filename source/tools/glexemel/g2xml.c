...
int g3d2xml(FILE *infile, FILE *outfile) {
    ...
    for (ii = 0; ii < modelHeader.meshCount; ii++) {
        ...
        fprintf(outfile, "vertexCount=\"%u\" ", meshHeader.vertexCount);
        fprintf(outfile, "indexCount=\"%u\" ", meshHeader.indexCount);
        fprintf(outfile, "specularPower=\"%f\" ", meshHeader.specularPower);
        ...
        fprintf(outfile, "\t\t\t<Color r=\"%f\" g=\"%f\" b=\"%f\"/>\n", meshHeader.diffuseColor[0], meshHeader.diffuseColor[1], meshHeader.diffuseColor[2]);
        fprintf(outfile, "\t\t</Diffuse>\n");
        fprintf(outfile, "\t\t<Specular>\n");
        fprintf(outfile, "\t\t\t<Color r=\"%f\" g=\"%f\" b=\"%f\"/>\n", meshHeader.specularColor[0], meshHeader.specularColor[1], meshHeader.specularColor[2]);
        fprintf(outfile, "\t\t</Specular>\n");
        ...
        for (jj = 0; jj < meshHeader.frameCount; jj++) {
            ...
            for (kk = 0; kk < meshHeader.vertexCount; kk++) {
                ...
                /*fprintf(outfile, "i=\"%d\" ",
					kk);*/
                fprintf(outfile, "x=\"%f\" ", fdata[3 * kk]);
                fprintf(outfile, "y=\"%f\" ", fdata[3 * kk + 1]);
                fprintf(outfile, "z=\"%f\"/>\n", fdata[3 * kk + 2]);
            }
        ...
        if (meshHeader.textures) {
            ...
            for (kk = 0; kk < meshHeader.vertexCount; kk++) {
                ...
                /*fprintf(outfile, "i=\"%d\" ",
					kk);*/
                fprintf(outfile, "s=\"%f\" ", fdata[2 * kk]);
                fprintf(outfile, "t=\"%f\"/>\n", fdata[2 * kk + 1]);
            }
            fprintf(outfile, "\t\t</TexCoords>\n");
            free(fdata);

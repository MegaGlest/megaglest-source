...
static bool createPipes(PipeType *pPipeParentRead, PipeType *pPipeParentWrite, PipeType *pPipeChildRead, PipeType *pPipeChildWrite) {
    ...
    if (!CreatePipe(pPipeParentRead, pPipeChildWrite, &pipeAttr, 0)) return 0;
...
static LPWSTR genCommandLine() {
    ...
    if (default_file_exists) {
        newcmdline = (LPWSTR)malloc(sizeof(TEXT(GAME_LAUNCH_NAME)) + sizeof(WCHAR) * (wcslen(cmdline) - iFirstArg + 9));
        wsprintf(newcmdline, TEXT("\".\\" GAME_LAUNCH_NAME ".exe\" %s"), cmdline + iFirstArg);
    } else {
        newcmdline = (LPWSTR)malloc(sizeof(TEXT(GAME_LAUNCH_NAME "x64")) + sizeof(WCHAR) * (wcslen(cmdline) - iFirstArg + 9));
...
static bool launchChild(ProcessType *pid) {
    ...
    if (default_file_exists) {
        return CreateProcessW(TEXT(".\\" GAME_LAUNCH_NAME ".exe"), genCommandLine(), NULL, NULL, TRUE, 0, NULL, NULL, &si, pid);
    }
    return CreateProcessW(TEXT(".\\" GAME_LAUNCH_NAME "x64.exe"), genCommandLine(), NULL, NULL, TRUE, 0, NULL, NULL, &si, pid);
...
static int closeProcess(ProcessType *pid) {
    ...
    if (!WIFEXITED(rc)) return 1; // oh well.
...
SteamBridge::SteamBridge(PipeType _fd)
    : m_CallbackUserStatsReceived(this, &SteamBridge::OnUserStatsReceived), m_CallbackUserStatsStored(this, &SteamBridge::OnUserStatsStored), fd(_fd) {
...
#undef PRINTGOTCMD
    ...
    case SHIMCMD_REQUESTSTATS:
        if ((!GSteamStats) || (!GSteamStats->RequestCurrentStats())) writeStatsReceived(fd, false);
        // callback later.
        break;

    case SHIMCMD_STORESTATS:
        if ((!GSteamStats) || (!GSteamStats->StoreStats())) writeStatsStored(fd, false);
        // callback later.
        break;
    ...
    while (!quit && ((br = readPipe(pipeParentRead, buf, sizeof(buf))) > 0)) {
        ...
        while (br > 0) {
            ...
            if ((br - 1) >= cmdlen) {
                ...
                br -= cmdlen + 1;
                if (br > 0) memmove(buf, buf + cmdlen + 1, br);
            } // if
            else // get more data.
            {
...
static bool setEnvironmentVars(PipeType pipeChildRead, PipeType pipeChildWrite) {
    ...
    if (!setEnvVar("STEAMSHIM_READHANDLE", buf)) return false;

    snprintf(buf, sizeof(buf), LLUFMT, (unsigned long long)pipeChildWrite);
    if (!setEnvVar("STEAMSHIM_WRITEHANDLE", buf)) return false;

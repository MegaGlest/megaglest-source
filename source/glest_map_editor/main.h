...
const char *object_descs[] = {
    "None (Erase)", "Tree",         "Dead tree",         "Stone (non harvest)",
    "Bush",         "Water object", "Big tree",          "Hanged/Impaled",
    "Statues",      "Mountain",     "Invisible blocking"};

const char *resource_descs[] = {"None (Erase)", "Gold",     "Stone",
                                "Custom 4",     "Custom 5", "Custom 6"};

const char *surface_descs[] = {"Grass", "Sec. grass", "Road", "Stone",
                               "Ground"};
...
class MainToolBar : public wxToolBar {
 ...
 public:
  MainToolBar(wxWindow *parent, wxWindowID id) : wxToolBar(parent, id) {}
...
class MainWindow : public wxFrame {
 ...
 public:
  ...
  void onMenuEditMirrorXY(
      wxCommandEvent &event);  // copy bottomleft to topright
  void onMenuEditRotatecopyX(
      wxCommandEvent &event);  // copy left to right, rotated
  void onMenuEditRotatecopyY(
      wxCommandEvent &event);  // copy top to bottom, rotated
  void onMenuEditRotatecopyXY(
      wxCommandEvent &event);  // copy bottomleft to topright, rotated
  void onMenuEditRotatecopyCorner(
...
class MsgDialog : public wxDialog {
 ...
 public:
  MsgDialog(wxWindow *parent, const wxString &message, const wxString &caption,
            long style = wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER | wxOK |
                         wxCENTRE,

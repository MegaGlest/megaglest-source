...
UndoPoint::UndoPoint() : change(ctNone), surface(0), object(0), resource(0), height(0) {
    w = Program::map->getW();
    h = Program::map->getH();
}
/*
...
void Program::rotatecopyCorner() { // rotate top left 1/4 to top right 1/4
    if (map) {
        ...
        for (int i = 0; i < map->getMaxFactions(); ++i) {
            if ((map->getStartLocationX(i) < w / 2 && map->getStartLocationY(i) < h / 2) &&
                (map->getStartLocationX(i) != 0 || map->getStartLocationY(i) != 0)) // any startpositions to copy?
                for (int ii = 0; ii < map->getMaxFactions(); ++ii)
                    if (map->getStartLocationX(ii) == 0 && map->getStartLocationY(ii) == 0) { // first free one
...
void Program::shiftLeft() {
    if (map) {
        ...
        for (int i = 0; i < map->getMaxFactions(); ++i) // move players
            map->changeStartLocation(map->getStartLocationX(i) - 1, map->getStartLocationY(i),
    ...
    if (map) {
        ...
        for (int i = 0; i < map->getMaxFactions(); ++i)                           // move players
            if (map->getStartLocationX(i) != 0 || map->getStartLocationY(i) != 0) // don't move the unset ones
                map->changeStartLocation(map->getStartLocationY(i), map->getStartLocationX(i),
                                         i); // it allready check limits
    }
}
...
void Program::shiftUp() {
    if (map) {
        ...
        for (int i = 0; i < map->getMaxFactions(); ++i) // move players
            map->changeStartLocation(map->getStartLocationX(i), map->getStartLocationY(i) - 1,
                                     i); // it allready check limits
    }
}

...
void Program::mirrorXY() {  // copy leftbottom to topright, can handle
                            // non-sqaure maps
  if (map) {
    int w = map->getW();
    int h = map->getH();
...
void Program::rotatecopyXY() {
  if (map) {
    ...
    int h = map->getH();
    int sw = w / h;
    if (sw < 1) sw = 1;  // x-squares per y
    int sh = h / w;
    if (sh < 1) sh = 1;  // y-squares per x
    if (sh == 1)
...
void Program::shiftLeft() {
  if (map) {
    ...
    int h = map->getH();
    for (int i = 0; i < w; i++)
      for (int j = 0; j < h; j++) map->copyXY(i, j, i + 1, j);
...
void Program::shiftUp() {
  if (map) {
    ...
    int h = map->getH() - 1;
    for (int i = 0; i < w; i++)
      for (int j = 0; j < h; j++) map->copyXY(i, j, i, j + 1);
    for (int i = 0; i < map->getMaxFactions(); ++i)  // move players
      map->changeStartLocation(map->getStartLocationX(i),
...
void Program::shiftDown() {
  if (map) {
    ...
    int h = map->getH() - 1;
    for (int i = 0; i < w; i++)
      for (int j = h; j > 0; j--) map->copyXY(i, j, i, j - 1);
    for (int i = 0; i < map->getMaxFactions(); ++i)  // move players
      if (map->getStartLocationX(i) != 0 ||
          map->getStartLocationY(i) != 0)  // don't move the unset ones
        map->changeStartLocation(map->getStartLocationX(i),

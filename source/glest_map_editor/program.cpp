...
UndoPoint::UndoPoint()
    : change(ctNone), surface(0), object(0), resource(0), height(0) {
  w = Program::map->getW();
  h = Program::map->getH();
}
/*
...
void UndoPoint::init(ChangeType change) {
  ...
  case ctGradient:
    ...
    // std::cout << "Building an array of heights to use for our UndoPoint" <<
    // std::endl;
...
void UndoPoint::revert() {
  // std::cout << "attempting to revert last changes to " << undoID <<
  // std::endl;
  switch (change) {
  // Revert Everything
  case ctAll:
  ...
  case ctSurface:
    ...
    for (int i = 0; i < w; i++) {
      for (int j = 0; j < h; j++) {
        Program::map->setSurface(
            i, j, static_cast<MapSurfaceType>(surface[j * w + i]));
      }
    }
    if (change != ctAll)
      break;
...
int Program::getCellX(int x) { return (x - ofsetX) / cellSize; }

int Program::getCellY(int y) { return (y + ofsetY) / cellSize; }
...
void Program::glestChangeMapHeight(int x, int y, int Height, int radius) {
  if (map)
    map->glestChangeHeight((x - ofsetX) / cellSize, (y + ofsetY) / cellSize,
                           Height, radius);
}

void Program::pirateChangeMapHeight(int x, int y, int Height, int radius) {
  if (map)
    map->pirateChangeHeight((x - ofsetX) / cellSize, (y + ofsetY) / cellSize,
                            Height, radius);
}

void Program::changeMapSurface(int x, int y, int surface, int radius) {
  if (map)
    map->changeSurface((x - ofsetX) / cellSize, (y + ofsetY) / cellSize,
                       static_cast<MapSurfaceType>(surface), radius);
}

void Program::changeMapObject(int x, int y, int object, int radius,
                              bool overwrite) {
  if (map)
    map->changeObject((x - ofsetX) / cellSize, (y + ofsetY) / cellSize, object,
                      radius, overwrite);
}
...
void Program::changeMapResource(int x, int y, int resource, int radius,
                                bool overwrite) {
  if (map)
    map->changeResource((x - ofsetX) / cellSize, (y + ofsetY) / cellSize,
                        resource, radius, overwrite);
}

void Program::changeStartLocation(int x, int y, int player) {
  if (map)
    map->changeStartLocation((x - ofsetX) / cellSize, (y + ofsetY) / cellSize,
                             player);
}

void Program::setUndoPoint(ChangeType change) {
  if (change == ctLocation)
    return;
...
void Program::renderMap(int w, int h, pair<int, int> *mouse_pos, int *radius) {
  ...
  if (map)
    renderer.renderMap(map, ofsetX, ofsetY, w, h, cellSize, grid, heightmap,
                       hideWater, mouse_pos, radius);
}
...
void Program::mirrorX() { // copy left to right
  if (map) {
    ...
    // move players
    for (int i = 0; i < map->getMaxFactions();
         ++i) // first remove players from target corner
      if (map->getStartLocationX(i) >= w / 2)
        map->changeStartLocation(0, 0, i);
    for (int i = 0; i < map->getMaxFactions(); ++i) {
      if ((map->getStartLocationX(i) < w / 2) &&
          (map->getStartLocationX(i) != 0 ||
           map->getStartLocationY(i) != 0)) // any startpositions to copy?
        for (int ii = 0; ii < map->getMaxFactions(); ++ii)
          if (map->getStartLocationX(ii) == 0 &&
              map->getStartLocationY(ii) == 0) { // first free one
            map->changeStartLocation(w - map->getStartLocationX(i) - 1,
                                     map->getStartLocationY(i), ii);
            break;
          }
    }
  }
...
void Program::mirrorY() { // copy top to bottom
  if (map) {
    ...
    for (int i = 0; i < map->getMaxFactions();
         ++i) // first remove players from target corner
      if (map->getStartLocationY(i) >= h / 2)
        map->changeStartLocation(0, 0, i);
    for (int i = 0; i < map->getMaxFactions(); ++i) {
      if ((map->getStartLocationY(i) < h / 2) &&
          (map->getStartLocationX(i) != 0 ||
        ...
        for (int ii = 0; ii < map->getMaxFactions(); ++ii)
          if (map->getStartLocationX(ii) == 0 &&
              map->getStartLocationY(ii) == 0) { // first free one
            map->changeStartLocation(map->getStartLocationX(i),
                                     h - map->getStartLocationY(i) - 1, ii);
            break;
          }
    }
  }
...
void Program::mirrorXY() { // copy leftbottom to topright, can handle non-sqaure
                           // maps
  if (map) {
    int w = map->getW();
    int h = map->getH();
    ...
    for (int i = 0; i < map->getMaxFactions();
         ++i) // first remove players from target corner
      if (map->getStartLocationX(i) > map->getStartLocationY(i))
        map->changeStartLocation(0, 0, i);
    for (int i = 0; i < map->getMaxFactions(); ++i) {
      if (map->getStartLocationX(i) <
          map->getStartLocationY(i)) // any startpositions to copy?
        for (int ii = 0; ii < map->getMaxFactions(); ++ii)
          if (map->getStartLocationX(ii) == 0 &&
              map->getStartLocationY(ii) == 0) { // first free one
            map->changeStartLocation(map->getStartLocationY(i) * w / h,
                                     map->getStartLocationX(i) * h / w, ii);
            break;
          }
    }
  }
...
void Program::rotatecopyXY() {
  if (map) {
    ...
    int sw = w / h;
    if (sw < 1)
      sw = 1; // x-squares per y
    int sh = h / w;
    if (sh < 1)
      sh = 1; // y-squares per x
    if (sh == 1)
  ...
  if (map) {
    ...
    // Non-sqaure maps:
      ...
      for (int i = 0; i < w / s / 2; i++) {
        for (int j = 0; j < h / 2; j++) {
          for (int p = 0; p < s; p++)
            map->copyXY(w - j * s - 1 - p, i, i * s + p, j);
        }
      }
    } else {         // copy vertical strips
      int s = h / w; // 2 if twice as heigh as wide
      for (int i = 0; i < w / 2; i++) {
        for (int j = 0; j < h / s / 2; j++) {
          for (int p = 0; p < s; p++)
            map->copyXY(w - j - 1, i * s + p, i, j * s + p);
        }
      }
    }
    ...
    for (int i = 0; i < map->getMaxFactions(); ++i) {
      if ((map->getStartLocationX(i) < w / 2 &&
        ...
        for (int ii = 0; ii < map->getMaxFactions(); ++ii)
          if (map->getStartLocationX(ii) == 0 &&
              map->getStartLocationY(ii) == 0) { // first free one
            map->changeStartLocation(w - map->getStartLocationY(i) * w / h - 1,
                                     map->getStartLocationX(i) * h / w, ii);
            break;
          }
    }
...
void Program::shiftLeft() {
  if (map) {
    ...
    for (int i = 0; i < map->getMaxFactions(); ++i) // move players
      map->changeStartLocation(map->getStartLocationX(i) - 1,
                               map->getStartLocationY(i),
                               i); // it allready check limits
  }
}
...
void Program::flipDiagonal() {
  if (map) {
    ...
    for (int i = 0; i < map->getMaxFactions(); ++i) // move players
      if (map->getStartLocationX(i) != 0 ||
          map->getStartLocationY(i) != 0) // don't move the unset ones
        map->changeStartLocation(map->getStartLocationY(i),
                                 map->getStartLocationX(i),
                                 i); // it allready check limits
  }
}
...
void Program::randomizeMapHeights(bool withReset, int minimumHeight,
                                  int maximumHeight, int chanceDivider,
                                  int smoothRecursions) {
  if (map)
    map->randomizeHeights(withReset, minimumHeight, maximumHeight,
                          chanceDivider, smoothRecursions);
}

void Program::importMapHeights(unsigned char *data) {
  if (map)
    map->importMapHeights(data);
}

void Program::randomizeFactions() {
  if (map)
    map->randomizeFactions();
}

void Program::switchMapSurfaces(int surf1, int surf2) {
  if (map)
    map->switchSurfaces(static_cast<MapSurfaceType>(surf1),
                        static_cast<MapSurfaceType>(surf2));
}
...
void Program::setMapAdvanced(int altFactor, int waterLevel, int cliffLevel,
                             int cameraHeight) {
  if (map)
    map->setAdvanced(altFactor, waterLevel, cliffLevel, cameraHeight);
}

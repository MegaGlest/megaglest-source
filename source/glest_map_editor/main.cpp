...
MainWindow::MainWindow(string appPath)
    : wxFrame(NULL, -1, ToUnicode(winHeader), wxPoint(0, 0), wxSize(1024, 768)), glCanvas(NULL), program(NULL), lastX(0), lastY(0), panel(NULL), menuBar(NULL),
      fileDialog(NULL)

      ,
      currentBrush(btHeight), height(0), surface(1), radius(1), object(0), resource(0), startLocation(1), enabledGroup(ctHeight), fileModified(false),
      boxsizer(NULL), startupSettingsInited(false) {
    menuFile = NULL;
    menuEdit = NULL;
    menuEditMirror = NULL;
    menuView = NULL;
...
void MainWindow::init(string fname) {
    ...
    wxMenuItem *pmi8 = new wxMenuItem(menuBrushStartLocation, miBrushStartLocation + 8,
...
void MainWindow::onMouseMove(wxMouseEvent &event, int x, int y) {
    ...
    if (event.LeftIsDown()) {
        ...
        SetStatusText(wxT("Pos (Ingame): ") + ToUnicode(intToStr(program->getCellX(x)) + "," + intToStr(program->getCellY(y)) + " (" +
                                                        intToStr(2 * (program->getCellX(x))) + "," + intToStr(2 * (program->getCellY(y))) + ")"),
                      siPOS_VALUE);
        // #ifdef WIN32
        // repaint = true;
        // #endif
...
void MainWindow::onMenuEditReset(wxCommandEvent &event) {
    ...
    try {
        program->reset(strToInt(simpleDialog.getValue("Width")), strToInt(simpleDialog.getValue("Height")), strToInt(simpleDialog.getValue("Altitude")),
...
void MainWindow::onMenuEditResize(wxCommandEvent &event) {
    ...
    try {
        program->resize(strToInt(simpleDialog.getValue("Width")), strToInt(simpleDialog.getValue("Height")));
    } catch (const exception &e) {
        MsgDialog(this, ToUnicode(e.what()), wxT("Exception"), wxOK | wxICON_ERROR).ShowModal();
    }
    setDirty();
...
void MainWindow::onMenuEditRandomizeHeights(wxCommandEvent &event) {
    ...
    while (true) {
        ...
        try {
            ...
            program->setUndoPoint(ctAll);
            program->randomizeMapHeights(randomWithReset, randomMinimumHeight, randomMaximumHeight, randomChanceDivider, randomRecursions);
        } catch (const exception &e) {
            MsgDialog(this, ToUnicode(e.what()), wxT("Exception"), wxOK | wxICON_ERROR).ShowModal();
        }
        setDirty();
...
void MainWindow::onMenuEditImportHeights(wxCommandEvent &event) {
    ...
    try {
        ...
        fileDialog->SetWildcard(wxT("All "
        ...
        for (int i = 0; i < map_w; i++) {
            for (int j = 0; j < map_h; j++) {
                ...
                img_data[(i + j * map_w) * 3] =
                    (unsigned char)((program->getMap()->getHeight(i, j) - MIN_MAP_CELL_HEIGHT) * 255 / (MAX_MAP_CELL_HEIGHT - MIN_MAP_CELL_HEIGHT) + 0.5);
...
void MainWindow::onMenuEditSwitchSurfaces(wxCommandEvent &event) {
    ...
    try {
        ...
        program->switchMapSurfaces(strToInt(simpleDialog.getValue("Surface1")), strToInt(simpleDialog.getValue("Surface2")));
    } catch (const exception &e) {
        MsgDialog(this, ToUnicode(e.what()), wxT("Exception"), wxOK | wxICON_ERROR).ShowModal();
    }
    setDirty();
...
void MainWindow::onMenuEditAdvanced(wxCommandEvent &event) {
    ...
    try {
        program->setMapAdvanced(strToInt(simpleDialog.getValue("Height Factor")), strToInt(simpleDialog.getValue("Water Level")),
                                strToInt(simpleDialog.getValue("Cliff Level")), strToInt(simpleDialog.getValue("Camera Height")));
    } catch (const exception &e) {
        MsgDialog(this, ToUnicode(e.what()), wxT("Exception"), wxOK | wxICON_ERROR).ShowModal();
    }
    setDirty();
...
void MainWindow::onMenuBrushSurface(wxCommandEvent &e) {
    ...
    SetStatusText(wxT("Value: ") + ToUnicode(intToStr(surface)) + wxT(" ") + ToUnicode(surface_descs[surface - 1]), siBRUSH_VALUE);
}
...
void MainWindow::onMenuBrushObject(wxCommandEvent &e) {
    ...
    SetStatusText(wxT("Value: ") + ToUnicode(intToStr(object)) + wxT(" ") + ToUnicode(object_descs[object]), siBRUSH_VALUE);
}
...
void MainWindow::onMenuBrushResource(wxCommandEvent &e) {
    ...
    SetStatusText(wxT("Value: ") + ToUnicode(intToStr(resource)) + wxT(" ") + ToUnicode(resource_descs[resource]), siBRUSH_VALUE);
}
...
void MainWindow::onKeyDown(wxKeyEvent &e) {
    ...
    if (e.GetKeyCode() == 'H') {
        ...
        onMenuBrushStartLocation(evt);
    } else if (e.GetKeyCode() == WXK_LEFT && e.GetModifiers() == wxMOD_SHIFT) { // shift-left/right/up/down to
...
void SimpleDialog::addValue(const string &key, const string &value, const string &help) {
    values.push_back(pair<string, string>(key, value + "|" + help)); // I guess I need map<,> instead but I don't
                                                                     // know how to do it
}
...
bool SimpleDialog::show(const string &title, bool wide) {
    ...
    for (Values::iterator it = values.begin(); it != values.end(); ++it) {
        ...
        if (wide) text->SetMinSize(wxSize((text->GetSize().GetWidth()) * 4,
...
MsgDialog::MsgDialog(wxWindow *parent, const wxString &message, const wxString &caption, long style, const wxPoint &pos) {
    m_sizerText = NULL;

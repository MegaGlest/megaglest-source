...
void MainWindow::init(string fname) {
  ...
  for (int i = 1; i <= radiusCount; ++i) {
    menuRadius->AppendCheckItem(
        miRadius + i,
        ToUnicode("&" + intToStr(i) + " (diameter " + intToStr(i * 2 - 1) +
                  ")" + "\tALT+" + intToStr(i)));
  }
...
void SimpleDialog::addValue(const string &key, const string &value,
  ...
  values.push_back(pair<string, string>(
      key, value + "|" + help));  // I guess I need map<,> instead but I don't
                                  // know how to do it
}
...
// #if defined(__MINGW32__)
  ...
  // wxConvCurrent->cWX2MB(wxFNCONV(exe_path)); 		appPath =
  // tmp_buf; #endif

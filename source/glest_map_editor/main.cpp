...
string getGameReadWritePath(const string &lookupKey) {
  ...
  if (path == "" && getenv("GLESTHOME") != NULL) {
    ...
    // path to be used for read/write files
...
const string MainWindow::winHeader =
    "MegaGlest Map Editor " + mapeditorVersionString;

// ===============================================
//	class Global functions
// ===============================================

wxString ToUnicode(const char *str) { return wxString(str, wxConvUTF8); }
...
MainWindow::MainWindow(string appPath)
    : wxFrame(NULL, -1, ToUnicode(winHeader), wxPoint(0, 0), wxSize(1024, 768)),
      glCanvas(NULL), program(NULL), lastX(0), lastY(0), panel(NULL),
      menuBar(NULL), fileDialog(NULL)

      ,
      currentBrush(btHeight), height(0), surface(1), radius(1), object(0),
      resource(0), startLocation(1), enabledGroup(ctHeight),
      fileModified(false), boxsizer(NULL), startupSettingsInited(false) {
  ...
  // default values for random height calculation that turned out to be quite
  // useful
...
void MainToolBar::onMouseMove(wxMouseEvent &event) {
  ...
  if (this->GetParent() != NULL && this->GetParent()->GetParent() != NULL) {
    MainWindow *mainWindow =
        dynamic_cast<MainWindow *>(this->GetParent()->GetParent());
    if (mainWindow != NULL) {
...
void MainWindow::init(string fname) {
  ...
  menuEditMirror->Append(miEditRotatecopyXY,
                         wxT("Rotate Botto&mLeft to TopRight"));
  menuEditMirror->Append(miEditRotatecopyCorner,
                         wxT("Rotate TopLeft &corner to TopRight"));
  ...
  menuBrushSurface->AppendCheckItem(miBrushSurface + 2,
                                    wxT("S&econdary grass"));
  ...
  menuBrushResource->AppendCheckItem(miBrushResource + 2,
                                     wxT("&Gold  (unwalkable)"));
  menuBrushResource->AppendCheckItem(miBrushResource + 3,
                                     wxT("&Stone (unwalkable)"));
  ...
  menuBrushObject->AppendCheckItem(miBrushObject + 1,
                                   wxT("&None (erase)\tALT+0"));
  menuBrushObject->AppendSeparator();
  menuBrushObject->AppendCheckItem(miBrushObject + 2,
                                   wxT("&Tree (harvestable)"));
  menuBrushObject->AppendCheckItem(miBrushObject + 3,
                                   wxT("&Dead tree/Cactuses/Thornbush"));
  menuBrushObject->AppendCheckItem(miBrushObject + 4,
                                   wxT("&Stone (unharvestable)"));
  menuBrushObject->AppendCheckItem(miBrushObject + 5,
                                   wxT("&Bush/Grass/Fern (walkable)"));
  menuBrushObject->AppendCheckItem(
      miBrushObject + 6, wxT("&Water object/Reed/Papyrus (walkable)"));
  menuBrushObject->AppendCheckItem(miBrushObject + 7,
                                   wxT("Big tree/&Old palm"));
  ...
  menuBrushObject->AppendCheckItem(miBrushObject + 11,
                                   wxT("&Invisible blocking object"));
  menuBar->Append(menuBrushObject, wxT("&Object"));

  // ZombiePirate height brush
  menuBrushGradient = new wxMenu();
  for (int i = 0; i < heightCount; ++i) {
    menuBrushGradient->AppendCheckItem(
        miBrushGradient + i + 1,
        ToUnicode((i > 4 ? "&" : "") + intToStr(i - heightCount / 2)));
  }
  ...
  for (int i = 0; i < heightCount; ++i) {
    menuBrushHeight->AppendCheckItem(
        miBrushHeight + i + 1,
        ToUnicode((i > 4 ? "&" : "") + intToStr(i - heightCount / 2)));
  }
  ...
  for (int i = 1; i <= radiusCount; ++i) {
    menuRadius->AppendCheckItem(miRadius + i,
                                ToUnicode("&" + intToStr(i) + " (diameter " +
                                          intToStr(i * 2 - 1) + ")" + "\tALT+" +
                                          intToStr(i)));
  }
  ...
  wxMenuItem *pmi1 = new wxMenuItem(
      menuBrushStartLocation, miBrushStartLocation + 1, wxT("Player &1 Red"));
  pmi1->SetBitmap(wxBitmap(brush_players_red));
  menuBrushStartLocation->Append(pmi1);
  wxMenuItem *pmi2 = new wxMenuItem(
      menuBrushStartLocation, miBrushStartLocation + 2, wxT("Player &2 Blue"));
  pmi2->SetBitmap(wxBitmap(brush_players_blue));
  menuBrushStartLocation->Append(pmi2);
  wxMenuItem *pmi3 = new wxMenuItem(
      menuBrushStartLocation, miBrushStartLocation + 3, wxT("Player &3 Green"));
  pmi3->SetBitmap(wxBitmap(brush_players_green));
  menuBrushStartLocation->Append(pmi3);
  wxMenuItem *pmi4 =
      new wxMenuItem(menuBrushStartLocation, miBrushStartLocation + 4,
                     wxT("Player &4 Yellow"));
  pmi4->SetBitmap(wxBitmap(brush_players_yellow));
  menuBrushStartLocation->Append(pmi4);
  wxMenuItem *pmi5 = new wxMenuItem(
      menuBrushStartLocation, miBrushStartLocation + 5, wxT("Player &5 White"));
  pmi5->SetBitmap(wxBitmap(brush_players_white));
  menuBrushStartLocation->Append(pmi5);
  wxMenuItem *pmi6 = new wxMenuItem(
      menuBrushStartLocation, miBrushStartLocation + 6, wxT("Player &6 Cyan"));
  pmi6->SetBitmap(wxBitmap(brush_players_cyan));
  menuBrushStartLocation->Append(pmi6);
  wxMenuItem *pmi7 =
      new wxMenuItem(menuBrushStartLocation, miBrushStartLocation + 7,
                     wxT("Player &7 Orange"));
  pmi7->SetBitmap(wxBitmap(brush_players_orange));
  menuBrushStartLocation->Append(pmi7);
  wxMenuItem *pmi8 =
      new wxMenuItem(menuBrushStartLocation, miBrushStartLocation + 8,
                     wxT("Player &8 Pink")); // = Light Magenta :-)
  ...
  toolbar->AddTool(miEditRandomizeHeights, _("randomizeHeights"),
                   wxBitmap(edit_randomize_heights), _("Randomize Heights"));
  //	toolbar->AddTool(miEditRandomize, _("randomize"),
  // wxBitmap(edit_randomize), _("Randomize"));
  toolbar->AddTool(miEditSwitchSurfaces, _("switch"),
                   wxBitmap(edit_switch_surfaces), _("Switch Surfaces"));
  toolbar->AddSeparator();
  toolbar->AddTool(miBrushSurface + 1, _("brush_grass1"),
                   wxBitmap(brush_surface_grass1), _("Grass"));
  toolbar->AddTool(miBrushSurface + 2, _("brush_grass2"),
                   wxBitmap(brush_surface_grass2), _("Secondary grass"));
  toolbar->AddTool(miBrushSurface + 3, _("brush_road"),
                   wxBitmap(brush_surface_road), _("Road"));
  toolbar->AddTool(miBrushSurface + 4, _("brush_stone"),
                   wxBitmap(brush_surface_stone), _("Stone"));
  toolbar->AddTool(miBrushSurface + 5, _("brush_custom"),
                   wxBitmap(brush_surface_custom), _("Ground"));
  toolbar->AddSeparator();
  toolbar->AddTool(miBrushResource + 2, _("resource1"),
                   wxBitmap(brush_resource_1_gold), _("gold  (unwalkable)"));
  toolbar->AddTool(miBrushResource + 3, _("resource2"),
                   wxBitmap(brush_resource_2_stone), _("stone (unwalkable)"));
  toolbar->AddTool(miBrushResource + 4, _("resource3"),
                   wxBitmap(brush_resource_3), _("custom3"));
  toolbar->AddTool(miBrushResource + 5, _("resource4"),
                   wxBitmap(brush_resource_4), _("custom4"));
  toolbar->AddTool(miBrushResource + 6, _("resource5"),
                   wxBitmap(brush_resource_5), _("custom5"));
  toolbar->AddSeparator();
  toolbar->AddTool(miBrushObject + 1, _("brush_none"), wxBitmap(brush_none),
                   _("None (erase)"));
  toolbar->AddTool(miBrushObject + 2, _("brush_tree"),
                   wxBitmap(brush_object_tree),
                   _("Tree (unwalkable/harvestable)"));
  toolbar->AddTool(miBrushObject + 3, _("brush_dead_tree"),
                   wxBitmap(brush_object_dead_tree),
                   _("Dead tree/Cactuses/Thornbush (unwalkable)"));
  toolbar->AddTool(miBrushObject + 4, _("brush_stone"),
                   wxBitmap(brush_object_stone),
                   _("Stone (unwalkable/not harvestable)"));
  toolbar->AddTool(miBrushObject + 5, _("brush_bush"),
                   wxBitmap(brush_object_bush),
                   _("Bush/Grass/Fern (walkable)"));
  toolbar->AddTool(miBrushObject + 6, _("brush_water"),
                   wxBitmap(brush_object_water_object),
                   _("Water object/Reed/Papyrus (walkable)"));
  toolbar->AddTool(miBrushObject + 7, _("brush_c1_bigtree"),
                   wxBitmap(brush_object_c1_bigtree),
                   _("Big tree/Old palm (unwalkable/not harvestable)"));
  toolbar->AddTool(miBrushObject + 8, _("brush_c2_hanged"),
                   wxBitmap(brush_object_c2_hanged),
                   _("Hanged/Impaled (unwalkable)"));
  toolbar->AddTool(miBrushObject + 9, _("brush_c3_statue"),
                   wxBitmap(brush_object_c3_statue), _("Statues (unwalkable)"));
  toolbar->AddTool(miBrushObject + 10, _("brush_c4_bigrock"),
                   wxBitmap(brush_object_c4_bigrock),
                   _("Mountain (unwalkable)"));
  toolbar->AddTool(miBrushObject + 11, _("brush_c5_blocking"),
                   wxBitmap(brush_object_c5_blocking),
                   _("Invisible blocking object (unwalkable)"));
  toolbar->AddSeparator();
  toolbar->AddTool(toolPlayer, _("brush_player"),
                   wxBitmap(brush_players_player), _("Player start position"));
  toolbar->Realize();

  wxToolBar *toolbar2 = new MainToolBar(this->panel, wxID_ANY);
  toolbar2->AddTool(miBrushGradient + 1, _("brush_gradient_n5"),
                    wxBitmap(brush_gradient_n5));
  toolbar2->AddTool(miBrushGradient + 2, _("brush_gradient_n4"),
                    wxBitmap(brush_gradient_n4));
  toolbar2->AddTool(miBrushGradient + 3, _("brush_gradient_n3"),
                    wxBitmap(brush_gradient_n3));
  toolbar2->AddTool(miBrushGradient + 4, _("brush_gradient_n2"),
                    wxBitmap(brush_gradient_n2));
  toolbar2->AddTool(miBrushGradient + 5, _("brush_gradient_n1"),
                    wxBitmap(brush_gradient_n1));
  toolbar2->AddTool(miBrushGradient + 6, _("brush_gradient_0"),
                    wxBitmap(brush_gradient_0));
  toolbar2->AddTool(miBrushGradient + 7, _("brush_gradient_p1"),
                    wxBitmap(brush_gradient_p1));
  toolbar2->AddTool(miBrushGradient + 8, _("brush_gradient_p2"),
                    wxBitmap(brush_gradient_p2));
  toolbar2->AddTool(miBrushGradient + 9, _("brush_gradient_p3"),
                    wxBitmap(brush_gradient_p3));
  toolbar2->AddTool(miBrushGradient + 10, _("brush_gradient_p4"),
                    wxBitmap(brush_gradient_p4));
  toolbar2->AddTool(miBrushGradient + 11, _("brush_gradient_p5"),
                    wxBitmap(brush_gradient_p5));
  toolbar2->AddSeparator();
  toolbar2->AddTool(miBrushHeight + 1, _("brush_height_n5"),
                    wxBitmap(brush_height_n5));
  toolbar2->AddTool(miBrushHeight + 2, _("brush_height_n4"),
                    wxBitmap(brush_height_n4));
  toolbar2->AddTool(miBrushHeight + 3, _("brush_height_n3"),
                    wxBitmap(brush_height_n3));
  toolbar2->AddTool(miBrushHeight + 4, _("brush_height_n2"),
                    wxBitmap(brush_height_n2));
  toolbar2->AddTool(miBrushHeight + 5, _("brush_height_n1"),
                    wxBitmap(brush_height_n1));
  toolbar2->AddTool(miBrushHeight + 6, _("brush_height_0"),
                    wxBitmap(brush_height_0));
  toolbar2->AddTool(miBrushHeight + 7, _("brush_height_p1"),
                    wxBitmap(brush_height_p1));
  toolbar2->AddTool(miBrushHeight + 8, _("brush_height_p2"),
                    wxBitmap(brush_height_p2));
  toolbar2->AddTool(miBrushHeight + 9, _("brush_height_p3"),
                    wxBitmap(brush_height_p3));
  toolbar2->AddTool(miBrushHeight + 10, _("brush_height_p4"),
                    wxBitmap(brush_height_p4));
  toolbar2->AddTool(miBrushHeight + 11, _("brush_height_p5"),
                    wxBitmap(brush_height_p5));
  toolbar2->AddSeparator();
  ...
  // program = new Program(glCanvas->GetClientSize().x,
  // glCanvas->GetClientSize().y);
...
void MainWindow::onClose(wxCloseEvent &event) {
  if (program != NULL && program->getMap()->getHasChanged() == true) {
    if (wxMessageDialog(NULL, ToUnicode("Do you want to save the current map?"),
                        ToUnicode("Question"), wxYES_NO | wxYES_DEFAULT)
            .ShowModal() == wxID_YES) {
      wxCommandEvent ev;
...
void MainWindow::setupStartupSettings() {
  ...
  program = new Program(glCanvas->GetClientSize().x,
                        glCanvas->GetClientSize().y, playerName);
  fileName = "New (unsaved) Map";
...
void MainWindow::onMouseMove(wxMouseEvent &event, int x, int y) {
  ...
  if (event.LeftIsDown()) {
    ...
    if (currResource > 0) {
      SetStatusText(wxT("Resource: ") + ToUnicode(resource_descs[currResource]),
                    siCURR_OBJECT);
      resourceUnderMouse = currResource;
      objectUnderMouse = 0;
    } else {
      int currObject = program->getObject(x, y);
      SetStatusText(wxT("Object: ") + ToUnicode(object_descs[currObject]),
                    siCURR_OBJECT);
      resourceUnderMouse = 0;
...
void MainWindow::refreshThings() {
  ...
  wxPaintDC dc(this); // "In a paint event handler must always create a
                      // wxPaintDC object even if you do not use it.  (?)
...
void MainWindow::refreshMapRender() {
  ...
  if (program && glCanvas) {
    if (enabledGroup == ctLocation) {
      program->renderMap(glCanvas->GetClientSize().x,
                         glCanvas->GetClientSize().y);
    } else {
      program->renderMap(glCanvas->GetClientSize().x,
                         glCanvas->GetClientSize().y, &mouse_pos, &radius);
    }
...
void MainWindow::onMenuFileLoad(wxCommandEvent &event) {
  ...
  try {
    ...
    fileDialog->SetWildcard(
        wxT("Glest&Mega Map (*.gbm *.mgm)|*.gbm;*.mgm|Glest Map "
    ...
    MsgDialog(this, ToUnicode(e), wxT("Exception"), wxOK | wxICON_ERROR)
        .ShowModal();
  } catch (const exception &e) {
    MsgDialog(this, ToUnicode(e.what()), wxT("Exception"), wxOK | wxICON_ERROR)
        .ShowModal();
  }
}
...
void MainWindow::onMenuFileExit(wxCommandEvent &event) { Close(); }
...
void MainWindow::onMenuEditReset(wxCommandEvent &event) {
  ...
  simpleDialog.addValue(
      "Width", "128",
      "(must be 16,32,64,128,256,512...)"); // must be an exponent of two
  ...
  try {
    program->reset(strToInt(simpleDialog.getValue("Width")),
                   strToInt(simpleDialog.getValue("Height")),
                   strToInt(simpleDialog.getValue("Altitude")),
                   strToInt(simpleDialog.getValue("Surface")));
    program->resetFactions(
        strToInt(simpleDialog.getValue("Number of players")));
  } catch (const exception &e) {
    MsgDialog(this, ToUnicode(e.what()), wxT("Exception"), wxOK | wxICON_ERROR)
        .ShowModal();
  }
...
void MainWindow::onMenuEditResetPlayers(wxCommandEvent &event) {
  ...
  SimpleDialog simpleDialog;
  simpleDialog.addValue("Number of players",
                        intToStr(program->getMap()->getMaxFactions()));
  if (!simpleDialog.show("Reset players"))
    return;
...
void MainWindow::onMenuEditResize(wxCommandEvent &event) {
  ...
  SimpleDialog simpleDialog;
  simpleDialog.addValue("Width", intToStr(program->getMap()->getW()),
                        "(must be 16,32,64,128,256,512...)");
  simpleDialog.addValue("Height", intToStr(program->getMap()->getH()),
                        "(must be 16,32,64,128,256,512...)");
  if (!simpleDialog.show("Resize"))
    return;

  try {
    program->resize(strToInt(simpleDialog.getValue("Width")),
                    strToInt(simpleDialog.getValue("Height")));
  } catch (const exception &e) {
    MsgDialog(this, ToUnicode(e.what()), wxT("Exception"), wxOK | wxICON_ERROR)
        .ShowModal();
...
void MainWindow::onMenuEditMirrorX(
    wxCommandEvent &event) { // copy left to right
  if (program == NULL) {
    return;
  }
...
void MainWindow::onMenuEditMirrorY(
    wxCommandEvent &event) { // copy top to bottom
  if (program == NULL) {
    return;
  }
...
void MainWindow::onMenuEditMirrorXY(
    wxCommandEvent &event) { // copy bottomleft tp topright
  if (program == NULL) {
    return;
  }
...
void MainWindow::onMenuEditRotatecopyX(
    wxCommandEvent &event) { // copy left to right, rotated
  if (program == NULL) {
    return;
  }
...
void MainWindow::onMenuEditRotatecopyY(
    wxCommandEvent &event) { // copy top to bottom, rotated
  if (program == NULL) {
    return;
  }
...
void MainWindow::onMenuEditRotatecopyXY(
    wxCommandEvent &event) { // copy bottomleft to topright, rotated
  if (program == NULL) {
    return;
  }
...
void MainWindow::onMenuEditRotatecopyCorner(
    wxCommandEvent &event) { // copy top left 1/4 to top right 1/4, rotated
  if (program == NULL) {
    return;
  }
...
void MainWindow::onMenuEditRandomizeHeights(wxCommandEvent &event) {
  ...
  while (true) {
    ...
    SimpleDialog simpleDialog;
    simpleDialog.addValue("Initial Reset", boolToStr(randomWithReset),
                          "(1 = true, 0 = false) If set to '0' no height reset "
                          "is done before calculating");
    simpleDialog.addValue("Min Height", intToStr(randomMinimumHeight),
                          "Lowest random height. example: -300 or below if you "
                          "want water , 0 if you don't want water.");
    simpleDialog.addValue("Max Height", intToStr(randomMaximumHeight),
                          "Max random height. A good value is 400");
    simpleDialog.addValue("Chance Divider", intToStr(randomChanceDivider),
                          "Defines how often you get a hill or hole default is "
                          "30. Bigger number, less hills/holes.");
    simpleDialog.addValue("Smooth Recursions", intToStr(randomRecursions),
                          "Number of recursions cycles to smooth the hills and "
                          "holes. 0<x<50 default is 3.");
    if (!simpleDialog.show("Randomize Height"))
      return;
    ...
    try {
      ...
      if (randomMinimumHeight >= randomMaximumHeight)
        randomMinimumHeight = randomMaximumHeight - 1;
      if (randomChanceDivider < 1)
        randomChanceDivider = 1;

      // set randomRecursions to something useful
      if (randomRecursions < 0)
        randomRecursions = 0;
      if (randomRecursions > 50)
        randomRecursions = 50;

      program->setUndoPoint(ctAll);
      program->randomizeMapHeights(randomWithReset, randomMinimumHeight,
                                   randomMaximumHeight, randomChanceDivider,
                                   randomRecursions);
    } catch (const exception &e) {
      MsgDialog(this, ToUnicode(e.what()), wxT("Exception"),
                wxOK | wxICON_ERROR)
          .ShowModal();
    }
    setDirty();
...
void MainWindow::onMenuEditImportHeights(wxCommandEvent &event) {
  ...
  try {
    ...
    fileDialog->SetWildcard(
        wxT("All "
            "Images|*.bmp;*.png;*.jpg;*.jpeg;*.gif;.*.tga;*.tiff;*.tif|PNG-"
            "Image (*.png)|*.png|JPEG-Image (*.jpg, "
  ...
  if (fd.ShowModal() == wxID_OK) {
    ...
    for (int i = 0; i < map_w; i++) {
      for (int j = 0; j < map_h; j++) {
        ...
        img_data[(i + j * map_w) * 3] =
            (unsigned char)((program->getMap()->getHeight(i, j) -
                             MIN_MAP_CELL_HEIGHT) *
                                255 /
                                (MAX_MAP_CELL_HEIGHT - MIN_MAP_CELL_HEIGHT) +
                            0.5);
        img_data[(i + j * map_w) * 3 + 1] =
            (unsigned char)((program->getMap()->getHeight(i, j) -
                             MIN_MAP_CELL_HEIGHT) *
                                255 /
                                (MAX_MAP_CELL_HEIGHT - MIN_MAP_CELL_HEIGHT) +
                            0.5);
        img_data[(i + j * map_w) * 3 + 2] =
            (unsigned char)((program->getMap()->getHeight(i, j) -
                             MIN_MAP_CELL_HEIGHT) *
                                255 /
                                (MAX_MAP_CELL_HEIGHT - MIN_MAP_CELL_HEIGHT) +
...
void MainWindow::onMenuEditSwitchSurfaces(wxCommandEvent &event) {
  ...
  if (!simpleDialog.show("Switch surfaces"))
    return;

  try {
    program->setUndoPoint(ctSurface);
    program->switchMapSurfaces(strToInt(simpleDialog.getValue("Surface1")),
...
void MainWindow::onMenuEditInfo(wxCommandEvent &event) {
  ...
  if (!simpleDialog.show("Info", true))
    return;

  bool ischanged = program->setMapTitle(simpleDialog.getValue("Title"));
  ischanged =
      (program->setMapDesc(simpleDialog.getValue("Description")) || ischanged);
  ischanged =
      (program->setMapAuthor(simpleDialog.getValue("Author")) || ischanged);
  if (ischanged)
    if (!isDirty()) {
      setDirty(true);
    }
...
void MainWindow::onMenuEditAdvanced(wxCommandEvent &event) {
  ...
  SimpleDialog simpleDialog;
  simpleDialog.addValue(
      "Height Factor", intToStr(program->getMap()->getHeightFactor()),
      "lower means more hill effect. Numbers above 100 are handled like "
      "this:\nx=x/100 ,so a 150 will mean 1.5 in the game.");
  simpleDialog.addValue(
      "Water Level", intToStr(program->getMap()->getWaterLevel()),
      "water is visible below this, and walkable until 1.5 less");
  simpleDialog.addValue(
      "Cliff Level", intToStr(program->getMap()->getCliffLevel()),
      "neighboring fields with at least this heights difference are cliffs");
  simpleDialog.addValue(
      "Camera Height", intToStr(program->getMap()->getCameraHeight()),
      "you can give a camera height here default is 0 ;ignored if <20");
  if (!simpleDialog.show("Advanced"))
    return;

  try {
    program->setMapAdvanced(strToInt(simpleDialog.getValue("Height Factor")),
...
void MainWindow::onMenuViewGrid(wxCommandEvent &event) {
  ...
  menuView->Check(miViewGrid,
                  program->setGridOnOff()); // miViewGrid event.GetId()
  refreshThings();
}
...
void MainWindow::onMenuViewHeightMap(wxCommandEvent &event) {
  ...
  menuView->Check(miViewHeightMap,
                  program->setHeightMapOnOff()); // miViewGrid event.GetId()
  refreshThings();
}
void MainWindow::onMenuHideWater(wxCommandEvent &event) {
  ...
  menuView->Check(miHideWater,
                  program->setHideWaterOnOff()); // miViewGrid event.GetId()
  refreshThings();
}
void MainWindow::onMenuViewAbout(wxCommandEvent &event) {
  MsgDialog(this,
            wxT("\n    Glest Map Editor\n    Copyright 2004-2010 The Glest "
...
void MainWindow::onMenuBrushSurface(wxCommandEvent &e) {
  ...
  SetStatusText(wxT("Value: ") + ToUnicode(intToStr(surface)) + wxT(" ") +
                    ToUnicode(surface_descs[surface - 1]),
                siBRUSH_VALUE);
}
...
void MainWindow::onMenuBrushObject(wxCommandEvent &e) {
  ...
  SetStatusText(wxT("Value: ") + ToUnicode(intToStr(object)) + wxT(" ") +
                    ToUnicode(object_descs[object]),
                siBRUSH_VALUE);
}
...
void MainWindow::onMenuBrushResource(wxCommandEvent &e) {
  ...
  SetStatusText(wxT("Value: ") + ToUnicode(intToStr(resource)) + wxT(" ") +
                    ToUnicode(resource_descs[resource]),
                siBRUSH_VALUE);
}
...
void MainWindow::onMenuBrushStartLocation(wxCommandEvent &e) {
  ...
  SetStatusText(wxT("Value: ") + ToUnicode(intToStr(startLocation)),
                siBRUSH_VALUE);
}
...
void MainWindow::onKeyDown(wxKeyEvent &e) {
  ...
  // WARNING: don't add any Ctrl or ALt key shortcuts below those are reserved
  // for internal menu use.
  ...
  if (e.GetKeyCode() == 'H') {
    wxCommandEvent evt(wxEVT_NULL,
                       miBrushHeight + height + heightCount / 2 + 1);
    ...
    wxCommandEvent evt(wxEVT_NULL,
                       miBrushGradient + height + heightCount / 2 + 1);
    ...
    onMenuBrushStartLocation(evt);
             ...
             e.GetModifiers() == wxMOD_SHIFT) { // shift-left/right/up/down to
                                                // shift the map one square
    program->setUndoPoint(ctAll);
    program->shiftLeft();
    setDirty();
...
// these are 'handled' by GlCanvas and funneled to these handlers. See
// BEGIN_EVENT_TABLE(GlCanvas, wxGLCanvas) below.
...
EVT_MENU_RANGE(miBrushHeight + 1, miBrushHeight + heightCount,
               MainWindow::onMenuBrushHeight)
EVT_MENU_RANGE(miBrushGradient + 1, miBrushGradient + heightCount,
               MainWindow::onMenuBrushGradient)
EVT_MENU_RANGE(miBrushSurface + 1, miBrushSurface + surfaceCount,
               MainWindow::onMenuBrushSurface)
EVT_MENU_RANGE(miBrushObject + 1, miBrushObject + objectCount,
               MainWindow::onMenuBrushObject)
EVT_MENU_RANGE(miBrushResource + 1, miBrushResource + resourceCount,
               MainWindow::onMenuBrushResource)
EVT_MENU_RANGE(miBrushStartLocation + 1,
               miBrushStartLocation + startLocationCount,
               MainWindow::onMenuBrushStartLocation)
...
#if wxCHECK_VERSION(2, 9, 1)
    : wxGLCanvas(parent, -1, args, wxDefaultPosition, wxDefaultSize, 0,
                 wxT("GLCanvas")) {
...
void GlCanvas::onMouseWheel(wxMouseEvent &event) {
  if (event.GetWheelRotation() > 0)
    mainWindow->onMouseWheelDown(event);
  else
    mainWindow->onMouseWheelUp(event);
}
...
void GlCanvas::onKeyUp(wxKeyEvent &event) { mainWindow->onKeyUp(event); }

void GlCanvas::onPaint(wxPaintEvent &event) {
  //	wxPaintDC dc(this); //N "In a paint event handler must always create a
  // wxPaintDC object even if you do not use it.  (?)
...
EVT_PAINT(
    GlCanvas::onPaint) // Because the drawing area needs to be repainted too.
END_EVENT_TABLE()
...
void SimpleDialog::addValue(const string &key, const string &value,
                            const string &help) {
  values.push_back(pair<string, string>(
      key,
      value + "|" +
          help)); // I guess I need map<,> instead but I don't know how to do it
}
...
bool SimpleDialog::show(const string &title, bool wide) {
  ...
  for (Values::iterator it = values.begin(); it != values.end(); ++it) {
    ...
    wxTextCtrl *text =
        new wxTextCtrl(this, -1, ToUnicode(it->second.substr(0, helptextpos)));
    if (wide)
      text->SetMinSize(
          wxSize((text->GetSize().GetWidth()) * 4,
                 text->GetSize().GetHeight())); // 4 time as wide as default
    sizer->Add(text, 0, wxALL, 5);
    texts.push_back(text);
    sizer->Add(new wxStaticText(this, -1,
                                ToUnicode(it->second.substr(helptextpos + 1))),
               0, wxALL, 5);
  }
  sizer2->Add(sizer);
  sizer2->Add(CreateButtonSizer(wxOK | wxCANCEL), 0,
              wxALIGN_RIGHT); // enable Cancel button
  SetSizerAndFit(sizer2);

  ShowModal();
  if (m_returnCode == wxID_CANCEL)
    return false; // don't change values if canceled
  ...
  if (argc == 2) {
    if (argv[1][0] == '-') { // any flag gives help and exits program.
      std::cout << std::endl
                << "MegaGlest map editor " << mapeditorVersionString
                << " [Using "
                << (const char *)wxConvCurrent->cWX2MB(wxVERSION_STRING) << "]"
                << std::endl
                << std::endl;
      // std::cout << "\nglest_map_editor [MGM FILE]" << std::endl << std::endl;
      std::cout << "Creates or edits megaglest/glest maps. [.mgm/.gbm]"
                << std::endl
                << std::endl;
      ...
      //			std::cout << " ~ more helps should be written
      // here ~"  << std::endl;
      std::cout << std::endl;
      exit(0);
    }
// #if defined(__MINGW32__)
  ...
  // wxConvCurrent->cWX2MB(wxFNCONV(exe_path)); 		appPath = tmp_buf;
  // #endif
...
MsgDialog::MsgDialog(wxWindow *parent, const wxString &message,
                     const wxString &caption, long style, const wxPoint &pos) {

  m_sizerText = NULL;
  if (!wxDialog::Create(parent, wxID_ANY, caption, pos, wxDefaultSize, style)) {
    return;
  }

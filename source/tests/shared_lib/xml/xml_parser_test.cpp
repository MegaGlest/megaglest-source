...
void createValidXMLTestFile(const string &test_filename) {
  ...
  xmlFile << "<?xml version=\"1.0\"?>" << std::endl
          ...
          << "<menu-background-model "
             "value=\"data/core/menu/main_model/menu_main1.g3d\"/>"
          << std::endl
          << "</menu>" << std::endl;
  xmlFile.close();
}
...
public:
  SafeRemoveTestFile(const string &filename) { this->filename = filename; }
  ~SafeRemoveTestFile() { removeTestFile(this->filename); }
};
//
...
class XmlIoTest : public CppUnit::TestFixture {
  ...
  CPPUNIT_TEST_EXCEPTION(test_load_file_malformed_content,
...
public:
  ...
  void test_load_file_missing() {
    XmlNode *rootNode = XmlIo::getInstance().load(
        "/some/path/that/does/not exist", std::map<string, string>());
    delete rootNode;
  }
  void test_load_file_valid() {
    ...
    CPPUNIT_ASSERT_EQUAL(string("menu"),
                         (rootNode != NULL ? rootNode->getName() : string("")));
  ...
  void test_save_file_valid_node() {
    ...
    XmlNode *rootNode = XmlIo::getInstance().load(test_filename_load,
...
public:
  ...
  void test_load_file_missing() {
    XmlNode *rootNode = XmlIoRapid::getInstance().load(
        "/some/path/that/does/not exist", std::map<string, string>());
    delete rootNode;
  }
  void test_load_file_valid() {
    ...
    XmlNode *rootNode = XmlIoRapid::getInstance().load(
        test_filename, std::map<string, string>());
  ...
  void test_save_file_valid_node() {
    ...
    XmlNode *rootNode = XmlIoRapid::getInstance().load(
        test_filename_load, std::map<string, string>());
...
class XmlTreeTest : public CppUnit::TestFixture {
  ...
  CPPUNIT_TEST_EXCEPTION(test_invalid_xml_engine_lowerbound,
                         megaglest_runtime_error);
  CPPUNIT_TEST_EXCEPTION(test_invalid_xml_engine_upperbound,
                         megaglest_runtime_error);
  CPPUNIT_TEST(test_valid_xml_engine);
  CPPUNIT_TEST(test_init);
  CPPUNIT_TEST_EXCEPTION(test_load_simultaneously_same_file,
                         megaglest_runtime_error);
...
public:
  ...
  void test_invalid_xml_engine_lowerbound() {
    xml_engine_parser_type testType =
        static_cast<xml_engine_parser_type>(XML_XERCES_ENGINE - 1);
  ...
  void test_invalid_xml_engine_lowerbound() {
    xml_engine_parser_type testType =
        static_cast<xml_engine_parser_type>(XML_RAPIDXML_ENGINE - 1);
  ...
  void test_init() {
    ...
    CPPUNIT_ASSERT(xmlInstance.getRootNode() != NULL);
    CPPUNIT_ASSERT_EQUAL(string("testRoot"),
                         xmlInstance.getRootNode()->getName());
  }
...
private:
  ...
  public:
    ...
    void manualParserRelease() { XmlIo::releaseDOMParser(); }
  };
...
public:
  ...
  void test_child_nodes() {
    ...
    XmlNode *childChildNode2 =
        childNode1->addChild("childchild2", "testValue2");
    CPPUNIT_ASSERT_EQUAL((size_t)2, childNode1->getChildCount());
    CPPUNIT_ASSERT_EQUAL(string("testValue2"), childChildNode2->getText());

    XmlNode *childChildNode3 =
        childNode1->addChild("childchild2", "testValue3");
    ...
    CPPUNIT_ASSERT_EQUAL(string("child2xValue"),
                         node.getChild("child2", 1)->getText());
  ...
  void test_node_attributes() {
    ...
    CPPUNIT_ASSERT_EQUAL((XmlAttribute *)NULL,
                         node.getAttribute("some-attribute", false));
    CPPUNIT_ASSERT_EQUAL(false, node.hasAttribute("some-attribute"));

    std::map<string, string> mapTagReplacementValues;
    XmlAttribute *attribute1 = node.addAttribute("some-attribute", "some-value",
    ...
    CPPUNIT_ASSERT_EQUAL(string("some-attribute"),
...
class XmlAttributeTest : public CppUnit::TestFixture {
  ...
  CPPUNIT_TEST(test_node_attributes);
  CPPUNIT_TEST_EXCEPTION(test_node_attributes_restricted,
                         megaglest_runtime_error);
  CPPUNIT_TEST_EXCEPTION(test_node_attributes_int_outofrange,
                         megaglest_runtime_error);
  CPPUNIT_TEST_EXCEPTION(test_node_attributes_float_outofrange,
                         megaglest_runtime_error);
...
public:
  ...
  void test_node_attributes() {
    ...
    // Nasty floating point issues shown by this test sometimes may need to
    // comment out
    attr.setValue("123456.123456");
    CPPUNIT_ASSERT_DOUBLES_EQUAL(
        123456.123456f, attr.getFloatValue(123456.01f, 123456.999f), 1e-6);
  }

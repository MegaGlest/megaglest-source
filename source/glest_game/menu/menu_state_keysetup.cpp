...
MenuStateKeysetup::MenuStateKeysetup(Program *program, MainMenu *mainMenu,
    ...
    : MenuState(program, mainMenu, "config"), buttonOk("KeySetup", "buttonOk"),
  ...
  try {
    ...
    for (int i = 0; i < (int)mergedProperties.size(); ++i) {
      ...
      GraphicButton *button = new GraphicButton(
          containerName, string("ScrollButton") + intToStr(i));
      ...
      GraphicLabel *label =
          new GraphicLabel(containerName, string("ScrollLabel") + intToStr(i));
    ...
    keyScrollBar.init(keyButtonsXBase + keyButtonsWidth + labelWidth + 20, 200,
                      false, 200, 20);
    keyScrollBar.setLength(400);
  ...
  if (mainMessageBox.getEnabled()) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d] delete file [%s] returned %d\n", __FILE__,
             __FUNCTION__, __LINE__, userKeysFile.c_str(), result);
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    ...
    if (userProperties.empty() == false) {
      ...
      string userKeysFile = configKeys.getFileName(true);
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("In [%s::%s Line: %d] save file [%s] userProperties.size() "
               "= " MG_SIZE_T_SPECIFIER "\n",
               __FILE__, __FUNCTION__, __LINE__, userKeysFile.c_str(),
               userProperties.size());
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                 "In [%s::%s Line: %d] save file [%s] "
                                 "userProperties.size() = " MG_SIZE_T_SPECIFIER
                                 "\n",
                                 __FILE__, __FUNCTION__, __LINE__,
...
void MenuStateKeysetup::update() {
  ...
  if (keyScrollBar.getElementCount() != 0) {
    for (int i = keyScrollBar.getVisibleStart();
         i <= keyScrollBar.getVisibleEnd(); ++i) {
      keyButtons[i]->setY(keyButtonsYBase -
                          keyButtonsLineHeight *
                              (i - keyScrollBar.getVisibleStart()));
...
void MenuStateKeysetup::showMessageBox(const string &text, const string &header,
                                       bool toggle) {
  if (!toggle) {
    mainMessageBox.setEnabled(false);
  }
...
void MenuStateKeysetup::keyDown(SDL_KeyboardEvent key) {
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("In [%s::%s Line: %d] keyName [%s] char [%d][%d]\n", __FILE__,
           __FUNCTION__, __LINE__, keyName.c_str(), hotkeyChar, key.keysym.sym);
  keyName = SDL_GetKeyName(hotkeyChar);
  // printf ("In [%s::%s Line: %d] keyName [%s] char
  ...
  //	if(keyName == "unknown key" || keyName == "") {
  //		Config &configKeys =
  // Config::getInstance(std::pair<ConfigType,ConfigType>(cfgMainKeys,cfgUserKeys));
  ...
  char szBuf[8096] = "";
  snprintf(szBuf, 8096, "  %s [%s][%d][%d][%d]", keyName.c_str(), utfStr,
           key.keysym.sym, hotkeyChar, key.keysym.mod);
  labelTestValue.setText(szBuf);
  // printf ("In [%s::%s Line: %d] szBuf
    ...
    if (hotkeyChar != 0) {
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("In [%s::%s Line: %d] char [%d][%d]\n", __FILE__, __FUNCTION__,
               __LINE__, hotkeyChar, key.keysym.sym);
      ...
      // Config::getInstance(std::pair<ConfigType,ConfigType>(cfgMainKeys,cfgUserKeys));
      //				keysym =
      // configKeys.translateSpecialStringToSDLKey(hotkeyChar);
      //
      //				if(SystemFlags::VERBOSE_MODE_ENABLED)
      // printf ("In [%s::%s Line: %d] keysym
      //[%d]\n",__FILE__,__FUNCTION__,__LINE__,keysym);
      //
      //				// SDL skips capital letters
      //				if(keysym >= 65 && keysym <= 90) {
      //					keysym = (SDLKey)((int)keysym +
      // 32);
      //				}
      //				if(keysym < 255) {
      //					key = keysym;
      ...
      if (keyName != "unknown key") {
        ...
        if (isNewUserKeyEntry == true) {
          ...
          //					if(keysym <= SDLK_ESCAPE || keysym >
          //255) { 						if(keysym <=
          //SDLK_ESCAPE) { 							newNameValuePair.second
          //= intToStr(extractKeyPressed(key));
          //						}
          //						else {
          //							newNameValuePair.second
          //= keyName;
          //						}
          //					}
          //					else {
          //						newNameValuePair.second
          //= extractKeyPressed(key);
          //					}

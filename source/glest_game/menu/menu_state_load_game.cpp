...
MenuStateLoadGame::MenuStateLoadGame(Program *program, MainMenu *mainMenu) : MenuState(program, mainMenu, "root") {
                        ...
                        try {
                            ...
                            xmlTree->loadAsync(filename, Properties::getTagReplacementValues(&mapExtraTagReplacementValues), true, false, true)
                                ->then([this, xmlTree, &lang, filename]() {
                                    ...
                                    char szBuf[8096] = "";
                                    snprintf(
                                        szBuf, 8096, lang.getString("LoadSavedGameInfo").c_str(), newGameSettings.getMap().c_str(),
                                        newGameSettings.getTileset().c_str(), newGameSettings.getTech().c_str(), newGameSettings.getScenario().c_str(),
                                        newGameSettings.getFactionCount(),
                                        (newGameSettings.getThisFactionIndex() >= 0 && newGameSettings.getThisFactionIndex() < newGameSettings.getFactionCount()
...
void MenuStateLoadGame::render() {
    ...
    if (versionWarningLabel.getText() != "") renderer.renderLabel(&versionWarningLabel);

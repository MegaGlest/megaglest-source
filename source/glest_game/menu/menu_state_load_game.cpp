...
MenuStateLoadGame::MenuStateLoadGame(Program *program, MainMenu *mainMenu)
  ...
  slotsScrollBar.init(500 - 20,
                      slotLinesYBase - slotsLineHeight * (slotsToRender - 1),
                      false, slotWidth, 20);
...
void MenuStateLoadGame::mouseDoubleClick(int x, int y,
                                         MouseButton mouseButton) {
  if (mouseButton == mbLeft && selectedButton != NULL &&
      selectedButton->mouseClick(x, y)) {
...
void MenuStateLoadGame::mouseClick(int x, int y, MouseButton mouseButton) {
  ...
  if (abortButton.mouseClick(x, y)) {
    ...
    if (selectedButton == NULL) {
      ...
      char szBuf[8096] = "";
      snprintf(szBuf, 8096,
               lang.getString("LoadGameDeletingFile", "", true).c_str(),
               filename.c_str());
      ...
      for (int i = slotsScrollBar.getVisibleStart();
        ...
        if (slots[i]->mouseClick(x, y) && selectedButton != slots[i]) {
          ...
          if (fileExists(filename) == true) {
            ...
            if (Config::getInstance().getBool("ForceXMLLoadGameUsingXerces",
                                              "false") == true) {
            ...
            try {
              ...
              if (rootNode == NULL) {
                char szBuf[8096] = "";
                snprintf(szBuf, 8096, "Invalid XML saved game file: [%s]",
                         filename.c_str());
              ...
              if (gameVer != glestVersionString &&
                  checkVersionComptability(gameVer, glestVersionString) ==
                      false) {
                char szBuf[8096] = "";
                snprintf(szBuf, 8096,
...
void MenuStateLoadGame::render() {
  ...
  if (previewTexture != NULL) {
    renderer.renderTextureQuad(550, slotLinesYBase - 300 + slotsLineHeight, 400,
                               300, previewTexture, 1.0f);
  }
...
void MenuStateLoadGame::showMessageBox(const string &text, const string &header,
                                       bool toggle) {
  if (toggle == false) {
    mainMessageBox.setEnabled(false);
  }

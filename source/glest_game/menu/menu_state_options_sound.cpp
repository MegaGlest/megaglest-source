...
MenuStateOptionsSound::MenuStateOptionsSound(Program *program,
  ...
  try {
    ...
    listBoxVolumeFx.setSelectedItem(
        intToStr(config.getInt("SoundVolumeFx") / 5 * 5));
    listBoxVolumeAmbient.setSelectedItem(
        intToStr(config.getInt("SoundVolumeAmbient") / 5 * 5));
    listBoxVolumeMusic.setSelectedItem(
        intToStr(config.getInt("SoundVolumeMusic") / 5 * 5));
...
void MenuStateOptionsSound::mouseClick(int x, int y, MouseButton mouseButton) {
  ...
  if (mainMessageBox.getEnabled()) {
    ...
    mainMenu->setState(new MenuStateKeysetup(
        program, mainMenu)); // open keyboard shortcuts setup screen
...
void MenuStateOptionsSound::saveConfig() {
  ...
  CoreData::getInstance().getMenuMusic()->setVolume(
      strToInt(listBoxVolumeMusic.getSelectedItem()) / 100.f);

...
void MenuStateJoinGame::DiscoveredServers(std::vector<string> serverList) {
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("In [%s::%s Line: %d] serverList.size() = %d\n", __FILE__,
  ...
  if (serverList.empty() == false) {
    Config &config = Config::getInstance();
    string bestIPMatch = "";
    int serverGamePort = config.getInt(
    ...
    if (serverList.size() > 1) {
      listBoxServerType.setSelectedItemIndex(
          MenuStateJoinGame::foundServersIndex);
...
void MenuStateJoinGame::mouseClick(int x, int y, MouseButton mouseButton) {
  ...
  if (clientInterface->isConnected() == false) {
    ...
    if (listBoxServerType.mouseClick(x, y)) {
      if (!listBoxServers.getText().empty()) {
        labelServerIp.setText(servers.getString(listBoxServers.getText()) +
                              "_");
      }
    }
    // server list
  ...
  // connect
    ...
    abortAutoFind = true;
    mainMenu->setState(
        new MenuStateCustomGame(program, mainMenu, true, pLanGame));
    return;
  }
  ...
  if (clientInterface->isConnected()) {
    ...
    if (clientInterface != NULL && clientInterface->isConnected()) {
      ...
      if (clientInterface->getLaunchGame()) {
        ...
        clientInterface->stopServerDiscovery();
        program->setState(
            new Game(program, clientInterface->getGameSettings(), false));
        return;
      }
    }

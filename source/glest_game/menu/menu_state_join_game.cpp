...
MenuStateJoinGame::MenuStateJoinGame(Program *program, MainMenu *mainMenu, bool *autoFindHost) : MenuState(program, mainMenu, "join-game") {
...
MenuStateJoinGame::MenuStateJoinGame(Program *program, MainMenu *mainMenu, bool connect, Ip serverIp, int portNumberOverride)
...
void MenuStateJoinGame::DiscoveredServers(std::vector<string> serverList) {
    ...
    if (serverList.empty() == false) {
        ...
        for (int idx = 0; idx < (int)serverList.size(); idx++) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] bestIPMatch = [%s]\n", __FILE__, __FUNCTION__, __LINE__,
                                         bestIPMatch.c_str());
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] bestIPMatch = [%s] localIPList[0] = [%s]\n", __FILE__, __FUNCTION__,
                                         __LINE__, bestIPMatch.c_str(), localIPList[0].c_str());
...
void MenuStateJoinGame::mouseClick(int x, int y, MouseButton mouseButton) {
    ...
    if (buttonReturn.mouseClick(x, y)) {
        ...
        if (clientInterface->getSocket() != NULL) {
            ...
            //    string sQuitText =
            //    Config::getInstance().getString("NetPlayerName",Socket::getHostName().c_str())
            //    + " has chosen to leave the game!";
    ...
    // connect
        ...
        // Triggers a thread which calls back into
        // MenuStateJoinGame::DiscoveredServers with the results
        if (clientInterface->isConnected() == false) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n", __FILE__, __FUNCTION__, __LINE__);
...
void MenuStateJoinGame::update() {
    ...
    if (clientInterface != NULL && clientInterface->getLaunchGame())
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s] clientInterface->getLaunchGame() - D\n", __FILE__, __FUNCTION__);
}
...
bool MenuStateJoinGame::connectToServer() {
    ...
    for (time_t elapsedWait = time(NULL);
         clientInterface->getIntroDone() == false && clientInterface->isConnected() && difftime(time(NULL), elapsedWait) <= 10;) {
        if (clientInterface->isConnected()) {
            // update lobby
            clientInterface->updateLobby();
            sleep(0);

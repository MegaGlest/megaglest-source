...
MenuBackground::MenuBackground() : rps(NULL) {
  ...
  if (water) {
    ...
    if (waterTexture) {
      ...
      waterTexture->getPixmap()->load(getGameCustomCoreDataPath(
  ...
  if (menuNode->hasChild("menu-background-model") == true) {
    ...
    const XmlNode *mainMenuModelNode =
        menuNode->getChild("menu-background-model");
    mainModelFile =
        mainMenuModelNode->getAttribute("value")->getRestrictedValue();
  }
  mainModel = renderer.newModel(
      rsMenu, getGameCustomCoreDataPath(data_path, mainModelFile));

  // models
  for (int i = 0; i < 5; ++i) {
    characterModels[i] = renderer.newModel(
        rsMenu, getGameCustomCoreDataPath(
                    data_path, "data/core/menu/about_models/character" +
                                   intToStr(i) + ".g3d"));
  }
...
void MenuBackground::update() {
  ...
  // animation
  anim += (0.6f / GameConstants::updateFps) / 5 +
          random.randRange(0.f, (0.6f / GameConstants::updateFps) / 5.f);
  if (anim > 1.f) {
    anim = 0.f;
  }
}

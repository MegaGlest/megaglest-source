...
MenuStateRoot::~MenuStateRoot() {
    if (updatesHttpServerThread != NULL) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line %d]\n", __FILE__, __FUNCTION__, __LINE__);
        if (updatesHttpServerThread->canShutdown(true) == true && updatesHttpServerThread->shutdownAndWait() == true) {
...
void MenuStateRoot::startFTPClientIfRequired() {
    if (ftpClientThread == NULL) {
        ...
        ftpClientThread =
            new FTPClientThread(-1, "", mapsPath, tilesetsPath, techtreesPath, scenariosPath, this, fileArchiveExtension, fileArchiveExtractCommand,
...
void MenuStateRoot::FTPClient_CallbackEvent(string itemName, FTP_Client_CallbackType type, pair<FTP_Client_ResultType, string> result, void *userdata) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line %d]\n", __FILE__, __FUNCTION__, __LINE__);
        ...
        if (result.first == ftp_crt_SUCCESS) {
            ...
            if (result == false) {
                ...
                string binaryNameOld =
                    Properties::getApplicationPath() + extractFileFromDirectoryPath(PlatformExceptionHandler::application_binary) + "__REMOVE";
            ...
            char szBuf[8096] = "";
            snprintf(szBuf, 8096, "FAILED to download the updates: [%s] using CURL version [%s] [%s]", itemName.c_str(), curlVersion->version,
                     result.second.c_str());
            console.addLine(szBuf, true);
...
void MenuStateRoot::render() {
    ...
    renderer.renderTextureQuad(logoMainX, logoMainY, logoMainW, logoMainH, coreData.getLogoTexture(), GraphicComponent::getFade());
            ...
            if (compareResult < 0) {
                ...
                if (props.getBool("AllowUpdateDownloads", "false") == false || ftpFileURL == "") {
                    ...
                    snprintf(szMsg, 8096,
                             ...
                             props.getString("LatestGameVersion", "?").c_str(), props.getString("LatestGameVersionReleaseDate", "?").c_str());
...
void MenuStateRoot::keyDown(SDL_KeyboardEvent key) {
    ...
    // else if(mainMessageBox.getEnabled() == true && key == vkReturn) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] keystate.mod [%d]\n", __FILE__, __FUNCTION__, __LINE__, keystate.mod);

...
MenuStateOptionsGraphics::MenuStateOptionsGraphics(Program *program, MainMenu *mainMenu, ProgramState **parentUI)
    : MenuState(program, mainMenu, "config"), buttonOk("Options_Graphics", "buttonOk"), buttonReturn("Options_Graphics", "buttonReturn"),
      buttonAutoConfig("Options_Graphics", "buttonAutoConfig"), buttonVideoInfo("Options_Graphics", "buttonVideoInfo"),

      buttonKeyboardSetup("Options_Graphics", "buttonKeyboardSetup"), buttonVideoSection("Options_Graphics", "buttonVideoSection"),
      buttonAudioSection("Options_Graphics", "buttonAudioSection"), buttonMiscSection("Options_Graphics", "buttonMiscSection"),
      buttonNetworkSettings("Options_Graphics", "buttonNetworkSettings"),

      labelShadows("Options_Graphics", "labelShadows"), listBoxShadows("Options_Graphics", "listBoxShadows"), labelFilter("Options_Graphics", "labelFilter"),
      listBoxFilter("Options_Graphics", "listBoxFilter"), labelFilterMaxAnisotropy("Options_Graphics", "labelFilterMaxAnisotropy"),
      listBoxFilterMaxAnisotropy("Options_Graphics", "listBoxFilterMaxAnisotropy"),

      labelTextures3D("Options_Graphics", "labelTextures3D"), checkBoxTextures3D("Options_Graphics", "checkBoxTextures3D"),
      labelLights("Options_Graphics", "labelLights"), listBoxLights("Options_Graphics", "listBoxLights"),
      labelUnitParticles("Options_Graphics", "labelUnitParticles"), checkBoxUnitParticles("Options_Graphics", "checkBoxUnitParticles"),

      labelTilesetParticles("Options_Graphics", "labelTilesetParticles"), checkBoxTilesetParticles("Options_Graphics", "checkBoxTilesetParticles"),
      ...
      labelScreenModes("Options_Graphics", "labelScreenModes"), listBoxScreenModes("Options_Graphics", "listBoxScreenModes"),

      labelFullscreenWindowed("Options_Graphics", "labelFullscreenWindowed"), checkBoxFullscreenWindowed("Options_Graphics", "checkBoxFullscreenWindowed"),

      labelMapPreview("Options_Graphics", "labelMapPreview"), checkBoxMapPreview("Options_Graphics", "checkBoxMapPreview"),
      ...
      labelRainEffect("Options_Graphics", "labelRainEffect"), labelRainEffectSeparator("Options_Graphics", "labelRainEffectSeparator"),
      checkBoxRainEffect("Options_Graphics", "checkBoxRainEffect"), checkBoxRainEffectMenu("Options_Graphics", "checkBoxRainEffectMenu"),

      labelGammaCorrection("Options_Graphics", "labelGammaCorrection"), listBoxGammaCorrection("Options_Graphics", "listBoxGammaCorrection"),

      labelShadowIntensity("Options_Graphics", "labelShadowIntensity"), listBoxShadowIntensity("Options_Graphics", "listBoxShadowIntensity"),

      labelShadowTextureSize("Options_Graphics", "labelShadowTextureSize"), listBoxShadowTextureSize("Options_Graphics", "listBoxShadowTextureSize"),

      labelVideos("Options_Graphics", "labelVideos"), checkBoxVideos("Options_Graphics", "checkBoxVideos"),

      labelSelectionType("Options_Graphics", "labelSelectionType"), listBoxSelectionType("Options_Graphics", "listBoxSelectionType")

{
    try {
        ...
        string currentResString = config.getString("ScreenWidth") + "x" + config.getString("ScreenHeight") + "-" + intToStr(config.getInt("ColorBits"));
...
void MenuStateOptionsGraphics::revertScreenMode() {
    ...
    window->ChangeVideoMode(true, config.getInt("ScreenWidth"), config.getInt("ScreenHeight"), !config.getBool("Windowed"), config.getInt("ColorBits"),
                            config.getInt("DepthBits"), config.getInt("StencilBits"), config.getBool("HardwareAcceleration", "false"),
                            config.getBool("FullScreenAntiAliasing", "false"), config.getFloat("GammaValue", "0.0"));
...
void MenuStateOptionsGraphics::mouseClick(int x, int y, MouseButton mouseButton) {
    ...
    if (mainMessageBox.getEnabled()) {
        ...
        if (currentResolution != selectedResolution || currentFullscreenWindowed != selectedFullscreenWindowed) {
            ...
            if (selectedMode == NULL) { // if we cannot find the selectedResolution
                                        // we try it with current one
                for (vector<ModeInfo>::const_iterator it = modeInfos.begin(); it != modeInfos.end(); ++it) {
...
#else
            WindowGl *window = this->program->getWindow();
            window->ChangeVideoMode(true, selectedMode->width, selectedMode->height, !selectedFullscreenWindowed, selectedMode->depth,
                                    config.getInt("DepthBits"), config.getInt("StencilBits"), config.getBool("HardwareAcceleration", "false"),
                                    config.getBool("FullScreenAntiAliasing", "false"), strToFloat(listBoxGammaCorrection.getSelectedItem()));
        ...
        soundRenderer.playFx(coreData.getClickSoundA());
        // mainMenu->setState(new MenuStateKeysetup(program, mainMenu)); // open
        // keyboard shortcuts setup screen mainMenu->setState(new
        // MenuStateOptionsGraphics(program, mainMenu)); // open keyboard shortcuts
        // setup screen mainMenu->setState(new MenuStateOptionsNetwork(program,
        // mainMenu)); // open keyboard shortcuts setup screen
        ...
        soundRenderer.playFx(coreData.getClickSoundA());
        mainMenu->setState(new MenuStateOptionsSound(program, mainMenu,
                                                     this->parentUI)); // open keyboard shortcuts setup screen
        return;
    } else if (buttonNetworkSettings.mouseClick(x, y)) {
        soundRenderer.playFx(coreData.getClickSoundA());
        mainMenu->setState(new MenuStateOptionsNetwork(program, mainMenu,
                                                       this->parentUI)); // open keyboard shortcuts setup screen
        return;
        ...
        soundRenderer.playFx(coreData.getClickSoundA());
        // mainMenu->setState(new MenuStateOptionsGraphics(program,

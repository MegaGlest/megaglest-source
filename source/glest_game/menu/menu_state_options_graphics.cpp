...

{
  try {
    ...
    ::Shared::PlatformCommon::getFullscreenVideoModes(
        &modeInfos, !config.getBool("Windowed"));
    ...
    listBoxFilterMaxAnisotropy.setSelectedItem(
        config.getString("FilterMaxAnisotropy", "1"));
    currentLine -= lineOffset;
    ...
    for (int i = 0; i < Renderer::sCount; ++i) {
      listBoxShadows.pushBackItem(lang.getString(
    ...
    listBoxShadows.setSelectedItemIndex(
        clamp(Renderer::strToShadows(str), 0, Renderer::sCount - 1));
    currentLine -= lineOffset;
    ...
    listBoxShadowTextureSize.setSelectedItem(
        intToStr(config.getInt("ShadowTextureSize", "512")), false);
    currentLine -= lineOffset;
    ...
    listBoxLights.setSelectedItemIndex(
        clamp(config.getInt("MaxLights") - 1, 0, 7));
    currentLine -= lineOffset;
    ...
    checkBoxTilesetParticles.setValue(
        config.getBool("TilesetParticles", "true"));
    currentLine -= lineOffset;

    // animated tileset objects
    labelAnimatedTilesetObjects.init(currentLabelStart, currentLine);
    labelAnimatedTilesetObjects.setText(
        lang.getString("AnimatedTilesetObjects"));
    ...
    listBoxAnimatedTilesetObjects.setSelectedItem(
        config.getString("AnimatedTilesetObjects", "-1"), false);
    currentLine -= lineOffset;
    ...
    labelEnableTextureCompression.setText(
        lang.getString("EnableTextureCompression"));

    checkBoxEnableTextureCompression.init(currentColumnStart, currentLine);
    checkBoxEnableTextureCompression.setValue(
...
void MenuStateOptionsGraphics::showMessageBox(const string &text,
                                              const string &header,
                                              bool toggle) {
  if (!toggle) {
    mainMessageBox.setEnabled(false);
...
void MenuStateOptionsGraphics::revertScreenMode() {
  ...
  window->ChangeVideoMode(
      true, config.getInt("ScreenWidth"), config.getInt("ScreenHeight"),
      !config.getBool("Windowed"), config.getInt("ColorBits"),
      config.getInt("DepthBits"), config.getInt("StencilBits"),
...
void MenuStateOptionsGraphics::mouseClick(int x, int y,
  ...
  if (mainMessageBox.getEnabled()) {
    ...
    string currentResolution = config.getString("ScreenWidth") + "x" +
                               config.getString("ScreenHeight") + "-" +
                               intToStr(config.getInt("ColorBits"));
    ...
    if (currentResolution != selectedResolution ||
      ...
      if (selectedMode == NULL) { // if we cannot find the selectedResolution we
                                  // try it with current one
        for (vector<ModeInfo>::const_iterator it = modeInfos.begin();
             it != modeInfos.end(); ++it) {
      ...
      WindowGl *window = this->program->getWindow();
      window->ChangeVideoMode(
          true, selectedMode->width, selectedMode->height,
          !selectedFullscreenWindowed, selectedMode->depth,
          config.getInt("DepthBits"), config.getInt("StencilBits"),
          config.getBool("HardwareAcceleration", "false"),
          config.getBool("FullScreenAntiAliasing", "false"),
...
void MenuStateOptionsGraphics::saveConfig() {
  ...
  config.setString(
      "Shadows", Renderer::shadowsToStr(static_cast<Renderer::Shadows>(index)));
  ...
  config.setInt("FilterMaxAnisotropy",
                strToInt(listBoxFilterMaxAnisotropy.getSelectedItem()));
  config.setFloat("GammaValue",
                  strToFloat(listBoxGammaCorrection.getSelectedItem()));
  config.setFloat("ShadowIntensity",
                  strToFloat(listBoxShadowIntensity.getSelectedItem()));
  ...
  if (listBoxAnimatedTilesetObjects.getSelectedItem() == "∞") {
    ...
    config.setInt(
        ...
        atoi(listBoxAnimatedTilesetObjects.getSelectedItem().c_str()));
  }
  ...
  string currentResolution =
      config.getString("ScreenWidth") + "x" + config.getString("ScreenHeight");
  string selectedResolution = listBoxScreenModes.getSelectedItem();
  if (currentResolution != selectedResolution) {
    for (vector<ModeInfo>::const_iterator it = modeInfos.begin();
         it != modeInfos.end(); ++it) {

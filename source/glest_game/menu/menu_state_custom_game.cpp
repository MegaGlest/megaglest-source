...
MenuStateCustomGame::MenuStateCustomGame(Program *program, MainMenu *mainMenu, bool openNetworkSlots, ParentMenuState parentMenuState, bool autostart,
                                         GameSettings *settings, bool masterserverMode, string autoloadScenarioName)
    ...
    try {
        ...
        if ((this->headlessServerMode == true || (openNetworkSlots == true && parentMenuState != pLanGame)) &&
        ...
        if (openNetworkSlots == true) {
            ...
            if (fileExists(data_path + DEFAULT_NETWORK_SETUP_FILENAME) == true) loadGameSettings(data_path + DEFAULT_NETWORK_SETUP_FILENAME);
        } else {
            string data_path = getGameReadWritePath(GameConstants::path_data_CacheLookupKey);

            if (fileExists(data_path + DEFAULT_SETUP_FILENAME) == true) loadGameSettings(data_path + DEFAULT_SETUP_FILENAME);
        }
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
...
void MenuStateCustomGame::cleanupThread(SimpleTaskThread **thread) {
    ...
    if (thread != NULL && *thread != NULL) {
        ...
        if (forceWaitForShutdown == true) {
            ...
            for (; (threadPtr->canShutdown(false) == false || threadPtr->getRunningStatus() == true) && difftime((long int)time(NULL), elapsed) <= 15;) {
                // sleep(150);
            }

            if (SystemFlags::VERBOSE_MODE_ENABLED) printf("\n\n#1b cleanupThread callingThread [%p]\n", *thread);

            if (threadPtr->canShutdown(true) == true && threadPtr->getRunningStatus() == false) {
...
void MenuStateCustomGame::mouseClick(int x, int y, MouseButton mouseButton) {
    ...
    try {
        ...
        if (mainMessageBox.getEnabled()) {
            ...
            MutexSafeWrapper safeMutexCLI((publishToClientsThread != NULL ? publishToClientsThread->getMutexThreadObjectAccessor() : NULL),
                                          string(__FILE__) + "_" + intToStr(__LINE__));
            ...
            if (buttonReturn.mouseClick(x, y) || serverInitError == true) {
                ...
                if (setupName != "") {
                    ...
                    if (loadGameSettings(fileNameToLoad)) console.addLine("<-- " + fileNameToLoad);
                }
            ...
            } else if (checkBoxAdvanced.getValue() == 1 && listBoxFallbackCpuMultiplier.getEditable() == true &&
                ...
                reloadFactions(listBoxTechTree.getItemCount() <= 1,
                               (checkBoxScenario.getValue() == true ? scenarioFiles[listBoxScenario.getSelectedItemIndex()] : ""));
                ...
                for (int i = 0; i < mapInfo.players; ++i) {
                    ...
                    if (selectedControlItemIndex != ctNetwork || (selectedControlItemIndex == ctNetwork && (slot == NULL || slot->isConnected() == false))) {
                    ...
                    if (selectedControlItemIndex != ctHuman && checkControTypeClicked == true && listBoxControls[i].mouseClick(x, y)) {
                        ...
                        ControlType currentControlType = static_cast<ControlType>(listBoxControls[i].getSelectedItemIndex());
                        ...
                        if (::Shared::Platform::Window::isKeyStateModPressed(KMOD_CTRL) == true && currentControlType != ctHuman) {
                            slotsToChangeStart = 0;
                            slotsToChangeEnd = mapInfo.players - 1;
                        }

                        for (int index = slotsToChangeStart; index <= slotsToChangeEnd; ++index) {
                            if (index != i && static_cast<ControlType>(listBoxControls[index].getSelectedItemIndex()) != ctHuman) {
                                listBoxControls[index].setSelectedItemIndex(listBoxControls[i].getSelectedItemIndex());
                            }
                            // Skip over networkunassigned
                            if (listBoxControls[index].getSelectedItemIndex() == ctNetworkUnassigned && selectedControlItemIndex != ctNetworkUnassigned) {
                            ...
                            // 2 humans
                                ...
                                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                                    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                                             ...
                                                             "[%s]\n",
                                                             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, closeSlotIndex,
                                                             origPlayName.c_str());
                                ...
                                if (serverSocket != NULL) {
                                    ...
                                    for (unsigned int j = 0; j < languageList.size(); ++j) {
                                        ...
                                        if (lang.hasString("BlockPlayerServerMsg", languageList[j]) == true) {
                                            snprintf(szMsg, 8096, lang.getString("BlockPlayerServerMsg", languageList[j]).c_str(),
                                                     serverInterface->getSlot(i, true)->getIpAddress().c_str());
                                        } else {
                                            snprintf(szMsg, 8096,
...
bool MenuStateCustomGame::checkNetworkPlayerDataSynch(bool checkMapCRC, bool checkTileSetCRC, bool checkTechTreeCRC) {
    ServerInterface *serverInterface = NetworkManager::getInstance().getServerInterface();
    ...
    for (int i = 0; i < mapInfo.players; ++i) {
        if (listBoxControls[i].getSelectedItemIndex() == ctNetwork) {
            ...
            if (slot != NULL && slot->isConnected() && (slot->getAllowDownloadDataSynch() == true || slot->getAllowGameDataSynchCheck() == true)) {
                if (checkMapCRC == true && slot->getNetworkGameDataSynchCheckOkMap() == false) {
                    dataSynchCheckOk = false;
                    break;
                }
                if (checkTileSetCRC == true && slot->getNetworkGameDataSynchCheckOkTile() == false) {
                    dataSynchCheckOk = false;
                    break;
                }
                if (checkTechTreeCRC == true && slot->getNetworkGameDataSynchCheckOkTech() == false) {
                    dataSynchCheckOk = false;
                    break;
                }
            }
...
void MenuStateCustomGame::PlayNow(bool saveGame) {
    ...
    for (int i = 0; i < mapInfo.players; ++i) {
        ...
        if (listBoxControls[i].getSelectedItemIndex() != ctClosed) {
            if (listBoxFactions[i].getSelectedItem() == formatString(GameConstants::RANDOMFACTION_SLOTNAME) && listBoxFactions[i].getItemCount() > 1) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                ...
                for (int findRandomFaction = 1; findRandomFaction < 1000; ++findRandomFaction) {
                    ...
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                                 ...
                                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, selectedFactionName.c_str(),
                                                 selectedFactionIndex, findRandomFaction);
                    ...
                    if (selectedFactionName != formatString(GameConstants::RANDOMFACTION_SLOTNAME) &&
                        ...
                        std::find(randomFactionSelectionList.begin(), randomFactionSelectionList.end(), selectedFactionName) ==
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                             ...
                                             listBoxFactions[i].getSelectedItem().c_str());
        ...
        if (listBoxControls[i].getSelectedItemIndex() == ctNetworkUnassigned) {
            ...
            for (unsigned int j = 0; j < languageList.size(); ++j) {
                ...
                if (lang.hasString("NetworkSlotUnassignedError", languageList[j]) == true) {
                    ...
                    strcpy(szMsg, "Cannot start game, some player(s) are not in a network game "
                                  "slot!");
                }
    ...
    if (dataSynchCheckOk == false) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
        if ((hasNetworkGameSettings() == true && needToSetChangedGameSettings == true) || (RandomCount > 0)) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                                         __LINE__);
        ...
        if (bOkToStart == true) {
            ...
            if (checkBoxPublishServer.getEditable() && checkBoxPublishServer.getValue() == true) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                                             __LINE__);
    ...
    try {
        ...
        if (mainMessageBox.getEnabled()) {
            ...
            std::map<int, Texture2D *> &crcPlayerTextureCache =
                CacheManager::getCachedItem<std::map<int, Texture2D *>>(GameConstants::playerTextureCacheLookupKey);
            ...
            if (hasNetworkGameSettings() == true) {
                ...
                if (serverInterface != NULL && buttonClearBlockedPlayers.getEditable() && serverInterface->getServerSocket() != NULL) {
            ...
            for (int i = 0; i < GameConstants::maxPlayers; ++i) {
                ...
                if (hasNetworkGameSettings() == true && listBoxControls[i].getSelectedItemIndex() != ctClosed) {
                ...
                if (hasNetworkGameSettings() == true && listBoxControls[i].getSelectedItemIndex() != ctClosed) {
                    ...
                    if (listBoxControls[i].getSelectedItemIndex() == ctNetwork || listBoxControls[i].getSelectedItemIndex() == ctNetworkUnassigned) {
                        ServerInterface *serverInterface = NetworkManager::getInstance().getServerInterface();
                        if (serverInterface != NULL && serverInterface->getSlot(i, true) != NULL && serverInterface->getSlot(i, true)->isConnected()) {
...
void MenuStateCustomGame::switchSetupForSlots(SwitchSetupRequest **switchSetupRequests, ServerInterface *&serverInterface, int startIndex, int endIndex,
                        ...
                        if ((switchSetupRequests[i]->getSwitchFlags() & ssrft_NetworkPlayerName) == ssrft_NetworkPlayerName) {
                            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                                SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                                         ...
                                                         "labelPlayerNames[i].getText() [%s]\n",
                                                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, i,
                                                         switchSetupRequests[i]->getSwitchFlags(), switchSetupRequests[i]->getNetworkPlayerName().c_str(),
...
void MenuStateCustomGame::update() {
    ...
    try {
        ...
        for (int i = 0; i < mapInfo.players; ++i) {
            ...
            if (listBoxControls[i].getSelectedItemIndex() == ctNetwork || listBoxControls[i].getSelectedItemIndex() == ctNetworkUnassigned) {
                ...
                if (serverInterface->getSlot(i, true) != NULL && serverInterface->getSlot(i, true)->isConnected()) {
                    ...
                    if (serverInterface->getSlot(i, true) != NULL && serverInterface->getSlot(i, true)->getConnectHasHandshaked()) {
                        currentConnectionCount++;
                    }
                    string label = (serverInterface->getSlot(i, true) != NULL ? serverInterface->getSlot(i, true)->getVersionString() : "");

                    if (serverInterface->getSlot(i, true) != NULL && serverInterface->getSlot(i, true)->getAllowDownloadDataSynch() == true &&
                        ...
                        if (serverInterface->getSlot(i, true) != NULL && serverInterface->getSlot(i, true)->getAllowGameDataSynchCheck() == true &&
                            ...
                            if (serverInterface->getSlot(i, true) != NULL && serverInterface->getSlot(i, true)->getNetworkGameDataSynchCheckOkTile() == false) {
                                label = label + " tile";
                            ...
                            if (serverInterface->getSlot(i, true) != NULL && serverInterface->getSlot(i, true)->getNetworkGameDataSynchCheckOkTech() == false) {
                                {
        ...
        bool checkDataSynch =
            (serverInterface->getAllowGameDataSynchCheck() == true && needToSetChangedGameSettings == true &&
             ((difftime((long int)time(NULL), lastSetChangedGameSettings) >= BROADCAST_SETTINGS_SECONDS) || (this->headlessServerMode == true)));
        ...
        if (this->headlessServerMode == true || hasOneNetworkSlotOpen == true || checkBoxAllowInGameJoinPlayer.getValue() == true) {
            if (this->headlessServerMode == true && GlobalStaticFlags::isFlagSet(gsft_lan_mode) == false) {
                checkBoxPublishServer.setValue(true);
            }
        ...
        bool callPublishNow = (checkBoxPublishServer.getEditable() && checkBoxPublishServer.getValue() == true && needToRepublishToMasterserver == true);
        ...
        if (needToPublishDelayed) {
            ...
            if ((difftime((long int)time(NULL), mapPublishingDelayTimer) >= BROADCAST_MAP_DELAY_SECONDS) || (this->headlessServerMode == true)) {
...
void MenuStateCustomGame::simpleTaskForMasterServer(BaseThread *callingThread) {
    try {
        ...
        if (republish == true) {
            ...
            int paramIndex = 0;
            for (std::map<string, string>::const_iterator iterMap = newPublishToServerInfo.begin(); iterMap != newPublishToServerInfo.end(); ++iterMap) {
...
void MenuStateCustomGame::copyToGameSettings(GameSettings *gameSettings, bool forceCloseUnusedSlots) {
    ...
    for (int i = 0; i < GameConstants::maxPlayers; ++i) {
        ...
        if (forceCloseUnusedSlots == true && (ct == ctNetworkUnassigned || ct == ctNetwork)) {
            if (serverInterface != NULL && (serverInterface->getSlot(i, true) == NULL || serverInterface->getSlot(i, true)->isConnected() == false)) {
                if (checkBoxScenario.getValue() == false) {
                    // printf("Closed A [%d]
            ...
            if (ct == ctHuman) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                             ...
                                             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, i, slotIndex,
        ...
        for (int i = 0; i < mapInfo.players; ++i) {
            ...
            if (listBoxControls[i].getSelectedItemIndex() == ctNetwork || listBoxControls[i].getSelectedItemIndex() == ctNetworkUnassigned) {
                ...
                if (serverInterface->getSlot(i, true) != NULL && serverInterface->getSlot(i, true)->isConnected()) {
                    ...
                    if (clientConnectedTime == 0 || (serverInterface->getSlot(i, true)->getConnectedTime() > 0 &&
                                                     serverInterface->getSlot(i, true)->getConnectedTime() < clientConnectedTime)) {
...
void MenuStateCustomGame::setupUIFromGameSettings(const GameSettings &gameSettings) {
    ...
    checkBoxAllowTeamResourceSharing.setValue((gameSettings.getFlagTypes1() & ft1_allow_shared_team_resources) == ft1_allow_shared_team_resources ? true
                                                                                                                                                  : false);
...
void MenuStateCustomGame::setRefreshedCrcToGameSettings(GameSettings *gameSettings) {
    ...
    if (gameSettings->getTileset() != "" && lastRecalculatedCRCTilesetName != gameSettings->getTileset()) {
        ...
        time_t lastUpdateDate = getFolderTreeContentsCheckSumRecursivelyLastGenerated(config.getPathListForType(ptTilesets, ""),
    ...
    if (gameSettings->getTech() != "" && lastRecalculatedCRCTechtreeName != gameSettings->getTech()) {
        ...
        time_t lastUpdateDate =
            getFolderTreeContentsCheckSumRecursivelyLastGenerated(config.getPathListForType(ptTechs, ""), "/" + gameSettings->getTech() + "/*", ".xml");
        if (difftime(lastUpdateDate, initTime) < 0) {
            forceRefresh = true;
        }
    }
    ...
    if (gameSettings->getTileset() != "") {
        if (lastCheckedCRCTilesetName != gameSettings->getTileset()) {
            ...
            lastCheckedCRCTilesetValue = getFolderTreeContentsCheckSumRecursively(
                config.getPathListForType(ptTilesets, ""), string("/") + gameSettings->getTileset() + string("/*"), ".xml", NULL, forceRefresh);
    ...
    if (config.getBool("DisableServerLobbyTechtreeCRCCheck", "false") == false) {
        if (gameSettings->getTech() != "") {
            if (lastCheckedCRCTechtreeName != gameSettings->getTech()) {
                ...
                uint32 oldCRC = 0;
                if (forceRefresh) {
                    oldCRC = getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTechs, ""), "/" + gameSettings->getTech() + "/*", ".xml",
                                                                      NULL, false);
                }
                ...
                for (unsigned int factionIdx = 0; factionIdx < factionFiles.size(); ++factionIdx) {
                    ...
                    if (factionName != GameConstants::RANDOMFACTION_SLOTNAME && factionName != GameConstants::OBSERVER_SLOTNAME) {
                        ...
                        // "/*", ".xml", NULL, true);
                        uint32 factionCRC = getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTechs, ""),
...
void MenuStateCustomGame::loadMapInfo(string file, MapInfo *mapInfo, bool loadMapPreview, bool doPlayerSetup) {
    try {
        ...
        SystemFlags::OutputDebug(SystemFlags::debugError, "In [%s::%s Line: %d] Error [%s] loading map [%s]\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__, e.what(), file.c_str());
        ...
        if (humanPlayer == false) {
            if (this->headlessServerMode == false) {
                ...
                if (foundNewSlotForHuman == false) {
                    for (int i = 0; i < mapInfo.players; ++i) {
                        if (listBoxControls[i].getSelectedItemIndex() == ctClosed || listBoxControls[i].getSelectedItemIndex() == ctCpuEasy ||
                            listBoxControls[i].getSelectedItemIndex() == ctCpu || listBoxControls[i].getSelectedItemIndex() == ctCpuUltra ||
...
void MenuStateCustomGame::closeUnusedSlots() {
    try {
        if (checkBoxScenario.getValue() == false) {
            ...
            for (int i = 0; i < GameConstants::maxPlayers; ++i) {
                if (listBoxControls[i].getSelectedItemIndex() == ctNetwork || listBoxControls[i].getSelectedItemIndex() == ctNetworkUnassigned) {
                    if (serverInterface->getSlot(i, true) == NULL || serverInterface->getSlot(i, true)->isConnected() == false ||
    ...
    try {
        ...
        if (hasNetworkGameSettings() == true) {
            if (hasCheckedForUPNP == false) {
                if (checkBoxPublishServer.getValue() == true || this->headlessServerMode == true) {
        ...
        for (int i = 0; i < GameConstants::maxPlayers; ++i) {
            ...
            if (slot == NULL && listBoxControls[i].getSelectedItemIndex() == ctNetwork) {
                try {
            ...
            if (slot != NULL) {
                if ((listBoxControls[i].getSelectedItemIndex() != ctNetwork) ||
                    (listBoxControls[i].getSelectedItemIndex() == ctNetwork && slot->isConnected() == false && i >= mapInfo.players)) {
...
string MenuStateCustomGame::getHumanPlayerName(int index) {
    ...
    if (index >= 0 && index < GameConstants::maxPlayers && labelPlayerNames[index].getText() != "" &&
    ...
    try {
        ...
        for (int i = 0; i < GameConstants::maxPlayers; ++i) {
            ...
            if (keepExistingSelectedItem == false ||
                (checkBoxAllowObservers.getValue() == 0 && originalValue == formatString(GameConstants::OBSERVER_SLOTNAME))) {

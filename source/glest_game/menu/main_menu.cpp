...
void MainMenu::initBackgroundVideo() {
    if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false && ::Shared::Graphics::VideoPlayer::hasBackEndVideoPlayer() == true &&
        ...
        menuBackgroundVideo = new VideoPlayer(&Renderer::getInstance(), introVideoFile, introVideoFileFallback, window, 0, 0, screen->w, screen->h,
        ...
        if (isKeyPressed(SDLK_BACKSPACE, c) == true && text.length() > 0) {
            ...
            if (found == string::npos || found != text.length() - 1) {
                ...
                if (activeInputLabel->getTextCharLengthList().size() >= 2 &&
                    activeInputLabel->getTextCharLengthList()[activeInputLabel->getTextCharLengthList().size() - 2] >= 1) {
                    delChar = true;
                }
            }
            if (delChar == true) {
                if (hasUnderscore) {
                    ...
                    if (activeInputLabel->getTextCharLengthList().size() > 1) {
                        ...
                        for (unsigned int i = 0;
                             i < (unsigned int)activeInputLabel->getTextCharLengthList()[activeInputLabel->getTextCharLengthList().size() - 2]; ++i) {
                    ...
                    for (unsigned int i = 0; i < (unsigned int)activeInputLabel->getTextCharLengthList()[activeInputLabel->getTextCharLengthList().size() - 1];
                         ++i) {

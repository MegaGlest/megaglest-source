...
MainMenu::MainMenu(Program *program)
    : ProgramState(program), menuBackgroundVideo(NULL) {
...
void MainMenu::initBackgroundVideo() {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false &&
    ...
    menuBackgroundVideo = new VideoPlayer(
        ...
        window, 0, 0, screen->w, screen->h, screen->format->BitsPerPixel, true,
...
void MainMenu::render() {
  ...
  if (state->isMasterserverMode() == false) {
    if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false) {
      ...
      // printf("In [%s::%s Line: %d] menuBackgroundVideo
...
bool MainMenu::textInput(std::string text) { return state->textInput(text); }
void MainMenu::keyDown(SDL_KeyboardEvent key) { state->keyDown(key); }

void MainMenu::keyUp(SDL_KeyboardEvent key) { state->keyUp(key); }

void MainMenu::keyPress(SDL_KeyboardEvent c) { state->keyPress(c); }

void MainMenu::setState(MenuState *newstate) {
  // printf("In [%s::%s Line: %d] oldstate [%p] newstate [%p] this->state
...
void MenuState::setActiveInputLabel(GraphicLabel *newLabel,
  ...
  if (activeInputLabelEdit != NULL &&
    ...
    size_t found = text.find_last_of("_");
    if (found != string::npos && found == text.length() - 1) {
      // printf("Removing trailing edit char, found = %d
      // [%d][%s]\n",found,text.length(),text.c_str());
      text = text.substr(0, found);
    }
    activeInputLabelEdit->setText(text);
    // textCharLength = extractKeyPressedUnicodeLength(text);
    activeInputLabelEdit->setTextCharLengthList(
        extractKeyPressedUnicodeLength(text));
...
bool MenuState::keyPressEditLabel(SDL_KeyboardEvent c,
                                  GraphicLabel **activeInputLabelPtr) {
...
bool MenuState::textInputEditLabel(string input,
                                   GraphicLabel **activeInputLabelPtr) {
      ...
      if ((int)activeInputLabel->getTextCharLengthList().size() < maxTextSize) {
        ...
        if (text.size() > 0) {
          ...
          if (found == string::npos || found != text.length() - 1) {
            ...
            // int lastCharLen =
            // activeInputLabel->getTextCharLengthList()[activeInputLabel->getTextCharLengthList().size()-1];
...
bool MenuState::keyDownEditLabel(SDL_KeyboardEvent c,
                                 GraphicLabel **activeInputLabelPtr) {
  bool eventHandled = false;
  GraphicLabel *activeInputLabel = *activeInputLabelPtr;
  if (activeInputLabel != NULL) {
    ...
    if (isKeyPressed(SDLK_BACKSPACE, c) == true && text.length() > 0) {
      ...
      size_t found = text.find_last_of("_");
      if (found == string::npos || found != text.length() - 1) {
        // printf("A text.length() = %d textCharLength.size() =
        // %d\n",text.length(),textCharLength.size());
        if (activeInputLabel->getTextCharLengthList()
                [activeInputLabel->getTextCharLengthList().size() - 1] >= 1) {
          delChar = true;
        }
      } else {
        // printf("B text.length() = %d textCharLength.size() =
        // %d\n",text.length(),textCharLength.size());
        hasUnderscore = true;
        if (activeInputLabel->getTextCharLengthList().size() >= 2 &&
            activeInputLabel->getTextCharLengthList()
                    [activeInputLabel->getTextCharLengthList().size() - 2] >=
                1) {
          delChar = true;
        }
      }
        ...
        if (hasUnderscore) {
          ...
          if (activeInputLabel->getTextCharLengthList().size() > 1) {
            ...
            for (unsigned int i = 0;
                 i < (unsigned int)activeInputLabel->getTextCharLengthList()
                         [activeInputLabel->getTextCharLengthList().size() - 2];
                 ++i) {
          ...
          for (unsigned int i = 0;
               i < (unsigned int)activeInputLabel->getTextCharLengthList()
                       [activeInputLabel->getTextCharLengthList().size() - 1];
               ++i) {

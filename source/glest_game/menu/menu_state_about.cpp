...
MenuStateAbout::MenuStateAbout(Program *program, MainMenu *mainMenu)
  ...
  for (int i = 0; i < teammateCount; ++i) {
    int xPos = (182 + i * 138);
    labelTeammateName[i].registerGraphicComponent(
        containerName, "labelTeammateName" + intToStr(i));
    labelTeammateName[i].init(xPos, 500);
    labelTeammateRole[i].registerGraphicComponent(
        containerName, "labelTeammateRole" + intToStr(i));
  ...
  if (enableCustomModCredits == true) {
    ...
    if (customModCreditsText != "") {
      ...
      int w = Config::getInstance().getInt(
          "CustomModCreditsTextW", intToStr(GraphicLabel::defW).c_str());
      int h = Config::getInstance().getInt(
          "CustomModCreditsTextH", intToStr(GraphicLabel::defH).c_str());
    ...
    int buttonReturnX = Config::getInstance().getInt(
        "CustomModCreditsReturnX", intToStr(buttonReturn.getX()).c_str());
    int buttonReturnY = Config::getInstance().getInt(
        "CustomModCreditsReturnY", intToStr(buttonReturn.getY()).c_str());
    int buttonReturnW = Config::getInstance().getInt(
        "CustomModCreditsReturnW", intToStr(buttonReturn.getW()).c_str());
    ...
    if (adjustModelText == true) {
      ...
      for (int i = 0; i < teammateCount; ++i) {
        ...
        if (characterPos < (int)characterMenuScreenPositionListCache.size()) {
          ...
          int newxPos =
              xPos -
              (fontMetrics->getTextWidth(labelTeammateName[i].getText()) / 2);
          ...
          newxPos =
              xPos -
              (fontMetrics->getTextWidth(labelTeammateRole[i].getText()) / 2);

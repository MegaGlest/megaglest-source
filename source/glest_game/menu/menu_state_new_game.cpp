...
MenuStateNewGame::MenuStateNewGame(Program *program, MainMenu *mainMenu) : MenuState(program, mainMenu, "root") {
...
void MenuStateNewGame::mouseClick(int x, int y, MouseButton mouseButton) {
    ...
    if (buttonCustomGame.mouseClick(x, y)) {
        ...
        soundRenderer.playFx(coreData.getClickSoundB());
        mainMenu->setState(new MenuStateScenario(program, mainMenu, false, Config::getInstance().getPathListForType(ptScenarios)));
        ...
        if (Config::getInstance().getString("InternetGamesBlockScenario", "") != "" &&
            ...
            string scenarioName = Config::getInstance().getString("InternetGamesBlockScenario");

            string scenarioPath = Scenario::getScenarioPath(dirList, scenarioName);

            if (scenarioPath != "") {
                mainMenu->setState(new MenuStateScenario(program, mainMenu, false, dirList, scenarioName));
                playScenario = true;
            }
        }
        ...
        soundRenderer.playFx(coreData.getClickSoundB());
        mainMenu->setState(new MenuStateScenario(program, mainMenu, true, Config::getInstance().getPathListForType(ptTutorials)));

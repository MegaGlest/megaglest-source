...
MenuStateMods::MenuStateMods(Program *program, MainMenu *mainMenu) : MenuState(program, mainMenu, "mods") {
    containerName = "Mods";
    ...
    ftpClientThread = new FTPClientThread(-1, "", mapsPath, tilesetsPath, techtreesPath, scenariosPath, this, fileArchiveExtension, fileArchiveExtractCommand,
                                          fileArchiveExtractCommandParameters, fileArchiveExtractCommandSuccessResult, tempFilePath);
        ...
        if (mainMessageBox.mouseClick(x, y, button)) {
            ...
            if (button == 0) {
                if (mainMessageBoxState == ftpmsg_Quit) {
                    ...
                    mainMenu->setState(new MenuStateRoot(program, mainMenu));
                    return;
                } else if (mainMessageBoxState == ftpmsg_GetMap || mainMessageBoxState == ftpmsg_ReplaceMap) {
                ...
                } else if (mainMessageBoxState == ftpmsg_GetTileset || mainMessageBoxState == ftpmsg_ReplaceTileset) {
                ...
                } else if (mainMessageBoxState == ftpmsg_GetTechtree || mainMessageBoxState == ftpmsg_ReplaceTechtree) {
                ...
                } else if (mainMessageBoxState == ftpmsg_GetScenario || mainMessageBoxState == ftpmsg_ReplaceScenario) {
                    ...
                    if (strToUInt(modInfo.crc) != 0 &&
                        ...
                        if (SystemFlags::VERBOSE_MODE_ENABLED)
                            printf("In [%s::%s Line %d] local CRC [%u] [%s]\n", __FILE__, __FUNCTION__, __LINE__,
                                   getFolderTreeContentsCheckSumRecursively(itemPath, ".xml", NULL), itemPath.c_str());
        ...
        if (selectedScenarioName != "") {
            ...
            if (alreadyHasScenario == true) {
                ...
                if (remoteHasScenario) {
                    ...
                    if (strToUInt(modInfo.crc) != 0 && strToUInt(modInfo.crc) != getFolderTreeContentsCheckSumRecursively(itemPath, "", NULL, forceRefresh)) {
                        if (SystemFlags::VERBOSE_MODE_ENABLED)
                            printf("In [%s::%s Line %d] local CRC [%u]\n", __FILE__, __FUNCTION__, __LINE__,
...
void MenuStateMods::eventMouseWheel(int x, int y, int zDelta) {
    ...
    if (!result) result = eventMouseWheel(&keyTechScrollBar, &keyTechButtons, x, y, zDelta);
    if (!result) result = eventMouseWheel(&keyScenarioScrollBar, &keyScenarioButtons, x, y, zDelta);
    if (!result) result = eventMouseWheel(&keyTilesetScrollBar, &keyTilesetButtons, x, y, zDelta);
}
...
void MenuStateMods::render() {
    try {
        ...
        if (modInfoSelected != NULL) {
            ...
            if (displayModPreviewImage.find(tempImage) != displayModPreviewImage.end() && displayModPreviewImage[tempImage] == true) {
                if (modPreviewImage == NULL) {
                    string tempImage = getPreviewImageFileForMod(modInfoSelected);
            ...
            for (std::map<string, pair<int, string>>::iterator iterMap = fileFTPProgressList.begin(); iterMap != fileFTPProgressList.end(); ++iterMap) {
                ...
                if (Renderer::renderText3DEnabled) {
                    renderer.renderProgressBar3D(iterMap->second.first,
                                                 // 10,
                                                 // yLocation,
                                                 xLocation, yLocation, CoreData::getInstance().getDisplayFontSmall3D(), 185, progressLabelPrefix, false);
                } else {
                    renderer.renderProgressBar(iterMap->second.first,
                                               // 10,
                                               // yLocation,
                                               xLocation, yLocation, CoreData::getInstance().getDisplayFontSmall(), 185, progressLabelPrefix, false);
                }
...
void MenuStateMods::FTPClient_CallbackEvent(string itemName, FTP_Client_CallbackType type, pair<FTP_Client_ResultType, string> result, void *userdata) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line %d]\n", __FILE__, __FUNCTION__, __LINE__);

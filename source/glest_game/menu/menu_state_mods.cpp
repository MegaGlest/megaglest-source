...
MenuStateMods::MenuStateMods(Program *program, MainMenu *mainMenu)
  ...
  keyTechScrollBarTitle2.setFont3D(
  ...
  keyMapScrollBarTitle1.init(mapInfoXPos + 5, scrollListsYPos + 30, labelWidth,
                             20);
  ...
  keyMapScrollBarTitle2.setFont3D(
      CoreData::getInstance().getMenuFontNormal3D());

  tilesetInfoXPos = 530;
  keyTilesetScrollBarTitle1.registerGraphicComponent(
      containerName, "keyTilesetScrollBarTitle1");
  keyTilesetScrollBarTitle1.init(tilesetInfoXPos + 5, scrollListsYPos + 30,
                                 labelWidth, 20);
  ...
  keyTilesetScrollBarTitle1.setFont3D(
      CoreData::getInstance().getMenuFontBig3D());

  scenarioInfoXPos = 755;
  keyScenarioScrollBarTitle1.registerGraphicComponent(
      containerName, "keyScenarioScrollBarTitle1");
  ...
  keyScenarioScrollBarTitle1.setFont3D(
      CoreData::getInstance().getMenuFontBig3D());
  ...
  buttonInstalled.setCustomTexture(
      CoreData::getInstance().getOnServerInstalledTexture());
  ...
  buttonAvailable.setCustomTexture(
      CoreData::getInstance().getOnServerTexture());
  ...
  buttonOnlyLocal.setCustomTexture(
  ...
  buttonConflict.setCustomTexture(
      CoreData::getInstance().getOnServerDifferentTexture());
  ...
  keyTilesetScrollBar.init(
      tilesetInfoXPos + keyButtonsWidthTil,
      scrollListsYPos - listBoxLength + keyButtonsLineHeight, false, 200, 20);
  ...
  keyTechScrollBar.init(techInfoXPos + keyButtonsWidthTech + labelWidth + 20,
                        scrollListsYPos - listBoxLength + keyButtonsLineHeight,
                        false, 200, 20);
  ...
  keyMapScrollBar.init(mapInfoXPos + keyButtonsWidthMap + labelWidth + 20,
                       scrollListsYPos - listBoxLength + keyButtonsLineHeight,
                       false, 200, 20);
  ...
  keyScenarioScrollBar.init(
      scenarioInfoXPos + keyButtonsWidthScen,
      scrollListsYPos - listBoxLength + keyButtonsLineHeight, false, 200, 20);
...
void MenuStateMods::simpleTask(BaseThread *callingThread, void *userdata) {
  ...
  if (findArchive == false) {
    ...
    showMessageBox(lang.getString("ModRequires7z"), lang.getString("Notice"),
                   true);
  }
  ...
  if (config.getString("Masterserver", "") != "") {
    ...
    string playerUUID =
        "&uuid=" +
        SystemFlags::escapeURL(Config::getInstance().getString("PlayerId", ""));
    string gameVersion =
  ...
  for (unsigned int i = 0; i < techListRemote.size(); i++) {
    ...
    if (result != "") {
      ...
      label->init(techInfoXPos + keyButtonsWidthTech + 10, keyButtonsYBase,
                  labelWidth, 20);
  ...
  for (unsigned int i = 0; i < scenarioListRemote.size(); i++) {
    ...
    if (result != "") {
      ...
      GraphicButton *button = new GraphicButton();
      button->init(scenarioInfoXPos, keyButtonsYBase, keyButtonsWidthScen,
                   keyButtonsHeight);
      button->setText(modinfo.name);
      button->setUseCustomTexture(true);
      button->setCustomTexture(CoreData::getInstance().getCustomTexture());
...
MapInfo MenuStateMods::loadMapInfo(string file) {
  try {
    ...
    MapPreview::loadMapInfo(file, &mapInfo, lang.getString("MaxPlayers"),
...
void MenuStateMods::getMapsLocalList() {
  ...
  /*
          ...
          vector<string> results;
          set<string> allMaps;
      findAll(config.getPathListForType(ptMaps), "*.gbm", results, false,
     false); copy(results.begin(), results.end(), std::inserter(allMaps,
     allMaps.begin())); results.clear();
      findAll(config.getPathListForType(ptMaps), "*.mgm", results, false,
     false); copy(results.begin(), results.end(), std::inserter(allMaps,
     allMaps.begin())); results.clear();
          ...
          if(config.getPathListForType(ptMaps).size() > 1) {
              ...
              findAll(path + "*.gbm", results2, false, false);
                  copy(results2.begin(), results2.end(), std::inserter(allMaps2,
     allMaps2.begin()));
     ...
     std::back_inserter(results2)); mapFilesUserData = results2;
                  //printf("\n\nMap path [%s] mapFilesUserData.size() =
  ...
  */
  ...
  vector<string> allMaps = MapPreview::findAllValidMaps(
      pathList, scenarioDir, false, false, &invalidMapList);
  if (allMaps.empty()) {
  ...
  mapFilesUserData = MapPreview::findAllValidMaps(pathList, scenarioDir, true,
                                                  false, &invalidMapList);
}
...
string MenuStateMods::refreshScenarioModInfo(string scenarioInfo) {
  ...
  if (scenarioInfoList.size() >= 5) {
    ...
    string itemPath = config.getPathListForType(ptScenarios, "")[1] + "/" +
                      modinfo.name + string("/*");
    if (itemPath.empty() == false) {
      bool forceRefresh =
          (mapCRCUpdateList.find(itemPath) == mapCRCUpdateList.end());
      uint32 crc = getFolderTreeContentsCheckSumRecursively(itemPath, ".xml",
                                                            NULL, forceRefresh);
      if (crc == 0) {
        itemPath = config.getPathListForType(ptScenarios, "")[0] + "/" +
                   modinfo.name + string("/*");
        if (itemPath.empty() == false) {
          forceRefresh =
              (mapCRCUpdateList.find(itemPath) == mapCRCUpdateList.end());
...
void MenuStateMods::mouseClick(int x, int y, MouseButton mouseButton) {
  ...
  if (buttonReturn.mouseClick(x, y)) {
    ...
    if (fileFTPProgressList.empty() == false) {
      ...
      char szBuf[8096] = "";
      snprintf(szBuf, 8096,
               lang.getString("ModDownloadInProgressCancelQuestion").c_str(),
               fileFTPProgressList.size());
            ...
            if (remoteHasTileset == false) {
              ...
              for (unsigned int i = 0; i < keyTilesetButtons.size(); ++i) {
                ...
                if (button != NULL &&
                  ...
                  keyTilesetScrollBar.setElementCount(
                      (int)keyTilesetButtons.size());
                  break;
                }
              }
            }
            ...
            if (remoteHasScenario == false) {
              for (unsigned int i = 0; i < keyScenarioButtons.size(); ++i) {
                ...
                if (button != NULL &&
                  ...
                  keyScenarioScrollBar.setElementCount(
                      (int)keyScenarioButtons.size());
                  break;
                }
              }
            }
    ...
    if (selectedTechName != "") {
      bool alreadyHasTech =
          (std::find(techTreeFiles.begin(), techTreeFiles.end(),
          ...
          if (strToUInt(modInfo.crc) != 0 &&
              strToUInt(modInfo.crc) !=
                  getFolderTreeContentsCheckSumRecursively(
                      itemPath, ".xml", NULL, forceRefresh)) {
            if (SystemFlags::VERBOSE_MODE_ENABLED)
    ...
    if (selectedTechName != "") {
      ...
      if (alreadyHasTech == true) {
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, lang.getString("ModRemoveTechConfirm").c_str(),
                 selectedTechName.c_str());
        showMessageBox(szBuf, lang.getString("Question"), true);
      } else {
        mainMessageBoxState = ftpmsg_None;
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 lang.getString("ModCannotRemoveTechNotInstalled").c_str(),
                 selectedTechName.c_str());
        showMessageBox(szBuf, lang.getString("Notice"), true);
      }
    } else {
    ...
    if (selectedTilesetName != "") {
      bool alreadyHasTileset =
          (std::find(tilesetFiles.begin(), tilesetFiles.end(),
                     selectedTilesetName) != tilesetFiles.end());
      if (alreadyHasTileset == true) {
        bool remoteHasTileset = (tilesetCacheList.find(selectedTilesetName) !=
                                 tilesetCacheList.end());
        if (remoteHasTileset) {
          ...
          if (strToUInt(modInfo.crc) != 0 &&
            ...
            char szBuf[8096] = "";
            snprintf(szBuf, 8096,
                     lang.getString("ModTilesetAlreadyInstalled").c_str(),
                     selectedTilesetName.c_str());
            showMessageBox(szBuf, lang.getString("Notice"), true);
          }
        }
    ...
    if (selectedTilesetName != "") {
      ...
      if (alreadyHasTileset == true) {
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, lang.getString("ModRemoveTilesetConfirm").c_str(),
                 selectedTilesetName.c_str());
        showMessageBox(szBuf, lang.getString("Question"), true);
      } else {
        mainMessageBoxState = ftpmsg_None;
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 lang.getString("ModCannotRemoveTilesetNotInstalled").c_str(),
                 selectedTilesetName.c_str());
        showMessageBox(szBuf, lang.getString("Notice"), true);
      }
    } else {
    ...
    soundRenderer.playFx(coreData.getClickSoundB());
    if (selectedMapName != "") {
      bool alreadyHasMap = (std::find(mapFiles.begin(), mapFiles.end(),
    ...
    if (selectedMapName != "") {
      ...
      if (alreadyHasMap == true) {
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, lang.getString("ModRemoveMapConfirm").c_str(),
                 selectedMapName.c_str());
        showMessageBox(szBuf, lang.getString("Question"), true);
      } else {
        mainMessageBoxState = ftpmsg_None;
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 lang.getString("ModCannotRemoveMapNotInstalled").c_str(),
                 selectedMapName.c_str());
        showMessageBox(szBuf, lang.getString("Notice"), true);
      }
    } else {
    ...
    if (selectedScenarioName != "") {
      bool alreadyHasScenario =
          (std::find(scenarioFiles.begin(), scenarioFiles.end(),
      ...
      if (alreadyHasScenario == true) {
        ...
        if (remoteHasScenario) {
          ...
          if (strToUInt(modInfo.crc) != 0 &&
              strToUInt(modInfo.crc) !=
                  getFolderTreeContentsCheckSumRecursively(itemPath, "", NULL,
                                                           forceRefresh)) {
            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf(
                  "In [%s::%s Line %d] local CRC [%u]\n", __FILE__,
                  __FUNCTION__, __LINE__,
                  getFolderTreeContentsCheckSumRecursively(itemPath, "", NULL));
            ...
            char szBuf[8096] = "";
            snprintf(szBuf, 8096,
                     lang.getString("ModScenarioAlreadyInstalled").c_str(),
                     selectedScenarioName.c_str());
            showMessageBox(szBuf, lang.getString("Notice"), true);
          }
        }
    ...
    if (selectedScenarioName != "") {
      ...
      if (alreadyHasScenario == true) {
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 lang.getString("ModCannotRemoveScenarioNotInstalled").c_str(),
                 selectedScenarioName.c_str());
        showMessageBox(szBuf, lang.getString("Notice"), true);
      }
    } else {
      ...
      for (int i = keyScenarioScrollBar.getVisibleStart();
        ...
        if (keyScenarioButtons[i]->mouseClick(x, y) &&
          ...
          if (scenarioName != "") {
            ...
            bool remoteHasScenario = (scenarioCacheList.find(scenarioName) !=
                                      scenarioCacheList.end());
            if (remoteHasScenario) {
...
bool MenuStateMods::eventMouseWheel(GraphicScrollBar *scrollBar,
                                    UserButtons *buttons, int x, int y,
                                    int zDelta) {
  int result = false;
  int overButton = false;
  if (scrollBar->getElementCount() != 0) {
    for (int i = scrollBar->getVisibleStart(); i <= scrollBar->getVisibleEnd();
         ++i) {
  ...
  if (result)
    layoutButtons(scrollBar, buttons, NULL);
...
void MenuStateMods::eventMouseWheel(int x, int y, int zDelta) {
  ...
  if (!result)
    result = eventMouseWheel(&keyScenarioScrollBar, &keyScenarioButtons, x, y,
                             zDelta);
  if (!result)
    result =
        eventMouseWheel(&keyTilesetScrollBar, &keyTilesetButtons, x, y, zDelta);
}
  ...
  try {
      ...
      for (int i = keyTechScrollBar.getVisibleStart();
        ...
        if (keyTechButtons[i]->getText() == selectedTechName) {
          ...
          if (remoteHasTech) {
            ...
            if (modInfo.crc == modInfo.localCRC) {
              keyTechButtons[i]->setCustomTexture(
                  CoreData::getInstance().getOnServerInstalledTexture());
            } else {
              // printf("modInfo.name=%s modInfo.crc=%s
              // modInfo.localCRC=%s\n",modInfo.name.c_str(),modInfo.crc.c_str(),modInfo.localCRC.c_str());
              keyTechButtons[i]->setCustomTexture(
                  CoreData::getInstance().getOnServerDifferentTexture());
            }
          ...
          Vec4f fontColor =
              Vec4f(200.0f / 255.0f, 187.0f / 255.0f, 190.0f / 255.0f, 0.75f);
    ...
    if (keyTilesetScrollBar.getElementCount() != 0) {
      for (int i = keyTilesetScrollBar.getVisibleStart();
        ...
        bool alreadyHasTileset =
            (std::find(tilesetFiles.begin(), tilesetFiles.end(),
                       keyTilesetButtons[i]->getText()) != tilesetFiles.end());
        if (keyTilesetButtons[i]->getText() == selectedTilesetName) {
          ...
          if (remoteHasTileset) {
            ...
            if (modInfo.crc == modInfo.localCRC) {
              keyTilesetButtons[i]->setCustomTexture(
                  CoreData::getInstance().getOnServerInstalledTexture());
            } else {
              // printf("modInfo.name=%s modInfo.crc=%s
              // modInfo.localCRC=%s\n",modInfo.name.c_str(),modInfo.crc.c_str(),modInfo.localCRC.c_str());
              // printf("name [%s] modInfo.crc [%s] modInfo.localCRC
              // [%s]\n",modInfo.name.c_str(),modInfo.crc.c_str(),modInfo.localCRC.c_str());
    ...
    if (keyMapScrollBar.getElementCount() != 0) {
      for (int i = keyMapScrollBar.getVisibleStart();
        ...
        if (keyMapButtons[i]->getText() == selectedMapName) {
          ...
          Vec4f buttonColor = WHITE;
          buttonColor.w = 0.75f;
          bool remoteHasMap = (mapCacheList.find(keyMapButtons[i]->getText()) !=
                               mapCacheList.end());
          if (remoteHasMap) {
            ModInfo &modInfo = mapCacheList[keyMapButtons[i]->getText()];
            if (modInfo.crc == modInfo.localCRC) {
              keyMapButtons[i]->setCustomTexture(
                  CoreData::getInstance().getOnServerInstalledTexture());
            } else {
              // printf("modInfo.name=%s modInfo.crc=%s
              // modInfo.localCRC=%s\n",modInfo.name.c_str(),modInfo.crc.c_str(),modInfo.localCRC.c_str());
    ...
    if (keyScenarioScrollBar.getElementCount() != 0) {
      for (int i = keyScenarioScrollBar.getVisibleStart();
        ...
        if (i >= (int)keyScenarioButtons.size()) {
          char szBuf[8096] = "";
          snprintf(szBuf, 8096,
                   "i >= keyScenarioButtons.size(), i = %d "
                   "keyScenarioButtons.size() = %d",
        ...
        if (keyScenarioButtons[i]->getText() == selectedScenarioName) {
          ...
          if (remoteHasScenario) {
            ...
            if (modInfo.crc == modInfo.localCRC) {
              keyScenarioButtons[i]->setCustomTexture(
                  CoreData::getInstance().getOnServerInstalledTexture());
            } else {
              // printf("modInfo.name=%s modInfo.crc=%s
              // modInfo.localCRC=%s\n",modInfo.name.c_str(),modInfo.crc.c_str(),modInfo.localCRC.c_str());
      ...
      for (std::map<string, pair<int, string>>::iterator iterMap =
        ...
        if (Renderer::renderText3DEnabled) {
          renderer.renderProgressBar3D(
              ...
              // yLocation,
              xLocation, yLocation,
              CoreData::getInstance().getDisplayFontSmall3D(), 185,
          ...
          renderer.renderProgressBar(
              ...
              // yLocation,
              xLocation, yLocation,
              CoreData::getInstance().getDisplayFontSmall(), 185,
              progressLabelPrefix, false);
        }
    ...
    if (modMenuState != mmst_None) {
      if (oldMenuState != modMenuState) {
        ...
        if (modMenuState == mmst_Loading) {
          pleaseWaitLabel.setText(
              lang.getString("GettingModlistFromMasterserver"));
...
void MenuStateMods::layoutButtons(GraphicScrollBar *scrollBar,
                                  UserButtons *buttons, GraphicLabels *labels) {
  if (scrollBar->getElementCount() != 0) {
    for (int i = scrollBar->getVisibleStart(); i <= scrollBar->getVisibleEnd();
         ++i) {
      if (i >= (int)buttons->size()) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 "i >= buttons->size(), i = %d, buttons->size() = %d", i,
                 (int)buttons->size());
        throw megaglest_runtime_error(szBuf);
      }
      (*buttons)[i]->setY(keyButtonsYBase -
                          keyButtonsLineHeight *
                              (i - scrollBar->getVisibleStart()));
      if (labels != NULL) {
...
void MenuStateMods::showMessageBox(const string &text, const string &header,
                                   bool toggle) {
  if (toggle == false) {
    mainMessageBox.setEnabled(false);
  }
...
void MenuStateMods::FTPClient_CallbackEvent(
  ...
  if (type == ftp_cct_DownloadProgress) {
    ...
    // printf("Got FTP extract Callback for [%s] result = %d
    // [%s]\n",itemName.c_str(),result.first,result.second.c_str());
    ...
    // printf("### downloaded file [%s] result =
    ...
    if (result.first == ftp_crt_SUCCESS) {
      refreshMaps();
      char szBuf[8096] = "";
      snprintf(szBuf, 8096, lang.getString("ModDownloadMapSuccess").c_str(),
               itemName.c_str());
      console.addLine(szBuf, true);
    } else {
      ...
      char szBuf[8096] = "";
      snprintf(szBuf, 8096, lang.getString("ModDownloadMapFail").c_str(),
    ...
    if (result.first == ftp_crt_SUCCESS) {
      ...
      char szBuf[8096] = "";
      snprintf(szBuf, 8096, lang.getString("ModDownloadTilesetSuccess").c_str(),
               itemName.c_str());
      console.addLine(szBuf, true);
      // END
    } else {
      ...
      char szBuf[8096] = "";
      snprintf(szBuf, 8096, lang.getString("ModDownloadTilesetFail").c_str(),
               itemName.c_str(), curlVersion->version, result.second.c_str());
      console.addLine(szBuf, true);
    }
    ...
    if (result.first == ftp_crt_SUCCESS) {
      ...
      char szBuf[8096] = "";
      snprintf(szBuf, 8096, lang.getString("ModDownloadTechSuccess").c_str(),
               itemName.c_str());
      console.addLine(szBuf, true);
      ...
      char szBuf[8096] = "";
      snprintf(szBuf, 8096, lang.getString("ModDownloadTechFail").c_str(),
               itemName.c_str(), curlVersion->version, result.second.c_str());
      console.addLine(szBuf, true);
    }
    ...
    if (result.first == ftp_crt_SUCCESS) {
      ...
      char szBuf[8096] = "";
      snprintf(szBuf, 8096, lang.getString("ModDownloadScenarioFail").c_str(),
               itemName.c_str(), curlVersion->version, result.second.c_str());
      console.addLine(szBuf, true);
    }
  }

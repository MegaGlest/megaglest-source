...
void MenuStateMasterserver::cleanup() {
    ...
    MutexSafeWrapper safeMutex((updateFromMasterserverThread != NULL ? updateFromMasterserverThread->getMutexThreadObjectAccessor() : NULL),
                               string(extractFileFromDirectoryPath(__FILE__).c_str()) + "_" + intToStr(__LINE__));
...
void MenuStateMasterserver::mouseClick(int x, int y, MouseButton mouseButton) {
    ...
    if (mainMessageBox.getEnabled()) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                                     __LINE__);
        if (ircClient != NULL && ircClient->isConnected() == true && ircClient->getHasJoinedChannel() == true) {
                    ...
                    if (connected) {
                        ...
                        if (ircClient != NULL && ircClient->isConnected() == true && ircClient->getHasJoinedChannel() == true) {
                            ircClient->SendIRCCmdMessage(IRC_CHANNEL,
                                                         "   <<   is connecting to '" + serverLines[i]->getMasterServerInfo()->getServerTitle() + "'");
                            sleep(30);
                        }
                        cleanup();
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        ...
        if (serverInfo != "") {
            ...
            for (int i = 0; i < (int)serverList.size(); i++) {
                ...
                if ((int)serverEntities.size() >= MIN_FIELDS_EXPECTED) {
                    ...
                    Lang &lang = Lang::getInstance();
                    labelTitle.setText("*** " + lang.getString("AvailableServers") + "[" + intToStr(serverEntities.size()) + "][" +
                                       intToStr(MIN_FIELDS_EXPECTED) + "] [" + serverInfo + "]");
...
bool MenuStateMasterserver::connectToServer(string ipString, int port) {
    ...
    if (clientInterface->isConnected() == false) {
        ...
        for (time_t elapsedWait = time(NULL);
             clientInterface->getIntroDone() == false && clientInterface->isConnected() && difftime(time(NULL), elapsedWait) <= 8;) {

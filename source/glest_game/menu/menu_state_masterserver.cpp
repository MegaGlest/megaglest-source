...
MenuStateMasterserver::MenuStateMasterserver(Program *program,
  ...
  serverScrollBar.init(
      1000 - 20,
      serverLinesYBase - serverLinesLineHeight * (serverLinesToRender - 1),
      false, 200, 20);
  ...
  lines[0].init(
      0, userButtonsYBase + userButtonsLineHeight + serverLinesLineHeight);
  ...
  ircOnlinePeopleStatusLabel.registerGraphicComponent(
      containerName, "ircOnlinePeopleStatusLabel");
  ircOnlinePeopleStatusLabel.init(
      userButtonsXBase, userButtonsYBase + userButtonsLineHeight - 20);
  ...
  // \""+configKeys.getCharKey("ToggleMusic")+"\"");
  ...
  string netPlayerName = Config::getInstance().getString(
  ...
  MutexSafeWrapper safeMutexIRCPtr(
      mutexIRCClient, string(extractFileFromDirectoryPath(__FILE__).c_str()) +
                          "_" + intToStr(__LINE__));

  if (SystemFlags::VERBOSE_MODE_ENABLED) printf("#1 IRCCLient Cache check\n");
  IRCThread *&ircThread = CacheManager::getCachedItem<IRCThread *>(
  ...
  // moment you are just called >>newbie<< !","Player name not set!",false);
...
void MenuStateMasterserver::cleanup() {
  ...
  MutexSafeWrapper safeMutex(
      (updateFromMasterserverThread != NULL
           ? updateFromMasterserverThread->getMutexThreadObjectAccessor()
           : NULL),
...
void MenuStateMasterserver::mouseClick(int x, int y, MouseButton mouseButton) {
  ...
  if (mainMessageBox.getEnabled()) {
    ...
    mainMenu->setState(
        new MenuStateCustomGame(program, mainMenu, true, pMasterServer));
      ...
      for (int i = serverScrollBar.getVisibleStart();
        ...
        if (serverLines[i]->buttonMouseClick(x, y)) {
          ...
          int connectServerPort =
              serverLines[i]->getMasterServerInfo()->getExternalConnectPort();
          ...
          if (connected) {
            ...
            if (ircClient != NULL && ircClient->isConnected() == true &&
                ircClient->getHasJoinedChannel() == true) {
              ircClient->SendIRCCmdMessage(
                  IRC_CHANNEL,
                  "   <<   is connecting to '" +
                      serverLines[i]->getMasterServerInfo()->getServerTitle() +
                      "'");
              sleep(30);
            }
            cleanup();
...
void MenuStateMasterserver::update() {
  ...
  setButtonLinePosition(serverLinesYBase -
                        (serverLinesToRender)*serverLinesLineHeight - 30);
    ...
    if (userScrollBar.getElementCount() != 0) {
      for (int i = userScrollBar.getVisibleStart();
        ...
        userButtons[i]->setY(userButtonsYBase -
                             userButtonsLineHeight *
                                 (i - userScrollBar.getVisibleStart()));
      }
    }
  }
  safeMutexIRCPtr.ReleaseLock();
    ...
    for (int i = serverScrollBar.getVisibleStart();
      ...
      serverLines[i]->setY(serverLinesYBase -
                           serverLinesLineHeight *
                               (i - serverScrollBar.getVisibleStart()));
    }
  }
...
void MenuStateMasterserver::simpleTask(BaseThread *callingThread,
  ...
  MutexSafeWrapper safeMutex(
      callingThread->getMutexThreadObjectAccessor(),
      string(extractFileFromDirectoryPath(__FILE__).c_str()) + "_" +
          intToStr(__LINE__));
  ...
  if (needUpdate == true) {
    try {
      ...
      if (announcementLoaded == false) {
        string announcementURL = Config::getInstance().getString(
            "AnnouncementURL",
        ...
        if (announcementURL != "") {
          ...
          if (StartsWith(announcementTxt, "Announcement from Masterserver:") ==
              true) {
            int newlineCount = 0;
            size_t lastIndex = 0;
        ...
        consoleIRC.addLine(
            Lang::getInstance().getString("ToSwitchOffMusicPress") + " - \"" +
      ...
      try {
        if (Config::getInstance().getString("Masterserver", "") != "") {
          ...
          std::string localServerInfoString = SystemFlags::getHTTP(
              baseURL + "showServersForGlest.php" + playerUUID, handle);
          SystemFlags::cleanupHTTP(&handle);
          if (callingThread->getQuitStatus() == true) {
            return;
          }
        ...
        SystemFlags::OutputDebug(SystemFlags::debugError,
                                 "In [%s::%s Line %d] error during Internet "
                                 "game status update: [%s]\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(),
    ...
    if (serverInfo != "") {
      ...
      for (int i = 0; i < (int)serverList.size(); i++) {
        ...
        if ((int)serverEntities.size() >= MIN_FIELDS_EXPECTED) {
          ...
          masterServerInfo->setExternalConnectPort(
              strToInt(serverEntities[11]));
          masterServerInfo->setCountry(serverEntities[12]);
          masterServerInfo->setStatus(strToInt(serverEntities[13]));

          // printf("--------------> Country [%s] Status
          // [%d]\n",masterServerInfo->getCountry().c_str(),masterServerInfo->getStatus());

          // printf("Getting Ping time for host
          // %s\n",masterServerInfo->getIpAddress().c_str()); float pingTime =
          // Socket::getAveragePingMS(masterServerInfo->getIpAddress().c_str(),1);
          // printf("Ping time = %f\n",pingTime);
          char szBuf[8096] = "";
          ...
          serverLines.push_back(
              new ServerLine(masterServerInfo, i, serverLinesYBase,
...
bool MenuStateMasterserver::connectToServer(string ipString, int port) {
  ...
  // int serverPort =
  // Config::getInstance().getInt("PortServer",intToStr(GameConstants::serverPort).c_str());
  int serverPort = port;
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(
  ...
  if (clientInterface->isConnected() == false) {
    ...
    // if(SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    // SystemFlags::OutputDebug(SystemFlags::debugSystem,"In [%s::%s] connection
    // failed\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__);
  } else {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugSystem,
                               ...
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, serverIp.getString().c_str());
...
void MenuStateMasterserver::keyDown(SDL_KeyboardEvent key) {
  ...
  if (ircClient != NULL && ircClient->isConnected() == true &&
    ...
    if (chatManager.getEditEnabled() == true) {
      // printf("keyDown key [%d] chatManager.getText()

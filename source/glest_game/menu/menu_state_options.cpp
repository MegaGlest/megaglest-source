...
MenuStateOptions::MenuStateOptions(Program *program, MainMenu *mainMenu,
    ...
    : MenuState(program, mainMenu, "config"), buttonOk("Options", "buttonOk"),
      buttonReturn("Options", "buttonReturn"),

      labelLang("Options", "labelLang"), listBoxLang("Options", "listBoxLang"),
      ...
      checkBoxDisableScreenshotConsoleText(
          "Options", "checkBoxDisableScreenshotConsoleText"),
...

{
  try {
    ...
    pair<string, string> defaultLang =
        Lang::getInstance().getNavtiveNameFromLanguageName(
            config.getString("Lang"));
    if (defaultLang.first == "" && defaultLang.second == "") {
      defaultLang = Lang::getInstance().getNavtiveNameFromLanguageName(
          Lang::getInstance().getDefaultLanguage());
    }
    ...
    listFontSizeAdjustment.setSelectedItem(
        intToStr(config.getInt("FontSizeAdjustment")));
    ...
    listBoxScreenShotType.setSelectedItem(
        config.getString("ScreenShotFileType", "jpg"));

    currentLine -= lineOffset;

    labelDisableScreenshotConsoleText.init(currentLabelStart, currentLine);
    labelDisableScreenshotConsoleText.setText(
        lang.getString("ScreenShotConsoleText"));

    checkBoxDisableScreenshotConsoleText.init(currentColumnStart, currentLine);
    checkBoxDisableScreenshotConsoleText.setValue(
        !config.getBool("DisableScreenshotConsoleText", "false"));
    ...
    checkBoxMouseMoveScrollsWorld.setValue(
        config.getBool("MouseMoveScrollsWorld", "true"));
    currentLine -= lineOffset;
    ...
    listCameraMoveSpeed.setSelectedItem(
        intToStr((int)(config.getFloat("CameraMoveSpeed", "15"))));
    currentLine -= lineOffset;
    ...
    listBoxHealthBars.pushBackItem(
    ...
    checkBoxChatStaysActive.setValue(
        config.getBool("ChatStaysActive", "false"));
    ...
    labelLuaDisableSecuritySandbox.setText(
        lang.getString("LuaDisableSecuritySandbox"));

    checkBoxLuaDisableSecuritySandbox.init(currentColumnStart, currentLine);
    checkBoxLuaDisableSecuritySandbox.setValue(
        config.getBool("DisableLuaSandbox", "false"));
    ...
    labelTransifexUser.setText(
        config.getString("TranslationGetURLUser", "<none>"));
    ...
    labelTransifexPwd.setText(
        config.getString("TranslationGetURLPassword", ""));
    ...
    labelTransifexI18N.setText(
        config.getString("TranslationGetURLLanguage", "en"));
    currentLine -= lineOffset;

    buttonGetNewLanguageFiles.init(currentLabelStart, currentLine, 250);
    buttonGetNewLanguageFiles.setText(
        lang.getString("TransifexGetLanguageFiles"));

    buttonDeleteNewLanguageFiles.init(currentLabelStart + 260, currentLine,
                                      250);
    buttonDeleteNewLanguageFiles.setText(
        lang.getString("TransifexDeleteLanguageFiles"));
...
void MenuStateOptions::showMessageBox(const string &text, const string &header,
                                      bool toggle) {
  if (!toggle) {
    mainMessageBox.setEnabled(false);
  }
...
void MenuStateOptions::showLuaMessageBox(const string &text,
...
void MenuStateOptions::mouseClick(int x, int y, MouseButton mouseButton) {
  ...
  if (mainMessageBox.getEnabled()) {
    ...
    if (checkBoxLuaDisableSecuritySandbox.getValue() == true) {
      ...
      Lang &lang = Lang::getInstance();
      showLuaMessageBox(lang.getString("LuaDisableSecuritySandboxWarning"),
    ...
    string selectedFontSizeAdjustment =
    ...
    if (currentFontSizeAdjustment != selectedFontSizeAdjustment) {
      ...
      showMessageBox(lang.getString("RestartNeeded"),
                     lang.getString("FontSizeAdjustmentChanged"), false);
      return;
    }
    saveConfig();
    // mainMenu->setState(new MenuStateRoot(program, mainMenu));
    ...
    soundRenderer.playFx(coreData.getClickSoundA());
    mainMenu->setState(new MenuStateOptionsNetwork(
        program, mainMenu,
        this->parentUI)); // open keyboard shortcuts setup screen
    return;
    ...
    if (labelTransifexI18N.getText() != "") {
      ...
      if (language != "") {
        ...
        if (data_path != "") {
          ...
          string txnURLFileListMapping = Config::getInstance().getString(
              "TranslationGetURLFileListMapping");
          ...
          if (tutorialPaths.size() > 1) {
            ...
            for (unsigned int i = 0; i < tutorialList.size(); ++i) {
              ...
              vector<string> langResults;
              findAll(tutorialPath + tutorial + "/*.lng", langResults, false,
                      false);
              for (unsigned int j = 0; j < langResults.size(); ++j) {
                string testLanguage = langResults[j];
    ...
    string txnURLLang =
        Config::getInstance().getString("TranslationGetURLLanguage");
    string txnURLFileList =
        Config::getInstance().getString("TranslationGetURLFileList");
    string txnURLFileListMapping =
        Config::getInstance().getString("TranslationGetURLFileListMapping");

    string txnURLDetails =
    ...
    printf("URL5 file count = " MG_SIZE_T_SPECIFIER ", " MG_SIZE_T_SPECIFIER
           " [%s]\n",
           languageFiles.size(), languageFileMappings.size(),
           (languageFiles.empty() == false ? languageFiles[0].c_str() : ""));
    ...
    if (languageFiles.empty() == false) {
      ...
      for (unsigned int i = 0; i < languageFiles.size(); ++i) {
        ...
        if (langName == "") {
          ...
          //		            "name": "Chinese"
          //		        },
          // curl -i -L --user softcoder -X GET
          // https://www.transifex.com/api/2/project/megaglest/resource/main-language-file/?details
          ...
          if (posDetails != fileDataDetails.npos) {
            posDetails = fileDataDetails.find(
                "\"name\": \"", posDetails + search_detail_key.length());

            if (posDetails != fileDataDetails.npos) {

              size_t posDetailsEnd = fileDataDetails.find("\"", posDetails + 9);

              langName = fileDataDetails.substr(
                  posDetails + 9, posDetailsEnd - (posDetails + 9));
              replaceAll(langName, ",", "");
              replaceAll(langName, "\\", "");
              replaceAll(langName, "/", "");
              replaceAll(langName, "?", "");
        ...
        if (pos != fileData.npos) {
          ...
          // printf("PARSED Language Name guessed to be
          ...
          // string lngFile = getGameCustomCoreDataPath(data_path, "data/lang/"
          // + toLower(languageName[1]) + ".lng");

...
// static const string ITEM_MISSING 					=
// "***missing***";
...
MenuStateConnectedGame::MenuStateConnectedGame(Program *program, MainMenu *mainMenu, JoinMenu joinMenuInfo, bool openNetworkSlots)
    ...
    if (config.getBool("EnableFTPXfer", "true") == true) {
        ...
        ftpClientThread =
            new FTPClientThread(portNumber, serverUrl, mapsPath, tilesetsPath, techtreesPath, scenariosPath, this, fileArchiveExtension,
                                fileArchiveExtractCommand, fileArchiveExtractCommandParameters, fileArchiveExtractCommandSuccessResult, tempFilePath);
...
MenuStateConnectedGame::~MenuStateConnectedGame() {
    ...
    if (modHttpServerThread != NULL) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line %d]\n", __FILE__, __FUNCTION__, __LINE__);
        if (modHttpServerThread->canShutdown(true) == true && modHttpServerThread->shutdownAndWait() == true) {
    ...
    if (ftpClientThread != NULL) {
        ...
        sleep(0);
        if (ftpClientThread->canShutdown(true) == true && ftpClientThread->shutdownAndWait() == true) {
...
string MenuStateConnectedGame::refreshTilesetModInfo(string tilesetInfo) {
    ...
    if (tilesetInfoList.size() >= 5) {
        ...
        if (itemPath.empty() == false) {
            ...
            // printf("#1 refreshTilesetModInfo name [%s] modInfo.crc [%s]
            // modInfo.localCRC
            // [%s]\n",modinfo.name.c_str(),modinfo.crc.c_str(),modinfo.localCRC.c_str());
        } else {
            modinfo.localCRC = "";

            // printf("#2 refreshTilesetModInfo name [%s] modInfo.crc [%s]
            // modInfo.localCRC
            // [%s]\n",modinfo.name.c_str(),modinfo.crc.c_str(),modinfo.localCRC.c_str());
        }
...
string MenuStateConnectedGame::getMapCRC(string mapName) {
    ...
    if (mappaths.empty() == false) {
        ...
        if (fileExists(itemPath)) {
            ...
            // printf("itemPath='%s' modinfo.name='%s' remote crc:'%s'  local crc:'%s'
            // crc='%d'
            ...
            if (fileExists(itemPath)) {
                ...
                uint32 crc = checksum.getSum();
                result = uIntToStr(crc);
                // printf("itemPath='%s' modinfo.name='%s' remote crc:'%s'  local
                // crc:'%s'   crc='%d'
                // \n",itemPath.c_str(),modinfo.name.c_str(),modinfo.crc.c_str(),modinfo.localCRC.c_str(),crc);
            } else {
                result = "";
...
void MenuStateConnectedGame::simpleTask(BaseThread *callingThread, void *userdata) {
    ...
    if (config.getString("Masterserver", "") != "") {
        ...
        if (curlResult == CURLE_OK || (curlResult != CURLE_COULDNT_RESOLVE_HOST && curlResult != CURLE_COULDNT_CONNECT)) {
...
void MenuStateConnectedGame::mouseClick(int x, int y, MouseButton mouseButton) {
    ...
    if (mapPreviewTexture != NULL) {
        ...
        if (x >= this->render_mapPreviewTexture_X && x <= this->render_mapPreviewTexture_X + this->render_mapPreviewTexture_W &&
            ...
            if (this->render_mapPreviewTexture_X == mapPreviewTexture_X && this->render_mapPreviewTexture_Y == mapPreviewTexture_Y &&
                this->render_mapPreviewTexture_W == mapPreviewTexture_W && this->render_mapPreviewTexture_H == mapPreviewTexture_H) {
                ...
                if (ftpMissingDataType == ftpmsg_MissingMap) {
                    ...
                    for (unsigned int i = 0; i < languageList.size(); ++i) {
                        ...
                        if (lang.hasString("DataMissingMapNowDownloading", languageList[i]) == true) {
                            snprintf(szMsg, 8096, lang.getString("DataMissingMapNowDownloading", languageList[i]).c_str(), getHumanPlayerName().c_str(),
                                     getMissingMapFromFTPServer.c_str());
                        } else {
                            snprintf(szMsg, 8096, "Player: %s is attempting to download the map: %s", getHumanPlayerName().c_str(),
                                     getMissingMapFromFTPServer.c_str());
                        }
                        bool localEcho = lang.isLanguageLocal(languageList[i]);
                        clientInterface->sendTextMessage(szMsg, -1, localEcho, languageList[i]);
                    ...
                    if (ftpClientThread != NULL) {
                        if (button == 0 && ftpMessageBox.getButtonCount() == 3) {
                            ...
                            MutexSafeWrapper safeMutexThread((modHttpServerThread != NULL ? modHttpServerThread->getMutexThreadObjectAccessor() : NULL),
                    ...
                    for (unsigned int i = 0; i < languageList.size(); ++i) {
                        ...
                        if (lang.hasString("DataMissingTilesetNowDownloading", languageList[i]) == true) {
                            snprintf(szMsg, 8096, lang.getString("DataMissingTilesetNowDownloading", languageList[i]).c_str(), getHumanPlayerName().c_str(),
                                     getMissingTilesetFromFTPServer.c_str());
                        } else {
                            snprintf(szMsg, 8096, "Player: %s is attempting to download the tileset: %s", getHumanPlayerName().c_str(),
                                     getMissingTilesetFromFTPServer.c_str());
                        }
                        bool localEcho = lang.isLanguageLocal(languageList[i]);
                        clientInterface->sendTextMessage(szMsg, -1, localEcho, languageList[i]);
                    ...
                    for (unsigned int i = 0; i < languageList.size(); ++i) {
                        ...
                        if (lang.hasString("DataMissingTechtreeNowDownloading", languageList[i]) == true) {
                            snprintf(szMsg, 8096, lang.getString("DataMissingTechtreeNowDownloading", languageList[i]).c_str(), getHumanPlayerName().c_str(),
                                     getMissingTechtreeFromFTPServer.c_str());
                        } else {
                            snprintf(szMsg, 8096, "Player: %s is attempting to download the techtree: %s", getHumanPlayerName().c_str(),
                                     getMissingTechtreeFromFTPServer.c_str());
                        }
                        bool localEcho = lang.isLanguageLocal(languageList[i]);
                        clientInterface->sendTextMessage(szMsg, -1, localEcho, languageList[i]);
    ...
    if (clientInterface != NULL && clientInterface->isConnected() == true) {
        ...
        // printf("myCurrentIndex = %d thisFactionIndex =
            ...
            for (int i = 0; i < GameConstants::maxPlayers; ++i) {
                    ...
                    if (listBoxFactions[i].mouseClick(x, y, advanceToItemStartingWith)) {
                        ...
                        if (clientInterface->isConnected()) {
                            ...
                            clientInterface->sendSwitchSetupRequest(listBoxFactions[i].getSelectedItem(), i, -1, listBoxTeams[i].getSelectedItemIndex(),
                                                                    getHumanPlayerName(), getNetworkPlayerStatus(), switchSetupRequestFlagType,
                                                                    lang.getLanguage());
                ...
                if (clientInterface != NULL && clientInterface->getJoinGameInProgress() == true) {
                    canGrabSlot = ((listBoxControls[i].getSelectedItemIndex() == ctNetwork &&
                                   ...
                                   (listBoxControls[i].getSelectedItemIndex() != ctHuman && listBoxControls[i].getSelectedItemIndex() != ctClosed &&
                ...
                if (canGrabSlot == true) {
                    if (clientInterface != NULL && i < mapInfo.players && grabSlotButton[i].mouseClick(x, y)) {
                        // printf("Send slot switch request for slot = %d, myCurrentIndex =
                        // %d\n",i,myCurrentIndex);
                        ...
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                            SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                                     "In [%s::%s Line %d] sending a switchSlot request from %d to "
                                                     "%d\n",
                                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
                        ...
                        clientInterface->sendSwitchSetupRequest(desiredFactionName, myCurrentIndex, i, desiredTeamIndex, getHumanPlayerName(),
                                                                getNetworkPlayerStatus(), switchSetupRequestFlagType, lang.getLanguage());
        ...
        if (listBoxPlayerStatus.mouseClick(x, y)) {
            ...
            if (clientInterface != NULL && clientInterface->isConnected()) {
                clientInterface->setGameSettingsReceived(false);
                clientInterface->sendSwitchSetupRequest(listBoxFactions[clientInterface->getPlayerIndex()].getSelectedItem(), clientInterface->getPlayerIndex(),
                                                        -1, listBoxTeams[clientInterface->getPlayerIndex()].getSelectedItemIndex(), getHumanPlayerName(),
                                                        getNetworkPlayerStatus(), switchSetupRequestFlagType, lang.getLanguage());
                switchSetupRequestFlagType = ssrft_None;
            }
        }
        ...
        if (isHeadlessAdmin() == true) {
            // printf("#1 admin key [%d] client key
            ...
            if (buttonPlayNow.mouseClick(x, y) && buttonPlayNow.getEnabled()) {
                ...
                bool dataSynchMismatch =
                    ((mapCRC != 0 && mapCRC != gameSettings->getMapCRC()) || (tilesetCRC != 0 && tilesetCRC != gameSettings->getTilesetCRC()) ||
                     (techCRC != 0 && techCRC != gameSettings->getTechCRC()));
                if (dataSynchMismatch == false) {
                    PlayNow(true);
                    return;
                } else {
                    showMessageBox("You cannot start the game because\none or more clients do not "
                                   "have the same game data!",
...
void MenuStateConnectedGame::broadCastGameSettingsToHeadlessServer(bool forceNow) {
    ...
    if (forceNow == true || ((needToBroadcastServerSettings == true) &&
                             (difftime((long int)time(NULL), broadcastServerSettingsDelayTimer) >= HEADLESSSERVER_BROADCAST_SETTINGS_SECONDS))) {
        // printf("In [%s:%s] Line: %d forceNow = %d
        // broadcastServerSettingsDelayTimer = " MG_SIZE_T_SPECIFIER ", now ="
        // MG_SIZE_T_SPECIFIER
        ...
        if (validDisplayedGamesettings) {
            ...
            // printf("Client sending map [%s] admin key
            // [%d]\n",gameSettings.getMap().c_str(),gameSettings.getMasterserver_admin());
            // printf("sending Settings map
...
void MenuStateConnectedGame::reloadFactions(bool keepExistingSelectedItem, string scenario) {
    ...
    if (results.empty() == true) {
        // throw megaglest_runtime_error("(2)There are no factions for the tech tree
        // [" + techTreeFiles[listBoxTechTree.getSelectedItemIndex()] + "]");
        // showGeneralError=true;
        // generalErrorToShow = "[#2] There are no factions for the tech tree [" +
        // techTreeFiles[listBoxTechTree.getSelectedItemIndex()] + "]";
    }
    ...
    for (int i = 0; i < GameConstants::maxPlayers; ++i) {
        ...
        if (keepExistingSelectedItem == false ||
            (checkBoxAllowObservers.getValue() == true && originalValue == formatString(GameConstants::OBSERVER_SLOTNAME))) {
...
void MenuStateConnectedGame::copyToGameSettings(GameSettings *gameSettings) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("In [%s::%s Line %d] listBoxMap.getSelectedItemIndex() = %d, "
               "mapFiles.size() = " MG_SIZE_T_SPECIFIER ", getCurrentMapFile() [%s]\n",
               extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, comboBoxMap.getSelectedItemIndex(), mapFiles.size(),
               getCurrentMapFile().c_str());
    ...
    if (comboBoxMap.getSelectedItemIndex() >= 0 && comboBoxMap.getSelectedItemIndex() < (int)mapFiles.size()) {
        ...
        for (unsigned int i = 0; i < languageList.size(); ++i) {
            ...
            if (lang.hasString("DataMissingMap=Player", languageList[i]) == true) {
                snprintf(szMsg, 8096, lang.getString("DataMissingMap=Player", languageList[i]).c_str(), getHumanPlayerName().c_str(),
                         comboBoxMap.getSelectedItem().c_str());
            } else {
    ...
    if (listBoxTileset.getSelectedItemIndex() >= 0 && listBoxTileset.getSelectedItemIndex() < (int)tilesetFiles.size()) {
        ...
        // printf("A loadGameSettings listBoxTileset.getSelectedItemIndex() = %d
        // tilesetFiles.size() =
        ...
        for (unsigned int i = 0; i < languageList.size(); ++i) {
            ...
            if (lang.hasString("DataMissingTileset=Player", languageList[i]) == true) {
                snprintf(szMsg, 8096, lang.getString("DataMissingTileset=Player", languageList[i]).c_str(), getHumanPlayerName().c_str(),
    ...
    if (listBoxTechTree.getSelectedItemIndex() >= 0 && listBoxTechTree.getSelectedItemIndex() < (int)techTreeFiles.size()) {
        ...
        for (unsigned int i = 0; i < languageList.size(); ++i) {
            ...
            if (lang.hasString("DataMissingTechtree=Player", languageList[i]) == true) {
                snprintf(szMsg, 8096, lang.getString("DataMissingTechtree=Player", languageList[i]).c_str(), getHumanPlayerName().c_str(),
    ...
    gameSettings->setFogOfWar(listBoxFogOfWar.getSelectedItemIndex() == 0 || listBoxFogOfWar.getSelectedItemIndex() == 1);

    gameSettings->setAllowObservers(checkBoxAllowObservers.getValue() == true);

    uint32 valueFlags1 = gameSettings->getFlagTypes1();
    if (listBoxFogOfWar.getSelectedItemIndex() == 1 || listBoxFogOfWar.getSelectedItemIndex() == 2) {
    ...
    for (int i = 0; i < GameConstants::maxPlayers; ++i) {
        ...
        if (ct != ctClosed) {
            ...
            if (ct == ctHuman) {
                ...
                // %d] i = %d, slotIndex = %d, getHumanPlayerName(i)
            ...
            // printf("!!! setStartLocationIndex #1 slotIndex = %d, i =
            // %d\n",slotIndex, i);
    ...
    for (int i = 0; i < GameConstants::maxPlayers; ++i) {
        ...
        if (ct == ctClosed) {
            ...
            gameSettings->setTeam(slotIndex, listBoxTeams[i].getSelectedItemIndex());
            gameSettings->setStartLocationIndex(slotIndex, i);
            // printf("!!! setStartLocationIndex #2 slotIndex = %d, i =
            // %d\n",slotIndex, i);
    ...
    if (hasNetworkGameSettings() == true) {
        if (gameSettings->getTileset() != "") {
            if (lastCheckedCRCTilesetName != gameSettings->getTileset()) {
                ...
                // + "]");
                lastCheckedCRCTilesetValue = getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTilesets, ""),
                ...
                if (lastCheckedCRCTilesetValue == 0 || lastCheckedCRCTilesetValue != gameSettings->getTilesetCRC()) {
                    lastCheckedCRCTilesetValue = getFolderTreeContentsCheckSumRecursively(
                        config.getPathListForType(ptTilesets, ""), string("/") + gameSettings->getTileset() + string("/*"), ".xml", NULL, true);
                }
            ...
            if (gameSettings->getTech() != "") {
                if (lastCheckedCRCTechtreeName != gameSettings->getTech()) {
                    ...
                    lastCheckedCRCTechtreeValue =
                        getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTechs, ""), "/" + gameSettings->getTech() + "/*", ".xml", NULL);
                    if (lastCheckedCRCTechtreeValue == 0 || lastCheckedCRCTechtreeValue != gameSettings->getTechCRC()) {
                        lastCheckedCRCTechtreeValue = getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTechs, ""),
                    ...
                    for (unsigned int factionIdx = 0; factionIdx < factionFiles.size(); ++factionIdx) {
                        ...
                        if (factionName != GameConstants::RANDOMFACTION_SLOTNAME && factionName != GameConstants::OBSERVER_SLOTNAME) {
                            // factionCRC   =
                            ...
                            uint32 factionCRC = getFolderTreeContentsCheckSumRecursively(
                                config.getPathListForType(ptTechs, ""), "/" + gameSettings->getTech() + "/factions/" + factionName + "/*", ".xml", NULL);
                            if (factionCRC == 0) {
                                factionCRC = getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTechs, ""),
                                                                                      "/" + gameSettings->getTech() + "/factions/" + factionName + "/*", ".xml",
                                                                                      NULL, true);
                            }
...
void MenuStateConnectedGame::render() {
    try {
        ...
        if (mapPreviewTexture != NULL) {
            ...
            renderer.renderTextureQuad(this->render_mapPreviewTexture_X, this->render_mapPreviewTexture_Y, this->render_mapPreviewTexture_W,
                                       this->render_mapPreviewTexture_H, mapPreviewTexture, 1.0f);
            if (this->zoomedMap == true) {
                return;
            }
        ...
        for (int i = 0; i < GameConstants::maxPlayers; ++i) {
            ...
            if (crcPlayerTextureCache[i] != NULL) {
                ...
                if (clientInterface != NULL && clientInterface->getGameSettings() != NULL && clientInterface->getGameSettings()->getMasterserver_admin() > 0 &&
                ...
                // int quadWidth = labelPlayerNames[i].getX() - labelPlayers[i].getX() -
                // 5; renderer.renderTextureQuad(labelPlayers[i].getX(),
                // labelPlayers[i].getY(), quadWidth, labelPlayers[i].getH(),
                // crcPlayerTextureCache[i],1.0f,&playerColor);
            } else {
            ...
            if (listBoxControls[i].getSelectedItemIndex() != ctClosed) {
                ...
                } else if (listBoxControls[i].getSelectedItemIndex() == ctNetwork || listBoxControls[i].getSelectedItemIndex() == ctNetworkUnassigned ||
        ...
        // fileFTPProgressList.clear();
        // fileFTPProgressList["test1a dsa asd asda sdasd asd ad ad"] =
        // make_pair(1,"testa"); fileFTPProgressList["test2 asdasdasdadas
        // dasdasdasda"] = make_pair(1,"testb"); fileFTPProgressList["test3 asdasdad
        // asd ada dasdadasdada"] = make_pair(1,"testc");
        // !!! END TEMP MV
        ...
        if (fileFTPProgressList.empty() == false) {
            ...
            for (std::map<string, pair<int, string>>::iterator iterMap = fileFTPProgressList.begin(); iterMap != fileFTPProgressList.end(); ++iterMap) {
                string progressLabelPrefix = lang.getString("ModDownloading") + " " + iterMap->first + " ";
                // if(SystemFlags::VERBOSE_MODE_ENABLED) printf("\nRendering file
                // progress with the following prefix
                // [%s]\n",progressLabelPrefix.c_str());

                if (Renderer::renderText3DEnabled) {
                    renderer.renderProgressBar3D(iterMap->second.first, xLocation,
                                                 // 10,
                                                 yLocation, CoreData::getInstance().getDisplayFontSmall3D(),
                                                 // 350,progressLabelPrefix);
                                                 300, progressLabelPrefix);
                } else {
                    renderer.renderProgressBar(iterMap->second.first,
                                               // 10,
                                               xLocation, yLocation, CoreData::getInstance().getDisplayFontSmall(),
                                               // 350,progressLabelPrefix);
                                               300, progressLabelPrefix);
                }
        ...
        if (enableMapPreview && (mapPreview.hasFileLoaded() == true)) {
            ...
            if (mapPreviewTexture == NULL) {
                ...
                // &mapPreviewTexture);
                renderer.renderMapPreview(&mapPreview, renderAll, this->render_mapPreviewTexture_X, this->render_mapPreviewTexture_Y, &mapPreviewTexture);
            }
        }
...
void MenuStateConnectedGame::update() {
    ...
    // Test progress bar
    // MutexSafeWrapper safeMutexFTPProgress((ftpClientThread != NULL ?
    // ftpClientThread->getProgressMutex() : NULL),string(__FILE__) + "_" +
    // intToStr(__LINE__)); fileFTPProgressList["test"] =
    // pair<int,string>(difftime(time(NULL),lastNetworkSendPing) * 20,"test file
    // 123"); safeMutexFTPProgress.ReleaseLock();
    //

    if (clientInterface != NULL && clientInterface->isConnected()) {
        // printf("#2 admin key [%d] client key
        // [%d]\n",settings->getMasterserver_admin(),clientInterface->getSessionKey());
        ...
        if (validDisplayedGamesettings == false || (notCurrentlySwitching && newMessage && receiveAllowedNow)) {
            // printf("I take the whole settings top broadcastDelay=%d
            // noReceiveTimer=%d\n", (int)difftime((long int) time(NULL),
            // broadcastServerSettingsDelayTimer),(int)difftime((long int) time(NULL),
            // noReceiveTimer));
        ...
        buttonPlayNow.setVisible(isHeadlessAdmin() || clientInterface->getJoinGameInProgress() == true);
        ...
        if (difftime((long int)time(NULL), lastNetworkSendPing) >= GameConstants::networkPingInterval) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                         "In [%s::%s Line: %d] pingCount = %d, "
                                         "clientInterface->getLastPingLag() = %f, "
                                         "GameConstants::networkPingInterval = %d\n",
                                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, pingCount, clientInterface->getLastPingLag(),
                                         GameConstants::networkPingInterval);

            // Starting checking timeout after sending at least 3 pings to server
            if (clientInterface->isConnected() && pingCount >= MAX_PING_LAG_COUNT &&
                clientInterface->getLastPingLag() >= (GameConstants::networkPingInterval * MAX_PING_LAG_COUNT)) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                                             __LINE__);
                ...
                if (clientInterface->isConnected() && clientInterface->getJoinGameInProgress() == false && pingCount >= MAX_PING_LAG_COUNT &&
                    clientInterface->getLastPingLag() >= (GameConstants::networkPingInterval * MAX_PING_LAG_COUNT)) {
                    MutexSafeWrapper safeMutexFTPProgress((ftpClientThread != NULL ? ftpClientThread->getProgressMutex() : NULL),
                                                          string(__FILE__) + "_" + intToStr(__LINE__));
                    if (fileFTPProgressList.empty() == true) {
                        ...
                        for (unsigned int i = 0; i < languageList.size(); ++i) {
                            clientInterface->sendTextMessage(lang.getString("ConnectionTimedOut", languageList[i]) + " : " +
                                                                 doubleToStr(clientInterface->getLastPingLag(), 2),
                                                             -1, false, languageList[i]);
                            sleep(1);
                            clientInterface->close();
                        }
                    }
        ...
        if (clientInterface->getAllowDownloadDataSynch() == false) {
            ...
            if (clientInterface->getAllowGameDataSynchCheck() == false && displayedGamesettings.getTileset() != "" && displayedGamesettings.getTech() != "" &&
                ...
                uint32 tilesetCRC = lastCheckedCRCTilesetValue;
                if (lastCheckedCRCTilesetName != displayedGamesettings.getTileset() && displayedGamesettings.getTileset() != "") {
                    // console.addLine("Checking tileset CRC [" +
                    // displayedGamesettings.getTileset() + "]");
                    tilesetCRC = getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTilesets, ""),
                                                                          string("/") + displayedGamesettings.getTileset() + string("/*"), ".xml", NULL);
                    if (tilesetCRC == 0 || tilesetCRC != displayedGamesettings.getTilesetCRC()) {
                        tilesetCRC = getFolderTreeContentsCheckSumRecursively(
                            config.getPathListForType(ptTilesets, ""), string("/") + displayedGamesettings.getTileset() + string("/*"), ".xml", NULL, true);
                    }
                ...
                uint32 techCRC = lastCheckedCRCTechtreeValue;
                if (lastCheckedCRCTechtreeName != displayedGamesettings.getTech() && displayedGamesettings.getTech() != "") {
                    // console.addLine("Checking techtree CRC [" +
                    // displayedGamesettings.getTech() + "]");
                    techCRC = getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTechs, ""),
                                                                       string("/") + displayedGamesettings.getTech() + string("/*"), ".xml", NULL);
                    // clientInterface->sendTextMessage("#1 TechCRC = " +
                    // intToStr(techCRC) + " remoteCRC = " +
                    // intToStr(displayedGamesettings.getTechCRC()),-1, true, "");

                    if (techCRC == 0 || techCRC != displayedGamesettings.getTechCRC()) {
                        techCRC = getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTechs, ""),
                                                                           string("/") + displayedGamesettings.getTech() + string("/*"), ".xml", NULL, true);
                        // clientInterface->sendTextMessage("#2 TechCRC = " +
                        // intToStr(techCRC) + " remoteCRC = " +
                        // intToStr(displayedGamesettings.getTechCRC()),-1, true, "");
                    }

                    if (techCRC != 0 && techCRC != displayedGamesettings.getTechCRC() && listBoxTechTree.getSelectedItemIndex() >= 0 &&
                        listBoxTechTree.getSelectedItem() != Lang::getInstance().getString("DataMissing", "", true)) {
                        // time_t now = time(NULL);
                        time_t lastUpdateDate = getFolderTreeContentsCheckSumRecursivelyLastGenerated(
                            config.getPathListForType(ptTechs, ""), string("/") + displayedGamesettings.getTech() + string("/*"), ".xml");

                        const time_t REFRESH_CRC_DAY_SECONDS = 60 * 60 * 1;
                        if (lastUpdateDate <= 0 || difftime((long int)time(NULL), lastUpdateDate) >= REFRESH_CRC_DAY_SECONDS) {
                            techCRC = getFolderTreeContentsCheckSumRecursively(
                                config.getPathListForType(ptTechs, ""), string("/") + displayedGamesettings.getTech() + string("/*"), ".xml", NULL, true);
                            // clientInterface->sendTextMessage("#3 TechCRC = " +
                            // intToStr(techCRC) + " remoteCRC = " +
                            // intToStr(displayedGamesettings.getTechCRC()),-1, true, "");
                        }
                    }
                    ...
                    for (unsigned int factionIdx = 0; factionIdx < factionFiles.size(); ++factionIdx) {
                        ...
                        if (factionName != GameConstants::RANDOMFACTION_SLOTNAME && factionName != GameConstants::OBSERVER_SLOTNAME &&
                            ...
                            // time_t now = time(NULL);
                            time_t lastUpdateDate = getFolderTreeContentsCheckSumRecursivelyLastGenerated(
                                config.getPathListForType(ptTechs, ""), "/" + displayedGamesettings.getTech() + "/factions/" + factionName + "/*", ".xml");

                            const time_t REFRESH_CRC_DAY_SECONDS = 60 * 60 * 24;
                            if (lastUpdateDate <= 0 || difftime((long int)time(NULL), lastUpdateDate) >= REFRESH_CRC_DAY_SECONDS ||
                                (techCRC != 0 && techCRC != displayedGamesettings.getTechCRC())) {
                                factionCRC = getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTechs, ""),
                                                                                      "/" + displayedGamesettings.getTech() + "/factions/" + factionName + "/*",
                                                                                      ".xml", NULL, true);
                            } else {
                                factionCRC = getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTechs, ""),
                                                                                      "/" + displayedGamesettings.getTech() + "/factions/" + factionName + "/*",
                                ...
                                for (unsigned int factionIdx1 = 0; factionIdx1 < serverFactionCRCList.size(); ++factionIdx1) {
                                    ...
                                    if (serverFaction.first == factionName) {
                                        if (serverFaction.second != factionCRC) {
                                            factionCRC = getFolderTreeContentsCheckSumRecursively(
                                                config.getPathListForType(ptTechs, ""),
                                                "/" + displayedGamesettings.getTech() + "/factions/" + factionName + "/*", ".xml", NULL, true);
                                        }
                                        break;
                                    }
                                }
                ...
                uint32 mapCRC = lastCheckedCRCMapValue;
                if (lastCheckedCRCMapName != displayedGamesettings.getMap() && displayedGamesettings.getMap() != "") {
                    Checksum checksum;
                ...
                bool dataSynchMismatch =
                    ((mapCRC != 0 && mapCRC != displayedGamesettings.getMapCRC()) || (tilesetCRC != 0 && tilesetCRC != displayedGamesettings.getTilesetCRC()) ||
                     (techCRC != 0 && techCRC != displayedGamesettings.getTechCRC()));

                // if(SystemFlags::VERBOSE_MODE_ENABLED) printf("\nmapCRC [%d]
                // displayedGamesettings.getMapCRC() [%d]\ntilesetCRC [%d]
                // displayedGamesettings.getTilesetCRC() [%d]\ntechCRC [%d]
                // displayedGamesettings.getTechCRC()
                ...
                if (dataSynchMismatch == true &&
                    (difftime((long int)time(NULL), broadcastServerSettingsDelayTimer) >= HEADLESSSERVER_BROADCAST_SETTINGS_SECONDS)) {
                    // printf("Data not synched: lmap %u rmap: %u ltile: %d rtile: %u
                    // ltech: %u rtech:
                    // %u\n",mapCRC,displayedGamesettings.getMapCRC(),tilesetCRC,displayedGamesettings.getTilesetCRC(),techCRC,displayedGamesettings.getTechCRC());

                    string labelSynch = lang.getString("DataNotSynchedTitle");

                    if (mapCRC != 0 && mapCRC != displayedGamesettings.getMapCRC() && comboBoxMap.getSelectedItemIndex() >= 0 &&
                    ...
                    if (tilesetCRC != 0 && tilesetCRC != displayedGamesettings.getTilesetCRC() && listBoxTileset.getSelectedItemIndex() >= 0 &&
                    ...
                    if (techCRC != 0 && techCRC != displayedGamesettings.getTechCRC() && listBoxTechTree.getSelectedItemIndex() >= 0 &&
                        ...
                        if (updateDataSynchDetailText == true &&
                            ...
                            // clientInterface->getGameSettings()->getUniqueNetworkPlayerLanguages();
                            for (unsigned int i = 0; i < languageList.size(); ++i) {
                                bool localEcho = lang.isLanguageLocal(languageList[i]);
                                        ...
                                        if (serverFaction.first == clientFaction.first) {
                                            ...
                                            if (serverFaction.second != clientFaction.second) {
                                                ...
                                                if (mismatchedFactionText == "") {
                                                    ...
                                                    mismatchedFactionText +=
                                                        " [" + intToStr(factionCRCList.size()) + "][" + intToStr(serverFactionCRCList.size()) + "] - ";
                                                } else {
                                                    mismatchedFactionText += ", ";
                                                }
            ...
            if (clientInterface->getAllowGameDataSynchCheck() == true && clientInterface->getNetworkGameDataSynchCheckOk() == false) {
                label = label + " -synch mismatch for:";

                if (clientInterface->getNetworkGameDataSynchCheckOkMap() == false) {
                    label = label + " map";

                    if (updateDataSynchDetailText == true && clientInterface->getReceivedDataSynchCheck() &&
    ...
    if (clientInterface != NULL && clientInterface->isConnected()) {
        ...
        try {
            if (clientInterface->getGameSettingsReceived() && validDisplayedGamesettings &&
                ...
                // printf("Menu got new settings thisfactionindex = %d startlocation: %d
                // control =
                ...
                if (difftime((long int)time(NULL), noReceiveTimer) < 3 ||
                    ...
                    if (isHeadlessAdmin()) {
                        ...
                        //  copy all received fields just not those which are editable for
                        //  normal client
                        // store my changes
            ...
            if (clientInterface->getJoinGameInProgress() == true && clientInterface->getJoinGameInProgressLaunch() == true &&
                clientInterface->getReadyForInGameJoin() == true && ftpClientThread != NULL) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
                    SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s Line: %d] took msecs: %lld\n",
                                             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, chrono.getMillis());
                if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0) chrono.start();
                ...
                if (readyToJoinInProgressGame == false) {
                    if (getInProgressSavedGameFromFTPServer == "") {
                        ...
                        ftpClientThread->addTempFileToRequests(GameConstants::saveNetworkGameFileClientCompressed,
            ...
            clientInterface = NetworkManager::getInstance().getClientInterface();
            if (clientInterface != NULL && clientInterface->isConnected()) {
                if (initialSettingsReceivedFromServer == true && clientInterface->getIntroDone() == true &&
                    (switchSetupRequestFlagType & ssrft_NetworkPlayerName) == ssrft_NetworkPlayerName) {
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                                 "In [%s::%s Line: %d] getHumanPlayerName() = [%s], "
                                                 "clientInterface->getGameSettings()->getThisFactionIndex() = "
                                                 "%d\n",
                                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, getHumanPlayerName().c_str(),
                                                 clientInterface->getGameSettings()->getThisFactionIndex());
                    clientInterface->sendSwitchSetupRequest("", clientInterface->getPlayerIndex(), -1, -1, getHumanPlayerName(), getNetworkPlayerStatus(),
                                                            switchSetupRequestFlagType, lang.getLanguage());
            ...
            if (clientInterface != NULL) {
                ...
                for (int i = 0; i < GameConstants::maxPlayers; ++i) {
                    if (displayedGamesettings.getFactionControl(i) == ctNetwork && displayedGamesettings.getNetworkPlayerName(i) != "" &&
...
bool MenuStateConnectedGame::loadFactions(const GameSettings *gameSettings, bool errorOnNoFactions) {
    ...
    if (gameSettings->getTech() != "") {
        ...
        for (int idx = 0; idx < (int)techPaths.size(); idx++) {
            string &techPath = techPaths[idx];
            endPathWithSlash(techPath);
            // findAll(techPath + gameSettings->getTech() + "/factions/*.", results,
            // false, false);
        ...
        if (results.empty() == true) {
            ...
            if (clientInterface->getAllowGameDataSynchCheck() == false) {
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                             "In [%s::%s Line: %d] (2)There are no factions for the tech tree "
                                             "[%s]\n",
            ...
            if (lastMissingTechtree != gameSettings->getTech() && gameSettings->getTech() != "") {
                ...
                for (unsigned int i = 0; i < languageList.size(); ++i) {
                    ...
                    if (lang.hasString("DataMissingTechtree", languageList[i]) == true) {
                        snprintf(szMsg, 8096, lang.getString("DataMissingTechtree", languageList[i]).c_str(), getHumanPlayerName().c_str(),
...
void MenuStateConnectedGame::keyDown(SDL_KeyboardEvent key) {
    if (activeInputLabel != NULL) {
        ...
        NetworkManager &networkManager = NetworkManager::getInstance();
        ClientInterface *clientInterface = networkManager.getClientInterface();
        if (clientInterface != NULL && clientInterface->isConnected() == true && clientInterface->getIntroDone() == true) {
            chatManager.keyDown(key);
        }
        if (chatManager.getEditEnabled() == false && (::Shared::Platform::Window::isKeyStateModPressed(KMOD_SHIFT) == false)) {
            Config &configKeys = Config::getInstance(std::pair<ConfigType, ConfigType>(cfgMainKeys, cfgUserKeys));
...
string MenuStateConnectedGame::getHumanPlayerName() {
    ...
    for (int j = 0; j < GameConstants::maxPlayers; ++j) {
        if (clientInterface != NULL && j == clientInterface->getPlayerIndex() && labelPlayerNames[j].getText() != "") {
            ...
            if (lastMissingMap != gameSettings->getMap()) {
                ...
                for (unsigned int i = 0; i < languageList.size(); ++i) {
                    ...
                    if (lang.hasString("DataMissingMap", languageList[i]) == true) {
                        snprintf(szMsg, 8096, lang.getString("DataMissingMap", languageList[i]).c_str(), getHumanPlayerName().c_str(),
...
void MenuStateConnectedGame::FTPClient_CallbackEvent(string itemName, FTP_Client_CallbackType type, pair<FTP_Client_ResultType, string> result,
                                                     void *userdata) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
    ...
    if (type == ftp_cct_DownloadProgress) {
            ...
            if (itemName != "" && (lastProgress.first / 25) < (fileProgress / 25)) {
                ...
                for (unsigned int i = 0; i < languageList.size(); ++i) {
                    ...
                    if (lang.hasString("FileDownloadProgress", languageList[i]) == true) {
                        snprintf(szMsg, 8096, lang.getString("FileDownloadProgress", languageList[i]).c_str(), getHumanPlayerName().c_str(), itemName.c_str(),
                                 fileProgress);
                    } else {
                    ...
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                                 "In [%s::%s Line %d] szMsg [%s] lastProgress.first = %d, "
                                                 "fileProgress = %d\n",
                                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, szMsg, lastProgress.first,
                                                 fileProgress);
                    clientInterface->sendTextMessage(szMsg, -1, lang.isLanguageLocal(languageList[i]), languageList[i]);
                }
                sleep(1);
        ...
        if (userdata == NULL) {
            ...
            for (unsigned int i = 0; i < languageList.size(); ++i) {
                ...
                if (lang.hasString("DataMissingExtractDownload", languageList[i]) == true) {
                    snprintf(szMsg, 8096, lang.getString("DataMissingExtractDownload", languageList[i]).c_str(), getHumanPlayerName().c_str(),
                             itemName.c_str());
                } else {
                    snprintf(szMsg, 8096, "Please wait, player: %s is extracting: %s", getHumanPlayerName().c_str(), itemName.c_str());
                }
                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line %d] szMsg [%s]\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                             __FUNCTION__, __LINE__, szMsg);
                clientInterface->sendTextMessage(szMsg, -1, lang.isLanguageLocal(languageList[i]), languageList[i]);
            }
            sleep(1);
        } else {
        ...
        if (result.first == ftp_crt_SUCCESS) {
            ...
            if (SystemFlags::VERBOSE_MODE_ENABLED)
                printf("Got map itemName [%s] file [%s] lastCheckedCRCMapName [%s] "
                       "gameSettings->getMap() [%s]\n",
                       itemName.c_str(), file.c_str(), lastCheckedCRCMapName.c_str(), gameSettings->getMap().c_str());

            if (gameSettings != NULL && itemName == gameSettings->getMap() && lastCheckedCRCMapName == gameSettings->getMap() && gameSettings->getMap() != "") {
            ...
            for (unsigned int i = 0; i < languageList.size(); ++i) {
                ...
                if (lang.hasString("DataMissingMapSuccessDownload", languageList[i]) == true) {
                    snprintf(szMsg, 8096, lang.getString("DataMissingMapSuccessDownload", languageList[i]).c_str(), getHumanPlayerName().c_str(),
                             itemName.c_str());
                } else {
            ...
            printf("FAILED map itemName [%s] lastCheckedCRCMapName [%s] "
                   "gameSettings->getMap() [%s]\n",
            ...
            for (unsigned int i = 0; i < languageList.size(); ++i) {
                ...
                if (lang.hasString("DataMissingMapFailDownload", languageList[i]) == true) {
                    snprintf(szMsg, 8096, lang.getString("DataMissingMapFailDownload", languageList[i]).c_str(), getHumanPlayerName().c_str(), itemName.c_str(),
                             curlVersion->version);
                } else {
                    snprintf(szMsg, 8096,
                             "Player: %s FAILED to download the map: [%s] using CURL "
                             "version [%s]",
                             getHumanPlayerName().c_str(), itemName.c_str(), curlVersion->version);
                }
                clientInterface->sendTextMessage(szMsg, -1, lang.isLanguageLocal(languageList[i]), languageList[i]);

                if (result.first == ftp_crt_HOST_NOT_ACCEPTING) {
                    if (lang.hasString("HostNotAcceptingDataConnections", languageList[i]) == true) {
                        clientInterface->sendTextMessage(lang.getString("HostNotAcceptingDataConnections", languageList[i]), -1,
                                                         lang.isLanguageLocal(languageList[i]), languageList[i]);
                    } else {
                        clientInterface->sendTextMessage("*Warning* the host is not accepting data connections.", -1, lang.isLanguageLocal(languageList[i]),
                                                         languageList[i]);
                    }
                }
            }
        ...
        if (result.first == ftp_crt_SUCCESS) {
            ...
            for (unsigned int i = 0; i < languageList.size(); ++i) {
                ...
                if (lang.hasString("DataMissingTilesetSuccessDownload", languageList[i]) == true) {
                    snprintf(szMsg, 8096, lang.getString("DataMissingTilesetSuccessDownload", languageList[i]).c_str(), getHumanPlayerName().c_str(),
                             itemName.c_str());
                } else {
            ...
            if (gameSettings != NULL && itemName == gameSettings->getTileset() && lastCheckedCRCTilesetName == gameSettings->getTileset() &&
            ...
            for (unsigned int i = 0; i < languageList.size(); ++i) {
                ...
                if (lang.hasString("DataMissingTilesetFailDownload", languageList[i]) == true) {
                    snprintf(szMsg, 8096, lang.getString("DataMissingTilesetFailDownload", languageList[i]).c_str(), getHumanPlayerName().c_str(),
                             (itemName + "(.7z)").c_str(), curlVersion->version);
                } else {
                    snprintf(szMsg, 8096,
                             "Player: %s FAILED to download the tileset: [%s] using CURL "
                             "version [%s]",
                             getHumanPlayerName().c_str(), (itemName + "(.7z)").c_str(), curlVersion->version);
                }
                clientInterface->sendTextMessage(szMsg, -1, lang.isLanguageLocal(languageList[i]), languageList[i]);
        ...
        if (result.first == ftp_crt_SUCCESS) {
            ...
            for (unsigned int i = 0; i < languageList.size(); ++i) {
                ...
                if (lang.hasString("DataMissingTechtreeSuccessDownload", languageList[i]) == true) {
                    snprintf(szMsg, 8096, lang.getString("DataMissingTechtreeSuccessDownload", languageList[i]).c_str(), getHumanPlayerName().c_str(),
                             itemName.c_str());
                } else {
            ...
            // Refresh CRC
            if (gameSettings != NULL && itemName == gameSettings->getTech() && lastCheckedCRCTechtreeName == gameSettings->getTech() &&
            ...
            for (unsigned int i = 0; i < (unsigned int)languageList.size(); ++i) {
                ...
                if (lang.hasString("DataMissingTechtreeFailDownload", languageList[i]) == true) {
                    snprintf(szMsg, 8096, lang.getString("DataMissingTechtreeFailDownload", languageList[i]).c_str(), getHumanPlayerName().c_str(),
                             (itemName + "(.7z)").c_str(), curlVersion->version);
                } else {
                    snprintf(szMsg, 8096,
                             "Player: %s FAILED to download the techtree: [%s] using "
                             "CURL version [%s]",
                             getHumanPlayerName().c_str(), (itemName + "(.7z)").c_str(), curlVersion->version);
                }
                clientInterface->sendTextMessage(szMsg, -1, lang.isLanguageLocal(languageList[i]), languageList[i]);
        ...
        // printf("Status update downloading saved game file:
        // [%s]\n",itemName.c_str());
        ...
        if (result.first == ftp_crt_SUCCESS) {
            ...
            for (unsigned int i = 0; i < languageList.size(); ++i) {
                ...
                if (lang.hasString("JoinPlayerToCurrentGameSuccessDownload", languageList[i]) == true) {
                    snprintf(szMsg, 8096, lang.getString("JoinPlayerToCurrentGameSuccessDownload", languageList[i]).c_str(), getHumanPlayerName().c_str(),
                             itemName.c_str());
                } else {
            ...
            for (unsigned int i = 0; i < languageList.size(); ++i) {
                ...
                if (lang.hasString("JoinPlayerToCurrentGameFailDownload", languageList[i]) == true) {
                    snprintf(szMsg, 8096, lang.getString("JoinPlayerToCurrentGameFailDownload", languageList[i]).c_str(), getHumanPlayerName().c_str(),
                             itemName.c_str(), curlVersion->version);
                } else {
                    snprintf(szMsg, 8096,
                             "Player: %s FAILED to download the saved game: [%s] using "
                             "CURL version [%s]",
                             getHumanPlayerName().c_str(), itemName.c_str(), curlVersion->version);
                }
                clientInterface->sendTextMessage(szMsg, -1, lang.isLanguageLocal(languageList[i]), languageList[i]);
...
void MenuStateConnectedGame::setupUIFromGameSettings(GameSettings *gameSettings, bool errorOnMissingData) {
    ...
    // printf("A gameSettings->getTileset()
            ...
            if (lastMissingTileSet != gameSettings->getTileset()) {
                ...
                for (unsigned int i = 0; i < languageList.size(); ++i) {
                    ...
                    if (lang.hasString("DataMissingTileset", languageList[i]) == true) {
                        snprintf(szMsg, 8096, lang.getString("DataMissingTileset", languageList[i]).c_str(), getHumanPlayerName().c_str(),
                                 gameSettings->getTileset().c_str());
                    } else {
    ...
    if (getMissingMapFromFTPServerInProgress == false && gameSettings->getMap() != "" &&
        gameSettings->getMapFilter() == listBoxMapFilter.getSelectedItemIndex()) {
        // map
        ...
        if (mapLoaded == false) {
            ...
            if (ftpClientThread != NULL &&
                (getMissingMapFromFTPServer != currentMap || difftime(time(NULL), getMissingMapFromFTPServerLastPrompted) > REPROMPT_DOWNLOAD_SECONDS)) {
                if (ftpMessageBox.getEnabled() == false) {
        ...
        if (isHeadlessAdmin() && !missingMap && mapFile != comboBoxMap.getSelectedItem()) {
            // console.addLine("Headless server does not have map, switching to next
            // one");
        ...
        // printf("Setting map from game settings map:%s , settingsfilter=%d ,
        // boxfilter=%d
    ...
    if (hasFactions == true && gameSettings != NULL) {
        ...
        for (int i = 0; i < GameConstants::maxPlayers; ++i) {
            ...
            if (gameSettings->getFactionControl(i) == ctNetwork || gameSettings->getFactionControl(i) == ctNetworkUnassigned) {
                ...
                if (gameSettings->getThisFactionIndex() != i && gameSettings->getNetworkPlayerName(i) != "" &&
            ...
            if ((gameSettings->getFactionControl(i) == ctNetwork || gameSettings->getFactionControl(i) == ctNetworkUnassigned) &&
                ...
                if (labelPlayerNames[slot].getText() == "" && gameSettings->getNetworkPlayerName(i) != "" &&
                    gameSettings->getNetworkPlayerName(i) != GameConstants::NETWORK_SLOT_UNCONNECTED_SLOTNAME) {
                    labelPlayerNames[slot].setText(gameSettings->getNetworkPlayerName(i));
                }
            }
    ...
    if (enableFactionTexturePreview == true) {
        if (clientInterface != NULL && clientInterface->isConnected() && gameSettings != NULL) {
    ...
    if (factionVideoUrl != "") {
        ...
        if (currentFactionLogo != factionVideoUrl) {
            ...
            if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false && ::Shared::Graphics::VideoPlayer::hasBackEndVideoPlayer() == true) {
                ...
                factionVideo = new VideoPlayer(&Renderer::getInstance(), introVideoFile, introVideoFileFallback, window, 0, 0, screen->w, screen->h,
                                               screen->format->BitsPerPixel, true, SystemFlags::VERBOSE_MODE_ENABLED);
...
int MenuStateConnectedGame::setupMapList(string scenario) {
    ...
    try {
        ...
        for (int i = 0; i < (int)mapFiles.size(); i++) { // fetch info and put map in right list
            ...
            if (GameConstants::maxPlayers + 1 <= mapInfo.players) {
                ...
                snprintf(szBuf, 8096,
                         ...
                         "[%d]\nfor file [%s]\nmap [%s]",
                         GameConstants::maxPlayers + 1, mapInfo.players, Config::getMapPath(mapFiles.at(i), "", false).c_str(), mapInfo.desc.c_str());
                throw megaglest_runtime_error(szBuf);
            }
        ...
        if (scenario != "") {
            ...
            if (SystemFlags::VERBOSE_MODE_ENABLED)
                printf("In [%s::%s Line %d] listBoxMap.getSelectedItemIndex() = %d, "
                       "mapFiles.size() = " MG_SIZE_T_SPECIFIER ", mapInfo.players = %d, "
                       "formattedPlayerSortedMaps[mapInfo.players].size() "
                       "= " MG_SIZE_T_SPECIFIER ", scenarioInfo.mapName [%s]\n",
                       extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, comboBoxMap.getSelectedItemIndex(), mapFiles.size(),
                       mapInfo.players, formattedPlayerSortedMaps[mapInfo.players].size(), scenarioInfo.mapName.c_str());

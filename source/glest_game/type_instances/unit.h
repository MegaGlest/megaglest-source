...
public:
  UnitPath()
      : UnitPathInterface(), blockCount(0), map(NULL) {
  ...
  // virtual Vec2i pop()		{ Vec2i p= front(); erase(begin()); return p;
  // }
...
public:
  ...
  giveCommand(Command *command, bool tryQueue = false); // give a command

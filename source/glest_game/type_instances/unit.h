...
enum CommandResult { crSuccess, crFailRes, crFailReqs, crFailUnitCount, crFailUndefined, crSomeFailed };

enum InterestingUnitType { iutIdleHarvester, iutBuiltBuilding, iutProducer, iutDamaged, iutStore };

enum CauseOfDeathType { ucodNone, ucodAttacked, ucodAttackBoost, ucodStarvedResource, ucodStarvedRegeneration };
...
class Unit : public BaseColorPickEntity, ValueCheckerVault, public ParticleOwner {
    ...
    inline bool isBadHarvestPos(const Vec2i &value, bool checkPeerUnits = true) const {
        ...
        if (iter != badHarvestPosList.end()) {
            ...
            for (int i = 0; i < this->getFaction()->getUnitCount(); ++i) {
                ...
                if (peerUnit != NULL && peerUnit->getId() != this->getId() && peerUnit->getType()->hasCommandClass(ccHarvest) == true &&
  ...
  private:
    ...
    void checkCustomizedUnitParticleListTriggers(const UnitParticleSystemTypes &unitParticleSystemTypesList, bool applySkillChangeParticles);

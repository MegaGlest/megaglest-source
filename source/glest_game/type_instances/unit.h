...
class UnitPath : public list<Vec2i>, public UnitPathInterface {
 private:
  static const int maxBlockCount =
      10; /**< number of command updates to wait on a blocked path */
 ...
 public:
  UnitPath()
      : UnitPathInterface(),
        blockCount(0),
        map(NULL) {} /**< Construct path object */
  ...
  // virtual Vec2i pop()		{ Vec2i p= front(); erase(begin());
  // return p; }	/**< pop the next position off the path */
  ...
  virtual vector<Vec2i> getQueue() const {
    vector<Vec2i> result;
    for (list<Vec2i>::const_iterator iter = this->begin(); iter != this->end();
         ++iter) {
...
class Unit : public BaseColorPickEntity,
             ValueCheckerVault,
 ...
 public:
  Unit(int id, UnitPathInterface *path, const Vec2i &pos, const UnitType *type,
       Faction *faction, Map *map, CardinalDir placeFacing);
  virtual ~Unit();
  ...
  const UnitAttackBoostEffectOriginator &getAttackBoostOriginatorEffect()
      const {
  ...
  // inline int getLastAnimProgress() const			{return
  // lastAnimProgress;} inline int getAnimProgress() const
  // {return animProgress;}
  inline float getLastAnimProgressAsFloat() const {
    return static_cast<float>(lastAnimProgress) / ANIMATION_SPEED_MULTIPLIER;
  }
  inline float getAnimProgressAsFloat() const {
    return static_cast<float>(animProgress) / ANIMATION_SPEED_MULTIPLIER;
  }

  inline float getHightlight() const { return highlight; }
  inline int getProgress2() const { return progress2; }
  inline int getFactionIndex() const { return faction->getIndex(); }
  inline int getTeam() const { return faction->getTeam(); }
  ...
  std::pair<CommandResult, string> giveCommand(
      Command *command, bool tryQueue = false);  // give a command
  ...
  const CommandType *computeCommandType(const Vec2i &pos,
                                        const Unit *targetUnit = NULL) const;
  ...
  // std::vector<std::pair<Vec2i,Chrono> > getBadHarvestPosList() const { return
  // badHarvestPosList; } void
  // setBadHarvestPosList(std::vector<std::pair<Vec2i,Chrono> > value) {
  // badHarvestPosList = value; }
  void addBadHarvestPos(const Vec2i &value);
  // void removeBadHarvestPos(const Vec2i &value);
  inline bool isBadHarvestPos(const Vec2i &value,
                              bool checkPeerUnits = true) const {
    bool result = false;
  ...
  // std::pair<Vec2i,std::vector<Vec2i> > getCurrentTargetPathTaken() const {
  // return currentTargetPathTaken; } void addCurrentTargetPathTakenCell(const
  // Vec2i &target,const Vec2i &cell);
  ...
  void saveGame(XmlNode *rootNode);
  static Unit *loadGame(const XmlNode *rootNode, GameSettings *settings,
 ...
 private:
  ...
  int64 getUpdatedProgress(int64 currentProgress, int64 updateFPS, int64 speed,

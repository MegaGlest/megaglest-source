  ...
  if (unitAttackBoostEffectNode->hasAttribute("source-faction") == true) {
    ...
    if (unitType != NULL) {
      ...
      if (skillType != NULL) {
        ...
        //				printf("#2 boost effect unitId = %d unitPtr
        //=
        //[%d - %s] source = %p attackBoost src [%p] dest [%p]\n",
...
Unit::Unit(int id, UnitPathInterface *unitpath, const Vec2i &pos,
  ...
  //"_"
  //+ intToStr(__LINE__)); 		deletedUnits.erase(this);
  //	}
...
// intToStr(__LINE__)); 	if(deletedUnits.find(unit) != deletedUnits.end())
// { 		result = true;
//	}
//	return result;
// }
...
void Unit::updateAttackBoostProgress(const Game *game) {
  ...
  if (debugBoost)
    printf("===================== START Unit [%d - %s] skill: %s affected unit "
        ...
        for (int i = (int)currentAttackBoostOriginatorEffect
          ...
          if (iterFound == candidateValidIdList.end()) {
            ...
            if (affectedUnit != NULL) {
              ...
              if (debugBoost)
                printf("Removed attack boost from Unit [%d - %s] since they "
                       "are NO LONGER in range\n",
                       affectedUnit->id,
  ...
  if (debugBoost) {
    if (currSkill->isAttackBoostEnabled() == true) {
      ...
      if (currentAttackBoostOriginatorEffect.currentAttackBoostUnits.empty() ==
          false) {
        printf("Found affected units "
               "currentAttackBoostOriginatorEffect.skillType [%p]\n",
               currentAttackBoostOriginatorEffect.skillType);
...
bool Unit::applyAttackBoost(const AttackBoost *boost, const Unit *source) {
  ...
  if (shouldApplyAttackBoost == true) {
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("=== APPLYING ATTACK BOOST START to unit [%s - %d] from unit [%s "
             "- %d] hp: %d\n",
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("APPLIED ATTACK BOOST START to unit [%s - %d] from unit [%s - %d] "
           "hp: %d\n",
           this->getType()->getName(false).c_str(), this->getId(),
           source->getType()->getName(false).c_str(), source->getId(), hp);
...
void Unit::deapplyAttackBoost(const AttackBoost *boost, const Unit *source) {
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("=== DE-APPLYING ATTACK BOOST START to unit [%s - %d] from unit [%s "
           "- %d] hp: %d\n",
           this->getType()->getName(false).c_str(), this->getId(),
           source->getType()->getName(false).c_str(), source->getId(), hp);
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("DE-APPLIED ATTACK BOOST START to unit [%s - %d] from unit [%s - "
           "%d] hp: %d\n",
           this->getType()->getName(false).c_str(), this->getId(),
           source->getType()->getName(false).c_str(), source->getId(), hp);
...
Unit *Unit::loadGame(const XmlNode *rootNode, GameSettings *settings,
  ...
  //&pos, 			   const UnitType *type, Faction *faction, Map *map,
  ...
  //damageParticleSystemsInUseNode2.size();
  //++i) { 			XmlNode *d2Node =
  //damageParticleSystemsInUseNode2[i];
  //
  //			vector<XmlNode *> unitParticleSystemNodeList =
  // damageParticleSystemsInUseNode->getChildList("UnitParticleSystem");
  //			for(unsigned int i = 0; i <
  //unitParticleSystemNodeList.size();
  //++i) { 				XmlNode *node =
  //unitParticleSystemNodeList[i];
  //
  ...
  // ++iterMap) { 		XmlNode *badHarvestPosListNode =

...
bool UnitPathBasic::isEmpty() const { return pathQueue.empty(); }

bool UnitPathBasic::isBlocked() const { return blockCount >= maxBlockCount; }
...
void UnitPathBasic::add(const Vec2i &path) {
  if (this->map != NULL) {
    if (this->map->isInside(path) == false) {
      throw megaglest_runtime_error(
          "Invalid map path position = " + path.getString() + " map w x h = " +
          intToStr(map->getW()) + " " + intToStr(map->getH()));
    } else if (this->map->isInsideSurface(this->map->toSurfCoords(path)) ==
               false) {
      throw megaglest_runtime_error(
          "Invalid map surface path position = " + path.getString() +
          " map surface w x h = " + intToStr(map->getSurfaceW()) + " " +
  ...
  if (Thread::isCurrentThreadMainThread() == false) {
    throw megaglest_runtime_error(
        "Invalid access to UnitPathBasic add from outside main thread current "
        "id = " +
        intToStr(Thread::getCurrentThreadId()) +
        " main = " + intToStr(Thread::getMainThreadId()));
  }
...
Vec2i UnitPathBasic::pop(bool removeFrontPos) {
  ...
  if (removeFrontPos == true) {
    if (Thread::isCurrentThreadMainThread() == false) {
      throw megaglest_runtime_error(
          "Invalid access to UnitPathBasic delete from outside main thread "
          "current id = " +
          intToStr(Thread::getCurrentThreadId()) +
          " main = " + intToStr(Thread::getMainThreadId()));
    }
...
std::string UnitPathBasic::toString() const {
  ...
  for (int idx = 0; idx < (int)pathQueue.size(); ++idx) {
    result +=
        " index = " + intToStr(idx) + " value = " + pathQueue[idx].getString();
  }
...
void UnitReference::saveGame(XmlNode *rootNode) {
  ...
  if (faction != NULL) {
    unitRefNode->addAttribute("factionIndex", intToStr(faction->getIndex()),
                              mapTagReplacements);
  }
}
...
UnitAttackBoostEffect::~UnitAttackBoostEffect() {
  ...
  if (ups != NULL) {
    bool particleValid =
        Renderer::getInstance().validateParticleSystemStillExists(ups, rsGame);
    if (particleValid == true) {
      ...
      Renderer::getInstance().cleanupUnitParticleSystems(particleSystemToRemove,
                                                         rsGame);
      ups = NULL;
    }
  }
...
const Unit *UnitAttackBoostEffect::getSource() {
  if (source == NULL && unitPtr != NULL && unitId > 0) {
    ...
    // printf("#1 Get source - boost effect unitId = %d unitPtr = %s source =
    // %p\n",unitId,unitPtr->getFullName(false).c_str(),source);
  }
  // printf("#2 Get source - boost effect unitId = %d unitPtr = %s source =
  // %p\n",unitId,unitPtr->getFullName(false).c_str(),source);
  return source;
}

void UnitAttackBoostEffect::setSource(const Unit *unit) { source = unit; }

void UnitAttackBoostEffect::applyLoadedAttackBoostParticles(
    UnitParticleSystemType *upstPtr, const XmlNode *node, Unit *unit) {
  if (upstPtr != NULL) {
    ...
    if (showUnitParticles == true) {
      ...
      *upst = *upstPtr;
      ...
      Renderer::getInstance().addToDeferredParticleSystemList(
          make_pair(ups, rsGame));
    }
  }
}

void UnitAttackBoostEffect::loadGame(const XmlNode *rootNode, Unit *unit,
  ...
  if (unitAttackBoostEffectNode->hasAttribute("source") == true) {
    ...
    //		printf("#1 Loaded boost effect unitId = %d unitPtr = [%d - %s]
    // source = %p\n",
    //				unitId,unitPtr->getId(),unitPtr->getType()->getName(false).c_str(),source);
  }

  // Lets determine the originator unit's attack boost based on the skill used
  // to trigger it
  if (unitAttackBoostEffectNode->hasAttribute("source-faction") == true) {
    string factionName =
        unitAttackBoostEffectNode->getAttribute("source-faction")->getValue();
    string unitTypeName =
        unitAttackBoostEffectNode->getAttribute("source-unit-type")->getValue();
    string skillTypeName =
        unitAttackBoostEffectNode->getAttribute("source-skill-type")
            ->getValue();
    SkillClass skillClass = static_cast<SkillClass>(
    ...
    if (unitType != NULL) {
      ...
      if (skillType != NULL) {
        ...
        //				printf("#2 boost effect unitId = %d
        // unitPtr = [%d - %s] source = %p attackBoost src [%p] dest [%p]\n",
  ...
  if (boost != NULL) {
    //		printf("unit [%d - %s] applyToOriginator: %d src [%p] dest [%p]
    // Boost attackBoost->enabled = %d:\n%s\n",
    ...
    if (applyToOriginator == true) {
      applyLoadedAttackBoostParticles(
          boost->unitParticleSystemTypeForSourceUnit, unitAttackBoostEffectNode,
          unit);
    } else {
      applyLoadedAttackBoostParticles(
...
void UnitAttackBoostEffect::saveGame(XmlNode *rootNode) {
  ...
  //	const Unit *source;
  if (getSource() != NULL) {
    unitAttackBoostEffectNode->addAttribute(
        "source", intToStr(getSource()->getId()), mapTagReplacements);

    unitAttackBoostEffectNode->addAttribute(
        "source-faction", getSource()->getFaction()->getType()->getName(false),
        mapTagReplacements);
    unitAttackBoostEffectNode->addAttribute(
        "source-unit-type", getSource()->getType()->getName(false),
        mapTagReplacements);
    unitAttackBoostEffectNode->addAttribute(
        "source-skill-type", getSource()->getCurrSkill()->getName(),
        mapTagReplacements);
    unitAttackBoostEffectNode->addAttribute(
        "source-skill-class", intToStr(getSource()->getCurrSkill()->getClass()),
        mapTagReplacements);
  }
  //	UnitParticleSystem *ups;
  if (ups != NULL && Renderer::getInstance().validateParticleSystemStillExists(
                         ups, rsGame) == true) {
...
void UnitAttackBoostEffectOriginator::loadGame(const XmlNode *rootNode,
  ...
  if (unitAttackBoostEffectOriginatorNode->hasAttribute("skillClass") ==
    ...
    skillClass = static_cast<SkillClass>(
  ...
  if (unitAttackBoostEffectOriginatorNode->hasChild("UnitAttackBoostEffect") ==
      true) {
    currentAppliedEffect = new UnitAttackBoostEffect();
    currentAppliedEffect->loadGame(unitAttackBoostEffectOriginatorNode, unit,
                                   world, true);
  }
}
...
void UnitAttackBoostEffectOriginator::saveGame(XmlNode *rootNode) {
  ...
  //	const SkillType *skillType;
  if (skillType != NULL) {
    unitAttackBoostEffectOriginatorNode->addAttribute(
        "skillType", skillType->getName(), mapTagReplacements);
    unitAttackBoostEffectOriginatorNode->addAttribute(
        "skillClass", intToStr(skillType->getClass()), mapTagReplacements);
  }
  ...
  for (unsigned int i = 0; i < currentAttackBoostUnits.size(); ++i) {
    XmlNode *currentAttackBoostUnitsNode =
        unitAttackBoostEffectOriginatorNode->addChild(
            "currentAttackBoostUnits");
    currentAttackBoostUnitsNode->addAttribute(
        "value", intToStr(currentAttackBoostUnits[i]), mapTagReplacements);
  }
...
const int Unit::maxDeadCount =
    1000;  // time in until the corpse disapears - should be about 40 seconds
...
Unit::Unit(int id, UnitPathInterface *unitpath, const Vec2i &pos,
  ...
  //		MutexSafeWrapper safeMutex(&mutexDeletedUnits,string(__FILE__) +
  //"_" + intToStr(__LINE__)); 		deletedUnits.erase(this);
  //	}
...
Unit::~Unit() {
  ...
  // MutexSafeWrapper safeMutex1(&mutexDeletedUnits,string(__FILE__) + "_" +
  // intToStr(__LINE__)); deletedUnits[this]=true;
...
void Unit::cleanupAllParticlesystems() {
  Renderer::getInstance().cleanupUnitParticleSystems(unitParticleSystems,
                                                     rsGame);
  ...
  if (Renderer::getInstance().validateParticleSystemStillExists(
          this->fire, rsGame) == false) {
    this->fire = NULL;
  }
...
string Unit::getParticleInfo() const {
  string result = "";
  if (networkCRCParticleInfoList.empty() == false) {
    for (unsigned int index = 0; index < networkCRCParticleInfoList.size();
         ++index) {
...
void Unit::end(ParticleSystem *particleSystem) {
  ...
  vector<UnitParticleSystem *>::iterator iterFind1 = find(
      smokeParticleSystems.begin(), smokeParticleSystems.end(), particleSystem);
  if (iterFind1 != smokeParticleSystems.end()) {
    smokeParticleSystems.erase(iterFind1);
    return;
  }
  iterFind = find(fireParticleSystems.begin(), fireParticleSystems.end(),
                  particleSystem);
  if (iterFind != fireParticleSystems.end()) {
    fireParticleSystems.erase(iterFind);
    return;
  }
  iterFind1 = find(damageParticleSystems.begin(), damageParticleSystems.end(),
                   particleSystem);
  ...
  iterFind1 = find(unitParticleSystems.begin(), unitParticleSystems.end(),
                   particleSystem);
...
//	bool result = false;
//	MutexSafeWrapper safeMutex(&mutexDeletedUnits,string(__FILE__) + "_" +
// intToStr(__LINE__)); 	if(deletedUnits.find(unit) !=
// deletedUnits.end()) { 		result = true;
//	}
//	return result;
// }
...
void Unit::calculateXZRotation() {
  ...
  // currSkill->getClass()==scMove){ if(currSkill->getClass()==scMove)
  if (lastPos != pos) {  // targetPosCalc ( maybe also sometimes needed if no
                         // move ? terrain flatting... )
    SurfaceCell *sc = map->getSurfaceCell(Map::toSurfCoords(pos));
    const Vec3f normal = sc->getNormal();

#ifdef USE_STREFLOP
    targetRotationZ =
        radToDeg(streflop::atan2(static_cast<streflop::Simple>(abs(normal.x)),
...
#ifdef USE_STREFLOP
    targetRotationX =
        radToDeg(streflop::atan2(static_cast<streflop::Simple>(abs(normal.z)),
                                 static_cast<streflop::Simple>(abs(normal.y))));
#else
...
float Unit::getRotationZ() const { return rotationZ; }

float Unit::getRotationX() const { return rotationX; }
...
bool Unit::isOperative() const { return isAlive() && isBuilt(); }
...
bool Unit::isBuilt() const { return (isBeingBuilt() == false); }
...
bool Unit::isBuildCommandPending() const {
  ...
  if (command != NULL) {
    const BuildCommandType *bct =
        dynamic_cast<const BuildCommandType *>(command->getCommandType());
    if (bct != NULL) {
...
void Unit::setCurrSkill(const SkillType *currSkill) {
  ...
  if (showUnitParticles == true &&
    ...
    currSkill->unitParticleSystemTypes.begin(); it !=
    currSkill->unitParticleSystemTypes.end(); ++it) { if((*it)->getStartTime()
    == 0.0) {
                    //printf("Adding NON-queued particle system type [%s] [%f]
    [%f]\n",(*it)->getType().c_str(),(*it)->getStartTime(),(*it)->getEndTime());
                    ...
                    //printf("Adding queued particle system type [%s] [%f]
    [%f]\n",(*it)->getType().c_str(),(*it)->getStartTime(),(*it)->getEndTime());
    ...
    */
    checkCustomizedUnitParticleListTriggers(currSkill->unitParticleSystemTypes,
                                            true);
  }
  progress2 = 0;
...
FowAlphaCellsLookupItem Unit::getFogOfWarRadius(bool useCache) const {
  ...
  while (pci.next()) {
    ...
    // compute max alpha
    float maxAlpha = 0.0f;
    if (surfPos.x > 1 && surfPos.y > 1 && surfPos.x < map->getSurfaceW() - 2 &&
    ...
    if (dist > sightRange) {
      alpha = clamp(1.f - (dist - sightRange) / (World::indirectSightRange),
                    0.f, maxAlpha);
    }
...
#ifdef USE_STREFLOP
  targetRotation =
      radToDeg(streflop::atan2(static_cast<streflop::Simple>(relPosf.x),
...
void Unit::setVisible(const bool visible) {
    ...
    for (UnitParticleSystems::iterator it = smokeParticleSystems.begin();
      ...
      if (Renderer::getInstance().validateParticleSystemStillExists(
        ...
        if ((*it)->getVisible() != visible) {
          // printf("Changing visibility for smoke particle system to:
          // %d\n",visible);
          (*it)->setVisible(visible);
        }
      }
  ...
  //	for(vector<ParticleSystem*>::iterator it= attackParticleSystems.begin();
  // it != attackParticleSystems.end(); ++it) {
  //		if(Renderer::getInstance().validateParticleSystemStillExists((*it),rsGame)
  //== true) {
  //  Not sure this is a good idea since the unit be not be visible but the
  //  attack particle might be. This means you won't see the attacking
  //  projectile until the unit moves into view.
  //(*it)->setVisible(visible);
  //		}
  //	}
  //}
...
Model *Unit::getCurrentModelPtr() {
  ...
  Model *result = currSkill->getAnimation(getAnimProgressAsFloat(), this,
...
//	if(currSkill == NULL) {
//		char szBuf[8096]="";
//		snprintf(szBuf,8096,"In [%s::%s Line: %d] ERROR: currSkill ==
// NULL, Unit =
...
//&animationRandomCycleCount); 	if(currentModelIndexForCurrSkillType !=
// lastModelIndexForCurrSkillType) { 		animationRandomCycleCount++;
// if(currSkill != NULL && animationRandomCycleCount >=
// currSkill->getAnimationCount()) { animationRandomCycleCount = 0;
//		}
//	}
//	return result;
// }
  ...
  if (currSkill != NULL && currSkill->getAnimationCount() > 1) {
    if (lastModelIndexForCurrSkillType >= 0) {
      ...
      if (attributes.fromHp != 0 || attributes.toHp != 0) {
        // printf("Check for RESET model state for [%d - %s] HP = %d [%d to
        // %d]\n",this->id,this->getType()->getName().c_str(),this->getHp(),attributes.fromHp,attributes.toHp);
        // if(this->getHp() >= attributes.fromHp && this->getHp() <=
        // attributes.toHp) {
        if (this->getHp() < attributes.fromHp ||
            this->getHp() > attributes.toHp) {
          // printf("RESET model state for [%d - %s] HP = %d [%d to
          // %d]\n",this->id,this->getType()->getName().c_str(),this->getHp(),attributes.fromHp,attributes.toHp);
        ...
        // printf("Check for RESET #2 model state for [%d - %s] HP = %d [%d to
        // %d] for skill
...
Vec3f Unit::getCurrVectorAsTarget() const {
  ...
  Vec3f result =
      getCurrVectorFlat() + Vec3f(0.f, type->getTargetHeight() / 2.f, 0.f);
  result.x = truncateDecimal<float>(result.x, 6);
  result.y = truncateDecimal<float>(result.y, 6);
  result.z = truncateDecimal<float>(result.z, 6);
...
Vec3f Unit::getCurrVectorFlat() const { return getVectorFlat(lastPos, pos); }
...
bool Unit::anyCommand(bool validateCommandtype) const {
  ...
  if (validateCommandtype == false) {
    ...
    for (Commands::const_iterator it = commands.begin(); it != commands.end();
         ++it) {
...
int Unit::getCountOfProducedUnitsPreExistence(const UnitType *ut) const {
  ...
  for (Commands::const_iterator it = commands.begin(); it != commands.end();
    ...
    if (ct->getClass() == ccBuild) {
      ...
      // we dont count builded units if they are already exist. So we just count
      // the units that will be build. otherwise we will get double counts of
      // alive units and units beeing build.
...
std::pair<CommandResult, string> Unit::giveCommand(Command *command,
  ...
  // printf("In [%s::%s] Line: %d unit [%d - %s] command [%s] tryQueue = %d
  // command->getCommandType()->isQueuable(tryQueue) =
  // %d\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,this->getId(),this->getType()->getName().c_str(),
  // command->getCommandType()->getName().c_str(),
  // tryQueue,command->getCommandType()->isQueuable(tryQueue));
  ...
  if (command->getCommandType()->isQueuable(tryQueue)) {
        ...
        if ((*i)->getPriority() < command_priority) {
          if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
            ...
            SystemFlags::OutputDebug(
                ...
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, (*i)->toString(false).c_str());
    ...
    if (commands.empty() == false &&
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
              .enabled)
        SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                                 "In [%s::%s Line: %d] Cancel command because "
                                 "last one is NOT queable [%s]\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__,
                                 commands.back()->toString(false).c_str());
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
            .enabled)
      SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                               "In [%s::%s Line: %d] Clear commands because "
                               "current is NOT queable.\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__);
    ...
    if (willChangedActiveCommand == true) {
      ...
      // printf("Line: %d getCurrCommand() [%s] command
    ...
    // printf("In [%s::%s] Line: %d cleared existing
    // commands\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__);
  ...
  // printf("In [%s::%s] Line: %d check command returned %d, commands.size() =
...
CommandResult Unit::finishCommand() {
  ...
  while (commands.empty() == false) {
    if (commands.front()->getUnit() != NULL &&
        this->faction->isUnitInLivingUnitsp(commands.front()->getUnit()) ==
            false) {
      safeMutex.Lock();
      delete commands.front();
      commands.erase(commands.begin());
...
void Unit::born(const CommandType *ct) {
  ...
  if (type->getStartHpType() == UnitType::stValue) {
    ...
    this->hp =
        type->getTotalMaxHp(&totalUpgrade) * type->getStartHpPercentage() / 100;
  }
  ...
  if (type->getStartEpType() == UnitType::stValue) {
    ...
    this->ep =
        type->getTotalMaxEp(&totalUpgrade) * type->getStartEpPercentage() / 100;
  }
}
...
void Unit::kill() {
  ...
  if (isBeingBuilt()) {
    faction->deApplyStaticConsumption(
        type,
        (getCurrCommand() != NULL ? getCurrCommand()->getCommandType() : NULL));
  } else {
    faction->deApplyStaticCosts(
        type,
        (getCurrCommand() != NULL ? getCurrCommand()->getCommandType() : NULL));
  }
  ...
  try {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(
          ...
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, this->id, this->getFullName(false).c_str(),
          this->getDesc(false).c_str());

    // Remove any units that were previously in attack-boost range
    if (currentAttackBoostOriginatorEffect.currentAttackBoostUnits.empty() ==
            false &&
      ...
      for (unsigned int i = 0;
        ...
        // printf("!!!! DE-APPLY ATTACK BOOST from unit [%s -
...
void Unit::notifyObservers(UnitObserver::Event event) {
  for (Observers::iterator it = observers.begin(); it != observers.end();
       ++it) {
...
void Unit::resetHighlight() { highlight = 1.f; }

const CommandType *Unit::computeCommandType(const Vec2i &pos,
                                            const Unit *targetUnit) const {
  ...
  // printf("Line: %d Unit::computeCommandType pos [%s] targetUnit
  // [%s]\n",__LINE__,pos.getString().c_str(),(targetUnit != NULL ?
  // targetUnit->getType()->getName().c_str() : "(null)"));
  if (targetUnit != NULL) {
    // attack enemies
    ...
    // repair allies
      ...
      // commandType [%p]\n",__LINE__,pos.getString().c_str(),(targetUnit !=
      // NULL ? targetUnit->getType()->getName().c_str() :
      // "(null)"),commandType);

      if (commandType == NULL && targetUnit != NULL) {
        // Command *command= this->getCurrCommand();
        // const HarvestCommandType *hct= dynamic_cast<const
        // HarvestCommandType*>((command != NULL ? command->getCommandType() :
        // NULL));

        // Check if we can return whatever resources we have
        if (targetUnit->getFactionIndex() == this->getFactionIndex() &&
            targetUnit->isOperative() == true && this->getLoadType() != NULL &&
            this->getLoadCount() != 0 && targetUnit->getType() != NULL &&
    ...
    if (resource != NULL) {
      commandType =
          type->getFirstHarvestCommand(resource->getType(), this->getFaction());
    }
  }
  ...
  if (targetUnit == NULL && commandType == NULL) {
    ...
    if (cell != NULL && cell->getUnit(this->getCurrField()) != NULL) {
      ...
      if (targetUnit != NULL) {
        // Check if we want to help build (repair) any buildings instead of just
        // moving
        if (targetUnit->getFactionIndex() == this->getFactionIndex() &&
            (targetUnit->isBuilt() == false ||
             targetUnit->isDamaged() == true)) {
          const RepairCommandType *rct =
              this->getType()->getFirstRepairCommand(targetUnit->getType());
          if (rct != NULL) {
            commandType = type->getFirstRepairCommand(targetUnit->getType());
            // printf("************ Unit will repair building built = %d, repair
...
int64 Unit::getUpdateProgress() {
  if (progress > PROGRESS_SPEED_MULTIPLIER) {
    ...
    snprintf(szBuf, 8096,
             ...
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__, PROGRESS_SPEED_MULTIPLIER, progress);
    throw megaglest_runtime_error(szBuf);
  }
  ...
  if (currSkill->getClass() != scDie) {
    ...
    if (changedActiveCommand == true) {
      if (changedActiveCommandFrame - lastChangedActiveCommandFrame >=
        ...
        // printf("Line: %d speed = %d changedActiveCommandFrame [%u]
        // lastChangedActiveCommandFrame [%u] skill [%s] command
        ...
        // printf("Line: %d speed = %d changedActiveCommandFrame [%u]
        // lastChangedActiveCommandFrame [%u] total =
        // %u\n",__LINE__,speed,changedActiveCommandFrame,lastChangedActiveCommandFrame,(changedActiveCommandFrame
        // - lastChangedActiveCommandFrame));
    ...
    //			throw megaglest_runtime_error("game->getWorld() ==
    // NULL");
    //		}

    newProgress = getUpdatedProgress(progress, GameConstants::updateFps, speed,
                                     diagonalFactor, heightFactor);
  }
  return newProgress;
}
...
int64 Unit::getHeightFactor(int64 speedMultiplier) {
  ...
  if (currSkill->getClass() == scMove) {
    ...
    int64 heightDiff =
        ((truncateDecimal<float>(unitCell->getHeight(), 2) * speedMultiplier) -
         (truncateDecimal<float>(targetCell->getHeight(), 2) *
          speedMultiplier));
    // heightFactor= clamp(speedMultiplier + heightDiff / (5.f *
    // speedMultiplier), 0.2f * speedMultiplier, 5.f * speedMultiplier);
    heightFactor = clamp(speedMultiplier + heightDiff / (5 * speedMultiplier),
...
int64 Unit::getUpdatedProgress(int64 currentProgress, int64 updateFPS,
  ...
  // Ensure we increment at least a value of 1 of the action will be stuck
  // infinitely
  ...
  if (isNetworkCRCEnabled() == true) {
    ...
    char szBuf[8096] = "";
    snprintf(szBuf, 8095,
             "currentProgress = " MG_I64_SPECIFIER
             " updateFPS = " MG_I64_SPECIFIER " speed = " MG_I64_SPECIFIER
             " diagonalFactor = " MG_I64_SPECIFIER
             " heightFactor = " MG_I64_SPECIFIER
             " speedDenominator = " MG_I64_SPECIFIER
             " progressIncrease = " MG_I64_SPECIFIER " [" MG_I64_SPECIFIER
             "] height [%f] airHeight [%f] cellUnitHeight [%d] "
             "cellObjectHeight [%d] skill [%s] pos [%s] lastpos [%s]",
             currentProgress, updateFPS, speed, diagonalFactor, heightFactor,
             speedDenominator, progressIncrease,
             ((speed * diagonalFactor * heightFactor) / speedDenominator),
             height, airHeight, cellUnitHeight, cellObjectHeight,
             (currSkill != NULL ? currSkill->getName().c_str() : "none"),
  ...
  //	if(currSkill->getClass() == scMove || (currSkill->getClass() == scStop
  //&& this->loadCount > 0)) { 		printf("speedDenominator: "
  // MG_I64_SPECIFIER " currentProgress: " MG_I64_SPECIFIER " speed: "
  // MG_I64_SPECIFIER " diagonalFactor: " MG_I64_SPECIFIER " heightFactor: "
  // MG_I64_SPECIFIER " progressIncrease: " MG_I64_SPECIFIER " newProgress: "
  // MG_I64_SPECIFIER " TOP
  // #: " MG_I64_SPECIFIER
  //"\n",speedDenominator,currentProgress,speed,diagonalFactor,heightFactor,progressIncrease,newProgress,(speed
  //* diagonalFactor * heightFactor));
  //	}
...
void Unit::updateAttackBoostProgress(const Game *game) {
  ...
  if (debugBoost)
    printf(
        "===================== START Unit [%d - %s] skill: %s affected unit "
        "size: " MG_SIZE_T_SPECIFIER "\n",
        this->id, this->getType()->getName(false).c_str(),
  ...
  if (currSkill != currentAttackBoostOriginatorEffect.skillType) {
    ...
    if (currentAttackBoostOriginatorEffect.currentAppliedEffect != NULL) {
      ...
      // printf("- #1 DE-APPLY ATTACK BOOST SELF PARTICLE to unit [%s -
    ...
    if (currentAttackBoostOriginatorEffect.currentAttackBoostUnits.empty() ==
      ...
      for (unsigned int i = 0;
        ...
        // printf("- #1 DE-APPLY ATTACK BOOST from unit [%s -
        // %d]\n",affectedUnit->getType()->getName().c_str(),affectedUnit->getId());
      }
      currentAttackBoostOriginatorEffect.currentAttackBoostUnits.clear();
    }
    ...
    if (currSkill->isAttackBoostEnabled() == true) {
      if (debugBoost)
        printf(
            "Line: %d affected unit size: " MG_SIZE_T_SPECIFIER "\n", __LINE__,
            currentAttackBoostOriginatorEffect.currentAttackBoostUnits.size());
      ...
      for (unsigned int i = 0; i < candidates.size(); ++i) {
        ...
        if (attackBoost->isAffected(this, affectedUnit) == true) {
          if (affectedUnit->applyAttackBoost(attackBoost, this) == true) {
            ...
            // printf("+ #1 APPLY ATTACK BOOST to unit [%s -
            // %d]\n",affectedUnit->getType()->getName().c_str(),affectedUnit->getId());
          }
        }
      }
      ...
      if (showUnitParticles == true) {
        if (currentAttackBoostOriginatorEffect.currentAttackBoostUnits
          ...
          if (attackBoost->unitParticleSystemTypeForSourceUnit != NULL) {
            ...
            currentAttackBoostOriginatorEffect.currentAppliedEffect->upst =
            ...
            *currentAttackBoostOriginatorEffect.currentAppliedEffect->upst =
            ...
            currentAttackBoostOriginatorEffect.currentAppliedEffect->upst
                ->setValues(currentAttackBoostOriginatorEffect
            ...
            Renderer::getInstance().manageParticleSystem(
                currentAttackBoostOriginatorEffect.currentAppliedEffect->ups,
                rsGame);
            // printf("+ #1 APPLY ATTACK BOOST SELF PARTICLE to unit [%s -
            // %d]\n",this->getType()->getName().c_str(),this->getId());
          }
        }
      }
    ...
    if (currSkill->isAttackBoostEnabled() == true) {
      ...
      for (unsigned int i = 0; i < candidates.size(); ++i) {
        ...
        if (attackBoost->isAffected(this, affectedUnit) == true) {
          if (iterFound == currentAttackBoostOriginatorEffect
                               .currentAttackBoostUnits.end()) {
            if (affectedUnit->applyAttackBoost(attackBoost, this) == true) {
              currentAttackBoostOriginatorEffect.currentAttackBoostUnits
                  .push_back(affectedUnit->getId());

              // printf("+ #2 APPLY ATTACK BOOST to unit [%s -
              // %d]\n",affectedUnit->getType()->getName().c_str(),affectedUnit->getId());
            }
          }
        } else {
          if (iterFound != currentAttackBoostOriginatorEffect
            ...
            // printf("- #2 DE-APPLY ATTACK BOOST from unit [%s -
            // %d]\n",affectedUnit->getType()->getName().c_str(),affectedUnit->getId());
          }
        }
      }
      ...
      if (currentAttackBoostOriginatorEffect.currentAttackBoostUnits.empty() ==
          false) {
        for (int i = (int)currentAttackBoostOriginatorEffect
          ...
          if (iterFound == candidateValidIdList.end()) {
            ...
            if (affectedUnit != NULL) {
              ...
              if (debugBoost)
                printf(
                    "Removed attack boost from Unit [%d - %s] since they are "
                    "NO LONGER in range\n",
                    affectedUnit->id,
        ...
        if (currentAttackBoostOriginatorEffect.currentAttackBoostUnits
          ...
          if (attackBoost->unitParticleSystemTypeForSourceUnit != NULL &&
            ...
            *currentAttackBoostOriginatorEffect.currentAppliedEffect->upst =
            ...
            // printf("+ #2 APPLY ATTACK BOOST SELF PARTICLE to unit [%s -
            // %d]\n",this->getType()->getName().c_str(),this->getId());
          }
        } else if (currentAttackBoostOriginatorEffect.currentAttackBoostUnits
                       .empty() == true) {
          if (currentAttackBoostOriginatorEffect.currentAppliedEffect != NULL) {
            delete currentAttackBoostOriginatorEffect.currentAppliedEffect;
            currentAttackBoostOriginatorEffect.currentAppliedEffect = NULL;

            // printf("- #2 DE-APPLY ATTACK BOOST SELF PARTICLE to unit [%s -
            // %d]\n",this->getType()->getName().c_str(),this->getId());
          }
        }
      }
  ...
  if (debugBoost) {
    if (currSkill->isAttackBoostEnabled() == true) {
      ...
      if (currentAttackBoostOriginatorEffect.currentAttackBoostUnits.empty() ==
          false) {
        printf(
            "Found affected units currentAttackBoostOriginatorEffect.skillType "
            "[%p]\n",
            currentAttackBoostOriginatorEffect.skillType);
...
bool Unit::update() {
  ...
  if (highlight > 0.f) {
    const Game *game = Renderer::getInstance().getGame();
    highlight -= 1.f / (Game::highlightTime * game->getWorld()->getUpdateFps(
  ...
  progress = getUpdatedProgress(progress, GameConstants::updateFps, speed,
                                diagonalFactor, heightFactor);

  // printf("Test progress = %d for unit [%d -
  ...
  if (isAnimProgressBound() == true) {
    ...
    if (this->animProgress < targetProgressIntValue) {
      ...
      float progressIncrease =
          static_cast<float>(this->animProgress) +
          diff / static_cast<float>(GameConstants::updateFps);
      // Ensure we increment at least a value of 1 of the action will be stuck
      // infinitely
      ...
      // if(currSkill->getClass() == scBeBuilt) {
      //	printf("targetProgress: %.10f this->animProgress: %d diff: %.10f
      // GameConstants::updateFps: %d progressIncrease:
      ...
      // if(currSkill->getClass() == scBeBuilt) {
      //	printf("Unit build progress: %d anim:
    ...
    int64 speedDenominator =
        speedDivider * game->getWorld()->getUpdateFps(this->getFactionIndex());

    // Override the animation speed for attacks that have upgraded the attack
    // speed
    int animSpeed = currSkill->getAnimSpeed();
    if (currSkill->getClass() == scAttack) {
      int animSpeedBoost =
          ((AttackSkillType *)currSkill)->getAnimSpeedBoost(&totalUpgrade);
    ...
    this->animProgress += progressIncrease;
    // this->animProgress += (currSkill->getAnimSpeed() * heightFactor) /
    // speedDenominator;
    ...
    if (getProgressAsFloat() < 1.f / rotFactor) {
      if (type->getFirstStOfClass(scMove)) {
        if (abs((int)(lastRotation - targetRotation)) < 180)
          ...
          rotation =
              lastRotation + (targetRotation - lastRotation + rotationTerm) *
                                 getProgressAsFloat() * rotFactor;
        }
      }
    }
  ...
  // this->getType()->getName(false).c_str(),(int)currentAttackBoostEffects.size());
  for (unsigned int i = 0; i < currentAttackBoostEffects.size(); ++i) {
    UnitAttackBoostEffect *effect = currentAttackBoostEffects[i];
    if (effect != NULL && effect->ups != NULL) {
      bool particleValid =
          Renderer::getInstance().validateParticleSystemStillExists(effect->ups,
                                                                    rsGame);
      if (particleValid == true) {
    ...
    for (int i = (int)queuedUnitParticleSystemTypes.size() - 1; i >= 0; i--) {
      ...
      if (pst != NULL) {
        if (truncateDecimal<float>(pst->getStartTime(), 6) <=
          ...
          if (getFaction()->getTexture()) {
            ups->setFactionColor(
                getFaction()->getTexture()->getPixmapConst()->getPixel3f(0, 0));
          }
          unitParticleSystems.push_back(ups);
          Renderer::getInstance().manageParticleSystem(ups, rsGame);
...
bool Unit::unitHasAttackBoost(const AttackBoost *boost, const Unit *source) {
  ...
  // printf("Unit has attack boost? source = [%d - %s] [%p] boost [%s] result =
  // %d\n",source->getId(),
  // source->getType()->getName(false).c_str(),source,boost->name.c_str(),result);
...
bool Unit::applyAttackBoost(const AttackBoost *boost, const Unit *source) {
  ...
  // printf("APPLYING ATTACK BOOST to unit [%s - %d] from unit [%s -
  ...
  if (shouldApplyAttackBoost == true) {
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "=== APPLYING ATTACK BOOST START to unit [%s - %d] from unit [%s - "
          "%d] hp: %d\n",
          this->getType()->getName(false).c_str(), this->getId(),
          source->getType()->getName(false).c_str(), source->getId(), hp);
    ...
    // printf("#1 wasAlive = %d hp = %d boosthp =
    ...
    if (totalUpgrade.getMaxHpRegeneration() != 0) {
      ...
      // printf("BEFORE Apply Hp Regen max = %d, prev = %d, hp =
      ...
      // printf("AFTER Apply Hp Regen max = %d, prev = %d, hp =
      // %d\n",totalUpgrade.getMaxHpRegeneration(),prevMaxHpRegen,hp);
    }
    ...
    // printf("#2 wasAlive = %d hp = %d boosthp =
    // %d\n",wasAlive,hp,boost->boostUpgrade.getMaxHp());
    ...
    if (wasAlive == true) {
      ...
      if (this->hp <= 0) {
        ...
        Unit::game->getWorld()->getStats()->die(
            getFactionIndex(), getType()->getCountUnitDeathInStats());
        game->getScriptManager()->onUnitDied(this);

        StaticSound *sound = static_cast<const DieSkillType *>(
                                 this->getType()->getFirstStOfClass(scDie))
                                 ->getSound();
        if (sound != NULL &&
            (this->getFactionIndex() ==
                 Unit::game->getWorld()->getThisFactionIndex() ||
             (game->getWorld()->showWorldForPlayer(
                  game->getWorld()->getThisTeamIndex()) == true))) {
    ...
    // printf("APPLYING ATTACK BOOST END to unit [%s - %d] from unit [%s -
    // %d]\n",this->getType()->getName().c_str(),this->getId(),source->getType()->getName().c_str(),source->getId());
  }

  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf(
        "APPLIED ATTACK BOOST START to unit [%s - %d] from unit [%s - %d] hp: "
        "%d\n",
        this->getType()->getName(false).c_str(), this->getId(),
        source->getType()->getName(false).c_str(), source->getId(), hp);
...
void Unit::deapplyAttackBoost(const AttackBoost *boost, const Unit *source) {
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf(
        "=== DE-APPLYING ATTACK BOOST START to unit [%s - %d] from unit [%s - "
        "%d] hp: %d\n",
        this->getType()->getName(false).c_str(), this->getId(),
        source->getType()->getName(false).c_str(), source->getId(), hp);
  ...
  if (totalUpgrade.getMaxHpRegeneration() != 0) {
    ...
    // printf("BEFORE DeApply Hp Regen max = %d, prev = %d, hp =
    // %d\n",totalUpgrade.getMaxHpRegeneration(),prevMaxHpRegen,hp);
    int original_hp = this->hp;
    this->hp -= (totalUpgrade.getMaxHpRegeneration() - prevMaxHpRegen);
    if (original_hp != this->hp) {
    ...
    // printf("AFTER DeApply Hp Regen max = %d, prev = %d, hp =
    // %d\n",totalUpgrade.getMaxHpRegeneration(),prevMaxHpRegen,hp);
  }
  ...
  if (wasAlive == true) {
    // printf("DE-APPLYING ATTACK BOOST wasalive = true to unit [%s - %d] from
    // unit [%s -
    // %d]\n",this->getType()->getName().c_str(),this->getId(),source->getType()->getName().c_str(),source->getId());
  ...
  // printf("DE-APPLYING ATTACK BOOST BEFORE END to unit [%s - %d] from unit [%s
  // -
  // %d]\n",this->getType()->getName().c_str(),this->getId(),source->getType()->getName().c_str(),source->getId());
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf(
        "DE-APPLIED ATTACK BOOST START to unit [%s - %d] from unit [%s - %d] "
        "hp: %d\n",
        this->getType()->getName(false).c_str(), this->getId(),
        source->getType()->getName(false).c_str(), source->getId(), hp);

  // printf("DE-APPLYING ATTACK BOOST END to unit [%s - %d] from unit [%s -
  // %d]\n",this->getType()->getName().c_str(),this->getId(),source->getType()->getName().c_str(),source->getId());
}
...
void Unit::tick() {
  if (isAlive()) {
    ...
    // if(this->getType()->getName() == "spearman") printf("Unit [%d - %s] start
    ...
    if (type->getTotalMaxHpRegeneration(&totalUpgrade) != 0) {
      if (currSkill->getClass() != scBeBuilt) {
        if (type->getTotalMaxHpRegeneration(&totalUpgrade) >= 0) {
          ...
          checkModelStateInfoForNewHpValue();
          // if(this->getType()->getName() == "spearman") printf("tick hp#2
          // [type->getTotalMaxHpRegeneration(&totalUpgrade)] = %d
          // type->getTotalMaxHp(&totalUpgrade) [%d] newhp =
    ...
    // regenerate hp
      ...
      if (type->getTotalMaxHpRegeneration(&totalUpgrade) >= 0) {
        if (currSkill->getClass() != scBeBuilt) {
          ...
          checkModelStateInfoForNewHpValue();
          // if(this->getType()->getName() == "spearman") printf("tick hp#1
          // [type->getHpRegeneration()] = %d type->getTotalMaxHp(&totalUpgrade)
          // [%d] newhp =
...
string Unit::getDescExtension(bool translatedValue) const {
  ...
  if (commands.empty() == false && commands.size() > 1) {
    ...
    for (unsigned int i = 0;
         i < min((size_t)maxQueuedCommandDisplayCount, commands.size()); ++i) {
      const CommandType *ct = (*it)->getCommandType();
      if (i == 0) {
...
string Unit::getDesc(bool translatedValue) const {
  ...
  if (type->getMaxUnitCount() > 0) {
    str += lang.getString("MaxUnitCount") + ": " +
  ...
  if (type->getHpRegeneration() != 0 ||
    ...
    str += " (" + lang.getString("Regeneration") + ": " +
  ...
  if (enemyKills > 0 || nextLevel != NULL) {
    ...
    if (nextLevel != NULL) {
      str += " (" + nextLevel->getName(translatedValue) + ": " +
  ...
  for (int i = 0; i < getType()->getCostCount(); ++i) {
    ...
    if (r->getType()->getClass() == rcConsumable) {
      str += "\n";
      str += r->getAmount() < 0 ? lang.getString("Produce") + ": "
                                : lang.getString("Consume") + ": ";
      str += intToStr(abs(r->getAmount())) + " " +
...
void Unit::morphAttackBoosts(Unit *unit) {
  ...
  if (currentAttackBoostOriginatorEffect.currentAttackBoostUnits.empty() ==
          false &&
      currentAttackBoostOriginatorEffect.skillType != NULL) {
    for (int i = (int)currentAttackBoostOriginatorEffect.currentAttackBoostUnits
                     .size() -
                 1;
         i >= 0; --i) {
      // Remove attack boost upgrades from unit
...
bool Unit::morph(const MorphCommandType *mct, int frameIndex) {
  ...
  if (map->canMorph(pos, this, morphUnitType)) {
    ...
    // printf("Now unapply attack-boost for unit [%d -
    ...
    for (int i = (int)currentAttackBoostEffects.size() - 1; i >= 0; --i) {
      ...
      if (effect != NULL) {
        Unit *sourceUnit =
            game->getWorld()->findUnitById(effect->getSource()->getId());
        if (sourceUnit == NULL) {
...
float Unit::computeHeight(const Vec2i &pos) const {
  ...
  if (map->isInside(pos) == false ||
      map->isInsideSurface(map->toSurfCoords(pos)) == false) {
    // printf("CRASHING FOR UNIT: %d [%s] alive =
  ...
  if (currField == fAir) {
    ...
    Unit *unit = map->getCell(pos)->getUnit(fLand);
    if (unit != NULL && unit->getType()->getHeight() > airHeight) {
      height += (std::min((float)unit->getType()->getHeight(),
                          Tileset::standardAirHeight * 3) -
                 airHeight);
      height = truncateDecimal<float>(height, 6);
    } else {
      SurfaceCell *sc = map->getSurfaceCell(map->toSurfCoords(pos));
      if (sc != NULL && sc->getObject() != NULL &&
          sc->getObject()->getType() != NULL) {
        if (sc->getObject()->getType()->getHeight() > airHeight) {
          height += (std::min((float)sc->getObject()->getType()->getHeight(),
                              Tileset::standardAirHeight * 3) -
                     airHeight);
          height = truncateDecimal<float>(height, 6);
        }
    ...
    // for flying above water  we use the water height + airHeight  if it is
...
std::pair<CommandResult, string> Unit::checkCommand(Command *command) const {
  ...
  if (isOperative() == false || command->getUnit() == this ||
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugLUA,
          "In [%s::%s Line: %d] isOperative() = %d, command->getUnit() = %p, "
          "getType()->hasCommandType(command->getCommandType()) = %d, "
          "this->getFaction()->reqsOk(command->getCommandType()) = %d\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, isOperative(), command->getUnit(),
          getType()->hasCommandType(command->getCommandType()),
  ...
  // if pos is not inside the world (if comand has not a pos, pos is (0, 0) and
  // is inside world
  ...
  if (command->getCommandType() == NULL) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] ERROR: command->getCommandType() == NULL, "
             "Unit = [%s]\n",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__, this->toString().c_str());
    throw megaglest_runtime_error(szBuf);
  ...
  if (produced != NULL) {
    if (ignoreCheckCommand == false && faction->reqsOk(produced) == false) {
      // printf("In [%s::%s Line: %d] command =
      // %p\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,command);
      // printf("To produce this unit you
      // need:\n%s\n",produced->getUnitAndUpgradeReqDesc().c_str());
      result.first = crFailReqs;

      Lang &lang = Lang::getInstance();
      result.second = " - " + lang.getString("Reqs") + " : " +
                      produced->getUnitAndUpgradeReqDesc(
                          false, this->showTranslatedTechTree());
      return result;
    }
    ...
    if (ignoreCheckCommand == false &&
      ...
      // printf("To produce this unit you
      // need:\n%s\n",produced->getResourceReqDesc().c_str()); printf("In
      // [%s::%s Line: %d] command =
      // %p\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,command);
      ...
      result.second =
          " - " + lang.getString("Reqs") + " : " +
          produced->getResourceReqDesc(false, this->showTranslatedTechTree());
      return result;
    }
  }
  ...
  if (command->getCommandType()->getClass() == ccBuild) {
    ...
    if (faction->reqsOk(builtUnit) == false) {
      // printf("To build this unit you
      // need:\n%s\n",builtUnit->getUnitAndUpgradeReqDesc().c_str()); printf("In
      // [%s::%s Line: %d] command =
      // %p\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,command);
      result.first = crFailReqs;
      Lang &lang = Lang::getInstance();
      result.second = " - " + lang.getString("Reqs") + " : " +
                      builtUnit->getUnitAndUpgradeReqDesc(
                          false, this->showTranslatedTechTree());
      return result;
    }
    if (faction->checkCosts(builtUnit, NULL) == false) {
      // printf("To build this unit you
      // need:\n%s\n",builtUnit->getResourceReqDesc().c_str()); printf("In
      // [%s::%s Line: %d] command =
      // %p\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,command);
      result.first = crFailRes;
      Lang &lang = Lang::getInstance();
      result.second =
          " - " + lang.getString("Reqs") + " : " +
          builtUnit->getResourceReqDesc(false, this->showTranslatedTechTree());
      return result;
    }
  }
  // upgrade command specific, check that upgrade is not upgraded
    ...
    const UpgradeCommandType *uct =
        static_cast<const UpgradeCommandType *>(command->getCommandType());
    ...
    if (faction->getUpgradeManager()->isUpgradingOrUpgraded(
...
void Unit::stopDamageParticles(bool force) {
  if (force == true || (hp > type->getTotalMaxHp(&totalUpgrade) / 2)) {
    // printf("Checking to stop damageparticles for unit [%s - %d] hp =
    ...
    if (smokeParticleSystems.empty() == false) {
      // printf("Checking to stop smokeparticles for unit [%s - %d] hp =
      // %d\n",this->getType()->getName().c_str(),this->getId(),hp);
...
void Unit::checkCustomizedUnitParticleListTriggers(
  ...
  if (showUnitParticles == true) {
    ...
    // printf("Check Particle start line: %d size:
    // %d\n",__LINE__,(int)unitParticleSystemTypesList.size());

    if (unitParticleSystemTypesList.empty() == false) {
      // for(unsigned int index = 0; index < unitParticleSystemTypesList.size();
      // ++index) {
      for (UnitParticleSystemTypes::const_iterator iterParticleType =
        ...
        vector<ParticleSystemTypeInterface *>::iterator iterFind =
            std::find(systemTypesInUse.begin(), systemTypesInUse.end(), pst);

        // printf("Check Particle line: %d   isenabled: %d  already in use:
        // %d\n",__LINE__,pst->getMinmaxEnabled(),(iterFind ==
        // systemTypesInUse.end()));
        ...
        if (showParticle == true) {
          if (pst->getStartTime() == 0.0) {
            ...
            if (getFaction()->getTexture()) {
              ups->setFactionColor(
                  getFaction()->getTexture()->getPixmapConst()->getPixel3f(0,
                                                                           0));
            }
            unitParticleSystems.push_back(ups);
            Renderer::getInstance().manageParticleSystem(ups, rsGame);
          } else {
            // do nothing, timed particles are handled below in
            // queueTimedParticles()
          }
        }
      }
...
void Unit::setMeshPosInParticleSystem(UnitParticleSystem *ups) {
  if (ups->getMeshName() != "") {
    ...
    // as it can happen that anim progress is a bit out of range we correct it
    // to get something valid for the particle positions.
    ...
    model->updateInterpolationVertices(currentAnimProgress,
                                       isAlive() && !isAnimProgressBound());
    ...
    if (foundMesh == false) {
      ...
      string errorString = "Warning: Particle system is trying to find mesh '" +
                           meshName + "', but just found:\n'" + meshesFound +
                           "' in file:\n'" + model->getFileName() + "'\n";
  ...
  if (damageParticleSystems.empty() == false) {
    for (int i = (int)damageParticleSystems.size() - 1; i >= 0; --i) {
      ...
      if (force == true || (pst != NULL && pst->getMinmaxEnabled() == true)) {
        ...
        // printf("CHECKING to STOP customized particle trigger by HP [%d to %d
        // percentbased = %d] current hp = %d stopParticle =
        // %d\n",pst->getMinHp(),pst->getMaxHp(),pst->getMinmaxIsPercent(),hp,stopParticle);

        if (stopParticle == true) {
          // printf("STOPPING customized particle trigger by HP [%d to %d]
  ...
  if (showUnitParticles && type->damageParticleSystemTypes.empty() == false &&
    ...
    for (unsigned int i = 0; i < type->damageParticleSystemTypes.size(); ++i) {
      ...
      if (pst->getMinmaxEnabled() == true &&
        ...
        // printf("CHECKING to START customized particle trigger by HP [%d to %d
        // percentbased = %d] current hp = %d showParticle =
        // %d\n",pst->getMinHp(),pst->getMaxHp(),pst->getMinmaxIsPercent(),hp,showParticle);

        if (showParticle == true) {
          // printf("STARTING customized particle trigger by HP [%d to %d]
          // current hp = %d\n",pst->getMinHp(),pst->getMaxHp(),hp);
...
void Unit::setMeetingPos(const Vec2i &meetingPos) {
  ...
  if (map->isInside(this->meetingPos) == false ||
      map->isInsideSurface(map->toSurfCoords(this->meetingPos)) == false) {
...
void Unit::exploreCells(bool forceRefresh) {
  if (this->isOperative() == true) {
    ...
    if (!forceRefresh && cacheExploredCellsKey.first == newPos &&
      ...
      cacheExploredCells =
          game->getWorld()->exploreCells(newPos, sightRange, teamIndex, this);
...
void Unit::logSynchDataCommon(string file, int line, string source,
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled ==
    ...
    snprintf(
        ...
        "= %s, targetVec = %s, meetingPos = %s, progress [" MG_I64_SPECIFIER
        "], progress2 [%d] random [%d]\nUnit Path [%s]\n",
        getFrameCount(), id, getFullName(false).c_str(),
        faction->getType()->getName(false).c_str(),
        // getDesc().c_str(),
        pos.getString().c_str(), lastPos.getString().c_str(),
        targetPos.getString().c_str(), targetVec.getString().c_str(),
        meetingPos.getString().c_str(),
        //				lastRotation,
        //				targetRotation,
        //				rotation,
        progress, progress2, random.getLastNumber(),
        (unitPath != NULL ? unitPath->toString().c_str() : "NULL"));

    if (lastSynchDataString != string(szBuf) || lastFile != file ||
        lastLine != line || lastSource != source) {
...
void Unit::cleanupOldBadHarvestPos() {
  ...
  // printf("========================> cleanupOldBadHarvestPos() [%d]
  // badHarvestPosList.size [%ld] cleanupInterval [%d] getFrameCount() [%d]
  // needToCleanup
  // [%d]\n",getFrameCount(),badHarvestPosList.size(),cleanupInterval,getFrameCount(),needToCleanup);

  if (needToCleanup == true) {
    // printf("========================> cleanupOldBadHarvestPos() [%d]
    // badHarvestPosList.size
    // [%ld]\n",getFrameCount(),badHarvestPosList.size());

    std::vector<Vec2i> purgeList;
    for (std::map<Vec2i, int>::iterator iter = badHarvestPosList.begin();
         iter != badHarvestPosList.end(); ++iter) {
      if (getFrameCount() - iter->second >= cleanupInterval) {
        // printf("cleanupOldBadHarvestPos()
  ...
  if (pos == NULL) {
    ...
    if (resourceLocation != lastHarvestResourceTarget.first) {
      ...
      if (lastHarvestResourceTarget.second - getFrameCount() >= addInterval) {
        // printf("-----------------------> setLastHarvestResourceTarget()
...
// void Unit::addCurrentTargetPathTakenCell(const Vec2i &target,const Vec2i
// &cell) { 	if(currentTargetPathTaken.first != target) {
...
bool Unit::isLastPathfindFailedFrameWithinCurrentFrameTolerance() const {
  ...
  if (enablePathfinderEnlargeMaxNodes) {
    ...
    result =
        (getFrameCount() - lastPathfindFailedFrame >= MIN_FRAME_ELAPSED_RETRY);
  }
  return result;
}
...
std::string Unit::toString(bool crcMode) const {
  ...
  if (this->type != NULL) {
    result += " name [" + this->type->getName(false) + "][" +
  ...
  // WARNING!!! Don't access the Unit pointer in this->targetRef in this method
  // or it causes a stack overflow
  if (this->targetRef.getUnitId() >= 0) {
    // result += " targetRef = " + this->targetRef.getUnit()->toString();
    result += " targetRef = " + intToStr(this->targetRef.getUnitId()) +
              " - factionIndex = " +
  ...
  //    int obsIdx = 0;
  //    for(Observers::const_iterator iterList = observers.begin(); iterList !=
  //    observers.end(); ++iterList) { 	const UnitObserver *observer =
  //    *iterList; 	if(observer != NULL) {
  //    	}
  //
  //    	obsIdx++;
  //    }
  ...
  if (attackParticleSystems.empty() == false) {
    result += "attackParticleSystems count = " +
  ...
  for (unsigned int index = 0; index < attackParticleSystems.size(); ++index) {
    ...
    if (ps != NULL && Renderer::getInstance().validateParticleSystemStillExists(
                          ps, rsGame) == true) {
      result += "attackParticleSystems #" + intToStr(index) + " = " +
                ps->toString() + "\n";
    }
  }
...
void Unit::saveGame(XmlNode *rootNode) {
  ...
  unitNode->addAttribute("targetRotation", floatToStr(targetRotation, 6),
                         mapTagReplacements);
  //	float rotation;
  unitNode->addAttribute("rotation", floatToStr(rotation, 6),
                         mapTagReplacements);
  //	float targetRotationZ;
  unitNode->addAttribute("targetRotationZ", floatToStr(targetRotationZ, 6),
                         mapTagReplacements);
  //	float targetRotationX;
  unitNode->addAttribute("targetRotationX", floatToStr(targetRotationX, 6),
                         mapTagReplacements);
  //	float rotationZ;
  ...
  unitNode->addAttribute(
      "preMorph_type",
      (preMorph_type != NULL ? preMorph_type->getName(false) : ""),
      mapTagReplacements);
  ...
  if (currSkill != NULL) {
    unitNode->addAttribute("currSkillName", currSkill->getName(),
                           mapTagReplacements);
    unitNode->addAttribute("currSkillClass", intToStr(currSkill->getClass()),
                           mapTagReplacements);
  }
  ...
  //	bool showUnitParticles;
  unitNode->addAttribute("showUnitParticles", intToStr(showUnitParticles),
                         mapTagReplacements);
  //    Faction *faction;
  //	ParticleSystem *fire;
  int linkFireIndex = -1;
  ...
  if (damageParticleSystemsInUse.empty() == false) {
    ...
    for (std::map<int, UnitParticleSystem *>::const_iterator iterMap =
      ...
      if (iterMap->second != NULL &&
          Renderer::getInstance().validateParticleSystemStillExists(
              iterMap->second, rsGame) == true) {
        XmlNode *damageParticleSystemsInUseNode2 =
            damageParticleSystemsInUseNode->addChild(
                "damageParticleSystemsInUse");

        damageParticleSystemsInUseNode2->addAttribute(
            "key", intToStr(iterMap->first), mapTagReplacements);
  ...
  if (fireParticleSystems.empty() == false) {
    ...
    if (linkFireIndex >= 0) {
      fireParticleSystemsNode->addAttribute(
          "fireParticleLink", intToStr(linkFireIndex), mapTagReplacements);
    }
  ...
  unitNode->addAttribute("retryCurrCommandCount",
                         intToStr(retryCurrCommandCount), mapTagReplacements);
  //	Vec3f screenPos;
  ...
  for (std::map<Vec2i, int>::const_iterator iterMap = badHarvestPosList.begin();
    ...
    badHarvestPosListNode->addAttribute("key", iterMap->first.getString(),
                                        mapTagReplacements);
    badHarvestPosListNode->addAttribute("value", intToStr(iterMap->second),
                                        mapTagReplacements);
  }
  ...
  lastHarvestResourceTargetNode->addAttribute(
      "key", lastHarvestResourceTarget.first.getString(), mapTagReplacements);
  lastHarvestResourceTargetNode->addAttribute(
      "value", intToStr(lastHarvestResourceTarget.second), mapTagReplacements);

  //	//std::pair<Vec2i,std::vector<Vec2i> > currentTargetPathTaken;
  //	static Game *game;
  //
  //	bool ignoreCheckCommand;
  unitNode->addAttribute("ignoreCheckCommand", intToStr(ignoreCheckCommand),
                         mapTagReplacements);
  ...
  unitNode->addAttribute("lastPathfindFailedFrame",
                         intToStr(lastPathfindFailedFrame), mapTagReplacements);
  //	Vec2i lastPathfindFailedPos;
  unitNode->addAttribute("lastPathfindFailedPos",
                         lastPathfindFailedPos.getString(), mapTagReplacements);
  ...
  unitNode->addAttribute("changedActiveCommand", intToStr(changedActiveCommand),
                         mapTagReplacements);
  //	int lastAttackerUnitId;
  unitNode->addAttribute("lastAttackerUnitId", intToStr(lastAttackerUnitId),
                         mapTagReplacements);
  //	int lastAttackedUnitId;
  unitNode->addAttribute("lastAttackedUnitId", intToStr(lastAttackedUnitId),
                         mapTagReplacements);
...
Unit *Unit::loadGame(const XmlNode *rootNode, GameSettings *settings,
  ...
  CardinalDir newModelFacing = static_cast<CardinalDir>(
      unitNode->getAttribute("modelFacing")->getIntValue());

  //	Unit *result = new Unit(int id, UnitPathInterface *unitpath, const Vec2i
  //&pos, 			   const UnitType *type, Faction *faction, Map
  //*map, CardinalDir placeFacing) : BaseColorPickEntity(), id(id) {
  ...
  // Unit *result = new Unit(getNextUnitId(f), newpath, Vec2i(0), ut, f, &map,
  // CardinalDir::NORTH); Unit(int id, UnitPathInterface *path, const Vec2i
  // &pos, const UnitType *type, Faction *faction, Map *map, CardinalDir
  // placeFacing);
  ...
  //	Field currField;
  result->currField =
      static_cast<Field>(unitNode->getAttribute("currField")->getIntValue());
  //    Field targetField;
  result->targetField =
      static_cast<Field>(unitNode->getAttribute("targetField")->getIntValue());
  //	const Level *level;
  //	if(level != NULL) {
  //		level->saveGame(unitNode);
  //	}
  ...
  //    const SkillType *currSkill;
  //	if(currSkill != NULL) {
  //		unitNode->addAttribute("currSkill",currSkill->getName(),
  // mapTagReplacements);
  //	}
  ...
  result->lastModelIndexForCurrSkillType =
      unitNode->getAttribute("lastModelIndexForCurrSkillType")->getIntValue();
  //    int animationRandomCycleCount;
  ...
  //	std::map<int, UnitParticleSystem *> damageParticleSystemsInUse;
  //	for(std::map<int, UnitParticleSystem *>::const_iterator iterMap =
  // damageParticleSystemsInUse.begin(); 			iterMap !=
  // damageParticleSystemsInUse.end(); ++iterMap) { 		XmlNode
  //*damageParticleSystemsInUseNode =
  // unitNode->addChild("damageParticleSystemsInUse");
  //
  //		damageParticleSystemsInUseNode->addAttribute("key",intToStr(iterMap->first),
  // mapTagReplacements);
  //		iterMap->second->saveGame(damageParticleSystemsInUseNode);
  //	}
  //	if(unitNode->hasChild("damageParticleSystemsInUse") == true) {
  //		XmlNode *damageParticleSystemsInUseNode =
  // unitNode->getChild("damageParticleSystemsInUse"); 		vector<XmlNode
  // *> damageParticleSystemsInUseNode2 =
  // damageParticleSystemsInUseNode->getChildList("damageParticleSystemsInUse");
  //		for(unsigned int i = 0; i <
  // damageParticleSystemsInUseNode2.size(); ++i) { 			XmlNode
  // *d2Node = damageParticleSystemsInUseNode2[i];
  //
  //			vector<XmlNode *> unitParticleSystemNodeList =
  // damageParticleSystemsInUseNode->getChildList("UnitParticleSystem");
  //			for(unsigned int i = 0; i <
  // unitParticleSystemNodeList.size(); ++i) { 				XmlNode
  // *node = unitParticleSystemNodeList[i];
  //
  //				UnitParticleSystem *ups = new
  // UnitParticleSystem(); 				ups->loadGame(node);
  //				result->unitParticleSystems.push_back(ups);
  //
  //				//Renderer::getInstance().manageParticleSystem(result->fire,
  // rsGame);
  //				Renderer::getInstance().addToDeferredParticleSystemList(make_pair(ups,
  // rsGame));
  //			}
  //
  //	}
  ...
  if (unitNode->hasChild("fireParticleSystems") == true) {
    ...
    if (fireParticleSystemsNode->hasAttribute("fireParticleLink") == true) {
      linkFireIndex = fireParticleSystemsNode->getAttribute("fireParticleLink")
                          ->getIntValue();
    }
    ...
    for (int i = 0; i < (int)unitParticleSystemNodeList.size(); ++i) {
      ...
      if (result->fire == NULL || linkFireIndex != i) {
        ...
        // printf("Load fire particle i = %d linkFireIndex = %d result->fire =
  ...
  //	std::string lastSynchDataString;
  //	unitNode->addAttribute("lastSynchDataString",lastSynchDataString,
  // mapTagReplacements); 	std::string lastFile;
  //	unitNode->addAttribute("lastFile",lastFile, mapTagReplacements);
  //	int lastLine;
  //	unitNode->addAttribute("lastLine",intToStr(lastLine),
  // mapTagReplacements); 	std::string lastSource;
  //	unitNode->addAttribute("lastSource",lastSource, mapTagReplacements);
  //	int lastRenderFrame;
  ...
  // badHarvestPosList.begin(); 			iterMap !=
  // badHarvestPosList.end(); ++iterMap) { 		XmlNode
  // *badHarvestPosListNode = unitNode->addChild("badHarvestPosList");
  //
  //		badHarvestPosListNode->addAttribute("key",iterMap->first.getString(),
  // mapTagReplacements);
  //		badHarvestPosListNode->addAttribute("value",intToStr(iterMap->second),
  ...
  //	lastHarvestResourceTargetNode->addAttribute("key",lastHarvestResourceTarget.first.getString(),
  // mapTagReplacements);
  //	lastHarvestResourceTargetNode->addAttribute("value",intToStr(lastHarvestResourceTarget.second),
  // mapTagReplacements);

  result->lastHarvestResourceTarget = make_pair(
      Vec2i::strToVec2(
          lastHarvestResourceTargetNode->getAttribute("key")->getValue()),
  ...
  //	Vec2i lastStuckPos;
  result->lastStuckPos =
      Vec2i::strToVec2(unitNode->getAttribute("lastStuckPos")->getValue());
  //	uint32 lastPathfindFailedFrame;
  result->lastPathfindFailedFrame =
      unitNode->getAttribute("lastPathfindFailedFrame")->getIntValue();
  //	Vec2i lastPathfindFailedPos;
  result->lastPathfindFailedPos = Vec2i::strToVec2(
      unitNode->getAttribute("lastPathfindFailedPos")->getValue());
  //	bool usePathfinderExtendedMaxNodes;
  result->usePathfinderExtendedMaxNodes =
      unitNode->getAttribute("usePathfinderExtendedMaxNodes")->getIntValue() !=
      0;
  //	int maxQueuedCommandDisplayCount;
  result->maxQueuedCommandDisplayCount =
      unitNode->getAttribute("maxQueuedCommandDisplayCount")->getIntValue();
  //	UnitAttackBoostEffectOriginator currentAttackBoostOriginatorEffect;

  // !!! TODO: Softcoder - in progress work to load attack boosts, not working
  // properly yet
  ...
  // printf("Unit [%d - %s] has currentAttackBoostEffects count:
  ...
  //	CauseOfDeathType causeOfDeath;
  result->causeOfDeath = static_cast<CauseOfDeathType>(
  ...
  if (unitNode->hasAttribute("currentPathFinderDesiredFinalPos")) {
    result->currentPathFinderDesiredFinalPos = Vec2i::strToVec2(
        unitNode->getAttribute("currentPathFinderDesiredFinalPos")->getValue());
  }

  if (unitNode->hasAttribute("random")) {
    result->random.setLastNumber(
        unitNode->getAttribute("random")->getIntValue());
  }
...
Checksum Unit::getCRC() {
  ...
  // printf("#9 Unit: %d CRC: %u lastModelIndexForCurrSkillType:
  // %d\n",id,crcForUnit.getSum(),lastModelIndexForCurrSkillType); printf("#9a
  // Unit: %d CRC: %u\n",id,crcForUnit.getSum());
  ...
  // printf("#11 Unit: %d CRC: %u observers.size():
  ...
  if (consoleDebug)
    printf(
        "#11 Unit: %d CRC: %u "
        "damageParticleSystems.size(): " MG_SIZE_T_SPECIFIER "\n",
  ...
  // UnitAttackBoostEffectOriginator currentAttackBoostOriginatorEffect;
  crcForUnit.addInt(
      (int)currentAttackBoostOriginatorEffect.currentAttackBoostUnits.size());

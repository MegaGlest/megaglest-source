...
void UnitPathBasic::dumpMemoryList() {
    ...
    for (std::map<UnitPathBasic *, bool>::iterator iterMap = UnitPathBasic::mapMemoryList.begin(); iterMap != UnitPathBasic::mapMemoryList.end(); ++iterMap) {
...
void UnitPathBasic::add(const Vec2i &path) {
    if (this->map != NULL) {
        if (this->map->isInside(path) == false) {
            throw megaglest_runtime_error("Invalid map path position = " + path.getString() + " map w x h = " + intToStr(map->getW()) + " " +
                                          intToStr(map->getH()));
        } else if (this->map->isInsideSurface(this->map->toSurfCoords(path)) == false) {
            throw megaglest_runtime_error("Invalid map surface path position = " + path.getString() + " map surface w x h = " + intToStr(map->getSurfaceW()) +
                                          " " + intToStr(map->getSurfaceH()));
        }
    }

    if (Thread::isCurrentThreadMainThread() == false) {
        throw megaglest_runtime_error("Invalid access to UnitPathBasic add from outside main thread current "
                                      "id = " +
...
Vec2i UnitPathBasic::pop(bool removeFrontPos) {
    ...
    if (removeFrontPos == true) {
        if (Thread::isCurrentThreadMainThread() == false) {
            throw megaglest_runtime_error("Invalid access to UnitPathBasic delete from outside main thread "
                                          "current id = " +
                                          intToStr(Thread::getCurrentThreadId()) + " main = " + intToStr(Thread::getMainThreadId()));
        }
...
void UnitAttackBoostEffect::loadGame(const XmlNode *rootNode, Unit *unit, World *world, bool applyToOriginator) {
    ...
    if (boost != NULL) {
        ...
        if (applyToOriginator == true) {
            applyLoadedAttackBoostParticles(boost->unitParticleSystemTypeForSourceUnit, unitAttackBoostEffectNode, unit);
        } else {
            applyLoadedAttackBoostParticles(boost->unitParticleSystemTypeForAffectedUnit, unitAttackBoostEffectNode, unit);
        }
    } else {
...
Unit::Unit(int id, UnitPathInterface *unitpath, const Vec2i &pos, const UnitType *type, Faction *faction, Map *map, CardinalDir placeFacing)
...
ParticleSystem *Unit::getFire() const {
    if (this->fire != NULL && Renderer::getInstance().validateParticleSystemStillExists(this->fire, rsGame) == false) {
        return NULL;
    }
    return this->fire;
}
...
void Unit::dumpMemoryList() {
    ...
    for (std::map<Unit *, bool>::iterator iterMap = Unit::mapMemoryList.begin(); iterMap != Unit::mapMemoryList.end(); ++iterMap) {
...
Vec2f Unit::getFloatCenteredPos() const {
    ...
    return Vec2f(truncateDecimal<float>(pos.x - 0.5f + type->getSize() / 2.f, 6), truncateDecimal<float>(pos.y - 0.5f + type->getSize() / 2.f, 6));
}
...
Vec2i Unit::getCellPos() const {
    ...
    if (type->hasCellMap()) {
        if (type->hasEmptyCellMap() == false || type->getAllowEmptyCellMap() == true) {
...
void Unit::setCurrSkill(const SkillType *currSkill) {
    ...
    if (this->currSkill->getClass() == scMove && currSkill->getClass() != scMove) {
        faction->removeUnitFromMovingList(this->getId());
    } else if (this->currSkill->getClass() != scMove && currSkill->getClass() == scMove) {
        faction->addUnitToMovingList(this->getId());
    }

    changedActiveCommand = false;
    if (currSkill->getClass() != this->currSkill->getClass() || currSkill->getName() != this->currSkill->getName()) {
        this->animProgress = 0;
        this->lastAnimProgress = 0;
        ...
        if (cmd != NULL && cmd->getCommandType() != NULL && cmd->getCommandType()->hasFogOfWarSkillType(currSkill->getName())) {
    ...
    if (showUnitParticles == true && currSkill->unitParticleSystemTypes.empty() == false && unitParticleSystems.empty() == true) {
...
RandomGen *Unit::getRandom(bool threadAccessAllowed) {
    if (threadAccessAllowed == false && Thread::isCurrentThreadMainThread() == false) {
        throw megaglest_runtime_error("Invalid access to unit random from outside main thread current id = " + intToStr(Thread::getCurrentThreadId()) +
...
void Unit::setVisible(const bool visible) {
    ...
    if (currentAttackBoostOriginatorEffect.currentAppliedEffect != NULL) {
        if (currentAttackBoostOriginatorEffect.currentAppliedEffect->ups != NULL) {
            bool particleValid =
                Renderer::getInstance().validateParticleSystemStillExists(currentAttackBoostOriginatorEffect.currentAppliedEffect->ups, rsGame);
            if (particleValid == true) {
...
std::pair<CommandResult, string> Unit::giveCommand(Command *command, bool tryQueue) {
    ...
    if (command->getCommandType()->isQueuable(tryQueue)) {
        ...
        if (commands.empty() == false && commands.back()->getCommandType()->isQueueAppendable() == false) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
        ...
        if (willChangedActiveCommand == true) {
            ...
            } else if (currCommandClass == ccAttack || currCommandClass == ccAttackStopped || commandClass == ccAttack || commandClass == ccAttackStopped) {
    ...
    try {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] about to undertake unit id = %d [%s] [%s]\n",
                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, this->id, this->getFullName(false).c_str(),
    ...
    if (targetUnit != NULL) {
        ...
        // repair allies
            ...
            if (commandType == NULL && targetUnit != NULL) {
                ...
                if (targetUnit->getFactionIndex() == this->getFactionIndex() && targetUnit->isOperative() == true && this->getLoadType() != NULL &&
                    this->getLoadCount() != 0 && targetUnit->getType() != NULL && targetUnit->getType()->getStore(this->getLoadType()) > 0) {
    ...
    if (targetUnit == NULL && commandType == NULL) {
        ...
        if (cell != NULL && cell->getUnit(this->getCurrField()) != NULL) {
            ...
            if (targetUnit != NULL) {
                ...
                // moving
                if (targetUnit->getFactionIndex() == this->getFactionIndex() && (targetUnit->isBuilt() == false || targetUnit->isDamaged() == true)) {
...
int64 Unit::getUpdateProgress() {
    ...
    if (currSkill->getClass() != scDie) {
        ...
        newProgress = getUpdatedProgress(progress, GameConstants::updateFps, speed, diagonalFactor, heightFactor);
    }
    return newProgress;
}
...
int64 Unit::getHeightFactor(int64 speedMultiplier) {
    ...
    if (currSkill->getClass() == scMove) {
        ...
        int64 heightDiff =
            ((truncateDecimal<float>(unitCell->getHeight(), 2) * speedMultiplier) - (truncateDecimal<float>(targetCell->getHeight(), 2) * speedMultiplier));
...
int64 Unit::getUpdatedProgress(int64 currentProgress, int64 updateFPS, int64 speed, int64 diagonalFactor, int64 heightFactor) {
    ...
    if (isNetworkCRCEnabled() == true) {
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8095,
                 "currentProgress = " MG_I64_SPECIFIER " updateFPS = " MG_I64_SPECIFIER " speed = " MG_I64_SPECIFIER " diagonalFactor = " MG_I64_SPECIFIER
                 " heightFactor = " MG_I64_SPECIFIER " speedDenominator = " MG_I64_SPECIFIER " progressIncrease = " MG_I64_SPECIFIER " [" MG_I64_SPECIFIER
                 "] height [%f] airHeight [%f] cellUnitHeight [%d] "
                 "cellObjectHeight [%d] skill [%s] pos [%s] lastpos [%s]",
                 currentProgress, updateFPS, speed, diagonalFactor, heightFactor, speedDenominator, progressIncrease,
                 ((speed * diagonalFactor * heightFactor) / speedDenominator), height, airHeight, cellUnitHeight, cellObjectHeight,
                 (currSkill != NULL ? currSkill->getName().c_str() : "none"), pos.getString().c_str(), lastPos.getString().c_str());
...
void Unit::updateAttackBoostProgress(const Game *game) {
    ...
    if (currSkill != currentAttackBoostOriginatorEffect.skillType) {
        if (debugBoost)
            printf("Line: %d new [%s]\n", __LINE__,
                   (currentAttackBoostOriginatorEffect.skillType != NULL ? currentAttackBoostOriginatorEffect.skillType->getBoostDesc(false).c_str() : ""));
                ...
                if (currentAttackBoostOriginatorEffect.currentAttackBoostUnits.empty() == false) {
                    if (attackBoost->unitParticleSystemTypeForSourceUnit != NULL) {
                        ...
                        *currentAttackBoostOriginatorEffect.currentAppliedEffect->upst = *attackBoost->unitParticleSystemTypeForSourceUnit;
                        ...
                        currentAttackBoostOriginatorEffect.currentAppliedEffect->ups->setParticleType(
        ...
        if (currSkill->isAttackBoostEnabled() == true) {
                ...
                for (int i = (int)currentAttackBoostOriginatorEffect.currentAttackBoostUnits.size() - 1; i >= 0; --i) {
                    ...
                    if (iterFound == candidateValidIdList.end()) {
                        ...
                        if (affectedUnit != NULL) {
                            ...
                            if (debugBoost)
                                printf("Removed attack boost from Unit [%d - %s] since they are "
                                       "NO LONGER in range\n",
                                       affectedUnit->id, affectedUnit->getType()->getName(false).c_str());
                        }
                        currentAttackBoostOriginatorEffect.currentAttackBoostUnits.erase(currentAttackBoostOriginatorEffect.currentAttackBoostUnits.begin() +
                                                                                         i);
                    }
                }
            }
            ...
            if (showUnitParticles == true) {
                if (currentAttackBoostOriginatorEffect.currentAttackBoostUnits.empty() == false) {
                    if (attackBoost->unitParticleSystemTypeForSourceUnit != NULL && currentAttackBoostOriginatorEffect.currentAppliedEffect == NULL) {
                        currentAttackBoostOriginatorEffect.currentAppliedEffect = new UnitAttackBoostEffect();
                        currentAttackBoostOriginatorEffect.currentAppliedEffect->upst = new UnitParticleSystemType();
                        *currentAttackBoostOriginatorEffect.currentAppliedEffect->upst = *attackBoost->unitParticleSystemTypeForSourceUnit;
                        // effect.upst = boost->unitParticleSystemTypeForAffectedUnit;
    ...
    if (debugBoost) {
        if (currSkill->isAttackBoostEnabled() == true) {
            ...
            if (currentAttackBoostOriginatorEffect.currentAttackBoostUnits.empty() == false) {
                printf("Found affected units currentAttackBoostOriginatorEffect.skillType "
                       "[%p]\n",
...
bool Unit::update() {
    ...
    progress = getUpdatedProgress(progress, GameConstants::updateFps, speed, diagonalFactor, heightFactor);
    ...
    if (isAnimProgressBound() == true) {
        ...
        int64 speedDenominator = speedDivider * game->getWorld()->getUpdateFps(this->getFactionIndex());
        ...
        if (getProgressAsFloat() < 1.f / rotFactor) {
            if (type->getFirstStOfClass(scMove)) {
                if (abs((int)(lastRotation - targetRotation)) < 180)
                    rotation = lastRotation + (targetRotation - lastRotation) * getProgressAsFloat() * rotFactor;
                else {
                    float rotationTerm = targetRotation > lastRotation ? -360.f : +360.f;
                    rotation = lastRotation + (targetRotation - lastRotation + rotationTerm) * getProgressAsFloat() * rotFactor;
                }
            }
        }
    }
...
bool Unit::unitHasAttackBoost(const AttackBoost *boost, const Unit *source) {
    ...
    for (unsigned int i = 0; i < currentAttackBoostEffects.size(); ++i) {
        UnitAttackBoostEffect *effect = currentAttackBoostEffects[i];
        if (effect != NULL && effect->boost->name == boost->name && effect->getSource()->getType()->getId() == source->getType()->getId()) {
            result = true;
            break;
        }
    }
...
bool Unit::applyAttackBoost(const AttackBoost *boost, const Unit *source) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("APPLIED ATTACK BOOST START to unit [%s - %d] from unit [%s - %d] hp: "
               "%d\n",
               this->getType()->getName(false).c_str(), this->getId(), source->getType()->getName(false).c_str(), source->getId(), hp);
...
void Unit::deapplyAttackBoost(const AttackBoost *boost, const Unit *source) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("DE-APPLIED ATTACK BOOST START to unit [%s - %d] from unit [%s - %d] "
               "hp: %d\n",
               this->getType()->getName(false).c_str(), this->getId(), source->getType()->getName(false).c_str(), source->getId(), hp);
...
std::pair<CommandResult, string> Unit::checkCommand(Command *command) const {
    ...
    if (isOperative() == false || command->getUnit() == this || getType()->hasCommandType(command->getCommandType()) == false ||
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugLUA,
                                     ...
                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, isOperative(), command->getUnit(),
                                     getType()->hasCommandType(command->getCommandType()), this->getFaction()->reqsOk(command->getCommandType()));

        // Allow self healing if able to heal own unit type
        if (command->getUnit() == this && command->getCommandType()->getClass() == ccRepair &&
    ...
    if (produced != NULL) {
        ...
        if (ignoreCheckCommand == false && faction->checkCosts(produced, command->getCommandType()) == false) {
            // printf("To produce this unit you
...
CommandResult Unit::undoCommand(Command *command) {
    ...
    if (getCurrCommand() == command && command->getCommandType()->getClass() == ccMorph && this->currSkill->getClass() == scMorph) {
...
void Unit::checkCustomizedUnitParticleListTriggers(const UnitParticleSystemTypes &unitParticleSystemTypesList, bool applySkillChangeParticles) {
    if (showUnitParticles == true) {
            ...
            for (int index = (int)unitParticleSystems.size() - 1; index >= 0; index--) {
                ...
                if (ps != NULL) {
                    if (Renderer::getInstance().validateParticleSystemStillExists(ps, rsGame) == true) {
                        ...
                        if ((ps->getParticleType() != NULL && ps->getParticleType()->getMinmaxEnabled())) {
...
void Unit::checkCustomizedParticleTriggers(bool force) {
    ...
    if (damageParticleSystems.empty() == false) {
        for (int i = (int)damageParticleSystems.size() - 1; i >= 0; --i) {
            ...
            int foundParticleIndexType = -2;
            if (Renderer::getInstance().validateParticleSystemStillExists(ps, rsGame) == true) {
                for (std::map<int, UnitParticleSystem *>::iterator iterMap = damageParticleSystemsInUse.begin(); iterMap != damageParticleSystemsInUse.end();
                     ++iterMap) {
                    if (iterMap->second == ps) {
                        foundParticleIndexType = iterMap->first;
                        if (foundParticleIndexType >= 0) {
    ...
    if (showUnitParticles && type->damageParticleSystemTypes.empty() == false && force == false && alive == true) {
...
void Unit::exploreCells(bool forceRefresh) {
    if (this->isOperative() == true) {
        ...
        if (!forceRefresh && cacheExploredCellsKey.first == newPos && cacheExploredCellsKey.second == sightRange) {
...
void Unit::logSynchDataCommon(string file, int line, string source, bool threadedMode) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true) {
        ...
        snprintf(szBuf, 8096,
                 ...
                 "= %s, targetVec = %s, meetingPos = %s, progress [" MG_I64_SPECIFIER "], progress2 [%d] random [%d]\nUnit Path [%s]\n",
                 getFrameCount(), id, getFullName(false).c_str(), faction->getType()->getName(false).c_str(),
                 // getDesc().c_str(),
                 pos.getString().c_str(), lastPos.getString().c_str(), targetPos.getString().c_str(), targetVec.getString().c_str(),
                 meetingPos.getString().c_str(),
                 //				lastRotation,
                 //				targetRotation,
                 //				rotation,
                 progress, progress2, random.getLastNumber(), (unitPath != NULL ? unitPath->toString().c_str() : "NULL"));

        if (lastSynchDataString != string(szBuf) || lastFile != file || lastLine != line || lastSource != source) {
...
void Unit::saveGame(XmlNode *rootNode) {
    ...
    if (damageParticleSystemsInUse.empty() == false) {
        ...
        for (std::map<int, UnitParticleSystem *>::const_iterator iterMap = damageParticleSystemsInUse.begin(); iterMap != damageParticleSystemsInUse.end();
             ++iterMap) {
    ...
    for (std::map<Vec2i, int>::const_iterator iterMap = badHarvestPosList.begin(); iterMap != badHarvestPosList.end(); ++iterMap) {
...
Unit *Unit::loadGame(const XmlNode *rootNode, GameSettings *settings, Faction *faction, World *world) {
    ...
    result->lastHarvestResourceTarget = make_pair(Vec2i::strToVec2(lastHarvestResourceTargetNode->getAttribute("key")->getValue()),

...
Object::Object(ObjectType *objectType, const Vec3f &pos, const Vec2i &mapPos)
  ...
  animated = false;
  this->mapPos = mapPos;
  this->pos = pos + Vec3f(random.randRange(-0.6f, 0.6f), 0.0f,
...
Object::~Object() {
  ...
  while (unitParticleSystems.empty() == false) {
    bool particleValid = renderer.validateParticleSystemStillExists(
        unitParticleSystems.back(), rsGame);
...
void Object::initParticles() {
  ...
  if (this->objectType->getTilesetModelType(variation)->hasParticles()) {
    ModelParticleSystemTypes *particleTypes =
        this->objectType->getTilesetModelType(variation)->getParticleTypes();
    initParticlesFromTypes(particleTypes);
  }
}

void Object::initParticlesFromTypes(
    const ModelParticleSystemTypes *particleTypes) {
  ...
  if (showTilesetParticles == true &&
    ...
    for (ObjectParticleSystemTypes::const_iterator it = particleTypes->begin();
         it != particleTypes->end(); ++it) {
...
void Object::end(ParticleSystem *particleSystem) {
  vector<UnitParticleSystem *>::iterator iterFind = find(
      unitParticleSystems.begin(), unitParticleSystems.end(), particleSystem);
...
void Object::updateHighlight() {
  ...
  if (highlight > 0.f) {
    ...
    // highlight -= 1.f / (Game::highlightTime *
    // game->getWorld()->getUpdateFps(-1));
...
void Object::update() {
  ...
  // {
  if (animated == true) {
    //		printf("#1 Object updating [%s] Speed [%d] animProgress
    ...
    if (objectType != NULL &&
      ...
      float f = 1.0f;
      if (objectType->getTilesetModelType(variation)->getSmoothTwoFrameAnim() ==
          true) {
        f = abs(std::sin(animProgress * 2 * 3.16)) + 0.4f;
      }

      float newAnimProgress =
          animProgress + f * (((float)objectType->getTilesetModelType(variation)
                                   ->getAnimSpeed() *
                               heightFactor) /
                              speedDenominator);
...
void Object::resetHighlight() { highlight = 1.f; }
...
void Object::saveGame(XmlNode *rootNode) {
  ...
  if (objectType != NULL) {
    objectNode->addAttribute("objectType", intToStr(objectType->getClass()),
                             mapTagReplacements);
  }

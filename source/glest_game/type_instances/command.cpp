...
Command::Command(const CommandType *ct, const Vec2i &pos) : unitRef() {
  // SystemFlags::OutputDebug(SystemFlags::debugSystem,"In [%s::%s Line: %d] ct
...
void Command::setPos(const Vec2i &pos) { this->pos = pos; }

// void Command::setOriginalPos(const Vec2i &pos) {
//      this->originalPos= pos;
// }

void Command::setPosToOriginalPos() { this->pos = this->originalPos; }

void Command::setUnit(Unit *unit) { this->unitRef = unit; }
...
std::string Command::toString(bool translatedValue) const {
  ...
  std::string result = "";
  if (commandType != NULL) {
    result = "commandType id = " + intToStr(commandType->getId()) +
  ...
  result += ", pos = " + pos.getString() +
            ", originalPos = " + originalPos.getString() +
            ", facing = " + intToStr(facing.asInt());
  ...
  result += ", stateType = " + intToStr(stateType) +
            ", stateValue = " + intToStr(stateValue);
...
void Command::saveGame(XmlNode *rootNode, Faction *faction) {
  ...
  if (commandType != NULL) {
    commandNode->addAttribute("commandType", intToStr(commandType->getId()),
                              mapTagReplacements);
  }
  //    Vec2i originalPos;
  ...
  //	UnitReference unitRef;		//target unit, used to move and attack
  // optionally
  ...
  if (unitType != NULL) {
    commandNode->addAttribute("unitTypeId", intToStr(unitType->getId()),
                              mapTagReplacements);
  ...
  commandNode->addAttribute("unitCommandGroupId", intToStr(unitCommandGroupId),
                            mapTagReplacements);
}
...
Command *Command::loadGame(const XmlNode *rootNode, const UnitType *ut,
  ...
  //    Vec2i originalPos;
  result->originalPos =
      Vec2i::strToVec2(commandNode->getAttribute("originalPos")->getValue());
  //    Vec2i pos;
  ...
  //	CardinalDir facing;			// facing, for build command
  result->facing = static_cast<CardinalDir>(
  ...
  //	CommandStateType stateType;
  result->stateType = static_cast<CommandStateType>(

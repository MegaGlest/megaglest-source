  ...
  for (unsigned int i = 0; i < units.size(); ++i) {
    ...
    if (this->findUnit(unitId) == NULL) {
      ...
      for (unsigned int j = 0; j < units.size(); ++j) {
        printf("%u / %d id = %d [%s]\n", j, (int)units.size(),
               units[j]->getId(), units[j]->getType()->getName(false).c_str());
      }
    }
...
void FactionThread::execute() {
  ...
  try {
      ...
      if (executeTask == true) {
        ...
        for (int j = 0; j < unitCount; ++j) {
          ...
          if (minorDebugPerformance && (chrono.getMillis() - elapsed1) >= 1)
            printf("Faction [%d - %s] #1-unit threaded updates on frame: %d "
                   "for [%d] unit # %d, unitCount = %d, took [%lld] msecs\n",
          ...
          if (update == true) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
              ...
              int64 speed =
                  unit->getCurrSkill()->getTotalSpeed(unit->getTotalUpgrade());
              ...
              snprintf(
                  ...
                  (long long int)speed, changedActiveCommand, (long long int)df,
                  (long long int)hf);
            ...
            world->getUnitUpdater()->updateUnitCommand(
                unit, currentTriggeredFrameIndex);

            codeLocation = "15";
            if (minorDebugPerformance && (chrono.getMillis() - elapsed2) >= 1)
              printf("Faction [%d - %s] #2-unit threaded updates on frame: %d "
                     "for [%d] unit # %d, unitCount = %d, took [%lld] msecs\n",
                     faction->getStartLocationIndex(),
                     faction->getType()->getName(false).c_str(),
                     currentTriggeredFrameIndex,
        ...
        if (minorDebugPerformance && chrono.getMillis() >= 1)
          printf("Faction [%d - %s] threaded updates on frame: %d for [%d] "
                 "units took [%lld] msecs\n",
                 faction->getStartLocationIndex(),
                 faction->getType()->getName(false).c_str(),
                 currentTriggeredFrameIndex,
    ...
    snprintf(szBuf, 8096, "In [%s::%s %d] UNKNOWN error Loc [%s]\n", __FILE__,
             __FUNCTION__, __LINE__, codeLocation.c_str());
...
int Faction::getAIBehaviorStaticOverideValue(
    AIBehaviorStaticValueCategory type) const {
...
bool Faction::canUnitsPathfind() {
  bool result = true;
  if (control == ctCpuEasy || control == ctCpu || control == ctCpuUltra ||
      control == ctCpuMega) {
...
void Faction::init(FactionType *factionType, ControlType control,
                   ...
                   bool giveResources, const XmlNode *loadWorldNode) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n",
                             __FILE__, __FUNCTION__, __LINE__);
  ...
  if (texture) {
    string playerTexture = getGameCustomCoreDataPath(
        data_path, "data/core/faction_textures/faction" +
...
bool Faction::getCpuControl(bool enableServerControlledAI, bool isNetworkGame,
    ...
    if (isNetworkGame == true) {
      if (role == nrServer) {
        ...
        result = (control == ctNetworkCpuEasy || control == ctNetworkCpu ||
                  control == ctNetworkCpuUltra || control == ctNetworkCpuMega);
      }
    }
  }
    ...
    for (int i = 0; i < p->getCostCount(); ++i) {
      ...
      if (rt == NULL) {
        throw megaglest_runtime_error(
            string(__FUNCTION__) + " rt == NULL for ProducibleType [" +
            p->getName(false) + "] index: " + intToStr(i));
      }
...
bool Faction::applyCostsOnInterval(const ResourceType *rtApply) {
  ...
  std::map<const ResourceType *, std::pair<int, std::vector<Unit *>>>
      resourceIntervalUsage;
        ...
        if (resource->getType() == rtApply &&
          ...
          if (resourceIntervalUsage.find(resource->getType()) ==
            ...
            resourceIntervalUsage[resource->getType()] =
                make_pair<int, std::vector<Unit *>>(0, std::vector<Unit *>());
          }
  ...
  if (resourceIntervalUsage.empty() == false) {
    for (std::map<const ResourceType *,
                  std::pair<int, std::vector<Unit *>>>::iterator iter =
      ...
      if (rt->getClass() == rcConsumable) {
        ...
        if (r->getBalance() * 5 + r->getAmount() < 0 && r->getAmount() >= 0) {
          ...
          bool sharedTeamResources = world->getGame()->isFlagType1BitEnabled(
              ft1_allow_shared_team_resources);
          bool sharedTeamUnits = world->getGame()->isFlagType1BitEnabled(
              ft1_allow_shared_team_units);
          bool isTeam = (this->getTeam() == world->getThisTeamIndex());

          if (this->getIndex() == world->getThisFactionIndex() ||
              (isTeam && (sharedTeamResources || sharedTeamUnits))) {
      ...
      if (getResource(rt)->getAmount() < 0) {
        ...
        for (int i = 0; i < (int)resourceConsumers.size(); ++i) {
          ...
          if (scriptManager->getPlayerModifiers(this->index)
            ...
            bool decHpResult = unit->decHp(
                unit->getType()->getTotalMaxHp(unit->getTotalUpgrade()) / 3);
            if (decHpResult) {
              unit->setCauseOfDeath(ucodStarvedResource);
              world->getStats()->die(
                  unit->getFactionIndex(),
                  unit->getType()->getCountUnitDeathInStats());
              scriptManager->onUnitDied(unit);
            }
            StaticSound *sound = static_cast<const DieSkillType *>(
                                     unit->getType()->getFirstStOfClass(scDie))
                                     ->getSound();
            if (sound != NULL && (thisFaction == true ||
                                  world->showWorldForPlayer(
                                      world->getThisTeamIndex()) == true)) {
...
int Faction::getAmountOfProducable(const ProducibleType *pt,
                                   const CommandType *ct) {
  assert(pt != NULL);
      ...
      if (r->getType() == rt) {
        ...
        if (r->getType()->getClass() != rcStatic &&
          ...
          r->setAmount(getStoreAmount(rt, false) -
                       (getResource(rt, false)->getAmount() - r->getAmount()));
        }
        return;
      }
    }
...
void Faction::limitResourcesToStore() {
  if (world != NULL && world->getGame() != NULL &&
    ...
    for (int i = 0; i < (int)resources.size(); ++i) {
      ...
      const ResourceType *rt = r->getType();
      if (rt->getClass() != rcStatic &&
          (getResource(rt, false)->getAmount()) > getStoreAmount(rt, false)) {
        r->setAmount(getStoreAmount(rt, false) -
                     (getResource(rt, false)->getAmount() - r->getAmount()));
      }
    }
        ...
        for (int k = -harvestDistance; k <= harvestDistance; ++k) {
          ...
          if (isResourceTargetInCache(newPos) == false) {
            if (map->isInside(newPos.x, newPos.y)) {
              Resource *r =
                  map->getSurfaceCell(map->toSurfCoords(newPos))->getResource();
              if (r != NULL) {
            ...
            if (map->isInside(newPos) == true &&
              ...
              if (sc != NULL && sc->getResource() != NULL) {
                ...
                if (resource->getType() != NULL &&
                  ...
                  if (result.x < 0 || unit->getPos().dist(newPos) <
          ...
          if (map->isInside(cache) == true) {
            ...
            if (sc != NULL && sc->getResource() != NULL) {
              ...
              if (resource->getType() != NULL && resource->getType() == type) {
                if (result.x < 0 ||
                    unit->getPos().dist(cache) < unit->getPos().dist(result)) {
      ...
      if (deleteListPtr != NULL || needToCleanup == true) {
        ...
        if (deleteListPtr != NULL) {
          ...
          for (std::map<Vec2i, int>::iterator iter =
            ...
            if (world->getMap()->getSurfaceCell(
                    world->getMap()->toSurfCoords(cache)) != NULL) {
              Resource *resource =
                  world->getMap()
...
// cacheList.size(); ++i) { 				FactionPathSuccessCache &cache = cacheList[i];
...
//>
//(path.first.size() - k)) {
//for(int l = k; l < path.first.size(); ++l) { 											result.push_back(path.first[l]);
//										}
//									}
//								}
//								foundCachedPath
...
//		if(successfulPathFinderTargetList.find(target) ==
// successfulPathFinderTargetList.end()) {
// FactionPathSuccessCache cache; 			cache.unitSize = unit->getType()->getSize();
...
//false;
//++i) { 				FactionPathSuccessCache &cache = cacheList[i];
//if(cache.unitSize <=
// unitSize) { 					vector<std::pair<vector<Vec2i>, int> >
// &pathQueue = cache.pathQueue;
//
//					for(int j = 0; j < pathQueue.size() &&
// finishedAdd == false; ++j) {
//						// Now start at the end of the path
//and
// see how many nodes are the same
// std::pair<vector<Vec2i>, int> &path = pathQueue[j];
// int minPathSize =
...
//++k)
//{ 							if(path.first[path.first.size() -
//k - 1] != currentTargetPathTaken.second[currentTargetPathTaken.second.size() -
// k - 1]) { 								intersectIndex = k;
// break;
//							}
//						}
//
//						// New path is same or longer than
//old path so replace
//						// old path with new
//						if(intersectIndex + 1 ==
// path.first.size()) { 							path.first =
// currentTargetPathTaken.second; 							path.second++; 							finishedAdd = true;
//						}
//						// Old path is same or longer than
//new path so
//						// do nothing
...
// curUnit->getType()->getFirstCtOfClass(cmdClass); 				bool
// isUnitPossibleCandidate = (cmdType != NULL);
//if(skillClassList.empty() == false) { 					isUnitPossibleCandidate = false;
//
//					for(int j = 0; j < skillClassList.size();
//++j) { 						SkillClass skValue =
//skillClassList[j]; 						if(curUnit->getCurrSkill()->getClass() == skValue) {
...
//
//			const CommandType *cmdType =
// curUnit->getType()->getFirstCtOfClass(cmdClass); 			if(cmdType !=
// NULL) { 				const RepairCommandType *rct =
// dynamic_cast<const RepairCommandType *>(cmdType); 				if(rct != NULL &&
//rct->isRepairableUnitType(unitType))
//{ 					isUnitPossibleCandidate = true;
//				}
//			}
//			else {
...
bool Faction::canCreateUnit(const UnitType *ut, bool checkBuild,
  ...
  for (int l = 0; l < this->getUnitCount() && foundUnit == false; ++l) {
    ...
    for (int j = 0; j < unitType2->getCommandTypeCount() && foundUnit == false;
         ++j) {
      ...
      if (cmdType != NULL) {
        ...
        if (checkProduce == true && cmdType->getClass() == ccProduce) {
          ...
          if (produce != NULL) {
            ...
            if (produceUnit != NULL && ut->getId() != unitType2->getId() &&
        ...
        // Check if this is a build command
          ...
          if (build != NULL) {
            for (int k = 0; k < build->getBuildingCount() && foundUnit == false;
                 ++k) {
              const UnitType *buildUnit = build->getBuilding(k);

              if (buildUnit != NULL && ut->getId() != unitType2->getId() &&
        ...
        // Check if this is a morph command
          ...
          if (morph != NULL) {
            ...
            if (morphUnit != NULL && ut->getId() != unitType2->getId() &&
...
std::string Faction::toString(bool crcMode) const {
  ...
  for (int idx = 0; idx < (int)allies.size(); idx++) {
    result += "index = " + intToStr(idx) +
              " name: " + allies[idx]->factionType->getName(false) +
              " factionindex = " + intToStr(allies[idx]->index) + "\n";
  }
...
void Faction::saveGame(XmlNode *rootNode) {
  ...
  for (unsigned int i = 0; i < allies.size(); ++i) {
    ...
    allyNode->addAttribute("allyFactionIndex", intToStr(ally->getIndex()),
                           mapTagReplacements);
  }
  for (unsigned int i = 0; i < units.size(); ++i) {
    Unit *unit = units[i];
  ...
  factionNode->addAttribute("teamIndex", intToStr(teamIndex),
                            mapTagReplacements);
  factionNode->addAttribute("startLocationIndex", intToStr(startLocationIndex),
                            mapTagReplacements);
  ...
  for (std::map<Vec2i, int>::iterator iterMap = cacheResourceTargetList.begin();
    ...
    cacheResourceTargetListNode->addAttribute("key", iterMap->first.getString(),
                                              mapTagReplacements);
    cacheResourceTargetListNode->addAttribute(
  ...
  for (std::map<Vec2i, bool>::iterator iterMap =
    ...
    cachedCloseResourceTargetLookupListNode->addAttribute(
        "key", iterMap->first.getString(), mapTagReplacements);
    cachedCloseResourceTargetLookupListNode->addAttribute(
        "value", intToStr(iterMap->second), mapTagReplacements);
  }
  ...
  factionNode->addAttribute("allowSharedTeamUnits",
                            intToStr(allowSharedTeamUnits), mapTagReplacements);
  ...
  for (std::map<int, int>::iterator iterMap = unitsMovingList.begin();
    ...
    unitsMovingListNode->addAttribute("key", intToStr(iterMap->first),
                                      mapTagReplacements);
    unitsMovingListNode->addAttribute("value", intToStr(iterMap->second),
                                      mapTagReplacements);
  }
  ...
  for (std::map<int, int>::iterator iterMap = unitsPathfindingList.begin();
    ...
    unitsPathfindingListNode->addAttribute("key", intToStr(iterMap->first),
                                           mapTagReplacements);
    unitsPathfindingListNode->addAttribute("value", intToStr(iterMap->second),
                                           mapTagReplacements);
  }
}
...
void Faction::loadGame(const XmlNode *rootNode, int factionIndex,
  ...
  if (factionNode != NULL) {
    ...
    control = static_cast<ControlType>(
        factionNode->getAttribute("control")->getIntValue());

    if (factionNode->hasAttribute("overridePersonalityType") == true) {
      overridePersonalityType = static_cast<FactionPersonalityType>(
          factionNode->getAttribute("overridePersonalityType")->getIntValue());
    }
    ...
    if (factionNode->hasAttribute("allowSharedTeamUnits") == true) {
      allowSharedTeamUnits =
          factionNode->getAttribute("allowSharedTeamUnits")->getIntValue() != 0;
    }
    ...
    for (unsigned int i = 0; i < cacheResourceTargetListNodeList.size(); ++i) {
      ...
      Vec2i vec = Vec2i::strToVec2(
          cacheResourceTargetListNode->getAttribute("key")->getValue());
      cacheResourceTargetList[vec] =
          cacheResourceTargetListNode->getAttribute("value")->getIntValue();
    }
    ...
    for (unsigned int i = 0;
      ...
      Vec2i vec = Vec2i::strToVec2(
          cachedCloseResourceTargetLookupListNode->getAttribute("key")
              ->getValue());
...
string Faction::getCRC_DetailsForWorldFrames() const {
  ...
  for (std::map<int, string>::const_iterator iterMap =
    ...
    result += string("========================================================="
                     "===================\n");
    result += string("** world frame: ") + intToStr(iterMap->first) +

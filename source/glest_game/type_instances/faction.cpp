...
bool CommandGroupUnitSorter::compare(const Unit *l, const Unit *r) {
    ...
    if (l == NULL || r == NULL)
        printf("Unit l [%s - %d] r [%s - %d]\n", (l != NULL ? l->getType()->getName(false).c_str() : "null"), (l != NULL ? l->getId() : -1),
               (r != NULL ? r->getType()->getName(false).c_str() : "null"), (r != NULL ? r->getId() : -1));
    ...
    if (command != NULL && command->getCommandType() != NULL &&
        (command->getCommandType()->getClass() == ccMove || command->getCommandType()->getClass() == ccAttack) && command->getUnitCommandGroupId() > 0) {
        ...
        else if (commandPeer->getCommandType()->getClass() != command->getCommandType()->getClass()) {
            result = true;
        }
    ...
    //	else if(command == NULL && j.unit->getCurrrentCommandThreadSafe() ==
    // NULL) { 		return this->unit->getId() < j.unit->getId();
    //	}
    else {
        // Command *commandPeer = j.unit->getCurrrentCommandThreadSafe();
        // if( commandPeer != NULL && commandPeer->getCommandType() != NULL &&
    ...
    // printf("Sorting, unit [%d - %s] cmd [%s] | unit2 [%d - %s] cmd [%s] result
    // =
    // %d\n",this->unit->getId(),this->unit->getFullName().c_str(),(this->unit->getCurrCommand()
    // == NULL ? "NULL" :
    // this->unit->getCurrCommand()->toString().c_str()),j.unit->getId(),j.unit->getFullName().c_str(),(j.unit->getCurrCommand()
    // == NULL ? "NULL" : j.unit->getCurrCommand()->toString().c_str()),result);
...
void Faction::sortUnitsByCommandGroups() {
    ...
    // printf("====== sortUnitsByCommandGroups for faction # %d [%s] unitCount =
    ...
    // printf("====== Done sorting for faction # %d [%s] unitCount =
    // %d\n",this->getIndex(),this->getType()->getName().c_str(),units.size());
...
bool FactionThread::isSignalPathfinderCompleted(int frameIndex) {
    ...
    // if(SystemFlags::VERBOSE_MODE_ENABLED) printf("In [%s::%s Line: %d] worker
    // thread this = %p, this->frameIndex.first = %d, this->frameIndex.second =
...
void FactionThread::execute() {
    ...
    try {
        ...
        if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] ****************** STARTING worker thread this "
                   "= %p\n",
        ...
        for (; this->faction != NULL;) {
            ...
            // frameIndex = %d this = %p executeTask =
            // %d\n",__FILE__,__FUNCTION__,__LINE__,frameIndex.first, this,
            // executeTask);
            ...
            if (executeTask == true) {
                ...
                // config.getBool("AllowGroupedUnitCommands","true"); bool
                // sortedUnitsAllowed = false; if(sortedUnitsAllowed == true) {
                ...
                for (int j = 0; j < unitCount; ++j) {
                    ...
                    codeLocation = "12";
                    if (minorDebugPerformance && (chrono.getMillis() - elapsed1) >= 1)
                        printf("Faction [%d - %s] #1-unit threaded updates on frame: %d for "
                               "[%d] unit # %d, unitCount = %d, took [%lld] msecs\n",
                               faction->getStartLocationIndex(), faction->getType()->getName(false).c_str(), currentTriggeredFrameIndex,
                               faction->getUnitPathfindingListCount(), j, unitCount, (long long int)chrono.getMillis() - elapsed1);
                    ...
                    if (update == true) {
                        ...
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true) {
                            ...
                            char szBuf[8096] = "";
                            snprintf(szBuf, 8096,
                                     "unit->needToUpdate() returned: %d updateProgressValue: %lld "
                                     "speed: %lld changedActiveCommand: %d df: %lld hf: %lld",
                                     update, (long long int)updateProgressValue, (long long int)speed, changedActiveCommand, (long long int)df,
                                     (long long int)hf);
                        ...
                        codeLocation = "15";
                        if (minorDebugPerformance && (chrono.getMillis() - elapsed2) >= 1)
                            printf("Faction [%d - %s] #2-unit threaded updates on frame: %d for "
                                   "[%d] unit # %d, unitCount = %d, took [%lld] msecs\n",
                                   faction->getStartLocationIndex(), faction->getType()->getName(false).c_str(), currentTriggeredFrameIndex,
                                   faction->getUnitPathfindingListCount(), j, unitCount, (long long int)chrono.getMillis() - elapsed2);
                    } else {
                        codeLocation = "16";
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true) {
                            int64 updateProgressValue = unit->getUpdateProgress();
                ...
                if (minorDebugPerformance && chrono.getMillis() >= 1)
                    printf("Faction [%d - %s] threaded updates on frame: %d for [%d] units "
                           "took [%lld] msecs\n",
                           faction->getStartLocationIndex(), faction->getType()->getName(false).c_str(), currentTriggeredFrameIndex,
                           faction->getUnitPathfindingListCount(), (long long int)chrono.getMillis());
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n", __FILE__, __FUNCTION__, __LINE__);
        if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] ****************** ENDING worker thread this = "
                   "%p\n",
                   __FILE__, __FUNCTION__, __LINE__, this);
...
void Faction::addUnitToPathfindingList(int unitId) {
    // printf("ADD (1) Faction [%d - %s] threaded updates for [%d]
    ...
    // printf("ADD (2) Faction [%d - %s] threaded updates for [%d]
    // units\n",this->getStartLocationIndex(),this->getType()->getName().c_str(),unitsPathfindingList.size());
}
...
int Faction::getUnitPathfindingListCount() {
    // printf("GET Faction [%d - %s] threaded updates for [%d]
    // units\n",this->getStartLocationIndex(),this->getType()->getName().c_str(),unitsPathfindingList.size());
    return (int)unitsPathfindingList.size();
}

void Faction::clearUnitsPathfinding() {
    // printf("CLEAR Faction [%d - %s] threaded updates for [%d]
    // units\n",this->getStartLocationIndex(),this->getType()->getName().c_str(),unitsPathfindingList.size());
...
bool Faction::canUnitsPathfind() {
    bool result = true;
    if (control == ctCpuEasy || control == ctCpu || control == ctCpuUltra || control == ctCpuMega) {
        // printf("AI player for faction index: %d (%s) current pathfinding:
        ...
        if (result == false) {
            // printf("WARNING limited AI player for faction index: %d (%s) current
            // pathfinding:
            // %d\n",index,factionType->getName().c_str(),getUnitPathfindingListCount());
        }
    }
...
void Faction::init(FactionType *factionType, ControlType control, TechTree *techTree, Game *game, int factionIndex, int teamIndex, int startLocationIndex,
                   bool thisFaction, bool giveResources, const XmlNode *loadWorldNode) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n", __FILE__, __FUNCTION__, __LINE__);
    ...
    // cachingDisabled = (Config::getInstance().getBool("DisableCaching","false")
    // == true);
    cachingDisabled = false;
...
bool Faction::hasUnitTypeWithResouceCost(const ResourceType *rt) {
    for (int factionUnitTypeIndex = 0; factionUnitTypeIndex < getType()->getUnitTypeCount(); ++factionUnitTypeIndex) {
...
const Resource *Faction::getResource(const ResourceType *rt, bool localFactionOnly) const {
    if (localFactionOnly == false && world != NULL && world->getGame() != NULL) {
...
bool Faction::getCpuControl() const {
    return control == ctCpuEasy || control == ctCpu || control == ctCpuUltra || control == ctCpuMega || control == ctNetworkCpuEasy ||
           control == ctNetworkCpu || control == ctNetworkCpuUltra || control == ctNetworkCpuMega;
}
...
// checks if all required units and upgrades are present and maxUnitCount is
// within limit
    ...
    if (ignoreResourceCosts == false) {
        ...
        for (int i = 0; i < p->getCostCount(); ++i) {
            ...
            if (rt == NULL) {
                char szBuf[8096] = "";
                snprintf(szBuf, 8096,
                         "cannot apply costs for p [%s] %d of %d costs resourcetype "
                         "[%s] is null",
                         p->getName(false).c_str(), i, p->getCostCount(), r->getDescription(false).c_str());
                throw megaglest_runtime_error(szBuf);
            }
...
// deapply all costs except static production (usually when a building is
// cancelled)
            ...
            if (getResource(rt)->getAmount() < 0) {
                ...
                for (int i = 0; i < (int)resourceConsumers.size(); ++i) {
                    ...
                    if (scriptManager->getPlayerModifiers(this->index)->getConsumeEnabled() == true) {
                        ...
                        if (sound != NULL && (thisFaction == true || world->showWorldForPlayer(world->getThisTeamIndex()) == true)) {
    ...
    if (ignoreResourceCosts == false) {
        ...
        for (int i = 0; i < pt->getCostCount(); ++i) {
            ...
            if (cost > 0) {
                ...
                if (maxAmount > possibleCount) maxAmount = possibleCount;
                if (maxAmount == 0) {
                    break;
                }
            }
...
bool Faction::isAlly(const Faction *faction) {
    assert(faction != NULL);
    return (teamIndex == faction->getTeam() || faction->getTeam() == GameConstants::maxPlayers - 1 + fpt_Observer);
}

// ================== misc ==================

void Faction::incResourceAmount(const ResourceType *rt, int amount) {
    if (world != NULL && world->getGame() != NULL && world->getGame()->isFlagType1BitEnabled(ft1_allow_shared_team_resources) == true) {
        for (int i = 0; i < (int)resources.size(); ++i) {
...
void Faction::addStore(const UnitType *unitType) {
    ...
    for (int newUnitStoredResourceIndex = 0; newUnitStoredResourceIndex < unitType->getStoredResourceCount(); ++newUnitStoredResourceIndex) {
        const Resource *newUnitStoredResource = unitType->getStoredResource(newUnitStoredResourceIndex);

        for (int currentStoredResourceIndex = 0; currentStoredResourceIndex < (int)store.size(); ++currentStoredResourceIndex) {
    ...
    if (cachingDisabled == false) {
        ...
        if (duplicateEntry == false) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true) {
                char szBuf[8096] = "";
                snprintf(szBuf, 8096,
                         "[addResourceTargetToCache] pos "
                         "[%s]cacheResourceTargetList.size() [" MG_SIZE_T_SPECIFIER "]",
        ...
        if (cacheResourceTargetList.empty() == false) {
            ...
            if (iter != cacheResourceTargetList.end()) {
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true) {
                    char szBuf[8096] = "";
                    snprintf(szBuf, 8096,
                             "[removeResourceTargetFromCache] pos "
                             "[%s]cacheResourceTargetList.size() [" MG_SIZE_T_SPECIFIER "]",
                             pos.getString().c_str(), cacheResourceTargetList.size());
...
Vec2i Faction::getClosestResourceTypeTargetFromCache(Unit *unit, const ResourceType *type, int frameIndex) {
    ...
    if (cachingDisabled == false) {
        if (cacheResourceTargetList.empty() == false) {
            ...
            //{ 				throw megaglest_runtime_error("#1
            // Invalid access to Faction random from outside main thread current id =
            // "
            // +
            ...
            if (foundCloseResource == false) {
                ...
                for (std::map<Vec2i, int>::iterator iter = cacheResourceTargetList.begin();
                     iter != cacheResourceTargetList.end() && foundCloseResource == false; ++iter) {
            ...
            if (deleteList.empty() == false) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true) {
                    ...
                    snprintf(szBuf, 8096,
                             ...
                             "[" MG_SIZE_T_SPECIFIER "] cacheResourceTargetList.size() [" MG_SIZE_T_SPECIFIER "] result [%s]",
...
// CANNOT MODIFY the cache here since the AI calls this method and the AI is
// only controlled by the server for network games and it would cause out of
// synch since clients do not call this method so DO NOT modify the cache here!
Vec2i Faction::getClosestResourceTypeTargetFromCache(const Vec2i &pos, const ResourceType *type) {
    ...
    if (cachingDisabled == false) {
        if (cacheResourceTargetList.empty() == false) {
            ...
            // 0 means start looking leftbottom to top right
            //			if(Thread::isCurrentThreadMainThread() == false)
            //{ 				throw megaglest_runtime_error("#2
            // Invalid access to Faction random from outside main thread current id =
            // "
            // +
            // intToStr(Thread::getCurrentThreadId())
        ...
        if (cacheResourceTargetList.empty() == false) {
            ...
            if (deleteListPtr != NULL || needToCleanup == true) {
                ...
                if (deleteListPtr != NULL) {
                    ...
                    for (std::map<Vec2i, int>::iterator iter = cacheResourceTargetList.begin(); iter != cacheResourceTargetList.end(); ++iter) {
                        const Vec2i &cache = iter->first;
                ...
                if (deleteList.empty() == false) {
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true) {
                        ...
                        snprintf(szBuf, 8095,
                                 "[cleaning old resource targets] deleteList.size() "
                                 "[" MG_SIZE_T_SPECIFIER "] cacheResourceTargetList.size() [" MG_SIZE_T_SPECIFIER "], needToCleanup [%d]",
...
//	if(cachingDisabled == false) {
//		if(successfulPathFinderTargetList.find(target) ==
// successfulPathFinderTargetList.end()) {
//			// Lets find the shortest and most successful path
// already taken by a
//			// similar sized unit
//
//			bool foundCachedPath = false;
//			std::vector<FactionPathSuccessCache> &cacheList =
// successfulPathFinderTargetList[target]; 			int unitSize =
// unit->getType()->getSize(); 			for(int i = 0; i <
// cacheList.size(); ++i) { 				FactionPathSuccessCache
// &cache = cacheList[i];
//				if(cache.unitSize <= unitSize) {
//					vector<std::pair<vector<Vec2i>, int> >
//&pathQueue = cache.pathQueue;
//
//					for(int j = 0; j < pathQueue.size();
//++j) {
//						// Now start at the end of the
// path and see how many nodes
//						// until we reach a cell near
// the unit's current position
// std::pair<vector<Vec2i>, int> &path = pathQueue[j];
//
//						for(int k = path.first.size() -
// 1; k >= 0; --k) {
...
//		if(successfulPathFinderTargetList.find(target) ==
// successfulPathFinderTargetList.end()) {
// FactionPathSuccessCache cache; 			cache.unitSize =
// unit->getType()->getSize();
//			cache.pathQueue.push_back(make_pair<vector<Vec2i>,
// int>(unit->getCurrentTargetPathTaken().second,1));
//			successfulPathFinderTargetList[target].push_back(cache);
//		}
//		else {
//			bool finishedAdd = false;
//			std::pair<Vec2i,std::vector<Vec2i> >
// currentTargetPathTaken = unit->getCurrentTargetPathTaken();
//			std::vector<FactionPathSuccessCache> &cacheList =
// successfulPathFinderTargetList[target]; 			int unitSize =
// unit->getType()->getSize();
//
//			for(int i = 0; i < cacheList.size() && finishedAdd ==
// false; ++i) { 				FactionPathSuccessCache &cache =
// cacheList[i]; 				if(cache.unitSize
//<= unitSize) {
...
// finishedAdd == false; ++j) {
//						// Now start at the end of the
// path and see how many nodes are the same
// std::pair<vector<Vec2i>, int> &path = pathQueue[j];
// int minPathSize =
// std::min(path.first.size(),currentTargetPathTaken.second.size());
// int intersectIndex = -1;
//
//						for(int k = 0; k < minPathSize;
//++k) {
// if(path.first[path.first.size() - k - 1] !=
// currentTargetPathTaken.second[currentTargetPathTaken.second.size() - k - 1])
// { 								intersectIndex =
// k; break;
//							}
//						}
//
//						// New path is same or longer
// than old path so replace
//						// old path with new
//						if(intersectIndex + 1 ==
// path.first.size()) {
// path.first = currentTargetPathTaken.second;
// path.second++; finishedAdd =
// true;
//						}
//						// Old path is same or longer
// than new path so
//						// do nothing
//						else if(intersectIndex + 1 ==
// currentTargetPathTaken.second.size()) {
// path.second++; finishedAdd = true;
//						}
//					}
//
//					// If new path is >= 10 cells add it
//					if(finishedAdd == false &&
// currentTargetPathTaken.second.size() >= 10) {
//						pathQueue.push_back(make_pair<vector<Vec2i>,
// int>(currentTargetPathTaken.second,1));
//					}
//				}
//			}
//		}
...
// const
// UnitType *unitType) { 	Unit *result = NULL;
//
///*
//	std::map<CommandClass,std::map<int,int> >::iterator iterFind =
// cacheUnitCommandClassList.find(cmdClass); 	if(iterFind !=
// cacheUnitCommandClassList.end()) { 		for(std::map<int,int>::iterator
// iter = iterFind->second.begin(); 				iter !=
// iterFind->second.end(); ++iter) { 			Unit *curUnit =
// findUnit(iter->second); 			if(curUnit != NULL) {
//
//				const CommandType *cmdType =
// curUnit->getType()->getFirstCtOfClass(cmdClass); bool isUnitPossibleCandidate
//= (cmdType != NULL); 				if(skillClassList.empty() ==
// false) { 					isUnitPossibleCandidate = false;
//
//					for(int j = 0; j <
// skillClassList.size(); ++j) {
// SkillClass skValue = skillClassList[j];
//						if(curUnit->getCurrSkill()->getClass()
//== skValue) {
...
//					if(result == NULL ||
// curUnit->getPos().dist(pos) < result->getPos().dist(pos)) {
// result = curUnit;
//					}
//				}
//			}
...
//
//			const CommandType *cmdType =
// curUnit->getType()->getFirstCtOfClass(cmdClass);
// if(cmdType != NULL) { 				const RepairCommandType
// *rct = dynamic_cast<const RepairCommandType *>(cmdType);
...
//
//			if(isUnitPossibleCandidate == true &&
// skillClassList.empty() == false) {
// isUnitPossibleCandidate = false;
//
//				for(int j = 0; j < (int)skillClassList.size();
//++j) { 					SkillClass skValue =
// skillClassList[j];
// if(curUnit->getCurrSkill()->getClass()
//== skValue) {
// isUnitPossibleCandidate = true; break;
//					}
//				}
//			}
//
//
//			if(isUnitPossibleCandidate == true) {
//				//cacheUnitCommandClassList[cmdClass][curUnit->getId()]
//= curUnit->getId();
//
//				if(result == NULL || curUnit->getPos().dist(pos)
//< result->getPos().dist(pos)) { 					result =
// curUnit;
//				}
//			}
//		}
...
const SwitchTeamVote *Faction::getFirstSwitchTeamVote() const {
    ...
    if (switchTeamVotes.empty() == false) {
        for (std::map<int, SwitchTeamVote>::const_iterator iterMap = switchTeamVotes.begin(); iterMap != switchTeamVotes.end(); ++iterMap) {
            ...
            if (cmdType != NULL) {
                ...
                if (checkProduce == true && cmdType->getClass() == ccProduce) {
                    ...
                    if (produce != NULL) {
                        ...
                        if (produceUnit != NULL && ut->getId() != unitType2->getId() && ut->getName(false) == produceUnit->getName(false)) {
                            foundUnit = true;
                            break;
                        }
                    }
                ...
                // Check if this is a build command
                    ...
                    if (build != NULL) {
                        for (int k = 0; k < build->getBuildingCount() && foundUnit == false; ++k) {
                            ...
                            if (buildUnit != NULL && ut->getId() != unitType2->getId() && ut->getName(false) == buildUnit->getName(false)) {
                                foundUnit = true;
                                break;
                            }
                        }
                ...
                // Check if this is a morph command
                    ...
                    if (morph != NULL) {
                        ...
                        if (morphUnit != NULL && ut->getId() != unitType2->getId() && ut->getName(false) == morphUnit->getName(false)) {
                            foundUnit = true;
                            break;
                        }
                    }
...
uint64 Faction::getCacheKBytes(uint64 *cache1Size, uint64 *cache2Size, uint64 *cache3Size, uint64 *cache4Size, uint64 *cache5Size) {
    ...
    for (std::map<Vec2i, int>::iterator iterMap1 = cacheResourceTargetList.begin(); iterMap1 != cacheResourceTargetList.end(); ++iterMap1) {
        cache1Count++;
    }
    for (std::map<Vec2i, bool>::iterator iterMap1 = cachedCloseResourceTargetLookupList.begin(); iterMap1 != cachedCloseResourceTargetLookupList.end();
         ++iterMap1) {
        cache2Count++;
    }
    for (std::map<int, const Unit *>::iterator iterMap1 = aliveUnitListCache.begin(); iterMap1 != aliveUnitListCache.end(); ++iterMap1) {
        cache3Count++;
    }
    for (std::map<int, const Unit *>::iterator iterMap1 = mobileUnitListCache.begin(); iterMap1 != mobileUnitListCache.end(); ++iterMap1) {
        cache4Count++;
    }
    for (std::map<int, const Unit *>::iterator iterMap1 = beingBuiltUnitListCache.begin(); iterMap1 != beingBuiltUnitListCache.end(); ++iterMap1) {
        cache5Count++;
    }
...
void Faction::saveGame(XmlNode *rootNode) {
    ...
    for (std::map<Vec2i, int>::iterator iterMap = cacheResourceTargetList.begin(); iterMap != cacheResourceTargetList.end(); ++iterMap) {
    ...
    for (std::map<Vec2i, bool>::iterator iterMap = cachedCloseResourceTargetLookupList.begin(); iterMap != cachedCloseResourceTargetLookupList.end();
         ++iterMap) {
    ...
    for (std::set<const UnitType *>::iterator iterMap = lockedUnits.begin(); iterMap != lockedUnits.end(); ++iterMap) {
    ...
    for (std::map<int, int>::iterator iterMap = unitsMovingList.begin(); iterMap != unitsMovingList.end(); ++iterMap) {
    ...
    for (std::map<int, int>::iterator iterMap = unitsPathfindingList.begin(); iterMap != unitsPathfindingList.end(); ++iterMap) {
...
void Faction::addCRC_DetailsForWorldFrame(int worldFrameCount, bool isNetworkServer) {
    ...
    // if(worldFrameCount <= 0) printf("Adding world frame: %d log entries:
...
string Faction::getCRC_DetailsForWorldFrames() const {
    string result = "";
    for (std::map<int, string>::const_iterator iterMap = crcWorldFrameDetails.begin(); iterMap != crcWorldFrameDetails.end(); ++iterMap) {
        result += string("======================================================================"
                         "======\n");

...
void FactionThread::execute() {
  ...
  try {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "In [%s::%s Line: %d] ****************** STARTING worker thread this "
          "= %p\n",
      ...
      if (executeTask == true) {
        ...
        for (int j = 0; j < unitCount; ++j) {
          ...
          if (minorDebugPerformance && (chrono.getMillis() - elapsed1) >= 1)
            printf(
                "Faction [%d - %s] #1-unit threaded updates on frame: %d for "
          ...
          if (update == true) {
            ...
            if (minorDebugPerformance && (chrono.getMillis() - elapsed2) >= 1)
              printf(
                  "Faction [%d - %s] #2-unit threaded updates on frame: %d for "
                  "[%d] unit # %d, unitCount = %d, took [%lld] msecs\n",
                  faction->getStartLocationIndex(),
                  faction->getType()->getName(false).c_str(),
                  currentTriggeredFrameIndex,
        ...
        if (minorDebugPerformance && chrono.getMillis() >= 1)
          printf(
              "Faction [%d - %s] threaded updates on frame: %d for [%d] units "
              "took [%lld] msecs\n",
              faction->getStartLocationIndex(),
              faction->getType()->getName(false).c_str(),
              currentTriggeredFrameIndex,
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "In [%s::%s Line: %d] ****************** ENDING worker thread this = "
          "%p\n",
          __FILE__, __FUNCTION__, __LINE__, this);
...
std::pair<int, string> Faction::getCRC_DetailsForWorldFrameIndex(

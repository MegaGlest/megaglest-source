...
void ProgramState::restoreToStartXY() {
  SDL_WarpMouseInWindow(this->program->getWindow()->getSDLWindow(), startX,
                        startY);
}

void ProgramState::incrementFps() { fps++; }
...
bool ProgramState::canRender(bool sleepIfCannotRender) {
  ...
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    ...
    sleepMillis =
        Config::getInstance().getInt("RenderFPSCapHeadlessSleepMillis", "1");
  }
  ...
  if (lastFps > maxFPSCap) {
    if (sleepIfCannotRender == true) {
      ...
      // sleeping because lastFps = %d, maxFPSCap = %d sleepMillis =
...
Program::ShowMessageProgramState::ShowMessageProgramState(Program *program,
                                                          const char *msg)
...
void Program::ShowMessageProgramState::keyPress(SDL_KeyboardEvent c) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                             ...
                             __FUNCTION__, __LINE__, c.keysym.sym);
...
void Program::ShowMessageProgramState::mouseMove(int x, int y,
...
void Program::initSavedGame(WindowGl *window, bool masterserverMode,
  ...
  // if(SystemFlags::VERBOSE_MODE_ENABLED) printf("Loading game from
  // [%s]\n",saveGameFile.c_str());
...
void Program::initServer(WindowGl *window, bool autostart,
  ...
  MainMenu *mainMenu = new MainMenu(this);
  setState(mainMenu);
  mainMenu->setState(new MenuStateCustomGame(this, mainMenu, openNetworkSlots,
                                             pNewGame, autostart, NULL,
                                             masterserverMode));
}
...
void Program::initServer(WindowGl *window, GameSettings *settings) {
  ...
  setState(mainMenu);
  mainMenu->setState(
      new MenuStateCustomGame(this, mainMenu, false, pNewGame, true, settings));
}
...
void Program::initClient(WindowGl *window, const Ip &serverIp, int portNumber) {
  ...
  setState(mainMenu);
  mainMenu->setState(
      new MenuStateJoinGame(this, mainMenu, true, serverIp, portNumber));
}
...
void Program::keyDown(SDL_KeyboardEvent key) {
  if (msgBox.getEnabled()) {
    ...
    //	((key == vkReturn || key == SDLK_RETURN || key == SDLK_KP_ENTER) &&
    //!(keystate.mod & (KMOD_LALT | KMOD_RALT)))) {
...
void Program::keyUp(SDL_KeyboardEvent key) { programState->keyUp(key); }

void Program::keyPress(SDL_KeyboardEvent c) { programState->keyPress(c); }
...
void Program::loopWorker() {
  ...
  programState->addPerformanceCount(ProgramState::MAIN_PROGRAM_RENDER_KEY,
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
    ...
    SystemFlags::OutputDebug(
        SystemFlags::debugPerformance,
        "In [%s::%s] Line: %d programState->render took msecs: %lld "
        "==============> MAIN LOOP RENDERING\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        chrono.getMillis());
  if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
  ...
  while (prevState == this->programState && updateTimer.isTime()) {
    ...
    if (showPerfStats) {
      sprintf(perfBuf,
              ...
              " updateCount: %d\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, chronoPerf.getMillis(), updateCount);
      perfList.push_back(perfBuf);
    }
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
      ...
      SystemFlags::OutputDebug(SystemFlags::debugPerformance,
                               "In [%s::%s Line: %d] programState->update took "
                               "msecs: %lld, updateCount = %d\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__,
                               chronoUpdateLoop.getMillis(), updateCount);
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
            .enabled &&
        chronoUpdateLoop.getMillis() > 0)
      chronoUpdateLoop.start();
    ...
    if (prevState == this->programState) {
      ...
      if (soundThreadManager == NULL ||
        ...
        if (soundThreadManager != NULL) {
          if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem)
            ...
            SystemFlags::OutputDebug(
                ...
                "In [%s::%s %d] ERROR / WARNING "
                "soundThreadManager->isThreadExecutionLagging is TRUE\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__);
          SystemFlags::OutputDebug(
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugPerformance,
              "In [%s::%s Line: %d] SoundRenderer::getInstance().update() took "
              "msecs: %lld, updateCount = %d\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, chronoUpdateLoop.getMillis(), updateCount);
        if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
                .enabled &&
            chronoUpdateLoop.getMillis() > 0)
          chronoUpdateLoop.start();
      ...
      programState->addPerformanceCount("SoundRenderer::getInstance().update()",
                                        chronoPerformanceCounts.getMillis());
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
        ...
        SystemFlags::OutputDebug(
            SystemFlags::debugPerformance,
            "In [%s::%s Line: %d] NetworkManager::getInstance().update() took "
            "msecs: %lld, updateCount = %d\n",
            extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
            __LINE__, chronoUpdateLoop.getMillis(), updateCount);
      if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
    ...
    SystemFlags::OutputDebug(
        SystemFlags::debugPerformance,
        "In [%s::%s] Line: %d AFTER programState->update took msecs: %lld "
        "==============> MAIN LOOP BODY LOGIC, updateCount = %d\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
  ...
  if (prevState == this->programState) {
    ...
    // chrono.getMillis() > 0)
    // SystemFlags::OutputDebug(SystemFlags::debugSystem,"In [%s::%s] Line: %d
    // programState->render took msecs: %lld ==============> MAIN LOOP
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugPerformance,
        "In [%s::%s Line: %d] ------------------------------- MAIN LOOP END, "
        "stats: loop took msecs: %lld -------------------------------\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
    ...
    if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false) {
      if (dynamic_cast<Game *>(programStateNew) != NULL) {
        try {
          ...
          Renderer &renderer = Renderer::getInstance();
          game->setQuitPendingIndicator(); // by this the world is no more
                                           // updated
...
void Program::exit() { window->destroy(); }
...
void Program::initResolution() {
  ...
  if (window->getScreenWidth() != metrics.getScreenW() ||
    ...
    printf("MainWindow forced change of resolution to desktop values (%d x %d) "
           "instead of (%d x %d)\n",
...
void Program::init(WindowGl *window, bool initSound, bool toggleFullScreen) {
  ...
  if (getGameReadWritePath(GameConstants::path_logs_CacheLookupKey) != "") {
    logFile =
        getGameReadWritePath(GameConstants::path_logs_CacheLookupKey) + logFile;
  } else {
  ...
  window->initGl(config.getInt("ColorBits"), config.getInt("DepthBits"),
  ...
  if (initSound == true && toggleFullScreen == false) {
    ...
    if (SoundRenderer::getInstance().runningThreaded() == true) {
      ...
      static string mutexOwnerId =
          ...
          intToStr(__LINE__);
      soundThreadManager = new SimpleTaskThread(
          &SoundRenderer::getInstance(), 0, SOUND_THREAD_UPDATE_MILLISECONDS);
      soundThreadManager->setUniqueID(mutexOwnerId);
      soundThreadManager->start();
    }
  }

  NetworkInterface::setAllowGameDataSynchCheck(
      Config::getInstance().getBool("AllowGameDataSynchCheck", "false"));
  NetworkInterface::setAllowDownloadDataSynch(
      Config::getInstance().getBool("AllowDownloadDataSynch", "false"));
...
bool Program::isMessageShowing() { return msgBox.getEnabled(); }
...
void Program::startSoundSystem() {
  ...
  if (SoundRenderer::getInstance().runningThreaded() == true) {
    static string mutexOwnerId =
        string(extractFileFromDirectoryPath(__FILE__).c_str()) + string("_") +
        intToStr(__LINE__);
    soundThreadManager = new SimpleTaskThread(&SoundRenderer::getInstance(), 0,

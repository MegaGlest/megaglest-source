...
Program::ShowMessageProgramState::ShowMessageProgramState(Program *program, const char *msg) : ProgramState(program) {
    userWantsExit = false;
...
void Program::initServer(WindowGl *window, bool autostart, bool openNetworkSlots, bool masterserverMode) {
...
void Program::initScenario(WindowGl *window, string autoloadScenarioName) {
    ...
    MainMenu *mainMenu = new MainMenu(this);
    setState(mainMenu);
    mainMenu->setState(new MenuStateScenario(this, mainMenu, false, Config::getInstance().getPathListForType(ptScenarios), autoloadScenarioName));
}
    ...
    while (prevState == this->programState && updateTimer.isTime()) {
        ...
        if (showPerfStats) {
            sprintf(perfBuf, "In [%s::%s] Line: %d took msecs: " MG_I64_SPECIFIER " updateCount: %d\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                    __FUNCTION__, __LINE__, chronoPerf.getMillis(), updateCount);
            perfList.push_back(perfBuf);
        }
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chronoUpdateLoop.getMillis() > 0)
            SystemFlags::OutputDebug(SystemFlags::debugPerformance,
...
void Program::initResolution() {
    ...
    if (window->getScreenWidth() != metrics.getScreenW() || window->getScreenHeight() != metrics.getScreenH()) {
...
void Program::init(WindowGl *window, bool initSound, bool toggleFullScreen) {
    ...
    window->initGl(config.getInt("ColorBits"), config.getInt("DepthBits"), config.getInt("StencilBits"), config.getBool("HardwareAcceleration", "false"),

...
Intro::Intro(Program *program) : ProgramState(program) {
  ...
  camera.setOrientation(Quaternion(EulerAngles(degToRad(startRotation.x),
  ...
  xmlTree.load(getGameCustomCoreDataPath(data_path, "data/core/menu/menu.xml"),
  ...
  if (menuNode->hasChild("intro") == true) {
    ...
    const XmlNode *showIntroPicturesNode =
        introNode->getChild("show-intro-pictures");
    ...
    const XmlNode *showIntroModelsNode =
        introNode->getChild("show-intro-models");
    showIntroModels =
    ...
    modelMinAnimSpeed =
        showIntroModelsNode->getAttribute("min-anim-speed")->getFloatValue();
    modelMaxAnimSpeed =
        showIntroModelsNode->getAttribute("max-anim-speed")->getFloatValue();
  }
  ...
  if (showIntroModels == true) {
    ...
    for (int i = 0; i < (int)introModels.size(); ++i) {
      ...
      Model *model =
          renderer.newModel(rsMenu, getGameCustomCoreDataPath(data_path, "") +
    ...
    if (models.empty() == true) {
      ...
      for (int i = 0; i < (int)introModels.size(); ++i) {
        string logo = introModels[i];
        Model *model = renderer.newModel(
            rsMenu, data_path + "data/core/menu/main_model/" + logo);
        if (model) {
          models.push_back(model);
    ...
    if (lang.hasString(introTagName, "", true) == true ||
      ...
      if (lang.hasString(introTagName, "", true) == false &&
        ...
        if (lang.hasString(introTagTextureWidthName, "", true) == true) {
          textureWidth =
              strToInt(lang.getString(introTagTextureWidthName, "", true));
        }
        ...
        if (lang.hasString(introTagTextureHeightName, "", true) == true) {
          textureHeight =
              strToInt(lang.getString(introTagTextureHeightName, "", true));
        }

        texts.push_back(new IntroText(
            logoTexture,
            Vec2i(w / 2 - (textureWidth / 2), h / 2 - (textureHeight / 2)),
            Vec2i(textureWidth, textureHeight), displayTime));
      }
      // This is a line of text
        ...
        texts.push_back(new IntroText(lineText, Vec2i(textX, textY),
  ...
  /*
          ...
          texts.push_back(new Text(lineText, Vec2i(-1, -1), appear,
     coreData.getMenuFontVeryBig(),coreData.getMenuFontVeryBig3D())); lineText =
     "the MegaGlest Team presents"; texts.push_back(new Text(lineText, Vec2i(-1,
     -1), disappear,
     coreData.getMenuFontVeryBig(),coreData.getMenuFontVeryBig3D())); lineText =
     "a libre software real-time strategy game"; texts.push_back(new
     Text(lineText, Vec2i(-1, -1), disappear *(++displayItemNumber),
     coreData.getMenuFontVeryBig(),coreData.getMenuFontVeryBig3D()));

          texts.push_back(new Text(coreData.getLogoTexture(), Vec2i(w/2-128,
     h/2-64), Vec2i(256, 128), disappear *(++displayItemNumber)));
          texts.push_back(new Text(glestVersionString, Vec2i(w/2+45, h/2-45),
     disappear *(displayItemNumber++),
     coreData.getMenuFontNormal(),coreData.getMenuFontNormal3D())); lineText =
     "www.megaglest.org";
          //texts.push_back(new Text(lineText, Vec2i(-1, -1), disappear
     *(displayItemNumber++),
     coreData.getMenuFontVeryBig(),coreData.getMenuFontVeryBig3D()));
          texts.push_back(new Text(lineText, Vec2i(-1, h/2-45-18), disappear
     *(displayItemNumber-1),
     coreData.getMenuFontVeryBig(),coreData.getMenuFontVeryBig3D()));
  */
  ...
  if (showIntroPics > 0 && coreData.getMiscTextureList().size() > 0) {
      ...
      for (; (int)intoTexList.size() < showIntroPics;) {
        ...
        if (usedIndex.find(picIndex) != usedIndex.end()) {
          ...
          if (failedLookups > 10000) {
            for (unsigned int i = 0; i < coreData.getMiscTextureList().size();
                 ++i) {
    ...
    for (unsigned int i = 0; i < intoTexList.size(); ++i) {
      ...
      if (i == 0 || i % 9 == 0) {
        ...
        texPlacement =
            Vec2i(w - tex->getTextureWidth(), h - tex->getTextureHeight());
      } else if (i == 4 || i % 9 == 4) {
        texPlacement = Vec2i(w / 2 - tex->getTextureWidth() / 2,
        ...
        texPlacement = Vec2i(w - tex->getTextureWidth(),
      ...
      if (lang.hasString("IntroTextureStartMilliseconds", "", true) == true) {
        textureStartTime =
            strToInt(lang.getString("IntroTextureStartMilliseconds", "", true));
      }

      texts.push_back(
          new IntroText(tex, texPlacement,
                        Vec2i(tex->getTextureWidth(), tex->getTextureHeight()),
                        textureStartTime + (showMiscTime * (i + 1))));
    }
  }
  ...
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false &&
    ...
    ::Shared::Graphics::VideoPlayer player(
        ...
        window, 0, 0, screen->w, screen->h, screen->format->BitsPerPixel, false,
        vlcPluginsPath, SystemFlags::VERBOSE_MODE_ENABLED);
    player.PlayVideo();
...
void Intro::update() {
  ...
  if (targetCamera != NULL) {
    t +=
        ((0.01f + (1.f - t) / 10.f) / 20.f) * (60.f / GameConstants::updateFps);

    // interpolate position
    camera.setPosition(
        lastCamera.getPosition().lerp(t, targetCamera->getPosition()));

    // interpolate orientation
    Quaternion q =
        lastCamera.getOrientation().lerp(t, targetCamera->getOrientation());
  ...
  anim += (maxSpeed / GameConstants::updateFps) / 5 +
...
void Intro::renderModelBackground() {
  ...
  if (models.empty() == false) {
    ...
    // printf("difTime = %d individualModelShowTime = %d modelIndex =
    ...
    if (difTime > 0) {
      if (difTime > ((modelIndex + 1) * individualModelShowTime)) {
        ...
        if (modelIndex + 1 < (int)models.size()) {
          ...
          //
          //					this->targetCamera =
          //&nextCamera; 					this->lastCamera= camera;
          //this->t= 0.f;
        }
        // printf("oldmodelIndex = %d, modelIndex =
...
void Intro::render() {
  ...
  for (int i = 0; i < (int)texts.size(); ++i) {
    ...
    if (difTime > 0 && difTime < appearTime + showTime + disapearTime) {
      ...
      if (difTime > 0 && difTime < appearTime) {
        ...
        // disappearing
        alpha = 1.f - static_cast<float>(difTime - appearTime - showTime) /
                          disapearTime;
      }
      ...
      if (text->getText().empty() == false) {
        ...
        if (Renderer::renderText3DEnabled) {
          if (renderX < 0) {
            const Metrics &metrics = Metrics::getInstance();
            int w = metrics.getVirtualW();
            renderX = (w / 2) - (text->getFont3D()->getMetrics()->getTextWidth(
                                     text->getText()) /
                                 2);
          }
          if (renderY < 0) {
            ...
            renderY =
                (h / 2) +
                (text->getFont3D()->getMetrics()->getHeight(text->getText()) /
                 2);
          }

          renderer.renderText3D(text->getText(), text->getFont3D(), alpha,
          ...
          renderer.renderText(text->getText(), text->getFont(), alpha, renderX,
                              renderY, true);
        }
      }
  ...
  if (showIntroTiming == true && Intro::introTime > 0) {
    ...
    if (Renderer::renderText3DEnabled) {
      ...
      renderer.renderText3D(timingText, coreData.getMenuFontVeryBig3D(), 1, 10,
                            20, false);
    } else {
      // const Metrics &metrics= Metrics::getInstance();
      // int w= metrics.getVirtualW();
      ...
      renderer.renderText(timingText, coreData.getMenuFontVeryBig(), 1, 10, 20,
                          false);
    }
  }
...
void Intro::keyDown(SDL_KeyboardEvent key) {
  ...
  // printf("keystate.mod = %d key = unicode[%d] regular[%d] lalt [%d] ralt [%d]
  // alt [%d]\n",keystate.mod,key.keysym.unicode,key.keysym.sym,(keystate.mod &
  // KMOD_LALT),(keystate.mod & KMOD_RALT),(keystate.mod & KMOD_ALT));

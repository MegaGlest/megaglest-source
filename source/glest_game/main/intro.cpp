...
Intro::Intro(Program *program) : ProgramState(program) {
    ...
    if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false &&
        (::Shared::Graphics::VideoPlayer::hasBackEndVideoPlayer() == false || CoreData::getInstance().hasIntroVideoFilename() == false)) {
    ...
    camera.setOrientation(Quaternion(EulerAngles(degToRad(startRotation.x), degToRad(startRotation.y), degToRad(startRotation.z))));
    ...
    for (unsigned int i = 1; i < maxIntroLines; ++i) {
        ...
        if (lang.hasString(introTagName, "", true) == true || lang.hasString(introTagTextureName, "", true) == true) {
            string lineText = "";
            ...
            // Is this a texture?
            if (lang.hasString(introTagName, "", true) == false && lang.hasString(introTagTextureName, "", true) == true) {
                ...
                texts.push_back(new IntroText(logoTexture, Vec2i(w / 2 - (textureWidth / 2), h / 2 - (textureHeight / 2)), Vec2i(textureWidth, textureHeight),
                                              displayTime));
            }
  ...
  */
    ...
    if (showIntroPics > 0 && coreData.getMiscTextureList().size() > 0) {
        ...
        if (showIntroPicsRandom == true) {
            ...
            for (; (int)intoTexList.size() < showIntroPics;) {
                ...
                if (usedIndex.find(picIndex) != usedIndex.end()) {
                    ...
                    if (failedLookups > 10000) {
                        for (unsigned int i = 0; i < coreData.getMiscTextureList().size(); ++i) {
            ...
            for (unsigned int i = 0; i < (unsigned int)coreData.getMiscTextureList().size() && i < (unsigned int)showIntroPics; ++i) {
        ...
        for (unsigned int i = 0; i < intoTexList.size(); ++i) {
            ...
            texts.push_back(
                new IntroText(tex, texPlacement, Vec2i(tex->getTextureWidth(), tex->getTextureHeight()), textureStartTime + (showMiscTime * (i + 1))));
        }
    }
    ...
    if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false && ::Shared::Graphics::VideoPlayer::hasBackEndVideoPlayer() == true &&
        ...
        ::Shared::Graphics::VideoPlayer player(&Renderer::getInstance(), introVideoFile, introVideoFileFallback, window, 0, 0, screen->w, screen->h,
...
void Intro::render() {
        ...
        if (difTime > 0 && difTime < appearTime + showTime + disapearTime) {
            ...
            if (text->getText().empty() == false) {
                ...
                if (Renderer::renderText3DEnabled) {
                    ...
                    renderer.renderText3D(text->getText(), text->getFont3D(), alpha, renderX, renderY, false);
                } else {
                    if (renderX < 0) {
                        const Metrics &metrics = Metrics::getInstance();
                        int w = metrics.getVirtualW();
                    ...
                    renderer.renderText(text->getText(), text->getFont(), alpha, renderX, renderY, true);
                }
            }

            if (text->getTexture() != NULL) {
                renderer.renderTextureQuad(text->getPos().x, text->getPos().y, text->getSize().x, text->getSize().y, text->getTexture(), alpha);
            }
        }
    }
    ...
    if (showIntroTiming == true && Intro::introTime > 0) {
        ...
        if (Renderer::renderText3DEnabled) {
            ...
            renderer.renderText3D(timingText, coreData.getMenuFontVeryBig3D(), 1, 10, 20, false);
        } else {
            // const Metrics &metrics= Metrics::getInstance();
            // int w= metrics.getVirtualW();
            // int h= metrics.getVirtualH();

            renderer.renderText(timingText, coreData.getMenuFontVeryBig(), 1, 10, 20, false);
        }
    }
...
void Intro::keyDown(SDL_KeyboardEvent key) {
    ...
    if (keystate.mod & (KMOD_LALT | KMOD_RALT)) {
        ...
        if (isKeyPressed(SDLK_RETURN, key) == true || isKeyPressed(SDLK_RALT, key) == true || isKeyPressed(SDLK_LALT, key) == true) {
            return;
        }
    }

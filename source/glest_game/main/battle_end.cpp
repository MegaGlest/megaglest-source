...
BattleEnd::BattleEnd(Program *program, const Stats *stats, ProgramState *originState) : ProgramState(program), menuBackgroundVideo(NULL), gameSettings(NULL) {
...
void BattleEnd::initBackgroundVideo() {
    if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false && ::Shared::Graphics::VideoPlayer::hasBackEndVideoPlayer() == true) {
        ...
        if (fileExists(videoFile) || fileExists(videoFileFallback)) {
            ...
            menuBackgroundVideo = new VideoPlayer(&Renderer::getInstance(), videoFile, videoFileFallback, window, 0, 0, screen->w, screen->h,
...
void BattleEnd::render() {
    ...
    if (menuBackgroundVideo == NULL && renderToTexture != NULL) {
        ...
        for (int i = 0; i < stats.getFactionCount(); ++i) {
            ...
            if (stats.getPersonalityType(i) == fpt_Observer) {
                ...
                if (stats.getVictory(i)) {
                    textRenderer->render(stats.getVictory(i) ? lang.getString("Victory").c_str() : lang.getString("Defeat").c_str(), textX, bm + 360, false,
                                         &highliteColor);
                } else {
                    textRenderer->render(stats.getVictory(i) ? lang.getString("Victory").c_str() : lang.getString("Defeat").c_str(), textX, bm + 360, false,
                                         &normalColor);
        ...
        string header2 =
            lang.getString("GameDurationTime", "", true) + ": " + getTimeDuationString(stats.getFramesToCalculatePlaytime(), GameConstants::updateFps);

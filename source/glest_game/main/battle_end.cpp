...
BattleEnd::~BattleEnd() {
  ...
  if (CoreData::getInstance().hasMainMenuVideoFilename() == false) {
    SoundRenderer::getInstance().playMusic(
    ...
    if (factionDefinitionXML != "" &&
      ...
      if (won == true) {
        if (factionNode->hasAttribute("battle-end-win-video") == true) {
          factionVideoUrl =
              factionNode->getAttribute("battle-end-win-video")->getValue();
        }
      } else {
        if (factionNode->hasAttribute("battle-end-lose-video") == true) {
          factionVideoUrl =
              factionNode->getAttribute("battle-end-lose-video")->getValue();
        }
      }
      ...
      if (factionVideoUrl != "" && fileExists(factionVideoUrl) == false) {
        ...
        if (factionPartsList.size() > 1) {
          ...
          string factionPath =
              techTreePath + "factions/" + currentFactionName_factionPreview;
      ...
      if (won == true) {
        factionVideoUrlFallback = Game::findFactionLogoFile(
            gameSettings, NULL, "battle_end_win_video.*");
      } else {
        factionVideoUrlFallback = Game::findFactionLogoFile(
            gameSettings, NULL, "battle_end_lose_video.*");
      }

      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("#3 factionVideoUrl [%s] factionVideoUrlFallback [%s]\n",
               factionVideoUrl.c_str(), factionVideoUrlFallback.c_str());
      // printf("#3 factionVideoUrl [%s] factionVideoUrlFallback
      // [%s]\n",factionVideoUrl.c_str(),factionVideoUrlFallback.c_str());
    ...
    if (factionDefinitionXML != "" &&
      ...
      if (won == true) {
        if (factionNode->hasAttribute("battle-end-win-music") == true) {
          result =
              factionNode->getAttribute("battle-end-win-music")->getValue();
        }
      } else {
        if (factionNode->hasAttribute("battle-end-lose-music") == true) {
          result =
              factionNode->getAttribute("battle-end-lose-music")->getValue();
        }
      }
      ...
      if (won == true) {
        resultFallback = Game::findFactionLogoFile(gameSettings, NULL,
      ...
      // printf("#3 result [%s] resultFallback
  ...
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false &&
    ...
    if (stats.getThisFactionIndex() >= 0 &&
      ...
      if (stats.getVictory(stats.getThisFactionIndex())) {
        ...
        // videoFileFallback =
        // CoreData::getInstance().getBattleEndVideoFilenameFallback(true);
        ...
        // videoFileFallback =
        // CoreData::getInstance().getBattleEndVideoFilenameFallback(false);
    ...
    if (fileExists(videoFile) || fileExists(videoFileFallback)) {
      ...
      menuBackgroundVideo = new VideoPlayer(
          &Renderer::getInstance(), videoFile, videoFileFallback, window, 0, 0,
          screen->w, screen->h, screen->format->BitsPerPixel, true,
...
void BattleEnd::render() {
  ...
  // printf("In [%s::%s Line: %d] renderToTexture
  ...
  if (menuBackgroundVideo == NULL && renderToTexture != NULL) {
    ...
    bool disableStatsColorCoding =
        Config::getInstance().getBool("DisableBattleEndColorCoding", "false");
    ...
    for (int i = 0; i < stats.getFactionCount(); ++i) {
      ...
      if (score == bestScore && stats.getVictory(i)) {
        if (CoreData::getInstance().getGameWinnerTexture() != NULL) {
          renderer.renderTextureQuad(
              textX, bm + 420, -1, -1,
              CoreData::getInstance().getGameWinnerTexture(), 0.7f);
        }
      }
      ...
      if (stats.getPlayerName(i) != "") {
        ...
        textRenderer->render(textToRender.c_str(), textX, bm + 400, false,
                             &color);
      } else {
        textRenderer->render(
            (lang.getString("Player") + " " + intToStr(i + 1)).c_str(), textX,
            bm + 400, false, &color);
      }
      ...
      if (stats.getPersonalityType(i) == fpt_Observer) {
        textRenderer->render(lang.getString("GameOver").c_str(), textX,
                             bm + 360, false, &normalColor);
      } else {
        if (stats.getVictory(i)) {
          textRenderer->render(stats.getVictory(i)
                                   ? lang.getString("Victory").c_str()
                                   : lang.getString("Defeat").c_str(),
                               textX, bm + 360, false, &highliteColor);
        } else {
          textRenderer->render(stats.getVictory(i)
                                   ? lang.getString("Victory").c_str()
                                   : lang.getString("Defeat").c_str(),
                               textX, bm + 360, false, &normalColor);
        }
      }

      textRenderer->render(controlString, textX, bm + 320, false, &normalColor);
      textRenderer->render(stats.getFactionTypeName(i), textX, bm + 280, false,
                           &normalColor);
      textRenderer->render(intToStr(team).c_str(), textX, bm + 240, false,
                           &normalColor);

      if (kills == bestKills) {
        textRenderer->render(intToStr(kills).c_str(), textX, bm + 200, false,
                             &highliteColor);
      } else {
        textRenderer->render(intToStr(kills).c_str(), textX, bm + 200, false,
                             &normalColor);
      ...
      if (deaths == leastDeaths) {
        textRenderer->render(intToStr(deaths).c_str(), textX, bm + 160, false,
                             &highliteColor);
      } else {
        textRenderer->render(intToStr(deaths).c_str(), textX, bm + 160, false,
                             &normalColor);
      ...
      if (resourcesHarvested == bestResourcesHarvested) {
        textRenderer->render(intToStr(resourcesHarvested).c_str(), textX,
                             bm + 80, false, &highliteColor);
      } else {
        textRenderer->render(intToStr(resourcesHarvested).c_str(), textX,
                             bm + 80, false, &normalColor);
      }
      if (score == bestScore) {
        textRenderer->render(intToStr(score).c_str(), textX, bm + 20, false,
                             &highliteColor);
      } else {
        textRenderer->render(intToStr(score).c_str(), textX, bm + 20, false,
                             &normalColor);
    ...
    textRenderer->render("\n" + (lang.getString("LeftAt")), lm, bm + 400, false,
                         &normalColor);
    ...
    // floatToStr(stats.getWorldTimeElapsed() / 24.0,2);
    ...
    header2 = lang.getString("GameMaxConcurrentUnitCount") + ": " +
              intToStr(stats.getMaxConcurrentUnitCount());
    textRenderer->render(header2, lm + 250, bm + 510);

    header2 = lang.getString("GameTotalEndGameConcurrentUnitCount") + ": " +
...
void BattleEnd::showMessageBox(const string &text, const string &header,
                               bool toggle) {
  if (toggle == false) {
    mainMessageBox.setEnabled(false);
  }

...
#if defined(__GNUC__) && !defined(__MINGW32__) && !defined(__FreeBSD__) && \
    !defined(BSD)
#include <signal.h>
#endif
...
class NavtiveLanguageNameListCacheGenerator
...
static void cleanupProcessObjects() {
  ...
  IRCThread *&ircClient = CacheManager::getCachedItem<IRCThread *>(
      ...
      if (lastLazyThreadDump != (int)difftime((long int)time(NULL), elapsed)) {
        ...
        for (int i = 0; i < (int)Thread::getThreadList().size(); ++i) {
          ...
          printf("Thread index: %d ptr [%p] isBaseThread: %d, Name: [%s]\n", i,
                 baseThread, (baseThread != NULL ? 1 : 0),
                 (baseThread != NULL ? baseThread->getUniqueID().c_str()
                                     : "<na>"));
        }
      }
    }
  ...
  std::map<string, vector<FileReader<Pixmap2D> const *> *> &list2d =
      FileReader<Pixmap2D>::getFileReadersMap();
  deleteMapValues(list2d.begin(), list2d.end());
  std::map<string, vector<FileReader<Pixmap3D> const *> *> &list3d =
      FileReader<Pixmap3D>::getFileReadersMap();
...
void write_file_and_line(string &out, HANDLE process, DWORD64 program_counter) {
  ...
  if (SymGetLineFromAddr64(process, program_counter, &dummy, &ih_line)) {
    out += string("|") + string(ih_line.FileName) + ":" +
...
#endif
                    process, thread, &sf,
                    (tryThreadContext == false ? &threadContext : &ctx), 0,
...
struct UntypedException {
  explicit UntypedException(const EXCEPTION_RECORD &er)
      : exception_object(reinterpret_cast<void *>(er.ExceptionInformation[1])),
        type_array(reinterpret_cast<_ThrowInfo *>(er.ExceptionInformation[2])
                       ->pCatchableTypeArray) {}
...
void *exception_cast_worker(const UntypedException &e, const type_info &ti) {
  for (int i = 0; i < e.type_array->nCatchableTypes; ++i) {
    ...
    const std::type_info &ti_i =
        *reinterpret_cast<std::type_info *>(type_i.pType);
    if (ti_i == ti) {
  ...
  switch (er->ExceptionCode) {
    ...
    case EXCEPTION_ACCESS_VIOLATION: {
      out += string("Access violation. Illegal ") +
             (er->ExceptionInformation[0] ? "write" : "read") + string(" by ") +
             intToStr((int)er->ExceptionAddress) + string(" at ") +
             intToStr(er->ExceptionInformation[1]);
    } break;
    default: {
      out += "SEH exception thrown. Exception code: " + er->ExceptionCode +
...
void ExceptionHandler::handle(LPEXCEPTION_POINTERS pointers) {
  string msg = "#1 An error occurred and " + string(mg_app_name) +
...
void ExceptionHandler::handleRuntimeError(const char *msg,
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("In [%s::%s Line: %d] program = %p gameInitialized = %d msg [%s]\n",
  ...
  if (mainProgram && gameInitialized == true) {
    ...
    if (mainProgram->getState() != NULL) {
      ...
      if (glActiveTexture != NULL) {
        for (; GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false &&
          ...
          try {
            ...
            printf("\n=====================================\n");
            printf(
                "\n** Already in error handler exiting error rendering, msg "
                "[%s]\n",
                e.what());
            fflush(stdout);
            break;
...
#ifdef WIN32
    err += string(", attaching the generated ") + getCrashDumpFileName() +
           string(" file.");
#endif
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d]\n", __FILE__, __FUNCTION__, __LINE__);
...
MainWindow::MainWindow(Program *program)
    : WindowGl(), popupMenu("MainWindow", "popupMenu") {
  this->program = program;
  // this->popupMenu.registerGraphicComponentOnlyFontCallbacks("MainWindow",
  // "popupMenu");
...
void MainWindow::eventKeyDown(SDL_KeyboardEvent key) {
  ...
  // printf("In mainwindow checking keypress for key [%d] mod [%d] modvalue:
  ...
  if (program != NULL && program->isInSpecialKeyCaptureEvent() == false) {
    ...
    if (isKeyPressed(configKeys.getSDLKey("HotKeyShowDebug"), key) == true) {
      ...
      if (keystate.mod & (KMOD_LALT | KMOD_RALT)) {
        ...
        renderer.setShowDebugUI(!showDebugUI);
               ...
               isKeyPressed(configKeys.getSDLKey("SwitchLanguage"), key) ==
                   true) {
    ...
    } else if (isKeyPressed(configKeys.getSDLKey("ReloadINI"), key,
                            modifiersToCheck) == true) {
      Config &config = Config::getInstance();
      config.reload();
    } else if (isKeyPressed(configKeys.getSDLKey("Screenshot"), key,
                            modifiersToCheck) == true) {
      ...
      if (isdir(path.c_str()) == true) {
        ...
        unsigned int queueSize =
            Renderer::getInstance().getSaveScreenQueueSize();

        for (int i = 0; i < 5000; ++i) {
          path = userData + GameConstants::folder_path_screenshots;
          path += string("screen") + intToStr(i + queueSize) + string(".") +
                  fileFormat;
#ifdef WIN32
          ...
          if (f == NULL) {
            ...
            if (lang.getString("ScreenshotSavedTo").length() > 0 &&
              ...
              snprintf(szBuf, 8096, lang.getString("ScreenshotSavedTo").c_str(),
                       path.c_str());
            } else {
            ...
            bool showScreenshotSavedMsg = Config::getInstance().getBool(
                "DisableScreenshotConsoleText", "false");
            if (SystemFlags::VERBOSE_MODE_ENABLED)
...
void MainWindow::eventKeyPress(SDL_KeyboardEvent c) {
  ...
  if (program != NULL && program->isInSpecialKeyCaptureEvent() == false) {
    ...
    Config &configKeys = Config::getInstance(
        std::pair<ConfigType, ConfigType>(cfgMainKeys, cfgUserKeys));
    if (isKeyPressed(configKeys.getSDLKey("HotKeyToggleOSMouseEnabled"), c) ==
        true) {
      SystemFlags::OutputDebug(SystemFlags::debugSystem,
                               "In [%s::%s Line: %d]\n", __FILE__, __FUNCTION__,
                               __LINE__);
      ...
      SystemFlags::OutputDebug(
          ...
          __FUNCTION__, __LINE__, Window::lastShowMouseState);
    }
  }
  SystemFlags::OutputDebug(SystemFlags::debugSystem,
                           "In [%s::%s Line: %d] [%d]\n", __FILE__,
...
void MainWindow::eventWindowEvent(SDL_WindowEvent event) {
  ...
  //    if(program->getState() != NULL && dynamic_cast<Intro
  //    *>(program->getState()) != NULL) { 	printf("In eventWindowEvent
  //    skip\n"); 	return;
  //    }
  // Renderer &renderer= Renderer::getInstance();
  switch (event.event) {
...
void MainWindow::setProgram(Program *program) { this->program = program; }
...
int setupGameItemPaths(int argc, char **argv, Config *config) {
  ...
  std::map<string, string> &pathCache =
      CacheManager::getCachedItem<std::map<string, string> >(
          GameConstants::pathCacheLookupKey);
  ...
  if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_DATA_PATH]) == true) {
    ...
    hasCommandArgument(argc, argv,
                       string(GAME_ARGS[GAME_ARG_DATA_PATH]) + string("="),
                       &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_DATA_PATH]),
                         &foundParamIndIndex);
    }
    ...
    if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
      ...
      Properties::setApplicationDataPath(
  ...
  if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_INI_PATH]) == true) {
    int foundParamIndIndex = -1;
    hasCommandArgument(argc, argv,
                       string(GAME_ARGS[GAME_ARG_INI_PATH]) + string("="),
                       &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_INI_PATH]),
                         &foundParamIndIndex);
    }
    string customPath = argv[foundParamIndIndex];
    vector<string> paramPartTokens;
  ...
  if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LOG_PATH]) == true) {
    int foundParamIndIndex = -1;
    hasCommandArgument(argc, argv,
                       string(GAME_ARGS[GAME_ARG_LOG_PATH]) + string("="),
                       &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_LOG_PATH]),
                         &foundParamIndIndex);
    }
    string customPath = argv[foundParamIndIndex];
    vector<string> paramPartTokens;
  ...
  if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_FONT_PATH]) == true) {
    int foundParamIndIndex = -1;
    hasCommandArgument(argc, argv,
                       string(GAME_ARGS[GAME_ARG_FONT_PATH]) + string("="),
                       &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_FONT_PATH]),
                         &foundParamIndIndex);
    }
    string customPath = argv[foundParamIndIndex];
    vector<string> paramPartTokens;
...
void setupLogging(Config &config, bool haveSpecialOutputCommandLineOption) {
  ...
  if (errorHandlerPtr.get() != NULL) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      wprintf(
          L"Hooking up google_breakpad::ExceptionHandler to save dmp files to "
          L"[%s]...\n",
          dumpfilepath.c_str());
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "Hooking up google_breakpad::ExceptionHandler to save dmp files to "
          "[%s]...\n",
          dumpFilePath.c_str());
    // errorHandlerPtr->set_dump_path(dumpfilepath);
  ...
  string debugPerformanceLogFile =
  ...
  string debugUnitCommandsLogFile =
  ...
  SystemFlags::getSystemSettingType(SystemFlags::debugSystem).debugLogFileName =
      debugLogFile;
  ...
  SystemFlags::getSystemSettingType(SystemFlags::debugLUA).debugLogFileName =
      debugLUALogFile;
  SystemFlags::getSystemSettingType(SystemFlags::debugSound).debugLogFileName =
      debugSoundLogFile;
  SystemFlags::getSystemSettingType(SystemFlags::debugError).debugLogFileName =
      debugErrorLogFile;
...
void runTilesetValidationForPath(string tilesetPath, string tilesetName,
  ...
  // Fixup paths with ..
  {
    ...
    for (std::map<string, vector<pair<string, string> > >::iterator iterMap =
             loadedFileList.begin();
  ...
  printf("Found tileset filecount = " MG_SIZE_T_SPECIFIER
  ...
  for (std::map<string, vector<pair<string, string> > >::iterator iterMap =
    ...
    if (loadedFileList.find(foundFile) == loadedFileList.end() &&
      ...
      if (foundUnusedFile == false) {
        printf(
            "\nLine ref: %d, Warning, unused files were detected - "
            "START:\n=====================\n",
            __LINE__);
      }
      ...
      if (loadedFileList.find(fileName) != loadedFileList.end()) {
        ...
        if (gitPurgeFiles == true) {
          ...
          if (gitOk == false) {
            throw megaglest_runtime_error("Call to command failed [" +
                                          string(szBuf) + "]");
          }
        } else {
    ...
    for (std::map<uint32, vector<string> >::iterator iterMap =
      ...
      if (fileList.size() > 1) {
        if (foundDuplicates == false) {
          ...
          printf(
              "\nWarning, duplicate files were detected - "
              "START:\n=====================\n");
        }
        ...
        for (unsigned int idx = 0; idx < fileList.size(); ++idx) {
          ...
          printf("[%s]\n", duplicateFile.c_str());
          std::map<string, vector<pair<string, string> > >::iterator iterFind =
        ...
        if (purgeDuplicateFiles == true) {
          ...
          for (unsigned int idx = 0; idx < fileList.size(); ++idx) {
            ...
            if (fileExt == "wav" || fileExt == "ogg") {
              off_t fileSize = getFileSize(duplicateFile);
              if (idx == 0) {
                newCommonFileName = "$COMMONDATAPATH/sounds/" +
                ...
                createDirectoryPaths(
                    extractDirectoryPathFromFile(expandedNewCommonFileName));
                ...
                if (gitPurgeFiles == true) {
                  ...
                  printf(
                      "*** Duplicate file:\n[%s]\nwas git rm and copied "
                      "to:\n[%s]\n",
                      duplicateFile.c_str(), expandedNewCommonFileName.c_str());
                } else {
                  // int result = 0;
                  int result = rename(duplicateFile.c_str(),
          ...
          for (unsigned int idx = 0; idx < fileList.size(); ++idx) {
            ...
            string fileExt = extractExtension(duplicateFile);
            if (fileExt == "wav" || fileExt == "ogg") {
              std::map<string, vector<pair<string, string> > >::iterator
                  iterFind2 = loadedFileList.find(duplicateFile);
              if (iterFind2 != loadedFileList.end()) {
                for (unsigned int jdx1 = 0; jdx1 < iterFind2->second.size();
                     jdx1++) {
                  ...
                  if (mapUniqueParentList.find(parentFile) ==
                    ...
                    bool foundText = searchAndReplaceTextInFile(
                        parentFile, searchText, newCommonFileName, false);
                    printf("foundText = %d\n", foundText);
                    if (foundText == false) {
                      char szBuf[8096] = "";
                      snprintf(
                          ...
                          __LINE__, searchText.c_str(), parentFile.c_str());
                      throw megaglest_runtime_error(szBuf);
                    }
          ...
          for (unsigned int idx = 0; idx < fileList.size(); ++idx) {
            ...
            if (fileExt == "wav" || fileExt == "ogg") {
              std::map<string, vector<pair<string, string> > >::iterator
                  iterFind4 = loadedFileList.find(duplicateFile);
              if (iterFind4 != loadedFileList.end()) {
                for (unsigned int jdx = 0; jdx < iterFind4->second.size();
                     jdx++) {
                  string parentFile = iterFind4->second[jdx].first;
                  string searchText = iterFind4->second[jdx].second;

                  bool foundText = searchAndReplaceTextInFile(
                      parentFile, searchText, newCommonFileName, true);
                  if (foundText == false) {
                    char szBuf[8096] = "";
                    snprintf(szBuf, 8096,
...
void runTechValidationForPath(string techPath, string techName,
                              const std::vector<string> &filteredFactionList,
                              World &world, bool purgeUnusedFiles,
                              bool purgeDuplicateFiles, bool showDuplicateFiles,
  ...
  if (factionsList.empty() == false) {
    ...
    for (int j = 0; j < (int)factionsList.size(); ++j) {
      if (filteredFactionList.empty() == true ||
          std::find(filteredFactionList.begin(), filteredFactionList.end(),
    ...
    if (factions.empty() == false) {
      ...
      try {
        ...
        {
          std::map<string, vector<pair<string, string> > > newLoadedFileList;
          for (std::map<string, vector<pair<string, string> > >::iterator
                   iterMap = loadedFileList.begin();
               iterMap != loadedFileList.end(); ++iterMap) {
            string loadedFile = iterMap->first;
        ...
        // Validate the faction setup to ensure we don't have any bad
        // associations
        ...
        // Validate the faction resource setup to ensure we don't have any bad
        // associations
        ...
        if (showDuplicateFiles == true) {
          ...
          for (std::map<string, vector<pair<string, string> > >::iterator
            ...
            if (crcValue == 0) {
              char szBuf[8096] = "";
              snprintf(szBuf, 8096, "Error calculating CRC for file [%s]",
                       fileName.c_str());
              throw megaglest_runtime_error(szBuf);
            }
            mapDuplicateFiles[crcValue].push_back(fileName);
          ...
          for (std::map<uint32, vector<string> >::iterator iterMap =
            ...
            if (fileList.size() > 1) {
              ...
              printf(
                  "----- START duplicate files for CRC [%u] count "
              ...
              for (unsigned int idx = 0; idx < fileList.size(); ++idx) {
                ...
                printf("[%s]\n", duplicateFile.c_str());
                std::map<string, vector<pair<string, string> > >::iterator
                    iterFind = loadedFileList.find(duplicateFile);
                if (iterFind != loadedFileList.end()) {
              ...
              printf(
                  "----- Finding parents for duplicate files "
                  "[" MG_SIZE_T_SPECIFIER "] first file is [%s]\n",
              ...
              if (purgeDuplicateFiles == true) {
                printf(
                    "----- move / remove duplicate files [" MG_SIZE_T_SPECIFIER
                    "] first file is [%s]\n",
                    fileList.size(), fileList[0].c_str());
                // First move first duplicate to commondata and delete all other
                // copies
                string newCommonFileName = "";
                for (unsigned int idx = 0; idx < fileList.size(); ++idx) {
                  string duplicateFile = fileList[idx];
                  ...
                  if (fileExt == "wav" || fileExt == "ogg") {
                    ...
                    if (idx == 0) {
                      ...
                      if (gitPurgeFiles == true) {
                        ...
                        printf(
                            "moving duplicate [%s] to common data [%s] "
                            "expanded to [%s]\n",
                            duplicateFile.c_str(), newCommonFileName.c_str(),
                            expandedNewCommonFileName.c_str());
                ...
                printf(
                    "----- update XML files for duplicate files "
                    "[" MG_SIZE_T_SPECIFIER "] first file is [%s]\n",
                    fileList.size(), fileList[0].c_str());
                std::map<string, int> mapUniqueParentList;

                // Update the XML files to point to the new single copy in
                // commondata
                for (unsigned int idx = 0; idx < fileList.size(); ++idx) {
                  string duplicateFile = fileList[idx];
                  string fileExt = extractExtension(duplicateFile);
                          ...
                          if (foundText == false) {
                            ...
                            if (StartsWith(searchText, techCommonData) ==
                              ...
                              replaceAll(searchText, techCommonData,
                                         "$COMMONDATAPATH/");
                              foundText = searchAndReplaceTextInFile(
                                  parentFile, searchText, newCommonFileName,
                                  false);

                              printf(
                                  "WARNING #2 [%d] techCommonData "
                                  "check\n[%s]\n[%s]\n[%s]\n[%s]\n",
                                  foundText, parentFile.c_str(),
                                  techCommonData.c_str(), searchText.c_str(),
                                  newCommonFileName.c_str());
                    ...
                    if (iterFind4 != loadedFileList.end()) {
                      for (unsigned int jdx = 0; jdx < iterFind4->second.size();
                        ...
                        if (foundText == false) {
                          ...
                          if (foundText == false) {
                            // Check if the sound file already references
                            // commandata
                            foundText = searchAndReplaceTextInFile(
                                parentFile, newCommonFileName,
        ...
        techtree_errors = true;
        printf(
            "\n\n****ERROR**** detected while validating the techName: "
            "%s\nMESSAGE: %s\n",
      ...
      if (techtree_errors == false) {
        printf(
            "\nValidation found NO ERRORS for techPath [%s] techName [%s] "
            "factions checked (count = %d):\n",
            techPath.c_str(), techName.c_str(), (int)factions.size());
        for (set<string>::iterator it = factions.begin(); it != factions.end();
             ++it) {
          printf("Faction [%s]\n", (*it).c_str());
        }
      }
...
void runTechTranslationExtractionForPath(
    string techPath, string techName,
      ...
      try {
        ...
        if (fileExists(translationFile) == false) {
          ...
          if (txFile.is_open() == true) {
            ...
            // txFile << "FactionName_" << GameConstants::OBSERVER_SLOTNAME <<
            // "=" << GameConstants::OBSERVER_SLOTNAME << std::endl; txFile <<
            // "FactionName_" << GameConstants::RANDOMFACTION_SLOTNAME << "=" <<
            // GameConstants::RANDOMFACTION_SLOTNAME << std::endl;
            for (int index = 0; index < techtree->getTypeCount(); ++index) {
              ...
              for (int upgradeIndex = 0;
                   upgradeIndex < ft->getUpgradeTypeCount(); ++upgradeIndex) {
              ...
              for (int unitIndex = 0; unitIndex < ft->getUnitTypeCount();
                ...
                for (int levelIndex = 0; levelIndex < ut->getLevelCount();
                     ++levelIndex) {
          ...
          printf(
              "\n** Cannot product techtree translation file [%s] for techPath "
              "[%s] techName [%s] because the file already exists!\n",
              translationFile.c_str(), techPath.c_str(), techName.c_str());
        }
      } catch (const megaglest_runtime_error &ex) {
        printf(
            "\n\n****ERROR**** detected while loading the techName: "
            "%s\nMESSAGE: %s\n",
            techName.c_str(), ex.what());
      }
    }
    ...
    for (int idx = 0; idx < (int)techPaths.size(); idx++) {
      ...
      for (int idx2 = 0; idx2 < (int)techTreeFiles.size(); idx2++) {
        ...
        if (filteredTechTreeList.empty() == true ||
            std::find(filteredTechTreeList.begin(), filteredTechTreeList.end(),
...
void runTechValidationReport(int argc, char **argv) {
  ...
  if (hasCommandArgument(argc, argv,
    ...
    int foundParamIndIndex = -1;
    hasCommandArgument(
        argc, argv, string(GAME_ARGS[GAME_ARG_VALIDATE_SCENARIO]) + string("="),
        &foundParamIndIndex);
        ...
        for (int idx = 0; idx < (int)scenarioPaths.size(); idx++) {
          ...
          for (int idx2 = 0; idx2 < (int)scenarioList.size(); idx2++) {
            ...
            if (scenarioName == validateScenarioName) {
              ...
              string techName = scenarioNode->getChild("tech-tree")
                                    ->getAttribute("value")
                                    ->getValue();

              // Self Contained techtree?
              string scenarioTechtree = scenarioPath + scenarioName + "/" +
                                        techName + "/" + techName + ".xml";
              ...
              if (fileExists(scenarioTechtree) == true) {
                ...
                for (int idx = 0; idx < (int)techPaths.size(); idx++) {
                  ...
                  if (fileExists(scenarioTechtree) == true) {
                    printf(
                        "\nFound Scenario [%s] with techtree [%s] "
                        "validating...\n",
                        scenarioName.c_str(), techName.c_str());
                    runTechValidationForPath(
                        techPath, techName, filteredFactionList, world,
                        purgeUnusedFiles, showDuplicateFiles, false, false,
                        purgedMegaBytes);
  ...
  if (hasCommandArgument(argc, argv,
    ...
    int foundParamIndIndex = -1;
    hasCommandArgument(
        argc, argv, string(GAME_ARGS[GAME_ARG_VALIDATE_FACTIONS]) + string("="),
        &foundParamIndIndex);
    ...
    if (paramPartTokens.size() >= 2) {
      ...
      if (paramPartTokens.size() >= 3) {
        if (paramPartTokens[2] == "purgeunused") {
          ...
          gitPurgeFiles = true;
          printf(
              "*NOTE All unused / duplicate techtree files will be removed "
              "from git!\n");
...
void runTilesetValidationReport(int argc, char **argv) {
  ...
  if (hasCommandArgument(
          argc, argv,
          string(GAME_ARGS[GAME_ARG_VALIDATE_TILESET]) + string("=")) == true) {
    int foundParamIndIndex = -1;
    hasCommandArgument(
        argc, argv, string(GAME_ARGS[GAME_ARG_VALIDATE_TILESET]) + string("="),
        &foundParamIndIndex);
        ...
        for (int idx = 0; idx < (int)tilesetPaths.size(); idx++) {
          ...
          for (int idx2 = 0; idx2 < (int)tilesetList.size(); idx2++) {
            ...
            if (tilesetName == validateTilesetName) {
              tilesetFound = true;
              runTilesetValidationForPath(tilesetPath, tilesetName, world,
                                          purgeUnusedFiles, showDuplicateFiles,
                                          false, false, purgedMegaBytes);
            }
          }
...
void ShowINISettings(int argc, char **argv, Config &config,
                     Config &configKeys) {
  if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SHOW_INI_SETTINGS]) ==
      true) {
    ...
    for (int i = 0; i < (int)mergedMainSettings.size(); ++i) {
      ...
      if (filteredPropertyList.empty() == false) {
        if (find(filteredPropertyList.begin(), filteredPropertyList.end(),
...
void setupSteamSettings(bool steamEnabled, bool steamResetStats,
                        bool debugEnabled) {
  Config &config = Config::getInstance();
  ...
  if (steamEnabled) {
    ...
    printf(
        "Steam Integration Enabled!\nSteam User Name is [%s] Language is "
        "[%s]\n",
  ...
  try {
    {
      ...
      vector<string> maps = MapPreview::findAllValidMaps(
          ...
          if (i != j) {
            ...
            if (tileSet1 == tileSet2) {
              if (std::find(duplicateTilesetsToRename.begin(),
                            ...
                            tileSet1) == duplicateTilesetsToRename.end()) {
        ...
        for (int i = 0; i < (int)duplicateTilesetsToRename.size(); ++i) {
          ...
          if (result != 0) {
            ...
            snprintf(szBuf, 8096, "tileset [%s] in [%s]\nwas renamed to [%s]",
                     duplicateTilesetsToRename[i].c_str(), oldFile.c_str(),
                     newFile.c_str());
            ...
            if (result != 0) {
              char *errmsg = strerror(errno);
              snprintf(szBuf, 8096,
                       "Error [%s]\nCould not rename [%s] to [%s]!", errmsg,
          ...
          if (i != j) {
            ...
            if (techtree1 == techtree2) {
              if (std::find(duplicateTechtreesToRename.begin(),
                            ...
                            techtree1) == duplicateTechtreesToRename.end()) {
        ...
        for (int i = 0; i < (int)duplicateTechtreesToRename.size(); ++i) {
          ...
          if (result != 0) {
            ...
            snprintf(szBuf, 8096, "techtree [%s] in [%s]\nwas renamed to [%s]",
                     duplicateTechtreesToRename[i].c_str(), oldFile.c_str(),
                     newFile.c_str());
...
int handleCreateDataArchivesCommand(int argc, char **argv) {
  int return_value = 1;
  if (hasCommandArgument(argc, argv,
                         GAME_ARGS[GAME_ARG_CREATE_DATA_ARCHIVES]) == true) {
    int foundParamIndIndex = -1;
    hasCommandArgument(
        argc, argv,
        string(GAME_ARGS[GAME_ARG_CREATE_DATA_ARCHIVES]) + string("="),
        &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv,
    ...
    if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
      ...
      string fileArchiveCompressCommand =
      ...
      int32 fileArchiveCompressCommandSuccessResult =
          ...
          for (unsigned int j = 0; j < pathList.size(); ++j) {
            ...
            if (results2.empty() == false) {
              ...
              string compressCmd = getFullFileArchiveCompressCommand(
                  ...
                  fileArchiveCompressCommandParameters, downloadArchive,
                  techtreePath);

              printf("Running compression command: %s\n", compressCmd.c_str());

              if (executeShellCommand(
                      compressCmd, fileArchiveCompressCommandSuccessResult) ==
                  false) {
...
int handleShowCRCValuesCommand(int argc, char **argv) {
  ...
  if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SHOW_MAP_CRC]) ==
    ...
    int foundParamIndIndex = -1;
    hasCommandArgument(argc, argv,
                       string(GAME_ARGS[GAME_ARG_SHOW_MAP_CRC]) + string("="),
                       &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_SHOW_MAP_CRC]),
                         &foundParamIndIndex);
    }
    string paramValue = argv[foundParamIndIndex];
    vector<string> paramPartTokens;
  ...
  else if (hasCommandArgument(argc, argv,
                              GAME_ARGS[GAME_ARG_SHOW_TILESET_CRC]) == true) {
    int foundParamIndIndex = -1;
    hasCommandArgument(
        argc, argv, string(GAME_ARGS[GAME_ARG_SHOW_TILESET_CRC]) + string("="),
        &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv,
    ...
    if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
      ...
      return_value = 1;
                              ...
                              GAME_ARGS[GAME_ARG_SHOW_TECHTREE_CRC]) == true) {
    int foundParamIndIndex = -1;
    hasCommandArgument(
        argc, argv, string(GAME_ARGS[GAME_ARG_SHOW_TECHTREE_CRC]) + string("="),
        &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv,
    ...
    if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
      ...
      Config &config = Config::getInstance();
      uint32 crcValue = getFolderTreeContentsCheckSumRecursively(
          config.getPathListForType(ptTechs, ""), "/" + itemName + "/*", ".xml",
          NULL, true);
      if (crcValue != 0) {
      ...
      return_value = 1;
                              ...
                              GAME_ARGS[GAME_ARG_SHOW_SCENARIO_CRC]) == true) {
    int foundParamIndIndex = -1;
    hasCommandArgument(
        argc, argv, string(GAME_ARGS[GAME_ARG_SHOW_SCENARIO_CRC]) + string("="),
        &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv,
    ...
    if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
      ...
      Config &config = Config::getInstance();
      uint32 crcValue = getFolderTreeContentsCheckSumRecursively(
          config.getPathListForType(ptScenarios, ""), "/" + itemName + "/*",
          ".xml", NULL, true);
      if (crcValue != 0) {
  ...
  else if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SHOW_PATH_CRC]) ==
           true) {
    int foundParamIndIndex = -1;
    hasCommandArgument(argc, argv,
                       string(GAME_ARGS[GAME_ARG_SHOW_PATH_CRC]) + string("="),
                       &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_SHOW_PATH_CRC]),
                         &foundParamIndIndex);
    }
    ...
    if (paramPartTokens.size() >= 3 && paramPartTokens[1].length() > 0) {
      ...
      uint32 crcValue = getFolderTreeContentsCheckSumRecursively(
          itemName, itemNameFilter, NULL, true);

      printf("CRC value for path [%s] filter [%s] is [%u]\n", itemName.c_str(),
             itemNameFilter.c_str(), crcValue);
      ...
      if (paramPartTokens.size() < 3) {
        printf(
            ...
            "[%s]\n\n",
            argv[foundParamIndIndex],
            (paramPartTokens.size() >= 3 ? paramPartTokens[2].c_str() : NULL));
      }
...
int handleListDataCommand(int argc, char **argv) {
  ...
  if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LIST_MAPS]) == true) {
    int foundParamIndIndex = -1;
    hasCommandArgument(argc, argv,
                       string(GAME_ARGS[GAME_ARG_LIST_MAPS]) + string("="),
                       &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_LIST_MAPS]),
                         &foundParamIndIndex);
    }
    ...
    if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
      ...
      for (unsigned int i = 0; i < maps.size(); ++i) {
        ...
        if (itemNameFilter.find("*") != itemNameFilter.npos) {
          if (StartsWith(mapName, itemNameFilter.substr(
                                      0, itemNameFilter.find("*"))) == true) {
  ...
  else if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LIST_TECHTRESS]) ==
           true) {
    int foundParamIndIndex = -1;
    hasCommandArgument(argc, argv,
                       string(GAME_ARGS[GAME_ARG_LIST_TECHTRESS]) + string("="),
                       &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_LIST_TECHTRESS]),
                         &foundParamIndIndex);
    }
    ...
    if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
      ...
      if (cmd == "showfactions") {
        ...
        throw megaglest_runtime_error("unknown command for techtreelist [" +
                                      cmd + "]");
      }
    ...
    printf(
        "===========================================\nTotal "
        "Techtrees: " MG_SIZE_T_SPECIFIER "\n",
        results.size());

    return_value = 0;
  }

  else if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LIST_SCENARIOS]) ==
           true) {
    int foundParamIndIndex = -1;
    hasCommandArgument(argc, argv,
                       string(GAME_ARGS[GAME_ARG_LIST_SCENARIOS]) + string("="),
                       &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_LIST_SCENARIOS]),
                         &foundParamIndIndex);
    }
    ...
    if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
      ...
      for (unsigned int i = 0; i < results.size(); ++i) {
        string name = results[i];
        if (itemNameFilter.find("*") != itemNameFilter.npos) {
          if (StartsWith(name, itemNameFilter.substr(
                                   0, itemNameFilter.find("*"))) == true) {
  ...
  else if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LIST_TILESETS]) ==
           true) {
    int foundParamIndIndex = -1;
    hasCommandArgument(argc, argv,
                       string(GAME_ARGS[GAME_ARG_LIST_TILESETS]) + string("="),
                       &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_LIST_TILESETS]),
                         &foundParamIndIndex);
    }
  ...
  else if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LIST_TUTORIALS]) ==
           true) {
    int foundParamIndIndex = -1;
    hasCommandArgument(argc, argv,
                       string(GAME_ARGS[GAME_ARG_LIST_TUTORIALS]) + string("="),
                       &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_LIST_TUTORIALS]),
                         &foundParamIndIndex);
    }
...
int glestMain(int argc, char **argv) {
  ...
  if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_DISABLE_BACKTRACE]) ==
      true) {
  ...
  for (int idx = 1; idx < argc; ++idx) {
    if (hasCommandArgument(knownArgCount, (char **)&GAME_ARGS[0], argv[idx],
                           NULL, 0, true) == false) {
  ...
  if (hasCommandArgument(
    ...
    int foundParamIndIndex = -1;
    hasCommandArgument(
        argc, argv, string(GAME_ARGS[GAME_ARG_MASTERSERVER_MODE]) + string("="),
        &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv,
  ...
  if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SERVER_TITLE]) ==
    ...
    int foundParamIndIndex = -1;
    hasCommandArgument(argc, argv,
                       string(GAME_ARGS[GAME_ARG_SERVER_TITLE]) + string("="),
                       &foundParamIndIndex);
    if (foundParamIndIndex < 0) {
      hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_SERVER_TITLE]),
                         &foundParamIndIndex);
    }
    string paramValue = argv[foundParamIndIndex];
    vector<string> paramPartTokens;
  ...
  if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_OPENGL_INFO]) == true ||
      ...
      hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_VALIDATE_TILESET]) ==
          true ||
      hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_TRANSLATE_TECHTREES]) ==
          true ||
  ...
  if (haveSpecialOutputCommandLineOption == false ||
    ...
    //		printf("\nCompiled using: %s on: %s platform: %s endianness:
    //%s",getCompilerNameString().c_str(),getCompileDateTime().c_str(),getPlatformNameString().c_str(),(::Shared::PlatformByteOrder::isBigEndian()
    //== true ? "big" : "little"));
    printf("\nCompiled using: %s platform: %s endianness: %s",
           getCompilerNameString().c_str(), getPlatformNameString().c_str(),
           (::Shared::PlatformByteOrder::isBigEndian() == true ? "big"
                                                               : "little"));

    //		printf("\n\nData type sizes int8 = " MG_SIZE_T_SPECIFIER " int16
    //= " MG_SIZE_T_SPECIFIER " int32 = " MG_SIZE_T_SPECIFIER " int64 = "
    // MG_SIZE_T_SPECIFIER
    //"\n\n",sizeof(int8),sizeof(int16),sizeof(int32),sizeof(int64));
    //
    //		Config::getInstance().setBool("DebugNetworkPackets",true,true);
    //		NetworkMessageIntro data(424336, "mg_version_x","player_x", 3,
    // nmgstOk,444444, 555555, "english"); 		unsigned char *buf =
    // data.packMessage(); 		printf("\nSend packet size =
    ...
    //		SwitchSetupRequest data("factionname", 3,-1,2,"softcoder",10,
    // 11,"eng");
    //
    //		unsigned char *buf = data.packMessage();
    //		printf("\nSend packet size = %u\n%s\nTeam = %d faction [%s]
    // currentFactionIndex = %d toFactionIndex = %d [%s] [%s] %d
  ...
  if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_CURL_INFO]) == true) {
    ...
    printf("CURL version: %s [%s] SSL enabled: %d\n", curlVersion->version,
           (curlVersion->ssl_version != NULL ? curlVersion->ssl_version : ""),
           ((curlVersion->features & CURL_VERSION_SSL) == CURL_VERSION_SSL
                ? true
                : false));
    if (curlVersion->protocols != NULL && curlVersion->protocols[0] != NULL) {
      printf("protocols: ");
      for (unsigned int i = 0;
           curlVersion->protocols != NULL && curlVersion->protocols[i] != NULL;
           ++i) {
  ...
  if ((hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_VERSION]) == true ||
       ...
       hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LUA_INFO]) == true ||
       hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_CURL_INFO]) == true ||
       hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_XERCES_INFO]) ==
           true) &&
      hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_OPENGL_INFO]) ==
          false &&
      hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_VALIDATE_TECHTREES]) ==
          false &&
      hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_VALIDATE_FACTIONS]) ==
      ...
      hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LIST_TUTORIALS]) ==
          false &&
      hasCommandArgument(argc, argv,
                         GAME_ARGS[GAME_ARG_CREATE_DATA_ARCHIVES]) == false) {
    return 0;
  }
  ...
  // SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled  =
  // true; SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled
  // = true;
  ...
  try {
    ...
    if (hasCommandArgument(argc, argv,
                           GAME_ARGS[GAME_ARG_DEBUG_NETWORK_PACKETS]) == true) {
      printf("*NOTE: debugging network packets.\n");
      config.setBool("DebugNetworkPackets", true, true);
    }

    if (hasCommandArgument(argc, argv,
                           GAME_ARGS[GAME_ARG_DEBUG_NETWORK_PACKET_SIZES]) ==
        true) {
    ...
    if (hasCommandArgument(argc, argv,
                           GAME_ARGS[GAME_ARG_DEBUG_NETWORK_PACKET_STATS]) ==
        true) {
    ...
    if (hasCommandArgument(argc, argv,
                           GAME_ARGS[GAME_ARG_ENABLE_NEW_PROTOCOL]) == true) {
      printf("*NOTE: enabling new network protocol.\n");
      NetworkMessage::useOldProtocol = false;
    }

    Socket::setBroadCastPort(config.getInt(
        "BroadcastPort", intToStr(Socket::getBroadCastPort()).c_str()));
    ...
    Socket::DEFAULT_SOCKET_SENDBUF_SIZE =
        config.getInt("DefaultSocketSendBufferSize",
                      intToStr(Socket::DEFAULT_SOCKET_SENDBUF_SIZE).c_str());
    if (Socket::DEFAULT_SOCKET_SENDBUF_SIZE >= 0) {
      printf(
          "*WARNING users wants to set default socket send buffer size to: "
          "%d\n",
          Socket::DEFAULT_SOCKET_SENDBUF_SIZE);
    }
    Socket::DEFAULT_SOCKET_RECVBUF_SIZE =
        config.getInt("DefaultSocketReceiveBufferSize",
                      intToStr(Socket::DEFAULT_SOCKET_RECVBUF_SIZE).c_str());
    if (Socket::DEFAULT_SOCKET_RECVBUF_SIZE >= 0) {
      printf(
          "*WARNING users wants to set default socket receive buffer size to: "
          "%d\n",
          Socket::DEFAULT_SOCKET_RECVBUF_SIZE);
    }
    ...
    string binaryNameOld = Properties::getApplicationPath() +
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_USE_PORTS]) == true) {
      int foundParamIndIndex = -1;
      hasCommandArgument(argc, argv,
                         string(GAME_ARGS[GAME_ARG_USE_PORTS]) + string("="),
                         &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_USE_PORTS]),
                           &foundParamIndIndex);
      }
      string paramValue = argv[foundParamIndIndex];
      vector<string> paramPartTokens;
    ...
    if (hasCommandArgument(argc, argv,
        ...
        true) {
      Ip ip("localhost");
      int port = Config::getInstance().getInt(
    ...
    if (hasCommandArgument(argc, argv,
                           GAME_ARGS[GAME_ARG_DISABLE_OPENGL_CAPS_CHECK]) ==
            true ||
    ...
    if (config.getBool("EnableLegacyFonts", "false") == true ||
        hasCommandArgument(argc, argv,
                           GAME_ARGS[GAME_ARG_ENABLE_LEGACYFONTS]) == true) {
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_USE_RESOLUTION]) ==
        true) {
      int foundParamIndIndex = -1;
      hasCommandArgument(
          argc, argv, string(GAME_ARGS[GAME_ARG_USE_RESOLUTION]) + string("="),
          &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv,
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_USE_COLORBITS]) ==
      ...
      int foundParamIndIndex = -1;
      hasCommandArgument(
          argc, argv, string(GAME_ARGS[GAME_ARG_USE_COLORBITS]) + string("="),
          &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv,
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_USE_DEPTHBITS]) ==
      ...
      int foundParamIndIndex = -1;
      hasCommandArgument(
          argc, argv, string(GAME_ARGS[GAME_ARG_USE_DEPTHBITS]) + string("="),
          &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv,
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_USE_FULLSCREEN]) ==
        true) {
      int foundParamIndIndex = -1;
      hasCommandArgument(
          argc, argv, string(GAME_ARGS[GAME_ARG_USE_FULLSCREEN]) + string("="),
          &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv,
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SET_GAMMA]) == true) {
      int foundParamIndIndex = -1;
      hasCommandArgument(argc, argv,
                         string(GAME_ARGS[GAME_ARG_SET_GAMMA]) + string("="),
                         &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_SET_GAMMA]),
                           &foundParamIndIndex);
      }
      string paramValue = argv[foundParamIndIndex];
      vector<string> paramPartTokens;
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_DISABLE_VIDEOS]) ==
        true) {
      VideoPlayer::setDisabled(true);
    ...
    FontGl::setDefault_fontType(
        config.getString("DefaultFont", FontGl::getDefault_fontType().c_str()));
    ...
    ::Shared::Graphics::Font::charCount =
        config.getInt("FONT_CHARCOUNT",
                      intToStr(::Shared::Graphics::Font::charCount).c_str());
    ::Shared::Graphics::Font::fontTypeName = config.getString(
        "FONT_TYPENAME", ::Shared::Graphics::Font::fontTypeName.c_str());
    ::Shared::Graphics::Font::fontIsMultibyte = config.getBool(
        "FONT_MULTIBYTE",
        intToStr(::Shared::Graphics::Font::fontIsMultibyte).c_str());
    ::Shared::Graphics::Font::fontIsRightToLeft = config.getBool(
        "FONT_RIGHTTOLEFT",
        intToStr(::Shared::Graphics::Font::fontIsRightToLeft).c_str());
    ::Shared::Graphics::Font::baseSize = config.getInt(
        "FONT_BASE_SIZE", intToStr(::Shared::Graphics::Font::baseSize).c_str());
    ::Shared::Graphics::Font::scaleFontValue = config.getFloat(
        "FONT_SCALE_SIZE",
        floatToStr(::Shared::Graphics::Font::scaleFontValue).c_str());
    ::Shared::Graphics::Font::scaleFontValueCenterHFactor = config.getFloat(
        "FONT_SCALE_CENTERH_FACTOR",
        floatToStr(::Shared::Graphics::Font::scaleFontValueCenterHFactor)
            .c_str());
    ::Shared::Graphics::Font::langHeightText = config.getString(
        "FONT_HEIGHT_TEXT", ::Shared::Graphics::Font::langHeightText.c_str());
    ::Shared::Graphics::Font::fontSupportMixedRightToLeft = config.getBool(
        "FONT_RIGHTTOLEFT_MIXED_SUPPORT",
        intToStr(::Shared::Graphics::Font::fontSupportMixedRightToLeft)
            .c_str());

    // Example values:
    // DEFAULT_CHARSET (English) = 1
    // GB2312_CHARSET (Chinese)  = 134
    ::Shared::Platform::PlatformContextGl::charSet = config.getInt(
        "FONT_CHARSET",
        intToStr(::Shared::Platform::PlatformContextGl::charSet).c_str());
    if (config.getBool("No2DMouseRendering", "false") == false) {
      showCursor(false);
      // showWindowCursorState = false;
    }
    if (config.getInt("DEFAULT_HTTP_TIMEOUT",
                      intToStr(SystemFlags::DEFAULT_HTTP_TIMEOUT).c_str()) >=
        0) {
    ...
    bool allowAltEnterFullscreenToggle = config.getBool(
        "AllowAltEnterFullscreenToggle",
        boolToStr(
            ::Shared::Platform::Window::getAllowAltEnterFullscreenToggle())
            .c_str());
    ::Shared::Platform::Window::setAllowAltEnterFullscreenToggle(
        allowAltEnterFullscreenToggle);
    ...
    SystemFlags::OutputDebug(
        SystemFlags::debugSystem,
        "In [%s::%s Line: %d] Font::charCount = %d, Font::fontTypeName [%s] "
        "Shared::Platform::PlatformContextGl::charSet = %d, "
        "Font::fontIsMultibyte = %d, fontIsRightToLeft = %d\n",
        __FILE__, __FUNCTION__, __LINE__, ::Shared::Graphics::Font::charCount,
        ::Shared::Graphics::Font::fontTypeName.c_str(),
    ...
    MenuStateMasterserver::setDisplayMessageFunction(
        ExceptionHandler::DisplayMessage);
    ...
    Config &configKeys = Config::getInstance(
        std::pair<ConfigType, ConfigType>(cfgMainKeys, cfgUserKeys),
        std::pair<string, string>(Config::glestkeys_ini_filename,
    ...
    if (config.getBool("DisableVBO", "false") == true ||
        hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_DISABLE_VBO]) ==
            true) {
    ...
    if (config.getBool("DisableVertexInterpolation", "false") ||
        hasCommandArgument(argc, argv,
                           GAME_ARGS[GAME_ARG_DISABLE_VERTEX_INTERPOLATION])) {
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_USE_LANGUAGE]) ==
      ...
      int foundParamIndIndex = -1;
      hasCommandArgument(argc, argv,
                         string(GAME_ARGS[GAME_ARG_USE_LANGUAGE]) + string("="),
                         &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_USE_LANGUAGE]),
                           &foundParamIndIndex);
      }
      string paramValue = argv[foundParamIndIndex];
      vector<string> paramPartTokens;
...
#ifdef _WIN32
      int localeBufferSize =
          GetLocaleInfo(LOCALE_SYSTEM_DEFAULT, LOCALE_SISO639LANGNAME, NULL, 0);
      wchar_t *sysLocale = new wchar_t[localeBufferSize];
      GetLocaleInfo(LOCALE_SYSTEM_DEFAULT, LOCALE_SISO639LANGNAME, sysLocale,
                    localeBufferSize);
    ...
    if (hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_AUTO_TEST])) ==
            true ||
    ...
    if (hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_AUTO_TEST])) ==
      ...
      int foundParamIndIndex = -1;
      hasCommandArgument(argc, argv,
                         string(GAME_ARGS[GAME_ARG_AUTO_TEST]) + string("="),
                         &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_AUTO_TEST]),
                           &foundParamIndIndex);
      }
      string paramValue = argv[foundParamIndIndex];
      vector<string> paramPartTokens;
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_FONT_BASESIZE]) ==
      ...
      int foundParamIndIndex = -1;
      hasCommandArgument(
          argc, argv, string(GAME_ARGS[GAME_ARG_FONT_BASESIZE]) + string("="),
          &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv,
    ...
    SystemFlags::OutputDebug(
        ...
        "In [%s::%s Line: %d] Font::charCount = %d, Font::fontTypeName [%s] "
        "Shared::Platform::PlatformContextGl::charSet = %d, "
        "Font::fontIsMultibyte = %d, Font::fontIsRightToLeft = %d\n",
        __FILE__, __FUNCTION__, __LINE__, ::Shared::Graphics::Font::charCount,
        ::Shared::Graphics::Font::fontTypeName.c_str(),
        ::Shared::Platform::PlatformContextGl::charSet,
        ::Shared::Graphics::Font::fontIsMultibyte,
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "Using Font::charCount = %d, Font::fontTypeName [%s] "
          "Shared::Platform::PlatformContextGl::charSet = %d, "
          "Font::fontIsMultibyte = %d, Font::fontIsRightToLeft = %d\n",
          ::Shared::Graphics::Font::charCount,
          ::Shared::Graphics::Font::fontTypeName.c_str(),
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_USE_FONT]) == true) {
      int foundParamIndIndex = -1;
      hasCommandArgument(argc, argv,
                         string(GAME_ARGS[GAME_ARG_USE_FONT]) + string("="),
                         &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_USE_FONT]),
                           &foundParamIndIndex);
      }
      string paramValue = argv[foundParamIndIndex];
      vector<string> paramPartTokens;
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SHOW_MAP_CRC]) ==
            true ||
        hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SHOW_TILESET_CRC]) ==
            true ||
        hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SHOW_TECHTREE_CRC]) ==
            true ||
        hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SHOW_SCENARIO_CRC]) ==
            true ||
        hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SHOW_PATH_CRC]) ==
            true) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPathFinder)
            .enabled == true) {
      renderer.setAllowRenderUnitTitles(
          SystemFlags::getSystemSettingType(SystemFlags::debugPathFinder)
              .enabled);
      SystemFlags::OutputDebug(
    ...
    for (int index = 0; index < GameConstants::maxPlayers; ++index) {
      string playerTexture = getGameCustomCoreDataPath(
          data_path,
          "data/core/faction_textures/faction" + intToStr(index) + ".tga");
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SERVER]) == true) {
      ...
      gameInitialized = true;
                   ...
                   argc, argv, string(GAME_ARGS[GAME_ARG_MASTERSERVER_MODE])) ==
               true) {
      ...
      hasCommandArgument(
          argc, argv,
          string(GAME_ARGS[GAME_ARG_AUTOSTART_LAST_SAVED_GAME]) + string("="),
          &foundParamIndIndex);
      if (foundParamIndIndex >= 0) {
        ...
        if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
          ...
          if (fileExists(fileName) == false) {
            ...
            if (getGameReadWritePath(GameConstants::path_logs_CacheLookupKey) !=
                "") {
              saveGameFile = getGameReadWritePath(
      ...
      int foundParamIndIndex = -1;
      hasCommandArgument(argc, argv,
                         string(GAME_ARGS[GAME_ARG_PREVIEW_MAP]) + string("="),
                         &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_PREVIEW_MAP]),
                           &foundParamIndIndex);
      }
      ...
      if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
        ...
        for (int i = 0; i < GameConstants::maxPlayers; ++i) {
          ...
          gameSettings->setNetworkPlayerName(
              i, GameConstants::NETWORK_SLOT_CLOSED_SLOTNAME);
        }
        ...
        gameSettings->setFactionTypeName(
            0, formatString(GameConstants::OBSERVER_SLOTNAME));
        ...
        return 1;
                                ...
                                string(GAME_ARGS[GAME_ARG_CONNECT])) == true) {
      int foundParamIndIndex = -1;
      hasCommandArgument(argc, argv,
                         string(GAME_ARGS[GAME_ARG_CONNECT]) + string("="),
                         &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_CONNECT]),
                           &foundParamIndIndex);
      }
      ...
      if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
        ...
        return 1;
                                ...
                                string(GAME_ARGS[GAME_ARG_CLIENT])) == true) {
      int foundParamIndIndex = -1;
      hasCommandArgument(argc, argv,
                         string(GAME_ARGS[GAME_ARG_CLIENT]) + string("="),
                         &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_CLIENT]),
                           &foundParamIndIndex);
      }
      string serverToConnectTo = argv[foundParamIndIndex];
      vector<string> paramPartTokens;
      ...
      int foundParamIndIndex = -1;
      hasCommandArgument(argc, argv,
                         string(GAME_ARGS[GAME_ARG_LOADSCENARIO]) + string("="),
                         &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_LOADSCENARIO]),
                           &foundParamIndIndex);
      }
      string scenarioName = argv[foundParamIndIndex];
      vector<string> paramPartTokens;
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_CONVERT_MODELS]) ==
        true) {
      int foundParamIndIndex = -1;
      hasCommandArgument(
          argc, argv, string(GAME_ARGS[GAME_ARG_CONVERT_MODELS]) + string("="),
          &foundParamIndIndex);
      if (foundParamIndIndex < 0) {
        hasCommandArgument(argc, argv,
      ...
      if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
        ...
        for (unsigned int i = 0; i < models.size(); ++i) {
          ...
          snprintf(szTextBuf, 8096,
                   "Please wait, converting models [%u of " MG_SIZE_T_SPECIFIER
                   "] ...",
                   i, models.size());

          if (CoreData::getInstance().getMenuFontBig3D() != NULL) {
            renderer.renderText3D(
                szTextBuf, CoreData::getInstance().getMenuFontBig3D(),
                Vec3f(1.f, 1.f, 0.f), (metrics.getScreenW() / 2) - 400,
                (metrics.getScreenH() / 2), true);
          } else {
            renderer.renderText(
                szTextBuf, CoreData::getInstance().getMenuFontBig(),
                Vec3f(1.f, 1.f, 0.f), (metrics.getScreenW() / 2) - 400,
                (metrics.getScreenH() / 2), true);
          }
          renderer.swapBuffers();
    ...
    if (hasCommandArgument(argc, argv,
                           GAME_ARGS[GAME_ARG_VALIDATE_TECHTREES]) == true ||
        hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_VALIDATE_FACTIONS]) ==
            true ||
        hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_VALIDATE_SCENARIO]) ==
            true) {
    ...
    if (hasCommandArgument(argc, argv,
                           GAME_ARGS[GAME_ARG_TRANSLATE_TECHTREES]) == true) {
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_VALIDATE_TILESET]) ==
        true) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d] precache thread enabled = %d\n", __FILE__,
             __FUNCTION__, __LINE__, startCRCPrecacheThread);
    ...
    bool startNativeLanguageNamesPrecacheThread =
...
#ifndef WIN32
      // Theoretically this should always be 0, but one fileno call isn't going
      // to hurt, and if we try to run somewhere that stdin isn't fd 0 then it
    ...
    while (program->isShutdownApplicationEnabled() == false &&
      ...
      if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
        if (disableheadless_console == false) {
          ...
          if (pollresult)
#else
          // This is problematic because input on Windows is not line-buffered
          // so this will return even if getline may block.  I haven't found a
          // good way to fix it, so for the moment I just strongly suggest only
          // running the server from the Python frontend, which does line buffer
          // input.  This does work okay as long as the user doesn't enter
          // characters without pressing enter, and then try to end the server
          // another way (say a remote console command), in which case we'll
          // still be waiting for the stdin EOL and hang.
      ...
      //			if(difftime(time(NULL),lastTextureLoadEvent) >=
      // 3) { 				lastTextureLoadEvent = time(NULL);
      // vector<Texture2D *> textureList =
      // preCacheThread->getPendingTextureList(1);
      // for(unsigned int i = 0; i < textureList.size(); ++i) { Texture2D *
      // factionLogo = textureList[i];
      // if(factionLogo != NULL) {
      //						printf("\n\n\n\n||||||||||||||||||||||||||
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d] starting normal application shutdown\n",
             __FILE__, __FUNCTION__, __LINE__);
    ...
    if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false) {
      ...
      if (program != NULL && program->getTryingRendererInit() == true &&
  ...
  if (soundThreadManager) {
    ...
    if (Config::getInstance().getString("FactorySound", "") != "None" &&
      ...
      for (; chronoshutdownFadeSound.getMillis() <=
             shutdownFadeSoundMilliseconds;) {
        sleep(10);
      }
    }
...
static bool MinidumpCallback(const wchar_t *dump_path,
  ...
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false) {
    wchar_t szBuf[8096];
    int bufBytes = _snwprintf(
        szBuf, 8096,
        L"An unhandled error was detected.\n\nA crash dump file has been "
        L"created in the folder:\n%s\nCrash dump filename is: %s.dmp",
...
int glestMainSEHWrapper(int argc, char **argv) {
  ...
  __try {
    ...
    if (hasCommandArgument(
            argc, argv, string(GAME_ARGS[GAME_ARG_DISABLE_SIGSEGV_HANDLER])) ==
        false) {
...
int glestMainWrapper(int argc, char **argv) {
  ...
  /*
          handler = new ExceptionHandler(const wstring& dump_path,
                                                                FilterCallback
     filter, MinidumpCallback callback, void* callback_context, int
     handler_types, MINIDUMP_TYPE dump_type, const wchar_t* pipe_name, const
     CustomClientInfo* custom_info);
  */

  // See this link about swallowed exceptions in Win 7:
  // http://blog.paulbetts.org/index.php/2010/07/20/the-case-of-the-disappearing-onload-exception-user-mode-callback-exceptions-in-x64/
  // DWORD dwFlags;
  // if (GetProcessUserModeExceptionPolicy(&dwFlags)) {
  //	SetProcessUserModeExceptionPolicy(dwFlags &
  //~PROCESS_CALLBACK_FILTER_ENABLED); // turn off bit 1
  //}

  // if(SystemFlags::VERBOSE_MODE_ENABLED) printf("Hooking up
  // google_breakpad::ExceptionHandler...\n");

#if defined(WIN32)
  wstring dumpfilepath = utf8_decode(".");
  // google_breakpad::ExceptionHandler handler(dumpfilepath, NULL,
  // MinidumpCallback, NULL, true);
  errorHandlerPtr.reset(new google_breakpad::ExceptionHandler(
      dumpfilepath, NULL, MinidumpCallback, NULL,
      google_breakpad::ExceptionHandler::HANDLER_ALL));
#else
  google_breakpad::MinidumpDescriptor descriptor(".");
  errorHandlerPtr.reset(new google_breakpad::ExceptionHandler(
      descriptor, NULL, MinidumpCallback, NULL, true, -1));
#endif

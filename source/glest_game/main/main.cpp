...
void setupLogging(Config &config, bool haveSpecialOutputCommandLineOption) {
  ...
  if (errorHandlerPtr.get() != NULL) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      wprintf(L"Hooking up google_breakpad::ExceptionHandler to save dmp files "
              L"to [%s]...\n",
              dumpfilepath.c_str());
    errorHandlerPtr->set_dump_path(dumpfilepath);
#else
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("Hooking up google_breakpad::ExceptionHandler to save dmp files "
             "to [%s]...\n",
             dumpFilePath.c_str());
    // errorHandlerPtr->set_dump_path(dumpfilepath);
                  ...
                  if (fileExt == "wav" || fileExt == "ogg") {
                    ...
                    if (idx == 0) {
                      ...
                      if (gitPurgeFiles == true) {
                        ...
                        printf("moving duplicate [%s] to common data [%s] "
                               "expanded to [%s]\n",
                        ...
                        if (mapUniqueParentList.find(parentFile) ==
                          ...
                          if (foundText == false) {
                            ...
                            if (StartsWith(searchText, techCommonData) ==
                                true) {
                              printf("WARNING #1 [%d] techCommonData "
                                     "check\n[%s]\n[%s]\n[%s]\n[%s]\n",
                              ...
                              printf("WARNING #2 [%d] techCommonData "
                                     "check\n[%s]\n[%s]\n[%s]\n[%s]\n",
                                     foundText, parentFile.c_str(),
                                     techCommonData.c_str(), searchText.c_str(),
                                     newCommonFileName.c_str());
    ...
    if (paramPartTokens.size() >= 2) {
      ...
      if (paramPartTokens.size() >= 3) {
        if (paramPartTokens[2] == "purgeunused") {
          ...
          gitPurgeFiles = true;
          printf("*NOTE All unused / duplicate techtree files will be removed "
                 "from git!\n");
...
int glestMain(int argc, char **argv) {
  ...
  if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SERVER_TITLE]) ==
    ...
    if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
      ...
      printf("\nInvalid missing server title specified on commandline [%s] "
             "value [%s]\n\n",
             argv[foundParamIndIndex],
             (paramPartTokens.size() >= 2 ? paramPartTokens[1].c_str() : NULL));
  ...
  try {
    ...
    if (Socket::DEFAULT_SOCKET_SENDBUF_SIZE >= 0) {
      printf("*WARNING users wants to set default socket send buffer size to: "
             "%d\n",
             Socket::DEFAULT_SOCKET_SENDBUF_SIZE);
    }
    ...
    if (Socket::DEFAULT_SOCKET_RECVBUF_SIZE >= 0) {
      printf("*WARNING users wants to set default socket receive buffer size "
             "to: %d\n",
             Socket::DEFAULT_SOCKET_RECVBUF_SIZE);
    }
    ...
    if (data_path_check == userDataPath_check) {
      printf("****WARNING**** your game data path and user data path are the "
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_USE_RESOLUTION]) ==
      ...
      if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
        ...
        if (paramPartTokens.size() >= 2) {
          ...
          printf("\nInvalid missing resolution settings specified on "
                 "commandline [%s] value [%s]\n\n",
                 argv[foundParamIndIndex],
                 (paramPartTokens.size() >= 2 ? paramPartTokens[1].c_str()
                                              : NULL));
          return 1;
    ...
    SystemFlags::OutputDebug(
        ...
        ::Shared::Graphics::Font::fontIsRightToLeft);
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("Using Font::charCount = %d, Font::fontTypeName [%s] "
             "Shared::Platform::PlatformContextGl::charSet = %d, "
             "Font::fontIsMultibyte = %d, Font::fontIsRightToLeft = %d\n",
             ::Shared::Graphics::Font::charCount,
             ::Shared::Graphics::Font::fontTypeName.c_str(),
    ...
    while (program->isShutdownApplicationEnabled() == false &&
      ...
      // vector<Texture2D *> textureList = preCacheThread->getPendingTextureList(1); 				for(unsigned int i = 0; i <
      // textureList.size(); ++i) { 					Texture2D
      // * factionLogo = textureList[i]; 					if(factionLogo != NULL) {
      //						printf("\n\n\n\n||||||||||||||||||||||||||

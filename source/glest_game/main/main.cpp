...
void cleanupCRCThread() {
    if (preCacheThread != NULL) {
        ...
        for (; preCacheThread->canShutdown(false) == false && difftime((long int)time(NULL), elapsed) <= MAX_THREAD_WAIT;) {
        }
        if (difftime((long int)time(NULL), elapsed) <= MAX_THREAD_WAIT) {
            ...
            for (; preCacheThread->shutdownAndWait() == false && difftime((long int)time(NULL), elapsed) <= MAX_THREAD_WAIT;) {
            }
...
static void cleanupProcessObjects() {
    ...
    for (; Thread::getThreadList().size() > 0 && difftime((long int)time(NULL), elapsed) <= 10;) {
            ...
            if (lastLazyThreadDump != (int)difftime((long int)time(NULL), elapsed)) {
                ...
                for (int i = 0; i < (int)Thread::getThreadList().size(); ++i) {
                    ...
                    printf("Thread index: %d ptr [%p] isBaseThread: %d, Name: [%s]\n", i, baseThread, (baseThread != NULL ? 1 : 0),
                           (baseThread != NULL ? baseThread->getUniqueID().c_str() : "<na>"));
                }
            }
        }
    }
...
#endif
                        process, thread, &sf, (tryThreadContext == false ? &threadContext : &ctx), 0, &SymFunctionTableAccess64, &SymGetModuleBase64, 0);
...
void stackdumper(unsigned int type, EXCEPTION_POINTERS *ep, bool fatalExit) {
    ...
    case EXCEPTION_ACCESS_VIOLATION: {
        out += string("Access violation. Illegal ") + (er->ExceptionInformation[0] ? "write" : "read") + string(" by ") + intToStr((int)er->ExceptionAddress) +
               string(" at ") + intToStr(er->ExceptionInformation[1]);
    } break;
    default: {
        out += "SEH exception thrown. Exception code: " + er->ExceptionCode + string(" at ") + intToStr((int)er->ExceptionAddress);
    }
    }
...
void MainWindow::eventMouseDown(int x, int y, MouseButton mouseButton) {
    ...
    if (popupMenu.getVisible() == true && popupMenu.mouseClick(vx, vy)) {
        ...
        if (result.first != cancelLanguageSelection) {
            ...
            for (map<string, string>::iterator iterMap = languageList.begin(); iterMap != languageList.end(); ++iterMap) {
...
void MainWindow::eventKeyDown(SDL_KeyboardEvent key) {
    ...
    if (program != NULL && program->isInSpecialKeyCaptureEvent() == false) {
        ...
        } else if ((keystate.mod & (KMOD_LCTRL | KMOD_RCTRL)) && isKeyPressed(configKeys.getSDLKey("SwitchLanguage"), key) == true) {
...
void setupLogging(Config &config, bool haveSpecialOutputCommandLineOption) {
    ...
    if (errorHandlerPtr.get() != NULL) {
        ...
        if (SystemFlags::VERBOSE_MODE_ENABLED)
            wprintf(L"Hooking up google_breakpad::ExceptionHandler to save dmp files to "
                    L"[%s]...\n",
                    dumpfilepath.c_str());
        errorHandlerPtr->set_dump_path(dumpfilepath);
#else
        if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("Hooking up google_breakpad::ExceptionHandler to save dmp files to "
                   "[%s]...\n",
                   dumpFilePath.c_str());
        // errorHandlerPtr->set_dump_path(dumpfilepath);
    ...
    if (haveSpecialOutputCommandLineOption == false) {
        if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("--- Startup log settings are ---\ndebugSystem "
                   ...
                   "\ndebugLUA [%d][%s]\ndebugSound [%d][%s]\ndebugError [%d][%s]\n",
                   SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled, debugLogFile.c_str(),
                   SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled, debugNetworkLogFile.c_str(),
                   SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled, debugPerformanceLogFile.c_str(),
                   SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled, debugWorldSynchLogFile.c_str(),
                   SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled, debugUnitCommandsLogFile.c_str(),
                   SystemFlags::getSystemSettingType(SystemFlags::debugPathFinder).enabled, debugPathFinderLogFile.c_str(),
                   SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled, debugLUALogFile.c_str(),
                   SystemFlags::getSystemSettingType(SystemFlags::debugSound).enabled, debugSoundLogFile.c_str(),
                   SystemFlags::getSystemSettingType(SystemFlags::debugError).enabled, debugErrorLogFile.c_str());
    }
}

void runTilesetValidationForPath(string tilesetPath, string tilesetName, World &world, bool purgeUnusedFiles, bool purgeDuplicateFiles, bool showDuplicateFiles,
    ...
    // Fixup paths with ..
    {
        ...
        for (std::map<string, vector<pair<string, string>>>::iterator iterMap = loadedFileList.begin(); iterMap != loadedFileList.end(); ++iterMap) {
    ...
    for (std::map<string, vector<pair<string, string>>>::iterator iterMap = foundFileList.begin(); iterMap != foundFileList.end(); ++iterMap) {
        string foundFile = iterMap->first;
        replaceAll(foundFile, "//", "/");
        replaceAll(foundFile, "\\\\", "\\");

        if (loadedFileList.find(foundFile) == loadedFileList.end() && foundFile.find("lang/") == foundFile.npos) {
    ...
    if (showDuplicateFiles == true) {
        ...
        for (std::map<uint32, vector<string>>::iterator iterMap = mapDuplicateFiles.begin(); iterMap != mapDuplicateFiles.end(); ++iterMap) {
            ...
            if (fileList.size() > 1) {
                ...
                for (map<string, int>::iterator iterMap1 = parentList.begin(); iterMap1 != parentList.end(); ++iterMap1) {
                            ...
                            if (idx == 0) {
                                ...
                                if (gitPurgeFiles == true) {
                                    ...
                                    if (result != 0) {
                                        ...
                                        snprintf(szBuf, 8096, "!!! Error [%s] Could not rename [%s] to [%s]!", errmsg, duplicateFile.c_str(),
                            ...
                            if (iterFind2 != loadedFileList.end()) {
                                for (unsigned int jdx1 = 0; jdx1 < iterFind2->second.size(); jdx1++) {
                                    ...
                                    if (mapUniqueParentList.find(parentFile) == mapUniqueParentList.end()) {
                                        ...
                                        if (foundText == false) {
                                            char szBuf[8096] = "";
                                            snprintf(szBuf, 8096, "Line ref = %d, Error finding text [%s] in file [%s]", __LINE__, searchText.c_str(),
                                                     parentFile.c_str());
                                            throw megaglest_runtime_error(szBuf);
                                        }
...
void runTechValidationForPath(string techPath, string techName, const std::vector<string> &filteredFactionList, World &world, bool purgeUnusedFiles,
                              bool purgeDuplicateFiles, bool showDuplicateFiles, bool gitPurgeFiles, double &purgedMegaBytes) {
    ...
    if (factionsList.empty() == false) {
        ...
        if (factions.empty() == false) {
            ...
            try {
                ...
                {
                    std::map<string, vector<pair<string, string>>> newLoadedFileList;
                    for (std::map<string, vector<pair<string, string>>>::iterator iterMap = loadedFileList.begin(); iterMap != loadedFileList.end();
                         ++iterMap) {
                        string loadedFile = iterMap->first;
                ...
                for (unsigned int i = 0; i < pathList.size(); ++i) {
                    ...
                    for (unsigned int j = 0; j < foundFiles.size(); ++j) {
                        ...
                        if (file.find(GameConstants::LOADING_SCREEN_FILE) != string::npos || file.find(GameConstants::PREVIEW_SCREEN_FILE) != string::npos ||
                                                    ...
                                                    if (foundText == false) {
                                                        ...
                                                        if (StartsWith(searchText, techCommonData) == true) {
                                                            printf("WARNING #1 [%d] techCommonData "
                                                                   "check\n[%s]\n[%s]\n[%s]\n[%s]\n",
                                                                   foundText, parentFile.c_str(), techCommonData.c_str(), searchText.c_str(),
...
void runTechTranslationExtractionForPath(string techPath, string techName, const std::vector<string> &filteredFactionList, World &world) {
    vector<string> factionsList;
        ...
        for (int idx = 0; idx < (int)techPaths.size(); idx++) {
            ...
            for (int idx2 = 0; idx2 < (int)techTreeFiles.size(); idx2++) {
                ...
                if (filteredTechTreeList.empty() == true ||
                    ...
                    runTechTranslationExtractionForPath(techPath, techName, filteredFactionList, world);
                }
            }
        }
                    ...
                    for (int idx2 = 0; idx2 < (int)scenarioList.size(); idx2++) {
                        ...
                        if (scenarioName == validateScenarioName) {
                            ...
                            if (fileExists(scenarioTechtree) == true) {
                                ...
                                runTechValidationForPath(techPath, techName, filteredFactionList, world, purgeUnusedFiles, showDuplicateFiles, false, false,
                                                         purgedMegaBytes);
                            } else {
                                vector<string> techPaths = config.getPathListForType(ptTechs);
                                for (int idx = 0; idx < (int)techPaths.size(); idx++) {
                                    ...
                                    if (fileExists(scenarioTechtree) == true) {
                                        printf("\nFound Scenario [%s] with techtree [%s] "
                                               "validating...\n",
                                               scenarioName.c_str(), techName.c_str());
                                        runTechValidationForPath(techPath, techName, filteredFactionList, world, purgeUnusedFiles, showDuplicateFiles, false,
...

    {
        ...
        for (int idx = 0; idx < (int)techPaths.size(); idx++) {
            ...
            for (int idx2 = 0; idx2 < (int)techTreeFiles.size(); idx2++) {
                ...
                if (filteredTechTreeList.empty() == true ||
                    std::find(filteredTechTreeList.begin(), filteredTechTreeList.end(), techName) != filteredTechTreeList.end()) {
                    runTechValidationForPath(techPath, techName, filteredFactionList, world, purgeUnusedFiles, purgeDuplicateFiles, showDuplicateFiles,
                                             gitPurgeFiles, purgedMegaBytes);
                }
            }
        }
                ...
                for (int idx = 0; idx < (int)tilesetPaths.size(); idx++) {
                    ...
                    for (int idx2 = 0; idx2 < (int)tilesetList.size(); idx2++) {
                        ...
                        if (tilesetName == validateTilesetName) {
                            tilesetFound = true;
                            runTilesetValidationForPath(tilesetPath, tilesetName, world, purgeUnusedFiles, showDuplicateFiles, false, false, purgedMegaBytes);
                        }
                    }
                }
            ...
            if (duplicateTilesetsToRename.empty() == false) {
                ...
                for (int i = 0; i < (int)duplicateTilesetsToRename.size(); ++i) {
                    ...
                    if (result != 0) {
                        ...
                        snprintf(szBuf, 8096, "tileset [%s] in [%s]\nwas renamed to [%s]", duplicateTilesetsToRename[i].c_str(), oldFile.c_str(),
                                 newFile.c_str());
                ...
                for (int j = 0; j < (int)techTrees.size(); ++j) {
                    if (i != j) {
                        ...
                        if (techtree1 == techtree2) {
                            if (std::find(duplicateTechtreesToRename.begin(), duplicateTechtreesToRename.end(), techtree1) ==
            ...
            if (duplicateTechtreesToRename.empty() == false) {
                ...
                for (int i = 0; i < (int)duplicateTechtreesToRename.size(); ++i) {
                    ...
                    if (result != 0) {
                        ...
                        snprintf(szBuf, 8096, "techtree [%s] in [%s]\nwas renamed to [%s]", duplicateTechtreesToRename[i].c_str(), oldFile.c_str(),
                                 newFile.c_str());
                ...
                for (unsigned int i = 0; i < results.size(); ++i) {
                    ...
                    for (unsigned int j = 0; j < pathList.size(); ++j) {
                        ...
                        if (results2.empty() == false) {
                            ...
                            string compressCmd = getFullFileArchiveCompressCommand(fileArchiveCompressCommand, fileArchiveCompressCommandParameters,
...
int handleShowCRCValuesCommand(int argc, char **argv) {
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SHOW_MAP_CRC]) == true) {
        ...
        if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
            ...
            Config &config = Config::getInstance();
            uint32 crcValue =
                getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTilesets, ""), string("/") + itemName + string("/*"), ".xml", NULL, true);
            if (crcValue != 0) {
...
int glestMain(int argc, char **argv) {
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_HELP]) == true || foundInvalidArgs == true) {
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_OPENGL_INFO]) == true || hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SDL_INFO]) == true ||
        hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LUA_INFO]) == true || hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_CURL_INFO]) == true ||
        hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_XERCES_INFO]) == true || hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_VERSION]) == true ||
        ...
        hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LIST_MAPS]) == true || hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LIST_TECHTRESS]) == true ||
    ...
    if (haveSpecialOutputCommandLineOption == false || hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_VERSION]) == true) {
        ...
        //== true ? "big" : "little"));
        printf("\nCompiled using: %s platform: %s endianness: %s", getCompilerNameString().c_str(), getPlatformNameString().c_str(),
               (::Shared::PlatformByteOrder::isBigEndian() == true ? "big" : "little"));
    ...
    if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_CURL_INFO]) == true) {
        ...
        printf("CURL version: %s [%s] SSL enabled: %d\n", curlVersion->version, (curlVersion->ssl_version != NULL ? curlVersion->ssl_version : ""),
               ((curlVersion->features & CURL_VERSION_SSL) == CURL_VERSION_SSL ? true : false));
    ...
    if ((hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_VERSION]) == true || hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_SDL_INFO]) == true ||
         hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LUA_INFO]) == true || hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_CURL_INFO]) == true ||
        ...
        hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LIST_MAPS]) == false && hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_LIST_TECHTRESS]) == false &&
    ...
    try {
        ...
        setupSteamSettings(hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_STEAM]), hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_STEAM_RESET_STATS]),
        ...
        if (Socket::DEFAULT_SOCKET_RECVBUF_SIZE >= 0) {
            printf("*WARNING users wants to set default socket receive buffer size to: "
                   "%d\n",
        ...
        if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_DISABLE_OPENGL_CAPS_CHECK]) == true || config.getBool("CheckGlCaps") == false) {
        ...
        ::Shared::Graphics::Font::scaleFontValueCenterHFactor =
            config.getFloat("FONT_SCALE_CENTERH_FACTOR", floatToStr(::Shared::Graphics::Font::scaleFontValueCenterHFactor).c_str());
        ...
        ::Shared::Graphics::Font::fontSupportMixedRightToLeft =
            config.getBool("FONT_RIGHTTOLEFT_MIXED_SUPPORT", intToStr(::Shared::Graphics::Font::fontSupportMixedRightToLeft).c_str());
        ...
        bool allowAltEnterFullscreenToggle =
            config.getBool("AllowAltEnterFullscreenToggle", boolToStr(::Shared::Platform::Window::getAllowAltEnterFullscreenToggle()).c_str());
        ...
        SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                 ...
                                 __FILE__, __FUNCTION__, __LINE__, ::Shared::Graphics::Font::charCount, ::Shared::Graphics::Font::fontTypeName.c_str(),
                                 ::Shared::Platform::PlatformContextGl::charSet, ::Shared::Graphics::Font::fontIsMultibyte,
        ...
        if (hasCommandArgument(argc, argv, string(GAME_ARGS[GAME_ARG_AUTO_TEST])) == true || Config::getInstance().getBool("AutoTest", "false") == true) {
        ...
        if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("Using Font::charCount = %d, Font::fontTypeName [%s] "
                   "Shared::Platform::PlatformContextGl::charSet = %d, "
                   "Font::fontIsMultibyte = %d, Font::fontIsRightToLeft = %d\n",
                   ::Shared::Graphics::Font::charCount, ::Shared::Graphics::Font::fontTypeName.c_str(), ::Shared::Platform::PlatformContextGl::charSet,
                   ::Shared::Graphics::Font::fontIsMultibyte, ::Shared::Graphics::Font::fontIsRightToLeft);
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugPathFinder).enabled == true) {
            ...
            SystemFlags::OutputDebug(SystemFlags::debugPathFinder, "In [%s::%s Line: %d] renderer.setAllowRenderUnitTitles = %d\n", __FILE__, __FUNCTION__,
                                     __LINE__, SystemFlags::getSystemSettingType(SystemFlags::debugPathFinder).enabled);
        }
        ...
        if (hasCommandArgument(argc, argv, GAME_ARGS[GAME_ARG_CONVERT_MODELS]) == true) {
            ...
            if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
                ...
                if (CoreData::getInstance().getMenuFontBig3D() != NULL) {
                    renderer.renderText3D("Please wait, converting models...", CoreData::getInstance().getMenuFontBig3D(), Vec3f(1.f, 1.f, 0.f),
                                          (metrics.getScreenW() / 2) - 400, (metrics.getScreenH() / 2), true);
                } else {
                    renderer.renderText("Please wait, converting models...", CoreData::getInstance().getMenuFontBig(), Vec3f(1.f, 1.f, 0.f),
                                        (metrics.getScreenW() / 2) - 400, (metrics.getScreenH() / 2), true);
                }
                ...
                for (unsigned int i = 0; i < models.size(); ++i) {
                    ...
                    if (CoreData::getInstance().getMenuFontBig3D() != NULL) {
                        renderer.renderText3D(szTextBuf, CoreData::getInstance().getMenuFontBig3D(), Vec3f(1.f, 1.f, 0.f), (metrics.getScreenW() / 2) - 400,
                                              (metrics.getScreenH() / 2), true);
                    } else {
                        renderer.renderText(szTextBuf, CoreData::getInstance().getMenuFontBig(), Vec3f(1.f, 1.f, 0.f), (metrics.getScreenW() / 2) - 400,
                                            (metrics.getScreenH() / 2), true);
                    }
                    renderer.swapBuffers();
        ...
        if (startCRCPrecacheThread == true && GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false) {
        ...
        if (startNativeLanguageNamesPrecacheThread == true && GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false) {
        ...
        if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false) {
            ...
            if (program != NULL && program->getTryingRendererInit() == true && program->getRendererInitOk() == false) {
        ...
        if (program == NULL || program->getTryingRendererInit() == false ||
            (program->getTryingRendererInit() == true && program->getRendererInitOk() == true)) {
    ...
    if (soundThreadManager) {
        SoundRenderer &soundRenderer = SoundRenderer::getInstance();
        if (Config::getInstance().getString("FactorySound", "") != "None" && soundRenderer.isVolumeTurnedOff() == false) {
            if (SystemFlags::VERBOSE_MODE_ENABLED) printf("In [%s::%s Line: %d]\n", __FILE__, __FUNCTION__, __LINE__);
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n", __FILE__, __FUNCTION__, __LINE__);
...
static bool MinidumpCallback(const wchar_t *dump_path, const wchar_t *minidump_id, void *context, EXCEPTION_POINTERS *exinfo, MDRawAssertionInfo *assertion,
                             bool succeeded) {
    printf("\n======= In MinidumpCallback...\n");
    ...
    if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false) {
        wchar_t szBuf[8096];
        int bufBytes = _snwprintf(szBuf, 8096,

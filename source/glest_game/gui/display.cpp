...
void Display::calculateUpDimensions(int index) {
  if (index > maxUpIndex) {
    ...
    if (maxUpIndex + 1 > upCellSideCount * upCellSideCount) {
      ...
      upImageSize = static_cast<float>(imageSize) *
                        static_cast<float>(cellSideCount) /
                        static_cast<float>(upCellSideCount) +
                    0.9f;
    }
  }
}
...
void Display::setUpImage(int i, const Texture2D *image) {
  if (i >= upCellCount)
    throw megaglest_runtime_error("i>=upCellCount in Display::setUpImage");
  upImages[i] = image;
...
void Display::switchColor() { currentColor = (currentColor + 1) % colorCount; }
...
int Display::computeUpIndex(int x, int y) const {
  y = y -
      (Metrics::getInstance().getDisplayH() - upCellSideCount * upImageSize);
...
int Display::computeUpY(int index) const {
  return Metrics::getInstance().getDisplayH() -
         (index / upCellSideCount) * upImageSize - upImageSize;
}
...
void Display::saveGame(XmlNode *rootNode) const {
  ...
  //	int progressBar;
  displayNode->addAttribute("progressBar", intToStr(progressBar),
                            mapTagReplacements);
  //	int downSelectedPos;
  displayNode->addAttribute("downSelectedPos", intToStr(downSelectedPos),
                            mapTagReplacements);
  //	Vec4f colors[colorCount];
  //	int currentColor;
  displayNode->addAttribute("currentColor", intToStr(currentColor),
                            mapTagReplacements);
  //	int upCellSideCount;
  //	int upImageSize;
  //	int maxUpIndex;
...
void Display::loadGame(const XmlNode *rootNode) {
  ...
  //	int downSelectedPos;
  // displayNode->addAttribute("downSelectedPos",intToStr(downSelectedPos),
  // mapTagReplacements); 	Vec4f colors[colorCount]; 	int
  // currentColor; currentColor =
  // displayNode->getAttribute("progressBar")->getIntValue();
  //	int upCellSideCount;
  //	int upImageSize;
  //	int maxUpIndex;
}

...
void Gui::init(Game *game) {
    ...
    selection.init(this, world->getThisFactionIndex(), world->getThisTeamIndex(), game->isFlagType1BitEnabled(ft1_allow_shared_team_units));
}
...
Object *Gui::getHighlightedResourceObject() const {
    if (highlightedResourceObjectPos.x == -1) {
        ...
        if (world->getMap()->getSurfaceCell(highlightedResourceObjectPos) == NULL) {
            throw megaglest_runtime_error("world->getMap()->getSurfaceCell(highlightedResourceObjectPos) == "
                                          "NULL");
        }
...
void Gui::mouseDownLeftGraphics(int x, int y, bool prepared) {
    if (selectingPos) {
        ...
        if (prepared || (game->isValidMouseCellPos() && world->getMap()->isInsideSurface(world->getMap()->toSurfCoords(targetPos)) == true)) {
    ...
    if (!selectingPos && !selectingMeetingPoint) {
        if (selectionQuad.isEnabled()) {
            ...
            if (selection.isCommandable() && random.randRange(0, 1) == 0) {
                SoundRenderer::getInstance().playFx(selection.getFrontUnit()->getType()->getSelectionSound(),
                                                    selection.getFrontUnit()->getCurrMidHeightVector(), gameCamera->getPos());
            }
...
void Gui::giveDefaultOrders(int x, int y, const Unit *targetUnit, bool paintMouse3d) {
    ...
    if (result.first == crSuccess || result.first == crSomeFailed) {
        if (paintMouse3d) mouse3d.enable();

        if (random.randRange(0, 1) == 0) {
            SoundRenderer::getInstance().playFx(selection.getFrontUnit()->getType()->getCommandSound(), selection.getFrontUnit()->getCurrMidHeightVector(),
                                                gameCamera->getPos());
        }
    }
...
void Gui::giveTwoClickOrders(int x, int y, bool prepared) {
    ...
    if (selectingBuilding == false) {
        ...
        // selecting building
        result = commander->tryGiveCommand(&selection, activeCommandType, posObjWorld, choosenBuildingType, selectedBuildingFacing, queueKeyDown);
    }
...
void Gui::clickCommonCommand(CommandClass commandClass) {
    for (int index = 0; index < Display::downCellCount; ++index) {
        ...
        if ((ct != NULL && ct->getClass() == commandClass) || display.getCommandClass(index) == commandClass) {
        ...
        if (activeCommandType != NULL && activeCommandType->getClass() == ccBuild) {
            ...
            if (bct != NULL) {
                ...
                if (unit != NULL && unit->getFaction() != NULL) {
                    if (selection.canSelectUnitFactionCheck(unit) == true && unit->getFaction()->reqsOk(ut)) {
            ...
            if (posDisplay == cancelPos) {
                ...
                if (activeCommandType != NULL && activeCommandType->getClass() == ccBuild) {
                    ...
                    if (unit->getFaction()->isUnitLocked(bct->getBuilding(posDisplay))) {
                        ...
                        string str = lang.getString("BuildSpeed", (translatedValue == true ? "" : "english")) + ": " +
                                     intToStr(bct->getBuildSkillType()->getSpeed()) + "\n";
                        str += "" + Lang::getInstance().getString("TimeSteps", (translatedValue == true ? "" : "english")) + ": " +
...
bool Gui::isSharedCommandClass(CommandClass commandClass) {
    for (int i = 0; i < selection.getCount(); ++i) {
        ...
        if (ct == NULL || !unit->getFaction()->reqsOk(ct)) return false;
    }
    return true;
}
    ...
    if (force || (lastQuadCalcFrame + selectionCalculationFrameSkip < game->getTotalRenderFps())) {
        ...
        if (doubleClick && units.empty() == false) {
            ...
            for (int i = 0; i < world->getFaction(factionIndex)->getUnitCount(); ++i) {
                ...
                if (unit->getPos().dist(refUnit->getPosNotThreadSafe()) < doubleClickSelectionRadius && unit->getType() == refUnit->getType() &&

...
bool Selection::canSelectUnitFactionCheck(const Unit *unit) const {
    ...
    if (unit->getFactionIndex() != factionIndex) {
        if (this->allowSharedTeamUnits == false || unit->getFaction()->getTeam() != teamIndex) {
            return false;
        }
    }
...
bool Selection::select(Unit *unit, bool addToSelection) {
    ...
    if (unit != NULL) {
        ...
        if (selectedUnits.size() == 1 && selectedUnits.front()->getType()->getMultiSelect() == false) {
            clear();
        }
...
bool Selection::isObserver() const {
    return selectedUnits.size() == 1 && (teamIndex == (GameConstants::maxPlayers - 1 + fpt_Observer));
}
...
bool Selection::isCommandable() const {
    ...
    return isEmpty() == false && isEnemy() == false && (selectedUnits.size() == 1 && selectedUnits.front()->isAlive() == false) == false &&
...
bool Selection::isMeetable() const {
    return isUniform() && isCommandable() && selectedUnits.front()->getType()->getMeetingPoint();
}

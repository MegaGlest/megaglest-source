...
Renderer::Renderer()
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf(
        "In [%s::%s Line: %d] Renderer::perspFarPlane [%f] "
        "this->no2DMouseRendering [%d] this->maxConsoleLines [%d]\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        Renderer::perspFarPlane, this->no2DMouseRendering,
...
// GL_DIFFUSE, Vec4f(lightColor, 1.f).ptr()); 	glLightfv(GL_LIGHT0,
// GL_SPECULAR, Vec4f(0.0f, 0.0f, 0.f, 1.f).ptr());
//
//	glFogfv(GL_FOG_COLOR, Vec4f(fogColor*lightColor, 1.f).ptr());
//
//	assertGl();
...
static Vec2i _unprojectMap(const Vec2i &pt, const GLdouble *model,
  ...
  if (std::fabs(d) < 0.00001) throw pti_D_IS_ZERO;

  const float nd = -(norm.x * w.x + norm.y * w.y + norm.z * w.z) / d;
  if (nd < 0.0 || nd >= 1.0) throw pti_N_OVER_D_IS_OUTSIDE;
...
// than the radius 		if(frustum[i][0] * x + frustum[i][1] * y +
// frustum[i][2] * z + frustum[i][3] <= -radius ) {
//			// The distance was greater than the radius so the
// sphere is outside of the frustum 			return false;
//		}
//	}
//
  ...
  IF_DEBUG_EDITION(
	...
	assertGl();
        ...
        if (useVBORendering == true) {
          ...
          //== mapSurfaceData.end()) {
          // recalcSurface = true;
          //					//printf("#1 Calculating surface
          // for Rendering using VA's
          //[%s]\n",snapshotOfvisibleQuad.getString().c_str());
          //				}
          ///*
          //				else
          // if(iterFind->second.first.getMillis() >= 250) {
          // recalcSurface = true;
          // mapSurfaceData.erase(snapshotOfvisibleQuad.getString());
          //					//printf("#2 RE-Calculating
          // surface for Rendering using VA's
          //[%s]\n",snapshotOfvisibleQuad.getString().c_str());
          //				}
          //*/
          //		    }
          ...
          //				//printf("Calculating surface for
          ...
          //
          //					SurfaceCell *tc00=
          // map->getSurfaceCell(pos.x, pos.y);
          // SurfaceCell *tc10= map->getSurfaceCell(pos.x+1, pos.y);
          // SurfaceCell *tc01= map->getSurfaceCell(pos.x, pos.y+1);
          // SurfaceCell *tc11= map->getSurfaceCell(pos.x+1, pos.y+1);
          //
          //					if(tc00 == NULL) {
          //						throw
          // megaglest_runtime_error("tc00 == NULL");
          //					}
          //					if(tc10 == NULL) {
          //						throw
          // megaglest_runtime_error("tc10 == NULL");
          //					}
          //					if(tc01 == NULL) {
          //						throw
          // megaglest_runtime_error("tc01 == NULL");
          //					}
          //					if(tc11 == NULL) {
          //						throw
          // megaglest_runtime_error("tc11 == NULL");
          //					}
          //
          //					triangleCount+= 2;
          //					pointCount+= 4;
          ...
          //				else {
          //					Vec2f *texCoords =
          //&data.texCoords[0]; 					Vec2f
          //*texCoordsSurface	= &data.texCoordsSurface[0];
        ...
        // assert
        GLenum glresult = glGetError();  // remove when first mtex problem
                                         // solved
        if (glresult) {
          assertGl();
        }
...
void Renderer::renderObjects(const int renderFps) {
  ...
  for (int visibleIndex = (int)qCache.visibleObjectList.size() - 1;
    ...
    if (modelRenderStarted == false) {
      ...
      if (effectsOffDueToMinRender == false && shadows == sShadowMapping) {
        glActiveTexture(shadowTexUnit);
        glEnable(GL_TEXTURE_2D);
...
Vec2f Renderer::computeWaveHeight(int x, int y) {
  const World *world = game->getWorld();
  const std::vector<waterWave> &waterWaves =
      world->getTileset()->getWaterWaves();
...
float Renderer::getRawWaveHeight(int x, int y) {
  ...
  if (waterWaveCache.empty() ||
      map->getSurfaceH() * map->getSurfaceW() != (int)waterWaveCache.size()) {
    waterWaveCache.assign(map->getSurfaceH() * map->getSurfaceW(),
                          Vec2f(-1.f, -1.f));
  }
...
float Renderer::computeWaterWaveCell(int i, int j) {
  ...
  if (!waterWaves.empty()) {
    ...
    for (vector<waterWave>::size_type k = 0; k < waterWaves.size(); k++) {
      ...
      float amplitude = tc0->getWaterWaveAmplitude() *
                        (1 + 0.5f * std::abs(waterLevel - tc0->getHeight()));
      float currentCellWaveHeigth =
          std::sin((xDirection * i + yDirection * j) * wt.frequency +
                   waterWavesAnim[k]) *
  ...
  for (int j = scaledRect.p[0].y; j < scaledRect.p[1].y; ++j) {
    ...
    for (int i = scaledRect.p[0].x; i <= scaledRect.p[1].x; ++i) {
      ...
      if (cellExplored == true && tc0->getNearSubmerged()) {
        ...
        glVertex3f(static_cast<float>(i) * Map::mapScale,
...
void Renderer::renderUnits(bool airUnits, const int renderFps) {
  ...
  if (qCache.visibleQuadUnitList.empty() == false) {
    ...
    for (int visibleUnitIndex = 0;
      ...
      // SKIP_INTERPOLATION_DISTANCE) {
      model->updateInterpolationData(
          unit->getAnimProgressAsFloat(),
  ...
  if (qCache.visibleQuadUnitList.empty() == false) {
    ...
    for (int k = 0; k < 2; k++) {
      ...
      for (int visibleUnitIndex = 0;
        ...
        model->updateInterpolationVertices(
            unit->getAnimProgressAsFloat(),
            unit->isAlive() && !unit->isAnimProgressBound());
...
void Renderer::checkGlCaps() {
  ...
  if (glewIsSupported("GL_VERSION_2_1") == false) {
    ...
    message +=
        "You may solve this problem by installing your latest video card "
        "drivers";
...
void Renderer::renderHealthBar(Vec3f v, Unit *unit, float height,
  ...
  // upVector*height)).ptr()); 			glVertex3fv((v -
  // (rightVector*width + upVector*height)).ptr());
  ...
  //			glVertex3fv((v - (rightVector*width -
  // upVector*height)).ptr()); 			glVertex3fv((v -
  // (rightVector*width + upVector*height*0.0f)).ptr());

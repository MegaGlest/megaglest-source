...
void MeshCallbackTeamColor::execute(const Mesh *mesh) {
    // team color
    if (mesh->getCustomTexture() && teamTexture != NULL && MeshCallbackTeamColor::noTeamColors == false) {
        // texture 0
...
Renderer::Renderer() : BaseRenderer(), saveScreenShotThreadAccessor(new Mutex(CODE_AT_LINE)) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("In [%s::%s Line: %d] Renderer::perspFarPlane [%f] "
               ...
               extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, Renderer::perspFarPlane, this->no2DMouseRendering,
...
void Renderer::cleanupScreenshotThread() {
    if (saveScreenShotThread) {
        ...
        if (getSaveScreenQueueSize() > 0) {
            ...
            for (std::list<std::pair<string, Pixmap2D *>>::iterator iter = saveScreenQueue.begin(); iter != saveScreenQueue.end(); ++iter) {
                delete iter->second;
            }
...
void Renderer::initGame(const Game *game, GameCamera *gameCamera) {
    ...
    if (shadows == sProjected || shadows == sShadowMapping) {
        ...
        if (shadows == sShadowMapping) {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32, shadowTextureSize, shadowTextureSize, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE, NULL);
        } else {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                                         __LINE__);
            ...
            // projected
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, shadowTextureSize, shadowTextureSize, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, NULL);
        }
...
void Renderer::manageDeferredParticleSystems() {
    ...
    for (unsigned int i = 0; i < deferredParticleSystems.size(); ++i) {
        ...
        if (ps->getTextureFileLoadDeferred() != "" && ps->getTexture() == NULL) {
            CoreData::TextureSystemType textureSystemId = static_cast<CoreData::TextureSystemType>(ps->getTextureFileLoadDeferredSystemId());
...
void Renderer::end() {
    ...
    std::map<string, Texture2D *> &crcFactionPreviewTextureCache =
        CacheManager::getCachedItem<std::map<string, Texture2D *>>(GameConstants::factionPreviewTextureCacheLookupKey);
...
void Renderer::endScenario() {
    ...
    if (shadowMapHandleValid == true && (shadows == sProjected || shadows == sShadowMapping)) {
...
void Renderer::endTexture(ResourceScope rs, Texture *texture, bool mustExistInList) {
    ...
    if (rs == rsGlobal) {
        ...
        if (crcFactionPreviewTextureCache.find(textureFilename) != crcFactionPreviewTextureCache.end()) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] textureFilename [%s]\n",
...
Model *Renderer::newModel(ResourceScope rs, const string &path, bool deletePixMapAfterLoad, std::map<string, vector<pair<string, string>>> *loadedFileList,
                          string *sourceLoader) {
    if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
        return NULL;
    }
...
void Renderer::setupLighting() {
    ...
    if (timeFlow->isTotalNight()) {
        ...
        if (qCache.visibleQuadUnitList.empty() == false) {
            ...
            for (int visibleUnitIndex = 0; visibleUnitIndex < (int)qCache.visibleQuadUnitList.size() && lightCount < maxLights; ++visibleUnitIndex) {
                Unit *unit = qCache.visibleQuadUnitList[visibleUnitIndex];

                if (world->toRenderUnit(unit) && unit->getCurrMidHeightVector().dist(gameCamera->getPos()) < maxLightDist && unit->getType()->getLight() &&
                    unit->isOperative()) {
...
void Renderer::loadGameCameraMatrix() {
    ...
    if (gameCamera != NULL) {
        ...
        glTranslatef(-(gameCamera->getPos().x + gameCamera->getShakeOffset().x), -gameCamera->getPos().y,
...
static Vec2i _unprojectMap(const Vec2i &pt, const GLdouble *model, const GLdouble *projection, const GLint *viewport, const char *label = NULL) {
    ...
    const Vec3f start(a.x, a.y, a.z), stop(b.x, b.y, b.z), plane(0, 0, 0), norm(0, 1, 0), u = stop - start, w = start - plane;
    ...
    if (false) { // print debug info
        ...
        printf("%d,%d -> %f,%f,%f -> %f,%f,%f -> %f,%f,%f -> %d,%d\n", pt.x, pt.y, start.x, start.y, start.z, stop.x, stop.y, stop.z, i.x, i.y, i.z, pos.x,
               pos.y);
    }
    return pos;
}
...
void Renderer::computeVisibleQuad() {
    ...
    if (frustumChanged && SystemFlags::VERBOSE_MODE_ENABLED) {
        printf("\nCamera: %d,%d %d,%d %d,%d %d,%d\n", visibleQuad.p[0].x, visibleQuad.p[0].y, visibleQuad.p[1].x, visibleQuad.p[1].y, visibleQuad.p[2].x,
               visibleQuad.p[2].y, visibleQuad.p[3].x, visibleQuad.p[3].y);
...
void Renderer::renderMouse3d() {
    ...
    if (game == NULL) {
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, "In [%s::%s] Line: %d game->getGui()->getMouse3d() == NULL", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                 __LINE__);
        throw megaglest_runtime_error(szBuf);
    }
...
void Renderer::renderConsoleLine3D(int lineIndex, int xPosition, int yPosition, int lineHeight, Font3D *font, string stringToHightlight,
    ...
    if (lineInfo->PlayerIndex >= 0) {
        ...
        if (gameNetInterface != NULL && gameNetInterface->getGameSettings() != NULL) {
            ...
            renderTextShadow3D(headerLine, font, fontColor, xPosition, lineIndex * lineHeight + yPosition);
    ...
    renderTextShadow3D(lineInfo->text, font, fontColor, xPosition, (lineIndex * lineHeight) + yPosition);
}

void Renderer::renderConsoleLine(int lineIndex, int xPosition, int yPosition, int lineHeight, Font2D *font, string stringToHightlight,
                                 const ConsoleLineInfo *lineInfo) {
    if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
        return;
    }
    ...
    if (lineInfo->PlayerIndex >= 0) {
        ...
        if (gameNetInterface != NULL && gameNetInterface->getGameSettings() != NULL) {
            ...
            renderTextShadow(headerLine, font, fontColor, xPosition, lineIndex * lineHeight + yPosition);
    ...
    renderTextShadow(lineInfo->text, font, fontColor, xPosition, (lineIndex * lineHeight) + yPosition);
}
    ...
    if (mode == consoleFull) {
        ...
        for (int i = 0; i < console->getStoredLineCount(); ++i) {
            ...
            if (renderText3DEnabled == true) {
                renderConsoleLine3D(i, console->getXPos(), console->getYPos(), console->getLineHeight(), console->getFont3D(), console->getStringToHighlight(),
                                    &lineInfo);
            } else {
                renderConsoleLine(i, console->getXPos(), console->getYPos(), console->getLineHeight(), console->getFont(), console->getStringToHighlight(),
                                  &lineInfo);
            }
        }
        ...
        for (int i = 0; i < console->getStoredLineCount() && consoleIndex < allowedMaxLines; ++i) {
            ...
            if (lineInfo.timeStamp < starttimestamp || starttimestamp == -1) {
                if (renderText3DEnabled == true) {
                    renderConsoleLine3D(consoleIndex, console->getXPos(), console->getYPos(), console->getLineHeight(), console->getFont3D(),
                                        console->getStringToHighlight(), &lineInfo);
                } else {
                    renderConsoleLine(consoleIndex, console->getXPos(), console->getYPos(), console->getLineHeight(), console->getFont(),
                                      console->getStringToHighlight(), &lineInfo);
                }
                consoleIndex++;
            }
...
void Renderer::renderChatManager(const ChatManager *chatManager) {
    ...
    if (chatManager->getEditEnabled()) {
        ...
        if (renderText3DEnabled == true) {
            renderTextShadow3D(text, chatManager->getFont3D(), fontColor, chatManager->getXPos(), chatManager->getYPos());
        } else {
            renderTextShadow(text, chatManager->getFont(), fontColor, chatManager->getXPos(), chatManager->getYPos());
        }
    } else {
...
void Renderer::renderPerformanceStats() {
    ...
    if (renderText3DEnabled == true) {
        renderTextShadow3D(str, CoreData::getInstance().getDisplayFontSmall3D(), fontColor, 10, metrics.getVirtualH() - 180, false);
    } else {
        renderTextShadow(str, CoreData::getInstance().getDisplayFontSmall(), fontColor, 10, metrics.getVirtualH() - 180, false);
    }
}
...
void Renderer::renderClock() {
    ...
    if (config.getBool("InGameClock", "true") == true) {
        ...
        snprintf(szBuf, 500, "%s %s", lang.getString("GameDurationTime", "", true).c_str(),
                 getTimeDuationString(world->getFrameCount(), GameConstants::updateFps).c_str());
        if (str != "") {
            str += " ";
        }
        str += szBuf;
    ...
    if (renderText3DEnabled == true) {
        renderTextShadow3D(str, CoreData::getInstance().getDisplayFontSmall3D(), fontColor, 10, metrics.getVirtualH() - 160, false);
    } else {
        renderTextShadow(str, CoreData::getInstance().getDisplayFontSmall(), fontColor, 10, metrics.getVirtualH() - 160, false);
    }
}
...
void Renderer::renderResourceStatus() {
    ...
    bool sharedTeamUnits = game != NULL && game->getGui() != NULL && game->isFlagType1BitEnabled(ft1_allow_shared_team_units) == true;
    bool sharedTeamResources = game != NULL && game->getGui() != NULL && game->isFlagType1BitEnabled(ft1_allow_shared_team_resources) == true;
    ...
    if (renderSharedTeamResources == true) {
        ...
        for (int techTreeResourceTypeIndex = 0; techTreeResourceTypeIndex < world->getTechTree()->getResourceTypeCount(); ++techTreeResourceTypeIndex) {
            ...
            if (showResource == true) {
                rowsRendered = renderResource(thisFaction, false, twoRessourceLines, rt, 0, resourceCountRendered);
            }
        }
    ...
    if (renderLocalFactionResources == true) {
        ...
        for (int techTreeResourceTypeIndex = 0; techTreeResourceTypeIndex < world->getTechTree()->getResourceTypeCount(); ++techTreeResourceTypeIndex) {
            ...
            if (twoRessourceLines)
                showResource = world->showResourceTypeForTeam(rt, factionForResourceView->getTeam());
            else
                showResource = world->showResourceTypeForFaction(rt, factionForResourceView);
            if (showResource == true) {
                renderResource(factionForResourceView, localFactionResourcesOnly, twoRessourceLines, rt, rowsRendered, resourceCountRendered);
            }
        }
        if (resourceCountRendered > 0) {
            rowsRendered++;
...
int Renderer::renderResource(const Faction *factionForResourceView, bool localFactionResourcesOnly, bool twoResourceLines, const ResourceType *rt, int startRow,
                             int &resourceCountRendered) {
    const Metrics &metrics = Metrics::getInstance();
    ...
    if (renderText3DEnabled == true) {
        renderTextShadow3D(str, CoreData::getInstance().getDisplayFontSmall3D(), resourceFontColor, resourceCol * 100 + 220,
                           resourceYStart - (resourceRowHeigth * resourceRow), false);
    } else {
        renderTextShadow(str, CoreData::getInstance().getDisplayFontSmall(), resourceFontColor, resourceCol * 100 + 220,
                         resourceYStart - (resourceRowHeigth * resourceRow), false);
    }
...
Vec2i computeCenteredPos(const string &text, Font2D *font, int x, int y) {
    ...
    Vec2i textPos(x - metrics.toVirtualX(virtualX), y - metrics.toVirtualY(virtualY));
...
void Renderer::renderTextSurroundingBox(int x, int y, int w, int h, int maxEditWidth, int maxEditRenderWidth) {
    // glColor4fv(color.ptr());
...
void Renderer::renderLabel(GraphicLabel *label, const Vec4f *color) {
    ...
    try {
        ...
        for (unsigned int i = 0; i < lines.size(); ++i) {
            ...
            if (color != NULL) {
                if (renderText3DEnabled == true) {
                    ...
                    renderTextBoundingBox3D(renderTextStr, label->getFont3D(), (*color), x, y, w, h, label->getCenteredW(), label->getCenteredH(),
                                            label->getEditModeEnabled(), label->getMaxEditWidth(), label->getMaxEditRenderWidth());
                } else {
                ...
                if (renderText3DEnabled == true) {
                    ...
                    // [%p]\n",lines[i].c_str(),label->getFont3D()); printf("Label render
                    // E\n");
                    renderTextBoundingBox3D(renderTextStr, label->getFont3D(), GraphicComponent::getFade(), x, y, w, h, label->getCenteredW(),
                                            label->getCenteredH(), label->getEditModeEnabled(), label->getMaxEditWidth(), label->getMaxEditRenderWidth());
                } else {
        ...
        snprintf(szBuf, 8096, "In [%s::%s Line: %d]\nError [%s] For Control [%s]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
                 e.what(), label->getInstanceName().c_str());
        SystemFlags::OutputDebug(SystemFlags::debugError, szBuf);
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled) SystemFlags::OutputDebug(SystemFlags::debugSystem, szBuf);
    ...
    try {
        ...
        if (button->getEditable()) {
            if (renderText3DEnabled == true) {
                ...
                // 2), y + (h / 2), true);
                renderTextBoundingBox3D(button->getText(), button->getFont3D(), color, x, y, w, h, true, true, false, -1, -1);
            } else {
            ...
            if (renderText3DEnabled == true) {
                ...
                //        x + (w / 2), y + (h / 2), true);
                renderTextBoundingBox3D(button->getText(), button->getFont3D(), disabledTextColor, x, y, w, h, true, true, false, -1, -1);
            } else {
                renderText(button->getText(), button->getFont(), disabledTextColor, x + (w / 2), y + (h / 2), true);
            }
        }
        ...
        snprintf(szBuf, 8096,
                 ...
                 "[%s]\n",
                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, e.what(), button->getContainerName().c_str(),
...
void Renderer::renderListBox(GraphicListBox *listBox) {
    ...
    if (listBox->getLeftControlled() == true) {
        label.init(listBox->getX() + listBox->getButton1()->getW() + listBox->getButton2()->getW() + 2, listBox->getY(), listBox->getW(), listBox->getH(),
                   false, listBox->getTextColor());
    } else {
...
void Renderer::renderComboBox(GraphicComboBox *comboBox) {
    ...
    if (comboBox->isDropDownShowing()) {
        ...
        if (comboBox->getPopupButtons()->size() != 0) {
            for (int i = comboBox->getScrollbar()->getVisibleStart(); i <= comboBox->getScrollbar()->getVisibleEnd(); ++i) {
...
void Renderer::renderMessageBox(GraphicMessageBox *messageBox) {
    ...
    try {
        ...
        if ((renderText3DEnabled == false && messageBox->getFont() == NULL) || (renderText3DEnabled == true && messageBox->getFont3D() == NULL)) {
        ...
        if (renderText3DEnabled == true) {
            // text
            renderTextShadow3D(wrappedText, messageBox->getFont3D(), fontColor, messageBox->getX() + 15,
                               messageBox->getY() + messageBox->getH() - headerHeight * 2, false);

            renderTextShadow3D(messageBox->getHeader(), messageBox->getFont3D(), fontColor, messageBox->getX() + 15,
                               messageBox->getY() + messageBox->getH() - headerHeight + 8, false);

        } else {
            // text
            renderTextShadow(wrappedText, messageBox->getFont(), fontColor, messageBox->getX() + 15, messageBox->getY() + messageBox->getH() - headerHeight * 2,
                             false);

            renderTextShadow(messageBox->getHeader(), messageBox->getFont(), fontColor, messageBox->getX() + 15,
                             messageBox->getY() + messageBox->getH() - headerHeight + 8, false);
        }
    } catch (const exception &e) {
        char szBuf[8096] = "";
...
void Renderer::MapRenderer::loadVisibleLayers(float coordStep, VisibleQuadContainerCache &qCache) {
    ...
    for (int visibleIndex = 0; visibleIndex < (int)qCache.visibleScaledCellList.size(); ++visibleIndex) {
...
void Renderer::MapRenderer::load(float coordStep) {
    ...
    for (int y = 0; y < map->getSurfaceH() - 1; y++) {
        for (int x = 0; x < map->getSurfaceW() - 1; x++) {
            ...
            SurfaceCell *tc[4] = {map->getSurfaceCell(x, y), map->getSurfaceCell(x + 1, y), map->getSurfaceCell(x, y + 1), map->getSurfaceCell(x + 1, y + 1)};
            int textureHandle = static_cast<const Texture2DGl *>(tc[0]->getSurfaceTexture())->getHandle();
            string texturePath = static_cast<const Texture2DGl *>(tc[0]->getSurfaceTexture())->getPath();
            // int32 textureCRC = CalculatePixelsCRC(static_cast<const
            // Texture2DGl*>(tc[0]->getSurfaceTexture()));
...
void Renderer::MapRenderer::Layer::render(VisibleQuadContainerCache &qCache) {
    ...
    if (rowsToRender.empty() == false) {
        ...
        for (unsigned int i = 0; i < rowsToRender.size(); ++i) {
            ...
            glDrawRangeElements(GL_TRIANGLES, rowsToRender[i].first, rowsToRender[i].second, indexCount, GL_UNSIGNED_INT,
...
void Renderer::MapRenderer::renderVisibleLayers(const Map *map, float coordStep, VisibleQuadContainerCache &qCache) {
    ...
    for (Layers::iterator layer = layers.begin(); layer != layers.end(); ++layer) (*layer)->renderVisibleLayer();
    glDisableClientState(GL_VERTEX_ARRAY);
    glBindBuffer(GL_ARRAY_BUFFER_ARB, 0);
...
void Renderer::MapRenderer::render(const Map *map, float coordStep, VisibleQuadContainerCache &qCache) {
    ...
    glEnableClientState(GL_VERTEX_ARRAY);
    glEnableClientState(GL_NORMAL_ARRAY);
    for (Layers::iterator layer = layers.begin(); layer != layers.end(); ++layer) (*layer)->render(qCache);
    glDisableClientState(GL_VERTEX_ARRAY);
    glBindBuffer(GL_ARRAY_BUFFER_ARB, 0);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER_ARB, 0);
    glDisableClientState(GL_NORMAL_ARRAY);
...
void Renderer::renderSurface(const int renderFps) {
    ...
    IF_DEBUG_EDITION(
        if (getDebugRenderer().willRenderSurface()) { getDebugRenderer().renderSurface(visibleQuad / Map::cellScale); } else {
        )
	assertGl();
        ...
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, fowTex->getPixmapConst()->getW(), fowTex->getPixmapConst()->getH(), GL_ALPHA, GL_UNSIGNED_BYTE,
...
void Renderer::renderObjects(const int renderFps) {
    ...
    // tileset objects
    for (int visibleIndex = (int)qCache.visibleObjectList.size() - 1; visibleIndex >= 0; --visibleIndex) {
    ...
    for (int j = scaledRect.p[0].y; j < scaledRect.p[1].y; ++j) {
        ...
        for (int i = scaledRect.p[0].x; i <= scaledRect.p[1].x; ++i) {
            ...
            if (cellExplored == true && tc0->getNearSubmerged()) {
                ...
                // vertex 1
                glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, computeWaterColor(waterLevel, tc1->getHeight()).ptr());
                glMultiTexCoord2fv(GL_TEXTURE1, tc1->getFowTexCoord().ptr());
                glTexCoord3f(i, 1.f, waterAnim);
                glVertex3f(static_cast<float>(i) * Map::mapScale, waterLevel + waveHeight.y, static_cast<float>(j + 1) * Map::mapScale);

                // vertex 2
                glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, computeWaterColor(waterLevel, tc0->getHeight()).ptr());
                glMultiTexCoord2fv(GL_TEXTURE1, tc0->getFowTexCoord().ptr());
                glTexCoord3f(i, 0.f, waterAnim);
                glVertex3f(static_cast<float>(i) * Map::mapScale, waterLevel + waveHeight.x, static_cast<float>(j) * Map::mapScale);
...
void Renderer::renderTeamColorCircle() {
    ...
    if (qCache.visibleQuadUnitList.empty() == false) {
        ...
        for (int visibleUnitIndex = 0; visibleUnitIndex < (int)qCache.visibleQuadUnitList.size(); ++visibleUnitIndex) {
            Unit *unit = qCache.visibleQuadUnitList[visibleUnitIndex];
...
void Renderer::renderUnits(bool airUnits, const int renderFps) {
    ...
    if (qCache.visibleQuadUnitList.empty() == false) {
        ...
        for (int visibleUnitIndex = 0; visibleUnitIndex < (int)qCache.visibleQuadUnitList.size(); ++visibleUnitIndex) {
            ...
            if (showDebugUI == true && (showDebugUILevel & debugui_unit_titles) == debugui_unit_titles) {
...
void Renderer::renderUnitsToBuild(const int renderFps) {
    ...
    if (qCache.visibleQuadUnitBuildList.empty() == false) {
        ...
        for (int visibleUnitIndex = 0; visibleUnitIndex < (int)qCache.visibleQuadUnitBuildList.size(); ++visibleUnitIndex) {
        ...
        for (int visibleUnitIndex = 0; visibleUnitIndex < (int)qCache.visibleQuadUnitList.size(); ++visibleUnitIndex) {
            ...
            if (unit->getCurrSkill() != NULL && unit->getCurrSkill()->getClass() == scMorph) {
                ...
                if (command != NULL && command->getCommandType()->commandTypeClass == ccMorph) {
                    ...
                    if (mType->getSize() > unit->getType()->getSize() || mType->getField() != unit->getType()->getField()) {
                        if (!initialized) {
                            const World *world = game->getWorld();
    ...
    for (int i = 0; i < selection->getCount(); ++i) {
        ...
        if (unit != NULL) {
            ...
            if (world->getThisFactionIndex() == unit->getFactionIndex()) {
                if (showDebugUI == true && ((showDebugUILevel & debugui_unit_titles) == debugui_unit_titles) && unit->getCommandSize() > 0 &&
    ...
    bool settingsWantToRenderThem =
        ...
        ((healthbarVisible & hbvAlways) ||
         ...
         ((healthbarVisible & hbvIfNeeded) && unit->getType()->getMaxEp() > 0 &&
          unit->getEp() < unit->getType()->getMaxEp() + unit->getTotalUpgrade()->getMaxEp()) ||
         ((healthbarVisible & hbvIfNeeded) && unit->getProductionPercent() > 0) || ((healthbarVisible & hbvSelected) && game->getGui()->isSelected(unit)));
...
void Renderer::renderMinimap() {
    ...
    Vec2f zoom = Vec2f(static_cast<float>(mw) / pixmap->getW(), static_cast<float>(mh) / pixmap->getH());
    ...
    if (visibleUnitList.empty() == false) {
        ...
        for (int visibleIndex = 0; visibleIndex < (int)visibleUnitList.size(); ++visibleIndex) {
...
void Renderer::renderHighlightedCellsOnMinimap() {
    ...
    if (highlightedCells->empty() == false) {
        ...
        Vec2f zoom = Vec2f(static_cast<float>(mw) / pixmap->getW() / 2, static_cast<float>(mh) / pixmap->getH() / 2);
...
void Renderer::renderMarkedCellsOnMinimap() {
    ...
    if (markedCells.empty() == false) {
        ...
        for (std::map<Vec2i, MarkedCell>::iterator iterMap = markedCells.begin(); iterMap != markedCells.end(); ++iterMap) {
            MarkedCell &bm = iterMap->second;
            if (bm.getPlayerIndex() < 0 || (bm.getFaction() != NULL && bm.getFaction()->getTeam() == game->getWorld()->getThisFaction()->getTeam())) {
...
void Renderer::renderVisibleMarkedCells(bool renderTextHint, int x, int y) {
    ...
    if (markedCells.empty() == false) {
        ...
        for (std::map<Vec2i, MarkedCell>::iterator iterMap = markedCells.begin(); iterMap != markedCells.end(); ++iterMap) {
            ...
            if (bm.getPlayerIndex() < 0 || (bm.getFaction() != NULL && bm.getFaction()->getTeam() == game->getWorld()->getThisFaction()->getTeam())) {
                const Map *map = game->getWorld()->getMap();
                std::pair<bool, Vec3f> bmVisible = posInCellQuadCache(map->toSurfCoords(bm.getTargetPos()));
                if (bmVisible.first == true) {
                    if (renderTextHint == true) {
                        if (bm.getNote() != "") {
                            bool validPosObjWorld = x > bmVisible.second.x && y > bmVisible.second.y + yOffset &&
                            ...
                            if (validPosObjWorld) {
                                ...
                                if (renderText3DEnabled == true) {
                                    renderTextShadow3D(bm.getNote(), CoreData::getInstance().getConsoleFont3D(), fontColor, bmVisible.second.x,
                                                       bmVisible.second.y);
                                } else {
                                    renderTextShadow(bm.getNote(), CoreData::getInstance().getConsoleFont(), fontColor, bmVisible.second.x, bmVisible.second.y);
                                }
                            }
                        }
                    } else {
            ...
            */
                        ...
                        renderTextureQuad(bmVisible.second.x, bmVisible.second.y + yOffset, texture->getTextureWidth(), texture->getTextureHeight(), texture,
                                          0.8f, &color);
...
void Renderer::renderDisplay() {
    ...
    if (renderText3DEnabled == true) {
        // infoString
        renderTextShadow3D(display->getInfoText().c_str(), coreData.getDisplayFont3D(), display->getColor(), metrics.getDisplayX(),
                           metrics.getDisplayY() + Display::infoStringY);

        // title
        renderTextShadow3D(display->getTitle().c_str(), coreData.getDisplayFont3D(), display->getColor(), metrics.getDisplayX() + 40,
                           metrics.getDisplayY() + metrics.getDisplayH() - 20);

        glColor3f(0.0f, 0.0f, 0.0f);

        // text
        renderTextShadow3D(display->getText().c_str(), coreData.getDisplayFont3D(), display->getColor(), metrics.getDisplayX() - 1,
                           metrics.getDisplayY() + metrics.getDisplayH() - 56);

        // progress Bar
        if (display->getProgressBar() != -1) {
            renderProgressBar3D(display->getProgressBar(), metrics.getDisplayX(), metrics.getDisplayY() + metrics.getDisplayH() - 50,
                                coreData.getDisplayFontSmall3D(), 175);
        }
    } else {
        // infoString
        renderTextShadow(display->getInfoText().c_str(), coreData.getDisplayFont(), display->getColor(), metrics.getDisplayX(),
                         metrics.getDisplayY() + Display::infoStringY);

        // title
        renderTextShadow(display->getTitle().c_str(), coreData.getDisplayFont(), display->getColor(), metrics.getDisplayX() + 40,
                         metrics.getDisplayY() + metrics.getDisplayH() - 20);

        glColor3f(0.0f, 0.0f, 0.0f);

        // text
        renderTextShadow(display->getText().c_str(), coreData.getDisplayFont(), display->getColor(), metrics.getDisplayX() - 1,
                         metrics.getDisplayY() + metrics.getDisplayH() - 56);

        // progress Bar
        if (display->getProgressBar() != -1) {
            renderProgressBar(display->getProgressBar(), metrics.getDisplayX(), metrics.getDisplayY() + metrics.getDisplayH() - 50,
    ...
    for (int i = 0; i < Display::upCellCount; ++i) {
        if (display->getUpImage(i) != NULL) {
            renderQuad(metrics.getDisplayX() + display->computeUpX(i), metrics.getDisplayY() + display->computeUpY(i), display->getUpImageSize(),
                       display->getUpImageSize(), display->getUpImage(i));
        }
    }
...
void Renderer::renderMenuBackground(const MenuBackground *menuBackground) {
    ...
    if (dist < minDist) {
        ...
        for (int i = 0; i < MenuBackground::characterCount; ++i) {
            ...
            gluProject(worldPos.x, worldPos.y, worldPos.z, modelview, projection, viewport, &screen_coords[0], &screen_coords[1], &screen_coords[2]);
...
bool Renderer::ccomputePosition(const Vec2i &screenPos, Vec2i &worldPos, bool exactCoords) {
    ...
    gluUnProject(screenX, screenY, depth, modelviewMatrix, projectionMatrix, viewport, &worldX, &worldY, &worldZ);
...
Vec3f Renderer::computeScreenPosition(const Vec3f &worldPos) {
    ...
    GLdouble screenY;
    GLdouble screenZ;
    gluProject(worldX, worldY, worldZ, modelviewMatrix, projectionMatrix, viewport, &screenX, &screenY, &screenZ);
...
void Renderer::computeSelected(Selection::UnitContainer &units, const Object *&obj, const bool withObjectSelection, const Vec2i &posDown, const Vec2i &posUp) {
...
void Renderer::selectUsingFrustumSelection(Selection::UnitContainer &units, const Object *&obj, const bool withObjectSelection, const Vec2i &posDown,
                                           const Vec2i &posUp) {
    ...
    if (qCache.visibleQuadUnitList.empty() == false) {
        for (int visibleUnitIndex = 0; visibleUnitIndex < (int)qCache.visibleQuadUnitList.size(); ++visibleUnitIndex) {
            ...
            if (unit != NULL && unit->isAlive()) {
                ...
                bool insideQuad = CubeInFrustum(quadSelectionCacheItem.frustumData, unitPos.x, unitPos.y, unitPos.z, unit->getType()->getRenderSize());
                if (insideQuad == true) {
    ...
    if (withObjectSelection == true) {
        if (qCache.visibleObjectList.empty() == false) {
            for (int visibleIndex = 0; visibleIndex < (int)qCache.visibleObjectList.size(); ++visibleIndex) {
                Object *object = qCache.visibleObjectList[visibleIndex];
                if (object != NULL) {
                    bool insideQuad = CubeInFrustum(quadSelectionCacheItem.frustumData, object->getPos().x, object->getPos().y, object->getPos().z, 1);
                    if (insideQuad == true) {
                        obj = object;
                        // if(withObjectSelection == true) {
                        break;
...
void Renderer::selectUsingSelectionBuffer(Selection::UnitContainer &units, const Object *&obj, const bool withObjectSelection, const Vec2i &posDown,
                                          const Vec2i &posUp) {
    // compute center and dimensions of selection rectangle
    int x = (posDown.x + posUp.x) / 2;
    int y = (posDown.y + posUp.y) / 2;
    ...
    if (renderModeResult < 0) {
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, "OpenGL error #%d [0x%X] : [%s] at file: [%s], line: %d", renderModeResult, renderModeResult, errorString,
    ...
    if (selCount > 0) {
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, "OpenGL error #%d [0x%X] : [%s] at file: [%s], line: %d", selCount, selCount, errorString,
                 extractFileFromDirectoryPath(__FILE__).c_str(), __LINE__);
...
void Renderer::selectUsingColorPicking(Selection::UnitContainer &units, const Object *&obj, const bool withObjectSelection, const Vec2i &posDown,
                                       const Vec2i &posUp) {
    int x1 = posDown.x;
    int y1 = posDown.y;
    int x2 = posUp.x;
...
void Renderer::renderShadowsToTexture(const int renderFps) {
    ...
    if (effectsOffDueToMinRender == false && (shadows == sProjected || shadows == sShadowMapping)) {
...
void Renderer::saveScreen(const string &path, int w, int h) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
    glReadPixels(0, 0, pixmapScreenShot->getW(), pixmapScreenShot->getH(), GL_RGB, GL_UNSIGNED_BYTE, pixmapScreenShot->getPixels());
...
void Renderer::checkGlCaps() {
    ...
    if (glewIsSupported("GL_VERSION_2_1") == false) {
        ...
        message += "You may solve this problem by installing your latest video card "
                   "drivers";
...
void Renderer::internalRenderHp(int numberOfBars, int barNumber, float hp, Vec3f posVector, float width, float singleHPheight, Vec3f rightVector,
                                Vec3f upVector) {
...
void Renderer::renderProgressBar3D(int size, int x, int y, Font3D *font, int customWidth, string prefixLabel, bool centeredText, int customHeight) {
    if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
        return;
    }
    ...
    renderTextBoundingBox3D(renderText, font, defColor, x, y, maxSize, progressbarHeight, true, true, false, -1, -1);
}

void Renderer::renderProgressBar(int size, int x, int y, Font2D *font, int customWidth, string prefixLabel, bool centeredText) {
    if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
        return;
    }
            ...
            if (unit != NULL) {
                if (unit->getVisible() == true) {
                    if (unit->getCurrentUnitTitle() != "") {
                        ...
                        string str = unit->getFullName(unit->showTranslatedTechTree()) + " - " + intToStr(unit->getId()) + " [" +
...
void Renderer::removeUnitFromQuadCache(const Unit *unit) {
    VisibleQuadContainerCache &qCache = getQuadCache();
    for (int visibleIndex = 0; visibleIndex < (int)qCache.visibleQuadUnitList.size(); ++visibleIndex) {
    ...
    for (int visibleIndex = 0; visibleIndex < (int)qCache.visibleUnitList.size(); ++visibleIndex) {
...
VisibleQuadContainerCache &Renderer::getQuadCache(bool updateOnDirtyFrame, bool forceNew) {
    // forceNew = true;
    if (game != NULL && game->getWorld() != NULL) {
        const World *world = game->getWorld();

        if (forceNew == true || (updateOnDirtyFrame == true && (world->getFrameCount() != quadCache.cacheFrame || visibleQuad != quadCache.lastVisibleQuad))) {
            // Dump cached info
            ...
            for (int i = 0; i < world->getFactionCount(); ++i) {
                ...
                for (int j = 0; j < faction->getUnitCount(); ++j) {
                    ...
                    if (VisibleQuadContainerCache::enableFrustumCalcs == true) {
                        ...
                        bool insideQuad = CubeInFrustum(quadCache.frustumData, unit->getCurrMidHeightVector().x, unit->getCurrMidHeightVector().y,
                                                        unit->getCurrMidHeightVector().z, unit->getType()->getRenderSize());
            ...
            if (forceNew == true || visibleQuad != quadCache.lastVisibleQuad) {
                ...
                for (int visibleIndex = 0; visibleIndex < (int)quadCache.visibleObjectList.size(); ++visibleIndex) {
...
std::pair<bool, Vec3f> Renderer::posInCellQuadCache(Vec2i pos) {
    ...
    if (std::find(quadCache.visibleScaledCellList.begin(), quadCache.visibleScaledCellList.end(), pos) != quadCache.visibleScaledCellList.end()) {
        result.first = true;
...
void Renderer::renderMapPreview(const MapPreview *map, bool renderAll, int screenPosX, int screenPosY, Texture2D **renderToTexture) {
    if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
        return;
    }
    ...
    for (int i = 0; i < map->getMaxFactions(); i++) {
        ...
        vertices[(i * 4) + 1] =
            Vec2f((map->getStartLocationX(i) + 1) * cellSize + playerCrossSize, clientH - (map->getStartLocationY(i) + 1) * cellSize - playerCrossSize);
...
void Renderer::cycleShowDebugUILevel() {
    ...
    //	showDebugUI = true;
    // }
    if ((showDebugUILevel & debugui_fps) != debugui_fps || (showDebugUILevel & debugui_unit_titles) != debugui_unit_titles) {
        showDebugUILevel |= debugui_fps;
...
void Renderer::renderFPSWhenEnabled(int lastFps) {
    if (getShowDebugUI() == true) {
        ...
        if (Renderer::renderText3DEnabled) {
            renderText3D("FPS: " + intToStr(lastFps), coreData.getMenuFontNormal3D(), Vec3f(1.f), 10, 10, false);
        } else {
...
void Renderer::renderPopupMenu(PopupMenu *menu) {
    ...
    if (renderText3DEnabled == true) {
        // text
        renderTextBoundingBox3D(menu->getHeader(), menu->getFont3D(), fontColor, menu->getX(), menu->getY() + 93 * menu->getH() / 100, menu->getW(), 0, true,
                                false, false, -1, -1);
        ...
        renderTextShadow(menu->getHeader(), menu->getFont(), fontColor, renderX, renderY, true);
...
void Renderer::renderVideoLoading(int progressPercent) {
    ...
    if (CoreData::getInstance().getMenuFontBig3D() != NULL) {
        ...
        renderText3D(textToRender, CoreData::getInstance().getMenuFontBig3D(), Vec4f(1.f, 1.f, 0.f, anim), renderX, renderY, false);
    } else {
        renderText(textToRender, CoreData::getInstance().getMenuFontBig(), Vec4f(1.f, 1.f, 0.f, anim), (metrics.getScreenW() / 2), (metrics.getScreenH() / 2),
                   true);
    }
    swapBuffers();

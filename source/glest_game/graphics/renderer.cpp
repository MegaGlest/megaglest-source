...
enum PROJECTION_TO_INFINITY { pti_D_IS_ZERO, pti_N_OVER_D_IS_OUTSIDE };
...
void MeshCallbackTeamColor::execute(const Mesh *mesh) {
  ...
  if (mesh->getCustomTexture() && teamTexture != NULL &&
    ...
    glBindTexture(GL_TEXTURE_2D,
                  static_cast<const Texture2DGl *>(teamTexture)->getHandle());
    glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_COMBINE);
...
const Vec4f Renderer::defAmbientColor =
    Vec4f(1.f * ambFactor, 1.f * ambFactor, 1.f * ambFactor, 1.f);
...
Renderer::Renderer()
    : BaseRenderer(), saveScreenShotThreadAccessor(new Mutex(CODE_AT_LINE)) {
  ...
  Renderer::perspFarPlane = config.getFloat(
      "PerspectiveFarPlane", floatToStr(Renderer::perspFarPlane).c_str());
  this->no2DMouseRendering = config.getBool("No2DMouseRendering", "false");
  this->maxConsoleLines = config.getInt("ConsoleMaxLines");

  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("In [%s::%s Line: %d] Renderer::perspFarPlane [%f] "
           "this->no2DMouseRendering [%d] this->maxConsoleLines [%d]\n",
           extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
           __LINE__, Renderer::perspFarPlane, this->no2DMouseRendering,
           this->maxConsoleLines);
...
void Renderer::cleanupScreenshotThread() {
  if (saveScreenShotThread) {
    ...
    //			getSaveScreenQueueSize() > 0 && difftime((long
    // int)time(NULL),elapsed) <= 7;) { 			sleep(0);
    //		}
    ...
    if (getSaveScreenQueueSize() > 0) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(
            ...
            "In [%s::%s Line %d] FORCING MEMORY CLEANUP and NOT SAVING "
            "screenshots, saveScreenQueue.size() = %d\n",
            extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
            __LINE__, saveScreenQueue.size());

      static string mutexOwnerId =
          string(extractFileFromDirectoryPath(__FILE__).c_str()) + string("_") +
          intToStr(__LINE__);
      MutexSafeWrapper safeMutex(saveScreenShotThreadAccessor, mutexOwnerId);
      for (std::list<std::pair<string, Pixmap2D *>>::iterator iter =
               saveScreenQueue.begin();
...
void Renderer::simpleTask(BaseThread *callingThread, void *userdata) {
  ...
  if (saveScreenQueue.empty() == false) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugSystem,
          "In [%s::%s Line %d] saveScreenQueue.size() = %d\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, saveScreenQueue.size());
...
bool Renderer::isEnded() { return Renderer::rendererEnded; }
...
void Renderer::init() {
  ...
  if (glActiveTexture == NULL) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "Error: glActiveTexture == NULL\nglActiveTexture is only "
             "supported if the GL version is 1.3 or greater,\nor if the "
...
void Renderer::initGame(const Game *game, GameCamera *gameCamera) {
  ...
  VisibleQuadContainerCache::enableFrustumCalcs =
      Config::getInstance().getBool("EnableFrustrumCalcs", "true");
  quadCache = VisibleQuadContainerCache();
  quadCache.clearFrustumData();
  ...
  if (shadows == sProjected || shadows == sShadowMapping) {
    ...
    if (game != NULL) {
      shadowIntensity =
          shadowIntensity * game->getWorld()->getTileset()->getShadowIntense();
    ...
    if (shadows == sShadowMapping) {
      ...
      // GL_TEXTURE_COMPARE_FAIL_VALUE_ARB, 1.0f-shadowAlpha);

      glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32, shadowTextureSize,
                   shadowTextureSize, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE,
                   NULL);
    } else {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem,
      ...
      // projected
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, shadowTextureSize,
                   shadowTextureSize, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, NULL);
    }
...
void Renderer::manageDeferredParticleSystems() {
  ...
  //		printf("deferredParticleSystems.size() =
  //%d\n",(int)deferredParticleSystems.size());
  //	}

  for (unsigned int i = 0; i < deferredParticleSystems.size(); ++i) {
    std::pair<ParticleSystem *, ResourceScope> &deferredParticleSystem =
        deferredParticleSystems[i];
    ...
    if (ps->getTextureFileLoadDeferred() != "" && ps->getTexture() == NULL) {
      ...
      // printf("Load DEFERRED particle i = %d textureSystemId =
      // %d\n",i,textureSystemId);

      if (textureSystemId != CoreData::tsyst_NONE) {
        Texture2D *texture =
            CoreData::getInstance().getTextureBySystemId(textureSystemId);
        // printf("Loading texture from system [%d]
        // [%p]\n",textureSystemId,texture);
        ps->setTexture(texture);

        // printf("#2 Load DEFERRED particle i = %d textureSystemId = %d,
        // texture = %p\n",i,textureSystemId,texture);
      } else {
        Texture2D *texture = newTexture2D(rs);
        if (texture) {
          texture->setFormat(ps->getTextureFileLoadDeferredFormat());
          texture->getPixmap()->init(
              ps->getTextureFileLoadDeferredComponents());
        }
        if (texture) {
        ...
        // printf("#3 Load DEFERRED particle i = %d textureSystemId = %d,
        // texture = %p\n",i,textureSystemId,texture);
      }
    }
    if (dynamic_cast<GameParticleSystem *>(ps) != NULL) {
      GameParticleSystem *gps = dynamic_cast<GameParticleSystem *>(ps);
      if (gps != NULL && gps->getModelFileLoadDeferred() != "" &&
          gps->getModel() == NULL) {
        std::map<string, vector<pair<string, string>>> loadedFileList;
        Model *model = newModel(rsGame, gps->getModelFileLoadDeferred(), false,
                                &loadedFileList, NULL);
        if (model)
          gps->setModel(model);
      }
  ...
  // printf("After deferredParticleSystems.size() =
...
void Renderer::reset3dMenu() {
  ...
  // printf("In [%s::%s Line: %d] this->custom_mm3d [%p] this->mm3d
...
void Renderer::end() {
  ...
  std::map<string, Texture2D *> &crcFactionPreviewTextureCache =
      CacheManager::getCachedItem<std::map<string, Texture2D *>>(
          GameConstants::factionPreviewTextureCacheLookupKey);
...
void Renderer::endTexture(ResourceScope rs, Texture *texture,
  ...
  if (rs == rsGlobal) {
    std::map<string, Texture2D *> &crcFactionPreviewTextureCache =
        CacheManager::getCachedItem<std::map<string, Texture2D *>>(
            GameConstants::factionPreviewTextureCacheLookupKey);
    if (crcFactionPreviewTextureCache.find(textureFilename) !=
        crcFactionPreviewTextureCache.end()) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                 "In [%s::%s Line: %d] textureFilename [%s]\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__,
                                 textureFilename.c_str());
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("In [%s::%s Line: %d] free texture from cache [%s]\n",
               extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
               __LINE__, textureFilename.c_str());
...
Model *Renderer::newModel(
    ResourceScope rs, const string &path, bool deletePixMapAfterLoad,
    std::map<string, vector<pair<string, string>>> *loadedFileList,
    string *sourceLoader) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return NULL;
  ...
  return modelManager[rs]->newModel(path, deletePixMapAfterLoad, loadedFileList,
                                    sourceLoader);
}
...
void Renderer::endFont(::Shared::Graphics::Font *font, ResourceScope rs,
                       bool mustExistInList) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  }
...
void Renderer::addToDeferredParticleSystemList(
    std::pair<ParticleSystem *, ResourceScope> deferredParticleSystem) {
...
bool Renderer::validateParticleSystemStillExists(ParticleSystem *particleSystem,
                                                 ResourceScope rs) const {
  return particleManager[rs]->validateParticleSystemStillExists(particleSystem);
}

void Renderer::removeParticleSystemsForParticleOwner(
    ParticleOwner *particleOwner, ResourceScope rs) {
  particleManager[rs]->removeParticleSystemsForParticleOwner(particleOwner);
}

void Renderer::cleanupParticleSystems(vector<ParticleSystem *> &particleSystems,
                                      ResourceScope rs) {
  particleManager[rs]->cleanupParticleSystems(particleSystems);
}

void Renderer::cleanupUnitParticleSystems(
    vector<UnitParticleSystem *> &particleSystems, ResourceScope rs) {
...
void Renderer::swapBuffers() {
  ...
  // glFlush(); // should not be required -
  // http://www.opengl.org/wiki/Common_Mistakes glFlush();
...
void Renderer::setupLighting() {
  ...
  Vec4f lightPos =
      timeFlow->isDay() ? computeSunPos(time) : computeMoonPos(time);
  nearestLightPos = lightPos;

  glLightfv(GL_LIGHT0, GL_POSITION, lightPos.ptr());
  glLightfv(GL_LIGHT0, GL_AMBIENT,
            Vec4f(lightColor * lightAmbFactor, 1.f).ptr());
  ...
  if (timeFlow->isTotalNight()) {
    ...
    if (qCache.visibleQuadUnitList.empty() == false) {
      ...
      for (int visibleUnitIndex = 0;
           visibleUnitIndex < (int)qCache.visibleQuadUnitList.size() &&
           lightCount < maxLights;
           ++visibleUnitIndex) {
        Unit *unit = qCache.visibleQuadUnitList[visibleUnitIndex];

        if (world->toRenderUnit(unit) &&
            unit->getCurrMidHeightVector().dist(gameCamera->getPos()) <
                maxLightDist &&
            unit->getType()->getLight() && unit->isOperative()) {
          // printf("$$$ Show light for faction: %s # %d / %d for Unit [%d -
          ...
          glLightfv(lightEnum, GL_AMBIENT,
                    Vec4f(unit->getType()->getLightColor()).ptr());
          glLightfv(lightEnum, GL_DIFFUSE,
                    Vec4f(unit->getType()->getLightColor()).ptr());
          glLightfv(lightEnum, GL_SPECULAR,
                    Vec4f(unit->getType()->getLightColor() * 0.3f).ptr());
          ...
          if (Vec3f(pos).dist(gameCamera->getPos()) <
...
//	Vec3f lightColor= timeFlow->computeLightColor();
//	Vec3f fogColor= world->getTileset()->getFogColor();
//	Vec4f lightPos= timeFlow->isDay()? computeSunPos(time):
// computeMoonPos(time);
//	//nearestLightPos= lightPos;
//
//	glLightfv(GL_LIGHT0, GL_POSITION, lightPos.ptr());
//	glLightfv(GL_LIGHT0, GL_AMBIENT,
// Vec4f(lightColor*lightAmbFactor, 1.f).ptr()); 	glLightfv(GL_LIGHT0,
// GL_DIFFUSE,
// Vec4f(lightColor, 1.f).ptr()); 	glLightfv(GL_LIGHT0, GL_SPECULAR,
// Vec4f(0.0f, 0.0f, 0.f, 1.f).ptr());
//
//	glFogfv(GL_FOG_COLOR, Vec4f(fogColor*lightColor, 1.f).ptr());
//
//	assertGl();
...
static Vec2i _unprojectMap(const Vec2i &pt, const GLdouble *model,
  ...
  // junk values if you were looking parallel to the XZ plane; this shouldn't
  // happen as the camera can't do this?
  const Vec3f start(a.x, a.y, a.z), stop(b.x, b.y, b.z), plane(0, 0, 0),
      norm(0, 1, 0), u = stop - start, w = start - plane;
  ...
  if (false) { // print debug info
    ...
    printf("%d,%d -> %f,%f,%f -> %f,%f,%f -> %f,%f,%f -> %d,%d\n", pt.x, pt.y,
           start.x, start.y, start.z, stop.x, stop.y, stop.z, i.x, i.y, i.z,
           pos.x, pos.y);
  }
  return pos;
}
...
bool Renderer::ExtractFrustum(VisibleQuadContainerCache &quadCacheItem) {
  ...
  const bool useFrustumCache =
      Config::getInstance().getBool("EnableFrustrumCache", "false");
  pair<vector<float>, vector<float>> lookupKey;
  if (useFrustumCache == true) {
    lookupKey = make_pair(proj, modl);
    map<pair<vector<float>, vector<float>>, vector<vector<float>>>::iterator
        iterFind = quadCacheItem.frustumDataCache.find(lookupKey);
    if (iterFind != quadCacheItem.frustumDataCache.end()) {
      ...
      frustumChanged =
          (quadCacheItem.proj != proj || quadCacheItem.modl != modl);
  ...
  if (quadCacheItem.proj != proj || quadCacheItem.modl != modl) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("\nCalc Frustum #%da: [%f][%f][%f][%f]\n", 0, frustum[0][0],
             frustum[0][1], frustum[0][2], frustum[0][3]);

    /* Normalize the result */
    t = std::sqrt(frustum[0][0] * frustum[0][0] +
    ...
    if (t != 0.0) {
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("\nCalc Frustum #%db: [%f][%f][%f][%f] t = %f\n", 0,
               frustum[0][0], frustum[0][1], frustum[0][2], frustum[0][3], t);
    }
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("\nCalc Frustum #%da: [%f][%f][%f][%f]\n", 1, frustum[1][0],
             frustum[1][1], frustum[1][2], frustum[1][3]);

    /* Normalize the result */
    t = std::sqrt(frustum[1][0] * frustum[1][0] +
    ...
    if (t != 0.0) {
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("\nCalc Frustum #%db: [%f][%f][%f][%f] t = %f\n", 1,
               frustum[1][0], frustum[1][1], frustum[1][2], frustum[1][3], t);
    }
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("\nCalc Frustum #%da: [%f][%f][%f][%f]\n", 2, frustum[2][0],
             frustum[2][1], frustum[2][2], frustum[2][3]);

    /* Normalize the result */

    t = std::sqrt(frustum[2][0] * frustum[2][0] +
    ...
    if (t != 0.0) {
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("\nCalc Frustum #%db: [%f][%f][%f][%f] t = %f\n", 2,
               frustum[2][0], frustum[2][1], frustum[2][2], frustum[2][3], t);
    }
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("\nCalc Frustum #%da: [%f][%f][%f][%f]\n", 3, frustum[3][0],
             frustum[3][1], frustum[3][2], frustum[3][3]);

    /* Normalize the result */

    t = std::sqrt(frustum[3][0] * frustum[3][0] +
    ...
    if (t != 0.0) {
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("\nCalc Frustum #%db: [%f][%f][%f][%f] t = %f\n", 3,
               frustum[3][0], frustum[3][1], frustum[3][2], frustum[3][3], t);
    }
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("\nCalc Frustum #%da: [%f][%f][%f][%f]\n", 4, frustum[4][0],
             frustum[4][1], frustum[4][2], frustum[4][3]);

    /* Normalize the result */

    t = std::sqrt(frustum[4][0] * frustum[4][0] +
    ...
    if (t != 0.0) {
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("\nCalc Frustum #%db: [%f][%f][%f][%f] t = %f\n", 4,
               frustum[4][0], frustum[4][1], frustum[4][2], frustum[4][3], t);
    }
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("\nCalc Frustum #%da: [%f][%f][%f][%f]\n", 5, frustum[5][0],
             frustum[5][1], frustum[5][2], frustum[5][3]);

    /* Normalize the result */

    t = std::sqrt(frustum[5][0] * frustum[5][0] +
    ...
    if (t != 0.0) {
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("\nCalc Frustum #%db: [%f][%f][%f][%f] t = %f\n", 5,
               frustum[5][0], frustum[5][1], frustum[5][2], frustum[5][3], t);
    }
...
// bool Renderer::PointInFrustum(vector<vector<float> > &frustum, float x, float
// y, float z ) {
//    unsigned int p=0;
//
...
// }
//
// bool Renderer::SphereInFrustum(vector<vector<float> > &frustum,  float x,
// float y, float z, float radius) {
//	// Go through all the sides of the frustum
//	for(int i = 0; i < (int)frustum.size(); i++ ) {
//		// If the center of the sphere is farther away from the plane
//than
// the radius 		if(frustum[i][0] * x + frustum[i][1] * y + frustum[i][2]
// * z + frustum[i][3] <= -radius ) {
//			// The distance was greater than the radius so the sphere
//is outside of the frustum 			return false;
//		}
//	}
//
...
bool Renderer::CubeInFrustum(vector<vector<float>> &frustum, float x, float y,
                             float z, float size) {
  unsigned int p = 0;
...
void Renderer::computeVisibleQuad() {
  ...
  if (frustumChanged && SystemFlags::VERBOSE_MODE_ENABLED) {
    printf("\nCamera: %d,%d %d,%d %d,%d %d,%d\n", visibleQuad.p[0].x,
           visibleQuad.p[0].y, visibleQuad.p[1].x, visibleQuad.p[1].y,
           visibleQuad.p[2].x, visibleQuad.p[2].y, visibleQuad.p[3].x,
           visibleQuad.p[3].y);

    for (unsigned int i = 0; i < quadCache.frustumData.size(); ++i) {
      printf("\nFrustum #%u [" MG_SIZE_T_SPECIFIER "]: ", i,
             quadCache.frustumData.size());
  ...
  if (newVisibleQuadCalc) {
    ...
    try {
      ...
      Vec2i tl = _unprojectMap(Vec2i(0, 0), model, projection, viewport, "tl"),
            tr = _unprojectMap(Vec2i(viewport[2], 0), model, projection,
                               viewport, "tr"),
            br = _unprojectMap(Vec2i(viewport[2], viewport[3]), model,
                               projection, viewport, "br"),
            bl = _unprojectMap(Vec2i(0, viewport[3]), model, projection,
                               viewport, "bl");
      ...
      if (debug)
        printf("hmm staring at the horizon %d\n", (int)e);
...
void Renderer::renderMouse2d(int x, int y, int anim, float fade) {
  ...
  if (game != NULL && game->getGui() != NULL) {
    ...
    // unit->getCurrCommand()->getCommandType()->getImage()!=NULL){
    // const Texture2D *texture =
    // unit->getCurrCommand()->getCommandType()->getImage();
    ...
    if (game->isMarkCellMode() == true) {
      ...
      renderTextureQuad(x, y, texture->getTextureWidth(),
                        texture->getTextureHeight(), texture, 0.8f);
    }
  ...
  float color2 =
      (abs(anim * (int)fadeFactor) / static_cast<float>(maxMouse2dAnim)) / 2.f +
      0.4f;
  float color1 =
      (abs(anim * (int)fadeFactor) / static_cast<float>(maxMouse2dAnim)) / 2.f +
      0.8f;
  ...
  // inside
  glColor4f(0.4f * fadeFactor, 0.2f * fadeFactor, 0.2f * fadeFactor,
            0.5f * fadeFactor);
  ...
  /*
      ...
      color2=
     (abs(anim*(int)fadeFactor)/static_cast<float>(maxMouse2dAnim))/2.f+0.4f;
      color1=
     (abs(anim*(int)fadeFactor)/static_cast<float>(maxMouse2dAnim))/2.f+0.8f;
      ...
      glEnable(GL_BLEND);
          ...
          glColor4f(0.4f*fadeFactor, 0.2f*fadeFactor, 0.2f*fadeFactor,
     0.5f*fadeFactor); glEnableClientState(GL_VERTEX_ARRAY); glVertexPointer(2,
     GL_INT, 0, &vertices[0]); glDrawArrays(GL_TRIANGLES, 0, 3);
...
void Renderer::renderMouse3d() {
  ...
  if (game == NULL) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "In [%s::%s] Line: %d game == NULL",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__);
    throw megaglest_runtime_error(szBuf);
  } else if (game->getGui() == NULL) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "In [%s::%s] Line: %d game->getGui() == NULL",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__);
    throw megaglest_runtime_error(szBuf);
    ...
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s] Line: %d game->getGui()->getMouse3d() == NULL",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__);
    throw megaglest_runtime_error(szBuf);
  }
  ...
  if (map == NULL) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "In [%s::%s] Line: %d map == NULL",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__);
    throw megaglest_runtime_error(szBuf);
  }
  ...
  if ((mouse3d->isEnabled() || gui->isPlacingBuilding()) &&
      gui->isValidPosObjWorld()) {
...
void Renderer::renderTextureQuad(int x, int y, int w, int h,
                                 const Texture2D *texture, float alpha,
                                 const Vec3f *color) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  }
...
void Renderer::renderConsoleLine3D(int lineIndex, int xPosition, int yPosition,
                                   int lineHeight, Font3D *font,
                                   string stringToHightlight,
                                   const ConsoleLineInfo *lineInfo) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  }
  ...
  if (game != NULL) {
    ...
    fontColor =
        Vec4f(lineInfo->color.x, lineInfo->color.y, lineInfo->color.z, 0.0f);
  }
  ...
  if (lineInfo->PlayerIndex >= 0) {
    ...
    Vec3f playerColor =
        crcPlayerTextureCache[lineInfo->PlayerIndex]->getPixmap()->getPixel3f(
            0, 0);
    fontColor.x = playerColor.x;
    fontColor.y = playerColor.y;
    fontColor.z = playerColor.z;

    GameNetworkInterface *gameNetInterface =
        NetworkManager::getInstance().getGameNetworkInterface();
    if (gameNetInterface != NULL &&
        gameNetInterface->getGameSettings() != NULL) {
      const GameSettings *gameSettings = gameNetInterface->getGameSettings();
      string playerName = gameSettings->getNetworkPlayerNameByPlayerIndex(
          lineInfo->PlayerIndex);
      if (playerName != lineInfo->originalPlayerName &&
          lineInfo->originalPlayerName != "") {
      ...
      renderTextShadow3D(headerLine, font, fontColor, xPosition,
                         lineIndex * lineHeight + yPosition);
      ...
      // xPosition +=
      // (metrics.toVirtualX(fontMetrics->getTextWidth(headerLine)));
  ...
  if (stringToHightlight != "" &&
      lineInfo->text.find(stringToHightlight) != string::npos) {
    fontColor = Vec4f(1.f, 0.5f, 0.5f, 0.0f);
  }
  renderTextShadow3D(lineInfo->text, font, fontColor, xPosition,
                     (lineIndex * lineHeight) + yPosition);
}

void Renderer::renderConsoleLine(int lineIndex, int xPosition, int yPosition,
                                 int lineHeight, Font2D *font,
                                 string stringToHightlight,
                                 const ConsoleLineInfo *lineInfo) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  ...
  if (lineInfo->PlayerIndex >= 0) {
    ...
    if (gameNetInterface != NULL &&
      ...
      renderTextShadow(headerLine, font, fontColor, xPosition,
                       lineIndex * lineHeight + yPosition);
  ...
  renderTextShadow(lineInfo->text, font, fontColor, xPosition,
                   (lineIndex * lineHeight) + yPosition);
}

void Renderer::renderConsole(const Console *console, ConsoleMode mode,
                             int overrideMaxConsoleLines) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  }
  ...
  if (mode == consoleFull) {
    ...
    int allowedMaxLines =
        (overrideMaxConsoleLines >= 0 ? overrideMaxConsoleLines
                                      : maxConsoleLines);
    for (int i = 0; i < console->getStoredLineCount() && i < allowedMaxLines;
         ++i) {
      const ConsoleLineInfo &lineInfo = console->getStoredLineItem(i);
      if (renderText3DEnabled == true) {
        renderConsoleLine3D(i, console->getXPos(), console->getYPos(),
    ...
    for (int i = 0; i < console->getLineCount() && i < allowedMaxLines; ++i) {
      ...
      if (starttimestamp > lineInfo.timeStamp || starttimestamp == -1)
        starttimestamp = lineInfo.timeStamp;
      if (renderText3DEnabled == true) {
        renderConsoleLine3D(i, console->getXPos(), console->getYPos(),
                            console->getLineHeight(), console->getFont3D(),
                            console->getStringToHighlight(), &lineInfo);
    ...
    for (int i = 0;
         i < console->getStoredLineCount() && consoleIndex < allowedMaxLines;
         ++i) {
      const ConsoleLineInfo &lineInfo = console->getStoredLineItem(i);
      if (lineInfo.timeStamp < starttimestamp || starttimestamp == -1) {
        if (renderText3DEnabled == true) {
          renderConsoleLine3D(consoleIndex, console->getXPos(),
                              console->getYPos(), console->getLineHeight(),
                              console->getFont3D(),
                              console->getStringToHighlight(), &lineInfo);
        } else {
          renderConsoleLine(consoleIndex, console->getXPos(),
                            console->getYPos(), console->getLineHeight(),
                            console->getFont(), console->getStringToHighlight(),
                            &lineInfo);
        }
        consoleIndex++;
      }
...
void Renderer::renderChatManager(const ChatManager *chatManager) {
  ...
  if (chatManager->getEditEnabled()) {
    ...
    if (chatManager->getInMenu()) {
      string text =
          "\t\t\t\t\t>> " + lang.getString("PressEnterToChat") + " <<";
...
void Renderer::renderPerformanceStats() {
  ...
  if (renderText3DEnabled == true) {
    renderTextShadow3D(str, CoreData::getInstance().getDisplayFontSmall3D(),
                       fontColor, 10, metrics.getVirtualH() - 180, false);
  } else {
    renderTextShadow(str, CoreData::getInstance().getDisplayFontSmall(),
                     fontColor, 10, metrics.getVirtualH() - 180, false);
  }
}
...
void Renderer::renderClock() {
  ...
  if (config.getBool("InGameClock", "true") == true) {
    ...
    // int minutes = (world->getTimeFlow()->getTime() - hours) * 100 * 0.6; //
    // scale 100 to 60 snprintf(szBuf,200,"%s
    // %.2d:%.2d",lang.getString("GameTime","",true).c_str(),hours,minutes);
    //  string header2 = lang.getString("GameDurationTime","",true) + ": " +
    //  getTimeString(stats.getFramesToCalculatePlaytime());
    snprintf(
        szBuf, 500, "%s %s",
        lang.getString("GameDurationTime", "", true).c_str(),
        getTimeDuationString(world->getFrameCount(), GameConstants::updateFps)
            .c_str());
    if (str != "") {
      str += " ";
    }
  ...
  if (config.getBool("InGameLocalClock", "true") == true) {
    ...
    Lang &lang = Lang::getInstance();
    char szBuf[200] = "";
    snprintf(szBuf, 200, "%s %s", lang.getString("LocalTime", "", true).c_str(),
             szBuf2);
    if (str != "") {
      str += " ";
    }
  ...
  if (renderText3DEnabled == true) {
    renderTextShadow3D(str, CoreData::getInstance().getDisplayFontSmall3D(),
                       fontColor, 10, metrics.getVirtualH() - 160, false);
  } else {
    renderTextShadow(str, CoreData::getInstance().getDisplayFontSmall(),
                     fontColor, 10, metrics.getVirtualH() - 160, false);
  }
...
void Renderer::renderResourceStatus() {
  ...
  bool sharedTeamUnits =
      game != NULL && game->getGui() != NULL &&
      game->isFlagType1BitEnabled(ft1_allow_shared_team_units) == true;
  bool sharedTeamResources =
      game != NULL && game->getGui() != NULL &&
  ...
  if (renderSharedTeamResources == true) {
    ...
    for (int techTreeResourceTypeIndex = 0;
      ...
      const ResourceType *rt =
          world->getTechTree()->getResourceType(techTreeResourceTypeIndex);
      ...
      bool showResource =
          world->showResourceTypeForTeam(rt, thisFaction->getTeam());
      if (showResource == true) {
        rowsRendered = renderResource(thisFaction, false, twoRessourceLines, rt,
                                      0, resourceCountRendered);
      }
    }
  ...
  if (renderLocalFactionResources == true) {
    ...
    for (int techTreeResourceTypeIndex = 0;
      ...
      if (twoRessourceLines)
        showResource = world->showResourceTypeForTeam(
            rt, factionForResourceView->getTeam());
      else
        showResource =
            world->showResourceTypeForFaction(rt, factionForResourceView);
      if (showResource == true) {
        renderResource(factionForResourceView, localFactionResourcesOnly,
                       twoRessourceLines, rt, rowsRendered,
...
int Renderer::renderResource(const Faction *factionForResourceView,
                             bool localFactionResourcesOnly,
                             bool twoResourceLines, const ResourceType *rt,
  ...
  if (localFactionResourcesOnly == true) {
    Vec4f resourceFontColor = Vec4f(
        factionForResourceView->getTexture()->getPixmapConst()->getPixel3f(0,
                                                                           0));
    int resourceCol = 0;
  ...
  const Resource *r =
      factionForResourceView->getResource(rt, localFactionResourcesOnly);
  ...
  int resourceRow =
      startRow + (resourceCountRendered > 0
                      ? resourceCountRendered / MAX_RESOURCES_PER_ROW
                      : 0);
  int resourceCol = resourceCountRendered % MAX_RESOURCES_PER_ROW;

  renderQuad(resourceCol * 100 + 200,
             resourceYStart - (resourceRowHeigth * resourceRow), 16, 16,
             rt->getImage());

  if (rt->getClass() != rcStatic) {
    str += "/" + intToStr(factionForResourceView->getStoreAmount(
                     rt, localFactionResourcesOnly));
  }
  if (rt->getClass() == rcConsumable) {
    str += "(";
    if (r->getBalance() > 0) {
  ...
  if (renderText3DEnabled == true) {
    renderTextShadow3D(str, CoreData::getInstance().getDisplayFontSmall3D(),
                       resourceFontColor, resourceCol * 100 + 220,
                       resourceYStart - (resourceRowHeigth * resourceRow),
                       false);
  } else {
    renderTextShadow(str, CoreData::getInstance().getDisplayFontSmall(),
                     resourceFontColor, resourceCol * 100 + 220,
                     resourceYStart - (resourceRowHeigth * resourceRow), false);
  }
  ++resourceCountRendered;
...
Vec2i computeCenteredPos(const string &text, Font2D *font, int x, int y) {
  ...
  int virtualX = (fontMetrics->getTextWidth(text) > 0
                      ? static_cast<int>(fontMetrics->getTextWidth(text) / 2.f)
                      : 5);
  int virtualY = (fontMetrics->getHeight(text) > 0
                      ? static_cast<int>(fontMetrics->getHeight(text) / 2.f)
                      : 5);

  Vec2i textPos(x - metrics.toVirtualX(virtualX),
                y - metrics.toVirtualY(virtualY));

  // printf("text [%s] x = %d y = %d virtualX = %d virtualY = %d
...
void Renderer::renderTextSurroundingBox(int x, int y, int w, int h,
  ...
  // printf("A w = %d maxEditWidth = %d maxEditRenderWidth =
  // %d\n",w,maxEditWidth,maxEditRenderWidth);
  if (maxEditWidth >= 0 || maxEditRenderWidth >= 0) {
    // printf("B w = %d maxEditWidth = %d maxEditRenderWidth =
    // %d\n",w,maxEditWidth,maxEditRenderWidth);
...
void Renderer::renderTextBoundingBox3D(const string &text, Font3D *font,
                                       float alpha, int x, int y, int w, int h,
                                       bool centeredW, bool centeredH,
                                       bool editModeEnabled, int maxEditWidth,
                                       int maxEditRenderWidth) {
  renderTextBoundingBox3D(text, font, Vec4f(1.f, 1.f, 1.f, alpha), x, y, w, h,
                          centeredW, centeredH, editModeEnabled,
                          maxEditRenderWidth, maxEditRenderWidth);
}

void Renderer::renderTextBoundingBox3D(const string &text, Font3D *font,
                                       const Vec3f &color, int x, int y, int w,
                                       int h, bool centeredW, bool centeredH,
                                       bool editModeEnabled, int maxEditWidth,
                                       int maxEditRenderWidth) {
  renderTextBoundingBox3D(text, font, Vec4f(color.x, color.y, color.z, 1.f), x,
                          y, w, h, centeredW, centeredH, editModeEnabled,
                          maxEditRenderWidth, maxEditRenderWidth);
}

void Renderer::renderTextBoundingBox3D(const string &text, Font3D *font,
                                       const Vec4f &color, int x, int y, int w,
                                       int h, bool centeredW, bool centeredH,
                                       bool editModeEnabled, int maxEditWidth,
                                       int maxEditRenderWidth) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
  ...
  if (editModeEnabled) {
    if (maxEditWidth >= 0 || maxEditRenderWidth >= 0) {
      ...
      float lineWidth = (font->getTextHandler()->Advance(temp.c_str()) *
...
void Renderer::renderText3D(const string &text, Font3D *font, float alpha,
                            int x, int y, bool centered) {
  renderText3D(text, font, Vec4f(1.f, 1.f, 1.f, alpha), x, y, centered);
}

void Renderer::renderText3D(const string &text, Font3D *font,
                            const Vec3f &color, int x, int y, bool centered) {
  renderText3D(text, font, Vec4f(color.x, color.y, color.z, 1.f), x, y,
               centered);
}

void Renderer::renderText3D(const string &text, Font3D *font,
                            const Vec4f &color, int x, int y, bool centered) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  }
...
// renderText
//
void Renderer::renderText(const string &text, Font2D *font, float alpha, int x,
                          int y, bool centered) {
  renderText(text, font, Vec4f(1.f, 1.f, 1.f, alpha), x, y, centered);
}

void Renderer::renderText(const string &text, Font2D *font, const Vec3f &color,
                          int x, int y, bool centered) {
  renderText(text, font, Vec4f(color.x, color.y, color.z, 1.f), x, y, centered);
}

void Renderer::renderText(const string &text, Font2D *font, const Vec4f &color,
                          int x, int y, bool centered) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  }
...
Vec2f Renderer::getCentered3DPos(const string &text, Font3D *font, Vec2f &pos,
                                 int w, int h, bool centeredW, bool centeredH) {
  if (centeredW == true) {
    if (font == NULL) {
      // abort();
      ...
      char szBuf[8096] = "";
      snprintf(szBuf, 8096,
               "font->getTextHandler() == NULL(5) text = [%s] FontPtr = [%p]\n",
               text.c_str(), font);
      throw megaglest_runtime_error(szBuf);
    }
  ...
  if (centeredH) {
    if (font == NULL) {
      ...
      throw megaglest_runtime_error(
          "font->getTextHandler() == NULL (6) text = " + text);
    }
    ...
    // lineHeight= lineHeight / (2.f + 0.2f *
    // FontMetrics::DEFAULT_Y_OFFSET_FACTOR); pos.y += (h / 2.f) - (lineHeight
    // / 2.f); pos.y += (h / 2.f) - (lineHeight); pos.y += (lineHeight / 2.f);
    // // y starts at the middle of the render position, so only move up 1/2 the
    // font height

    if (lineHeight < h) {
      // printf("line %d, lineHeight [%f] h [%d] text
      ...
      //				float heightGap = (float)(((float)h -
      // lineHeight) / 2.0); 				pos.y -= heightGap;
      //
      //				//printf("h = %d lineHeight = %f
...
void Renderer::renderTextShadow3D(const string &text, Font3D *font,
                                  const Vec4f &color, int x, int y,
                                  bool centered) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  }
...
void Renderer::renderTextShadow(const string &text, Font2D *font,
                                const Vec4f &color, int x, int y,
                                bool centered) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
...
void Renderer::renderLabel(GraphicLabel *label) {
  ...
  if (label->getTexture() != NULL) {
    ...
    if (h > 0) {
      ...
      glBindTexture(
          GL_TEXTURE_2D,
          static_cast<Texture2DGl *>(label->getTexture())->getHandle());
  ...
  Vec4f colorWithAlpha = Vec4f(labelColor.x, labelColor.y, labelColor.z,
...
void Renderer::renderLabel(GraphicLabel *label, const Vec4f *color) {
  ...
  try {
    ...
    vector<string> lines;
    string renderTextString = (label->getTextNativeTranslation() != ""
                                   ? label->getTextNativeTranslation()
                                   : label->getText());
    ...
    for (unsigned int i = 0; i < lines.size(); ++i) {
      ...
      int h = label->getH();
      int w = label->getW();
      // if(label->getInstanceName() == "modDescrLabel") printf("~~~
      // lines.size() [%u] i = %d lines[i] [%s] y =
      ...
      if (color != NULL) {
        if (renderText3DEnabled == true) {
          // renderText3D(lines[i], label->getFont3D(), (*color), textPos.x,
          // textPos.y, label->getCentered()); printf("Text Render3D [%s] font3d
          // [%p]\n",lines[i].c_str(),label->getFont3D()); printf("Label render
          // C\n");

          renderTextBoundingBox3D(
              renderTextStr, label->getFont3D(), (*color), x, y, w, h,
          ...
          renderText(renderTextStr, label->getFont(), (*color), textPos.x,
                     textPos.y, label->getCentered());
        }
      } else {
        if (renderText3DEnabled == true) {
          // renderText3D(lines[i], label->getFont3D(),
          // GraphicComponent::getFade(), textPos.x, textPos.y,
          // label->getCentered()); printf("Text Render3D [%s] font3d
          // [%p]\n",lines[i].c_str(),label->getFont3D()); printf("Label render
          // E\n");
          renderTextBoundingBox3D(
              renderTextStr, label->getFont3D(), GraphicComponent::getFade(), x,
              y, w, h, label->getCenteredW(), label->getCenteredH(),
              label->getEditModeEnabled(), label->getMaxEditWidth(),
              label->getMaxEditRenderWidth());
        } else {
          // printf("Label render F\n");
          renderText(renderTextStr, label->getFont(),
                     GraphicComponent::getFade(), textPos.x, textPos.y,
                     label->getCentered());
        }
      }
    ...
    snprintf(szBuf, 8096, "In [%s::%s Line: %d]\nError [%s] For Control [%s]\n",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__, e.what(), label->getInstanceName().c_str());
    SystemFlags::OutputDebug(SystemFlags::debugError, szBuf);
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugSystem, szBuf);
...
void Renderer::renderButton(GraphicButton *button,
                            const Vec4f *fontColorOverride,
                            bool *lightedOverride) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  }
  ...
  try {
    ...
    // char szBuf[8096]="";
    // snprintf(szBuf,8096,"In [%s::%s Line: %d]\n For Control container [%s]
    // name
    // [%s]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,button->getContainerName().c_str(),
    // button->getInstanceName().c_str()); printf(szBuf);
    ...
    if (button->getUseCustomTexture() == true) {
      ...
      backTexture = w > 3 * h / 2 ? coreData.getButtonBigTexture()
                                  : coreData.getButtonSmallTexture();
    }
    ...
    if (backTexture != NULL) {
      glBindTexture(GL_TEXTURE_2D,
                    static_cast<Texture2DGl *>(backTexture)->getHandle());
    } else {
    ...
    if (button->getEditable()) {
      if (renderText3DEnabled == true) {
        // renderText3D(button->getText(), button->getFont3D(), color,x + (w /
        // 2), y + (h / 2), true);
        renderTextBoundingBox3D(button->getText(), button->getFont3D(), color,
                                x, y, w, h, true, true, false, -1, -1);
      } else {
        renderText(button->getText(), button->getFont(), color, x + (w / 2),
                   y + (h / 2), true);
      }
    } else {
      if (renderText3DEnabled == true) {
        // renderText3D(button->getText(),
        // button->getFont3D(),disabledTextColor,
        //        x + (w / 2), y + (h / 2), true);
        renderTextBoundingBox3D(button->getText(), button->getFont3D(),
                                disabledTextColor, x, y, w, h, true, true,
                                false, -1, -1);
      } else {
    ...
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d]\nError [%s] For Control container [%s] name "
             "[%s]\n",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__, e.what(), button->getContainerName().c_str(),
             button->getInstanceName().c_str());
    SystemFlags::OutputDebug(SystemFlags::debugError, szBuf);
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugSystem, szBuf);
...
void Renderer::renderCheckBox(const GraphicCheckBox *box) {
  ...
  // background
  CoreData &coreData = CoreData::getInstance();
  Texture2D *backTexture = box->getValue()
                               ? coreData.getCheckedCheckBoxTexture()
                               : coreData.getCheckBoxTexture();
...
void Renderer::renderLine(const GraphicLine *line) {
  ...
  // background
  CoreData &coreData = CoreData::getInstance();
  Texture2D *backTexture = line->getHorizontal()
                               ? coreData.getHorizontalLineTexture()
                               : coreData.getVerticalLineTexture();
...
void Renderer::renderScrollBar(const GraphicScrollBar *sb) {
  ...
  if (sb->getElementCount() < sb->getVisibleSize()) {
    int realLength =
        sb->getElementCount() * sb->getLength() / sb->getVisibleSize();
    if (sb->getHorizontal()) {
      x = x - (w - realLength);
    } else {
      y = y + (h - realLength);
  ...
  glBindTexture(GL_TEXTURE_2D,
                static_cast<Texture2DGl *>(selectTexture)->getHandle());
...
void Renderer::renderListBox(GraphicListBox *listBox) {
  ...
  if (listBox->getLeftControlled() == true) {
    label.init(listBox->getX() + listBox->getButton1()->getW() +
                   listBox->getButton2()->getW() + 2,
               listBox->getY(), listBox->getW(), listBox->getH(), false,
               listBox->getTextColor());
  } else {
    label.init(listBox->getX(), listBox->getY(), listBox->getW(),
               listBox->getH(), true, listBox->getTextColor());
  }
  ...
  if (renderLighted) {
    ...
    int y = listBox->getY();
    int h = listBox->getH();
    int w = listBox->getW() - listBox->getButton1()->getW() -
            listBox->getButton2()->getW();
...
void Renderer::renderComboBox(GraphicComboBox *comboBox) {
  ...
  label.init(comboBox->getX(), comboBox->getY(), comboBox->getW(),
             comboBox->getH(), true, comboBox->getTextColor());
  ...
  try {
    ...
    if (messageBox->getAutoWordWrap() == true) {
      if (renderText3DEnabled == false) {
        wrappedText = messageBox->getFont()->getMetrics()->wordWrapText(
            wrappedText, messageBox->getW() * 0.90);
      } else {
        wrappedText = messageBox->getFont3D()->getMetrics()->wordWrapText(
            wrappedText, messageBox->getW() * 0.90);
      }
    }
    ...
    glColor4f(0.0f, 0.0f, 0.0f, 0.8f);
    glBegin(GL_TRIANGLE_STRIP);
    glVertex2i(messageBox->getX(),
               messageBox->getY() + messageBox->getH() - headerHeight);
    glVertex2i(messageBox->getX(), messageBox->getY());
    glVertex2i(messageBox->getX() + messageBox->getW(),
               messageBox->getY() + messageBox->getH() - headerHeight);
    ...
    glVertex2i(messageBox->getX(),
               messageBox->getY() + messageBox->getH() - headerHeight);
    glVertex2i(messageBox->getX() + messageBox->getW(),
               messageBox->getY() + messageBox->getH());
    glVertex2i(messageBox->getX() + messageBox->getW(),
               messageBox->getY() + messageBox->getH() - headerHeight);
    glEnd();
    ...
    for (int i = 0; i < messageBox->getButtonCount(); i++) {
      ...
      if ((renderText3DEnabled == false &&
           messageBox->getButton(i)->getFont() == NULL) ||
          (renderText3DEnabled == true &&
           messageBox->getButton(i)->getFont3D() == NULL)) {
        messageBox->getButton(i)->setFont(
            CoreData::getInstance().getMenuFontNormal());
        messageBox->getButton(i)->setFont3D(
            CoreData::getInstance().getMenuFontNormal3D());
      }
    ...
    if (renderText3DEnabled == true) {
      ...
      renderTextShadow3D(
          ...
          messageBox->getX() + 15,
          messageBox->getY() + messageBox->getH() - headerHeight * 2, false);

      renderTextShadow3D(
          messageBox->getHeader(), messageBox->getFont3D(), fontColor,
          messageBox->getX() + 15,
          messageBox->getY() + messageBox->getH() - headerHeight + 8, false);
...
// VisibleQuadContainerVBOCache * Renderer::GetSurfaceVBOs(SurfaceData
// *cellData) { 	std::map<uint32,VisibleQuadContainerVBOCache >::iterator
// iterFind = mapSurfaceVBOCache.find(cellData->uniqueId); 	if(iterFind ==
// mapSurfaceVBOCache.end()) { 		Vec2f *texCoords		=
//&cellData->texCoords[0]; 		Vec2f *texCoordsSurface	=
//&cellData->texCoordsSurface[0]; 		Vec3f *vertices =
//&cellData->vertices[0]; 		Vec3f *normals			=
//&cellData->normals[0];
//
...
//
//		// don't need the data in computer RAM anymore its in the GPU
// now 		cellData->texCoords.clear();
// cellData->texCoordsSurface.clear(); 		cellData->vertices.clear();
...
void Renderer::ReleaseSurfaceVBOs() {
  for (std::map<uint32, VisibleQuadContainerVBOCache>::iterator iterFind =
           mapSurfaceVBOCache.begin();
    ...
    if (item.hasBuiltVBOs == true) {
      ...
      glDeleteBuffersARB(
          1, (GLuint *)&item.m_nVBOFowTexCoords); // Get A Valid Name
      glDeleteBuffersARB(
          1, (GLuint *)&item.m_nVBOSurfaceTexCoords);       // Get A Valid Name
...
Renderer::MapRenderer::Layer::~Layer() {
  ...
  if (vbo_fowTexCoords)
    glDeleteBuffersARB(1, &vbo_fowTexCoords);
  if (vbo_surfTexCoords)
    glDeleteBuffersARB(1, &vbo_surfTexCoords);
  if (vbo_indices)
    glDeleteBuffersARB(1, &vbo_indices);
}

template <typename T>
void _loadVBO(GLuint &vbo, std::vector<T> buf,
              int target = GL_ARRAY_BUFFER_ARB) {
  assert(buf.size());
  if (true /* vbo enabled? */) {
    glGenBuffersARB(1, &vbo);
    assert(vbo);
    glBindBufferARB(target, vbo);
    glBufferDataARB(target, sizeof(T) * buf.size(), &buf[0],
                    GL_STATIC_DRAW_ARB);
...
void Renderer::MapRenderer::loadVisibleLayers(
    float coordStep, VisibleQuadContainerCache &qCache) {
  int totalCellCount = 0;
  // we create a layer for each visible texture in the map
  for (int visibleIndex = 0;
       visibleIndex < (int)qCache.visibleScaledCellList.size();
       ++visibleIndex) {
    ...
    SurfaceCell *tc[4] = {tc00, tc10, tc01, tc11};
    int textureHandle =
        static_cast<const Texture2DGl *>(tc[0]->getSurfaceTexture())
            ->getHandle();
    string texturePath =
        static_cast<const Texture2DGl *>(tc[0]->getSurfaceTexture())->getPath();
    // int32 textureCRC = CalculatePixelsCRC(static_cast<const
    // Texture2DGl*>(tc[0]->getSurfaceTexture()));
    Layer *layer = NULL;
    ...
    layer->surfTexCoords.push_back(
        Vec2f(surfCoord.x + coordStep, surfCoord.y + coordStep));
  ...
  for (Layers::iterator layer = layers.begin(); layer != layers.end();
       ++layer) {
    (*layer)->load_vbos(false);
  }

  // printf("Total # of layers for this map = %d totalCellCount = %d overall
  // render reduction ratio = %d
...
void Renderer::MapRenderer::load(float coordStep) {
  ...
  for (int y = 0; y < map->getSurfaceH() - 1; y++) {
    for (int x = 0; x < map->getSurfaceW() - 1; x++) {
      ...
      SurfaceCell *tc[4] = {
          map->getSurfaceCell(x, y), map->getSurfaceCell(x + 1, y),
          map->getSurfaceCell(x, y + 1), map->getSurfaceCell(x + 1, y + 1)};
      int textureHandle =
          static_cast<const Texture2DGl *>(tc[0]->getSurfaceTexture())
              ->getHandle();
      string texturePath =
      ...
      // we'll be super-lazy and re-emit all four corners just because its
      // easier
      int index[4];
      int loopIndexes[4] = {2, 0, 3, 1};
      for (int i = 0; i < 4; i++) {
      ...
      layer->surfTexCoords.push_back(tc[0]->getSurfTexCoord() +
                                     Vec2f(0, coordStep));
      layer->surfTexCoords.push_back(tc[0]->getSurfTexCoord() + Vec2f(0, 0));
      layer->surfTexCoords.push_back(tc[0]->getSurfTexCoord() +
                                     Vec2f(coordStep, coordStep));
      layer->surfTexCoords.push_back(tc[0]->getSurfTexCoord() +
                                     Vec2f(coordStep, 0));
...
template <typename T>
void *_bindVBO(GLuint vbo, std::vector<T> &buf,
               int target = GL_ARRAY_BUFFER_ARB) {
  void *result = NULL;
  if (vbo) {
...
void Renderer::MapRenderer::Layer::renderVisibleLayer() {
  ...
  // glBindTexture(GL_TEXTURE_2D, static_cast<const
  // Texture2DGl*>(fowTex)->getHandle());
...
void Renderer::MapRenderer::Layer::render(VisibleQuadContainerCache &qCache) {
  ...
  if (rowsToRenderCache.find(qCache.lastVisibleQuad) !=
      rowsToRenderCache.end()) {
    ...
    for (int visibleIndex = 0;
      ...
      if (cellToIndicesMap.find(pos) != cellToIndicesMap.end()) {
        // printf("Layer Render, visibleindex = %d pos [%s]
        // cellToIndicesMap[pos] = %d lastValidIndex =
  ...
  if (rowsToRender.empty() == false) {
    // printf("Layer has %d rows in visible quad, visible quad has %d
    ...
    glTexCoordPointer(2, GL_FLOAT, 0,
                      _bindVBO(vbo_surfTexCoords, surfTexCoords));
    ...
    for (unsigned int i = 0; i < rowsToRender.size(); ++i) {
      ...
      glDrawRangeElements(
          GL_TRIANGLES, rowsToRender[i].first, rowsToRender[i].second,
...
void Renderer::MapRenderer::renderVisibleLayers(
    const Map *map, float coordStep, VisibleQuadContainerCache &qCache) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  }
...
void Renderer::MapRenderer::render(const Map *map, float coordStep,
                                   VisibleQuadContainerCache &qCache) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  }
...
void Renderer::renderSurface(const int renderFps) {
  ...
  IF_DEBUG_EDITION(
	...
	assertGl();
        ...
        float coordStep =
            world->getTileset()->getSurfaceAtlas()->getCoordStep();
        ...
        glPushAttrib(GL_LIGHTING_BIT | GL_ENABLE_BIT | GL_FOG_BIT |
                     GL_TEXTURE_BIT);
        ...
        glEnable(GL_TEXTURE_2D);
        glBindTexture(GL_TEXTURE_2D,
                      static_cast<const Texture2DGl *>(fowTex)->getHandle());

        glTexSubImage2D(
            GL_TEXTURE_2D, 0, 0, 0, fowTex->getPixmapConst()->getW(),
            fowTex->getPixmapConst()->getH(), GL_ALPHA, GL_UNSIGNED_BYTE,
            fowTex->getPixmapConst()->getPixels());
        ...
        if (shadowsOffDueToMinRender == false) {
          ...
          if (shadows == sProjected || shadows == sShadowMapping) {
            ...
            static_cast<ModelRendererGl *>(modelRenderer)
                ->setDuplicateTexCoords(true);
            enableProjectiveTexturing();
          }
        }

        const Rect2i mapBounds(0, 0, map->getSurfaceW() - 1,
                               map->getSurfaceH() - 1);
        ...
        if (useVBORendering == true) {
          ...
          for (int visibleIndex = 0;
            ...
            SurfaceCell *tc00 = map->getSurfaceCell(pos.x, pos.y);
            int cellTex =
                static_cast<const Texture2DGl *>(tc00->getSurfaceTexture())
                    ->getHandle();

            uniqueVisibleTextures[cellTex]++;
          }

          // printf("Current renders = %d possible =
          ...
          for (int visibleIndex = 0;
            ...
            /*
                                            ...
                                            glMultiTexCoord2fv(fowTexUnit,
               tc01->getFowTexCoord().ptr()); glMultiTexCoord2f(baseTexUnit,
               surfCoord.x, surfCoord.y + coordStep);
                                            glNormal3fv(tc01->getNormal().ptr());
                                            glVertex3fv(tc01->getVertex().ptr());

                                            glMultiTexCoord2fv(fowTexUnit,
               tc00->getFowTexCoord().ptr()); glMultiTexCoord2f(baseTexUnit,
               surfCoord.x, surfCoord.y); glNormal3fv(tc00->getNormal().ptr());
                                            glVertex3fv(tc00->getVertex().ptr());

                                            glMultiTexCoord2fv(fowTexUnit,
               tc11->getFowTexCoord().ptr()); glMultiTexCoord2f(baseTexUnit,
               surfCoord.x+coordStep, surfCoord.y+coordStep);
                                            glNormal3fv(tc11->getNormal().ptr());
                                            glVertex3fv(tc11->getVertex().ptr());

                                            glMultiTexCoord2fv(fowTexUnit,
               tc10->getFowTexCoord().ptr()); glMultiTexCoord2f(baseTexUnit,
               surfCoord.x + coordStep, surfCoord.y);
          ...
          //				std::map<string,std::pair<Chrono,
          // std::vector<SurfaceData> > >::iterator iterFind =
          // mapSurfaceData.find(snapshotOfvisibleQuad.getString());
          // if(iterFind
          //== mapSurfaceData.end()) { 					recalcSurface =
          //true;
          //					//printf("#1 Calculating surface for
          //Rendering using VA's
          // [%s]\n",snapshotOfvisibleQuad.getString().c_str());
          //				}
          ///*
          //				else if(iterFind->second.first.getMillis() >=
          //250) { 					recalcSurface = true;
          //					mapSurfaceData.erase(snapshotOfvisibleQuad.getString());
          //					//printf("#2 RE-Calculating surface
          //for Rendering using VA's
          //[%s]\n",snapshotOfvisibleQuad.getString().c_str());
          //				}
          //*/
          //		    }
          ...
          //				//printf("Calculating surface for Rendering
          //using VA's [%s]\n",snapshotOfvisibleQuad.getString().c_str());
          //
          //		    	std::vector<SurfaceData> *surface =
          //&surfaceData; 		    	if(useSurfaceCache == true) {
          //std::pair<Chrono, std::vector<SurfaceData> > &surfaceCacheEntity =
          // mapSurfaceData[snapshotOfvisibleQuad.getString()];
          ...
          //				for(int visibleIndex = 0;
          //						visibleIndex <
          //(int)qCache.visibleScaledCellList.size(); ++visibleIndex) {
          //Vec2i &pos = qCache.visibleScaledCellList[visibleIndex];
          //
          //					SurfaceCell *tc00=
          //map->getSurfaceCell(pos.x,
          // pos.y); 					SurfaceCell *tc10=
          // map->getSurfaceCell(pos.x+1, pos.y); 					SurfaceCell *tc01=
          //map->getSurfaceCell(pos.x,
          // pos.y+1); 					SurfaceCell *tc11=
          // map->getSurfaceCell(pos.x+1, pos.y+1);
          //
          //					if(tc00 == NULL) {
          //						throw megaglest_runtime_error("tc00
          //== NULL");
          //					}
          //					if(tc10 == NULL) {
          //						throw megaglest_runtime_error("tc10
          //== NULL");
          //					}
          //					if(tc01 == NULL) {
          //						throw megaglest_runtime_error("tc01
          //== NULL");
          //					}
          //					if(tc11 == NULL) {
          //						throw megaglest_runtime_error("tc11
          //== NULL");
          //					}
          //
          //					triangleCount+= 2;
          ...
          //
          //					//set texture
          //					if(tc00->getSurfaceTexture() ==
          //NULL) { 						throw
          //megaglest_runtime_error("tc00->getSurfaceTexture() == NULL");
          //					}
          //
          //					int surfaceDataIndex = -1;
          //					currTex= static_cast<const
          // Texture2DGl*>(tc00->getSurfaceTexture())->getHandle();
          // if(currTex !=
          // lastTex) { 						lastTex =
          // currTex;
          //					}
          ...
          // SurfaceData::nextUniqueId++; 						newData.bufferCount=0;
          //						newData.textureHandle =
          // currTex;
          // surface->push_back(newData);
          //
          //						surfaceDataIndex = (int)surface->size()
          //- 1;
          //					}
          //
          //					lastSurfaceDataIndex =
          // surfaceDataIndex;
          //
          //					SurfaceData *cellData =
          //&(*surface)[surfaceDataIndex];
          //
          //					const Vec2f &surfCoord=
          // tc00->getSurfTexCoord();
          //
          //					cellData->texCoords.push_back(tc01->getFowTexCoord());
          //					cellData->texCoordsSurface.push_back(Vec2f(surfCoord.x,
          // surfCoord.y + coordStep));
          ...
          //					cellData->texCoordsSurface.push_back(Vec2f(surfCoord.x+coordStep,
          // surfCoord.y+coordStep));
          ...
          //		    if(useSurfaceCache == true) {
          //		    	std::pair<Chrono, std::vector<SurfaceData> >
          //&surfaceCacheEntity =
          // mapSurfaceData[snapshotOfvisibleQuad.getString()];
          // surface = &surfaceCacheEntity.second;
          //		    }
          ...
          //				else {
          //					Vec2f *texCoords =
          //&data.texCoords[0]; 					Vec2f *texCoordsSurface
          //=
          //&data.texCoordsSurface[0]; 					Vec3f *vertices
          //=
          ...
          // vertices); 					glNormalPointer(GL_FLOAT, 0,
          // normals);
          //
          //					glDrawArrays(GL_TRIANGLE_STRIP,
          //0, data.bufferCount);
          ...
          //			glDisableClientState(GL_VERTEX_ARRAY);
          //
          //			//printf("Surface Render before [%d] after
        ...
        IF_DEBUG_EDITION(
      } // end else, if not renderering debug textures instead of regular
        // terrain
      getDebugRenderer()
          .renderEffects(visibleQuad / Map::cellScale);)
}
...
void Renderer::renderObjects(const int renderFps) {
  ...
  Vec3f baseFogColor = world->getTileset()->getFogColor() *
                       world->getTimeFlow()->computeLightColor();
  ...
  //	for(int visibleIndex = 0;
  //			visibleIndex < qCache.visibleObjectList.size();
  //++visibleIndex)
  //{
  // render from last to first object so animated objects which are on bottom of
  // screen are rendered first which looks better for limited number of animated
  // tileset objects
  for (int visibleIndex = (int)qCache.visibleObjectList.size() - 1;
    ...
    if (modelRenderStarted == false) {
      ...
      glPushAttrib(GL_ENABLE_BIT | GL_COLOR_BUFFER_BIT | GL_FOG_BIT |
                   GL_LIGHTING_BIT | GL_TEXTURE_BIT);

      if (shadowsOffDueToMinRender == false && shadows == sShadowMapping) {
        glActiveTexture(shadowTexUnit);
        glEnable(GL_TEXTURE_2D);
    ...
    float fowFactor = fowTexPixmap->getPixelf(
        o->getMapPos().x / Map::cellScale, o->getMapPos().y / Map::cellScale);
    ...
    // We use OpenGL Lights so no manual action is needed here. In fact this
    // call did bad things on lighting big rocks for example
    // if(o->getRotation()
    //!= 0.0) { 			setupLightingForRotatedModel();
    //		}

    // objModel->updateInterpolationData(0.f, true);
    // if(this->gameCamera->getPos().dist(o->getPos()) <=
    // SKIP_INTERPOLATION_DISTANCE) {
...
void Renderer::renderWater() {
  ...
  if (textures3D) {
    ...
    glBindTexture(GL_TEXTURE_3D,
                  static_cast<Texture3DGl *>(waterTex)->getHandle());
  } else {
    glEnable(GL_COLOR_MATERIAL);
  ...
  for (int j = scaledRect.p[0].y; j < scaledRect.p[1].y; ++j) {
    ...
    for (int i = scaledRect.p[0].x; i <= scaledRect.p[1].x; ++i) {
      ...
      if (cellExplored == false) {
        cellExplored =
            (tc0->isExplored(thisTeamIndex) || tc1->isExplored(thisTeamIndex));
      }
      ...
      if (cellExplored == true && tc0->getNearSubmerged()) {
        ...
        glVertex3f(static_cast<float>(i) * Map::mapScale, waterLevel,
...
void Renderer::renderTeamColorCircle() {
  ...
  if (qCache.visibleQuadUnitList.empty() == false) {
    ...
    for (int visibleUnitIndex = 0;
      ...
      Vec3f color =
          unit->getFaction()->getTexture()->getPixmapConst()->getPixel3f(0, 0);
...
void Renderer::renderSpecialHighlightUnits(
    std::map<int, HighlightSpecialUnitInfo> unitHighlightList) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true ||
      unitHighlightList.empty() == true) {
    return;
  }
  ...
  if (qCache.visibleQuadUnitList.empty() == false) {
    ...
    for (int visibleUnitIndex = 0;
      ...
      std::map<int, HighlightSpecialUnitInfo>::iterator iterFindSpecialUnit =
          unitHighlightList.find(unit->getId());
      if (iterFindSpecialUnit != unitHighlightList.end()) {
        Vec3f color =
            unit->getFaction()->getTexture()->getPixmapConst()->getPixel3f(0,
                                                                           0);
        float radius = 1.0f;
        ...
        Vec3f currVec = unit->getCurrVectorFlat();
        renderSelectionCircle(currVec, unit->getType()->getSize(), radius,
                              thickness);
      }
    }
    glPopAttrib();
...
void Renderer::renderTeamColorPlane() {
  ...
  if (qCache.visibleQuadUnitList.empty() == false) {
    ...
    for (int visibleUnitIndex = 0;
      ...
      if (unit->isAlive()) {
        Vec3f currVec = unit->getCurrVectorFlat();
        renderTeamColorEffect(
            currVec, visibleUnitIndex, unit->getType()->getSize(),
            unit->getFaction()->getTexture()->getPixmapConst()->getPixel3f(0,
                                                                           0),
            texture);
      }
...
void Renderer::renderGhostModel(const UnitType *building, const Vec2i pos,
                                CardinalDir facing, Vec4f *forceColor) {
  // const UnitType *building= gui->getBuilding();
  // const Vec2i &pos= gui->getPosObjWorld();
...
void Renderer::renderUnits(bool airUnits, const int renderFps) {
  ...
  if (visibleFrameUnitList.empty() == false) {
    visibleFrameUnitList.clear();
    // visibleFrameUnitListCameraKey = "";
    // if(visibleFrameUnitListCameraKey !=
  ...
  if (qCache.visibleQuadUnitList.empty() == false) {
    ...
    for (int visibleUnitIndex = 0;
      ...
      if ((airUnits == false && unit->getType()->getField() == fAir) ||
          (airUnits == true && unit->getType()->getField() != fAir)) {
        continue;
      }
      ...
      if (modelRenderStarted == false) {
        ...
        glPushAttrib(GL_ENABLE_BIT | GL_FOG_BIT | GL_LIGHTING_BIT |
                     GL_TEXTURE_BIT);
        glEnable(GL_COLOR_MATERIAL);
      ...
      if (st->getClass() == scDie &&
          static_cast<const DieSkillType *>(st)->getFade()) {
        float alpha = 1.0f - unit->getAnimProgressAsFloat();
        glDisable(GL_COLOR_MATERIAL);
        glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE,
                     Vec4f(1.0f, 1.0f, 1.0f, alpha).ptr());
      } else {
        glEnable(GL_COLOR_MATERIAL);
        // we cut off a tiny bit here to avoid problems with fully transparent
        // texture parts cutting units in background rendered later.
      ...
      // if(this->gameCamera->getPos().dist(unit->getCurrVector()) <=
      // SKIP_INTERPOLATION_DISTANCE) {
      model->updateInterpolationData(unit->getAnimProgressAsFloat(),
                                     unit->isAlive() &&
                                         !unit->isAnimProgressBound());
      //}
...
void Renderer::renderUnitsToBuild(const int renderFps) {
  ...
  if (qCache.visibleQuadUnitBuildList.empty() == false) {
    ...
    for (int visibleUnitIndex = 0;
         visibleUnitIndex < (int)qCache.visibleQuadUnitBuildList.size();
         ++visibleUnitIndex) {
      const UnitBuildInfo &buildUnit =
          qCache.visibleQuadUnitBuildList[visibleUnitIndex];
      // Vec4f modelColor= Vec4f(0.f, 1.f, 0.f, 0.5f);
      const Vec3f teamColor = buildUnit.unit->getFaction()
                                  ->getTexture()
                                  ->getPixmapConst()
                                  ->getPixel3f(0, 0);
      Vec4f modelColor = Vec4f(teamColor.x, teamColor.y, teamColor.z, 0.4f);
      renderGhostModel(buildUnit.buildUnit, buildUnit.pos, buildUnit.facing,
                       &modelColor);
...
void Renderer::renderTeamColorEffect(Vec3f &v, int heigth, int size,
                                     Vec3f color, const Texture2D *texture) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  }
  ...
  glPushMatrix();
  glBindTexture(GL_TEXTURE_2D,
                static_cast<const Texture2DGl *>(texture)->getHandle());
  ...
  if (qCache.visibleQuadUnitList.empty() == false) {
    ...
    for (int visibleUnitIndex = 0;
      ...
      if (unit->getCurrSkill() != NULL &&
        ...
        if (command != NULL &&
            command->getCommandType()->commandTypeClass == ccMorph) {
          const MorphCommandType *mct =
              static_cast<const MorphCommandType *>(command->getCommandType());
          ...
          if (mType->getSize() > unit->getType()->getSize() ||
            ...
            if (mType->getField() == fAir &&
                unit->getType()->getField() == fLand) {
              currVec = Vec3f(
                  currVec.x,
                  currVec.y + game->getWorld()->getTileset()->getAirHeight(),
                  currVec.z);
            }
            if (mType->getField() == fLand &&
                unit->getType()->getField() == fAir) {
              currVec = Vec3f(
                  currVec.x,
                  currVec.y - game->getWorld()->getTileset()->getAirHeight(),
                  currVec.z);
            ...
            renderSelectionCircle(currVec, mType->getSize(),
                                  frameCycle * 0.85f / 40, 0.2f);
          }
        }
      }
    }
...
void Renderer::renderSelectionEffects(int healthbarMode) {
  ...
  const Object *selectedResourceObject =
      game->getGui()->getSelectedResourceObject();
  ...
  for (int i = 0; i < selection->getCount(); ++i) {
    ...
    if (unit != NULL) {
      ...
      if (healthbarMode == hbvUndefined) {
        finalHealthbarMode =
            unit->getFaction()->getType()->getHealthbarVisible();
      } else {
      ...
      if (world->getThisFactionIndex() == unit->getFactionIndex()) {
        if (showDebugUI == true &&
            ...
            dynamic_cast<const BuildCommandType *>(
      ...
      renderSelectionCircle(currVec, unit->getType()->getSize(),
                            selectionCircleRadius, selectionCircleThickness);

      if (showDebugUI == true &&
          (showDebugUILevel & debugui_unit_titles) == debugui_unit_titles) {

        const UnitPathInterface *path = unit->getPath();
        const UnitPathBasic *pathfinder =
            (path == NULL ? NULL : dynamic_cast<const UnitPathBasic *>(path));
        if (pathfinder != NULL) {
      ...
      // magic circle
      if (!healthbarsVisible &&
          world->getThisFactionIndex() == unit->getFactionIndex() &&
          unit->getType()->getMaxEp() > 0) {
        glColor4f(unit->getEpRatio() / 2.f, unit->getEpRatio(),
                  unit->getEpRatio(), 0.5f);
        renderSelectionCircle(currVec, unit->getType()->getSize(),
                              magicCircleRadius);
      }
      ...
      if (showDebugUI == true) {
        // const std::pair<const SkillType *,std::vector<Unit *> >
        // &currentAttackBoostUnits = unit->getCurrentAttackBoostUnits();
        const UnitAttackBoostEffectOriginator &effect =
            unit->getAttackBoostOriginatorEffect();

        if (effect.skillType->isAttackBoostEnabled() == true) {
          glColor4f(MAGENTA.x, MAGENTA.y, MAGENTA.z, MAGENTA.w);
          renderSelectionCircle(
              currVec, 1, effect.skillType->getAttackBoost()->radius,
              .25f / effect.skillType->getAttackBoost()->radius);

          for (unsigned int i = 0; i < effect.currentAttackBoostUnits.size();
               ++i) {
            ...
            if (affectedUnit != NULL) {
              ...
              renderSelectionCircle(currVecBoost,
                                    affectedUnit->getType()->getSize(), 1.f);
            }
          }
        }
      }
      ...
      if (unit->getType()->getMeetingPoint()) {
        ...
        renderArrow(unit->getCurrVectorFlat(), arrowTarget,
                    Vec3f(0.f, 0.f, 1.f), 0.3f);
      }
    }
  }
  ...
  if (selectedResourceObject != NULL &&
    ...
    float colorValue =
        static_cast<float>(r->getAmount()) / static_cast<float>(defaultValue);
    glColor4f(0.1f, 0.1f, colorValue, 0.4f);
    renderSelectionCircle(selectedResourceObject->getPos(), 2,
  ...
  if (game->getGui()->getHighlightedUnit() != NULL) {
    ...
    if (unit->isHighlighted()) {
      ...
      v.y += 0.3f;
      renderSelectionCircle(v, unit->getType()->getSize(),
                            0.5f + 0.4f * highlight);
    }
  }
  ...
  //			if(unit->isHighlighted()) {
  //				float highlight= unit->getHightlight();
  //				if(game->getWorld()->getThisFactionIndex() ==
  // unit->getFactionIndex()) { 					glColor4f(0.f, 1.f, 0.f,
  // highlight);
  //				}
  //				else{
  //					glColor4f(1.f, 0.f, 0.f, highlight);
  ...
  //				Vec3f v= unit->getCurrVectorFlat();
  //				v.y+= 0.3f;
  //				renderSelectionCircle(v,
  //unit->getType()->getSize(), 0.5f+0.4f*highlight );
  //			}
  //		}
  //	}
  ...
  if (qCache.visibleQuadUnitList.empty() == false) {
    for (int visibleUnitIndex = 0;
      ...
      if (isHealthBarVisible(unit, healthbarMode)) {
        ...
        healthbarthickness =
            unit->getFaction()->getType()->getHealthbarThickness();
        healthbarLineBorder =
            unit->getFaction()->getType()->isHealthbarLineBorder();
        CoreData &coreData = CoreData::getInstance();
        // First try faction texture then use core Texture
        if (unit->getFaction()->getType()->isHealthbarBorderTextureEnabled()) {
          healthbarTexture =
              unit->getFaction()->getType()->getHealthbarTexture();
        ...
        if (unit->getFaction()
                ->getType()
                ->isHealthbarBackgroundTextureEnabled()) {
          healthbarBackgroundTexture =
              unit->getFaction()->getType()->getHealthbarBackgroundTexture();
          if (healthbarBackgroundTexture == NULL) {
            healthbarBackgroundTexture =
                coreData.getHealthbarBackgroundTexture();
          }
        } else {
        ...
        if (unit->getType()->getHealthbarVisible() != hbvOff &&
            unit->getType()->getHealthbarVisible() != hbvUndefined) {
        ...
        renderHealthBar(currVec, unit, healthbarthickness, healthbarLineBorder,
                        healthbarTexture, healthbarBackgroundTexture);
      }
    }
  }
  glPopAttrib();
...
bool Renderer::isHealthBarVisible(const Unit *unit, int healthbarMode) {
  ...
  if (unit->getType()->getHealthbarVisible() != hbvOff &&
      unit->getType()->getHealthbarVisible() != hbvUndefined) {
    if (healthbarMode == hbvUndefined) { // don't override the visible setting
                                         // when hotkey is not hbvUndefined
      healthbarVisible = unit->getType()->getHealthbarVisible();
    }
  }

  bool settingsWantToRenderThem =
      !(healthbarVisible == hbvUndefined || (healthbarVisible & hbvOff)) &&
      ((healthbarVisible & hbvAlways) ||
       ((healthbarVisible & hbvIfNeeded) &&
        unit->getHp() < unit->getType()->getMaxHp() +
                            unit->getTotalUpgrade()->getMaxHp()) ||
       ((healthbarVisible & hbvIfNeeded) && unit->getType()->getMaxEp() > 0 &&
        unit->getEp() < unit->getType()->getMaxEp() +
                            unit->getTotalUpgrade()->getMaxEp()) ||
...
void Renderer::renderWaterEffects() {
  ...
  // splashes
  glBindTexture(GL_TEXTURE_2D,
                static_cast<Texture2DGl *>(coreData.getWaterSplashTexture())
                    ->getHandle());
  ...
  for (int i = 0; i < we->getWaterSplashCount(); ++i) {
    ...
    if (ws->getEnabled()) {
      ...
      Vec2i intPos = Vec2i(static_cast<int>(ws->getPos().x),
                           static_cast<int>(ws->getPos().y));
      const Vec2i &mapPos = Map::toSurfCoords(intPos);

      bool visible =
          map->getSurfaceCell(mapPos)->isVisible(world->getThisTeamIndex());
      if (visible == false &&
          world->showWorldForPlayer(world->getThisFactionIndex()) == true) {
        visible = true;
      }
      ...
      if (visible == true) {
        ...
        vertices[0] =
            Vec3f(ws->getPos().x - scale, height, ws->getPos().y + scale);
        ...
        /*
                                        glBegin(GL_TRIANGLE_STRIP);
                                                glTexCoord2f(0.f, 1.f);
                                                glVertex3f(ws->getPos().x-scale,
           height, ws->getPos().y+scale); glTexCoord2f(0.f, 0.f);
                                                glVertex3f(ws->getPos().x-scale,
           height, ws->getPos().y-scale); glTexCoord2f(1.f, 1.f);
                                                glVertex3f(ws->getPos().x+scale,
           height, ws->getPos().y+scale); glTexCoord2f(1.f, 0.f);
                                                glVertex3f(ws->getPos().x+scale,
           height, ws->getPos().y-scale); glEnd();
        */
      }
    }
  }
...
void Renderer::renderHud() {
  ...
  if (hudTexture != NULL) {
    const Metrics &metrics = Metrics::getInstance();
    renderTextureQuad(0, 0, metrics.getVirtualW(), metrics.getVirtualH(),
                      hudTexture, 1.0f);
  }
}
...
void Renderer::renderMinimap() {
  ...
  Vec2f zoom = Vec2f(static_cast<float>(mw) / pixmap->getW(),
  ...
  glBindTexture(
      GL_TEXTURE_2D,
      static_cast<const Texture2DGl *>(minimap->getFowTexture())->getHandle());
  glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_COMBINE);
  ...
  glBindTexture(
      GL_TEXTURE_2D,
      static_cast<const Texture2DGl *>(minimap->getTexture())->getHandle());
  ...
  if (itemCount > 0) {
    ...
    for (int i = 0; i < attackEffects->getWaterSplashCount(); ++i) {
      ...
      if (ws->getEnabled()) {
        //			glBegin(GL_QUADS);
        //				glVertex2f(attackX-scale,
        // attackY-scale); 				glVertex2f(attackX-scale,
        // attackY+scale); 				glVertex2f(attackX+scale, attackY+scale);
        // glVertex2f(attackX+scale, attackY-scale); 			glEnd();
  ...
  const bool showAllUnitsInMinimap =
      Config::getInstance().getBool("DebugGameSynchUI", "false");
  ...
  if (visibleUnitList.empty() == false) {
    ...
    for (int visibleIndex = 0; visibleIndex < (int)visibleUnitList.size();
         ++visibleIndex) {
      ...
      unit_vertices[unitIdx] =
          Vec2f(mx + pos.x * zoom.x, my + mh - (pos.y * zoom.y));
      unitIdx++;

      unit_colors[unitIdx] = color;
      unit_vertices[unitIdx] =
          Vec2f(mx + (pos.x + 1) * zoom.x + size, my + mh - (pos.y * zoom.y));
      unitIdx++;

      unit_colors[unitIdx] = color;
      unit_vertices[unitIdx] = Vec2f(mx + (pos.x + 1) * zoom.x + size,
                                     my + mh - ((pos.y + size) * zoom.y));
      unitIdx++;

      unit_colors[unitIdx] = color;
      unit_vertices[unitIdx] =
          Vec2f(mx + pos.x * zoom.x, my + mh - ((pos.y + size) * zoom.y));
      unitIdx++;
    }
  ...
  // draw camera
  float wRatio =
      static_cast<float>(metrics.getMinimapW()) / world->getMap()->getW();
  float hRatio =
      static_cast<float>(metrics.getMinimapH()) / world->getMap()->getH();
...
void Renderer::renderHighlightedCellsOnMinimap() {
  ...
  if (highlightedCells->empty() == false) {
    ...
    Vec2f zoom = Vec2f(static_cast<float>(mw) / pixmap->getW() / 2,
    ...
    for (int i = 0; i < (int)highlightedCells->size(); i++) {
      ...
      if (mc->getFaction() == NULL ||
          (mc->getFaction()->getTeam() ==
           game->getWorld()->getThisFaction()->getTeam())) {
        const Texture2D *texture = game->getHighlightCellTexture();
        Vec3f color(MarkedCell::static_system_marker_color);
        if (mc->getFaction() != NULL) {
          color = mc->getFaction()->getTexture()->getPixmapConst()->getPixel3f(
              0, 0);
        }
        int lighting = (mc->getAliveCount() % 15);
        Vec3f myColor =
            Vec3f(color.x / 2 + .5f / lighting, color.y / 2 + .5f / lighting,
                  color.z / 2 + .5f / lighting);

        Vec2i pos = mc->getTargetPos();
        if (texture != NULL) {
          // float alpha = 0.49f+0.5f/(mc->getAliveCount()%15);
          float alpha = 1.0f;
          renderTextureQuad((int)(pos.x * zoom.x) + pointersize,
                            my + mh - (int)(pos.y * zoom.y), pointersize,
                            pointersize, texture, alpha, &myColor);
        }
      }
    }
  }
    ...
    for (std::map<Vec2i, MarkedCell>::iterator iterMap = markedCells.begin();
      ...
      if (bm.getPlayerIndex() < 0 ||
        ...
        if (bm.getFaction() != NULL) {
          color =
              bm.getFaction()->getTexture()->getPixmapConst()->getPixel3f(0, 0);
        }
        float alpha = 0.65f;
      ...
      if (bm.getPlayerIndex() < 0 ||
          (bm.getFaction() != NULL &&
           bm.getFaction()->getTeam() ==
               game->getWorld()->getThisFaction()->getTeam())) {
        const Map *map = game->getWorld()->getMap();
        std::pair<bool, Vec3f> bmVisible =
            posInCellQuadCache(map->toSurfCoords(bm.getTargetPos()));
        if (bmVisible.first == true) {
          if (renderTextHint == true) {
            if (bm.getNote() != "") {
              bool validPosObjWorld =
                  x > bmVisible.second.x && y > bmVisible.second.y + yOffset &&
                  x < bmVisible.second.x + texture->getTextureWidth() &&
                  y < bmVisible.second.y + yOffset +
                          texture->getTextureHeight();

              if (validPosObjWorld) {
                // printf("Checking for hint text render mouse [%d,%d] marker
                // pos [%d,%d] validPosObjWorld = %d, hint
                ...
                if (renderText3DEnabled == true) {
                  renderTextShadow3D(
                      bm.getNote(), CoreData::getInstance().getConsoleFont3D(),
                      fontColor, bmVisible.second.x, bmVisible.second.y);
                } else {
                  renderTextShadow(
                      bm.getNote(), CoreData::getInstance().getConsoleFont(),
                      fontColor, bmVisible.second.x, bmVisible.second.y);
                }
              }
            }
            ...
            /*
               ...
               GL_TEXTURE_ENV_MODE, GL_COMBINE);
                                                            ...
                                                            //set color to
               interpolation glTexEnvi(GL_TEXTURE_ENV, GL_COMBINE_RGB,
               GL_INTERPOLATE);
               ...
               GL_OPERAND0_RGB, GL_SRC_COLOR);
                                                            ...
                                                            glTexEnvi(GL_TEXTURE_ENV,
               GL_SOURCE1_RGB, GL_TEXTURE1); glTexEnvi(GL_TEXTURE_ENV,
               GL_OPERAND1_RGB, GL_SRC_COLOR);

                                                            glTexEnvi(GL_TEXTURE_ENV,
               GL_SOURCE2_RGB, GL_TEXTURE); glTexEnvi(GL_TEXTURE_ENV,
               GL_OPERAND2_RGB, GL_SRC_ALPHA);

                                                            //set alpha to 1
                                                            glTexEnvi(GL_TEXTURE_ENV,
               GL_COMBINE_ALPHA, GL_REPLACE); glTexEnvi(GL_TEXTURE_ENV,
               GL_SOURCE0_ALPHA, GL_TEXTURE); glTexEnvi(GL_TEXTURE_ENV,
               GL_OPERAND0_ALPHA, GL_SRC_ALPHA);

                                                            //texture 1
                                                            glActiveTexture(GL_TEXTURE1);
                                                            glMultiTexCoord2f(GL_TEXTURE1,
               0.f, 0.f); glEnable(GL_TEXTURE_2D);

                                                            glBindTexture(GL_TEXTURE_2D,
               static_cast<const
               Texture2DGl*>(bm.getFaction()->getTexture())->getHandle());
                                                            glTexEnvi(GL_TEXTURE_ENV,
               GL_TEXTURE_ENV_MODE, GL_COMBINE);
               ...
               GL_COMBINE_RGB, GL_MODULATE);
                                                            ...
                                                            glTexEnvi(GL_TEXTURE_ENV,
               GL_SOURCE0_RGB, GL_PRIMARY_COLOR); glTexEnvi(GL_TEXTURE_ENV,
               GL_OPERAND0_RGB, GL_SRC_COLOR);

                                                            glTexEnvi(GL_TEXTURE_ENV,
               GL_SOURCE1_RGB, GL_PREVIOUS); glTexEnvi(GL_TEXTURE_ENV,
               GL_OPERAND1_RGB, GL_SRC_COLOR);

                                                            //set alpha to 1
                                                            glTexEnvi(GL_TEXTURE_ENV,
               GL_COMBINE_ALPHA, GL_REPLACE); glTexEnvi(GL_TEXTURE_ENV,
               GL_SOURCE0_ALPHA, GL_PRIMARY_COLOR); glTexEnvi(GL_TEXTURE_ENV,
               GL_OPERAND0_ALPHA, GL_SRC_ALPHA);
            ...
            */
            ...
            if (bm.getFaction() != NULL) {
              color =
                  bm.getFaction()->getTexture()->getPixmapConst()->getPixel3f(
                      0, 0);
            }

            renderTextureQuad(bmVisible.second.x, bmVisible.second.y + yOffset,
...
void Renderer::renderDisplay() {
  ...
  if (renderText3DEnabled == true) {
    // infoString
    renderTextShadow3D(display->getInfoText().c_str(),
                       coreData.getDisplayFont3D(), display->getColor(),
                       metrics.getDisplayX(),
                       metrics.getDisplayY() + Display::infoStringY);

    // title
    renderTextShadow3D(display->getTitle().c_str(), coreData.getDisplayFont3D(),
                       display->getColor(), metrics.getDisplayX() + 40,
    ...
    renderTextShadow3D(display->getText().c_str(), coreData.getDisplayFont3D(),
                       display->getColor(), metrics.getDisplayX() - 1,
                       metrics.getDisplayY() + metrics.getDisplayH() - 56);

    // progress Bar
    if (display->getProgressBar() != -1) {
      renderProgressBar3D(display->getProgressBar(), metrics.getDisplayX(),
                          metrics.getDisplayY() + metrics.getDisplayH() - 50,
                          coreData.getDisplayFontSmall3D(), 175);
    }
  } else {
    // infoString
    renderTextShadow(display->getInfoText().c_str(), coreData.getDisplayFont(),
                     display->getColor(), metrics.getDisplayX(),
                     metrics.getDisplayY() + Display::infoStringY);

    // title
    renderTextShadow(display->getTitle().c_str(), coreData.getDisplayFont(),
                     display->getColor(), metrics.getDisplayX() + 40,
                     metrics.getDisplayY() + metrics.getDisplayH() - 20);
    ...
    // text
    renderTextShadow(display->getText().c_str(), coreData.getDisplayFont(),
                     display->getColor(), metrics.getDisplayX() - 1,
                     metrics.getDisplayY() + metrics.getDisplayH() - 56);

    // progress Bar
    if (display->getProgressBar() != -1) {
      renderProgressBar(display->getProgressBar(), metrics.getDisplayX(),
                        metrics.getDisplayY() + metrics.getDisplayH() - 50,
  ...
  for (int i = 0; i < Display::upCellCount; ++i) {
    if (display->getUpImage(i) != NULL) {
      renderQuad(metrics.getDisplayX() + display->computeUpX(i),
                 metrics.getDisplayY() + display->computeUpY(i),
                 display->getUpImageSize(), display->getUpImageSize(),
                 display->getUpImage(i));
    }
  }
...
void Renderer::renderMenuBackground(const MenuBackground *menuBackground) {
  ...
  menuBackground->getMainModelPtr()->updateInterpolationData(
      menuBackground->getAnim(), true);
  ...
  if (dist < minDist) {
    ...
    for (int i = 0; i < MenuBackground::characterCount; ++i) {
      ...
      gluProject(worldPos.x, worldPos.y, worldPos.z, modelview, projection,
                 viewport, &screen_coords[0], &screen_coords[1],
                 &screen_coords[2]);
      characterMenuScreenPositionListCache.push_back(
          Vec3f(screen_coords[0], screen_coords[1], screen_coords[2]));
      // Get the screen coordinates for each character model - END
      //

      menuBackground->getCharacterModelPtr(i)->updateInterpolationData(
          menuBackground->getAnim(), true);
  ...
  if (menuBackground->getWater()) {
    ...
    glNormal3f(0.f, 1.f, 0.f);
    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE,
                 Vec4f(1.f, 1.f, 1.f, 1.f).ptr());
    GLuint waterHandle =
        static_cast<Texture2DGl *>(menuBackground->getWaterTexture())
            ->getHandle();
    ...
    for (int i = 1; i < waterTesselation; ++i) {
      ...
      for (int j = 1; j < waterTesselation; ++j) {
        glTexCoord2i(1, 2 % j);
        glVertex3f(-waterSize + i * waterQuadSize, waterHeight,
                   -waterSize + j * waterQuadSize);
        glTexCoord2i(0, 2 % j);
        glVertex3f(-waterSize + (i + 1) * waterQuadSize, waterHeight,
                   -waterSize + j * waterQuadSize);
      }
      glEnd();
    }
...
void Renderer::renderMenuBackground(Camera *camera, float fade,
                                    Model *mainModel,
                                    vector<Model *> characterModels,
                                    const Vec3f characterPosition, float anim) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  }
...
bool Renderer::ccomputePosition(const Vec2i &screenPos, Vec2i &worldPos,
                                bool exactCoords) {
  assertGl();
  const Map *map = game->getWorld()->getMap();
  const Metrics &metrics = Metrics::getInstance();
  ...
  gluUnProject(screenX, screenY, depth, modelviewMatrix, projectionMatrix,
  ...
  if (exactCoords == true) {
    ...
    worldPos =
        Vec2i(static_cast<int>(worldX + 0.5f), static_cast<int>(worldZ + 0.5f));
  }
...
Vec3f Renderer::computeScreenPosition(const Vec3f &worldPos) {
  ...
  GLdouble screenY;
  GLdouble screenZ;
  gluProject(worldX, worldY, worldZ, modelviewMatrix, projectionMatrix,
...
void Renderer::computeSelected(Selection::UnitContainer &units,
  ...
  const string selectionType = toLower(
      Config::getInstance().getString("SelectionType", Config::colorPicking));
  ...
  /// Frustum approach --> Currently not accurate enough
    ...
    selectUsingFrustumSelection(units, obj, withObjectSelection, posDown,
                                posUp);
  } else {
...
void Renderer::selectUsingFrustumSelection(Selection::UnitContainer &units,
  ...
  gluPerspective(perspFov, metrics.getAspectRatio(), perspNearPlane,
                 perspFarPlane);
  loadGameCameraMatrix();
  ...
  if (withObjectSelection == true) {
    if (qCache.visibleObjectList.empty() == false) {
      for (int visibleIndex = 0;
           visibleIndex < (int)qCache.visibleObjectList.size();
           ++visibleIndex) {
        Object *object = qCache.visibleObjectList[visibleIndex];
        if (object != NULL) {
          bool insideQuad = CubeInFrustum(
              quadSelectionCacheItem.frustumData, object->getPos().x,
              object->getPos().y, object->getPos().z, 1);
          if (insideQuad == true) {
            obj = object;
            // if(withObjectSelection == true) {
            break;
...
void Renderer::selectUsingSelectionBuffer(Selection::UnitContainer &units,
  ...
  if (renderModeResult < 0) {
    const char *errorString =
        reinterpret_cast<const char *>(gluErrorString(renderModeResult));
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "OpenGL error #%d [0x%X] : [%s] at file: [%s], line: %d",
             renderModeResult, renderModeResult, errorString,
  ...
  // gluPerspective(perspFov, (float)view[2]/(float)view[3], perspNearPlane,
  // perspFarPlane);
  loadGameCameraMatrix();
  ...
  // (see http://www.unknownroad.com/rtfm/graphics/glselection.html section:
  // [0x4])
  // glFlush();
  ...
  if (selCount > 0) {
    ...
    const char *errorString =
        reinterpret_cast<const char *>(gluErrorString(selCount));
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "OpenGL error #%d [0x%X] : [%s] at file: [%s], line: %d", selCount,
             selCount, errorString,
             extractFileFromDirectoryPath(__FILE__).c_str(), __LINE__);
...
void Renderer::selectUsingColorPicking(Selection::UnitContainer &units,
  ...
  // printf("In [%s::%s] Line: %d rendererUnits =
  ...
  if (rendererUnits.empty() == false) {
    copy(rendererUnits.begin(), rendererUnits.end(),
         std::inserter(unitsVector, unitsVector.begin()));
  }

  if (unitsVector.empty() == false) {
    vector<int> pickedList =
        BaseColorPickEntity::getPickedList(x, y, w, h, unitsVector);
    // printf("In [%s::%s] Line: %d pickedList = %d models rendered =
  ...
  if (withObjectSelection == true && unitFound == false) {
    ...
    if (rendererObjects.empty() == false) {
      copy(rendererObjects.begin(), rendererObjects.end(),
           std::inserter(objectsVector, objectsVector.begin()));
    }

    if (objectsVector.empty() == false) {
      vector<int> pickedList =
          BaseColorPickEntity::getPickedList(x, y, w, h, objectsVector);
      // printf("In [%s::%s] Line: %d pickedList = %d models rendered =
      // %d\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,pickedList.size(),rendererModels.size());
      ...
      if (pickedList.empty() == false) {
        for (unsigned int i = 0; i < pickedList.size(); ++i) {
          ...
          if (rendererObjects.empty() == false &&
              index < (int)rendererObjects.size()) {
  ...
  if (shadowsOffDueToMinRender == false &&
    ...
    if (shadowMapFrame == 0) {
      ...
      if (nearestLightPos.w == 0.f) {
        ...
        assert(tf != NULL);
        float ang = tf->isDay() ? computeSunAngle(tf->getTime())
                                : computeMoonAngle(tf->getTime());
        ang = radToDeg(ang);
        ...
        glLoadIdentity();
        glRotatef(-90, -1, 0, 0);
        glTranslatef(-nearestLightPos.x, -nearestLightPos.y - 2,
                     -nearestLightPos.z);
      ...
      glBindTexture(GL_TEXTURE_2D, shadowMapHandle);
      assertGl();
      glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0, shadowTextureSize,
                          shadowTextureSize);
...
void Renderer::autoConfig() {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false) {
    ...
    // isGlExtensionSupported("GL_ARB_shadow_ambient");
    bool shadowExtensions = isGlExtensionSupported("GL_ARB_shadow");
...
void Renderer::clearZBuffer() { glClear(GL_DEPTH_BUFFER_BIT); }
...
void Renderer::loadConfig() {
  ...
  Texture2D::Filter textureFilter =
      strToTextureFilter(config.getString("Filter"));
...
//	Config &config= Config::getInstance();
//	Texture2D::Filter textureFilter =
// strToTextureFilter(config.getString("Filter")); 	int maxAnisotropy
//= config.getInt("FilterMaxAnisotropy");
//
//	Texture2D *texture =
...
void Renderer::saveScreen(const string &path, int w, int h) {
  ...
  Pixmap2D *pixmapScreenShot =
      new Pixmap2D(sm.getScreenW(), sm.getScreenH(), 3);
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__);
  glReadPixels(0, 0, pixmapScreenShot->getW(), pixmapScreenShot->getH(), GL_RGB,
               GL_UNSIGNED_BYTE, pixmapScreenShot->getPixels());
  ...
  MutexSafeWrapper safeMutex(
      saveScreenShotThreadAccessor,
      string(extractFileFromDirectoryPath(__FILE__).c_str()) + "_" +
          intToStr(__LINE__));
...
vector<Unit *> Renderer::renderUnitsFast(bool renderingShadows,
  ...
  if (qCache.visibleQuadUnitList.empty() == false) {
    ...
    for (int k = 0; k < 2; k++) {
      if (k == 0) {
        ...
        if (colorPickingSelection == true) {
          // clear depth buffer to get units behind buildings rendered in front
          // of them
          glClear(GL_DEPTH_BUFFER_BIT);
        }
      ...
      for (int visibleUnitIndex = 0;
        ...
        if (renderOnlyBuildings == true &&
            unit->getType()->hasSkillClass(scMove)) {
          continue;
        }

        if (renderOnlyBuildings == false &&
            !unit->getType()->hasSkillClass(scMove)) {
          continue;
        }
        ...
        model->updateInterpolationVertices(unit->getAnimProgressAsFloat(),
                                           unit->isAlive() &&
                                               !unit->isAnimProgressBound());
        ...
        modelRenderer->render(model,
                              renderingShadows ? rmShadows : rmSelection);
...
vector<Object *> Renderer::renderObjectsFast(bool renderingShadows,
                                             bool resourceOnly,
                                             bool colorPickingSelection) {
...
void Renderer::checkGlCaps() {
  ...
  if (glewIsSupported("GL_VERSION_2_1") == false) {
    ...
    message += "You may solve this problem by installing your latest video "
               "card drivers";
...
void Renderer::checkExtension(const string &extension, const string &msg) {
  ...
  if (!isGlExtensionSupported(extension.c_str())) {
    string str = "OpenGL extension not supported: " + extension +
                 ", required for " + msg;
...
void Renderer::init3dList() {
  ...
  // need to execute, because if not gluPerspective takes no effect and
  // gluLoadMatrix is wrong render3dSetup(); glEndList();
...
void Renderer::render3dSetup() {
  // if(SystemFlags::VERBOSE_MODE_ENABLED) printf("In [%s::%s
  // %d]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__);
  const Metrics &metrics = Metrics::getInstance();
  // if(SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
  // SystemFlags::OutputDebug(SystemFlags::debugSystem,"In [%s::%s Line:
...
void Renderer::render2dMenuSetup() {
  ...
  if (glActiveTexture != NULL) {
    ...
    snprintf(szBuf, 8096,
             ...
             "is only supported if the GL version is 1.3 or greater,\nor if "
             "the ARB_multitexture extension is supported!",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__);
    throw megaglest_runtime_error(szBuf);
  }
...
//		glMatrixMode(GL_PROJECTION);
//		glLoadIdentity();
//		gluPerspective(perspFov, metrics.getAspectRatio(),
//perspNearPlane, 1000000);
//
//		//texture state
//		glEnable(GL_TEXTURE_2D);
//		glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
...
// defSpecularColor.ptr()); 		glMaterialfv(GL_FRONT_AND_BACK,
// GL_AMBIENT, defAmbientColor.ptr()); 		glMaterialfv(GL_FRONT_AND_BACK,
// GL_DIFFUSE, defDiffuseColor.ptr()); 		glColor4fv(defColor.ptr());
//		glColorMaterial(GL_FRONT_AND_BACK, GL_DIFFUSE);
//
//		//blend state
//		glDisable(GL_BLEND);
...
// ==================== private aux drawing ====================
void Renderer::renderHealthBar(Vec3f v, Unit *unit, float height,
                               bool lineBorder, const Texture2D *texture,
  ...
  if (backgroundTexture != NULL) {
    ...
    glBindTexture(
        GL_TEXTURE_2D,
        static_cast<const Texture2DGl *>(backgroundTexture)->getHandle());
    glColor4f(1.f, 1.f, 1.f, 1.f);
    // glColor4f(red+0.1f,green+0.1f,0.1f,0.5f);
    glBegin(GL_QUADS);
    glTexCoord2i(0, 1);
    glVertex3fv(
        (v - (rightVectorTexture * width - upVectorTexture * height * yOffset))
            .ptr());
    glTexCoord2i(0, 0);
    glVertex3fv(
        (v - (rightVectorTexture * width + upVectorTexture * height * yOffset))
  ...
  // hpbar
  barCount++;
  internalRenderHp(numberOfBars, barCount, hp, v, width, height, rightVector,
                   upVector);
  ...
  if (ep > -1.0f) {
    ...
    internalRenderHp(numberOfBars, barCount, ep, v, width, height, rightVector,
                     upVector);
  }
  if (productionPercent != -1) {
    barCount++;
    ...
    internalRenderHp(numberOfBars, barCount, (float)productionPercent / 100, v,
                     width, height, rightVector, upVector);
  }

  //	glBegin(GL_QUADS);
  //		if(ep < -2.0f) {
  //			//hpbar
  //			glVertex3fv((v - (rightVector*width -
  // upVector*height)).ptr()); 			glVertex3fv((v - (rightVector*width
  // +
  // upVector*height)).ptr()); 			glVertex3fv((v + (rightVector*hp*width
  // -
  // upVector*height)).ptr()); 			glVertex3fv((v + (rightVector*hp*width
  // + upVector*height)).ptr());
  //
  ...
  // upVector*height)).ptr()); 			glVertex3fv((v - (rightVector*width
  // +
  // upVector*height*0.0f)).ptr()); 			glVertex3fv((v +
  // (rightVector*hp*width - upVector*height*0.0f)).ptr());
  // glVertex3fv((v + (rightVector*hp*width + upVector*height)).ptr());
  //			//epbar
  //			glColor4f(brightness,0,brightness,0.4f);
  //			glVertex3fv((v - (rightVector*width +
  // upVector*height*0.0f)).ptr()); 			glVertex3fv((v - (rightVector*width
  // +
  // upVector*height)).ptr()); 			glVertex3fv((v + (rightVector*ep*width
  // -
  // upVector*height)).ptr()); 			glVertex3fv((v + (rightVector*ep*width
  // - upVector*height*0.0f)).ptr());
  //		}
  //	glEnd();
  ...
  if (lineBorder) {
    ...
    glBegin(GL_LINE_LOOP);
    glVertex3fv(
        (v - (rightVector * width - upVector * height * yOffset)).ptr());
    glVertex3fv(
        (v - (rightVector * width + upVector * height * yOffset)).ptr());
    glVertex3fv(
        (v + (rightVector * width - upVector * height * yOffset)).ptr());
...
void Renderer::internalRenderHp(int numberOfBars, int barNumber, float hp,
                                Vec3f posVector, float width,
                                float singleHPheight, Vec3f rightVector,
                                Vec3f upVector) {
  ...
  glVertex3fv(
      (posVector - (rightVector * width + upVector * offsetBottom)).ptr());
  glVertex3fv(
      (posVector + (rightVector * hp * width - upVector * offsetBottom)).ptr());
  glVertex3fv(
      (posVector + (rightVector * hp * width + upVector * offsetTop)).ptr());
  glEnd();
}
...
void Renderer::renderSelectionCircle(Vec3f v, int size, float radius,
  ...
  gluCylinder(disc, radius * (size - thickness), radius * size, thickness, 30,
              1);
  ...
  //		float j=degToRad(k);
  //		glVertex3f(v.x+std::cos(j)*.9*radius*size, v.y+thickness,
  // v.z+std::sin(j)*.9*radius*size);
...
void Renderer::renderArrow(const Vec3f &pos1, const Vec3f &pos2,
  ...
  Vec3f pos2Right =
      pos2 - normal * (width - 0.05f) - dir * arrowEndSize * width;
...
void Renderer::renderProgressBar3D(int size, int x, int y, Font3D *font,
                                   int customWidth, string prefixLabel,
                                   bool centeredText, int customHeight) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  }

  // Makiong this smaller than 14 is a bad idea (since the font size is never
  // smaller than that)
  ...
  // printf("Render progress bar3d renderText [%s] y = %d, centeredText =
...
//			glDisable(GL_CULL_FACE);
//
//			float h1 = map->getCell(renderPos.x,
// renderPos.y)->getHeight(); 			float h2 =
// map->getCell(renderPos.x,
// renderPos.y+1)->getHeight(); 			float h3 =
// map->getCell(renderPos.x+1,
// renderPos.y)->getHeight(); 			float h4 =
...
void Renderer::renderQuad(int x, int y, int w, int h,
                          const Texture2D *texture) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) {
    return;
  }
...
Texture2D::Filter Renderer::strToTextureFilter(const string &s) {
  ...
  throw megaglest_runtime_error(
      "Error converting from string to FilterType, found: " + s);
}
...
void Renderer::renderUnitTitles3D(Font3D *font, Vec3f color) {
      ...
      if (unit != NULL) {
        if (unit->getVisible() == true) {
          if (unit->getCurrentUnitTitle() != "") {
            ...
            renderText3D(unit->getCurrentUnitTitle(), font, color,
                         std::fabs(screenPos.x) + 5, std::fabs(screenPos.y) + 5,
                         false);
            // unitRenderedList[unit->getId()] = true;
          } else {
            string str = unit->getFullName(unit->showTranslatedTechTree()) +
                         " - " + intToStr(unit->getId()) + " [" +
                         unit->getPosNotThreadSafe().getString() + "]";
            Vec3f screenPos = unit->getScreenPos();
            renderText3D(str, font, color, std::fabs(screenPos.x) + 5,
                         std::fabs(screenPos.y) + 5, false);
          }
        }
      }
    }
  ...
  if(renderUnitTitleList.empty() == false) {
          for(int idx = 0; idx < renderUnitTitleList.size(); idx++) {
                  ...
                  if(validUnit != NULL &&
  unitRenderedList.find(validUnit->getId()) == unitRenderedList.end()) { string
  str = validUnit->getFullName() + " - " + intToStr(validUnit->getId());
                          //get the screen coordinates
                          Vec3f &screenPos = unitInfo.second;
                          renderText(str, font, color, fabs(screenPos.x) + 5,
  fabs(screenPos.y) + 5, false);
                          //SystemFlags::OutputDebug(SystemFlags::debugSystem,"In
  [%s::%s Line: %d] screenPos.x = %f, screenPos.y = %f, screenPos.z =
    ...
    for (int idx = 0; idx < (int)visibleFrameUnitList.size(); idx++) {
      ...
      if (unit != NULL) {
        if (unit->getCurrentUnitTitle() != "") {
          // get the screen coordinates
          Vec3f screenPos = unit->getScreenPos();
          renderText(unit->getCurrentUnitTitle(), font, color,
                     std::fabs(screenPos.x) + 5, std::fabs(screenPos.y) + 5,
                     false);
          ...
          string str = unit->getFullName(unit->showTranslatedTechTree()) +
                       ...
                       unit->getPosNotThreadSafe().getString() + "]";
          Vec3f screenPos = unit->getScreenPos();
          renderText(str, font, color, std::fabs(screenPos.x) + 5,
                     std::fabs(screenPos.y) + 5, false);
        }
      }
    }
...
void Renderer::removeObjectFromQuadCache(const Object *o) {
  ...
  for (int visibleIndex = 0;
    ...
    if (currentObj == o) {
      qCache.visibleObjectList.erase(qCache.visibleObjectList.begin() +
                                     visibleIndex);
      break;
    }
  }
...
void Renderer::removeUnitFromQuadCache(const Unit *unit) {
  ...
  for (int visibleIndex = 0;
    ...
    if (currentUnit == unit) {
      qCache.visibleQuadUnitList.erase(qCache.visibleQuadUnitList.begin() +
                                       visibleIndex);
      break;
    }
  }
  for (int visibleIndex = 0; visibleIndex < (int)qCache.visibleUnitList.size();
       ++visibleIndex) {
    Unit *currentUnit = qCache.visibleUnitList[visibleIndex];
    if (currentUnit == unit) {
      qCache.visibleUnitList.erase(qCache.visibleUnitList.begin() +
                                   visibleIndex);
      break;
    }
  }
...
VisibleQuadContainerCache &Renderer::getQuadCache(bool updateOnDirtyFrame,
  ...
  if (game != NULL && game->getWorld() != NULL) {
    ...
    if (forceNew == true || (updateOnDirtyFrame == true &&
      ...
      for (int i = 0; i < world->getFactionCount(); ++i) {
        ...
        for (int j = 0; j < faction->getUnitCount(); ++j) {
          ...
          if (VisibleQuadContainerCache::enableFrustumCalcs == true) {
            // bool insideQuad 	= PointInFrustum(quadCache.frustumData,
            // unit->getCurrVector().x, unit->getCurrVector().y,
            // unit->getCurrVector().z );
            bool insideQuad = CubeInFrustum(quadCache.frustumData,
                                            unit->getCurrMidHeightVector().x,
                                            unit->getCurrMidHeightVector().y,
                                            unit->getCurrMidHeightVector().z,
                                            unit->getType()->getRenderSize());
          ...
          if (unitBuildPending == true) {
            const UnitBuildInfo &pendingUnit =
                unit->getBuildCommandPendingInfo();
            ...
            if (VisibleQuadContainerCache::enableFrustumCalcs == true) {
              ...
              // unit->getCurrVector().z );
              bool insideQuad = CubeInFrustum(
                  quadCache.frustumData, pos3f.x, pos3f.y, pos3f.z,
                  pendingUnit.buildUnit->getRenderSize());
              bool renderInMap = world->toRenderUnit(pendingUnit);
              if (insideQuad == false || renderInMap == false) {
                if (renderInMap == true) {
                  quadCache.visibleQuadUnitBuildList.push_back(pendingUnit);
                }
                unitBuildCheckedForRender =
                    true; // no more need to check any further;
                          // Currently don't need this list
                          // quadCache.inVisibleUnitList.push_back(unit);
              }

              // printf("#2 Unit build added? insideQuad = %d, renderInMap =
            ...
            if (unitBuildCheckedForRender == false) {
              ...
              // printf("#3 Unit build added? insideQuad = %d, renderInMap =
              // %d\n",insideQuad,renderInMap);
            }

            // printf("#4 quadCache.visibleQuadUnitBuildList.size() =
      ...
      if (forceNew == true || visibleQuad != quadCache.lastVisibleQuad) {
        ...
        for (int visibleIndex = 0;
             visibleIndex < (int)quadCache.visibleObjectList.size();
             ++visibleIndex) {
        ...
        while (pqi.next()) {
          ...
          if (map->isInside(pos)) {
            ...
            if (VisibleQuadContainerCache::enableFrustumCalcs == true) {
              if (o != NULL) {
                // bool insideQuad 	= PointInFrustum(quadCache.frustumData,
                // o->getPos().x, o->getPos().y, o->getPos().z );
                bool insideQuad =
                    CubeInFrustum(quadCache.frustumData, o->getPos().x,
                                  o->getPos().y, o->getPos().z, 1);
                if (insideQuad == false) {
                  o->setVisible(false);
                  continue;
                }
            ...
            bool cellExplored =
                world->showWorldForPlayer(world->getThisFactionIndex());
            if (cellExplored == false) {
              cellExplored = sc->isExplored(world->getThisTeamIndex());
            }

            bool isExplored = (cellExplored == true && o != NULL);
            // bool isVisible = (sc->isVisible(world->getThisTeamIndex()) && o
            // != NULL);
            bool isVisible = true;
            ...
            if (VisibleQuadContainerCache::enableFrustumCalcs == false) {
              ...
              if (markedCells.empty() == false) {
                if (markedCells.find(pos) != markedCells.end()) {
                  // printf("#1 ******** VISIBLE SCALED CELL FOUND in marked
                  // list pos [%s] markedCells.size() = " MG_SIZE_T_SPECIFIER
                  ...
                  // printf("#1 VISIBLE SCALED CELL NOT FOUND in marked list pos
                  // [%s] markedCells.size() = " MG_SIZE_T_SPECIFIER
                  // "\n",pos.getString().c_str(),markedCells.size());
                }
              }
            } else {
              ...
              bool insideQuad =
                  CubeInFrustum(quadCache.frustumData, sc->getVertex().x,
                                sc->getVertex().y, sc->getVertex().z, 0);
              //}
              if (!insideQuad) {
              ...
              if (insideQuad == true) {
                ...
                if (markedCells.empty() == false) {
                  if (markedCells.find(pos) != markedCells.end()) {
                    // printf("#2 ******** VISIBLE SCALED CELL FOUND in marked
                    // list pos [%s] markedCells.size() = " MG_SIZE_T_SPECIFIER
                    // "\n",pos.getString().c_str(),markedCells.size());
                    // if(markedCells.empty() == false) {
                    // quadCache.visibleScaledCellToScreenPosList[pos]=computeScreenPosition(sc->getVertex());
                    ...
                    // printf("#2 VISIBLE SCALED CELL NOT FOUND in marked list
                    // pos [%s] markedCells.size() = " MG_SIZE_T_SPECIFIER
                    // "\n",pos.getString().c_str(),markedCells.size());
                  }
                }
              }
...
void Renderer::updateMarkedCellScreenPosQuadCache(Vec2i pos) {
  ...
  SurfaceCell *sc = map->getSurfaceCell(pos);
  quadCache.visibleScaledCellToScreenPosList[pos] =
      computeScreenPosition(sc->getVertex());
}
...
std::pair<bool, Vec3f> Renderer::posInCellQuadCache(Vec2i pos) {
  ...
  if (std::find(quadCache.visibleScaledCellList.begin(),
...
void Renderer::renderMapPreview(const MapPreview *map, bool renderAll,
  ...
  static bool supportFBOs = Texture2DGl().supports_FBO_RBO();
  if (Config::getInstance().getBool("LegacyMapPreviewRendering", "false") ==
      true) {
    supportFBOs = false;
  }
  ...
  if (supportFBOs == false || renderToTexture == NULL ||
      *renderToTexture == NULL) {
    glTranslatef(static_cast<float>(screenPosX),
                 static_cast<float>(screenPosY) - clientH, 0.0f);
  }
  ...
  for (int j = 0; j < map->getH(); j++) {
    for (int i = 0; i < map->getW(); i++) {
      ...
      if (renderAll == true) {
        ...
        if (renderAll && (map->getObject(i, j) != 0) &&
            (map->getObject(i, j) != 10)) {
          glPointSize(cellSize / 2.f);
          glBegin(GL_POINTS);
          glVertex2f(i * cellSize + cellSize / 2.f,
                     clientH - j * cellSize - cellSize / 2.f);
          glEnd();
        }
      }
      ...
      if (renderAll == true) {
        ...
        if (renderAll && map->getResource(i, j) != 0) {
          ...
          glVertex2f(i * cellSize + cellSize, clientH - j * cellSize);
          glVertex2f(i * cellSize, clientH - j * cellSize);
          glVertex2f(i * cellSize + cellSize,
                     clientH - j * cellSize - cellSize);
          glEnd();
        }
      }
    }
  ...
  for (int i = 0; i < map->getMaxFactions(); i++) {
    ...
    vertices[i * 4] =
        Vec2f((map->getStartLocationX(i) - 1) * cellSize,
              clientH - (map->getStartLocationY(i) - 1) * cellSize);
    vertices[(i * 4) + 1] = Vec2f(
        (map->getStartLocationX(i) + 1) * cellSize + playerCrossSize,
        clientH - (map->getStartLocationY(i) + 1) * cellSize - playerCrossSize);
    vertices[(i * 4) + 2] = Vec2f(
        (map->getStartLocationX(i) - 1) * cellSize,
        clientH - (map->getStartLocationY(i) + 1) * cellSize - playerCrossSize);
    vertices[(i * 4) + 3] =
        Vec2f((map->getStartLocationX(i) + 1) * cellSize + playerCrossSize,
              clientH - (map->getStartLocationY(i) - 1) * cellSize);
  }
...
void Renderer::setLastRenderFps(int value) {
  lastRenderFps = value;
  smoothedRenderFps =
      (MIN_FPS_NORMAL_RENDERING * smoothedRenderFps + lastRenderFps) /
      (MIN_FPS_NORMAL_RENDERING + 1.0f);
...
std::size_t Renderer::getCurrentPixelByteCount(ResourceScope rs) const {
  ...
  for (int i = (rs == rsCount ? 0 : rs); i < rsCount; ++i) {
    if (textureManager[i] != NULL) {
      const ::Shared::Graphics::TextureContainer &textures =
          textureManager[i]->getTextures();
...
Texture2D *Renderer::preloadTexture(string logoFilename) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                             "In [%s::%s Line: %d] logoFilename [%s]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, logoFilename.c_str());
  ...
  if (logoFilename != "") {
    ...
    if (crcFactionPreviewTextureCache.find(logoFilename) !=
        crcFactionPreviewTextureCache.end()) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                 "In [%s::%s Line: %d] logoFilename [%s]\n",
...
void Renderer::cycleShowDebugUILevel() {
  // printf("#1 showDebugUILevel = %d, debugui_fps = %d, debugui_unit_titles =
...
void Renderer::renderFPSWhenEnabled(int lastFps) {
  if (getShowDebugUI() == true) {
    ...
    if (Renderer::renderText3DEnabled) {
      renderText3D("FPS: " + intToStr(lastFps), coreData.getMenuFontNormal3D(),
                   Vec3f(1.f), 10, 10, false);
    } else {
      renderText("FPS: " + intToStr(lastFps), coreData.getMenuFontNormal(),
                 Vec3f(1.f), 10, 10, false);
    }
  }
}
...
void Renderer::renderPopupMenu(PopupMenu *menu) {
  ...
  glColor4f(0.5f, 0.5f, 0.5f, 0.25f);
  glVertex2i(menu->getX() + menu->getW(),
             menu->getY() + 90 * menu->getH() / 100);
  glEnd();
  ...
  if (renderText3DEnabled == true) {
    // text
    renderTextBoundingBox3D(menu->getHeader(), menu->getFont3D(), fontColor,
                            menu->getX(),
                            menu->getY() + 93 * menu->getH() / 100,
                            menu->getW(), 0, true, false, false, -1, -1);
    ...
    int renderY =
        menu->getY() + menu->getH() - fontMetrics->getHeight(menu->getHeader());
    renderTextShadow(menu->getHeader(), menu->getFont(), fontColor, renderX,
                     renderY, true);

    // renderText(button->getText(), button->getFont(), color,x + (w / 2), y +
    // (h / 2), true);
  }
  ...
  // menu->getFont3D()->getMetrics(); 			currentButtonWidth =
  // fontMetrics->getTextWidth(button->getText());
  //		}
  //		else {
  //			FontMetrics *fontMetrics= menu->getFont()->getMetrics();
  //			currentButtonWidth =
...
void Renderer::renderVideoLoading(int progressPercent) {
  ...
  if (CoreData::getInstance().getMenuFontBig3D() != NULL) {
    ...
    int renderX =
        (w / 2) -
        (CoreData::getInstance().getMenuFontBig3D()->getMetrics()->getTextWidth(
             textToRender) /
         2);
    int h = metrics.getVirtualH();
    int renderY =
        (h / 2) +
        (CoreData::getInstance().getMenuFontBig3D()->getMetrics()->getHeight(
             textToRender) /
         2);

    renderText3D(textToRender, CoreData::getInstance().getMenuFontBig3D(),
                 Vec4f(1.f, 1.f, 0.f, anim), renderX, renderY, false);
  } else {
    renderText(textToRender, CoreData::getInstance().getMenuFontBig(),

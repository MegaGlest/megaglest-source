...
void UnitParticleSystemType::load(
  ...
  if (particleSystemNode->hasChild("shape")) {
    ...
    shape = UnitParticleSystem::strToShape(
        shapeNode->getAttribute("value")->getRestrictedValue());
    if (shape == UnitParticleSystem::sConical) {
      angle =
          shapeNode->getChild("angle")->getAttribute("value")->getFloatValue();
    }
  } else {
    shape = UnitParticleSystem::sLinear;
  }
  ...
  if ((minRadius == 0) && (shape == UnitParticleSystem::sConical)) {
    minRadius = 0.001f; // fudge it so we aren't generating particles that are
                        // exactly centred
    if (minRadius > radius)
      radius = minRadius;
  }
  ...
  if (particleSystemNode->hasChild("relativeDirection")) {
    const XmlNode *relativeDirectionNode =
        particleSystemNode->getChild("relativeDirection");
    relativeDirection =
        relativeDirectionNode->getAttribute("value")->getBoolValue();
  } else {
  ...
  if (particleSystemNode->hasChild("static-particle-count")) {
    // staticParticleCount
    const XmlNode *staticParticleCountNode =
        particleSystemNode->getChild("static-particle-count");
    staticParticleCount =
        staticParticleCountNode->getAttribute("value")->getIntValue();
  } else {
    staticParticleCount = 0;
  }
  ...
  if (particleSystemNode->hasChild("isVisibleAtNight")) {
    const XmlNode *isVisibleAtNightNode =
        particleSystemNode->getChild("isVisibleAtNight");
    isVisibleAtNight =
        isVisibleAtNightNode->getAttribute("value")->getBoolValue();
  } else {
    isVisibleAtNight = true;
  }
  ...
  if (particleSystemNode->hasChild("isVisibleAtDay")) {
    const XmlNode *isVisibleAtDayNode =
        particleSystemNode->getChild("isVisibleAtDay");
  ...
  if (particleSystemNode->hasChild("radiusBasedStartenergy")) {
    const XmlNode *isVisibleAtDayNode =
        particleSystemNode->getChild("radiusBasedStartenergy");
    radiusBasedStartenergy =
        isVisibleAtDayNode->getAttribute("value")->getBoolValue();
  } else {
  ...
  if (particleSystemNode->hasChild("lifetime")) {
    ...
    const float lifetime_secs =
        lifetimeNode->getAttribute("value")->getFloatValue();
    if (lifetime_secs < 0 && lifetime_secs != -1)
      throw megaglest_runtime_error(
          "particle effect lifetime cannot be negative (-1 means inherited "
          "from parent particle)");
...
const void UnitParticleSystemType::setValues(UnitParticleSystem *ups) {
  // whilst we extend ParticleSystemType we don't use
  // ParticleSystemType::setValues() add instances of all children; some
  // settings will cascade to all children
...
void UnitParticleSystemType::loadGame(const XmlNode *rootNode) {
  ...
  const XmlNode *unitParticleSystemTypeNode =
      rootNode->getChild("UnitParticleSystemType");

  shape = static_cast<UnitParticleSystem::Shape>(
      unitParticleSystemTypeNode->getAttribute("shape")->getIntValue());
  ...
  minRadius =
      unitParticleSystemTypeNode->getAttribute("minRadius")->getFloatValue();
  emissionRateFade =
      unitParticleSystemTypeNode->getAttribute("emissionRateFade")
          ->getFloatValue();
  direction = Vec3f::strToVec3(
      unitParticleSystemTypeNode->getAttribute("direction")->getValue());
  relative =
  ...
  relativeDirection =
      (unitParticleSystemTypeNode->getAttribute("relativeDirection")
           ->getIntValue() != 0);
  fixed =
      (unitParticleSystemTypeNode->getAttribute("fixed")->getIntValue() != 0);
  staticParticleCount =
      unitParticleSystemTypeNode->getAttribute("staticParticleCount")
          ->getIntValue();
  isVisibleAtNight =
      (unitParticleSystemTypeNode->getAttribute("isVisibleAtNight")
           ->getIntValue() != 0);
  isVisibleAtDay = (unitParticleSystemTypeNode->getAttribute("isVisibleAtDay")
                        ->getIntValue() != 0);
  isDaylightAffected =
      (unitParticleSystemTypeNode->getAttribute("isDaylightAffected")
           ->getIntValue() != 0);
  radiusBasedStartenergy =
      (unitParticleSystemTypeNode->getAttribute("radiusBasedStartenergy")
           ->getIntValue() != 0);
  delay = unitParticleSystemTypeNode->getAttribute("delay")->getIntValue();
  lifetime =
      unitParticleSystemTypeNode->getAttribute("lifetime")->getIntValue();
  startTime =
      unitParticleSystemTypeNode->getAttribute("startTime")->getFloatValue();
  endTime =
      unitParticleSystemTypeNode->getAttribute("endTime")->getFloatValue();
}
...
void UnitParticleSystemType::saveGame(XmlNode *rootNode) {
  ...
  XmlNode *unitParticleSystemTypeNode =
      rootNode->addChild("UnitParticleSystemType");

  //	UnitParticleSystem::Shape shape;
  unitParticleSystemTypeNode->addAttribute("shape", intToStr(shape),
                                           mapTagReplacements);
  //	float angle;
  unitParticleSystemTypeNode->addAttribute("angle", floatToStr(angle, 6),
                                           mapTagReplacements);
  //	float radius;
  unitParticleSystemTypeNode->addAttribute("radius", floatToStr(radius, 6),
                                           mapTagReplacements);
  //	float minRadius;
  unitParticleSystemTypeNode->addAttribute(
      "minRadius", floatToStr(minRadius, 6), mapTagReplacements);
  //	float emissionRateFade;
  unitParticleSystemTypeNode->addAttribute(
      "emissionRateFade", floatToStr(emissionRateFade, 6), mapTagReplacements);
  //	Vec3f direction;
  unitParticleSystemTypeNode->addAttribute("direction", direction.getString(),
                                           mapTagReplacements);
  //    bool relative;
  unitParticleSystemTypeNode->addAttribute("relative", intToStr(relative),
                                           mapTagReplacements);
  //    string meshName;
  unitParticleSystemTypeNode->addAttribute("meshName", meshName,
                                           mapTagReplacements);
  //    bool relativeDirection;
  unitParticleSystemTypeNode->addAttribute(
      "relativeDirection", intToStr(relativeDirection), mapTagReplacements);
  //    bool fixed;
  unitParticleSystemTypeNode->addAttribute("fixed", intToStr(fixed),
                                           mapTagReplacements);
  ...
  unitParticleSystemTypeNode->addAttribute("radiusBasedStartenergy",
                                           intToStr(radiusBasedStartenergy),
                                           mapTagReplacements);
  //	int delay;
  unitParticleSystemTypeNode->addAttribute("delay", intToStr(delay),
                                           mapTagReplacements);
  //	int lifetime;
  unitParticleSystemTypeNode->addAttribute("lifetime", intToStr(lifetime),
                                           mapTagReplacements);
  //	float startTime;
  unitParticleSystemTypeNode->addAttribute(
      "startTime", floatToStr(startTime, 6), mapTagReplacements);
  //	float endTime;
  unitParticleSystemTypeNode->addAttribute("endTime", floatToStr(endTime, 6),
                                           mapTagReplacements);
}

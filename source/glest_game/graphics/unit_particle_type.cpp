...
void UnitParticleSystemType::load(const XmlNode *particleSystemNode, const string &dir, RendererInterface *renderer,
    ...
    if (shape != UnitParticleSystem::sSpherical) {
        ...
        if ((shape == UnitParticleSystem::sConical) && (0.0 == direction.length())) throw megaglest_runtime_error("direction cannot be zero");
    ...
    if (particleSystemNode->hasChild("min-radius")) {
        ...
        if (minRadius > radius) throw megaglest_runtime_error("min-radius cannot be bigger than radius");
    } else {
        minRadius = 0;
    }
    ...
    if (particleSystemNode->hasChild("delay")) {
        ...
        if (delay_secs < 0) throw megaglest_runtime_error("particle effect delay cannot be negative");
    ...
    if (particleSystemNode->hasChild("lifetime")) {
        ...
        if (lifetime_secs < 0 && lifetime_secs != -1)
            throw megaglest_runtime_error("particle effect lifetime cannot be negative (-1 means inherited "
...
void UnitParticleSystemType::load(const XmlNode *particleFileNode, const string &dir, const string &path, RendererInterface *renderer,
                                  std::map<string, vector<pair<string, string>>> &loadedFileList, string parentLoader, string techtreePath) {
    try {
        XmlTree xmlTree;
        ...
        UnitParticleSystemType::load(particleSystemNode, dir, renderer, loadedFileList, parentLoader, techtreePath);

...
enum DebugUILevelType { debugui_fps = 0x01, debugui_unit_titles = 0x02 };
...
class MeshCallbackTeamColor : public MeshCallback {
 ...
 public:
  MeshCallbackTeamColor() : MeshCallback() { teamTexture = NULL; }
...
class VisibleQuadContainerCache {
 ...
 public:
  ...
  inline VisibleQuadContainerCache &operator=(
      const VisibleQuadContainerCache &obj) {
    CopyAll(obj);
    return *this;
  }
  ...
  vector<float> proj;
  vector<float> modl;
  map<pair<vector<float>, vector<float> >, vector<vector<float> > >
      frustumDataCache;
};
...
class VisibleQuadContainerVBOCache {
 public:
  ...
  uint32 m_nVBOFowTexCoords;      // Texture Coordinate VBO Name for fog of war
                                  // texture coords
  uint32 m_nVBOSurfaceTexCoords;  // Texture Coordinate VBO Name for surface
                                  // texture coords
  uint32 m_nVBONormals;           // Normal VBO Name
                                  // uint32	m_nVBOIndexes;
                                  // // Indexes VBO
  // Name
};
...
class Renderer : public RendererInterface,
 ...
 private:
  ...
  std::vector<std::pair<ParticleSystem *, ResourceScope> >
      deferredParticleSystems;
  ...
  std::map<string, std::pair<Chrono, std::vector<SurfaceData> > >
      mapSurfaceData;
  ...
  class MapRenderer {
   public:
    ...
    void renderVisibleLayers(const Map *map, float coordStep,
                             VisibleQuadContainerCache &qCache);
    void destroy();
   ...
   private:
    ...
    struct Layer {
      ...
      inline explicit Layer(Layer &obj) { *this = obj; }
      ...
      GLuint vbo_vertices, vbo_normals, vbo_fowTexCoords, vbo_surfTexCoords,
          vbo_indices;
      int indexCount;
      int textureHandle;
      string texturePath;
  ...
  // bool PointInFrustum(vector<vector<float> > &frustum, float x, float y,
  // float z ); bool SphereInFrustum(vector<vector<float> > &frustum,  float x,
  // float y, float z, float radius);
  bool CubeInFrustum(vector<vector<float> > &frustum, float x, float y, float z,
                     float size);
 ...
 public:
  ...
  void addToDeferredParticleSystemList(
  ...
  void endFont(::Shared::Graphics::Font *font, ResourceScope rs,
               bool mustExistInList = false);
  ...
  void cleanupUnitParticleSystems(vector<UnitParticleSystem *> &particleSystems,
                                  ResourceScope rs);
  bool validateParticleSystemStillExists(ParticleSystem *particleSystem,
  ...
  void renderGhostModel(const UnitType *building, const Vec2i pos,
                        CardinalDir facing, Vec4f *forceColor = NULL);

  void renderBackground(const Texture2D *texture);
  void renderTextureQuad(int x, int y, int w, int h, const Texture2D *texture,
                         float alpha = 1.f, const Vec3f *color = NULL);
  void renderConsole(const Console *console, ConsoleMode mode = consoleNormal,
                     int overrideMaxConsoleLines = -1);
  void renderConsoleLine3D(int lineIndex, int xPosition, int yPosition,
                           int lineHeight, Font3D *font,
                           string stringToHightlight,
                           const ConsoleLineInfo *lineInfo);
  void renderConsoleLine(int lineIndex, int xPosition, int yPosition,
  ...
  void renderText(const string &text, Font2D *font, float alpha, int x, int y,
                  bool centered = false);
  void renderText(const string &text, Font2D *font, const Vec3f &color, int x,
                  int y, bool centered = false);
  void renderText(const string &text, Font2D *font, const Vec4f &color, int x,
                  int y, bool centered = false);
  void renderTextShadow(const string &text, Font2D *font, const Vec4f &color,
                        int x, int y, bool centered = false);

  void renderText3D(const string &text, Font3D *font, float alpha, int x, int y,
                    bool centered);
  void renderText3D(const string &text, Font3D *font, const Vec3f &color, int x,
                    int y, bool centered);
  void renderText3D(const string &text, Font3D *font, const Vec4f &color, int x,
                    int y, bool centered);
  void renderTextShadow3D(const string &text, Font3D *font, const Vec4f &color,
                          int x, int y, bool centered = false);
  void renderProgressBar3D(int size, int x, int y, Font3D *font,
                           int customWidth = -1, string prefixLabel = "",
                           bool centeredText = true, int customHeight = -1);

  Vec2f getCentered3DPos(const string &text, Font3D *font, Vec2f &pos, int w,
                         int h, bool centeredW, bool centeredH);
  void renderTextBoundingBox3D(const string &text, Font3D *font,
                               const Vec4f &color, int x, int y, int w, int h,
                               bool centeredW, bool centeredH,
                               bool editModeEnabled, int maxEditWidth,
                               int maxEditRenderWidth);
  void renderTextBoundingBox3D(const string &text, Font3D *font,
                               const Vec3f &color, int x, int y, int w, int h,
                               bool centeredW, bool centeredH,
                               bool editModeEnabled, int maxEditWidth,
                               int maxEditRenderWidth);
  void renderTextBoundingBox3D(const string &text, Font3D *font, float alpha,
                               int x, int y, int w, int h, bool centeredW,
                               bool centeredH, bool editModeEnabled,
  ...
  void renderButton(GraphicButton *button,
                    const Vec4f *fontColorOverride = NULL,
                    bool *lightedOverride = NULL);
  ...
  void renderMapPreview(const MapPreview *map, bool renderAll, int screenX,
                        int screenY, Texture2D **renderToTexture = NULL);
  void renderMenuBackground(Camera *camera, float fade, Model *mainModel,
                            vector<Model *> characterModels,
                            const Vec3f characterPosition, float anim);

  // computing
  bool ccomputePosition(const Vec2i &screenPos, Vec2i &worldPos,
                        bool exactCoords = false);
  void computeSelected(Selection::UnitContainer &units, const Object *&obj,
                       const bool withObjectSelection, const Vec2i &posDown,
                       const Vec2i &posUp);
  void selectUsingColorPicking(Selection::UnitContainer &units,
                               const Object *&obj,
                               const bool withObjectSelection,
                               const Vec2i &posDown, const Vec2i &posUp);
  void selectUsingSelectionBuffer(Selection::UnitContainer &units,
                                  const Object *&obj,
                                  const bool withObjectSelection,
                                  const Vec2i &posDown, const Vec2i &posUp);
  void selectUsingFrustumSelection(Selection::UnitContainer &units,
                                   const Object *&obj,
                                   const bool withObjectSelection,
                                   const Vec2i &posDown, const Vec2i &posUp);
  ...
  void renderSpecialHighlightUnits(
  ...
  void renderVisibleMarkedCells(bool renderTextHint = false, int x = -1,
                                int y = -1);
  ...
  // void setCustom3dMenuList(GLuint *customlist3dMenu) { this->customlist3dMenu
  // = customlist3dMenu; } inline GLuint * getCustom3dMenuList() const { return
  // this->customlist3dMenu; }
 ...
 private:
  ...
  inline Vec4f computeWaterColor(float waterLevel, float cellHeight) {
    ...
    return Vec4f(1.f, 1.f, 1.f,
                 clamp((waterLevel - cellHeight) * waterFactor, 0.f, 1.f));
  }
  ...
  // selection render
  vector<Object *> renderObjectsFast(bool renderingShadows = false,
                                     bool resourceOnly = false,
                                     bool colorPickingSelection = false);
  vector<Unit *> renderUnitsFast(bool renderingShadows = false,
                                 bool colorPickingSelection = false);
  ...
  void renderHealthBar(Vec3f v, Unit *unit, float height, bool lineBorder,
                       const Texture2D *texture = NULL,
                       const Texture2D *backgroundTexture = NULL);
  void internalRenderHp(int numberOfBars, int barNumber, float hp,
                        Vec3f posVector, float width, float singleHPheight,
                        Vec3f rightVector, Vec3f upVector);
  void renderTeamColorEffect(Vec3f &v, int heigth, int size, Vec3f color,
                             const Texture2D *texture);
  void renderArrow(const Vec3f &pos1, const Vec3f &pos2, const Vec3f &color,
                   float width);
  ...
  int renderResource(const Faction *factionForResourceView,
                     bool localFactionResourcesOnly, bool twoResourceLines,
                     const ResourceType *rt, int startRow,

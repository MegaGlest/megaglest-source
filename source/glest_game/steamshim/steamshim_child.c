...
static char *getEnvVar(const char *key, char *buf, const size_t buflen) {
    ...
    if (!envr || (strlen(envr) >= buflen)) return NULL;
    strcpy(buf, envr);
    return buf;
} /* getEnvVar */
...
static int initPipes(void) {
    ...
    if (!getEnvVar("STEAMSHIM_READHANDLE", buf, sizeof(buf))) return 0;
    GPipeRead = (PipeType)strtoull(buf, 0, 10);

    if (!getEnvVar("STEAMSHIM_WRITEHANDLE", buf, sizeof(buf))) return 0;
...
void STEAMSHIM_deinit(void) {
    ...
    if (GPipeRead != NULLPIPE) closePipe(GPipeRead);
...
#undef PRINTGOTEVENT
    ...
    case SHIMEVENT_GETACHIEVEMENT:
        ...
        if (event.ivalue == 2) event.ivalue = event.okay = 0;
...
const STEAMSHIM_Event *STEAMSHIM_pump(void) {
    ...
    if (evlen && (br > evlen)) {
        ...
        br -= evlen + 1;
        if (br > 0) memmove(buf, buf + evlen + 1, br);
        return retval;
    } /* if */

...
const int NetworkInterface::readyWaitTimeout =
    99000; // 99 seconds to 0 looks good on the screen
...
NetworkInterface::NetworkInterface() {
  ...
  for (unsigned int index = 0; index < (unsigned int)GameConstants::maxPlayers;
       ++index) {
     ...
     %d\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,messageType,sizeof(messageType),dataSize);
          ...
          if(messageType < 0 || messageType >= nmtCount) {
                  if(getConnectHasHandshaked() == true) {
                          throw megaglest_runtime_error("Invalid message type: "
     + intToStr(messageType));
                  }
                  else {
                          if(SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
     SystemFlags::OutputDebug(SystemFlags::debugNetwork,"In [%s::%s Line: %d]
     Invalid message type = %d (no packet handshake yet so
  ...
  */
  ...
  if (socket != NULL &&
    ...
    if (dataSize >= (int)sizeof(messageType)) {
      ...
      // %d] socket->getDataToRead() dataSize =
      // %d\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,dataSize);
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s Line: %d] before recv\n",
      ...
      //			if(SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      // SystemFlags::OutputDebug(SystemFlags::debugNetwork,"In [%s::%s Line:
      // %d] socket->getDataToRead() iPeek = %d, messageType = %d [size =
      //%d]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,iPeek,messageType,sizeof(messageType));
      //			if(iPeek > 0) {
      int bytesReceived =
          socket->receive(&messageType, sizeof(messageType), true);
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s Line: %d] socket->getDataToRead() "
                                 "iPeek = %d, messageType = %d [size = %d]\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(),
      ...
      if (getConnectHasHandshaked() == true) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              ...
              "In [%s::%s Line: %d] Invalid message type = %d (no packet "
              "handshake yet so ignored)\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, messageType);
      }
    }
  }
...
double NetworkInterface::getLastPingLag() {
  ...
  return difftime((long int)time(NULL),
                  lastPingInfo.getPingReceivedLocalTime());
}

void NetworkInterface::DisplayErrorMessage(string sErr, bool closeSocket) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s Line: %d] sErr [%s]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, sErr.c_str());
  // SystemFlags::OutputDebug(SystemFlags::debugError,"In [%s::%s Line: %d] sErr
...
void NetworkInterface::clearChatInfo() {
  ...
  if (chatTextList.empty() == false) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] chatTextList.size() = %d\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, chatTextList.size());
    chatTextList.clear();
  }
}
...
void NetworkInterface::clearMarkedCellList() {
  ...
  if (markedCellList.empty() == false) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] markedCellList.size() = %d\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, markedCellList.size());
    markedCellList.clear();
  }
}

std::vector<UnMarkedCell>
NetworkInterface::getUnMarkedCellList(bool clearList) {
...
void NetworkInterface::clearUnMarkedCellList() {
  ...
  if (unmarkedCellList.empty() == false) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] unmarkedCellList.size() = %d\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, unmarkedCellList.size());
    unmarkedCellList.clear();
  }
}

std::vector<MarkedCell>
NetworkInterface::getHighlightedCellList(bool clearList) {
  std::vector<MarkedCell> result;
...
void GameNetworkInterface::requestCommand(const NetworkCommand *networkCommand,
                                          bool insertAtStart) {

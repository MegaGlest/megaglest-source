...
NetworkMessageType NetworkInterface::getNextMessageType(int waitMilliseconds) {
  ...
  */
    ...
    if (socket != NULL &&
        ((waitMilliseconds <= 0 && socket->hasDataToRead() == true) || (waitMilliseconds > 0 && socket->hasDataToReadWithWait(waitMilliseconds) == true))) {
        // peek message type
        int dataSize = socket->getDataToRead();
        if (dataSize >= (int)sizeof(messageType)) {
            // if(SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            ...
            // %d\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,dataSize);
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] before recv\n", extractFileFromDirectoryPath(__FILE__).c_str(),
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                         ...
                                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, bytesReceived, messageType,
                                         sizeof(messageType));
            //}
        }
        // else {
...
void NetworkInterface::clearMarkedCellList() {
    ...
    if (markedCellList.empty() == false) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] markedCellList.size() = %d\n",
...
void NetworkInterface::clearUnMarkedCellList() {
    ...
    if (unmarkedCellList.empty() == false) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] unmarkedCellList.size() = %d\n",

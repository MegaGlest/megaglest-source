...
void ClientInterface::updateLobby() {
  ...
  switch (networkMessageType) {
    ...
    case nmtIntro: {
      ...
      if (receiveMessage(&networkMessageIntro)) {
        ...
        this->joinGameInProgress =
            (networkMessageIntro.getGameInProgress() != 0);
        this->joinGameInProgressLaunch = false;
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              ...
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, networkMessageIntro.getGameState(),
              versionString.c_str(), sessionKey, playerIndex, serverFTPPort);

        // check consistency
        bool compatible =
            checkVersionComptability(networkMessageIntro.getVersionString(),
        ...
        if (compatible == false) {
          ...
          if (strncmp(platformFreeVersion.c_str(),
            ...
            string playerNameStr = getHumanPlayerName();
            sErr =
                "Server and client binary mismatch!\nYou have to use the "
            ...
            sendTextMessage(" Server:" + networkMessageIntro.getVersionString(),
                            -1, true, "");
            sendTextMessage(" Client: " + getNetworkVersionGITString(), -1,
                            true, "");
            ...
            string playerNameStr = getHumanPlayerName();
            sErr =
                "Warning, Server and client are using the same version but "
                "different platforms.\n\nServer: " +
                networkMessageIntro.getVersionString() +
                "\nClient: " + getNetworkVersionGITString() + " player [" +
                playerNameStr + "]";
        ...
        if (networkMessageIntro.getGameState() == nmgstOk) {
          ...
          NetworkMessageIntro sendNetworkMessageIntro(
              sessionKey, getNetworkVersionGITString(), getHumanPlayerName(),
              -1, nmgstOk, this->getSocket()->getConnectedIPAddress(),
              serverFTPPort, lang.getLanguage(),
          ...
          string sErr =
              "Cannot join the server because there are no open slots for new "
              "players.";
          DisplayErrorMessage(sErr);
          setQuit(true);
          close();
    ...
    case nmtBroadCastSetup: {
      ...
      if (receiveMessage(&networkMessageLaunch, networkMessageType)) {
        ...
        for (int factionIndex = 0;
             factionIndex < gameSettings.getFactionCount(); ++factionIndex) {
    ...
    case nmtQuit: {
      ...
      if (gotCmd == false) {
        throw megaglest_runtime_error(
            "error retrieving nmtQuit returned false!");
      }
      if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
              .enabled &&
          chrono.getMillis() > 0)
    ...
    default: {
      ...
      sendTextMessage(
          "Unexpected network message: " + intToStr(networkMessageType), -1,
          true, "");
      DisplayErrorMessage(sErr);
      sleep(1);
      ...
      switch (networkMessageType) {
        case nmtCommandList: {
          ...
          if (checkFrame != NULL) {
            ...
            for (int index = 0; index < GameConstants::maxPlayers; ++index) {
              printf(
                  "Frame: %d faction: %d local CRC: %u Remote CRC: %u\n",
        ...
        default: {
          ...
          DisplayErrorMessage(
              string(extractFileFromDirectoryPath(__FILE__).c_str()) +
              "::" + string(__FUNCTION__) +
              " Unexpected message in client interface: " +
              intToStr(networkMessageType));
          sleep(1);
...
bool ClientInterface::getNetworkCommand(int frameCount,
    ...
    for (; getQuit() == false && getQuitThread() == false;) {
      ...
      if (cachedPendingCommands.find(frameCount) !=
        ...
        if (waitForData == false) {
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf(
                "Client waiting for packet for frame: %d, "
  ...
  if (waitForData == true) {
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "Client waiting for packet FINISHED for frame: %d, "

...
void ClientInterface::update() {
    ...
    try {
        ...
        if ((currentFrameCount >= this->gameSettings.getNetworkFramePeriod() && currentFrameCount % this->gameSettings.getNetworkFramePeriod() == 0) ||
        ...
        if (lastNetworkCommandListSendTime > 0 && lastSendElapsed >= ClientInterface::maxNetworkCommandListSendTimeWait) {
            lastSentFrameCount = currentFrameCount;
            sendMessage(&networkMessageCommandList);
            lastNetworkCommandListSendTime = time(NULL);
        }
...
void ClientInterface::updateLobby() {
        ...
        if (receiveMessage(&networkMessageIntro)) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                         ...
                                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, networkMessageIntro.getGameState(),
                                         versionString.c_str(), sessionKey, playerIndex, serverFTPPort);
            ...
            if (networkMessageIntro.getGameState() == nmgstOk) {
                ...
                NetworkMessageIntro sendNetworkMessageIntro(
                    sessionKey, getNetworkVersionGITString(), getHumanPlayerName(), -1, nmgstOk, this->getSocket()->getConnectedIPAddress(), serverFTPPort,
                    lang.getLanguage(), networkMessageIntro.getGameInProgress(), Config::getInstance().getString("PlayerId", ""), getPlatformNameString());
                ...
                string sErr = "Cannot join the server because there are no open slots for new "
                              "players.";
                DisplayErrorMessage(sErr);
                setQuit(true);
                close();
        ...
        if (receiveMessage(&networkMessageSynchNetworkGameData)) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                         ...
                                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
                                         networkMessageSynchNetworkGameData.getTechCRCFileCount());
            ...
            try {
                ...
                tilesetCRC =
                    getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTilesets, scenarioDir),
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                             "In [%s::%s Line: %d] tilesetCRC info, local = %d, remote = "
                                             "%d, networkMessageSynchNetworkGameData.getTileset() = [%s]\n",
                                             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, tilesetCRC,
                                             networkMessageSynchNetworkGameData.getTilesetCRC(), networkMessageSynchNetworkGameData.getTileset().c_str());

                // tech, load before map because of resources
                techCRC = getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTechs, scenarioDir),
                                                                   string("/") + networkMessageSynchNetworkGameData.getTech() + string("/*"), ".xml", NULL);
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                             ...
                                             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, techCRC,
                                             networkMessageSynchNetworkGameData.getTechCRC(), networkMessageSynchNetworkGameData.getTech().c_str());
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                             "In [%s::%s Line: %d] mapCRC info, local = %d, remote = %d, "
                                             "file = [%s]\n",
                                             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, mapCRC,
                                             networkMessageSynchNetworkGameData.getMapCRC(), file.c_str());
    ...
    case nmtSynchNetworkGameDataFileCRCCheck: {
        ...
        if (receiveMessage(&networkMessageSynchNetworkGameDataFileCRCCheck)) {
            ...
            if (fileCRC != networkMessageSynchNetworkGameDataFileCRCCheck.getFileCRC()) {
                ...
                NetworkMessageSynchNetworkGameDataFileGet sendNetworkMessageSynchNetworkGameDataFileGet(
            ...
            if (networkMessageSynchNetworkGameDataFileCRCCheck.getFileIndex() < networkMessageSynchNetworkGameDataFileCRCCheck.getTotalFileCount()) {
                NetworkMessageSynchNetworkGameDataFileCRCCheck sendNetworkMessageSynchNetworkGameDataFileCRCCheck(
                    networkMessageSynchNetworkGameDataFileCRCCheck.getTotalFileCount(), networkMessageSynchNetworkGameDataFileCRCCheck.getFileIndex() + 1, 0,
                    "");
    ...
    case nmtMarkCell: {
        ...
        if (receiveMessage(&networkMessageMarkCell)) {
            ...
            this->setLastPingInfoToNow();
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s] got nmtMarkCell\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                         __FUNCTION__);
    ...
    case nmtHighlightCell: {
        ...
        if (receiveMessage(&networkMessageHighlightCell)) {
            ...
            this->setLastPingInfoToNow();
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s] got nmtHighlightCell\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                         __FUNCTION__);
        ...
        if (receiveMessage(&networkMessageLaunch, networkMessageType)) {
            ...
            for (int factionIndex = 0; factionIndex < gameSettings.getFactionCount(); ++factionIndex) {
                ...
                if (gameSettings.getStartLocationIndex(factionIndex) == playerIndex) {
                    ...
                    // printf("Client got game settings playerIndex = %d factionIndex =
                    // %d control = %d name =
                    ...
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                                 ...
                                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, gameSettings.getThisFactionIndex(),
                                                 playerIndex, factionIndex);
                }
            }
    ...
    case nmtPlayerIndexMessage: {
        ...
        if (receiveMessage(&playerIndexMessage)) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                         "In [%s::%s Line: %d] got nmtPlayerIndexMessage, playerIndex = "
                                         "%d\n",
    ...
    default: {
        string sErr = string(extractFileFromDirectoryPath(__FILE__).c_str()) + "::" + string(__FUNCTION__) +
    ...
    if (clientSocket != NULL && clientSocket->isConnected() == true && gotIntro == false &&
    ...
    if (isConnected() == true && getQuitThread() == false) {
        ...
        while (done == false && getQuitThread() == false) {
            ...
            case nmtCommandList: {
                ...
                if (checkFrame != NULL) {
                    if (networkMessageCommandList.getFrameCount() != *checkFrame) {
                        string sErr = "Player: " + getHumanPlayerName() +
                                      " got a Network synchronization error, frame counts do not "
                                      "match, server frameCount = " +
                                      intToStr(networkMessageCommandList.getFrameCount()) + ", local frameCount = " + intToStr(*checkFrame);
                    ...
                    for (int index = 0; index < GameConstants::maxPlayers; ++index) {
                        ...
                        if (networkMessageCommandList.getNetworkPlayerFactionCRC(index) != getNetworkPlayerFactionCRC(index)) {
                            string sErr = "Player: " + getHumanPlayerName() +
                                          ...
                                          "= " +
                                          uIntToStr(networkMessageCommandList.getNetworkPlayerFactionCRC(index)) +
                                          ", local CRC = " + uIntToStr(getNetworkPlayerFactionCRC(index));
                            sendTextMessage(sErr, -1, true, "");
                            DisplayErrorMessage(sErr);
                            sleep(1);
                ...
                for (int i = 0; i < networkMessageCommandList.getCommandCount(); ++i) {
                    ...
                    if (cachedPendingCommandCRCs.find(networkMessageCommandList.getFrameCount()) == cachedPendingCommandCRCs.end()) {
                        ...
                        for (int index = 0; index < GameConstants::maxPlayers; ++index) {
                            cachedPendingCommandCRCs[networkMessageCommandList.getFrameCount()].push_back(
                                networkMessageCommandList.getNetworkPlayerFactionCRC(index));
                        }
                    }
                }
                    ...
                    for (int i = 0; i < gameSettings.getFactionCount(); ++i) {
                        ...
                        if (gameSettings.getStartLocationIndex(i) == playerIndex) {
                            ...
                            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                                SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                                         "In [%s::%s Line: %d] "
                                                         "gameSettings.getThisFactionIndex(i) = %d, playerIndex = "
                                                         "%d, i = %d\n",
                                                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
                                                         gameSettings.getThisFactionIndex(), playerIndex, i);
                        }
                    }
                }
            } break;
            ...
            default: {
                ...
                DisplayErrorMessage(string(extractFileFromDirectoryPath(__FILE__).c_str()) + "::" + string(__FUNCTION__) +
                                    " Unexpected message in client interface: " + intToStr(networkMessageType));
                sleep(1);
            ...
            if (networkMessageType == nmtReady) {
                ...
                if (chrono.getMillis() > readyWaitTimeout) {
                    ...
                    if (chrono.getMillis() % 100 == 0) {
                        ...
                        char szBuf[8096] = "";
                        string updateTextFormat = "Waiting for network: %lld seconds elapsed (maximum wait time: "
                                                  "%d seconds)";
        ...
        for (unsigned int langIndex = 0; langIndex < languageList.size(); ++langIndex) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s] Line: %d %s %s %s\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                         __FUNCTION__, __LINE__, sErr.c_str(), sErr1.c_str(), sErr2.c_str());
...
void ClientInterface::sendTextMessage(const string &text, int teamIndex, bool echoLocal, string targetLanguage) {
...
NetworkMessageType ClientInterface::waitForMessage(int waitMicroseconds) {
  ...
  */
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 1)
        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s Line: %d] waiting took %lld msecs, msg = %d\n",
...
void ClientInterface::sendSwitchSetupRequest(string selectedFactionName, int8 currentSlotIndex, int8 toSlotIndex, int8 toTeam, string networkPlayerName,
    ...
    SwitchSetupRequest message =
        SwitchSetupRequest(selectedFactionName, currentSlotIndex, toSlotIndex, toTeam, networkPlayerName, networkPlayerStatus, flags, language);
    sendMessage(&message);
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
}

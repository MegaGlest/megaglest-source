...
NetworkCommand::NetworkCommand(World *world, int networkCommandType, int unitId, int commandTypeId, const Vec2i &pos, int unitTypeId, int targetId, int facing,
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] Created NetworkCommand as follows:\n%s\n", __FILE__, __FUNCTION__, __LINE__,
                                 toString().c_str());
}
...
void NetworkCommand::preprocessNetworkCommand(World *world) {
    if (networkCommandType == nctGiveCommand) {
        ...
        if (unit != NULL) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] (unit == NULL) %s\n", __FILE__, __FUNCTION__, __LINE__,
                                         toString().c_str());
        }
    }
}
...
string NetworkCommand::toString() const {
    ...
    snprintf(szBuf, 8096,
             ...
             networkCommandType, unitId, commandTypeId, positionX, positionY, unitTypeId, targetId, wantQueue, fromFactionIndex, unitFactionUnitCount,
             unitFactionIndex, commandStateType, commandStateValue, unitCommandGroupId);

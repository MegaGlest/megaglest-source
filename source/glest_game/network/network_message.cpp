...
bool NetworkMessage::receive(Socket *socket, void *data, int dataSize,
                             bool tryReceiveUntilDataSizeMet) {
  if (socket != NULL) {
    int dataReceived =
        socket->receive(data, dataSize, tryReceiveUntilDataSizeMet);
    if (dataReceived != dataSize) {
      ...
      if (socket != NULL && socket->isSocketValid() == true) {
        throw megaglest_runtime_error(
            "Error receiving NetworkMessage, dataReceived = " +
            intToStr(dataReceived) + ", dataSize = " + intToStr(dataSize));
      } else {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                .enabled)
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "In [%s::%s Line: %d] socket has been disconnected\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__);
      }
    } else {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "In [%s::%s Line: %d] dataSize = %d, dataReceived = %d\n",
            extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
            __LINE__, dataSize, dataReceived);
...
void NetworkMessage::send(Socket *socket, const void *data, int dataSize) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s Line: %d] socket = %p, data = %p, dataSize = %d\n",
  ...
  if (socket != NULL) {
    ...
    if (sendResult != dataSize) {
      if (socket != NULL && socket->isSocketValid() == true) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 "Error sending NetworkMessage, sendResult = %d, dataSize = %d",
                 sendResult, dataSize);
...
void NetworkMessage::send(Socket *socket, const void *data, int dataSize,
                          int8 messageType, uint32 compressedLength) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s Line: %d] socket = %p, data = %p, dataSize = %d\n",
  ...
  if (socket != NULL) {
    ...
    memcpy(&out_buffer[msgTypeSize + compressedSize], (const char *)data,
           dataSize);
...
string NetworkMessage::getNetworkPacketStats() {
  string result = "Current Timer Milliseconds: " +
                  intToStr(NetworkMessage::statsTimer.getMillis()) + "\n";

  for (std::map<NetworkMessageStatisticType, int64>::iterator iterMap =
           mapMessageStats.begin();
       iterMap != mapMessageStats.end(); ++iterMap) {
    switch (iterMap->first) {
    case netmsgstPacketsPerMillisecondSend:
      result += "send p / msec: " + intToStr(iterMap->second) +
                (iterMap->second > 10 ? "  ****WARNING WIN32 LIMIT EXCEEDED****"
                                      : "") +
                "\n";
      break;
    ...
    case netmsgstPacketsPerSecondRecv:
      result += "recv p / sec: " + intToStr(iterMap->second) +
                (iterMap->second > 10 ? "  ****WARNING WIN32 LIMIT EXCEEDED****"
                                      : "") +
                "\n";
      break;
...
void NetworkMessage::dump_packet(string label, const void *data, int dataSize,
  ...
  if (config.getBool("DebugNetworkPacketStats", "false") == true) {
    ...
    if (NetworkMessage::statsTimer.getSeconds() -
            NetworkMessage::mapMessageStats[netmsgstLastEvent] >=
        3) {

      NetworkMessage::mapMessageStats[netmsgstLastEvent] =
          NetworkMessage::statsTimer.getSeconds();
      secondChanged = true;
    }
    ...
    if (isSend == true) {
      ...
      if (millisecondsSinceLastSend ==
          NetworkMessage::mapMessageStats
      ...
      if (secondsSinceLastSend ==
          NetworkMessage::mapMessageStats[netmsgstPacketsPerSecondSend_last]) {
        NetworkMessage::mapMessageStats
      ...
      if (secondsSinceLastRecv ==
          NetworkMessage::mapMessageStats[netmsgstPacketsPerSecondRecv_last]) {
        NetworkMessage::mapMessageStats
...
NetworkMessageIntro::NetworkMessageIntro(
    int32 sessionId, const string &versionString, const string &name,
    int playerIndex, NetworkGameStateType gameState, uint32 externalIp,
    uint32 ftpPort, const string &playerLanguage, int gameInProgress,
    const string &playerUUID, const string &platform) {
...
unsigned int NetworkMessageIntro::getPackedSize() {
  ...
  if (result == 0) {
    ...
    result = pack(buf, getPackedMessageFormat(), messageType,
                  packedData.sessionId, packedData.versionString.getBuffer(),
                  packedData.name.getBuffer(), packedData.playerIndex,
                  packedData.gameState, packedData.externalIp,
                  packedData.ftpPort, packedData.language.getBuffer(),
                  data.gameInProgress, packedData.playerUUID.getBuffer(),
...
void NetworkMessageIntro::unpackMessage(unsigned char *buf) {
  ...
  unpack(buf, getPackedMessageFormat(), &messageType, &data.sessionId,
         data.versionString.getBuffer(), data.name.getBuffer(),
         &data.playerIndex, &data.gameState, &data.externalIp, &data.ftpPort,
         data.language.getBuffer(), &data.gameInProgress,
         data.playerUUID.getBuffer(), data.platform.getBuffer());
  if (SystemFlags::VERBOSE_MODE_ENABLED)
...
unsigned char *NetworkMessageIntro::packMessage() {
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("\nIn [%s] about to pack...\n", __FUNCTION__);
  pack(buf, getPackedMessageFormat(), messageType, data.sessionId,
       data.versionString.getBuffer(), data.name.getBuffer(), data.playerIndex,
       data.gameState, data.externalIp, data.ftpPort, data.language.getBuffer(),
       data.gameInProgress, data.playerUUID.getBuffer(),
...
bool NetworkMessageIntro::receive(Socket *socket) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             ...
                             __FUNCTION__, __LINE__, data.playerIndex,
                             data.sessionId);
  return result;
}
...
void NetworkMessageIntro::toEndian() {
  ...
  if (bigEndianSystem == true) {
    ...
    data.playerIndex =
        Shared::PlatformByteOrder::toCommonEndian(data.playerIndex);
    data.gameState = Shared::PlatformByteOrder::toCommonEndian(data.gameState);
    data.externalIp =
        Shared::PlatformByteOrder::toCommonEndian(data.externalIp);
    data.ftpPort = Shared::PlatformByteOrder::toCommonEndian(data.ftpPort);

    data.gameInProgress =
        Shared::PlatformByteOrder::toCommonEndian(data.gameInProgress);
  }
}
void NetworkMessageIntro::fromEndian() {
  ...
  if (bigEndianSystem == true) {
    ...
    data.sessionId =
        Shared::PlatformByteOrder::fromCommonEndian(data.sessionId);
    data.playerIndex =
        Shared::PlatformByteOrder::fromCommonEndian(data.playerIndex);
    data.gameState =
        Shared::PlatformByteOrder::fromCommonEndian(data.gameState);
    data.externalIp =
        Shared::PlatformByteOrder::fromCommonEndian(data.externalIp);
...
const char *NetworkMessagePing::getPackedMessageFormat() const { return "clq"; }
...
unsigned int NetworkMessagePing::getPackedSize() {
  ...
  if (result == 0) {
    ...
    unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
    result = pack(buf, getPackedMessageFormat(), messageType,
                  packedData.pingFrequency, packedData.pingTime);
    delete[] buf;
  }
  return result;
}
void NetworkMessagePing::unpackMessage(unsigned char *buf) {
  unpack(buf, getPackedMessageFormat(), &messageType, &data.pingFrequency,
         &data.pingTime);
}

unsigned char *NetworkMessagePing::packMessage() {
  unsigned char *buf = new unsigned char[getPackedSize() + 1];
  pack(buf, getPackedMessageFormat(), messageType, data.pingFrequency,
       data.pingTime);
  return buf;
}
...
void NetworkMessagePing::toEndian() {
  ...
  if (bigEndianSystem == true) {
    messageType = Shared::PlatformByteOrder::toCommonEndian(messageType);
    data.pingFrequency =
        Shared::PlatformByteOrder::toCommonEndian(data.pingFrequency);
...
void NetworkMessagePing::fromEndian() {
  ...
  if (bigEndianSystem == true) {
    messageType = Shared::PlatformByteOrder::fromCommonEndian(messageType);
    data.pingFrequency =
        Shared::PlatformByteOrder::fromCommonEndian(data.pingFrequency);
...
const char *NetworkMessageReady::getPackedMessageFormat() const { return "cL"; }
...
unsigned int NetworkMessageReady::getPackedSize() {
  ...
  if (result == 0) {
    ...
    unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
    result =
        pack(buf, getPackedMessageFormat(), messageType, packedData.checksum);
    delete[] buf;
  }
  return result;
}
...
NetworkMessageLaunch::NetworkMessageLaunch(const GameSettings *gameSettings,
  ...
  vector<pair<string, uint32>> factionCRCList =
      gameSettings->getFactionCRCList();
  for (unsigned int i = 0;
       i < factionCRCList.size() && i < (unsigned int)maxFactionCRCCount; ++i) {
  ...
  for (int i = 0; i < GameConstants::maxPlayers; ++i) {
    ...
    data.resourceMultiplierIndex[i] =
  ...
  data.masterserver_admin_factionIndex =
      gameSettings->getMasterserver_admin_faction_index();
...
void NetworkMessageLaunch::buildGameSettings(GameSettings *gameSettings) const {
  ...
  gameSettings->setDefaultVictoryConditions(
      (data.defaultVictoryConditions != 0));
  ...
  gameSettings->setEnableObserverModeAtEndGame(
      (data.enableObserverModeAtEndGame != 0));
  gameSettings->setEnableServerControlledAI(
      (data.enableServerControlledAI != 0));
  gameSettings->setNetworkFramePeriod(data.networkFramePeriod);
  gameSettings->setNetworkPauseGameForLaggedClients(
      (data.networkPauseGameForLaggedClients != 0));
  gameSettings->setPathFinderType(
      static_cast<PathFinderType>(data.pathFinderType));
  ...
  for (int i = 0; i < GameConstants::maxPlayers; ++i) {
    ...
    gameSettings->setNetworkPlayerUUID(i,
                                       data.networkPlayerUUID[i].getString());
    gameSettings->setNetworkPlayerPlatform(
        i, data.networkPlayerPlatform[i].getString());
  ...
  gameSettings->setNetworkAllowNativeLanguageTechtree(
      (data.networkAllowNativeLanguageTechtree != 0));
}
...
unsigned int NetworkMessageLaunch::getPackedSize() {
  ...
  if (result == 0) {
    ...
    for (unsigned int index = 0; index < (unsigned int)maxFactionCRCCount;
         ++index) {
      packedData.factionCRCList[index] = 0;
    }
    for (unsigned int index = 0;
         index < (unsigned int)GameConstants::maxPlayers; ++index) {
    ...
    unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
    result = pack(
        buf, getPackedMessageFormat(), messageType,
        packedData.description.getBuffer(), packedData.map.getBuffer(),
        packedData.tileset.getBuffer(), packedData.tech.getBuffer(),
        ...
        packedData.networkPlayerLanguages[7].getBuffer(), packedData.mapCRC,
        packedData.mapFilter, packedData.tilesetCRC, packedData.techCRC,
        ...
        packedData.factionCRCList[0], packedData.factionCRCList[1],
        packedData.factionCRCList[2], packedData.factionCRCList[3],
        packedData.factionCRCList[4], packedData.factionCRCList[5],
        packedData.factionCRCList[6], packedData.factionCRCList[7],
        packedData.factionCRCList[8], packedData.factionCRCList[9],
        packedData.factionCRCList[10], packedData.factionCRCList[11],
        packedData.factionCRCList[12], packedData.factionCRCList[13],
        packedData.factionCRCList[14], packedData.factionCRCList[15],
        packedData.factionCRCList[16], packedData.factionCRCList[17],
        packedData.factionCRCList[18], packedData.factionCRCList[19],
        packedData.factionControls[0], packedData.factionControls[1],
        packedData.factionControls[2], packedData.factionControls[3],
        packedData.factionControls[4], packedData.factionControls[5],
        packedData.factionControls[6], packedData.factionControls[7],
        ...
        packedData.resourceMultiplierIndex[7], packedData.thisFactionIndex,
        packedData.factionCount, packedData.teams[0], packedData.teams[1],
        packedData.teams[2], packedData.teams[3], packedData.teams[4],
        packedData.teams[5], packedData.teams[6], packedData.teams[7],
        packedData.startLocationIndex[0], packedData.startLocationIndex[1],
        packedData.startLocationIndex[2], packedData.startLocationIndex[3],
        packedData.startLocationIndex[4], packedData.startLocationIndex[5],
        packedData.startLocationIndex[6], packedData.startLocationIndex[7],
        packedData.defaultResources, packedData.defaultUnits,
        packedData.defaultVictoryConditions, packedData.fogOfWar,
        packedData.allowObservers, packedData.enableObserverModeAtEndGame,
        packedData.enableServerControlledAI, packedData.networkFramePeriod,
        packedData.networkPauseGameForLaggedClients, packedData.pathFinderType,
        packedData.flagTypes1, packedData.aiAcceptSwitchTeamPercentChance,
        packedData.cpuReplacementMultiplier, packedData.masterserver_admin,
...
void NetworkMessageLaunch::unpackMessage(unsigned char *buf) {
  unpack(
      buf, getPackedMessageFormat(), &messageType, data.description.getBuffer(),
      data.map.getBuffer(), data.tileset.getBuffer(), data.tech.getBuffer(),
      ...
      data.networkPlayerPlatform[7].getBuffer(), &data.networkPlayerStatuses[0],
      &data.networkPlayerStatuses[1], &data.networkPlayerStatuses[2],
      &data.networkPlayerStatuses[3], &data.networkPlayerStatuses[4],
      &data.networkPlayerStatuses[5], &data.networkPlayerStatuses[6],
      ...
      data.networkPlayerLanguages[7].getBuffer(), &data.mapFilter, &data.mapCRC,
      &data.tilesetCRC, &data.techCRC, data.factionNameList[0].getBuffer(),
      data.factionNameList[1].getBuffer(), data.factionNameList[2].getBuffer(),
      data.factionNameList[3].getBuffer(), data.factionNameList[4].getBuffer(),
      data.factionNameList[5].getBuffer(), data.factionNameList[6].getBuffer(),
      data.factionNameList[7].getBuffer(), data.factionNameList[8].getBuffer(),
      data.factionNameList[9].getBuffer(), data.factionNameList[10].getBuffer(),
      ...
      data.factionNameList[19].getBuffer(), &data.factionCRCList[0],
      &data.factionCRCList[1], &data.factionCRCList[2], &data.factionCRCList[3],
      &data.factionCRCList[4], &data.factionCRCList[5], &data.factionCRCList[6],
      &data.factionCRCList[7], &data.factionCRCList[8], &data.factionCRCList[9],
      &data.factionCRCList[10], &data.factionCRCList[11],
      &data.factionCRCList[12], &data.factionCRCList[13],
      &data.factionCRCList[14], &data.factionCRCList[15],
      &data.factionCRCList[16], &data.factionCRCList[17],
      &data.factionCRCList[18], &data.factionCRCList[19],
      &data.factionControls[0], &data.factionControls[1],
      &data.factionControls[2], &data.factionControls[3],
      &data.factionControls[4], &data.factionControls[5],
      &data.factionControls[6], &data.factionControls[7],
      &data.resourceMultiplierIndex[0], &data.resourceMultiplierIndex[1],
      &data.resourceMultiplierIndex[2], &data.resourceMultiplierIndex[3],
      &data.resourceMultiplierIndex[4], &data.resourceMultiplierIndex[5],
      &data.resourceMultiplierIndex[6], &data.resourceMultiplierIndex[7],
      &data.thisFactionIndex, &data.factionCount, &data.teams[0],
      &data.teams[1], &data.teams[2], &data.teams[3], &data.teams[4],
      &data.teams[5], &data.teams[6], &data.teams[7],
      &data.startLocationIndex[0], &data.startLocationIndex[1],
      &data.startLocationIndex[2], &data.startLocationIndex[3],
      &data.startLocationIndex[4], &data.startLocationIndex[5],
      &data.startLocationIndex[6], &data.startLocationIndex[7],
      &data.defaultResources, &data.defaultUnits,
      &data.defaultVictoryConditions, &data.fogOfWar, &data.allowObservers,
      &data.enableObserverModeAtEndGame, &data.enableServerControlledAI,
      &data.networkFramePeriod, &data.networkPauseGameForLaggedClients,
      &data.pathFinderType, &data.flagTypes1,
      &data.aiAcceptSwitchTeamPercentChance, &data.cpuReplacementMultiplier,
      &data.masterserver_admin, &data.masterserver_admin_factionIndex,
      data.scenario.getBuffer(), data.networkPlayerUUID[0].getBuffer(),
      ...
      &data.networkAllowNativeLanguageTechtree, data.gameUUID.getBuffer());
}
...
unsigned char *NetworkMessageLaunch::packMessage() {
  ...
  pack(
      ...
      data.networkPlayerStatuses[3], data.networkPlayerStatuses[4],
      data.networkPlayerStatuses[5], data.networkPlayerStatuses[6],
      data.networkPlayerStatuses[7], data.networkPlayerLanguages[0].getBuffer(),
      data.networkPlayerLanguages[1].getBuffer(),
      data.networkPlayerLanguages[2].getBuffer(),
      data.networkPlayerLanguages[3].getBuffer(),
      data.networkPlayerLanguages[4].getBuffer(),
      ...
      data.factionCRCList[4], data.factionCRCList[5], data.factionCRCList[6],
      data.factionCRCList[7], data.factionCRCList[8], data.factionCRCList[9],
      data.factionCRCList[10], data.factionCRCList[11], data.factionCRCList[12],
      data.factionCRCList[13], data.factionCRCList[14], data.factionCRCList[15],
      data.factionCRCList[16], data.factionCRCList[17], data.factionCRCList[18],
      data.factionCRCList[19], data.factionControls[0], data.factionControls[1],
      data.factionControls[2], data.factionControls[3], data.factionControls[4],
      data.factionControls[5], data.factionControls[6], data.factionControls[7],
      data.resourceMultiplierIndex[0], data.resourceMultiplierIndex[1],
      data.resourceMultiplierIndex[2], data.resourceMultiplierIndex[3],
      data.resourceMultiplierIndex[4], data.resourceMultiplierIndex[5],
      data.resourceMultiplierIndex[6], data.resourceMultiplierIndex[7],
      data.thisFactionIndex, data.factionCount, data.teams[0], data.teams[1],
      data.teams[2], data.teams[3], data.teams[4], data.teams[5], data.teams[6],
      data.teams[7], data.startLocationIndex[0], data.startLocationIndex[1],
      data.startLocationIndex[2], data.startLocationIndex[3],
      data.startLocationIndex[4], data.startLocationIndex[5],
      data.startLocationIndex[6], data.startLocationIndex[7],
      data.defaultResources, data.defaultUnits, data.defaultVictoryConditions,
      data.fogOfWar, data.allowObservers, data.enableObserverModeAtEndGame,
      data.enableServerControlledAI, data.networkFramePeriod,
      data.networkPauseGameForLaggedClients, data.pathFinderType,
      data.flagTypes1, data.aiAcceptSwitchTeamPercentChance,
      data.cpuReplacementMultiplier, data.masterserver_admin,
      data.masterserver_admin_factionIndex, data.scenario.getBuffer(),
...
bool NetworkMessageLaunch::receive(Socket *socket) {
  ...
  if (useOldProtocol == true) {
    ...
    result = NetworkMessage::receive(socket, &compressedLength,
                                     sizeof(compressedLength), true);
    ...
    if (result == true && compressedLength > 0 && socket != NULL &&
        socket->isSocketValid()) {
      // printf("UnCompressed launch packet before: %u after:
      // %d\n",compressedLength,(int)getDataSize());

      unsigned char *compressedMessage =
          new unsigned char[compressedLength + 1];
      memset(compressedMessage, 0, compressedLength + 1);

      result = NetworkMessage::receive(socket, compressedMessage,
      ...
      if (result == true && socket != NULL && socket->isSocketValid()) {
        ...
        //++index) { 					printf("%u[%X][%d]
        //",index,buf[index],buf[index]); 					if(index
        //% 10 == 0) { 						printf("\n");
        //					}
        //				}
        //				printf("\n");
        ...
        std::pair<unsigned char *, unsigned long> decompressedBuffer =
            Shared::CompressionUtil::extractMemoryToMemory(
                compressedMessage, buffer_size, maxNetworkMessageSize);
...
NetworkMessageLaunch::getCompressedMessage() {
...
void NetworkMessageLaunch::send(Socket *socket) {
  ...
  if (useOldProtocol == true) {
    ...
    std::pair<unsigned char *, unsigned long> compressionResult =
    ...
    //*>(compressionResult.first); 		for(unsigned int index = 0; index <
    //(unsigned int)compressionResult.second; ++index) {
    // printf("%u[%X][%d]
    ...
    */
    NetworkMessage::send(socket, compressionResult.first,
                         compressionResult.second, messageType,
                         compressedLength);
...
void NetworkMessageLaunch::toEndian() {
  ...
  if (bigEndianSystem == true) {
    messageType = Shared::PlatformByteOrder::toCommonEndian(messageType);
    for (int i = 0; i < GameConstants::maxPlayers; ++i) {
      data.networkPlayerStatuses[i] = Shared::PlatformByteOrder::toCommonEndian(
          data.networkPlayerStatuses[i]);
      data.factionCRCList[i] =
          Shared::PlatformByteOrder::toCommonEndian(data.factionCRCList[i]);
      data.factionControls[i] =
          Shared::PlatformByteOrder::toCommonEndian(data.factionControls[i]);
      data.resourceMultiplierIndex[i] =
          Shared::PlatformByteOrder::toCommonEndian(
              data.resourceMultiplierIndex[i]);
      data.teams[i] = Shared::PlatformByteOrder::toCommonEndian(data.teams[i]);
      data.startLocationIndex[i] =
          Shared::PlatformByteOrder::toCommonEndian(data.startLocationIndex[i]);
    }
    ...
    data.tilesetCRC =
        Shared::PlatformByteOrder::toCommonEndian(data.tilesetCRC);
    data.techCRC = Shared::PlatformByteOrder::toCommonEndian(data.techCRC);
    data.thisFactionIndex =
        Shared::PlatformByteOrder::toCommonEndian(data.thisFactionIndex);
    data.factionCount =
        Shared::PlatformByteOrder::toCommonEndian(data.factionCount);
    data.defaultResources =
        Shared::PlatformByteOrder::toCommonEndian(data.defaultResources);
    data.defaultUnits =
        Shared::PlatformByteOrder::toCommonEndian(data.defaultUnits);

    data.defaultVictoryConditions = Shared::PlatformByteOrder::toCommonEndian(
        data.defaultVictoryConditions);
    data.fogOfWar = Shared::PlatformByteOrder::toCommonEndian(data.fogOfWar);
    data.allowObservers =
        Shared::PlatformByteOrder::toCommonEndian(data.allowObservers);
    data.enableObserverModeAtEndGame =
        Shared::PlatformByteOrder::toCommonEndian(
            data.enableObserverModeAtEndGame);
    data.enableServerControlledAI = Shared::PlatformByteOrder::toCommonEndian(
        data.enableServerControlledAI);
    data.networkFramePeriod =
        Shared::PlatformByteOrder::toCommonEndian(data.networkFramePeriod);
    data.networkPauseGameForLaggedClients =
        Shared::PlatformByteOrder::toCommonEndian(
            data.networkPauseGameForLaggedClients);
    data.pathFinderType =
        Shared::PlatformByteOrder::toCommonEndian(data.pathFinderType);
    data.flagTypes1 =
        Shared::PlatformByteOrder::toCommonEndian(data.flagTypes1);

    data.aiAcceptSwitchTeamPercentChance =
        Shared::PlatformByteOrder::toCommonEndian(
            data.aiAcceptSwitchTeamPercentChance);
    data.cpuReplacementMultiplier = Shared::PlatformByteOrder::toCommonEndian(
        data.cpuReplacementMultiplier);
    data.masterserver_admin =
        Shared::PlatformByteOrder::toCommonEndian(data.masterserver_admin);
    data.masterserver_admin_factionIndex =
        Shared::PlatformByteOrder::toCommonEndian(
            data.masterserver_admin_factionIndex);
...
void NetworkMessageLaunch::fromEndian() {
  ...
  if (bigEndianSystem == true) {
    ...
    for (int i = 0; i < GameConstants::maxPlayers; ++i) {
      data.networkPlayerStatuses[i] =
          Shared::PlatformByteOrder::fromCommonEndian(
              data.networkPlayerStatuses[i]);
      data.factionCRCList[i] =
          Shared::PlatformByteOrder::fromCommonEndian(data.factionCRCList[i]);
      data.factionControls[i] =
          Shared::PlatformByteOrder::fromCommonEndian(data.factionControls[i]);
      data.resourceMultiplierIndex[i] =
          Shared::PlatformByteOrder::fromCommonEndian(
              data.resourceMultiplierIndex[i]);
      data.teams[i] =
          Shared::PlatformByteOrder::fromCommonEndian(data.teams[i]);
      data.startLocationIndex[i] = Shared::PlatformByteOrder::fromCommonEndian(
          data.startLocationIndex[i]);
    }
    data.mapFilter =
        Shared::PlatformByteOrder::fromCommonEndian(data.mapFilter);
    data.mapCRC = Shared::PlatformByteOrder::fromCommonEndian(data.mapCRC);
    data.tilesetCRC =
        Shared::PlatformByteOrder::fromCommonEndian(data.tilesetCRC);
    data.techCRC = Shared::PlatformByteOrder::fromCommonEndian(data.techCRC);
    data.thisFactionIndex =
        Shared::PlatformByteOrder::fromCommonEndian(data.thisFactionIndex);
    data.factionCount =
        Shared::PlatformByteOrder::fromCommonEndian(data.factionCount);
    data.defaultResources =
        Shared::PlatformByteOrder::fromCommonEndian(data.defaultResources);
    data.defaultUnits =
        Shared::PlatformByteOrder::fromCommonEndian(data.defaultUnits);

    data.defaultVictoryConditions = Shared::PlatformByteOrder::fromCommonEndian(
        data.defaultVictoryConditions);
    data.fogOfWar = Shared::PlatformByteOrder::fromCommonEndian(data.fogOfWar);
    data.allowObservers =
        Shared::PlatformByteOrder::fromCommonEndian(data.allowObservers);
    data.enableObserverModeAtEndGame =
        Shared::PlatformByteOrder::fromCommonEndian(
            data.enableObserverModeAtEndGame);
    data.enableServerControlledAI = Shared::PlatformByteOrder::fromCommonEndian(
        data.enableServerControlledAI);
    data.networkFramePeriod =
        Shared::PlatformByteOrder::fromCommonEndian(data.networkFramePeriod);
    data.networkPauseGameForLaggedClients =
        Shared::PlatformByteOrder::fromCommonEndian(
            data.networkPauseGameForLaggedClients);
    data.pathFinderType =
        Shared::PlatformByteOrder::fromCommonEndian(data.pathFinderType);
    data.flagTypes1 =
        Shared::PlatformByteOrder::fromCommonEndian(data.flagTypes1);

    data.aiAcceptSwitchTeamPercentChance =
        Shared::PlatformByteOrder::fromCommonEndian(
            data.aiAcceptSwitchTeamPercentChance);
    data.cpuReplacementMultiplier = Shared::PlatformByteOrder::fromCommonEndian(
        data.cpuReplacementMultiplier);
    data.masterserver_admin =
...
bool NetworkMessageCommandList::addCommand(
...
unsigned int NetworkMessageCommandList::getPackedSizeHeader() {
  ...
  if (result == 0) {
    ...
    init(packedData);
    unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
    result = pack(buf, getPackedMessageFormatHeader(), packedData.messageType,
                  packedData.header.commandCount, packedData.header.frameCount,
...
void NetworkMessageCommandList::unpackMessageHeader(unsigned char *buf) {
  unpack(buf, getPackedMessageFormatHeader(), &data.messageType,
         &data.header.commandCount, &data.header.frameCount,
...
unsigned char *NetworkMessageCommandList::packMessageHeader() {
  ...
  pack(buf, getPackedMessageFormatHeader(), data.messageType,
       data.header.commandCount, data.header.frameCount,
       data.header.networkPlayerFactionCRC[0],
       data.header.networkPlayerFactionCRC[1],
       data.header.networkPlayerFactionCRC[2],
...
unsigned int NetworkMessageCommandList::getPackedSizeDetail(int count) {
  ...
  for (unsigned int i = 0; i < (unsigned int)count; ++i) {
    ...
    result +=
        pack(buf, getPackedMessageFormatDetail(), packedData.networkCommandType,
             packedData.unitId, packedData.unitTypeId, packedData.commandTypeId,
             packedData.positionX, packedData.positionY, packedData.targetId,
             packedData.wantQueue, packedData.fromFactionIndex,
             packedData.unitFactionUnitCount, packedData.unitFactionIndex,
             packedData.commandStateType, packedData.commandStateValue,
...
void NetworkMessageCommandList::unpackMessageDetail(unsigned char *buf,
                                                    int count) {
  data.commands.clear();
  ...
  for (unsigned int i = 0; i < (unsigned int)count; ++i) {
    unsigned int bytes_processed = unpack(
        bufMove, getPackedMessageFormatDetail(),
        &data.commands[i].networkCommandType, &data.commands[i].unitId,
        &data.commands[i].unitTypeId, &data.commands[i].commandTypeId,
        &data.commands[i].positionX, &data.commands[i].positionY,
        &data.commands[i].targetId, &data.commands[i].wantQueue,
        &data.commands[i].fromFactionIndex,
        &data.commands[i].unitFactionUnitCount,
        &data.commands[i].unitFactionIndex, &data.commands[i].commandStateType,
...
unsigned char *
NetworkMessageCommandList::packMessageDetail(uint16 totalCommand) {
  ...
  for (unsigned int i = 0; i < totalCommand; ++i) {
    unsigned int bytes_processed = pack(
        bufMove, getPackedMessageFormatDetail(),
        data.commands[i].networkCommandType, data.commands[i].unitId,
        data.commands[i].unitTypeId, data.commands[i].commandTypeId,
        data.commands[i].positionX, data.commands[i].positionY,
...
bool NetworkMessageCommandList::receive(Socket *socket) {
  ...
  bool result = false;
  if (useOldProtocol == true) {
    result = NetworkMessage::receive(socket, &data.header,
                                     commandListHeaderSize, true);
    if (result == true) {
  ...
  if (result == true) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               ...
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__, data.messageType,
                               data.header.commandCount,
    ...
    if (data.header.commandCount > 0) {
      ...
      if (useOldProtocol == true) {
        ...
        result = NetworkMessage::receive(socket, &data.commands[0],
                                         totalMsgSize, true);

        //				if(data.commands[0].getNetworkCommandType()
        //==
        // nctPauseResume) { 					printf("=====> IN Network
        // cmd type: %d [%d] frame:
        ...
        buf = new unsigned char[getPackedSizeDetail(data.header.commandCount) +
                                1];
        result = NetworkMessage::receive(
            socket, buf, getPackedSizeDetail(data.header.commandCount), true);
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          for (int idx = 0; idx < data.header.commandCount; ++idx) {
            ...
            SystemFlags::OutputDebug(
                ...
                "[%s]\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, idx, cmd.toString().c_str());
          }
        }
      } else {
        // if(SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] ERROR header not received as expected\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__);
    SystemFlags::OutputDebug(
...
void NetworkMessageCommandList::send(Socket *socket) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        ...
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        data.header.frameCount, data.header.commandCount, data.messageType);
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled ==
    ...
    if (totalCommand > 0) {
      for (int idx = 0; idx < totalCommand; ++idx) {
        ...
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "In [%s::%s Line: %d] index = %d, sent networkCommand [%s]\n",
            extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
            __LINE__, idx, cmd.toString().c_str());
      }
      ...
      SystemFlags::OutputDebug(
          ...
          "data.header.commandCount = %d, data.header.messageType = %d\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, data.header.frameCount, totalCommand, data.messageType);
    }
  }
}
...
void NetworkMessageCommandList::toEndianHeader() {
  ...
  if (bigEndianSystem == true) {
    data.messageType =
        Shared::PlatformByteOrder::toCommonEndian(data.messageType);
    data.header.commandCount =
        Shared::PlatformByteOrder::toCommonEndian(data.header.commandCount);
    data.header.frameCount =
        Shared::PlatformByteOrder::toCommonEndian(data.header.frameCount);
    for (int index = 0; index < GameConstants::maxPlayers; ++index) {
      data.header.networkPlayerFactionCRC[index] =
          Shared::PlatformByteOrder::toCommonEndian(
              data.header.networkPlayerFactionCRC[index]);
    }
  }
...
void NetworkMessageCommandList::fromEndianHeader() {
  ...
  if (bigEndianSystem == true) {
    data.messageType =
        Shared::PlatformByteOrder::fromCommonEndian(data.messageType);
    data.header.commandCount =
        Shared::PlatformByteOrder::fromCommonEndian(data.header.commandCount);
    data.header.frameCount =
        Shared::PlatformByteOrder::fromCommonEndian(data.header.frameCount);
    for (int index = 0; index < GameConstants::maxPlayers; ++index) {
      data.header.networkPlayerFactionCRC[index] =
          Shared::PlatformByteOrder::fromCommonEndian(
              data.header.networkPlayerFactionCRC[index]);
...
NetworkMessageText::NetworkMessageText() { messageType = nmtText; }
NetworkMessageText::NetworkMessageText(const string &text, int teamIndex,
                                       int playerIndex,
  ...
  if (result == 0) {
    ...
    result =
        pack(buf, getPackedMessageFormat(), messageType,
             packedData.text.getBuffer(), packedData.teamIndex,
             packedData.playerIndex, packedData.targetLanguage.getBuffer());
    delete[] buf;
  }
  return result;
}
void NetworkMessageText::unpackMessage(unsigned char *buf) {
  unpack(buf, getPackedMessageFormat(), &messageType, data.text.getBuffer(),
         &data.teamIndex, &data.playerIndex, data.targetLanguage.getBuffer());
}

unsigned char *NetworkMessageText::packMessage() {
  unsigned char *buf = new unsigned char[getPackedSize() + 1];
  pack(buf, getPackedMessageFormat(), messageType, data.text.getBuffer(),
       data.teamIndex, data.playerIndex, data.targetLanguage.getBuffer());
  return buf;
}
...
void NetworkMessageText::fromEndian() {
  ...
  if (bigEndianSystem == true) {
    messageType = Shared::PlatformByteOrder::fromCommonEndian(messageType);
    data.teamIndex =
        Shared::PlatformByteOrder::fromCommonEndian(data.teamIndex);
    data.playerIndex =
        Shared::PlatformByteOrder::fromCommonEndian(data.playerIndex);
  }
}
...
NetworkMessageQuit::NetworkMessageQuit() { messageType = nmtQuit; }

const char *NetworkMessageQuit::getPackedMessageFormat() const { return "c"; }
...
bool NetworkMessageQuit::receive(Socket *socket) {
  bool result = false;
  if (useOldProtocol == true) {
    result = NetworkMessage::receive(socket, &messageType, sizeof(messageType),
                                     true);
  } else {
    // fromEndian();
    unsigned char *buf = new unsigned char[getPackedSize() + 1];
...
NetworkMessageSynchNetworkGameData::NetworkMessageSynchNetworkGameData(
  ...
  if (gameSettings->getScenarioDir() != "") {
    ...
    if (EndsWith(scenarioDir, ".xml") == true) {
      ...
      scenarioDir = scenarioDir.erase(scenarioDir.size() -
  ...
  data.header.tilesetCRC = getFolderTreeContentsCheckSumRecursively(
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s Line: %d] data.tilesetCRC = %d, [%s]\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        data.header.tilesetCRC, gameSettings->getTileset().c_str());

  // tech, load before map because of resources
  data.header.techCRC = getFolderTreeContentsCheckSumRecursively(
      config.getPathListForType(ptTechs, scenarioDir),
      string("/") + gameSettings->getTech() + string("/*"), ".xml", NULL);

  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s Line: %d] data.techCRC = %d, [%s]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, data.header.techCRC,
  ...
  vctFileList = getFolderTreeContentsCheckSumListRecursively(
      config.getPathListForType(ptTechs, scenarioDir),
      string("/") + gameSettings->getTech() + string("/*"), ".xml",
      &vctFileList);
  data.header.techCRCFileCount =
      min((int)vctFileList.size(), (int)maxFileCRCCount);

  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s Line: %d] vctFileList.size() = %d, maxFileCRCCount = %d\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        vctFileList.size(), maxFileCRCCount);
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s Line: %d] data.mapCRC = %d, [%s]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, data.header.mapCRC,
                             gameSettings->getMap().c_str());
}

string NetworkMessageSynchNetworkGameData::getTechCRCFileMismatchReport(
    vector<std::pair<string, uint32>> &vctFileList) {
  string result = "Techtree: [" + data.header.tech.getString() +
                  "] Filecount local: " + intToStr(vctFileList.size()) +
  ...
  if (vctFileList.size() <= 0) {
    ...
    for (int idx = 0; idx < (int)vctFileList.size(); ++idx) {
      ...
      if (fileFound == false) {
        result =
            result + "local file [" + fileInfo.first + "] missing remotely.\n";
    ...
    for (int i = 0; i < (int)data.header.techCRCFileCount; ++i) {
      ...
      if (fileFound == false) {
        result =
            result + "remote file [" + networkFile + "] missing locally.\n";
...
const char *
NetworkMessageSynchNetworkGameData::getPackedMessageFormatHeader() const {
...
unsigned int NetworkMessageSynchNetworkGameData::getPackedSizeHeader() {
  ...
  if (result == 0) {
    ...
    result = pack(buf, getPackedMessageFormatHeader(), packedData.messageType,
                  ...
                  packedData.header.tech.getBuffer(), packedData.header.mapCRC,
                  packedData.header.tilesetCRC, packedData.header.techCRC,
                  packedData.header.techCRCFileCount);
    delete[] buf;
  }
  return result;
}
void NetworkMessageSynchNetworkGameData::unpackMessageHeader(
    unsigned char *buf) {
  unpack(buf, getPackedMessageFormatHeader(), &data.messageType,
         data.header.map.getBuffer(), data.header.tileset.getBuffer(),
         data.header.tech.getBuffer(), &data.header.mapCRC,
         &data.header.tilesetCRC, &data.header.techCRC,
         &data.header.techCRCFileCount);
}
...
unsigned char *NetworkMessageSynchNetworkGameData::packMessageHeader() {
  ...
  pack(buf, getPackedMessageFormatHeader(), data.messageType,
       data.header.map.getBuffer(), data.header.tileset.getBuffer(),
       data.header.tech.getBuffer(), data.header.mapCRC, data.header.tilesetCRC,
       data.header.techCRC, data.header.techCRCFileCount);
...
unsigned int NetworkMessageSynchNetworkGameData::getPackedSizeDetail() {
  ...
  if (result == 0) {
    ...
    for (unsigned int index = 0; index < (unsigned int)maxFileCRCCount;
         ++index) {
...
void NetworkMessageSynchNetworkGameData::unpackMessageDetail(
    unsigned char *buf) {
  for (unsigned int i = 0; i < (unsigned int)maxFileCRCCount; ++i) {
    unsigned int bytes_processed =
        unpack(buf, "255s", data.detail.techCRCFileList[i].getBuffer());
    buf += bytes_processed;
  }
  for (unsigned int i = 0; i < (unsigned int)maxFileCRCCount; ++i) {
    unsigned int bytes_processed =
        unpack(buf, "L", &data.detail.techCRCFileCRCList[i]);
    buf += bytes_processed;
  }
}
...
unsigned char *NetworkMessageSynchNetworkGameData::packMessageDetail() {
  ...
  for (unsigned int i = 0; i < (unsigned int)maxFileCRCCount; ++i) {
    unsigned int bytes_processed =
        pack(bufMove, "255s", data.detail.techCRCFileList[i].getBuffer());
    bufMove += bytes_processed;
  }
  for (unsigned int i = 0; i < (unsigned int)maxFileCRCCount; ++i) {
    unsigned int bytes_processed =
        pack(bufMove, "L", data.detail.techCRCFileCRCList[i]);
    bufMove += bytes_processed;
  }
...
bool NetworkMessageSynchNetworkGameData::receive(Socket *socket) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s Line: %d] about to get nmtSynchNetworkGameData\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
  ...
  if (result == true && data.header.techCRCFileCount > 0) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] messageType = %d, data.techCRCFileCount = %d\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, data.messageType, data.header.techCRCFileCount);

    // Here we loop possibly multiple times
    int packetLoopCount = 1;
    if (data.header.techCRCFileCount >
        (uint32)NetworkMessageSynchNetworkGameData::maxFileCRCPacketCount) {
      packetLoopCount =
          (data.header.techCRCFileCount /
           NetworkMessageSynchNetworkGameData::maxFileCRCPacketCount);
      if (data.header.techCRCFileCount %
              NetworkMessageSynchNetworkGameData::maxFileCRCPacketCount >
          0) {
        packetLoopCount++;
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "In [%s::%s Line: %d] packetLoopCount = %d\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__, packetLoopCount);

    for (int iPacketLoop = 0; result == true && iPacketLoop < packetLoopCount;
         ++iPacketLoop) {
      ...
      int packetFileCount = min((uint32)maxFileCountPerPacket,
                                data.header.techCRCFileCount - packetIndex);
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(
            ...
            extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
            __LINE__, iPacketLoop, packetIndex, maxFileCountPerPacket,
            packetFileCount, packetDetail1DataSize, packetDetail2DataSize);

      // Wait a max of x seconds for this message
      result = NetworkMessage::receive(
          socket, &data.detail.techCRCFileList[packetIndex],
          packetDetail1DataSize, true);
      if (result == true) {
        ...
        result = NetworkMessage::receive(
            socket, &data.detail.techCRCFileCRCList[packetIndex],
            packetDetail2DataSize, true);
      }
    }
    fromEndianDetail();
  }
...
void NetworkMessageSynchNetworkGameData::send(Socket *socket) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s Line: %d] about to send nmtSynchNetworkGameData\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
  ...
  if (totalFileCount > 0) {
    ...
    for (int iPacketLoop = 0; iPacketLoop < packetLoopCount; ++iPacketLoop) {
      ...
      int maxFileCountPerPacket = maxFileCRCPacketCount;
      int packetFileCount =
          min((uint32)maxFileCountPerPacket, totalFileCount - packetIndex);

      NetworkMessage::send(socket, &data.detail.techCRCFileList[packetIndex],
                           (DetailSize1 * packetFileCount));
      NetworkMessage::send(socket, &data.detail.techCRCFileCRCList[packetIndex],
                           (DetailSize2 * packetFileCount));
    }
...
void NetworkMessageSynchNetworkGameData::toEndianHeader() {
  ...
  if (bigEndianSystem == true) {
    data.messageType =
        Shared::PlatformByteOrder::toCommonEndian(data.messageType);
    data.header.mapCRC =
        Shared::PlatformByteOrder::toCommonEndian(data.header.mapCRC);
    data.header.tilesetCRC =
        Shared::PlatformByteOrder::toCommonEndian(data.header.tilesetCRC);
    data.header.techCRC =
        Shared::PlatformByteOrder::toCommonEndian(data.header.techCRC);
    data.header.techCRCFileCount =
        Shared::PlatformByteOrder::toCommonEndian(data.header.techCRCFileCount);
  }
}
void NetworkMessageSynchNetworkGameData::fromEndianHeader() {
  ...
  if (bigEndianSystem == true) {
    data.messageType =
        Shared::PlatformByteOrder::fromCommonEndian(data.messageType);
    data.header.mapCRC =
        Shared::PlatformByteOrder::fromCommonEndian(data.header.mapCRC);
    data.header.tilesetCRC =
        Shared::PlatformByteOrder::fromCommonEndian(data.header.tilesetCRC);
    data.header.techCRC =
        Shared::PlatformByteOrder::fromCommonEndian(data.header.techCRC);
    data.header.techCRCFileCount = Shared::PlatformByteOrder::fromCommonEndian(
        data.header.techCRCFileCount);
  }
}
...
NetworkMessageSynchNetworkGameDataStatus::
    NetworkMessageSynchNetworkGameDataStatus(
        uint32 mapCRC, uint32 tilesetCRC, uint32 techCRC,
        vector<std::pair<string, uint32>> &vctFileList) {
...
string NetworkMessageSynchNetworkGameDataStatus::getTechCRCFileMismatchReport(
    string techtree, vector<std::pair<string, uint32>> &vctFileList) {
  string result = "Techtree: [" + techtree +
                  "] Filecount local: " + intToStr(vctFileList.size()) +
                  " remote: " + intToStr(data.header.techCRCFileCount) + "\n";
  if (vctFileList.size() <= 0) {
    result = result + "Local player has no files.\n";
...
bool NetworkMessageSynchNetworkGameDataStatus::receive(Socket *socket) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s Line: %d] about to get nmtSynchNetworkGameDataStatus\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
  ...
  if (result == true && data.header.techCRCFileCount > 0) {
    ...
    for (uint32 iPacketLoop = 0; iPacketLoop < packetLoopCount; ++iPacketLoop) {
      ...
      uint32 packetFileCount = min((uint32)maxFileCountPerPacket,
                                   data.header.techCRCFileCount - packetIndex);
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 ...
                                 extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__, iPacketLoop,
                                 packetIndex, packetFileCount);

      result = NetworkMessage::receive(
          socket, &data.detail.techCRCFileList[packetIndex],
          ((uint32)DetailSize1 * packetFileCount), true);
      if (result == true) {
        for (int i = 0; i < (int)data.header.techCRCFileCount; ++i) {
          data.detail.techCRCFileList[i].nullTerminate();
        }
        ...
        result = NetworkMessage::receive(
            socket, &data.detail.techCRCFileCRCList[packetIndex],
            ((uint32)DetailSize2 * packetFileCount), true);
      }
    }
    fromEndianDetail();
  }
...
void NetworkMessageSynchNetworkGameDataStatus::toEndianDetail(
...
NetworkMessageSynchNetworkGameDataFileCRCCheck::
    NetworkMessageSynchNetworkGameDataFileCRCCheck(uint32 totalFileCount,
                                                   uint32 fileIndex,
                                                   uint32 fileCRC,
...
const char *
NetworkMessageSynchNetworkGameDataFileCRCCheck::getPackedMessageFormat() const {
  return "cLLL256s";
}
...
unsigned int NetworkMessageSynchNetworkGameDataFileCRCCheck::getPackedSize() {
  ...
  if (result == 0) {
    ...
    unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
    result = pack(buf, getPackedMessageFormat(), messageType,
                  packedData.totalFileCount, packedData.fileIndex,
                  packedData.fileCRC, packedData.fileName.getBuffer());
    delete[] buf;
  }
  return result;
}
void NetworkMessageSynchNetworkGameDataFileCRCCheck::unpackMessage(
    unsigned char *buf) {
  unpack(buf, getPackedMessageFormat(), &messageType, &data.totalFileCount,
         &data.fileIndex, &data.fileCRC, data.fileName.getBuffer());
}

unsigned char *NetworkMessageSynchNetworkGameDataFileCRCCheck::packMessage() {
  unsigned char *buf = new unsigned char[getPackedSize() + 1];
  pack(buf, getPackedMessageFormat(), messageType, data.totalFileCount,
       data.fileIndex, data.fileCRC, data.fileName.getBuffer());
  return buf;
}
...
void NetworkMessageSynchNetworkGameDataFileCRCCheck::send(Socket *socket) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s Line: %d] nmtSynchNetworkGameDataFileCRCCheck\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
...
void NetworkMessageSynchNetworkGameDataFileCRCCheck::toEndian() {
  ...
  if (bigEndianSystem == true) {
    messageType = Shared::PlatformByteOrder::toCommonEndian(messageType);
    data.totalFileCount =
        Shared::PlatformByteOrder::toCommonEndian(data.totalFileCount);
...
void NetworkMessageSynchNetworkGameDataFileCRCCheck::fromEndian() {
  ...
  if (bigEndianSystem == true) {
    messageType = Shared::PlatformByteOrder::fromCommonEndian(messageType);
    data.totalFileCount =
        Shared::PlatformByteOrder::fromCommonEndian(data.totalFileCount);
    data.fileIndex =
        Shared::PlatformByteOrder::fromCommonEndian(data.fileIndex);
...
const char *
NetworkMessageSynchNetworkGameDataFileGet::getPackedMessageFormat() const {
  return "c256s";
}
...
void NetworkMessageSynchNetworkGameDataFileGet::unpackMessage(
    unsigned char *buf) {
  unpack(buf, getPackedMessageFormat(), &messageType,
...
void NetworkMessageSynchNetworkGameDataFileGet::send(Socket *socket) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s Line: %d] nmtSynchNetworkGameDataFileGet\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
...
SwitchSetupRequest::SwitchSetupRequest(string selectedFactionName,
                                       int8 currentFactionIndex,
                                       int8 toFactionIndex, int8 toTeam,
...
unsigned int SwitchSetupRequest::getPackedSize() {
  ...
  if (result == 0) {
    ...
    result = pack(buf, getPackedMessageFormat(), messageType,
                  ...
                  packedData.currentSlotIndex, packedData.toSlotIndex,
                  packedData.toTeam, packedData.networkPlayerName.getBuffer(),
                  packedData.networkPlayerStatus, packedData.switchFlags,
...
void SwitchSetupRequest::unpackMessage(unsigned char *buf) {
  unpack(buf, getPackedMessageFormat(), &messageType,
         data.selectedFactionName.getBuffer(), &data.currentSlotIndex,
         &data.toSlotIndex, &data.toTeam, data.networkPlayerName.getBuffer(),
         &data.networkPlayerStatus, &data.switchFlags,
...
unsigned char *SwitchSetupRequest::packMessage() {
  ...
  pack(buf, getPackedMessageFormat(), messageType,
       data.selectedFactionName.getBuffer(), data.currentSlotIndex,
       data.toSlotIndex, data.toTeam, data.networkPlayerName.getBuffer(),
       data.networkPlayerStatus, data.switchFlags, data.language.getBuffer());
  return buf;
}
...
bool SwitchSetupRequest::receive(Socket *socket) {
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("In [%s::%s Line %d] data.networkPlayerName [%s]\n",
           extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
           __LINE__, data.networkPlayerName.getString().c_str());
...
void SwitchSetupRequest::toEndian() {
  ...
  if (bigEndianSystem == true) {
    ...
    data.currentSlotIndex =
        Shared::PlatformByteOrder::toCommonEndian(data.currentSlotIndex);
    data.toSlotIndex =
        Shared::PlatformByteOrder::toCommonEndian(data.toSlotIndex);
    data.toTeam = Shared::PlatformByteOrder::toCommonEndian(data.toTeam);
    data.networkPlayerStatus =
        Shared::PlatformByteOrder::toCommonEndian(data.networkPlayerStatus);
    data.switchFlags =
        Shared::PlatformByteOrder::toCommonEndian(data.switchFlags);
  }
}
void SwitchSetupRequest::fromEndian() {
  ...
  if (bigEndianSystem == true) {
    messageType = Shared::PlatformByteOrder::fromCommonEndian(messageType);
    data.currentSlotIndex =
        Shared::PlatformByteOrder::fromCommonEndian(data.currentSlotIndex);
    data.toSlotIndex =
        Shared::PlatformByteOrder::fromCommonEndian(data.toSlotIndex);
    data.toTeam = Shared::PlatformByteOrder::fromCommonEndian(data.toTeam);
    data.networkPlayerStatus =
        Shared::PlatformByteOrder::fromCommonEndian(data.networkPlayerStatus);
    data.switchFlags =
        Shared::PlatformByteOrder::fromCommonEndian(data.switchFlags);
  }
}
...
const char *PlayerIndexMessage::getPackedMessageFormat() const { return "ch"; }
...
unsigned int NetworkMessageLoadingStatus::getPackedSize() {
  ...
  if (result == 0) {
    ...
    unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
    result =
        pack(buf, getPackedMessageFormat(), messageType, packedData.status);
    delete[] buf;
  }
  return result;
}
...
// =====================================================
NetworkMessageMarkCell::NetworkMessageMarkCell() { messageType = nmtMarkCell; }
...
unsigned int NetworkMessageMarkCell::getPackedSize() {
  ...
  if (result == 0) {
    ...
    unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
    result =
        pack(buf, getPackedMessageFormat(), messageType, packedData.targetX,
             packedData.targetY, packedData.factionIndex,
             packedData.playerIndex, packedData.text.getBuffer());
    delete[] buf;
  }
  return result;
}
void NetworkMessageMarkCell::unpackMessage(unsigned char *buf) {
  unpack(buf, getPackedMessageFormat(), &messageType, &data.targetX,
         &data.targetY, &data.factionIndex, &data.playerIndex,
         data.text.getBuffer());
}

unsigned char *NetworkMessageMarkCell::packMessage() {
  unsigned char *buf = new unsigned char[getPackedSize() + 1];
  pack(buf, getPackedMessageFormat(), messageType, data.targetX, data.targetY,
       data.factionIndex, data.playerIndex, data.text.getBuffer());
...
void NetworkMessageMarkCell::toEndian() {
  ...
  if (bigEndianSystem == true) {
    ...
    data.factionIndex =
        Shared::PlatformByteOrder::toCommonEndian(data.factionIndex);
    data.playerIndex =
        Shared::PlatformByteOrder::toCommonEndian(data.playerIndex);
  }
}
void NetworkMessageMarkCell::fromEndian() {
  ...
  if (bigEndianSystem == true) {
    ...
    data.factionIndex =
        Shared::PlatformByteOrder::fromCommonEndian(data.factionIndex);
    data.playerIndex =
        Shared::PlatformByteOrder::fromCommonEndian(data.playerIndex);
  }
}
...
NetworkMessageUnMarkCell::NetworkMessageUnMarkCell(Vec2i target,
                                                   int factionIndex) {
  messageType = nmtUnMarkCell;
  data.targetX = target.x;
  data.targetY = target.y;
...
void NetworkMessageUnMarkCell::unpackMessage(unsigned char *buf) {
  unpack(buf, getPackedMessageFormat(), &messageType, &data.targetX,
         &data.targetY, &data.factionIndex);
}
...
NetworkMessageHighlightCell::NetworkMessageHighlightCell(Vec2i target,
                                                         int factionIndex) {
  messageType = nmtHighlightCell;
  data.targetX = target.x;
  data.targetY = target.y;

    ...
    if (socket != NULL) {
        ...
        if (dataReceived != dataSize) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] WARNING, dataReceived = %d dataSize = %d\n",
            ...
            if (SystemFlags::VERBOSE_MODE_ENABLED)
                printf("\nIn [%s::%s Line: %d] WARNING, dataReceived = %d dataSize = %d\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                       __LINE__, dataReceived, dataSize);

            if (socket != NULL && socket->isSocketValid() == true) {
                throw megaglest_runtime_error("Error receiving NetworkMessage, dataReceived = " + intToStr(dataReceived) +
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] dataSize = %d, dataReceived = %d\n",
...
string NetworkMessage::getNetworkPacketStats() {
    ...
    for (std::map<NetworkMessageStatisticType, int64>::iterator iterMap = mapMessageStats.begin(); iterMap != mapMessageStats.end(); ++iterMap) {
...
void NetworkMessage::dump_packet(string label, const void *data, int dataSize, bool isSend) {
    ...
    if (config.getBool("DebugNetworkPacketStats", "false") == true) {
        ...
        if (isSend == true) {
            ...
            // long)millisecondsSinceLastSend,(long
            // long)NetworkMessage::mapMessageStats[netmsgstPacketsPerMillisecondSend_last]
            // ); printf("%s",szBuf);
            ...
            if (secondsSinceLastSend == NetworkMessage::mapMessageStats[netmsgstPacketsPerSecondSend_last]) {
                ...
                NetworkMessage::mapMessageStats[netmsgstPacketsPerSecondSend] = (NetworkMessage::mapMessageStats[netmsgstPacketsPerSecondSend] +
                                                                                 NetworkMessage::mapMessageStats[netmsgstPacketsPerSecondSend_current_count]) /
                                                                                2;
            }

            NetworkMessage::mapMessageStats[netmsgstAverageSendSize] = (NetworkMessage::mapMessageStats[netmsgstAverageSendSize] + dataSize) / 2;
        } else {
            ...
            // long)millisecondsSinceLastRecv,(long
            // long)NetworkMessage::mapMessageStats[netmsgstPacketsPerMillisecondRecv_last]
            // ); printf("%s",szBuf);
            ...
            if (secondsSinceLastRecv == NetworkMessage::mapMessageStats[netmsgstPacketsPerSecondRecv_last]) {
                ...
                NetworkMessage::mapMessageStats[netmsgstPacketsPerSecondRecv] = (NetworkMessage::mapMessageStats[netmsgstPacketsPerSecondRecv] +
                                                                                 NetworkMessage::mapMessageStats[netmsgstPacketsPerSecondRecv_current_count]) /
                                                                                2;
            }

            NetworkMessage::mapMessageStats[netmsgstAverageRecvSize] = (NetworkMessage::mapMessageStats[netmsgstAverageRecvSize] + dataSize) / 2;
        }
    ...
    if (config.getBool("DebugNetworkPackets", "false") == true || config.getBool("DebugNetworkPacketSizes", "false") == true) {
...
NetworkMessageIntro::NetworkMessageIntro(int32 sessionId, const string &versionString, const string &name, int playerIndex, NetworkGameStateType gameState,
                                         uint32 externalIp, uint32 ftpPort, const string &playerLanguage, int gameInProgress, const string &playerUUID,
...
unsigned int NetworkMessageIntro::getPackedSize() {
    ...
    if (result == 0) {
        ...
        result = pack(buf, getPackedMessageFormat(), messageType, packedData.sessionId, packedData.versionString.getBuffer(), packedData.name.getBuffer(),
                      packedData.playerIndex, packedData.gameState, packedData.externalIp, packedData.ftpPort, packedData.language.getBuffer(),
                      data.gameInProgress, packedData.playerUUID.getBuffer(), packedData.platform.getBuffer());
        delete[] buf;
    }
    return result;
}
void NetworkMessageIntro::unpackMessage(unsigned char *buf) {
    ...
    unpack(buf, getPackedMessageFormat(), &messageType, &data.sessionId, data.versionString.getBuffer(), data.name.getBuffer(), &data.playerIndex,
           &data.gameState, &data.externalIp, &data.ftpPort, data.language.getBuffer(), &data.gameInProgress, data.playerUUID.getBuffer(),
...
unsigned char *NetworkMessageIntro::packMessage() {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED) printf("\nIn [%s] about to pack...\n", __FUNCTION__);
    pack(buf, getPackedMessageFormat(), messageType, data.sessionId, data.versionString.getBuffer(), data.name.getBuffer(), data.playerIndex, data.gameState,
         data.externalIp, data.ftpPort, data.language.getBuffer(), data.gameInProgress, data.playerUUID.getBuffer(), data.platform.getBuffer());
    return buf;
}
...
bool NetworkMessageIntro::receive(Socket *socket) {
    ...
    if (useOldProtocol == true) {
        ...
        unpackMessage(buf);
        // printf("Got packet size = %u data.messageType =
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s Line: %d] get nmtIntro, "
                                 "data.playerIndex = %d, data.sessionId = %d\n",
...
void NetworkMessageIntro::send(Socket *socket) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s Line: %d] sending nmtIntro, "
                                 "data.playerIndex = %d, data.sessionId = %d\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, data.playerIndex, data.sessionId);
    ...
    if (useOldProtocol == true) {
        ...
        // printf("Send packet size = %u data.messageType =
        // %d\n[%s]\n",getPackedSize(),data.messageType,buf);
...
unsigned int NetworkMessagePing::getPackedSize() {
    ...
    if (result == 0) {
        ...
        unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
        result = pack(buf, getPackedMessageFormat(), messageType, packedData.pingFrequency, packedData.pingTime);
        delete[] buf;
    }
    return result;
}
void NetworkMessagePing::unpackMessage(unsigned char *buf) {
    unpack(buf, getPackedMessageFormat(), &messageType, &data.pingFrequency, &data.pingTime);
}

unsigned char *NetworkMessagePing::packMessage() {
    unsigned char *buf = new unsigned char[getPackedSize() + 1];
    pack(buf, getPackedMessageFormat(), messageType, data.pingFrequency, data.pingTime);
    return buf;
}
...
unsigned int NetworkMessageReady::getPackedSize() {
    ...
    if (result == 0) {
        ...
        messageType = 0;
        unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
        result = pack(buf, getPackedMessageFormat(), messageType, packedData.checksum);
        delete[] buf;
    }
    return result;
}
void NetworkMessageReady::unpackMessage(unsigned char *buf) {
    unpack(buf, getPackedMessageFormat(), &messageType, &data.checksum);
}

unsigned char *NetworkMessageReady::packMessage() {
    unsigned char *buf = new unsigned char[getPackedSize() + 1];
    pack(buf, getPackedMessageFormat(), messageType, data.checksum);
    return buf;
}
...
bool NetworkMessageReady::receive(Socket *socket) {
    ...
    if (useOldProtocol == true) {
        ...
        unsigned char *buf = new unsigned char[getPackedSize() + 1];
        // bool result = NetworkMessage::receive(socket, buf, getPackedSize(),
        // true);
...
NetworkMessageLaunch::NetworkMessageLaunch(const GameSettings *gameSettings, int8 messageType) {
    ...
    for (int i = 0; i < GameConstants::maxPlayers; ++i) {
        ...
        // printf("Build netmsg for index: %d
...
void NetworkMessageLaunch::buildGameSettings(GameSettings *gameSettings) const {
    ...
    for (int i = 0; i < GameConstants::maxPlayers; ++i) {
        ...
        gameSettings->setNetworkPlayerPlatform(i, data.networkPlayerPlatform[i].getString());
        // printf("Build game settings for index: %d
        // [%s]\n",i,data.networkPlayerUUID[i].getString().c_str());
    }
...
const char *NetworkMessageLaunch::getPackedMessageFormat() const {
    return "c256s60s60s60s60s60s60s60s60s60s60s60s60s60s60s60s60s60s60s60s60s60s6"
           "0s60s60s60s60s60sllllllll60s60s60s60s60s60s60s60sLLL60s60s60s60s60s60"
           "s60s60s60s60s60s60s60s60s60s60s60s60s60s60sLLLLLLLLLLLLLLLLLLLLcccccc"
...
unsigned int NetworkMessageLaunch::getPackedSize() {
    ...
    if (result == 0) {
        ...
        unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
        result = pack(
            buf, getPackedMessageFormat(), messageType, packedData.description.getBuffer(), packedData.map.getBuffer(), packedData.tileset.getBuffer(),
            packedData.tech.getBuffer(), packedData.factionTypeNames[0].getBuffer(), packedData.factionTypeNames[1].getBuffer(),
            packedData.factionTypeNames[2].getBuffer(), packedData.factionTypeNames[3].getBuffer(), packedData.factionTypeNames[4].getBuffer(),
            packedData.factionTypeNames[5].getBuffer(), packedData.factionTypeNames[6].getBuffer(), packedData.factionTypeNames[7].getBuffer(),
            packedData.networkPlayerNames[0].getBuffer(), packedData.networkPlayerNames[1].getBuffer(), packedData.networkPlayerNames[2].getBuffer(),
            packedData.networkPlayerNames[3].getBuffer(), packedData.networkPlayerNames[4].getBuffer(), packedData.networkPlayerNames[5].getBuffer(),
            packedData.networkPlayerNames[6].getBuffer(), packedData.networkPlayerNames[7].getBuffer(), packedData.networkPlayerPlatform[0].getBuffer(),
            packedData.networkPlayerPlatform[1].getBuffer(), packedData.networkPlayerPlatform[2].getBuffer(), packedData.networkPlayerPlatform[3].getBuffer(),
            packedData.networkPlayerPlatform[4].getBuffer(), packedData.networkPlayerPlatform[5].getBuffer(), packedData.networkPlayerPlatform[6].getBuffer(),
            packedData.networkPlayerPlatform[7].getBuffer(), packedData.networkPlayerStatuses[0], packedData.networkPlayerStatuses[1],
            packedData.networkPlayerStatuses[2], packedData.networkPlayerStatuses[3], packedData.networkPlayerStatuses[4], packedData.networkPlayerStatuses[5],
            packedData.networkPlayerStatuses[6], packedData.networkPlayerStatuses[7], packedData.networkPlayerLanguages[0].getBuffer(),
            packedData.networkPlayerLanguages[1].getBuffer(), packedData.networkPlayerLanguages[2].getBuffer(),
            packedData.networkPlayerLanguages[3].getBuffer(), packedData.networkPlayerLanguages[4].getBuffer(),
            packedData.networkPlayerLanguages[5].getBuffer(), packedData.networkPlayerLanguages[6].getBuffer(),
            packedData.networkPlayerLanguages[7].getBuffer(), packedData.mapCRC, packedData.mapFilter, packedData.tilesetCRC, packedData.techCRC,
            packedData.factionNameList[0].getBuffer(), packedData.factionNameList[1].getBuffer(), packedData.factionNameList[2].getBuffer(),
            packedData.factionNameList[3].getBuffer(), packedData.factionNameList[4].getBuffer(), packedData.factionNameList[5].getBuffer(),
            packedData.factionNameList[6].getBuffer(), packedData.factionNameList[7].getBuffer(), packedData.factionNameList[8].getBuffer(),
            packedData.factionNameList[9].getBuffer(), packedData.factionNameList[10].getBuffer(), packedData.factionNameList[11].getBuffer(),
            packedData.factionNameList[12].getBuffer(), packedData.factionNameList[13].getBuffer(), packedData.factionNameList[14].getBuffer(),
            packedData.factionNameList[15].getBuffer(), packedData.factionNameList[16].getBuffer(), packedData.factionNameList[17].getBuffer(),
            packedData.factionNameList[18].getBuffer(), packedData.factionNameList[19].getBuffer(), packedData.factionCRCList[0], packedData.factionCRCList[1],
            packedData.factionCRCList[2], packedData.factionCRCList[3], packedData.factionCRCList[4], packedData.factionCRCList[5],
            packedData.factionCRCList[6], packedData.factionCRCList[7], packedData.factionCRCList[8], packedData.factionCRCList[9],
            packedData.factionCRCList[10], packedData.factionCRCList[11], packedData.factionCRCList[12], packedData.factionCRCList[13],
            packedData.factionCRCList[14], packedData.factionCRCList[15], packedData.factionCRCList[16], packedData.factionCRCList[17],
            packedData.factionCRCList[18], packedData.factionCRCList[19], packedData.factionControls[0], packedData.factionControls[1],
            packedData.factionControls[2], packedData.factionControls[3], packedData.factionControls[4], packedData.factionControls[5],
            packedData.factionControls[6], packedData.factionControls[7], packedData.resourceMultiplierIndex[0], packedData.resourceMultiplierIndex[1],
            packedData.resourceMultiplierIndex[2], packedData.resourceMultiplierIndex[3], packedData.resourceMultiplierIndex[4],
            packedData.resourceMultiplierIndex[5], packedData.resourceMultiplierIndex[6], packedData.resourceMultiplierIndex[7], packedData.thisFactionIndex,
            packedData.factionCount, packedData.teams[0], packedData.teams[1], packedData.teams[2], packedData.teams[3], packedData.teams[4],
            packedData.teams[5], packedData.teams[6], packedData.teams[7], packedData.startLocationIndex[0], packedData.startLocationIndex[1],
            packedData.startLocationIndex[2], packedData.startLocationIndex[3], packedData.startLocationIndex[4], packedData.startLocationIndex[5],
            packedData.startLocationIndex[6], packedData.startLocationIndex[7], packedData.defaultResources, packedData.defaultUnits,
            packedData.defaultVictoryConditions, packedData.fogOfWar, packedData.allowObservers, packedData.enableObserverModeAtEndGame,
            packedData.enableServerControlledAI, packedData.networkFramePeriod, packedData.networkPauseGameForLaggedClients, packedData.pathFinderType,
            packedData.flagTypes1, packedData.aiAcceptSwitchTeamPercentChance, packedData.cpuReplacementMultiplier, packedData.masterserver_admin,
            packedData.masterserver_admin_factionIndex, packedData.scenario.getBuffer(), packedData.networkPlayerUUID[0].getBuffer(),
            packedData.networkPlayerUUID[1].getBuffer(), packedData.networkPlayerUUID[2].getBuffer(), packedData.networkPlayerUUID[3].getBuffer(),
            packedData.networkPlayerUUID[4].getBuffer(), packedData.networkPlayerUUID[5].getBuffer(), packedData.networkPlayerUUID[6].getBuffer(),
            packedData.networkPlayerUUID[7].getBuffer(), packedData.networkAllowNativeLanguageTechtree, packedData.gameUUID.getBuffer());
        delete[] buf;
    }
    return result;
}
void NetworkMessageLaunch::unpackMessage(unsigned char *buf) {
    unpack(
        buf, getPackedMessageFormat(), &messageType, data.description.getBuffer(), data.map.getBuffer(), data.tileset.getBuffer(), data.tech.getBuffer(),
        data.factionTypeNames[0].getBuffer(), data.factionTypeNames[1].getBuffer(), data.factionTypeNames[2].getBuffer(), data.factionTypeNames[3].getBuffer(),
        data.factionTypeNames[4].getBuffer(), data.factionTypeNames[5].getBuffer(), data.factionTypeNames[6].getBuffer(), data.factionTypeNames[7].getBuffer(),
        data.networkPlayerNames[0].getBuffer(), data.networkPlayerNames[1].getBuffer(), data.networkPlayerNames[2].getBuffer(),
        data.networkPlayerNames[3].getBuffer(), data.networkPlayerNames[4].getBuffer(), data.networkPlayerNames[5].getBuffer(),
        data.networkPlayerNames[6].getBuffer(), data.networkPlayerNames[7].getBuffer(), data.networkPlayerPlatform[0].getBuffer(),
        data.networkPlayerPlatform[1].getBuffer(), data.networkPlayerPlatform[2].getBuffer(), data.networkPlayerPlatform[3].getBuffer(),
        data.networkPlayerPlatform[4].getBuffer(), data.networkPlayerPlatform[5].getBuffer(), data.networkPlayerPlatform[6].getBuffer(),
        data.networkPlayerPlatform[7].getBuffer(), &data.networkPlayerStatuses[0], &data.networkPlayerStatuses[1], &data.networkPlayerStatuses[2],
        &data.networkPlayerStatuses[3], &data.networkPlayerStatuses[4], &data.networkPlayerStatuses[5], &data.networkPlayerStatuses[6],
        &data.networkPlayerStatuses[7], data.networkPlayerLanguages[0].getBuffer(), data.networkPlayerLanguages[1].getBuffer(),
        data.networkPlayerLanguages[2].getBuffer(), data.networkPlayerLanguages[3].getBuffer(), data.networkPlayerLanguages[4].getBuffer(),
        data.networkPlayerLanguages[5].getBuffer(), data.networkPlayerLanguages[6].getBuffer(), data.networkPlayerLanguages[7].getBuffer(), &data.mapFilter,
        &data.mapCRC, &data.tilesetCRC, &data.techCRC, data.factionNameList[0].getBuffer(), data.factionNameList[1].getBuffer(),
        data.factionNameList[2].getBuffer(), data.factionNameList[3].getBuffer(), data.factionNameList[4].getBuffer(), data.factionNameList[5].getBuffer(),
        data.factionNameList[6].getBuffer(), data.factionNameList[7].getBuffer(), data.factionNameList[8].getBuffer(), data.factionNameList[9].getBuffer(),
        data.factionNameList[10].getBuffer(), data.factionNameList[11].getBuffer(), data.factionNameList[12].getBuffer(), data.factionNameList[13].getBuffer(),
        data.factionNameList[14].getBuffer(), data.factionNameList[15].getBuffer(), data.factionNameList[16].getBuffer(), data.factionNameList[17].getBuffer(),
        data.factionNameList[18].getBuffer(), data.factionNameList[19].getBuffer(), &data.factionCRCList[0], &data.factionCRCList[1], &data.factionCRCList[2],
        &data.factionCRCList[3], &data.factionCRCList[4], &data.factionCRCList[5], &data.factionCRCList[6], &data.factionCRCList[7], &data.factionCRCList[8],
        &data.factionCRCList[9], &data.factionCRCList[10], &data.factionCRCList[11], &data.factionCRCList[12], &data.factionCRCList[13],
        &data.factionCRCList[14], &data.factionCRCList[15], &data.factionCRCList[16], &data.factionCRCList[17], &data.factionCRCList[18],
        &data.factionCRCList[19], &data.factionControls[0], &data.factionControls[1], &data.factionControls[2], &data.factionControls[3],
        &data.factionControls[4], &data.factionControls[5], &data.factionControls[6], &data.factionControls[7], &data.resourceMultiplierIndex[0],
        &data.resourceMultiplierIndex[1], &data.resourceMultiplierIndex[2], &data.resourceMultiplierIndex[3], &data.resourceMultiplierIndex[4],
        &data.resourceMultiplierIndex[5], &data.resourceMultiplierIndex[6], &data.resourceMultiplierIndex[7], &data.thisFactionIndex, &data.factionCount,
        &data.teams[0], &data.teams[1], &data.teams[2], &data.teams[3], &data.teams[4], &data.teams[5], &data.teams[6], &data.teams[7],
        &data.startLocationIndex[0], &data.startLocationIndex[1], &data.startLocationIndex[2], &data.startLocationIndex[3], &data.startLocationIndex[4],
        &data.startLocationIndex[5], &data.startLocationIndex[6], &data.startLocationIndex[7], &data.defaultResources, &data.defaultUnits,
        &data.defaultVictoryConditions, &data.fogOfWar, &data.allowObservers, &data.enableObserverModeAtEndGame, &data.enableServerControlledAI,
        &data.networkFramePeriod, &data.networkPauseGameForLaggedClients, &data.pathFinderType, &data.flagTypes1, &data.aiAcceptSwitchTeamPercentChance,
        &data.cpuReplacementMultiplier, &data.masterserver_admin, &data.masterserver_admin_factionIndex, data.scenario.getBuffer(),
        data.networkPlayerUUID[0].getBuffer(), data.networkPlayerUUID[1].getBuffer(), data.networkPlayerUUID[2].getBuffer(),
        data.networkPlayerUUID[3].getBuffer(), data.networkPlayerUUID[4].getBuffer(), data.networkPlayerUUID[5].getBuffer(),
        data.networkPlayerUUID[6].getBuffer(), data.networkPlayerUUID[7].getBuffer(), &data.networkAllowNativeLanguageTechtree, data.gameUUID.getBuffer());
}

unsigned char *NetworkMessageLaunch::packMessage() {
    unsigned char *buf = new unsigned char[getPackedSize() + 1];
    pack(buf, getPackedMessageFormat(), messageType, data.description.getBuffer(), data.map.getBuffer(), data.tileset.getBuffer(), data.tech.getBuffer(),
         data.factionTypeNames[0].getBuffer(), data.factionTypeNames[1].getBuffer(), data.factionTypeNames[2].getBuffer(), data.factionTypeNames[3].getBuffer(),
         data.factionTypeNames[4].getBuffer(), data.factionTypeNames[5].getBuffer(), data.factionTypeNames[6].getBuffer(), data.factionTypeNames[7].getBuffer(),
         data.networkPlayerNames[0].getBuffer(), data.networkPlayerNames[1].getBuffer(), data.networkPlayerNames[2].getBuffer(),
         data.networkPlayerNames[3].getBuffer(), data.networkPlayerNames[4].getBuffer(), data.networkPlayerNames[5].getBuffer(),
         ...
         data.factionNameList[18].getBuffer(), data.factionNameList[19].getBuffer(), data.factionCRCList[0], data.factionCRCList[1], data.factionCRCList[2],
         data.factionCRCList[3], data.factionCRCList[4], data.factionCRCList[5], data.factionCRCList[6], data.factionCRCList[7], data.factionCRCList[8],
         data.factionCRCList[9], data.factionCRCList[10], data.factionCRCList[11], data.factionCRCList[12], data.factionCRCList[13], data.factionCRCList[14],
         data.factionCRCList[15], data.factionCRCList[16], data.factionCRCList[17], data.factionCRCList[18], data.factionCRCList[19], data.factionControls[0],
         data.factionControls[1], data.factionControls[2], data.factionControls[3], data.factionControls[4], data.factionControls[5], data.factionControls[6],
         data.factionControls[7], data.resourceMultiplierIndex[0], data.resourceMultiplierIndex[1], data.resourceMultiplierIndex[2],
         data.resourceMultiplierIndex[3], data.resourceMultiplierIndex[4], data.resourceMultiplierIndex[5], data.resourceMultiplierIndex[6],
         data.resourceMultiplierIndex[7], data.thisFactionIndex, data.factionCount, data.teams[0], data.teams[1], data.teams[2], data.teams[3], data.teams[4],
         data.teams[5], data.teams[6], data.teams[7], data.startLocationIndex[0], data.startLocationIndex[1], data.startLocationIndex[2],
         data.startLocationIndex[3], data.startLocationIndex[4], data.startLocationIndex[5], data.startLocationIndex[6], data.startLocationIndex[7],
         data.defaultResources, data.defaultUnits, data.defaultVictoryConditions, data.fogOfWar, data.allowObservers, data.enableObserverModeAtEndGame,
         data.enableServerControlledAI, data.networkFramePeriod, data.networkPauseGameForLaggedClients, data.pathFinderType, data.flagTypes1,
         data.aiAcceptSwitchTeamPercentChance, data.cpuReplacementMultiplier, data.masterserver_admin, data.masterserver_admin_factionIndex,
         data.scenario.getBuffer(), data.networkPlayerUUID[0].getBuffer(), data.networkPlayerUUID[1].getBuffer(), data.networkPlayerUUID[2].getBuffer(),
         data.networkPlayerUUID[3].getBuffer(), data.networkPlayerUUID[4].getBuffer(), data.networkPlayerUUID[5].getBuffer(),
         data.networkPlayerUUID[6].getBuffer(), data.networkPlayerUUID[7].getBuffer(), data.networkAllowNativeLanguageTechtree, data.gameUUID.getBuffer());
    return buf;
}
...
bool NetworkMessageLaunch::receive(Socket *socket) {
    ...
    if (useOldProtocol == true) {
        ...
        // printf("UnCompressed launch packet after read compressed size:
        // %d\n",compressedLength);
        ...
        if (result == true && compressedLength > 0 && socket != NULL && socket->isSocketValid()) {
            // printf("UnCompressed launch packet before: %u after:
            ...
            if (result == true && socket != NULL && socket->isSocketValid()) {
                ...
                // static_cast<const unsigned char *>(compressedMessage);
                // for(unsigned int index = 0; index < (unsigned int)compressedLength;
                // ++index) { 					printf("%u[%X][%d]
                ...
                // printf("SUCCESS UnCompressed launch packet before: %u after:
    ...
    // for(int i= 0; i < GameConstants::maxPlayers; ++i){
    //	printf("Receive index: %d resource multiplier index: %d sizeof(data):
...
std::pair<unsigned char *, unsigned long> NetworkMessageLaunch::getCompressedMessage() {
    ...
    std::pair<unsigned char *, unsigned long> result = Shared::CompressionUtil::compressMemoryToMemory(buffer, getDataSize());
    delete[] buffer;
    return result;
}
...
void NetworkMessageLaunch::send(Socket *socket) {
    ...
    // for(int i= 0; i < GameConstants::maxPlayers; ++i){
    //	printf("Send index: %d resource multiplier index: %d sizeof(data):
    //%d\n",i,data.resourceMultiplierIndex[i],sizeof(data));
    // }
    ...
    if (useOldProtocol == true) {
        ...
        // printf("Compressed launch packet before: %d after:
        // %lu\n",(int)getDataSize(),compressionResult.second);

        //		printf("\n");
        //		const unsigned char *buf = static_cast<const unsigned char
        //*>(compressionResult.first); 		for(unsigned int index = 0;
        // index < (unsigned int)compressionResult.second; ++index) {
        // printf("%u[%X][%d]
        //",index,buf[index],buf[index]); 			if(index % 10 == 0) {
        // printf("\n");
        //			}
        //		}
        //		printf("\n");
        ...
        /*
                    NetworkMessage::send(socket, &messageType,
       sizeof(messageType)); if(socket != NULL && socket->isSocketValid()) {
                            NetworkMessage::send(socket, &compressedLength,
       sizeof(compressedLength)); if(socket != NULL && socket->isSocketValid())
       { NetworkMessage::send(socket, compressionResult.first,
       compressionResult.second);
                            }
                    }
    */
...
unsigned int NetworkMessageCommandList::getPackedSizeHeader() {
    ...
    if (result == 0) {
        ...
        init(packedData);
        unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
        result = pack(buf, getPackedMessageFormatHeader(), packedData.messageType, packedData.header.commandCount, packedData.header.frameCount,
                      packedData.header.networkPlayerFactionCRC[0], packedData.header.networkPlayerFactionCRC[1], packedData.header.networkPlayerFactionCRC[2],
                      packedData.header.networkPlayerFactionCRC[3], packedData.header.networkPlayerFactionCRC[4], packedData.header.networkPlayerFactionCRC[5],
                      packedData.header.networkPlayerFactionCRC[6], packedData.header.networkPlayerFactionCRC[7]);
        delete[] buf;
    }
    return result;
}
void NetworkMessageCommandList::unpackMessageHeader(unsigned char *buf) {
    unpack(buf, getPackedMessageFormatHeader(), &data.messageType, &data.header.commandCount, &data.header.frameCount, &data.header.networkPlayerFactionCRC[0],
           &data.header.networkPlayerFactionCRC[1], &data.header.networkPlayerFactionCRC[2], &data.header.networkPlayerFactionCRC[3],
           &data.header.networkPlayerFactionCRC[4], &data.header.networkPlayerFactionCRC[5], &data.header.networkPlayerFactionCRC[6],
...
unsigned char *NetworkMessageCommandList::packMessageHeader() {
    ...
    pack(buf, getPackedMessageFormatHeader(), data.messageType, data.header.commandCount, data.header.frameCount, data.header.networkPlayerFactionCRC[0],
         data.header.networkPlayerFactionCRC[1], data.header.networkPlayerFactionCRC[2], data.header.networkPlayerFactionCRC[3],
         data.header.networkPlayerFactionCRC[4], data.header.networkPlayerFactionCRC[5], data.header.networkPlayerFactionCRC[6],
         data.header.networkPlayerFactionCRC[7]);
    return buf;
...
unsigned int NetworkMessageCommandList::getPackedSizeDetail(int count) {
    ...
    for (unsigned int i = 0; i < (unsigned int)count; ++i) {
        ...
        result += pack(buf, getPackedMessageFormatDetail(), packedData.networkCommandType, packedData.unitId, packedData.unitTypeId, packedData.commandTypeId,
                       packedData.positionX, packedData.positionY, packedData.targetId, packedData.wantQueue, packedData.fromFactionIndex,
                       packedData.unitFactionUnitCount, packedData.unitFactionIndex, packedData.commandStateType, packedData.commandStateValue,
...
void NetworkMessageCommandList::unpackMessageDetail(unsigned char *buf, int count) {
    ...
    for (unsigned int i = 0; i < (unsigned int)count; ++i) {
        unsigned int bytes_processed =
            unpack(bufMove, getPackedMessageFormatDetail(), &data.commands[i].networkCommandType, &data.commands[i].unitId, &data.commands[i].unitTypeId,
                   &data.commands[i].commandTypeId, &data.commands[i].positionX, &data.commands[i].positionY, &data.commands[i].targetId,
                   &data.commands[i].wantQueue, &data.commands[i].fromFactionIndex, &data.commands[i].unitFactionUnitCount, &data.commands[i].unitFactionIndex,
                   &data.commands[i].commandStateType, &data.commands[i].commandStateValue, &data.commands[i].unitCommandGroupId);
...
unsigned char *NetworkMessageCommandList::packMessageDetail(uint16 totalCommand) {
    ...
    for (unsigned int i = 0; i < totalCommand; ++i) {
        unsigned int bytes_processed =
            pack(bufMove, getPackedMessageFormatDetail(), data.commands[i].networkCommandType, data.commands[i].unitId, data.commands[i].unitTypeId,
                 data.commands[i].commandTypeId, data.commands[i].positionX, data.commands[i].positionY, data.commands[i].targetId, data.commands[i].wantQueue,
                 data.commands[i].fromFactionIndex, data.commands[i].unitFactionUnitCount, data.commands[i].unitFactionIndex, data.commands[i].commandStateType,
                 data.commands[i].commandStateValue, data.commands[i].unitCommandGroupId);
        bufMove += bytes_processed;
        // bytes_processed_total += bytes_processed;
    }
    // printf("\nPacked detail size = %u, allocated =
...
bool NetworkMessageCommandList::receive(Socket *socket) {
    ...
    if (useOldProtocol == true) {
        ...
        // printf("!!! =====> IN Network hdr cmd get frame: %d
        // data.header.commandCount:
    ...
    if (result == true) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                     "In [%s::%s Line: %d] got header, messageType = "
                                     "%d, commandCount = %u, frameCount = %d\n",
                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, data.messageType, data.header.commandCount,
                                     data.header.frameCount);

        // printf("!!! =====> IN Network cmd get frame: %d data.header.commandCount:
        // %u\n",data.header.frameCount,data.header.commandCount);
        ...
        if (data.header.commandCount > 0) {
            ...
            if (useOldProtocol == true) {
                ...
                //				if(data.commands[0].getNetworkCommandType()
                //== nctPauseResume) { 					printf("=====>
                // IN Network cmd type: %d [%d] frame:
                //%d\n",data.commands[0].getNetworkCommandType(),nctPauseResume,data.header.frameCount);
                //				}
            } else {
                // int totalMsgSize = (sizeof(NetworkCommand) *
                // data.header.commandCount); result = NetworkMessage::receive(socket,
                // &data.commands[0], totalMsgSize, true);
            ...
            // data.header.commandCount,cmd.toString().c_str());
            //	        }
            ...
            if (result == true) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled == true) {
                    for (int idx = 0; idx < data.header.commandCount; ++idx) {
                        ...
                        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                                 "In [%s::%s Line: %d] index = %d, received networkCommand "
                                                 "[%s]\n",
                ...
                // if(SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                // SystemFlags::OutputDebug(SystemFlags::debugNetwork,"In [%s::%s Line:
                // %d] ERROR Failed to get command data, totalMsgSize =
                // %d.\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,totalMsgSize);
            }
        }
    } else {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] ERROR header not received as expected\n",
                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
...
unsigned char *NetworkMessageCommandList::getData() {
    ...
    if (totalCommand > data.commands.size()) {
        throw std::runtime_error("NetworkMessageCommandList::getData - commandCount exceeds vector "
                                 "size");
    }
...
void NetworkMessageCommandList::send(Socket *socket) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s Line: %d] nmtCommandList, frameCount = %d, "
                                 "data.header.commandCount = %d, data.header.messageType = %d\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, data.header.frameCount, data.header.commandCount,
                                 data.messageType);
    ...
    // bool result = false;
    if (useOldProtocol == true) {
        // printf("<===== OUT Network hdr cmd type: frame: %d totalCommand: %u
        // [%u]\n",data.header.frameCount,totalCommand,data.header.commandCount);
        // NetworkMessage::send(socket, &data.messageType,
        // sizeof(data.messageType));
        ...
        // if(totalCommand) printf("\n\nSend packet size = %u data.messageType =
        // %d\n%s\ncommandcount [%u] framecount
    ...
    if (totalCommand > 0) {
        // printf("\n#2 Send packet commandcount [%u] framecount
        // [%d]\n",totalCommand,data.header.frameCount);
        // toEndianDetail(totalCommand);
        // printf("\n#3 Send packet commandcount [%u] framecount
        // [%d]\n",totalCommand,data.header.frameCount);

        // bool result = false;
        if (useOldProtocol == true) {
            //			if(data.commands[0].getNetworkCommandType() ==
            // nctPauseResume) { 				printf("<===== OUT
            // Network cmd type: %d [%d] frame: %d totalCommand: %u
            //[%u]\n",data.commands[0].getNetworkCommandType(),nctPauseResume,data.header.frameCount,totalCommand,data.header.commandCount);
            //			}
            // NetworkMessage::send(socket, &data.commands[0], (sizeof(NetworkCommand)
            // * totalCommand));
        } else {
            buf = packMessageDetail(totalCommand);
            // printf("\n#4 Send packet commandcount [%u] framecount
            // [%d]\n",totalCommand,data.header.frameCount); printf("Send packet size
            // = %u data.messageType =
            // %d\n[%s]\n",getPackedSize(),data.messageType,buf);
            NetworkMessage::send(socket, buf, getPackedSizeDetail(totalCommand));
            // printf("\n#5 Send packet commandcount [%u] framecount
            // [%d]\n",totalCommand,data.header.frameCount);
            delete[] buf;
            // printf("\n#6 Send packet commandcount [%u] framecount
            // [%d]\n",totalCommand,data.header.frameCount);

            //        for(int idx = 0 ; idx < totalCommand; ++idx) {
            //            const NetworkCommand &cmd = data.commands[idx];
            //            printf("========> Send index = %d / %u, sent networkCommand
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled == true) {
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s Line: %d] messageType = %d, "
                                 "frameCount = %d, data.commandCount = %d\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, data.messageType, data.header.frameCount,
                                 data.header.commandCount);
        ...
        if (totalCommand > 0) {
            ...
            SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                     "In [%s::%s Line: %d] END of loop, nmtCommandList, frameCount = %d, "
                                     "data.header.commandCount = %d, data.header.messageType = %d\n",
                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, data.header.frameCount, totalCommand,
                                     data.messageType);
        }
    }
}
...
NetworkMessageText::NetworkMessageText(const string &text, int teamIndex, int playerIndex, const string targetLanguage) {
    if ((int)text.length() >= maxTextStringSize) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                     "In [%s::%s Line: %d] WARNING / ERROR - text "
                                     "[%s] length = %d, max = %d\n",
...
unsigned int NetworkMessageText::getPackedSize() {
    ...
    if (result == 0) {
        ...
        unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
        result = pack(buf, getPackedMessageFormat(), messageType, packedData.text.getBuffer(), packedData.teamIndex, packedData.playerIndex,
...
void NetworkMessageText::unpackMessage(unsigned char *buf) {
    unpack(buf, getPackedMessageFormat(), &messageType, data.text.getBuffer(), &data.teamIndex, &data.playerIndex, data.targetLanguage.getBuffer());
}

unsigned char *NetworkMessageText::packMessage() {
    unsigned char *buf = new unsigned char[getPackedSize() + 1];
    pack(buf, getPackedMessageFormat(), messageType, data.text.getBuffer(), data.teamIndex, data.playerIndex, data.targetLanguage.getBuffer());
    return buf;
}
...
unsigned int NetworkMessageQuit::getPackedSize() {
    ...
    if (result == 0) {
        ...
        result = pack(buf, getPackedMessageFormat(), messageType);
        delete[] buf;
    }
    return result;
}
void NetworkMessageQuit::unpackMessage(unsigned char *buf) {
    unpack(buf, getPackedMessageFormat(), &messageType);
}

unsigned char *NetworkMessageQuit::packMessage() {
    unsigned char *buf = new unsigned char[getPackedSize() + 1];
    pack(buf, getPackedMessageFormat(), messageType);
    return buf;
}
...
NetworkMessageSynchNetworkGameData::NetworkMessageSynchNetworkGameData(const GameSettings *gameSettings) {
    ...
    data.header.tilesetCRC = getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTilesets, scenarioDir),
                                                                      string("/") + gameSettings->getTileset() + string("/*"), ".xml", NULL);

    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] data.tilesetCRC = %d, [%s]\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__, data.header.tilesetCRC, gameSettings->getTileset().c_str());

    // tech, load before map because of resources
    data.header.techCRC = getFolderTreeContentsCheckSumRecursively(config.getPathListForType(ptTechs, scenarioDir),
                                                                   string("/") + gameSettings->getTech() + string("/*"), ".xml", NULL);

    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] data.techCRC = %d, [%s]\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__, data.header.techCRC, gameSettings->getTech().c_str());

    vector<std::pair<string, uint32>> vctFileList;
    vctFileList = getFolderTreeContentsCheckSumListRecursively(config.getPathListForType(ptTechs, scenarioDir),
                                                               string("/") + gameSettings->getTech() + string("/*"), ".xml", &vctFileList);
    data.header.techCRCFileCount = min((int)vctFileList.size(), (int)maxFileCRCCount);

    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] vctFileList.size() = %d, maxFileCRCCount = %d\n",
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] data.mapCRC = %d, [%s]\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__, data.header.mapCRC, gameSettings->getMap().c_str());
}

string NetworkMessageSynchNetworkGameData::getTechCRCFileMismatchReport(vector<std::pair<string, uint32>> &vctFileList) {
    string result = "Techtree: [" + data.header.tech.getString() + "] Filecount local: " + intToStr(vctFileList.size()) +
                    " remote: " + intToStr(data.header.techCRCFileCount) + "\n";
...
unsigned int NetworkMessageSynchNetworkGameData::getPackedSizeHeader() {
    ...
    if (result == 0) {
        ...
        unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
        result = pack(buf, getPackedMessageFormatHeader(), packedData.messageType, packedData.header.map.getBuffer(), packedData.header.tileset.getBuffer(),
                      packedData.header.tech.getBuffer(), packedData.header.mapCRC, packedData.header.tilesetCRC, packedData.header.techCRC,
...
void NetworkMessageSynchNetworkGameData::unpackMessageHeader(unsigned char *buf) {
    unpack(buf, getPackedMessageFormatHeader(), &data.messageType, data.header.map.getBuffer(), data.header.tileset.getBuffer(), data.header.tech.getBuffer(),
           &data.header.mapCRC, &data.header.tilesetCRC, &data.header.techCRC, &data.header.techCRCFileCount);
}

unsigned char *NetworkMessageSynchNetworkGameData::packMessageHeader() {
    unsigned char *buf = new unsigned char[getPackedSizeHeader() + 1];
    pack(buf, getPackedMessageFormatHeader(), data.messageType, data.header.map.getBuffer(), data.header.tileset.getBuffer(), data.header.tech.getBuffer(),
         data.header.mapCRC, data.header.tilesetCRC, data.header.techCRC, data.header.techCRCFileCount);
...
unsigned int NetworkMessageSynchNetworkGameData::getPackedSizeDetail() {
    ...
    if (result == 0) {
        ...
        for (unsigned int i = 0; i < (unsigned int)maxFileCRCCount; ++i) {
            result += pack(buf, "255s", packedData.techCRCFileList[i].getBuffer());
            buf += result;
        }
        for (unsigned int i = 0; i < (unsigned int)maxFileCRCCount; ++i) {
            result += pack(buf, "L", packedData.techCRCFileCRCList[i]);
            buf += result;
        }
...
void NetworkMessageSynchNetworkGameData::unpackMessageDetail(unsigned char *buf) {
    for (unsigned int i = 0; i < (unsigned int)maxFileCRCCount; ++i) {
        unsigned int bytes_processed = unpack(buf, "255s", data.detail.techCRCFileList[i].getBuffer());
        buf += bytes_processed;
    }
    for (unsigned int i = 0; i < (unsigned int)maxFileCRCCount; ++i) {
        unsigned int bytes_processed = unpack(buf, "L", &data.detail.techCRCFileCRCList[i]);
        buf += bytes_processed;
    }
}
...
unsigned char *NetworkMessageSynchNetworkGameData::packMessageDetail() {
    ...
    unsigned char *bufMove = buf;
    for (unsigned int i = 0; i < (unsigned int)maxFileCRCCount; ++i) {
        unsigned int bytes_processed = pack(bufMove, "255s", data.detail.techCRCFileList[i].getBuffer());
        bufMove += bytes_processed;
    }
    for (unsigned int i = 0; i < (unsigned int)maxFileCRCCount; ++i) {
        unsigned int bytes_processed = pack(bufMove, "L", data.detail.techCRCFileCRCList[i]);
        bufMove += bytes_processed;
    }
    ...
    if (result == true && data.header.techCRCFileCount > 0) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] packetLoopCount = %d\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                     __FUNCTION__, __LINE__, packetLoopCount);
        ...
        for (int iPacketLoop = 0; result == true && iPacketLoop < packetLoopCount; ++iPacketLoop) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                         "In [%s::%s Line: %d] iPacketLoop = %d, packetIndex = %d, "
                                         "maxFileCountPerPacket = %d, packetFileCount = %d, "
                                         "packetDetail1DataSize = %d, packetDetail2DataSize = %d\n",
                                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, iPacketLoop, packetIndex,
                                         maxFileCountPerPacket, packetFileCount, packetDetail1DataSize, packetDetail2DataSize);
...
NetworkMessageSynchNetworkGameDataStatus::NetworkMessageSynchNetworkGameDataStatus(uint32 mapCRC, uint32 tilesetCRC, uint32 techCRC,
                                                                                   vector<std::pair<string, uint32>> &vctFileList) {
...
string NetworkMessageSynchNetworkGameDataStatus::getTechCRCFileMismatchReport(string techtree, vector<std::pair<string, uint32>> &vctFileList) {
    string result =
        "Techtree: [" + techtree + "] Filecount local: " + intToStr(vctFileList.size()) + " remote: " + intToStr(data.header.techCRCFileCount) + "\n";
    if (vctFileList.size() <= 0) {
        result = result + "Local player has no files.\n";
    } else if (data.header.techCRCFileCount <= 0) {
        result = result + "Remote player has no files.\n";
...
bool NetworkMessageSynchNetworkGameDataStatus::receive(Socket *socket) {
    ...
    if (result == true && data.header.techCRCFileCount > 0) {
        ...
        for (uint32 iPacketLoop = 0; iPacketLoop < packetLoopCount; ++iPacketLoop) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                         "In [%s::%s Line: %d] iPacketLoop = %u, "
                                         "packetIndex = %u, packetFileCount = %u\n",
...
void NetworkMessageSynchNetworkGameDataStatus::send(Socket *socket) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s Line: %d] about to send nmtSynchNetworkGameDataStatus, "
                                 "data.header.techCRCFileCount = %d\n",
    ...
    if (totalFileCount > 0) {
        ...
        for (int iPacketLoop = 0; iPacketLoop < packetLoopCount; ++iPacketLoop) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                         "In [%s::%s Line: %d] packetLoop = %d, "
                                         "packetIndex = %d, packetFileCount = %d\n",
                                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, iPacketLoop, packetIndex, packetFileCount);
...
NetworkMessageSynchNetworkGameDataFileCRCCheck::NetworkMessageSynchNetworkGameDataFileCRCCheck(uint32 totalFileCount, uint32 fileIndex, uint32 fileCRC,
...
unsigned int NetworkMessageSynchNetworkGameDataFileCRCCheck::getPackedSize() {
    ...
    if (result == 0) {
        ...
        unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
        result = pack(buf, getPackedMessageFormat(), messageType, packedData.totalFileCount, packedData.fileIndex, packedData.fileCRC,
...
void NetworkMessageSynchNetworkGameDataFileCRCCheck::unpackMessage(unsigned char *buf) {
    unpack(buf, getPackedMessageFormat(), &messageType, &data.totalFileCount, &data.fileIndex, &data.fileCRC, data.fileName.getBuffer());
}

unsigned char *NetworkMessageSynchNetworkGameDataFileCRCCheck::packMessage() {
    unsigned char *buf = new unsigned char[getPackedSize() + 1];
    pack(buf, getPackedMessageFormat(), messageType, data.totalFileCount, data.fileIndex, data.fileCRC, data.fileName.getBuffer());
    return buf;
}
...
unsigned int NetworkMessageSynchNetworkGameDataFileGet::getPackedSize() {
    ...
    if (result == 0) {
        ...
        messageType = nmtSynchNetworkGameDataFileGet;
        unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
        result = pack(buf, getPackedMessageFormat(), messageType, packedData.fileName.getBuffer());
        delete[] buf;
    }
    return result;
}
void NetworkMessageSynchNetworkGameDataFileGet::unpackMessage(unsigned char *buf) {
    unpack(buf, getPackedMessageFormat(), &messageType, data.fileName.getBuffer());
}

unsigned char *NetworkMessageSynchNetworkGameDataFileGet::packMessage() {
    unsigned char *buf = new unsigned char[getPackedSize() + 1];
    pack(buf, getPackedMessageFormat(), messageType, data.fileName.getBuffer());
    return buf;
}
...
unsigned int SwitchSetupRequest::getPackedSize() {
    ...
    if (result == 0) {
        ...
        result = pack(buf, getPackedMessageFormat(), messageType, packedData.selectedFactionName.getBuffer(), packedData.currentSlotIndex,
                      packedData.toSlotIndex, packedData.toTeam, packedData.networkPlayerName.getBuffer(), packedData.networkPlayerStatus,
                      packedData.switchFlags, packedData.language.getBuffer());
        delete[] buf;
    }
    return result;
}
void SwitchSetupRequest::unpackMessage(unsigned char *buf) {
    unpack(buf, getPackedMessageFormat(), &messageType, data.selectedFactionName.getBuffer(), &data.currentSlotIndex, &data.toSlotIndex, &data.toTeam,
           data.networkPlayerName.getBuffer(), &data.networkPlayerStatus, &data.switchFlags, data.language.getBuffer());
}

unsigned char *SwitchSetupRequest::packMessage() {
    unsigned char *buf = new unsigned char[getPackedSize() + 1];
    pack(buf, getPackedMessageFormat(), messageType, data.selectedFactionName.getBuffer(), data.currentSlotIndex, data.toSlotIndex, data.toTeam,
         data.networkPlayerName.getBuffer(), data.networkPlayerStatus, data.switchFlags, data.language.getBuffer());
    return buf;
}
...
bool SwitchSetupRequest::receive(Socket *socket) {
    ...
    if (useOldProtocol == true) {
        ...
        // [%s] currentFactionIndex = %d toFactionIndex =
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("In [%s::%s Line %d] data.networkPlayerName [%s]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
               data.networkPlayerName.getString().c_str());
...
void SwitchSetupRequest::send(Socket *socket) {
    ...
    if (useOldProtocol == true) {
        ...
        // faction [%s] currentFactionIndex = %d toFactionIndex =
        // %d\n",getPackedSize(),data.messageType,buf,data.toTeam,data.selectedFactionName.getBuffer(),data.currentFactionIndex,data.toFactionIndex);
        NetworkMessage::send(socket, buf, getPackedSize());
        delete[] buf;
    }
...
unsigned int PlayerIndexMessage::getPackedSize() {
    ...
    if (result == 0) {
        ...
        packedData.playerIndex = 0;
        unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
        result = pack(buf, getPackedMessageFormat(), messageType, packedData.playerIndex);
        delete[] buf;
    }
    return result;
}
void PlayerIndexMessage::unpackMessage(unsigned char *buf) {
    unpack(buf, getPackedMessageFormat(), &messageType, &data.playerIndex);
}

unsigned char *PlayerIndexMessage::packMessage() {
    unsigned char *buf = new unsigned char[getPackedSize() + 1];
    pack(buf, getPackedMessageFormat(), messageType, data.playerIndex);
    return buf;
}
...
unsigned int NetworkMessageLoadingStatus::getPackedSize() {
    ...
    if (result == 0) {
        ...
        packedData.status = 0;
        unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
        result = pack(buf, getPackedMessageFormat(), messageType, packedData.status);
        delete[] buf;
    }
    return result;
}
void NetworkMessageLoadingStatus::unpackMessage(unsigned char *buf) {
    unpack(buf, getPackedMessageFormat(), &messageType, &data.status);
}

unsigned char *NetworkMessageLoadingStatus::packMessage() {
    unsigned char *buf = new unsigned char[getPackedSize() + 1];
    pack(buf, getPackedMessageFormat(), messageType, data.status);
    return buf;
}
...
unsigned int NetworkMessageMarkCell::getPackedSize() {
    ...
    if (result == 0) {
        ...
        unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
        result = pack(buf, getPackedMessageFormat(), messageType, packedData.targetX, packedData.targetY, packedData.factionIndex, packedData.playerIndex,
...
void NetworkMessageMarkCell::unpackMessage(unsigned char *buf) {
    unpack(buf, getPackedMessageFormat(), &messageType, &data.targetX, &data.targetY, &data.factionIndex, &data.playerIndex, data.text.getBuffer());
}

unsigned char *NetworkMessageMarkCell::packMessage() {
    unsigned char *buf = new unsigned char[getPackedSize() + 1];
    pack(buf, getPackedMessageFormat(), messageType, data.targetX, data.targetY, data.factionIndex, data.playerIndex, data.text.getBuffer());
...
void NetworkMessageMarkCell::send(Socket *socket) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] nmtMarkCell\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                                 __LINE__);
...
unsigned int NetworkMessageUnMarkCell::getPackedSize() {
    ...
    if (result == 0) {
        ...
        packedData.targetY = 0;
        unsigned char *buf = new unsigned char[sizeof(packedData) * 3];
        result = pack(buf, getPackedMessageFormat(), messageType, packedData.targetX, packedData.targetY, packedData.factionIndex);
        delete[] buf;
    }
    return result;
}
void NetworkMessageUnMarkCell::unpackMessage(unsigned char *buf) {
    unpack(buf, getPackedMessageFormat(), &messageType, &data.targetX, &data.targetY, &data.factionIndex);
}

unsigned char *NetworkMessageUnMarkCell::packMessage() {
    unsigned char *buf = new unsigned char[getPackedSize() + 1];
    pack(buf, getPackedMessageFormat(), messageType, data.targetX, data.targetY, data.factionIndex);
...
void NetworkMessageUnMarkCell::send(Socket *socket) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] nmtUnMarkCell\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__);

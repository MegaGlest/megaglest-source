  ...
  for (std::map<NetworkMessageStatisticType, int64>::iterator iterMap =
    ...
    switch (iterMap->first) {
      case netmsgstPacketsPerMillisecondSend:
        result +=
            "send p / msec: " + intToStr(iterMap->second) +
      ...
      case netmsgstPacketsPerSecondRecv:
        result +=
            "recv p / sec: " + intToStr(iterMap->second) +
            (iterMap->second > 10 ? "  ****WARNING WIN32 LIMIT EXCEEDED****"
                                  : "") +
            "\n";
        break;
...
unsigned char *NetworkMessageCommandList::packMessageDetail(
    uint16 totalCommand) {
...
const char *NetworkMessageSynchNetworkGameData::getPackedMessageFormatHeader()
    const {
...
const char *NetworkMessageSynchNetworkGameDataFileGet::getPackedMessageFormat()
    const {
  return "c256s";
}

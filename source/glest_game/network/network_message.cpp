...
NetworkMessageGameStats::NetworkMessageGameStats() {
	...
	for(int i=0 ; i<GameConstants::maxPlayers;i++){
		...
		data.enemykills[i]=0;
		data.deaths[i]=0;
		data.unitsProduced[i]=0;
		data.resourcesHarvested[i]=0;
	}
	data.maxConcurrentUnitCount=0;
	data.totalEndGameConcurrentUnitCount=0;
}

NetworkMessageGameStats::NetworkMessageGameStats(Stats *stats) {
	data.messageType	= nmtGameStats;
	// ==================
	for(int i=0 ; i<GameConstants::maxPlayers;i++){
		PlayerStats *playerStats=stats->getPlayerStats(i);
		data.victory[i]=playerStats->victory;
		data.kills[i]=playerStats->kills;
		data.enemykills[i]=playerStats->enemykills;
		data.deaths[i]=playerStats->deaths;
		data.unitsProduced[i]=playerStats->unitsProduced;
		data.resourcesHarvested[i]=playerStats->resourcesHarvested;
	}
	data.maxConcurrentUnitCount=stats->getMaxConcurrentUnitCount();
	data.totalEndGameConcurrentUnitCount=stats->getTotalEndGameConcurrentUnitCount();
}

void NetworkMessageGameStats::copyToGameStats(Stats *stats) {
	for(int i=0 ; i<GameConstants::maxPlayers;i++){
		PlayerStats *playerStats=stats->getPlayerStats(i);
		playerStats->victory=data.victory[i];
		playerStats->kills=data.kills[i];
		playerStats->enemykills=data.enemykills[i];
		playerStats->deaths=data.deaths[i];
		playerStats->unitsProduced=data.unitsProduced[i];
		playerStats->resourcesHarvested=data.resourcesHarvested[i];
	}
	stats->setMaxConcurrentUnitCount(data.maxConcurrentUnitCount);
...
unsigned int NetworkMessageGameStats::getPackedSize() {
	return 0;
}
void NetworkMessageGameStats::unpackMessage(unsigned char *buf) {
//	unpack(buf, getPackedMessageFormat(),
//			&data.messageType,
//			&data.targetX,
//			&data.targetY,
...
unsigned char * NetworkMessageGameStats::packMessage() {
	unsigned char *buf = new unsigned char[getPackedSize()+1];
//	pack(buf, getPackedMessageFormat(),
//			data.messageType,
//			data.targetX,
...
bool NetworkMessageGameStats::receive(Socket* socket) {
	bool result = false;
	if(useOldProtocol == true) {
		result = NetworkMessage::receive(socket, &data, sizeof(data), true);
	}
...
void NetworkMessageGameStats::send(Socket* socket) {
	if(SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled) SystemFlags::OutputDebug(SystemFlags::debugNetwork,"In [%s::%s Line: %d] nmtMarkCell\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__);

	assert(data.messageType == nmtGameStats);
	toEndian();
...
void NetworkMessageGameStats::toEndian() {
}
void NetworkMessageGameStats::fromEndian() {
}

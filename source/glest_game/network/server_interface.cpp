...
ServerInterface::ServerInterface(bool publishEnabled, ClientLagCallbackInterface *clientLagCallbackInterface) : GameNetworkInterface() {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 ...
                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, maxFrameCountLagAllowed, maxFrameCountLagAllowedEver,
    ...
    if (Config::getInstance().getBool("EnableFTPServer", "true") == true) {
        ...
        ftpServer = new FTPServerThread(mapsPath, tilesetsPath, techtreesPath, publishEnabled, allowInternetTilesetFileTransfers,
                                        allowInternetTechtreeFileTransfers, portNumber, GameConstants::maxPlayers, this, tempFilePath);
        ftpServer->start();
    }
...
void ServerInterface::shutdownMasterserverPublishThread() {
    ...
    if (publishToMasterserverThread != NULL) {
        ...
        for (; publishToMasterserverThread->canShutdown(false) == false && difftime((long int)time(NULL), elapsed) <= 15;) {
            // sleep(150);
        }
...
int ServerInterface::isClientAllowedToGetFile(uint32 clientIp, const char *username, const char *filename) {
    ...
    if (username != NULL && strlen(username) > 0 && filename != NULL && strlen(filename) > 0) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s] Line: %d username [%s] file [%s]\n",
        ...
        if (StartsWith(user, "tilesets") == true && EndsWith(file, "7z") == false) {
            if (Config::getInstance().getBool("DisableFTPServerXferUncompressedTilesets", "false") == true) {
                ...
                for (unsigned int index = 0; index < serverList.size(); ++index) {
                    ...
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s] Line: %d clientIP [%s] serverIP [%s] %d / %d\n",
                                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, clientIP.c_str(), serverIP.c_str(),
                    ...
                    if (clientTokens.size() == 4 && serverTokens.size() == 4) {
                        if (clientTokens[0] == serverTokens[0] || clientTokens[1] == serverTokens[1] || clientTokens[2] == serverTokens[2]) {
                            result = 1;

                            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s] Line: %d clientIP [%s] IS NOT BLOCKED\n",
...
void ServerInterface::addSlot(int playerIndex) {
    ...
    if (playerIndex < 0 || playerIndex >= GameConstants::maxPlayers) {
        ...
        snprintf(szBuf, 8096, "In [%s::%s %d] playerIndex is invalid = %d",
                 extractFileFromDirectoryPath(extractFileFromDirectoryPath(__FILE__).c_str()).c_str(), __FUNCTION__, __LINE__, playerIndex);
...
bool ServerInterface::switchSlot(int fromPlayerIndex, int toPlayerIndex) {
    ...
    if (fromPlayerIndex < 0 || fromPlayerIndex >= GameConstants::maxPlayers) {
        ...
        snprintf(szBuf, 8096, "In [%s::%s %d] fromPlayerIndex is invalid = %d",
                 extractFileFromDirectoryPath(extractFileFromDirectoryPath(__FILE__).c_str()).c_str(), __FUNCTION__, __LINE__, fromPlayerIndex);
        throw megaglest_runtime_error(szBuf);
    }
    ...
    if (toPlayerIndex < 0 || toPlayerIndex >= GameConstants::maxPlayers) {
        ...
        snprintf(szBuf, 8096, "In [%s::%s %d] toPlayerIndex is invalid = %d",
                 extractFileFromDirectoryPath(extractFileFromDirectoryPath(__FILE__).c_str()).c_str(), __FUNCTION__, __LINE__, toPlayerIndex);
        throw megaglest_runtime_error(szBuf);
    }
    ...
    if (slots[toPlayerIndex] != NULL && slots[toPlayerIndex]->hasValidSocketId() == false) {
...
int ServerInterface::getConnectedSlotCount(bool authenticated) {
    ...
    for (int index = 0; exitServer == false && index < GameConstants::maxPlayers; ++index) {
        ...
        if (slots[index] != NULL && slots[index]->isConnected() == true) {
            if (authenticated == false || (authenticated == true && slots[index]->getConnectHasHandshaked() == true)) {
                ++connectedSlotCount;
            }
        }
    }
                ...
                if (this->getCurrentFrameCount() > 0) {
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                                 ...
                                                 "%f\n",
                                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, connectionSlot->getPlayerIndex(),
                                                 clientLag, clientLagCount, this->getCurrentFrameCount(), connectionSlot->getCurrentFrameCount(),
                                                 clientLagTime);
                }
        ...
        */
                ...
                if ((maxFrameCountLagAllowed > 0 && clientLagCount > maxFrameCountLagAllowed) ||
                    ...
                    for (unsigned int index = 0; index < languageList.size(); ++index) {
                        ...
                        string msgTemplate = "DROPPING %s, exceeded max allowed LAG count of %f [time = "
                        ...
                        if (gameSettings.getNetworkPauseGameForLaggedClients() == true &&
                            ((maxFrameCountLagAllowedEver <= 0 || clientLagCount <= maxFrameCountLagAllowedEver) &&
                             (maxClientLagTimeAllowedEver <= 0 || clientLagTime <= maxClientLagTimeAllowedEver))) {
                            msgTemplate = "PAUSING GAME TEMPORARILY for %s, exceeded max allowed LAG "
...
#ifdef WIN32
                        _snprintf(szBuf, 4095, msgTemplate.c_str(), connectionSlot->getName().c_str(), maxFrameCountLagAllowed, maxClientLagTimeAllowed,
                                  clientLagCount, clientLagTime);
#else
                        snprintf(szBuf, 4095, msgTemplate.c_str(), connectionSlot->getName().c_str(), maxFrameCountLagAllowed, maxClientLagTimeAllowed,
                                 clientLagCount, clientLagTime);
#endif
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] %s\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                ...
                // New lag check warning
                    ...
                    if (connectionSlot->getLagCountWarning() == false) {
                        ...
                        for (unsigned int index = 0; index < languageList.size(); ++index) {
                            ...
                            string msgTemplate = "LAG WARNING for %s, may exceed max allowed LAG count of %f "
...
void ServerInterface::signalClientsToRecieveData(std::map<PLATFORM_SOCKET, bool> &socketTriggeredList, std::map<int, ConnectionSlotEvent> &eventList,
    ...
    if (newThreadManager == true) {
        ...
        for (int i = 0; exitServer == false && i < GameConstants::maxPlayers; ++i) {
            ...
            if (connectionSlot != NULL) {
                ...
                } else if (this->getGameHasBeenInitiated() == true && this->getAllowInGameConnections() == true) {
...
void ServerInterface::checkForCompletedClientsUsingThreadManager(std::map<int, bool> &mapSlotSignalledList, std::vector<string> &errorMsgList) {
...
void ServerInterface::checkForCompletedClientsUsingLoop(std::map<int, bool> &mapSlotSignalledList, std::vector<string> &errorMsgList,
                                                        std::map<int, ConnectionSlotEvent> &eventList) {
    // time_t waitForThreadElapsed = time(NULL);
    Chrono waitForThreadElapsed(true);

    std::map<int, bool> slotsCompleted;
    for (bool threadsDone = false; exitServer == false && threadsDone == false && waitForThreadElapsed.getMillis() <= MAX_SLOT_THREAD_WAIT_TIME_MILLISECONDS;) {
        threadsDone = true;
        ...
        for (int index = 0; exitServer == false && index < GameConstants::maxPlayers; ++index) {
            ...
            ConnectionSlot *connectionSlot = slots[index];
            if (connectionSlot != NULL && connectionSlot->isConnected() == true && mapSlotSignalledList[index] == true &&
                connectionSlot->getJoinGameInProgress() == false && slotsCompleted.find(index) == slotsCompleted.end()) {
                try {
                    std::vector<std::string> errorList = connectionSlot->getThreadErrorList();
                    // Collect any collected errors from threads
                    if (errorList.empty() == false) {
...
void ServerInterface::checkForCompletedClients(std::map<int, bool> &mapSlotSignalledList, std::vector<string> &errorMsgList,
                                               std::map<int, ConnectionSlotEvent> &eventList) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
...
void ServerInterface::checkForAutoPauseForLaggingClient(int index, ConnectionSlot *connectionSlot) {
    if (gameSettings.getNetworkPauseGameForLaggedClients() == true && this->clientsAutoPausedDueToLag == false) {
        if (connectionSlot != NULL && connectionSlot->isConnected() == true) {
...
void ServerInterface::checkForLaggingClients(std::map<int, bool> &mapSlotSignalledList, std::map<int, ConnectionSlotEvent> &eventList,
                                             std::map<PLATFORM_SOCKET, bool> &socketTriggeredList, std::vector<string> &errorMsgList) {
    ...
    if (gameHasBeenInitiated == true) {
        ...
        for (bool threadsDone = false;
             exitServer == false && threadsDone == false && waitForThreadElapsed.getMillis() <= MAX_SLOT_THREAD_WAIT_TIME_MILLISECONDS;) {
            threadsDone = true;
            // Examine all threads for completion of delegation
            for (int index = 0; exitServer == false && index < GameConstants::maxPlayers; ++index) {
                ...
                if (connectionSlot != NULL && connectionSlot->isConnected() == true && connectionSlot->getSkipLagCheck() == false &&
                    mapSlotSignalledList[index] == true && slotsCompleted.find(index) == slotsCompleted.end()) {
                    ...
                    try {
                        ...
                        if (updateFinished == false) {
                            ...
                            // New lag check
                            std::pair<bool, bool> clientLagExceededOrWarned = std::make_pair(false, false);
                            if (gameHasBeenInitiated == true && connectionSlot != NULL && connectionSlot->isConnected() == true) {
                                ...
                                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                                    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                                             ...
                                                             "%d\n",
                                                             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
                                                             clientLagExceededOrWarned.first, clientLagExceededOrWarned.second,
                                                             gameSettings.getNetworkPauseGameForLaggedClients());
                            ...
                            if ((clientLagExceededOrWarned.second == true && gameSettings.getNetworkPauseGameForLaggedClients() == true)) {
                                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                                    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                                             "In [%s::%s] Line: %d, clientLagExceededOrWarned.first = "
                                                             "%d, clientLagExceededOrWarned.second = %d, "
                                                             ...
                                                             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
                                                             clientLagExceededOrWarned.first, clientLagExceededOrWarned.second,
                                                             (int)waitForClientsElapsed.getMillis(), MAX_CLIENT_WAIT_SECONDS_FOR_PAUSE_MILLISECONDS);
                ...
                if (connectionSlot != NULL && connectionSlot->isConnected() == true && connectionSlot->getSkipLagCheck() == false) {
                    ...
                    try {
                        if (gameHasBeenInitiated == true && difftime((long int)time(NULL), gameStartTime) >= LAG_CHECK_GRACE_PERIOD &&
...
void ServerInterface::dispatchPendingChatMessages(std::vector<string> &errorMsgList) {
    for (int index = 0; exitServer == false && index < GameConstants::maxPlayers; ++index) {
        ...
        if (connectionSlot != NULL && connectionSlot->getChatTextList(false).empty() == false) {
            try {
                std::vector<ChatMsgInfo> chatText = connectionSlot->getChatTextList(true);
                for (int chatIdx = 0; exitServer == false && slots[index] != NULL && chatIdx < (int)chatText.size(); chatIdx++) {
                    ...
                    if (connectionSlot != NULL) {
                        ...
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                            SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                                     ...
                                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, newChatText.c_str(),
                                                     newChatTeamIndex, newChatPlayerIndex);
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] index = %d\n", extractFileFromDirectoryPath(__FILE__).c_str(),
...
void ServerInterface::dispatchPendingMarkCellMessages(std::vector<string> &errorMsgList) {
    for (int index = 0; exitServer == false && index < GameConstants::maxPlayers; ++index) {
        ...
        if (connectionSlot != NULL && connectionSlot->getMarkedCellList(false).empty() == false) {
            try {
...
void ServerInterface::dispatchPendingHighlightCellMessages(std::vector<string> &errorMsgList) {
    for (int index = 0; exitServer == false && index < GameConstants::maxPlayers; ++index) {
        MutexSafeWrapper safeMutexSlot(slotAccessorMutexes[index], CODE_AT_LINE_X(index));
        ConnectionSlot *connectionSlot = slots[index];
        if (connectionSlot != NULL && connectionSlot->getHighlightedCellList(false).empty() == false) {
            try {
                std::vector<MarkedCell> highlightedCells = connectionSlot->getHighlightedCellList(true);
                for (int chatIdx = 0; exitServer == false && slots[index] != NULL && chatIdx < (int)highlightedCells.size(); chatIdx++) {
                    connectionSlot = slots[index];
                    if (connectionSlot != NULL) {
...
void ServerInterface::dispatchPendingUnMarkCellMessages(std::vector<string> &errorMsgList) {
    for (int index = 0; exitServer == false && index < GameConstants::maxPlayers; ++index) {
        MutexSafeWrapper safeMutexSlot(slotAccessorMutexes[index], CODE_AT_LINE_X(index));
        ConnectionSlot *connectionSlot = slots[index];
        if (connectionSlot != NULL && connectionSlot->getUnMarkedCellList(false).empty() == false) {
            try {
...
void ServerInterface::checkForAutoResumeForLaggingClients() {
    if (gameSettings.getNetworkPauseGameForLaggedClients() == true && this->clientsAutoPausedDueToLag == true &&
        this->clientsAutoPausedDueToLagTimer.getMillis() >= MAX_CLIENT_WAIT_SECONDS_FOR_PAUSE_MILLISECONDS) {
...
void ServerInterface::update() {
    ...
    try {
        ...
        if (gameHasBeenInitiated == false || socketTriggeredList.empty() == false) {
            ...
            if (hasData && SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s] hasData == true\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                         __FUNCTION__);
            ...
            if (gameHasBeenInitiated == false || hasData == true) {
                ...
                } else if (gameHasBeenInitiated == true && difftime((long int)time(NULL), gameStartTime) >= LAG_CHECK_GRACE_PERIOD &&
                           difftime((long int)time(NULL), lastGlobalLagCheckTime) >= LAG_CHECK_INTERVAL_PERIOD) {
...
bool ServerInterface::shouldDiscardNetworkMessage(NetworkMessageType networkMessageType, ConnectionSlot *connectionSlot) {
    ...
    if (connectionSlot != NULL) {
        ...
        case nmtMarkCell: {
            ...
            MarkedCell msg(networkMessageMarkCell.getTarget(), networkMessageMarkCell.getFactionIndex(), networkMessageMarkCell.getText().c_str(),
                           networkMessageMarkCell.getPlayerIndex());

            this->addMarkedCell(msg);

            NetworkMessageMarkCell networkMessageMarkCellBroadcast(networkMessageMarkCell.getTarget(), networkMessageMarkCell.getFactionIndex(),
                                                                   networkMessageMarkCell.getText().c_str(), networkMessageMarkCell.getPlayerIndex());
        ...
        case nmtUnMarkCell: {
            ...
            UnMarkedCell msg(networkMessageMarkCell.getTarget(), networkMessageMarkCell.getFactionIndex());

            this->addUnMarkedCell(msg);

            NetworkMessageUnMarkCell networkMessageMarkCellBroadcast(networkMessageMarkCell.getTarget(), networkMessageMarkCell.getFactionIndex());
            broadcastMessage(&networkMessageMarkCellBroadcast, connectionSlot->getPlayerIndex());
        ...
        case nmtHighlightCell: {
            ...
            MarkedCell msg(networkMessageHighlightCell.getTarget(), networkMessageHighlightCell.getFactionIndex(), "none", -1);

            this->setHighlightedCell(msg);

            NetworkMessageHighlightCell networkMessageHighlightCellBroadcast(networkMessageHighlightCell.getTarget(),
            ...
            if (chrono.getMillis() > readyWaitTimeout) {
                ...
                if (chrono.getMillis() - lastStatusUpdate > 200) {
                    ...
                    if (updateTextFormat == "" || updateTextFormat[0] == '?') {
                        updateTextFormat = "Waiting for network: %lld seconds elapsed (maximum wait time: "
                                           "%d seconds)";
                    }
...
void ServerInterface::queueTextMessage(const string &text, int teamIndex, bool echoLocal, string targetLanguage) {
...
void ServerInterface::sendTextMessage(const string &text, int teamIndex, bool echoLocal, string targetLanguage) {
...
bool ServerInterface::launchGame(const GameSettings *gameSettings) {
    ...
    for (int index = 0; exitServer == false && index < GameConstants::maxPlayers; ++index) {
        MutexSafeWrapper safeMutexSlot(slotAccessorMutexes[index], CODE_AT_LINE_X(index));
        ConnectionSlot *connectionSlot = slots[index];
        if (connectionSlot != NULL && (connectionSlot->getAllowDownloadDataSynch() == true || connectionSlot->getAllowGameDataSynchCheck() == true) &&
            connectionSlot->isConnected()) {
            if (connectionSlot->getNetworkGameDataSynchCheckOk() == false) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] map [%d] tile [%d] techtree [%d]\n",
                                             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
                                             connectionSlot->getNetworkGameDataSynchCheckOkMap(), connectionSlot->getNetworkGameDataSynchCheckOkTile(),
    ...
    if (bOkToStart == true) {
        ...
        for (int startIndex = 0; startIndex < GameConstants::maxPlayers; ++startIndex) {
            ...
            if ((connectionSlot == NULL || connectionSlot->isConnected() == false) && this->getAllowInGameConnections() == true) {
                // Open slots for joining in progress game
                if (gameSettings->getFactionControl(factionIndex) != ctClosed && gameSettings->getFactionControl(factionIndex) != ctHuman) {
        ...
        if ((needToRepublishToMasterserver == true || GlobalStaticFlags::getIsNonGraphicalModeEnabled() == true) && requiresUPNPTrigger == true) {
        ...
        if (difftime((long int)time(NULL), lastListenerSlotCheckTime) >= 7) {
            ...
            for (int startIndex = 0; startIndex < GameConstants::maxPlayers; ++startIndex) {
                ...
                if (gameSettings.getFactionControl(factionIndex) != ctClosed && gameSettings.getFactionControl(factionIndex) != ctHuman) {
                    ...
                    // Open slots for joining in progress game
                    if (connectionSlot == NULL) {
                        printf("Opening slot for in game connections, slot: %d, factionindex: "
                               "%d name: %s\n",
                        ...
                        connectionSlot->setCanAcceptConnections(true);
                    } else if (connectionSlot != NULL && connectionSlot->getCanAcceptConnections() == false && connectionSlot->isConnected() == false) {
...
void ServerInterface::broadcastGameSetup(GameSettings *gameSettingsBuffer, bool setGameSettingsBuffer) {
    ...
    for (unsigned int factionIndex = 0; factionIndex < (unsigned int)gameSettingsBuffer->getFactionCount(); ++factionIndex) {
        ...
        if (gameSettingsBuffer->getFactionControl(factionIndex) == ctNetwork && isClientConnected(slotIndex) == false) {
...
void ServerInterface::broadcastMessage(NetworkMessage *networkMessage, int excludeSlot, int lockedSlotIndex) {
    try {
        ...
        for (int slotIndex = 0; exitServer == false && slotIndex < GameConstants::maxPlayers; ++slotIndex) {
            ...
            } else if (slotIndex == excludeSlot && gameHasBeenInitiated == true && connectionSlot != NULL && connectionSlot->isConnected() == false) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        ...
        if (needToRepublishToMasterserver == true) {
            try {
                if (Config::getInstance().getString("Masterserver", "") != "") {
                    ...
                    for (std::map<string, string>::const_iterator iterMap = newPublishToServerInfo.begin(); iterMap != newPublishToServerInfo.end();
                         ++iterMap) {
                    ...
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line %d] the request is:\n%s\n",
                    ...
                    if (newPublishToServerInfoStats.empty() == false) {
                        ...
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line %d] the result is:\n'%s'\n",
        ...
        for (int slotIndex = 0; exitServer == false && slotIndex < GameConstants::maxPlayers; ++slotIndex) {
            ...
            if (connectionSlot != NULL) {
                if (connectionSlot->getUUID() == uuid) {
                    if (unConnectedOnly == false || (unConnectedOnly == true && connectionSlot->isConnected() == false)) {
                        if (result == NULL || (result->getConnectedTime() > connectionSlot->getConnectedTime())) result = connectionSlot;
                    }
                }
            }
        }

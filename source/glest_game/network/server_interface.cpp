...
ServerInterface::ServerInterface(
    bool publishEnabled, ClientLagCallbackInterface *clientLagCallbackInterface)
  ...
  try {
    ...
    serverSocketAdmin->setBindPort(Config::getInstance().getInt(
        "ServerAdminPort", intToStr(GameConstants::serverAdminPort).c_str()));
    serverSocketAdmin->setBindSpecificAddress(
        Config::getInstance().getString("ServerAdminBindAddress", "127.0.0.1"));
    serverSocketAdmin->listen(5);
  } catch (const std::exception &ex) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] Warning Server admin port bind/listen "
             "error:\n%s\n",
  ...
  maxFrameCountLagAllowed = Config::getInstance().getInt(
      "MaxFrameCountLagAllowed", intToStr(maxFrameCountLagAllowed).c_str());
  maxFrameCountLagAllowedEver = Config::getInstance().getInt(
      "MaxFrameCountLagAllowedEver",
      intToStr(maxFrameCountLagAllowedEver).c_str());
  maxClientLagTimeAllowedEver = Config::getInstance().getInt(
      "MaxClientLagTimeAllowedEver",
      intToStr(maxClientLagTimeAllowedEver).c_str());
  maxClientLagTimeAllowed = Config::getInstance().getInt(
      "MaxClientLagTimeAllowed", intToStr(maxClientLagTimeAllowed).c_str());
  warnFrameCountLagPercent = Config::getInstance().getFloat(
      "WarnFrameCountLagPercent",
      doubleToStr(warnFrameCountLagPercent).c_str());

  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s Line: %d] maxFrameCountLagAllowed = %f, "
        "maxFrameCountLagAllowedEver = %f, maxClientLagTimeAllowed = %f, "
        "maxClientLagTimeAllowedEver = %f\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        maxFrameCountLagAllowed, maxFrameCountLagAllowedEver,
        maxClientLagTimeAllowed, maxClientLagTimeAllowedEver);
  ...
  for (int i = 0; i < (int)mapFiles.size();
    ...
    // printf("mapFiles.at(i) %s allMaps.at[i]
    // %s\n",mapFiles[i].c_str(),allMaps.at(i).c_str());
    MapPreview::loadMapInfo(Config::getMapPath(mapFiles.at(i)), &mapInfo,
                            "MaxPlayers", "Size", true);
  ...
  if (Config::getInstance().getBool("EnableFTPServer", "true") == true) {
    ...
    int portNumber = Config::getInstance().getInt(
        "FTPServerPort", intToStr(ServerSocket::getFTPServerPort()).c_str());
    ServerSocket::setFTPServerPort(portNumber);
    // printf("In [%s::%s] portNumber = %d ServerSocket::getFTPServerPort() =
    // %d\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,portNumber,ServerSocket::getFTPServerPort());

    bool allowInternetTilesetFileTransfers = Config::getInstance().getBool(
        "EnableFTPServerInternetTilesetXfer", "true");
    bool allowInternetTechtreeFileTransfers = Config::getInstance().getBool(
        "EnableFTPServerInternetTechtreeXfer", "true");
    ...
    ftpServer = new FTPServerThread(
        mapsPath, tilesetsPath, techtreesPath, publishEnabled,
        allowInternetTilesetFileTransfers, allowInternetTechtreeFileTransfers,
        portNumber, GameConstants::maxPlayers, this, tempFilePath);
    ftpServer->start();
  }
  ...
  if (publishToMasterserverThread == NULL) {
    if (needToRepublishToMasterserver == true ||
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "In [%s::%s Line: %d] needToRepublishToMasterserver = %d\n",
            extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
            __LINE__, needToRepublishToMasterserver);
    }
  }
...
ServerInterface::~ServerInterface() {
  ...
  for (int index = 0; index < GameConstants::maxPlayers; ++index) {
    if (slots[index] != NULL) {
      MutexSafeWrapper safeMutex(slotAccessorMutexes[index],
                                 CODE_AT_LINE_X(index));
      delete slots[index];
      slots[index] = NULL;
    }
...
void ServerInterface::setSwitchSetupRequests(int index,
                                             SwitchSetupRequest *ptr) {
  MutexSafeWrapper safeMutex(switchSetupRequestsSynchAccessor, CODE_AT_LINE);
...
int ServerInterface::isValidClientType(uint32 clientIp) {
  int result = 0;
  for (int index = 0; exitServer == false && index < GameConstants::maxPlayers;
       ++index) {
    MutexSafeWrapper safeMutex(slotAccessorMutexes[index],
                               CODE_AT_LINE_X(index));
    if (slots[index] != NULL) {
...
int ServerInterface::isClientAllowedToGetFile(uint32 clientIp,
                                              const char *username,
                                              const char *filename) {
  int result = 1;

  if (username != NULL && strlen(username) > 0 && filename != NULL &&
      strlen(filename) > 0) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "In [%s::%s] Line: %d username [%s] file [%s]\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__, username, filename);

    if (StartsWith(user, "tilesets") == true && EndsWith(file, "7z") == false) {
      if (Config::getInstance().getBool(
              "DisableFTPServerXferUncompressedTilesets", "false") == true) {
        result = 0;
      } else {
        char szIP[100] = "";
        ...
        for (unsigned int index = 0; index < serverList.size(); ++index) {
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s] Line: %d clientIP [%s] serverIP [%s] %d / %d\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, clientIP.c_str(), serverIP.c_str(), index,
                serverList.size());
          ...
          if (clientTokens.size() == 4 && serverTokens.size() == 4) {
            if (clientTokens[0] == serverTokens[0] ||
              ...
              if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                ...
                SystemFlags::OutputDebug(
                    SystemFlags::debugNetwork,
                    "In [%s::%s] Line: %d clientIP [%s] IS NOT BLOCKED\n",
                    extractFileFromDirectoryPath(__FILE__).c_str(),
                    __FUNCTION__, __LINE__, clientIP.c_str());
...
void ServerInterface::addClientToServerIPAddress(uint32 clientIp,
                                                 uint32 ServerIp) {
  FTPServerThread::addClientToServerIPAddress(clientIp, ServerIp);
}

void ServerInterface::addSlot(int playerIndex) {
  // printf("Adding slot for playerIndex = %d, serverSocket.isPortBound() =
  ...
  if (playerIndex < 0 || playerIndex >= GameConstants::maxPlayers) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "In [%s::%s %d] playerIndex is invalid = %d",
  ...
  if (serverSocketAdmin != NULL &&
      serverSocketAdmin->isSocketValid() == false) {
    serverSocketAdmin->listen(5);
  }
  ...
  MutexSafeWrapper safeMutexSlot(slotAccessorMutexes[playerIndex],
                                 CODE_AT_LINE_X(playerIndex));
...
void ServerInterface::removeSlot(int playerIndex, int lockedSlotIndex) {
  // printf("Removing slot for playerIndex = %d, serverSocket.isPortBound() =
  // %d\n",playerIndex,serverSocket.isPortBound());
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork,
        "In [%s::%s Line: %d] playerIndex = %d, lockedSlotIndex = %d\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        playerIndex, lockedSlotIndex);
  ...
  if (playerIndex != lockedSlotIndex) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          ...
          "In [%s::%s Line: %d] playerIndex = %d, lockedSlotIndex = %d\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, playerIndex, lockedSlotIndex);
    safeMutexSlot.setMutex(slotAccessorMutexes[playerIndex],
                           CODE_AT_LINE_X(playerIndex));
  }
    ...
    if (slot->getLastReceiveCommandListTime() > 0) {
      ...
      for (unsigned int index = 0; index < languageList.size(); ++index) {
        ...
        if (lang.hasString("PlayerDisconnected", languageList[index]) == true) {
          msgTemplate =
              lang.getString("PlayerDisconnected", languageList[index]);
        }
#ifdef WIN32
...
bool ServerInterface::switchSlot(int fromPlayerIndex, int toPlayerIndex) {
  ...
  if (fromPlayerIndex < 0 || fromPlayerIndex >= GameConstants::maxPlayers) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "In [%s::%s %d] fromPlayerIndex is invalid = %d",
             extractFileFromDirectoryPath(
                 extractFileFromDirectoryPath(__FILE__).c_str())
                 .c_str(),
             __FUNCTION__, __LINE__, fromPlayerIndex);
    throw megaglest_runtime_error(szBuf);
  }

  if (toPlayerIndex < 0 || toPlayerIndex >= GameConstants::maxPlayers) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "In [%s::%s %d] toPlayerIndex is invalid = %d",
             extractFileFromDirectoryPath(
                 extractFileFromDirectoryPath(__FILE__).c_str())
                 .c_str(),
             __FUNCTION__, __LINE__, toPlayerIndex);
    throw megaglest_runtime_error(szBuf);
  }
  ...
  MutexSafeWrapper safeMutex(serverSynchAccessor, CODE_AT_LINE);
  MutexSafeWrapper safeMutexSlot(slotAccessorMutexes[fromPlayerIndex],
                                 CODE_AT_LINE_X(fromPlayerIndex));
  MutexSafeWrapper safeMutexSlot2(slotAccessorMutexes[toPlayerIndex],
                                  CODE_AT_LINE_X(toPlayerIndex));
  ...
  if (slots[toPlayerIndex] != NULL &&
    ...
    // printf("#3 Server is switching slots aborted, is slot already
    // connected?\n");
...
ConnectionSlot *ServerInterface::getSlot(int playerIndex, bool lockMutex) {
  ...
  MutexSafeWrapper safeMutexSlot(
      (lockMutex == true ? slotAccessorMutexes[playerIndex] : NULL),
      CODE_AT_LINE_X(playerIndex));
...
int ServerInterface::getSlotCount() {
  ...
  for (int index = 0; exitServer == false && index < GameConstants::maxPlayers;
       ++index) {
    MutexSafeWrapper safeMutexSlot(slotAccessorMutexes[index],
                                   CODE_AT_LINE_X(index));
    if (slots[index] != NULL) {
      ++slotCount;
    }
  ...
  for (int index = 0; exitServer == false && index < GameConstants::maxPlayers;
    ...
    if (slots[index] != NULL && slots[index]->isConnected() == true) {
      if (authenticated == false ||
          (authenticated == true &&
           slots[index]->getConnectHasHandshaked() == true)) {
        ++connectedSlotCount;
      }
    }
  }
...
std::pair<bool, bool>
ServerInterface::clientLagCheck(ConnectionSlot *connectionSlot,
                                bool skipNetworkBroadCast) {
  std::pair<bool, bool> clientLagExceededOrWarned =
      std::make_pair(false, false);
  static bool alreadyInLagCheck = false;

  if (alreadyInLagCheck == true ||
      (connectionSlot != NULL &&
       (connectionSlot->getSkipLagCheck() == true ||
  ...
  try {
    ...
    if ((gameStartTime > 0 && difftime((long int)time(NULL), gameStartTime) >=
                                  LAG_CHECK_GRACE_PERIOD) &&
        (resumeGameStartTime == 0 ||
         (resumeGameStartTime > 0 &&
          difftime((long int)time(NULL), resumeGameStartTime) >=
              LAG_CHECK_GRACE_PERIOD))) {
      if (connectionSlot != NULL && connectionSlot->isConnected() == true) {

        double clientLag = this->getCurrentFrameCount() -
        ...
        double clientLagTime =
            difftime((long int)time(NULL),
                     connectionSlot->getLastReceiveCommandListTime());
        ...
        if (this->getCurrentFrameCount() > 0) {
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] playerIndex = %d, clientLag = %f, "
                "clientLagCount = %f, this->getCurrentFrameCount() = %d, "
                "connectionSlot->getCurrentFrameCount() = %d, clientLagTime = "
                "%f\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, connectionSlot->getPlayerIndex(), clientLag,
                clientLagCount, this->getCurrentFrameCount(),
                connectionSlot->getCurrentFrameCount(), clientLagTime);
        }
        ...
        warnFrameCountLagPercent) + 1;
        }
        */
        // END test

        // printf("skipNetworkBroadCast [%d] clientLagCount [%f][%f][%f]
        // clientLagTime
        // [%f][%f][%f]\n",skipNetworkBroadCast,clientLagCount,(maxFrameCountLagAllowed
        // * warnFrameCountLagPercent),maxFrameCountLagAllowed,clientLagTime,(maxClientLagTimeAllowed * warnFrameCountLagPercent),maxClientLagTimeAllowed);

        // New lag check
        if ((maxFrameCountLagAllowed > 0 &&
             clientLagCount > maxFrameCountLagAllowed) ||
            (maxClientLagTimeAllowed > 0 &&
             clientLagTime > maxClientLagTimeAllowed) ||
            (maxFrameCountLagAllowedEver > 0 &&
             clientLagCount > maxFrameCountLagAllowedEver) ||
            (maxClientLagTimeAllowedEver > 0 &&
             clientLagTime > maxClientLagTimeAllowedEver)) {
          ...
          for (unsigned int index = 0; index < languageList.size(); ++index) {
            ...
            string msgTemplate =
                "DROPPING %s, exceeded max allowed LAG count of %f [time = "
                "%f], clientLag = %f [%f], disconnecting client.";
            if (lang.hasString("ClientLagDropping") == true) {
              msgTemplate =
                  lang.getString("ClientLagDropping", languageList[index]);
            }
            if (gameSettings.getNetworkPauseGameForLaggedClients() == true &&
                ((maxFrameCountLagAllowedEver <= 0 ||
                 ...
                 (maxClientLagTimeAllowedEver <= 0 ||
                  clientLagTime <= maxClientLagTimeAllowedEver))) {
              msgTemplate = "PAUSING GAME TEMPORARILY for %s, exceeded max "
                            "allowed LAG count of %f [time = %f], clientLag = "
                            "%f [%f], waiting for client to catch up...";
              if (lang.hasString("ClientLagPausing") == true) {
                msgTemplate =
                    lang.getString("ClientLagPausing", languageList[index]);
              }
            }
#ifdef WIN32
            _snprintf(szBuf, 4095, msgTemplate.c_str(),
                      connectionSlot->getName().c_str(),
                      maxFrameCountLagAllowed, maxClientLagTimeAllowed,
                      clientLagCount, clientLagTime);
#else
            snprintf(szBuf, 4095, msgTemplate.c_str(),
                     connectionSlot->getName().c_str(), maxFrameCountLagAllowed,
                     maxClientLagTimeAllowed, clientLagCount, clientLagTime);
#endif
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                    .enabled)
              SystemFlags::OutputDebug(
            ...
            if (skipNetworkBroadCast == false) {
              ...
              sendTextMessage(sMsg, -1, echoLocal, languageList[index],
        ...
        // New lag check warning
                  ...
                  clientLagCount >
                      (maxFrameCountLagAllowed * warnFrameCountLagPercent)) ||
                 (maxClientLagTimeAllowed > 0 && warnFrameCountLagPercent > 0 &&
                  clientLagTime >
                      (maxClientLagTimeAllowed * warnFrameCountLagPercent))) {
          ...
          if (connectionSlot->getLagCountWarning() == false) {
            ...
            for (unsigned int index = 0; index < languageList.size(); ++index) {
              ...
              string msgTemplate =
                  "LAG WARNING for %s, may exceed max allowed LAG count of %f "
                  "[time = %f], clientLag = %f [%f], WARNING...";
              if (lang.hasString("ClientLagWarning") == true) {
                msgTemplate =
                    lang.getString("ClientLagWarning", languageList[index]);
              }
...
void ServerInterface::updateSocketTriggeredList(
...
bool ServerInterface::signalClientReceiveCommands(
    ConnectionSlot *connectionSlot, int slotIndex, bool socketTriggered,
...
void ServerInterface::signalClientsToRecieveData(
    std::map<PLATFORM_SOCKET, bool> &socketTriggeredList,
  ...
  if (newThreadManager == true) {
    ...
    for (int index = 0;
         exitServer == false && index < GameConstants::maxPlayers; ++index) {
      MutexSafeWrapper safeMutexSlot(slotAccessorMutexes[index],
                                     CODE_AT_LINE_X(index));
      ConnectionSlot *connectionSlot = slots[index];
      ...
      if (connectionSlot != NULL) {
        ...
        bool socketSignalled = signalClientReceiveCommands(
            connectionSlot, index, socketTriggered, event);
...
void ServerInterface::checkForCompletedClientsUsingThreadManager(
  ...
  masterController.waitTillSlavesTrigger(
      MAX_SLOT_THREAD_WAIT_TIME_MILLISECONDS);
  masterController.clearSlaves(true);
...
void ServerInterface::checkForCompletedClientsUsingLoop(
  ...
  for (bool threadsDone = false; exitServer == false && threadsDone == false &&
                                 waitForThreadElapsed.getMillis() <=
                                     MAX_SLOT_THREAD_WAIT_TIME_MILLISECONDS;) {
    ...
    for (int index = 0;
      ...
      if (connectionSlot != NULL && connectionSlot->isConnected() == true &&
        ...
        try {
          ...
          bool updateFinished =
              (connectionSlot != NULL
                   ? connectionSlot->updateCompleted(&eventList[index])
                   : true);
...
void ServerInterface::setClientLagCallbackInterface(
    ClientLagCallbackInterface *intf) {
...
void ServerInterface::checkForCompletedClients(
    std::map<int, bool> &mapSlotSignalledList,
    std::vector<string> &errorMsgList,
    std::map<int, ConnectionSlotEvent> &eventList) {
  ...
  const bool newThreadManager =
      Config::getInstance().getBool("EnableNewThreadManager", "false");
  if (newThreadManager == true) {
    checkForCompletedClientsUsingThreadManager(mapSlotSignalledList,
                                               errorMsgList);
  } else {
    checkForCompletedClientsUsingLoop(mapSlotSignalledList, errorMsgList,
                                      eventList);
  }
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
...
void ServerInterface::checkForAutoPauseForLaggingClient(
    int index, ConnectionSlot *connectionSlot) {

  if (gameSettings.getNetworkPauseGameForLaggedClients() == true &&
      this->clientsAutoPausedDueToLag == false) {
    if (connectionSlot != NULL && connectionSlot->isConnected() == true) {
      if (connectionSlot->getAutoPauseGameCountForLag() <
          MAX_CLIENT_PAUSE_FOR_LAG_COUNT) {
        if (this->clientLagCallbackInterface != NULL) {

          if (this->clientsAutoPausedDueToLagTimer.isStarted() == false ||
              this->clientsAutoPausedDueToLagTimer.getMillis() >=
            ...
            if (this->clientLagCallbackInterface->clientLagHandler(
                    index, true) == false) {
              connectionSlot->close();
            } else {
...
void ServerInterface::checkForLaggingClients(
    std::map<int, bool> &mapSlotSignalledList,
    std::map<int, ConnectionSlotEvent> &eventList,
    std::map<PLATFORM_SOCKET, bool> &socketTriggeredList,
    std::vector<string> &errorMsgList) {
    ...
    for (bool threadsDone = false;
      ...
      for (int index = 0;
        ...
        if (connectionSlot != NULL && connectionSlot->isConnected() == true &&
          ...
          // printf("#2 Check lag for i: %d playerindex: %d name [%s] socket:
          ...
          try {
            ...
            if (errorList.empty() == false) {
              for (int iErrIdx = 0; iErrIdx < (int)errorList.size();
                   ++iErrIdx) {
                string &sErr = errorList[iErrIdx];
                if (sErr != "") {
                  errorMsgList.push_back(sErr);
            ...
            if (updateFinished == false) {
              ...
              if (gameHasBeenInitiated == true && connectionSlot != NULL &&
                ...
                clientLagExceededOrWarned =
                    clientLagCheck(connectionSlot, slotsWarnedList[index]);

                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                        .enabled)
                  SystemFlags::OutputDebug(
                      SystemFlags::debugNetwork,
                      "In [%s::%s Line: %d] clientLagExceededOrWarned.first = "
                      "%d, clientLagExceededOrWarned.second = %d, "
                      "gameSettings.getNetworkPauseGameForLaggedClients() = "
                      "%d\n",
                      extractFileFromDirectoryPath(__FILE__).c_str(),
                      __FUNCTION__, __LINE__, clientLagExceededOrWarned.first,
                      clientLagExceededOrWarned.second,
                      gameSettings.getNetworkPauseGameForLaggedClients());
              ...
              if ((clientLagExceededOrWarned.second == true &&
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                  ...
                  SystemFlags::OutputDebug(
                      ...
                      "In [%s::%s] Line: %d, clientLagExceededOrWarned.first = "
                      "%d, clientLagExceededOrWarned.second = %d, "
                      "waitForClientsElapsed.getMillis() = %d, "
                      "MAX_CLIENT_WAIT_SECONDS_FOR_PAUSE_MILLISECONDS = %d\n",
                      extractFileFromDirectoryPath(__FILE__).c_str(),
                      __FUNCTION__, __LINE__, clientLagExceededOrWarned.first,
                      clientLagExceededOrWarned.second,
                      (int)waitForClientsElapsed.getMillis(),
                      MAX_CLIENT_WAIT_SECONDS_FOR_PAUSE_MILLISECONDS);
        ...
        if (connectionSlot != NULL && connectionSlot->isConnected() == true &&
          ...
          try {
            if (gameHasBeenInitiated == true &&
                difftime((long int)time(NULL), gameStartTime) >=
                    LAG_CHECK_GRACE_PERIOD &&
                difftime((long int)time(NULL), lastGlobalLagCheckTime) >=
                    LAG_CHECK_INTERVAL_PERIOD) {
...
void ServerInterface::executeNetworkCommandsFromClients() {
  if (gameHasBeenInitiated == true) {
    for (int index = 0;
      ...
      ConnectionSlot *connectionSlot = slots[index];
      if (connectionSlot != NULL && connectionSlot->isConnected() == true) {
        vector<NetworkCommand> pendingList =
            connectionSlot->getPendingNetworkCommandList(true);
        if (pendingList.empty() == false) {
          for (int idx = 0;
               exitServer == false && idx < (int)pendingList.size(); ++idx) {
            NetworkCommand &cmd = pendingList[idx];
            this->requestCommand(&cmd);
          }
          // printf("Executed: %d commands from slot:
...
void ServerInterface::dispatchPendingChatMessages(
    std::vector<string> &errorMsgList) {
  for (int index = 0; exitServer == false && index < GameConstants::maxPlayers;
       ++index) {
    ...
    if (connectionSlot != NULL &&
      ...
      try {
        std::vector<ChatMsgInfo> chatText =
            connectionSlot->getChatTextList(true);
        for (int chatIdx = 0; exitServer == false && slots[index] != NULL &&
                              chatIdx < (int)chatText.size();
             chatIdx++) {
          ...
          if (connectionSlot != NULL) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
              ...
              SystemFlags::OutputDebug(
                  SystemFlags::debugNetwork,
                  "In [%s::%s Line: %d] #1 about to broadcast nmtText chatText "
                  "[%s] chatTeamIndex = %d, newChatPlayerIndex = %d\n",
                  extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                  __LINE__, newChatText.c_str(), newChatTeamIndex,
                  newChatPlayerIndex);
            ...
            if (newChatLanguage == "" ||
              ...
              NetworkMessageText networkMessageText(
                  newChatText.c_str(), newChatTeamIndex, newChatPlayerIndex,
                  newChatLanguage);
              broadcastMessage(&networkMessageText,
                               connectionSlot->getPlayerIndex(), index);
            }
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
              ...
              SystemFlags::OutputDebug(
                  SystemFlags::debugNetwork,
                  "In [%s::%s Line: %d] after broadcast nmtText chatText [%s] "
                  "chatTeamIndex = %d\n",
                  extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                  __LINE__, newChatText.c_str(), newChatTeamIndex);
          }
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                .enabled)
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork, "In [%s::%s Line: %d] index = %d\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, index);
...
void ServerInterface::dispatchPendingMarkCellMessages(
    std::vector<string> &errorMsgList) {
  for (int index = 0; exitServer == false && index < GameConstants::maxPlayers;
       ++index) {
    ...
    if (connectionSlot != NULL &&
      ...
      try {
        std::vector<MarkedCell> chatText =
            connectionSlot->getMarkedCellList(true);
        for (int chatIdx = 0; exitServer == false && slots[index] != NULL &&
                              chatIdx < (int)chatText.size();
             chatIdx++) {
          ...
          if (connectionSlot != NULL) {
            ...
            NetworkMessageMarkCell networkMessageMarkCell(
                msg.getTargetPos(), msg.getFactionIndex(), msg.getNote(),
                msg.getPlayerIndex());
            broadcastMessage(&networkMessageMarkCell,
                             connectionSlot->getPlayerIndex(), index);
            // if(SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
            // SystemFlags::OutputDebug(SystemFlags::debugNetwork,"In [%s::%s
            // Line: %d] after broadcast nmtText chatText [%s] chatTeamIndex =
...
void ServerInterface::dispatchPendingHighlightCellMessages(
    std::vector<string> &errorMsgList) {
  for (int index = 0; exitServer == false && index < GameConstants::maxPlayers;
       ++index) {
    ...
    if (connectionSlot != NULL &&
      ...
      try {
        std::vector<MarkedCell> highlightedCells =
            connectionSlot->getHighlightedCellList(true);
        for (int chatIdx = 0; exitServer == false && slots[index] != NULL &&
                              chatIdx < (int)highlightedCells.size();
             chatIdx++) {
          ...
          if (connectionSlot != NULL) {
            ...
            NetworkMessageHighlightCell networkMessageHighlightCell(
                msg.getTargetPos(), msg.getFactionIndex());
            broadcastMessage(&networkMessageHighlightCell,
                             connectionSlot->getPlayerIndex(), index);
          }
        }
...
void ServerInterface::dispatchPendingUnMarkCellMessages(
    std::vector<string> &errorMsgList) {
  for (int index = 0; exitServer == false && index < GameConstants::maxPlayers;
       ++index) {
    ...
    if (connectionSlot != NULL &&
      ...
      try {
        std::vector<UnMarkedCell> chatText =
            connectionSlot->getUnMarkedCellList(true);
        for (int chatIdx = 0; exitServer == false && slots[index] != NULL &&
                              chatIdx < (int)chatText.size();
             chatIdx++) {
...
void ServerInterface::checkForAutoResumeForLaggingClients() {
  if (gameSettings.getNetworkPauseGameForLaggedClients() == true &&
    ...
    // printf("this->clientsAutoPausedDueToLag: %d
    // [%lld]\n",this->clientsAutoPausedDueToLag,(long
    // long)this->clientsAutoPausedDueToLagTimer.getMillis());
    if (this->clientLagCallbackInterface != NULL) {
  ...
  try {
    ...
    if (gameHasBeenInitiated == false || socketTriggeredList.empty() == false) {
      ...
      if (gameHasBeenInitiated == false || hasData == true) {
        ...
        // data
        if (gameHasBeenInitiated == false) {
          signalClientsToRecieveData(socketTriggeredList, eventList,
        ...
        if (gameHasBeenInitiated == false || hasData == true) {
          ...
          // Step #2 check all connection slot worker threads for completed
          // status
          if (gameHasBeenInitiated == false) {
            checkForCompletedClients(mapSlotSignalledList, errorMsgList,
                                     eventList);
          }
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                  .enabled)
          ...
          // Step #3 check clients for any lagging scenarios and try to deal
          // with them
          if (gameHasBeenInitiated == false) {
            checkForLaggingClients(mapSlotSignalledList, eventList,
                                   socketTriggeredList, errorMsgList);
          }
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                  .enabled)
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] ============ Step #4\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__);
          ...
          // printf("In [%s::%s Line:
          // %d]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__);
          //  Step #4 dispatch network commands to the pending list so that they
          //  are done in proper order
          executeNetworkCommandsFromClients();
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                  .enabled)
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] ============ Step #5\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__);
    ...
    // Check if we need to switch masterserver admin to a new player because
    // original admin disconnected
    if (gameHasBeenInitiated == true &&
      ...
      for (int index = 0; index < GameConstants::maxPlayers; ++index) {
        ...
        if (slots[index] != NULL && slots[index]->isConnected() == true) {
          ...
          if (this->gameSettings.getMasterserver_admin() ==
              slots[index]->getSessionKey()) {
            foundAdminSlot = true;
            break;
          }
        }
      ...
      if (foundAdminSlot == false && iFirstConnectedSlot >= 0) {
        ...
        MutexSafeWrapper safeMutexSlot(slotAccessorMutexes[iFirstConnectedSlot],
                                       CODE_AT_LINE_X(iFirstConnectedSlot));
        if (slots[iFirstConnectedSlot] != NULL) {
          string sMsg = "Switching player to admin mode: " +
                        slots[iFirstConnectedSlot]->getName();
          sendTextMessage(sMsg, -1, true, "");

          this->gameSettings.setMasterserver_admin(
              slots[iFirstConnectedSlot]->getSessionKey());
          this->gameSettings.setMasterserver_admin_faction_index(
              slots[iFirstConnectedSlot]->getPlayerIndex());
...
void ServerInterface::updateKeyframe(int frameCount) {
  ...
  try {
    ...
    if (requestedCommands.empty() == false) {
      ...
      string sMsg = "may go out of synch: server requestedCommands.size() = " +
    ...
    if (sendBroadcastMessage == false) {
      ...
      // broadcasts every
      // MAX_EMPTY_NETWORK_COMMAND_LIST_BROADCAST_INTERVAL_MILLISECONDS
      else if (
          this->getClientsAutoPausedDueToLag() == true &&
          (lastBroadcastCommandsTimer.isStarted() == false ||
           lastBroadcastCommandsTimer.getMillis() >=
               MAX_EMPTY_NETWORK_COMMAND_LIST_BROADCAST_INTERVAL_MILLISECONDS)) {
...
bool ServerInterface::shouldDiscardNetworkMessage(
    NetworkMessageType networkMessageType, ConnectionSlot *connectionSlot) {
  bool discard = false;
  if (connectionSlot != NULL) {
    ...
    case nmtText: {
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugNetwork,
            "In [%s::%s Line: %d] #1 about to broadcast nmtText chatText [%s] "
            "chatTeamIndex = %d, newChatPlayerIndex = %d\n",
            extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
            __LINE__, newChatText.c_str(), newChatTeamIndex,
            newChatPlayerIndex);
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s Line: %d] after broadcast nmtText "
                                 "chatText [%s] chatTeamIndex = %d\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__, newChatText.c_str(),
                                 newChatTeamIndex);
    ...
    case nmtMarkCell: {
      ...
      NetworkMessageMarkCell networkMessageMarkCellBroadcast(
          ...
          networkMessageMarkCell.getText().c_str(),
          networkMessageMarkCell.getPlayerIndex());
      broadcastMessage(&networkMessageMarkCellBroadcast,
                       connectionSlot->getPlayerIndex());

      // if(SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      // SystemFlags::OutputDebug(SystemFlags::debugNetwork,"In [%s::%s Line:
      // %d] after broadcast nmtMarkCell chatText [%s] chatTeamIndex =
      // %d\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,newChatText.c_str(),newChatTeamIndex);
    ...
    case nmtHighlightCell: {
      ...
      broadcastMessage(&networkMessageHighlightCellBroadcast,
                       connectionSlot->getPlayerIndex());

      // if(SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      // SystemFlags::OutputDebug(SystemFlags::debugNetwork,"In [%s::%s Line:
      // %d] after broadcast nmtMarkCell  chatTeamIndex =
...
void ServerInterface::waitUntilReady(Checksum *checksum) {
  ...
  if (Config::getInstance().getBool("EnableGameServerLoadCancel", "false") ==
      true) {
  ...
  MAX_LOOP_COUNT_BEFORE_SLEEP = Config::getInstance().getInt(
      "NetworkServerLoopGameLoadingCap",
  ...
  int sleepMillis = Config::getInstance().getInt(
      "NetworkServerLoopGameLoadingCapSleepMillis", "10");
    ...
    for (int index = 0;
      ...
      if (connectionSlot != NULL && connectionSlot->isConnected() == true) {
        if (connectionSlot->isReady() == false) {
          ...
          bool discarded =
              shouldDiscardNetworkMessage(networkMessageType, connectionSlot);
          if (discarded == false) {
            ...
            if (networkMessageType == nmtReady &&
              ...
              if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                ...
                SystemFlags::OutputDebug(
                    SystemFlags::debugNetwork,
                    "In [%s] networkMessageType==nmtReady\n", __FUNCTION__);
      ...
      if (chrono.getMillis() > readyWaitTimeout) {
        ...
        for (unsigned int langIndex = 0; langIndex < languageList.size();
          ...
          if (lang.hasString("TimeoutWaitingForClients") == true) {
            sErr = lang.getString("TimeoutWaitingForClients",
        ...
        if (chrono.getMillis() - lastStatusUpdate > 200) {
          ...
          string waitForHosts = "";
          for (int hostIndex = 0; hostIndex < (int)waitingForHosts.size();
               hostIndex++) {
            if (waitForHosts != "") {
              waitForHosts += ", ";
            }
          ...
          string updateTextFormat =
              lang.getString("NetworkGameServerLoadStatus");
          if (updateTextFormat == "" || updateTextFormat[0] == '?') {

            updateTextFormat = "Waiting for network: %lld seconds elapsed "
                               "(maximum wait time: %d seconds)";
          }
          snprintf(szBuf, 8096, updateTextFormat.c_str(),
                   (long long int)(chrono.getMillis() / 1000),
          ...
          for (int slotIndex = 0;
               exitServer == false && slotIndex < GameConstants::maxPlayers;
               ++slotIndex) {
            MutexSafeWrapper safeMutexSlot(slotAccessorMutexes[slotIndex],
                                           CODE_AT_LINE_X(slotIndex));
          ...
          for (int slotIndex = 0;
            ...
            if (connectionSlot != NULL &&
                connectionSlot->isConnected() == true) {
              NetworkMessageLoadingStatus networkMessageLoadingStatus(
                  loadingStatus);
  ...
  if (logger.getCancelLoading() == true) {
    ...
    for (unsigned int langIndex = 0; langIndex < languageList.size();
      ...
      string sErr =
          lang.getString("GameCancelledByUser", languageList[langIndex]);
      bool localEcho = lang.isLanguageLocal(languageList[langIndex]);
      sendTextMessage(sErr, -1, localEcho, languageList[langIndex]);
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s] PART B (telling client we are ready!\n",
                             __FUNCTION__);
  try {
    // send ready message after, so clients start delayed
    for (int slotIndex = 0;
...
void ServerInterface::processBroadCastMessageQueue() {
  MutexSafeWrapper safeMutexSlot(broadcastMessageQueueThreadAccessor,
                                 CODE_AT_LINE);
  if (broadcastMessageQueue.empty() == false) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] broadcastMessageQueue.size() = %d\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, broadcastMessageQueue.size());
    for (int index = 0; index < (int)broadcastMessageQueue.size(); ++index) {
      pair<NetworkMessage *, int> &item = broadcastMessageQueue[index];
      if (item.first != NULL) {
...
void ServerInterface::queueBroadcastMessage(NetworkMessage *networkMessage,
                                            int excludeSlot) {
  MutexSafeWrapper safeMutexSlot(broadcastMessageQueueThreadAccessor,
                                 CODE_AT_LINE);
...
void ServerInterface::processTextMessageQueue() {
  ...
  if (textMessageQueue.empty() == false) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugNetwork,
          "In [%s::%s Line: %d] textMessageQueue.size() = %d\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, textMessageQueue.size());
...
void ServerInterface::sendTextMessage(const string &text, int teamIndex,
                                      bool echoLocal, string targetLanguage,
                                      int lockedSlotIndex) {
  // printf("Line: %d text [%s] echoLocal =
  // %d\n",__LINE__,text.c_str(),echoLocal);

  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s Line: %d] text [%s] teamIndex = %d, "
                             "echoLocal = %d, lockedSlotIndex = %d\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, text.c_str(), teamIndex,
                             echoLocal, lockedSlotIndex);
  ...
  if (echoLocal == true) {
    ...
    ChatMsgInfo msg(text.c_str(), teamIndex,
...
void ServerInterface::sendMarkCellMessage(Vec2i targetPos, int factionIndex,
  ...
  for (int index = 0; exitServer == false && index < GameConstants::maxPlayers;
    ...
    if (connectionSlot != NULL) {
      if (connectionSlot->isConnected()) {
        ...
        double lastClientCommandListTimeLag =
            difftime((long int)time(NULL),
                     connectionSlot->getLastReceiveCommandListTime());
        // float pingTime =
        // connectionSlot->getThreadedPingMS(connectionSlot->getIpAddress().c_str());
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, ", lag = %d [%.2f]", clientLagCount,
                 lastClientCommandListTimeLag);
        str += connectionSlot->getName() + " [" + connectionSlot->getUUID() +
               "] " + string(szBuf);
      }
    } else {
...
bool ServerInterface::launchGame(const GameSettings *gameSettings) {
    ...
    if (connectionSlot != NULL &&
      ...
      if (connectionSlot->getNetworkGameDataSynchCheckOk() == false) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "In [%s::%s Line: %d] map [%d] tile [%d] techtree [%d]\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, connectionSlot->getNetworkGameDataSynchCheckOkMap(),
              connectionSlot->getNetworkGameDataSynchCheckOkTile(),
              connectionSlot->getNetworkGameDataSynchCheckOkTech());
        if (SystemFlags::VERBOSE_MODE_ENABLED)
          printf("In [%s::%s Line: %d] map [%d] tile [%d] techtree [%d]\n",
                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                 __LINE__, connectionSlot->getNetworkGameDataSynchCheckOkMap(),
                 connectionSlot->getNetworkGameDataSynchCheckOkTile(),
                 connectionSlot->getNetworkGameDataSynchCheckOkTech());
  ...
  if (bOkToStart == true) {
    ...
    bool useInGameBlockingClientSockets =
        Config::getInstance().getBool("EnableInGameBlockingSockets", "true");
    ...
    for (int startIndex = 0; startIndex < GameConstants::maxPlayers;
         ++startIndex) {

      int factionIndex =
          gameSettings->getFactionIndexForStartLocation(startIndex);
      MutexSafeWrapper safeMutexSlot(slotAccessorMutexes[startIndex],
                                     CODE_AT_LINE_X(startIndex));
      ...
      if ((connectionSlot == NULL || connectionSlot->isConnected() == false) &&
        ...
        if (gameSettings->getFactionControl(factionIndex) != ctClosed &&
          ...
          // printf("Opening slot for in game connections for slot: %d, faction:
          // %d\n",i,factionIndex);
          if (connectionSlot == NULL) {
            addSlot(startIndex);
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "In [%s::%s Line: %d] ftpServer = %p\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__, ftpServer);
    ...
    if (difftime((long int)time(NULL), lastListenerSlotCheckTime) >= 7) {
      ...
      for (int startIndex = 0; startIndex < GameConstants::maxPlayers;
        ...
        if (gameSettings.getFactionControl(factionIndex) != ctClosed &&
          ...
          // Open slots for joining in progress game
          if (connectionSlot == NULL) {
            printf("Opening slot for in game connections, slot: %d, "
                   "factionindex: %d name: %s\n",
                   startIndex, factionIndex,
                   gameSettings.getFactionTypeName(factionIndex).c_str());
            ...
            printf("Removing slot for in game connections, slot: %d, "
                   "factionindex: %d name: %s\n",
                   startIndex, factionIndex,
                   gameSettings.getFactionTypeName(factionIndex).c_str());
...
void ServerInterface::broadcastGameSetup(GameSettings *gameSettingsBuffer,
                                         bool setGameSettingsBuffer) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugNetwork, "In [%s::%s] Line: %d\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
  ...
  for (unsigned int factionIndex = 0;
       factionIndex < (unsigned int)gameSettingsBuffer->getFactionCount();
       ++factionIndex) {
    ...
    if (gameSettingsBuffer->getFactionControl(factionIndex) == ctNetwork &&
      ...
      gameSettingsBuffer->setNetworkPlayerName(
          factionIndex, GameConstants::NETWORK_SLOT_UNCONNECTED_SLOTNAME);
    }
  }
  ...
  NetworkMessageLaunch networkMessageLaunch(gameSettingsBuffer,
                                            nmtBroadCastSetup);
  broadcastMessage(&networkMessageLaunch);
...
void ServerInterface::broadcastMessage(NetworkMessage *networkMessage,
                                       int excludeSlot, int lockedSlotIndex) {
  try {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "In [%s::%s Line: %d]\n",
    ...
    for (int slotIndex = 0;
      ...
      if (slotIndex != lockedSlotIndex) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "In [%s::%s Line: %d] i = %d, lockedSlotIndex = %d\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, slotIndex, lockedSlotIndex);
        safeMutexSlot.setMutex(slotAccessorMutexes[slotIndex],
                               CODE_AT_LINE_X(slotIndex));
      }
      ...
      if (slotIndex != excludeSlot && connectionSlot != NULL) {
        if (connectionSlot->isConnected()) {
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] before sendMessage\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__);
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] after sendMessage\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__);
        }
        if (gameHasBeenInitiated == true &&
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] #1 before removeSlot for slot# %d\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, slotIndex);
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugNetwork,
                "In [%s::%s Line: %d] #1 after removeSlot for slot# %d\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, slotIndex);
        }
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "In [%s::%s Line: %d] #2 before removeSlot for slot# %d\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, slotIndex);
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugNetwork,
              "In [%s::%s Line: %d] #2 after removeSlot for slot# %d\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, slotIndex);
      }
    }
...
void ServerInterface::updateListen() {
  ...
  for (int slotIndex = 0;
    ...
    bool isSlotOpen =
        (slots[slotIndex] != NULL && slots[slotIndex]->isConnected() == false);
    if (isSlotOpen == true) {
      ++openSlotCount;
    }
  }
...
int ServerInterface::getGameSettingsUpdateCount() {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s] START gameSettingsUpdateCount = %d\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, gameSettingsUpdateCount);
...
void ServerInterface::validateGameSettings(GameSettings *serverGameSettings) {
  ...
  // printf("Trying to set map to [%s]. Current map is
  ...
  if (find(playerSortedMaps[playerIndex].begin(),
           playerSortedMaps[playerIndex].end(),
           mapFile) == playerSortedMaps[playerIndex].end()) {
  ...
  if (find(tilesetFiles.begin(), tilesetFiles.end(), tilesetFile) ==
    ...
    printf("Reverting tileset from [%s] to [%s]\n",
           serverGameSettings->getTileset().c_str(),
  ...
  if (find(techTreeFiles.begin(), techTreeFiles.end(), techtreeFile) ==
    ...
    printf("Reverting tech from [%s] to [%s]\n",
           serverGameSettings->getTech().c_str(),
...
void ServerInterface::setGameSettings(GameSettings *serverGameSettings,
                                      bool waitForClientAck) {
  MutexSafeWrapper safeMutex(serverSynchAccessor, CODE_AT_LINE);
  if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                             "In [%s::%s] START gameSettingsUpdateCount = %d, "
  ...
  if (serverGameSettings->getScenario() == "") {
    ...
    if (find(mapFiles.begin(), mapFiles.end(), mapFile) == mapFiles.end()) {
      printf("Reverting map from [%s] to [%s]\n",
             serverGameSettings->getMap().c_str(),
  ...
  if (getAllowGameDataSynchCheck() == true) {
    if (waitForClientAck == true && gameSettingsUpdateCount > 0) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s] Waiting for client acks #1\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__);

      time_t tStart = time(NULL);
      bool gotAckFromAllClients = false;
      while (gotAckFromAllClients == false &&
             difftime((long int)time(NULL), tStart) <= 5) {
    ...
    if (waitForClientAck == true) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                 "In [%s::%s] Waiting for client acks #2\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__);
      ...
      while (gotAckFromAllClients == false &&
        ...
        for (int slotIndex = 0;
             exitServer == false && slotIndex < GameConstants::maxPlayers;
             ++slotIndex) {
          // printf("===> START slot %d - About to setGameSettings
          // 2\n",slotIndex);
          ...
          // printf("===> END slot %d - About to setGameSettings
          // 2\n",slotIndex);
        }
      }
    }
...
std::map<string, string> ServerInterface::publishToMasterserver() {
  ...
  if (gameStats != NULL) {
    for (int factionIndex = 0; factionIndex < gameStats->getFactionCount();
         ++factionIndex) {
...
std::map<string, string> ServerInterface::publishToMasterserverStats() {
  ...
  if (gameStats != NULL) {
    publishToServerInfo["gameUUID"] = this->getGameSettings()->getGameUUID();
    publishToServerInfo["tech"] = this->getGameSettings()->getTech();
    publishToServerInfo["factionCount"] =
        intToStr(gameStats->getFactionCount());
    publishToServerInfo["framesPlayed"] =
        intToStr(gameStats->getFramesPlayed());
    publishToServerInfo["framesToCalculatePlaytime"] =
        intToStr(gameStats->getFramesToCalculatePlaytime());
    publishToServerInfo["maxConcurrentUnitCount"] =
        intToStr(gameStats->getMaxConcurrentUnitCount());
    publishToServerInfo["totalEndGameConcurrentUnitCount"] =
        intToStr(gameStats->getTotalEndGameConcurrentUnitCount());
    publishToServerInfo["isHeadlessServer"] =
        intToStr(gameStats->getIsMasterserverMode());

    for (int factionIndex = 0; factionIndex < gameStats->getFactionCount();
         ++factionIndex) {
      publishToServerInfo["factionIndex_" + intToStr(factionIndex)] =
          intToStr(factionIndex);
      publishToServerInfo["controlType_" + intToStr(factionIndex)] =
          intToStr(gameStats->getControl(factionIndex));
      publishToServerInfo["resourceMultiplier_" + intToStr(factionIndex)] =
          floatToStr(gameStats->getResourceMultiplier(factionIndex));
      publishToServerInfo["factionTypeName_" + intToStr(factionIndex)] =
          gameStats->getFactionTypeName(factionIndex);
      publishToServerInfo["personalityType_" + intToStr(factionIndex)] =
          intToStr(gameStats->getPersonalityType(factionIndex));
      publishToServerInfo["teamIndex_" + intToStr(factionIndex)] =
          intToStr(gameStats->getTeam(factionIndex));
      publishToServerInfo["wonGame_" + intToStr(factionIndex)] =
          intToStr(gameStats->getVictory(factionIndex));
      publishToServerInfo["killCount_" + intToStr(factionIndex)] =
          intToStr(gameStats->getKills(factionIndex));
      publishToServerInfo["enemyKillCount_" + intToStr(factionIndex)] =
          intToStr(gameStats->getEnemyKills(factionIndex));
      publishToServerInfo["deathCount_" + intToStr(factionIndex)] =
          intToStr(gameStats->getDeaths(factionIndex));
      publishToServerInfo["unitsProducedCount_" + intToStr(factionIndex)] =
          intToStr(gameStats->getUnitsProduced(factionIndex));
      publishToServerInfo["resourceHarvestedCount_" + intToStr(factionIndex)] =
          intToStr(gameStats->getResourcesHarvested(factionIndex));
      publishToServerInfo["playerName_" + intToStr(factionIndex)] =
          gameStats->getPlayerName(factionIndex);
      publishToServerInfo["quitBeforeGameEnd_" + intToStr(factionIndex)] =
          intToStr(gameStats->getPlayerLeftBeforeEnd(factionIndex));
      publishToServerInfo["quitTime_" + intToStr(factionIndex)] =
          intToStr(gameStats->getTimePlayerLeft(factionIndex));
      publishToServerInfo["playerUUID_" + intToStr(factionIndex)] =
          this->getGameSettings()->getNetworkPlayerUUID(factionIndex);
      publishToServerInfo["platform_" + intToStr(factionIndex)] =
          this->getGameSettings()->getNetworkPlayerPlatform(factionIndex);
    }
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "In [%s::%s Line %d]\n",
...
void ServerInterface::simpleTask(BaseThread *callingThread, void *userdata) {
  ...
  if (difftime((long int)time(NULL), lastMasterserverHeartbeatTime) >=
      MASTERSERVER_HEARTBEAT_GAME_STATUS_SECONDS) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                               "In [%s::%s Line %d]\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("Checking to see masterserver needs an update of the game status "
             "[%d] callingThread [%p] publishToMasterserverThread [%p]\n",
             needToRepublishToMasterserver, callingThread,
    ...
    if (needToRepublishToMasterserver == true) {
      try {
        if (Config::getInstance().getString("Masterserver", "") != "") {
          ...
          std::map<string, string> newPublishToServerInfoStats =
              publishToMasterserverStats();
          if (newPublishToServerInfoStats.empty() == false) {
            for (std::map<string, string>::const_iterator iterMap =
                     newPublishToServerInfoStats.begin();
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
              ...
              SystemFlags::OutputDebug(
                  ...
                  "In [%s::%s Line %d] the request is:\n%s\n",
                  extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                  __LINE__, requestStats.c_str());
            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf("Calling masterserver [%s]...\n", requestStats.c_str());

            std::string serverInfoStats =
                SystemFlags::getHTTP(requestStats, handle);
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
              ...
              SystemFlags::OutputDebug(
                  ...
                  extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                  __LINE__, serverInfoStats.c_str());
          }

          SystemFlags::cleanupHTTP(&handle);

          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("Done Calling masterserver\n");
        ...
        SystemFlags::OutputDebug(
            SystemFlags::debugError,
            "In [%s::%s Line %d] error during game status update: [%s]\n",
            extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
            __LINE__, ex.what());
      }
    }
...
std::string ServerInterface::DumpStatsToLog(bool dumpToStringOnly) const {
  string headlessLogFile =
      Config::getInstance().getString("HeadlessLogFile", "headless.log");
  if (getGameReadWritePath(GameConstants::path_logs_CacheLookupKey) != "") {
    headlessLogFile =
        getGameReadWritePath(GameConstants::path_logs_CacheLookupKey) +
        headlessLogFile;
  ...
  for (int slotIndex = 0;
    ...
    if (slot != NULL) {
      ...
      out << "Handshake received: "
          << boolToStr(slot->getConnectHasHandshaked()) << std::endl;
      if (slot->isConnected() == true) {
        ...
        int InSeconds =
            difftime((long int)time(NULL), slot->getConnectedTime());
        // compute seconds
        ...
        out << "Connection duration: " << hours << " hours " << minutes
...
void ServerInterface::notifyBadClientConnectAttempt(string ipAddress) {
  // printf("In [%s::%s Line: %d] ipAddress
  ...
  pair<uint64, time_t> &lastBadConnectionAttempt =
      badClientConnectIPList[ipAddress];

  const uint64 BLOCK_BAD_CLIENT_CONNECT_MAX_SECONDS =
      Config::getInstance().getInt("BlockBadClientConnectMaxSeconds", "60");
  const uint64 BLOCK_BAD_CLIENT_CONNECT_MAX_ATTEMPTS =
      Config::getInstance().getInt("BlockBadClientConnectMaxAttempts", "6");
  bool addToBlockedClientsList = false;

  if (difftime((long int)time(NULL), lastBadConnectionAttempt.second) <=
      BLOCK_BAD_CLIENT_CONNECT_MAX_SECONDS) {

    if (lastBadConnectionAttempt.first + 1 >
        BLOCK_BAD_CLIENT_CONNECT_MAX_ATTEMPTS) {
...
bool ServerInterface::getStartInGameConnectionLaunch() {
  ...
  for (int slotIndex = 0;
       exitServer == false && slotIndex < GameConstants::maxPlayers;
       ++slotIndex) {
    MutexSafeWrapper safeMutex(slotAccessorMutexes[slotIndex],
                               CODE_AT_LINE_X(slotIndex));
    if (slots[slotIndex] != NULL) {
...
ConnectionSlot *ServerInterface::findSlotForUUID(string uuid,
                                                 bool unConnectedOnly) {
      ...
      if (connectionSlot != NULL) {
        if (connectionSlot->getUUID() == uuid) {
          if (unConnectedOnly == false ||
            ...
            if (result == NULL || (result->getConnectedTime() >
                                   connectionSlot->getConnectedTime()))
...
void ServerInterface::saveGame(XmlNode *rootNode) {
  ...
  for (int slotIndex = 0;
    ...
    if (slots[slotIndex] != NULL) {
      ...
      ConnectionSlot *slot = slots[slotIndex];
      if (slot != NULL) {
        slotNode->addAttribute("isconnected", intToStr(slot->isConnected()),
                               mapTagReplacements);
        slotNode->addAttribute("sessionkey", intToStr(slot->getSessionKey()),
                               mapTagReplacements);
        slotNode->addAttribute("ipaddress",
                               slot->getSocket(false)->getIpAddress(),
                               mapTagReplacements);
        slotNode->addAttribute("name", slot->getName(), mapTagReplacements);
        slotNode->addAttribute("uuid", slot->getUUID(), mapTagReplacements);
      } else {
        slotNode->addAttribute("isconnected", intToStr(false),
                               mapTagReplacements);
      }
    }
  }

...
#pragma pack(push, 1)
template <int S> class NetworkString {
  private:
    char buffer[S];
...
class NetworkCommand {
  public:
    ...
    NetworkCommand(World *world, int networkCommandType, int unitId, int commandTypeId = -1, const Vec2i &pos = Vec2i(0), int unitTypeId = -1,
                   int targetId = -1, int facing = -1, bool wantQueue = false, CommandStateType commandStateType = cst_None, int commandTypeStateValue = -1,

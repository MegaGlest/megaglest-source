    ...
    if (socketInfo.second == NULL) {
      ...
      if (socketInfo.first == true) {
              ...
              if (gotIntro == true) {
                ...
                if (receiveMessage(&networkMessageCommandList)) {
                  ...
                  MutexSafeWrapper safeMutexSlot(mutexPendingNetworkCommandList,
                                                 CODE_AT_LINE);
                  for (int i = 0;
                       i < networkMessageCommandList.getCommandCount(); ++i) {
              ...
              if (receiveMessage(&networkMessageIntro)) {
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork)
                  ...
                  SystemFlags::OutputDebug(
                      ...
                      versionString.c_str(), msgSessionId);
                ...
                if (msgSessionId != sessionKey) {
                  ...
                  if (compatible == false) {
                    ...
                    if (strncmp(platformFreeVersion.c_str(),
                      ...
                      sErr =
                          ...
                          getNetworkVersionGITString() + "\nClient: " +
                          networkMessageIntro.getVersionString() + " player [" +
                          playerNameStr + "]";
                      printf("%s\n", sErr.c_str());
                      ...
                      string playerNameStr = name;
                      sErr =
                          "Warning, Server and client are using the same "
                    ...
                    if (Config::getInstance().getBool(
                            "PlatformConsistencyChecks", "true") &&
                        versionMatched ==
                            false) {  // error message and disconnect only if
                                      // checked
                      if (SystemFlags::getSystemSettingType(
                              SystemFlags::debugNetwork)
                              .enabled)
                  ...
                  if (serverInterface->getGameHasBeenInitiated() == true &&
                      serverInterface->getAllowInGameConnections() == true) {
                    ConnectionSlot *slot = serverInterface->findSlotForUUID(
                ...
                if (receiveMessage(&networkMessageLaunch, networkMessageType)) {
                  ...
                  if (this->joinGameInProgress == false &&
                    ...
                    for (unsigned int index = 0; index < languageList.size();
                      ...
                      char szBuf[4096] = "";
                      string msgTemplate =
                          "You must have have at least %d player(s) connected "
                          "to start this game!";
                      ...
                      string sMsg = szBuf;
                      bool echoLocal =
                          lang.isLanguageLocal(languageList[index]);
                    ...
                    if (this->joinGameInProgress == false) {
                      ...
                      networkMessageLaunch.buildGameSettings(
                          &gameSettingsBuffer);
                    ...
                    if (this->joinGameInProgress == false &&
                      ...
                      this->serverInterface->gameSettings.setNetworkPlayerUUID(
                          factionIndex, this->playerUUID);
                      this->serverInterface->gameSettings
                          .setNetworkPlayerPlatform(factionIndex,
                                                    this->platform);
                  ...
                  if (networkGameDataSynchCheckOkMap == true &&
                    ...
                    if (SystemFlags::getSystemSettingType(
                      ...
                      SystemFlags::OutputDebug(
                          ...
                          __FILE__, __FUNCTION__, tilesetCRC,
                          networkMessageSynchNetworkGameDataStatus
                              .getTilesetCRC());
                    if (SystemFlags::getSystemSettingType(
                            SystemFlags::debugNetwork)
                    ...
                    if (allowDownloadDataSynch == true) {
                      ...
                      Config &config = Config::getInstance();
                      string scenarioDir = "";
                      if (serverInterface->getGameSettings()
                              ->getScenarioDir() != "") {
                        scenarioDir = serverInterface->getGameSettings()
                                          ->getScenarioDir();
                        if (EndsWith(scenarioDir, ".xml") == true) {
                          scenarioDir =
                              scenarioDir.erase(scenarioDir.size() - 4, 4);
            ...
            case nmtSwitchSetupRequest: {
              ...
              if (gotIntro == true) {
                ...
                if (receiveMessage(&switchSetupRequest)) {
                  ...
                  if (serverInterface->getSwitchSetupRequests(slotIdx) ==
                      NULL) {
            ...
            case nmtReady: {
              ...
              if (joinGameInProgress == true) {
                ...
                this->currentFrameCount =
                    serverInterface->getCurrentFrameCount();
                // printf("#2 Server slot got currentFrameCount =
                // %d\n",currentFrameCount);
              ...
              if (this->isConnected() == true && this->gotIntro == true &&
                ...
                if ((clientLagCount > maxFrameCountLagAllowed) ||
                  ...
                  if (waitedForLaggingClient == false) {
                    ...
                    printf(
                        "*TESTING*: START Waiting for lagging client "
        ...
        if (waitedForLaggingClient == true) {
          printf(
              "*TESTING*: FINISHED Waiting for lagging client playerIndex = %d "
              "[%s]\n",
...
vector<NetworkCommand> ConnectionSlot::getPendingNetworkCommandList(
    bool clearList) {

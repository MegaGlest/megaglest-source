...
void ConnectionSlotThread::execute() {
    ...
    try {
        ...
        for (; this->slotInterface != NULL;) {
            ...
            if (this->slotInterface->getAllowInGameConnections() == true && this->slotInterface->isClientConnected(slotIndex) == false) {
...
ConnectionSlot::~ConnectionSlot() {
    ...
    if (slotThreadWorker != NULL && slotThreadWorker->canShutdown(false) == true && slotThreadWorker->getRunningStatus() == false) {
...
void ConnectionSlot::updateSlot(ConnectionSlotEvent *event) {
    if (event != NULL) {
        ...
        bool checkForNewClients = (serverInterface->getGameHasBeenInitiated() == false || serverInterface->getAllowInGameConnections() == true);
        ...
        if (socketTriggered == true ||
            ((serverInterface->getGameHasBeenInitiated() == false || serverInterface->getAllowInGameConnections() == true) && this->isConnected() == false)) {
    ...
    try {
        ...
        if (socketInfo.second == NULL) {
            ...
            if (checkForNewClients == true && this->canAcceptConnections == true) {
                ...
                if (hasData == true) {
                    ...
                    if (this->isConnected() == true) {
                        ...
                        NetworkMessageIntro networkMessageIntro(sessionKey, getNetworkVersionGITString(), getHostName(), playerIndex, nmgstOk, 0,
                                                                ServerSocket::getFTPServerPort(), "", serverInterface->getGameHasBeenInitiated(),
                                                                Config::getInstance().getString("PlayerId", ""), getPlatformNameString());
            ...
            if (socketInfo.first == true) {
                ...
                for (; waitForLaggingClient == true || (this->hasDataToRead() == true && (gotTextMsg == true || gotCellMarkerMsg == true));) {
                        ...
                        if (receiveMessage(&networkMessagePing)) {
                            ...
                            if (SystemFlags::getSystemSettingType(SystemFlags::debugError).enabled)
                                SystemFlags::OutputDebug(SystemFlags::debugError,
                                                         ...
                                                         "[%s].\n",
                                                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, networkMessageType,
                    ...
                    case nmtText: {
                        ...
                        if (gotIntro == true) {
                            ...
                            if (receiveMessage(&networkMessageText)) {
                                ...
                                if (SystemFlags::getSystemSettingType(SystemFlags::debugError).enabled)
                                    SystemFlags::OutputDebug(SystemFlags::debugError,
                                                             "In [%s::%s Line: %d]\nInvalid message type before "
                                                             "intro handshake [%d]\nDisconnecting socket for slot: "
                                                             "%d [%s].\n",
                                                             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, networkMessageType,
                                                             this->playerIndex, this->getIpAddress().c_str());
                                this->serverInterface->notifyBadClientConnectAttempt(this->getIpAddress());
                    ...
                    case nmtMarkCell: {
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] got nmtMarkCell gotIntro = %d\n", __FILE__, __FUNCTION__,
                                                     __LINE__, gotIntro);

                        if (gotIntro == true) {
                            NetworkMessageMarkCell networkMessageMarkCell;
                            if (receiveMessage(&networkMessageMarkCell)) {
                                MarkedCell msg(networkMessageMarkCell.getTarget(), networkMessageMarkCell.getFactionIndex(),
                    ...
                    case nmtUnMarkCell: {
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] got nmtUnMarkCell gotIntro = %d\n", __FILE__,
                    ...
                    case nmtIntro: {
                        ...
                        if (receiveMessage(&networkMessageIntro)) {
                            ...
                            if (msgSessionId != sessionKey) {
                                ...
                                string sErr = "Client gave invalid sessionid for player [" + playerNameStr + "] actual [" + intToStr(msgSessionId) +
                                              "] expected [" + intToStr(sessionKey) + "]";
                                printf("%s\n", sErr.c_str());
                                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                                ...
                                if (compatible == false) {
                                    ...
                                    if (strncmp(platformFreeVersion.c_str(), networkMessageIntro.getVersionString().c_str(),
                                                strlen(platformFreeVersion.c_str())) != 0) {
                                        string playerNameStr = name;
                                        sErr = "Server and client binary mismatch!\nYou have to use "
                                               "the exactly same binaries!\n\nServer: " +
                                               getNetworkVersionGITString() + "\nClient: " + networkMessageIntro.getVersionString() + " player [" +
                                               playerNameStr + "]";
                                        printf("%s\n", sErr.c_str());
                                    ...
                                    if (Config::getInstance().getBool("PlatformConsistencyChecks", "true") &&
                                        versionMatched == false) { // error message and disconnect only if
                                                                   // checked
                                        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                                            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] %s\n", __FILE__, __FUNCTION__, __LINE__,
                                                                     sErr.c_str());
                                ...
                                this->serverInterface->addClientToServerIPAddress(this->getSocket()->getConnectedIPAddress(this->getSocket()->getIpAddress()),
                                ...
                                if (serverInterface->getGameHasBeenInitiated() == true && serverInterface->getAllowInGameConnections() == true) {
                    ...
                    case nmtBroadCastSetup: {
                        if (gotIntro == true) {
                            ...
                            if (receiveMessage(&networkMessageLaunch, networkMessageType)) {
                                ...
                                if (this->joinGameInProgress == false && networkMessageLaunch.getMessageType() == nmtLaunch && this->ready == false &&
                                    ...
                                    for (unsigned int index = 0; index < languageList.size(); ++index) {
                                        char szBuf[4096] = "";
                                        string msgTemplate = "You must have have at least %d player(s) connected "
                                                             "to start this game!";
                            ...
                            if (receiveMessage(&networkMessageSynchNetworkGameDataStatus)) {
                                ...
                                if (serverInterface->getGameSettings()->getScenarioDir() != "") {
                                    ...
                                    if (EndsWith(scenarioDir, ".xml") == true) {
                                        ...
                                        scenarioDir = scenarioDir.erase(scenarioDir.size() - serverInterface->getGameSettings()->getScenario().size(),
                                                                        serverInterface->getGameSettings()->getScenario().size() + 1);
                                    }

                                    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                                        SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                                                 "In [%s::%s Line: %d] gameSettings.getScenarioDir() "
                                                                 "= [%s] gameSettings.getScenario() = [%s] "
                                                                 "scenarioDir = [%s]\n",
                                                                 __FILE__, __FUNCTION__, __LINE__, serverInterface->getGameSettings()->getScenarioDir().c_str(),
                                                                 serverInterface->getGameSettings()->getScenario().c_str(), scenarioDir.c_str());
                                }

                                // tileset
                                uint32 tilesetCRC = getFolderTreeContentsCheckSumRecursively(
                                    config.getPathListForType(ptTilesets, scenarioDir),
                                    string("/") + serverInterface->getGameSettings()->getTileset() + string("/*"), ".xml", NULL);
                                uint32 techCRC = getFolderTreeContentsCheckSumRecursively(
                                    config.getPathListForType(ptTechs, scenarioDir), "/" + serverInterface->getGameSettings()->getTech() + "/*", ".xml", NULL);
                                Checksum checksum;
                                ...
                                if (networkGameDataSynchCheckOkMap == true && networkGameDataSynchCheckOkTile == true &&
                                    ...
                                    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                                        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s] mapCRC = %d, remote = %d\n", __FILE__, __FUNCTION__,
                                                                 mapCRC, networkMessageSynchNetworkGameDataStatus.getMapCRC());
                                    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                                        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s] tilesetCRC = %d, remote = %d\n", __FILE__,
                                                                 __FUNCTION__, tilesetCRC, networkMessageSynchNetworkGameDataStatus.getTilesetCRC());
                                    if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                                        SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s] techCRC = %d, remote = %d\n", __FILE__, __FUNCTION__,
                                                                 techCRC, networkMessageSynchNetworkGameDataStatus.getTechCRC());
                                    ...
                                    if (allowDownloadDataSynch == true) {
                                        ...
                                        if (serverInterface->getGameSettings()->getScenarioDir() != "") {
                                            ...
                                            if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                                                SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                                                         ...
                                                                         "[%s]\n",
                                                                         __FILE__, __FUNCTION__, __LINE__,
                                                                         serverInterface->getGameSettings()->getScenarioDir().c_str(),
                                                                         serverInterface->getGameSettings()->getScenario().c_str(), scenarioDir.c_str());
                                        }
                                        ...
                                        if (networkGameDataSynchCheckOkTile == false) {
                                            if (tilesetCRC == 0) {
                                                vctFileList = getFolderTreeContentsCheckSumListRecursively(
                                                    config.getPathListForType(ptTilesets, scenarioDir),
                                                    string("/") + serverInterface->getGameSettings()->getTileset() + string("/*"), "", &vctFileList);
                                            } else {
                                                vctFileList = getFolderTreeContentsCheckSumListRecursively(
                                                    config.getPathListForType(ptTilesets, scenarioDir),
                                                    "/" + serverInterface->getGameSettings()->getTileset() + "/*", ".xml", &vctFileList);
                                            }
                                        }
                                        if (networkGameDataSynchCheckOkTech == false) {
                                            if (techCRC == 0) {
                                                vctFileList = getFolderTreeContentsCheckSumListRecursively(
                                                    config.getPathListForType(ptTechs, scenarioDir), "/" + serverInterface->getGameSettings()->getTech() + "/*",
                                                    "", &vctFileList);
                                            } else {
                                                vctFileList = getFolderTreeContentsCheckSumListRecursively(
                                                    config.getPathListForType(ptTechs, scenarioDir), "/" + serverInterface->getGameSettings()->getTech() + "/*",
                                            ...
                                            string report = networkMessageSynchNetworkGameDataStatus.getTechCRCFileMismatchReport(
                                                serverInterface->getGameSettings()->getTech(), vctFileList);
                                        ...
                                        if (networkGameDataSynchCheckOkMap == false) {
                                            vctFileList.push_back(std::pair<string, uint32>(
                                                Config::getMapPath(serverInterface->getGameSettings()->getMap(), scenarioDir, false), mapCRC));
                                        }

                                        NetworkMessageSynchNetworkGameDataFileCRCCheck networkMessageSynchNetworkGameDataFileCRCCheck(
                                            (int)vctFileList.size(), 1, vctFileList[0].second, vctFileList[0].first);
                    ...
                    case nmtSynchNetworkGameDataFileCRCCheck: {
                        ...
                        if (gotIntro == true) {
                            ...
                            if (receiveMessage(&networkMessageSynchNetworkGameDataFileCRCCheck)) {
                                ...
                                NetworkMessageSynchNetworkGameDataFileCRCCheck networkMessageSynchNetworkGameDataFileCRCCheck(
                                    (int)vctFileList.size(), fileIndex, vctFileList[fileIndex - 1].second, vctFileList[fileIndex - 1].first);
                                sendMessage(&networkMessageSynchNetworkGameDataFileCRCCheck);
                            } else {
                                if (SystemFlags::getSystemSettingType(SystemFlags::debugError).enabled)
                                    SystemFlags::OutputDebug(SystemFlags::debugError,
                    ...
                    case nmtSwitchSetupRequest: {
                        ...
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                            SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                                     "In [%s::%s Line: %d] got nmtSwitchSetupRequest gotIntro = "
                                                     "%d\n",
                                                     __FILE__, __FUNCTION__, __LINE__, gotIntro);
                                ...
                                *(serverInterface->getSwitchSetupRequests(slotIdx)) = switchSetupRequest;
                                ...
                                if (SystemFlags::VERBOSE_MODE_ENABLED)
                                    printf("In [%s::%s Line %d] networkPlayerName [%s]\n", __FILE__, __FUNCTION__, __LINE__,
                                           serverInterface->getSwitchSetupRequests()[slotIdx]->getNetworkPlayerName().c_str());
                                if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                                    SystemFlags::OutputDebug(SystemFlags::debugNetwork,
                                                             ...
                                                             __FILE__, __FUNCTION__, __LINE__, slotIdx, switchSetupRequest.getNetworkPlayerName().c_str(),
                                                             switchSetupRequest.getNetworkPlayerStatus(), switchSetupRequest.getSwitchFlags());
                            } else {
                                if (SystemFlags::getSystemSettingType(SystemFlags::debugError).enabled)
                                    SystemFlags::OutputDebug(SystemFlags::debugError,
                                                             "In [%s::%s Line: %d]\nInvalid message type before "
                    ...
                    default: {
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugNetwork).enabled)
                            SystemFlags::OutputDebug(SystemFlags::debugNetwork, "In [%s::%s Line: %d] networkMessageType = %d\n", __FILE__, __FUNCTION__,
                                                     __LINE__, networkMessageType);

                        if (gotIntro == true) {
                            // throw megaglest_runtime_error("Unexpected message in
                            // connection slot: " + intToStr(networkMessageType));
                    ...
                    if (Config::getInstance().getBool("AutoClientLagCorrection", "true") == true) {
                        ...
                        if (this->serverInterface->getGameStartTime() > 0 &&
                            ...
                            if (this->isConnected() == true && this->gotIntro == true && this->skipLagCheck == false) {
                                ...
                                // New lag check
                                if ((clientLagCount > maxFrameCountLagAllowed) || (maxClientLagTimeAllowed > 0 && clientLagTime > maxClientLagTimeAllowed)) {
...
bool ConnectionSlot::updateCompleted(ConnectionSlotEvent *event) {
    bool waitingForThread = (slotThreadWorker != NULL && slotThreadWorker->isSignalCompleted(event) == false && slotThreadWorker->getQuitStatus() == false &&
...
void ConnectionSlot::sendMessage(NetworkMessage *networkMessage) {
    ...
    if (textMsg != NULL) {
        ...
        if (textMsg->getTargetLanguage() != "" && textMsg->getTargetLanguage() != this->getNetworkPlayerLanguage()) {
            return;
        }
    }

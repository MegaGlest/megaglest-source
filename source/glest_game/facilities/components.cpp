...
std::map<std::string, std::map<std::string, GraphicComponent *> >
    GraphicComponent::registeredGraphicComponentList;

GraphicComponent::GraphicComponent(const std::string &containerName,
  ...
  if (containerName == "" || objName == "") {
    // char szBuf[8096]="";
    // snprintf(szBuf,8096,"Control not properly registered Container [%s]
    // Control [%s]\n",containerName.c_str(),objName.c_str()); throw
...
void GraphicComponent::clearRegisterGraphicComponent(std::string containerName,
                                                     std::string objName) {
  GraphicComponent *obj = findRegisteredComponent(containerName, objName);
  if (obj) {
    GraphicComponent::registeredGraphicComponentList[containerName].erase(
        objName);
  }
}

void GraphicComponent::clearRegisterGraphicComponent(
    std::string containerName, std::vector<std::string> objNameList) {
  for (int idx = 0; idx < (int)objNameList.size(); ++idx) {
    GraphicComponent::clearRegisterGraphicComponent(containerName,
                                                    objNameList[idx]);
  }
}

void GraphicComponent::registerGraphicComponent(std::string containerName,
                                                std::string objName) {
  // unregistered old name if we have been renamed
  if (this->getInstanceName() != "") {
    // printf("RENAME Register Callback detected calling: Control old [%s] new
    // [%s]\n",this->getInstanceName().c_str(),objName.c_str());
    clearRegisterGraphicComponent(this->containerName, this->getInstanceName());
  } else {
    // printf("NEW Register Callback detected calling: Control container [%s]
  ...
  if (containerName == "" || objName == "") {
    // char szBuf[8096]="";
    // snprintf(szBuf,8096,"Control not properly registered Container [%s]
    // Control [%s]\n",this->containerName.c_str(),objName.c_str()); throw
    // megaglest_runtime_error(szBuf);
  }
  ...
  // registered [%s] [%s] count =
  // %d\n",__FILE__,__FUNCTION__,__LINE__,containerName.c_str(),instanceName.c_str(),registeredGraphicComponentList[containerName].size());
}

void GraphicComponent::registerGraphicComponentOnlyFontCallbacks(
    std::string containerName, std::string objName) {
  if (this->getInstanceName() != "") {
    // printf("(FONT ONLY) RENAME Register Callback detected calling: Control
    // old [%s] new [%s]\n",this->getInstanceName().c_str(),objName.c_str());
    clearRegisterGraphicComponent(this->containerName, this->getInstanceName());
  } else {
    // printf("(FONT ONLY) NEW Register Callback detected calling: Control
    // container [%s] name [%s]\n",containerName.c_str(),objName.c_str());
  }
...
GraphicComponent *GraphicComponent::findRegisteredComponent(
    std::string containerName, std::string objName) {
  GraphicComponent *result = NULL;

  std::map<std::string, std::map<std::string, GraphicComponent *> >::iterator
      iterFind1 =
          GraphicComponent::registeredGraphicComponentList.find(containerName);
  if (iterFind1 != GraphicComponent::registeredGraphicComponentList.end()) {
    std::map<std::string, GraphicComponent *>::iterator iterFind2 =
        iterFind1->second.find(objName);
...
void GraphicComponent::applyAllCustomProperties(std::string containerName) {
  ...
  std::map<std::string, std::map<std::string, GraphicComponent *> >::iterator
      iterFind1 =
          GraphicComponent::registeredGraphicComponentList.find(containerName);
  if (iterFind1 != GraphicComponent::registeredGraphicComponentList.end()) {
    for (std::map<std::string, GraphicComponent *>::iterator iterFind2 =
             iterFind1->second.begin();
...
void GraphicComponent::applyCustomProperties(std::string containerName) {
  if (instanceName != "") {
    ...
    std::map<std::string, std::map<std::string, GraphicComponent *> >::iterator
        iterFind1 = GraphicComponent::registeredGraphicComponentList.find(
            containerName);
    if (iterFind1 != GraphicComponent::registeredGraphicComponentList.end()) {
      std::map<std::string, GraphicComponent *>::iterator iterFind2 =
          iterFind1->second.find(instanceName);
      if (iterFind2 != iterFind1->second.end()) {
        ...
        GraphicComponent *ctl =
            dynamic_cast<GraphicComponent *>(iterFind2->second);

        // First check default overrides
        ctl->x = config.getInt(containerName + "_" + iterFind2->first + "_x",
                               intToStr(ctl->x).c_str());
        ctl->y = config.getInt(containerName + "_" + iterFind2->first + "_y",
                               intToStr(ctl->y).c_str());
        ctl->w = config.getInt(containerName + "_" + iterFind2->first + "_w",
                               intToStr(ctl->w).c_str());
        ctl->h = config.getInt(containerName + "_" + iterFind2->first + "_h",
                               intToStr(ctl->h).c_str());
        ctl->visible =
            config.getBool(containerName + "_" + iterFind2->first + "_visible",
                           boolToStr(ctl->visible).c_str());

        // Now check language specific overrides
        ctl->x = config.getInt(
            containerName + "_" + iterFind2->first + "_x_" + languageToken,
            intToStr(ctl->x).c_str());
        ctl->y = config.getInt(
            containerName + "_" + iterFind2->first + "_y_" + languageToken,
            intToStr(ctl->y).c_str());
        ctl->w = config.getInt(
            containerName + "_" + iterFind2->first + "_w_" + languageToken,
            intToStr(ctl->w).c_str());
        ctl->h = config.getInt(
            containerName + "_" + iterFind2->first + "_h_" + languageToken,
            intToStr(ctl->h).c_str());
        ctl->visible = config.getBool(containerName + "_" + iterFind2->first +
                                          "_visible_" + languageToken,
                                      boolToStr(ctl->visible).c_str());
      }
    }
  }
...
bool GraphicComponent::saveAllCustomProperties(std::string containerName) {
  SystemFlags::OutputDebug(
      ...
      "In [%s::%s Line: %d] registered [%s] count = %d\n", __FILE__,
      __FUNCTION__, __LINE__, containerName.c_str(),
      registeredGraphicComponentList[containerName].size());
  Config &config = Config::getInstance();
...
bool GraphicComponent::saveCustomProperties(std::string containerName) {
  ...
  if (instanceName != "") {
    ...
    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                             "In [%s::%s Line: %d] looking for [%s] [%s]\n",
                             __FILE__, __FUNCTION__, __LINE__,
                             containerName.c_str(), instanceName.c_str());
    ...
    if (iterFind1 != GraphicComponent::registeredGraphicComponentList.end()) {
      ...
      if (iterFind2 != iterFind1->second.end()) {
        SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                 "In [%s::%s Line: %d] FOUND [%s]\n", __FILE__,
                                 __FUNCTION__, __LINE__, instanceName.c_str());
        ...
        savedChange = true;
        // Now check language specific overrides
        // ctl->x = config.getInt(containerName + "_" + iterFind2->first + "_x_"
        // + languageToken, intToStr(ctl->x).c_str()); ctl->y =
        // config.getInt(containerName + "_" + iterFind2->first + "_y_" +
        // languageToken, intToStr(ctl->y).c_str()); ctl->w =
        // config.getInt(containerName + "_" + iterFind2->first + "_w_" +
        // languageToken, intToStr(ctl->w).c_str()); ctl->h =
        // config.getInt(containerName + "_" + iterFind2->first + "_h_" +
        // languageToken, intToStr(ctl->h).c_str());
...
void GraphicComponent::FontChangedCallback(std::string fontUniqueId,
                                           Font *font) {
...
void GraphicComponent::reloadFontsForRegisterGraphicComponents(
...
void GraphicComponent::resetFade() { fade = 0.f; }
...
GraphicLabel::GraphicLabel(const std::string &containerName,
                           const std::string &objName, bool registerControl)
...
bool GraphicLabel::mouseMove(int x, int y) {
  ...
  if (text.length() > 0 && font3D != NULL) {
    float lineWidth = (font3D->getTextHandler()->Advance(text.c_str()) *
  ...
  return x > this->x && y > this->y && x < this->x + useWidth &&
         y < this->y + h;
}
...
GraphicButton::GraphicButton(const std::string &containerName,
                             const std::string &objName, bool registerControl)
    : GraphicComponent(containerName, objName, registerControl) {
  lighted = false;
  alwaysLighted = false;
...
GraphicListBox::GraphicListBox(const std::string &containerName,
                               const std::string &objName)
    : GraphicComponent(containerName, objName),
      graphButton1(containerName, objName + "_button1"),
...
const string &GraphicListBox::getTextNativeTranslation() {
  if (this->translated_items.empty() == true || this->selectedItemIndex < 0 ||
...
void GraphicListBox::setItems(const vector<string> &items,
                              const vector<string> translated_items) {
  this->items = items;
...
void GraphicListBox::setSelectedItemIndex(int index, bool errorOnMissing) {
  if (errorOnMissing == true && (index < 0 || index >= (int)items.size())) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "Index not found in listbox name: [%s] value index: %d size: %lu",
             this->instanceName.c_str(), index, (unsigned long)items.size());
...
void GraphicListBox::setSelectedItem(string item, bool errorOnMissing) {
  ...
  if (iter == items.end()) {
    if (errorOnMissing == true) {
      ...
      char szBuf[8096] = "";
      snprintf(szBuf, 8096, "Value not found in listbox name: [%s] value: %s",
               this->instanceName.c_str(), item.c_str());
      throw megaglest_runtime_error(szBuf);
    }
  } else {
...
bool GraphicListBox::mouseMove(int x, int y) {
  ...
  return graphButton1.mouseMove(x, y) || graphButton2.mouseMove(x, y);
}
    ...
    if (b1) {
      ...
      if (advanceToItemStartingWith != "") {
        for (int i = selectedItemIndex - 1; i >= 0; --i) {
          ...
          if (StartsWith(toLower(item), toLower(advanceToItemStartingWith)) ==
              true) {
            bFound = true;
            selectedItemIndex = i;
            break;
        ...
        if (bFound == false) {
          for (int i = (int)items.size() - 1; i >= selectedItemIndex; --i) {
            string item = items[i];
            if ((int)translated_items.size() > i) item = translated_items[i];
            // printf("Trying to match [%s] with item
...
GraphicComboBox::GraphicComboBox(const std::string &containerName,
                                 const std::string &objName)
    : GraphicComponent(containerName, objName),
      dropDownButton(containerName, objName + "_button") {
  selectedItemIndex = 0;
  lighted = false;
}
...
const string &GraphicComboBox::getTextNativeTranslation(int index) {
  if (this->translated_items.empty() == true || index < 0 ||
...
GraphicButton *GraphicComboBox::createButton(string item) {
  ...
  button->init(getX(), scrollBar.getY(), getW() - scrollBar.getW(),
  ...
  if (scrollBar.getElementCount() != 0) {
    ...
    for (int i = scrollBar.getVisibleStart(); i <= scrollBar.getVisibleEnd();
      ...
      if (i >= (int)popupButtons.size()) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 "i >= popupButtons.size(), i = %d, popupButtons.size() = %d",
                 i, (int)popupButtons.size());
        throw megaglest_runtime_error(szBuf);
      }
      popupButtons[i]->setY(keyButtonsYBase -
                            getPopupButtonHeight() *
                                (i + 1 - scrollBar.getVisibleStart()));
    }
  }
}
...
void GraphicComboBox::setItems(const vector<string> &items,
                               const vector<string> translated_items) {
  clearItems();
  this->items = items;
  this->translated_items = translated_items;
...
void GraphicComboBox::mouseUp(int x, int y) { scrollBar.mouseUp(x, y); }
...
GraphicMessageBox::GraphicMessageBox(const std::string &containerName,
                                     const std::string &objName)
    : GraphicComponent(containerName, objName) {
  header = "";
  autoWordWrap = true;
...
GraphicMessageBox::~GraphicMessageBox() { removeButtons(); }
...
void GraphicMessageBox::init(const string &button1Str, int newWidth,
                             int newHeight) {
...
void GraphicMessageBox::addButton(const string &buttonStr, int width,
                                  int height) {
  GraphicButton *newButton =
      new GraphicButton(containerName, instanceName + "_Button_" + buttonStr);
  newButton->init(0, 0);
...
GraphicLine::GraphicLine(const std::string &containerName,
                         const std::string &objName)
    : GraphicComponent(containerName, objName) {
  horizontal = false;
}
...
GraphicCheckBox::GraphicCheckBox(const std::string &containerName,
                                 const std::string &objName)
    : GraphicComponent(containerName, objName) {
  value = false;
  lighted = false;
...
GraphicScrollBar::GraphicScrollBar(const std::string &containerName,
                                   const std::string &objName)
    : GraphicComponent(containerName, objName) {
  lighted = false;
  activated = false;
...
void GraphicScrollBar::init(int x, int y, bool horizontal, int length,
                            int thickness) {
  GraphicComponent::init(x, y, horizontal ? length : thickness,
...
bool GraphicScrollBar::eventMouseWheel(int x, int y, int zDelta,
  ...
  if (ignorePos || GraphicComponent::mouseMove(x, y)) {
    int newVisibleStart = this->getVisibleStart() - zDelta / 120;
    if (newVisibleStart < 0) newVisibleStart = 0;
...
int GraphicScrollBar::getLength() const { return horizontal ? getW() : getH(); }
...
void GraphicScrollBar::arrangeComponents(vector<GraphicComponent *> &gcs) {
  if (getElementCount() != 0) {
    for (int i = getVisibleStart(); i <= getVisibleEnd(); ++i) {
      if (horizontal) {
        gcs[i]->setX(getX() + getLength() - gcs[i]->getW() -
                     gcs[i]->getW() * (i - getVisibleStart()));
      } else {
        gcs[i]->setY(getY() + getLength() - gcs[i]->getH() -
                     gcs[i]->getH() * (i - getVisibleStart()));
      }
    }
  }
}
...
PopupMenu::PopupMenu(const std::string &containerName,
                     const std::string &objName)
...
void PopupMenu::init(string menuHeader, std::vector<string> menuItems) {
  ...
  int maxH = (offsetH +
              (((int)menuItems.size() - 1) * (textHeight + textHeightSpacing)));
  if (maxH >= h) {
    h = maxH;
    y = (metrics.getVirtualH() - h) / 2;
  ...
  for (unsigned int i = 0; i < menuItems.size(); ++i) {
    GraphicButton button(containerName,
                         instanceName + "_Popup_Button_" + menuItems[i], false);
    button.registerGraphicComponentOnlyFontCallbacks(
        containerName, instanceName + "_Popup_Button_" + menuItems[i]);
    button.init(x + (w - maxButtonWidth) / 2,
                yStartOffset - (i * (textHeight + textHeightSpacing)));

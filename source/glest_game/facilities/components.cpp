...
void GraphicComponent::applyAllCustomProperties(std::string containerName) {
    ...
    std::map<std::string, std::map<std::string, GraphicComponent *>>::iterator iterFind1 = GraphicComponent::registeredGraphicComponentList.find(containerName);
    if (iterFind1 != GraphicComponent::registeredGraphicComponentList.end()) {
        for (std::map<std::string, GraphicComponent *>::iterator iterFind2 = iterFind1->second.begin(); iterFind2 != iterFind1->second.end(); ++iterFind2) {
...
void GraphicComponent::applyCustomProperties(std::string containerName) {
    if (instanceName != "") {
        ...
        std::map<std::string, std::map<std::string, GraphicComponent *>>::iterator iterFind1 =
...
bool GraphicComponent::saveAllCustomProperties(std::string containerName) {
    SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] registered [%s] count = %d\n", __FILE__, __FUNCTION__, __LINE__,
                             containerName.c_str(), registeredGraphicComponentList[containerName].size());
    Config &config = Config::getInstance();
...
bool GraphicLabel::mouseMove(int x, int y) {
    ...
    return x > this->x && y > this->y && x < this->x + useWidth && y < this->y + h;
}
...
GraphicListBox::GraphicListBox(const std::string &containerName, const std::string &objName)
    : GraphicComponent(containerName, objName), graphButton1(containerName, objName + "_button1"), graphButton2(containerName, objName + "_button2") {
    selectedItemIndex = 0;
    lighted = false;
...
const string &GraphicListBox::getTextNativeTranslation() {
    if (this->translated_items.empty() == true || this->selectedItemIndex < 0 || this->selectedItemIndex >= (int)this->translated_items.size() ||
...
void GraphicListBox::setSelectedItemIndex(int index, bool errorOnMissing) {
    if (errorOnMissing == true && (index < 0 || index >= (int)items.size())) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, "Index not found in listbox name: [%s] value index: %d size: %lu", this->instanceName.c_str(), index,
...
const string &GraphicComboBox::getTextNativeTranslation(int index) {
    if (this->translated_items.empty() == true || index < 0 || index >= (int)this->translated_items.size() ||
        this->items.size() != this->translated_items.size()) {
        return this->text;
    } else {
...
void GraphicComboBox::layoutButtons() {
    if (scrollBar.getElementCount() != 0) {
        ...
        for (int i = scrollBar.getVisibleStart(); i <= scrollBar.getVisibleEnd(); ++i) {
            ...
            popupButtons[i]->setY(keyButtonsYBase - getPopupButtonHeight() * (i + 1 - scrollBar.getVisibleStart()));
        }
    }
}
...
GraphicMessageBox::GraphicMessageBox(const std::string &containerName, const std::string &objName) : GraphicComponent(containerName, objName) {
    header = "";
    autoWordWrap = true;
}
...
GraphicLine::GraphicLine(const std::string &containerName, const std::string &objName) : GraphicComponent(containerName, objName) {
    horizontal = false;
}
...
GraphicCheckBox::GraphicCheckBox(const std::string &containerName, const std::string &objName) : GraphicComponent(containerName, objName) {
    value = false;
    lighted = false;
}
...
GraphicScrollBar::GraphicScrollBar(const std::string &containerName, const std::string &objName) : GraphicComponent(containerName, objName) {
    lighted = false;
    activated = false;
    horizontal = false;
    elementCount = 0;
...
bool GraphicScrollBar::eventMouseWheel(int x, int y, int zDelta, bool ignorePos) {
    if (ignorePos || GraphicComponent::mouseMove(x, y)) {
        ...
        if (newVisibleStart > this->getLength() - this->getVisibleSize()) newVisibleStart = this->getLength() - this->getVisibleSize();
...
PopupMenu::PopupMenu(const std::string &containerName, const std::string &objName) : GraphicComponent(containerName, objName, false) {

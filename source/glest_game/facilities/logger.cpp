...
Logger::Logger()
    : buttonCancel("Logger", "buttonCancel"),
      buttonNextHint("Logger", "buttonCancel") {
  progress = 0;
  string logs_path =
...
void Logger::loadGameHints(string filePathEnglish, string filePathTranslation,
                           bool clearList) {
  SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n",
                           __FILE__, __FUNCTION__, __LINE__);
  ...
  if ((filePathEnglish == "") || (filePathTranslation == "")) {
    ...
    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                             ...
                             __FILE__, __FUNCTION__, __LINE__,
                             filePathEnglish.c_str(),
                             filePathTranslation.c_str());
...
void Logger::showNextHint() {
  ...
  if (tmpString != "") {
    ...
    SystemFlags::OutputDebug(
        ...
        "In [%s::%s Line: %d] key [%s] not found for [%s] hint translation\n",
        __FILE__, __FUNCTION__, __LINE__, key.c_str(),
        Lang::getInstance().getLanguage().c_str());
...
void Logger::handleMouseClick(int x, int y) {
  ...
  if (buttonNextHint.getEnabled() == true &&
      buttonNextHint.mouseClick(x, y) == true) {
    showNextHint();
...
void Logger::renderLoadingScreen() {
  ...
  if (showProgressBar == true) {
    if (Renderer::renderText3DEnabled) {
      renderer.renderProgressBar3D(
          progress, metrics.getVirtualW() / 4, 59 * metrics.getVirtualH() / 100,
          coreData.getDisplayFontSmall3D(), 500,
          ""); // no string here, because it has to be language specific and
               // does not give much information
    } else {
      renderer.renderProgressBar(
          progress, metrics.getVirtualW() / 4, 59 * metrics.getVirtualH() / 100,
          coreData.getDisplayFontSmall(), 500,
          ""); // no string here, because it has to be language specific and
               // does not give much information
    }
  }
  ...
  if (Renderer::renderText3DEnabled) {
    ...
    renderer.renderTextShadow3D(state, coreData.getMenuFontBig3D(),
                                displayColor, xLocation,
                                65 * metrics.getVirtualH() / 100, false);

    renderer.renderTextShadow3D(current, coreData.getMenuFontNormal3D(),
                                displayColor, xLocation,
    ...
    if (this->statusText != "") {
      renderer.renderTextShadow3D(
          ...
          xLocation, 56 * metrics.getVirtualH() / 100, false);
    }
  } else {
    renderer.renderTextShadow(state, coreData.getMenuFontBig(), displayColor,
                              xLocation, 65 * metrics.getVirtualH() / 100,
                              false);

    renderer.renderTextShadow(current, coreData.getMenuFontNormal(),
                              displayColor, xLocation,
                              62 * metrics.getVirtualH() / 100, false);

    if (this->statusText != "") {
      renderer.renderTextShadow(this->statusText, coreData.getMenuFontNormal(),
                                displayColor, xLocation,
  ...
  if (gameHintToShow != "") {
    ...
    if (Renderer::renderText3DEnabled) {
      int xLocationHint =
          (metrics.getVirtualW() / 2) -
          (coreData.getMenuFontBig3D()->getMetrics()->getTextWidth(hintText) /
           2);
    ...
    if (buttonNextHint.getEnabled() == false) {
      buttonNextHint.init((metrics.getVirtualW() / 2) - (175 / 2),
                          90 * metrics.getVirtualH() / 100 + 20, 175);
      buttonNextHint.setText(lang.getString("ShowNextHint", "", true));
...
void Logger::setCancelLoadingEnabled(bool value) {
  ...
  buttonCancel.init((metrics.getVirtualW() / 2) - (125 / 2),
                    50 * metrics.getVirtualH() / 100, 125);
  buttonCancel.setText(lang.getString("Cancel"));

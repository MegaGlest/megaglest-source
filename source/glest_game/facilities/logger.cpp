...
void Logger::loadGameHints(string filePathEnglish, string filePathTranslation, bool clearList) {
    ...
    if ((filePathEnglish == "") || (filePathTranslation == "")) {
        ...
        SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                 "In [%s::%s Line: %d] filePathEnglish = [%s]\n "
...
void Logger::showNextHint() {
    ...
    if (tmpString != "") {
        ...
        SystemFlags::OutputDebug(SystemFlags::debugError, "In [%s::%s Line: %d] key [%s] not found for [%s] hint translation\n", __FILE__, __FUNCTION__,
                                 __LINE__, key.c_str(), Lang::getInstance().getLanguage().c_str());
...
void Logger::renderLoadingScreen() {
    ...
    if (showProgressBar == true) {
        if (Renderer::renderText3DEnabled) {
            renderer.renderProgressBar3D(progress, metrics.getVirtualW() / 4, 59 * metrics.getVirtualH() / 100, coreData.getDisplayFontSmall3D(), 500,
                                         ""); // no string here, because it has to be language specific and
                                              // does not give much information
        } else {
            renderer.renderProgressBar(progress, metrics.getVirtualW() / 4, 59 * metrics.getVirtualH() / 100, coreData.getDisplayFontSmall(), 500,
                                       ""); // no string here, because it has to be language specific and
                                            // does not give much information
        }
    }
    ...
    if (Renderer::renderText3DEnabled) {
        renderer.renderTextShadow3D(state, coreData.getMenuFontBig3D(), displayColor, xLocation, 65 * metrics.getVirtualH() / 100, false);

        renderer.renderTextShadow3D(current, coreData.getMenuFontNormal3D(), displayColor, xLocation, 62 * metrics.getVirtualH() / 100, false);

        if (this->statusText != "") {
            renderer.renderTextShadow3D(this->statusText, coreData.getMenuFontNormal3D(), displayColor, xLocation, 56 * metrics.getVirtualH() / 100, false);
        }
    } else {
        renderer.renderTextShadow(state, coreData.getMenuFontBig(), displayColor, xLocation, 65 * metrics.getVirtualH() / 100, false);

        renderer.renderTextShadow(current, coreData.getMenuFontNormal(), displayColor, xLocation, 62 * metrics.getVirtualH() / 100, false);

        if (this->statusText != "") {
            renderer.renderTextShadow(this->statusText, coreData.getMenuFontNormal(), displayColor, xLocation, 56 * metrics.getVirtualH() / 100, false);
        }
    }
    ...
    if (gameHintToShow != "") {
        ...
        if (Renderer::renderText3DEnabled) {
            ...
            renderer.renderTextShadow3D(hintText, coreData.getMenuFontBig3D(), displayColor,
                                        // xLocation*1.5f,
                                        xLocationHint, 90 * metrics.getVirtualH() / 100, false);
        } else {
            int xLocationHint = (metrics.getVirtualW() / 2) - (coreData.getMenuFontBig()->getMetrics()->getTextWidth(hintText) / 2);

            renderer.renderTextShadow(hintText, coreData.getMenuFontBig(), displayColor,
                                      // xLocation*1.5f,
                                      xLocationHint, 90 * metrics.getVirtualH() / 100, false);
        }
        // Show next Hint
        ...
        /*
                    if(Renderer::renderText3DEnabled) {
                            int xLocationHint =  (metrics.getVirtualW() / 2) -
       (coreData.getMenuFontBig3D()->getMetrics()->getTextWidth(hintText) / 2);

                            renderer.renderText3D(
                                            lang.getString("ShowNextHint","",true),
       coreData.getMenuFontNormal3D(), nextHintTitleColor,
                                            //xLocation*1.5f,
                                            xLocationHint,
                                            93 * metrics.getVirtualH() / 100,
       false);
                    }
                    else {
                            int xLocationHint =  (metrics.getVirtualW() / 2) -
       (coreData.getMenuFontBig()->getMetrics()->getTextWidth(hintText) / 2);

                            renderer.renderText(
                                            lang.getString("ShowNextHint","",true),
       coreData.getMenuFontNormal(), nextHintTitleColor,
                                    //xLocation*1.5f,
                                    xLocationHint,

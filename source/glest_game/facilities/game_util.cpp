...
string getRAWGITRevisionString() { return GIT_RawRev; }
...
string getGITRevisionString() { return GIT_Rev; }
...
string getCompilerNameString() {
  ...
  if (version == "") {
    ...
    version = "Clang: " + intToStr(__clang_major__) + "." +
              intToStr(__clang_minor__) + "." + intToStr(__clang_patchlevel__);
...
string getNetworkVersionString() {
  static string version = "";
  if (version == "") {
    version = glestVersionString + "-" + getCompilerNameString() + "-" +
              getCompileDateTime();
  }
  return version;
}
...
string getNetworkVersionGITString() {
  static string version = "";
  if (version == "") {
    version = glestVersionString + "-" + getGITRevisionString() + "-" +
...
string getNetworkPlatformFreeVersionString() { return glestVersionString; }

string getAboutString1(int i) {
  // case 1: return "Built: " + string(__DATE__) + " " + GIT_Rev;
  switch (i) {
  case 0:
    return "MegaGlest " + glestVersionString + " (" + "Shared Library " +
           sharedLibVersionString + ")";
  case 1:
    return GIT_Rev;
  case 2:
    return "Copyright 2001-2010 The Glest Team";
  case 3:
    return "Copyright 2010-2021 The MegaGlest Team";
  }
  return "";
}
...
string getAboutString2(int i) {
  switch (i) {
  case 0:
    return "Web: https://megaglest.org";
  case 1:
    return "Bug reports: " + string(mailString);
  case 2:
    return "IRC: irc://irc.libera.chat/megaglest";
  }
  return "";
}
...
string getGameCustomCoreDataPath(string originalBasePath,
  ...
  string custom_mod_path =
      config.getCustomRuntimeProperty(Config::ACTIVE_MOD_PROPERTY_NAME);
  if (custom_mod_path != "") {
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("Looking for [%s] in\n#1: [%s]\n#2: [%s]\n#3: [%s]\n",
           uniqueFilePath.c_str(), custom_mod_path.c_str(), data_path.c_str(),
           originalBasePath.c_str());
...
bool upgradeFilesInTemp() {
  ...
  vector<string> fileList =
      getFolderTreeContentsListRecursively(tempFilePath, "", false, NULL);
  for (unsigned int index = 0; index < fileList.size(); ++index) {
    string fileName = fileList[index];
    string newFileName = Properties::getApplicationPath() +

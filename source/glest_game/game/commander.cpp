...
std::pair<CommandResult, string> Commander::tryGiveCommand(
...
std::pair<CommandResult, string> Commander::tryGiveCommand(
    ...
    const UnitType *unitType, CardinalDir facing, bool tryQueue,
    Unit *targetUnit, int unitGroupCommandId) const {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__);
...
std::pair<CommandResult, string> Commander::tryGiveCommand(
    const Selection *selection, CommandClass commandClass, const Vec2i &pos,
...
// auto command
std::pair<CommandResult, string> Commander::tryGiveCommand(
    const Selection *selection, const Vec2i &pos, const Unit *targetUnit,
    bool tryQueue, int unitCommandGroupId) const {
  if (this->pauseNetworkCommands == true) {
    return std::pair<CommandResult, string>(crFailUndefined, "");
  }
...
std::pair<CommandResult, string> Commander::computeResult(
...
std::pair<CommandResult, string> Commander::pushNetworkCommand(
...
void Commander::giveNetworkCommand(NetworkCommand *networkCommand) const {
        ...
        if (factionIndex == world->getThisFactionIndex()) {
          ...
          if (gameNetworkInterface != NULL) {
            ...
            for (unsigned int i = 0; i < languageList.size(); ++i) {
              char szMsg[8096] = "";
              if (lang.hasString("PlayerSwitchedTeam", languageList[i]) ==
                  true) {
                snprintf(szMsg, 8096,
                         lang.getString("PlayerSwitchedTeam", languageList[i])
                             .c_str(),
          ...
          if (gameNetworkInterface != NULL) {
            ...
            for (unsigned int i = 0; i < languageList.size(); ++i) {
              ...
              if (lang.hasString("PlayerSwitchedTeamDenied", languageList[i]) ==
                ...
                snprintf(szMsg, 8096,
                         "Player %s was denied the request to switch from "
                         "team# %d to team# %d.",
                         settings->getNetworkPlayerName(factionIndex).c_str(),
                         oldTeam, vote->newTeam);
              }
  ...
  if (commandWasHandled == false) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "Running command NetworkCommandType = %d, unitid = %d [%p] "
          "factionindex = %d\n",
          networkCommand->getNetworkCommandType(), networkCommand->getUnitId(),
          unit, (unit != NULL ? unit->getFactionIndex() : -1));

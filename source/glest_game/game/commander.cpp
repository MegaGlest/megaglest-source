...
void Commander::init(World *world) { this->world = world; }

bool Commander::canSubmitCommandType(const Unit *unit,
                                     const CommandType *commandType) const {
  bool canSubmitCommand = true;
  const MorphCommandType *mct =
      dynamic_cast<const MorphCommandType *>(commandType);
  if (mct && unit->getCommandSize() > 0) {
    Command *cur_command = unit->getCurrCommand();
    if (cur_command != NULL) {
      const MorphCommandType *cur_mct =
          dynamic_cast<const MorphCommandType *>(cur_command->getCommandType());
      if (cur_mct && unit->getCurrSkill() &&
        ...
        if (morphUnitType != NULL && cur_morphUnitType != NULL &&
            morphUnitType->getId() == cur_morphUnitType->getId()) {
  ...
  if (selection->isEmpty() == false && commandType != NULL) {
    ...
    for (int i = 0; i < selection->getCount(); ++i) {
      ...
      if (canSubmitCommand == true) {
        ...
        Vec2i currPos = world->getMap()->computeDestPos(
            refPos, unit->getPosNotThreadSafe(), pos);
        ...
        if (useCommandtype != NULL) {
          NetworkCommand networkCommand(
              this->world, nctGiveCommand, unitId, useCommandtype->getId(),
              usePos, unitType->getId(),
              (targetUnit != NULL ? targetUnit->getId() : -1), facing, tryQueue,
...
std::pair<CommandResult, string>
Commander::tryGiveCommand(const Unit *unit, const CommandType *commandType,
  ...
  if (canSubmitCommand == true) {
    NetworkCommand networkCommand(
        this->world, nctGiveCommand, unit->getId(), commandType->getId(), pos,
        unitType->getId(), (targetUnit != NULL ? targetUnit->getId() : -1),
...
std::pair<CommandResult, string>
Commander::tryGiveCommand(const Selection *selection, CommandClass commandClass,
                          const Vec2i &pos, const Unit *targetUnit,
                          bool tryQueue) const {
    ...
    for (int i = 0; i < selection->getCount(); ++i) {
      ...
      if (ct != NULL) {
        ...
        if (canSubmitCommand == true) {
          ...
          int targetId =
              targetUnit == NULL ? Unit::invalidId : targetUnit->getId();
          int unitId = selection->getUnit(i)->getId();
          Vec2i currPos = world->getMap()->computeDestPos(
              refPos, selection->getUnit(i)->getPosNotThreadSafe(), pos);
          NetworkCommand networkCommand(
              this->world, nctGiveCommand, unitId, ct->getId(), currPos, -1,
              targetId, -1, tryQueue, cst_None, -1, unitCommandGroupId);
        ...
        results.push_back(
            std::pair<CommandResult, string>(crFailUndefined, ""));
      }
    }
    return computeResult(results);
  } else {
    ...
    for (int i = 0; i < selection->getCount(); ++i) {
      ...
      if (canSubmitCommand == true) {
        ...
        Vec2i currPos = world->getMap()->computeDestPos(
            refPos, unit->getPosNotThreadSafe(), pos);
        NetworkCommand networkCommand(
            this->world, nctGiveCommand, unitId, commandType->getId(), currPos,
            -1, targetId, -1, tryQueue, cst_None, -1, unitCommandGroupId);
...
// auto command
std::pair<CommandResult, string>
Commander::tryGiveCommand(const Selection *selection, const Vec2i &pos,
                          const Unit *targetUnit, bool tryQueue,
  ...
  if (selection->isEmpty() == false) {
    ...
    for (int i = 0; i < selection->getCount(); ++i) {
      ...
      currPos = world->getMap()->computeDestPos(
          refPos, unit->getPosNotThreadSafe(), pos);

      // get command type
      const CommandType *commandType =
          unit->computeCommandType(pos, targetUnit);
      ...
      if (commandType != NULL) {
        ...
        NetworkCommand command(this->world, nctSetMeetingPoint, unit->getId(),
                               -1, currPos, -1, -1, -1, false, cst_None, -1,
                               unitCommandGroupId);
...
void Commander::trySetMeetingPoint(const Unit *unit, const Vec2i &pos) const {
  ...
  NetworkCommand command(this->world, nctSetMeetingPoint, unit->getId(), -1,
                         pos);
  pushNetworkCommand(&command);
}
...
void Commander::trySwitchTeam(const Faction *faction, int teamIndex) const {
  ...
  NetworkCommand command(this->world, nctSwitchTeam, faction->getIndex(),
                         teamIndex);
  pushNetworkCommand(&command);
}

void Commander::trySwitchTeamVote(const Faction *faction,
                                  SwitchTeamVote *vote) const {
  if (this->pauseNetworkCommands == true) {
    return;
  }

  NetworkCommand command(this->world, nctSwitchTeamVote, faction->getIndex(),
                         vote->factionIndex, Vec2i(0), vote->allowSwitchTeam);
  pushNetworkCommand(&command);
}

void Commander::tryDisconnectNetworkPlayer(const Faction *faction,
                                           int playerIndex) const {
  NetworkCommand command(this->world, nctDisconnectNetworkPlayer,
                         faction->getIndex(), playerIndex);
  pushNetworkCommand(&command);
}
...
void Commander::tryNetworkPlayerDisconnected(int factionIndex) const {
  ...
  NetworkCommand command(this->world, nctPlayerStatusChange, factionIndex,
                         npst_Disconnected);
  pushNetworkCommand(&command);
}
...
std::pair<CommandResult, string>
Commander::pushNetworkCommand(const NetworkCommand *networkCommand) const {
  ...
  if (networkCommand->getNetworkCommandType() != nctSwitchTeam &&
    ...
    if (unit == NULL) {
      char szBuf[8096] = "";
      snprintf(szBuf, 8096,
               "In [%s::%s - %d] Command refers to non existent unit id = %d. "
               "Game out of synch.",
               extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
               __LINE__, networkCommand->getUnitId());
      GameNetworkInterface *gameNetworkInterface =
          NetworkManager::getInstance().getGameNetworkInterface();
      if (gameNetworkInterface != NULL) {
        char szMsg[8096] = "";
        snprintf(szMsg, 8096,
                 "Player detected an error: Command refers to non existent "
                 "unit id = %d. Game out of synch.",
  ...
  if (unit != NULL &&
    ...
    // printf("In [%s::%s Line: %d] result.first =
...
void Commander::signalNetworkUpdate(Game *game) { updateNetwork(game); }
...
bool Commander::getReplayCommandListForFrame(int worldFrameCount) {
  ...
  if (replayCommandList.empty() == false) {
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "worldFrameCount = %d replayCommandList.size() = " MG_SIZE_T_SPECIFIER
          "\n",
    ...
    if (haveReplyCommands == true) {
      replayCommandList.erase(replayCommandList.begin(),
                              replayCommandList.begin() + replayList.size());

      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("worldFrameCount = %d GIVING COMMANDS replayList.size() "
               "= " MG_SIZE_T_SPECIFIER "\n",
    ...
    if (networkManager.isNetworkGame() == false ||
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(
            ...
            "In [%s::%s Line: %d] networkManager.isNetworkGame() = "
            "%d,world->getFrameCount() = %d, "
            "gameSettings->getNetworkFramePeriod() = %d\n",
            extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
            __LINE__, networkManager.isNetworkGame(), world->getFrameCount(),
      ...
      if (getReplayCommandListForFrame(world->getFrameCount()) == false) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
                .enabled &&
            perfTimer.getMillis() > 0)
          SystemFlags::OutputDebug(
              SystemFlags::debugPerformance,
              "In [%s::%s Line: %d] gameNetworkInterface->updateKeyframe for "
              "%d took %lld msecs\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, world->getFrameCount(), perfTimer.getMillis());
        ...
        for (int i = 0; i < gameNetworkInterface->getPendingCommandCount();
             ++i) {
...
void Commander::addToReplayCommandList(NetworkCommand &command,
                                       int worldFrameCount) {
...
void Commander::giveNetworkCommand(NetworkCommand *networkCommand) const {
  ...
  world->getGame()->addNetworkCommandToReplayList(networkCommand,
                                                  world->getFrameCount());
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
    ...
    SystemFlags::OutputDebug(SystemFlags::debugPerformance,
                             ...
                             "networkCommand->preprocessNetworkCommand]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, chrono.getMillis());

  bool commandWasHandled = false;
  // Handle special commands first (that just use network command members as
  // placeholders)
        ...
        if (gameNetworkInterface != NULL) {
          ...
          for (unsigned int i = 0; i < languageList.size(); ++i) {
            ...
            if (lang.hasString("PlayerSwitchedTeam", languageList[i]) == true) {
              snprintf(
                  szMsg, 8096,
                  lang.getString("PlayerSwitchedTeam", languageList[i]).c_str(),
                  settings->getNetworkPlayerName(factionIndex).c_str(), oldTeam,
                  newTeam);
            } else {
              snprintf(szMsg, 8096,
  ...
  case nctSwitchTeamVote: {
    ...
    if (newTeamTotalMemberCount > 0 && newTeamVotedYes > 0 &&
        static_cast<float>(newTeamVotedYes) /
                static_cast<float>(newTeamTotalMemberCount) >
            0.5) {
      Faction *faction = world->getFaction(factionIndex);
      int oldTeam = faction->getTeam();
      ...
      if (factionIndex == world->getThisFactionIndex()) {
        ...
        if (gameNetworkInterface != NULL) {
          ...
          for (unsigned int i = 0; i < languageList.size(); ++i) {
            char szMsg[8096] = "";
            if (lang.hasString("PlayerSwitchedTeamDenied", languageList[i]) ==
                true) {
              snprintf(
                  szMsg, 8096,
                  lang.getString("PlayerSwitchedTeamDenied", languageList[i])
                      .c_str(),
                  settings->getNetworkPlayerName(factionIndex).c_str(), oldTeam,
                  vote->newTeam);
            } else {
              snprintf(szMsg, 8096,
                       "Player %s was denied the request to switch from team# "
                       "%d to team# %d.",
                       settings->getNetworkPlayerName(factionIndex).c_str(),
                       oldTeam, vote->newTeam);
            }
    ...
    if (role == nrServer) {
      ...
      if (gameNetworkInterface != NULL) {
        ...
        if (server != NULL && server->isClientConnected(playerIndex) == true) {
          ...
          MutexSafeWrapper safeMutex(server->getSlotMutex(playerIndex),
                                     CODE_AT_LINE);
          ...
          if (slot != NULL) {
            ...
            if (server != NULL) {
              MutexSafeWrapper safeMutex2(server->getSlotMutex(playerIndex),
                                          CODE_AT_LINE);
  ...
  case nctPlayerStatusChange: {
    ...
    // printf("#1 nctPlayerStatusChange factionIndex = %d playerStatus =
    // %d\n",factionIndex,playerStatus);

    GameSettings *settings = world->getGameSettingsPtr();
    if (playerStatus == npst_Disconnected) {
      // printf("Commander nctPlayerStatusChange factionIndex:
      // %d\n",factionIndex);

      settings->setNetworkPlayerStatuses(factionIndex, npst_Disconnected);

      // printf("nctPlayerStatusChange -> faction->getPersonalityType() = %d
      // index [%d] control [%d] networkstatus [%d]\n",
      ...
      // printf("#2 nctPlayerStatusChange factionIndex = %d playerStatus =
      // %d\n",factionIndex,playerStatus);
      settings->setFactionControl(factionIndex, ctCpuUltra);
      settings->setResourceMultiplierIndex(
          factionIndex, settings->getFallbackCpuMultiplier());
      // Game *game = this->world->getGame();
      // game->get
      ...
      if (!world->getGame()->getGameOver() &&
          !this->world->getGame()->factionLostGame(factionIndex)) {
        ...
        this->world->getStats()->setTimePlayerLeft(
            factionIndex,
            this->world->getStats()->getFramesToCalculatePlaytime());
      }
    }
  ...
  if (commandWasHandled == false) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("Running command NetworkCommandType = %d, unitid = %d [%p] "
             "factionindex = %d\n",
    ...
    if (unit != NULL) {
      ...
      case nctGiveCommand: {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
          SystemFlags::OutputDebug(
              SystemFlags::debugSystem,
              "In [%s::%s Line: %d] found nctGiveCommand "
              "networkCommand->getUnitId() = %d\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, networkCommand->getUnitId());
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
          SystemFlags::OutputDebug(
              SystemFlags::debugSystem, "In [%s::%s Line: %d] command = %p\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, command);
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugSystem,
            "In [%s::%s Line: %d] NULL Unit for id = %d, "
            "networkCommand->getNetworkCommandType() = %d\n",
            extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
            __LINE__, networkCommand->getUnitId(),
            networkCommand->getNetworkCommandType());
    }
...
Command *Commander::buildCommand(const NetworkCommand *networkCommand) const {
  ...
  if (unit == NULL) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] Can not find unit with id: %d. Game out of "
             "synch.",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__, networkCommand->getUnitId());
    ...
    if (gameNetworkInterface != NULL) {
      char szMsg[8096] = "";
      snprintf(szMsg, 8096,
               "Player detected an error: Can not find unit with id: %d. Game "
               "out of synch.",
               networkCommand->getUnitId());
      gameNetworkInterface->sendTextMessage(szMsg, -1, true, "");
    }
  ...
  if (unit->getFaction()->getIndex() != networkCommand->getUnitFactionIndex()) {
    ...
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d]\nUnit / Faction mismatch for network "
             "command = [%s]\n%s\nfor unit = %d\n[%s]\n[%s]\nactual local "
             "factionIndex = %d.\nGame out of synch.",
             __FILE__, __FUNCTION__, __LINE__,
             networkCommand->toString().c_str(),
             unit->getType()->getCommandTypeListDesc().c_str(), unit->getId(),
             unit->getFullName(false).c_str(), unit->getDesc(false).c_str(),
    ...
    if (gameNetworkInterface != NULL &&
      ...
      char szMsg[8096] = "";
      snprintf(szMsg, 8096,
               "Player detected an error: Connection lost, possible Unit / "
               "Faction mismatch for unitId: %d",
               networkCommand->getUnitId());
      gameNetworkInterface->sendTextMessage(szMsg, -1, true, "");
      snprintf(
          szMsg, 8096,
    ...
    std::string sError = "Error [#1]: Game is out of sync (Unit / Faction "
                         "mismatch)\nplease check log files for details.";
    throw megaglest_runtime_error(sError);
  }

  const UnitType *unitType = world->findUnitTypeById(
      unit->getFaction()->getType(), networkCommand->getUnitTypeId());
  ...
  if (ct == NULL && unit->getPreMorphType() != NULL) {
    const CommandType *ctPreMorph =
        unit->getPreMorphType()->findCommandTypeById(
  ...
  if (ct == NULL) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d]\nCan not find command type for network "
             "command = [%s]\n%s\nfor unit = %d\n[%s]\n[%s]\nactual local "
             "factionIndex = %d.\nUnit Type Info:\n[%s]\nNetwork unit "
             "type:\n[%s]\nisCancelPreMorphCommand: %d\nGame out of synch.",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__, networkCommand->toString().c_str(),
             unit->getType()->getCommandTypeListDesc().c_str(), unit->getId(),
             unit->getFullName(false).c_str(), unit->getDesc(false).c_str(),
             unit->getFaction()->getIndex(),
             unit->getType()->toString().c_str(),
             (unitType != NULL ? unitType->getName(false).c_str() : "null"),
    ...
    if (gameNetworkInterface != NULL) {
      ...
      snprintf(
          szMsg, 8096,
          "Player detected an error: Can not find command type: %d for unitId: "
          "%d [%s]. isCancelPreMorphCommand: %d Game out of synch.",
          networkCommand->getCommandTypeId(), networkCommand->getUnitId(),
          (unitType != NULL ? unitType->getName(false).c_str() : "null"),
          isCancelPreMorphCommand);
      gameNetworkInterface->sendTextMessage(szMsg, -1, true, "");
    }
  ...
  if (isCancelPreMorphCommand == false) {
    if (ct->getClass() == ccBuild) {
      if (networkCommand->getTargetId() < 0 ||
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 "networkCommand->getTargetId() >= 0 && "
                 "networkCommand->getTargetId() < 4, [%s]",
                 networkCommand->toString().c_str());
        throw megaglest_runtime_error(szBuf);
      }
  ...
  if (isCancelPreMorphCommand == false) {
    if (unitType != NULL) {
      command =
          new Command(ct, networkCommand->getPosition(), unitType, facing);
    } else if (target == NULL) {

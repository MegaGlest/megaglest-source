...
std::pair<CommandResult, string> Commander::tryGiveCommand(const Selection *selection, const CommandType *commandType, const Vec2i &pos,
                                                           const UnitType *unitType, CardinalDir facing, bool tryQueue, Unit *targetUnit) const {
        ...
        for (int i = 0; i < selection->getCount(); ++i) {
            ...
            if (canSubmitCommand == true) {
                ...
                if (useCommandtype != NULL) {
                    NetworkCommand networkCommand(this->world, nctGiveCommand, unitId, useCommandtype->getId(), usePos, unitType->getId(),
                                                  (targetUnit != NULL ? targetUnit->getId() : -1), facing, tryQueue, commandStateType, commandStateValue,
                                                  unitCommandGroupId);
...
std::pair<CommandResult, string> Commander::tryGiveCommand(const Unit *unit, const CommandType *commandType, const Vec2i &pos, const UnitType *unitType,
                                                           CardinalDir facing, bool tryQueue, Unit *targetUnit, int unitGroupCommandId) const {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
    ...
    if (canSubmitCommand == true) {
        NetworkCommand networkCommand(this->world, nctGiveCommand, unit->getId(), commandType->getId(), pos, unitType->getId(),
                                      (targetUnit != NULL ? targetUnit->getId() : -1), facing, tryQueue, cst_None, -1, unitGroupCommandId);
...
std::pair<CommandResult, string> Commander::tryGiveCommand(const Selection *selection, CommandClass commandClass, const Vec2i &pos, const Unit *targetUnit,
                                                           bool tryQueue) const {
    if (this->pauseNetworkCommands == true) {
        return std::pair<CommandResult, string>(crFailUndefined, "");
    }
...
std::pair<CommandResult, string> Commander::tryGiveCommand(const Selection *selection, const CommandType *commandType, const Vec2i &pos, const Unit *targetUnit,
                                                           bool tryQueue) const {
    if (this->pauseNetworkCommands == true) {
        return std::pair<CommandResult, string>(crFailUndefined, "");
    }
...
// auto command
std::pair<CommandResult, string> Commander::tryGiveCommand(const Selection *selection, const Vec2i &pos, const Unit *targetUnit, bool tryQueue,
    ...
    if (selection->isEmpty() == false) {
        ...
        for (int i = 0; i < selection->getCount(); ++i) {
            ...
            if (commandType != NULL) {
                ...
                NetworkCommand command(this->world, nctSetMeetingPoint, unit->getId(), -1, currPos, -1, -1, -1, false, cst_None, -1, unitCommandGroupId);
...
CommandResult Commander::tryCancelCommand(const Selection *selection) const {
    ...
    for (int i = 0; i < selection->getCount(); ++i) {
        NetworkCommand command(this->world, nctCancelCommand, selection->getUnit(i)->getId(), -1, Vec2i(0), -1, -1, -1, false, cst_None, -1,
                               unitCommandGroupId);
...
std::pair<CommandResult, string> Commander::pushNetworkCommand(const NetworkCommand *networkCommand) const {
    ...
    if (networkCommand->getNetworkCommandType() != nctSwitchTeam && networkCommand->getNetworkCommandType() != nctSwitchTeamVote &&
        networkCommand->getNetworkCommandType() != nctPauseResume && networkCommand->getNetworkCommandType() != nctPlayerStatusChange &&
...
bool Commander::getReplayCommandListForFrame(int worldFrameCount) {
    ...
    if (replayCommandList.empty() == false) {
        if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("worldFrameCount = %d replayCommandList.size() = " MG_SIZE_T_SPECIFIER "\n", worldFrameCount, replayCommandList.size());
...
void Commander::updateNetwork(Game *game) {
    ...
    if (game != NULL) {
        ...
        if (networkManager.isNetworkGame() == false || (world->getFrameCount() % gameSettings->getNetworkFramePeriod()) == 0) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                         "In [%s::%s Line: %d] networkManager.isNetworkGame() = "
                                         "%d,world->getFrameCount() = %d, "
                                         "gameSettings->getNetworkFramePeriod() = %d\n",
                                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, networkManager.isNetworkGame(),
            ...
            if (getReplayCommandListForFrame(world->getFrameCount()) == false) {
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && perfTimer.getMillis() > 0)
                    SystemFlags::OutputDebug(SystemFlags::debugPerformance,
                                             ...
                                             "%d took %lld msecs\n",
                                             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, world->getFrameCount(),
                                             perfTimer.getMillis());
...
void Commander::giveNetworkCommand(NetworkCommand *networkCommand) const {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s Line: %d] took msecs: %lld [START]\n",
    ...
    case nctSwitchTeam: {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] found nctSwitchTeam\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                ...
                if (gameNetworkInterface != NULL) {
                    ...
                    for (unsigned int i = 0; i < languageList.size(); ++i) {
                        ...
                        if (lang.hasString("PlayerSwitchedTeam", languageList[i]) == true) {
                            ...
                            snprintf(szMsg, 8096, "Player %s switched from team# %d to team# %d.", settings->getNetworkPlayerName(factionIndex).c_str(),
                                     oldTeam, newTeam);
                        }
    ...
    case nctSwitchTeamVote: {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] found nctSwitchTeamVote\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                     __FUNCTION__, __LINE__);
        ...
        if (newTeamTotalMemberCount > 0 && newTeamVotedYes > 0 && static_cast<float>(newTeamVotedYes) / static_cast<float>(newTeamTotalMemberCount) > 0.5) {
            Faction *faction = world->getFaction(factionIndex);
            int oldTeam = faction->getTeam();
                ...
                if (gameNetworkInterface != NULL) {
                    ...
                    for (unsigned int i = 0; i < languageList.size(); ++i) {
                        ...
                        if (lang.hasString("PlayerSwitchedTeamDenied", languageList[i]) == true) {
                            snprintf(szMsg, 8096, lang.getString("PlayerSwitchedTeamDenied", languageList[i]).c_str(),
                                     settings->getNetworkPlayerName(factionIndex).c_str(), oldTeam, vote->newTeam);
                        } else {
                            snprintf(szMsg, 8096,
    ...
    case nctDisconnectNetworkPlayer: {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] found nctDisconnectNetworkPlayer\n",
                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
    ...
    case nctPauseResume: {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] found nctPauseResume\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                     __FUNCTION__, __LINE__);
    ...
    case nctPlayerStatusChange: {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "nctPlayerStatusChange factionIndex = %d playerStatus = %d\n", factionIndex, playerStatus);
    ...
    if (commandWasHandled == false) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
            SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s Line: %d] took msecs: %lld [after world->findUnitById]\n",
                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, chrono.getMillis());
        ...
        if (unit != NULL) {
            ...
            case nctGiveCommand: {
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
                    SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s Line: %d] took msecs: %lld [after buildCommand]\n",
                                             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, chrono.getMillis());
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s Line: %d] took msecs: %lld [END]\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__, chrono.getMillis());
}
...
Command *Commander::buildCommand(const NetworkCommand *networkCommand) const {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] networkCommand [%s]\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__, networkCommand->toString().c_str());

    if (world == NULL) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, "In [%s::%s Line: %d] world == NULL for unit with id: %d", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
                 networkCommand->getUnitId());
        throw megaglest_runtime_error(szBuf);
    }
    ...
    if (unit->getFaction()->getIndex() != networkCommand->getUnitFactionIndex()) {
        ...
        snprintf(szBuf, 8096,
                 ...
                 "command = [%s]\n%s\nfor unit = %d\n[%s]\n[%s]\nactual local "
                 "factionIndex = %d.\nGame out of synch.",
                 __FILE__, __FUNCTION__, __LINE__, networkCommand->toString().c_str(), unit->getType()->getCommandTypeListDesc().c_str(), unit->getId(),
                 unit->getFullName(false).c_str(), unit->getDesc(false).c_str(), unit->getFaction()->getIndex());
        ...
        if (gameNetworkInterface != NULL && gameNetworkInterface->isConnected() == true) {
            ...
            gameNetworkInterface->sendTextMessage(szMsg, -1, true, "");
            snprintf(szMsg, 8096, "Local faction index = %d, remote index = %d. Game out of synch.", unit->getFaction()->getIndex(),
    ...
    if (ct == NULL) {
        ...
        snprintf(szBuf, 8096,
                 ...
                 unit->getType()->getCommandTypeListDesc().c_str(), unit->getId(), unit->getFullName(false).c_str(), unit->getDesc(false).c_str(),
                 unit->getFaction()->getIndex(), unit->getType()->toString().c_str(), (unitType != NULL ? unitType->getName(false).c_str() : "null"),
        ...
        if (gameNetworkInterface != NULL) {
            ...
            snprintf(szMsg, 8096,
                     ...
                     networkCommand->getCommandTypeId(), networkCommand->getUnitId(), (unitType != NULL ? unitType->getName(false).c_str() : "null"),
                     isCancelPreMorphCommand);
            gameNetworkInterface->sendTextMessage(szMsg, -1, true, "");
        }

...
Game::Game() : ProgramState(NULL) {
  ...
  // printf("In [%s:%s] Line: %d currentAmbientSound =
  ...
  GAME_STATS_DUMP_INTERVAL =
      Config::getInstance().getInt("GameStatsDumpIntervalSeconds",
                                   intToStr(GAME_STATS_DUMP_INTERVAL).c_str());
}
...
void Game::resetMembers() {
  ...
  // MIN_RENDER_FPS_ALLOWED =
  // Config::getInstance().getInt("MIN_RENDER_FPS_ALLOWED",intToStr(MIN_RENDER_FPS_ALLOWED).c_str());
...
Game::Game(Program *program, const GameSettings *gameSettings,
  ...
  Lang::getInstance().setAllowNativeLanguageTechtree(
      this->gameSettings.getNetworkAllowNativeLanguageTechtree());
...
void Game::endGame() {
  ...
  logger.add(Lang::getInstance().getString("LogScreenGameLoading", "", true),
             false);
  logger.hideProgress();
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                             ...
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, aiInterfaces.size());

  // MUST DO THIS LAST!!!! Because objects above have pointers to things like
  // unit particles and fade them out etc and this end method deletes the
  // original object pointers.
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugWorldSynch,
                             "==== END GAME ====\n");

  FileCRCPreCacheThread *&preCacheCRCThreadPtr =
      CacheManager::getCachedItem<FileCRCPreCacheThread *>(
...
bool Game::quitTriggered() { return quitTriggeredIndicator; }
...
// Texture2D * Game::findFactionLogoTexture(const GameSettings *settings, Logger
// *logger,string factionLogoFilter, bool useTechDefaultIfFilterNotFound) {
//	Texture2D *result = NULL;
//	string logoFilename = Game::findFactionLogoFile(settings,
// logger,factionLogoFilter); 	if(logoFilename == "" && factionLogoFilter != ""
// && useTechDefaultIfFilterNotFound == true) { 		logoFilename =
// Game::findFactionLogoFile(settings, logger);
//	}
//
...
string Game::extractScenarioLogoFile(const GameSettings *settings,
  ...
  if (settings->getScenarioDir() != "") {
    ...
    if (EndsWith(scenarioDir, ".xml") == true) {
      ...
      scenarioDir =
          scenarioDir.erase(scenarioDir.size() - settings->getScenario().size(),
                            settings->getScenario().size() + 1);
    }

    // printf("!!! extractScenarioLogoFile scenarioDir [%s] factionLogoFilter
    ...
    if (loadScreenList.empty() == false) {
      ...
      if (loadScreenList.size() > 1 &&
        ...
        if (SystemFlags::VERBOSE_MODE_ENABLED)
          printf("\nLooking for best logo from a list of: " MG_SIZE_T_SPECIFIER
                 " using filter: [%s]\n",
        ...
        for (unsigned int logoIndex = 0; logoIndex < loadScreenList.size();
             ++logoIndex) {
          ...
          if (fileExists(senarioLogo) == true) {
            ...
            if (checkLogo != NULL) {
              ...
              int minWidthDifference = abs(metrics.getScreenW() -
                                           checkLogo->getPixmapConst()->getW());
              int minHeightDifference = abs(
                  metrics.getScreenH() - checkLogo->getPixmapConst()->getH());

              if (SystemFlags::VERBOSE_MODE_ENABLED)
                printf("Logo info: %d x %d (%d,%d)\n",
                       checkLogo->getPixmapConst()->getW(),
                       checkLogo->getPixmapConst()->getH(), minWidthDifference,
                       minHeightDifference);
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(
          ...
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, settings->getScenarioDir().c_str(),
          settings->getScenario().c_str(), scenarioDir.c_str());
  }
  return scenarioDir;
}

string Game::extractFactionLogoFile(bool &loadingImageUsed,
  ...
  if (factionName == formatString(GameConstants::OBSERVER_SLOTNAME)) {
    ...
    const string factionLogo =
        data_path + "data/core/misc_textures/observer.jpg";
    // printf("In [%s::%s Line: %d] looking for loading screen
  ...
  // else if(settings->getFactionTypeName(i) ==
      ...
      if (isdir(path.c_str()) == true) {
        ...
        if (loadScreenList.empty() == false) {
          ...
          if (loadScreenList.size() > 1 &&
            ...
            for (unsigned int logoIndex = 0;
                 logoIndex < (unsigned int)loadScreenList.size(); ++logoIndex) {
              ...
              if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem)
                ...
                SystemFlags::OutputDebug(
                    ...
                    "In [%s::%s Line: %d] looking for loading screen '%s'\n",
                    extractFileFromDirectoryPath(__FILE__).c_str(),
                    __FUNCTION__, __LINE__, factionLogo.c_str());
        ...
        // Check if this is a linked faction
          ...
          if (fileExists(factionXMLFile) == true) {
            ...
            // printf("B factionXMLFile [%s] root name [%s] root first child
            // name
            // [%s]\n",factionXMLFile.c_str(),rootNode->getName().c_str(),rootNode->getChild(0)->getName().c_str());
            // printf("B factionXMLFile [%s] root name
            ...
            if (rootNode->getName() == "link" &&
              ...
              string linkedTechTreeName =
                  techtreeNode->getAttribute("name")->getValue();

              // printf("D factionXMLFile [%s] linkedTechTreeName
              ...
              if (linkedTechTreeName != "") {
                ...
                // string linkedTmppath= linkedCurrentPath + factionName
                // +".xml";
                ...
                if (isdir(path.c_str()) == true) {
                  ...
                  if (loadScreenList.empty() == false) {
                    ...
                    if (SystemFlags::getSystemSettingType(
                      ...
                      SystemFlags::OutputDebug(
                          SystemFlags::debugSystem,
                          "In [%s::%s Line: %d] looking for loading screen "
                          "'%s'\n",
                          extractFileFromDirectoryPath(__FILE__).c_str(),
                          __FUNCTION__, __LINE__, factionLogo.c_str());
  ...
  if (factionName != "") {
    ...
    for (int idx = 0; hudFound == false && idx < (int)pathList.size(); idx++) {
      ...
      string path =
          currentPath + techName + "/" + "factions" + "/" + factionName;
      endPathWithSlash(path);
      findAll(path + GameConstants::HUD_SCREEN_FILE_FILTER, hudList, false,
              false);
      if (hudList.empty() == false) {
        for (unsigned int hudIdx = 0;
             hudFound == false && hudIdx < (unsigned int)hudList.size();
             ++hudIdx) {
          string hudImageFileName = path + hudList[hudIdx];
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] looking for a HUD [%s]\n",
                   extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                   __LINE__, hudImageFileName.c_str());
          if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem)
                  .enabled)
            SystemFlags::OutputDebug(
                SystemFlags::debugSystem,
          ...
          if (fileExists(hudImageFileName) == true) {
            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf("In [%s::%s Line: %d] found HUD image [%s]\n",
                     extractFileFromDirectoryPath(__FILE__).c_str(),
  ...
  if (settings == NULL) {
    ...
    if (logger != NULL) {
      ...
      if (scenarioName.empty()) {
        ...
        TechTree techTree(
            Config::getInstance().getPathListForType(ptTechs, scenarioDir));

        logger->setSubtitle(formatString(mapName) + " - " +
                            formatString(tilesetName) + " - " +
                            formatString(techTree.getTranslatedName(techName)));
      } else {
    ...
    string scenarioDir = extractScenarioLogoFile(
        settings, result, loadingImageUsed, logger, factionLogoFilter);
    //}
    ...
    if (loadingImageUsed == false) {
      for (int i = 0; i < settings->getFactionCount(); ++i) {
        if (settings->getFactionControl(i) == ctHuman ||
          ...
          result = extractFactionLogoFile(
              loadingImageUsed, settings->getFactionTypeName(i), scenarioDir,
              techName, logger, factionLogoFilter);
          break;
        }
      }
    }
    ...
    // if(skipCustomLoadScreen == false && loadingImageUsed == false){
    if (loadingImageUsed == false) {
      result = extractTechLogoFile(scenarioDir, techName, loadingImageUsed,
      ...
      if (factions.empty() == false) {
        for (unsigned int factionIdx = 0;
          ...
          string factionLogo = Game::extractFactionLogoFile(
              loadingImageUsed, currentFactionName_factionPreview, "", techName,
              NULL, GameConstants::PREVIEW_SCREEN_FILE_FILTER);

          if (factionLogo == "") {
            factionLogo = Game::extractFactionLogoFile(
                loadingImageUsed, currentFactionName_factionPreview, "",
                techName, NULL, GameConstants::LOADING_SCREEN_FILE_FILTER);
          }
          if (factionLogo != "") {
...
void Game::load(int loadTypes) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(
        ...
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        loadTypes, this->gameSettings.toString().c_str());
  ...
  string englishFile = getGameCustomCoreDataPath(
      data_path, "data/lang/hint/hint_" +
                     Lang::getInstance().getDefaultLanguage() + ".lng");
  string languageFile = getGameCustomCoreDataPath(
      data_path,
      "data/lang/hint/hint_" + Lang::getInstance().getLanguage() + ".lng");
  string languageFileUserData = user_data_path + "data/lang/hint/hint_" +
  ...
  if ((loadTypes & lgt_Scenario) == lgt_Scenario) {
    ...
    if (scenarioName.empty() == false) {
      ...
      // printf("Loading scenario gameSettings.getScenarioDir() [%s]
      // scenarioName [%s] isTutorial:
      // %d\n",gameSettings.getScenarioDir().c_str(),scenarioName.c_str(),isTutorial);

      Lang::getInstance().loadScenarioStrings(gameSettings.getScenarioDir(),
                                              scenarioName, isTutorial);

      // printf("In [%s::%s Line: %d] rootNode
      // [%p][%s]\n",__FILE__,__FUNCTION__,__LINE__,loadGameNode,(loadGameNode
      // != NULL ? loadGameNode->getName().c_str() : "none"));
      world.loadScenario(gameSettings.getScenarioDir(), &checksum, false,
                         loadGameNode);
    }
  }
...
void Game::init(bool initForPreviewOnly) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                             ...
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, initForPreviewOnly);
  ...
  if (initForPreviewOnly == false) {
    ...
    for (int i = 0; i < world.getFactionCount(); ++i) {
      ...
      if (faction->getCpuControl(enableServerControlledAI, isNetworkGame,
        ...
        snprintf(
            szBuf, 8096,
            Lang::getInstance()
                .getString("LogScreenGameLoadingCreatingAIFaction", "", true)
                .c_str(),
            i);
    ...
    if (Config::getInstance().getBool("EnableNewThreadManager", "false") ==
        true) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(
        ...
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__);
  logger.add(Lang::getInstance().getString("LogScreenGameLoadingInitRenderer",
                                           "", true),
             true);
  ...
  if (initForPreviewOnly == false) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(
          ...
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__);
    logger.add(Lang::getInstance().getString(
                   "LogScreenGameLoadingWaitForNetworkPlayers", "", true),
               true);
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugSystem,
                               ...
                               __FUNCTION__, __LINE__);
    logger.add(Lang::getInstance().getString(
                   "LogScreenGameLoadingStartingMusic", "", true),
               true);
    ...
    if (this->masterserverMode == false) {
      ...
      //	throw
      // megaglest_runtime_error("world.getThisFaction()->getType()->getMusic()
      //== NULL");
      // }
    }
    ...
    if (tileset->getWeather() == wRainy && ambientSounds->isEnabledRain()) {
      logger.add(Lang::getInstance().getString(
                     "LogScreenGameLoadingStartingAmbient", "", true),
                 true);
    ...
    logger.add(Lang::getInstance().getString("LogScreenGameLoadingLaunchGame",
                                             "", true));
  }
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugSystem,
        "================ STARTING GAME ================\n");
  if (SystemFlags::getSystemSettingType(SystemFlags::debugPathFinder).enabled)
    SystemFlags::OutputDebug(
  ...
  for (int i = 0; i < world.getFactionCount(); ++i) {
    ...
    // printf("Check for team switch to observer i = %d, team = %d
    // [%d]\n",i,faction->getTeam(),(GameConstants::maxPlayers -1 +
    // fpt_Observer));
    if (faction != NULL &&
  ...
  if (role == nrClient) {
    ClientInterface *clientInterface =
        dynamic_cast<ClientInterface *>(networkManager.getClientInterface());
...
void Game::initCamera(Map *map) {
  ...
  if (world.getThisFaction() != NULL) {
    const Vec2i &v =
        map->getStartLocation(world.getThisFaction()->getStartLocationIndex());
...
void Game::reInitGUI() { gui.init(this); }
...
void Game::setupPopupMenus(bool checkClientAdminOverrideOnly) {
  ...
  if (role == nrServer) {
    ...
    if (disconnectPlayerPopupMenuIndex == -1) {
      serverInterface =
          dynamic_cast<ServerInterface *>(networkManager.getServerInterface());
      if (serverInterface != NULL && checkClientAdminOverrideOnly == true) {
        for (int i = 0; i < world.getFactionCount(); ++i) {
          Faction *faction = world.getFaction(i);

          MutexSafeWrapper safeMutex(
              serverInterface->getSlotMutex(faction->getStartLocationIndex()),
              CODE_AT_LINE);
          ConnectionSlot *slot =
              serverInterface->getSlot(faction->getStartLocationIndex(), false);
          if (slot != NULL && slot->getConnectHasHandshaked() == true &&
              slot->getCurrentFrameCount() <= 0) {
            // printf("Connected slot can be disconnected:
            // %d\n",slot->getPlayerIndex());
      ...
      if (calculateNetworkCRC == true) {
        ...
        for (int index = 0; index < GameConstants::maxPlayers; ++index) {
          if (index < world.getFactionCount()) {
            ...
            if (settings != NULL) {
              if (isFlagType1BitEnabled(ft1_network_synch_checks_verbose) ==
                  true) {
                faction->addCRC_DetailsForWorldFrame(world.getFrameCount(),
                                                     role == nrServer);
              } else if (isFlagType1BitEnabled(ft1_network_synch_checks) ==
                             true &&
...
void Game::update() {
  try {
    ...
    if (showPerfStats) {
      sprintf(perfBuf,
              "=============== FRAME: %d In [%s::%s] Line: %d took "
              "msecs: " MG_I64_SPECIFIER "\n",
              world.getFrameCount(),
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, chronoPerf.getMillis());
      perfList.push_back(perfBuf);
    ...
    if (this->masterserverMode == false) {
      if (world.getFactionCount() > 0 &&
        ...
        if (lang.hasString("AllowPlayerJoinTeam") == true) {
          snprintf(szBuf, 8096, lang.getString("AllowPlayerJoinTeam").c_str(),
                   settings->getNetworkPlayerName(vote->factionIndex).c_str(),
        ...
        world.getThisFactionPtr()->setCurrentSwitchTeamVoteFactionIndex(
            vote->factionIndex);
      }
    }
    ...
    if (role == nrClient && updateLoops == 1 &&
        world.getFrameCount() >= (gameSettings.getNetworkFramePeriod() * 2)) {
      ClientInterface *clientInterface =
          dynamic_cast<ClientInterface *>(networkManager.getClientInterface());
      if (clientInterface != NULL) {
        uint64 lastNetworkFrameFromServer =
        ...
        receivedTooEarlyInFrames[(index + 1) %
                                 GameConstants::networkSmoothInterval] = -1;
        framesNeededToWaitForServerMessage
            [(index + 1) % GameConstants::networkSmoothInterval] = -1;
        ...
        if (receivedTooEarlyInFrames[index] == -1) {
          ...
          if (lastNetworkFrameFromServer > 0 &&
              lastNetworkFrameFromServer > (uint64)world.getFrameCount()) {
        ...
        if (framesNeededToWaitForServerMessage[index] == -1) {
          ...
          if (timeClientWaitedForLastMessage > 0) {
            ...
            framesNeededToWaitForServerMessage[index] =
                timeClientWaitedForLastMessage * GameConstants::updateFps /
                1000;
            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf("ClienttimeClientWaitedForLastMessage:%d ms  which is %d "
                     "frames \n",
                     (int)timeClientWaitedForLastMessage,
                     framesNeededToWaitForServerMessage[index]);
          } else {
        ...
        if (countOfMessagesReceivedTooEarly ==
            GameConstants::networkSmoothInterval -
                1) // -1 because slot for next frame is already initialized
        ...
        if (cleanupStats == true) {
          // Once we decided to use the stats to do some correction, we
          // reset/cleanup our recorded stats
          for (int i = 0; i < GameConstants::networkSmoothInterval; i++) {
            receivedTooEarlyInFrames[i] = -1;
            framesNeededToWaitForServerMessage[i] = -1;
          }
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
        ...
        chrono.getMillis() > 0)
      SystemFlags::OutputDebug(SystemFlags::debugPerformance,
                               "In [%s::%s] Line: %d took msecs: %lld [before "
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
        ...
        chrono.getMillis() > 0)
      SystemFlags::OutputDebug(SystemFlags::debugPerformance,
                               "In [%s::%s] Line: %d took msecs: %lld [after "
                               "ReplaceDisconnectedNetworkPlayersWithAI]\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__, chrono.getMillis());
    if (showPerfStats) {
      ...
      do {
        ...
        for (int i = 0; i < updateLoops; ++i) {
          ...
          if (commander.hasReplayCommandListForFrame() == false) {
            ...
            if (newThreadManager == true) {
              ...
              for (int j = 0; j < world.getFactionCount(); ++j) {
                ...
                // printf("Faction Index = %d enableServerControlledAI = %d,
                // isNetworkGame = %d, role = %d isCPU player = %d
                ...
                if (faction->getCpuControl(enableServerControlledAI,
                  ...
                  if (SystemFlags::getSystemSettingType(
                    ...
                    SystemFlags::OutputDebug(
                        SystemFlags::debugPerformance,
                        "In [%s::%s Line: %d] [i = %d] faction = %d, "
                        "factionCount = %d, took msecs: %lld [before AI "
                        "updates]\n",
                        extractFileFromDirectoryPath(__FILE__).c_str(),
                        __FUNCTION__, __LINE__, i, j, world.getFactionCount(),
                        chrono.getMillis());
            ...
            if (lastReplaySecond < chronoReplay.getSeconds()) {
              ...
              if (Renderer::renderText3DEnabled) {
                ...
                int renderX =
                    (w / 2) - (font->getMetrics()->getTextWidth(text) / 2);
                int h = metrics.getVirtualH();
                int renderY =
                    (h / 2) + (font->getMetrics()->getHeight(text) / 2);

                renderer.renderText3D(text, font, Vec3f(1.f, 1.f, 0.f), renderX,
                                      renderY, false);
              } else {
                ...
                renderer.renderText(text, font, Vec3f(1.f, 1.f, 0.f), renderX,
                                    renderY, true);
              }
          ...
          if (currentCameraFollowUnit != NULL) {
            ...
            c.y = c.y + currentCameraFollowUnit->getType()->getHeight() / 2.f +
                  2.0f;
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugPerformance,
                "In [%s::%s] Line: %d took msecs: %lld [commander "
                "updateNetwork i = %d]\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, chrono.getMillis(), i);
          if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugPerformance,
                "In [%s::%s] Line: %d took msecs: %lld [weather particle "
                "updating i = %d]\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, chrono.getMillis(), i);
          if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugPerformance,
                "In [%s::%s] Line: %d took msecs: %lld [particle manager "
                "updating i = %d]\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, chrono.getMillis(), i);
          if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
    ...
    if (role == nrServer) {
      ...
      if (this->networkPauseGameForLaggedClientsRequested == true) {
        ...
        if (getPaused() == false) {
          ...
          printf("[CMDR] Pausing game for lagging client(s), current world "
                 "frame [%d]\n",
                 world.getFrameCount());
        ...
        if (getPaused() == true) {
          ...
          printf("[CMDR] Resuming game after Pause for lagging client(s), "
                 "current world frame [%d]\n",
                 world.getFrameCount());
      ...
      if (server->getPauseForInGameConnection() == true) {
        ...
        for (int i = 0; i < world.getFactionCount(); ++i) {
          ...
          MutexSafeWrapper safeMutex(
              server->getSlotMutex(faction->getStartLocationIndex()),
              CODE_AT_LINE);
          ConnectionSlot *slot =
              server->getSlot(faction->getStartLocationIndex(), false);
        ...
        if (pausedForJoinGame == false || clientNeedsGameSetup == true) {
          ...
          for (int i = 0; i < world.getFactionCount(); ++i) {
            ...
            // printf("Switching player check %d from: %d connected: %d,
            // startindex = %d, connected #2:
            ...
            if (faction->getControlType() != ctNetwork &&
              ...
              // printf("Switching player %d from: %d to
              // %d\n",i,faction->getControlType(),ctNetwork); printf("Slot: %d
              // faction name: %s GS faction:
              ...
              server->gameSettings.setNetworkPlayerPlatform(
                  i, slot->getPlatform());
              safeMutex.ReleaseLock();
              server->gameSettings.setNetworkPlayerStatuses(i, npst_None);

              this->gameSettings.setFactionControl(i, ctNetwork);
              this->gameSettings.setNetworkPlayerName(
                  i, server->gameSettings.getNetworkPlayerName(i));
              this->gameSettings.setNetworkPlayerUUID(
                  i, server->gameSettings.getNetworkPlayerUUID(i));
              this->gameSettings.setNetworkPlayerPlatform(
                  i, server->gameSettings.getNetworkPlayerPlatform(i));
              this->gameSettings.setNetworkPlayerStatuses(i, npst_None);
            }
          }
          // printf("#1 Data synch: lmap %u ltile: %d ltech:
          // %u\n",gameSettings.getMapCRC(),gameSettings.getTilesetCRC(),gameSettings.getTechCRC());
          // printf("#2 Data synch: lmap %u ltile: %d ltech:
        ...
        for (int i = 0; i < world.getFactionCount(); ++i) {
          ...
          if (slot != NULL && slot->getJoinGameInProgress() == true) {
            // printf("$$$ signalling client to start game [deleting AI player]
            // factionIndex: %d slot: %d startlocation:
            ...
            // this->gameSettings.setNetworkPlayerName(i,lang.getString("AI") +
            // intToStr(i+1));
            // server->gameSettings.setNetworkPlayerName(i,lang.getString("AI")
            // + intToStr(i+1));
          } else {
            safeMutex.ReleaseLock();
          }
      ...
      if (pausedForJoinGame == true) {
        ...
        if (server->getUnPauseForInGameConnection() == true) {
          ...
          if (saveNetworkGame == true) {
            ...
            string file = this->saveGame(
                GameConstants::saveNetworkGameFileServer, "temp/");

            string saveGameFilePath = "temp/";
            string saveGameFileCompressed =
                saveGameFilePath +
                string(GameConstants::saveNetworkGameFileServerCompressed);
            if (getGameReadWritePath(GameConstants::path_logs_CacheLookupKey) !=
                "") {
              saveGameFilePath = getGameReadWritePath(
      ...
      if (switchRequested == true) {
        ...
        // for(int i= 0; i < gameSettings.getFactionCount(); ++i) {
        // printf("#1 Faction Index: %d control: %d startlocation:
        // %d\n",i,gameSettings.getFactionControl(i),gameSettings.getStartLocationIndex(i));

        // printf("#2 Faction Index: %d control: %d startlocation:
    ...
    if (world.getQueuedScenario() != "") {
      ...
      try {
        ...
        // printf("\nname [%s] scenarioFile [%s] results.size() = "
        // MG_SIZE_T_SPECIFIER
        ...
        if (keepFactions == false) {
          ...
          BaseColorPickEntity::resetUniqueColors();
          // MUST DO THIS LAST!!!! Because objects above have pointers to things
          // like unit particles and fade them out etc and this end method
          // deletes the original object pointers.
          Renderer &renderer = Renderer::getInstance();
          renderer.endGame(true);
    ...
    // printf("#100 quitPendingIndicator = %d, errorMessageBox.getEnabled() =
...
string Game::getGamePerformanceCounts(bool displayWarnings) const {
  ...
  for (std::map<string, int64>::const_iterator iterMap =
    ...
    if (iterMap->first == ProgramState::MAIN_PROGRAM_RENDER_KEY) {
      ...
      // else {
      //	printf("iterMap->second: " MG_I64_SPECIFIER "
...
bool Game::switchSetupForSlots(ServerInterface *&serverInterface,
  ...
  MutexSafeWrapper safeMutex(serverInterface->getSwitchSetupRequestsMutex(),
                             CODE_AT_LINE);
  ...
  for (int i = startIndex; i < endIndex; ++i) {
    if (switchSetupRequests[i] != NULL) {
      // printf("Faction Index: %d Switch slot = %d to = %d current control =
      ...
      if (gameSettings.getFactionControl(i) == ctNetwork ||
          gameSettings.getFactionControl(i) == ctNetworkUnassigned ||
          //(gameSettings.getFactionControl(i) != ctClosed &&
        ...
        if (switchSetupRequests[i]->getToSlotIndex() != -1) {
          ...
          if (serverInterface->switchSlot(switchSlotIdx, newSlotIdx)) {
            ...
            int oldFactionIndex =
                gameSettings.getFactionIndexForStartLocation(switchSlotIdx);
            int newFactionIndex =
                gameSettings.getFactionIndexForStartLocation(newSlotIdx);

            // printf("Switching faction for index %d [%d] to
            ...
            serverInterface->gameSettings.setNetworkPlayerPlatform(
                oldFactionIndex, "");

            gameSettings.setFactionControl(newFactionIndex, ctNetwork);
            serverInterface->gameSettings.setFactionControl(newFactionIndex,
                                                            ctNetwork);

            // printf("#1a Faction Index: %d control: %d startlocation:
            // %d\n",newFactionIndex,gameSettings.getFactionControl(newFactionIndex),gameSettings.getStartLocationIndex(newFactionIndex));
            // printf("#2a Faction Index: %d control: %d startlocation:
            ...
            try {
              ...
              if (switchSetupRequests[i]->getNetworkPlayerName() != "") {
                // if(SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                // SystemFlags::OutputDebug(SystemFlags::debugSystem,"In [%s::%s
                // Line %d] i = %d, labelPlayerNames[newFactionIdx].getText()
                // [%s] switchSetupRequests[i]->getNetworkPlayerName()
                ...
                serverInterface->gameSettings.setNetworkPlayerName(
                    newFactionIndex,
                    switchSetupRequests[i]->getNetworkPlayerName());
              }

              //							if(gameSettings.getFactionControl(switchFactionIdx)
              //== ctNetworkUnassigned) {
              ...
              //
              //								labelPlayers[switchFactionIdx].setVisible(switchFactionIdx+1
              //<= mapInfo.players);
              //								labelPlayerNames[switchFactionIdx].setVisible(switchFactionIdx+1
              //<= mapInfo.players);
              //								listBoxControls[switchFactionIdx].setVisible(switchFactionIdx+1
              //<= mapInfo.players);
              //								listBoxFactions[switchFactionIdx].setVisible(switchFactionIdx+1
              //<= mapInfo.players);
              //								listBoxTeams[switchFactionIdx].setVisible(switchFactionIdx+1
              //<= mapInfo.players);
              //								labelNetStatus[switchSlotIdx].setVisible(switchSlotIdx+1
              //<= mapInfo.players);
              //							}
          ...
          try {
            ...
            if ((switchSetupRequests[i]->getSwitchFlags() &
              ...
              // if(SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
              // SystemFlags::OutputDebug(SystemFlags::debugSystem,"In [%s::%s
              // Line: %d] i = %d, switchSetupRequests[i]->getSwitchFlags() =
              // %d, switchSetupRequests[i]->getNetworkPlayerName() [%s],
              ...
              if (switchSetupRequests[i]->getNetworkPlayerName() !=
                ...
                gameSettings.setNetworkPlayerName(
                    i, switchSetupRequests[i]->getNetworkPlayerName());
                serverInterface->gameSettings.setNetworkPlayerName(
                    i, switchSetupRequests[i]->getNetworkPlayerName());
                switchRequested = true;
              } else {
    ...
    if (gameNetworkInterface != NULL &&
      ...
      for (int idx = 0; idx < (int)chatList.size(); idx++) {
        ...
        if (mc.getFactionIndex() >= 0) {
          mc.setFaction(
              (const Faction *)world.getFaction(mc.getFactionIndex()));
        }
  ...
  try {
    ...
    if (gameNetworkInterface != NULL &&
      ...
      for (int idx = 0; idx < (int)highlighList.size(); idx++) {
        ...
        if (mc.getFactionIndex() >= 0) {
          mc.setFaction((const Faction *)world.getFaction(
              mc.getFactionIndex())); // set faction pointer
        }
...
void Game::ReplaceDisconnectedNetworkPlayersWithAI(bool isNetworkGame,
                                                   NetworkRole role) {
  if (role == nrServer && isNetworkGame == true &&
      difftime((long int)time(NULL), lastNetworkPlayerConnectionCheck) >=
      ...
      if (faction->getFactionDisconnectHandled() == false &&
        ...
        if (aiInterfaces[i] == NULL &&
          ...
          if (faction->getPersonalityType() != fpt_Observer) {
            ...
            snprintf(szBuf, 8096,
                     Lang::getInstance()
                         .getString("LogScreenGameLoadingCreatingAIFaction", "",
                                    true)
                         .c_str(),
                     i);
            logger.add(szBuf, true);
          ...
          for (unsigned int j = 0; j < (unsigned int)languageList.size(); ++j) {
            if (isPlayerObserver == false) {
              string msg = "Player #%d [%s] has disconnected, switching player "
                           "to AI mode!";
              if (lang.hasString("GameSwitchPlayerToAI", languageList[j],
                                 true)) {
                msg = lang.getString("GameSwitchPlayerToAI", languageList[j],
                                     true);
              }
              snprintf(szBuf, 8096, msg.c_str(), i + 1,
                       this->gameSettings.getNetworkPlayerName(i).c_str());
            } else {
              string msg = "Player #%d [%s] has disconnected, but player was "
                           "only an observer!";
              if (lang.hasString("GameSwitchPlayerObserverToAI",
                                 languageList[j], true)) {
                msg = lang.getString("GameSwitchPlayerObserverToAI",
                                     languageList[j], true);
              }
              snprintf(szBuf, 8096, msg.c_str(), i + 1,
                       this->gameSettings.getNetworkPlayerName(i).c_str());
    ...
    if (newAIPlayerCreated == true &&
        Config::getInstance().getBool("EnableNewThreadManager", "false") ==
            true) {
      bool enableServerControlledAI =
          this->gameSettings.getEnableServerControlledAI();
...
void Game::render() {
  ...
  if (this->masterserverMode == false) {
    ...
    if (connectedClients == 0) {
      ...
      if (difftime((long int)time(NULL), lastMasterServerGameStatsDump) >=
          GAME_STATS_DUMP_INTERVAL) {
...
void Game::tryPauseToggle(bool pauseValue) {
  ...
  if (role == nrServer) {
    ...
    if (clientInterface != NULL && gameSettings.getMasterserver_admin() ==
                                       clientInterface->getSessionKey()) {
      allowAdminMenuItems = true;
    }
  }
  ...
  bool isNetworkGame = this->gameSettings.isNetworkGame();
  // printf("Try Pause allowAdminMenuItems = %d, pauseValue =
...
void Game::processInputText(string text, bool cancelled) {
  ...
  if (cancelled == false) {
    ...
    //			playerIndex =
    // cellMarkedData.getFaction()->getStartLocationIndex();
    //		}
...
void Game::addCellMarker(Vec2i cellPos, MarkedCell cellData) {
  ...
  // printf("Adding Cell marker pos [%s] factionIndex [%d] note [%s] playerIndex
  // =
  // %d\n",cellData.getTargetPos().getString().c_str(),factionIndex,cellData.getNote().c_str(),playerIndex);

  gameNetworkInterface->sendMarkCellMessage(
      cellData.getTargetPos(), factionIndex, cellData.getNote(), playerIndex);
...
void Game::removeCellMarker(Vec2i surfaceCellPos, const Faction *faction) {
  ...
  // printf("Remove Cell marker lookup pos [%s] factionIndex
  // [%d]\n",surfaceCellPos.getString().c_str(),(faction != NULL ?
  // faction->getIndex() : -1));
  ...
  if (mapMarkedCellList.find(surfaceCellPos) != mapMarkedCellList.end()) {
    ...
    if (mc.getFaction() == faction) {
      ...
      // printf("Remvoing Cell marker pos [%s] factionIndex [%d] note
  ...
  // printf("#1 ADDED in marked list pos [%s] markedCells.size() = "
  // MG_SIZE_T_SPECIFIER
...
void Game::showMarker(Vec2i cellPos, MarkedCell cellData) {
  ...
  // MarkedCell
  // mc(targetPos,world.getThisFaction(),"none",world.getThisFaction()->getStartLocationIndex());
  addOrReplaceInHighlightedCells(cellData);

  GameNetworkInterface *gameNetworkInterface =
      NetworkManager::getInstance().getGameNetworkInterface();
  gameNetworkInterface->sendHighlightCellMessage(cellData.getTargetPos(),
...
void Game::mouseDownLeft(int x, int y) {
  ...
  try {
    ...
    if (popupMenu.mouseClick(x, y)) {
      ...
      // printf("popupMenu.mouseClick == true result.first = %d
      ...
      if (result.first == exitGamePopupMenuIndex) {
        showMessageBox(Lang::getInstance().getString("ExitBattleQuestion"), "",
                       true);
        ...
        for (unsigned int i = 0; i < (unsigned int)world.getFactionCount();
          ...
          if (faction->getPersonalityType() != fpt_Observer &&
            ...
            if (lang.hasString("JoinPlayerTeam") == true) {
              snprintf(szBuf, 8096,
                       ("  " + lang.getString("JoinPlayerTeam") + "  ").c_str(),
                       faction->getIndex(),
                       this->gameSettings.getNetworkPlayerName(i).c_str(),
                       faction->getTeam());
            } else {
              snprintf(szBuf, 8096, "  Join player #%d - %s on Team: %d  ",
                       faction->getIndex(),
                       this->gameSettings.getNetworkPlayerName(i).c_str(),
                       faction->getTeam());
            }
        ...
        popupMenuSwitchTeams.init("  " + lang.getString("SwitchTeams") + "  ",
                                  menuItems);
        ...
        ServerInterface *serverInterface = NULL;
        if (role == nrServer) {
          serverInterface = dynamic_cast<ServerInterface *>(
        ...
        for (unsigned int i = 0; i < (unsigned int)world.getFactionCount();
          ...
          // printf("faction->getPersonalityType() = %d index [%d,%d] control
          // [%d] networkstatus
          ...
          if (serverInterface != NULL) {
            ...
            MutexSafeWrapper safeMutex(
                serverInterface->getSlotMutex(faction->getStartLocationIndex()),
                CODE_AT_LINE);
            ConnectionSlot *slot = serverInterface->getSlot(
                faction->getStartLocationIndex(), false);
            if (slot != NULL && slot->getConnectHasHandshaked() == true &&
                slot->getCurrentFrameCount() <= 0) {
          ...
          // printf("isSlotJoinInProgressClient: %d [%d] [%d][%d] [%d] [%d]
          // [%d]\n",
          ...
          if (isSlotJoinInProgressClient == true ||
            ...
            if (lang.hasString("DisconnectNetorkPlayerIndex") == true) {
              snprintf(
                  szBuf, 8096,
                  ("  " + lang.getString("DisconnectNetorkPlayerIndex") + "  ")
                      .c_str(),
                  faction->getIndex() + 1,
                  this->gameSettings.getNetworkPlayerName(i).c_str());
            } else {
              snprintf(szBuf, 8096, "  Disconnect player #%d - %s:  ",
                       faction->getIndex() + 1,
                       this->gameSettings.getNetworkPlayerName(i).c_str());
            }
            ...
            // disconnectPlayerIndexMap[menuItems.size()-1] =
            // faction->getStartLocationIndex();
        ...
        popupMenuDisconnectPlayer.init(
            "  " + lang.getString("DisconnectNetorkPlayer") + "  ", menuItems);
        ...
        MainMenu *newMenu =
            new MainMenu(program); // open keyboard shortcuts setup screen
        ...
        newMenu->setState(
            new MenuStateKeysetup(program, newMenu, &currentUIState));
        ...
        if (role == nrServer) {
          ...
          ClientInterface *clientInterface = dynamic_cast<ClientInterface *>(
      ...
      std::pair<int, string> result =
          popupMenuSwitchTeams.mouseClickedMenuItem(x, y);
      // printf("In popup callback menuItemSelected [%s] menuIndexSelected =
      // %d\n",result.second.c_str(),result.first);
      ...
      // popupMenuSwitchTeams
      std::pair<int, string> result =
          popupMenuDisconnectPlayer.mouseClickedMenuItem(x, y);
      // printf("In popup callback menuItemSelected [%s] menuIndexSelected =
      // %d\n",result.second.c_str(),result.first);
      ...
      default:
        ...
        if (lang.hasString("DisconnectNetorkPlayerIndexConfirm") == true) {
          snprintf(szBuf, 8096,
                   ("  " +
                    lang.getString("DisconnectNetorkPlayerIndexConfirm") + "  ")
                       .c_str(),
                   factionIndex + 1,
                   settings->getNetworkPlayerName(factionIndex).c_str());
        } else {
          snprintf(szBuf, 8096,
        ...
        if (gameNetworkInterface != NULL) {
          ...
          for (unsigned int i = 0; i < (unsigned int)languageList.size(); ++i) {
            ...
            if (lang.hasString("DisconnectNetorkPlayerIndexConfirmed",
                               languageList[i]) == true) {
              snprintf(szMsg, 8096,
                       lang.getString("DisconnectNetorkPlayerIndexConfirmed",
                                      languageList[i])
                           .c_str(),
                       factionIndex + 1,
                       settings->getNetworkPlayerName(factionIndex).c_str());
            ...
            gameNetworkInterface->sendTextMessage(szMsg, -1, localEcho,
                                                  languageList[i]);
          }
    ...
    if (switchTeamConfirmMessageBox.getEnabled() == true) {
      ...
      if (switchTeamConfirmMessageBox.mouseClick(x, y, button)) {
        ...
        SwitchTeamVote *vote = world.getThisFactionPtr()->getSwitchTeamVote(
            world.getThisFaction()->getCurrentSwitchTeamVoteFactionIndex());
        vote->voted = true;
    ...
    if (messageBoxClick == false) {
      if (metrics.isInMinimap(x, y)) {
        ...
        int xCell = static_cast<int>(
            xm * (static_cast<float>(map->getW()) / metrics.getMinimapW()));
        int yCell = static_cast<int>(
            map->getH() -
            ym * (static_cast<float>(map->getH()) / metrics.getMinimapH()));

        if (map->isInside(xCell, yCell) &&
            map->isInsideSurface(map->toSurfCoords(Vec2i(xCell, yCell)))) {
          if (gui.isSelectingPos()) {
            ...
            if (!setMarker) {
              ...
              gameCamera.setPos(
                  Vec2f(static_cast<float>(xCell), static_cast<float>(yCell)));
            }
          }
          ...
          if (setMarker) {
            ...
            GameNetworkInterface *gameNetworkInterface =
                NetworkManager::getInstance().getGameNetworkInterface();
            gameNetworkInterface->sendHighlightCellMessage(
                mc.getTargetPos(), mc.getFaction()->getIndex());
          }
          ...
          if (originalIsMarkCellEnabled == true && isMarkCellEnabled == true) {
            ...
            MarkedCell mc(targetPos, world.getThisFaction(),
          ...
          if (originalIsUnMarkCellEnabled == true &&
            ...
            //						if(mapMarkedCellList.find(surfaceCellPos)
            //!= mapMarkedCellList.end()) {
            // MarkedCell mc = mapMarkedCellList[surfaceCellPos];
            // if(mc.getFaction() == world.getThisFaction()) {
            // mapMarkedCellList.erase(surfaceCellPos); 								GameNetworkInterface
      ...
      // graphics panel
        ...
        if (originalIsMarkCellEnabled == true && isMarkCellEnabled == true) {
          ...
          // printf("#2 ADDED in marked list pos [%s] markedCells.size() = "
          // MG_SIZE_T_SPECIFIER
          // "\n",surfaceCellPos.getString().c_str(),mapMarkedCellList.size());
        ...
        if (originalIsUnMarkCellEnabled == true &&
          ...
          //					if(mapMarkedCellList.find(surfaceCellPos)
          //!=
          // mapMarkedCellList.end()) { 						MarkedCell mc
          // =
          // mapMarkedCellList[surfaceCellPos];
          // if(mc.getFaction() == world.getThisFaction()) {
          // mapMarkedCellList.erase(surfaceCellPos); 							GameNetworkInterface
          //*gameNetworkInterface=
          // NetworkManager::getInstance().getGameNetworkInterface();
  ...
  try {
    ...
    if (metrics.isInMinimap(x, y)) {
      ...
      int xCell = static_cast<int>(
          xm * (static_cast<float>(map->getW()) / metrics.getMinimapW()));
      int yCell =
          static_cast<int>(map->getH() - ym * (static_cast<float>(map->getH()) /
                                               metrics.getMinimapH()));
  ...
  try {
    ...
    if (ms->get(mbCenter)) {
      ...
      if (currentCameraFollowUnit == NULL) {
        ...
        int oldy = ((metrics.getScreenH() - oldPos.y) * metrics.getVirtualH() /
                    metrics.getScreenH());
        lastMousePos.x = oldx;
        lastMousePos.y = oldy;
          ...
          if (map->isInside(xCell, yCell) &&
            ...
            if (gui.isSelectingPos()) {
              ...
              if (cameraDragAllowed == true) {
                gameCamera.setPos(Vec2f(static_cast<float>(xCell),
                                        static_cast<float>(yCell)));
              }
            }
          }
        } else {
          bool mouseMoveScrollsWorld =
              Config::getInstance().getBool("MouseMoveScrollsWorld", "true");
...
bool Game::sdlKeyDown(SDL_KeyboardEvent key) {
  ...
  for (int idx = 1; idx <= Selection::maxGroups; idx++) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugSystem,
                               "In [%s::%s Line: %d] keyName [%s] group index "
                               "= %d, key = [%c] [%d]\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__, keyName.c_str(), idx,
                               groupHotKey, groupHotKey);

    // if(SystemFlags::VERBOSE_MODE_ENABLED) printf("input.keysym.mod = %d
    // groupHotKey = %d key = %d (%d) [%s] isgroup =
    // %d\n",key.keysym.mod,groupHotKey,key.keysym.sym,key.keysym.unicode,keyName.c_str(),isKeyPressed(groupHotKey,key));
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("input.keysym.mod = %d groupHotKey = %d key = (%d) [%s] isgroup = "
             "%d\n",
  ...
  try {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(
          ...
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, key.keysym.sym, key.keysym.sym, gameStarted);
    if (gameStarted == false || totalRenderFps <= 0) {
      return;
    }
    if (currentUIState != NULL) {
    ...
    if (chatManager.getEditEnabled() == false) {
      ...
      Config &configKeys = Config::getInstance(
          std::pair<ConfigType, ConfigType>(cfgMainKeys, cfgUserKeys));
      // if(SystemFlags::VERBOSE_MODE_ENABLED) printf ("In [%s::%s Line: %d] key
      // = [%d - %c] pausegame
      ...
      // printf("In game checking keypress for key [%d] camera left
      // [%d]\n",key.keysym.sym,configKeys.getSDLKey("CameraModeLeft"));

      if (isKeyPressed(configKeys.getSDLKey("RenderInGamePerformance"), key,
                       false) == true) {
      ...
      else if (isKeyPressed(configKeys.getSDLKey("RenderNetworkStatus"), key,
                            false) == true) {
      ...
      else if (isKeyPressed(configKeys.getSDLKey("TogglePhotoMode"), key,
                            false) == true) {
      ...
      // Toggle Healthbars
      else if (isKeyPressed(configKeys.getSDLKey("ToggleHealthbars"), key,
                            false) == true) {
        ...
        case (hbvSelected | hbvIfNeeded):
          ...
          break;
        default:
          printf("In [%s::%s Line: %d] Toggle Healthbars Hotkey - Invalid "
                 "Value. Setting to default.\n",
                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                 __LINE__);
          healthbarMode = hbvUndefined;
          break;
      ...
      else if (isKeyPressed(configKeys.getSDLKey("ToggleMusic"), key, false) ==
               true) {
      ...
      else if (isKeyPressed(configKeys.getSDLKey("CameraModeLeft"), key,
                            false) == true) {
      ...
      else if (isKeyPressed(configKeys.getSDLKey("CameraModeRight"), key,
                            false) == true) {
        gameCamera.setMoveX(1);
      ...
      else if (isKeyPressed(configKeys.getSDLKey("CameraModeUp"), key, false) ==
               true) {
      ...
      else if (isKeyPressed(configKeys.getSDLKey("CameraModeDown"), key,
                            false) == true) {
        gameCamera.setMoveZ(-1);
      ...
      else if (isKeyPressed(configKeys.getSDLKey("ResetCameraMode"), key,
                            false) == true) {
        if (currentCameraFollowUnit != NULL) {
          currentCameraFollowUnit = NULL;
        }
      ...
      else if (isKeyPressed(configKeys.getSDLKey("PauseGame"), key, false) ==
               true) {
      ...
      } else if (isKeyPressed(configKeys.getSDLKey("ExtraTeamColorMarker"), key,
                              false) == true) {
      ...
      else if (isKeyPressed(configKeys.getSDLKey("ChangeFontColor"), key,
                            false) == true) {
        gui.switchToNextDisplayColor();
      }
      // increment speed
      // else if(key == configKeys.getCharKey("GameSpeedIncrease")) {
      else if (isKeyPressed(configKeys.getSDLKey("GameSpeedIncrease"), key,
                            false) == true) {
        bool speedChangesAllowed =
            !NetworkManager::getInstance().isNetworkGameWithConnectedClients();
      ...
      else if (isKeyPressed(configKeys.getSDLKey("GameSpeedDecrease"), key,
                            false) == true) {
        bool speedChangesAllowed =
            !NetworkManager::getInstance().isNetworkGameWithConnectedClients();
        if (speedChangesAllowed) {
      ...
      } else if (isKeyPressed(configKeys.getSDLKey("BookmarkAdd"), key,
                              false) == true) {
        startMarkCell();
      } else if (isKeyPressed(configKeys.getSDLKey("BookmarkRemove"), key,
                              false) == true) {
        isUnMarkCellEnabled = true;
      } else if (isKeyPressed(configKeys.getSDLKey("CameraFollowSelectedUnit"),
                              key, false) == true) {
        startCameraFollowUnit();
      }
      // exit
      else if (isKeyPressed(configKeys.getSDLKey("ExitKey"), key, false) ==
               true) {
      ...
      if (gameCamera.getState() != GameCamera::sFree) {
        ...
        if (isKeyPressed(configKeys.getSDLKey("CameraRotateLeft"), key) ==
            true) {
        ...
        else if (isKeyPressed(configKeys.getSDLKey("CameraRotateUp"), key) ==
                 true) {
          gameCamera.setMoveY(1);
        }
        // camera down
        // else if(key == configKeys.getCharKey("CameraRotateDown")) {
        else if (isKeyPressed(configKeys.getSDLKey("CameraRotateDown"), key) ==
                 true) {
          gameCamera.setMoveY(-1);
        }
      }
...
void Game::keyUp(SDL_KeyboardEvent key) {
  ...
  try {
    ...
    if (chatManager.getEditEnabled()) {
      ...
      else if (isKeyPressed(configKeys.getSDLKey("CameraRotateLeft"), key) ==
                   true ||
               isKeyPressed(configKeys.getSDLKey("CameraRotateRight"), key) ==
                   true) {
      ...
      //		key == configKeys.getCharKey("CameraRotateUp")) {
                   ...
                   true ||
               isKeyPressed(configKeys.getSDLKey("CameraRotateUp"), key) ==
                   true) {
...
void Game::DumpCRCWorldLogIfRequired(string fileSuffix) {
  ...
  if (isNetworkGame == true) {
    ...
    if (settings != NULL &&
      ...
      string debugCRCWorldLogFile = Config::getInstance().getString(
          "DebugCRCWorldLogFile", "debugCRCWorld.log");
      ...
      for (unsigned int worldFrameIndex = 0;
        ...
        for (int i = 0; i < world.getFactionCount(); ++i) {
          ...
          std::pair<int, string> details =
              world.getFaction(i)->getCRC_DetailsForWorldFrameIndex(
                  worldFrameIndex);
...
void savePlayerStats(Game *game, Stats &endStats,
  ...
  double elapsedGameMinutes =
      (game->getWorld()->getStats()->getFramesToCalculatePlaytime() /
       GameConstants::updateFps / 60.0);

  for (int factionIndex = 0; factionIndex < game->getWorld()->getFactionCount();
       ++factionIndex) {
    if (factionIndex == game->getWorld()->getThisFactionIndex()) {
      // printf("\nWriting out game stats for Faction Index: %d won status:
      // %d\n",factionIndex,endStats.getVictory(factionIndex));
      if (elapsedGameMinutes >= MIN_PLAY_TIME_MINUTES) {
        int gamesPlayedCount =
            playerStats->getStatAsInt(
                EnumParser<SteamStatName>::getString(games_played).c_str()) +
            1;
        playerStats->setStatAsInt(
            EnumParser<SteamStatName>::getString(games_played).c_str(),
            gamesPlayedCount);

        if (gamesPlayedCount >= 50 &&
            playerStats->isUnlocked(EnumParser<SteamAchievementName>::getString(
                                        ACH_PLAY_FIFTY_GAMES)
                                        .c_str()) == false) {
          playerStats->unlock(
              EnumParser<SteamAchievementName>::getString(ACH_PLAY_FIFTY_GAMES)
                  .c_str());
        }
        if (gamesPlayedCount >= 100 &&
            playerStats->isUnlocked(EnumParser<SteamAchievementName>::getString(
                                        ...
                                        .c_str()) == false) {
          playerStats->unlock(EnumParser<SteamAchievementName>::getString(
                                  ACH_PLAY_ONE_HUNDRED_GAMES)
                                  .c_str());
        }
        if (gamesPlayedCount >= 250 &&
            playerStats->isUnlocked(EnumParser<SteamAchievementName>::getString(
        ...
        if (NetworkManager::getInstance().isNetworkGame()) {
          int networkGamesPlayedCount =
              playerStats->getStatAsInt(
                  EnumParser<SteamStatName>::getString(network_games_played)
                      .c_str()) +
              1;
          playerStats->setStatAsInt(
              EnumParser<SteamStatName>::getString(network_games_played)
                  .c_str(),
              networkGamesPlayedCount);

          if (networkGamesPlayedCount >= 50 &&
              playerStats->isUnlocked(
                  EnumParser<SteamAchievementName>::getString(
          ...
          if (networkGamesPlayedCount >= 100 &&
              playerStats->isUnlocked(
                  EnumParser<SteamAchievementName>::getString(
          ...
          if (networkGamesPlayedCount >= 250 &&
              playerStats->isUnlocked(
                  EnumParser<SteamAchievementName>::getString(
          ...
          if (networkGamesPlayedCount >= 500 &&
              playerStats->isUnlocked(
                  EnumParser<SteamAchievementName>::getString(
          ...
          if (networkGamesPlayedCount > 1000 &&
              playerStats->isUnlocked(
                  EnumParser<SteamAchievementName>::getString(
        ...
        if (endStats.getVictory(factionIndex)) {
          if (playerStats->isUnlocked(
                  EnumParser<SteamAchievementName>::getString(ACH_WIN_ONE_GAME)
                      .c_str()) == false) {
            playerStats->unlock(
                EnumParser<SteamAchievementName>::getString(ACH_WIN_ONE_GAME)
                    .c_str());
          ...
          if (NetworkManager::getInstance().isNetworkGame()) {
            int networkGamesWonCount =
                playerStats->getStatAsInt("network-games-won") + 1;
            playerStats->setStatAsInt("network-games-won",
                                      networkGamesWonCount);
          }

          // printf("\nPlayer won the game with at least 10 minutes of play:
          // %f!\n",elapsedGameMinutes);
        }
      } else {
        // printf("\nPlayer won the game BUT NOT with at least 10 minutes of
        // play: %f!\n",elapsedGameMinutes);
      }
    }
  }
...
void saveStatsToSteam(Game *game, Stats &endStats) {
  ...
  if (steamInstance != NULL) {
    ...
    if (NetworkManager::getInstance().isNetworkGame()) {
      ...
      for (int factionIndex = 0;
        ...
        if (factionIndex == game->getWorld()->getThisFactionIndex()) {
          // printf("\nWriting out game stats for Faction Index:
          // %d!\n",factionIndex);
          if (endStats.getVictory(factionIndex)) {
            steamInstance->setStatAsInt(
                EnumParser<SteamStatName>::getString(stat_online_wins).c_str(),
                steamInstance->getStatAsInt(
                    EnumParser<SteamStatName>::getString(stat_online_wins)
                        .c_str()) +
                    1);
          } else {
            steamInstance->setStatAsInt(
                EnumParser<SteamStatName>::getString(stat_online_loses).c_str(),
                steamInstance->getStatAsInt(
                    EnumParser<SteamStatName>::getString(stat_online_loses)
                        .c_str()) +
                    1);
          }
          steamInstance->setStatAsInt(
              EnumParser<SteamStatName>::getString(stat_online_kills).c_str(),
              steamInstance->getStatAsInt(
                  EnumParser<SteamStatName>::getString(stat_online_kills)
                      .c_str()) +
                  endStats.getKills(factionIndex));
          steamInstance->setStatAsInt(
              EnumParser<SteamStatName>::getString(stat_online_kills_enemy)
                  .c_str(),
              steamInstance->getStatAsInt(
                  EnumParser<SteamStatName>::getString(stat_online_kills_enemy)
                      .c_str()) +
          ...
          steamInstance->setStatAsInt(
              EnumParser<SteamStatName>::getString(stat_online_deaths).c_str(),
              steamInstance->getStatAsInt(
                  EnumParser<SteamStatName>::getString(stat_online_deaths)
                      .c_str()) +
                  endStats.getDeaths(factionIndex));
          steamInstance->setStatAsInt(
              EnumParser<SteamStatName>::getString(stat_online_units).c_str(),
              steamInstance->getStatAsInt(
                  EnumParser<SteamStatName>::getString(stat_online_units)
                      .c_str()) +
                  endStats.getUnitsProduced(factionIndex));
          steamInstance->setStatAsInt(
              EnumParser<SteamStatName>::getString(
                  stat_online_resources_harvested)
                  .c_str(),
              steamInstance->getStatAsInt(EnumParser<SteamStatName>::getString(
                                              stat_online_resources_harvested)
                                              .c_str()) +
                  endStats.getResourcesHarvested(factionIndex));
          ...
          steamInstance->setStatAsDouble(
              ...
              steamInstance->getStatAsDouble(
                  EnumParser<SteamStatName>::getString(
                      ...
                      .c_str()) +
                  getTimeDuationMinutes(endStats.getFramesToCalculatePlaytime(),
  ...
  // We'll clean this up to use an interface that both steam and lcoal comply
  // with so the 'update' logic doesn't need to be duplicated.
  else {
    printf("\n**STEAM NOT detected, writing out NON STEAM end game stats for "
           "player!\n");
    string saveFilePlayerLocalStats = "player-stats.ini";
    if (getGameReadWritePath(GameConstants::path_logs_CacheLookupKey) != "") {
      saveFilePlayerLocalStats =
          getGameReadWritePath(GameConstants::path_logs_CacheLookupKey) +
          saveFilePlayerLocalStats;
    } else {
      string userData = Config::getInstance().getString("UserData_Root", "");
...
void Game::highlightUnit(int unitId, float radius, float thickness,
                         Vec4f color) {
...
void Game::unhighlightUnit(int unitId) { unitHighlightList.erase(unitId); }
...
void Game::render3d() {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
      ...
      chrono.getMillis() > 0)
    SystemFlags::OutputDebug(SystemFlags::debugPerformance,
                             "In [%s::%s Line: %d] renderFps = %d took msecs: "
                             "%lld [loadGameCameraMatrix]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, renderFps,
                             chrono.getMillis());
  if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
          .enabled &&
      chrono.getMillis() > 0)
  ...
  if ((renderExtraTeamColor & renderTeamColorCircleBit) > 0) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
        ...
        chrono.getMillis() > 0)
      SystemFlags::OutputDebug(SystemFlags::debugPerformance,
                               "In [%s::%s Line: %d] renderFps = %d took "
                               "msecs: %lld [renderObjects]\n",
                               extractFileFromDirectoryPath(__FILE__).c_str(),
                               __FUNCTION__, __LINE__, renderFps,
                               chrono.getMillis());
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
...
string Game::getDebugStats(std::map<int, string> &factionDebugInfo) {
  ...
  if (this->masterserverMode == false) {
    ...
    if (world.getMap()->isInsideSurface(
            world.getMap()->toSurfCoords(mouseCellPos)) == true) {
      str += "MouseXY cell coords: " + intToStr(mouseCellPos.x) + "," +
             intToStr(mouseCellPos.y) + "\n";
    }

    str += "PosObjWord: " + intToStr(gui.getPosObjWorld().x) + "," +
  ...
  if (this->masterserverMode == false) {
    ...
    // str+= "Cached surfacedata: " +
    // intToStr(renderer.getCachedSurfaceDataSize())+"\n";
  }

  // intToStr(stats.getFramesToCalculatePlaytime()/GameConstants::updateFps/60
  str += "Time: " + floatToStr(world.getTimeFlow()->getTime(), 2) + " [" +
         floatToStr((double)world.getStats()->getFramesToCalculatePlaytime() /
                        (float)GameConstants::updateFps / 60.0,
                    2) +
         "]\n";

  if (SystemFlags::getThreadedLoggerRunning() == true) {
    str +=
        "Log buffer count: " + intToStr(SystemFlags::getLogEntryBufferCount()) +
        "\n";
  }

  str += "UnitRangeCellsLookupItemCache: " +
         world.getUnitUpdater()->getUnitRangeCellsLookupItemCacheStats() + "\n";
  str += "ExploredCellsLookupItemCache: " +
         world.getExploredCellsLookupItemCacheStats() + "\n";
  str += "FowAlphaCellsLookupItemCache: " +
  ...
  if (selectionType == Config::colorPicking) {
    str += "Color picking used color list size: " +
           intToStr(BaseColorPickEntity::getUsedColorIDListSize()) + "\n";
  }

  // str+= "AllFactionsCacheStats: "			+
  // world.getAllFactionsCacheStats()+"\n"; str+= "AttackWarningCount: " +
  // intToStr(world.getUnitUpdater()->getAttackWarningCount()) + "\n";
  ...
  if (this->masterserverMode == false) {
    ...
    str += "Visible quad:        ";
    for (int i = 0; i < 4; ++i) {
      str += "(" + intToStr(visibleQuad.p[i].x) + "," +
             intToStr(visibleQuad.p[i].y) + ") ";
    }
    //		str+= "\n";
    //		str+= "Visible quad camera: ";
    //		for(int i= 0; i<4; ++i){
    //			str+= "(" + intToStr(visibleQuadCamera.p[i].x) + ","
    //+intToStr(visibleQuadCamera.p[i].y) + ") ";
    //		}
    str += "\n";

    str += "Visible quad area:        " + floatToStr(visibleQuad.area()) + "\n";
    //		str+= "Visible quad camera area: " +
    // floatToStr(visibleQuadCamera.area()) +"\n";

    //		Rect2i boundingRect= visibleQuad.computeBoundingRect();
    //		Rect2i scaledRect= boundingRect/Map::cellScale;
    //		scaledRect.clamp(0, 0, world.getMap()->getSurfaceW()-1,
    // world.getMap()->getSurfaceH()-1);
    // renderer.renderText3D("#1", coreData.getMenuFontNormal3D(), Vec3f(1.0f),
    // scaledRect.p[0].x, scaledRect.p[0].y, false);
    // renderer.renderText3D("#2", coreData.getMenuFontNormal3D(), Vec3f(1.0f),
    // scaledRect.p[1].x, scaledRect.p[1].y, false);
  }
  ...
  for (int i = 0; i < world.getFactionCount(); ++i) {
    ...
    //			factionInfo +=" res: ";
    //			for(int j = 0; j < world.getTechTree()->getResourceTypeCount();
    //++j) { 				factionInfo +=
    // world.getFaction(i)->getResource(j)->getType()->getName()+":"+intToStr(world.getFaction(i)->getResource(j)->getAmount());
    //				factionInfo += " ";
    //			}
    //		}
...
void Game::render2d() {
  ...
  if (renderer.getShowDebugUI() == true) {
    ...
    if (Renderer::renderText3DEnabled == true) {
      ...
      renderer.renderTextShadow(str, coreData.getMenuFontNormal(), fontColor,
                                10, metrics.getVirtualH() - mh - 60, false);
    }

    vector<string> lineTokens;
    Tokenize(str, lineTokens, "\n");
    int fontHeightNormal =
        (Renderer::renderText3DEnabled == true
             ? coreData.getMenuFontNormal3D()->getMetrics()->getHeight("W")
             : coreData.getMenuFontNormal()->getMetrics()->getHeight("W"));
    int fontHeightBig =
        (Renderer::renderText3DEnabled == true
             ? coreData.getMenuFontBig3D()->getMetrics()->getHeight("W")
             : coreData.getMenuFontBig()->getMetrics()->getHeight("W"));
    ...
    for (int i = 0; i < world.getFactionCount(); ++i) {
      ...
      Vec3f playerColor =
          world.getFaction(i)->getTexture()->getPixmapConst()->getPixel3f(0, 0);

      if (Renderer::renderText3DEnabled == true) {
        renderer.renderText3D(
            factionInfo, coreData.getMenuFontBig3D(),
            Vec4f(playerColor.x, playerColor.y, playerColor.z, 1.0), 10,
            // metrics.getVirtualH() - mh - 90 - 280 - (i * 16),
            metrics.getVirtualH() - mh - 60 - playerPosY - fontHeightBig -
                (fontHeightBig * i),
            false);
      } else {
        renderer.renderText(
            factionInfo, coreData.getMenuFontBig(),
            Vec4f(playerColor.x, playerColor.y, playerColor.z, 1.0), 10,
            // metrics.getVirtualH() - mh - 90 - 280 - (i * 16),
            metrics.getVirtualH() - mh - 60 - playerPosY - fontHeightBig -
                (fontHeightBig * i),
  ...
  if (photoModeEnabled == false) {
    ...
    renderer.renderConsole(&console,
                           showFullConsole ? consoleFull : consoleNormal);
  }
    ...
    for (int factionIndex = 0; factionIndex < world.getFactionCount();
      ...
      if (factionIndex != world.getThisFactionIndex()) {
        if (world.getFaction(factionIndex)->getPersonalityType() !=
          ...
          if (factionLostGame(world.getFaction(factionIndex)) == false &&
              world.getFaction(factionIndex)->isAlly(world.getThisFaction()) ==
                  false) {
  ...
  if (scriptManager.getIsGameOver()) {
    ...
    if (wasGameOverAlready == false) {
      ...
      if (this->masterserverMode == true ||
        ...
        scriptManager.onGameOver(
            scriptManager.getPlayerModifiers(world.getThisFactionIndex())
                ->getWinner());

        if (scriptManager.getPlayerModifiers(world.getThisFactionIndex())
                ->getWinner()) {
          showWinMessageBox();
        } else {
          showLoseMessageBox();
...
void Game::incSpeed() {
  ...
  if (this->speed < Config::getInstance().getInt("FastSpeedLoops")) {
    ...
    console.addLine(lang.getString("GameSpeedSet") + " " +
                    ((this->speed == 0)   ? lang.getString("Slow")
                     : (this->speed == 1) ? lang.getString("Normal")
                                          : "x" + intToStr(this->speed)));
  }
}
...
void Game::setPaused(bool value, bool forceAllowPauseStateChange,
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled)
    SystemFlags::OutputDebug(
        ...
        "game.cpp line: %d setPaused value: %d clearCaches: %d "
        "forceAllowPauseStateChange: %d speedChangesAllowed: %d "
        "pausedForJoinGame: %d joinNetworkGame: %d\n",
        __LINE__, value, clearCaches, forceAllowPauseStateChange,
        speedChangesAllowed, pausedForJoinGame, joinNetworkGame);
  // printf("Line: %d setPaused value: %d clearCaches: %d
  // forceAllowPauseStateChange: %d speedChangesAllowed: %d pausedForJoinGame:
  // %d joinNetworkGame:
      ...
      if (clearCaches == true) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
          ...
          SystemFlags::OutputDebug(
              ...
              __LINE__);
        // printf("Line: %d Clear Caches for resume in progress
        // game\n",__LINE__);
...
bool Game::getPaused() {
  ...
  if (speedChangesAllowed) {
    ...
    //		if(mainMessageBox.getEnabled() == true ||
    //errorMessageBox.getEnabled()
    //== true){ 			return true;
    //		}
    if (currentUIState != NULL) {
      return true;
...
void Game::showMessageBox(const string &text, const string &header,
                          bool toggle) {
  if (toggle == false) {
    mainMessageBox.setEnabled(false);
  }
...
void Game::showErrorMessageBox(const string &text, const string &header,
                               bool toggle) {
  if (toggle == false) {
    errorMessageBox.setEnabled(false);
  }
...
void Game::endPerformanceTimer() { captureAvgTestStatus = false; }
...
void Game::consoleAddLine(string line) { console.addLine(line); }
void Game::toggleTeamColorMarker() {
  renderExtraTeamColor++;
  renderExtraTeamColor = renderExtraTeamColor % 4;
}

void Game::addNetworkCommandToReplayList(NetworkCommand *networkCommand,
                                         int worldFrameCount) {
  Config &config = Config::getInstance();
...
void Game::playStaticVideo(const string &playVideo) {
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false &&
    ...
    //		Context *c=
    // GraphicsInterface::getInstance().getCurrentContext(); 		SDL_Surface
    // *screen =
...
bool Game::clientLagHandler(int slotIndex,
                            bool networkPauseGameForLaggedClients) {
  if (networkPauseGameForLaggedClients == true) {
    printf("**WARNING** Detected lag from client: %d "
           "networkPauseGameForLaggedClients: %d\n",
...
string Game::saveGame(string name, const string &path) {
  ...
  if (config.getBool("SaveCommandsForReplay", "false") == true) {
    ...
    for (unsigned int i = 0; i < replayCommandList.size(); ++i) {
      ...
      networkCommandNode->addAttribute("worldFrameCount", intToStr(cmd.first),
                                       mapTagReplacements);
    }
  ...
  //	int totalRenderFps, renderFps, lastRenderFps,
  // avgRenderFps,currentAvgRenderFpsTotal; Uint64 tickCount;
  ...
  gameNode->addAttribute("original_updateFps", intToStr(original_updateFps),
                         mapTagReplacements);
  // int original_cameraFps;
  gameNode->addAttribute("original_cameraFps", intToStr(original_cameraFps),
                         mapTagReplacements);

  // bool captureAvgTestStatus;
  gameNode->addAttribute("captureAvgTestStatus", intToStr(captureAvgTestStatus),
                         mapTagReplacements);
  ...
  gameNode->addAttribute("renderExtraTeamColor", intToStr(renderExtraTeamColor),
                         mapTagReplacements);
  ...
  gameNode->addAttribute("lastMaxUnitCalcTime", intToStr(lastMaxUnitCalcTime),
                         mapTagReplacements);
  ...
  gameNode->addAttribute("disableSpeedChange", intToStr(disableSpeedChange),
                         mapTagReplacements);
  ...
  if (masterserverMode == false) {
    ...
    // menu is already disabled, last rendered screen is still with enabled one.
    // Lets render again:
    render3d();
    render2d();
    Renderer::getInstance().saveScreen(
        jpgFileName, config.getInt("SaveGameScreenshotWidth", "800"),
        config.getInt("SaveGameScreenshotHeight", "600"));
  }
...
void Game::loadGame(string name, Program *programPtr, bool isMasterserverMode,
  ...
  if (joinGameSettings == NULL &&
    ...
    xmlTreeReplay.load(
        name + ".replay",
        Properties::getTagReplacementValues(&mapExtraTagReplacementValues),
        true);
    ...
    if (gameVer != glestVersionString &&
        checkVersionComptability(gameVer, glestVersionString) == false) {
      char szBuf[8096] = "";
      snprintf(szBuf, 8096, lang.getString("SavedGameBadVersion").c_str(),
               gameVer.c_str(), glestVersionString.c_str());
      throw megaglest_runtime_error(szBuf, true);
    }

    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("Found saved game version that matches your application version: "
             "[%s] --> [%s]\n",
             gameVer.c_str(), glestVersionString.c_str());
  ...
  std::map<string, string> mapExtraTagReplacementValues;
  xmlTree.load(
      name, Properties::getTagReplacementValues(&mapExtraTagReplacementValues),
      true);
  if (SystemFlags::VERBOSE_MODE_ENABLED)
  ...
  Lang &lang = Lang::getInstance();
  string gameVer = versionNode->getAttribute("version")->getValue();
  // this is the version check for loading normal save games from
  // menu_state_load_game
  if (gameVer != glestVersionString &&
      (compareMajorMinorVersion(gameVer, lastCompatibleSaveGameVersionString) <
           0 ||
  ...
  //    Vec2i mouseCellPos;
  //	newGame->mouseCellPos =
  // Vec2i::strToVec2(gameNode->getAttribute("mouseCellPos")->getValue());
  // int updateFps, lastUpdateFps, avgUpdateFps; 	int totalRenderFps,
  // renderFps, lastRenderFps, avgRenderFps,currentAvgRenderFpsTotal; Uint64
  // tickCount;
  ...
  // float scrollSpeed;
  //	newGame->scrollSpeed =
  // gameNode->getAttribute("scrollSpeed")->getFloatValue(); bool
  // camLeftButtonDown; bool camRightButtonDown; bool camUpButtonDown; bool
  // camDownButtonDown;
  ...
  // Vec2i lastMousePos;
  //	gameNode->addAttribute("lastMousePos",lastMousePos.getString(),
  // mapTagReplacements); time_t lastRenderLog2d;
  //	gameNode->addAttribute("lastRenderLog2d",intToStr(lastRenderLog2d),
  // mapTagReplacements); DisplayMessageFunction originalDisplayMsgCallback;
  // bool
  // isFirstRender;
  //	gameNode->addAttribute("isFirstRender",intToStr(isFirstRender),
  // mapTagReplacements);
  ...
  // int original_updateFps;
  //	gameNode->addAttribute("original_updateFps",intToStr(original_updateFps),
  // mapTagReplacements); int original_cameraFps;
  //	gameNode->addAttribute("original_cameraFps",intToStr(original_cameraFps),
  // mapTagReplacements);

  // bool captureAvgTestStatus;
  //	gameNode->addAttribute("captureAvgTestStatus",intToStr(captureAvgTestStatus),
  // mapTagReplacements); int updateFpsAvgTest;
  //	gameNode->addAttribute("updateFpsAvgTest",intToStr(updateFpsAvgTest),
  // mapTagReplacements); int renderFpsAvgTest;
  //	gameNode->addAttribute("renderFpsAvgTest",intToStr(renderFpsAvgTest),
  // mapTagReplacements);
  ...
  if (joinGameSettings == NULL) {
    if (gameNode->hasChild("unitHighlightList") == true) {
      ...
      for (unsigned int i = 0; i < infoNodeList.size(); ++i) {
        ...
        info.color =
            Vec4f::strToVec4(infoNode->getAttribute("color")->getValue());
  ...
  // mapTagReplacements);
  //	newGame->gameStarted =
  // gameNode->getAttribute("gameStarted")->getIntValue();
  ...
  // time_t lastNetworkPlayerConnectionCheck;
  // gameNode->addAttribute("lastNetworkPlayerConnectionCheck",intToStr(lastNetworkPlayerConnectionCheck),
  // mapTagReplacements);

  // time_t lastMasterServerGameStatsDump;
  // gameNode->addAttribute("lastMasterServerGameStatsDump",intToStr(lastMasterServerGameStatsDump),
  // mapTagReplacements);

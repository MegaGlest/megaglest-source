...
Game::Game(Program *program, const GameSettings *gameSettings, bool masterserverMode) : ProgramState(program), lastMousePos(0), isFirstRender(true) {
    if (SystemFlags::VERBOSE_MODE_ENABLED) printf("In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
...
string Game::extractScenarioLogoFile(const GameSettings *settings, string &result, bool &loadingImageUsed, Logger *logger, string factionLogoFilter) {
    string scenarioDir = "";
    if (settings->getScenarioDir() != "") {
                ...
                for (unsigned int logoIndex = 0; logoIndex < loadScreenList.size(); ++logoIndex) {
                    ...
                    if (fileExists(senarioLogo) == true) {
                        ...
                        if (checkLogo != NULL) {
                            ...
                            if (SystemFlags::VERBOSE_MODE_ENABLED)
                                printf("Logo info: %d x %d (%d,%d)\n", checkLogo->getPixmapConst()->getW(), checkLogo->getPixmapConst()->getH(),
                            ...
                            } else if (minWidthDifference == bestMinWidthDiff && minHeightDifference < bestMinHeightDiff) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                     ...
                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, settings->getScenarioDir().c_str(),
                                     settings->getScenario().c_str(), scenarioDir.c_str());
    }
    return scenarioDir;
}

string Game::extractFactionLogoFile(bool &loadingImageUsed, const string &factionName, string scenarioDir, const string &techName, Logger *logger,
...
string Game::extractTechLogoFile(string scenarioDir, const string &techName, bool &loadingImageUsed, Logger *logger, const string &factionLogoFilter) {
    string result = "";
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
...
void Game::loadHudTexture(const GameSettings *settings) {
    ...
    for (int i = 0; i < settings->getFactionCount(); ++i) {
        if ((settings->getFactionControl(i) == ctHuman) || (settings->getFactionControl(i) == ctNetwork && settings->getThisFactionIndex() == i)) {
...
string Game::findFactionLogoFile(const GameSettings *settings, Logger *logger, const string &factionLogoFilter) {
    string result = "";
    if (settings == NULL) {
        result = "";
    } else {
        ...
        if (logger != NULL) {
            ...
            if (scenarioName.empty()) {
                ...
                logger->setSubtitle(formatString(mapName) + " - " + formatString(tilesetName) + " - " + formatString(techTree.getTranslatedName(techName)));
            } else {
        ...
        string scenarioDir = extractScenarioLogoFile(settings, result, loadingImageUsed, logger, factionLogoFilter);
        //}
        ...
        if (loadingImageUsed == false) {
            for (int i = 0; i < settings->getFactionCount(); ++i) {
                if (settings->getFactionControl(i) == ctHuman || (settings->getFactionControl(i) == ctNetwork && settings->getThisFactionIndex() == i)) {
                    result = extractFactionLogoFile(loadingImageUsed, settings->getFactionTypeName(i), scenarioDir, techName, logger, factionLogoFilter);
                    break;
                }
            }
        }
        ...
        // if(skipCustomLoadScreen == false && loadingImageUsed == false){
        if (loadingImageUsed == false) {
            result = extractTechLogoFile(scenarioDir, techName, loadingImageUsed, logger, factionLogoFilter);
        }
    }
    return result;
}
        ...
        for (int idx = 0; idx < (int)techPaths.size(); idx++) {
            ...
            if (factions.empty() == false) {
                for (unsigned int factionIdx = 0; factionIdx < (unsigned int)factions.size(); ++factionIdx) {
                    ...
                    string factionLogo = Game::extractFactionLogoFile(loadingImageUsed, currentFactionName_factionPreview, "", techName, NULL,
                                                                      GameConstants::PREVIEW_SCREEN_FILE_FILTER);

                    if (factionLogo == "") {
                        factionLogo = Game::extractFactionLogoFile(loadingImageUsed, currentFactionName_factionPreview, "", techName, NULL,
...
void Game::load(int loadTypes) {
    ...
    if ((loadTypes & lgt_TileSet) == lgt_TileSet) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                                     __LINE__);
        world.loadTileset(config.getPathListForType(ptTilesets, scenarioDir), tilesetName, &checksum, loadedFileList);
    }
    ...
    if ((loadTypes & lgt_TechTree) == lgt_TechTree) {
        ...
        world.loadTech(config.getPathListForType(ptTechs, scenarioDir), techName, factions, &checksum, loadedFileList);
...
void Game::init(bool initForPreviewOnly) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugWorldSynch, "=============================================\n");
    if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugWorldSynch, "Starting framecount: %d\n", world.getFrameCount());
...
void Game::setupPopupMenus(bool checkClientAdminOverrideOnly) {
    ...
    if (role == nrServer) {
        ...
        if (disconnectPlayerPopupMenuIndex == -1) {
            ...
            if (serverInterface != NULL && checkClientAdminOverrideOnly == true) {
                for (int i = 0; i < world.getFactionCount(); ++i) {
                    ...
                    if (slot != NULL && slot->getConnectHasHandshaked() == true && slot->getCurrentFrameCount() <= 0) {
        ...
        if (clientInterface != NULL &&
            (gameSettings.getMasterserver_admin() == clientInterface->getSessionKey() || clientInterface->isMasterServerAdminOverride() == true)) {
            allowAdminMenuItems = true;
        }
    }

    if (checkClientAdminOverrideOnly == false || forceJoinInProgressUpdate == true ||
        ...
        if ((gameSettings.getFlagTypes1() & ft1_allow_team_switching) == ft1_allow_team_switching && world.getThisFaction() != NULL &&
...
void Game::processNetworkSynchChecksIfRequired() {
    ...
    if (isNetworkGame == true && NetworkManager::getInstance().getGameNetworkInterface() != NULL) {
        ...
        if (settings != NULL) {
            ...
            if (isFlagType1BitEnabled(ft1_network_synch_checks) == true || isFlagType1BitEnabled(ft1_network_synch_checks_verbose) == true) {
                ...
                for (int index = 0; index < GameConstants::maxPlayers; ++index) {
                    if (index < world.getFactionCount()) {
                        ...
                        if (settings != NULL) {
                            ...
                            } else if (isFlagType1BitEnabled(ft1_network_synch_checks) == true && world.getFrameCount() % 20 == 0) {
                                faction->addCRC_DetailsForWorldFrame(world.getFrameCount(), role == nrServer);
                            }
                        }
                    } else {
...
void Game::update() {
    try {
        ...
        bool pendingQuitError = (quitPendingIndicator == true || (NetworkManager::getInstance().getGameNetworkInterface() != NULL &&
                                                                  NetworkManager::getInstance().getGameNetworkInterface()->getQuit()));

        if (pendingQuitError == true && (this->masterserverMode == true || (mainMessageBox.getEnabled() == false && errorMessageBox.getEnabled() == false))) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                                         __LINE__);
        ...
        if (this->masterserverMode == false) {
            if (world.getFactionCount() > 0 && world.getThisFaction()->getFirstSwitchTeamVote() != NULL) {
                ...
                if (lang.hasString("AllowPlayerJoinTeam") == true) {
                    snprintf(szBuf, 8096, lang.getString("AllowPlayerJoinTeam").c_str(), settings->getNetworkPlayerName(vote->factionIndex).c_str(),
                             vote->oldTeam, vote->newTeam);
                } else {
                    snprintf(szBuf, 8096,
                             "Allow player [%s] to join your team\n(changing from team# "
                             "%d to team# %d)?",
                             settings->getNetworkPlayerName(vote->factionIndex).c_str(), vote->oldTeam, vote->newTeam);
                }
        ...
        if (role == nrClient && updateLoops == 1 && world.getFrameCount() >= (gameSettings.getNetworkFramePeriod() * 2)) {
            ...
            if (clientInterface != NULL) {
                ...
                int index = ((world.getFrameCount() - (world.getFrameCount() % gameSettings.getNetworkFramePeriod())) / gameSettings.getNetworkFramePeriod()) %
        ...
        if (updateLoops > 0) {
            ...
            do {
                ...
                for (int i = 0; i < updateLoops; ++i) {
                    ...
                    if (commander.hasReplayCommandListForFrame() == false) {
                                ...
                                if (faction->getCpuControl(enableServerControlledAI, isNetworkGame, role) == true &&
                                    ...
                                    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
                                        SystemFlags::OutputDebug(SystemFlags::debugPerformance,
                                                                 ...
                                                                 "updates]\n",
                                                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, i, j,
                                                                 world.getFactionCount(), chrono.getMillis());
                        ...
                        if (lastReplaySecond < chronoReplay.getSeconds()) {
                            ...
                            if (Renderer::renderText3DEnabled) {
                                ...
                                renderer.renderText3D(text, font, Vec3f(1.f, 1.f, 0.f), renderX, renderY, false);
                            } else {
                                ...
                                renderer.renderText(text, font, Vec3f(1.f, 1.f, 0.f), renderX, renderY, true);
                            }
                    ...
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
                        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s] Line: %d took msecs: %lld [AI updates]\n",
                                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, chrono.getMillis());
                    ...
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
                        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s] Line: %d took msecs: %lld [world update i = %d]\n",
                    ...
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
                        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s] Line: %d took msecs: %lld [gui updating i = %d]\n",
                                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, chrono.getMillis(), i);
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0) chrono.start();
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
            SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s] Line: %d took msecs: %lld [chatManager.updateNetwork]\n",
                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, chrono.getMillis());
        if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0) chrono.start();
        ...
        if (NetworkManager::getInstance().getGameNetworkInterface() != NULL && NetworkManager::getInstance().getGameNetworkInterface()->getQuit() &&
        ...
        if (role == nrServer) {
            if (this->networkPauseGameForLaggedClientsRequested == true) {
                ...
                if (getPaused() == false) {
                    printf("[CMDR] Pausing game for lagging client(s), current world frame "
                           "[%d]\n",
                           world.getFrameCount());
                ...
                if (getPaused() == true) {
                    printf("[CMDR] Resuming game after Pause for lagging client(s), current "
                           "world frame [%d]\n",
                           world.getFrameCount());
            ...
            if (server->getPauseForInGameConnection() == true) {
                ...
                if (pausedForJoinGame == false || clientNeedsGameSetup == true) {
                    ...
                    for (int i = 0; i < world.getFactionCount(); ++i) {
                        ...
                        if (faction->getControlType() != ctNetwork && faction->getControlType() != ctHuman &&
                ...
                for (int i = 0; i < world.getFactionCount(); ++i) {
                    ...
                    if (slot != NULL && slot->getJoinGameInProgress() == true) {
                        ...
                        // pauseAndSaveGameForNewClient = true;
                    } else if ((slot == NULL || slot->isConnected() == false) && this->gameSettings.getFactionControl(i) == ctNetwork &&
                               aiInterfaces[i] == NULL) {
                        safeMutex.ReleaseLock();
                ...
                if (pauseAndSaveGameForNewClient == true && pausedForJoinGame == false && pauseRequestSent == false) {
            ...
            if (pausedForJoinGame == true) {
                ...
                if (server->getUnPauseForInGameConnection() == true) {
                    ...
                    for (int i = 0; i < world.getFactionCount(); ++i) {
                        ...
                        MutexSafeWrapper safeMutex(server->getSlotMutex(faction->getStartLocationIndex()), CODE_AT_LINE);
                        ConnectionSlot *slot = server->getSlot(faction->getStartLocationIndex(), false);
                        if (slot != NULL && slot->getJoinGameInProgress() == true && slot->getStartInGameConnectionLaunch() == true &&
                    ...
                    if (saveNetworkGame == true) {
                        ...
                        bool compressed_result = compressFileToZIPFile(file, saveGameFileCompressed);
                        if (SystemFlags::VERBOSE_MODE_ENABLED)
                        ...
                        for (int i = 0; i < world.getFactionCount(); ++i) {
                            ...
                            MutexSafeWrapper safeMutex(server->getSlotMutex(faction->getStartLocationIndex()), CODE_AT_LINE);
                            ConnectionSlot *slot = server->getSlot(faction->getStartLocationIndex(), false);
                            if (slot != NULL && slot->getJoinGameInProgress() == true && slot->getSentSavedGameInfo() == false) {
                                safeMutex.ReleaseLock();
...
string Game::getGamePerformanceCounts(bool displayWarnings) const {
    ...
    string result = "";
    for (std::map<string, int64>::const_iterator iterMap = gamePerformanceCounts.begin(); iterMap != gamePerformanceCounts.end(); ++iterMap) {
...
bool Game::switchSetupForSlots(ServerInterface *&serverInterface, int startIndex, int endIndex, bool onlyNetworkUnassigned) {
    ...
    for (int i = startIndex; i < endIndex; ++i) {
        if (switchSetupRequests[i] != NULL) {
            ...
            if (gameSettings.getFactionControl(i) == ctNetwork || gameSettings.getFactionControl(i) == ctNetworkUnassigned ||
...
void Game::updateNetworkMarkedCells() {
    try {
        ...
        if (gameNetworkInterface != NULL && gameNetworkInterface->getMarkedCellList(false).empty() == false) {
...
void Game::updateNetworkUnMarkedCells() {
    try {
        ...
        if (gameNetworkInterface != NULL && gameNetworkInterface->getUnMarkedCellList(false).empty() == false) {
            // Lang &lang= Lang::getInstance();
...
void Game::updateNetworkHighligtedCells() {
    try {
        ...
        if (gameNetworkInterface != NULL && gameNetworkInterface->getHighlightedCellList(false).empty() == false) {
            // Lang &lang= Lang::getInstance();
...
void Game::addOrReplaceInHighlightedCells(MarkedCell mc) {
    ...
    if (this->masterserverMode == false) {
        ...
        if ((faction == NULL) || (faction->getTeam() == getWorld()->getThisFaction()->getTeam())) {
    ...
    if (role == nrServer && isNetworkGame == true &&
        ...
        for (int i = 0; i < world.getFactionCount(); ++i) {
            ...
            if (faction->getFactionDisconnectHandled() == false &&
                (faction->getControlType() == ctNetwork || faction->getControlType() == ctNetworkCpuEasy || faction->getControlType() == ctNetworkCpu ||
                 faction->getControlType() == ctNetworkCpuUltra || faction->getControlType() == ctNetworkCpuMega)) {
                if (aiInterfaces[i] == NULL && server->isClientConnected(faction->getStartLocationIndex()) == false) {
                    ...
                    for (unsigned int j = 0; j < (unsigned int)languageList.size(); ++j) {
                        if (isPlayerObserver == false) {
                            string msg = "Player #%d [%s] has disconnected, switching player to AI "
                                         "mode!";
                            ...
                            string msg = "Player #%d [%s] has disconnected, but player was only an "
                                         "observer!";
...
void Game::renderWorker() {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s Line: %d] renderFps = %d took msecs: %d [render3d]\n",
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s Line: %d] renderFps = %d took msecs: %d [render2d]\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, renderFps, chrono.getMillis());
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0) chrono.start();

    Renderer::getInstance().swapBuffers();
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s Line: %d] renderFps = %d took msecs: %d [swap buffers]\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, renderFps, chrono.getMillis());
}
...
void Game::tryPauseToggle(bool pauseValue) {
    ...
    if (role == nrServer) {
        ...
        if (clientInterface != NULL && gameSettings.getMasterserver_admin() == clientInterface->getSessionKey()) {
            allowAdminMenuItems = true;
        }
    }
...
void Game::startMarkCell() {
    ...
    if (world.getThisFaction() != NULL) {
        for (std::map<Vec2i, MarkedCell>::iterator iterMap = mapMarkedCellList.begin(); iterMap != mapMarkedCellList.end(); ++iterMap) {
...
void Game::addCellMarker(Vec2i cellPos, MarkedCell cellData) {
    ...
    gameNetworkInterface->sendMarkCellMessage(cellData.getTargetPos(), factionIndex, cellData.getNote(), playerIndex);
...
void Game::mouseDownLeft(int x, int y) {
    ...
    try {
        ...
        if (popupMenu.mouseClick(x, y)) {
            ...
            if (result.first == exitGamePopupMenuIndex) {
                ...
                for (unsigned int i = 0; i < (unsigned int)world.getFactionCount(); ++i) {
                    ...
                    if (faction->getPersonalityType() != fpt_Observer && uniqueTeamNumbersUsed.find(faction->getTeam()) == uniqueTeamNumbersUsed.end()) {
                        uniqueTeamNumbersUsed[faction->getTeam()] = true;
                    }

                    if (faction->getPersonalityType() != fpt_Observer && world.getThisFaction()->getIndex() != faction->getIndex() &&
                        world.getThisFaction()->getTeam() != faction->getTeam()) {
                        char szBuf[8096] = "";
                        if (lang.hasString("JoinPlayerTeam") == true) {
                            snprintf(szBuf, 8096, ("  " + lang.getString("JoinPlayerTeam") + "  ").c_str(), faction->getIndex(),
                                     this->gameSettings.getNetworkPlayerName(i).c_str(), faction->getTeam());
                        } else {
                            snprintf(szBuf, 8096, "  Join player #%d - %s on Team: %d  ", faction->getIndex(),
                                     this->gameSettings.getNetworkPlayerName(i).c_str(), faction->getTeam());
                        }
                ...
                for (unsigned int i = 0; i < (unsigned int)world.getFactionCount(); ++i) {
                    ...
                    if (isSlotJoinInProgressClient == true ||
                        (faction->getPersonalityType() != fpt_Observer && world.getThisFaction()->getIndex() != faction->getIndex() &&
                         faction->getControlType() == ctNetwork && this->gameSettings.getNetworkPlayerStatuses(i) != npst_Disconnected)) {
                        char szBuf[8096] = "";
                        if (lang.hasString("DisconnectNetorkPlayerIndex") == true) {
                            snprintf(szBuf, 8096, ("  " + lang.getString("DisconnectNetorkPlayerIndex") + "  ").c_str(), faction->getIndex() + 1,
                                     this->gameSettings.getNetworkPlayerName(i).c_str());
                        } else {
                            snprintf(szBuf, 8096, "  Disconnect player #%d - %s:  ", faction->getIndex() + 1,
                                     this->gameSettings.getNetworkPlayerName(i).c_str());
                        }
            ...
            default:
                ...
                if (lang.hasString("DisconnectNetorkPlayerIndexConfirm") == true) {
                    snprintf(szBuf, 8096, ("  " + lang.getString("DisconnectNetorkPlayerIndexConfirm") + "  ").c_str(), factionIndex + 1,
                             settings->getNetworkPlayerName(factionIndex).c_str());
                } else {
                    snprintf(szBuf, 8096, "  Confirm disconnection for player #%d - %s?  ", factionIndex + 1,
                             settings->getNetworkPlayerName(factionIndex).c_str());
                }
                ...
                if (gameNetworkInterface != NULL) {
                    ...
                    for (unsigned int i = 0; i < (unsigned int)languageList.size(); ++i) {
                        ...
                        if (lang.hasString("DisconnectNetorkPlayerIndexConfirmed", languageList[i]) == true) {
                            snprintf(szMsg, 8096, lang.getString("DisconnectNetorkPlayerIndexConfirmed", languageList[i]).c_str(), factionIndex + 1,
                                     settings->getNetworkPlayerName(factionIndex).c_str());
                        } else {
                            snprintf(szMsg, 8096, "Notice - Admin is warning to disconnect player #%d - %s!", factionIndex + 1,
                                     settings->getNetworkPlayerName(factionIndex).c_str());
                        }
        ...
        if (mainMessageBox.getEnabled() == false && errorMessageBox.getEnabled() == false && scriptManager.getMessageBox()->getEnabled()) {
            int button = 0;
    ...
    try {
        ...
        if (chatManager.getEditEnabled() == false) {
            ...
            // else if(key == configKeys.getCharKey("TogglePhotoMode")) {
                ...
                if (photoModeEnabled == true && this->gameSettings.isNetworkGame() == false) {
...
void Game::keyUp(SDL_KeyboardEvent key) {
    ...
    try {
        ...
        if (chatManager.getEditEnabled()) {
            ...
            else if (isKeyPressed(configKeys.getSDLKey("CameraRotateDown"), key) == true || isKeyPressed(configKeys.getSDLKey("CameraRotateUp"), key) == true) {
...
void Game::DumpCRCWorldLogIfRequired(string fileSuffix) {
    ...
    if (isNetworkGame == true) {
        ...
        GameSettings *settings = world.getGameSettingsPtr();
        if (settings != NULL && (isFlagType1BitEnabled(ft1_network_synch_checks_verbose) == true || isFlagType1BitEnabled(ft1_network_synch_checks) == true)) {
...
void savePlayerStats(Game *game, Stats &endStats, PlayerAchievementsInterface *playerStats) {
    ...
    for (int factionIndex = 0; factionIndex < game->getWorld()->getFactionCount(); ++factionIndex) {
        if (factionIndex == game->getWorld()->getThisFactionIndex()) {
            ...
            if (elapsedGameMinutes >= MIN_PLAY_TIME_MINUTES) {
                ...
                if (gamesPlayedCount >= 100 &&
                    playerStats->isUnlocked(EnumParser<SteamAchievementName>::getString(ACH_PLAY_ONE_HUNDRED_GAMES).c_str()) == false) {
                    playerStats->unlock(EnumParser<SteamAchievementName>::getString(ACH_PLAY_ONE_HUNDRED_GAMES).c_str());
                }
                if (gamesPlayedCount >= 250 &&
                    playerStats->isUnlocked(EnumParser<SteamAchievementName>::getString(ACH_PLAY_TWO_HUNDRED_FIFTY_GAMES).c_str()) == false) {
                ...
                if (gamesPlayedCount >= 500 &&
                    playerStats->isUnlocked(EnumParser<SteamAchievementName>::getString(ACH_PLAY_FIVE_HUNDRED_GAMES).c_str()) == false) {
                    playerStats->unlock(EnumParser<SteamAchievementName>::getString(ACH_PLAY_FIVE_HUNDRED_GAMES).c_str());
                }
                if (gamesPlayedCount > 1000 &&
                    playerStats->isUnlocked(EnumParser<SteamAchievementName>::getString(ACH_PLAY_OVER_THOUSAND_GAMES).c_str()) == false) {
                ...
                if (NetworkManager::getInstance().isNetworkGame()) {
                    ...
                    if (networkGamesPlayedCount >= 50 &&
                        playerStats->isUnlocked(EnumParser<SteamAchievementName>::getString(ACH_PLAY_FIFTY_GAMES_ONLINE).c_str()) == false) {
                    ...
                    if (networkGamesPlayedCount >= 100 &&
                        playerStats->isUnlocked(EnumParser<SteamAchievementName>::getString(ACH_PLAY_ONE_HUNDRED_GAMES_ONLINE).c_str()) == false) {
                    ...
                    if (networkGamesPlayedCount >= 250 &&
                        playerStats->isUnlocked(EnumParser<SteamAchievementName>::getString(ACH_PLAY_TWO_HUNDRED_FIFTY_GAMES_ONLINE).c_str()) == false) {
                    ...
                    if (networkGamesPlayedCount >= 500 &&
                        playerStats->isUnlocked(EnumParser<SteamAchievementName>::getString(ACH_PLAY_FIVE_HUNDRED_GAMES_ONLINE).c_str()) == false) {
                    ...
                    if (networkGamesPlayedCount > 1000 &&
                        playerStats->isUnlocked(EnumParser<SteamAchievementName>::getString(ACH_PLAY_OVER_THOUSAND_GAMES_ONLINE).c_str()) == false) {
                ...
                if (factionIndex == game->getWorld()->getThisFactionIndex()) {
                    ...
                    if (endStats.getVictory(factionIndex)) {
                        steamInstance->setStatAsInt(EnumParser<SteamStatName>::getString(stat_online_wins).c_str(),
                                                    steamInstance->getStatAsInt(EnumParser<SteamStatName>::getString(stat_online_wins).c_str()) + 1);
                    } else {
                        steamInstance->setStatAsInt(EnumParser<SteamStatName>::getString(stat_online_loses).c_str(),
                                                    steamInstance->getStatAsInt(EnumParser<SteamStatName>::getString(stat_online_loses).c_str()) + 1);
                    }
                    ...
                    steamInstance->setStatAsInt(EnumParser<SteamStatName>::getString(stat_online_kills_enemy).c_str(),
                                                steamInstance->getStatAsInt(EnumParser<SteamStatName>::getString(stat_online_kills_enemy).c_str()) +
                    ...
                    steamInstance->setStatAsInt(EnumParser<SteamStatName>::getString(stat_online_resources_harvested).c_str(),
                                                steamInstance->getStatAsInt(EnumParser<SteamStatName>::getString(stat_online_resources_harvested).c_str()) +
                                                    endStats.getResourcesHarvested(factionIndex));
                    if (endStats.getPlayerLeftBeforeEnd(factionIndex)) {
                        steamInstance->setStatAsInt(EnumParser<SteamStatName>::getString(stat_online_quit_before_end).c_str(),
                                                    steamInstance->getStatAsInt(EnumParser<SteamStatName>::getString(stat_online_quit_before_end).c_str()) + 1);
                    }
                    steamInstance->setStatAsDouble(EnumParser<SteamStatName>::getString(stat_online_minutes_played).c_str(),
                                                   steamInstance->getStatAsDouble(EnumParser<SteamStatName>::getString(stat_online_minutes_played).c_str()) +
                                                       getTimeDuationMinutes(endStats.getFramesToCalculatePlaytime(), GameConstants::updateFps));
                }
            }
        }
...
void Game::exitGameState(Program *program, Stats &endStats) {
    ...
    if ((game != NULL && game->isMasterserverMode() == true) || Config::getInstance().getBool("AutoTest") == true) {
...
void Game::render3d() {
    ...
    renderer.reset3d();
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [reset3d]\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, renderFps, chrono.getMillis());
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0) chrono.start();
    ...
    renderer.clearBuffers();
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s] Line: %d renderFps = %d took msecs: %lld\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, renderFps, chrono.getMillis());
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0) chrono.start();
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [renderUnits]\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, renderFps, chrono.getMillis());
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0) chrono.start();
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s Line: %d] renderFps = %d took msecs: %lld [renderWater]\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, renderFps, chrono.getMillis());
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0) chrono.start();
...
string Game::getDebugStats(std::map<int, string> &factionDebugInfo) {
    ...
    if (this->masterserverMode == false) {
        str +=
            "GameCamera pos: " + floatToStr(gameCamera.getPos().x) + "," + floatToStr(gameCamera.getPos().y) + "," + floatToStr(gameCamera.getPos().z) + "\n";
    ...
    str += "Time: " + floatToStr(world.getTimeFlow()->getTime(), 2) + " [" +
           floatToStr((double)world.getStats()->getFramesToCalculatePlaytime() / (float)GameConstants::updateFps / 60.0, 2) + "]\n";
...
void Game::render2d() {
    ...
    // script message box
    if (mainMessageBox.getEnabled() == false && errorMessageBox.getEnabled() == false && scriptManager.getMessageBoxEnabled()) {
    ...
    if (!scriptManager.getDisplayText().empty() && !scriptManager.getMessageBoxEnabled()) {
        ...
        if (Renderer::renderText3DEnabled == true) {
            renderer.renderText3D(scriptManager.getDisplayText(), coreData.getMenuFontNormal3D(), Vec3f(fontColor.x, fontColor.y, fontColor.z), 200, 660,
                                  false);
        } else {
            renderer.renderText(scriptManager.getDisplayText(), coreData.getMenuFontNormal(), Vec3f(fontColor.x, fontColor.y, fontColor.z), 200, 660, false);
        }
    }
    ...
    if (renderer.getShowDebugUI() == true || (perfLogging == true && difftime((long int)time(NULL), lastRenderLog2d) >= 1)) {
        str = getDebugStats(factionDebugInfo);
    }
    ...
    if (renderer.getShowDebugUI() == true) {
        ...
        if (Renderer::renderText3DEnabled == true) {
            renderer.renderTextShadow3D(str, coreData.getMenuFontNormal3D(), fontColor, 10, metrics.getVirtualH() - mh - 60, false);
        } else {
            renderer.renderTextShadow(str, coreData.getMenuFontNormal(), fontColor, 10, metrics.getVirtualH() - mh - 60, false);
        }

        vector<string> lineTokens;
        Tokenize(str, lineTokens, "\n");
        int fontHeightNormal = (Renderer::renderText3DEnabled == true ? coreData.getMenuFontNormal3D()->getMetrics()->getHeight("W")
                                                                      : coreData.getMenuFontNormal()->getMetrics()->getHeight("W"));
        int fontHeightBig = (Renderer::renderText3DEnabled == true ? coreData.getMenuFontBig3D()->getMetrics()->getHeight("W")
        ...
        for (int i = 0; i < world.getFactionCount(); ++i) {
            ...
            if (Renderer::renderText3DEnabled == true) {
                renderer.renderText3D(factionInfo, coreData.getMenuFontBig3D(), Vec4f(playerColor.x, playerColor.y, playerColor.z, 1.0), 10,
                                      // metrics.getVirtualH() - mh - 90 - 280 - (i * 16),
                                      metrics.getVirtualH() - mh - 60 - playerPosY - fontHeightBig - (fontHeightBig * i), false);
            } else {
                renderer.renderText(factionInfo, coreData.getMenuFontBig(), Vec4f(playerColor.x, playerColor.y, playerColor.z, 1.0), 10,
                                    // metrics.getVirtualH() - mh - 90 - 280 - (i * 16),
                                    metrics.getVirtualH() - mh - 60 - playerPosY - fontHeightBig - (fontHeightBig * i), false);
            }
        }
    ...
    if (renderNetworkStatus == true) {
        if (NetworkManager::getInstance().getGameNetworkInterface() != NULL) {
            ...
            if (Renderer::renderText3DEnabled == true) {
                renderer.renderTextShadow3D(NetworkManager::getInstance().getGameNetworkInterface()->getNetworkStatus(), coreData.getMenuFontNormal3D(),
                                            fontColor, mx + mw + 5, metrics.getVirtualH() - 30 - 20, false);
            } else {
                renderer.renderTextShadow(NetworkManager::getInstance().getGameNetworkInterface()->getNetworkStatus(), coreData.getMenuFontNormal(), fontColor,
                                          mx + mw + 5, metrics.getVirtualH() - 30 - 20, false);
            }
        }
    }
...
void Game::setEndGameTeamWinnersAndLosers() {
    ...
    if (teamsAlive.size() <= 1) {
        ...
        if (this->gameSettings.isNetworkGame() == false || this->gameSettings.getEnableObserverModeAtEndGame() == true) {
    ...
    for (int factionIndex = 0; factionIndex < world.getFactionCount(); ++factionIndex) {
        if (factionIndex != world.getThisFactionIndex()) {
            if (factionLostGame(world.getFaction(factionIndex)) == false) {
                teamsAlive[world.getFaction(factionIndex)->getTeam()] = teamsAlive[world.getFaction(factionIndex)->getTeam()] + 1;
            }
        }
    }
    // did some team win
...
void Game::checkWinnerStandardHeadlessOrObserver() {
    ...
    if (teamsAlive.size() <= 1) {
        ...
        for (int factionIndex = 0; factionIndex < world.getFactionCount(); ++factionIndex) {
            ...
            if (factionIndex != world.getThisFactionIndex() && teamsAlive.find(faction->getTeam()) != teamsAlive.end()) {
                world.getStats()->setVictorious(factionIndex);
                if (this->masterserverMode == true) {
                    printf("Player: %s is on the winning team #: %d\n", this->gameSettings.getNetworkPlayerName(factionIndex).c_str(), faction->getTeam());
                }
            }
        }
...
void Game::checkWinnerStandardPlayer() {
    ...
    if (factionLostGame(world.getThisFaction()) == true) {
        ...
        if (isFlagType1BitEnabled(ft1_allow_shared_team_units) == true) {
            ...
            for (int factionIndex = 0; factionIndex < world.getFactionCount(); ++factionIndex) {
                if (world.getFaction(factionIndex)->getPersonalityType() != fpt_Observer) {
                    if (world.getFaction(factionIndex)->isAlly(world.getThisFaction()) == true && factionLostGame(world.getFaction(factionIndex)) == false) {
        ...
        if (playerLostGame == true) {
            ...
            for (int factionIndex = 0; factionIndex < world.getFactionCount(); ++factionIndex) {
                if (world.getFaction(factionIndex)->getPersonalityType() != fpt_Observer) {
                    if (world.getFaction(factionIndex)->isAlly(world.getThisFaction()) == false && factionLostGame(world.getFaction(factionIndex)) == false) {
                        world.getStats()->setVictorious(factionIndex);
                    }
                }
            }
    ...
    if (lose == false) {
        ...
        for (int factionIndex = 0; factionIndex < world.getFactionCount(); ++factionIndex) {
            if (factionIndex != world.getThisFactionIndex()) {
                if (world.getFaction(factionIndex)->getPersonalityType() != fpt_Observer) {
                    if (factionLostGame(world.getFaction(factionIndex)) == false && world.getFaction(factionIndex)->isAlly(world.getThisFaction()) == false) {
                        win = false;
                    }
                }
            }
...
void Game::setPaused(bool value, bool forceAllowPauseStateChange, bool clearCaches, bool joinNetworkGame) {
    ...
    if (forceAllowPauseStateChange == true || speedChangesAllowed == true) {
        ...
        if (pausedForJoinGame == true && joinNetworkGame == false && networkManager.getNetworkRole() == nrServer) {
            ServerInterface *server = NetworkManager::getInstance().getServerInterface();
            Lang &lang = Lang::getInstance();
            const vector<string> languageList = this->gameSettings.getUniqueNetworkPlayerLanguages();
        ...
        if (value == false) {
            ...
            if (clearCaches == true) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugWorldSynch, "game.cpp line: %d Clear Caches for resume in progress game\n", __LINE__);
            ...
            if (networkManager.getNetworkRole() == nrClient && wasPausedForJoinGame == true) {
...
void Game::playStaticVideo(const string &playVideo) {
    if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false && ::Shared::Graphics::VideoPlayer::hasBackEndVideoPlayer() == true) {
...
void Game::playStreamingVideo(const string &playVideo) {
    if (videoPlayer == NULL) {
        if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false && ::Shared::Graphics::VideoPlayer::hasBackEndVideoPlayer() == true) {
            ...
            videoPlayer = new ::Shared::Graphics::VideoPlayer(&Renderer::getInstance(), playVideo, "", window, 0, 0, screen->w, screen->h,
                                                              screen->format->BitsPerPixel, false, SystemFlags::VERBOSE_MODE_ENABLED);
        }
    } else {
        if (videoPlayer->isPlaying() == false) {
            delete videoPlayer;
...
string Game::saveGame(string name, const string &path) {
    ...
    for (std::map<int, HighlightSpecialUnitInfo>::iterator iterMap = unitHighlightList.begin(); iterMap != unitHighlightList.end(); ++iterMap) {
...
void Game::loadGame(string name, Program *programPtr, bool isMasterserverMode, const GameSettings *joinGameSettings) {
    ...
    if (joinGameSettings == NULL && config.getBool("SaveCommandsForReplay", "false") == true) {
        ...
        if (newGameSettingsReplay.getScenarioDir() != "" && fileExists(newGameSettingsReplay.getScenarioDir()) == false) {
            newGameSettingsReplay.setScenarioDir(
                Scenario::getScenarioPath(Config::getInstance().getPathListForType(ptScenarios), newGameSettingsReplay.getScenario()));
    ...
    if (gameVer != glestVersionString &&
        (compareMajorMinorVersion(gameVer, lastCompatibleSaveGameVersionString) < 0 || compareMajorMinorVersion(glestVersionString, gameVer) < 0)) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, lang.getString("SavedGameBadVersion").c_str(), gameVer.c_str(), glestVersionString.c_str());
        throw megaglest_runtime_error(szBuf, true);
    }
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("Found saved game version that matches your application version: [%s] "
               "--> [%s]\n",
               gameVer.c_str(), glestVersionString.c_str());

  ...
  try {
      ...
      if (clientInterface != NULL) {
        ...
        if (framesNeededToWaitForServerMessage[index] == -1) {
          ...
          if (timeClientWaitedForLastMessage > 0) {
            ...
            if (SystemFlags::VERBOSE_MODE_ENABLED)
              printf(
                  "ClienttimeClientWaitedForLastMessage:%d ms  which is %d "
                  "frames \n",
                  (int)timeClientWaitedForLastMessage,
                  framesNeededToWaitForServerMessage[index]);
          } else {
    ...
    if (role == nrServer) {
      if (this->networkPauseGameForLaggedClientsRequested == true) {
        ...
        if (getPaused() == false) {
          printf(
              "[CMDR] Pausing game for lagging client(s), current world frame "
              "[%d]\n",
              world.getFrameCount());
        ...
        if (getPaused() == true) {
          printf(
              "[CMDR] Resuming game after Pause for lagging client(s), current "
              "world frame [%d]\n",
              world.getFrameCount());
        ...
        if (aiInterfaces[i] == NULL &&
          ...
          for (unsigned int j = 0; j < (unsigned int)languageList.size(); ++j) {
            if (isPlayerObserver == false) {
              string msg =
                  "Player #%d [%s] has disconnected, switching player to AI "
                  "mode!";
              ...
              string msg =
                  "Player #%d [%s] has disconnected, but player was only an "
                  "observer!";
...
bool Game::sdlKeyDown(SDL_KeyboardEvent key) {
  ...
  for (int idx = 1; idx <= Selection::maxGroups; idx++) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "input.keysym.mod = %d groupHotKey = %d key = (%d) [%s] isgroup = "
          "%d\n",
    ...
    if (chatManager.getEditEnabled() == false) {
      ...
      // Toggle Healthbars
        ...
        switch (healthbarMode) {
          ...
          default:
            printf(
                "In [%s::%s Line: %d] Toggle Healthbars Hotkey - Invalid "
...
string Game::getDebugStats(std::map<int, string> &factionDebugInfo) {
  ...
  for (int i = 0; i < world.getFactionCount(); ++i) {
    ...
    //			factionInfo +=" res: ";
    //			for(int j = 0; j <
    // world.getTechTree()->getResourceTypeCount(); ++j) {
    // factionInfo +=
...
bool Game::getPaused() {
  ...
  if (speedChangesAllowed) {
    ...
    // errorMessageBox.getEnabled() == true){ 			return true;
    //		}
    if (currentUIState != NULL) {
      return true;
    }
...
void Game::showLoseMessageBox() {
  ...
  if (networkManager.isNetworkGame() == true &&
    ...
    showMessageBox(
        lang.getString("YouLose") + " " + lang.getString("ExitBattleQuestion"),
        lang.getString("BattleOver"), false);
  }
}
...
void Game::showWinMessageBox() {
  ...
  if (this->masterserverMode == true ||
      world.getThisFaction()->getPersonalityType() == fpt_Observer) {
    showMessageBox(
        lang.getString("GameOver") + " " + lang.getString("ExitBattleQuestion"),
        lang.getString("BattleOver"), false);
  } else {
    showMessageBox(
        lang.getString("YouWin") + " " + lang.getString("ExitBattleQuestion"),
        lang.getString("BattleOver"), false);
  }
}
...
bool Game::clientLagHandler(int slotIndex,
  ...
  if (networkPauseGameForLaggedClients == true) {
    printf(
        "**WARNING** Detected lag from client: %d "
...
void Game::loadGame(string name, Program *programPtr, bool isMasterserverMode,
  ...
  if (joinGameSettings == NULL &&
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "Found saved game version that matches your application version: "
          "[%s] --> [%s]\n",
          gameVer.c_str(), glestVersionString.c_str());
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf(
        "Found saved game version that matches your application version: [%s] "
        "--> [%s]\n",
        gameVer.c_str(), glestVersionString.c_str());

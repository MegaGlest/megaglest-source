...
template <typename T> class EnumParser {
  ...
  public:
    ...
    static string getString(const T &value) {
        static EnumParser<T> parser;
        for (enumMapTypeIter iValue = parser.enumMap.begin(); iValue != parser.enumMap.end(); ++iValue) {
...
enum PathFinderType { pfBasic };

enum TravelState { tsArrived, tsMoving, tsBlocked, tsImpossible };
...
enum NetworkRole { nrServer, nrClient, nrIdle };
...
enum PathType { ptMaps, ptScenarios, ptTechs, ptTilesets, ptTutorials };
...
struct CardinalDir {
  public:
    ...
    void operator++() { value = static_cast<Enum>((value + 1) % 4); }
    void operator--() { // mod with negative numbers is a 'grey area', hence the
                        // +3 rather than -1

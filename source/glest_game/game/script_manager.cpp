...
ScriptManagerMessage::ScriptManagerMessage(const string &textIn, const string &headerIn, int factionIndex, int teamIndex, bool messageNotTranslated)
...
void ScriptManager::init(World *world, GameCamera *gameCamera, const XmlNode *rootNode) {
    ...
    if (scenario->getExternalScript() == nullptr) {
        ...
        luaScript.loadCode(scenario->getExternalScript()->getCode(), scenario->getExternalScript()->getName());
    }
...
void ScriptManager::onTimerTriggerEvent() {
    ...
    for (std::map<int, TimerTriggerEvent>::iterator iterMap = TimerTriggerEventList.begin(); iterMap != TimerTriggerEventList.end(); ++iterMap) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugLUA,
                                     ...
                                     __FILE__, __FUNCTION__, __LINE__, event.running, (long long int)event.startFrame, (long long int)event.endFrame,
...
void ScriptManager::onCellTriggerEvent(Unit *movingUnit) {
    ...
    if (movingUnit != NULL) {
        ...
        for (std::map<int, CellTriggerEvent>::iterator iterMap = CellTriggerEventList.begin(); iterMap != CellTriggerEventList.end(); ++iterMap) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugLUA,
                                         ...
                                         "%d, event.destPos = %s\n",
                                         __FILE__, __FUNCTION__, __LINE__, movingUnit->getId(), event.type, movingUnit->getPos().getString().c_str(),
                    ...
                    if (movingUnit->getId() == event.sourceId) {
                        ...
                        if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
                            SystemFlags::OutputDebug(SystemFlags::debugLUA,
                                                     ...
                                                     movingUnit->getPos().getString().c_str(), event.sourceId, event.destId, event.destPos.getString().c_str(),
                        ...
                        if (srcInDst == true) {
                            ...
                            if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
                                SystemFlags::OutputDebug(SystemFlags::debugLUA,
                                                         ...
                                                         __FILE__, __FUNCTION__, __LINE__, movingUnit->getId(), event.type,
                                                         movingUnit->getPos().getString().c_str(), event.sourceId, event.destId,
                                                         event.destPos.getString().c_str(), destUnit->getPos().getString().c_str(), srcInDst);
                        }
                ...
                if (movingUnit->getId() == event.sourceId) {
                    ...
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugLUA,
                                                 ...
                                                 __FILE__, __FUNCTION__, __LINE__, movingUnit->getId(), event.type, movingUnit->getPos().getString().c_str(),
                                                 event.sourceId, event.destId, event.destPos.getString().c_str(), srcInDst);
            ...
            case ctet_Faction: {
                Unit *destUnit = world->findUnitById(event.destId);
                if (destUnit != NULL && movingUnit->getFactionIndex() == event.sourceId) {
                    bool srcInDst = world->getMap()->isInUnitTypeCells(destUnit->getType(), destUnit->getPos(), movingUnit->getPos());
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugLUA,
                                                 "In [%s::%s Line: %d] movingUnit = %d, event.type = %d, "
...
void ScriptManager::playStaticVideo(const string &playVideo) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s::%s Line: %d] playVideo [%s]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                                 __LINE__, playVideo.c_str());
    ...
    if (TimerTriggerEventList.find(eventId) != TimerTriggerEventList.end()) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugLUA,
                                     ...
                                     extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, TimerTriggerEventList.size(), eventId,
                                     (long long int)trigger.startFrame, (long long int)trigger.endFrame, result);
    }
    return result;
}
...
Vec2i ScriptManager::getUnitPosition(int unitId) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s] unitId = %d, pos [%s]\n", __FUNCTION__, unitId, result.getString().c_str());
...
int ScriptManager::isFreeCellsOrHasUnit(int field, int unitId, Vec2i pos) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s] unitId = %d, [%s] pos [%s] field = %d result = %d\n", __FUNCTION__, unitId,
                                 unit->getType()->getName(false).c_str(), pos.getString().c_str(), field, result);
...
int ScriptManager::isFreeCells(int unitSize, int field, Vec2i pos) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s] unitSize = %d, pos [%s] field = %d result = %d\n", __FUNCTION__, unitSize,
...
void ScriptManager::onDayNightTriggerEvent() {
    if (registeredDayNightEvent == true) {
        ...
        if ((lastDayNightTriggerStatus != 1 && isDay == true) || (lastDayNightTriggerStatus != 2 && isDay == false)) {
            if (isDay == true) {
...
int ScriptManager::createUnit(LuaHandle *luaHandle) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s::%s Line: %d] unit [%s] factionIndex = %d\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__, luaArguments.getString(-3).c_str(), luaArguments.getInt(-2));
...
int ScriptManager::createUnitNoSpacing(LuaHandle *luaHandle) {
    ...
    try {
        thisScriptManager->createUnitNoSpacing(luaArguments.getString(-3), luaArguments.getInt(-2), luaArguments.getVec2i(-1));
    } catch (const megaglest_runtime_error &ex) {
        error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
    }
...
int ScriptManager::destroyUnit(LuaHandle *luaHandle) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s::%s Line: %d] unit [%d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                                 __LINE__, luaArguments.getInt(-1));
...
int ScriptManager::setLockedUnitForFaction(LuaHandle *luaHandle) {
    ...
    try {
        thisScriptManager->setLockedUnitForFaction(luaArguments.getString(-3), luaArguments.getInt(-2), (luaArguments.getInt(-1) == 0 ? false : true));
    } catch (const megaglest_runtime_error &ex) {
        error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
    }
...
int ScriptManager::morphToUnit(LuaHandle *luaHandle) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s::%s Line: %d] unit [%d] morphName [%s] forceUpgrade = %d\n",
                                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, luaArguments.getInt(-3),
                                 luaArguments.getString(-2).c_str(), luaArguments.getInt(-1));
...
int ScriptManager::moveToUnit(LuaHandle *luaHandle) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s::%s Line: %d] unit [%d] dest unit [%d]\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __FUNCTION__, __LINE__, luaArguments.getInt(-2), luaArguments.getInt(-1));
...
int ScriptManager::playStaticSound(LuaHandle *luaHandle) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s::%s Line: %d] sound [%s]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                                 __LINE__, luaArguments.getString(-1).c_str());
...
int ScriptManager::giveAttackCommand(LuaHandle *luaHandle) {
    LuaArguments luaArguments(luaHandle);
    try {
        thisScriptManager->giveAttackCommand(luaArguments.getInt(-2), luaArguments.getInt(-1));
    } catch (const megaglest_runtime_error &ex) {
        error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
    }
...
int ScriptManager::giveAttackStoppedCommand(LuaHandle *luaHandle) {
    LuaArguments luaArguments(luaHandle);
    try {
        thisScriptManager->giveAttackStoppedCommand(luaArguments.getInt(-3), luaArguments.getString(-2), luaArguments.getInt(-1));
    } catch (const megaglest_runtime_error &ex) {
        error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
    }
    ...
    try {
        ...
        if (lua_checkstack(luaHandle, args + 1)) {
            ...
            if (args == 1) {
                ...
                thisScriptManager->DisplayFormattedText(fmt.c_str(), luaArguments.getGenericData(-args + 1));
            } else if (args == 3) {
                thisScriptManager->DisplayFormattedText(fmt.c_str(), luaArguments.getGenericData(-args + 1), luaArguments.getGenericData(-args + 2));
            } else if (args == 4) {
                thisScriptManager->DisplayFormattedText(fmt.c_str(), luaArguments.getGenericData(-args + 1), luaArguments.getGenericData(-args + 2),
                                                        luaArguments.getGenericData(-args + 3));
            } else if (args == 5) {
                thisScriptManager->DisplayFormattedText(fmt.c_str(), luaArguments.getGenericData(-args + 1), luaArguments.getGenericData(-args + 2),
                                                        luaArguments.getGenericData(-args + 3), luaArguments.getGenericData(-args + 4));
            } else if (args == 6) {
                thisScriptManager->DisplayFormattedText(fmt.c_str(), luaArguments.getGenericData(-args + 1), luaArguments.getGenericData(-args + 2),
                                                        luaArguments.getGenericData(-args + 3), luaArguments.getGenericData(-args + 4),
                ...
                thisScriptManager->DisplayFormattedText(fmt.c_str(), luaArguments.getGenericData(-args + 1), luaArguments.getGenericData(-args + 2),
                                                        luaArguments.getGenericData(-args + 3), luaArguments.getGenericData(-args + 4),
                                                        luaArguments.getGenericData(-args + 5), luaArguments.getGenericData(-args + 6));
    ...
    try {
        ...
        if (lua_checkstack(luaHandle, args + 1)) {
            ...
            if (args == 1) {
                ...
                thisScriptManager->addConsoleLangText(Lang::getInstance().getScenarioString(fmt).c_str(), luaArguments.getGenericData(-args + 1));
            } else if (args == 3) {
                thisScriptManager->addConsoleLangText(Lang::getInstance().getScenarioString(fmt).c_str(), luaArguments.getGenericData(-args + 1),
                                                      luaArguments.getGenericData(-args + 2));
            } else if (args == 4) {
                thisScriptManager->addConsoleLangText(Lang::getInstance().getScenarioString(fmt).c_str(), luaArguments.getGenericData(-args + 1),
                                                      luaArguments.getGenericData(-args + 2), luaArguments.getGenericData(-args + 3));
            } else if (args == 5) {
                thisScriptManager->addConsoleLangText(Lang::getInstance().getScenarioString(fmt).c_str(), luaArguments.getGenericData(-args + 1),
                                                      luaArguments.getGenericData(-args + 2), luaArguments.getGenericData(-args + 3),
                ...
                thisScriptManager->addConsoleLangText(Lang::getInstance().getScenarioString(fmt).c_str(), luaArguments.getGenericData(-args + 1),
                                                      luaArguments.getGenericData(-args + 2), luaArguments.getGenericData(-args + 3),
                                                      luaArguments.getGenericData(-args + 4), luaArguments.getGenericData(-args + 5));
            } else if (args == 7) {
                thisScriptManager->addConsoleLangText(Lang::getInstance().getScenarioString(fmt).c_str(), luaArguments.getGenericData(-args + 1),
                                                      luaArguments.getGenericData(-args + 2), luaArguments.getGenericData(-args + 3),
                                                      luaArguments.getGenericData(-args + 4), luaArguments.getGenericData(-args + 5),
                ...
                thisScriptManager->addConsoleLangText(Lang::getInstance().getScenarioString(fmt).c_str(), luaArguments.getGenericData(-args + 1),
                                                      luaArguments.getGenericData(-args + 2), luaArguments.getGenericData(-args + 3),
                                                      luaArguments.getGenericData(-args + 4), luaArguments.getGenericData(-args + 5),
                                                      luaArguments.getGenericData(-args + 6), luaArguments.getGenericData(-args + 7));
            } else {
                char szBuf[8096] = "";
                snprintf(szBuf, 8096,
                         "Invalid parameter count in method [%s] args = %d [argument "
    ...
    try {
        ...
        if (lua_checkstack(luaHandle, args + 1)) {
            ...
            if (args == 1) {
                ...
                thisScriptManager->DisplayFormattedLangText(Lang::getInstance().getScenarioString(fmt).c_str(), luaArguments.getGenericData(-args + 1));
            } else if (args == 3) {
                thisScriptManager->DisplayFormattedLangText(Lang::getInstance().getScenarioString(fmt).c_str(), luaArguments.getGenericData(-args + 1),
                                                            luaArguments.getGenericData(-args + 2));
            } else if (args == 4) {
...
int ScriptManager::isFreeCellsOrHasUnit(LuaHandle *luaHandle) {
    ...
    try {
        int result = thisScriptManager->isFreeCellsOrHasUnit(luaArguments.getInt(-3), luaArguments.getInt(-2), luaArguments.getVec2i(-1));
...
int ScriptManager::isFreeCells(LuaHandle *luaHandle) {
    ...
    try {
        int result = thisScriptManager->isFreeCells(luaArguments.getInt(-3), luaArguments.getInt(-2), luaArguments.getVec2i(-1));
...
void ScriptManager::saveGame(XmlNode *rootNode) {
    ...
    for (std::map<string, string>::iterator iterMap = luaSavedGameData.begin(); iterMap != luaSavedGameData.end(); ++iterMap) {
    ...
    for (std::map<int, UnitTriggerEventType>::iterator iterMap = UnitTriggerEventList.begin(); iterMap != UnitTriggerEventList.end(); ++iterMap) {

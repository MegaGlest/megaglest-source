...
void ScriptManagerMessage::saveGame(XmlNode *rootNode) {
  ...
  XmlNode *scriptManagerMessageNode =
  ...
  scriptManagerMessageNode->addAttribute("factionIndex", intToStr(factionIndex),
                                         mapTagReplacements);
  scriptManagerMessageNode->addAttribute("teamIndex", intToStr(teamIndex),
                                         mapTagReplacements);
...
void ScriptManagerMessage::loadGame(const XmlNode *rootNode) {
  ...
  factionIndex =
      scriptManagerMessageNode->getAttribute("factionIndex")->getIntValue();
  teamIndex =
...
void PlayerModifiers::saveGame(XmlNode *rootNode) {
  ...
  playerModifiersNode->addAttribute("aiEnabled", intToStr(aiEnabled),
                                    mapTagReplacements);
  // bool consumeEnabled;
  playerModifiersNode->addAttribute("consumeEnabled", intToStr(consumeEnabled),
                                    mapTagReplacements);
}
...
void PlayerModifiers::loadGame(const XmlNode *rootNode) {
  ...
  consumeEnabled =
      playerModifiersNode->getAttribute("consumeEnabled")->getIntValue() != 0;
}
...
void CellTriggerEvent::saveGame(XmlNode *rootNode) {
  ...
  cellTriggerEventNode->addAttribute("destPos", destPos.getString(),
                                     mapTagReplacements);
  //	int triggerCount;
  cellTriggerEventNode->addAttribute("triggerCount", intToStr(triggerCount),
                                     mapTagReplacements);

  //	Vec2i destPosEnd;
  cellTriggerEventNode->addAttribute("destPosEnd", destPosEnd.getString(),
                                     mapTagReplacements);
}
...
void CellTriggerEvent::loadGame(const XmlNode *rootNode) {
  ...
  type = static_cast<CellTriggerEventType>(
  ...
  destPos = Vec2i::strToVec2(
      cellTriggerEventNode->getAttribute("destPos")->getValue());
  triggerCount =
      cellTriggerEventNode->getAttribute("triggerCount")->getIntValue();

  if (cellTriggerEventNode->hasAttribute("destPosEnd") == true) {
    destPosEnd = Vec2i::strToVec2(
        cellTriggerEventNode->getAttribute("destPosEnd")->getValue());
  }
}
...
void TimerTriggerEvent::saveGame(XmlNode *rootNode) {
  ...
  //	//time_t endTime;
  //	int startFrame;
  timerTriggerEventNode->addAttribute("startFrame", intToStr(startFrame),
                                      mapTagReplacements);
  //	int endFrame;
  ...
  if (triggerSecondsElapsed > 0) {
    timerTriggerEventNode->addAttribute("triggerSecondsElapsed",
                                        intToStr(triggerSecondsElapsed),
                                        mapTagReplacements);
  }
}

void TimerTriggerEvent::loadGame(const XmlNode *rootNode) {
  const XmlNode *timerTriggerEventNode =
      rootNode->getChild("TimerTriggerEvent");
...
void ScriptManager::init(World *world, GameCamera *gameCamera,
  ...
  // [%p][%s]\n",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__,rootNode,(rootNode
  // != NULL ? rootNode->getName().c_str() : "none"));
  ...
  luaScript.registerFunction(registerCellTriggerEventForFactionToLocation,
                             "registerCellTriggerEventForFactionToLocation");

  luaScript.registerFunction(registerCellAreaTriggerEventForUnitToLocation,
                             "registerCellAreaTriggerEventForUnitToLocation");
  luaScript.registerFunction(
  ...
  // load code
  if (scenario->getExternalScript() == nullptr) {
    for (int i = 0; i < scenario->getScriptCount(); ++i) {
      const Script *script = scenario->getScript(i);
      luaScript.loadCode(
          "function " + script->getName() + "()" + script->getCode() + "end\n",
          script->getName());
    }
  } else {
    luaScript.loadCode(scenario->getExternalScript()->getCode(),
                       scenario->getExternalScript()->getName());
  }
  ...
  //	}
  //	string sandboxScriptFilename = data_path +
  //"data/core/scripts/sandbox.lua"; 	string sandboxLuaCode =
  // getFileTextContents(sandboxScriptFilename);
  //
  ...
  try {
    ...
    char szErrBuf[8096] = "";
    // snprintf(szErrBuf,8096,"In [%s::%s
    // %d]",extractFileFromDirectoryPath(__FILE__).c_str(),__FUNCTION__,__LINE__);
    string sErrBuf = string(szErrBuf) +
    ...
    thisScriptManager->addMessageToQueue(
        ScriptManagerMessage(sErrBuf.c_str(), "error", -1, -1, true));
...
void ScriptManager::onMessageBoxOk(bool popFront) {
  ...
  for (int index = 0; messageQueue.empty() == false; ++index) {
    // printf("i = %d messageQueue.size() = %d popFront =
    // %d\n",i,messageQueue.size(),popFront);
    if (popFront == true) {
      messageQueue.pop_front();
    }
    if (messageQueue.empty() == false) {
      //			printf("onMessageBoxOk [%s] factionIndex = %d
      //[%d] teamIndex = %d [%d][%d]\n",
      //					wrapString(lang.getScenarioString(messageQueue.front().getText()),
      // messageWrapCount).c_str(),
...
void ScriptManager::onTimerTriggerEvent() {
  ...
  for (std::map<int, TimerTriggerEvent>::iterator iterMap =
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugLUA,
          "In [%s::%s Line: %d] event.running = %d, event.startTime = %lld, "
          "event.endTime = %lld, diff = %f\n",
          __FILE__, __FUNCTION__, __LINE__, event.running,
          (long long int)event.startFrame, (long long int)event.endFrame,
    ...
    if (event.running == true) {
      ...
      if (event.triggerSecondsElapsed > 0) {
        int elapsed = (world->getFrameCount() - event.startFrame) /
...
void ScriptManager::onCellTriggerEvent(Unit *movingUnit) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA,
                             "In [%s::%s Line: %d] movingUnit = %p, "
                             "CellTriggerEventList.size() = %d\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, movingUnit,
                             CellTriggerEventList.size());
  ...
  if (movingUnit != NULL) {
    ...
    for (std::map<int, CellTriggerEvent>::iterator iterMap =
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugLUA,
            "In [%s::%s Line: %d] movingUnit = %d, event.type = %d, "
            "movingUnit->getPos() = %s, event.sourceId = %d, event.destId = "
            "%d, event.destPos = %s\n",
            __FILE__, __FUNCTION__, __LINE__, movingUnit->getId(), event.type,
            movingUnit->getPos().getString().c_str(), event.sourceId,
            event.destId, event.destPos.getString().c_str());
      ...
      switch (event.type) {
        case ctet_Unit: {
          ...
          if (destUnit != NULL) {
            if (movingUnit->getId() == event.sourceId) {
              bool srcInDst = world->getMap()->isInUnitTypeCells(
                  destUnit->getType(), destUnit->getPos(),
                  movingUnit->getPos());
              if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA)
                ...
                SystemFlags::OutputDebug(
                    SystemFlags::debugLUA,
                    "In [%s::%s Line: %d] movingUnit = %d, event.type = %d, "
                    "movingUnit->getPos() = %s, event.sourceId = %d, "
                    "event.destId = %d, event.destPos = %s, destUnit->getPos() "
                    "= %s, srcInDst = %d\n",
                    __FILE__, __FUNCTION__, __LINE__, movingUnit->getId(),
                    event.type, movingUnit->getPos().getString().c_str(),
                    event.sourceId, event.destId,
                    event.destPos.getString().c_str(),
                    destUnit->getPos().getString().c_str(), srcInDst);
              ...
              if (srcInDst == true) {
                ...
                srcInDst = world->getMap()->isNextToUnitTypeCells(
                    destUnit->getType(), destUnit->getPos(),
                    movingUnit->getPos());
                if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA)
                        .enabled)
                  SystemFlags::OutputDebug(
                      ...
                      "In [%s::%s Line: %d] movingUnit = %d, event.type = %d, "
                      "movingUnit->getPos() = %s, event.sourceId = %d, "
                      "event.destId = %d, event.destPos = %s, "
                      "destUnit->getPos() = %s, srcInDst = %d\n",
                      __FILE__, __FUNCTION__, __LINE__, movingUnit->getId(),
                      event.type, movingUnit->getPos().getString().c_str(),
                      event.sourceId, event.destId,
                      event.destPos.getString().c_str(),
        ...
        case ctet_UnitPos: {
          if (movingUnit->getId() == event.sourceId) {
            bool srcInDst = world->getMap()->isInUnitTypeCells(
                movingUnit->getType(), event.destPos, movingUnit->getPos());
            if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA)
                    .enabled)
              SystemFlags::OutputDebug(
                  SystemFlags::debugLUA,
                  "In [%s::%s Line: %d] movingUnit = %d, event.type = %d, "
                  "movingUnit->getPos() = %s, event.sourceId = %d, "
                  "event.destId = %d, event.destPos = %s, srcInDst = %d\n",
                  __FILE__, __FUNCTION__, __LINE__, movingUnit->getId(),
                  event.type, movingUnit->getPos().getString().c_str(),
                  event.sourceId, event.destId,
                  event.destPos.getString().c_str(), srcInDst);
        ...
        case ctet_UnitAreaPos: {
          if (movingUnit->getId() == event.sourceId) {
            ...
            std::map<int, std::map<Vec2i, bool> >::iterator iterFind1 =
                event.eventLookupCache.find(movingUnit->getType()->getSize());
            ...
            if (foundInCache == false) {
              for (int x = event.destPos.x;
                   srcInDst == false && x <= event.destPosEnd.x; ++x) {
                for (int y = event.destPos.y;
                     srcInDst == false && y <= event.destPosEnd.y; ++y) {
                  srcInDst = world->getMap()->isInUnitTypeCells(
                      movingUnit->getType(), Vec2i(x, y), movingUnit->getPos());
                  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA)
                          .enabled)
                    SystemFlags::OutputDebug(
                        SystemFlags::debugLUA,
                        "In [%s::%s Line: %d] movingUnit = %d, event.type = "
                        "%d, movingUnit->getPos() = %s, event.sourceId = %d, "
                        "event.destId = %d, event.destPos = %s, srcInDst = "
                        "%d\n",
                        __FILE__, __FUNCTION__, __LINE__, movingUnit->getId(),
                        event.type, movingUnit->getPos().getString().c_str(),
                        event.sourceId, event.destId,
                        Vec2i(x, y).getString().c_str(), srcInDst);
                }
              }

              event.eventLookupCache[movingUnit->getType()->getSize()]
                                    [movingUnit->getPos()] = srcInDst;
            }
        ...
        case ctet_Faction: {
          ...
          if (destUnit != NULL &&
            ...
            bool srcInDst = world->getMap()->isInUnitTypeCells(
                destUnit->getType(), destUnit->getPos(), movingUnit->getPos());
            if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA)
                    .enabled)
              SystemFlags::OutputDebug(
                  SystemFlags::debugLUA,
        ...
        case ctet_FactionPos: {
          if (movingUnit->getFactionIndex() == event.sourceId) {
            // printf("ctet_FactionPos event.destPos = [%s],
            // movingUnit->getPos()
        ...
        case ctet_FactionAreaPos: {
          if (movingUnit->getFactionIndex() == event.sourceId) {
            // if(event.sourceId == 1) printf("ctet_FactionPos event.destPos =
            // [%s], movingUnit->getPos() [%s] Unit id =
            ...
            triggerEvent = srcInDst;
            if (triggerEvent == true) {
              // printf("!!!UNIT IN AREA!!! Faction area pos, moving unit
              // faction= %d, trigger faction = %d, unit id =
        ...
        case ctet_AreaPos: {
          // Is the unit already in the cell range? If no check if they are
          // entering it
              ...
              for (int y = event.destPos.y;
                ...
                if (srcInDst == true) {
                  ...
                  currentCellTriggeredEventAreaEntryUnitId =
                      movingUnit->getId();
                  event.eventStateInfo[movingUnit->getId()] =
                      Vec2i(x, y).getString();
                }
              }
            }
      ...
      //			ScenarioInfo scenarioInfoEnd =
      // world->getScenario()->getInfo();
...
void ScriptManager::networkShowMessageForFaction(const string &text,
                                                 const string &header,
                                                 int factionIndex) {
  messageQueue.push_back(ScriptManagerMessage(text, header, factionIndex));
  thisScriptManager->onMessageBoxOk(false);
}
void ScriptManager::networkShowMessageForTeam(const string &text,
                                              const string &header,
                                              int teamIndex) {
  // Team indexes are 0 based internally (but 1 based in the lua script) so
  // convert
  teamIndex--;
...
void ScriptManager::networkSetCameraPositionForFaction(int factionIndex,
                                                       const Vec2i &pos) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s::%s Line: %d]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
...
void ScriptManager::networkSetCameraPositionForTeam(int teamIndex,
                                                    const Vec2i &pos) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s::%s Line: %d]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
...
void ScriptManager::clearDisplayText() { displayText = ""; }

void ScriptManager::setDisplayText(const string &text) {
  displayText = wrapString(Lang::getInstance().getScenarioString(text),
...
void ScriptManager::shakeCamera(int shakeIntensity, int shakeDuration,
  ...
  if (cameraDistanceAffected) {
    ...
    if (unit) {
      gameCamera->shake(shakeDuration, shakeIntensity, cameraDistanceAffected,
...
void ScriptManager::createUnit(const string &unitName, int factionIndex,
                               Vec2i pos) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugLUA,
...
void ScriptManager::createUnitNoSpacing(const string &unitName,
...
void ScriptManager::setLockedUnitForFaction(const string &unitName,
  ...
  if (world->getFactionCount() > factionIndex) {
    const UnitType *ut =
        world->getFaction(factionIndex)->getType()->getUnitType(unitName);
...
void ScriptManager::playStaticVideo(const string &playVideo) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA,
                             ...
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, playVideo.c_str());
...
void ScriptManager::togglePauseGame(int pauseStatus) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA,
                             ...
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, pauseStatus);
  world->togglePauseGame((pauseStatus != 0), true);
}

void ScriptManager::morphToUnit(int unitId, const string &morphName,
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA,
                             ...
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, unitId, morphName.c_str(),
                             ignoreRequirements);
...
void ScriptManager::moveToUnit(int unitId, int destUnitId) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA,
                             ...
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, unitId, destUnitId);
...
void ScriptManager::givePositionCommand(int unitId, const string &commandName,
                                        const Vec2i &pos) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s::%s Line: %d]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
...
void ScriptManager::giveProductionCommand(int unitId,
...
void ScriptManager::giveAttackStoppedCommand(int unitId, const string &itemName,
                                             int ignoreRequirements) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s::%s Line: %d]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
...
void ScriptManager::disableAi(int factionIndex) {
  ...
  if (factionIndex >= 0 && factionIndex < GameConstants::maxPlayers) {
    ...
    disableConsume(
        factionIndex);  // by this we stay somehow compatible with old behaviour
  }
}
...
int ScriptManager::registerCellTriggerEventForUnitToUnit(int sourceUnitId,
                                                         int destUnitId) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA,
                             "In [%s::%s Line: %d] Unit: %d will trigger cell "
                             "event when reaching unit: %d, eventId = %d\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, sourceUnitId, destUnitId,
                             eventId);

  return eventId;
}

int ScriptManager::registerCellTriggerEventForUnitToLocation(int sourceUnitId,
                                                             const Vec2i &pos) {
  CellTriggerEvent trigger;
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA,
                             "In [%s::%s Line: %d] Unit: %d will trigger cell "
                             "event when reaching pos: %s, eventId = %d\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, sourceUnitId,
                             pos.getString().c_str(), eventId);

  return eventId;
}

int ScriptManager::registerCellAreaTriggerEventForUnitToLocation(
    int sourceUnitId, const Vec4i &pos) {
  CellTriggerEvent trigger;
...
int ScriptManager::registerCellTriggerEventForFactionToUnit(int sourceFactionId,
                                                            int destUnitId) {
  CellTriggerEvent trigger;
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugLUA,
        "In [%s::%s Line: %d] Faction: %d will trigger cell event when "
        "reaching unit: %d, eventId = %d\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
...
int ScriptManager::registerCellTriggerEventForFactionToLocation(
    int sourceFactionId, const Vec2i &pos) {
  CellTriggerEvent trigger;
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA,
                             "In [%s::%s Line: %d]Faction: %d will trigger "
                             "cell event when reaching pos: %s, eventId = %d\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, sourceFactionId,
                             pos.getString().c_str(), eventId);

  return eventId;
}

int ScriptManager::registerCellAreaTriggerEventForFactionToLocation(
    int sourceFactionId, const Vec4i &pos) {
  CellTriggerEvent trigger;
...
int ScriptManager::registerCellAreaTriggerEvent(const Vec4i &pos) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA,
                             "In [%s::%s Line: %d] trigger cell event when "
                             "reaching pos: %s, eventId = %d\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, pos.getString().c_str(),
                             eventId);
...
int ScriptManager::startTimerEvent() {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugLUA,
        "In [%s::%s Line: %d] TimerTriggerEventList.size() = %d, eventId = %d, "
        "trigger.startTime = %lld, trigger.endTime = %lld\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
...
int ScriptManager::resetTimerEvent(int eventId) {
  ...
  if (TimerTriggerEventList.find(eventId) != TimerTriggerEventList.end()) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
      SystemFlags::OutputDebug(
          SystemFlags::debugLUA,
          "In [%s::%s Line: %d] TimerTriggerEventList.size() = %d, eventId = "
          "%d, trigger.startTime = %lld, trigger.endTime = %lld, result = %d\n",
          extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
          __LINE__, TimerTriggerEventList.size(), eventId,
          (long long int)trigger.startFrame, (long long int)trigger.endFrame,
          result);
  }
  return result;
...
int ScriptManager::getTimerEventSecondsElapsed(int eventId) {
  ...
  if (TimerTriggerEventList.find(eventId) != TimerTriggerEventList.end()) {
    ...
    if (trigger.running) {
      ...
      result = (world->getFrameCount() - trigger.startFrame) /
               GameConstants::updateFps;
    } else {
      // result = (int)difftime(trigger.endTime,trigger.startTime);
      result =
          (trigger.endFrame - trigger.startFrame) / GameConstants::updateFps;
    }
...
void ScriptManager::addCellMarker(Vec2i pos, int factionIndex,
                                  const string &note,
...
void ScriptManager::showMarker(Vec2i pos, int factionIndex, const string &note,
...
int ScriptManager::getResourceAmount(const string &resourceName,
                                     int factionIndex) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s::%s Line: %d]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
...
void ScriptManager::setRandomGenInit(int seed) { random.init(seed); }

int ScriptManager::getRandomGen(int minVal, int maxVal) {
  return random.randRange(minVal, maxVal);
}

int ScriptManager::getWorldFrameCount() { return world->getFrameCount(); }

bool ScriptManager::getGameWon() const { return gameWon; }
...
int ScriptManager::getUnitCountOfType(int factionIndex,
...
vector<int> ScriptManager::getUnitsForFaction(int factionIndex,
...
int ScriptManager::isFreeCellsOrHasUnit(int field, int unitId, Vec2i pos) {
  ...
  int result = world->getMap()->isFreeCellsOrHasUnit(
      pos, unit->getType()->getSize(), static_cast<Field>(field), unit);

  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugLUA,
        "In [%s] unitId = %d, [%s] pos [%s] field = %d result = %d\n",
        __FUNCTION__, unitId, unit->getType()->getName(false).c_str(),
...
int ScriptManager::isFreeCells(int unitSize, int field, Vec2i pos) {
  ...
  int result = world->getMap()->isFreeCellsOrHasUnit(
      pos, unitSize, static_cast<Field>(field), NULL);
...
void ScriptManager::highlightUnit(int unitId, float radius, float thickness,
                                  Vec4f color) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA, "In [%s::%s Line: %d]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
...
int ScriptManager::getUnitProperty(int unitId, UnitTriggerEventType type) {
  ...
  // printf("File: %s line: %d result:
  // %d\n",extractFileFromDirectoryPath(__FILE__).c_str(),__LINE__,result);
  return result;
}
const string ScriptManager::getUnitPropertyName(int unitId,
                                                UnitTriggerEventType type) {
  string result = "";
  Unit *unit = world->findUnitById(unitId);
  if (unit != NULL) {
    switch (type) {
...
void ScriptManager::onUnitTriggerEvent(const Unit *unit,
                                       UnitTriggerEventType event) {
  // static bool inEvent = false;
  // if(inEvent == true) {
  //	printf("\n\n!!!!!!!!!!!!!!! File: %s line: %d unit [%d -
  ...
  if (UnitTriggerEventList.empty() == false) {
    // printf("File: %s line:
    // %d\n",extractFileFromDirectoryPath(__FILE__).c_str(),__LINE__);
    std::map<int, UnitTriggerEventType>::iterator iterFind =
        UnitTriggerEventList.find(unit->getId());
...
void ScriptManager::registerDayNightEvent() { registeredDayNightEvent = true; }
...
int ScriptManager::networkShowMessageForFaction(LuaHandle *luaHandle) {
  ...
  try {
    thisScriptManager->networkShowMessageForFaction(luaArguments.getString(-3),
...
int ScriptManager::networkShowMessageForTeam(LuaHandle *luaHandle) {
  ...
  try {
    thisScriptManager->networkShowMessageForTeam(luaArguments.getString(-3),
                                                 luaArguments.getString(-2),
                                                 luaArguments.getInt(-1));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
...
int ScriptManager::networkSetCameraPositionForFaction(LuaHandle *luaHandle) {
  ...
  try {
    thisScriptManager->networkSetCameraPositionForFaction(
        luaArguments.getInt(-2), luaArguments.getVec2i(-1));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::networkSetCameraPositionForTeam(LuaHandle *luaHandle) {
  ...
  try {
    thisScriptManager->networkSetCameraPositionForTeam(
        luaArguments.getInt(-2), luaArguments.getVec2i(-1));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::createUnit(LuaHandle *luaHandle) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(
        ...
        "In [%s::%s Line: %d] unit [%s] factionIndex = %d\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        luaArguments.getString(-3).c_str(), luaArguments.getInt(-2));
...
void ScriptManager::error(LuaHandle *luaHandle,
  ...
  thisScriptManager->addMessageToQueue(ScriptManagerMessage(
      sErrBuf.c_str(), "error " + intToStr(thisScriptManager->errorCount), -1,
      -1, true));
...
int ScriptManager::destroyUnit(LuaHandle *luaHandle) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA,
                             "In [%s::%s Line: %d] unit [%d]\n",
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, luaArguments.getInt(-1));
...
int ScriptManager::setLockedUnitForFaction(LuaHandle *luaHandle) {
  ...
  try {
    thisScriptManager->setLockedUnitForFaction(
        luaArguments.getString(-3), luaArguments.getInt(-2),
...
int ScriptManager::morphToUnit(LuaHandle *luaHandle) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(
        ...
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        luaArguments.getInt(-3), luaArguments.getString(-2).c_str(),
        luaArguments.getInt(-1));
...
int ScriptManager::moveToUnit(LuaHandle *luaHandle) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA,
                             ...
                             extractFileFromDirectoryPath(__FILE__).c_str(),
                             __FUNCTION__, __LINE__, luaArguments.getInt(-2),
                             luaArguments.getInt(-1));
...
int ScriptManager::registerCellTriggerEventForUnitToUnit(LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    int result = thisScriptManager->registerCellTriggerEventForUnitToUnit(
...
int ScriptManager::registerCellTriggerEventForUnitToLocation(
    LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    int result = thisScriptManager->registerCellTriggerEventForUnitToLocation(
        luaArguments.getInt(-2), luaArguments.getVec2i(-1));
    luaArguments.returnInt(result);
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
...
int ScriptManager::registerCellAreaTriggerEventForUnitToLocation(
  ...
  try {
    int result =
        thisScriptManager->registerCellAreaTriggerEventForUnitToLocation(
            luaArguments.getInt(-2), luaArguments.getVec4i(-1));
    luaArguments.returnInt(result);
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::registerCellTriggerEventForFactionToUnit(
    LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    int result = thisScriptManager->registerCellTriggerEventForFactionToUnit(
        luaArguments.getInt(-2), luaArguments.getInt(-1));
    luaArguments.returnInt(result);
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
...
int ScriptManager::registerCellTriggerEventForFactionToLocation(
  ...
  try {
    int result =
        thisScriptManager->registerCellTriggerEventForFactionToLocation(
            luaArguments.getInt(-2), luaArguments.getVec2i(-1));
    luaArguments.returnInt(result);
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
...
int ScriptManager::registerCellAreaTriggerEventForFactionToLocation(
  ...
  try {
    int result =
        thisScriptManager->registerCellAreaTriggerEventForFactionToLocation(
            luaArguments.getInt(-2), luaArguments.getVec4i(-1));
    luaArguments.returnInt(result);
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
...
int ScriptManager::registerCellAreaTriggerEvent(LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    int result = thisScriptManager->registerCellAreaTriggerEvent(
        luaArguments.getVec4i(-1));
    luaArguments.returnInt(result);
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::getCellTriggerEventCount(LuaHandle *luaHandle) {
  ...
  try {
    int result =
        thisScriptManager->getCellTriggerEventCount(luaArguments.getInt(-1));
    luaArguments.returnInt(result);
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::startEfficientTimerEvent(LuaHandle *luaHandle) {
  ...
  try {
    int result =
        thisScriptManager->startEfficientTimerEvent(luaArguments.getInt(-1));
    luaArguments.returnInt(result);
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::getTimerEventSecondsElapsed(LuaHandle *luaHandle) {
  ...
  try {
    int result =
        thisScriptManager->getTimerEventSecondsElapsed(luaArguments.getInt(-1));
    luaArguments.returnInt(result);
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::addCellMarker(LuaHandle *luaHandle) {
  ...
  try {
    ...
    // printf("LUA addCellMarker --> faction [%d] pos [%s] note [%s] texture
...
int ScriptManager::getResourceAmount(LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    luaArguments.returnInt(thisScriptManager->getResourceAmount(
        luaArguments.getString(-2), luaArguments.getInt(-1)));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::getCellTriggeredEventAreaEntryUnitId(LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    luaArguments.returnInt(
        thisScriptManager->getCellTriggeredEventAreaEntryUnitId());
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::getCellTriggeredEventAreaExitUnitId(LuaHandle *luaHandle) {
  ...
  try {
    luaArguments.returnInt(
        thisScriptManager->getCellTriggeredEventAreaExitUnitId());
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::getRandomGen(LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    luaArguments.returnInt(thisScriptManager->getRandomGen(
        luaArguments.getInt(-2), luaArguments.getInt(-1)));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::getSystemMacroValue(LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    luaArguments.returnString(
        thisScriptManager->getSystemMacroValue(luaArguments.getString(-1)));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::scenarioDir(LuaHandle *luaHandle) {
  ...
  try {
    luaArguments.returnString(
        thisScriptManager->getSystemMacroValue("$SCENARIO_PATH"));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::getPlayerName(LuaHandle *luaHandle) {
  ...
  try {
    luaArguments.returnString(
        thisScriptManager->getPlayerName(luaArguments.getInt(-1)));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::getUnitCount(LuaHandle *luaHandle) {
  ...
  try {
    luaArguments.returnInt(
        thisScriptManager->getUnitCount(luaArguments.getInt(-1)));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::getUnitCountOfType(LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    luaArguments.returnInt(thisScriptManager->getUnitCountOfType(
        luaArguments.getInt(-2), luaArguments.getString(-1)));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::DisplayFormattedText(LuaHandle *luaHandle) {
  ...
  try {
    ...
    if (lua_checkstack(luaHandle, args + 1)) {
      ...
      if (args == 1) {
        ...
        thisScriptManager->DisplayFormattedText(
            fmt.c_str(), luaArguments.getGenericData(-args + 1));
      } else if (args == 3) {
        thisScriptManager->DisplayFormattedText(
            fmt.c_str(), luaArguments.getGenericData(-args + 1),
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 "Invalid parameter count in method [%s] args = %d [argument "
  ...
  */
  ...
  /*
                  ...
                  lua_call(L, n, 1); // Call the function, n args, one return
     value. lua_replace(L, idxs[0]); // Replace the initial argument with the
     new object. return 0;
  */
}
...
int ScriptManager::addConsoleLangText(LuaHandle *luaHandle) {
  ...
  try {
    ...
    if (lua_checkstack(luaHandle, args + 1)) {
      ...
      const int max_args_allowed = 8;
      if (args == 1) {
        thisScriptManager->addConsoleLangText(
            Lang::getInstance().getScenarioString(fmt).c_str());
      } else if (args == 2) {
        thisScriptManager->addConsoleLangText(
            Lang::getInstance().getScenarioString(fmt).c_str(),
...
int ScriptManager::DisplayFormattedLangText(LuaHandle *luaHandle) {
  ...
  try {
    ...
    if (lua_checkstack(luaHandle, args + 1)) {
      ...
      const int max_args_allowed = 8;
      if (args == 1) {
        thisScriptManager->DisplayFormattedLangText(
            Lang::getInstance().getScenarioString(fmt).c_str());
      } else if (args == 2) {
        thisScriptManager->DisplayFormattedLangText(
            Lang::getInstance().getScenarioString(fmt).c_str(),
...
int ScriptManager::getUnitsForFaction(LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    vector<int> units = thisScriptManager->getUnitsForFaction(
        luaArguments.getInt(-3), luaArguments.getString(-2),
        luaArguments.getInt(-1));
...
int ScriptManager::getUnitCurrentField(LuaHandle *luaHandle) {
  ...
  try {
    luaArguments.returnInt(
        thisScriptManager->getUnitCurrentField(luaArguments.getInt(-1)));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::getIsUnitAlive(LuaHandle *luaHandle) {
  ...
  try {
    luaArguments.returnInt(
        thisScriptManager->getIsUnitAlive(luaArguments.getInt(-1)));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::isFreeCellsOrHasUnit(LuaHandle *luaHandle) {
  ...
  try {
    int result = thisScriptManager->isFreeCellsOrHasUnit(
        luaArguments.getInt(-3), luaArguments.getInt(-2),
        luaArguments.getVec2i(-1));
...
int ScriptManager::isFreeCells(LuaHandle *luaHandle) {
  ...
  try {
    int result = thisScriptManager->isFreeCells(luaArguments.getInt(-3),
                                                luaArguments.getInt(-2),
                                                luaArguments.getVec2i(-1));
...
int ScriptManager::highlightUnit(LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    thisScriptManager->highlightUnit(
        luaArguments.getInt(-4), luaArguments.getFloat(-3),
        luaArguments.getFloat(-2), luaArguments.getVec4f(-1));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::selectUnit(LuaHandle *luaHandle) {
  ...
  try {
    luaArguments.returnInt(
        thisScriptManager->selectUnit(luaArguments.getInt(-1)));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::removeUnitFromGroupSelection(LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    thisScriptManager->removeUnitFromGroupSelection(luaArguments.getInt(-2),
                                                    luaArguments.getInt(-1));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::setAttackWarningsEnabled(LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    thisScriptManager->setAttackWarningsEnabled(
        (luaArguments.getInt(-1) == 0 ? false : true));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::getLastUnitTriggerEventType(LuaHandle *luaHandle) {
  ...
  try {
    luaArguments.returnInt(
        static_cast<int>(thisScriptManager->getLastUnitTriggerEventType()));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::getUnitProperty(LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    int value = thisScriptManager->getUnitProperty(
        luaArguments.getInt(-2),
        static_cast<UnitTriggerEventType>(luaArguments.getInt(-1)));
...
int ScriptManager::getUnitPropertyName(LuaHandle *luaHandle) {
  LuaArguments luaArguments(luaHandle);
  try {
    const string unitname = thisScriptManager->getUnitPropertyName(
        luaArguments.getInt(-2),
        static_cast<UnitTriggerEventType>(luaArguments.getInt(-1)));
    luaArguments.returnString(unitname);
  } catch (const megaglest_runtime_error &ex) {
...
int ScriptManager::loadSaveGameData(LuaHandle *luaHandle) {
  ...
  try {
    luaArguments.returnString(
        thisScriptManager->loadSaveGameData(luaArguments.getString(-1)));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
int ScriptManager::getFactionPlayerType(LuaHandle *luaHandle) {
  ...
  try {
    luaArguments.returnInt(
        thisScriptManager->getFactionPlayerType(luaArguments.getInt(-1)));
  } catch (const megaglest_runtime_error &ex) {
    error(luaHandle, &ex, __FILE__, __FUNCTION__, __LINE__);
  }
...
void ScriptManager::saveGame(XmlNode *rootNode) {
  ...
  for (std::list<ScriptManagerMessage>::iterator it = messageQueue.begin();
       it != messageQueue.end(); ++it) {
    (*it).saveGame(scriptManagerNode);
  }

  // printf("==== ScriptManager Savegame messageBox [%d][%s][%s] displayText
  ...
  scriptManagerNode->addAttribute("messageBox_text", messageBox.getText(),
                                  mapTagReplacements);
  scriptManagerNode->addAttribute("messageBox_header", messageBox.getHeader(),
                                  mapTagReplacements);
  ...
  scriptManagerNode->addAttribute("lastCreatedUnitName", lastCreatedUnitName,
                                  mapTagReplacements);
  //	int lastCreatedUnitId;
  scriptManagerNode->addAttribute(
      "lastCreatedUnitId", intToStr(lastCreatedUnitId), mapTagReplacements);
  //
  //	//last dead unit
  ...
  scriptManagerNode->addAttribute("lastDeadUnitName", lastDeadUnitName,
                                  mapTagReplacements);
  //	int lastDeadUnitId;
  scriptManagerNode->addAttribute("lastDeadUnitId", intToStr(lastDeadUnitId),
                                  mapTagReplacements);
  ...
  scriptManagerNode->addAttribute("lastDeadUnitKillerName",
                                  lastDeadUnitKillerName, mapTagReplacements);
  ...
  scriptManagerNode->addAttribute("lastAttackedUnitName", lastAttackedUnitName,
                                  mapTagReplacements);
  //	int lastAttackedUnitId;
  scriptManagerNode->addAttribute(
      "lastAttackedUnitId", intToStr(lastAttackedUnitId), mapTagReplacements);
  //
  //	//last attacking unit
  //	string lastAttackingUnitName;
  scriptManagerNode->addAttribute("lastAttackingUnitName",
                                  lastAttackingUnitName, mapTagReplacements);
  //	int lastAttackingUnitId;
  scriptManagerNode->addAttribute(
      "lastAttackingUnitId", intToStr(lastAttackingUnitId), mapTagReplacements);
  //
  //	// end game state
  //	bool gameOver;
  ...
  scriptManagerNode->addAttribute("currentTimerTriggeredEventId",
                                  intToStr(currentTimerTriggeredEventId),
                                  mapTagReplacements);
  ...
  scriptManagerNode->addAttribute("currentCellTriggeredEventId",
                                  ...
                                  mapTagReplacements);
  //	int currentEventId;
  scriptManagerNode->addAttribute("currentEventId", intToStr(currentEventId),
                                  mapTagReplacements);
  //	std::map<int,CellTriggerEvent> CellTriggerEventList;
  for (std::map<int, CellTriggerEvent>::iterator iterMap =
           CellTriggerEventList.begin();
       iterMap != CellTriggerEventList.end(); ++iterMap) {
    XmlNode *cellTriggerEventListNode =
        scriptManagerNode->addChild("CellTriggerEventList");

    cellTriggerEventListNode->addAttribute("key", intToStr(iterMap->first),
                                           mapTagReplacements);
  ...
  for (std::map<int, TimerTriggerEvent>::iterator iterMap =
           TimerTriggerEventList.begin();
       iterMap != TimerTriggerEventList.end(); ++iterMap) {
    XmlNode *timerTriggerEventListNode =
        scriptManagerNode->addChild("TimerTriggerEventList");

    timerTriggerEventListNode->addAttribute("key", intToStr(iterMap->first),
                                            mapTagReplacements);
  ...
  scriptManagerNode->addAttribute(
      "inCellTriggerEvent", intToStr(inCellTriggerEvent), mapTagReplacements);
  //	std::vector<int> unRegisterCellTriggerEventList;
  for (unsigned int i = 0; i < unRegisterCellTriggerEventList.size(); ++i) {
    XmlNode *unRegisterCellTriggerEventListNode =
        scriptManagerNode->addChild("unRegisterCellTriggerEventList");
    unRegisterCellTriggerEventListNode->addAttribute(
        "eventId", intToStr(unRegisterCellTriggerEventList[i]),
        mapTagReplacements);
  }
  ...
  for (std::map<int, UnitTriggerEventType>::iterator iterMap =
    ...
    XmlNode *unitTriggerEventListNode =
        scriptManagerNode->addChild("UnitTriggerEventList");

    unitTriggerEventListNode->addAttribute("unitId", intToStr(iterMap->first),
                                           mapTagReplacements);
    unitTriggerEventListNode->addAttribute(
        "eventType", intToStr(iterMap->second), mapTagReplacements);
  }
...
void ScriptManager::loadGame(const XmlNode *rootNode) {
  ...
  //	GraphicMessageBox messageBox;
  messageBox.setEnabled(
      scriptManagerNode->getAttribute("messageBox_enabled")->getIntValue() !=
      0);
  messageBox.setText(
      scriptManagerNode->getAttribute("messageBox_text")->getValue());
  messageBox.setHeader(
      scriptManagerNode->getAttribute("messageBox_header")->getValue());
  ...
  //	string lastDeadUnitKillerName;
  lastDeadUnitKillerName =
      scriptManagerNode->getAttribute("lastDeadUnitKillerName")->getValue();
  //	int lastDeadUnitKillerId;
  lastDeadUnitKillerId =
      scriptManagerNode->getAttribute("lastDeadUnitKillerId")->getIntValue();
  //
  //	//last attacked unit
  //	string lastAttackedUnitName;
  lastAttackedUnitName =
  ...
  //	int lastAttackedUnitId;
  lastAttackedUnitId =
      scriptManagerNode->getAttribute("lastAttackedUnitId")->getIntValue();
  //
  //	//last attacking unit
  //	string lastAttackingUnitName;
  lastAttackingUnitName =
      scriptManagerNode->getAttribute("lastAttackingUnitName")->getValue();
  //	int lastAttackingUnitId;
  lastAttackingUnitId =
      scriptManagerNode->getAttribute("lastAttackingUnitId")->getIntValue();
  //
  //	// end game state
  //	bool gameOver;
  ...
  //	bool inCellTriggerEvent;
  inCellTriggerEvent =
      scriptManagerNode->getAttribute("inCellTriggerEvent")->getIntValue() != 0;
  //	std::vector<int> unRegisterCellTriggerEventList;
  vector<XmlNode *> unRegisterCellTriggerEventListNodeList =
      scriptManagerNode->getChildList("unRegisterCellTriggerEventList");
  for (unsigned int i = 0; i < unRegisterCellTriggerEventListNodeList.size();
       ++i) {
    XmlNode *node = unRegisterCellTriggerEventListNodeList[i];
    unRegisterCellTriggerEventList.push_back(
        node->getAttribute("eventId")->getIntValue());
  }
  ...
  for (unsigned int i = 0; i < unitTriggerEventListNodeList.size(); ++i) {
    ...
    if (node->hasAttribute("eventType") == true) {
      eventType = static_cast<UnitTriggerEventType>(
  ...
  if (scriptManagerNode->hasAttribute("lastUnitTriggerEventType") == true) {
    lastUnitTriggerEventType = static_cast<UnitTriggerEventType>(

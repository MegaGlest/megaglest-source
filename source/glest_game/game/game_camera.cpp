...
GameCamera::GameCamera() : pos(0.f, defaultHeight, 0.f), destPos(0.f, defaultHeight, 0.f), destAng(startingVAng, startingHAng) {
...
Quad2i GameCamera::computeVisibleQuad() {
    ...
    if (hAng >= 135 && hAng <= 225) {
        ...
        if (MaxVisibleQuadItemCache != 0 && (MaxVisibleQuadItemCache < 0 || (int)cacheVisibleQuad[fov][hAng].size() <= MaxVisibleQuadItemCache)) {

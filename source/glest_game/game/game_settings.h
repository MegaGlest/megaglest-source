...
enum ModType { mt_None, mt_Map, mt_Tileset, mt_Techtree, mt_Scenario };
...
public:
  ...
  const vector<string> getUniqueNetworkPlayerLanguages() const {
    ...
    for (int i = 0; i < GameConstants::maxPlayers; ++i) {
      if (networkPlayerLanguages[i] != "") {
        if (std::find(languageList.begin(), languageList.end(),
  ...
  bool getEnableObserverModeAtEndGame() const {
    return enableObserverModeAtEndGame;
  }
  ...
  bool getNetworkPauseGameForLaggedClients() const {
    return networkPauseGameForLaggedClients;
  }
  ...
  void setScenarioDir(const string &scenarioDir) {
  ...
  void setResourceMultiplierIndex(int factionIndex, int multiplierIndex) {
    ...
    //	printf("gamesettings Line: %d multiplier index: %d factionIndex:
  ...
  void setDefaultResources(bool defaultResources) {
    this->defaultResources = defaultResources;
  }
  void setDefaultVictoryConditions(bool defaultVictoryConditions) {
  ...
  void setEnableObserverModeAtEndGame(bool value) {
    this->enableObserverModeAtEndGame = value;
  }
  void setEnableServerControlledAI(bool value) {
  ...
  void setFactionCRCList(const vector<pair<string, uint32>> &value) {
    factionCRCList = value;
  }

  int getAiAcceptSwitchTeamPercentChance() const {
    return aiAcceptSwitchTeamPercentChance;
  }
  void setAiAcceptSwitchTeamPercentChance(int value) {
  ...
  int getMasterserver_admin_faction_index() const {
    return masterserver_admin_factionIndex;
  }
  void setMasterserver_admin_faction_index(int value) {
    masterserver_admin_factionIndex = value;
  }

  bool getNetworkAllowNativeLanguageTechtree() const {
    return networkAllowNativeLanguageTechtree;
  }
  void setNetworkAllowNativeLanguageTechtree(bool value) {
  ...
  string toString() const {
    ...
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      ...
      result += "resourceMultiplierIndex = " +
                intToStr(resourceMultiplierIndex[idx]) + "\n";
      result += "team = " + intToStr(teams[idx]) + "\n";
      result +=
          "startLocationIndex = " + intToStr(startLocationIndex[idx]) + "\n";
    ...
    result +=
        "defaultVictoryConditions = " + intToStr(defaultVictoryConditions) +
        "\n";
    result += "fogOfWar = " + intToStr(fogOfWar) + "\n";
    result += "allowObservers = " + intToStr(allowObservers) + "\n";
    result += "enableObserverModeAtEndGame = " +
              intToStr(enableObserverModeAtEndGame) + "\n";
    result +=
        "enableServerControlledAI = " + intToStr(enableServerControlledAI) +
        "\n";
    result += "networkFramePeriod = " + intToStr(networkFramePeriod) + "\n";
    result += "networkPauseGameForLaggedClients = " +
    ...
    for (unsigned int i = 0; i < factionCRCList.size(); ++i) {
      result += "factionCRCList name [" + factionCRCList[i].first +
                "] CRC = " + uIntToStr(factionCRCList[i].second) + "\n";
    }

    result += "aiAcceptSwitchTeamPercentChance = " +
              intToStr(aiAcceptSwitchTeamPercentChance) + "\n";
    result += "masterserver_admin = " + intToStr(masterserver_admin) + "\n";
    result += "masterserver_admin_factionIndex = " +
              intToStr(masterserver_admin_factionIndex) + "\n";

    result += "networkAllowNativeLanguageTechtree = " +
  ...
  void saveGame(XmlNode *rootNode) const {
    ...
    //		string description;
    gameSettingsNode->addAttribute("description", description,
                                   mapTagReplacements);
    //		string map;
    ...
    //		string scenarioDir;
    gameSettingsNode->addAttribute("scenarioDir", scenarioDir,
                                   mapTagReplacements);
    //		string factionTypeNames[GameConstants::maxPlayers]; //faction
    // names
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      XmlNode *factionTypeNamesNode =
          gameSettingsNode->addChild("factionTypeNames");
      factionTypeNamesNode->addAttribute("name", factionTypeNames[idx],
                                         mapTagReplacements);
    }

    //		string networkPlayerNames[GameConstants::maxPlayers];
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      XmlNode *networkPlayerNamesNode =
          gameSettingsNode->addChild("networkPlayerNames");
      networkPlayerNamesNode->addAttribute("name", networkPlayerNames[idx],
                                           mapTagReplacements);
    }
    ...
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      ...
      networkPlayerNamesNode->addAttribute("name", networkPlayerPlatform[idx],
                                           mapTagReplacements);
    }

    //		int    networkPlayerStatuses[GameConstants::maxPlayers];
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      XmlNode *networkPlayerStatusesNode =
          gameSettingsNode->addChild("networkPlayerStatuses");
      networkPlayerStatusesNode->addAttribute(
          "status", intToStr(networkPlayerStatuses[idx]), mapTagReplacements);
    }
    ...
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      ...
      networkPlayerStatusesNode->addAttribute(
          "game_status", intToStr(networkPlayerGameStatus[idx]),
          mapTagReplacements);
    }

    //		string networkPlayerLanguages[GameConstants::maxPlayers];
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      XmlNode *networkPlayerLanguagesNode =
          gameSettingsNode->addChild("networkPlayerLanguages");
      networkPlayerLanguagesNode->addAttribute(
          "name", networkPlayerLanguages[idx], mapTagReplacements);
    }

    //		ControlType factionControls[GameConstants::maxPlayers];
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      XmlNode *factionControlsNode =
          gameSettingsNode->addChild("factionControls");
      factionControlsNode->addAttribute(
          "control", intToStr(factionControls[idx]), mapTagReplacements);
    }

    //		int resourceMultiplierIndex[GameConstants::maxPlayers];
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      XmlNode *resourceMultiplierIndexNode =
          gameSettingsNode->addChild("resourceMultiplierIndex");
      resourceMultiplierIndexNode->addAttribute(
          "multiplier", intToStr(resourceMultiplierIndex[idx]),
          mapTagReplacements);
    }

    //		int thisFactionIndex;
    gameSettingsNode->addAttribute(
        "thisFactionIndex", intToStr(thisFactionIndex), mapTagReplacements);
    //		int factionCount;
    gameSettingsNode->addAttribute("factionCount", intToStr(factionCount),
                                   mapTagReplacements);
    //		int teams[GameConstants::maxPlayers];
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
    ...
    //		int startLocationIndex[GameConstants::maxPlayers];
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      XmlNode *startLocationIndexNode =
          gameSettingsNode->addChild("startLocationIndex");
      startLocationIndexNode->addAttribute(
          "location", intToStr(startLocationIndex[idx]), mapTagReplacements);
    }

    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      XmlNode *networkPlayerUUIDNode =
          gameSettingsNode->addChild("networkPlayerUUID");
      networkPlayerUUIDNode->addAttribute("value", networkPlayerUUID[idx],
                                          mapTagReplacements);
    }

    //		int mapFilterIndex;
    gameSettingsNode->addAttribute("mapFilterIndex", intToStr(mapFilter),
                                   mapTagReplacements);
    //
    //
    //		bool defaultUnits;
    gameSettingsNode->addAttribute("defaultUnits", intToStr(defaultUnits),
                                   mapTagReplacements);
    //		bool defaultResources;
    gameSettingsNode->addAttribute(
        "defaultResources", intToStr(defaultResources), mapTagReplacements);
    //		bool defaultVictoryConditions;
    ...
    gameSettingsNode->addAttribute("fogOfWar", intToStr(fogOfWar),
                                   mapTagReplacements);
    //		bool allowObservers;
    gameSettingsNode->addAttribute("allowObservers", intToStr(allowObservers),
                                   mapTagReplacements);
    //		bool enableObserverModeAtEndGame;
    ...
    //		int networkFramePeriod;
    gameSettingsNode->addAttribute(
        "networkFramePeriod", intToStr(networkFramePeriod), mapTagReplacements);
    //		bool networkPauseGameForLaggedClients;
    gameSettingsNode->addAttribute("networkPauseGameForLaggedClients",
                                   intToStr(networkPauseGameForLaggedClients),
                                   mapTagReplacements);
    //		PathFinderType pathFinderType;
    gameSettingsNode->addAttribute("pathFinderType", intToStr(pathFinderType),
                                   mapTagReplacements);
    //		uint32 flagTypes1;
    ...
    //	    int masterserver_admin;
    gameSettingsNode->addAttribute(
        "masterserver_admin", intToStr(masterserver_admin), mapTagReplacements);
  ...
  void loadGame(const XmlNode *rootNode) {
    ...
    // names
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      const XmlNode *factionTypeNamesNode =
    ...
    //		string networkPlayerNames[GameConstants::maxPlayers];
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      const XmlNode *networkPlayerNamesNode =
    ...
    //		int    networkPlayerStatuses[GameConstants::maxPlayers];
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      const XmlNode *networkPlayerStatusesNode =
          gameSettingsNode->getChild("networkPlayerStatuses", idx);
      networkPlayerStatuses[idx] =
          networkPlayerStatusesNode->getAttribute("status")->getIntValue();
    }

    //		int    networkPlayerStatuses[GameConstants::maxPlayers];
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      if (gameSettingsNode->hasChildAtIndex("networkPlayerGameStatus", idx) ==
          true) {
        const XmlNode *networkPlayerGameStatusNode =
    ...
    //		string networkPlayerLanguages[GameConstants::maxPlayers];
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      const XmlNode *networkPlayerLanguagesNode =
    ...
    //		ControlType factionControls[GameConstants::maxPlayers];
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      const XmlNode *factionControlsNode =
          gameSettingsNode->getChild("factionControls", idx);
      factionControls[idx] = static_cast<ControlType>(
          factionControlsNode->getAttribute("control")->getIntValue());
    }

    //		int resourceMultiplierIndex[GameConstants::maxPlayers];
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      const XmlNode *resourceMultiplierIndexNode =
    ...
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      if (gameSettingsNode->hasChildAtIndex("networkPlayerUUID", idx) == true) {
        const XmlNode *networkPlayerUUIDNode =
    ...
    //		int startLocationIndex[GameConstants::maxPlayers];
    for (int idx = 0; idx < GameConstants::maxPlayers; idx++) {
      const XmlNode *startLocationIndexNode =
          gameSettingsNode->getChild("startLocationIndex", idx);
      startLocationIndex[idx] =
          startLocationIndexNode->getAttribute("location")->getIntValue();
    }
    ...
    //		bool defaultUnits;
    defaultUnits =
        gameSettingsNode->getAttribute("defaultUnits")->getIntValue() != 0;
    //		bool defaultResources;
    defaultResources =
        gameSettingsNode->getAttribute("defaultResources")->getIntValue() != 0;
    //		bool defaultVictoryConditions;
    ...
    //		bool allowObservers;
    allowObservers =
        gameSettingsNode->getAttribute("allowObservers")->getIntValue() != 0;
    //		bool enableObserverModeAtEndGame;
    ...
    //		int networkFramePeriod;
    networkFramePeriod =
        gameSettingsNode->getAttribute("networkFramePeriod")->getIntValue();
    //		bool networkPauseGameForLaggedClients;
    networkPauseGameForLaggedClients =
        gameSettingsNode->getAttribute("networkPauseGameForLaggedClients")
            ->getIntValue() != 0;
    //		PathFinderType pathFinderType;
    pathFinderType = static_cast<PathFinderType>(
    ...
    // mapTagReplacements);
    //			factionCRCListNode->addAttribute("value",intToStr(item.second),
    // mapTagReplacements);
    //		}
    //	    int aiAcceptSwitchTeamPercentChance;
    ...
    //	    int masterserver_admin;
    masterserver_admin =
        gameSettingsNode->getAttribute("masterserver_admin")->getIntValue();

    if (gameSettingsNode->hasAttribute("masterserver_admin_factionIndex") ==
        true) {
      masterserver_admin_factionIndex =
          gameSettingsNode->getAttribute("masterserver_admin_factionIndex")
              ->getIntValue();
    }

    if (gameSettingsNode->hasAttribute("networkAllowNativeLanguageTechtree") ==
        true) {
      networkAllowNativeLanguageTechtree =
          gameSettingsNode->getAttribute("networkAllowNativeLanguageTechtree")
              ->getIntValue() != 0;
    }
  }
};

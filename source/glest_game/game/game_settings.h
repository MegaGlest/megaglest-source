...
enum NetworkPlayerStatusType { npst_None = 0, npst_PickSettings = 1, npst_BeRightBack = 2, npst_Ready = 3, npst_Disconnected = 4 };
  ...
  public:
    ...
    bool isNetworkGame() const {
        ...
        for (int idx = 0; idx < GameConstants::maxPlayers; ++idx) {
            if (factionControls[idx] == ctNetwork || factionControls[idx] == ctNetworkUnassigned || networkPlayerStatuses[idx] == npst_Disconnected) {
                result = true;
                break;
            }
        }

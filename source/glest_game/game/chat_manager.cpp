...
void ChatManager::keyDown(SDL_KeyboardEvent key) {
    ...
    try {
        ...
        // toggle team mode
        if (editEnabled == false && isKeyPressed(configKeys.getSDLKey("ChatTeamMode"), key) == true) {
            if (disableTeamMode == true) {
                if (!inMenu) {
        ...
        if (isKeyPressed(SDLK_RETURN, key, false) == true) {
            ...
            if (text.empty() == false) {
                ...
                for (unsigned int factionIndex = 0; factionIndex < (unsigned int)settings->getFactionCount(); ++factionIndex) {
                    ...
                    if (playerName.length() > autoCompleteName.length() && StartsWith(toLower(playerName), toLower(autoCompleteName)) == true) {
                ...
                if (autoCompleteResult == "") {
                    ...
                    for (unsigned int index = 0; index < (unsigned int)autoCompleteTextList.size(); ++index) {
                        ...
                        if (autoText.length() > autoCompleteName.length() && StartsWith(toLower(autoText), toLower(autoCompleteName)) == true) {
...
void ChatManager::updateNetwork() {
    try {
        ...
        if (gameNetworkInterface != NULL && gameNetworkInterface->getChatTextList(false).empty() == false) {
            Lang &lang = Lang::getInstance();

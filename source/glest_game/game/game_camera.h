...
class GameCamera {
  ...
  public:
    ...
    inline bool isMoving() const { return (this->move.x != 0.0 || this->move.y != 0.0 || this->move.z != 0.0); }
    ...
    void moveForwardH(float dist,
                      float response); // response: 1.0 for immediate, 0 for full inertia

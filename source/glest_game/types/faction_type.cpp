...
void FactionType::load(
    const string &factionName, const TechTree *techTree, Checksum *checksum,
  ...
  for (bool realFactionPathFound = false; realFactionPathFound == false;) {
    ...
    if (personalityType == fpt_Normal) {
      ...
      XmlTree xmlTree;
      xmlTree.load(tmppath, Properties::getTagReplacementValues(
      ...
      if (rootNode->getName() == "link") {
        ...
        loadedFileList[linkedTmppath].push_back(
            make_pair(linkedCurrentPath, linkedCurrentPath));
  ...
  snprintf(szBuf, 8096,
           Lang::getInstance()
               .getString("LogScreenGameLoadingFactionType", "", true)
               .c_str(),
  ...
  if (personalityType == fpt_Normal) {
    ...
    try {
      ...
      for (int i = 0; i < (int)unitTypes.size(); ++i) {
        ...
        try {
          unitTypes[i].loaddd(i, str, techTree, techTreePath, this, checksum,
                              techtreeChecksum, loadedFileList, validationMode);
          logger.setProgress(
              progressBaseValue +
              (int)((((double)i + 1.0) / (double)unitTypes.size()) * 100.0 /
                    techTree->getTypeCount()));
          SDL_PumpEvents();
      ...
      throw megaglest_runtime_error(
          "Error loading units: " + currentPath + "\nMessage: " + ex.what(),
          !ex.wantStackTrace());
      ...
      try {
        ...
        if (validationMode == false) {
          ...
          SystemFlags::OutputDebug(
              ...
              "StartResource: %s\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, ex.what(), this->name.c_str(), name.c_str());
        }
      }
    ...
    if (value) {
      music = new StrSound();
      music->open(
          musicNode->getAttribute("path")->getRestrictedValue(currentPath));
      loadedFileList[musicNode->getAttribute("path")->getRestrictedValue(
                         currentPath)]
          .push_back(make_pair(
              path, musicNode->getAttribute("path")->getRestrictedValue()));
    }
    ...
    if (factionNode->hasChild("healthbar")) {
      ...
      if (healthbarNode->hasChild("borderTexture")) {
        healthbarBorderTextureEnabled = healthbarNode->getChild("borderTexture")
                                            ->getAttribute("enabled")
                                            ->getBoolValue();
        if (healthbarBorderTextureEnabled &&
            healthbarNode->getChild("borderTexture")->hasAttribute("path")) {
          healthbarTexture = Renderer::getInstance().newTexture2D(rsGame);
          if (healthbarTexture) {
            healthbarTexture->load(healthbarNode->getChild("borderTexture")
                                       ->getAttribute("path")
          ...
          loadedFileList[healthbarNode->getChild("borderTexture")
                             ...
                             ->getRestrictedValue(currentPath)]
              .push_back(
                  make_pair(path, healthbarNode->getChild("borderTexture")
                                      ->getAttribute("path")
                                      ->getRestrictedValue()));
        }
      }
      if (healthbarNode->hasChild("backgroundTexture")) {
        ...
        if (healthbarBackgroundTextureEnabled &&
          ...
          if (healthbarBackgroundTexture) {
            healthbarBackgroundTexture->load(
                healthbarNode->getChild("backgroundTexture")
                    ->getAttribute("path")
                    ->getRestrictedValue(currentPath));
          }
          loadedFileList[healthbarNode->getChild("backgroundTexture")
                             ...
                             ->getRestrictedValue(currentPath)]
              .push_back(
                  make_pair(path, healthbarNode->getChild("backgroundTexture")
                                      ->getAttribute("path")
                                      ->getRestrictedValue()));
        }
      }
      if (healthbarNode->hasChild("lineBorder")) {
        healthbarLineBorder = healthbarNode->getChild("lineBorder")
                                  ->getAttribute("enabled")
                                  ->getBoolValue();
      }
    }
    ...
    if (factionNode->hasChild("ai-behavior") == true) {
      ...
      if (aiNode->hasAttribute("min-static-resource-count") == true) {
        mapAIBehaviorStaticOverrideValues[aibsvcMinStaticResourceCount] =
            aiNode->getAttribute("min-static-resource-count")->getIntValue();
      }
      ...
      if (aiNode->hasChild("worker-units") == true) {
        ...
        for (int i = 0; i < (int)aiNodeUnits->getChildCount(); ++i) {
          ...
          mapAIBehaviorUnitCategories[aibcWorkerUnits].push_back(
              PairPUnitTypeInt(getUnitType(name), minimum));
        }
      }
      if (aiNode->hasChild("warrior-units") == true) {
        ...
        for (int i = 0; i < (int)aiNodeUnits->getChildCount(); ++i) {
          ...
          mapAIBehaviorUnitCategories[aibcWarriorUnits].push_back(
              PairPUnitTypeInt(getUnitType(name), minimum));
        }
      }
      if (aiNode->hasChild("resource-producer-units") == true) {
        ...
        for (int i = 0; i < (int)aiNodeUnits->getChildCount(); ++i) {
          ...
          mapAIBehaviorUnitCategories[aibcResourceProducerUnits].push_back(
              PairPUnitTypeInt(getUnitType(name), minimum));
        }
      }
      if (aiNode->hasChild("building-units") == true) {
        ...
        for (int i = 0; i < (int)aiNodeUnits->getChildCount(); ++i) {
          ...
          mapAIBehaviorUnitCategories[aibcBuildingUnits].push_back(
              PairPUnitTypeInt(getUnitType(name), minimum));
        }
      }
...
int FactionType::getAIBehaviorStaticOverideValue(
...
std::vector<std::string> FactionType::validateFactionType() {
  ...
  if (factionMusic != NULL &&
    ...
    snprintf(szBuf, 8096,
             ...
             this->getName().c_str(), factionMusic->getFileName().c_str(),
             factionMusic->getInfo()->getBitRate(), MAX_BITRATE_WARNING);
  ...
  for (int i = 0; i < (int)unitTypes.size(); ++i) {
    ...
    for (int i = 0; i < (int)unitType.getSelectionSounds().getSounds().size();
         ++i) {
      ...
      if (sound != NULL &&
        ...
        snprintf(szBuf, 8096,
                 ...
                 sound->getFileName().c_str(), sound->getInfo()->getBitRate(),
                 MAX_BITRATE_WARNING);
        results.push_back(szBuf);
      }
    }
    for (int i = 0; i < (int)unitType.getCommandSounds().getSounds().size();
         ++i) {
      ...
      if (cmdType != NULL) {
        ...
        for (int k = 0; k < cmdType->getUpgradeReqCount(); ++k) {
          ...
          if (upgradeType != NULL) {
            ...
            for (int l = 0;
                 l < (int)unitTypes.size() && foundUpgraderUnit == false; ++l) {
              UnitType &unitType2 = unitTypes[l];
              for (int m = 0; m < unitType2.getCommandTypeCount() &&
                              foundUpgraderUnit == false;
                   ++m) {
                ...
                if (cmdType2 != NULL &&
                  ...
                  if (uct != NULL) {
                    ...
                    if (upgradeType2 != NULL &&
                        upgradeType2->getName() == upgradeType->getName()) {
        ...
        if (cmdType != NULL) {
          ...
          if (cmdType->getClass() == ccBuild) {
            ...
            for (int k = 0; k < build->getBuildingCount() && foundUnit == false;
              ...
              if (buildUnit != NULL && unitType.getId() != unitType2.getId() &&
          ...
          if (cmdType->getClass() == ccMorph) {
            ...
            if (morphUnit != NULL && unitType.getId() != unitType2.getId() &&
        ...
        if (st != NULL && dynamic_cast<const AttackSkillType *>(st) != NULL) {
          ...
          if (ast != NULL && ast->getAttackVar() < 0) {
            ...
            snprintf(szBuf, 8096,
                     ...
                     unitType.getName().c_str(), this->getName().c_str(),
                     ast->getName().c_str(), ast->getAttackVar());
            results.push_back(szBuf);
          }
        }
      }
...
std::vector<std::string> FactionType::validateFactionTypeResourceTypes(
...
std::vector<std::string> FactionType::validateFactionTypeUpgradeTypes() {
  ...
  for (int i = 0; i < (int)upgradeTypes.size(); ++i) {
    ...
    for (int j = 0; j < (int)unitTypes.size() && foundUnit == false; ++j) {
      ...
      for (int k = 0; k < unitType.getCommandTypeCount() && foundUnit == false;
           ++k) {
...
const UnitType *FactionType::getUnitType(const string &name) const {
  ...
  for (int i = 0; i < (int)unitTypes.size(); i++) {
    printf("In [%s::%s Line: %d] scanning [%s] idx = %d [%s]\n",
           extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
           __LINE__, name.c_str(), i, unitTypes[i].getName(false).c_str());
  }
...
const UpgradeType *FactionType::getUpgradeType(const string &name) const {
  ...
  for (int i = 0; i < (int)upgradeTypes.size(); i++) {
    printf("In [%s::%s Line: %d] scanning [%s] idx = %d [%s]\n",
           extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
           __LINE__, name.c_str(), i, upgradeTypes[i].getName().c_str());
  }
...
int FactionType::getStartingResourceAmount(
...
bool FactionType::factionUsesResourceType(const ResourceType *rt) const {
  ...
  if (rt != NULL) {
    for (unsigned int j = 0; factionUsesResourceType == false &&
                             j < (unsigned int)this->getUnitTypeCount();
         ++j) {
      const UnitType *ut = this->getUnitType(j);
      for (int k = 0;
           factionUsesResourceType == false && k < ut->getCostCount(); ++k) {
      ...
      if (factionUsesResourceType == false) {
        for (unsigned int k = 0; factionUsesResourceType == false &&
                                 k < (unsigned int)ut->getCommandTypeCount();
             ++k) {
          ...
          if (commandType != NULL && commandType->getClass() == ccHarvest) {
            ...
            if (hct != NULL && hct->getHarvestedResourceCount() > 0) {
              for (unsigned int l = 0;
                   ...
                   l < (unsigned int)hct->getHarvestedResourceCount();
                   ++l) {
...
std::string FactionType::toString() const {
  ...
  result +=
      "Upgrade Type List count = " + intToStr(this->getUpgradeTypeCount()) +
      "\n";
  for (int i = 0; i < (int)upgradeTypes.size(); i++) {
    result += "index: " + intToStr(i) + " " +

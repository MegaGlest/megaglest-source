...
void FactionType::load(const string &factionName, const TechTree *techTree, Checksum *checksum, Checksum *techtreeChecksum,
    ...
    for (bool realFactionPathFound = false; realFactionPathFound == false;) {
        ...
        if (personalityType == fpt_Normal) {
            ...
            // techTreePath + "/commondata/"; printf("current $COMMONDATAPATH =
            ...
            if (rootNode->getName() == "link") {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugSystem, "Faction [%s] is a linked faction\n", name.c_str());
                ...
                //			const XmlNode *factionLinkNode=
                // rootNode->getChild("faction"); 			string
                // linkedFactionName=factionLinkNode->getAttribute("name")->getRestrictedValue();
                ...
                // printf("linkedTmppath [%s] linkedCurrentPath
                ...
                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugSystem, "techTreePath [%s] techTreeName [%s]\n", techTreePath.c_str(), techTreeName.c_str());
            } else {
    ...
    if (personalityType == fpt_Normal) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "Loading faction [%s] currentPath [%s]\n", path.c_str(), currentPath.c_str());
        ...
        try {
            ...
            for (int i = 0; i < (int)unitTypes.size(); ++i) {
                ...
                try {
                    unitTypes[i].loaddd(i, str, techTree, techTreePath, this, checksum, techtreeChecksum, loadedFileList, validationMode);
        ...
        try {
            for (int i = 0; i < (int)upgradeTypes.size(); ++i) {
                ...
                try {
                    upgradeTypes[i].load(str, techTree, this, checksum, techtreeChecksum, loadedFileList, validationMode);
                } catch (megaglest_runtime_error &ex) {
                    if (validationMode == false) {
                        throw;
                    } else {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "Loading faction xml [%s]\n", tmppath.c_str());

        std::map<string, string> mapExtraTagReplacementValues;
        mapExtraTagReplacementValues["$COMMONDATAPATH"] = techTreePath + "/commondata/";
        // printf("current $COMMONDATAPATH =
        // %s\n",mapExtraTagReplacementValues["$COMMONDATAPATH"].c_str());
        xmlTree.load(tmppath, Properties::getTagReplacementValues(&mapExtraTagReplacementValues));
            ...
            try {
                ...
                if (validationMode == false) {
                    ...
                    SystemFlags::OutputDebug(SystemFlags::debugError,
                                             "In [%s::%s Line: %d] Error [%s]\nFor FactionType: %s for "
                                             "StartResource: %s\n",
                                             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, ex.what(), this->name.c_str(),
                                             name.c_str());
                }
            }
        ...
        if (value) {
            ...
            loadedFileList[musicNode->getAttribute("path")->getRestrictedValue(currentPath)].push_back(
                make_pair(path, musicNode->getAttribute("path")->getRestrictedValue()));
        }
        ...
        if (factionNode->hasChild("healthbar")) {
            ...
            if (healthbarNode->hasChild("borderTexture")) {
                ...
                if (healthbarBorderTextureEnabled && healthbarNode->getChild("borderTexture")->hasAttribute("path")) {
                    ...
                    loadedFileList[healthbarNode->getChild("borderTexture")->getAttribute("path")->getRestrictedValue(currentPath)].push_back(
                        make_pair(path, healthbarNode->getChild("borderTexture")->getAttribute("path")->getRestrictedValue()));
                }
            }
            if (healthbarNode->hasChild("backgroundTexture")) {
                ...
                if (healthbarBackgroundTextureEnabled && healthbarNode->getChild("backgroundTexture")->hasAttribute("path")) {
                    ...
                    loadedFileList[healthbarNode->getChild("backgroundTexture")->getAttribute("path")->getRestrictedValue(currentPath)].push_back(
                        make_pair(path, healthbarNode->getChild("backgroundTexture")->getAttribute("path")->getRestrictedValue()));
                }
            }
            ...
            if (aiNode->hasChild("static-values") == true) {
                ...
                for (int i = 0; i < (int)aiNodeUnits->getChildCount(); ++i) {
                    ...
                    if (unitNode->hasAttribute("type") == true) {
                        type = static_cast<AIBehaviorStaticValueCategory>(unitNode->getAttribute("type")->getIntValue());
                    } else {
                        type = EnumParser<AIBehaviorStaticValueCategory>::getEnum(unitNode->getAttribute("type-name")->getValue());
                        // printf("Discovered overriden static value for AI, type = %d,
                        // value = %d\n",type,value);
                    }

                    int value = unitNode->getAttribute("value")->getIntValue();
                    mapAIBehaviorStaticOverrideValues[type] = value;
                    // printf("Discovered overriden static value for AI, type = %d, value
                    // = %d\n",type,value);
                }
            }
...
int FactionType::getAIBehaviorStaticOverideValue(AIBehaviorStaticValueCategory type) const {
    int result = INT_MAX;
    std::map<AIBehaviorStaticValueCategory, int>::const_iterator iterFind = mapAIBehaviorStaticOverrideValues.find(type);
...
std::vector<std::string> FactionType::validateFactionType() {
    ...
    if (factionMusic != NULL && factionMusic->getInfo()->getBitRate() > MAX_BITRATE_WARNING) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 "The Faction [%s] has the music [%s]\nwhich has a bitrate of [%u] "
                 "which may cause some sound drivers to crash, please use a "
                 "bitrate of %d or less!",
                 this->getName().c_str(), factionMusic->getFileName().c_str(), factionMusic->getInfo()->getBitRate(), MAX_BITRATE_WARNING);
    ...
    for (int i = 0; i < (int)unitTypes.size(); ++i) {
        ...
        for (int i = 0; i < (int)unitType.getSelectionSounds().getSounds().size(); ++i) {
            ...
            if (sound != NULL && sound->getInfo()->getBitRate() > MAX_BITRATE_WARNING) {
                ...
                snprintf(szBuf, 8096,
                         ...
                         "a bitrate of [%u] which may cause some sound drivers to "
                         "crash, please use a bitrate of %d or less!",
                         unitType.getName().c_str(), this->getName().c_str(), sound->getFileName().c_str(), sound->getInfo()->getBitRate(),
                         MAX_BITRATE_WARNING);
                results.push_back(szBuf);
            }
        }
        ...
        for (int j = 0; j < (int)unitType.getCommandTypeCount(); ++j) {
            ...
            if (cmdType != NULL) {
                // Check every unit's commands to validate that for every
                // upgrade-requirements upgrade we have a unit that can do the upgrade
                // in the faction.
                for (int k = 0; k < cmdType->getUpgradeReqCount(); ++k) {
                    ...
                    if (upgradeType != NULL) {
                        ...
                        if (foundUpgraderUnit == false) {
                            char szBuf[8096] = "";
                            snprintf(szBuf, 8096,
                                     "The Unit [%s] in Faction [%s] has the command "
                                     "[%s]\nwhich has upgrade requirement [%s] but there are "
                                     "no units able to perform the upgrade!",
                                     unitType.getName().c_str(), this->getName().c_str(), cmdType->getName().c_str(), upgradeType->getName().c_str());
                            results.push_back(szBuf);
                        }
                    }
                }
                ...
                if (cmdType->getClass() == ccBuild) {
                    ...
                    for (int k = 0; k < build->getBuildingCount(); ++k) {
                        ...
                        for (int l = 0; l < (int)unitTypes.size() && foundUnit == false; ++l) {
                            ...
                            if (unitType2.getName() == buildUnit->getName()) {
                                ...
                                if (buildUnit->hasSkillClass(scBeBuilt) == false) {
                                    ...
                                    snprintf(szBuf, 8096,
                                             "The Unit [%s] in Faction [%s] has the command "
                                             "[%s]\nwhich can build the Unit [%s] but the Unit "
                                             "to be built\ndoes not have the skill class "
                                             "[be_built_skill] in this faction!",
                                             unitType.getName().c_str(), this->getName().c_str(), cmdType->getName().c_str(), buildUnit->getName().c_str());
                                    results.push_back(szBuf);
                                }
                        ...
                        if (foundUnit == false) {
                            ...
                            snprintf(szBuf, 8096,
                                     "The Unit [%s] in Faction [%s] has the command "
                                     "[%s]\nwhich can build the Unit [%s] but the Unit to be "
                                     "built does not exist in this faction!",
                                     unitType.getName().c_str(), this->getName().c_str(), cmdType->getName().c_str(), buildUnit->getName().c_str());
                            results.push_back(szBuf);
                        }
                    }
                ...
                if (cmdType->getClass() == ccRepair) {
                    ...
                    for (int k = 0; k < repair->getRepairCount(); ++k) {
                        ...
                        if (foundUnit == false) {
                            ...
                            snprintf(szBuf, 8096,
                                     "The Unit [%s] in Faction [%s] has the command "
                                     "[%s]\nwhich can repair the Unit [%s] but the Unit to "
                                     "be repaired does not exist in this faction!",
                                     unitType.getName().c_str(), this->getName().c_str(), cmdType->getName().c_str(), repairUnit->getName().c_str());
                            results.push_back(szBuf);
                        }
                    }
                }
                ...
                if (cmdType->getClass() == ccMorph) {
                    ...
                    if (morph != NULL) {
                        ...
                        if (foundUnit == false) {
                            ...
                            snprintf(szBuf, 8096,
                                     "The Unit [%s] in Faction [%s] has the command "
                                     "[%s]\nwhich can morph into the Unit [%s] but the Unit "
                                     "to be morphed to does not exist in this faction!",
                                     unitType.getName().c_str(), this->getName().c_str(), cmdType->getName().c_str(), morphUnit->getName().c_str());
                            results.push_back(szBuf);
                        }
                    }
                }
        ...
        if (morphCommandCount > maxMorphsAllowed) {
            char szBuf[8096] = "";
            snprintf(szBuf, 8096,
                     "The Unit [%s] in Faction [%s] has more than %d morph commands "
                     "which is too many to display in the UI!",
                     unitType.getName().c_str(), this->getName().c_str(), maxMorphsAllowed);
            results.push_back(szBuf);
        }

        // Check every unit's unit requirements to validate that for every
        // unit-requirements we have the units required in the faction.
            ...
            if (unitType2 != NULL) {
                ...
                if (foundUnit == false) {
                    ...
                    snprintf(szBuf, 8096,
                             ...
                             "the required unit does not exist in this faction!",
                             unitType.getName().c_str(), this->getName().c_str(), unitType2->getName().c_str());
                    results.push_back(szBuf);
                }
            }
        }

        // Now check that at least 1 other unit can produce, build or morph this
        // unit
        bool foundUnit = false;
        for (int l = 0; l < (int)unitTypes.size() && foundUnit == false; ++l) {
            UnitType &unitType2 = unitTypes[l];
                ...
                if (cmdType != NULL) {
                    ...
                    if (cmdType->getClass() == ccProduce) {
                        ...
                        if (produce != NULL) {
                            ...
                            if (produceUnit != NULL && unitType.getId() != unitType2.getId() && unitType.getName() == produceUnit->getName()) {
                                foundUnit = true;
                                break;
                            }
                        }
                    ...
                    if (cmdType->getClass() == ccBuild) {
                        ...
                        for (int k = 0; k < build->getBuildingCount() && foundUnit == false; ++k) {
                            ...
                            if (buildUnit != NULL && unitType.getId() != unitType2.getId() && unitType.getName() == buildUnit->getName()) {
                                foundUnit = true;
                                break;
                            }
                        }
                    ...
                    if (cmdType->getClass() == ccMorph) {
                        ...
                        if (morphUnit != NULL && unitType.getId() != unitType2.getId() && unitType.getName() == morphUnit->getName()) {
                            foundUnit = true;
                            break;
                        }
                    }
                    ...
                    if (cmdType->getClass() == ccAttack) {
                        ...
                        if (act != NULL && act->getAttackSkillType() != NULL && act->getAttackSkillType()->getSpawnUnit() != "" &&
                            act->getAttackSkillType()->getSpawnUnitCount() > 0) {
                            if (unitType.getId() != unitType2.getId() && unitType.getName() == act->getAttackSkillType()->getSpawnUnit()) {
                                foundUnit = true;
                                break;
                            }
                        }
        ...
        if (foundUnit == false) {
            ...
            char szBuf[8096] = "";
            snprintf(szBuf, 8096,
                     "The Unit [%s] in Faction [%s] has no other units that can "
                     "produce, build or morph into it in this faction!",
            ...
            for (int j = 0; j < unitType.getSkillTypeCount(); ++j) {
                ...
                if (st != NULL && dynamic_cast<const AttackSkillType *>(st) != NULL) {
                    ...
                    if (ast != NULL && ast->getAttackVar() < 0) {
                        char szBuf[8096] = "";
                        snprintf(szBuf, 8096,
                                 "The Unit [%s] in Faction [%s] has the skill [%s] with an "
                                 "INVALID attack var value which is < 0 [%d]!",
                                 unitType.getName().c_str(), this->getName().c_str(), ast->getName().c_str(), ast->getAttackVar());
                        results.push_back(szBuf);
                    }
                }
            }
        ...
        // Check if the unit has both be_built and harvest skills, this may cause
        // issues with the AI
        if (unitType.hasSkillClass(scBeBuilt) == true && unitType.hasSkillClass(scHarvest) == true) {
            char szBuf[8096] = "";
            snprintf(szBuf, 8096,
                     "The Unit [%s] in Faction [%s] has both a bebuilt and harvest "
                     "skill which will cause AI problems for CPU players!",
                     unitType.getName().c_str(), this->getName().c_str());
            results.push_back(szBuf);
        }
        // end
        ...
        if (unitType.hasSkillClass(scHarvest) == true && unitType.hasSkillClass(scMove) == false) {
            char szBuf[8096] = "";
            snprintf(szBuf, 8096,
                     "The Unit [%s] in Faction [%s] has a harvest skill but no move "
                     "skill so it cannot harvest!",
                     unitType.getName().c_str(), this->getName().c_str());
            results.push_back(szBuf);
        }
        // end
...
std::vector<std::string> FactionType::validateFactionTypeResourceTypes(vector<ResourceType> &resourceTypes) {
    ...
    for (int i = 0; i < (int)unitTypes.size(); ++i) {
        ...
        // Check every unit's required resources to validate that for every
        // resource-requirements we have a resource in the faction.
        for (int j = 0; j < unitType.getCostCount(); ++j) {
            ...
            if (r != NULL && r->getType() != NULL) {
                ...
                if (foundResourceType == false) {
                    char szBuf[8096] = "";
                    snprintf(szBuf, 8096,
                             "The Unit [%s] in Faction [%s] has the resource req "
                             "[%s]\nbut there are no such resources in this tech!",
                             unitType.getName().c_str(), this->getName().c_str(), r->getType()->getName().c_str());
                    results.push_back(szBuf);
                }
            }
        }

        // Check every unit's stored resources to validate that for every
        // resources-stored we have a resource in the faction.
        for (int j = 0; j < unitType.getStoredResourceCount(); ++j) {
            ...
            if (r != NULL && r->getType() != NULL) {
                ...
                if (foundResourceType == false) {
                    char szBuf[8096] = "";
                    snprintf(szBuf, 8096,
                             "The Unit [%s] in Faction [%s] has the stored resource "
                             "[%s]\nbut there are no such resources in this tech!",
                             unitType.getName().c_str(), this->getName().c_str(), r->getType()->getName().c_str());
                    results.push_back(szBuf);
                }
                    ...
                    for (int k = 0; k < harvest->getHarvestedResourceCount(); ++k) {
                        ...
                        if (foundResourceType == false) {
                            ...
                            snprintf(szBuf, 8096,
                                     ...
                                     "which can harvest the resource [%s]\nbut there are no "
                                     "such resources in this tech!",
                                     unitType.getName().c_str(), this->getName().c_str(), cmdType->getName().c_str(), harvestResource->getName().c_str());
                            results.push_back(szBuf);
                        }
                    }
                }
...
std::vector<std::string> FactionType::validateFactionTypeUpgradeTypes() {
    ...
    for (int i = 0; i < (int)upgradeTypes.size(); ++i) {
        ...
        if (foundUnit == false) {
            char szBuf[8096] = "";
            snprintf(szBuf, 8096,
                     "The Upgrade Type [%s] in Faction [%s] has no Unit able to "
                     "produce this upgrade in this faction!",
                     upgradeType.getName().c_str(), this->getName().c_str());
            results.push_back(szBuf);
        }
    }
...
const UnitType *FactionType::getUnitType(const string &name) const {
    ...
    for (int i = 0; i < (int)unitTypes.size(); i++) {
        printf("In [%s::%s Line: %d] scanning [%s] idx = %d [%s]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, name.c_str(), i,
...
//     for(int i=0; i < (int)unitTypes.size();i++){
//     	printf("In [%s::%s Line: %d] scanning [%s] idx = %d
...
//     }
//
//	throw megaglest_runtime_error("Unit type not found: [" + intToStr(id) +
//"] in faction type [" + this->name + "]",true);
// }
    ...
    if (rt != NULL) {
        for (unsigned int j = 0; factionUsesResourceType == false && j < (unsigned int)this->getUnitTypeCount(); ++j) {
            ...
            for (int k = 0; factionUsesResourceType == false && k < ut->getCostCount(); ++k) {
                ...
                // printf("#1 factionUsesResourceType, unit [%s] resource [%s] cost
                // [%s]\n",ut->getName().c_str(),rt->getName().c_str(),costResource->getType()->getName().c_str());

                if (costResource != NULL && costResource->getType() != NULL && costResource->getType()->getName() == rt->getName()) {
                    ...
                    if (commandType != NULL && commandType->getClass() == ccHarvest) {
                        ...
                        if (hct != NULL && hct->getHarvestedResourceCount() > 0) {
                            for (unsigned int l = 0; factionUsesResourceType == false && l < (unsigned int)hct->getHarvestedResourceCount(); ++l) {
                                // printf("#2 factionUsesResourceType, unit [%s] resource [%s]
                                // harvest

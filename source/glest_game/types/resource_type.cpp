...
void ResourceType::load(const string &dir, Checksum *checksum, Checksum *techtreeChecksum, std::map<string, vector<pair<string, string>>> &loadedFileList,
                        string techtreePath) {
    string path, str;
            ...
            if (modelNode->hasChild("particles")) {
                ...
                if (particleEnabled == true) {
                    for (int k = 0; k < (int)particleNode->getChildCount(); ++k) {
                        ...
                        objectParticleSystemType->load(particleFileNode, dir, currentPath + particlePath, &Renderer::getInstance(), loadedFileList,
                                                       sourceXMLFile, techtreePath);
                        loadedFileList[currentPath + particlePath].push_back(

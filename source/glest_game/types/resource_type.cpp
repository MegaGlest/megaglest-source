...
void ResourceType::load(
    const string &dir, Checksum *checksum, Checksum *techtreeChecksum,
  ...
  try {
    ...
    snprintf(szBuf, 8096,
             Lang::getInstance()
                 .getString("LogScreenGameLoadingResourceType", "", true)
                 .c_str(),
    ...
    switch (resourceClass) {
      case rcTech: {
        ...
        model = renderer.newModel(rsGame, modelPath, false, &loadedFileList,
                                  &sourceXMLFile);
        loadedFileList[modelPath].push_back(
            make_pair(sourceXMLFile,
                      modelNode->getAttribute("path")->getRestrictedValue()));
          ...
          if (particleEnabled == true) {
            for (int k = 0; k < (int)particleNode->getChildCount(); ++k) {
              ...
              objectParticleSystemType->load(
                  particleFileNode, dir, currentPath + particlePath,
                  &Renderer::getInstance(), loadedFileList, sourceXMLFile,
                  techtreePath);
              loadedFileList[currentPath + particlePath].push_back(make_pair(
    ...
    throw megaglest_runtime_error(
        "Error loading resource type: " + path + "\nMessage: " + ex.what(),
        !ex.wantStackTrace());
...
void ResourceType::saveGame(XmlNode *rootNode) {
  ...
  resourceTypeNode->addAttribute("resourceClass", intToStr(resourceClass),
                                 mapTagReplacements);
  //    int tilesetObject;	//used only if class==rcTileset
  resourceTypeNode->addAttribute("tilesetObject", intToStr(tilesetObject),
                                 mapTagReplacements);
  //    int resourceNumber;	//used only if class==rcTech, resource number in
  //    the map
  resourceTypeNode->addAttribute("resourceNumber", intToStr(resourceNumber),
                                 mapTagReplacements);
  ...
  resourceTypeNode->addAttribute("defResPerPatch", intToStr(defResPerPatch),
                                 mapTagReplacements);
  //	bool recoup_cost;

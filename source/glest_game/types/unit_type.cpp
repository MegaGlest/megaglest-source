...
void UnitType::loaddd(
  ...
  try {
    ...
    if (hasDup) {
      printf("WARNING, unit type [%s] has one or more duplicate unit "
             "requirements\n",
    ...
    if (hasDup) {
      printf("WARNING, unit type [%s] has one or more duplicate upgrade "
             "requirements\n",
             this->getName(false).c_str());
    }
    ...
    if (hasDup) {
      printf("WARNING, unit type [%s] has one or more duplicate resource "
             "requirements\n",
             this->getName(false).c_str());
    }
    ...
    if (hasDup) {
      printf("WARNING, unit type [%s] has one or more duplicate stored "
             "resources\n",
             this->getName(false).c_str());
    }
    ...
    if (parametersNode->hasChild("resources-death")) {
      ...
      for (unsigned int i = 0; i < deathResourcesNode->getChildCount(); ++i) {
        ...
        if (std::find(lootableResources.begin(), lootableResources.end(),
          ...
          printf("WARNING, unit type [%s] has one or more duplicate lootable "
                 "resources\n",
                 this->getName(false).c_str());
        }
...
const CommandType *
UnitType::getFirstCtOfClass(CommandClass commandClass) const {
...
UnitType::getFirstHarvestCommand(const ResourceType *resourceType,
...
UnitType::getFirstAttackStoppedCommand(Field field) const {
  // printf("$$$ Unit [%s] commandTypes.size() =
  // %d\n",this->getName().c_str(),(int)commandTypes.size());
...
UnitType::getFirstRepairCommand(const UnitType *repaired) const {
  for (int i = 0; i < (int)commandTypes.size(); ++i) {

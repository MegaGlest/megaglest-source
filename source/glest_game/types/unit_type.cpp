...
void UnitType::loaddd(int id, const string &dir, const TechTree *techTree, const string &techTreePath, const FactionType *factionType, Checksum *checksum,
                      Checksum *techtreeChecksum, std::map<string, vector<pair<string, string>>> &loadedFileList, bool validationMode) {
    ...
    try {
        ...
        if (parametersNode->getChild("max-hp")->hasAttribute("start-value") && parametersNode->getChild("max-hp")->hasAttribute("start-percentage")) {
            throw megaglest_runtime_error("Unit " + name + " has both start-value and start-percentage for HP", true);
        }
        ...
        // Check that we don't use both start-value and start-percentage, as they
        // are mutually exclusive
        if (parametersNode->getChild("max-ep")->hasAttribute("start-value") && parametersNode->getChild("max-ep")->hasAttribute("start-percentage")) {
            throw megaglest_runtime_error("Unit " + name + " has both start-value and start-percentage for EP", true);
        }
        ...
        for (int i = 0; i < (int)levels.size(); ++i) {
            ...
            levels[i].init(levelNode->getAttribute("name")->getRestrictedValue(), levelNode->getAttribute("kills")->getIntValue());
        }
        ...
        if (parametersNode->hasChild("damage-particles")) {
            ...
            if (particleEnabled) {
                for (int i = 0; i < (int)particleNode->getChildCount(); ++i) {
                    ...
                    unitParticleSystemType->load(particleFileNode, dir, currentPath + path, &Renderer::getInstance(), loadedFileList, sourceXMLFile,
                                                 techTree->getPath());
        ...
        loadedFileList[imageNode->getAttribute("path")->getRestrictedValue(currentPath)].push_back(
            make_pair(sourceXMLFile, imageNode->getAttribute("path")->getRestrictedValue()));
        ...
        loadedFileList[imageCancelNode->getAttribute("path")->getRestrictedValue(currentPath)].push_back(
            make_pair(sourceXMLFile, imageCancelNode->getAttribute("path")->getRestrictedValue()));
        ...
        if (meetingPoint) {
            ...
            loadedFileList[meetingPointNode->getAttribute("image-path")->getRestrictedValue(currentPath)].push_back(
                make_pair(sourceXMLFile, meetingPointNode->getAttribute("image-path")->getRestrictedValue()));
        }
        ...
        snprintf(szBuf, 8096, Lang::getInstance().getString("LogScreenGameLoadingUnitTypeSkills", "", true).c_str(), formatString(this->getName(true)).c_str(),
                 skillTypes.size());
        Logger::getInstance().add(szBuf, true);
        ...
        for (int i = 0; i < (int)commandTypes.size(); ++i) {
            ...
            commandTypes[i] = NULL;
            try {
                commandType->load(i, commandNode, dir, techTree, factionType, *this, loadedFileList, sourceXMLFile);
...
const CommandType *UnitType::getCommandType(int i) const {
    if (i >= (int)commandTypes.size()) {
        ...
        snprintf(szBuf, 8096,
                 ...
                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, i, commandTypes.size());

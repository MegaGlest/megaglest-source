...
auto_ptr<CommandType> UnitType::ctHarvestEmergencyReturnCommandType(
    new HarvestEmergencyReturnCommandType());
// ===============================
// 	class Level
// ===============================
...
void UnitType::preLoad(const string &dir) { name = lastDir(dir); }

void UnitType::loaddd(
    int id, const string &dir, const TechTree *techTree,
  ...
  try {
    ...
    snprintf(szBuf, 8096,
             Lang::getInstance()
                 .getString("LogScreenGameLoadingUnitType", "", true)
                 .c_str(),
    ...
    if (parametersNode->hasChild("count-in-victory-conditions") == true) {
      bool countUnit = parametersNode->getChild("count-in-victory-conditions")
                           ->getAttribute("value")
                           ->getBoolValue();
      if (countUnit == true) {
    ...
    size =
        parametersNode->getChild("size")->getAttribute("value")->getIntValue();
    addItemToVault(&(this->size), this->size);
    renderSize = size;
    if (parametersNode->hasChild("render-size")) {
      renderSize = parametersNode->getChild("render-size")
                       ->getAttribute("value")
                       ->getIntValue();
    }
    aiBuildSize = size;
    if (parametersNode->hasChild("ai-build-size")) {
      aiBuildSize = parametersNode->getChild("ai-build-size")
                        ->getAttribute("value")
                        ->getIntValue();
    }
    ...
    if (parametersNode->hasChild("target-height")) {
      targetHeight = parametersNode->getChild("target-height")
                         ->getAttribute("value")
                         ->getIntValue();
    ...
    // burnHeight
    if (parametersNode->hasChild("target-height")) {
      burnHeight = parametersNode->getChild("burn-height")
                       ->getAttribute("value")
                       ->getIntValue();
    ...
    maxHp = parametersNode->getChild("max-hp")
                ->getAttribute("value")
                ->getIntValue();
    addItemToVault(&(this->maxHp), this->maxHp);
    ...
    hpRegeneration = parametersNode->getChild("max-hp")
                         ->getAttribute("regeneration")
                         ->getIntValue();
    addItemToVault(&(this->hpRegeneration), this->hpRegeneration);

    // maxEp
    // checkItemInVault(&(this->maxEp),this->maxEp);
    maxEp = parametersNode->getChild("max-ep")
                ->getAttribute("value")
                ->getIntValue();
    addItemToVault(&(this->maxEp), this->maxEp);
    ...
    // Check that we don't use both start-value and start-percentage, as they
    ...
    if (parametersNode->getChild("max-hp")->hasAttribute("start-value") &&
      ...
      throw megaglest_runtime_error(
          "Unit " + name + " has both start-value and start-percentage for HP",
          true);
    }
    ...
    if (parametersNode->getChild("max-hp")->hasAttribute("start-value")) {
      ...
      startHpValue = parametersNode->getChild("max-hp")
                         ->getAttribute("start-value")
                         ->getIntValue();
      startHpType = stValue;
    }
    ...
    if (parametersNode->getChild("max-hp")->hasAttribute("start-percentage")) {
      startHpPercentage = parametersNode->getChild("max-hp")
                              ->getAttribute("start-percentage")
                              ->getIntValue();
    ...
    if (parametersNode->getChild("max-ep")->hasAttribute("start-value") &&
      ...
      throw megaglest_runtime_error(
          "Unit " + name + " has both start-value and start-percentage for EP",
          true);
    }
    ...
    if (parametersNode->getChild("max-ep")->hasAttribute("start-percentage")) {
      startEpPercentage = parametersNode->getChild("max-ep")
                              ->getAttribute("start-percentage")
                              ->getIntValue();
    ...
    if (parametersNode->hasChild("max-unit-count")) {
      ...
      maxUnitCount = parametersNode->getChild("max-unit-count")
                         ->getAttribute("value")
                         ->getIntValue();
    }
    addItemToVault(&(this->maxUnitCount), this->maxUnitCount);
    ...
    armor =
        parametersNode->getChild("armor")->getAttribute("value")->getIntValue();
    addItemToVault(&(this->armor), this->armor);

    // armor type string
    string armorTypeName = parametersNode->getChild("armor-type")
                               ->getAttribute("value")
                               ->getRestrictedValue();
    ...
    sight =
        parametersNode->getChild("sight")->getAttribute("value")->getIntValue();
    addItemToVault(&(this->sight), this->sight);

    // prod time
    productionTime =
        parametersNode->getChild("time")->getAttribute("value")->getIntValue();

    // multi selection
    multiSelect = parametersNode->getChild("multi-selection")
                      ->getAttribute("value")
                      ->getBoolValue();

    // uniform selection
    if (parametersNode->hasChild("uniform-selection")) {
      uniformSelect = parametersNode->getChild("uniform-selection")
                          ->getAttribute("value")
                          ->getBoolValue();
    }
    ...
    if (parametersNode->hasChild("commandable")) {
      commandable = parametersNode->getChild("commandable")
                        ->getAttribute("value")
                        ->getBoolValue();
    }
    // cellmap
    ...
    for (int i = 0; i < (int)levels.size(); ++i) {
      ...
      levels[i].init(levelNode->getAttribute("name")->getRestrictedValue(),
    ...
    for (int i = 0; i < (int)fieldsNode->getChildCount(); ++i) {
      ...
      if (fieldName == "land") {
        ...
        throw megaglest_runtime_error(
            "Not a valid field: " + fieldName + ": " + path, true);
      }
    }
      ...
      if (particleEnabled) {
        for (int i = 0; i < (int)particleNode->getChildCount(); ++i) {
          ...
          unitParticleSystemType->load(particleFileNode, dir,
                                       ...
                                       sourceXMLFile, techTree->getPath());
          ...
          if (particleFileNode->getAttribute("minHp", false) != NULL &&
            ...
            unitParticleSystemType->setMinHp(
                particleFileNode->getAttribute("minHp")->getIntValue());
            unitParticleSystemType->setMaxHp(
                particleFileNode->getAttribute("maxHp")->getIntValue());

            if (particleFileNode->getAttribute("ispercentbased", false) !=
                NULL) {
              unitParticleSystemType->setMinmaxIsPercent(
                  particleFileNode->getAttribute("ispercentbased")
                      ->getBoolValue());
            }
          }
    ...
    if (parametersNode->hasChild("healthbar")) {
      ...
      if (healthbarNode->hasChild("height")) {
        healthbarheight = healthbarNode->getChild("height")
                              ->getAttribute("value")
                              ->getFloatValue();
      }
      if (healthbarNode->hasChild("thickness")) {
        healthbarthickness = healthbarNode->getChild("thickness")
                                 ->getAttribute("value")
                                 ->getFloatValue(0.f, 1.f);
      }
      if (healthbarNode->hasChild("visible")) {
        string healthbarVisibleString = healthbarNode->getChild("visible")
                                            ->getAttribute("value")
                                            ->getValue();
    ...
    if (hasDup) {
      printf(
          "WARNING, unit type [%s] has one or more duplicate unit "
          "requirements\n",
    ...
    if (hasDup) {
      printf(
          "WARNING, unit type [%s] has one or more duplicate upgrade "
          "requirements\n",
          this->getName(false).c_str());
    }
    ...
    if (hasDup) {
      printf(
          "WARNING, unit type [%s] has one or more duplicate resource "
          "requirements\n",
          this->getName(false).c_str());
    }
    ...
    if (hasDup) {
      printf(
          "WARNING, unit type [%s] has one or more duplicate stored "
          "resources\n",
          this->getName(false).c_str());
    }
    ...
    for (std::map<string, int>::iterator iterMap = sortedItems.begin();
         iterMap != sortedItems.end(); ++iterMap) {
      try {
        storedResources[index].init(techTree->getResourceType(iterMap->first),
                                    iterMap->second);
        index++;
      } catch (megaglest_runtime_error &ex) {
        if (validationMode == false) {
    ...
    if (parametersNode->hasChild("resources-death")) {
      ...
      for (unsigned int i = 0; i < deathResourcesNode->getChildCount(); ++i) {
        ...
        // All attributes are optional, although nothing happens if they aren't
        // used. They can be combined freely. Percentages will take affect
        // before absolute values.
        if (resourceNode->hasAttribute("amount-value")) {
          resource.setAmountValue(
              resourceNode->getAttribute("amount-value")->getIntValue());
        } else {
          resource.setAmountValue(0);
        }

        if (resourceNode->hasAttribute("amount-faction-percent")) {
          resource.setAmountFactionPercent(
              resourceNode->getAttribute("amount-faction-percent")
                  ->getIntValue());
        } else {
        ...
        if (resourceNode->hasAttribute("loss-value")) {
          resource.setLossValue(
              resourceNode->getAttribute("loss-value")->getIntValue());
        } else {
          resource.setLossValue(0);
        }

        if (resourceNode->hasAttribute("loss-faction-percent")) {
          resource.setLossFactionPercent(
              resourceNode->getAttribute("loss-faction-percent")
                  ->getIntValue());
        } else {
        ...
        if (resourceNode->hasAttribute("allow-negative")) {
          resource.setNegativeAllowed(
              resourceNode->getAttribute("allow-negative")->getBoolValue());
        } else {
          resource.setNegativeAllowed(false);
        }

        // Figure out if there are duplicate resources. The value stored in the
        // map is arbitrary, and exists solely because
        if (std::find(lootableResources.begin(), lootableResources.end(),
                      resource) != lootableResources.end()) {
          printf(
              "WARNING, unit type [%s] has one or more duplicate lootable "
              "resources\n",
              this->getName(false).c_str());
        }
    ...
    if (image) {
      image->load(
          imageNode->getAttribute("path")->getRestrictedValue(currentPath));
    }
    loadedFileList[imageNode->getAttribute("path")->getRestrictedValue(
                       currentPath)]
        .push_back(
            make_pair(sourceXMLFile,
    ...
    if (cancelImage) {
      cancelImage->load(
          imageCancelNode->getAttribute("path")->getRestrictedValue(
              currentPath));
    }
    loadedFileList[imageCancelNode->getAttribute("path")->getRestrictedValue(
                       currentPath)]
        .push_back(make_pair(
            sourceXMLFile,
            imageCancelNode->getAttribute("path")->getRestrictedValue()));
    ...
    if (meetingPoint) {
      ...
      if (meetingPointImage) {
        meetingPointImage->load(meetingPointNode->getAttribute("image-path")
                                    ->getRestrictedValue(currentPath));
      }
      loadedFileList[meetingPointNode->getAttribute("image-path")
                         ->getRestrictedValue(currentPath)]
          .push_back(make_pair(sourceXMLFile,
    ...
    if (parametersNode->hasChild("count-unit-death-in-stats")) {
      ...
      countUnitDeathInStats =
          countUnitDeathInStatsNode->getAttribute("value")->getBoolValue();
    } else {
    ...
    if (parametersNode->hasChild("count-unit-production-in-stats")) {
      ...
      countUnitProductionInStats =
          countUnitProductionInStatsNode->getAttribute("value")->getBoolValue();
    } else {
    ...
    if (parametersNode->hasChild("count-kill-for-unit-upgrade")) {
      ...
      countKillForUnitUpgrade =
          countKillForUnitUpgradeNode->getAttribute("value")->getBoolValue();
    } else {
    ...
    if (selectionSoundNode->getAttribute("enabled")->getBoolValue()) {
      ...
      for (int i = 0; i < (int)selectionSounds.getSounds().size(); ++i) {
        ...
        string path =
            soundNode->getAttribute("path")->getRestrictedValue(currentPath);
        StaticSound *sound = new StaticSound();
        sound->load(path);
        loadedFileList[path].push_back(
            make_pair(sourceXMLFile,
                      soundNode->getAttribute("path")->getRestrictedValue()));
    ...
    snprintf(szBuf, 8096,
             Lang::getInstance()
                 .getString("LogScreenGameLoadingUnitTypeSkills", "", true)
                 .c_str(),
             formatString(this->getName(true)).c_str(), skillTypes.size());
    Logger::getInstance().add(szBuf, true);
    ...
    for (int i = 0; i < (int)skillTypes.size(); ++i) {
      ...
      try {
        ...
        if (validationMode == false) {
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugError,
              "In [%s::%s Line: %d] Error [%s]\nFor UnitType: %s SkillType: "
              "%s\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, ex.what(), name.c_str(), classId.c_str());
        }
      }
    }
...
const CommandType *UnitType::getFirstCtOfClass(
...
const HarvestCommandType *UnitType::getFirstHarvestCommand(
    const ResourceType *resourceType, const Faction *faction) const {
  for (int i = 0; i < (int)commandTypes.size(); ++i) {
...
UnitType::getFirstHarvestEmergencyReturnCommand() const {
...
const AttackCommandType *UnitType::getFirstAttackCommand(Field field) const {
  // printf("$$$ Unit [%s] commandTypes.size() =
  ...
  for (int i = 0; i < (int)commandTypes.size(); ++i) {
    ...
    // printf("$$$ Unit [%s] i = %d, commandTypes[i]
    // [%s]\n",this->getName().c_str(),(int)i,
    // commandTypes[i]->toString().c_str());
...
const AttackStoppedCommandType *UnitType::getFirstAttackStoppedCommand(
    Field field) const {
  // printf("$$$ Unit [%s] commandTypes.size() =
  // %d\n",this->getName().c_str(),(int)commandTypes.size());
  ...
  for (int i = 0; i < (int)commandTypes.size(); ++i) {
    ...
    if (commandTypes[i]->getClass() == ccAttackStopped) {
      const AttackStoppedCommandType *act =
          dynamic_cast<const AttackStoppedCommandType *>(commandTypes[i]);
      if (act != NULL && act->getAttackSkillType()->getAttackField(field)) {
        // printf("## Unit [%s] i = %d, is
        // found\n",this->getName().c_str(),(int)i);
        return act;
...
const RepairCommandType *UnitType::getFirstRepairCommand(
    const UnitType *repaired) const {
  for (int i = 0; i < (int)commandTypes.size(); ++i) {
...
int UnitType::getTotalMaxHpRegeneration(
...
int UnitType::getTotalMaxEpRegeneration(
    const TotalUpgrade *totalUpgrade) const {
  checkItemInVault(&(this->epRegeneration), this->epRegeneration);
...
void UnitType::sortCommandTypes(CommandTypes cts) {
  try {
    ...
    for (int i = 0; i < (int)cts.size(); i++) {
      for (auto &&cc : CommandHelper::getBasicsCC()) {
        if (cts[i]->getClass() == cc &&
            basicIndexes.find(cc) == basicIndexes.end()) {
          basicIndexes[cc] = i;
        }
      }
    }
    for (int i = (int)cts.size(); i-- > 0;) {
      for (auto kv : basicIndexes) {
        if (kv.second == i) {
          ...
          cts.erase(cts.begin() + i);
          basicNulls.erase(
              std::remove(basicNulls.begin(), basicNulls.end(), ccPos),
              basicNulls.end());
        }
      }
    }
    ...
    for (auto &&cc : CommandHelper::getCoresCC()) {
      std::copy_if(cts.begin(), cts.end(), std::back_inserter(ctCores),
                   [cc](CommandType *i) { return i->getClass() == cc; });
    }
    ...
    commandTypesSorted.insert(commandTypesSorted.end(), ctCores.begin(),
                              ctCores.end());
    commandTypesSorted.insert(commandTypesSorted.end(), ctBasics.begin(),
                              ctBasics.end());
    commandTypesSorted.insert(commandTypesSorted.end(), ctMorphs.begin(),
                              ctMorphs.end());
  } catch (exception &ex) {
  }
}
...
const CommandType *UnitType::getCommandType(int i) const {
  if (i >= (int)commandTypes.size()) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] i >= commandTypes.size(), i = %d, "
             "commandTypes.size() = " MG_SIZE_T_SPECIFIER "",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__, i, commandTypes.size());
...
const CommandType *UnitType::getCommandTypeSorted(int i) const {
  if (i >= (int)commandTypesSorted.size()) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] i >= commandTypesSorted.size(), i = %d, "
             "commandTypesSorted.size() = " MG_SIZE_T_SPECIFIER "",
             extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
             __LINE__, i, commandTypesSorted.size());
    throw megaglest_runtime_error(szBuf);
  }
...
std::string UnitType::toString() const {
  ...
  for (int i = 0; i < fieldCount; i++) {
    result +=
        " fields index = " + intToStr(i) + " value = " + intToStr(fields[i]);
  }
  for (int i = 0; i < pCount; i++) {
    result += " properties index = " + intToStr(i) +
              " value = " + intToStr(properties[i]);
  }

  result += " armor = " + intToStr(armor);

  if (armorType != NULL) {
    result += " armorType Name: [" + armorType->getName(false) +
              " id = " + intToStr(armorType->getId());
  }
  ...
  if (cellMap != NULL) {
    ...
    for (int i = 0; i < size; ++i) {
      for (int j = 0; j < size; ++j) {
        result += " i = " + intToStr(i) + " j = " + intToStr(j) +
                  " value = " + intToStr(cellMap[i * size + j]);
      }
    }
  }
  ...
  for (int i = 0; i < (int)storedResources.size(); ++i) {
    result +=
        " i = " + intToStr(i) + " " + storedResources[i].getDescription(false);
  }

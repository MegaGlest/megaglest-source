...
class FactionType {
 ...
 private:
  ...
  std::map<AIBehaviorUnitCategory, std::vector<PairPUnitTypeInt> >
      mapAIBehaviorUnitCategories;
  std::vector<const UpgradeType *> vctAIBehaviorUpgrades;
  std::map<AIBehaviorStaticValueCategory, int>
 ...
 public:
  ...
  const std::vector<FactionType::PairPUnitTypeInt> getAIBehaviorUnits(
      AIBehaviorUnitCategory category) const;
  ...
  std::vector<std::string> validateFactionTypeResourceTypes(

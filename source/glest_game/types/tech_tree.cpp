...
string TechTree::getName(bool translatedValue) {
    ...
    if (lang.getTechNameLoaded() != name || lang.getLanguage() != languageUsedForCache) {
...
string TechTree::getTranslatedName(string techName, bool forceLoad, bool forceTechtreeActiveFile) {
    ...
    Lang &lang = Lang::getInstance();
    if (forceTechtreeActiveFile == false && translatedTechNames.find(techName) != translatedTechNames.end() && lang.getLanguage() == languageUsedForCache) {
...
string TechTree::getTranslatedFactionName(string techName, string factionName) {
    ...
    Lang &lang = Lang::getInstance();
    if (lang.getTechNameLoaded() != techName || lang.getLanguage() != languageUsedForCache) {
...
Checksum TechTree::loadTech(const string &techName, set<string> &factions, Checksum *checksum, std::map<string, vector<pair<string, string>>> &loadedFileList,
                            bool validationMode) {
    name = "";
    ...
    if (path != "") {
        // printf(">>> path=%s\n",path.c_str());
        load(path, factions, checksum, &techtreeChecksum, loadedFileList, validationMode);
    } else {
...
void TechTree::load(const string &dir, set<string> &factions, Checksum *checksum, Checksum *techtreeChecksum,
    ...
    try {
        ...
        for (int i = 0; i < (int)filenames.size(); ++i) {
            ...
            resourceTypes[i].load(str, checksum, &checksumValue, loadedFileList, treePath);
            Window::handleEvent();
            SDL_PumpEvents();
        }
    ...
    try {
        ...
        for (set<string>::iterator it = factions.begin(); it != factions.end(); ++it) {
            ...
            char szBuf[8096] = "";
            snprintf(szBuf, 8096, "%s %s [%d / %d] - %s", Lang::getInstance().getString("Loading").c_str(), Lang::getInstance().getString("Faction").c_str(),
                     i + 1, (int)factions.size(), formatString(this->getTranslatedFactionName(name, factionName)).c_str());
            Logger &logger = Logger::getInstance();
            logger.setState(szBuf);
            logger.setProgress((int)((((double)i) / (double)factions.size()) * 100.0));

            factionTypes[i++].load(factionName, this, checksum, &checksumValue, loadedFileList, validationMode);
...
const ResourceType *TechTree::getTechResourceType(int i) const {
    for (int j = 0; j < getResourceTypeCount(); ++j) {
        ...
        if (rt->getResourceNumber() == i && rt->getClass() == rcTech) return getResourceType(j);
    }
...
const ResourceType *TechTree::getFirstTechResourceType() const {
    for (int i = 0; i < getResourceTypeCount(); ++i) {
        ...
        assert(rt != NULL);
        if (rt->getResourceNumber() == 1 && rt->getClass() == rcTech) return getResourceType(i);
    }

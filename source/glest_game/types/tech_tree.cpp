...
string TechTree::getNameUntranslated() const { return name; }
...
string TechTree::getName(bool translatedValue) {
  ...
  if (lang.getTechNameLoaded() != name ||
    ...
    foundTranslation = lang.loadTechTreeStrings(
        name, lang.getLanguage() != languageUsedForCache);
...
string TechTree::getTranslatedName(string techName, bool forceLoad,
  ...
  // printf("Line: %d Tech [%s] forceLoad = %d forceTechtreeActiveFile =
  ...
  if (forceTechtreeActiveFile == false &&
    ...
    if (path != "") {
      ...
      // printf("techName [%s] name [%s] result
...
string TechTree::getTranslatedFactionName(string techName, string factionName) {
  // printf("Line: %d Tech [%s] name [%s] factionName
  // [%s]\n",__LINE__,techName.c_str(),name.c_str(),factionName.c_str());

  Lang &lang = Lang::getInstance();
  if (lang.getTechNameLoaded() != techName ||
      lang.getLanguage() != languageUsedForCache) {
    // printf("Line: %d Tech [%s] name [%s] lang.getTechNameLoaded() [%s]
    // factionName
  ...
  if (iterMap != translatedTechFactionNames.end()) {
    if (iterMap->second.find(factionName) != iterMap->second.end()) {
      // printf("Line: %d Tech [%s] factionName
  ...
  string result = lang.getTechTreeString("FactionName_" + factionName,
                                         formatString(factionName).c_str());
  // printf(">>result = %s\n",result.c_str());
  translatedTechFactionNames[techName][factionName] = result;

  // printf("Line: %d Translated faction for Tech [%s] faction [%s] result
  // [%s]\n",__LINE__,techName.c_str(),factionName.c_str(),result.c_str());

  return result;
}

Checksum TechTree::loadTech(
    const string &techName, set<string> &factions, Checksum *checksum,
...
void TechTree::load(
    const string &dir, set<string> &factions, Checksum *checksum,
    Checksum *techtreeChecksum,
    std::map<string, vector<pair<string, string> > > &loadedFileList,
    bool validationMode) {
  ...
  snprintf(szBuf, 8096,
           Lang::getInstance()
               .getString("LogScreenGameLoadingTechtree", "", true)
               .c_str(),
  ...
  try {
    ...
    throw megaglest_runtime_error(
        "Error loading Resource Types in: " + currentPath +
            "\nMessage: " + ex.what(),
        !ex.wantStackTrace() || isValidationModeEnabled);
  ...
  try {
    ...
    for (int i = 0; i < (int)attackTypes.size(); ++i) {
      ...
      attackTypes[i].setName(
          attackTypeNode->getAttribute("name")->getRestrictedValue());
    ...
    for (int i = 0; i < (int)armorTypes.size(); ++i) {
      ...
      armorTypes[i].setName(
          armorTypeNode->getAttribute("name")->getRestrictedValue());
    ...
    for (int i = 0; i < (int)damageMultipliersNode->getChildCount(); ++i) {
      ...
      const AttackType *attackType = getAttackType(
          damageMultiplierNode->getAttribute("attack")->getRestrictedValue());
      const ArmorType *armorType = getArmorType(
          damageMultiplierNode->getAttribute("armor")->getRestrictedValue());
      double multiplier =
    ...
    SystemFlags::OutputDebug(SystemFlags::debugError,
                             ...
                             __FUNCTION__, __LINE__, ex.what());
    throw megaglest_runtime_error(
        "Error loading Tech Tree: " + currentPath + "\nMessage: " + ex.what(),
        !ex.wantStackTrace() || isValidationModeEnabled);
  } catch (const exception &e) {
    SystemFlags::OutputDebug(SystemFlags::debugError,
                             "In [%s::%s Line: %d] Error [%s]\n",
  ...
  try {
    ...
    for (set<string>::iterator it = factions.begin(); it != factions.end();
      ...
      snprintf(szBuf, 8096, "%s %s [%d / %d] - %s",
               ...
               Lang::getInstance().getString("Faction").c_str(), i + 1,
               (int)factions.size(),
               formatString(this->getTranslatedFactionName(name, factionName))
                   .c_str());
      Logger &logger = Logger::getInstance();
      logger.setState(szBuf);
      logger.setProgress(
          (int)((((double)i) / (double)factions.size()) * 100.0));
    ...
    SystemFlags::OutputDebug(SystemFlags::debugError,
                             ...
                             __FUNCTION__, __LINE__, ex.what());
    throw megaglest_runtime_error(
        "Error loading Faction Types: " + currentPath +
            "\nMessage: " + ex.what(),
        !ex.wantStackTrace() || isValidationModeEnabled);
  } catch (const exception &e) {
    SystemFlags::OutputDebug(SystemFlags::debugError,
...
TechTree::~TechTree() {
  ...
  Logger::getInstance().add(
      Lang::getInstance().getString("LogScreenGameUnLoadingTechtree", "", true),
      true);
  resourceTypes.clear();
  factionTypes.clear();
  armorTypes.clear();
...
std::vector<std::string> TechTree::validateResourceTypes() {
  ...
  for (unsigned int i = 0; i < factionTypes.size(); ++i) {
    ...
    for (int j = (int)resourceTypesNotUsed.size() - 1; j >= 0; --j) {
      ...
      // printf("Validating [%d / %d] resourcetype
      // [%s]\n",j,(int)resourceTypesNotUsed.size(),rt.getName().c_str());

      if (factionTypes[i].factionUsesResourceType(&rt) == true) {
        // printf("FOUND FACTION CONSUMER FOR RESOURCE - [%d / %d] faction [%s]
        // resource [%d / %d] resourcetype
  ...
  if (resourceTypesNotUsed.empty() == false) {
    // printf("FOUND unused resource Types
...
const ResourceType *TechTree::getFirstTechResourceType() const {
  ...
  char szBuf[8096] = "";
  snprintf(szBuf, 8096,
           "The referenced tech tree [%s] is either missing or has no "
           "resources defined but at least one resource is required.",
           this->name.c_str());
...
void TechTree::saveGame(XmlNode *rootNode) {
  ...
  //		pathListNode->addAttribute("value",pathList[i],
  // mapTagReplacements);
  //	}
  ...
  techTreeNode->addAttribute("checksumValue", intToStr(checksumValue.getSum()),
                             mapTagReplacements);
}

...
// attackStrengthMultiplierValueNodeList.size(); ++i) { 		XmlNode
// *node = attackStrengthMultiplierValueNodeList[i];
//
...
void UpgradeType::load(
  ...
  try {
    ...
    if (hasDup) {
      printf(
          "WARNING, upgrade type [%s] has one or more duplicate unit "
          "requirements\n",
          this->getName().c_str());
    }
    ...
    if (hasDup) {
      printf(
          "WARNING, upgrade type [%s] has one or more duplicate upgrade "
          "requirements\n",
          this->getName().c_str());
    }
    ...
    if (hasDup) {
      printf(
          "WARNING, upgrade type [%s] has one or more duplicate resource "
          "requirements\n",
          this->getName().c_str());
    }
    ...
    throw megaglest_runtime_error(
        "Error loading UpgradeType: " + currentPath + "\nMessage: " + ex.what(),
        !ex.wantStackTrace());
...
/// upgradeTypeNode->getChildList("UnitType"); /	for(unsigned int i = 0;
/// i < unitTypeNodeList.size(); ++i) { /		XmlNode *node =
/// unitTypeNodeList[i]; /	}
//}
...
void TotalUpgrade::deapply(int sourceUnitId, const UpgradeTypeBase *ut,
  ...
  if (removedBoost == false) {
    printf(
        "\n\n!!!!!! de-apply boost NOT FOUND for sourceUnitId = %d, destUnitId "
        "= %d\n%s\n\nCurrent Boosts:\n",

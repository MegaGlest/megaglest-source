    ...
    if (upgradeNode->hasChildWithAliases(attackStrengthXMLTags) == true) {
        ...
        if (upgradeNode->getChildWithAliases(attackStrengthXMLTags)->getAttribute(VALUE_PERCENT_MULTIPLIER_KEY_NAME, false) != NULL) {
            attackStrengthIsMultiplier =
                upgradeNode->getChildWithAliases(attackStrengthXMLTags)->getAttribute(VALUE_PERCENT_MULTIPLIER_KEY_NAME)->getBoolValue();
...
string UpgradeType::getReqDesc(bool translatedValue) const {
    ...
    if (!effects.empty() || !tags.empty()) {
        ...
        for (tagIter = outputTags.begin(); tagIter != outputTags.end(); ++tagIter) {
            string tag = *tagIter;
            str += indent + lang.getString("TagDesc", (translatedValue == true ? "" : "english")) + " " + getTagName(tag, translatedValue) + "\n";
        }
    }
    return str;
}
...
void UpgradeType::load(const string &dir, const TechTree *techTree, const FactionType *factionType, Checksum *checksum, Checksum *techtreeChecksum,
    ...
    try {
        ...
        for (std::map<string, int>::iterator iterMap = sortedItems.begin(); iterMap != sortedItems.end(); ++iterMap) {
...
bool UpgradeType::isAffected(const UnitType *unitType) const {
    ...
    set_intersection(tags.begin(), tags.end(), unitTags.begin(), unitTags.end(), std::inserter(intersect, intersect.begin()));
...
void TotalUpgrade::deapply(int sourceUnitId, const UpgradeTypeBase *ut, int destUnitId) {
    ...
    for (unsigned int index = 0; index < boostUpgrades.size(); ++index) {
        ...
        if (boost->boostUpgradeSourceUnit == sourceUnitId && boost->boostUpgradeBase->getUpgradeName() == ut->getUpgradeName() &&
...
void TotalUpgrade::saveGame(XmlNode *rootNode) const {
    ...
    //    std::map<string,int> attackStrengthMultiplierValueList;
    for (std::map<string, int>::const_iterator iterMap = attackStrengthMultiplierValueList.begin(); iterMap != attackStrengthMultiplierValueList.end();
         ++iterMap) {
    ...
    //    std::map<string,int> attackRangeMultiplierValueList;
    for (std::map<string, int>::const_iterator iterMap = attackRangeMultiplierValueList.begin(); iterMap != attackRangeMultiplierValueList.end(); ++iterMap) {
    ...
    //    std::map<string,int> moveSpeedIsMultiplierValueList;
    for (std::map<string, int>::const_iterator iterMap = moveSpeedIsMultiplierValueList.begin(); iterMap != moveSpeedIsMultiplierValueList.end(); ++iterMap) {
    ...
    //    std::map<string,int> prodSpeedProduceIsMultiplierValueList;
    for (std::map<string, int>::const_iterator iterMap = prodSpeedProduceIsMultiplierValueList.begin(); iterMap != prodSpeedProduceIsMultiplierValueList.end();
         ++iterMap) {
    ...
    //    std::map<string,int> prodSpeedUpgradeIsMultiplierValueList;
    for (std::map<string, int>::const_iterator iterMap = prodSpeedUpgradeIsMultiplierValueList.begin(); iterMap != prodSpeedUpgradeIsMultiplierValueList.end();
         ++iterMap) {
    ...
    //    std::map<string,int> prodSpeedMorphIsMultiplierValueList;
    for (std::map<string, int>::const_iterator iterMap = prodSpeedMorphIsMultiplierValueList.begin(); iterMap != prodSpeedMorphIsMultiplierValueList.end();
         ++iterMap) {
    ...
    for (std::map<string, int>::const_iterator iterMap = attackSpeedIsMultiplierValueList.begin(); iterMap != attackSpeedIsMultiplierValueList.end();
         ++iterMap) {
...
void TotalUpgrade::loadGame(const XmlNode *rootNode) {
    ...
    for (unsigned int i = 0; i < attackRangeMultiplierValueNodeList.size(); ++i) {
        ...
        attackRangeMultiplierValueList[node->getAttribute("key")->getValue()] = node->getAttribute("value")->getIntValue();
    }
    ...
    for (unsigned int i = 0; i < prodSpeedProduceIsMultiplierValueNodeList.size(); ++i) {
        ...
        prodSpeedProduceIsMultiplierValueList[node->getAttribute("key")->getValue()] = node->getAttribute("value")->getIntValue();
    }

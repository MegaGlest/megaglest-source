...
void UpgradeTypeBase::load(const XmlNode *upgradeNode, string upgradename) {
  ...
  if (upgradeNode->hasChild("max-hp") == true) {
    maxHp =
        upgradeNode->getChild("max-hp")->getAttribute("value")->getIntValue();
    if (upgradeNode->getChild("max-hp")->getAttribute(
            VALUE_PERCENT_MULTIPLIER_KEY_NAME, false) != NULL) {
      maxHpIsMultiplier = upgradeNode->getChild("max-hp")
  ...
  if (upgradeNode->hasChild("max-hp") == true) {
    if (upgradeNode->getChild("max-hp")->getAttribute(VALUE_REGEN_KEY_NAME,
                                                      false) != NULL) {
      maxHpRegeneration = upgradeNode->getChild("max-hp")
                              ->getAttribute(VALUE_REGEN_KEY_NAME)
                              ->getIntValue();
  ...
  if (upgradeNode->hasChild("max-ep") == true) {
    maxEp =
        upgradeNode->getChild("max-ep")->getAttribute("value")->getIntValue();
    if (upgradeNode->getChild("max-ep")->getAttribute(
            VALUE_PERCENT_MULTIPLIER_KEY_NAME, false) != NULL) {
      maxEpIsMultiplier = upgradeNode->getChild("max-ep")
                              ->getAttribute(VALUE_PERCENT_MULTIPLIER_KEY_NAME)
                              ->getBoolValue();
  ...
  if (upgradeNode->hasChild("max-ep") == true) {
    if (upgradeNode->getChild("max-ep")->getAttribute(VALUE_REGEN_KEY_NAME,
                                                      false) != NULL) {
      maxEpRegeneration = upgradeNode->getChild("max-ep")
                              ->getAttribute(VALUE_REGEN_KEY_NAME)
                              ->getIntValue();
  ...
  if (upgradeNode->hasChild("sight") == true) {
    sight =
        upgradeNode->getChild("sight")->getAttribute("value")->getIntValue();
    if (upgradeNode->getChild("sight")->getAttribute(
            VALUE_PERCENT_MULTIPLIER_KEY_NAME, false) != NULL) {
      sightIsMultiplier = upgradeNode->getChild("sight")
                              ->getAttribute(VALUE_PERCENT_MULTIPLIER_KEY_NAME)
                              ->getBoolValue();
  ...
  if (upgradeNode->hasChildWithAliases(attackStrengthXMLTags) == true) {
    attackStrength = upgradeNode->getChildWithAliases(attackStrengthXMLTags)
                         ->getAttribute("value")
                         ->getIntValue();
    if (upgradeNode->getChildWithAliases(attackStrengthXMLTags)
      ...
      // printf("Found attackStrengthIsMultiplier =
      // %d\n",attackStrengthIsMultiplier);
    }
  } else {
    attackStrength = 0;
  ...
  if (upgradeNode->hasChild("attack-range") == true) {
    attackRange = upgradeNode->getChild("attack-range")
                      ->getAttribute("value")
                      ->getIntValue();
  ...
  if (upgradeNode->hasChild("armor") == true) {
    armor =
        upgradeNode->getChild("armor")->getAttribute("value")->getIntValue();
    if (upgradeNode->getChild("armor")->getAttribute(
            VALUE_PERCENT_MULTIPLIER_KEY_NAME, false) != NULL) {
      armorIsMultiplier = upgradeNode->getChild("armor")
                              ->getAttribute(VALUE_PERCENT_MULTIPLIER_KEY_NAME)
                              ->getBoolValue();
  ...
  if (upgradeNode->hasChild("move-speed") == true) {
    moveSpeed = upgradeNode->getChild("move-speed")
                    ->getAttribute("value")
                    ->getIntValue();
  ...
  if (upgradeNode->hasChild("production-speed") == true) {
    prodSpeed = upgradeNode->getChild("production-speed")
                    ->getAttribute("value")
                    ->getIntValue();
  ...
  if (upgradeNode->hasChild("attack-speed") == true) {
    attackSpeed = upgradeNode->getChild("attack-speed")
                      ->getAttribute("value")
                      ->getIntValue();
...
int UpgradeTypeBase::getMoveSpeed(const MoveSkillType *st) const {
  if (st == NULL) {
    ...
    // printf("getMoveSpeed moveSpeedIsMultiplier mst->getSpeed() = %d for skill
  ...
  if (st == NULL) {
    ...
    if (dynamic_cast<const ProduceSkillType *>(st) != NULL) {
      if (prodSpeedProduceIsMultiplierValueList.find(st->getName()) !=
        ...
        result =
            prodSpeedProduceIsMultiplierValueList.find(st->getName())->second;
      }
      ...
      if (prodSpeedUpgradeIsMultiplierValueList.find(st->getName()) !=
        ...
        result =
            prodSpeedUpgradeIsMultiplierValueList.find(st->getName())->second;
      }
      ...
      if (prodSpeedMorphIsMultiplierValueList.find(st->getName()) !=
        ...
        result =
            prodSpeedMorphIsMultiplierValueList.find(st->getName())->second;
      }
    } else {
...
string UpgradeTypeBase::getDesc(bool translatedValue) const {
  ...
  if (getMaxHp() != 0 || getMaxHpRegeneration() != 0) {
    str += indent +
           lang.getString("Hp", (translatedValue == true ? "" : "english")) +
           " +" + intToStr(maxHp);
    ...
    if (getMaxHpRegeneration() != 0) {
      ...
      //				str += " +" +
      // intToStr(getMaxHpRegenerationFromBoosts());
      //			}
      str += ")";
    }
  }
  ...
  if (getSight() != 0) {
    ...
    str += indent +
           lang.getString("Sight", (translatedValue == true ? "" : "english")) +
           " +" + intToStr(sight);
  ...
  if (getMaxEp() != 0 || getMaxEpRegeneration() != 0) {
    ...
    str += indent +
           lang.getString("Ep", (translatedValue == true ? "" : "english")) +
           " +" + intToStr(maxEp);
    ...
    if (getMaxEpRegeneration() != 0) {
      ...
      //				str += " +" +
      // intToStr(getMaxEpRegenerationFromBoosts());
      //			}
      str += ")";
    }
  }
  ...
  if (getAttackStrength() != 0) {
    ...
    //			str += " +" +
    // intToStr(getAttackStrengthFromBoosts(NULL));
    //		}
  }
  ...
  if (getArmor() != 0) {
    ...
    str += indent +
           lang.getString("Armor", (translatedValue == true ? "" : "english")) +
           " +" + intToStr(armor);
...
//	XmlNode *upgradeTypeBaseNode =
// rootNode->addChild("UpgradeTypeBaseBoost");
//
...
//	upgradeTypeBaseNode->addAttribute("maxHpIsMultiplier",intToStr(maxHpIsMultiplier),
// mapTagReplacements);
////	int maxHpRegeneration;
//	upgradeTypeBaseNode->addAttribute("maxHpRegeneration",intToStr(maxHpRegeneration),
// mapTagReplacements);
...
//	upgradeTypeBaseNode->addAttribute("sightIsMultiplier",intToStr(sightIsMultiplier),
// mapTagReplacements);
////    int maxEp;
//	upgradeTypeBaseNode->addAttribute("maxEp",intToStr(maxEp),
// mapTagReplacements);
////    bool maxEpIsMultiplier;
//	upgradeTypeBaseNode->addAttribute("maxEpIsMultiplier",intToStr(maxEpIsMultiplier),
// mapTagReplacements);
////	int maxEpRegeneration;
//	upgradeTypeBaseNode->addAttribute("maxEpRegeneration",intToStr(maxEpRegeneration),
// mapTagReplacements);
...
//	upgradeTypeBaseNode->addAttribute("armorIsMultiplier",intToStr(armorIsMultiplier),
// mapTagReplacements);
////    int attackStrength;
//	upgradeTypeBaseNode->addAttribute("attackStrength",intToStr(attackStrength),
// mapTagReplacements);
////    bool attackStrengthIsMultiplier;
//	upgradeTypeBaseNode->addAttribute("attackStrengthIsMultiplier",intToStr(attackStrengthIsMultiplier),
// mapTagReplacements);
////    std::map<string,int> attackStrengthMultiplierValueList;
//	for(std::map<string,int>::const_iterator iterMap =
// attackStrengthMultiplierValueList.begin(); 			iterMap !=
// attackStrengthMultiplierValueList.end(); ++iterMap) { 		XmlNode
//*attackStrengthMultiplierValueListNode =
// upgradeTypeBaseNode->addChild("attackStrengthMultiplierValueList");
//
//		attackStrengthMultiplierValueListNode->addAttribute("key",iterMap->first,
// mapTagReplacements);
//		attackStrengthMultiplierValueListNode->addAttribute("value",intToStr(iterMap->second),
// mapTagReplacements);
//	}
////    int attackRange;
//	upgradeTypeBaseNode->addAttribute("attackRange",intToStr(attackRange),
// mapTagReplacements);
////    bool attackRangeIsMultiplier;
//	upgradeTypeBaseNode->addAttribute("attackRangeIsMultiplier",intToStr(attackRangeIsMultiplier),
// mapTagReplacements);
////    std::map<string,int> attackRangeMultiplierValueList;
//	for(std::map<string,int>::const_iterator iterMap =
// attackRangeMultiplierValueList.begin(); 			iterMap !=
// attackRangeMultiplierValueList.end(); ++iterMap) { 		XmlNode
//*attackRangeMultiplierValueListNode =
// upgradeTypeBaseNode->addChild("attackRangeMultiplierValueList");
//
//		attackRangeMultiplierValueListNode->addAttribute("key",iterMap->first,
// mapTagReplacements);
//		attackRangeMultiplierValueListNode->addAttribute("value",intToStr(iterMap->second),
// mapTagReplacements);
//	}
//
////    int moveSpeed;
...
//	upgradeTypeBaseNode->addAttribute("moveSpeedIsMultiplier",intToStr(moveSpeedIsMultiplier),
// mapTagReplacements);
////    std::map<string,int> moveSpeedIsMultiplierValueList;
//	for(std::map<string,int>::const_iterator iterMap =
// moveSpeedIsMultiplierValueList.begin(); 			iterMap !=
// moveSpeedIsMultiplierValueList.end(); ++iterMap) { 		XmlNode
//*moveSpeedIsMultiplierValueListNode =
// upgradeTypeBaseNode->addChild("moveSpeedIsMultiplierValueList");
//
//		moveSpeedIsMultiplierValueListNode->addAttribute("key",iterMap->first,
// mapTagReplacements);
//		moveSpeedIsMultiplierValueListNode->addAttribute("value",intToStr(iterMap->second),
// mapTagReplacements);
//	}
//
////    int prodSpeed;
...
//	upgradeTypeBaseNode->addAttribute("prodSpeedIsMultiplier",intToStr(prodSpeedIsMultiplier),
// mapTagReplacements);
////    std::map<string,int> prodSpeedProduceIsMultiplierValueList;
//	for(std::map<string,int>::const_iterator iterMap =
// prodSpeedProduceIsMultiplierValueList.begin(); 			iterMap
// != prodSpeedProduceIsMultiplierValueList.end(); ++iterMap) {
// XmlNode *prodSpeedProduceIsMultiplierValueListNode =
// upgradeTypeBaseNode->addChild("prodSpeedProduceIsMultiplierValueList");
//
//		prodSpeedProduceIsMultiplierValueListNode->addAttribute("key",iterMap->first,
// mapTagReplacements);
//		prodSpeedProduceIsMultiplierValueListNode->addAttribute("value",intToStr(iterMap->second),
// mapTagReplacements);
//	}
//
////    std::map<string,int> prodSpeedUpgradeIsMultiplierValueList;
//	for(std::map<string,int>::const_iterator iterMap =
// prodSpeedUpgradeIsMultiplierValueList.begin(); 			iterMap
// != prodSpeedUpgradeIsMultiplierValueList.end(); ++iterMap) {
// XmlNode *prodSpeedUpgradeIsMultiplierValueListNode =
// upgradeTypeBaseNode->addChild("prodSpeedUpgradeIsMultiplierValueList");
//
//		prodSpeedUpgradeIsMultiplierValueListNode->addAttribute("key",iterMap->first,
// mapTagReplacements);
//		prodSpeedUpgradeIsMultiplierValueListNode->addAttribute("value",intToStr(iterMap->second),
// mapTagReplacements);
//	}
//
////    std::map<string,int> prodSpeedMorphIsMultiplierValueList;
//	for(std::map<string,int>::const_iterator iterMap =
// prodSpeedMorphIsMultiplierValueList.begin(); 			iterMap
// != prodSpeedMorphIsMultiplierValueList.end(); ++iterMap) { 		XmlNode
//*prodSpeedMorphIsMultiplierValueListNode =
// upgradeTypeBaseNode->addChild("prodSpeedMorphIsMultiplierValueList");
//
//		prodSpeedMorphIsMultiplierValueListNode->addAttribute("key",iterMap->first,
// mapTagReplacements);
//		prodSpeedMorphIsMultiplierValueListNode->addAttribute("value",intToStr(iterMap->second),
// mapTagReplacements);
//	}
//}
...
void UpgradeTypeBase::saveGame(XmlNode *rootNode) const {
  ...
  //	for(std::map<string,int>::const_iterator iterMap =
  // prodSpeedMorphIsMultiplierValueList.begin(); 			iterMap
  // != prodSpeedMorphIsMultiplierValueList.end(); ++iterMap) {
...
// rootNode->getChild("UpgradeTypeBaseBoost");
//
//	//description =
// upgradeTypeBaseNode->getAttribute("description")->getValue();
//
//	upgradename =
...
//	//    bool maxHpIsMultiplier;
//	maxHpIsMultiplier =
//(upgradeTypeBaseNode->getAttribute("maxHpIsMultiplier")->getIntValue() != 0);
//	//	int maxHpRegeneration;
//	maxHpRegeneration =
// upgradeTypeBaseNode->getAttribute("maxHpRegeneration")->getIntValue();
//	//	//bool maxHpRegenerationIsMultiplier;
//	//
//	//    int sight;
...
//	//    bool sightIsMultiplier;
//	sightIsMultiplier =
//(upgradeTypeBaseNode->getAttribute("sightIsMultiplier")->getIntValue() != 0);
//	//    int maxEp;
//	maxEp = upgradeTypeBaseNode->getAttribute("maxEp")->getIntValue();
//	//    bool maxEpIsMultiplier;
//	maxEpIsMultiplier =
//(upgradeTypeBaseNode->getAttribute("maxEpIsMultiplier")->getIntValue() != 0);
//	//	int maxEpRegeneration;
//	maxEpRegeneration =
// upgradeTypeBaseNode->getAttribute("maxEpRegeneration")->getIntValue();
//	//	//bool maxEpRegenerationIsMultiplier;
//	//    int armor;
//	armor = upgradeTypeBaseNode->getAttribute("armor")->getIntValue();
//	//    bool armorIsMultiplier;
//	armorIsMultiplier =
//(upgradeTypeBaseNode->getAttribute("armorIsMultiplier")->getIntValue() != 0);
//	//    int attackStrength;
//	attackStrength =
// upgradeTypeBaseNode->getAttribute("attackStrength")->getIntValue();
//	//    bool attackStrengthIsMultiplier;
//	attackStrengthIsMultiplier =
//(upgradeTypeBaseNode->getAttribute("attackStrengthIsMultiplier")->getIntValue()
//!= 0);
//	//    std::map<string,int> attackStrengthMultiplierValueList;
//	vector<XmlNode *> attackStrengthMultiplierValueNodeList =
// upgradeTypeBaseNode->getChildList("attackStrengthMultiplierValueList");
//	for(unsigned int i = 0; i <
// attackStrengthMultiplierValueNodeList.size(); ++i) { 		XmlNode *node
// = attackStrengthMultiplierValueNodeList[i];
//
...
//	//    bool attackRangeIsMultiplier;
//	attackRangeIsMultiplier =
//(upgradeTypeBaseNode->getAttribute("attackRangeIsMultiplier")->getIntValue()
//!= 0);
//	//    std::map<string,int> attackRangeMultiplierValueList;
...
//++i) { 		XmlNode *node = attackRangeMultiplierValueNodeList[i];
//
...
//	//    bool moveSpeedIsMultiplier;
//	moveSpeedIsMultiplier =
//(upgradeTypeBaseNode->getAttribute("moveSpeedIsMultiplier")->getIntValue() !=
// 0);
//	//    std::map<string,int> moveSpeedIsMultiplierValueList;
...
//++i) { 		XmlNode *node = moveSpeedIsMultiplierValueNodeList[i];
//
...
//	//    bool prodSpeedIsMultiplier;
//	prodSpeedIsMultiplier =
//(upgradeTypeBaseNode->getAttribute("prodSpeedIsMultiplier")->getIntValue() !=
// 0);
//	//    std::map<string,int> prodSpeedProduceIsMultiplierValueList;
//	vector<XmlNode *> prodSpeedProduceIsMultiplierValueNodeList =
// upgradeTypeBaseNode->getChildList("prodSpeedProduceIsMultiplierValueList");
//	for(unsigned int i = 0; i <
// prodSpeedProduceIsMultiplierValueNodeList.size(); ++i) { 		XmlNode
// *node = prodSpeedProduceIsMultiplierValueNodeList[i];
//
...
//	for(unsigned int i = 0; i <
// prodSpeedUpgradeIsMultiplierValueNodeList.size(); ++i) { 		XmlNode
// *node = prodSpeedUpgradeIsMultiplierValueNodeList[i];
//
...
// upgradeTypeBaseNode->getChildList("prodSpeedMorphIsMultiplierValueList");
//	for(unsigned int i = 0; i <
// prodSpeedMorphIsMultiplierValueNodeList.size(); ++i) { 		XmlNode
// *node = prodSpeedMorphIsMultiplierValueNodeList[i];
//
...
const UpgradeType *UpgradeTypeBase::loadGame(const XmlNode *rootNode,
  ...
  //	//    bool maxHpIsMultiplier;
  //	maxHpIsMultiplier =
  // upgradeTypeBaseNode->getAttribute("maxHpIsMultiplier")->getIntValue();
  //	//	int maxHpRegeneration;
  //	maxHpRegeneration =
  // upgradeTypeBaseNode->getAttribute("maxHpRegeneration")->getIntValue();
  //	//	//bool maxHpRegenerationIsMultiplier;
  ...
  //	//    bool sightIsMultiplier;
  //	sightIsMultiplier =
  // upgradeTypeBaseNode->getAttribute("sightIsMultiplier")->getIntValue();
  //	//    int maxEp;
  //	maxEp = upgradeTypeBaseNode->getAttribute("maxEp")->getIntValue();
  //	//    bool maxEpIsMultiplier;
  //	maxEpIsMultiplier =
  // upgradeTypeBaseNode->getAttribute("maxEpIsMultiplier")->getIntValue();
  //	//	int maxEpRegeneration;
  //	maxEpRegeneration =
  // upgradeTypeBaseNode->getAttribute("maxEpRegeneration")->getIntValue();
  //	//	//bool maxEpRegenerationIsMultiplier;
  ...
  //	//    bool armorIsMultiplier;
  //	armorIsMultiplier =
  // upgradeTypeBaseNode->getAttribute("armorIsMultiplier")->getIntValue();
  //	//    int attackStrength;
  //	attackStrength =
  // upgradeTypeBaseNode->getAttribute("attackStrength")->getIntValue();
  //	//    bool attackStrengthIsMultiplier;
  ...
  // upgradeTypeBaseNode->getChildList("prodSpeedMorphIsMultiplierValueList");
  //	for(unsigned int i = 0; i <
  // prodSpeedMorphIsMultiplierValueNodeList.size(); ++i) { 		XmlNode *node
  // = prodSpeedMorphIsMultiplierValueNodeList[i];
  //
  //		prodSpeedMorphIsMultiplierValueList[node->getAttribute("key")->getValue()]
  //= node->getAttribute("value")->getIntValue();
  //	}
...
string UpgradeType::getReqDesc(bool translatedValue) const {
  ...
  if (!effects.empty() || !tags.empty()) {
    str += lang.getString("AffectedUnits",
                          (translatedValue == true ? "" : "english")) +
           "\n";

    // We want the output to be sorted, so convert the set to a vector and sort
    // that
...
void UpgradeType::preLoad(const string &dir) { name = lastDir(dir); }

void UpgradeType::load(
    const string &dir, const TechTree *techTree, const FactionType *factionType,
  ...
  snprintf(szBuf, 8096,
           Lang::getInstance()
               .getString("LogScreenGameLoadingUpgradeType", "", true)
               .c_str(),
  ...
  try {
    ...
    if (hasDup) {
      printf("WARNING, upgrade type [%s] has one or more duplicate unit "
             "requirements\n",
             this->getName().c_str());
    }
    ...
    if (hasDup) {
      printf("WARNING, upgrade type [%s] has one or more duplicate upgrade "
             "requirements\n",
             this->getName().c_str());
    }
    ...
    // if(hasDup || sortedItems.size() != costs.size()) printf("Found duplicate
    // resource requirement, costs.size() = %d sortedItems.size() =
    // %d\n",costs.size(),sortedItems.size());

    if (hasDup) {
      printf("WARNING, upgrade type [%s] has one or more duplicate resource "
             "requirements\n",
             this->getName().c_str());
    }
      ...
      try {
        ...
        if (validationMode == false) {
          ...
          SystemFlags::OutputDebug(
              ...
              __LINE__, ex.what(), name.c_str(), iterMap->second);
        }
      }
    }
    sortedItems.clear();
...
bool UpgradeType::isAffected(const UnitType *unitType) const {
  if (std::find(effects.begin(), effects.end(), unitType) != effects.end())
    return true;
...
//
//	//upgradeTypeNode->addAttribute("maxHp",intToStr(maxHp),
// mapTagReplacements);
...
//		unitTypeNode->addAttribute("name",ut->getName(),
// mapTagReplacements);
//	}
// }
//
...
//
////	vector<XmlNode *> unitTypeNodeList =
/// upgradeTypeNode->getChildList("UnitType"); /	for(unsigned int i = 0; i
/// < unitTypeNodeList.size(); ++i) { /		XmlNode *node =
/// unitTypeNodeList[i]; /	}
//}
...
TotalUpgrade::TotalUpgrade() { reset(); }
...
void TotalUpgrade::sum(const UpgradeTypeBase *ut, const Unit *unit,
...

  {
    ...
    if (ut->getMaxHpIsMultiplier() == true) {
      upgradeValue = ((double)unit->getType()->getMaxHp() *
...

  {
    ...
    if (ut->getMaxEpIsMultiplier() == true) {
      upgradeValue = ((double)unit->getType()->getHpRegeneration() *
...

  {
    int upgradeValue;
    if (ut->getMaxEpIsMultiplier() == true) {
      upgradeValue = ((double)unit->getType()->getMaxEp() *
...

  {
    int upgradeValue;
    if (ut->getMaxEpIsMultiplier() == true) {
      upgradeValue = ((double)unit->getType()->getEpRegeneration() *
...

  {
    ...
    if (ut->getSightIsMultiplier() == true) {
      upgradeValue = ((double)unit->getType()->getSight() *
...

  {
    ...
    if (ut->getArmorIsMultiplier() == true) {
      upgradeValue = ((double)unit->getType()->getArmor() *
  ...
  for (unsigned int i = 0;
    ...
    if (ast != NULL) {
      ...
      if (ut->getAttackStrengthIsMultiplier() == true) {
        newValue = ((double)ast->getAttackStrength() *
  ...
  for (unsigned int i = 0;
    ...
    if (ast != NULL) {
      ...
      if (ut->getAttackRangeIsMultiplier() == true) {
        newValue = ((double)ast->getAttackRange() *
  ...
  if (ut->getAttackSpeedIsMultiplier() == true) {
    for (unsigned int i = 0;
      ...
      const AttackSkillType *ast =
          dynamic_cast<const AttackSkillType *>(skillType);
      if (ast != NULL) {
        int newValue = ((double)ast->getSpeed() *
...
void TotalUpgrade::deapply(int sourceUnitId, const UpgradeTypeBase *ut,
  ...
  if (removedBoost == false) {
    printf("\n\n!!!!!! de-apply boost NOT FOUND for sourceUnitId = %d, "
           "destUnitId = %d\n%s\n\nCurrent Boosts:\n",
...
int TotalUpgrade::getMaxHp() const { return maxHp + getMaxHpFromBoosts(); }
...
int TotalUpgrade::getSight() const { return sight + getSightFromBoosts(); }
...
int TotalUpgrade::getMaxEp() const { return maxEp + getMaxEpFromBoosts(); }
...
int TotalUpgrade::getArmor() const { return armor + getArmorFromBoosts(); }
...
void TotalUpgrade::saveGame(XmlNode *rootNode) const {
  ...
  upgradeTypeBaseNode->addAttribute("maxHp", intToStr(maxHp),
                                    mapTagReplacements);
  //    bool maxHpIsMultiplier;
  upgradeTypeBaseNode->addAttribute(
      "maxHpIsMultiplier", intToStr(maxHpIsMultiplier), mapTagReplacements);
  //	int maxHpRegeneration;
  upgradeTypeBaseNode->addAttribute(
      "maxHpRegeneration", intToStr(maxHpRegeneration), mapTagReplacements);
  //	//bool maxHpRegenerationIsMultiplier;
  //
  //    int sight;
  upgradeTypeBaseNode->addAttribute("sight", intToStr(sight),
  ...
  //    bool sightIsMultiplier;
  upgradeTypeBaseNode->addAttribute(
      "sightIsMultiplier", intToStr(sightIsMultiplier), mapTagReplacements);
  //    int maxEp;
  upgradeTypeBaseNode->addAttribute("maxEp", intToStr(maxEp),
                                    mapTagReplacements);
  //    bool maxEpIsMultiplier;
  upgradeTypeBaseNode->addAttribute(
      "maxEpIsMultiplier", intToStr(maxEpIsMultiplier), mapTagReplacements);
  //	int maxEpRegeneration;
  upgradeTypeBaseNode->addAttribute(
      "maxEpRegeneration", intToStr(maxEpRegeneration), mapTagReplacements);
  //	//bool maxEpRegenerationIsMultiplier;
  //    int armor;
  upgradeTypeBaseNode->addAttribute("armor", intToStr(armor),
                                    mapTagReplacements);
  //    bool armorIsMultiplier;
  upgradeTypeBaseNode->addAttribute(
      "armorIsMultiplier", intToStr(armorIsMultiplier), mapTagReplacements);
  //    int attackStrength;
  upgradeTypeBaseNode->addAttribute("attackStrength", intToStr(attackStrength),
                                    mapTagReplacements);
  //    bool attackStrengthIsMultiplier;
  upgradeTypeBaseNode->addAttribute("attackStrengthIsMultiplier",
                                    intToStr(attackStrengthIsMultiplier),
                                    mapTagReplacements);
  //    std::map<string,int> attackStrengthMultiplierValueList;
  for (std::map<string, int>::const_iterator iterMap =
    ...
    attackStrengthMultiplierValueListNode->addAttribute("key", iterMap->first,
                                                        mapTagReplacements);
    attackStrengthMultiplierValueListNode->addAttribute(
        "value", intToStr(iterMap->second), mapTagReplacements);
  }
  //    int attackRange;
  upgradeTypeBaseNode->addAttribute("attackRange", intToStr(attackRange),
                                    mapTagReplacements);
  ...
  for (std::map<string, int>::const_iterator iterMap =
    ...
    attackRangeMultiplierValueListNode->addAttribute("key", iterMap->first,
                                                     mapTagReplacements);
    attackRangeMultiplierValueListNode->addAttribute(
        "value", intToStr(iterMap->second), mapTagReplacements);
  }
  ...
  for (std::map<string, int>::const_iterator iterMap =
    ...
    moveSpeedIsMultiplierValueListNode->addAttribute("key", iterMap->first,
                                                     mapTagReplacements);
    moveSpeedIsMultiplierValueListNode->addAttribute(
        "value", intToStr(iterMap->second), mapTagReplacements);
  }
  ...
  for (std::map<string, int>::const_iterator iterMap =
    ...
    prodSpeedProduceIsMultiplierValueListNode->addAttribute(
        "key", iterMap->first, mapTagReplacements);
    prodSpeedProduceIsMultiplierValueListNode->addAttribute(
        "value", intToStr(iterMap->second), mapTagReplacements);
  }
  ...
  for (std::map<string, int>::const_iterator iterMap =
    ...
    prodSpeedUpgradeIsMultiplierValueListNode->addAttribute(
        "key", iterMap->first, mapTagReplacements);
    prodSpeedUpgradeIsMultiplierValueListNode->addAttribute(
        "value", intToStr(iterMap->second), mapTagReplacements);
  }
  ...
  for (std::map<string, int>::const_iterator iterMap =
    ...
    prodSpeedMorphIsMultiplierValueListNode->addAttribute("key", iterMap->first,
                                                          mapTagReplacements);
    prodSpeedMorphIsMultiplierValueListNode->addAttribute(
        "value", intToStr(iterMap->second), mapTagReplacements);
  }

  upgradeTypeBaseNode->addAttribute("attackSpeed", intToStr(attackSpeed),
                                    mapTagReplacements);
  ...
  for (std::map<string, int>::const_iterator iterMap =
    ...
    attackSpeedIsMultiplierValueListNode->addAttribute("key", iterMap->first,
                                                       mapTagReplacements);
    attackSpeedIsMultiplierValueListNode->addAttribute(
        "value", intToStr(iterMap->second), mapTagReplacements);
  }

  //	for(unsigned int index = 0; index < boostUpgrades.size(); ++index) {
  //		TotalUpgrade *boost = boostUpgrades[index];
  //		XmlNode *attackBoostListNode =
  // upgradeTypeBaseNode->addChild("attackBoostList");
  //		attackBoostListNode->addAttribute("unitId",intToStr(boost->boostUpgradeUnit->getId()),
  // mapTagReplacements);
  //
  //		std::map<string,string> mapTagReplacements;
  //		if(boost != NULL) {
...
void TotalUpgrade::loadGame(const XmlNode *rootNode) {
  ...
  vector<XmlNode *> attackStrengthMultiplierValueNodeList =
      upgradeTypeBaseNode->getChildList("attackStrengthMultiplierValueList");
  for (unsigned int i = 0; i < attackStrengthMultiplierValueNodeList.size();
       ++i) {
  ...
  //    std::map<string,int> prodSpeedProduceIsMultiplierValueList;
  vector<XmlNode *> prodSpeedProduceIsMultiplierValueNodeList =
      upgradeTypeBaseNode->getChildList(
          "prodSpeedProduceIsMultiplierValueList");
  for (unsigned int i = 0; i < prodSpeedProduceIsMultiplierValueNodeList.size();
       ++i) {
  ...
  vector<XmlNode *> prodSpeedUpgradeIsMultiplierValueNodeList =
      upgradeTypeBaseNode->getChildList(
          "prodSpeedUpgradeIsMultiplierValueList");
  for (unsigned int i = 0; i < prodSpeedUpgradeIsMultiplierValueNodeList.size();
       ++i) {
  ...
  vector<XmlNode *> prodSpeedMorphIsMultiplierValueNodeList =
      upgradeTypeBaseNode->getChildList("prodSpeedMorphIsMultiplierValueList");
  for (unsigned int i = 0; i < prodSpeedMorphIsMultiplierValueNodeList.size();
       ++i) {
  ...
  if (upgradeTypeBaseNode->hasAttribute("attackSpeed")) {
    ...
    vector<XmlNode *> attackSpeedIsMultiplierValueNodeList =
        upgradeTypeBaseNode->getChildList("attackSpeedIsMultiplierValueList");
    for (unsigned int i = 0; i < attackSpeedIsMultiplierValueNodeList.size();
         ++i) {
  ...
  //	vector<XmlNode *> boostNodeList =
  // upgradeTypeBaseNode->getChildList("attackBoostList"); 	for(unsigned int
  // index = 0; index < boostNodeList.size(); ++index) { 		XmlNode
  //*boostNode = boostNodeList[index];
  //
  //	//for(unsigned int index = 0; index < boostUpgrades.size(); ++index) {
  //	//	TotalUpgrade *boost = boostUpgrades[index];
  //	//	XmlNode *attackBoostListNode =

...
int CommandHelper::getBasicPos(CommandClass cc) {
  ...
  if (it != basics.end())
    ...
    throw megaglest_runtime_error("Basics command have not class: " +
                                  intToStr(cc));
}
...
void CommandType::load(
    int id, const XmlNode *n, const string &dir, const TechTree *tt,
  ...
  loadedFileList[imageNode->getAttribute("path")->getRestrictedValue(
                     currentPath)]
      .push_back(make_pair(
          parentLoader, imageNode->getAttribute("path")->getRestrictedValue()));
  ...
  if (n->hasChild("fog-of-war-skill") == true) {
    string skillName = n->getChild("fog-of-war-skill")
                           ->getAttribute("value")
                           ->getRestrictedValue();
    fogOfWarSkillType = static_cast<const FogOfWarSkillType *>(
        ut.getSkillType(skillName, scFogOfWar));

    string skillAttachmentNames = n->getChild("fog-of-war-skill")
                                      ->getAttribute("skill-attachments")
                                      ->getValue();
...
void StopCommandType::load(
  ...
  // stop
  string skillName =
      n->getChild("stop-skill")->getAttribute("value")->getRestrictedValue();
  stopSkillType =
      static_cast<const StopSkillType *>(ut.getSkillType(skillName, scStop));
}
...
void MoveCommandType::load(
  ...
  // move
  string skillName =
      n->getChild("move-skill")->getAttribute("value")->getRestrictedValue();
  moveSkillType =
      static_cast<const MoveSkillType *>(ut.getSkillType(skillName, scMove));
}
...
string MoveCommandType::getDesc(const TotalUpgrade *totalUpgrade,
  ...
  str = getName(translatedValue) + "\n";
  str +=
      lang.getString("WalkSpeed", (translatedValue == true ? "" : "english")) +
...
void AttackCommandType::load(
  ...
  // attack
  skillName =
      n->getChild("attack-skill")->getAttribute("value")->getRestrictedValue();
  attackSkillType = static_cast<const AttackSkillType *>(
      ut.getSkillType(skillName, scAttack));
}
...
string AttackCommandType::getDesc(const TotalUpgrade *totalUpgrade,
  ...
  // attack strength
  str += lang.getString("AttackStrenght",
                        (translatedValue == true ? "" : "english")) +
         ": ";
  str += intToStr(attackSkillType->getAttackStrength() -
  ...
  if (attackSkillType->getSplashRadius() != 0) {
    str += lang.getString("SplashRadius",
                          (translatedValue == true ? "" : "english")) +
           ": " + intToStr(attackSkillType->getSplashRadius()) + "\n";
  }

  // attack distance
  str += lang.getString("AttackDistance",
                        (translatedValue == true ? "" : "english")) +
  ...
  // attack speed
  str += lang.getString("AttackSpeed",
                        (translatedValue == true ? "" : "english")) +
...
void AttackStoppedCommandType::update(UnitUpdater *unitUpdater, Unit *unit,
                                      int frameIndex) const {
  unitUpdater->updateAttackStopped(unit, frameIndex);
}

void AttackStoppedCommandType::load(
    int id, const XmlNode *n, const string &dir, const TechTree *tt,
    const FactionType *ft, const UnitType &ut,
    std::map<string, vector<pair<string, string> > > &loadedFileList,
    string parentLoader) {
...
string AttackStoppedCommandType::getDesc(const TotalUpgrade *totalUpgrade,
                                         bool translatedValue) const {
  Lang &lang = Lang::getInstance();
  string str;
...
void BuildCommandType::load(
  ...
  // build
  skillName =
      n->getChild("build-skill")->getAttribute("value")->getRestrictedValue();
  buildSkillType =
      static_cast<const BuildSkillType *>(ut.getSkillType(skillName, scBuild));
...
string BuildCommandType::getDesc(const TotalUpgrade *totalUpgrade,
  ...
  str = getName(translatedValue) + "\n";
  str +=
      lang.getString("BuildSpeed", (translatedValue == true ? "" : "english")) +
...
void HarvestCommandType::load(
  ...
  // harvest
  skillName =
      n->getChild("harvest-skill")->getAttribute("value")->getRestrictedValue();
  harvestSkillType = static_cast<const HarvestSkillType *>(
      ut.getSkillType(skillName, scHarvest));

  // stop loaded
  skillName = n->getChild("stop-loaded-skill")
                  ->getAttribute("value")
                  ->getRestrictedValue();
  stopLoadedSkillType =
      static_cast<const StopSkillType *>(ut.getSkillType(skillName, scStop));
  ...
  // move loaded
  skillName = n->getChild("move-loaded-skill")
                  ->getAttribute("value")
                  ->getRestrictedValue();
  moveLoadedSkillType =
      static_cast<const MoveSkillType *>(ut.getSkillType(skillName, scMove));
  ...
  for (int i = 0; i < (int)resourcesNode->getChildCount(); ++i) {
    ...
    harvestedResources.push_back(tt->getResourceType(
...
string HarvestCommandType::getDesc(const TotalUpgrade *totalUpgrade,
  ...
  str = getName(translatedValue) + "\n";
  str += lang.getString("HarvestSpeed",
                        (translatedValue == true ? "" : "english")) +
         ": " + intToStr(harvestSkillType->getSpeed() / hitsPerUnit) + "\n";
  str += lang.getString("MaxLoad", (translatedValue == true ? "" : "english")) +
         ": " + intToStr(maxLoad) + "\n";
  str += lang.getString("LoadedSpeed",
                        (translatedValue == true ? "" : "english")) +
  ...
  str +=
      lang.getString("Resources", (translatedValue == true ? "" : "english")) +
      ":\n";
...
void HarvestEmergencyReturnCommandType::load(
    int id, const XmlNode *n, const string &dir, const TechTree *tt,
    const FactionType *ft, const UnitType &ut,
    std::map<string, vector<pair<string, string> > > &loadedFileList,
    string parentLoader) {
...
string HarvestEmergencyReturnCommandType::getDesc(
    const TotalUpgrade *totalUpgrade, bool translatedValue) const {
...
void RepairCommandType::load(
  ...
  // move skill (no longer required by means unit must already be beside unit to
  // repair) for example a hospital
  ...
  // repair
  string skillName =
      n->getChild("repair-skill")->getAttribute("value")->getRestrictedValue();
  repairSkillType = static_cast<const RepairSkillType *>(
      ut.getSkillType(skillName, scRepair));
  ...
  for (int i = 0; i < (int)unitsNode->getChildCount(); ++i) {
    ...
    repairableUnits.push_back(
        ft->getUnitType(unitNode->getAttribute("name")->getRestrictedValue()));
  }
...
string RepairCommandType::getDesc(const TotalUpgrade *totalUpgrade,
  ...
  str = getName(translatedValue) + "\n";
  str += lang.getString("RepairSpeed",
                        (translatedValue == true ? "" : "english")) +
  ...
  for (int i = 0; i < (int)repairableUnits.size(); ++i) {
    str += (static_cast<const UnitType *>(repairableUnits[i]))
...
bool RepairCommandType::isRepairableUnitType(const UnitType *unitType) const {
  for (int i = 0; i < (int)repairableUnits.size(); ++i) {
    ...
    // printf("Lookup index = %d Can repair unittype [%s][%p] looking for
    // [%s][%p] lookup found result =
    // %d\n",i,curUnitType->getName().c_str(),curUnitType,unitType->getName().c_str(),unitType,(curUnitType
    // == unitType));
...
void ProduceCommandType::load(
  ...
  // produce
  string skillName =
      n->getChild("produce-skill")->getAttribute("value")->getRestrictedValue();
  produceSkillType = static_cast<const ProduceSkillType *>(
      ut.getSkillType(skillName, scProduce));

  string producedUnitName =
      n->getChild("produced-unit")->getAttribute("name")->getRestrictedValue();
...
string ProduceCommandType::getDesc(const TotalUpgrade *totalUpgrade,
  ...
  // prod speed
  str += lang.getString("ProductionSpeed",
                        (translatedValue == true ? "" : "english")) +
  ...
  int64 speed = produceSkillType->getSpeed() +
...
void UpgradeCommandType::load(
  ...
  // upgrade
  string skillName =
      n->getChild("upgrade-skill")->getAttribute("value")->getRestrictedValue();
  upgradeSkillType = static_cast<const UpgradeSkillType *>(
      ut.getSkillType(skillName, scUpgrade));

  string producedUpgradeName = n->getChild("produced-upgrade")
                                   ->getAttribute("name")
                                   ->getRestrictedValue();
...
string UpgradeCommandType::getDesc(const TotalUpgrade *totalUpgrade,
  ...
  str = getName(translatedValue) + "\n";
  str += lang.getString("UpgradeSpeed",
                        (translatedValue == true ? "" : "english")) +
  ...
  int64 speed = upgradeSkillType->getSpeed() +
...
void MorphCommandType::load(
  ...
  // morph skill
  string skillName =
      n->getChild("morph-skill")->getAttribute("value")->getRestrictedValue();
  morphSkillType =
      static_cast<const MorphSkillType *>(ut.getSkillType(skillName, scMorph));

  // morph unit
  string morphUnitName =
      n->getChild("morph-unit")->getAttribute("name")->getRestrictedValue();
  ...
  if (n->hasChild("ignore-resource-requirements") == true) {
    ignoreResourceRequirements = n->getChild("ignore-resource-requirements")
                                     ->getAttribute("value")
                                     ->getBoolValue();
...
string MorphCommandType::getDesc(const TotalUpgrade *totalUpgrade,
  ...
  // prod speed
  str +=
      lang.getString("MorphSpeed", (translatedValue == true ? "" : "english")) +
  ...
  int64 speed =
      morphSkillType->getSpeed() + totalUpgrade->getProdSpeed(morphSkillType);
  ...
  if (discount != 0) {
    str +=
        lang.getString("Discount", (translatedValue == true ? "" : "english")) +
        ": " + intToStr(discount) + "%\n";
  }
...
void SwitchTeamCommandType::update(UnitUpdater *unitUpdater, Unit *unit,
                                   int frameIndex) const {
  unitUpdater->updateSwitchTeam(unit, frameIndex);
}

void SwitchTeamCommandType::load(
    int id, const XmlNode *n, const string &dir, const TechTree *tt,
    const FactionType *ft, const UnitType &ut,
    std::map<string, vector<pair<string, string> > > &loadedFileList,
    string parentLoader) {
  ...
  // string skillName=
  // n->getChild("morph-skill")->getAttribute("value")->getRestrictedValue();
  // morphSkillType= static_cast<const
  // MorphSkillType*>(ut.getSkillType(skillName, scMorph));

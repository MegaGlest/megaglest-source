...
// 	class LootResource
//
///	Stores information about a lootable resource. Lootable resources are
/// stolen by the attacker on death.
// ===============================
...
class LootableResource {
  ...
  public:
    ...
    bool operator==(const LootableResource &other) { return type == other.getResourceType(); }
};
...
enum UnitClass { ucWarrior, ucWorker, ucBuilding };

typedef vector<UnitParticleSystemType *> DamageParticleSystemTypes;

enum HealthbarVisible { hbvUndefined = 0, hbvOff = 1, hbvAlways = 2, hbvIfNeeded = 4, hbvSelected = 8 };

enum UnitCountsInVictoryConditions { ucvcNotSet, ucvcTrue, ucvcFalse };
...
class UnitType : public ProducibleType, public ValueCheckerVault {
  public:
    ...
    enum StartType { stValue, stPercentage };
  ...
  public:
    ...
    void loaddd(int id, const string &dir, const TechTree *techTree, const string &techTreePath, const FactionType *factionType, Checksum *checksum,
                Checksum *techtreeChecksum, std::map<string, vector<pair<string, string>>> &loadedFileList, bool validationMode = false);
    ...
    inline bool hasCommandClass(CommandClass commandClass) const { return firstCommandTypeOfClass[commandClass] != NULL; }
...
/**
 * Used to sort UnitType. Sorts by *translated* unit name. Sorting is case
 * sensitive and done in lexical order.
 */
struct UnitTypeSorter {
    bool operator()(const UnitType *left, const UnitType *right) const { return left->getName(true) < right->getName(true); }
};

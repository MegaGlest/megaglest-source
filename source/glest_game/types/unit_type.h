...
class UnitType : public ProducibleType, public ValueCheckerVault {
 ...
 public:
  ...
  inline int getCommandTypeSortedCount() const {
    return (int)commandTypesSorted.size();
  }
  ...
  const HarvestCommandType *getFirstHarvestCommand(
  ...
  const AttackStoppedCommandType *getFirstAttackStoppedCommand(
      Field field) const;
  const RepairCommandType *getFirstRepairCommand(

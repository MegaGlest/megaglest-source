...
bool Config::tryCustomPath(std::pair<ConfigType, ConfigType> &type,
  ...
  if ((type.first == cfgMainGame && type.second == cfgUserGame &&
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("-=-=-=-=-=-=-= looking for file in possible location  [%s]\n",
             linuxPath.c_str());

#if defined(__linux__)
    string nixFile = linuxPath + "linux_" + file.first;
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("-=-=-=-=-=-=-= looking for linux specific file in possible "
             "location  [%s]\n",
             nixFile.c_str());
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("-=-=-=-=-=-=-= looking for windows specific file in possible "
             "location  [%s]\n",
             winFile.c_str());
...
Config::Config(std::pair<ConfigType, ConfigType> type,
  ...
  if (foundPath == false) {
#ifndef NO_APPIMAGE
    foundPath = tryCustomPath(cfgType, fileName,
                              Properties::appendAppDirPath(formatPath(
...
// Look in standard linux shared paths for ini files
  ...
  if (foundPath == false) {
    foundPath =
        tryCustomPath(cfgType, fileName, "/usr/local/share/games/megaglest/");
  }
#endif
...
#if defined(WIN32)
  // string test =
  // "C:\\Code\\megaglest\\mk\\windoze\\.\\..\\..\\data\\glest_game\\glest.ini";
  ...
  if (cfgType.first == cfgMainGame) {
    if (properties.first.getString("UserData_Root",
    ...
    } else if (properties.first.getString("UserOverrideFile",
                                          defaultNotFoundValue.c_str()) !=
               defaultNotFoundValue) {
    ...
    if (mainCfg.getString("UserData_Root", defaultNotFoundValue.c_str()) !=
        defaultNotFoundValue) {
      ...
      fileName.second = userData + fileNameParameter.second;
    } else if (mainCfg.getString("UserOverrideFile",
                                 defaultNotFoundValue.c_str()) !=
               defaultNotFoundValue) {
    ...
    if (fileName.second != "" && fileExists(fileName.second) == false) {
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                 "In [%s::%s Line: %d] attempting to "
                                 "auto-create cfgFile.second = [%s]\n",
...
void Config::reload() {
  ...
  Config newconfig(
      type, std::make_pair(fileNameParameter.first, fileNameParameter.second),
  ...
  if (fileLoaded.second == true) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf("In [%s::%s Line: %d] save file [%s]\n", __FILE__, __FUNCTION__,
             __LINE__, fileName.second.c_str());
...
int Config::getInt(const char *key, const char *defaultValueIfNotFound) const {
  ...
  if (fileLoaded.second == true &&
      properties.second.getString(key, defaultNotFoundValue.c_str()) !=
          defaultNotFoundValue) {
...
Config::getPropertiesFromContainer(const Properties &propertiesObj) const {
...
string Config::toString() { return properties.first.toString(); }
...
vector<string> Config::getPathListForType(PathType type, string scenarioDir) {
  ...
  if (userData != "") {
    ...
    // else {
    //	if(SystemFlags::VERBOSE_MODE_ENABLED)
    // printf("extractLastDirectoryFromPath(userData) [%s] from userData

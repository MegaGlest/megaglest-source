...
bool Config::tryCustomPath(std::pair<ConfigType, ConfigType> &type,
  ...
  if ((type.first == cfgMainGame && type.second == cfgUserGame &&
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "-=-=-=-=-=-=-= looking for linux specific file in possible location "
          " [%s]\n",
          nixFile.c_str());
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED)
      printf(
          "-=-=-=-=-=-=-= looking for windows specific file in possible "
          "location  [%s]\n",
          winFile.c_str());
...
vector<pair<string, string> > Config::getPropertiesFromContainer(

...
bool Config::tryCustomPath(std::pair<ConfigType, ConfigType> &type, std::pair<string, string> &file, string custom_path) {
    bool wasFound = false;
    if ((type.first == cfgMainGame && type.second == cfgUserGame && file.first == glest_ini_filename && file.second == glestuser_ini_filename) ||
        (type.first == cfgMainKeys && type.second == cfgUserKeys && file.first == glestkeys_ini_filename && file.second == glestuserkeys_ini_filename)) {
        ...
        if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("-=-=-=-=-=-=-= looking for linux specific file in possible location "
                   " [%s]\n",
                   nixFile.c_str());
...
// Look in standard linux shared paths for ini files
    ...
    if (fileMustExist.first == true || (fileMustExist.first == false && fileExists(fileName.first) == true)) {
    ...
    if (fileMustExist.second == true || (fileMustExist.second == false && fileExists(fileName.second) == true)) {
...
int Config::getInt(const char *key, const char *defaultValueIfNotFound) const {
    ...
    if (fileLoaded.second == true && properties.second.getString(key, defaultNotFoundValue.c_str()) != defaultNotFoundValue) {

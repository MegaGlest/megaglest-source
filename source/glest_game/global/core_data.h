...
class CoreData {
  ...
  private:
    ...
    string getDataPath();
    void loadTextureIfRequired(Texture2D **tex, string data_path, string uniqueFilePath, int texSystemId, bool setMipMap, bool setAlpha,
                               bool loadUniqueFilePath, bool compressionDisabled = false);
    ...
    template <typename T>
    T *loadFont(Font *menuFontNormal, string menuFontNameNormal, int menuFontNameNormalSize, string fontType, string fontTypeFamily, string fontUniqueKey);
};

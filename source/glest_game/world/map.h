...
class Cell {
 private:
  ...
  Unit *unitsWithEmptyCellMap[fieldCount];  // units with an empty cellmap on
                                            // this cell
  float height;
 ...
 public:
  ...
  inline bool isFree(Field field) const {
    ...
    if (result == false) {
      // printf("[%s] Line: %d returning false, unit id = %d
  ...
  inline bool isFreeOrMightBeFreeSoon(Vec2i originPos, Vec2i cellPos,
                                      Field field) const {
    Unit *unit = getUnit(field);
    bool result = (unit == NULL || unit->isPutrefacting());
...
class SurfaceCell {
 private:
  ...
  // tex coords
  Vec2f fowTexCoord;   // tex coords for TEXTURE1 when multitexturing and
                       // fogOfWar
 ...
 public:
  ...
  inline void setHeight(float height,
                        bool cellChangedFromOriginalMapLoadValue = false);
  ...
  inline void setWaterWaveAmplitude(float waterWaveAmplitude) {
    this->waterWaveAmplitude = waterWaveAmplitude;
  }
...
class FastAINodeCache {
 public:
  explicit FastAINodeCache(Unit *unit) { this->unit = unit; }
  Unit *unit;
...
class Map {
 ...
 public:
  ...
  inline Cell *getCell(int x, int y, bool errorOnInvalid = true) const {
    ...
    if (arrayIndex < 0 || arrayIndex >= getCellArraySize()) {
      ...
      throw megaglest_runtime_error(
          "arrayIndex >= getCellArraySize(), arrayIndex = " +
          intToStr(arrayIndex) + " w = " + intToStr(w) + " h = " + intToStr(h));
    } else if (cells == NULL) {
      if (errorOnInvalid == false) {
        return NULL;
      }
  ...
  inline Cell *getCell(const Vec2i &pos) const { return getCell(pos.x, pos.y); }

  inline int getCellArraySize() const { return (w * h); }
  ...
  inline SurfaceCell *getSurfaceCell(int sx, int sy) const {
    ...
    if (arrayIndex < 0 || arrayIndex >= getSurfaceCellArraySize()) {
      throw megaglest_runtime_error(
          "arrayIndex >= getSurfaceCellArraySize(), arrayIndex = " +
          intToStr(arrayIndex) + " surfaceW = " + intToStr(surfaceW) +
          " surfaceH = " + intToStr(surfaceH) + " sx: " + intToStr(sx) +
          " sy: " + intToStr(sy));
  ...
  bool isResourceNear(int frameIndex, const Vec2i &pos, const ResourceType *rt,
                      Vec2i &resourcePos, int size, Unit *unit = NULL,
                      bool fallbackToPeersHarvestingSameResource = false,
                      Vec2i *resourceClickPos = NULL) const;

  // free cells
  bool isFreeCell(const Vec2i &pos, Field field,
                  bool buildingsOnly = false) const;
  bool isFreeCellOrHasUnit(const Vec2i &pos, Field field,
  ...
  bool isFreeCells(const Vec2i &pos, int size, Field field,
                   bool buildingsOnly = false) const;
  bool isFreeCellsOrHasUnit(const Vec2i &pos, int size, Field field,
                            const Unit *unit) const;
  bool isAproxFreeCells(const Vec2i &pos, int size, Field field,
                        int teamIndex) const;
  bool canMorph(const Vec2i &pos, const Unit *currentUnit,
  ...
  const Unit *findClosestUnitToPos(const Selection *selection,
                                   Vec2i originalBuildPos,
                                   const UnitType *ut) const;
  bool isInUnitTypeCells(const UnitType *ut, const Vec2i &pos,
                         const Vec2i &testPos) const;
  bool isNextToUnitTypeCells(const UnitType *ut, const Vec2i &pos,
                             const Vec2i &testPos) const;
  Vec2i findBestBuildApproach(const Unit *unit, Vec2i originalBuildPos,
                              const UnitType *ut) const;
  // std::pair<float,Vec2i> getUnitDistanceToPos(const Unit *unit,Vec2i
  // pos,const UnitType *ut);
  ...
  inline bool isFreeCellOrMightBeFreeSoon(Vec2i originPos, const Vec2i &pos,
                                          Field field) const {
  ...
  inline bool isAproxFreeCellOrMightBeFreeSoon(Vec2i originPos,
                                               const Vec2i &pos, Field field,
                                               int teamIndex) const {
  ...
  // checks if a unit can move from between 2 cells using only visible cells
  // (for pathfinding)
  inline bool aproxCanMoveSoon(Unit *unit, const Vec2i &pos1,
                               const Vec2i &pos2) const {
    if (isInside(pos1) == false ||
        isInsideSurface(toSurfCoords(pos1)) == false ||
        isInside(pos2) == false ||
        isInsideSurface(toSurfCoords(pos2)) == false) {
      // printf("[%s] Line: %d returning false\n",__FUNCTION__,__LINE__);
    ...
    // single cell units
    if (size == 1) {
      bool tryPosResult = isAproxFreeCellOrMightBeFreeSoon(
          unit->getPosNotThreadSafe(), pos2, field, teamIndex);

      if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
                  .enabled == true &&
          SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynchMax)
                  .enabled == true) {
        string extraInfo = (string("tryPosResult = ") +
                            (tryPosResult ? string("true") : string("false")));
        const SurfaceCell *sc = getSurfaceCell(toSurfCoords(pos2));
        if (sc->isVisible(teamIndex)) {
          bool testCond = isFreeCellOrMightBeFreeSoon(
              unit->getPosNotThreadSafe(), pos2, field);
          extraInfo += (string("isFreeCellOrMightBeFreeSoon = ") +
                        (testCond ? string("true") : string("false")));
        } else if (sc->isExplored(teamIndex)) {
          bool testCond = field == fLand
                              ? sc->isFree() && !getDeepSubmerged(getCell(pos2))
                              : true;
          extraInfo += (string("field==fLand = ") +
                        (testCond ? string("true") : string("false")));
        }

        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 "In aproxCanMoveSoon() pos2 = %s extraInfo = %s %s %s",
                 pos2.getString().c_str(), extraInfo.c_str(),
      ...
      if (pos1.x != pos2.x && pos1.y != pos2.y) {
        ...
        bool tryPosResult = isAproxFreeCellOrMightBeFreeSoon(
            unit->getPosNotThreadSafe(), tryPos, field, teamIndex);
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
          ...
          if (sc->isVisible(teamIndex)) {
            bool testCond = isFreeCellOrMightBeFreeSoon(
                unit->getPosNotThreadSafe(), tryPos, field);
            extraInfo += (string("isFreeCellOrMightBeFreeSoon = ") +
                          (testCond ? string("true") : string("false")));
          } else if (sc->isExplored(teamIndex)) {
            bool testCond =
                field == fLand
                    ? sc->isFree() && !getDeepSubmerged(getCell(tryPos))
                    : true;
            extraInfo += (string("field==fLand = ") +
                          (testCond ? string("true") : string("false")));
          }
          ...
          char szBuf[8096] = "";
          snprintf(szBuf, 8096, "In aproxCanMoveSoon() extraInfo = %s",
                   extraInfo.c_str());
          if (Thread::isCurrentThreadMainThread() == false) {
            unit->logSynchDataThreaded(__FILE__, __LINE__, szBuf);
          } else {
        ...
        tryPosResult = isAproxFreeCellOrMightBeFreeSoon(
            unit->getPosNotThreadSafe(), tryPos, field, teamIndex);
        ...
        for (int j = pos2.y; j < pos2.y + size; ++j) {
          ...
          if (isInside(cellPos) && isInsideSurface(toSurfCoords(cellPos))) {
            if (getCell(cellPos)->getUnit(unit->getCurrField()) != unit) {
              if (isAproxFreeCellOrMightBeFreeSoon(unit->getPosNotThreadSafe(),
                                                   cellPos, field,
                                                   teamIndex) == false) {
 ...
 private:
  ...
  void putUnitCellsPrivate(Unit *unit, const Vec2i &pos, const UnitType *ut,

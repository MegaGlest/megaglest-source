...
void AmbientSounds::load(const string &dir, const XmlNode *xmlNode, std::map<string, vector<pair<string, string>>> &loadedFileList, string parentLoader) {
    string path = "";
...
void Tileset::load(const string &dir, Checksum *checksum, Checksum *tilesetChecksum, std::map<string, vector<pair<string, string>>> &loadedFileList) {
    ...
    try {
        ...
        for (int i = 0; i < surfCount; ++i) {
            ...
            if (partsize == 0) {
                ...
                for (int j = 0; j < childCount; ++j) {
                    ...
                    loadedFileList[textureNode->getAttribute("path")->getRestrictedValue(currentPath)].push_back(
                        make_pair(sourceXMLFile, textureNode->getAttribute("path")->getRestrictedValue()));
                ...
                if (modelNode->hasChild("particles")) {
                    ...
                    if (particleEnabled) {
                        for (int k = 0; k < (int)particleNode->getChildCount(); ++k) {
                            ...
                            objectParticleSystemType->load(particleFileNode, dir, currentPath + path, &Renderer::getInstance(), loadedFileList, sourceXMLFile,
                                                           "");
                            loadedFileList[currentPath + path].push_back(
                                make_pair(sourceXMLFile, particleFileNode->getAttribute("path")->getRestrictedValue()));
        ...
        for (vector<XmlNode *>::size_type i = 0; i < wavesChildList.size(); i++) {
            waterWave wt = waterWave{
                wavesChildList[i]->getAttribute("amplitude")->getFloatValue(), wavesChildList[i]->getAttribute("frequency")->getFloatValue(),
        ...
        for (std::vector<XmlNode *>::size_type i = 0; i < waterFrameNodes.size(); ++i) {
            ...
            loadedFileList[waterFrameNode->getAttribute("path")->getRestrictedValue(currentPath)].push_back(
                make_pair(sourceXMLFile, waterFrameNode->getAttribute("path")->getRestrictedValue()));
        }
...
void Tileset::addSurfTex(int leftUp, int rightUp, int leftDown, int rightDown, Vec2f &coord, const Texture2D *&texture, int mapX, int mapY) {
    ...
    // spatted textures
        ...
        SurfaceInfo si(getSurfPixmap(leftUp, var), getSurfPixmap(rightUp, var), getSurfPixmap(leftDown, var), getSurfPixmap(rightDown, var));
        surfaceAtlas.addSurface(&si);
        coord = si.getCoord();
        texture = si.getTexture();
    }

...
// This limit is to keep RAM use under control while offering better
// performance.
...
void World::cleanup() {
  ...
  for (std::map<string, ::Shared::Sound::StaticSound *>::iterator iterMap =
           staticSoundList.begin();
       iterMap != staticSoundList.end(); ++iterMap) {
    delete iterMap->second;
  }
  staticSoundList.clear();

  for (std::map<string, ::Shared::Sound::StrSound *>::iterator iterMap =
           streamSoundList.begin();
...
void World::endScenario() {
  ...
  Logger::getInstance().add(
      Lang::getInstance().getString("LogScreenGameUnLoadingWorld", "", true),
      true);
...
void World::addFogOfWarSkillType(const Unit *unit,
  ...
  if ((fowst->getApplyToTeam() == true &&
       unit->getTeam() == this->getThisTeamIndex()) ||
      (fowst->getApplyToTeam() == false &&
       unit->getFactionIndex() == this->getThisFactionIndex())) {
    if ((fowst->getFowEnable() == false && fogOfWarSkillTypeValue != 0) ||
        (fowst->getFowEnable() == true && fogOfWarSkillTypeValue != 1)) {

      // printf("In [%s::%s Line: %d] current = %d new =
...
void World::removeFogOfWarSkillType(const Unit *unit) {
  ...
  if (removedFromList == true) {
    if (mapFogOfWarUnitList.empty() == true) {
      ...
      for (std::map<int, std::pair<const Unit *, const FogOfWarSkillType *>>::
               const_iterator iterMap = mapFogOfWarUnitList.begin();
...
void World::init(Game *game, bool createUnits, bool initFactions) {
  ...
  initCells(
      fogOfWar); // must be done after knowing faction number and dimensions
  initMap();
      ...
      for (int i = 0; i < map.getSurfaceW(); ++i) {
        for (int j = 0; j < map.getSurfaceH(); ++j) {
          ...
          for (int k = 0; k < GameConstants::maxPlayers; k++) {
            // sc->setExplored(k, (game->getGameSettings()->getFlagTypes1() &
            // ft1_show_map_resources) == ft1_show_map_resources);
            sc->setVisible(k, !fogOfWar);
          }
          for (int k = GameConstants::maxPlayers;
               k < GameConstants::maxPlayers + GameConstants::specialFactions;
               k++) {
  ...
  // initExplorationState(); ... was only for !fog-of-war, now handled in
  // initCells()
  computeFow();
  if (getFrameCount() > 1) {
    // this is needed for games that are loaded to "switch the light on".
    // otherwise the FowTexture is completely black like in the beginning of a
    // game.
...
Checksum World::loadTileset(
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n",
                             __FILE__, __FUNCTION__, __LINE__);
  tilsetChecksum =
      tileset.loadTileset(pathList, tilesetName, checksum, loadedFileList);
...
// load tech
Checksum
World::loadTech(const vector<string> pathList, const string &techName,
  ...
  techtreeChecksum = techTree->loadTech(techName, factions, checksum,
...
Checksum World::loadScenario(const string &path, Checksum *checksum,
                             bool resetCurrentScenario,
                             const XmlNode *rootNode) {
  // printf("[%s:%s] Line: %d path
...
void World::updateAllFactionUnits() {
  ...
  //	Config &config= Config::getInstance();
  //	bool sortedUnitsAllowed =
  // config.getBool("AllowGroupedUnitCommands","true");
  //
  //	int factionCount = getFactionCount();
  ...
  //			if(faction == NULL) {
  //				throw megaglest_runtime_error("faction ==
  // NULL");
  //			}
  //
  ...
  if (newThreadManager == true) {
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED && chrono.getMillis() >= 10)
      printf(
          "In [%s::%s Line: %d] *** Faction thread preprocessing took [%lld] "
          "msecs for %d factions for frameCount = %d slavesCompleted = %d.\n",
          __FILE__, __FUNCTION__, __LINE__, (long long int)chrono.getMillis(),
          factionCount, frameCount, slavesCompleted);
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED && chrono.getMillis() >= 10)
      printf("In [%s::%s Line: %d] *** Faction thread preprocessing took "
             "[%lld] msecs for %d factions for frameCount = %d.\n",
             __FILE__, __FUNCTION__, __LINE__,
             (long long int)chrono.getMillis(), factionCount, frameCount);
  }
  ...
  for (int i = 0; i < factionCount; ++i) {
    ...
    for (int j = 0; j < unitCount; ++j) {
      ...
      if (showPerfStats)
        chronoPerfUnit.start();

      int unitBlockCount = unit->getPath()->getBlockCount();
      bool isStuck = unit->getPath()->isStuck();
      // bool isStuckWithinTolerance =
      // unit->isLastStuckFrameWithinCurrentFrameTolerance(false); uint32
      // lastStuckFrame = unit->getLastStuckFrame();
      ...
      if (unitUpdater.updateUnit(unit) == true) {
        ...
        mapCommandCount[unitCommandClass] =
            mapCommandCount[unitCommandClass] + 1;
      ...
      if (showPerfStats && chronoPerfUnit.getMillis() >= 10) {
        // sprintf(perfBuf,"In [%s::%s] Line: %d took msecs: " MG_I64_SPECIFIER
        // " stuck: %d [%d] for unit:\n%sBEFORE unitBlockCount = %d, AFTER = %d,
        // BEFORE lastStuckFrame = %u, AFTER:
        ...
        sprintf(perfBuf,
                "In [%s::%s] Line: %d took msecs: " MG_I64_SPECIFIER
                " stuck: %d for unit:\n%sBEFORE unitBlockCount = %d, AFTER = "
                "%d, BEFORE , AFTER: %u\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, chronoPerfUnit.getMillis(), isStuck,
                unit->toString().c_str(), unitBlockCount,
                unit->getPath()->getBlockCount(), unit->getLastStuckFrame());
        perfList.push_back(perfBuf);
      }
    }
    ...
    if (showPerfStats) {
      sprintf(perfBuf,
              "In [%s::%s] Line: %d took msecs: " MG_I64_SPECIFIER
              " faction: %d / %d unitCount = %d unitCountUpdated = %d "
              "unitCountStuck = %d\n",
              extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
              __LINE__, chronoPerf.getMillis(), i + 1, factionCount, unitCount,
              unitCountUpdated, unitCountStuck);
      perfList.push_back(perfBuf);

      for (std::map<CommandClass, int>::iterator iterMap =
               mapCommandCount.begin();
  ...
  if (showPerfStats) {
    sprintf(perfBuf,
            "In [%s::%s] Line: %d took msecs: " MG_I64_SPECIFIER
            " totalUnitsProcessed = %d\n",
            extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
            __LINE__, chronoPerf.getMillis(), totalUnitsProcessed);
    perfList.push_back(perfBuf);
  }
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED && chrono.getMillis() >= 20)
    printf("In [%s::%s Line: %d] *** Faction MAIN thread processing took "
           "[%lld] msecs for %d factions for frameCount = %d.\n",
           __FILE__, __FUNCTION__, __LINE__, (long long int)chrono.getMillis(),
           factionCount, frameCount);
}
...
void World::updateAllFactionConsumableCosts() {
  ...
  for (int i = 0; i < resourceTypeCount; ++i) {
    ...
    if (rt != NULL && rt->getClass() == rcConsumable &&
        frameCount % (rt->getInterval() * GameConstants::updateFps) == 0) {
      for (int j = 0; j < factionCount; ++j) {
        ...
        warningSoundNeeded =
            warningSoundNeeded || faction->applyCostsOnInterval(rt);
      }
    }
  }
...
void World::update() {
  ...
  if (this->game)
    this->game->addPerformanceCount("updateAllTilesetObjects",
  ...
  if (getFactionCount() > 0) {
    ...
    if (this->game)
      this->game->addPerformanceCount("updateAllFactionUnits",
                                      chronoGamePerformanceCounts.getMillis());
    ...
    if (this->game)
      this->game->addPerformanceCount("underTakeDeadFactionUnits",
                                      chronoGamePerformanceCounts.getMillis());
    ...
    if (this->game)
      this->game->addPerformanceCount("updateAllFactionConsumableCosts",
                                      chronoGamePerformanceCounts.getMillis());
    ...
    if (fogOfWarSmoothing &&
        ((frameCount + 1) % (fogOfWarSmoothingFrameSkip + 1)) == 0) {
      if (this->game)
        chronoGamePerformanceCounts.start();

      float fogFactor =
          static_cast<float>(frameCount % GameConstants::updateFps) /
          GameConstants::updateFps;
      minimap.updateFowTex(clamp(fogFactor, 0.f, 1.f));

      if (this->game)
        this->game->addPerformanceCount(
            "minimap.updateFowTex", chronoGamePerformanceCounts.getMillis());
    }
    ...
    if (needToTick == true) {
      // printf("=========== World is about to be updated, current frameCount =
      // %d\n",frameCount);
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                 "In [%s::%s Line: %d]\n",
      ...
      if (this->game)
        this->game->addPerformanceCount(
            "world->tick", chronoGamePerformanceCounts.getMillis());
    }
...
void World::tick() {
  ...
  if (this->game)
    this->game->addPerformanceCount("world->computeFow",
                                    chronoGamePerformanceCounts.getMillis());
  ...
  if (showPerfStats) {
    sprintf(perfBuf,
            ...
            " fogOfWar: %d\n",
            extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
            __LINE__, chronoPerf.getMillis(), fogOfWar);
    perfList.push_back(perfBuf);
  }
  ...
  if (fogOfWarSmoothing == false) {
    ...
    if (this->game)
      this->game->addPerformanceCount("minimap.updateFowTex",
                                      chronoGamePerformanceCounts.getMillis());
  }
  ...
  if (this->game)
    this->game->addPerformanceCount("world unit->tick()",
                                    chronoGamePerformanceCounts.getMillis());
  ...
  std::map<const UnitType *, std::map<const ResourceType *, const Resource *>>
      resourceCostCache;
  ...
  for (int factionIndex = 0; factionIndex < factionCount; ++factionIndex) {
    ...
    for (int resourceTypeIndex = 0;
         resourceTypeIndex < techTree->getResourceTypeCount();
         ++resourceTypeIndex) {
      ...
      if (rt != NULL && rt->getClass() == rcConsumable) {
        ...
        int balance = 0;
        for (int unitIndex = 0; unitIndex < faction->getUnitCount();
             ++unitIndex) {
          ...
          if (unit != NULL && unit->isOperative()) {
            ...
            std::map<const UnitType *,
                     std::map<const ResourceType *, const Resource *>>::iterator
                iterFind = resourceCostCache.find(ut);
  ...
  if (this->game)
    this->game->addPerformanceCount("world faction->setResourceBalance()",
                                    chronoGamePerformanceCounts.getMillis());
...
const UnitType *World::findUnitTypeById(const FactionType *factionType,
                                        int id) {
  if (factionType == NULL) {
...
const UnitType *World::findUnitTypeByName(const string factionName,
                                          const string unitTypeName) {
  const UnitType *unitTypeResult = NULL;

  for (int index = 0; unitTypeResult == NULL && index < getFactionCount();
       ++index) {
    const Faction *faction = getFaction(index);
    if (factionName == "" ||
        factionName == faction->getType()->getName(false)) {
      for (int unitIndex = 0;
           unitTypeResult == NULL &&
           unitIndex < faction->getType()->getUnitTypeCount();
           ++unitIndex) {
...
// looks for a place for a unit around a start location, returns true if
// succeded
bool World::placeUnit(const Vec2i &startLoc, int radius, Unit *unit,
                      bool spaciated, bool threaded) {
  if (unit == NULL) {
    throw megaglest_runtime_error("unit == NULL");
  }
  ...
  for (int r = 1; r < radius; r++) {
    for (int i = -r; i < r; ++i) {
      for (int j = -r; j < r; ++j) {
        ...
        if (spaciated) {
          const int spacing = 2;
          freeSpace = map.isFreeCells(pos - Vec2i(spacing), size + spacing * 2,
                                      currField);
        } else {
    ...
    if (map.getSubmerged(map.getCell(unit->getLastPos()))) {
      ...
      if (Thread::isCurrentThreadMainThread() == false) {
        throw megaglest_runtime_error(
            "#1 Invalid access to World random from outside main thread "
            "current id = " +
      ...
      for (int i = 0; i < 3; ++i) {
        waterEffects.addWaterSplash(
            Vec2f(unit->getLastPos().x + (float)unitSize / 2.0f +
                      random.randRange(-0.9f, -0.1f),
                  unit->getLastPos().y + random.randRange(-0.9f, -0.1f) +
                      (float)unitSize / 2.0f),
...
// returns the nearest unit that can store a type of resource given a position
// and a faction
Unit *World::nearestStore(const Vec2i &pos, int factionIndex,
  ...
  for (int i = 0; i < getFaction(factionIndex)->getUnitCount(); ++i) {
    ...
    if (u != NULL) {
      ...
      if (tmpDist < currDist && u->getType() != NULL &&
          u->getType()->getStore(rt) > 0 && u->isOperative()) {
        currDist = tmpDist;
        currUnit = u;
      }
    }
...
bool World::toRenderUnit(const Unit *unit, const Quad2i &visibleQuad) const {
  ...
  // a unit is rendered if it is in a visible cell or is attacking a unit in a
  // visible cell
  return visibleQuad.isInside(unit->getPosNotThreadSafe()) &&
         toRenderUnit(unit);
}
...
bool World::toRenderUnit(const Unit *unit) const {
  ...
  return (map.getSurfaceCell(Map::toSurfCoords(unit->getCenteredPos()))
              ->isVisible(thisTeamIndex) &&
          map.getSurfaceCell(Map::toSurfCoords(unit->getCenteredPos()))
              ->isExplored(thisTeamIndex)) ||
...
bool World::toRenderUnit(const UnitBuildInfo &pendingUnit) const {
  ...
  return (map.getSurfaceCell(Map::toSurfCoords(pendingUnit.pos))
              ->isVisible(thisTeamIndex) &&
          map.getSurfaceCell(Map::toSurfCoords(pendingUnit.pos))
              ->isExplored(thisTeamIndex));
}

void World::morphToUnit(int unitId, const string &morphName,
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA,
                             "In [%s::%s Line: %d] unit [%d] morphName [%s] "
                             "forceUpgradesIfRequired = %d\n",
  ...
  if (unit != NULL) {
    for (int i = 0; i < unit->getType()->getCommandTypeCount(); ++i) {
      ...
      if (mct != NULL && mct->getName(false) == morphName) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
          SystemFlags::OutputDebug(SystemFlags::debugLUA,
                                   "In [%s::%s Line: %d] unit [%d] morphName "
                                   "[%s] comparing mct [%s]\n",
                                   __FILE__, __FUNCTION__, __LINE__, unitId,
                                   morphName.c_str(),
                                   mct->getName(false).c_str());

        std::pair<CommandResult, string> cr(crFailUndefined, "");
        try {
          if (unit->getFaction()->reqsOk(mct) == false &&
              ignoreRequirements == true) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA)
                    .enabled)
              SystemFlags::OutputDebug(
                  SystemFlags::debugLUA,
                  "In [%s::%s Line: %d] unit [%d] morphName [%s] comparing mct "
                  "[%s] mct->getUpgradeReqCount() = %d\n",
                  __FILE__, __FUNCTION__, __LINE__, unitId, morphName.c_str(),
                  mct->getName(false).c_str(), mct->getUpgradeReqCount());
            unit->setIgnoreCheckCommand(true);
          }

          const UnitType *unitType = mct->getMorphUnit();
          cr = this->game->getCommander()->tryGiveCommand(
              unit, mct, unit->getPos(), unitType,
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
          SystemFlags::OutputDebug(SystemFlags::debugLUA,
                                   "In [%s::%s Line: %d] unit [%d] morphName "
                                   "[%s] comparing mct [%s] returned = %d\n",
                                   __FILE__, __FUNCTION__, __LINE__, unitId,
                                   morphName.c_str(),
                                   mct->getName(false).c_str(), cr.first);
...
void World::createUnit(const string &unitName, int factionIndex,
                       const Vec2i &pos, bool spaciated) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugUnitCommands,
        "In [%s::%s Line: %d] unitName [%s] factionIndex = %d\n", __FILE__,
        __FUNCTION__, __LINE__, unitName.c_str(), factionIndex);
  ...
  if (factionIndex < (int)factions.size()) {
    ...
    Unit *unit = new Unit(getNextUnitId(faction), newpath, pos, ut, faction,
                          &map, CardinalDir(CardinalDir::NORTH));
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
      ...
      SystemFlags::OutputDebug(
          ...
          __FUNCTION__, __LINE__, unit->toString().c_str());
    ...
    if (placeUnit(pos, generationArea, unit, spaciated)) {
      ...
      if (game->getPaused() == true) {
        // new units added in pause mode might change the Fow. ( Scenarios do
        // this )
        computeFow();
        minimap.updateFowTex(1.f);
      }
    ...
    throw megaglest_runtime_error(
        "Invalid faction index in createUnitAtPosition: " +
...
void World::giveResource(const string &resourceName, int factionIndex,
                         int amount) {
  if (factionIndex < (int)factions.size()) {
    Faction *faction = factions[factionIndex];
    ...
    throw megaglest_runtime_error("Invalid faction index in giveResource: " +
                                      intToStr(factionIndex),
                                  true);
  }
}
...
vector<int> World::getUnitsForFaction(int factionIndex,
  ...
  if (factionIndex < 0 || factionIndex > getFactionCount()) {
    throw megaglest_runtime_error(
        "Invalid faction index in getUnitsForFaction: " +
            intToStr(factionIndex),
        true);
  }
  Faction *faction = getFaction(factionIndex);
  if (faction != NULL) {
    ...
    if (commandTypeName != "") {
      commandType =
          CommandTypeFactory::getInstance().newInstance(commandTypeName);
    }
    int unitCount = faction->getUnitCount();
        ...
        if (commandType != NULL) {
          if (commandType->getClass() == ccAttack && field >= 0) {
            const AttackCommandType *act =
                unit->getType()->getFirstAttackCommand(
                    static_cast<Field>(field));
            addToList = (act != NULL);
          } else if (commandType->getClass() == ccAttackStopped && field >= 0) {
            const AttackStoppedCommandType *asct =
                unit->getType()->getFirstAttackStoppedCommand(
                    static_cast<Field>(field));
            addToList = (asct != NULL);
          } else {
            addToList =
                unit->getType()->hasCommandClass(commandType->getClass());
          }
        } else if (field >= 0) {
...
void World::givePositionCommand(int unitId, const string &commandName,
                                const Vec2i &pos) {
  Unit *unit = findUnitById(unitId);
  if (unit != NULL) {
    CommandClass cc;
    ...
    if (commandName == "move") {
      ...
      throw megaglest_runtime_error("Invalid position command: " + commandName,
                                    true);
    }

    if (unit->getType()->getFirstCtOfClass(cc) == NULL) {
      throw megaglest_runtime_error(
          "Invalid command: [" + commandName + "] for unit: [" +
              unit->getType()->getName(false) + "] id [" +
              intToStr(unit->getId()) + "]",
          true);
    }
    if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
    ...
    throw megaglest_runtime_error(
        "Invalid unitId index in givePositionCommand: " + intToStr(unitId) +
  ...
  if (unit != NULL) {
    ...
    if (ct != NULL) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
              .enabled)
        SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                                 "In [%s::%s Line: %d] Unit [%s] will stop\n",
                                 __FILE__, __FUNCTION__, __LINE__,
                                 unit->getFullName(false).c_str());
      ...
      throw megaglest_runtime_error(
          "Invalid ct in giveStopCommand: " + intToStr(unitId), true);
    }
  } else {
    throw megaglest_runtime_error(
        "Invalid unitId index in giveStopCommand: " + intToStr(unitId), true);
  }
}
  ...
  if (unit != NULL) {
    ...
    if (targetUnit != NULL) {
      const CommandType *ct =
          unit->getType()->getFirstAttackCommand(targetUnit->getCurrField());
      if (ct != NULL) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
                .enabled)
          SystemFlags::OutputDebug(
              SystemFlags::debugUnitCommands,
              "In [%s::%s Line: %d] Unit [%s] is attacking [%s]\n", __FILE__,
              __FUNCTION__, __LINE__, unit->getFullName(false).c_str(),
              targetUnit->getFullName(false).c_str());
        ...
        throw megaglest_runtime_error(
            "Invalid ct in giveAttackCommand: " + intToStr(unitId) +
                " unitToAttackId = " + intToStr(unitToAttackId),
            true);
      }
    } else {
      throw megaglest_runtime_error(
          "Invalid unitToAttackId index in giveAttackCommand: " +
              intToStr(unitId) +
              " unitToAttackId = " + intToStr(unitToAttackId),
          true);
    }
    ...
    throw megaglest_runtime_error(
        "Invalid unitId index in giveAttackCommand: " + intToStr(unitId) +
            " unitToAttackId = " + intToStr(unitToAttackId),
        true);
  }
}
...
void World::giveProductionCommand(int unitId, const string &producedName) {
  ...
  if (unit != NULL) {
    ...
    for (int i = 0; i < ut->getCommandTypeCount(); ++i) {
      ...
      if (ct != NULL && ct->getClass() == ccProduce) {
        const ProduceCommandType *pct =
            dynamic_cast<const ProduceCommandType *>(ct);
    ...
    throw megaglest_runtime_error(
        "Invalid unitId index in giveProductionCommand: " + intToStr(unitId) +
...
void World::giveAttackStoppedCommand(int unitId, const string &itemName,
                                     bool ignoreRequirements) {
  Unit *unit = findUnitById(unitId);
  if (unit != NULL) {
    const UnitType *ut = unit->getType();
    ...
    for (int i = 0; i < ut->getCommandTypeCount(); ++i) {
      ...
      if (ct != NULL && ct->getClass() == ccAttackStopped) {
        ...
        if (act != NULL && act->getName(false) == itemName) {
          ...
          try {
            if (unit->getFaction()->reqsOk(act) == false &&
                ignoreRequirements == true) {
    ...
    throw megaglest_runtime_error(
        "Invalid unitId index in giveAttackStoppedCommand: " +
            intToStr(unitId) + " itemName = " + itemName,
        true);
  }
}
...
void World::playStaticSound(const string &playSound) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugLUA).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugLUA,
                             ...
                             __FUNCTION__, __LINE__, playSound.c_str());
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("\n\nPlay static sound changed: %d [%s] [%s]\n\n", changed,
           playSound.c_str(), calculatedFilePath.c_str());
  ...
  if (unit != NULL) {
    ...
    if (destUnit != NULL) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
        ...
        SystemFlags::OutputDebug(
            SystemFlags::debugUnitCommands,
            "In [%s::%s Line: %d] cc = %d Unit [%s] destUnit [%s]\n", __FILE__,
            __FUNCTION__, __LINE__, cc, unit->getFullName(false).c_str(),
            destUnit->getFullName(false).c_str());
      unit->giveCommand(
          new Command(unit->getType()->getFirstCtOfClass(cc), destUnit));
      if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
              .enabled)
        SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                                 "In [%s::%s Line: %d]\n", __FILE__,
    ...
    throw megaglest_runtime_error(
        "Invalid unitId index in followUnit: " + intToStr(unitId), true);
  }
}
...
void World::togglePauseGame(bool pauseStatus, bool forceAllowPauseStateChange) {
  ...
  // ensures that the Fow is really up to date when the game switches to pause
  // mode. ( mainly for scenarios )
  computeFow();
  minimap.updateFowTex(1.f);
}
...
const string World::getSystemMacroValue(const string key) {
  ...
  if (tagApplied == false) {
    ...
    if (key == "$SCENARIO_PATH") {
      result = extractDirectoryPathFromFile(
          game->getGameSettings()->getScenarioDir());
    }
  }
  ...
  if (unit != NULL) {
    ...
    for (int i = 0; i < ut->getCommandTypeCount(); ++i) {
      ...
      if (ct != NULL && ct->getClass() == ccUpgrade) {
        ...
        if (uct != NULL &&
            uct->getProducedUpgrade()->getName() == upgradeName) {
          if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
                  .enabled)
            SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                                     "In [%s::%s Line: %d]\n", __FILE__,
    ...
    throw megaglest_runtime_error(
        "Invalid unitId index in giveUpgradeCommand: " + intToStr(unitId) +
...
int World::getResourceAmount(const string &resourceName, int factionIndex) {
  if (factionIndex < (int)factions.size()) {
    ...
    throw megaglest_runtime_error(
        "Invalid faction index in giveResource: " + intToStr(factionIndex) +
...
Vec2i World::getStartLocation(int factionIndex) {
  if (factionIndex < (int)factions.size()) {
    ...
    throw megaglest_runtime_error(
        "Invalid faction index in getStartLocation: " + intToStr(factionIndex) +
            " : " + intToStr(factions.size()),
        true);
  }
}
...
Vec2i World::getUnitPosition(int unitId) {
  ...
  if (unit == NULL) {
    throw megaglest_runtime_error(
        "Can not find unit to get position unitId = " + intToStr(unitId), true);
  }
  return unit->getPos();
}
...
void World::setUnitPosition(int unitId, Vec2i pos) {
  ...
  if (unit == NULL) {
    throw megaglest_runtime_error(
        "Can not find unit to set position unitId = " + intToStr(unitId), true);
  }
  unit->setTargetPos(pos);
  this->moveUnitCells(unit, false);
}

void World::addCellMarker(Vec2i pos, int factionIndex, const string &note,
  ...
  // printf("pos [%s] scPos [%s][%p] targetPos
  // [%s]\n",pos.getString().c_str(),surfaceCellPos.getString().c_str(),sc,targetPos.getString().c_str());

  MarkedCell mc(targetPos, faction, note,
                (faction != NULL ? faction->getStartLocationIndex() : -1));
...
void World::showMarker(Vec2i pos, int factionIndex, const string &note,
...
void World::highlightUnit(int unitId, float radius, float thickness,
                          Vec4f color) {
  Unit *unit = findUnitById(unitId);
  if (unit == NULL) {
    throw megaglest_runtime_error(
        "Can not find unit to set highlight unitId = " + intToStr(unitId),
        true);
  }
...
int World::getUnitFactionIndex(int unitId) {
  ...
  if (unit == NULL) {
    throw megaglest_runtime_error(
        "Can not find Faction unit to get position unitId = " +
            intToStr(unitId),
        true);
  }
...
int World::getUnitCount(int factionIndex) {
  if (factionIndex < (int)factions.size()) {
    ...
    throw megaglest_runtime_error("Invalid faction index in getUnitCount: " +
                                      intToStr(factionIndex),
                                  true);
  }
}
...
int World::getUnitCountOfType(int factionIndex, const string &typeName) {
  if (factionIndex < (int)factions.size()) {
    ...
    for (int i = 0; i < faction->getUnitCount(); ++i) {
      const Unit *unit = faction->getUnit(i);
      if (unit != NULL && unit->isAlive() &&
          unit->getType()->getName(false) == typeName) {
        ++count;
      }
    }
    return count;
    ...
    throw megaglest_runtime_error(
        "Invalid faction index in getUnitCountOfType: " +
            intToStr(factionIndex),
        true);
  }
}
...
void World::initCells(bool fogOfWar) {
  ...
  Logger::getInstance().add(
      Lang::getInstance().getString("LogScreenGameLoadingStateCells", "", true),
      true);
  for (int i = 0; i < map.getSurfaceW(); ++i) {
    for (int j = 0; j < map.getSurfaceH(); ++j) {
      ...
      sc->setFowTexCoord(Vec2f(i / (next2Power(map.getSurfaceW()) - 1.f),
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
              .enabled == true) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, "In initCells() x = %d y = %d %s %s", i, j,
                 sc->isVisibleString().c_str(), sc->isExploredString().c_str());
...
void World::initSplattedTextures() {
  ...
  for (int i = 0; i < map.getSurfaceW() - 1; ++i) {
    for (int j = 0; j < map.getSurfaceH() - 1; ++j) {
      ...
      tileset.addSurfTex(sc00->getSurfaceType(), sc10->getSurfaceType(),
                         sc01->getSurfaceType(), sc11->getSurfaceType(), coord,
                         texture, j, i);
...
void World::initFactionTypes(GameSettings *gs) {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n",
                             __FILE__, __FUNCTION__, __LINE__);
  Logger::getInstance().add(Lang::getInstance().getString(
                                "LogScreenGameLoadingFactionTypes", "", true),
                            true);
  ...
  if (gs->getFactionCount() > map.getMaxPlayers()) {
    throw megaglest_runtime_error(
        "This map only supports " + intToStr(map.getMaxPlayers()) +
        " players, factionCount is " + intToStr(gs->getFactionCount()));
  }
  ...
  stats.init(gs->getFactionCount(), this->thisFactionIndex,
             gs->getDescription(), gs->getTech());
  ...
  for (int i = 0; i < (int)factions.size(); ++i) {
    ...
    factions[i]->init(ft, gs->getFactionControl(i), techTree, game, i,
                      gs->getTeam(i), gs->getStartLocationIndex(i),
    ...
    stats.setResourceMultiplier(i,
                                (gs->getResourceMultiplierIndex(i) + 5) * 0.1f);
    stats.setPlayerName(i, gs->getNetworkPlayerName(i));
    if (getFaction(i)->getTexture()) {
      stats.setPlayerColor(
          i, getFaction(i)->getTexture()->getPixmapConst()->getPixel3f(0, 0));
    }
  }
  ...
  if (loadWorldNode != NULL) {
    ...
    thisFactionIndex =
        loadWorldNode->getAttribute("thisFactionIndex")->getIntValue();
    //	int thisTeamIndex;
    ...
    MutexSafeWrapper safeMutex(mutexFactionNextUnitId,
                               string(__FILE__) + "_" + intToStr(__LINE__));
    //	std::map<int,int> mapFactionNextUnitId;
    //		for(std::map<int,int>::iterator iterMap =
    // mapFactionNextUnitId.begin(); 				iterMap !=
    // mapFactionNextUnitId.end();
    //++iterMap) { 			XmlNode *factionNextUnitIdNode =
    // worldNode->addChild("FactionNextUnitId");
    //
    //			factionNextUnitIdNode->addAttribute("key",intToStr(iterMap->first),
    // mapTagReplacements);
    //			factionNextUnitIdNode->addAttribute("value",intToStr(iterMap->second),
    // mapTagReplacements);
    //		}
    //!!!
    vector<XmlNode *> factionNextUnitIdNodeList =
        loadWorldNode->getChildList("FactionNextUnitId");
    ...
    fogOfWarOverride =
        loadWorldNode->getAttribute("fogOfWarOverride")->getIntValue() != 0;
    //	bool fogOfWar;
    fogOfWar = loadWorldNode->getAttribute("fogOfWar")->getIntValue() != 0;
    //	int fogOfWarSmoothingFrameSkip;
    fogOfWarSmoothingFrameSkip =
        loadWorldNode->getAttribute("fogOfWarSmoothingFrameSkip")
            ->getIntValue();
    //	bool fogOfWarSmoothing;
    fogOfWarSmoothing =
        loadWorldNode->getAttribute("fogOfWarSmoothing")->getIntValue() != 0;
    //	Game *game;
    //	Chrono chronoPerfTimer;
    //	bool perfTimerEnabled;
    //
    //	bool unitParticlesEnabled;
    unitParticlesEnabled =
        loadWorldNode->getAttribute("unitParticlesEnabled")->getIntValue() != 0;
    //	std::map<string,StaticSound *> staticSoundList;
    //	std::map<string,StrSound *> streamSoundList;
    //
    //	uint32 nextCommandGroupId;
    nextCommandGroupId =
        loadWorldNode->getAttribute("nextCommandGroupId")->getIntValue();
    //	string queuedScenarioName;
    queuedScenarioName =
        loadWorldNode->getAttribute("queuedScenarioName")->getValue();
    //	bool queuedScenarioKeepFactions;
    queuedScenarioKeepFactions =
    ...
    if (loadWorldNode->hasAttribute("disableAttackEffects") == true) {
      disableAttackEffects =
          loadWorldNode->getAttribute("disableAttackEffects")->getIntValue() !=
          0;
    }
  }
...
void World::initMinimap() {
  ...
  minimap.init(map.getW(), map.getH(), this,
               game->getGameSettings()->getFogOfWar());
  Logger::getInstance().add(Lang::getInstance().getString(
                                "LogScreenGameLoadingMinimapSurface", "", true),
                            true);
...
void World::initUnitsForScenario() {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n",
                             __FILE__, __FUNCTION__, __LINE__);
  Logger::getInstance().add(
      Lang::getInstance().getString("LogScreenGameLoadingGenerateGameElements",
                                    "", true),
      true);
  ...
  for (int i = 0; i < getFactionCount(); ++i) {
    ...
    for (int j = 0; j < f->getUnitCount(); ++j) {
      ...
      if (placeUnit(map.getStartLocation(startLocationIndex), generationArea,
                    unit, true)) {
        ...
        throw megaglest_runtime_error(
            "Unit: " + unitName +
            " can't be placed, this error is caused because there\nis not "
            "enough room to put all units near their start location.\nmake a "
            "better/larger map. Faction: #" +
            intToStr(i) + " name: " + ft->getName(false));
      }
...
void World::placeUnitAtLocation(const Vec2i &location, int radius, Unit *unit,
                                bool spaciated) {
  if (placeUnit(location, generationArea, unit, spaciated)) {
    ...
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "Unit: [%s] can't be placed, this error is caused because "
             "there\nis not enough room to put all units near their start "
      ...
      for (int i = 0; i < getFactionCount(); ++i) {
        ...
        for (int j = 0; j < ft->getStartingUnitCount(); ++j) {
          ...
          for (int l = 0; l < initNumber; l++) {
            ...
            Unit *unit = new Unit(getNextUnitId(f), newpath, Vec2i(0), ut, f,
                                  &map, CardinalDir(CardinalDir::NORTH));
            int startLocationIndex = f->getStartLocationIndex();
            placeUnitAtLocation(map.getStartLocation(startLocationIndex),
                                generationArea, unit, true);
          }
        }
      }
      // the following is done here in an extra loop and not in the loop above,
      // because shared resources games need to load all factions first, before
      // calculating limitResourcesToStore()
      for (int i = 0; i < getFactionCount(); ++i) {
        Faction *f = factions[i];
        // Ensure Starting Resource Amount are adjusted to max store levels
        f->limitResourcesToStore();
...
void World::exploreCells(int teamIndex,
...
ExploredCellsLookupItem World::exploreCells(const Vec2i &newPos, int sightRange,
  ...
  if (MaxExploredCellsLookupItemCache > 0) {
    if (difftime(time(NULL), ExploredCellsLookupItem::lastDebug) >= 10) {
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(
            SystemFlags::debugSystem,
            "In [%s::%s Line: %d] ExploredCellsLookupItemCache.size() = %d\n",
            __FILE__, __FUNCTION__, __LINE__,
            ExploredCellsLookupItemCache.size());
    ...
    if ((int)ExploredCellsLookupItemCache.size() >=
        MaxExploredCellsLookupItemCache) {
      MaxExploredCellsLookupItemCacheTriggered = true;
      // Snag the oldest entry in the list
      std::map<int, ExploredCellsLookupKey>::reverse_iterator purgeItem =
          ExploredCellsLookupItemCacheTimer.rbegin();

      ExploredCellsLookupItemCache[purgeItem->second.pos].erase(
          purgeItem->second.sightRange);
    ...
    std::map<Vec2i, std::map<int, ExploredCellsLookupItem>>::iterator iterFind =
        ExploredCellsLookupItemCache.find(newPos);
    if (iterFind != ExploredCellsLookupItemCache.end()) {

      std::map<int, ExploredCellsLookupItem>::iterator iterFind2 =
          iterFind->second.find(sightRange);
      if (iterFind2 != iterFind->second.end()) {
        ...
        if (MaxExploredCellsLookupItemCacheTriggered == true) {
          ...
          ExploredCellsLookupItemCacheTimer.erase(
              exploredCellsCache.ExploredCellsLookupItemCacheTimerCountIndex);
          exploredCellsCache.ExploredCellsLookupItemCacheTimerCountIndex =
              ExploredCellsLookupItemCacheTimerCount++;
          ...
          ExploredCellsLookupItemCacheTimer
              [exploredCellsCache.ExploredCellsLookupItemCacheTimerCountIndex] =
                  lookupKey;
        }
  ...
  ExploredCellsLookupItem exploredCellsCache;
  exploredCellsCache.exploredCellList.reserve(surfSightRange +
                                              indirectSightRange * 4);
  exploredCellsCache.visibleCellList.reserve(surfSightRange +
                                             indirectSightRange * 4);

  // int loopCount = 0;
  for (int i = -surfSightRange - indirectSightRange - 1;
       i <= surfSightRange + indirectSightRange + 1; ++i) {
    for (int j = -surfSightRange - indirectSightRange - 1;
         j <= surfSightRange + indirectSightRange + 1; ++j) {
      // loopCount++;
      ...
      if (map.isInsideSurface(currPos) == true) {
        ...
        bool updateExplored =
            (posLength < surfSightRange + indirectSightRange + 1);
        bool updateVisible = (posLength < surfSightRange);

        if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
                    .enabled == true &&
            SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynchMax)
                    .enabled == true) {
          char szBuf[8096] = "";
          snprintf(szBuf, 8096,
                   "In exploreCells() currRelPos = %s currPos = %s posLength = "
                   "%f updateExplored = %d updateVisible = %d sightRange = %d "
                   "teamIndex = %d",
                   currRelPos.getString().c_str(), currPos.getString().c_str(),
                   posLength, updateExplored, updateVisible, sightRange,
                   teamIndex);
          if (Thread::isCurrentThreadMainThread() == false) {
            unit->logSynchDataThreaded(__FILE__, __LINE__, szBuf);
          } else {
  ...
  if (MaxExploredCellsLookupItemCache > 0) {
    if (exploredCellsCache.exploredCellList.empty() == false ||
        exploredCellsCache.visibleCellList.empty() == false) {
      exploredCellsCache.ExploredCellsLookupItemCacheTimerCountIndex =
          ExploredCellsLookupItemCacheTimerCount++;
...
bool World::showWorldForPlayer(int factionIndex,
                               bool excludeFogOfWarCheck) const {
  bool ret = false;
  if (factionIndex >= 0) {
    if (excludeFogOfWarCheck == false && fogOfWarSkillTypeValue == 0 &&
        fogOfWarOverride == true) {
      ret = true;
    } else if (factionIndex == thisFactionIndex && game != NULL) {

      // printf("Show FOW thisTeamIndex = %d (%d) game->getGameOver() = %d
      // game->getGameSettings()->isNetworkGame() = %d
      // game->getGameSettings()->getEnableObserverModeAtEndGame() = %d
      // thisFactionIndex = %d Is Winning Faction = %d\n",
      //		thisTeamIndex,(GameConstants::maxPlayers -1 +
      // fpt_Observer),game->getGameOver(),
      ...
      // turn off fog of war?
               ...
               game->getGameSettings()->getEnableObserverModeAtEndGame() ==
                   true) {
        ret = true;
        ...
        if (getStats()->getVictory(factionIndex) == false) {
          ...
          //                for(int i = 0; i < faction->getUnitCount(); ++i) {
          //                    Unit *unit =
          //                    getFaction(factionIndex)->getUnit(i); if(unit !=
          //                    NULL && unit->isAlive() &&
          //                    unit->getType()->isMobile() == true) {
          //                        ret = false;
          //                        break;
          //                    }
          //                }
...
void World::computeFow() {
  ...
  if (this->game)
    this->game->addPerformanceCount("world minimap.resetFowTex",
                                    chronoGamePerformanceCounts.getMillis());
  ...
  for (int factionIndex = 0; factionIndex < GameConstants::maxPlayers +
    ...
    //			indexTeamFaction < GameConstants::maxPlayers +
    // GameConstants::specialFactions;
    //			++indexTeamFaction) {

    // If fog of war enabled set cell visible to false and later set those close
    // to units to true
    if (fogOfWar) {
      for (int indexSurfaceW = 0; indexSurfaceW < map.getSurfaceW();
           ++indexSurfaceW) {
        for (int indexSurfaceH = 0; indexSurfaceH < map.getSurfaceH();
             ++indexSurfaceH) {
          // set all cells to not visible
          map.getSurfaceCell(indexSurfaceW, indexSurfaceH)
              ->setVisible(faction->getTeam(), false);
        }
      }
    }
    ...
    if (!fogOfWar || (faction->getTeam() != thisTeamIndex)) {
      ...
      // printf("showWorldForFaction indexFaction = %d thisTeamIndex = %d
      // showWorldForFaction =
      ...
      for (int indexSurfaceW = 0; indexSurfaceW < map.getSurfaceW();
        ...
        for (int indexSurfaceH = 0; indexSurfaceH < map.getSurfaceH();
          ...
          if (!fogOfWar ||
              (cacheFowAlphaTexture == false && showWorldForFaction == true &&
    ...
    // Remove fog of war for factions on my team
      ...
      bool showWorldForFaction = showWorldForPlayer(factionIndex);
      // printf("#2 showWorldForFaction thisFactionIndex = %d thisTeamIndex = %d
      // showWorldForFaction =
  ...
  // Once we have calculated fog of war texture alpha, will we cache it so that
  // we can restore it later
  if (fogOfWar && cacheFowAlphaTexture == false &&
      resetFowAlphaFactionCount > 0) {
  ...
  if (this->game)
    this->game->addPerformanceCount("world reset cells",
                                    chronoGamePerformanceCounts.getMillis());
  ...
  for (int factionIndex = 0; factionIndex < getFactionCount(); ++factionIndex) {
    ...
    // printf("computeFow thisFactionIndex = %d factionIndex = %d thisTeamIndex
    // = %d faction->getTeam() = %d cellVisibleForFaction =
    ...
    for (int unitIndex = 0; unitIndex < unitCount; ++unitIndex) {
      ...
      if (fogOfWar == true && faction->getTeam() == thisTeamIndex &&
          unit->isOperative() == true) {

        // printf("computeFow unit->isOperative() == true\n");

        const FowAlphaCellsLookupItem &cellList = unit->getCachedFow();
        for (std::map<Vec2i, float>::const_iterator iterMap =
                 cellList.surfPosAlphaList.begin();
  ...
  if (this->game)
    this->game->addPerformanceCount("world compute cells",
                                    chronoGamePerformanceCounts.getMillis());
}
...
int World::getNextUnitId(Faction *faction) {
  MutexSafeWrapper safeMutex(mutexFactionNextUnitId,
                             string(__FILE__) + "_" + intToStr(__LINE__));
  if (mapFactionNextUnitId.find(faction->getIndex()) ==
      mapFactionNextUnitId.end()) {
...
int World::getNextCommandGroupId() { return ++nextCommandGroupId; }
...
void World::playStaticVideo(const string &playVideo) {
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("\n\nPlay static video changed: %d [%s] [%s]\n\n", changed,
           playVideo.c_str(), calculatedFilePath.c_str());
...
const Resource *World::getResourceForTeam(const ResourceType *rt,
                                          int teamIndex) {
  if (rt == NULL) {
    return NULL;
  }
  ...
  for (int index = 0; index < (int)factions.size(); ++index) {
    ...
    if (faction != NULL && faction->getTeam() == teamIndex) {
      ...
      if (factionResource != NULL && factionResource->getType() != NULL) {
        ...
        teamResource.setAmount(teamResourceAmount +
                               factionResource->getAmount());
        teamResource.setBalance(teamResourceBalance +
...
bool World::showResourceTypeForTeam(const ResourceType *rt,
                                    int teamIndex) const {
  // if any unit produces the resource
  bool showResource = false;
  for (int index = 0; showResource == false && index < (int)factions.size();
       ++index) {
...
bool World::showResourceTypeForFaction(const ResourceType *rt,
...
string World::getExploredCellsLookupItemCacheStats() {
  ...
  for (std::map<Vec2i, std::map<int, ExploredCellsLookupItem>>::iterator
           iterMap1 = ExploredCellsLookupItemCache.begin();
       iterMap1 != ExploredCellsLookupItemCache.end(); ++iterMap1) {
    posCount++;

    for (std::map<int, ExploredCellsLookupItem>::iterator iterMap2 =
             iterMap1->second.begin();
  ...
  char szBuf[8096] = "";
  snprintf(szBuf, 8096, "pos [%d] sight [%d] [%d][%d] total KB: %s", posCount,
           sightCount, exploredCellCount, visibleCellCount,
...
string World::getFowAlphaCellsLookupItemCacheStats() {
  ...
  char szBuf[8096] = "";
  snprintf(szBuf, 8096, "surface count [%d] total KB: %s", surfPosAlphaCount,
           formatNumber(totalBytes).c_str());
  result = szBuf;
  return result;
}
...
string World::getAllFactionsCacheStats() {
  ...
  for (int i = 0; i < getFactionCount(); ++i) {
    ...
    totalBytes += getFaction(i)->getCacheKBytes(
        &cache1Size, &cache2Size, &cache3Size, &cache4Size, &cache5Size);
  ...
  snprintf(
      szBuf, 8096,
      "Cache1 [%llu] Cache2 [%llu] Cache3 [%llu] Cache4 [%llu] Cache5 [%llu] "
      "total KB: %s",
      (long long unsigned)totalCache1Size, (long long unsigned)totalCache2Size,
      (long long unsigned)totalCache3Size, (long long unsigned)totalCache4Size,
      (long long unsigned)totalCache5Size, formatNumber(totalBytes).c_str());
  result = szBuf;
  return result;
}
...
std::string World::DumpWorldToLog(bool consoleBasicInfoOnly) const {
  ...
  string debugWorldLogFile =
      Config::getInstance().getString("DebugWorldLogFile", "debugWorld.log");
  ...
  if (consoleBasicInfoOnly == true) {
    ...
    for (int i = 0; i < getFactionCount(); ++i) {
      ...
      std::cout << "FactionName = " << getFaction(i)->getType()->getName(false)
                << std::endl;
      std::cout << "FactionIndex = " << intToStr(getFaction(i)->getIndex())
                << std::endl;
      std::cout << "teamIndex = " << intToStr(getFaction(i)->getTeam())
                << std::endl;
      std::cout << "startLocationIndex = "
                << intToStr(getFaction(i)->getStartLocationIndex())
                << std::endl;
      std::cout << "thisFaction = " << intToStr(getFaction(i)->getThisFaction())
                << std::endl;
      std::cout << "control = " << intToStr(getFaction(i)->getControlType())
                << std::endl;
    }
  } else {
    ...
    for (int i = 0; i < getFactionCount(); ++i) {
      logFile << "Faction: " << getFaction(i)->getType()->getName(false)
              << std::endl;
      int unitCount = getFaction(i)->getUnitCount();
      for (int j = unitCount - 1; j >= 0; j--) {
        Unit *unit = getFaction(i)->getUnit(j);
        if (unit->getToBeUndertaken()) {
          logFile << "Undertake unit index = " << j << unit->getFullName(false)
                  << std::endl;
        }
      }
    }
...
void World::saveGame(XmlNode *rootNode) {
  ...
  worldNode->addAttribute("techTree",
                          (techTree != NULL ? techTree->getName() : ""),
                          mapTagReplacements);
  //	TimeFlow timeFlow;
  ...
  //	RandomGen random;
  worldNode->addAttribute("random", intToStr(random.getLastNumber()),
                          mapTagReplacements);
  //	ScriptManager* scriptManager;
  //
  //	int thisFactionIndex;
  worldNode->addAttribute("thisFactionIndex", intToStr(thisFactionIndex),
                          mapTagReplacements);
  //	int thisTeamIndex;
  worldNode->addAttribute("thisTeamIndex", intToStr(thisTeamIndex),
                          mapTagReplacements);
  //	int frameCount;
  worldNode->addAttribute("frameCount", intToStr(frameCount),
                          mapTagReplacements);
  //	//int nextUnitId;
  ...
  //	//config
  //	bool fogOfWarOverride;
  worldNode->addAttribute("fogOfWarOverride", intToStr(fogOfWarOverride),
                          mapTagReplacements);
  //	bool fogOfWar;
  worldNode->addAttribute("fogOfWar", intToStr(fogOfWar), mapTagReplacements);
  //	int fogOfWarSmoothingFrameSkip;
  worldNode->addAttribute("fogOfWarSmoothingFrameSkip",
                          intToStr(fogOfWarSmoothingFrameSkip),
                          mapTagReplacements);
  //	bool fogOfWarSmoothing;
  worldNode->addAttribute("fogOfWarSmoothing", intToStr(fogOfWarSmoothing),
                          mapTagReplacements);
  //	Game *game;
  //	Chrono chronoPerfTimer;
  //	bool perfTimerEnabled;
  //
  //	bool unitParticlesEnabled;
  worldNode->addAttribute("unitParticlesEnabled",
                          intToStr(unitParticlesEnabled), mapTagReplacements);
  //	std::map<string,StaticSound *> staticSoundList;
  //	std::map<string,StrSound *> streamSoundList;
  //
  //	uint32 nextCommandGroupId;
  worldNode->addAttribute("nextCommandGroupId", intToStr(nextCommandGroupId),
                          mapTagReplacements);
  //	string queuedScenarioName;
  worldNode->addAttribute("queuedScenarioName", queuedScenarioName,
                          mapTagReplacements);
  //	bool queuedScenarioKeepFactions;
  worldNode->addAttribute("queuedScenarioKeepFactions",
                          intToStr(queuedScenarioKeepFactions),
                          mapTagReplacements);

  worldNode->addAttribute("disableAttackEffects",
                          intToStr(disableAttackEffects), mapTagReplacements);
}

void World::loadGame(const XmlNode *rootNode) { loadWorldNode = rootNode; }

...
void World::cleanup() {
    ...
    for (std::map<string, ::Shared::Sound::StaticSound *>::iterator iterMap = staticSoundList.begin(); iterMap != staticSoundList.end(); ++iterMap) {
        delete iterMap->second;
    }
    staticSoundList.clear();

    for (std::map<string, ::Shared::Sound::StrSound *>::iterator iterMap = streamSoundList.begin(); iterMap != streamSoundList.end(); ++iterMap) {
        delete iterMap->second;
    }
...
void World::addFogOfWarSkillType(const Unit *unit, const FogOfWarSkillType *fowst) {
    ...
    if ((fowst->getApplyToTeam() == true && unit->getTeam() == this->getThisTeamIndex()) ||
        ...
        if ((fowst->getFowEnable() == false && fogOfWarSkillTypeValue != 0) || (fowst->getFowEnable() == true && fogOfWarSkillTypeValue != 1)) {
...
void World::removeFogOfWarSkillType(const Unit *unit) {
    ...
    if (removedFromList == true) {
        if (mapFogOfWarUnitList.empty() == true) {
            ...
            if ((fowEnabled == false && fogOfWarSkillTypeValue != 0) || (fowEnabled == true && fogOfWarSkillTypeValue != 1)) {
                // printf("In [%s::%s Line: %d] current = %d new =
...
Checksum World::loadTech(const vector<string> pathList, const string &techName, set<string> &factions, Checksum *checksum,
                         std::map<string, vector<pair<string, string>>> &loadedFileList, bool validationMode) {
    ...
    techtreeChecksum = techTree->loadTech(techName, factions, checksum, loadedFileList, validationMode);
...
void World::updateAllFactionUnits() {
    ...
    if (newThreadManager == true) {
        ...
        if (SystemFlags::VERBOSE_MODE_ENABLED && chrono.getMillis() >= 10)
            printf("In [%s::%s Line: %d] *** Faction thread preprocessing took [%lld] "
    ...
    for (int i = 0; i < factionCount; ++i) {
        ...
        for (int j = 0; j < unitCount; ++j) {
            ...
            if (showPerfStats && chronoPerfUnit.getMillis() >= 10) {
                ...
                sprintf(perfBuf,
                        "In [%s::%s] Line: %d took msecs: " MG_I64_SPECIFIER " stuck: %d for unit:\n%sBEFORE unitBlockCount = %d, AFTER = "
                        "%d, BEFORE , AFTER: %u\n",
                        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, chronoPerfUnit.getMillis(), isStuck, unit->toString().c_str(),
                        unitBlockCount, unit->getPath()->getBlockCount(), unit->getLastStuckFrame());
                perfList.push_back(perfBuf);
            }
        }
        ...
        if (showPerfStats) {
            sprintf(perfBuf,
                    "In [%s::%s] Line: %d took msecs: " MG_I64_SPECIFIER " faction: %d / %d unitCount = %d unitCountUpdated = %d "
                    "unitCountStuck = %d\n",
                    extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, chronoPerf.getMillis(), i + 1, factionCount, unitCount,
                    unitCountUpdated, unitCountStuck);
            perfList.push_back(perfBuf);

            for (std::map<CommandClass, int>::iterator iterMap = mapCommandCount.begin(); iterMap != mapCommandCount.end(); ++iterMap) {
                sprintf(perfBuf, "Command class = %d, count = %d\n", iterMap->first, iterMap->second);
                perfList.push_back(perfBuf);
            }

            for (std::map<SkillClass, int>::iterator iterMap = mapSkillCount.begin(); iterMap != mapSkillCount.end(); ++iterMap) {
    ...
    if (showPerfStats) {
        sprintf(perfBuf, "In [%s::%s] Line: %d took msecs: " MG_I64_SPECIFIER " totalUnitsProcessed = %d\n", extractFileFromDirectoryPath(__FILE__).c_str(),
                __FUNCTION__, __LINE__, chronoPerf.getMillis(), totalUnitsProcessed);
        perfList.push_back(perfBuf);
    }
    ...
    if (SystemFlags::VERBOSE_MODE_ENABLED && chrono.getMillis() >= 20)
        printf("In [%s::%s Line: %d] *** Faction MAIN thread processing took [%lld] "
               "msecs for %d factions for frameCount = %d.\n",
               __FILE__, __FUNCTION__, __LINE__, (long long int)chrono.getMillis(), factionCount, frameCount);
}
...
void World::underTakeDeadFactionUnits() {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] factionCount = %d\n", __FILE__, __FUNCTION__, __LINE__, factionCount);
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n", extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__);
    ...
    for (int i = 0; i < factionCount; ++i) {
        ...
        int unitCount = faction->getUnitCount();
        if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] i = %d, unitCount = %d\n", __FILE__, __FUNCTION__, __LINE__, i,
                                     unitCount);
...
void World::tick() {
    ...
    for (int factionIndex = 0; factionIndex < factionCount; ++factionIndex) {
        ...
        // for each resource
        for (int resourceTypeIndex = 0; resourceTypeIndex < techTree->getResourceTypeCount(); ++resourceTypeIndex) {
            ...
            if (rt != NULL && rt->getClass() == rcConsumable) {
                ...
                for (int unitIndex = 0; unitIndex < faction->getUnitCount(); ++unitIndex) {
                    ...
                    if (unit != NULL && unit->isOperative()) {
                        ...
                        if (iterFind != resourceCostCache.end() && iterFind->second.find(rt) != iterFind->second.end()) {
...
const UnitType *World::findUnitTypeByName(const string factionName, const string unitTypeName) {
    ...
    for (int index = 0; unitTypeResult == NULL && index < getFactionCount(); ++index) {
        ...
        if (factionName == "" || factionName == faction->getType()->getName(false)) {
            for (int unitIndex = 0; unitTypeResult == NULL && unitIndex < faction->getType()->getUnitTypeCount(); ++unitIndex) {
    ...
    if (tileset.getWaterEffects() && unit->getCurrField() == fLand) {
        if (map.getSubmerged(map.getCell(unit->getLastPos()))) {
            if (Thread::isCurrentThreadMainThread() == false) {
                throw megaglest_runtime_error("#1 Invalid access to World random from outside main thread "
                                              "current id = " +
            ...
            for (int i = 0; i < 3; ++i) {
                waterEffects.addWaterSplash(Vec2f(unit->getLastPos().x + (float)unitSize / 2.0f + random.randRange(-0.9f, -0.1f),
                                                  unit->getLastPos().y + random.randRange(-0.9f, -0.1f) + (float)unitSize / 2.0f),
...
void World::addAttackEffects(const Unit *unit) {
    attackEffects.addWaterSplash(Vec2f(unit->getPosNotThreadSafe().x, unit->getPosNotThreadSafe().y), 1);
}
...
bool World::toRenderUnit(const Unit *unit) const {
    ...
    return (map.getSurfaceCell(Map::toSurfCoords(unit->getCenteredPos()))->isVisible(thisTeamIndex) &&
           ...
           (unit->getCurrSkill()->getClass() == scAttack && map.getSurfaceCell(Map::toSurfCoords(unit->getTargetPos()))->isVisible(thisTeamIndex) &&
...
bool World::toRenderUnit(const UnitBuildInfo &pendingUnit) const {
    ...
    return (map.getSurfaceCell(Map::toSurfCoords(pendingUnit.pos))->isVisible(thisTeamIndex) &&
    ...
    if (unit != NULL) {
        ...
        if (unit->getType()->getFirstCtOfClass(cc) == NULL) {
            throw megaglest_runtime_error(
                "Invalid command: [" + commandName + "] for unit: [" + unit->getType()->getName(false) + "] id [" + intToStr(unit->getId()) + "]", true);
        }
        if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
...
void World::giveStopCommand(int unitId) {
    ...
    if (unit != NULL) {
        ...
        if (ct != NULL) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] Unit [%s] will stop\n", __FILE__, __FUNCTION__, __LINE__,
                                         unit->getFullName(false).c_str());
...
void World::giveAttackCommand(int unitId, int unitToAttackId) {
    ...
    if (unit != NULL) {
        ...
        if (targetUnit != NULL) {
            ...
            if (ct != NULL) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] Unit [%s] is attacking [%s]\n", __FILE__, __FUNCTION__,
                                             __LINE__, unit->getFullName(false).c_str(), targetUnit->getFullName(false).c_str());
            ...
            throw megaglest_runtime_error(
                "Invalid unitToAttackId index in giveAttackCommand: " + intToStr(unitId) + " unitToAttackId = " + intToStr(unitToAttackId), true);
        }
    } else {
    ...
    if (unit != NULL) {
        ...
        for (int i = 0; i < ut->getCommandTypeCount(); ++i) {
            ...
            if (ct != NULL && ct->getClass() == ccProduce) {
                ...
                if (pct != NULL && pct->getProducedUnit() != NULL && pct->getProducedUnit()->getName(false) == producedName) {
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d]\n", __FILE__, __FUNCTION__, __LINE__);
        ...
        if (destUnit != NULL) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] cc = %d Unit [%s] destUnit [%s]\n", __FILE__, __FUNCTION__,
                                         __LINE__, cc, unit->getFullName(false).c_str(), destUnit->getFullName(false).c_str());
...
void World::initCells(bool fogOfWar) {
    ...
    for (int i = 0; i < map.getSurfaceW(); ++i) {
        for (int j = 0; j < map.getSurfaceH(); ++j) {
            ...
            sc->setFowTexCoord(Vec2f(i / (next2Power(map.getSurfaceW()) - 1.f), j / (next2Power(map.getSurfaceH()) - 1.f)));
...
void World::initSplattedTextures() {
    ...
    for (int i = 0; i < map.getSurfaceW() - 1; ++i) {
        for (int j = 0; j < map.getSurfaceH() - 1; ++j) {
            ...
            tileset.addSurfTex(sc00->getSurfaceType(), sc10->getSurfaceType(), sc01->getSurfaceType(), sc11->getSurfaceType(), coord, texture, j, i);
...
void World::initFactionTypes(GameSettings *gs) {
    ...
    if (gs->getFactionCount() > map.getMaxPlayers()) {
        throw megaglest_runtime_error("This map only supports " + intToStr(map.getMaxPlayers()) + " players, factionCount is " +
                                      intToStr(gs->getFactionCount()));
    }
    ...
    for (int i = 0; i < (int)factions.size(); ++i) {
        ...
        factions[i]->init(ft, gs->getFactionControl(i), techTree, game, i, gs->getTeam(i), gs->getStartLocationIndex(i), i == thisFactionIndex,
    ...
    for (int i = 0; i < getFactionCount(); ++i) {
        ...
        for (int j = 0; j < f->getUnitCount(); ++j) {
            ...
            if (placeUnit(map.getStartLocation(startLocationIndex), generationArea, unit, true)) {
                ...
                delete unit;
                unit = NULL;
                throw megaglest_runtime_error("Unit: " + unitName +
        ...
        if (difftime(time(NULL), ExploredCellsLookupItem::lastDebug) >= 10) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] ExploredCellsLookupItemCache.size() = %d\n", __FILE__, __FUNCTION__,
                                         __LINE__, ExploredCellsLookupItemCache.size());
            if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s Line: %d] ExploredCellsLookupItemCache.size() = %d\n", __FILE__,
                                         __FUNCTION__, __LINE__, ExploredCellsLookupItemCache.size());
        }
...
bool World::showWorldForPlayer(int factionIndex, bool excludeFogOfWarCheck) const {
    ...
    if (factionIndex >= 0) {
        if (excludeFogOfWarCheck == false && fogOfWarSkillTypeValue == 0 && fogOfWarOverride == true) {
            ...
            else if (game->getGameOver() == true && game->getGameSettings()->isNetworkGame() == false) {
                ret = true;
            }
            // Game is over but playing a Network game so check if we can
            // turn off fog of war?
            else if (game->getGameOver() == true && game->getGameSettings()->isNetworkGame() == true &&
...
void World::computeFow() {
        ...
        if (!fogOfWar || (faction->getTeam() != thisTeamIndex)) {
            ...
            for (int indexSurfaceW = 0; indexSurfaceW < map.getSurfaceW(); ++indexSurfaceW) {
                for (int indexSurfaceH = 0; indexSurfaceH < map.getSurfaceH(); ++indexSurfaceH) {
                    ...
                    if (!fogOfWar || (cacheFowAlphaTexture == false && showWorldForFaction == true && resetFowAlphaFactionCount <= 1)) {
        ...
        // Remove fog of war for factions on my team
            ...
            if (showWorldForFaction == true) {
                for (int indexSurfaceW = 0; indexSurfaceW < map.getSurfaceW(); ++indexSurfaceW) {
                    for (int indexSurfaceH = 0; indexSurfaceH < map.getSurfaceH(); ++indexSurfaceH) {
                        // reset fog of ware texture alpha values
                        if (!fogOfWar || (cacheFowAlphaTexture == false && showWorldForFaction == true)) {
                            const Vec2i surfPos(indexSurfaceW, indexSurfaceH);
    ...
    for (int factionIndex = 0; factionIndex < getFactionCount(); ++factionIndex) {
        ...
        for (int unitIndex = 0; unitIndex < unitCount; ++unitIndex) {
            ...
            if (fogOfWar == true && faction->getTeam() == thisTeamIndex && unit->isOperative() == true) {
                // printf("computeFow unit->isOperative() == true\n");

                const FowAlphaCellsLookupItem &cellList = unit->getCachedFow();
                for (std::map<Vec2i, float>::const_iterator iterMap = cellList.surfPosAlphaList.begin(); iterMap != cellList.surfPosAlphaList.end();
                     ++iterMap) {
...
bool World::showResourceTypeForFaction(const ResourceType *rt, const Faction *faction) const {
    // if any unit produces the resource
    bool showResource = false;
    for (int factionUnitTypeIndex = 0; factionUnitTypeIndex < faction->getType()->getUnitTypeCount(); ++factionUnitTypeIndex) {
...
string World::getExploredCellsLookupItemCacheStats() {
    ...
    for (std::map<Vec2i, std::map<int, ExploredCellsLookupItem>>::iterator iterMap1 = ExploredCellsLookupItemCache.begin();
        ...
        for (std::map<int, ExploredCellsLookupItem>::iterator iterMap2 = iterMap1->second.begin(); iterMap2 != iterMap1->second.end(); ++iterMap2) {
            sightCount++;
    ...
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "pos [%d] sight [%d] [%d][%d] total KB: %s", posCount, sightCount, exploredCellCount, visibleCellCount,
...
string World::getAllFactionsCacheStats() {
    ...
    snprintf(szBuf, 8096,
             ...
             "total KB: %s",
             (long long unsigned)totalCache1Size, (long long unsigned)totalCache2Size, (long long unsigned)totalCache3Size, (long long unsigned)totalCache4Size,
...
void World::saveGame(XmlNode *rootNode) {
    ...
    MutexSafeWrapper safeMutex(mutexFactionNextUnitId, string(__FILE__) + "_" + intToStr(__LINE__));
    //	std::map<int,int> mapFactionNextUnitId;
    for (std::map<int, int>::iterator iterMap = mapFactionNextUnitId.begin(); iterMap != mapFactionNextUnitId.end(); ++iterMap) {

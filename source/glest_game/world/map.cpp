...
bool Map::isResourceNear(int frameIndex, const Vec2i &pos, const ResourceType *rt, Vec2i &resourcePos, int size, Unit *unit,
                         bool fallbackToPeersHarvestingSameResource, Vec2i *resourceClickPos) const {
            ...
            if (isInside(resPos) && isInsideSurface(surfCoords)) {
                ...
                if (r != NULL) {
                    if (r->getType() == rt) {
                        ...
                        if (resourceClickPos == NULL || (distanceFromClick < 0 || resourceClickPos->dist(resPos) <= distanceFromClick)) {
                            if (unit == NULL || (distanceFromUnit < 0 || unit->getCenteredPos().dist(resPos) <= distanceFromUnit)) {
                ...
                if (isInside(resPos) && isInsideSurface(surfCoords)) {
                    ...
                    if (r != NULL) {
                        if (r->getType() == rt) {
                            ...
                            if (unit == NULL || (distanceFromUnit < 0 || unit->getCenteredPos().dist(resPos) <= (distanceFromUnit + 2.0))) {
...
bool Map::isFreeCell(const Vec2i &pos, Field field, bool buildingsOnly) const {
    return isInside(pos) && isInsideSurface(toSurfCoords(pos)) &&
           ...
           (field == fAir || getSurfaceCell(toSurfCoords(pos))->isFree()) && (field != fLand || getDeepSubmerged(getCell(pos)) == false);
}
...
bool Map::canMove(const Unit *unit, const Vec2i &pos1, const Vec2i &pos2,
                  std::map<Vec2i, std::map<Vec2i, std::map<int, std::map<Field, bool>>>> *lookupCache) const {
...
// pathfinding)
bool Map::aproxCanMove(const Unit *unit, const Vec2i &pos1, const Vec2i &pos2,
                       std::map<Vec2i, std::map<Vec2i, std::map<int, std::map<int, std::map<Field, bool>>>>> *lookupCache) const {
    if (isInside(pos1) == false || isInsideSurface(toSurfCoords(pos1)) == false || isInside(pos2) == false || isInsideSurface(toSurfCoords(pos2)) == false) {
        // printf("[%s] Line: %d returning false\n",__FUNCTION__,__LINE__);
        return false;
    }
...
Vec2i Map::computeDestPos(const Vec2i &refUnitPos, const Vec2i &unitPos, const Vec2i &commandPos) const {
    Vec2i pos;
...
const Unit *Map::findClosestUnitToPos(const Selection *selection, Vec2i originalBuildPos, const UnitType *ut) const {
...
bool Map::isNextToUnitTypeCells(const UnitType *ut, const Vec2i &pos, const Vec2i &testPos) const {
...
bool Map::isInUnitTypeCells(const UnitType *ut, const Vec2i &pos, const Vec2i &testPos) const {
    assert(ut != NULL);
    if (ut == NULL) {
        throw megaglest_runtime_error("ut == NULL");
    }
...
void Map::putUnitCells(Unit *unit, const Vec2i &pos, bool ignoreSkill, bool threaded) {
    ...
    if (ignoreSkill == false && unit->getCurrSkill() != NULL && unit->getCurrSkill()->getClass() == scMorph) {
        Command *command = unit->getCurrCommand();
        ...
        for (int j = 0; j < ut->getSize(); ++j) {
            ...
            if (ut->hasCellMap() == false || ut->getCellMapCell(i, j, unit->getModelFacing())) {
                if (getCell(currPos)->getUnit(field) != NULL && getCell(currPos)->getUnit(field) != unit) {
                    ...
                    if (unit->getCurrSkill() != NULL && unit->getCurrSkill()->getClass() == scMove) {
                        canPutInCell = false;
                ...
                if (getCell(currPos)->getUnit(field) == NULL || getCell(currPos)->getUnit(field) == unit ||
                    ...
                    snprintf(szBuf, 8096,
                             ...
                             unit->getId(), unit->getType()->getName(false).c_str(), pos.getString().c_str(), field, getCell(currPos)->getUnit(field)->getId(),
                             getCell(currPos)->getUnit(field)->getType()->getName(false).c_str());
                    throw megaglest_runtime_error(szBuf);
                }
            } else if (ut->hasCellMap() == true && ut->getAllowEmptyCellMap() == true && ut->hasEmptyCellMap() == true) {
...
void Map::clearUnitCells(Unit *unit, const Vec2i &pos, bool ignoreSkill) {
    ...
    if (ignoreSkill == false && unit->getCurrSkill() != NULL && unit->getCurrSkill()->getClass() == scMorph && unit->getMorphFieldsBlocked() == true) {
        Command *command = unit->getCurrCommand();
        const MorphCommandType *mct = static_cast<const MorphCommandType *>(command->getCommandType());
    ...
    for (int i = 1; i < surfaceW - 1; ++i) {
        for (int j = 1; j < surfaceH - 1; ++j) {
            getSurfaceCell(i, j)->setNormal(
                getSurfaceCell(i, j)->getVertex().normal(getSurfaceCell(i, j - 1)->getVertex(), getSurfaceCell(i + 1, j)->getVertex(),
                                                         getSurfaceCell(i, j + 1)->getVertex(), getSurfaceCell(i - 1, j)->getVertex()));
        }
    }
}
            ...
            for (int k = 0; k < cellScale; ++k) {
                for (int l = 0; l < cellScale; ++l) {
                    if (k == 0 && l == 0) {
                        ...
                        getCell(i * cellScale + k, j * cellScale)->setHeight((getSurfaceCell(i, j)->getHeight() + getSurfaceCell(i + 1, j)->getHeight()) / 2.f);
                    } else if (l != 0 && k == 0) {
                        getCell(i * cellScale, j * cellScale + l)->setHeight((getSurfaceCell(i, j)->getHeight() + getSurfaceCell(i, j + 1)->getHeight()) / 2.f);
                    } else {
                        getCell(i * cellScale + k, j * cellScale + l)
                            ->setHeight((getSurfaceCell(i, j)->getHeight() + getSurfaceCell(i, j + 1)->getHeight() + getSurfaceCell(i + 1, j)->getHeight() +
                ...
                for (int l = -1; l <= 1; ++l) {
                    if (cliffLevel <= 0.1f || cliffLevel > fabs(oldHeights[(j)*surfaceW + (i)] - oldHeights[(j + k) * surfaceW + (i + l)])) {
                        ...
                        if (formerObject != NULL) {
                            if (formerObject->getWalkable() || formerObject->getResource() != NULL) {
                                delete formerObject;
                                formerObject = NULL;
                            }
                        }
                        if (formerObject == NULL) {
                            Object *o = new Object(tileset->getObjectType(9), getSurfaceCell(i, j)->getVertex(), Vec2i(i, j));
                ...
                if (getSurfaceCell(pos)->getNearSubmerged()) {
                    ...
                    for (double phi = 0; phi <= 2 * M_PI; phi += 0.02) {
                        ...
                        if (std::abs(rayDirection.x) + std::abs(rayDirection.y) < 0.9) {
                            throw megaglest_runtime_error("Invalid ray direction in water waves computation");
                        }
                        length = 0;
                        Vec2i rayTip = pos;
                        while (length < 100 && isInsideSurface(rayTip) && isInsideSurface(toSurfCoords(rayTip)) &&
                               (getSubmerged(getSurfaceCell(rayTip)) || rayTip == pos)) {
                            length++;
                            rayTip = pos + (Vec2i)(rayDirection * length);
                        }
                        if (!(isInsideSurface(rayTip) && isInsideSurface(toSurfCoords(rayTip)))) {
                            length = 100;
                        }
                    ...
                    for (vector<double>::size_type i = rays.size() / 2; i < rays.size(); i++) {
                        score += rays[i];
                    }
                    score /= rays.size() / 2;

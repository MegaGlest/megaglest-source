...
void SurfaceCell::saveGame(XmlNode *rootNode, int index) const {
  ...
  if (saveCell == true) {
    ...
    //
    //	//tex coords
    //	Vec2f fowTexCoord;		//tex coords for TEXTURE1 when multitexturing
    //and fogOfWar
    ...
    // GameConstants::specialFactions]; 		for(unsigned int i = 0; i
    // < GameConstants::maxPlayers; ++i) { 			if(explored[i]
...
void SurfaceCell::loadGame(const XmlNode *rootNode, int index, World *world) {
  if (rootNode->hasChild("SurfaceCell" + intToStr(index)) == true) {
    ...
    // cellNode->getChild("visible",i); 				int indexCell
    // = visibleNode->getAttribute("index")->getIntValue();
    ...
    // cellNode->getChild("explored",i); 				int indexCell
    // = exploredNode->getAttribute("index")->getIntValue();
    // bool value = exploredNode->getAttribute("value")->getIntValue();
    // explored[indexCell] = value;
    //
    //				//printf("Loading game, sc explored cell index [%d]
    //exploredIndex
    //[%d] value [%d]\n",index,indexCell,value);
    //			}
    //		}
...
// ut->getSize(); ++y) { 			for (int x=0; x < ut->getSize(); ++x) { 				Vec2i cellPos =
//pos + Vec2i(x, y); 				if(isInside(cellPos) &&
// isInsideSurface(toSurfCoords(cellPos))) { 					if
// (ut->getCellMapCell(x, y,
// facing)) { 						if (isFreeCell(cellPos, field) ==
// false) { 							return false;
//						}
//					}
//				}

  ...
  if (saveCell == true) {
    ...
    for (unsigned int i = 0; i < fieldCount; ++i) {
      if (units[i] != NULL) {
        ...
        unitsNode->addAttribute("unitid", intToStr(units[i]->getId()),
                                mapTagReplacements);
      }
    }
    //    Unit *unitsWithEmptyCellMap[fieldCount];	//units with an empty
    //    cellmap on this cell
    for (unsigned int i = 0; i < fieldCount; ++i) {
      if (unitsWithEmptyCellMap[i] != NULL) {
        XmlNode *unitsWithEmptyCellMapNode =
            cellNode->addChild("unitsWithEmptyCellMap");
        unitsWithEmptyCellMapNode->addAttribute("field", intToStr(i),
                                                mapTagReplacements);
        unitsWithEmptyCellMapNode->addAttribute(
            "unitid", intToStr(unitsWithEmptyCellMap[i]->getId()),
            mapTagReplacements);
      }
    }

    //	float height;
    cellNode->addAttribute("height", floatToStr(getHeight(), 6),
                           mapTagReplacements);
  }
}
  ...
  if (rootNode->hasChild("Cell" + intToStr(index)) == true) {
    ...
    for (unsigned int i = 0; i < unitCount; ++i) {
      ...
      if (cellNode->hasChildAtIndex("unitsWithEmptyCellMap", i) == true) {
        const XmlNode *unitsNode =
            cellNode->getChild("unitsWithEmptyCellMap", i);
        int field = unitsNode->getAttribute("field")->getIntValue();
        int unitId = unitsNode->getAttribute("unitid")->getIntValue();
...
SurfaceCell::SurfaceCell() {
  ...
  waterWaveAmplitude = 0.f;

  for (int index = 0;
       index < GameConstants::maxPlayers + GameConstants::specialFactions;
       ++index) {
...
void SurfaceCell::setHeight(float height,
                            bool cellChangedFromOriginalMapLoadValue) {
...
void SurfaceCell::setExplored(int teamIndex, bool explored) {
  if (teamIndex < 0 ||
      teamIndex >= GameConstants::maxPlayers + GameConstants::specialFactions) {
    char szBuf[8096] = "";
...
void SurfaceCell::setVisible(int teamIndex, bool visible) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled ==
      ...
      SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynchMax)
              .enabled == true) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "In setVisible() teamIndex %d visible %d", teamIndex,
             visible);
...
void SurfaceCell::saveGame(XmlNode *rootNode, int index) const {
  ...
  if (saveCell == true) {
    std::map<string, string> mapTagReplacements;
    XmlNode *surfaceCellNode =
        rootNode->addChild("SurfaceCell" + intToStr(index));
    surfaceCellNode->addAttribute("index", intToStr(index), mapTagReplacements);

    //	//geometry
    //	Vec3f vertex;
    surfaceCellNode->addAttribute("vertex", vertex.getString(),
                                  mapTagReplacements);
    //	Vec3f normal;
    // surfaceCellNode->addAttribute("normal",normal.getString(),
    // mapTagReplacements);
    //	Vec3f color;
    // surfaceCellNode->addAttribute("color",color.getString(),
    // mapTagReplacements);
    //
    //	//tex coords
    //	Vec2f fowTexCoord;		//tex coords for TEXTURE1 when
    // multitexturing and fogOfWar
    // surfaceCellNode->addAttribute("fowTexCoord",fowTexCoord.getString(),
    // mapTagReplacements);
    //	Vec2f surfTexCoord;		//tex coords for TEXTURE0
    // surfaceCellNode->addAttribute("surfTexCoord",surfTexCoord.getString(),
    // mapTagReplacements);
    //	//surface
    //	int surfaceType;
    // surfaceCellNode->addAttribute("surfaceType",intToStr(surfaceType),
    // mapTagReplacements);
    ...
    //			if(visible[i] == true) {
    //				XmlNode *visibleNode =
    // surfaceCellNode->addChild("visible");
    //				visibleNode->addAttribute("index",intToStr(i),
    // mapTagReplacements);
    //				visibleNode->addAttribute("value",intToStr(visible[i]),
    // mapTagReplacements);
    //			}
    //		}
    //	//    bool explored[GameConstants::maxPlayers +
    // GameConstants::specialFactions]; 		for(unsigned int i = 0;
    // i < GameConstants::maxPlayers; ++i) { 			if(explored[i]
    // == true) { 				XmlNode *exploredNode =
    // surfaceCellNode->addChild("explored");
    //				exploredNode->addAttribute("index",intToStr(i),
    // mapTagReplacements);
    //				exploredNode->addAttribute("value",intToStr(explored[i]),
    // mapTagReplacements);
    //			}
    //		}
    ...
    //	//cache
    //	bool nearSubmerged;
    // surfaceCellNode->addAttribute("nearSubmerged",intToStr(nearSubmerged),
    // mapTagReplacements);
  }
}
...
void SurfaceCell::loadGame(const XmlNode *rootNode, int index, World *world) {
  if (rootNode->hasChild("SurfaceCell" + intToStr(index)) == true) {
    const XmlNode *surfaceCellNode =
        rootNode->getChild("SurfaceCell" + intToStr(index));

    if (surfaceCellNode->hasAttribute("vertex") == true) {
      vertex =
          Vec3f::strToVec3(surfaceCellNode->getAttribute("vertex")->getValue());
    }
    ...
    //				const XmlNode *visibleNode =
    // cellNode->getChild("visible",i); 				int
    // indexCell = visibleNode->getAttribute("index")->getIntValue();
    // bool value = visibleNode->getAttribute("value")->getIntValue();
    // visible[indexCell] = value;
    //
    //				//printf("Loading game, sc visible index
    //[%d][%d][%d]\n",index,indexCell,value);
    //			}
    //			if(cellNode->hasChildAtIndex("explored",i) == true) {
    //				const XmlNode *exploredNode =
    // cellNode->getChild("explored",i); 				int
    // indexCell = exploredNode->getAttribute("index")->getIntValue();
    // bool value = exploredNode->getAttribute("value")->getIntValue();
    // explored[indexCell] = value;
    //
    //				//printf("Loading game, sc explored cell index
    //[%d] exploredIndex [%d] value [%d]\n",index,indexCell,value);
    //			}
    //		}
  }
}
...
Map::~Map() {
  Logger::getInstance().add(
      Lang::getInstance().getString("LogScreenGameUnLoadingMapCells", "", true),
      true);
...
void Map::end() {
  ...
  Logger::getInstance().add(
      Lang::getInstance().getString("LogScreenGameUnLoadingMap", "", true),
      true);
  // read heightmap
...
Vec2i Map::getStartLocation(int locationIndex) const {
  if (locationIndex >= maxPlayers) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "locationIndex >= maxPlayers [%d] [%d]",
             locationIndex, maxPlayers);
    printf("%s\n", szBuf);
    throw megaglest_runtime_error(szBuf);
...
Checksum Map::load(const string &path, TechTree *techTree, Tileset *tileset) {
  ...
  try {
    ...
    if (f != NULL) {
      ...
      if (readBytes != 1) {
        throw megaglest_runtime_error("Invalid map header detected for file: " +
                                      path);
      }
      ...
      waterLevel =
          static_cast<float>((header.waterLevel - 0.01f) / heightFactor);
      ...
      if (header.version == 1) {
        ...
        if (header.version2.cliffLevel > 0 &&
            header.version2.cliffLevel < 5000) {
          cliffLevel = static_cast<float>((header.version2.cliffLevel - 0.01f) /
                                          (heightFactor));
          cliffLevel = truncateDecimal<float>(cliffLevel, 6);
        }
        if (header.version2.cameraHeight > 0 &&
            header.version2.cameraHeight < 5000) {
      ...
      for (int j = 0; j < h; j += cellScale) {
        for (int i = 0; i < w; i += cellScale) {
          ...
          if (objNumber <= 0) {
            ...
            Object *o = new Object(tileset->getObjectType(objNumber - 1),
                                   sc->getVertex(), Vec2i(i, j));
            sc->setObject(o);
            for (int k = 0; k < techTree->getResourceTypeCount(); ++k) {
              const ResourceType *rt = techTree->getResourceType(k);
              if (rt->getClass() == rcTileset &&
                  rt->getTilesetObject() == objNumber) {
                o->setResource(rt, Vec2i(i, j));
              }
            }
          } else {
            const ResourceType *rt =
                techTree->getTechResourceType(objNumber - Tileset::objCount);
...
// returns if there is a resource next to a unit, in "resourcePos" is stored the
// relative position of the resource
bool Map::isResourceNear(int frameIndex, const Vec2i &pos,
                         const ResourceType *rt, Vec2i &resourcePos, int size,
                         Unit *unit, bool fallbackToPeersHarvestingSameResource,
  ...
  if (resourceClickPos) {
    // printf("+++++++++ unit [%s - %d] pos = [%s] resourceClickPos
    ...
    for (int j = -size; j <= size; ++j) {
      ...
      if (isInside(resPos) && isInsideSurface(surfCoords)) {
        ...
        if (r != NULL) {
          if (r->getType() == rt) {
            if (resourceClickPos) {
              // printf("****** unit [%s - %d] resPos = [%s]
              // resourceClickPos->dist(resPos) [%f] distanceFromClick [%f]
              // unit->getCenteredPos().dist(resPos) [%f] distanceFromUnit
            ...
            if (resourceClickPos == NULL ||
                (distanceFromClick < 0 ||
                 resourceClickPos->dist(resPos) <= distanceFromClick)) {
              if (unit == NULL ||
                  (distanceFromUnit < 0 ||
                   unit->getCenteredPos().dist(resPos) <= distanceFromUnit)) {
                bool isResourceNextToUnit = (resourceClickPos == NULL);
                for (int i1 = -size;
                     isResourceNextToUnit == false && i1 <= size; ++i1) {
                ...
                if (isResourceNextToUnit == true) {
                  ...
                  if (unit == NULL ||
                      unit->isBadHarvestPos(resourcePos) == false) {
                    resourceNear = true;

                    if (resourceClickPos) {
                      // printf("@@@@@@@@ unit [%s - %d] resPos = [%s]
                      // resourceClickPos->dist(resPos) [%f] distanceFromClick
                      // [%f] unit->getCenteredPos().dist(resPos) [%f]
                      // distanceFromUnit
                      // [%f]\n",unit->getFullName().c_str(),unit->getId(),resPos.getString().c_str(),resourceClickPos->dist(resPos),distanceFromClick,unit->getCenteredPos().dist(resPos),distanceFromUnit);
                    }
                  }
  ...
  if (resourceNear == false) {
    if (fallbackToPeersHarvestingSameResource == true && unit != NULL) {
      ...
      // this resource type
      Vec2i result = unit->getFaction()->getClosestResourceTypeTargetFromCache(
          unit, rt, frameIndex);
      if (result.x >= 0) {
        resourcePos = result;
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
          ...
          char szBuf[8096] = "";
          snprintf(szBuf, 8096,
                   "[found peer harvest pos] pos [%s] resourcePos [%s] "
                   "unit->getFaction()->getCacheResourceTargetListSize() [%d]",
                   pos.getString().c_str(), resourcePos.getString().c_str(),
                   unit->getFaction()->getCacheResourceTargetListSize());
        ...
        if (isInside(resPos) && isInsideSurface(surfCoords)) {
          ...
          if (r != NULL) {
            if (r->getType() == rt) {
              ...
              if (unit == NULL || (distanceFromUnit < 0 ||
                                   unit->getCenteredPos().dist(resPos) <=
                                       (distanceFromUnit + 2.0))) {
                if (resourceClickPos->dist(resPos) <= 1.0) {
                  if (unit != NULL) {
                    FindBestPos bestPosItem;

                    bestPosItem.distanceFromUnitNoAdjustment =
                        unit->getCenteredPos().dist(resPos);
                    bestPosItem.distanceFromClickNoAdjustment =
                        distanceFromClick = resourceClickPos->dist(resPos);
                ...
                if (distanceFromClick < 0 ||
                  ...
                  *resourceClickPos = resPos;
                  ...
                  if (unit == NULL ||
                      unit->isBadHarvestPos(*resourceClickPos) == false) {
                    // resourceNear = true;
      ...
      if (bestPosItem.distanceFromUnitNoAdjustment < bestUnitDist) {
        ...
        *resourceClickPos = bestPosItem.resourcePosNoAdjustment;
        ...
        if (unit == NULL || unit->isBadHarvestPos(*resourceClickPos) == false) {
          // printf("%%----------- unit [%s - %d] resourceClickPos [%s]
          // bestUnitDist
...
bool Map::isFreeCell(const Vec2i &pos, Field field, bool buildingsOnly) const {
  return isInside(pos) && isInsideSurface(toSurfCoords(pos)) &&
         (getCell(pos)->isFree(field)
              ? true
              : (buildingsOnly &&
                 !getCell(pos)->getUnit(field)->getType()->hasSkillClass(
                     scBeBuilt))) &&
         (field == fAir || getSurfaceCell(toSurfCoords(pos))->isFree()) &&
         (field != fLand || getDeepSubmerged(getCell(pos)) == false);
}

bool Map::isFreeCellOrHasUnit(const Vec2i &pos, Field field,
...
// TT: this is much more complicated compared with the old one above. I think
// its no more needed bool Map::isFreeCellOrHasUnit(const Vec2i &pos, Field
// field, const Unit *unit) const { 	if(isInside(pos) &&
// isInsideSurface(toSurfCoords(pos))) { 		if(unit->getCurrField()
// != field) { 			return isFreeCell(pos, field);
//		}
//		Cell *c= getCell(pos);
...
// getSurfaceCell(toSurfCoords(pos)); 				if(sc != NULL) {
// if(getDeepSubmerged(sc) == true) { return false;
//					}
//					else if(field == fLand) {
//						if(sc->isFree() == false) {
//							return false;
//						}
//						else if(c->getUnit(field) !=
// NULL) { 							return false;
//						}
//					}
//				}
...
bool Map::isAproxFreeCell(const Vec2i &pos, Field field, int teamIndex) const {
  if (isInside(pos) && isInsideSurface(toSurfCoords(pos))) {
    ...
    if (sc->isVisible(teamIndex)) {
      ...
      return field == fLand ? sc->isFree() && !getDeepSubmerged(getCell(pos))
                            : true;
    } else {
      return true;
    }
...
bool Map::isFreeCells(const Vec2i &pos, int size, Field field,
                      bool buildingsOnly) const {
...
bool Map::isAproxFreeCells(const Vec2i &pos, int size, Field field,
                           int teamIndex) const {
  for (int i = pos.x; i < pos.x + size; ++i) {
    for (int j = pos.y; j < pos.y + size; ++j) {
...
bool Map::canMorph(const Vec2i &pos, const Unit *currentUnit,
                   const UnitType *targetUnitType) const {
...
// CardinalDir facing) { 	if (ut->hasCellMap() && isInside(pos) &&
// isInsideSurface(toSurfCoords(pos))) { 		for (int y=0; y <
// ut->getSize(); ++y) { 			for (int x=0; x < ut->getSize();
// ++x) { 				Vec2i cellPos =
// pos + Vec2i(x, y); 				if(isInside(cellPos) &&
// isInsideSurface(toSurfCoords(cellPos))) {
// if (ut->getCellMapCell(x, y, facing)) {
...
bool Map::canMove(
    const Unit *unit, const Vec2i &pos1, const Vec2i &pos2,
    std::map<Vec2i, std::map<Vec2i, std::map<int, std::map<Field, bool> > > >
        *lookupCache) const {
  ...
  if (lookupCache != NULL) {
    std::map<Vec2i, std::map<Vec2i, std::map<int, std::map<Field, bool> > > >::
        const_iterator iterFind1 = lookupCache->find(pos1);
    if (iterFind1 != lookupCache->end()) {
      std::map<Vec2i, std::map<int, std::map<Field, bool> > >::const_iterator
          iterFind2 = iterFind1->second.find(pos2);
      if (iterFind2 != iterFind1->second.end()) {
        std::map<int, std::map<Field, bool> >::const_iterator iterFind3 =
            iterFind2->second.find(size);
        if (iterFind3 != iterFind2->second.end()) {
          std::map<Field, bool>::const_iterator iterFind4 =
              iterFind3->second.find(field);
...
// checks if a unit can move from between 2 cells using only visible cells (for
// pathfinding)
bool Map::aproxCanMove(
    const Unit *unit, const Vec2i &pos1, const Vec2i &pos2,
    std::map<Vec2i,
             std::map<Vec2i,
                      std::map<int, std::map<int, std::map<Field, bool> > > > >
        *lookupCache) const {
  ...
  if (lookupCache != NULL) {
    std::map<
        ...
        const_iterator iterFind1 = lookupCache->find(pos1);
    if (iterFind1 != lookupCache->end()) {
      std::map<Vec2i, std::map<int, std::map<int, std::map<Field, bool> > > >::
          const_iterator iterFind2 = iterFind1->second.find(pos2);
      if (iterFind2 != iterFind1->second.end()) {
        std::map<int, std::map<int, std::map<Field, bool> > >::const_iterator
            iterFind3 = iterFind2->second.find(teamIndex);
        if (iterFind3 != iterFind2->second.end()) {
          std::map<int, std::map<Field, bool> >::const_iterator iterFind4 =
              iterFind3->second.find(size);
          if (iterFind4 != iterFind3->second.end()) {
            std::map<Field, bool>::const_iterator iterFind5 =
                iterFind4->second.find(field);
            if (iterFind5 != iterFind4->second.end()) {
              // Found this result in the cache
              if (iterFind5->second == false) {
                // printf("[%s] Line: %d returning
                // false\n",__FUNCTION__,__LINE__);
              }
  ...
  if (size == 1) {
    ...
    if (pos1.x != pos2.x && pos1.y != pos2.y) {
      if (isAproxFreeCell(Vec2i(pos1.x, pos2.y), field, teamIndex) == false) {
        ...
        // Object * obj = getSurfaceCell(toSurfCoords(Vec2i(pos1.x,
        // pos2.y)))->getObject();

        // printf("[%s] Line: %d returning false cell [%s] free [%d] cell unitid
        // = %d object class = %d\n",__FUNCTION__,__LINE__,Vec2i(pos1.x,
        // pos2.y).getString().c_str(),this->isFreeCell(Vec2i(pos1.x,
        // pos2.y),field),(cellUnit != NULL ? cellUnit->getId() : -1),(obj !=
        // NULL ? obj->getType()->getClass() : -1)); printf("[%s] Line: %d
        // returning false\n",__FUNCTION__,__LINE__);
        return false;
      }
  ...
  for (int i = 0; i < selection->getCount(); ++i) {
    if (selection->getUnit(i) == NULL) {
      throw megaglest_runtime_error(
          "selection == NULL || selection->getUnit(i) == NULL");
    }
    total = total + selection->getUnit(i)->getPosNotThreadSafe();
  }

  return Vec2i(total.x / selection->getCount(),
               total.y / selection->getCount());
}
...
// std::pair<float,Vec2i> Map::getUnitDistanceToPos(const Unit *unit,Vec2i
// pos,const UnitType *ut) { 	if(unit == NULL) { 		throw
// megaglest_runtime_error("unit == NULL");
//	}
//
//	std::pair<float,Vec2i> result(-1,Vec2i(0));
//	//int unitId= unit->getId();
//	Vec2i unitPos= computeDestPos(unit->getPosNotThreadSafe(),
// unit->getPosNotThreadSafe(), pos);
//
...
//			Vec2i testPos(i,j);
//
//			if(ut == NULL || isInUnitTypeCells(ut, pos,testPos) ==
// false) { 				float distance = unitPos.dist(testPos);
// if(result.first < 0 || result.first > distance) {
// result.first = distance; 					result.second =
// testPos;
//				}
//			}
//		}
...
const Unit *Map::findClosestUnitToPos(const Selection *selection,
                                      Vec2i originalBuildPos,
  ...
  for (int i = 0; i < selection->getCount(); ++i) {
    ...
    // int unitId= unit->getId();
    Vec2i unitBuilderPos =
        computeDestPos(refPos, unit->getPosNotThreadSafe(), pos);
...
Vec2i Map::findBestBuildApproach(const Unit *unit, Vec2i originalBuildPos,
                                 const UnitType *ut) const {
  if (unit == NULL) {
    throw megaglest_runtime_error("unit == NULL");
  }
    ...
    for (int j = start.y; j <= end.y; ++j) {
      ...
      if (isInUnitTypeCells(ut, originalBuildPos, testPos) == false) {
        ...
        if (bestRange < 0 || bestRange > distance) {
          ...
          if (isFreeCellOrHasUnit(testPos, unit->getType()->getField(), unit) ==
              true) {
            bestRange = distance;
            pos = testPos;
          }
    ...
    for (int i = -1; i <= ut->getSize(); ++i) {
      for (int j = -1; j <= ut->getSize(); ++j) {
        ...
        if (isInside(currPos) == true &&
            isInsideSurface(toSurfCoords(currPos)) == true) {
...
void Map::putUnitCells(Unit *unit, const Vec2i &pos, bool ignoreSkill,
  ...
  if (ignoreSkill == false && unit->getCurrSkill() != NULL &&
...
void Map::putUnitCellsPrivate(Unit *unit, const Vec2i &pos, const UnitType *ut,
                              bool isMorph, bool threaded) {
  assert(unit != NULL);
  if (unit == NULL) {
    throw megaglest_runtime_error("ut == NULL");
  }
  ...
  for (int i = 0; i < ut->getSize(); ++i) {
    for (int j = 0; j < ut->getSize(); ++j) {
      ...
      if (ut->hasCellMap() == false ||
          ut->getCellMapCell(i, j, unit->getModelFacing())) {
        if (getCell(currPos)->getUnit(field) != NULL &&
          ...
          if (unit->getCurrSkill() != NULL &&
            ...
            // Line: %d] POSSIBLE ERROR
            // [getCell(currPos)->getUnit(unit->getCurrField()) != NULL] currPos
            // [%s] unit [%s] cell unit [%s]\n",
            //       __FILE__,__FUNCTION__,__LINE__,
          ...
          // TT: Nonsens?
          //                     else {
          //                         // If the unit trying to move into the cell
          //                         is not in the moving state
          //                         // it is likely being created or morphed so
          //                         we will will log the error canPutInCell =
          //                         false;
          //				        // throw
          // megaglest_runtime_error("getCell(currPos)->getUnit(unit->getCurrField())
          //!= NULL");
          //                         SystemFlags::OutputDebug(SystemFlags::debugError,"In
          //                         [%s::%s Line: %d] ERROR
          //                         [getCell(currPos)->getUnit(unit->getCurrField())
          //                         != NULL] currPos [%s] unit [%s] cell unit
          //                         [%s]\n",
          //                               __FILE__,__FUNCTION__,__LINE__,
          //                               currPos.getString().c_str(),
          //                               unit->toString().c_str(),
        ...
        if (getCell(currPos)->getUnit(field) == NULL ||
            getCell(currPos)->getUnit(field) == unit ||
            (unit->getType()->hasSkillClass(scBeBuilt) !=
             getCell(currPos)->getUnit(field)->getType()->hasSkillClass(
                 scBeBuilt))) {
          if (isMorph) {
          ...
          char szBuf[8096] = "";
          snprintf(szBuf, 8096,
                   "Trying to move unit [%d - %s] into occupied cell [%s] and "
                   "field = %d, unit already in cell [%d - %s] ",
                   unit->getId(), unit->getType()->getName(false).c_str(),
                   pos.getString().c_str(), field,
                   getCell(currPos)->getUnit(field)->getId(),
                   getCell(currPos)
                       ->getUnit(field)
                       ->getType()
                       ->getName(false)
  ...
  for (int i = 0; i < ut->getSize(); ++i) {
    for (int j = 0; j < ut->getSize(); ++j) {
      ...
      if (ut->hasCellMap() == false ||
        ...
        // getCell(currPos)->getUnit(unit->getCurrField()) == NULL);
        // if(getCell(currPos)->getUnit(unit->getCurrField()) != unit &&
        // getCell(currPos)->getUnit(unit->getCurrField()) != NULL) { 	throw
        // megaglest_runtime_error("getCell(currPos)->getUnit(unit->getCurrField())
        //!= unit"); SystemFlags::OutputDebug(SystemFlags::debugError,"In
        // [%s::%s Line: %d] ERROR
        // [getCell(currPos)->getUnit(unit->getCurrField()) != unit] currPos
        // [%s] unit [%s] cell unit [%s]\n",
        //          __FILE__,__FUNCTION__,__LINE__,
        //          currPos.getString().c_str(),
        //          unit->toString().c_str(),
  ...
  for (int i = -1; i <= 1; ++i) {
    for (int j = -1; j <= 1; ++j) {
      if (isInside(pos.x + i, pos.y + j) &&
          isInsideSurface(toSurfCoords(Vec2i(pos.x + i, pos.y + j)))) {
        if (getCell(pos.x + i, pos.y + j)->getUnit(fLand) == unit) {
          return true;
        } else if (getCell(pos.x + i, pos.y + j)
                       ->getUnitWithEmptyCellMap(fLand) == unit) {
          return true;
        }
      }
    }
...
bool Map::isNextTo(const Vec2i &pos, const Vec2i &nextToPos) const {
  for (int i = -1; i <= 1; ++i) {
    for (int j = -1; j <= 1; ++j) {
      if (isInside(pos.x + i, pos.y + j) &&
          isInsideSurface(toSurfCoords(Vec2i(pos.x + i, pos.y + j)))) {
        if (getCell(pos.x + i, pos.y + j) ==
            getCell(nextToPos.x, nextToPos.y)) {
          return true;
        }
      }
    }
...
void Map::flatternTerrain(const Unit *unit) {
  float refHeight =
      getSurfaceCell(toSurfCoords(unit->getCenteredPos()))->getHeight();
  for (int i = -1; i <= unit->getType()->getSize(); ++i) {
    for (int j = -1; j <= unit->getType()->getSize(); ++j) {
      ...
      if (isInside(pos) && isInsideSurface(toSurfCoords(pos))) {
        ...
        // we change height if pos is inside world, if its free or ocupied by
        // the currenty building
        if (sc->getObject() == NULL &&
            (c->getUnit(fLand) == NULL || c->getUnit(fLand) == unit)) {
...
void Map::computeNormals() {
  ...
  for (int i = 1; i < surfaceW - 1; ++i) {
    for (int j = 1; j < surfaceH - 1; ++j) {
      getSurfaceCell(i, j)->setNormal(getSurfaceCell(i, j)->getVertex().normal(
...
void Map::computeInterpolatedHeights() {
  for (int i = 0; i < w; ++i) {
    for (int j = 0; j < h; ++j) {
      getCell(i, j)->setHeight(
          getSurfaceCell(toSurfCoords(Vec2i(i, j)))->getHeight());
    }
  }
    ...
    for (int j = 1; j < surfaceH - 1; ++j) {
      for (int k = 0; k < cellScale; ++k) {
        for (int l = 0; l < cellScale; ++l) {
          if (k == 0 && l == 0) {
            getCell(i * cellScale, j * cellScale)
                ->setHeight(getSurfaceCell(i, j)->getHeight());
          } else if (k != 0 && l == 0) {
            getCell(i * cellScale + k, j * cellScale)
                ->setHeight((getSurfaceCell(i, j)->getHeight() +
                             getSurfaceCell(i + 1, j)->getHeight()) /
                            2.f);
          } else if (l != 0 && k == 0) {
            getCell(i * cellScale, j * cellScale + l)
                ->setHeight((getSurfaceCell(i, j)->getHeight() +
                             getSurfaceCell(i, j + 1)->getHeight()) /
                            2.f);
          } else {
            getCell(i * cellScale + k, j * cellScale + l)
                ->setHeight((getSurfaceCell(i, j)->getHeight() +
                             getSurfaceCell(i, j + 1)->getHeight() +
                             getSurfaceCell(i + 1, j)->getHeight() +
                             getSurfaceCell(i + 1, j + 1)->getHeight()) /
                            4.f);
          }
        }
      }
...
#ifdef USE_STREFLOP
          if (cliffLevel <= 0.1f ||
              cliffLevel > streflop::fabs(static_cast<streflop::Simple>(
                               oldHeights[(j)*surfaceW + (i)] -
                               oldHeights[(j + k) * surfaceW + (i + l)]))) {
#else
          if (cliffLevel <= 0.1f ||
              cliffLevel > fabs(oldHeights[(j)*surfaceW + (i)] -
                                oldHeights[(j + k) * surfaceW + (i + l)])) {
#endif
            ...
            Object *formerObject = getSurfaceCell(i, j)->getObject();
            ...
            if (formerObject == NULL) {
              Object *o =
                  new Object(tileset->getObjectType(9),
                             getSurfaceCell(i, j)->getVertex(), Vec2i(i, j));
      ...
      for (int k = -1; k <= 2; ++k) {
        for (int l = -1; l <= 2; ++l) {
          ...
          if (isInsideSurface(pos) && isInsideSurface(toSurfCoords(pos))) {
            if (getSubmerged(getSurfaceCell(pos))) anySubmerged = true;
          }
        }
      }
...
void Map::computeWaterWaveAmplitude() {
  for (int i = 0; i < surfaceW; ++i) {
    for (int j = 0; j < surfaceH; ++j) {
      Vec2i pos = Vec2i(i, j);
      if (isInsideSurface(pos) && isInsideSurface(toSurfCoords(pos))) {
        if (getSurfaceCell(pos)->getNearSubmerged()) {
          ...
          // then count the length of the rays to compute score of the wave
          // heigth
          vector<double> rays = {};
          int length;
          Vec2d rayDirection;
          for (double phi = 0; phi <= 2 * M_PI; phi += 0.02) {
            rayDirection.x = std::cos(phi);
            rayDirection.y = std::sin(phi);
            if (std::abs(rayDirection.x) + std::abs(rayDirection.y) < 0.9) {
              throw megaglest_runtime_error(
                  "Invalid ray direction in water waves computation");
            }
            length = 0;
            Vec2i rayTip = pos;
            while (length < 100 && isInsideSurface(rayTip) &&
                   isInsideSurface(toSurfCoords(rayTip)) &&
                   (getSubmerged(getSurfaceCell(rayTip)) || rayTip == pos)) {
              length++;
              rayTip = pos + (Vec2i)(rayDirection * length);
            }
            if (!(isInsideSurface(rayTip) &&
                  isInsideSurface(toSurfCoords(rayTip)))) {
              length = 100;
            }
            rays.push_back(length);
          }
          float score = 0;
          sort(rays.begin(), rays.end());
          for (vector<double>::size_type i = rays.size() / 2; i < rays.size();
               i++) {
            score += rays[i];
          }
          score /= rays.size() / 2;
          score = score > 80 ? 80 : score;  // cap score at 80
          score /= 100;
          getSurfaceCell(pos)->setWaterWaveAmplitude(score);
        }
      }
    }
  }
...
void Map::saveGame(XmlNode *rootNode) const {
  ...
  //	float waterLevel;
  mapNode->addAttribute("waterLevel", floatToStr(waterLevel, 6),
                        mapTagReplacements);
  //	float heightFactor;
  mapNode->addAttribute("heightFactor", floatToStr(heightFactor, 6),
                        mapTagReplacements);
  //	float cliffLevel;
  mapNode->addAttribute("cliffLevel", floatToStr(cliffLevel, 6),
                        mapTagReplacements);
  //	int cameraHeight;
  mapNode->addAttribute("cameraHeight", intToStr(cameraHeight),
                        mapTagReplacements);
  //	int w;
  ...
  for (unsigned int i = 0; i < (unsigned int)getSurfaceCellArraySize(); ++i) {
    ...
    if (i > 0 && i % 100 == 0) {
      ...
      surfaceCellNode->addAttribute("batchIndex", intToStr(i),
                                    mapTagReplacements);
      surfaceCellNode->addAttribute("exploredList", exploredList,
                                    mapTagReplacements);
      surfaceCellNode->addAttribute("visibleList", visibleList,
                                    mapTagReplacements);
  ...
  if (exploredList != "") {
    XmlNode *surfaceCellNode = mapNode->addChild("SurfaceCell");
    surfaceCellNode->addAttribute(
        "batchIndex", intToStr(getSurfaceCellArraySize()), mapTagReplacements);
    surfaceCellNode->addAttribute("exploredList", exploredList,
                                  mapTagReplacements);
    surfaceCellNode->addAttribute("visibleList", visibleList,
                                  mapTagReplacements);
  ...
  for (unsigned int i = 0; i < (unsigned int)maxPlayers; ++i) {
    ...
    startLocationsNode->addAttribute("location", startLocations[i].getString(),
                                     mapTagReplacements);
  }
  //	Checksum checksumValue;
  //	mapNode->addAttribute("checksumValue",intToStr(checksumValue.getSum()),
  // mapTagReplacements); 	float maxMapHeight;
  mapNode->addAttribute("maxMapHeight", floatToStr(maxMapHeight, 6),
                        mapTagReplacements);
  //	string mapFile;
...
void Map::loadGame(const XmlNode *rootNode, World *world) {
  ...
  for (unsigned int i = 0; i < surfaceCellNodeList.size(); ++i) {
    ...
    string exploredList =
        surfaceCellNode->getAttribute("exploredList")->getValue();
    string visibleList =
        surfaceCellNode->getAttribute("visibleList")->getValue();
    // int batchIndex =
    // surfaceCellNode->getAttribute("batchIndex")->getIntValue();

    vector<string> tokensExplored;
    Tokenize(exploredList, tokensExplored, ",");

    // printf("=====================\nNew batchIndex = %d batchsize =
    // %d\n",batchIndex,tokensExplored.size()); for(unsigned int j = 0; j <
    // tokensExplored.size(); ++j) { string valueList = tokensExplored[j];
    // printf("valueList [%s]\n",valueList.c_str());
    //}
    for (unsigned int j = 0; j < tokensExplored.size(); ++j) {
      ...
      // printf("Loading sc = %d batchIndex =
      ...
      //			if(tokensExploredValue.size() !=
      // GameConstants::maxPlayers) { 				for(unsigned int
      // k = 0; k < tokensExploredValue.size(); ++k) {
      // string value = tokensExploredValue[k];
      // printf("k = %d [%s]\n",k,value.c_str());
      //				}
      //				throw
      // megaglest_runtime_error("tokensExploredValue.size() [" +
      // intToStr(tokensExploredValue.size()) + "] !=
      ...
      for (unsigned int k = 0; k < tokensExploredValue.size(); ++k) {
        ...
        //	printf("Setting cell at index: %d for team: %d to: %d
    ...
    for (unsigned int j = 0; j < tokensVisible.size(); ++j) {
      ...
      //			if(tokensVisibleValue.size() !=
      // GameConstants::maxPlayers) { 				throw
      // megaglest_runtime_error("tokensVisibleValue.size() [" +
      // intToStr(tokensVisibleValue.size()) + "] !=
      // GameConstants::maxPlayers");
      //			}
...
PosCircularIterator::PosCircularIterator(const Map *map, const Vec2i &center,
                                         int radius) {
  this->map = map;
  this->radius = radius;
  this->center = center;
...
#ifdef USE_STREFLOP
  while (streflop::floor(static_cast<streflop::Simple>(pos.dist(center))) >=
             (radius + 1) ||
         !map->isInside(pos) || !map->isInsideSurface(map->toSurfCoords(pos)));
#else
...
const Vec2i &PosCircularIterator::getPos() { return pos; }
...
bool PosQuadIterator::next() {
  do {
    ...
    // printf("pos [%s] boundingRect.p[0] [%s] boundingRect.p[1]
...
const Vec2i &PosQuadIterator::getPos() { return pos; }

...
enum FogMode { fmExp, fmExp2 };

enum Weather { wSunny, wRainy, wSnowy };
...
class AmbientSounds {
  ...
  public:
    ...
    void load(const string &dir, const XmlNode *xmlNode, std::map<string, vector<pair<string, string>>> &loadedFileList, string parentLoader);
};
...
class Tileset {
  ...
  public:
    ...
    void load(const string &dir, Checksum *checksum, Checksum *tilesetChecksum, std::map<string, vector<pair<string, string>>> &loadedFileList);

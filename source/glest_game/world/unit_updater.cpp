...
UnitUpdater::UnitUpdater() : mutexAttackWarnings(new Mutex(CODE_AT_LINE)), mutexUnitRangeCellsLookupItemCache(new Mutex(CODE_AT_LINE)) {
    this->game = NULL;
    this->gui = NULL;
...
bool UnitUpdater::updateUnit(Unit *unit) {
    ...
    if (currSkill->getShake()) {
        ...
        if (shakeStartTime >= unit->getLastAnimProgressAsFloat() && shakeStartTime < unit->getAnimProgressAsFloat()) {
            ...
            if (visibility && cameraAffected && enabled) {
                game->getGameCameraPtr()->shake(currSkill->getShakeDuration(), currSkill->getShakeIntensity(), cameraDistanceAffected,
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s] Line: %d took msecs: %lld [after playsound]\n", __FILE__, __FUNCTION__, __LINE__,
                                 chrono.getMillis());
    ...
    if (unit->getCurrSkill()->getClass() == scDie) {
        ...
        if (dst->getSpawn() == true) {
            ...
            if (startSpawnNow) {
                //	spawn the units
                spawn(unit, dst->getSpawnUnit(), dst->getSpawnUnitCount(), dst->getSpawnUnitHealthPercentMin(), dst->getSpawnUnitHealthPercentMax(),
    ...
    if (unit->getCurrSkill()->getClass() == scAttack) {
        ...
        snprintf(szBuf, 8095,
                 ...
                 attackStartTime, lastAnimProgress, animProgress, startAttackParticleSystemNow);
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s] Line: %d took msecs: %lld [after unit->update()]\n", __FILE__, __FUNCTION__,
    ...
    if (update == true) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
            SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s] Line: %d took msecs: %lld [after updateUnitCommand()]\n", __FILE__,
        ...
        if (unit->computeEp() == true) {
            ...
            if (unit->getCurrCommand() != NULL && unit->getCurrCommand()->getCommandType()->getClass() == ccAttackStopped) {
                ...
                for (int i = 0; i < ut->getCommandTypeCount(); ++i) {
                    ...
                    if (ct != NULL && ct->getClass() == ccAttackStopped) {
                        ...
                        if (act != NULL && act->getName(false) == holdPositionName) {
                            ...
                            if (unit->getFaction()->reqsOk(act) == true && unit->getEp() >= act->getStopSkillType()->getEpCost()) {
        ...
        if (unit->getCurrSkill()->getClass() == scMove) {
            ...
            if (map->getCell(unit->getPos())->getHeight() < map->getWaterLevel() && unit->getCurrField() == fLand) {
                if (Config::getInstance().getBool("DisableWaterSounds", "false") == false) {
                    soundRenderer.playFx(CoreData::getInstance().getWaterSound(), unit->getCurrMidHeightVector(), gameCamera->getPos());

                    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
                        SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s] Line: %d took msecs: %lld [after soundFx()]\n", __FILE__,
                                                 __FUNCTION__, __LINE__, chrono.getMillis());
                }
            }
        }
...
void UnitUpdater::spawn(Unit *unit, string spawnUnit, int spawnUnitcount, int healthMin, int healthMax, int probability) {
    if (spawnUnit != "" && spawnUnitcount > 0) {
        ...
        for (int y = 0; y < spawnCount; ++y) {
            if (probability > 0 && probability < 100 && unit->getRandom()->randRange(1, 100) <= probability) {
                continue;
            }
...
void UnitUpdater::spawnAttack(Unit *unit, string spawnUnit, int spawnUnitcount, int healthMin, int healthMax, int probability, bool spawnUnitAtTarget,
                              Vec2i targetPos) {
    if (spawnUnit != "" && spawnUnitcount > 0) {
        int spawnCount = spawnUnitcount;
        for (int y = 0; y < spawnCount; ++y) {
...
Unit *UnitUpdater::spawnUnit(Unit *unit, string spawnUnit, Vec2i spawnPos) {
    ...
    Unit *spawned = new Unit(world->getNextUnitId(unit->getFaction()), newpath, Vec2i(0), spawnUnitType, unit->getFaction(), world->getMap(),
        ...
        if (hasCommand == true) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] unit [%s] has command [%s]\n", __FILE__, __FUNCTION__, __LINE__,
                                         unit->toString(false).c_str(), unit->getCurrCommand()->toString(false).c_str());
            ...
            if ((minorDebugPerformance && frameIndex > 0) && (chrono.getMillis() - elapsed1) >= 1) {
                ...
                printf("UnitUpdate [%d - %s] #2-unit threaded updates on frame: %d "
                       ...
                       unit->getId(), unit->getType()->getName(false).c_str(), frameIndex, commandUsesPathFinder, (long long int)chrono.getMillis() - elapsed1,
...
void UnitUpdater::updateMove(Unit *unit, int frameIndex) {
    try {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true && frameIndex < 0) {
            char szBuf[8096] = "";
            snprintf(szBuf, 8096, "[updateMove] pos [%s] unit [%d - %s] cmd [%s]", pos.getString().c_str(), unit->getId(), unit->getFullName(false).c_str(),
    ...
    try {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] unit [%s] will build using command [%s]\n", __FILE__, __FUNCTION__,
                                     __LINE__, unit->toString(false).c_str(), unit->getCurrCommand()->toString(false).c_str());
        ...
        if (unit->getCurrSkill() != NULL && unit->getCurrSkill()->getClass() != scBuild) {
            ...
            if (frameIndex < 0) {
                ...
                case tsArrived: {
                    ...
                    if (canOccupyCell == true) {
                        ...
                        Unit *builtUnit =
                            new Unit(world->getNextUnitId(unit->getFaction()), newpath, buildPos, builtUnitType, unit->getFaction(), world->getMap(), facing);
                        ...
                        if (builtUnitType->hasSkillClass(scBeBuilt) == false) {
                            throw megaglest_runtime_error("Unit [" + builtUnitType->getName(false) + "] has no be_built skill, producer was [" +
                                                          intToStr(unit->getId()) + " - " + unit->getType()->getName(false) + "].");
                        }
                        ...
                        if (unit->getFactionIndex() == world->getThisFactionIndex() ||
                            (game->getWorld()->showWorldForPlayer(game->getWorld()->getThisTeamIndex()) == true)) {
                            SoundRenderer::getInstance().playFx(bct->getStartSound(), unit->getCurrMidHeightVector(), gameCamera->getPos());
                        }
            ...
            if (frameIndex < 0) {
                ...
                if (builtUnit != NULL && builtUnit != command->getUnit()) {
                    ...
                    if (unit->getFactionIndex() == world->getThisFactionIndex() ||
                        (game->getWorld()->showWorldForPlayer(game->getWorld()->getThisTeamIndex()) == true)) {
                        SoundRenderer::getInstance().playFx(bct->getBuiltSound(), unit->getCurrMidHeightVector(), gameCamera->getPos());
                    }
                }
            }
            if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
            ...
            // NULL)); if(hct != NULL) {
            {
                ...
                if (cell != NULL && cell->getUnit(unit->getCurrField()) != NULL) {
                    ...
                    if (targetUnit != NULL) {
                        ...
                        if (targetUnit->getFactionIndex() == unit->getFactionIndex() && targetUnit->isOperative() == true && unit->getLoadType() != NULL &&
                            ...
                            if (previousHarvestCmd != NULL) {
                                ...
                                NetworkCommand networkCommand(this->world, nctGiveCommand, unit->getId(), previousHarvestCmd->getId(),
                                                              unit->getLastHarvestedResourcePos(), -1, Unit::invalidId, -1, false, cst_None, -1, -1);
                                ...
                                if (cr.first == crSuccess) {
                                    ...
                                    unit->setCurrSkill(previousHarvestCmd->getStopLoadedSkillType()); // make sure we use the
                                                                                                      // right harvest
                                                                                                      // animation
                                } else {
...
void UnitUpdater::updateHarvest(Unit *unit, int frameIndex) {
    try {
        ...
        if (unit->getCurrSkill() != NULL && unit->getCurrSkill()->getClass() != scHarvest) {
            bool forceReturnToStore = (command != NULL && command->getStateType() == cst_EmergencyReturnResource && command->getStateValue() == 1);

            // if not working
            if (unit->getLoadCount() == 0 ||
                (forceReturnToStore == false && unit->getLoadType() != NULL && harvestResource != NULL && (unit->getLoadCount() < hct->getMaxLoad()) &&
                    ...
                    if (r != NULL && hct != NULL && hct->canHarvest(r->getType())) {
                        ...
                        case pfBasic: {
                            ...
                            if (isNearResource == true) {
                                if ((unit->getPos().dist(command->getPos()) < harvestDistance || unit->getPos().dist(targetPos) < harvestDistance) &&
                                    isNearResource == true) {
                        ...
                        if (canHarvestDestPos == true) {
                            ...
                            canHarvestDestPos = (map->getSurfaceCell(Map::toSurfCoords(targetPos)) != NULL &&
                                                 map->getSurfaceCell(Map::toSurfCoords(targetPos))->getResource() != NULL &&
                                                 map->getSurfaceCell(Map::toSurfCoords(targetPos))->getResource()->getType() != NULL);
                            ...
                            if (canHarvestDestPos == true) {
                                if (frameIndex < 0) {
                                    ...
                                    if (unit->getLoadType() == NULL || harvestResource == NULL || harvestResource->getType() == NULL ||
                        ...
                        if (canHarvestDestPos == false) {
                            ...
                            if ((wasStuck == true || tsValue == tsBlocked) && unit->isAlive() == true) {
                                ...
                                case pfBasic: {
                                    bool isNearResource =
                                        map->isResourceNear(frameIndex, unit->getPos(), r->getType(), targetPos, unit->getType()->getSize(), unit, true);
                                    if (isNearResource == true) {
                                        if ((unit->getPos().dist(command->getPos()) < harvestDistance || unit->getPos().dist(targetPos) < harvestDistance) &&
                                            isNearResource == true) {
                                            canHarvestDestPos = true;
                    ...
                    if (frameIndex < 0) {
                        ...
                        case tsMoving: {
                            if (hct->canHarvest(unit->getLoadType()) == false) {
                                ...
                                if (previousHarvestCmd != NULL) {
                                    // printf("\n\n#1\n\n");
                                    unit->setCurrSkill(previousHarvestCmd->getMoveLoadedSkillType()); // make sure we use
                                                                                                      // the right harvest
                                                                                                      // animation
                                } else {
                                    // printf("\n\n#2\n\n");
            ...
            if (frameIndex < 0) {
                ...
                if (r != NULL) {
                    if (hct->canHarvest(r->getType()) == false || r->getType()->getName() != unit->getLoadType()->getName()) {
                        // hct has changed to a different harvest command.
                        if (r->getType()->getName() != unit->getLoadType()->getName()) {
                            ...
                            unit->setCurrSkill(hct->getStopLoadedSkillType()); // this is actually the wrong
                                                                               // animation
                        }
                        unit->getPath()->clear();
...
void UnitUpdater::SwapActiveCommandState(Unit *unit, CommandStateType commandStateType, const CommandType *commandType, int originalValue, int newValue) {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d]\n", __FILE__, __FUNCTION__, __LINE__);
                ...
                if (peerUnit != NULL) {
                    if (peerUnit->getCommandSize() > 0) {
                        ...
                        if (peerCommand != NULL) {
                            ...
                            if (peerCommand->getStateType() == commandStateType && peerCommand->getStateValue() == originalValue) {
                                if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
                                    SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d]\n", __FILE__, __FUNCTION__, __LINE__);
...
Unit *UnitUpdater::findPeerUnitBuilder(Unit *unit) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] returning foundUnitBuilder = [%s]\n", __FILE__, __FUNCTION__, __LINE__,
                                 (foundUnitBuilder != NULL ? foundUnitBuilder->toString(false).c_str() : "null"));
    ...
    try {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] unit doing the repair [%s] - %d\n", __FILE__, __FUNCTION__, __LINE__,
                                     unit->getFullName(false).c_str(), unit->getId());
        //}
        Command *command = unit->getCurrCommand();
        if (command == NULL) {
            throw megaglest_runtime_error("command == NULL");
        ...
        if (repaired != NULL) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] unit to repair [%s] - %d\n", __FILE__, __FUNCTION__, __LINE__,
                                         repaired->getFullName(false).c_str(), repaired->getId());
        }
        ...
        if (peerUnitBuilder != NULL) {
            SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] unit peer [%s] - %d\n", __FILE__, __FUNCTION__, __LINE__,
                                     peerUnitBuilder->getFullName(false).c_str(), peerUnitBuilder->getId());
        }
            ...
            if (peerUnitBuilder->getCurrCommand()->getUnit() != NULL) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] peerbuilder's unitid = %d\n", __FILE__, __FUNCTION__,
                                             __LINE__, peerUnitBuilder->getCurrCommand()->getUnit()->getId());
        ...
        // UnitPathInterface *path= unit->getPath();
        if (unit->getCurrSkill()->getClass() != scRepair || (nextToRepaired == false && peerUnitBuilder == NULL)) {
            if (command == NULL) {
                throw megaglest_runtime_error("command == NULL");
            }
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
                SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] repairPos = %s, startRepairing = %d\n", __FILE__, __FUNCTION__,
                                         __LINE__, repairPos.getString().c_str(), startRepairing);
                ...
                if (nextToRepaired == true) {
                    ...
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true && frameIndex < 0) {
                        ...
                        snprintf(szBuf, 8096,
                                 ...
                                 "[%s] repairPos [%s]",
                                 unit->getPos().getString().c_str(), command->getPos().getString().c_str(), repairPos.getString().c_str());
                        unit->logSynchData(__FILE__, __LINE__, szBuf);
                    }
            ...
            if (cancelRepair == false && (repaired == NULL || repaired->repair()) && peerUnitBuilder == NULL) {
                if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] about to call [scStop]\n", __FILE__, __FUNCTION__, __LINE__);
    ...
    try {
        ...
        if (unit->getCurrSkill()->getClass() != scProduce) {
            ...
            if (unit->getProgress2() > pct->getProduced()->getProductionTime()) {
                ...
                produced = new Unit(world->getNextUnitId(unit->getFaction()), newpath, Vec2i(0), pct->getProducedUnit(), unit->getFaction(), world->getMap(),
                                    CardinalDir(CardinalDir::NORTH));

                if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
                    SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] about to place unit for unit [%s]\n", __FILE__, __FUNCTION__,
                                             __LINE__, produced->toString(false).c_str());
                ...
                if (!world->placeUnit(unit->getCenteredPos(), 10, produced, false, frameIndex < 0)) {
                    if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugUnitCommands, "In [%s::%s Line: %d] COULD NOT PLACE UNIT for unitID [%d]\n", __FILE__,
                                                 __FUNCTION__, __LINE__, produced->getId());
                    delete produced;
                } else {
                    produced->create();
                    produced->born(ct);
...
void UnitUpdater::hit(Unit *attacker, const AttackSkillType *ast, const Vec2i &targetPos, Field targetField, int damagePercent) {
    ...
    if (ast != NULL && ast->getSplash()) {
        ...
        snprintf(szBuf, 8095,
                 ...
                 "damageall = %d",
                 ast->getSplash(), ast->getSplashRadius(), ast->getSplashDamageAll());
        ...
        while (pci.next()) {
            ...
            if (attacked != NULL) {
                if (ast->getSplashDamageAll() || attacker->isAlly(attacked) == false || (targetPos.x == pci.getPos().x && targetPos.y == pci.getPos().y)) {
...
void UnitUpdater::damage(Unit *attacker, const AttackSkillType *ast, Unit *attacked, float distance, int damagePercent) {
    ...
    if (attacked->decHp(damageVal)) {
        world->getStats()->kill(attacker->getFactionIndex(), attacked->getFactionIndex(), attacker->getTeam() != attacked->getTeam(),
                                attacked->getType()->getCountUnitDeathInStats(), attacked->getType()->getCountUnitKillInStats());
            ...
            for (int i = 0; i < lootableResourceCount; i++) {
                ...
                // in the negatives
                    ...
                    attacked->getFaction()->incResourceAmount(resource.getResourceType(),
                                                              -(std::min)(factionTotalResource * resource.getLossFactionPercent() / 100, factionTotalResource));
                    ...
                    attacker->getFaction()->incResourceAmount(
                        resource.getResourceType(), (std::min)(factionTotalResource * resource.getAmountFactionPercent() / 100, factionTotalResource));
...
bool UnitUpdater::findCachedCellsEnemies(Vec2i center, int range, int size, vector<Unit *> &enemies, const AttackSkillType *ast, const Unit *unit,
...
void UnitUpdater::findEnemiesForCell(const AttackSkillType *ast, Cell *cell, const Unit *unit, const Unit *commandTarget, vector<Unit *> &enemies) {
    // all fields
    for (int k = 0; k < fieldCount; k++) {
        ...
        if ((ast == NULL || ast->getAttackField(f))) {
            ...
            if (possibleEnemy != NULL && possibleEnemy->isAlive()) {
                if ((unit->isAlly(possibleEnemy) == false && commandTarget == NULL) || commandTarget == possibleEnemy) {
    ...
    for (int k = 0; k < fieldCount; k++) {
        ...
        for (int i = pos.x - sightRange; i < pos.x + size + sightRange; ++i) {
            for (int j = pos.y - sightRange; j < pos.y + size + sightRange; ++j) {
                Vec2i testPos(i, j);
                if (map->isInside(testPos) && map->isInsideSurface(map->toSurfCoords(testPos))) {
...
bool UnitUpdater::unitOnRange(Unit *unit, int range, Unit **rangedPtr, const AttackSkillType *ast, bool evalMode) {
    bool result = false;
    ...
    try {
        ...
        if (findCachedCellsEnemies(center, range, size, enemies, ast, unit, commandTarget) == false) {
            // foundInCache = false;
            // nearby cells
            ...
            if (enemy != NULL && enemy->isAlive() == true) {
                ...
                if (enemy->getType()->hasSkillClass(scAttack) == true) {
                    ...
                    if (distToStandingUnit < 0 || currentDist < distToStandingUnit) {
                        if (enemy->getCurrSkill() != NULL && enemy->getCurrSkill()->getClass() == scAttack) {
                    ...
                    if (enemy->getCurrSkill() != NULL && enemy->getCurrSkill()->getClass() == scAttack) {
                        if (enemy->getId() == unit->getLastAttackedUnitId()) myFightingEnemyInRange = enemy;
        ...
        if (result == true) {
            ...
            if (evalMode == false && onlyEnemyUnits == false && enemyUnit->getTeam() != world->getThisTeamIndex()) {
...
string UnitUpdater::getUnitRangeCellsLookupItemCacheStats() {
    ...
    for (std::map<Vec2i, std::map<int, std::map<int, UnitRangeCellsLookupItem>>>::iterator iterMap1 = UnitRangeCellsLookupItemCache.begin();
        ...
        for (std::map<int, std::map<int, UnitRangeCellsLookupItem>>::iterator iterMap2 = iterMap1->second.begin(); iterMap2 != iterMap1->second.end();
             ++iterMap2) {
            sizeCount++;

            for (std::map<int, UnitRangeCellsLookupItem>::iterator iterMap3 = iterMap2->second.begin(); iterMap3 != iterMap2->second.end(); ++iterMap3) {
                rangeCount++;
    ...
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "pos [%d] size [%d] range [%d][%d] total KB: %s", posCount, sizeCount, rangeCount, rangeCountCellCount,
...
void ParticleDamager::update(ParticleSystem *particleSystem) {
    ...
    if (attacker != NULL) {
        ...
        if (projectileType->getSpawnUnit() != "" && projectileType->getSpawnUnitcount() > 0) {
            unitUpdater->spawnAttack(attacker, projectileType->getSpawnUnit(), projectileType->getSpawnUnitcount(), 100, 100, 100,
        ...
        if (projectileType->isShake() == true) {
            ...
            if (visibility && isInCameraView) {
                game->getGameCameraPtr()->shake(projectileType->getShakeDuration(), projectileType->getShakeIntensity(),
                                                projectileType->isShakeCameraDistanceAffected(), map->getSurfaceCell(surfaceTargetPos)->getVertex());
            }
        }
    }

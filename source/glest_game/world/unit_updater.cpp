...
void UnitUpdater::updateUnitCommand(Unit *unit, int frameIndex) {
  try {
    ...
    if ((minorDebugPerformance && frameIndex > 0) && chrono.getMillis() >= 1)
      printf(
          "UnitUpdate [%d - %s] #1-unit threaded updates on frame: %d took "
          "[%lld] msecs\n",
    ...
    if (hasCommand == true) {
      ...
      if ((minorDebugPerformance && frameIndex > 0) &&
        ...
        printf(
            "UnitUpdate [%d - %s] #2-unit threaded updates on frame: %d "
            "commandUsesPathFinder = %d took [%lld] msecs\nCommand: %s\n",
            unit->getId(), unit->getType()->getName(false).c_str(), frameIndex,
            commandUsesPathFinder, (long long int)chrono.getMillis() - elapsed1,
            unit->getCurrCommand()->toString(false).c_str());
      }
    }
    ...
    if ((minorDebugPerformance && frameIndex > 0) && chrono.getMillis() >= 1)
      printf(
          "UnitUpdate [%d - %s] #3-unit threaded updates on frame: %d took "
          "[%lld] msecs\n",
          unit->getId(), unit->getType()->getName(false).c_str(), frameIndex,
          (long long int)chrono.getMillis());
          ...
          case tsArrived: {
            ...
            switch (this->game->getGameSettings()->getPathFinderType()) {
              case pfBasic:
                ...
                canOccupyCell = map->isFreeCells(command->getPos(),
                                                 ut->getSize(), fLand, true);
                break;
              default:
                throw megaglest_runtime_error(
                    "detected unsupported pathfinder type!");
              ...
              case pfBasic: {
                ...
                if (isNearResource == true) {
                  ...
                  if (clickPos != command->getOriginalPos()) {
                    // printf("%%----------- unit [%s - %d] CHANGING RESOURCE
                    // POS from [%s] to
                ...
                if (canHarvestDestPos == false) {
                  ...
                  if (targetPos.x >= 0) {
                    ...
                    switch (
                        this->game->getGameSettings()->getPathFinderType()) {
                      case pfBasic:
                        tsValue = pathFinder->findPath(unit, targetPos,
                                                       &wasStuck, frameIndex);
                        if (tsValue == tsMoving && frameIndex < 0) {
                          unit->setCurrSkill(hct->getMoveSkillType());
                          command->setPos(targetPos);
            ...
            switch (tsValue) {
              case tsMoving: {
                if (hct->canHarvest(unit->getLoadType()) == false) {
                  ...
                  const HarvestCommandType *previousHarvestCmd =
                      unit->getType()->getFirstHarvestCommand(
                          unit->getLoadType(), unit->getFaction());
                  if (previousHarvestCmd != NULL) {
                    // printf("\n\n#1\n\n");
                    unit->setCurrSkill(
                        previousHarvestCmd
                            ->getMoveLoadedSkillType());  // make sure we use
                                                          // the right harvest
                                                          // animation
                  } else {
                    // printf("\n\n#2\n\n");
  ...
  try {
    ...
    if (unit->getCurrSkill()->getClass() != scRepair ||
      ...
      if (startRepairing == true) {
        ...
        // rct->getRepairCount(); ++i) { 				const
        // UnitType *rUnit = rct->getRepair(i);
        // printf("Can repair unittype
        //[%s]\n",rUnit->getName().c_str());
        //			}
      }
...
bool UnitUpdater::findCachedCellsEnemies(Vec2i center, int range, int size,
  ...
  if (iterFind != UnitRangeCellsLookupItemCache.end()) {
    std::map<int, std::map<int, UnitRangeCellsLookupItem> >::iterator
        iterFind3 = iterFind->second.find(size);
...
#else
          if (map->isInside(i, j) && floor(floatCenter.dist(Vec2f(
                                         (float)i, (float)j))) <= (range + 1)) {
#endif
...
string UnitUpdater::getUnitRangeCellsLookupItemCacheStats() {
  ...
  MutexSafeWrapper safeMutex(mutexUnitRangeCellsLookupItemCache,
                             string(__FILE__) + "_" + intToStr(__LINE__));
  for (std::map<Vec2i,
                std::map<int, std::map<int, UnitRangeCellsLookupItem> > >::

...
UnitUpdater::UnitUpdater()
    : mutexAttackWarnings(new Mutex(CODE_AT_LINE)),
...
UnitUpdater::~UnitUpdater() {
  ...
  MutexSafeWrapper safeMutex(mutexAttackWarnings,
...
bool UnitUpdater::updateUnit(Unit *unit) {
  ...
  for (SkillSoundList::const_iterator it =
           currSkill->getSkillSoundList()->begin();
       it != currSkill->getSkillSoundList()->end(); ++it) {
    float soundStartTime = (*it)->getStartTime();
    if (soundStartTime >= unit->getLastAnimProgressAsFloat() &&
        soundStartTime < unit->getAnimProgressAsFloat()) {
      if (map->getSurfaceCell(Map::toSurfCoords(unit->getPos()))
              ->isVisible(world->getThisTeamIndex()) ||
          (game->getWorld()->showWorldForPlayer(
               game->getWorld()->getThisTeamIndex()) == true)) {
        soundRenderer.playFx((*it)->getSoundContainer()->getRandSound(),
  ...
  if (currSkill->getShake()) {
    ...
    if (shakeStartTime >= unit->getLastAnimProgressAsFloat() &&
      ...
      bool visibility = (!visibleAffected) ||
      ...
      if (visibility && cameraAffected && enabled) {
        game->getGameCameraPtr()->shake(
            currSkill->getShakeDuration(), currSkill->getShakeIntensity(),
            cameraDistanceAffected, unit->getCurrMidHeightVector());
      }
    }
  }
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
    ...
    SystemFlags::OutputDebug(
        SystemFlags::debugPerformance,
        "In [%s::%s] Line: %d took msecs: %lld [after playsound]\n", __FILE__,
  ...
  if (unit->getCurrSkill()->getClass() == scDie) {
    const DieSkillType *dst =
        static_cast<const DieSkillType *>(unit->getCurrSkill());

    if (dst->getSpawn() == true) {
      float spawnStartTime =
          truncateDecimal<float>(dst->getSpawnStartTime(), 6);
      float lastAnimProgress =
          truncateDecimal<float>(unit->getLastAnimProgressAsFloat(), 6);
      float animProgress =
          truncateDecimal<float>(unit->getAnimProgressAsFloat(), 6);

      bool startSpawnNow =
          (spawnStartTime >= lastAnimProgress && spawnStartTime < animProgress);
      if (startSpawnNow) {
        //	spawn the units
        spawn(unit, dst->getSpawnUnit(), dst->getSpawnUnitCount(),
              dst->getSpawnUnitHealthPercentMin(),
              dst->getSpawnUnitHealthPercentMax(), dst->getSpawnProbability());
      }
    }
  }
  ...
  if (unit->getCurrSkill()->getClass() == scAttack) {
    const AttackSkillType *ast =
        static_cast<const AttackSkillType *>(unit->getCurrSkill());

    float attackStartTime =
        truncateDecimal<float>(ast->getAttackStartTime(), 6);
    float lastAnimProgress =
        truncateDecimal<float>(unit->getLastAnimProgressAsFloat(), 6);
    float animProgress =
        truncateDecimal<float>(unit->getAnimProgressAsFloat(), 6);
    ...
    if (ast->projectileTypes.empty() == true) {
      startAttackParticleSystemNow = (attackStartTime >= lastAnimProgress &&
                                      attackStartTime < animProgress);
    } else { // start projectile attack
      for (ProjectileTypes::const_iterator it = ast->projectileTypes.begin();
           it != ast->projectileTypes.end(); ++it) {
    ...
    char szBuf[8096] = "";
    snprintf(szBuf, 8095,
             "attackStartTime = %f, lastAnimProgress = %f, animProgress = %f "
             "startAttackParticleSystemNow = %d",
             attackStartTime, lastAnimProgress, animProgress,
             startAttackParticleSystemNow);
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
    ...
    SystemFlags::OutputDebug(
        SystemFlags::debugPerformance,
        "In [%s::%s] Line: %d took msecs: %lld [after unit->update()]\n",
        __FILE__, __FUNCTION__, __LINE__, chrono.getMillis());

  // printf("Update Unit [%d - %s] =
  ...
  if (update == true) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
      ...
      SystemFlags::OutputDebug(
          SystemFlags::debugPerformance,
          "In [%s::%s] Line: %d took msecs: %lld [after updateUnitCommand()]\n",
          __FILE__, __FUNCTION__, __LINE__, chrono.getMillis());
    ...
    if (unit->computeEp() == true) {
      ...
      if (unit->getCurrCommand() != NULL &&
        ...
        holdPositionName =
            unit->getCurrCommand()->getCommandType()->getName(false);
      }
        ...
        for (int i = 0; i < ut->getCommandTypeCount(); ++i) {
          ...
          if (ct != NULL && ct->getClass() == ccAttackStopped) {
            const AttackStoppedCommandType *act =
                static_cast<const AttackStoppedCommandType *>(ct);
            if (act != NULL && act->getName(false) == holdPositionName) {
              ...
              // printf("Re-Queing hold pos = %d, ep = %d skillep = %d skillname
              // [%s]\n
    ...
    if (unit->getCurrSkill() != NULL &&
      ...
      // unit->computeHp();
    } else if (unit->getCurrSkill() != NULL &&
               unit->getCurrSkill()->getClass() == scAttack) {
      const AttackSkillType *attackSkill =
          dynamic_cast<const AttackSkillType *>(unit->getCurrSkill());
      if (attackSkill != NULL && attackSkill->getSpawnUnit() != "" &&
          attackSkill->getSpawnUnitCount() > 0) {
        spawnAttack(unit, attackSkill->getSpawnUnit(),
                    attackSkill->getSpawnUnitCount(), 100, 100, 100,
                    attackSkill->getSpawnUnitAtTarget());
      }
    }
    ...
    if (unit->getCurrSkill()->getClass() == scMove) {
      ...
      if (map->getCell(unit->getPos())->getHeight() < map->getWaterLevel() &&
          unit->getCurrField() == fLand) {
        if (Config::getInstance().getBool("DisableWaterSounds", "false") ==
            false) {
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugPerformance,
                "In [%s::%s] Line: %d took msecs: %lld [after soundFx()]\n",
                __FILE__, __FUNCTION__, __LINE__, chrono.getMillis());
        }
      }
    }
...
void UnitUpdater::spawn(Unit *unit, string spawnUnit, int spawnUnitcount,
                        int healthMin, int healthMax, int probability) {
  if (spawnUnit != "" && spawnUnitcount > 0) {
    ...
    for (int y = 0; y < spawnCount; ++y) {
      ...
      if (spawned != NULL) {
        if (healthMin > 0 && healthMin < 100 && healthMax >= healthMin &&
            healthMax <= 100) {
          int damagePercent =
              100 - unit->getRandom()->randRange(healthMin, healthMax);
...
void UnitUpdater::spawnAttack(Unit *unit, string spawnUnit, int spawnUnitcount,
                              int healthMin, int healthMax, int probability,
                              bool spawnUnitAtTarget, Vec2i targetPos) {
  if (spawnUnit != "" && spawnUnitcount > 0) {
    ...
    for (int y = 0; y < spawnCount; ++y) {
      ...
      Unit *spawned = this->spawnUnit(
          unit, spawnUnit,
          spawnUnitAtTarget ? targetPos : unit->getCenteredPos());
      if (spawned != NULL) {
        if (healthMin > 0 && healthMin < 100 && healthMax >= healthMin &&
            healthMax <= 100) {
          int damagePercent =
        ...
        const CommandType *ct =
            spawned->getType()->getFirstAttackCommand(unit->getTargetField());
        if (ct == NULL) {
          ct = spawned->computeCommandType(
              targetPos,
              map->getCell(targetPos)->getUnit(unit->getTargetField()));
        }
        if (ct != NULL) {
...
Unit *UnitUpdater::spawnUnit(Unit *unit, string spawnUnit, Vec2i spawnPos) {
  ...
  if (spawnUnitType->getMaxUnitCount() > 0) {
    if (spawnUnitType->getMaxUnitCount() <=
        unit->getFaction()->getCountForMaxUnitCount(spawnUnitType)) {
      return NULL;
    }
  }
...
void UnitUpdater::updateUnitCommand(Unit *unit, int frameIndex) {
  try {
    ...
    Chrono chrono;
    if ((minorDebugPerformance == true && frameIndex > 0) ||
    ...
    if ((minorDebugPerformance && frameIndex > 0) && chrono.getMillis() >= 1)
      printf("UnitUpdate [%d - %s] #1-unit threaded updates on frame: %d took "
             "[%lld] msecs\n",
             unit->getId(), unit->getType()->getName(false).c_str(), frameIndex,
             (long long int)chrono.getMillis());

    int64 elapsed1 = 0;
    if (minorDebugPerformance && frameIndex > 0)
    ...
    if (hasCommand == true) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
        ...
        SystemFlags::OutputDebug(
            SystemFlags::debugUnitCommands,
            "In [%s::%s Line: %d] unit [%s] has command [%s]\n", __FILE__,
            __FUNCTION__, __LINE__, unit->toString(false).c_str(),
            unit->getCurrCommand()->toString(false).c_str());

      bool commandUsesPathFinder = (frameIndex < 0);
      if (frameIndex > 0) {
        if (unit->getCurrCommand() != NULL &&
            unit->getCurrCommand()->getCommandType() != NULL) {
          commandUsesPathFinder =
              unit->getCurrCommand()->getCommandType()->usesPathfinder();
      ...
      if ((minorDebugPerformance && frameIndex > 0) &&
        ...
        // CommandClass cc =
        // unit->getCurrCommand()->getCommandType()->commandTypeClass;
        printf("UnitUpdate [%d - %s] #2-unit threaded updates on frame: %d "
               "commandUsesPathFinder = %d took [%lld] msecs\nCommand: %s\n",
               unit->getId(), unit->getType()->getName(false).c_str(),
               frameIndex, commandUsesPathFinder,
    ...
    if (frameIndex < 0) {
      ...
      if (unit->anyCommand() == false && unit->isOperative()) {
        ...
        if (unit->getType()->hasCommandClass(ccStop)) {
          unit->giveCommand(
              new Command(unit->getType()->getFirstCtOfClass(ccStop)));
        }
      }
    }
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
      ...
      SystemFlags::OutputDebug(
          ...
          __FILE__, __FUNCTION__, __LINE__, chrono.getMillis());
    if ((minorDebugPerformance && frameIndex > 0) && chrono.getMillis() >= 1)
      printf("UnitUpdate [%d - %s] #3-unit threaded updates on frame: %d took "
             "[%lld] msecs\n",
             unit->getId(), unit->getType()->getName(false).c_str(), frameIndex,
             (long long int)chrono.getMillis());
...
void UnitUpdater::updateStop(Unit *unit, int frameIndex) {
  try {
    ...
    const StopCommandType *sct =
        static_cast<const StopCommandType *>(command->getCommandType());
    Unit *sighted = NULL;
    ...
    if (unit->getType()->hasSkillClass(scAttack)) {
      ...
      for (int i = 0; i < cmdTypeCount; ++i) {
        ...
        if (ct->getClass() == ccAttack) {
          ast =
              static_cast<const AttackCommandType *>(ct)->getAttackSkillType();
        } else if (ct->getClass() == ccAttackStopped) {
          ast = static_cast<const AttackStoppedCommandType *>(ct)
                    ->getAttackSkillType();
        }
    ...
    // see any unit and cant attack it => run
      ...
      if (attackerOnSight(unit, &sighted, (frameIndex >= 0))) {
        ...
        // SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,"In [%s::%s
        // Line: %d]\n",__FILE__,__FUNCTION__,__LINE__);
        unit->giveCommand(
            new Command(unit->getType()->getFirstCtOfClass(ccMove), escapePos));
      }
...
void UnitUpdater::updateMove(Unit *unit, int frameIndex) {
  try {
    ...
    const MoveCommandType *mct =
        static_cast<const MoveCommandType *>(command->getCommandType());

    Vec2i pos = command->getUnit() != NULL
                    ? command->getUnit()->getCenteredPos()
                    : command->getPos();
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
        ...
        frameIndex < 0) {
      char szBuf[8096] = "";
      snprintf(szBuf, 8096, "[updateMove] pos [%s] unit [%d - %s] cmd [%s]",
               pos.getString().c_str(), unit->getId(),
...
void UnitUpdater::updateAttack(Unit *unit, int frameIndex) {
  try {
    ...
    const AttackCommandType *act =
        static_cast<const AttackCommandType *>(command->getCommandType());
    if (act == NULL) {
    ...
    if ((command->getUnit() == NULL || !(command->getUnit()->isAlive())) &&
        unit->getCommandSize() > 1) {

      if (frameIndex < 0) {
        unit->finishCommand(); // all queued "ground attacks" are skipped if
                               // somthing else is queued after them.
    ...
    // if(frameIndex < 0) {
    {
      if (attackableOnRange(unit, &target, act->getAttackSkillType(),
        ...
        // if(frameIndex < 0) {
        {
          // printf("In [%s::%s Line: %d] START pathfind for attacker [%d -
          // %s]\n",__FILE__,__FUNCTION__,__LINE__,unit->getId(),
          // unit->getType()->getName().c_str()); fflush(stdout);
          switch (this->game->getGameSettings()->getPathFinderType()) {
          case pfBasic:
            tsValue = pathFinder->findPath(unit, pos, NULL, frameIndex);
            break;
          default:
            throw megaglest_runtime_error(
                "detected unsupported pathfinder type!");
          }
          // printf("In [%s::%s Line: %d] END pathfind for attacker [%d -
          // %s]\n",__FILE__,__FUNCTION__,__LINE__,unit->getId(),
          // unit->getType()->getName().c_str()); fflush(stdout);
        }
        ...
        if (frameIndex < 0) {
          if (command->getUnit() != NULL && !command->getUnit()->isAlive() &&
              unit->getCommandSize() > 1) {
            // don't run over to dead body if there is still something to do in
            // the queue
            unit->finishCommand();
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
                ...
                SystemFlags::getSystemSettingType(
                    SystemFlags::debugWorldSynchMax)
                        .enabled == true) {
              char szBuf[8096] = "";
            ...
            /*
                                                    case tsMoving:
                                                            ...
                                                            {
                                                                    std::pair<bool,Unit
               *> beingAttacked = unitBeingAttacked(unit);
                                                                    if(beingAttacked.first
               == true) { Unit *enemy = beingAttacked.second; const
               AttackCommandType *act_forenemy =
               unit->getType()->getFirstAttackCommand(enemy->getCurrField());
                                                                            if(act_forenemy
               != NULL) { if(unit->getEp() >=
               act_forenemy->getAttackSkillType()->getEpCost()) {
               ...
               unit->getType()->getFirstAttackStoppedCommand(enemy->getCurrField());
  ...
  try {
    ...
    if (frameIndex >= 0) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
        ...
        unit->logSynchDataThreaded(
            extractFileFromDirectoryPath(__FILE__).c_str(), __LINE__, szBuf);
      }
      clearUnitPrecache(unit);
      return;
    }
    ...
    const AttackStoppedCommandType *asct =
        static_cast<const AttackStoppedCommandType *>(
    ...
    if (unit->getCommandSize() > 1) {
      ...
      unit->finishCommand(); // attackStopped is skipped if somthing else is
                             // queued after this.
      return;
    }
...
void UnitUpdater::unitBeingAttacked(std::pair<bool, Unit *> &result,
                                    const Unit *unit,
                                    const AttackSkillType *ast,
                                    float *currentDistToUnit) {
  // std::pair<bool,Unit *> result = make_pair(false,(Unit *)NULL);
  ...
  //	if(result.first == true) {
  //		printf("~~~~~~~~ Unit [%s - %d] found enemy [%s - %d] distToUnit
  //=
...
void UnitUpdater::updateBuild(Unit *unit, int frameIndex) {
  try {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
      ...
      SystemFlags::OutputDebug(
          SystemFlags::debugUnitCommands,
          "In [%s::%s Line: %d] unit [%s] will build using command [%s]\n",
    ...
    const BuildCommandType *bct =
        static_cast<const BuildCommandType *>(command->getCommandType());

    if (unit->getCurrSkill() != NULL &&
        unit->getCurrSkill()->getClass() != scBuild) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
              .enabled)
        SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                                 "In [%s::%s Line: %d]\n", __FILE__,
      ...
      case pfBasic: {
        Vec2i buildPos =
            map->findBestBuildApproach(unit, command->getPos(), ut);
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
          ...
          char szBuf[8096] = "";
          snprintf(szBuf, 8096,
                   "[updateBuild] unit->getPos() [%s] command->getPos() [%s] "
                   "buildPos [%s]",
                   unit->getPos().getString().c_str(),
                   command->getPos().getString().c_str(),
                   buildPos.getString().c_str());
          unit->logSynchData(__FILE__, __LINE__, szBuf);
        }
        ...
        case tsArrived: {
          ...
          case pfBasic:
            if (SystemFlags::getSystemSettingType(
              ...
              SystemFlags::OutputDebug(
                  SystemFlags::debugUnitCommands,
                  "In [%s::%s Line: %d] tsArrived about to call "
                  "map->isFreeCells() for command->getPos() = %s, "
                  "ut->getSize() = %d\n",
                  __FILE__, __FUNCTION__, __LINE__,
                  command->getPos().getString().c_str(), ut->getSize());
            canOccupyCell =
                map->isFreeCells(command->getPos(), ut->getSize(), fLand);
            break;
          default:
            throw megaglest_runtime_error(
                "detected unsupported pathfinder type!");
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugUnitCommands,
                "In [%s::%s Line: %d] canOccupyCell = %d\n", __FILE__,
                __FUNCTION__, __LINE__, canOccupyCell);
          ...
          if (canOccupyCell == true) {
            ...
            Unit *builtUnit = new Unit(
                world->getNextUnitId(unit->getFaction()), newpath, buildPos,
                builtUnitType, unit->getFaction(), world->getMap(), facing);
            ...
            if (builtUnitType->hasSkillClass(scBeBuilt) == false) {
              throw megaglest_runtime_error(
                  "Unit [" + builtUnitType->getName(false) +
                  "] has no be_built skill, producer was [" +
                  intToStr(unit->getId()) + " - " +
                  unit->getType()->getName(false) + "].");
            }
            ...
            if (unit->getFactionIndex() == world->getThisFactionIndex() ||
              ...
              SoundRenderer::getInstance().playFx(
                  bct->getStartSound(), unit->getCurrMidHeightVector(),
                  gameCamera->getPos());
            }
            ...
            if (SystemFlags::getSystemSettingType(
              ...
              SystemFlags::OutputDebug(
                  SystemFlags::debugUnitCommands,
                  "In [%s::%s Line: %d] unit created for unit [%s]\n", __FILE__,
                  __FUNCTION__, __LINE__, builtUnit->toString(false).c_str());
          } else {
            ...
            if (SystemFlags::getSystemSettingType(
              ...
              SystemFlags::OutputDebug(
                  SystemFlags::debugUnitCommands,
                  "In [%s::%s Line: %d] got BuildingNoPlace\n", __FILE__,
                  __FUNCTION__, __LINE__);
          }
        } break;
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
              .enabled)
        SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                                 "In [%s::%s Line: %d] tsArrived unit = %s\n",
                                 __FILE__, __FUNCTION__, __LINE__,
                                 unit->toString(false).c_str());

      if (frameIndex < 0) {
        // if building
        Unit *builtUnit = map->getCell(unit->getTargetPos())->getUnit(fLand);
        if (builtUnit == NULL) {
          builtUnit = map->getCell(unit->getTargetPos())
                          ->getUnitWithEmptyCellMap(fLand);
        }
        ...
        if (builtUnit != NULL) {
          if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
                  .enabled)
            SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                                     "In [%s::%s Line: %d] builtUnit = %s\n",
                                     __FILE__, __FUNCTION__, __LINE__,
                                     builtUnit->toString(false).c_str());
        }

        if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
                .enabled)
          SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                                   "In [%s::%s Line: %d] builtUnit = [%p]\n",
        ...
        if (builtUnit != NULL && builtUnit != command->getUnit()) {
          if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugUnitCommands,
                "In [%s::%s Line: %d] builtUnit is not the command's unit!\n",
                __FILE__, __FUNCTION__, __LINE__);
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
            ...
            SystemFlags::OutputDebug(
                SystemFlags::debugUnitCommands,
                "In [%s::%s Line: %d] builtUnit is NULL or ALREADY built\n",
                __FILE__, __FUNCTION__, __LINE__);
          ...
          if (unit->getFactionIndex() == world->getThisFactionIndex() ||
              (game->getWorld()->showWorldForPlayer(
                   game->getWorld()->getThisTeamIndex()) == true)) {
            SoundRenderer::getInstance().playFx(bct->getBuiltSound(),
                                                unit->getCurrMidHeightVector(),
                                                gameCamera->getPos());
          }
        }
        ...
        if (cell != NULL && cell->getUnit(unit->getCurrField()) != NULL) {
          ...
          if (targetUnit != NULL) {
            ...
            if (targetUnit->getFactionIndex() == unit->getFactionIndex() &&
                ...
                unit->getLoadType() != NULL && targetUnit->getType() != NULL &&
                targetUnit->getType()->getStore(unit->getLoadType()) > 0) {
              // printf("\n#6 updateHarvestEmergencyReturn\n");

              const HarvestCommandType *previousHarvestCmd =
                  unit->getType()->getFirstHarvestCommand(unit->getLoadType(),
                                                          unit->getFaction());
              if (previousHarvestCmd != NULL) {
                // printf("\n\n#1a return harvested resources\n\n");
                NetworkCommand networkCommand(
                    this->world, nctGiveCommand, unit->getId(),
                    previousHarvestCmd->getId(),
                    unit->getLastHarvestedResourcePos(), -1, Unit::invalidId,
                    -1, false, cst_None, -1, -1);
                ...
                Command *new_command =
                    this->game->getCommander()->buildCommand(&networkCommand);
                new_command->setStateType(cst_EmergencyReturnResource);
                new_command->setStateValue(1);
                std::pair<CommandResult, string> cr =
                    unit->checkCommand(new_command);
                if (cr.first == crSuccess) {
                  ...
                  unit->setCurrSkill(
                      previousHarvestCmd
                          ->getStopLoadedSkillType()); // make sure we use the
                                                       // right harvest
                                                       // animation
                } else {
...
void UnitUpdater::updateHarvest(Unit *unit, int frameIndex) {
  try {
    ...
    const HarvestCommandType *hct =
        dynamic_cast<const HarvestCommandType *>(command->getCommandType());
    if (hct == NULL) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
      ...
      SystemFlags::OutputDebug(SystemFlags::debugPerformance,
                               ...
                               __FILE__, __FUNCTION__, __LINE__,
                               chrono.getMillis());
    // printf("In UpdateHarvest [%d - %s] unit->getCurrSkill()->getClass() =
    ...
    if (unit->getCurrSkill() != NULL &&
        unit->getCurrSkill()->getClass() != scHarvest) {
      bool forceReturnToStore =
          (command != NULL &&
           command->getStateType() == cst_EmergencyReturnResource &&
           command->getStateValue() == 1);

      // if not working
      if (unit->getLoadCount() == 0 ||
          (forceReturnToStore == false && unit->getLoadType() != NULL &&
           harvestResource != NULL &&
           (unit->getLoadCount() < hct->getMaxLoad()) &&
           harvestResource->getType() != NULL &&
           unit->getLoadType() == harvestResource->getType())) {
        // if not loaded go for resources
        SurfaceCell *sc =
            map->getSurfaceCell(Map::toSurfCoords(command->getPos()));
        if (sc != NULL) {
          ...
          if (r != NULL && hct != NULL && hct->canHarvest(r->getType())) {
            ...
            case pfBasic: {
              ...
              if (newHarvestPath == true) {
                isNearResource = map->isResourceNear(
                    frameIndex, unit->getPos(), r->getType(), targetPos,
                    unit->getType()->getSize(), unit, false, &clickPos);
              } else {
                isNearResource = map->isResourceNear(
                    frameIndex, unit->getPos(), r->getType(), targetPos,
                    unit->getType()->getSize(), unit);
              }
              if (isNearResource == true) {
                if ((unit->getPos().dist(command->getPos()) < harvestDistance ||
                     unit->getPos().dist(targetPos) < harvestDistance) &&
                    isNearResource == true) {
                  canHarvestDestPos = true;
                }
              } else if (newHarvestPath == true) {
                if (clickPos != command->getOriginalPos()) {
                  // printf("%%----------- unit [%s - %d] CHANGING RESOURCE POS
                  // from [%s] to
                  ...
                  if (frameIndex < 0) {
                    if (SystemFlags::getSystemSettingType(
                        ...
                        frameIndex < 0) {
                      char szBuf[8096] = "";
                      snprintf(szBuf, 8096, "[updateHarvest] clickPos [%s]",
                               clickPos.getString().c_str());
                      unit->logSynchData(
            ...
            if (canHarvestDestPos == true) {
              ...
              canHarvestDestPos =
                  (map->getSurfaceCell(Map::toSurfCoords(targetPos)) != NULL &&
                   map->getSurfaceCell(Map::toSurfCoords(targetPos))
                           ->getResource() != NULL &&
                   map->getSurfaceCell(Map::toSurfCoords(targetPos))
                           ->getResource()
                           ->getType() != NULL);
              ...
              if (canHarvestDestPos == true) {
                if (frameIndex < 0) {
                  ...
                  if (SystemFlags::getSystemSettingType(
                      ...
                      frameIndex < 0) {
                    char szBuf[8096] = "";
                    snprintf(szBuf, 8096, "[updateHarvest] targetPos [%s]",
            ...
            if (canHarvestDestPos == false) {
              ...
              if ((wasStuck == true || tsValue == tsBlocked) &&
                  unit->isAlive() == true) {
                switch (this->game->getGameSettings()->getPathFinderType()) {
                case pfBasic: {
                  bool isNearResource = map->isResourceNear(
                      frameIndex, unit->getPos(), r->getType(), targetPos,
                      unit->getType()->getSize(), unit, true);
                  if (isNearResource == true) {
                    if ((unit->getPos().dist(command->getPos()) <
                             harvestDistance ||
                         unit->getPos().dist(targetPos) < harvestDistance) &&
                        isNearResource == true) {
                      canHarvestDestPos = true;
                ...
                if (canHarvestDestPos == true) {
                  ...
                  if (canHarvestDestPos == true) {
                    if (frameIndex < 0) {
                      ...
                      switch (
                          this->game->getGameSettings()->getPathFinderType()) {
                      case pfBasic:
                        unit->setLoadType(r->getType());
                        break;
                      default:
                ...
                if (canHarvestDestPos == false) {
                  ...
                  if (targetPos.x >= 0) {
                    ...
                    if (SystemFlags::getSystemSettingType(
                      ...
                      char szBuf[8096] = "";
                      snprintf(szBuf, 8096,
                               "[updateHarvest #2] unit->getPos() [%s] "
                               "command->getPos() [%s] targetPos [%s]",
                               unit->getPos().getString().c_str(),
                               command->getPos().getString().c_str(),
                               targetPos.getString().c_str());
                      unit->logSynchData(__FILE__, __LINE__, szBuf);
        ...
        Unit *store =
            world->nearestStore(unit->getPos(), unit->getFaction()->getIndex(),
                                unit->getLoadType());
        if (store != NULL) {
          if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
                      .enabled == true &&
            ...
            char szBuf[8096] = "";
            snprintf(szBuf, 8096,
                     "[updateHarvest #3] unit->getPos() [%s] "
                     "store->getCenteredPos() [%s]",
                     unit->getPos().getString().c_str(),
                     store->getCenteredPos().getString().c_str());
            unit->logSynchData(__FILE__, __LINE__, szBuf);
          }
          ...
          switch (this->game->getGameSettings()->getPathFinderType()) {
          case pfBasic:
            tsValue = pathFinder->findPath(unit, store->getCenteredPos(), NULL,
                                           frameIndex);
            break;
          default:
            throw megaglest_runtime_error(
          ...
          if (frameIndex < 0) {
            ...
            case tsMoving: {
              if (hct->canHarvest(unit->getLoadType()) == false) {
                ...
                if (previousHarvestCmd != NULL) {
                  ...
                  unit->setCurrSkill(
                      previousHarvestCmd
                          ->getMoveLoadedSkillType()); // make sure we use the
                                                       // right harvest
                                                       // animation
                } else {
                  // printf("\n\n#2\n\n");
            ...
            if (map->isNextTo(unit, store)) {
              ...
              if (unit->getFaction()->getCpuControl()) {
                int resourceMultiplierIndex =
                    game->getGameSettings()->getResourceMultiplierIndex(
                        unit->getFaction()->getIndex());
                resourceAmount =
                    (resourceAmount * (resourceMultiplierIndex + 5)) / 10;
              }
              unit->getFaction()->incResourceAmount(unit->getLoadType(),
                                                    resourceAmount);
              world->getStats()->harvest(unit->getFactionIndex(),
                                         resourceAmount);
        ...
        if (r != NULL) {
          if (hct->canHarvest(r->getType()) == false ||
            ...
            if (r->getType()->getName() != unit->getLoadType()->getName()) {
              ...
              unit->setCurrSkill(
                  hct->getStopLoadedSkillType()); // this is actually the wrong
                                                  // animation
            }
            unit->getPath()->clear();
...
void UnitUpdater::SwapActiveCommandState(Unit *unit,
      ...
      for (int i = 0; i < unit->getFaction()->getUnitCount(); ++i) {
        ...
        if (peerUnit != NULL) {
          if (peerUnit->getCommandSize() > 0) {
            ...
            // BuildCommandType*>(peerCommand->getCommandType()); if(bct !=
            // NULL) {
            if (peerCommand != NULL) {
              if (SystemFlags::getSystemSettingType(
                      SystemFlags::debugUnitCommands)
...
Unit *UnitUpdater::findPeerUnitBuilder(Unit *unit) {
  ...
  if (unit->getCommandSize() > 0) {
    ...
    if (command != NULL) {
      const RepairCommandType *rct =
          dynamic_cast<const RepairCommandType *>(command->getCommandType());
      if (rct != NULL && command->getStateType() == cst_linkedUnit) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
                .enabled)
          SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                                   "In [%s::%s Line: %d] looking for "
                                   "command->getStateValue() = %d\n",
                                   __FILE__, __FUNCTION__, __LINE__,
            ...
            if (peerUnit->getCommandSize() > 0) {
              ...
              if (bct != NULL) {
                ...
                if (SystemFlags::getSystemSettingType(
                        ...
                        .enabled)
                  SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                                           "In [%s::%s Line: %d] **peer NOT "
                                           "building**, peerUnit = [%s]\n",
                                           __FILE__, __FUNCTION__, __LINE__,
                                           peerUnit->toString(false).c_str());

                if (firstLinkedPeerRepairer == NULL) {
                  const RepairCommandType *prct =
                      dynamic_cast<const RepairCommandType *>(
                          peerCommand->getCommandType());
                  if (prct != NULL) {
                    if (SystemFlags::getSystemSettingType(
                            SystemFlags::debugUnitCommands)
                    ...
                    if (unit->getId() != peerUnit->getId() &&
                        command->getStateValue() == peerUnit->getId()) {
                      if (SystemFlags::getSystemSettingType(
                              SystemFlags::debugUnitCommands)
                              .enabled)
                        SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands).enabled)
    SystemFlags::OutputDebug(
        SystemFlags::debugUnitCommands,
        "In [%s::%s Line: %d] returning foundUnitBuilder = [%s]\n", __FILE__,
        __FUNCTION__, __LINE__,
        (foundUnitBuilder != NULL ? foundUnitBuilder->toString(false).c_str()
                                  : "null"));
...
void UnitUpdater::updateRepair(Unit *unit, int frameIndex) {
  try {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
      ...
      SystemFlags::OutputDebug(
          SystemFlags::debugUnitCommands,
          "In [%s::%s Line: %d] unit doing the repair [%s] - %d\n", __FILE__,
          __FUNCTION__, __LINE__, unit->getFullName(false).c_str(),
          unit->getId());
    //}
    Command *command = unit->getCurrCommand();
    if (command == NULL) {
    ...
    const RepairCommandType *rct =
        static_cast<const RepairCommandType *>(command->getCommandType());
    const CommandType *ct =
        (command != NULL ? command->getCommandType() : NULL);
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
      ...
      SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                               "In [%s::%s Line: %d] rct = %p\n", __FILE__,
                               __FUNCTION__, __LINE__, rct);

    Unit *repaired =
        (command != NULL
             ? map->getCell(command->getPos())->getUnitWithEmptyCellMap(fLand)
             : NULL);
    ...
    if (repaired != NULL) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
        ...
        SystemFlags::OutputDebug(
            SystemFlags::debugUnitCommands,
            "In [%s::%s Line: %d] unit to repair [%s] - %d\n", __FILE__,
            __FUNCTION__, __LINE__, repaired->getFullName(false).c_str(),
            repaired->getId());
    }
    ...
    if (peerUnitBuilder != NULL) {
      SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                               "In [%s::%s Line: %d] unit peer [%s] - %d\n",
                               __FILE__, __FUNCTION__, __LINE__,
                               peerUnitBuilder->getFullName(false).c_str(),
                               peerUnitBuilder->getId());
    }
    ...
    if (peerUnitBuilder != NULL) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
              .enabled)
        SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                                 "In [%s::%s Line: %d] peerUnitBuilder = %p\n",
                                 __FILE__, __FUNCTION__, __LINE__,
                                 peerUnitBuilder);
      ...
      if (peerUnitBuilder->getCurrCommand()->getUnit() != NULL) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugUnitCommands,
              "In [%s::%s Line: %d] peerbuilder's unitid = %d\n", __FILE__,
              __FUNCTION__, __LINE__,
              peerUnitBuilder->getCurrCommand()->getUnit()->getId());
    ...
    if (repaired == NULL) {
      ...
      if (peerUnitBuilder != NULL) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugUnitCommands,
              "In [%s::%s Line: %d] peerUnitBuilder = %p\n", __FILE__,
              __FUNCTION__, __LINE__, peerUnitBuilder);
        ...
        if (peerUnitBuilder->getCurrCommand()->getUnit() != NULL) {
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
            ...
            SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                                     "In [%s::%s Line: %d]\n", __FILE__,
                                     __FUNCTION__, __LINE__);
          Vec2i buildPos = map->findBestBuildApproach(
              unit, command->getPos(),
              peerUnitBuilder->getCurrCommand()->getUnitType());
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
                  .enabled)
            SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                                     "In [%s::%s Line: %d] peerUnitBuilder = "
                                     "%p, nextToRepaired = %d\n",
                                     __FILE__, __FUNCTION__, __LINE__,
                                     peerUnitBuilder, nextToRepaired);
          ...
          if (nextToRepaired == true) {
            ...
            const RepairCommandType *rct =
                dynamic_cast<const RepairCommandType *>(
                    peerCommand->getCommandType());
            // If the peer is also scheduled to do a repair we CANNOT swap their
            // commands or it will result in a stack overflow as each swaps the
            // others repair command. We must convert this unit's repair into a
            // build right now!
            if (rct != NULL) {
              if (SystemFlags::getSystemSettingType(
                      SystemFlags::debugUnitCommands)
              ...
              const CommandType *ctbuild =
                  unit->getType()->getFirstCtOfClass(ccBuild);
              NetworkCommand networkCommand(
                  this->world, nctGiveCommand, unit->getId(), ctbuild->getId(),
                  command->getPos(), command->getUnitType()->getId(), -1,
                  CardinalDir(CardinalDir::NORTH), true,
                  command->getStateType(), command->getStateValue());
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
              .enabled)
        SystemFlags::OutputDebug(SystemFlags::debugUnitCommands,
                                 "In [%s::%s Line: %d] unit to repair[%s]\n",
                                 __FILE__, __FUNCTION__, __LINE__,
                                 repaired->getFullName(false).c_str());
    }
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
      ...
      SystemFlags::OutputDebug(
          SystemFlags::debugUnitCommands,
          "In [%s::%s Line: %d] repaired = %p, nextToRepaired = %d, "
          "unit->getCurrSkill()->getClass() = %d\n",
          __FILE__, __FUNCTION__, __LINE__, repaired, nextToRepaired,
          unit->getCurrSkill()->getClass());
    ...
    if (unit->getCurrSkill()->getClass() != scRepair ||
      ...
      bool startRepairing =
          (repaired != NULL && rct->isRepairableUnitType(repaired->getType()) &&
           repaired->isDamaged());

      if (startRepairing == true) {
        // printf("STARTING REPAIR, unit [%s - %d] for unit [%s -
        // %d]\n",unit->getType()->getName().c_str(),unit->getId(),repaired->getType()->getName().c_str(),repaired->getId());
        //			for(unsigned int i = 0; i <
        //rct->getRepairCount();
        //++i) { 				const UnitType *rUnit =
        //rct->getRepair(i); 				printf("Can repair
        // unittype [%s]\n",rUnit->getName().c_str());
        //			}
      }
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
        ...
        SystemFlags::OutputDebug(
            SystemFlags::debugUnitCommands,
            "In [%s::%s Line: %d] repairPos = %s, startRepairing = %d\n",
            __FILE__, __FUNCTION__, __LINE__, repairPos.getString().c_str(),
            startRepairing);
      ...
      if (startRepairing == false && peerUnitBuilder != NULL) {
        ...
        // No need to adjust repair pos since we already did this above via:
        // Vec2i buildPos = map->findBestBuildApproach(unit->getPos(),
        // command->getPos(), peerUnitBuilder->getCurrCommand()->getUnitType());
      ...
      if (startRepairing == true) {
        ...
        if (nextToRepaired == true) {
          ...
          if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
            ...
            char szBuf[8096] = "";
            snprintf(szBuf, 8096,
                     "[updateRepair] unit->getPos() [%s] command->getPos()() "
                     "[%s] repairPos [%s]",
                     unit->getPos().getString().c_str(),
                     command->getPos().getString().c_str(),
                     repairPos.getString().c_str());
            unit->logSynchData(__FILE__, __LINE__, szBuf);
          }
          ...
          if (rct->getMoveSkillType() == NULL) {
            ...
            // Vec2i repairPos = command->getPos();
            // bool startRepairing = (repaired != NULL &&
            // rct->isRepairableUnitType(repaired->getType()) &&
            // repaired->isDamaged()); bool nextToRepaired = repaired != NULL &&
            // map->isNextTo(unit, repaired);

            // printf("repairPos [%s] startRepairing = %d nextToRepaired = %d
            // unit->getPos() [%s] repaired->getPos()
            ...
            case tsBlocked:
              ...
              if (unit->getPath()->isBlocked()) {
                if (SystemFlags::getSystemSettingType(
                  ...
                  SystemFlags::OutputDebug(
                      SystemFlags::debugUnitCommands,
                      "In [%s::%s Line: %d] about to call [scStop]\n", __FILE__,
                      __FUNCTION__, __LINE__);
                ...
                if (unit->getRetryCurrCommandCount() > 0) {
                  if (SystemFlags::getSystemSettingType(
                    ...
                    SystemFlags::OutputDebug(
                        SystemFlags::debugUnitCommands,
                        "In [%s::%s Line: %d] will retry command, "
                        "unit->getRetryCurrCommandCount() = %d\n",
                        __FILE__, __FUNCTION__, __LINE__,
                        unit->getRetryCurrCommandCount());
      ...
      if (repaired != NULL) {
        ...
        if (canStillRepair == true) {
          ...
          // printf("CANCELLING CURRENT REPAIR, unit [%s - %d] for unit [%s -
          // %d]\n",unit->getType()->getName().c_str(),unit->getId(),repaired->getType()->getName().c_str(),repaired->getId());
          //				for(unsigned int i = 0; i <
          //rct->getRepairCount();
          //++i) { 					const UnitType *rUnit =
          //rct->getRepair(i); 					printf("Can
          //repair unittype [%s]\n",rUnit->getName().c_str());
          //				}
          unit->setCurrSkill(scStop);
          unit->finishCommand();
...
void UnitUpdater::updateProduce(Unit *unit, int frameIndex) {
  try {
    ...
    const ProduceCommandType *pct =
        static_cast<const ProduceCommandType *>(command->getCommandType());
    Unit *produced;
    ...
    if (unit->getCurrSkill()->getClass() != scProduce) {
      ...
      if (unit->getProgress2() > pct->getProduced()->getProductionTime()) {
        ...
        produced =
            new Unit(world->getNextUnitId(unit->getFaction()), newpath,
                     Vec2i(0), pct->getProducedUnit(), unit->getFaction(),
                     world->getMap(), CardinalDir(CardinalDir::NORTH));
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugUnitCommands,
              "In [%s::%s Line: %d] about to place unit for unit [%s]\n",
              __FILE__, __FUNCTION__, __LINE__,
              produced->toString(false).c_str());
        ...
        if (!world->placeUnit(unit->getCenteredPos(), 10, produced, false,
                              frameIndex < 0)) {
          if (SystemFlags::getSystemSettingType(SystemFlags::debugUnitCommands)
                  .enabled)
            SystemFlags::OutputDebug(
                SystemFlags::debugUnitCommands,
                "In [%s::%s Line: %d] COULD NOT PLACE UNIT for unitID [%d]\n",
          ...
          world->getStats()->produce(
              unit->getFactionIndex(),
              produced->getType()->getCountUnitProductionInStats());
          const CommandType *ct =
              produced->computeCommandType(unit->getMeetingPos());
          if (ct != NULL) {
            if (SystemFlags::getSystemSettingType(
                    SystemFlags::debugUnitCommands)
                    .enabled)
...
void UnitUpdater::hit(Unit *attacker) {
  hit(attacker, dynamic_cast<const AttackSkillType *>(attacker->getCurrSkill()),
      attacker->getTargetPos(), attacker->getTargetField(), 100);
}

void UnitUpdater::hit(Unit *attacker, const AttackSkillType *ast,
                      const Vec2i &targetPos, Field targetField,
                      int damagePercent) {
  // hit attack positions
  if (ast != NULL && ast->getSplash()) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8095,
             "Unit hitting [UnitUpdater::hit] hasSplash = %d radius = %d "
             "damageall = %d",
             ast->getSplash(), ast->getSplashRadius(),
             ast->getSplashDamageAll());
    ...
    while (pci.next()) {
      ...
      if (attacked != NULL) {
        if (ast->getSplashDamageAll() || attacker->isAlly(attacked) == false ||
    ...
    char szBuf[8096] = "";
    snprintf(szBuf, 8095, "Unit hitting [UnitUpdater::hit 2] attacked = %d",
             (attacked != NULL ? attacked->getId() : -1));
    attacker->addNetworkCRCDecHp(szBuf);
...
void UnitUpdater::damage(Unit *attacker, const AttackSkillType *ast,
                         Unit *attacked, float distance, int damagePercent) {
  if (attacker == NULL) {
  ...
  float damageMultiplier = world->getTechTree()->getDamageMultiplier(
      ast->getAttackType(), attacked->getType()->getArmorType());
  damageMultiplier = truncateDecimal<float>(damageMultiplier, 6);

  // compute damage
  // damage += random.randRange(-var, var);
  damage += attacker->getRandom()->randRange(
      -var, var, extractFileFromDirectoryPath(__FILE__) + intToStr(__LINE__));
  damage /= distance + 1;
  damage -= armor;
  ...
  char szBuf[8096] = "";
  snprintf(szBuf, 8095, "Unit hitting [UnitUpdater::damage] damageVal = %d",
           damageVal);
  attacker->addNetworkCRCDecHp(szBuf);

  // damage the unit
  if (attacked->decHp(damageVal)) {
    world->getStats()->kill(attacker->getFactionIndex(),
                            attacked->getFactionIndex(),
                            attacker->getTeam() != attacked->getTeam(),
                            attacked->getType()->getCountUnitDeathInStats(),
                            attacked->getType()->getCountUnitKillInStats());
    ...
    if (attacker->getFaction() != attacked->getFaction()) {
      int lootableResourceCount =
          attacked->getType()->getLootableResourceCount();
      for (int i = 0; i < lootableResourceCount; i++) {
        LootableResource resource = attacked->getType()->getLootableResource(i);

        // Figure out how much of the resource in question that the attacked
        // unit's faction has
        int factionTotalResource = 0;
        for (int j = 0;
             j < attacked->getFaction()->getTechTree()->getResourceTypeCount();
             j++) {
          if (attacked->getFaction()->getTechTree()->getResourceType(j) ==
              resource.getResourceType()) {
            factionTotalResource =
                attacked->getFaction()->getResource(j)->getAmount();
            break;
          }
        }

        if (resource.isNegativeAllowed()) {
          attacked->getFaction()->incResourceAmount(
              resource.getResourceType(),
              -(factionTotalResource * resource.getLossFactionPercent() / 100));
          attacked->getFaction()->incResourceAmount(resource.getResourceType(),
                                                    -resource.getLossValue());
          attacker->getFaction()->incResourceAmount(
              resource.getResourceType(),
              factionTotalResource * resource.getAmountFactionPercent() / 100);
          attacker->getFaction()->incResourceAmount(resource.getResourceType(),
                                                    resource.getAmountValue());
        }
        // Can't take more resources than the faction has, otherwise we end up
        // in the negatives
        else {
          attacked->getFaction()->incResourceAmount(
              resource.getResourceType(),
              -(std::min)(factionTotalResource *
                              resource.getLossFactionPercent() / 100,
                          factionTotalResource));
          attacked->getFaction()->incResourceAmount(
              resource.getResourceType(),
              -(std::min)(resource.getLossValue(), factionTotalResource));
          attacker->getFaction()->incResourceAmount(
              resource.getResourceType(),
              (std::min)(factionTotalResource *
                             resource.getAmountFactionPercent() / 100,
                         factionTotalResource));
          attacker->getFaction()->incResourceAmount(
              resource.getResourceType(),
              (std::min)(resource.getAmountValue(), factionTotalResource));
        }
      }
    }
...
void UnitUpdater::startAttackParticleSystem(Unit *unit, float lastAnimProgress,
                                            float animProgress) {
  Renderer &renderer = Renderer::getInstance();

  // const AttackSkillType *ast= dynamic_cast<const
  // AttackSkillType*>(unit->getCurrSkill());
  const AttackSkillType *ast =
      static_cast<const AttackSkillType *>(unit->getCurrSkill());
  ...
  const SurfaceCell *sc =
      map->getSurfaceCell(Map::toSurfCoords(unit->getPos()));
  const SurfaceCell *tsc =
      map->getSurfaceCell(Map::toSurfCoords(unit->getTargetPos()));
  ...
  // for(ProjectileParticleSystemTypes::const_iterator pit=
  // unit->getCurrSkill()->projectileParticleSystemTypes.begin(); pit !=
  // unit->getCurrSkill()->projectileParticleSystemTypes.end(); ++pit) {
  for (ProjectileTypes::const_iterator pt = ast->projectileTypes.begin();
       pt != ast->projectileTypes.end(); ++pt) {
    bool startAttackParticleSystemNow =
        ((*pt)->getAttackStartTime() >= lastAnimProgress &&
         (*pt)->getAttackStartTime() < animProgress);
    if (startAttackParticleSystemNow) {
      ProjectileParticleSystem *psProj =
          (*pt)->getProjectileParticleSystemType()->create(unit);
      ...
      if (pstSplash != NULL) {
        ...
        if (unit->getFaction()->getTexture()) {
          psSplash->setFactionColor(
              unit->getFaction()->getTexture()->getPixmapConst()->getPixel3f(
                  0, 0));
        }
  ...
  for (int radius = 0; radius < maxResSearchRadius; radius++) {
    for (int i = pos.x - radius; i <= pos.x + radius; ++i) {
      for (int j = pos.y - radius; j <= pos.y + radius; ++j) {
        if (map->isInside(i, j)) {
          Resource *r = map->getSurfaceCell(Map::toSurfCoords(Vec2i(i, j)))
                            ->getResource();
          if (r != NULL) {
...
bool UnitUpdater::attackableOnSight(Unit *unit, Unit **rangedPtr,
                                    const AttackSkillType *ast, bool evalMode) {
  int range = unit->getType()->getTotalSight(unit->getTotalUpgrade());
  return unitOnRange(unit, range, rangedPtr, ast, evalMode);
}

bool UnitUpdater::attackableOnRange(Unit *unit, Unit **rangedPtr,
                                    const AttackSkillType *ast, bool evalMode) {
  int range = ast->getTotalAttackRange(unit->getTotalUpgrade());
  return unitOnRange(unit, range, rangedPtr, ast, evalMode);
}

bool UnitUpdater::findCachedCellsEnemies(Vec2i center, int range, int size,
                                         vector<Unit *> &enemies,
                                         const AttackSkillType *ast,
                                         const Unit *unit,
                                         const Unit *commandTarget) {
  bool result = false;
  MutexSafeWrapper safeMutex(mutexUnitRangeCellsLookupItemCache,
                             string(__FILE__) + "_" + intToStr(__LINE__));
  std::map<Vec2i,
           std::map<int, std::map<int, UnitRangeCellsLookupItem>>>::iterator
      iterFind = UnitRangeCellsLookupItemCache.find(center);

  if (iterFind != UnitRangeCellsLookupItemCache.end()) {
    std::map<int, std::map<int, UnitRangeCellsLookupItem>>::iterator iterFind3 =
        iterFind->second.find(size);
    if (iterFind3 != iterFind->second.end()) {
      std::map<int, UnitRangeCellsLookupItem>::iterator iterFind4 =
          iterFind3->second.find(range);
...
void UnitUpdater::findEnemiesForCell(const AttackSkillType *ast, Cell *cell,
                                     const Unit *unit,
                                     const Unit *commandTarget,
                                     vector<Unit *> &enemies) {
  // all fields
...
void UnitUpdater::findEnemiesForCell(const Vec2i pos, int size, int sightRange,
                                     const Faction *faction,
                                     vector<Unit *> &enemies,
                                     bool attackersOnly) const {
  // all fields
  for (int k = 0; k < fieldCount; k++) {
    Field f = static_cast<Field>(k);
            ...
            if (faction->getTeam() != possibleEnemy->getTeam()) {
              if (attackersOnly == true) {
                if (possibleEnemy->getType()->hasCommandClass(ccAttack) ||
                    possibleEnemy->getType()->hasCommandClass(
                        ccAttackStopped)) {
                  enemies.push_back(possibleEnemy);
                }
              } else {
...
bool UnitUpdater::unitOnRange(Unit *unit, int range, Unit **rangedPtr,
  ...
  try {
    ...
    if (unit->anyCommand() && unit->getCurrCommand() != NULL) {
      commandTarget =
          static_cast<const Unit *>(unit->getCurrCommand()->getUnit());
    }
    ...
    if (findCachedCellsEnemies(center, range, size, enemies, ast, unit,
                               commandTarget) == false) {
...
#ifdef USE_STREFLOP
          if (map->isInside(i, j) &&
              streflop::floor(static_cast<streflop::Simple>(floatCenter.dist(
                  Vec2f((float)i, (float)j)))) <= (range + 1)) {
#else
          if (map->isInside(i, j) &&
              floor(floatCenter.dist(Vec2f((float)i, (float)j))) <=
                  (range + 1)) {
#endif
    ...
    bool isUltra =
        controlType == ctCpuUltra || controlType == ctNetworkCpuUltra;
    ...
    for (int i = 0; i < (int)enemies.size(); ++i) {
      ...
      if (enemy != NULL && enemy->isAlive() == true) {
        ...
        // randomInfoData += " i = " + intToStr(i) + " alive = true result = " +
        // intToStr(result);
        ...
        if (enemy->getType()->hasSkillClass(scAttack) == true) {
          ...
          float currentDist =
              unit->getCenteredPos().dist(enemy->getCenteredPos());
          ...
          if (enemy->getCurrSkill() != NULL &&
            ...
            AttackSkillType *ast = (AttackSkillType *)(enemy->getCurrSkill());
            int enemyAttackRange =
                ast->getTotalAttackRange(unit->getTotalUpgrade());
      ...
      if (isMega) {
        if (myFightingEnemyInRange != NULL) {
          ...
          *rangedPtr = myFightingEnemyInRange;
          ...
          bool doit = unit->getRandom()->randRange(
                          0, 2,
    ...
    if (result == true) {
      ...
      if (evalMode == false && onlyEnemyUnits == false &&
        ...
        for (int i = (int)attackWarnings.size() - 1; i >= 0; --i) {
          if (world->getFrameCount() - attackWarnings[i]->lastFrameCount >
              200) { // after 200 frames attack break we warn again
            ...
            currentDistance = streflop::floor(
                static_cast<streflop::Simple>(enemyFloatCenter.dist(
                    attackWarnings[i]
                        ->attackPosition))); // no need for streflops here!
#else
            currentDistance = floor(enemyFloatCenter.dist(
                attackWarnings[i]
                    ->attackPosition)); // no need for streflops here!
#endif
        ...
        if (nearest == NULL) {
          ...
          MutexSafeWrapper safeMutex(
              mutexAttackWarnings, string(__FILE__) + "_" + intToStr(__LINE__));
          attackWarnings.push_back(awd);

          if (world->getAttackWarningsEnabled() == true) {

            SoundRenderer::getInstance().playFx(
                CoreData::getInstance().getAttentionSound(), true);
...
// if the unit has any enemy on range
vector<Unit *> UnitUpdater::enemyUnitsOnRange(const Unit *unit,
                                              const AttackSkillType *ast) {
  vector<Unit *> enemies;
  enemies.reserve(100);
  ...
  try {
    ...
    const Unit *commandTarget = NULL;
    //	if(unit->anyCommand()) {
    //		commandTarget = static_cast<const
    // Unit*>(unit->getCurrCommand()->getUnit());
    //	}
    //	if(commandTarget != NULL && commandTarget->isDead()) {
    //		commandTarget = NULL;
    //	}
...
#ifdef USE_STREFLOP
      if (map->isInside(i, j) &&
          streflop::floor(static_cast<streflop::Simple>(
              floatCenter.dist(Vec2f((float)i, (float)j)))) <= (range + 1)) {
#else
      if (map->isInside(i, j) &&
          floor(floatCenter.dist(Vec2f((float)i, (float)j))) <= (range + 1)) {
#endif
        Cell *cell = map->getCell(i, j);
...
string UnitUpdater::getUnitRangeCellsLookupItemCacheStats() {
  ...
  MutexSafeWrapper safeMutex(mutexUnitRangeCellsLookupItemCache,
                             string(__FILE__) + "_" + intToStr(__LINE__));
  for (std::map<Vec2i, std::map<int, std::map<int, UnitRangeCellsLookupItem>>>::
           iterator iterMap1 = UnitRangeCellsLookupItemCache.begin();
       iterMap1 != UnitRangeCellsLookupItemCache.end(); ++iterMap1) {
    posCount++;

    for (std::map<int, std::map<int, UnitRangeCellsLookupItem>>::iterator
             iterMap2 = iterMap1->second.begin();
         iterMap2 != iterMap1->second.end(); ++iterMap2) {
      sizeCount++;

      for (std::map<int, UnitRangeCellsLookupItem>::iterator iterMap3 =
               iterMap2->second.begin();
  ...
  char szBuf[8096] = "";
  snprintf(szBuf, 8096, "pos [%d] size [%d] range [%d][%d] total KB: %s",
           posCount, sizeCount, rangeCount, rangeCountCellCount,
           formatNumber(totalBytes).c_str());
  result = szBuf;
  return result;
}
...
void UnitUpdater::saveGame(XmlNode *rootNode) {
  ...
  //	Game *game;
  //	RandomGen random;
  // unitupdaterNode->addAttribute("random",intToStr(random.getLastNumber()),
  // mapTagReplacements);
  //	float attackWarnRange;
  unitupdaterNode->addAttribute(
      "attackWarnRange", floatToStr(attackWarnRange, 6), mapTagReplacements);
...
void UnitUpdater::loadGame(const XmlNode *rootNode) {
  ...
  //	float attackWarnRange;
  attackWarnRange =
      unitupdaterNode->getAttribute("attackWarnRange")->getFloatValue();
}
// =====================================================
...
void ParticleDamager::update(ParticleSystem *particleSystem) {
  ...
  if (attacker != NULL) {
    ...
    char szBuf[8096] = "";
    snprintf(szBuf, 8095,
             "Unit hitting [ParticleDamager::update] [%s] targetField = %d",
             targetPos.getString().c_str(), targetField);
    attacker->addNetworkCRCDecHp(szBuf);

    unitUpdater->hit(attacker, ast, targetPos, targetField,
                     projectileType->getDamagePercentage());

    // char szBuf[8096]="";
    // snprintf(szBuf,8095,"ParticleDamager::update attacker particleSystem
    // before: %s\nafter:
    // %s",auditBeforeHit.c_str(),particleSystem->toString().c_str());
    ...
    if (particleSystem->getVisible() && projSound != NULL) {
      SoundRenderer::getInstance().playFx(
          projSound, attacker->getCurrMidHeightVector(), gameCamera->getPos());
    }

    // check for spawnattack
    if (projectileType->getSpawnUnit() != "" &&
        projectileType->getSpawnUnitcount() > 0) {
      unitUpdater->spawnAttack(attacker, projectileType->getSpawnUnit(),
                               projectileType->getSpawnUnitcount(), 100, 100,
                               100, projectileType->getSpawnUnitAtTarget(),
                               targetPos);
    }
    ...
    if (projectileType->isShake() == true) {
      ...
      bool visibility = (!projectileType->isShakeVisible()) ||
                        (map->getSurfaceCell(surfaceTargetPos)
                             ->isVisible(world->getThisTeamIndex()) ||
                         (game->getWorld()->showWorldForPlayer(
                              game->getWorld()->getThisTeamIndex()) == true));

      bool isInCameraView =
          (!projectileType->isShakeInCameraView()) ||
          Renderer::getInstance().posInCellQuadCache(surfaceTargetPos).first;

      if (visibility && isInCameraView) {
        game->getGameCameraPtr()->shake(
            projectileType->getShakeDuration(),
            projectileType->getShakeIntensity(),
            projectileType->isShakeCameraDistanceAffected(),
            map->getSurfaceCell(surfaceTargetPos)->getVertex());
      }
    }
  }
...
void ParticleDamager::saveGame(XmlNode *rootNode) {
  ...
  //	const AttackSkillType* ast;
  particleDamagerNode->addAttribute("astName", ast->getName(),
                                    mapTagReplacements);
  particleDamagerNode->addAttribute("astClass", intToStr(ast->getClass()),
                                    mapTagReplacements);
  //	UnitUpdater *unitUpdater;
  //	const GameCamera *gameCamera;
  //	Vec2i targetPos;
  particleDamagerNode->addAttribute("targetPos", targetPos.getString(),
                                    mapTagReplacements);
  //	Field targetField;
  particleDamagerNode->addAttribute("targetField", intToStr(targetField),
                                    mapTagReplacements);
}
...
void ParticleDamager::loadGame(const XmlNode *rootNode, void *genericData) {
  ...
  std::pair<Game *, Unit *> *pairData =
      (std::pair<Game *, Unit *> *)genericData;
  ...
  SkillClass astClass = static_cast<SkillClass>(
      particleDamagerNode->getAttribute("astClass")->getIntValue());
  ast = dynamic_cast<const AttackSkillType *>(
      pairData->second->getType()->getSkillType(astName, astClass));
  //	UnitUpdater *unitUpdater;
  unitUpdater = pairData->first->getWorld()->getUnitUpdater();
  //	const GameCamera *gameCamera;
  gameCamera = pairData->first->getGameCamera();
  //	Vec2i targetPos;
  targetPos = Vec2i::strToVec2(
      particleDamagerNode->getAttribute("targetPos")->getValue());
  //	Field targetField;
  targetField = static_cast<Field>(
      particleDamagerNode->getAttribute("targetField")->getIntValue());
}

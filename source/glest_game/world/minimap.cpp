...
void Minimap::incFowTextureAlphaSurface(const Vec2i sPos, float alpha, bool isIncrementalUpdate) {
    if (fowPixmap1) {
...
void Minimap::resetFowTex() {
    if (fowTex && fowPixmap0 && fowPixmap1) {
        ...
        for (int indexPixelWidth = 0; indexPixelWidth < fowTex->getPixmap()->getW(); ++indexPixelWidth) {
            for (int indexPixelHeight = 0; indexPixelHeight < fowTex->getPixmap()->getH(); ++indexPixelHeight) {
                ...
                } else if ((fogOfWar && overridefogOfWarValue) || (gameSettings->getFlagTypes1() & ft1_show_map_resources) == ft1_show_map_resources) {
                    // printf("Line: %d\n",__LINE__);
...
void Minimap::updateFowTex(float t) {
    if (fowTex && fowPixmap0 && fowPixmap1) {
        for (int indexPixelWidth = 0; indexPixelWidth < fowPixmap0->getW(); ++indexPixelWidth) {
            for (int indexPixelHeight = 0; indexPixelHeight < fowPixmap0->getH(); ++indexPixelHeight) {
                float p1 = fowPixmap1->getPixelf(indexPixelWidth, indexPixelHeight);

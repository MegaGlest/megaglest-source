...
void Minimap::init(int w, int h, const World *world, bool fogOfWar) {
  ...
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                             "In [%s::%s Line: %d] scaledW = %d, scaledH = %d, "
                             "potW = %d, potH = %d\n",
                             __FILE__, __FUNCTION__, __LINE__, scaledW, scaledH,
                             potW, potH);
  ...
  if (GlobalStaticFlags::getIsNonGraphicalModeEnabled() == false) {
    ...
    if ((this->gameSettings->getFlagTypes1() & ft1_show_map_resources) ==
  ...
  if (fowTex) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugSystem,
                               "In [%s::%s Line: %d] scaledW = %d, scaledH = "
                               "%d, potW = %d, potH = %d\n",
                               __FILE__, __FUNCTION__, __LINE__, scaledW,
                               scaledH, potW, potH);
...
Minimap::~Minimap() {
  Logger::getInstance().add(
      Lang::getInstance().getString("LogScreenGameUnLoadingMiniMap", "", true),
      true);
  delete fowPixmap0;
  fowPixmap0 = NULL;
...
void Minimap::resetFowTex() {
  if (fowTex && fowPixmap0 && fowPixmap1) {
    ...
    for (int indexPixelWidth = 0; indexPixelWidth < fowTex->getPixmap()->getW();
         ++indexPixelWidth) {
      for (int indexPixelHeight = 0;
           indexPixelHeight < fowTex->getPixmap()->getH(); ++indexPixelHeight) {
        if ((fogOfWar == false && overridefogOfWarValue == false)) {
          //(gameSettings->getFlagTypes1() & ft1_show_map_resources) !=
          // ft1_show_map_resources) { printf("Line: %d\n",__LINE__);
          ...
          if (p1 > exploredAlpha) {
            fowPixmap1->setPixel(indexPixelWidth, indexPixelHeight,
                                 exploredAlpha);
          }
          if (p0 > p1) {
            fowPixmap1->setPixel(indexPixelWidth, indexPixelHeight, p0);
...
void Minimap::updateFowTex(float t) {
  if (fowTex && fowPixmap0 && fowPixmap1) {
    for (int indexPixelWidth = 0; indexPixelWidth < fowPixmap0->getW();
         ++indexPixelWidth) {
      for (int indexPixelHeight = 0; indexPixelHeight < fowPixmap0->getH();
           ++indexPixelHeight) {
        float p1 = fowPixmap1->getPixelf(indexPixelWidth, indexPixelHeight);
        float p2 =
            fowTex->getPixmap()->getPixelf(indexPixelWidth, indexPixelHeight);
        if (p1 != p2) {
          float p0 = fowPixmap0->getPixelf(indexPixelWidth, indexPixelHeight);
          fowTex->getPixmap()->setPixel(indexPixelWidth, indexPixelHeight,
                                        p0 + (t * (p1 - p0)));
        }
      }
    }
    ...
    for (int j = 0; j < tex->getPixmap()->getH(); ++j) {
      for (int i = 0; i < tex->getPixmap()->getW(); ++i) {
        ...
        if (sc->getObject() == NULL || sc->getObject()->getType() == NULL) {
          const Pixmap2D *p =
              world->getTileset()->getSurfPixmap(sc->getSurfaceType(), 0);
...
void Minimap::saveGame(XmlNode *rootNode) {
  ...
  if (fowPixmap1 != NULL) {
    for (std::size_t index = 0; index < fowPixmap1->getPixelByteCount();
         ++index) {
      if (fowPixmap1->getPixels()[index] != 0) {
        XmlNode *fowPixmap1Node = minimapNode->addChild("fowPixmap1");
        fowPixmap1Node->addAttribute("index", intToStr(index),
                                     mapTagReplacements);
        fowPixmap1Node->addAttribute("pixel",
                                     intToStr(fowPixmap1->getPixels()[index]),
                                     mapTagReplacements);
      }
    }
  }
...
void Minimap::loadGame(const XmlNode *rootNode) {
  ...
  if (minimapNode->hasChild("fowPixmap1") == true) {
    vector<XmlNode *> fowPixmap1NodeList =
        minimapNode->getChildList("fowPixmap1");
    for (unsigned int i = 0; i < fowPixmap1NodeList.size(); ++i) {
      ...
      fowPixmap1->getPixels()[pixelIndex] =
          fowPixmap1Node->getAttribute("pixel")->getIntValue();
    }
  }
}

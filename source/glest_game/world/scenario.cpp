...
Checksum Scenario::load(const string &path) {
  ...
  try {
    ...
    snprintf(szBuf, 8096,
             Lang::getInstance()
                 .getString("LogScreenGameLoadingScenario", "", true)
                 .c_str(),
    ...
    if (scriptsNode->hasAttribute("external") &&
        scriptsNode->getAttribute("external")->getBoolValue() == true) {
...
#endif
      if (!luafile.is_open())
        throw megaglest_runtime_error("Can not open file: [" + fileName + "]",
                                      true);
      else {
      ...
      for (int i = 0; i < (int)scriptsNode->getChildCount(); ++i) {
        ...
        scripts.push_back(
            Script(getFunctionName(scriptNode), scriptNode->getText()));
      }
    }
  }
...
string Scenario::getScenarioDir(const vector<string> dir,
  ...
  for (int idx = 0; idx < (int)dir.size(); idx++) {
    ...
    // printf("\n[%s:%s] Line: %d scenarioName [%s] scenarioFile
...
bool Scenario::isGameTutorial(string path) {
  ...
  if (tutorialPaths.empty() == false) {
    for (unsigned int tutorialIndex = 0; tutorialIndex < tutorialPaths.size();
         ++tutorialIndex) {
...
void Scenario::loadScenarioInfo(string file, ScenarioInfo *scenarioInfo,
  ...
  // printf("[%s:%s] Line: %d scenarioDir [%s] scenarioName
  ...
  if (scenarioInfo->difficulty < dVeryEasy ||
    ...
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "Invalid difficulty value specified in scenario: %d must be "
             "between %d and %d",
  ...
  for (int i = 0; i < GameConstants::maxPlayers; ++i) {
    ...
    if (factionControl != ctClosed) {
      ...
      if (teamIndex < 1 || teamIndex > GameConstants::maxPlayers +
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 "Invalid team value specified in scenario: %d must be between "
                 "%d and %d",
    ...
    scenarioInfo->mapName =
        scenarioNode->getChild("map")->getAttribute("value")->getValue();
    scenarioInfo->tilesetName =
        scenarioNode->getChild("tileset")->getAttribute("value")->getValue();
    scenarioInfo->techTreeName =
        scenarioNode->getChild("tech-tree")->getAttribute("value")->getValue();
    scenarioInfo->defaultUnits = scenarioNode->getChild("default-units")
                                     ->getAttribute("value")
                                     ->getBoolValue();
    scenarioInfo->defaultResources = scenarioNode->getChild("default-resources")
                                         ->getAttribute("value")
                                         ->getBoolValue();
    scenarioInfo->defaultVictoryConditions =
        scenarioNode->getChild("default-victory-conditions")
            ->getAttribute("value")
            ->getBoolValue();
  }
  ...
  scenarioInfo->desc += "\n";
  scenarioInfo->desc += lang.getString("Difficulty") + ": " +
                        lang.getString(difficultyString) + "\n";
  scenarioInfo->desc +=
      lang.getString("Map") + ": " + formatString(scenarioInfo->mapName) + "\n";
  scenarioInfo->desc += lang.getString("Tileset") + ": " +
                        formatString(scenarioInfo->tilesetName) + "\n";
  scenarioInfo->desc += lang.getString("TechTree") + ": " +
  ...
  if (scenarioNode->hasChild("fog-of-war") == true) {
    if (scenarioNode->getChild("fog-of-war")
      ...
      scenarioInfo->fogOfWar = scenarioNode->getChild("fog-of-war")
                                   ->getAttribute("value")
                                   ->getBoolValue();
  ...
  if (scenarioNode->hasChild("shared-team-units") == true) {
    scenarioInfo->allowTeamUnitSharing =
        scenarioNode->getChild("shared-team-units")
            ->getAttribute("value")
            ->getBoolValue();
    // printf("\nallowTeamUnitSharing is set to
    // [%s]\n",scenarioInfo->allowTeamUnitSharing);
  } else {
  ...
  if (scenarioNode->hasChild("shared-team-resources") == true) {
    scenarioInfo->allowTeamResourceSharing =
        scenarioNode->getChild("shared-team-resources")
            ->getAttribute("value")
            ->getBoolValue();
    // printf("\nallowTeamResourceSharing is set to
    // [%s]\n",scenarioInfo->allowTeamResourceSharing);
  } else {
...
ControlType Scenario::strToControllerType(const string &str) {
  ...
  snprintf(
      szBuf, 8096,
      "Invalid controller value specified in scenario: [%s] must be one of the "
      "following: closed, cpu-easy, cpu, cpu-ultra, cpu-mega, human",
      str.c_str());
  throw std::runtime_error(szBuf);
}
...
void Scenario::loadGameSettings(const vector<string> &dirList,
    ...
    if (ct != ctClosed) {
      if (ct == ctHuman) {
        ...
        gameSettings->setNetworkPlayerUUID(
            factionCount, Config::getInstance().getString("PlayerId", ""));
        ...
        if (gameSettings->getNetworkPlayerName(factionCount) == "") {
          gameSettings->setNetworkPlayerName(
              factionCount, lang.getString("AI") + intToStr(AIPlayerCount));
        }
      }
      gameSettings->setFactionControl(factionCount, ct);
      gameSettings->setResourceMultiplierIndex(
          factionCount, (scenarioInfo->resourceMultipliers[i] - 0.5f) / 0.1f);
  ...
  gameSettings->setPathFinderType(
      static_cast<PathFinderType>(Config::getInstance().getInt(
          "ScenarioPathFinderType", intToStr(pfBasic).c_str())));
}

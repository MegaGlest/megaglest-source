...
void TimeFlow::init(Tileset *tileset) {
  ...
  timeInc =
      24.f * (1.f / config.getFloat("DayTime")) / GameConstants::updateFps;
  // printf("#1 timeInc = %f\n",timeInc);

  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(SystemFlags::debugSystem,
                             "In [%s::%s Line %d] timeInc = %f\n", __FILE__,
                             __FUNCTION__, __LINE__, timeInc);
}
  ...
  if ((lastTime < dawn && time >= dawn) || firstTime) {
    ...
    if (ambientSounds != NULL) {
      ...
      if (ambientSounds->isEnabledDay()) {
        if (ambientSounds->getAlwaysPlayDay() ||
            tileset->getWeather() == wSunny) {
  ...
  if (lastTime < dusk && time >= dusk) {
    ...
    if (ambientSounds != NULL) {
      ...
      if (ambientSounds->isEnabledNight()) {
        if (ambientSounds->getAlwaysPlayNight() ||
            tileset->getWeather() == wSunny) {
...
Vec3f TimeFlow::computeLightColor() const {
  ...
  if (tileset != NULL) {
    ...
    if (time > dayStart && time < dayEnd) {
      ...
      color = tileset->getSunLightColor().lerp((time - dayEnd) / transition,
                                               tileset->getMoonLightColor());
    } else if (time >= nightEnd && time <= dayStart) {
      color = tileset->getMoonLightColor().lerp((time - nightEnd) / transition,
                                                tileset->getSunLightColor());
    } else {
      assert(false);
      color = tileset->getSunLightColor();
    }
...
void TimeFlow::saveGame(XmlNode *rootNode) {
  ...
  timeflowNode->addAttribute("firstTime", intToStr(firstTime),
                             mapTagReplacements);
  //	bool firstTime;
  //	Tileset *tileset;
  //	float time;
  ...
  //	float lastTime;
  timeflowNode->addAttribute("lastTime", floatToStr(lastTime, 6),
                             mapTagReplacements);
  //	float timeInc;
  // printf("#2 timeInc = %f\n",timeInc);
  timeflowNode->addAttribute("timeInc", floatToStr(timeInc, 6),
                             mapTagReplacements);
  // printf("#3 timeInc = %f\n",timeInc);
}

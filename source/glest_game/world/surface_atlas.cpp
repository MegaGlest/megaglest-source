...
SurfaceInfo::SurfaceInfo(const Pixmap2D *lu, const Pixmap2D *ru,
                         const Pixmap2D *ld, const Pixmap2D *rd) {
  this->leftDown = ld;
  this->leftUp = lu;
  this->rightDown = rd;
  this->rightUp = ru;
...
bool SurfaceInfo::operator==(const SurfaceInfo &si) const {
  return this->center == si.getCenter() && this->leftDown == si.getLeftDown() &&
...
SurfaceAtlas::SurfaceAtlas() { surfaceSize = -1; }
...
void SurfaceAtlas::addSurface(SurfaceInfo *si) {
  ...
  // add info
  SurfaceInfos::iterator it =
      find(surfaceInfos.begin(), surfaceInfos.end(), *si);
  if (it == surfaceInfos.end()) {
    // add new texture
    Texture2D *t = Renderer::getInstance().newTexture2D(rsGame);
    if (t) {
      // if(t == NULL) {
      //	throw megaglest_runtime_error("Could not create new texture
      //(NULL)");
      // }
    ...
    if (si->getCenter() != NULL) {
      ...
      if (t) {
        t->getPixmap()->splat(si->getLeftUp(), si->getRightUp(),
                              si->getLeftDown(), si->getRightDown());
      }
    }
  } else {
...
float SurfaceAtlas::getCoordStep() const { return 1.f; }
...
void SurfaceAtlas::checkDimensions(const Pixmap2D *p) {
  ...
  if (p == NULL) {
    ...
    surfaceSize = p->getW();
    // printf("Setting surfaceSize = %d for pixmap
    // [%s]\n",surfaceSize,p->getPath().c_str());
  } else if (p->getW() != surfaceSize || p->getH() != surfaceSize) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "Bad surface texture dimensions, expected surfaceSize = %d, "
             "texture w = %d, h = %d",
             surfaceSize, p->getW(), p->getH());

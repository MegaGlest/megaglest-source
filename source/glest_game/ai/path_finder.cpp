...
int PathFinder::getPathFindExtendRefreshNodeCount(FactionState &faction) {
  // int refreshNodeCount =
  // faction.random.randRange(PathFinder::pathFindExtendRefreshNodeCountMin,PathFinder::pathFindExtendRefreshNodeCountMax);
...
TravelState PathFinder::findPath(Unit *unit, const Vec2i &finalPos,
  ...
  try {
    ...
    static string mutexOwnerId =
        string(__FILE__) + string("_") + intToStr(__LINE__);
    MutexSafeWrapper safeMutexPrecache(faction.getMutexPreCache(),
                                       mutexOwnerId);
    ...
    if (finalPos == unit->getPos()) {
      if (frameIndex < 0) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
                .enabled == true) {
          char szBuf[8096] = "";
          snprintf(szBuf, 8096, "Unit finalPos [%s] == unit->getPos() [%s]",
                   finalPos.getString().c_str(),
                   unit->getPos().getString().c_str());
          unit->logSynchData(extractFileFromDirectoryPath(__FILE__).c_str(),
                             __LINE__, szBuf);
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugPathFinder)
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, "State: arrived#1 at pos: %s, command [%s]",
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
                ...
                SystemFlags::getSystemSettingType(
                    SystemFlags::debugWorldSynchMax)
                        .enabled == true) {
              char szBuf[8096] = "";
              snprintf(szBuf, 8096, "#1 map->canMove to pos [%s] from [%s]",
                       pos.getString().c_str(),
                       unit->getPos().getString().c_str());
              unit->logSynchData(extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __LINE__, szBuf);
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
                ...
                SystemFlags::getSystemSettingType(
                    SystemFlags::debugWorldSynchMax)
                        .enabled == true) {
              char szBuf[8096] = "";
              snprintf(szBuf, 8096, "#2 map->canMove to pos [%s] from [%s]",
                       pos.getString().c_str(),
                       unit->getPos().getString().c_str());
              unit->logSynchData(extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __LINE__, szBuf);
    ...
    if (path->isStuck() == true &&
        ...
        unit->isLastStuckFrameWithinCurrentFrameTolerance(frameIndex >= 0) ==
            true) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
                  .enabled == true &&
          frameIndex < 0) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 "path->isStuck() == true unit->getLastStuckPos() [%s] "
                 "finalPos [%s] path->getBlockCount() [%d]",
    ...
    switch (ts) {
      case tsBlocked:
      case tsArrived:
        // The unit is stuck (not only blocked but unable to go anywhere for a
        // while) We will try to bail out of the immediate area
        if (ts == tsBlocked && unit->getInBailOutAttempt() == false &&
            path->isStuck() == true) {
          if (minorDebugPathfinder)
            printf(
                "Pathfind Unit [%d - %s] START BAILOUT ATTEMPT frameIndex = "
                "%d\n",
                unit->getId(), unit->getType()->getName(false).c_str(),
                frameIndex);
          ...
          // First check if unit currently blocked all around them, if so don't
          // try to pathfind
          ...
          if (unitImmediatelyBlocked == false) {
            ...
            //	throw megaglest_runtime_error("#2 Invalid access to FactionState
            // random from outside main thread current id = " +
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
                    .enabled == true) {
              char szBuf[8096] = "";
              snprintf(szBuf, 8096, "In astar bailout() tryRadius %d",
                       tryRadius);
            ...
            // Try to bail out up to PathFinder::pathFindBailoutRadius cells
            // away
            if (tryRadius == 2) {
              for (int bailoutX = -PathFinder::pathFindBailoutRadius;
                   ...
                   ts == tsBlocked;
                   ++bailoutX) {
                for (int bailoutY = -PathFinder::pathFindBailoutRadius;
                     bailoutY <= PathFinder::pathFindBailoutRadius &&
                     ts == tsBlocked;
                     ++bailoutY) {
                  const Vec2i newFinalPos =
                  ...
                  if (SystemFlags::getSystemSettingType(
                    ...
                    char szBuf[8096] = "";
                    snprintf(szBuf, 8096,
                             "[attempting to BAIL OUT] finalPos [%s] "
                             "newFinalPos [%s] ts [%d] canUnitMove [%d]",
                             finalPos.getString().c_str(),
                             newFinalPos.getString().c_str(), ts, canUnitMove);
                    unit->logSynchData(
                  ...
                  if (canUnitMove) {
                    ...
                    ts = aStar(unit, newFinalPos, true, frameIndex,
                               maxBailoutNodeCount, &searched_node_count);
                  }
                }
              }
            } else {
        ...
        if (dynamic_cast<UnitPathBasic *>(path) != NULL) {
          ...
          if (frameIndex < 0 && basicPath != NULL) {
            ...
            if (faction.precachedPath[unit->getId()].size() <= 0) {
              throw megaglest_runtime_error(
                  "factions[unit->getFactionIndex()].precachedPath[unit->getId("
                  ")].size() <= 0!");
            }
          ...
          if (map->canMove(unit, unit->getPos(), pos)) {
            ...
            if (minorDebugPathfinderPerformance && chrono.getMillis() >= 1)
              printf(
                  "Unit [%d - %s] astar #2 took [%lld] msecs, ts = %d "
                  "searched_node_count = %d.\n",
                  unit->getId(), unit->getType()->getName(false).c_str(),
          ...
          if (map->canMove(unit, unit->getPos(), pos)) {
            ...
            if (minorDebugPathfinder)
              printf(
                  "Pathfind Unit [%d - %s] INT BAILOUT ATTEMPT BLOCKED "
                  "frameIndex = %d\n",
                  unit->getId(), unit->getType()->getName(false).c_str(),
                  frameIndex);
            ...
            if (minorDebugPathfinderPerformance && chrono.getMillis() >= 1)
              printf(
                  "Unit [%d - %s] astar #3 took [%lld] msecs, ts = %d "
                  "searched_node_count = %d.\n",
                  unit->getId(), unit->getType()->getName(false).c_str(),
                  (long long int)chrono.getMillis(), ts, searched_node_count);
            return tsBlocked;
    ...
    if (minorDebugPathfinderPerformance && chrono.getMillis() >= 1)
      printf(
          "Unit [%d - %s] astar took [%lld] msecs, ts = %d searched_node_count "
          "= %d.\n",
          unit->getId(), unit->getType()->getName(false).c_str(),
          (long long int)chrono.getMillis(), ts, searched_node_count);
  ...
  try {
    ...
    if (frameIndex < 0) {
      ...
      if (foundPrecacheTravelState == true) {
        ...
        //== true && frameIndex < 0) { 				char
        // szBuf[8096]="";
        //				snprintf(szBuf,8096,"factions[unitFactionIndex].precachedTravelState[unit->getId()]:
        //%d",faction.precachedTravelState[unit->getId()]);
        ...
        if (foundPrecacheTravelStateIsMoving == true) {
          ...
          for (int i = 0; i < unitPrecachePathSize; i++) {
            ...
            if (map->isInside(nodePos) == false ||
                map->isInsideSurface(map->toSurfCoords(nodePos)) == false) {
              throw megaglest_runtime_error(
          ...
          if (canMoveToCells == true) {
            path->clear();
            // UnitPathBasic *basicPathFinder = dynamic_cast<UnitPathBasic
            // *>(path);
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
              ...
              snprintf(szBuf, 8096,
                       "return "
                       "factions[unitFactionIndex].precachedTravelState[unit->"
                       "getId()];");
              unit->logSynchData(extractFileFromDirectoryPath(__FILE__).c_str(),
                                 __LINE__, szBuf);
            }
          ...
          if (foundPrecacheTravelStateIsBlocked == true) {
            ...
            //== true && frameIndex < 0) {
            // char szBuf[8096]="";
            // snprintf(szBuf,8096,"return
            // factions[unitFactionIndex].precachedTravelState[unit->getId()];");
            //						unit->logSynchData(extractFileFromDirectoryPath(__FILE__).c_str(),__LINE__,szBuf);
            //					}
    ...
    // First check if unit currently blocked all around them, if so don't try to
    // pathfind
    if (inBailout == false && unitPos != finalPos) {
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
        ...
        SystemFlags::OutputDebug(
            ...
            "In [%s::%s Line: %d] **Check if dest blocked, distance for unit "
            "[%d - %s] from [%s] to [%s] is %.2f took msecs: %lld "
            "nodeLimitReached = %d, failureCount = %d\n",
            extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
            __LINE__, unit->getId(), unit->getFullName(false).c_str(),
            unitPos.getString().c_str(), finalPos.getString().c_str(), dist,
            (long long int)chrono.getMillis(), nodeLimitReached, failureCount);
      ...
      if (nodeLimitReached == false) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
          ...
          SystemFlags::OutputDebug(
              ...
              __LINE__, unit->getId(), unit->getFullName(false).c_str(),
              unitPos.getString().c_str(), finalPos.getString().c_str(), dist,
              (long long int)chrono.getMillis(), nodeLimitReached,
              failureCount);
      }
    } else {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
          ...
          frameIndex < 0) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, "inBailout: %d unitPos: [%s] finalPos [%s]",
    ...
    if (nodeLimitReached == false) {
      if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 "Calling doAStarPathSearch nodeLimitReached: %d "
                 "whileLoopCount: %d unitFactionIndex: %d pathFound: %d "
                 "finalPos [%s] maxNodeCount: %d frameIndex: %d",
                 nodeLimitReached, whileLoopCount, unitFactionIndex, pathFound,
                 finalPos.getString().c_str(), maxNodeCount, frameIndex);
        unit->logSynchData(extractFileFromDirectoryPath(__FILE__).c_str(),
                           __LINE__, szBuf);
      }

      doAStarPathSearch(nodeLimitReached, whileLoopCount, unitFactionIndex,
                        pathFound, node, finalPos, closedNodes, cameFrom,
                        canAddNode, unit, maxNodeCount, frameIndex);
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
        ...
        snprintf(
            szBuf, 8096,
            "Calling doAStarPathSearch nodeLimitReached: %d whileLoopCount: %d "
            "unitFactionIndex: %d pathFound: %d finalPos [%s] maxNodeCount: %d "
            "pathFindNodesAbsoluteMax: %d frameIndex: %d",
            nodeLimitReached, whileLoopCount, unitFactionIndex, pathFound,
            finalPos.getString().c_str(), maxNodeCount,
            pathFindNodesAbsoluteMax, frameIndex);
        unit->logSynchData(extractFileFromDirectoryPath(__FILE__).c_str(),
                           __LINE__, szBuf);
      }
    ...
    if (nodeLimitReached == true) {
      if (faction.closedNodesList.empty() == false) {
        float bestHeuristic =
            truncateDecimal<float>(faction.closedNodesList.begin()->first, 6);
    ...
    if (pathFound == false || lastNode == firstNode) {
      if (minorDebugPathfinder)
        printf(
            "Legacy Pathfind Unit [%d - %s] NOT FOUND PATH count = %d "
            "frameIndex = %d\n",
            unit->getId(), unit->getType()->getName().c_str(), whileLoopCount,
            frameIndex);
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugPathFinder)
        ...
        std::pair<Vec2i, int> lastHarvest =
            unit->getLastHarvestResourceTarget();

        char szBuf[8096] = "";
        snprintf(
            szBuf, 8096,
            "State: blocked, cmd [%s] pos: [%s], dest pos: [%s], lastHarvest = "
            "[%s - %d], reason A= %d, B= %d, C= %d, D= %d, E= %d, F = %d",
            commandDesc.c_str(), unit->getPos().getString().c_str(),
            targetPos.getString().c_str(),
            lastHarvest.first.getString().c_str(), lastHarvest.second,
            pathFound, (lastNode == firstNode), path->getBlockCount(),
      ...
      if (minorDebugPathfinder)
        printf(
            "Legacy Pathfind Unit [%d - %s] FOUND PATH count = %d frameIndex = "
            "%d\n",
            unit->getId(), unit->getType()->getName().c_str(), whileLoopCount,
            frameIndex);
      // on the way
      ...
      for (int i = 0; currNode->next != NULL; currNode = currNode->next, i++) {
        ...
        if (map->isInside(nodePos) == false ||
            map->isInsideSurface(map->toSurfCoords(nodePos)) == false) {
          throw megaglest_runtime_error(
              "Pathfinder invalid node path position = " + nodePos.getString() +
              " i = " + intToStr(i));
        }
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
        ...
        if (frameIndex < 0) {
          ...
          for (unsigned int index = 0;
               index < faction.precachedPath[unit->getId()].size(); ++index) {
    ...
    if (frameIndex >= 0) {
      ...
      if (SystemFlags::VERBOSE_MODE_ENABLED && chrono.getMillis() >= 5)
        printf("In [%s::%s Line: %d] astar took [%lld] msecs, ts = %d.\n",
               extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
               __LINE__, (long long int)chrono.getMillis(), ts);
    }
...
void PathFinder::processNearestFreePos(const Vec2i &finalPos, int i, int j,
...
Vec2i PathFinder::computeNearestFreePos(const Unit *unit,
  ...
  try {
    ...
    for (int i = -maxFreeSearchRadius; i <= maxFreeSearchRadius; ++i) {
      for (int j = -maxFreeSearchRadius; j <= maxFreeSearchRadius; ++j) {
        processNearestFreePos(finalPos, i, j, size, field, teamIndex, unitPos,
...
//	try {
//	// First check if unit currently blocked all around them, if so don't
// try to pathfind 	const Vec2i unitPos = unit->getPos(); 	int failureCount
// = 0; 	int cellCount = 0;
//
//	for(int i = -1; i <= 1; ++i) {
//		for(int j = -1; j <= 1; ++j) {
//			Vec2i pos = unitPos + Vec2i(i, j);
//			if(pos != unitPos) {
//				bool canUnitMoveToCell = map->aproxCanMove(unit,
// unitPos, pos); 				if(canUnitMoveToCell == false) {
// failureCount++;
//				}
//				cellCount++;
//			}
...
void PathFinder::saveGame(XmlNode *rootNode) {
  ...
  pathfinderNode->addAttribute("pathFindNodesMax", intToStr(pathFindNodesMax),
                               mapTagReplacements);
  ...
  for (unsigned int i = 0; i < (unsigned int)factions.size(); ++i) {
    ...
    for (unsigned int j = 0; j < (unsigned int)factionState.nodePool.size();
         ++j) {
      ...
      nodePoolNode->addAttribute("heuristic", floatToStr(curNode->heuristic, 6),
                                 mapTagReplacements);
      nodePoolNode->addAttribute(
          "exploredCell", intToStr(curNode->exploredCell), mapTagReplacements);
    }
    ...
    factionsNode->addAttribute("random",
                               intToStr(factionState.random.getLastNumber()),
                               mapTagReplacements);
...
void PathFinder::loadGame(const XmlNode *rootNode) {
  ...
  for (unsigned int i = 0; i < (unsigned int)factionsNodeList.size(); ++i) {
    ...
    for (unsigned int j = 0; j < (unsigned int)nodePoolListNode.size() &&
                             j < (unsigned int)pathFindNodesAbsoluteMax;
         ++j) {
      XmlNode *nodePoolNode = nodePoolListNode[j];

      Node *curNode = &factionState.nodePool[j];
      curNode->pos =
          Vec2i::strToVec2(nodePoolNode->getAttribute("pos")->getValue());
    ...
    factionState.random.setLastNumber(
        factionsNode->getAttribute("random")->getIntValue());

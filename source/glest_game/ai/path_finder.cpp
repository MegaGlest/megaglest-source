...
TravelState PathFinder::findPath(Unit *unit, const Vec2i &finalPos, bool *wasStuck, int frameIndex) {
    ...
    try {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true && frameIndex < 0) {
            char szBuf[8096] = "";
            snprintf(szBuf, 8096, "[findPath] unit->getPos() [%s] finalPos [%s]", unit->getPos().getString().c_str(), finalPos.getString().c_str());
            unit->logSynchData(extractFileFromDirectoryPath(__FILE__).c_str(), __LINE__, szBuf);
        }
        ...
        if (path->isStuck() == true && (unit->getLastStuckPos() == finalPos || path->getBlockCount() > 500) &&
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true && frameIndex < 0) {
                ...
                snprintf(szBuf, 8096,
                         ...
                         unit->getLastStuckPos().getString().c_str(), finalPos.getString().c_str(), path->getBlockCount());
                unit->logSynchData(extractFileFromDirectoryPath(__FILE__).c_str(), __LINE__, szBuf);
            }
        ...
        if (minorDebugPathfinder)
            printf("Legacy Pathfind Unit [%d - %s] from = %s to = %s frameIndex = %d\n", unit->getId(), unit->getType()->getName(false).c_str(),
                   unit->getPos().getString().c_str(), finalPos.getString().c_str(), frameIndex);
        ...
        case tsArrived:
            ...
            if (ts == tsBlocked && unit->getInBailOutAttempt() == false && path->isStuck() == true) {
                if (minorDebugPathfinder)
                    printf("Pathfind Unit [%d - %s] START BAILOUT ATTEMPT frameIndex = "
                           "%d\n",
                           unit->getId(), unit->getType()->getName(false).c_str(), frameIndex);

                if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true && frameIndex < 0) {
                    char szBuf[8096] = "";
                    snprintf(szBuf, 8096, "[attempting to BAIL OUT] finalPos [%s] ts [%d]", finalPos.getString().c_str(), ts);
                    unit->logSynchData(extractFileFromDirectoryPath(__FILE__).c_str(), __LINE__, szBuf);
                }
                ...
                if (unitImmediatelyBlocked == false) {
                    ...
                    if (tryRadius == 2) {
                        for (int bailoutX = -PathFinder::pathFindBailoutRadius; bailoutX <= PathFinder::pathFindBailoutRadius && ts == tsBlocked; ++bailoutX) {
                            for (int bailoutY = -PathFinder::pathFindBailoutRadius; bailoutY <= PathFinder::pathFindBailoutRadius && ts == tsBlocked;
                                 ++bailoutY) {
            ...
            if (dynamic_cast<UnitPathBasic *>(path) != NULL) {
                ...
                if (frameIndex < 0 && basicPath != NULL) {
                    ...
                    if (faction.precachedPath[unit->getId()].size() <= 0) {
                        throw megaglest_runtime_error("factions[unit->getFactionIndex()].precachedPath[unit->getId("
                                                      ")].size() <= 0!");
                    }
        ...
        if (minorDebugPathfinderPerformance && chrono.getMillis() >= 1)
            printf("Unit [%d - %s] astar took [%lld] msecs, ts = %d searched_node_count "
                   "= %d.\n",
                   unit->getId(), unit->getType()->getName(false).c_str(), (long long int)chrono.getMillis(), ts, searched_node_count);
...
TravelState PathFinder::aStar(Unit *unit, const Vec2i &targetPos, bool inBailout, int frameIndex, int maxNodeCount, uint32 *searched_node_count) {
    ...
    try {
                ...
                if (foundPrecacheTravelStateIsMoving == true) {
                    ...
                    for (int i = 0; i < unitPrecachePathSize; i++) {
                        ...
                        if (i < unit->getPathFindRefreshCellCount() ||
                            (unitPrecachePathSize >= pathFindExtendRefreshForNodeCount && i < getPathFindExtendRefreshNodeCount(faction))) {
        ...
        if (inBailout == false && unitPos != finalPos) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled == true && chrono.getMillis() > 1)
                SystemFlags::OutputDebug(SystemFlags::debugPerformance,
                                         ...
                                         extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__, unit->getId(),
                                         unit->getFullName(false).c_str(), unitPos.getString().c_str(), finalPos.getString().c_str(), dist,
        ...
        if (nodeLimitReached == false) {
            if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true && frameIndex < 0) {
                ...
                snprintf(szBuf, 8096,
                         ...
                         nodeLimitReached, whileLoopCount, unitFactionIndex, pathFound, finalPos.getString().c_str(), maxNodeCount, frameIndex);
                unit->logSynchData(extractFileFromDirectoryPath(__FILE__).c_str(), __LINE__, szBuf);
            }

            doAStarPathSearch(nodeLimitReached, whileLoopCount, unitFactionIndex, pathFound, node, finalPos, closedNodes, cameFrom, canAddNode, unit,
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true && frameIndex < 0) {
                ...
                snprintf(szBuf, 8096,
                         ...
                         nodeLimitReached, whileLoopCount, unitFactionIndex, pathFound, finalPos.getString().c_str(), maxNodeCount, pathFindNodesAbsoluteMax,
                         frameIndex);
                unit->logSynchData(extractFileFromDirectoryPath(__FILE__).c_str(), __LINE__, szBuf);
            }
        ...
        if (pathFound == false || lastNode == firstNode) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugPathFinder).enabled == true) {
                ...
                snprintf(szBuf, 8096,
                         ...
                         commandDesc.c_str(), unit->getPos().getString().c_str(), targetPos.getString().c_str(), lastHarvest.first.getString().c_str(),
                         lastHarvest.second, pathFound, (lastNode == firstNode), path->getBlockCount(), path->isBlocked(), nodeLimitReached, path->isStuck());
                unit->setCurrentUnitTitle(szBuf);
            }
            ...
            if (minorDebugPathfinder)
                printf("Legacy Pathfind Unit [%d - %s] FOUND PATH count = %d frameIndex = "
                       "%d\n",
                       unit->getId(), unit->getType()->getName().c_str(), whileLoopCount, frameIndex);
            // on the way
            ts = tsMoving;
            ...
            for (int i = 0; currNode->next != NULL; currNode = currNode->next, i++) {
                ...
                if (frameIndex >= 0) {
                    ...
                    if (i < unit->getPathFindRefreshCellCount() ||
                        (whileLoopCount >= pathFindExtendRefreshForNodeCount && i < getPathFindExtendRefreshNodeCount(faction))) {
                        path->add(nodePos);
                    }
                }
            }
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugPathFinder).enabled == true) {
                ...
                char szBuf[8096] = "";
                snprintf(szBuf, 8096, "State: moving, cmd [%s] pos: %s dest pos: %s, Queue= %d", commandDesc.c_str(), unit->getPos().getString().c_str(),
...
void PathFinder::processNearestFreePos(const Vec2i &finalPos, int i, int j, int size, Field field, int teamIndex, Vec2i unitPos, Vec2i &nearestPos,
                                       float &nearestDist) {
    try {

...
TravelState PathFinder::findPath(Unit *unit, const Vec2i &finalPos,
  ...
  try {
            ...
            for (int bailoutX = -PathFinder::pathFindBailoutRadius;
              ...
              for (int bailoutY = -PathFinder::pathFindBailoutRadius;
                ...
                if (SystemFlags::getSystemSettingType(
                  ...
                  char szBuf[8096] = "";
                  snprintf(szBuf, 8096,
                           "[attempting to BAIL OUT] finalPos [%s] newFinalPos "
                           "[%s] ts [%d] canUnitMove [%d]",
                           finalPos.getString().c_str(),
      ...
      if (dynamic_cast<UnitPathBasic *>(path) != NULL) {
        ...
        if (map->canMove(unit, unit->getPos(), pos)) {
          ...
          if (minorDebugPathfinderPerformance && chrono.getMillis() >= 1)
            printf("Unit [%d - %s] astar #2 took [%lld] msecs, ts = %d "
        ...
        if (map->canMove(unit, unit->getPos(), pos)) {
          ...
          if (minorDebugPathfinder)
            printf("Pathfind Unit [%d - %s] INT BAILOUT ATTEMPT BLOCKED "
                   "frameIndex = %d\n",
                   unit->getId(), unit->getType()->getName(false).c_str(),
                   frameIndex);

          if (minorDebugPathfinderPerformance && chrono.getMillis() >= 1)
            printf("Unit [%d - %s] astar #3 took [%lld] msecs, ts = %d "
                   "searched_node_count = %d.\n",
                   unit->getId(), unit->getType()->getName(false).c_str(),
                   (long long int)chrono.getMillis(), ts, searched_node_count);
          return tsBlocked;
    ...
    if (minorDebugPathfinderPerformance && chrono.getMillis() >= 1)
      printf("Unit [%d - %s] astar took [%lld] msecs, ts = %d "
             "searched_node_count = %d.\n",
             unit->getId(), unit->getType()->getName(false).c_str(),
             (long long int)chrono.getMillis(), ts, searched_node_count);
...
TravelState PathFinder::aStar(Unit *unit, const Vec2i &targetPos,
  ...
  try {
      ...
      if (foundPrecacheTravelState == true) {
        ...
        if (foundPrecacheTravelStateIsMoving == true) {
          ...
          if (foundPrecacheTravelStateIsBlocked == true) {
            ...
            //					if(SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled
            //== true && frameIndex < 0) { 						char
            //szBuf[8096]=""; 						snprintf(szBuf,8096,"return
    ...
    if (pathFound == false || lastNode == firstNode) {
      if (minorDebugPathfinder)
        printf("Legacy Pathfind Unit [%d - %s] NOT FOUND PATH count = %d "
               "frameIndex = %d\n",
      ...
      if (minorDebugPathfinder)
        printf("Legacy Pathfind Unit [%d - %s] FOUND PATH count = %d "
               "frameIndex = %d\n",
               unit->getId(), unit->getType()->getName().c_str(),
               whileLoopCount, frameIndex);
      // on the way

...
void ProduceTask::saveGame(XmlNode *rootNode) const {
  ...
  //	UnitClass unitClass;
  produceTaskNode->addAttribute("unitClass", intToStr(unitClass),
                                mapTagReplacements);
  //	const UnitType *unitType;
  if (unitType != NULL) {
    produceTaskNode->addAttribute("unitType", unitType->getName(false),
                                  mapTagReplacements);
  }
  //	const ResourceType *resourceType;
  if (resourceType != NULL) {
    produceTaskNode->addAttribute("resourceType", resourceType->getName(false),
                                  mapTagReplacements);
  }
}
...
ProduceTask *ProduceTask::loadGame(const XmlNode *rootNode, Faction *faction) {
  ...
  //	UnitClass unitClass;
  newTask->unitClass = static_cast<UnitClass>(
...
void BuildTask::saveGame(XmlNode *rootNode) const {
  ...
  //	const ResourceType *resourceType;
  if (resourceType != NULL) {
    buildTaskNode->addAttribute("resourceType", resourceType->getName(),
                                mapTagReplacements);
  }
  //	bool forcePos;
...
void UpgradeTask::saveGame(XmlNode *rootNode) const {
  ...
  if (upgradeType != NULL) {
    ...
    upgradeTaskNode->addAttribute("upgradeType", upgradeType->getName(),
                                  mapTagReplacements);
  }
}
...
void Ai::init(AiInterface *aiInterface, int useStartLocation) {
  ...
  if (faction->getAIBehaviorStaticOverideValue(aibsvcMaxBuildRadius) !=
    ...
    // printf("Discovered overriden static value for AI, maxBuildRadius =
    // %d\n",maxBuildRadius);
  }
  if (faction->getAIBehaviorStaticOverideValue(aibsvcMinMinWarriors) !=
    ...
    // printf("Discovered overriden static value for AI, minMinWarriors =
    // %d\n",minMinWarriors);
  }
  if (faction->getAIBehaviorStaticOverideValue(
          aibsvcMinMinWarriorsExpandCpuEasy) != INT_MAX) {
    minMinWarriorsExpandCpuEasy = faction->getAIBehaviorStaticOverideValue(
        aibsvcMinMinWarriorsExpandCpuEasy);
    // printf("Discovered overriden static value for AI,
    // minMinWarriorsExpandCpuEasy = %d\n",minMinWarriorsExpandCpuEasy);
  }
  if (faction->getAIBehaviorStaticOverideValue(
          aibsvcMinMinWarriorsExpandCpuMega) != INT_MAX) {
    minMinWarriorsExpandCpuMega = faction->getAIBehaviorStaticOverideValue(
        aibsvcMinMinWarriorsExpandCpuMega);
    // printf("Discovered overriden static value for AI,
    // minMinWarriorsExpandCpuMega = %d\n",minMinWarriorsExpandCpuMega);
  }
  if (faction->getAIBehaviorStaticOverideValue(
          aibsvcMinMinWarriorsExpandCpuUltra) != INT_MAX) {
    minMinWarriorsExpandCpuUltra = faction->getAIBehaviorStaticOverideValue(
        aibsvcMinMinWarriorsExpandCpuUltra);
    // printf("Discovered overriden static value for AI,
    // minMinWarriorsExpandCpuUltra = %d\n",minMinWarriorsExpandCpuUltra);
  }
  if (faction->getAIBehaviorStaticOverideValue(
          aibsvcMinMinWarriorsExpandCpuNormal) != INT_MAX) {
    minMinWarriorsExpandCpuNormal = faction->getAIBehaviorStaticOverideValue(
        aibsvcMinMinWarriorsExpandCpuNormal);
    // printf("Discovered overriden static value for AI,
    // minMinWarriorsExpandCpuNormal = %d\n",minMinWarriorsExpandCpuNormal);
  }
  if (faction->getAIBehaviorStaticOverideValue(aibsvcMaxMinWarriors) !=
    ...
    // printf("Discovered overriden static value for AI, maxMinWarriors =
    // %d\n",maxMinWarriors);
  }
  if (faction->getAIBehaviorStaticOverideValue(aibsvcMaxExpansions) !=
    ...
    // printf("Discovered overriden static value for AI, maxExpansions =
    // %d\n",maxExpansions);
  }
  if (faction->getAIBehaviorStaticOverideValue(aibsvcVillageRadius) !=
    ...
    // printf("Discovered overriden static value for AI, villageRadius =
    // %d\n",villageRadius);
  }
  if (faction->getAIBehaviorStaticOverideValue(aibsvcScoutResourceRange) !=
      INT_MAX) {
    scoutResourceRange =
        faction->getAIBehaviorStaticOverideValue(aibsvcScoutResourceRange);
    // printf("Discovered overriden static value for AI, scoutResourceRange =
    // %d\n",scoutResourceRange);
  }
  if (faction->getAIBehaviorStaticOverideValue(
          aibsvcMinWorkerAttackersHarvesting) != INT_MAX) {
    minWorkerAttackersHarvesting = faction->getAIBehaviorStaticOverideValue(
...
Ai::~Ai() {
  if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    SystemFlags::OutputDebug(
        ...
        __FUNCTION__, __LINE__, aiInterface);
...
RandomGen *Ai::getRandom() {
  ...
  //		throw megaglest_runtime_error("Invalid access to AI random from
  // outside main thread current id = " +
...
void Ai::update() {
  ...
  if (aiInterface->getMyFaction()->getFirstSwitchTeamVote() != NULL) {
    ...
    aiInterface->getMyFaction()->setCurrentSwitchTeamVoteFactionIndex(
        vote->factionIndex);

    factionSwitchTeamRequestCount[vote->factionIndex]++;
    int factionSwitchTeamRequestCountCurrent =
        factionSwitchTeamRequestCount[vote->factionIndex];
    ...
    SwitchTeamVote *voteResult =
        aiInterface->getMyFaction()->getSwitchTeamVote(vote->factionIndex);
    ...
    if (aiInterface->getWorld()->factionLostGame(
            aiInterface->getFactionIndex()) == true) {
    ...
    snprintf(
        ...
        aiInterface->getMyFaction()->getIndex(),
        (voteResult->allowSwitchTeam ? "Yes" : "No"), allowJoinTeam,
  ...
  for (unsigned int ruleIdx = 0; ruleIdx < aiRules.size(); ++ruleIdx) {
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
      ...
      SystemFlags::OutputDebug(
          ...
          __FUNCTION__, __LINE__, chrono.getMillis(), ruleIdx);

    if ((aiInterface->getTimer() %
         (rule->getTestInterval() * GameConstants::updateFps / 1000)) == 0) {
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
        ...
        SystemFlags::OutputDebug(SystemFlags::debugPerformance,
                                 ...
                                 "[ruleIdx = %d, before rule->test()]\n",
                                 __FILE__, __FUNCTION__, __LINE__,
                                 chrono.getMillis(), ruleIdx);

      // printf("Testing AI Faction # %d RULE
      ...
      if (rule->test()) {
        ...
        aiInterface->printLog(3, intToStr(1000 * aiInterface->getTimer() /
                                          GameConstants::updateFps) +
                                     ": Executing rule: " + rule->getName() +
                                     '\n');
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugPerformance,
              "In [%s::%s Line: %d] took msecs: %lld [ruleIdx = %d, before "
              "rule->execute() [%s]]\n",
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
          ...
          SystemFlags::OutputDebug(
              SystemFlags::debugPerformance,
              "In [%s::%s Line: %d] took msecs: %lld [ruleIdx = %d, after "
              "rule->execute() [%s]]\n",
              __FILE__, __FUNCTION__, __LINE__, chrono.getMillis(), ruleIdx,
              rule->getName().c_str());
      }
...
float Ai::getRatioOfClass(UnitClass uc,
  ...
  if (aiInterface->getMyUnitCount() == 0) {
    ...
    // return
    // static_cast<float>(getCountOfClass(uc,additionalUnitClassToExcludeFromCount))
    // / aiInterface->getMyUnitCount();
    return truncateDecimal<float>(
        static_cast<float>(
            getCountOfClass(uc, additionalUnitClassToExcludeFromCount)) /
            aiInterface->getMyUnitCount(),
        6);
  }
}
...
const ResourceType *Ai::getNeededResource(int unitIndex) {
    ...
    if (rt->getClass() != rcStatic && rt->getClass() != rcConsumable &&
      ...
      if ((int)unitsGettingResource.size() <= maxUnitsToHarvestResource) {
        ...
        const HarvestCommandType *hct =
            unit->getType()->getFirstHarvestCommand(rt, unit->getFaction());
        Vec2i resPos;
        if (hct != NULL &&
            aiInterface->getNearestSightedResource(
                rt, aiInterface->getHomeLocation(), resPos, false)) {
          amount = r->getAmount();
          neededResource = rt;
        }
      }
  ...
  aiInterface->printLog(3, szBuf);
  snprintf(
      szBuf, 8096, "[resource type count %d] Needed resource [%s].",
      tt->getResourceTypeCount(),
      (neededResource != NULL ? neededResource->getName().c_str() : "<none>"));
  aiInterface->printLog(3, szBuf);
...
bool Ai::isStableBase() {
  ...
  if (getCountOfClass(ucWarrior, &ucWorkerType) > minWarriors) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "Base is stable [minWarriors = %d found = %d]",
    ...
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "Base is NOT stable [minWarriors = %d found = %d]",
             minWarriors, ucWorkerType);
    aiInterface->printLog(4, szBuf);
  ...
  for (int i = 0; i < aiInterface->getMyUnitCount(); ++i) {
    ...
    if (unit->getType()->isCommandable() &&
      ...
      if (!idleOnly || !unit->anyCommand() ||
          unit->getCurrCommand()->getCommandType()->getClass() == ccStop) {
        units.push_back(i);
      }
    }
  }
  ...
  for (int i = 0; i < aiInterface->getMyUnitCount(); ++i) {
    ...
    if (unit->getType()->isCommandable()) {
      if (unit->getType()->hasCommandClass(ccHarvest)) {
        if (unit->anyCommand() &&
            unit->getCurrCommand()->getCommandType()->getClass() == ccHarvest) {
          Command *command = unit->getCurrCommand();
          const HarvestCommandType *hct =
              dynamic_cast<const HarvestCommandType *>(
        ...
        if (unit->anyCommand() &&
          ...
          const ProduceCommandType *pct =
              dynamic_cast<const ProduceCommandType *>(
                  command->getCommandType());
          if (pct != NULL) {
...
//		const Unit *unit= aiInterface->getMyUnit(i);
//		if(unit->getType()->isCommandable() &&
// unit->getType()->hasCommandClass(currentCommand)) {
// if(unit->anyCommand() && unit->getCurrCommand()->getCommandType()->getClass()
// == currentCommand) { 				units.push_back(i);
//			}
//		}
//	}
//
...
bool Ai::findPosForBuilding(const UnitType *building, const Vec2i &searchPos,
                            Vec2i &outPos) {

  for (int currRadius = 0; currRadius < maxBuildRadius; ++currRadius) {
    for (int i = searchPos.x - currRadius; i < searchPos.x + currRadius; ++i) {
      for (int j = searchPos.y - currRadius; j < searchPos.y + currRadius;
           ++j) {
        outPos = Vec2i(i, j);
        if (aiInterface->isFreeCells(
                outPos - Vec2i(minBuildSpacing),
                building->getAiBuildSize() + minBuildSpacing * 2, fLand)) {
          int aiBuildSizeDiff =
...
bool Ai::anyTask() { return !tasks.empty(); }
...
Vec2i Ai::getRandomHomePosition() {
  ...
  return expansionPositions[random.randRange(0, (int)expansionPositions.size() -
                                                    1)];
}
...
void Ai::sendScoutPatrol() {
  ...
  if (megaResourceAttack || ultraResourceAttack) {
    ...
    while (possibleTargetFound == false) {
      ...
      pos = Vec2i(random.randRange(2, width - 2),
  ...
  if (aiInterface->getHomeLocation() != pos) {
    if (findAbleUnit(&unit, ccAttack, false)) {
      ...
      aiInterface->printLog(2, "Scout patrol sent to: " + intToStr(pos.x) +
                                   "," + intToStr(pos.y) + "\n");
    }
  }
}
...
void Ai::massiveAttack(const Vec2i &pos, Field field, bool ultraAttack) {
  ...
  for (int i = 0; i < unitCount; ++i) {
    ...
    if (alreadyAttacking == false && unit->getType()->hasSkillClass(scAttack) &&
        (aiInterface->getControlType() == ctCpuUltra ||
         aiInterface->getControlType() == ctCpuMega ||
         aiInterface->getControlType() == ctNetworkCpuUltra ||
         aiInterface->getControlType() == ctNetworkCpuMega)) {
      // printf("~~~~~~~~ Unit [%s - %d] checking if unit is being
      // attacked\n",unit->getFullName().c_str(),unit->getId());

      std::pair<bool, Unit *> beingAttacked =
          aiInterface->getWorld()->getUnitUpdater()->unitBeingAttacked(unit);
      if (beingAttacked.first == true) {
        ...
        // printf("~~~~~~~~ Unit [%s - %d] attacked by enemy [%s - %d]
        // act_forenemy [%p] enemy->getCurrField() =
        ...
        if (act_forenemy != NULL) {
          ...
          const AttackStoppedCommandType *asct_forenemy =
              unit->getType()->getFirstAttackStoppedCommand(
                  enemy->getCurrField());
          // printf("~~~~~~~~ Unit [%s - %d] found enemy [%s - %d] asct_forenemy
          // [%p] enemy->getCurrField() =
          ...
          if (asct_forenemy != NULL) {
            bool shouldAttack = true;
            if (unit->getType()->hasSkillClass(scHarvest)) {
              shouldAttack = (attackerWorkersHarvestingCount >
            ...
            if (shouldAttack) {
              //								printf("~~~~~~~~
              // Unit [%s - %d] WILL AttackStoppedCommand [%s - %d]\n",
              ...
              aiInterface->giveCommand(i, asct_forenemy,
                                       beingAttacked.second->getCenteredPos(),
                                       unitGroupCommandId);
              unitSignalledToAttack = true;
            }
          }
    ...
    if (alreadyAttacking == false && act != NULL &&
      ...
      // Mega CPU does not send ( far away ) units which are currently producing
      // something
      if (aiInterface->getControlType() == ctCpuMega ||
          aiInterface->getControlType() == ctNetworkCpuMega) {
        if (!isWarrior) {
  ...
  aiInterface->printLog(2, "Massive attack to pos: " + intToStr(pos.x) + ", " +
                               intToStr(pos.y) + "\n");
}
...
void Ai::returnBase(int unitIndex) {
  ...
  // aiInterface->printLog(1, "Order return to base pos:" + intToStr(pos.x)+",
  // "+intToStr(pos.y)+": "+rrToStr(r)+"\n");
}
...
void Ai::harvest(int unitIndex) {
  ...
  if (rt != NULL) {
    const HarvestCommandType *hct =
        aiInterface->getMyUnit(unitIndex)->getType()->getFirstHarvestCommand(
            rt, aiInterface->getMyUnit(unitIndex)->getFaction());

    Vec2i resPos;
    if (hct != NULL && aiInterface->getNearestSightedResource(
                           rt, aiInterface->getHomeLocation(), resPos, false)) {
      resPos = resPos + Vec2i(random.randRange(-2, 2), random.randRange(-2, 2));
      aiInterface->giveCommand(unitIndex, hct, resPos, -1);
      // aiInterface->printLog(4, "Order harvest pos:" + intToStr(resPos.x)+",
      // "+intToStr(resPos.y)+": "+rrToStr(r)+"\n");
    }
  }
}
...
bool Ai::haveBlockedUnits() {
  ...
  for (int j = 0; j < unitCount; ++j) {
    ...
    if (u->isAlive() && ut->isMobile() && u->getPath() != NULL &&
        (u->getPath()->isBlocked() || u->getPath()->getBlockCount())) {
      Vec2i unitPos = u->getPosNotThreadSafe();

      // printf("#1 AI found blocked unit [%d -
      ...
      // printf("#1 unitImmediatelyBlocked = %d, failureCount = %d, cellCount =
      // %d\n",unitImmediatelyBlocked,failureCount,cellCount);

      if (unitImmediatelyBlocked) {
        // printf("#1 AI unit IS BLOCKED [%d -
        // %s]\n",u->getId(),u->getFullName().c_str());
        if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance)
                .enabled &&
            chrono.getMillis() > 0)
      ...
      if (map->isInside(pos) && map->isInsideSurface(map->toSurfCoords(pos))) {
        if (pos != unitPos) {
          ...
          if (adjacentUnit != NULL &&
              adjacentUnit->getFactionIndex() == unit->getFactionIndex()) {
            if (adjacentUnit->getType()->isMobile() &&
              ...
              std::map<float, std::map<int, const Unit *>>::iterator iterFind1 =
                  signalAdjacentUnits.find(dist);
              if (iterFind1 == signalAdjacentUnits.end()) {
                ...
                std::map<int, const Unit *>::iterator iterFind2 =
...
void Ai::unblockUnits() {
  ...
  for (int idx = 0; idx < unitCount; ++idx) {
    ...
    if (u->isAlive() && ut->isMobile() && u->getPath() != NULL &&
      ...
      // printf("#2 AI found blocked unit [%d -
      // %s]\n",u->getId(),u->getFullName().c_str());
      ...
      // bool unitImmediatelyBlocked = (failureCount == cellCount);
      // printf("#2 unitImmediatelyBlocked = %d, failureCount = %d, cellCount =
      // %d, signalAdjacentUnits.size() =
  ...
  if (signalAdjacentUnits.empty() == false) {
    ...
    for (std::map<float, std::map<int, const Unit *>>::reverse_iterator
             iterMap = signalAdjacentUnits.rbegin();
      ...
      for (std::map<int, const Unit *>::iterator iterMap2 =
        ...
        if (adjacentUnit != NULL &&
            adjacentUnit->getType()->getFirstCtOfClass(ccMove) != NULL) {
          ...
          for (int moveAttempt = 1; moveAttempt <= villageRadius;
               ++moveAttempt) {
            Vec2i pos =
                Vec2i(random.randRange(-villageRadius * 2, villageRadius * 2),
                      random.randRange(-villageRadius * 2, villageRadius * 2)) +
                adjacentUnit->getPosNotThreadSafe();

            bool canUnitMoveToCell = map->aproxCanMove(
                adjacentUnit, adjacentUnit->getPosNotThreadSafe(), pos);
            if (canUnitMoveToCell == true) {
              ...
              if (ct != NULL) {
                ...
                // std::pair<CommandResult,string> r =
                // aiInterface->giveCommand(adjacentUnit,ct, pos,
                // unitGroupCommandId);
                aiInterface->giveCommand(adjacentUnit, ct, pos,
                                         unitGroupCommandId);
              }
...
bool Ai::outputAIBehaviourToConsole() const { return false; }
...
void Ai::saveGame(XmlNode *rootNode) const {
  ...
  aiNode->addAttribute("randomMinWarriorsReached",
  ...
  aiNode->addAttribute("scoutResourceRange", intToStr(scoutResourceRange),
                       mapTagReplacements);
...
void Ai::loadGame(const XmlNode *rootNode, Faction *faction) {
  ...
  randomMinWarriorsReached =
      aiNode->getAttribute("randomMinWarriorsReached")->getIntValue() != 0;
  ...
  for (unsigned int i = 0; i < taskNodeList.size(); ++i) {
    ...
    TaskClass taskClass = static_cast<TaskClass>(
  ...
  for (unsigned int i = 0; i < expansionPositionsNodeList.size(); ++i) {
    ...
    Vec2i pos = Vec2i::strToVec2(
        expansionPositionsNode->getAttribute("pos")->getValue());
  ...
  //	int minMinWarriorsExpandCpuNormal;
  minMinWarriorsExpandCpuNormal =
      aiNode->getAttribute("minMinWarriorsExpandCpuNormal")->getIntValue();
  //	int maxMinWarriors;

...
void Ai::update() {
    ...
    if (aiInterface->getMyFaction()->getFirstSwitchTeamVote() != NULL) {
        ...
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, "AI for faction# %d voted %s [%d] CountCurrent [%d] PercentChance [%d]", aiInterface->getMyFaction()->getIndex(),
                 (voteResult->allowSwitchTeam ? "Yes" : "No"), allowJoinTeam, factionSwitchTeamRequestCountCurrent,
    ...
    for (unsigned int ruleIdx = 0; ruleIdx < aiRules.size(); ++ruleIdx) {
        ...
        if (SystemFlags::getSystemSettingType(SystemFlags::debugPerformance).enabled && chrono.getMillis() > 0)
            SystemFlags::OutputDebug(SystemFlags::debugPerformance, "In [%s::%s Line: %d] took msecs: %lld [ruleIdx = %d]\n", __FILE__, __FUNCTION__, __LINE__,
...
const ResourceType *Ai::getNeededResource(int unitIndex) {
    ...
    for (int i = 0; i < tt->getResourceTypeCount(); ++i) {
        ...
        if (rt->getClass() != rcStatic && rt->getClass() != rcConsumable && r->getAmount() < amount) {
    ...
    aiInterface->printLog(3, szBuf);
    snprintf(szBuf, 8096, "[resource type count %d] Needed resource [%s].", tt->getResourceTypeCount(),
...
void Ai::sendScoutPatrol() {
    ...
    bool ultraResourceAttack =
        (aiInterface->getControlType() == ctCpuUltra || aiInterface->getControlType() == ctNetworkCpuUltra) && random.randRange(0, 2) == 1;
    bool megaResourceAttack = (aiInterface->getControlType() == ctCpuMega || aiInterface->getControlType() == ctNetworkCpuMega) && random.randRange(0, 1) == 1;
...
void Ai::massiveAttack(const Vec2i &pos, Field field, bool ultraAttack) {
    ...
    for (int i = 0; i < unitCount; ++i) {
        ...
        if (aiInterface->getControlType() == ctCpuMega || aiInterface->getControlType() == ctNetworkCpuMega) {
            if (producerWarriorCount > maxProducerWarriors) {
                if (unit->getCommandSize() > 0 && unit->getCurrCommand()->getCommandType() != NULL &&
                    (unit->getCurrCommand()->getCommandType()->getClass() == ccBuild || unit->getCurrCommand()->getCommandType()->getClass() == ccMorph ||
        ...
        if (alreadyAttacking == false && unit->getType()->hasSkillClass(scAttack) &&
            (aiInterface->getControlType() == ctCpuUltra || aiInterface->getControlType() == ctCpuMega || aiInterface->getControlType() == ctNetworkCpuUltra ||
        ...
        if (alreadyAttacking == false && act != NULL && (ultraAttack || isWarrior) && unitSignalledToAttack == false) {
            bool shouldAttack = true;
            if (unit->getType()->hasSkillClass(scHarvest)) {
                shouldAttack = (attackerWorkersHarvestingCount > minWorkerAttackersHarvesting);
                if (shouldAttack == false) {
    ...
    if (aiInterface->getControlType() == ctCpuEasy || aiInterface->getControlType() == ctNetworkCpuEasy) {
    ...
    } else if (aiInterface->getControlType() == ctCpuMega || aiInterface->getControlType() == ctNetworkCpuMega) {
        ...
        if (aiInterface->getControlType() == ctCpuUltra || aiInterface->getControlType() == ctNetworkCpuUltra) {
...
void Ai::returnBase(int unitIndex) {
    ...
    pos = Vec2i(random.randRange(-villageRadius, villageRadius), random.randRange(-villageRadius, villageRadius)) + getRandomHomePosition();
...
void Ai::harvest(int unitIndex) {
    ...
    if (rt != NULL) {
        const HarvestCommandType *hct =
            aiInterface->getMyUnit(unitIndex)->getType()->getFirstHarvestCommand(rt, aiInterface->getMyUnit(unitIndex)->getFaction());
...
void Ai::unblockUnits() {
    ...
    if (signalAdjacentUnits.empty() == false) {
        ...
        for (std::map<float, std::map<int, const Unit *>>::reverse_iterator iterMap = signalAdjacentUnits.rbegin(); iterMap != signalAdjacentUnits.rend();
             ++iterMap) {
            for (std::map<int, const Unit *>::iterator iterMap2 = iterMap->second.begin(); iterMap2 != iterMap->second.end(); ++iterMap2) {
                ...
                if (adjacentUnit != NULL && adjacentUnit->getType()->getFirstCtOfClass(ccMove) != NULL) {
                    ...
                    for (int moveAttempt = 1; moveAttempt <= villageRadius; ++moveAttempt) {
                        Vec2i pos = Vec2i(random.randRange(-villageRadius * 2, villageRadius * 2), random.randRange(-villageRadius * 2, villageRadius * 2)) +

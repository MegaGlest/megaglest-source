...
AiInterface::AiInterface(Game &game, int factionIndex, int teamIndex, int useStartLocation) : fp(NULL) {
    ...
    if (logLevel > 0) {
        ...
        fprintf(fp, "MegaGlest AI log file for Tech [%s] Faction [%s] #%d\n\n", this->gameSettings->getTech().c_str(),
                this->world->getFaction(this->factionIndex)->getType()->getName().c_str(), this->factionIndex);
    }
...
AiInterface::~AiInterface() {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
        SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d] deleting AI factionIndex = %d, teamIndex = %d\n", __FILE__, __FUNCTION__,
                                 __LINE__, this->factionIndex, this->teamIndex);
    cacheUnitHarvestResourceLookup.clear();

    if (workerThread != NULL) {
        workerThread->signalQuit();
        sleep(0);
        if (workerThread->canShutdown(true) == true && workerThread->shutdownAndWait() == true) {
            delete workerThread;
        }
        workerThread = NULL;
    }
...
std::pair<CommandResult, string> AiInterface::giveCommand(const Unit *unit, const CommandType *commandType, const Vec2i &pos, int unitGroupCommandId) {
    ...
    if (ct == NULL) {
        ...
        snprintf(szBuf, 8096,
                 ...
                 __FILE__, __FUNCTION__, __LINE__, unit->getId(), unit->getFullName(false).c_str(), unit->getDesc(false).c_str(),
    ...
    if (executeCommandOverNetwork() == true) {
        result = commander->tryGiveCommand(unit, commandType, pos, unit->getType(), CardinalDir(CardinalDir::NORTH), false, NULL, unitGroupCommandId);
        return result;
    } else {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
            SystemFlags::OutputDebug(SystemFlags::debugSystem, "In [%s::%s Line: %d]\n", __FILE__, __FUNCTION__, __LINE__);
...
Unit *AiInterface::getMyUnitPtr(int unitIndex) {
    if (unitIndex < 0 || unitIndex >= world->getFaction(factionIndex)->getUnitCount()) {
        ...
        snprintf(szBuf, 8096,
                 ...
                 __FILE__, __FUNCTION__, __LINE__, unitIndex, world->getFaction(factionIndex)->getUnitCount());
        throw megaglest_runtime_error(szBuf);
    }
...
bool AiInterface::isResourceNear(const Vec2i &pos, const ResourceType *rt, Vec2i &resourcePos, Faction *faction,
...
bool AiInterface::getNearestSightedResource(const ResourceType *rt, const Vec2i &pos, Vec2i &resultPos, bool usableResourceTypeOnly) {
    Faction *faction = world->getFaction(factionIndex);
    // float tmpDist=0;
    ...
    if (usableResourceTypeOnly == true) {
        ...
        if (iterFind != cacheUnitHarvestResourceLookup.end() && faction->findUnit(iterFind->second) != NULL) {
...
const Unit *AiInterface::getFirstOnSightEnemyUnit(Vec2i &pos, Field &field, int radius) {
    ...
    for (int i = 0; i < world->getFactionCount(); ++i) {
        for (int j = 0; j < world->getFaction(i)->getUnitCount(); ++j) {
            ...
            bool cannotSeeUnit =
                (unit->getType()->hasCellMap() == true && unit->getType()->getAllowEmptyCellMap() == true && unit->getType()->hasEmptyCellMap() == true);

            if (sc->isVisible(teamIndex) && cannotSeeUnit == false && isAlly(unit) == false && unit->isAlive() == true) {
                pos = unit->getPos();
                                ...
                                if (scAI != NULL && cAI != NULL && cAI->getUnit(field) != NULL && sc->isVisible(teamIndex)) {
                                    ...
                                    if (foundEnemyList.find(checkUnit->getId()) == foundEnemyList.end()) {
                                        bool cannotSeeUnitAI =
                                            (checkUnit->getType()->hasCellMap() == true && checkUnit->getType()->getAllowEmptyCellMap() == true &&
                                             checkUnit->getType()->hasEmptyCellMap() == true);
                                        if (cannotSeeUnitAI == false && isAlly(checkUnit) == false && checkUnit->isAlive() == true) {
                                            foundEnemies++;
...
void AiInterface::saveGame(XmlNode *rootNode) const {
    ...
    for (std::map<const ResourceType *, int>::const_iterator iterMap = cacheUnitHarvestResourceLookup.begin(); iterMap != cacheUnitHarvestResourceLookup.end();
         ++iterMap) {

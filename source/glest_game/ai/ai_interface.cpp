...
void AiInterfaceThread::signalQuit() {
  if (this->aiIntf != NULL) {
    MutexSafeWrapper safeMutex(this->aiIntf->getMutex(),
...
AiInterface::AiInterface(Game &game, int factionIndex, int teamIndex,
  ...
  if (getGameReadWritePath(GameConstants::path_logs_CacheLookupKey) != "") {
    aiLogFile = getGameReadWritePath(GameConstants::path_logs_CacheLookupKey) +
                aiLogFile;
  } else {
...
void AiInterface::printLog(int logLevel, const string &s) {
  if (isLogLevelEnabled(logLevel) == true) {
    ...
    MutexSafeWrapper safeMutex(aiMutex,
                               string(__FILE__) + "_" + intToStr(__LINE__));
    // print log to file
    if (fp != NULL) {
      fprintf(fp, "%s\n", logString.c_str());
...
Faction *AiInterface::getMyFaction() { return world->getFaction(factionIndex); }
...
std::pair<CommandResult, string>
AiInterface::giveCommand(int unitIndex, CommandClass commandClass,
                         const Vec2i &pos) {
  assert(this->gameSettings != NULL);
  ...
  if (executeCommandOverNetwork() == true) {
    ...
    result = commander->tryGiveCommand(
        unit, unit->getType()->getFirstCtOfClass(commandClass), pos,
        unit->getType(), CardinalDir(CardinalDir::NORTH));
    return result;
  } else {
    Command *c = new Command(world->getFaction(factionIndex)
                                 ->getUnit(unitIndex)
                                 ->getType()
                                 ->getFirstCtOfClass(commandClass),
                             pos);
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugSystem,
                               ...
                               __LINE__);
    result =
        world->getFaction(factionIndex)->getUnit(unitIndex)->giveCommand(c);
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
      SystemFlags::OutputDebug(SystemFlags::debugSystem,
                               "In [%s::%s Line: %d]\n", __FILE__, __FUNCTION__,
                               __LINE__);
...
std::pair<CommandResult, string>
AiInterface::giveCommand(const Unit *unit, const CommandType *commandType,
                         const Vec2i &pos, int unitGroupCommandId) {
  assert(this->gameSettings != NULL);
  ...
  if (unit == NULL) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] Can not find AI unit in AI factionIndex = "
             "%d. Game out of synch.",
  ...
  if (unitType == NULL) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] Can not find AI unittype with unit id: %d, "
             "AI factionIndex = %d. Game out of synch.",
  ...
  if (commandType == NULL) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] commandType == NULL, unit id: %d, AI "
             "factionIndex = %d. Game out of synch.",
             __FILE__, __FUNCTION__, __LINE__, unit->getId(), factionIndex);
    throw megaglest_runtime_error(szBuf);
  }
  ...
  if (executeCommandOverNetwork() == true) {
    result = commander->tryGiveCommand(unit, commandType, pos, unit->getType(),
                                       CardinalDir(CardinalDir::NORTH), false,
...
std::pair<CommandResult, string>
AiInterface::giveCommand(int unitIndex, const CommandType *commandType,
                         const Vec2i &pos, int unitGroupCommandId) {
  assert(this->gameSettings != NULL);
  ...
  if (unit == NULL) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] Can not find AI unit with index: %d, AI "
             "factionIndex = %d. Game out of synch.",
  ...
  if (unitType == NULL) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] Can not find AI unittype with unit index: "
             "%d, AI factionIndex = %d. Game out of synch.",
             __FILE__, __FUNCTION__, __LINE__, unitIndex, factionIndex);
    throw megaglest_runtime_error(szBuf);
  }
  const CommandType *ct =
  ...
  if (executeCommandOverNetwork() == true) {
    ...
    cmd->setUnitCommandGroupId(unitGroupCommandId);
    result =
        world->getFaction(factionIndex)->getUnit(unitIndex)->giveCommand(cmd);
...
std::pair<CommandResult, string>
AiInterface::giveCommand(int unitIndex, const CommandType *commandType,
                         const Vec2i &pos, const UnitType *ut) {
  assert(this->gameSettings != NULL);
  ...
  if (executeCommandOverNetwork() == true) {
    const Unit *unit = getMyUnit(unitIndex);
    result = commander->tryGiveCommand(unit, commandType, pos, ut,
                                       CardinalDir(CardinalDir::NORTH));
    return result;
  } else {
    if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
    ...
    result = world->getFaction(factionIndex)
                 ->getUnit(unitIndex)
                 ->giveCommand(new Command(commandType, pos, ut,
                                           CardinalDir(CardinalDir::NORTH)));
...
AiInterface::giveCommand(int unitIndex, const CommandType *commandType,
  ...
  if (executeCommandOverNetwork() == true) {
    ...
    result = commander->tryGiveCommand(
        unit, commandType, Vec2i(0), unit->getType(),
        CardinalDir(CardinalDir::NORTH), false, targetUnit);
...
int AiInterface::getMapMaxPlayers() { return world->getMaxPlayers(); }

Vec2i AiInterface::getHomeLocation() {
  return world->getMap()->getStartLocation(
...
int AiInterface::getFactionCount() { return world->getFactionCount(); }
...
//			SurfaceCell *sc=
// map->getSurfaceCell(Map::toSurfCoords(unit->getPos())); 			bool
// cannotSeeUnit = (unit->getType()->hasCellMap() == true &&
//								  unit->getType()->getAllowEmptyCellMap()
//== true &&
//unit->getType()->hasEmptyCellMap() == true); 			if(sc->isVisible(teamIndex) &&
//cannotSeeUnit == false) { 				count++;
//			}
//		}
//	}
//     return count;
...
Unit *AiInterface::getMyUnitPtr(int unitIndex) {
  if (unitIndex < 0 ||
    ...
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "In [%s::%s Line: %d] unitIndex >= "
             "world->getFaction(factionIndex)->getUnitCount(), unitIndex = %d, "
             "world->getFaction(factionIndex)->getUnitCount() = %d",
...
//			bool cannotSeeUnit = (unit->getType()->hasCellMap() ==
//true
//&&
...
const TechTree *AiInterface::getTechTree() { return world->getTechTree(); }

bool AiInterface::isResourceInRegion(const Vec2i &pos, const ResourceType *rt,
  ...
  for (int i = -range; i <= range; ++i) {
    for (int j = -range; j <= range; ++j) {
      ...
      if (map->isInside(pos.x + ii, pos.y + jj)) {
        Resource *r = map->getSurfaceCell(
                             map->toSurfCoords(Vec2i(pos.x + ii, pos.y + jj)))
                          ->getResource();
        if (r != NULL) {
          if (r->getType() == rt) {
...
bool AiInterface::isResourceNear(
    const Vec2i &pos, const ResourceType *rt, Vec2i &resourcePos,
    Faction *faction, bool fallbackToPeersHarvestingSameResource) const {
  const Map *map = world->getMap();
  int size = 1;
  for (int i = -1; i <= size; ++i) {
    for (int j = -1; j <= size; ++j) {
      if (map->isInside(pos.x + i, pos.y + j)) {
        Resource *r =
            map->getSurfaceCell(map->toSurfCoords(Vec2i(pos.x + i, pos.y + j)))
                ->getResource();
        if (r != NULL) {
          if (r->getType() == rt) {
...
bool AiInterface::getNearestSightedResource(const ResourceType *rt,
  ...
  if (usableResourceTypeOnly == true) {
    ...
    std::map<const ResourceType *, int>::iterator iterFind =
        cacheUnitHarvestResourceLookup.find(rt);
...
const Unit *AiInterface::getFirstOnSightEnemyUnit(Vec2i &pos, Field &field,
                                                  int radius) {
  Map *map = world->getMap();
    ...
    for (int j = 0; j < world->getFaction(i)->getUnitCount(); ++j) {
      ...
      if (sc->isVisible(teamIndex) && cannotSeeUnit == false &&
        ...
        if (pos.dist(getHomeLocation()) < radius) {
          printLog(2, "Being attacked at pos " + intToStr(pos.x) + "," +
                          intToStr(pos.y) + "\n");
          ...
          for (int aiX = pos.x - CHECK_RADIUS; aiX < pos.x + CHECK_RADIUS;
               ++aiX) {
            for (int aiY = pos.y - CHECK_RADIUS; aiY < pos.y + CHECK_RADIUS;
                 ++aiY) {
              ...
              if (map->isInside(checkPos) &&
                ...
                if (scAI != NULL && cAI != NULL &&
                    cAI->getUnit(field) != NULL && sc->isVisible(teamIndex)) {
...
bool AiInterface::factionUsesResourceType(const FactionType *factionType,
...
void AiInterface::saveGame(XmlNode *rootNode) const {
  ...
  //    int factionIndex;
  aiInterfaceNode->addAttribute("factionIndex", intToStr(factionIndex),
                                mapTagReplacements);
  //    int teamIndex;
  aiInterfaceNode->addAttribute("teamIndex", intToStr(teamIndex),
                                mapTagReplacements);
  //	//config
  //	bool redir;
  aiInterfaceNode->addAttribute("redir", intToStr(redir), mapTagReplacements);
  //    int logLevel;
  aiInterfaceNode->addAttribute("logLevel", intToStr(logLevel),
                                mapTagReplacements);
  ...
  for (std::map<const ResourceType *, int>::const_iterator iterMap =
    ...
    cacheUnitHarvestResourceLookupNode->addAttribute(
        "key", iterMap->first->getName(), mapTagReplacements);
    cacheUnitHarvestResourceLookupNode->addAttribute(
        "value", intToStr(iterMap->second), mapTagReplacements);
  }
}

// AiInterface::AiInterface(Game &game, int factionIndex, int teamIndex, int
// useStartLocation) {
void AiInterface::loadGame(const XmlNode *rootNode, Faction *faction) {
  ...
  if (aiInterfaceNode != NULL) {
    ...
    //    std::map<const ResourceType *,int> cacheUnitHarvestResourceLookup;
    //	for(std::map<const ResourceType *,int>::const_iterator iterMap =
    // cacheUnitHarvestResourceLookup.begin(); 			iterMap !=
    // cacheUnitHarvestResourceLookup.end(); ++iterMap) { 		XmlNode
    //*cacheUnitHarvestResourceLookupNode =
    // aiInterfaceNode->addChild("cacheUnitHarvestResourceLookup");
    //
    //		cacheUnitHarvestResourceLookupNode->addAttribute("key",iterMap->first->getName(),
    // mapTagReplacements);
    //		cacheUnitHarvestResourceLookupNode->addAttribute("value",intToStr(iterMap->second),
    // mapTagReplacements);
    //	}
  }
}

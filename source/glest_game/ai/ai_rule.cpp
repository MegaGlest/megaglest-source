...
AiRule::AiRule(Ai *ai) { this->ai = ai; }
...
void AiRuleWorkerHarvest::execute() { ai->harvest(stoppedWorkerIndex); }

// =====================================================
//	class AiRuleRefreshHarvester
// =====================================================

AiRuleRefreshHarvester::AiRuleRefreshHarvester(Ai *ai) : AiRule(ai) {
  workerIndex = -1;
}
...
void AiRuleRefreshHarvester::execute() { ai->harvest(workerIndex); }

// =====================================================
//	class AiRuleScoutPatrol
// =====================================================

AiRuleScoutPatrol::AiRuleScoutPatrol(Ai *ai) : AiRule(ai) {}

bool AiRuleScoutPatrol::test() { return ai->isStableBase(); }

void AiRuleScoutPatrol::execute() { ai->sendScoutPatrol(); }
// =====================================================
//	class AiRuleRepair
// =====================================================

AiRuleRepair::AiRuleRepair(Ai *ai) : AiRule(ai) {
  damagedUnitIndex = 0;
  damagedUnitIsCastle = false;
}

double AiRuleRepair::getMinCastleHpRatio() const { return 0.6; }
    ...
    if (u->getHpRatio() < 1.f) {
        ...
        if (ct->getClass() == ccProduce || ct->getClass() == ccMorph) {
          ...
          if (pt != NULL) {
            ...
            if (ut != NULL && ut->hasCommandClass(ccHarvest) == true &&
              ...
              // printf("\n\n\n\n!!!!!! found candidate castle unit to repair
      ...
      if (unitCanProduceWorker == true) {
        ...
        for (int i1 = 0; i1 < aiInterface->getMyUnitCount(); ++i1) {
          ...
          const RepairCommandType *rct = static_cast<const RepairCommandType *>(
              u1->getType()->getFirstCtOfClass(ccRepair));
          // if(rct) printf("\n\n\n\n^^^^^^^^^^ possible repairer unit [%d - %s]
          // current skill [%d] can reapir damaged unit
          // [%d]\n",u1->getId(),u1->getType()->getName().c_str(),u->getCurrSkill()->getClass(),rct->isRepairableUnitType(u->getType()));

          if (rct != NULL) {
            // printf("\n\n\n\n^^^^^^^^^^ possible repairer unit [%d - %s]
            // current skill [%d] can repair damaged unit [%d] Castles hp-ratio
            // =
        ...
        if (candidatedamagedUnitIndex >= 0 &&
          ...
          // printf("\n\n\n\n^^^^^^^^^^ AI test will repair damaged unit [%d -
          // %s]\n",u->getId(),u->getType()->getName().c_str());
  ...
  for (int i = 0; i < aiInterface->getMyUnitCount(); ++i) {
    ...
    if (u->getHpRatio() < 1.f) {
      ...
      for (int i1 = 0; i1 < aiInterface->getMyUnitCount(); ++i1) {
        ...
        if (rct != NULL && (u1->getCurrSkill()->getClass() == scStop ||
                            u1->getCurrSkill()->getClass() == scMove)) {
          if (rct->isRepairableUnitType(u->getType())) {
            damagedUnitIndex = i;
            // random if return or not so we get different targets from time to
            // time
...
void AiRuleRepair::execute() {
  ...
  // printf("\n\n\n\n###^^^^^^^^^^ Looking for repairer for damaged unit [%d -
  ...
  if (minUnitsRepairingCastle > 2) {
    if (damagedUnit->getCurrSkill()->getClass() ==
        scBeBuilt) { // if build is still be build 2 helpers are enough
      minUnitsRepairingCastle = 2;
    }
  ...
  for (int i1 = 0; i1 < aiInterface->getMyUnitCount(); ++i1) {
    ...
    if (cmd != NULL && cmd->getCommandType()->getClass() == ccRepair) {
      // if(cmd->getUnit() != NULL && cmd->getUnit()->getId() ==
      // damagedUnit->getId()) if(cmd->getUnit() != NULL && cmd->getPos() ==
      // damagedUnit->getPosWithCellMapSet())

      if (rct != NULL) {
        // printf("\n\n\n\n^^^^^^^^^^ possible repairer unit [%d - %s] current
        // skill [%d] can repair damaged unit [%d] Castles hp-ratio =
        // %f\n",u1->getId(),u1->getType()->getName().c_str(),u1->getCurrSkill()->getClass(),rct->isRepairableUnitType(u->getType()),u->getHpRatio());

        if (((RepairCommandType *)(cmd->getCommandType()))
                ->isRepairableUnitType(damagedUnit->getType())) {
          // printf("^^^^test^^^^^^ unit is ALREADY repairer unit [%d -
          // %s]\nminUnitsRepairingCastle=%d\n",u1->getId(),
          // u1->getType()->getName().c_str(), minUnitsRepairingCastle);
          unitCountAlreadyRepairingDamagedUnit++;
        }
      }
    }
  ...
  for (int i = 0; i < aiInterface->getMyUnitCount(); ++i) {
    ...
    if (rct != NULL) {
      // printf("\n\n\n\n^^^^^^^^^^ possible excute repairer unit [%d - %s]
      // current skill [%d] can repair damaged unit [%d] Castles hp-ratio =
      ...
      if ((u->getCurrSkill()->getClass() == scStop ||
        ...
        if ((u->getCurrCommand() == NULL ||
             (u->getCurrCommand()->getCommandType()->getClass() != ccBuild &&
          ...
          // AI execute will repair damaged unit [%d - %s] at pos [%s]
          // cellmapPos [%s] using unit [%d
          ...
          */
          ...
          if (unitCountAlreadyRepairingDamagedUnit < minUnitsRepairingCastle) {
            ...
            unitCountAlreadyRepairingDamagedUnit++;
            //						printf(
            //						        "^^^^^^^^^^adding one unit to repair
            //...
            // unitCountAlreadyRepairingDamagedUnit/minUnitsRepairingCastle=%d/%d\n",
            //						        unitCountAlreadyRepairingDamagedUnit,
...
AiRuleReturnBase::AiRuleReturnBase(Ai *ai) : AiRule(ai) {
  stoppedUnitIndex = -1;
}
...
void AiRuleReturnBase::execute() { ai->returnBase(stoppedUnitIndex); }

// =====================================================
//	class AiRuleMassiveAttack
// =====================================================

AiRuleMassiveAttack::AiRuleMassiveAttack(Ai *ai) : AiRule(ai) {
  ultraAttack = false;
  field = fLand;
}
...
AiRuleAddTasks::AiRuleAddTasks(Ai *ai) : AiRule(ai) {}
...
void AiRuleAddTasks::execute() {
  ...
  if (ai->outputAIBehaviourToConsole())
    printf("Add a TASK - AiRuleAddTasks adding ProduceTask(ucWorker) "
           "workerCount = %d, RULE Name[%s]\n",
  ...
  if (workerCount < 4) {
    ...
    if (ai->getAiInterface()->getControlType() == ctCpuMega ||
      ...
      if (ai->outputAIBehaviourToConsole())
        printf("AAA AiRuleAddTasks adding #1 workerCount = %d[%.2f], "
               "buildingCount = %d[%.2f] warriorCount = %d[%.2f] upgradeCount "
               "= %d RULE Name[%s]\n",
      ...
      if (ai->outputAIBehaviourToConsole())
        printf("AAA AiRuleAddTasks adding #2 workerCount = %d[%.2f], "
               "buildingCount = %d[%.2f] warriorCount = %d[%.2f] upgradeCount "
               "= %d RULE Name[%s]\n",
               workerCount, workerRatio, buildingCount, buildingRatio,
               warriorCount, warriorRatio, upgradeCount,
      ...
      if (ai->outputAIBehaviourToConsole())
        printf("AAA AiRuleAddTasks adding #3 workerCount = %d[%.2f], "
               "buildingCount = %d[%.2f] warriorCount = %d[%.2f] upgradeCount "
               "= %d RULE Name[%s]\n",
               workerCount, workerRatio, buildingCount, buildingRatio,
               warriorCount, warriorRatio, upgradeCount,
...
AiRuleBuildOneFarm::AiRuleBuildOneFarm(Ai *ai) : AiRule(ai) { farm = NULL; }

bool AiRuleBuildOneFarm::test() {
  AiInterface *aiInterface = ai->getAiInterface();

  // for all units
  for (int i = 0; i < aiInterface->getMyFactionType()->getUnitTypeCount();
       ++i) {
    ...
    for (int j = 0; j < ut->getCommandTypeCount(); ++j) {
      ...
      if (ct->getClass() == ccProduce) {
        const UnitType *producedType =
            static_cast<const ProduceCommandType *>(ct)->getProducedUnit();
        ...
        for (int k = 0; k < producedType->getCostCount(); ++k) {
          ...
          if (r->getAmount() < 0 && r->getType()->getClass() == rcConsumable &&
            ...
            if (aiInterface->reqsOk(ct) &&
                aiInterface->getMyFaction()->canCreateUnit(ut, true, true,
                                                           true) == true) {
              farm = ut;
              // printf("AiRuleBuildOneFarm returning true, RULE Name[%s] ut
              // [%s] producedType
...
void AiRuleBuildOneFarm::execute() { ai->addPriorityTask(new BuildTask(farm)); }
...
bool AiRuleProduceResourceProducer::test() {
  ...
  for (int i = 0; i < aiInterface->getTechTree()->getResourceTypeCount(); ++i) {
    ...
    if (aiInterface->isLogLevelEnabled(4) == true) {
      char szBuf[8096] = "";
      snprintf(szBuf, 8096, "CONSUMABLE [%s][%d] Testing AI RULE Name[%s]",
               rt->getName().c_str(), r->getBalance(), this->getName().c_str());
      aiInterface->printLog(4, szBuf);
    }

    bool factionUsesResourceType = aiInterface->factionUsesResourceType(
        aiInterface->getMyFactionType(), rt);
  ...
  for (int i = 0; i < aiInterface->getTechTree()->getResourceTypeCount(); ++i) {
    ...
    if (ai->outputAIBehaviourToConsole())
      printf("STATIC [%s][%d] [min %d] Testing AI RULE Name[%s]\n",
             rt->getName().c_str(), r->getAmount(), targetStaticResourceCount,
             this->getName().c_str());
    if (aiInterface->isLogLevelEnabled(4) == true) {
      char szBuf[8096] = "";
      snprintf(
...
AiRuleProduce::AiRuleProduce(Ai *ai) : AiRule(ai) {
  produceTask = NULL;
  newResourceBehaviour =
      Config::getInstance().getBool("NewResourceBehaviour", "false");
}
  ...
  if (produceTask != NULL) {
    ...
    if (ai->outputAIBehaviourToConsole())
      printf("AiRuleProduce producing [%s]\n",
             (produceTask->getUnitType() != NULL
                  ? produceTask->getUnitType()->getName(false).c_str()
                  : "null"));
    if (aiInterface->isLogLevelEnabled(4) == true) {
      char szBuf[8096] = "";
    ...
    // generic produce task, produce random unit that has the skill or produces
    // the resource
...
bool AiRuleProduce::canUnitTypeOfferResourceType(const UnitType *ut,
  ...
  if (ut != NULL && rt != NULL && aiInterface != NULL &&
    ...
    if (r != NULL && r->getAmount() < 0) {
      ...
      for (int commandIndex = 0; commandIndex < ut->getCommandTypeCount();
           ++commandIndex) {
  ...
  if (aiInterface != NULL && aiInterface->isLogLevelEnabled(4) == true) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "canUnitTypeOfferResourceType for unit type [%s] for resource "
             "type [%s] returned: %d",
  ...
  if (aiInterface->getMyFactionType()
    ...
    for (unsigned int i = 0; i < unitList.size(); ++i) {
      ...
      if (ai->getCountOfType(ut) < priorityUnit.second &&
          ...
          aiInterface->getMyFaction()->canCreateUnit(priorityUnit.first, false,
                                                     true, true) == true) {
...
void AiRuleProduce::addUnitTypeToCandidates(
    const UnitType *producedUnit, UnitTypes &ableUnits,
    UnitTypesGiveBack &ableUnitsGiveBack, bool unitCanGiveBackResource) {
  ...
  if (find(ableUnits.begin(), ableUnits.end(), producedUnit) ==
    ...
    if (aiInterface->isLogLevelEnabled(4) == true) {
      char szBuf[8096] = "";
      snprintf(szBuf, 8096,
               "addUnitTypeToCandidates for unit type [%s] "
               "unitCanGiveBackResource = %d",
               producedUnit->getName(false).c_str(), unitCanGiveBackResource);
      aiInterface->printLog(4, szBuf);
    }
  }
}
...
void AiRuleProduce::produceGenericNew(const ProduceTask *pt) {
  ...
  if (pt->getResourceType() != NULL) {
    if (aiInterface->isLogLevelEnabled(4) == true) {
      char szBuf[8096] = "";
      snprintf(szBuf, 8096, "****START: produceGeneric for resource type [%s]",
    ...
    if (aiInterface->getMyFactionType()
            ...
            .size() > 0) {
      const std::vector<FactionType::PairPUnitTypeInt> &unitList =
          aiInterface->getMyFactionType()->getAIBehaviorUnits(aibcWorkerUnits);
      for (unsigned int i = 0; i < unitList.size(); ++i) {
        const FactionType::PairPUnitTypeInt &priorityUnit = unitList[i];
        if (ai->getCountOfType(priorityUnit.first) < priorityUnit.second &&
            aiInterface->getMyFaction()->canCreateUnit(
                priorityUnit.first, false, true, true) == true) {
          ai->addTask(new ProduceTask(priorityUnit.first));
          return;
        }
      }
    ...
    if (aiInterface->getMyFactionType()
            ...
            .size() > 0) {
      const std::vector<FactionType::PairPUnitTypeInt> &unitList =
          aiInterface->getMyFactionType()->getAIBehaviorUnits(aibcWarriorUnits);
      for (unsigned int i = 0; i < unitList.size(); ++i) {
        const FactionType::PairPUnitTypeInt &priorityUnit = unitList[i];
        if (ai->getCountOfType(priorityUnit.first) < priorityUnit.second &&
            aiInterface->getMyFaction()->canCreateUnit(
  ...
  for (int i = 0; i < aiInterface->getMyUnitCount(); ++i) {
        ...
        if (pt->getResourceType() != NULL) {
          ...
          if (r != NULL && r->getAmount() < 0) {
            if (aiInterface->reqsOk(ct) && aiInterface->reqsOk(producedUnit)) {
              // produceIt= true;
              addUnitTypeToCandidates(producedUnit, ableUnits,
    ...
    // This is likely a unit that it BUILT by another and that is handled by a
    // different AI task type: Build
    //		if(produceIt == false && pt->getResourceType() != NULL) {
    //			const Resource *r= ut->getCost(pt->getResourceType());
    //			if(r != NULL) {
    //				if(ai->outputAIBehaviourToConsole()) printf("#2 produceGeneric r
    //=
    //[%s][%d] Testing AI RULE
    // Name[%s]\n",r->getDescription(false).c_str(),r->getAmount(),
    // this->getName().c_str());
    ...
    //					produceIt= true;
    //					addUnitTypeToCandidates(ut,
    //ableUnits,ableUnitsGiveBack, true);
    //				}
    //			}
    //		}
  }
  ...
  if (ableUnits.empty() == false) {
    ...
    if (ai->outputAIBehaviourToConsole())
      printf("produceGeneric !ableUnits.empty(), ableUnits.size() = [%d] "
             "Testing AI RULE Name[%s]\n",
    ...
    if (pt->getResourceType() != NULL && ableUnits.size() > 1) {
      ...
      for (unsigned int i = 0; i < ableUnits.size(); ++i) {
        ...
        if (aiInterface->isLogLevelEnabled(4) == true) {
          char szBuf[8096] = "";
          snprintf(szBuf, 8096,
                   "In produceGeneric for unit type [%s] givesBack: %d count "
                   "of unit type: %d",
      ...
      if (aiInterface->isLogLevelEnabled(4) == true) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, "haveEnoughProducers [%d] haveNonProducers [%d]",
      ...
      if (ai->getCountOfType(ableUnits[i]) == 0) {
        if (ai->getRandom()->randRange(0, 1) == 0) {
          if (aiInterface->isLogLevelEnabled(4) == true) {
            char szBuf[8096] = "";
            snprintf(szBuf, 8096,
                     "In produceGeneric priority adding produce task: %d "
                     "of " MG_SIZE_T_SPECIFIER " for unit type [%s]",
                     i, ableUnits.size(), ableUnits[i]->getName(false).c_str());
            aiInterface->printLog(4, szBuf);
          }
    ...
    // normal case
    int randomUnitTypeIndex =
        ai->getRandom()->randRange(0, (int)ableUnits.size() - 1);
    if (aiInterface->isLogLevelEnabled(4) == true) {
      char szBuf[8096] = "";
      snprintf(
          szBuf, 8096,
          "In produceGeneric randomUnitTypeIndex = %d of " MG_SIZE_T_SPECIFIER
          " equals unit type [%s]",
    ...
    if (aiInterface->isLogLevelEnabled(4) == true) {
      char szBuf[8096] = "";
      snprintf(szBuf, 8096,
               "== END In produceGeneric normal adding produce task for unit "
               "type [%s]",
               ut->getName(false).c_str());
      aiInterface->printLog(4, szBuf);
    }
...
void AiRuleProduce::produceGeneric(const ProduceTask *pt) {
  ...
  if (ableUnits.empty() == false) {
    ...
    // normal case
    ai->addTask(new ProduceTask(
        ableUnits[ai->getRandom()->randRange(0, (int)ableUnits.size() - 1)]));
  }
}
...
void AiRuleProduce::produceSpecific(const ProduceTask *pt) {
  ...
  if (ai->outputAIBehaviourToConsole())
    printf("produceSpecific aiInterface->reqsOk(pt->getUnitType()) = [%s][%d] "
           "Testing AI RULE Name[%s]\n",
  ...
  if (aiInterface->isLogLevelEnabled(4) == true) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "== START produceSpecific aiInterface->reqsOk(pt->getUnitType()) "
             "= [%s][%d] Testing AI RULE Name[%s]",
             pt->getUnitType()->getName().c_str(),
             aiInterface->reqsOk(pt->getUnitType()), this->getName().c_str());
    aiInterface->printLog(4, szBuf);
  }
  ...
  if (aiInterface->reqsOk(pt->getUnitType())) {
      ...
      for (int j = 0; j < ut->getCommandTypeCount(); ++j) {
        ...
        if (ct->getClass() == ccProduce || ct->getClass() == ccMorph) {
          ...
          if (producedUnit == pt->getUnitType()) {
            ...
            if (ai->outputAIBehaviourToConsole())
              printf("produceSpecific aiInterface->reqsOk(ct) = [%d] Testing "
                     "AI RULE Name[%s]\n",
    ...
    if (ai->outputAIBehaviourToConsole())
      printf("produceSpecific aiInterface->checkCosts(pt->getUnitType()) = "
             "[%d] Testing AI RULE Name[%s]\n",
             aiInterface->checkCosts(pt->getUnitType(), ctypeForCostCheck),
             this->getName().c_str());
    if (aiInterface->isLogLevelEnabled(4) == true) {
      char szBuf[8096] = "";
      snprintf(szBuf, 8096,
               "produceSpecific aiInterface->checkCosts(pt->getUnitType()) = "
               "[%d] Testing AI RULE Name[%s]",
               aiInterface->checkCosts(pt->getUnitType(), ctypeForCostCheck),
               this->getName().c_str());
      aiInterface->printLog(4, szBuf);
    ...
    if (producers.empty() == false) {
      ...
      if (ai->outputAIBehaviourToConsole())
        printf("produceSpecific producers.empty() = [%d] Testing AI RULE "
               "Name[%s]\n",
      ...
      for (unsigned int i = 0; i < producers.size(); ++i) {
        ...
        if (currentProducerIndex >= aiInterface->getMyUnitCount()) {
          char szBuf[8096] = "";
          printf("In [%s::%s Line: %d] currentProducerIndex >= "
                 "aiInterface->getMyUnitCount(), currentProducerIndex = %d, "
                 "aiInterface->getMyUnitCount() = %d, i = %u,producers.size() "
                 "= " MG_SIZE_T_SPECIFIER "\n",
                 __FILE__, __FUNCTION__, __LINE__, currentProducerIndex,
                 aiInterface->getMyUnitCount(), i, producers.size());
          snprintf(szBuf, 8096,
                   "In [%s::%s Line: %d] currentProducerIndex >= "
                   "aiInterface->getMyUnitCount(), currentProducerIndex = %d, "
                   "aiInterface->getMyUnitCount() = %u, i = "
                   "%u,producers.size() = " MG_SIZE_T_SPECIFIER "",
                   __FILE__, __FUNCTION__, __LINE__, currentProducerIndex,
                   aiInterface->getMyUnitCount(), i, producers.size());
      ...
      if (aiInterface->getControlType() == ctCpuMega ||
          aiInterface->getControlType() ==
              ctNetworkCpuMega) { // mega cpu trys to balance the commands to
                                  // the producers
        int randomstart =
            ai->getRandom()->randRange(0, (int)producers.size() - 1);
        int lowestCommandCount = 1000000;
        int currentProducerIndex = producers[randomstart];
        int bestIndex = -1;
        // int besti=0;
        int currentCommandCount = 0;
        for (unsigned int i = randomstart; i < producers.size() + randomstart;
             i++) {
          int prIndex = i;
          if (i >= producers.size()) {
          ...
          if (currentProducerIndex >= aiInterface->getMyUnitCount()) {
            ...
            printf("In [%s::%s Line: %d] currentProducerIndex >= "
                   "aiInterface->getMyUnitCount(), currentProducerIndex = %d, "
                   "aiInterface->getMyUnitCount() = %d, i = "
                   "%u,producers.size() = " MG_SIZE_T_SPECIFIER "\n",
                   __FILE__, __FUNCTION__, __LINE__, currentProducerIndex,
                   aiInterface->getMyUnitCount(), i, producers.size());
            snprintf(szBuf, 8096,
                     "In [%s::%s Line: %d] currentProducerIndex >= "
                     "aiInterface->getMyUnitCount(), currentProducerIndex = "
                     "%d, aiInterface->getMyUnitCount() = %u, i = "
                     "%u,producers.size() = " MG_SIZE_T_SPECIFIER "",
                     __FILE__, __FUNCTION__, __LINE__, currentProducerIndex,
                     aiInterface->getMyUnitCount(), i, producers.size());
            throw megaglest_runtime_error(szBuf);
          ...
          if (prIndex >= (int)producers.size()) {
            char szBuf[8096] = "";
            printf("In [%s::%s Line: %d] prIndex >= producers.size(), "
                   "currentProducerIndex = %d, i = %u,producers.size() "
                   "= " MG_SIZE_T_SPECIFIER " \n",
                   __FILE__, __FUNCTION__, __LINE__, prIndex, i,
                   producers.size());
            snprintf(szBuf, 8096,
                     "In [%s::%s Line: %d] currentProducerIndex >= "
                     "producers.size(), currentProducerIndex = %d, i = "
                     "%u,producers.size() = " MG_SIZE_T_SPECIFIER "",
                     __FILE__, __FUNCTION__, __LINE__, currentProducerIndex, i,
                     producers.size());
            throw megaglest_runtime_error(szBuf);
          }
        ...
        if (bestIndex >= 0) {
          if (aiInterface->getMyUnit(bestIndex)->getCommandSize() > 2) {
            ...
            // need to calculate another producer, maybe its better to produce
            // another warrior with another producer
            vector<int> backupProducers;
            // find another producer unit which is free and produce any kind of
            // warrior.
            // for each unit
            for (int i = 0; i < aiInterface->getMyUnitCount(); ++i) {
              const UnitType *ut = aiInterface->getMyUnit(i)->getType();
            ...
            if (!backupProducers.empty()) {
              int randomstart = ai->getRandom()->randRange(
                  0, (int)backupProducers.size() - 1);
              int lowestCommandCount = 1000000;
              ...
              for (unsigned int i = randomstart;
                ...
                if (currentProducerIndex >= aiInterface->getMyUnitCount()) {
                  ...
                  printf(
                      "In [%s::%s Line: %d] currentProducerIndex >= "
                      "aiInterface->getMyUnitCount(), currentProducerIndex = "
                      "%d, aiInterface->getMyUnitCount() = %d, i = "
                      "%u,backupProducers.size() = " MG_SIZE_T_SPECIFIER "\n",
                      __FILE__, __FUNCTION__, __LINE__, currentProducerIndex,
                      aiInterface->getMyUnitCount(), i, backupProducers.size());
                  snprintf(
                      szBuf, 8096,
                      "In [%s::%s Line: %d] currentProducerIndex >= "
                      "aiInterface->getMyUnitCount(), currentProducerIndex = "
                ...
                if (prIndex >= (int)backupProducers.size()) {
                  char szBuf[8096] = "";
                  printf("In [%s::%s Line: %d] prIndex >= "
                         "backupProducers.size(), currentProducerIndex = %d, i "
                         "= %u,backupProducers.size() = " MG_SIZE_T_SPECIFIER
                         " \n",
                         __FILE__, __FUNCTION__, __LINE__, prIndex, i,
              ...
              if (bestIndex >= 0) {
                ...
                int commandIndex =
                    productionCommandIndexes[ai->getRandom()->randRange(
                        0, (int)productionCommandIndexes.size() - 1)];
                ...
                if (ai->outputAIBehaviourToConsole())
                  printf("mega #1 produceSpecific giveCommand to unit [%s] "
                         "commandType [%s]\n",
                         aiInterface->getMyUnit(bestIndex)
                             ->getType()
                             ->getName()
                             .c_str(),
                         ut->getCommandType(commandIndex)->getName().c_str());
                if (aiInterface->isLogLevelEnabled(4) == true) {
                  char szBuf[8096] = "";
                  snprintf(szBuf, 8096,
                           "mega #1 produceSpecific giveCommand to unit [%s] "
                           "commandType [%s]",
                           aiInterface->getMyUnit(bestIndex)
                               ->getType()
                               ->getName()
              ...
              if (producersDefaultCommandType.find(bestIndex) !=
                ...
                int bestCommandTypeCount =
                    (int)producersDefaultCommandType[bestIndex].size();
                int bestCommandTypeIndex =
                    ai->getRandom()->randRange(0, bestCommandTypeCount - 1);

                if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem)
                        .enabled)
                  SystemFlags::OutputDebug(
                      SystemFlags::debugSystem,
                      "In [%s::%s Line: %d] bestCommandTypeIndex = %d, "
                      "bestCommandTypeCount = %d\n",
                      __FILE__, __FUNCTION__, __LINE__, bestCommandTypeIndex,
              ...
              if (ai->outputAIBehaviourToConsole())
                printf("mega #2 produceSpecific giveCommand to unit [%s] "
                       "commandType [%s]\n",
                       aiInterface->getMyUnit(bestIndex)
                           ->getType()
                           ->getName()
              ...
              if (aiInterface->isLogLevelEnabled(4) == true) {
                char szBuf[8096] = "";
                snprintf(szBuf, 8096,
                         "mega #2 produceSpecific giveCommand to unit [%s] "
                         "commandType [%s]",
                         aiInterface->getMyUnit(bestIndex)
                             ->getType()
                             ->getName()
            ...
            if (currentCommandCount == 0) {
              ...
              if (ai->outputAIBehaviourToConsole())
                printf("mega #3 produceSpecific giveCommand to unit [%s] "
                       "commandType [%s]\n",
                       aiInterface->getMyUnit(bestIndex)
                           ->getType()
                           ->getName()
              ...
              if (aiInterface->isLogLevelEnabled(4) == true) {
                char szBuf[8096] = "";
                snprintf(szBuf, 8096,
                         "mega #3 produceSpecific giveCommand to unit [%s] "
                         "commandType [%s]",
                         aiInterface->getMyUnit(bestIndex)
                             ->getType()
                             ->getName()
            ...
            if (producersDefaultCommandType.find(bestIndex) !=
              ...
              defCt =
                  producersDefaultCommandType[bestIndex][bestCommandTypeIndex];
            }
            if (ai->outputAIBehaviourToConsole())
              printf("mega #4 produceSpecific giveCommand to unit [%s] "
                     "commandType [%s]\n",
                     aiInterface->getMyUnit(bestIndex)
                         ->getType()
                         ->getName()
            ...
            if (aiInterface->isLogLevelEnabled(4) == true) {
              char szBuf[8096] = "";
              snprintf(szBuf, 8096,
                       "mega #4 produceSpecific giveCommand to unit [%s] "
                       "commandType [%s]",
                       aiInterface->getMyUnit(bestIndex)
                           ->getType()
                           ->getName()
        ...
        if (producersDefaultCommandType.find(producerIndex) !=
          ...
          defCt =
              producersDefaultCommandType[producerIndex][bestCommandTypeIndex];
        }

        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
          SystemFlags::OutputDebug(
              SystemFlags::debugSystem,
              "In [%s::%s Line: %d] producers.size() = %d, producerIndex = %d, "
              "pIndex = %d, producersDefaultCommandType.size() = %d\n",
              __FILE__, __FUNCTION__, __LINE__, producers.size(), producerIndex,
              pIndex, producersDefaultCommandType.size());

        if (ai->outputAIBehaviourToConsole())
          printf("produceSpecific giveCommand to unit [%s] commandType [%s]\n",
        ...
        if (aiInterface->isLogLevelEnabled(4) == true) {
          ...
          snprintf(szBuf, 8096,
                   ...
                   aiInterface->getMyUnit(producerIndex)
                       ...
                       ->getName()
                       .c_str(),
                   (defCt != NULL ? defCt->getName().c_str() : "(null)"));
          aiInterface->printLog(4, szBuf);
        }
...
AiRuleBuild::AiRuleBuild(Ai *ai) : AiRule(ai) { buildTask = NULL; }
  ...
  if (buildTask != NULL) {
    if (ai->outputAIBehaviourToConsole())
      printf("BUILD AiRuleBuild Unit Name[%s]\n",
             (buildTask->getUnitType() != NULL
                  ? buildTask->getUnitType()->getName(false).c_str()
                  : "null"));
    ...
    // specific building task, build if possible, retry if not enough resources
    // or not position
    else {
...
void AiRuleBuild::buildGeneric(const BuildTask *bt) {
  ...
  if (aiInterface->isLogLevelEnabled(4) == true) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096, "== START: buildGeneric for resource type [%s]",
             (bt->getResourceType() != NULL
  ...
  if (bt->getResourceType() != NULL) {
    if (aiInterface->getMyFactionType()
      ...
      for (unsigned int i = 0; i < unitList.size(); ++i) {
        const FactionType::PairPUnitTypeInt &priorityUnit = unitList[i];
        if (ai->getCountOfType(priorityUnit.first) < priorityUnit.second &&
            aiInterface->getMyFaction()->canCreateUnit(priorityUnit.first, true,
                                                       false, false) == true) {
          // if(ai->getRandom()->randRange(0, 1)==0) {
          ...
          if (aiInterface->isLogLevelEnabled(4) == true) {
            char szBuf[8096] = "";
            snprintf(szBuf, 8096,
                     "In buildGeneric for resource type [%s] "
                     "aibcResourceProducerUnits = " MG_SIZE_T_SPECIFIER
                     " priorityUnit.first: [%s]\n",
                     bt->getResourceType()->getName().c_str(), unitList.size(),
        ...
        for (int k = 0; k < bct->getBuildingCount(); ++k) {
          ...
          if (aiInterface->reqsOk(bct) && aiInterface->reqsOk(building)) {
            ...
            if (rt == NULL || (cost != NULL && cost->getAmount() < 0)) {
              if (find(buildings.begin(), buildings.end(), building) ==
                  buildings.end()) {
...
void AiRuleBuild::buildBestBuilding(const vector<const UnitType *> &buildings) {
  ...
  if (aiInterface->isLogLevelEnabled(4) == true) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "==> START buildBestBuilding buildings.size = " MG_SIZE_T_SPECIFIER
             "\n",
             buildings.size());
    aiInterface->printLog(4, szBuf);
  }
  ...
  if (!buildings.empty()) {
    ...
    for (int i = 0; i < 10 && !buildingFound; ++i) {
      ...
      if (i > 0) {
        ...
        for (int j = 0; j < (int)buildings.size() && buildingFound == false;
             ++j) {
          ...
          if (ai->getCountOfType(building) <= i + 1 && isDefensive(building)) {
            ...
            if (aiInterface->isLogLevelEnabled(4) == true) {
              char szBuf[8096] = "";
              snprintf(szBuf, 8096,
                       "In buildBestBuilding defensive building unit type: "
                       "[%s] i = %d j = %d\n",
        ...
        for (unsigned int j = 0; j < buildings.size() && !buildingFound; ++j) {
          ...
          if (ai->getCountOfType(building) <= i + 1 &&
            ...
            if (aiInterface->isLogLevelEnabled(4) == true) {
              char szBuf[8096] = "";
              snprintf(szBuf, 8096,
                       "In buildBestBuilding warriorproducer building unit "
                       "type: [%s] i = %d j = %u\n",
                       building->getName().c_str(), i, j);
              aiInterface->printLog(4, szBuf);
            }
        ...
        for (unsigned int j = 0; j < buildings.size() && !buildingFound; ++j) {
          ...
          if (ai->getCountOfType(building) <= i + 1 &&
            ...
            if (aiInterface->isLogLevelEnabled(4) == true) {
              char szBuf[8096] = "";
              snprintf(szBuf, 8096,
                       "In buildBestBuilding resourceproducer building unit "
                       "type: [%s] i = %d j = %u\n",
                       building->getName().c_str(), i, j);
              aiInterface->printLog(4, szBuf);
            }
      ...
      for (unsigned int j = 0; j < buildings.size() && !buildingFound; ++j) {
        ...
        if (ai->getCountOfType(building) <= i) {
          ...
          if (aiInterface->isLogLevelEnabled(4) == true) {
            char szBuf[8096] = "";
            snprintf(szBuf, 8096,
                     "In buildBestBuilding ANY building unit type: [%s] i = %d "
                     "j = %u\n",
                     building->getName().c_str(), i, j);
            aiInterface->printLog(4, szBuf);
          }
  ...
  if (aiInterface->isLogLevelEnabled(4) == true) {
    char szBuf[8096] = "";
    snprintf(szBuf, 8096,
             "==> END buildBestBuilding buildings.size = " MG_SIZE_T_SPECIFIER
             "\n",
             buildings.size());
    aiInterface->printLog(4, szBuf);
  }
  ...
  if (aiInterface->isLogLevelEnabled(4) == true) {
    ...
    snprintf(
        ...
        (bt->getResourceType() != NULL
             ? bt->getResourceType()->getName().c_str()
             : "null"),
        (bt->getUnitType() != NULL ? bt->getUnitType()->getName(false).c_str()
                                   : "null"));
    aiInterface->printLog(4, szBuf);
  }
  ...
  if (aiInterface->reqsOk(bt->getUnitType())) {
    ...
    if (aiInterface->checkCosts(bt->getUnitType(), NULL) == false) {
      if (aiInterface->isLogLevelEnabled(4) == true) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096,
                 "In buildSpecific for resource type [%s] checkcosts == false "
                 "RETRYING",
                 (bt->getResourceType() != NULL
                      ? bt->getResourceType()->getName().c_str()
                      : "null"));
        ...
        for (int j = 0; j < ut->getCommandTypeCount(); ++j) {
          ...
          if (ct->getClass() == ccBuild) {
            const BuildCommandType *bct =
                static_cast<const BuildCommandType *>(ct);
    ...
    if (builders.empty() == false) {
      ...
      if (bt->getForcePos() == false) {
        ...
        vector<Unit *> enemies;
        ai->getAiInterface()->getWorld()->getUnitUpdater()->findEnemiesForCell(
            searchPos, bt->getUnitType()->getSize(), enemySightDistanceToAvoid,
        ...
        if (enemies.empty() == false) {
          for (int i1 = 0; i1 < 25 && enemies.empty() == false; ++i1) {
            for (int j1 = 0; j1 < 25 && enemies.empty() == false; ++j1) {
              ...
              const int spacing = 1;
              if (ai->getAiInterface()->isFreeCells(
                      tryPos - Vec2i(spacing),
                      bt->getUnitType()->getSize() + spacing * 2, fLand)) {
                enemies.clear();
                ai->getAiInterface()
                    ->getWorld()
                    ->getUnitUpdater()
                    ->findEnemiesForCell(tryPos, bt->getUnitType()->getSize(),
      ...
      if (ai->findPosForBuilding(bt->getUnitType(), searchPos, pos)) {
        ...
        if (buildersDefaultCommandType.find(builderIndex) !=
          ...
          defBct =
              buildersDefaultCommandType[builderIndex][bestCommandTypeIndex];
        }
        if (SystemFlags::getSystemSettingType(SystemFlags::debugSystem).enabled)
          SystemFlags::OutputDebug(SystemFlags::debugSystem,
                                   "In [%s::%s Line: %d] builderIndex = %d, "
                                   "bIndex = %d, defBct = %p\n",
...
AiRuleUpgrade::AiRuleUpgrade(Ai *ai) : AiRule(ai) { upgradeTask = NULL; }
...
void AiRuleUpgrade::upgradeGeneric(const UpgradeTask *upgt) {
  ...
  if (aiInterface->getMyFactionType()->getAIBehaviorUpgrades().size() > 0) {
    const std::vector<const UpgradeType *> &upgradeList =
      ...
      for (int k = 0; k < aiInterface->getMyUnitCount(); ++k) {
        ...
        for (int j = 0; j < ut->getCommandTypeCount(); ++j) {
          ...
          if (ct->getClass() == ccUpgrade) {
            const UpgradeCommandType *upgct =
                dynamic_cast<const UpgradeCommandType *>(ct);
            if (upgct != NULL) {
  ...
  if (!upgrades.empty()) {
    ai->addTask(new UpgradeTask(
        upgrades[ai->getRandom()->randRange(0, (int)upgrades.size() - 1)]));
  }
}
...
AiRuleExpand::AiRuleExpand(Ai *ai) : AiRule(ai) { storeType = NULL; }
  ...
  for (int i = 0; i < aiInterface->getTechTree()->getResourceTypeCount(); ++i) {
    ...
    if (rt->getClass() == rcTech) {
      ...
      if (factionUsesResourceType == true) {
        ...
        if (aiInterface->getNearestSightedResource(
                rt, aiInterface->getHomeLocation(), expandPos, true)) {
          ...
          for (int j = 0; j < unitCount; ++j) {
            ...
            if (ut->getStore(rt) > 0) {
              storeType = ut;
              int distance =
                  static_cast<int>(u->getPosNotThreadSafe().dist(expandPos));
...
AiRuleUnBlock::AiRuleUnBlock(Ai *ai) : AiRule(ai) {}

bool AiRuleUnBlock::test() { return ai->haveBlockedUnits(); }

void AiRuleUnBlock::execute() { ai->unblockUnits(); }

      ...
      for (int i1 = 0; i1 < aiInterface->getMyUnitCount(); ++i1) {
        ...
        if (rct != NULL && (u1->getCurrSkill()->getClass() == scStop ||
          ...
          if (rct->isRepairableUnitType(u->getType())) {
            ...
            // time
            if (ai->getRandom()->randRange(0, 1) == 0) return true;
          }
        }
      }
    }
...
void AiRuleAddTasks::execute() {
  ...
  if (ai->outputAIBehaviourToConsole())
    printf(
        "Add a TASK - AiRuleAddTasks adding ProduceTask(ucWorker) workerCount "
        "= %d, RULE Name[%s]\n",
  ...
  if (workerCount < 4) {
    if (ai->outputAIBehaviourToConsole())
      printf(
          "AAA AiRuleAddTasks adding ProduceTask(ucWorker) workerCount = %d, "
          "RULE Name[%s]\n",
          workerCount, this->getName().c_str());
    ...
    if (ai->getAiInterface()->getControlType() == ctCpuMega ||
      ...
      if (ai->outputAIBehaviourToConsole())
        printf(
            "AAA AiRuleAddTasks adding #1 workerCount = %d[%.2f], "
            "buildingCount = %d[%.2f] warriorCount = %d[%.2f] upgradeCount = "
            "%d RULE Name[%s]\n",
            workerCount, workerRatio, buildingCount, buildingRatio,
            warriorCount, warriorRatio, upgradeCount, this->getName().c_str());
      ...
      if (ai->outputAIBehaviourToConsole())
        printf(
            "AAA AiRuleAddTasks adding #2 workerCount = %d[%.2f], "
            "buildingCount = %d[%.2f] warriorCount = %d[%.2f] upgradeCount = "
            "%d RULE Name[%s]\n",
            workerCount, workerRatio, buildingCount, buildingRatio,
            warriorCount, warriorRatio, upgradeCount, this->getName().c_str());
      ...
      if (ai->outputAIBehaviourToConsole())
        printf(
            "AAA AiRuleAddTasks adding #3 workerCount = %d[%.2f], "
            "buildingCount = %d[%.2f] warriorCount = %d[%.2f] upgradeCount = "
            "%d RULE Name[%s]\n",
            workerCount, workerRatio, buildingCount, buildingRatio,
            warriorCount, warriorRatio, upgradeCount, this->getName().c_str());
...
void AiRuleProduce::produceGenericNew(const ProduceTask *pt) {
  ...
  for (int i = 0; i < aiInterface->getMyUnitCount(); ++i) {
    ...
    //			if(r != NULL) {
    //				if(ai->outputAIBehaviourToConsole()) printf("#2
    // produceGeneric r = [%s][%d] Testing AI RULE
    // Name[%s]\n",r->getDescription(false).c_str(),r->getAmount(),
    // this->getName().c_str());
    //			}
    //
  ...
  if (ableUnits.empty() == false) {
    if (ai->outputAIBehaviourToConsole())
      printf(
          "produceGeneric !ableUnits.empty(), ableUnits.size() = [%d] Testing "
          "AI RULE Name[%s]\n",
...
void AiRuleProduce::produceSpecific(const ProduceTask *pt) {
  ...
  if (ai->outputAIBehaviourToConsole())
    printf(
        "produceSpecific aiInterface->reqsOk(pt->getUnitType()) = [%s][%d] "
  ...
  if (aiInterface->reqsOk(pt->getUnitType())) {
      ...
      for (int j = 0; j < ut->getCommandTypeCount(); ++j) {
        ...
        if (ct->getClass() == ccProduce || ct->getClass() == ccMorph) {
          ...
          if (producedUnit == pt->getUnitType()) {
            if (ai->outputAIBehaviourToConsole())
              printf(
                  "produceSpecific aiInterface->reqsOk(ct) = [%d] Testing AI "
                  "RULE Name[%s]\n",
    ...
    if (ai->outputAIBehaviourToConsole())
      printf(
          "produceSpecific aiInterface->checkCosts(pt->getUnitType()) = [%d] "
          "Testing AI RULE Name[%s]\n",
    ...
    if (producers.empty() == false) {
      if (ai->outputAIBehaviourToConsole())
        printf(
            "produceSpecific producers.empty() = [%d] Testing AI RULE "
            "Name[%s]\n",
      ...
      if (aiInterface->getControlType() == ctCpuMega ||
        ...
        for (unsigned int i = randomstart; i < producers.size() + randomstart;
          ...
          if (prIndex >= (int)producers.size()) {
            char szBuf[8096] = "";
            printf(
                "In [%s::%s Line: %d] prIndex >= producers.size(), "
            ...
            if (!backupProducers.empty()) {
              ...
              for (unsigned int i = randomstart;
                ...
                if (prIndex >= (int)backupProducers.size()) {
                  char szBuf[8096] = "";
                  printf(
                      "In [%s::%s Line: %d] prIndex >= backupProducers.size(), "
                      "currentProducerIndex = %d, i = "
                      "%u,backupProducers.size() = " MG_SIZE_T_SPECIFIER " \n",

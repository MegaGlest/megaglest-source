...
bool AiRuleRepair::test() {
    ...
    for (int i = 0; i < aiInterface->getMyUnitCount(); ++i) {
        ...
        if (u->getHpRatio() < 1.f) {
            ...
            for (int j = 0; unitCanProduceWorker == false && j < u->getType()->getCommandTypeCount(); ++j) {
                ...
                if (ct->getClass() == ccProduce || ct->getClass() == ccMorph) {
                    ...
                    if (pt != NULL) {
                        ...
                        if (ut != NULL && ut->hasCommandClass(ccHarvest) == true && u->getType()->getStoredResourceCount() > 0) {
            ...
            if (unitCanProduceWorker == true) {
                ...
                for (int i1 = 0; i1 < aiInterface->getMyUnitCount(); ++i1) {
                    ...
                    if (rct != NULL) {
                        ...
                        if (u1->getCurrSkill()->getClass() == scStop || u1->getCurrSkill()->getClass() == scMove || u->getHpRatio() <= minCastleHpRatio) {
...
void AiRuleRepair::execute() {
    ...
    if (aiInterface->getControlType() == ctCpuEasy || aiInterface->getControlType() == ctNetworkCpuEasy) {
        if (!damagedUnitIsCastle) {
            // cpu easy does not repair!
            minUnitsRepairingCastle = 0;
        }
    }
    if (aiInterface->getControlType() == ctCpu || aiInterface->getControlType() == ctNetworkCpu) {
        if (!damagedUnitIsCastle) {
        ...
        if (rct != NULL) {
            ...
            if ((u->getCurrSkill()->getClass() == scStop || u->getCurrSkill()->getClass() == scMove || damagedUnit->getHpRatio() <= minCastleHpRatio)) {
                if ((u->getCurrCommand() == NULL ||
                     (u->getCurrCommand()->getCommandType()->getClass() != ccBuild && u->getCurrCommand()->getCommandType()->getClass() != ccProduce)) &&
...
void AiRuleAddTasks::execute() {
    ...
    if (workerCount < 4) {
        if (ai->outputAIBehaviourToConsole())
            printf("AAA AiRuleAddTasks adding ProduceTask(ucWorker) workerCount = %d, "
                   "RULE Name[%s]\n",
                   workerCount, this->getName().c_str());

        ai->addPriorityTask(new ProduceTask(ucWorker));
    } else {
        if (ai->getAiInterface()->getControlType() == ctCpuMega || ai->getAiInterface()->getControlType() == ctNetworkCpuMega) {
            if (ai->outputAIBehaviourToConsole())
        ...
        } else if (ai->getAiInterface()->getControlType() == ctCpuEasy || ai->getAiInterface()->getControlType() == ctNetworkCpuEasy) { // Easy CPU
...
AiRuleBuildOneFarm::AiRuleBuildOneFarm(Ai *ai) : AiRule(ai) {
    farm = NULL;
}
...
AiRuleProduceResourceProducer::AiRuleProduceResourceProducer(Ai *ai) : AiRule(ai) {
...
bool AiRuleProduceResourceProducer::test() {
    ...
    for (int i = 0; i < aiInterface->getTechTree()->getResourceTypeCount(); ++i) {
        ...
        if (aiInterface->isLogLevelEnabled(4) == true) {
            char szBuf[8096] = "";
            snprintf(szBuf, 8096, "STATIC resource check [%s][%d] [min %d] Testing AI RULE Name[%s]", rt->getName().c_str(), r->getAmount(),
...
bool AiRuleProduce::canUnitTypeOfferResourceType(const UnitType *ut, const ResourceType *rt) {
    ...
    if (aiInterface != NULL && aiInterface->isLogLevelEnabled(4) == true) {
        ...
        snprintf(szBuf, 8096,
                 ...
                 "type [%s] returned: %d",
                 (ut != NULL ? ut->getName(false).c_str() : "n/a"), (rt != NULL ? rt->getName(false).c_str() : "n/a"), unitTypeOffersResourceType);
        aiInterface->printLog(4, szBuf);
    }
...
bool AiRuleProduce::setAIProduceTaskForResourceType(const ProduceTask *pt, AiInterface *aiInterface) {
    ...
    if (aiInterface->getMyFactionType()->getAIBehaviorUnits(aibcResourceProducerUnits).size() > 0) {
        ...
        for (unsigned int i = 0; i < unitList.size(); ++i) {
            ...
            if (ai->getCountOfType(ut) < priorityUnit.second && canUnitTypeOfferResourceType(ut, pt->getResourceType()) == true &&
...
void AiRuleProduce::addUnitTypeToCandidates(const UnitType *producedUnit, UnitTypes &ableUnits, UnitTypesGiveBack &ableUnitsGiveBack,
...
void AiRuleProduce::produceGenericNew(const ProduceTask *pt) {
    ...
    if (ableUnits.empty() == false) {
        if (ai->outputAIBehaviourToConsole())
            printf("produceGeneric !ableUnits.empty(), ableUnits.size() = [%d] Testing "
                   "AI RULE Name[%s]\n",
            ...
            for (unsigned int i = 0; i < ableUnits.size(); ++i) {
                ...
                if (aiInterface->isLogLevelEnabled(4) == true) {
                    ...
                    snprintf(szBuf, 8096,
                             ...
                             "of unit type: %d",
                             ut->getName(false).c_str(), givesBack, ai->getCountOfType(ut));
                    aiInterface->printLog(4, szBuf);
                }
            }
        ...
        if (aiInterface->isLogLevelEnabled(4) == true) {
            char szBuf[8096] = "";
            snprintf(szBuf, 8096, "In produceGeneric randomUnitTypeIndex = %d of " MG_SIZE_T_SPECIFIER " equals unit type [%s]", randomUnitTypeIndex,
                     ableUnits.size() - 1, ableUnits[randomUnitTypeIndex]->getName(false).c_str());
            aiInterface->printLog(4, szBuf);
        }
...
void AiRuleProduce::produceSpecific(const ProduceTask *pt) {
    ...
    if (aiInterface->reqsOk(pt->getUnitType())) {
            ...
            for (int j = 0; j < ut->getCommandTypeCount(); ++j) {
                ...
                if (ct->getClass() == ccProduce || ct->getClass() == ccMorph) {
                    ...
                    if (producedUnit == pt->getUnitType()) {
                        if (ai->outputAIBehaviourToConsole())
                            printf("produceSpecific aiInterface->reqsOk(ct) = [%d] Testing AI "
                                   "RULE Name[%s]\n",
        ...
        if (ai->outputAIBehaviourToConsole())
            printf("produceSpecific aiInterface->checkCosts(pt->getUnitType()) = [%d] "
                   "Testing AI RULE Name[%s]\n",
        ...
        if (producers.empty() == false) {
            ...
            if (aiInterface->getControlType() == ctCpuMega || aiInterface->getControlType() == ctNetworkCpuMega) { // mega cpu trys to balance the commands to
                                                                                                                   // the producers
                ...
                for (unsigned int i = randomstart; i < producers.size() + randomstart; i++) {
                    ...
                    if (currentCommandCount == 1 &&
                        aiInterface->getMyUnit(currentProducerIndex)->getCurrCommand()->getCommandType()->getClass() == ccStop) { // special for non buildings
                        ...
                        for (int i = 0; i < aiInterface->getMyUnitCount(); ++i) {
                            ...
                            for (int j = 0; j < ut->getCommandTypeCount(); ++j) {
                                ...
                                if (ct->getClass() == ccProduce) {
                                    ...
                                    if (unitType->hasSkillClass(scAttack) && !unitType->hasCommandClass(ccHarvest) &&
                        ...
                        if (!backupProducers.empty()) {
                            ...
                            for (unsigned int i = randomstart; i < backupProducers.size() + randomstart; i++) {
                                ...
                                if (prIndex >= (int)backupProducers.size()) {
                                    char szBuf[8096] = "";
                                    printf("In [%s::%s Line: %d] prIndex >= backupProducers.size(), "
                                    ...
                                    snprintf(szBuf, 8096,
                                             ...
                                             "%u,backupProducers.size() = " MG_SIZE_T_SPECIFIER "",
                                             __FILE__, __FUNCTION__, __LINE__, currentProducerIndex, i, backupProducers.size());
                                    throw megaglest_runtime_error(szBuf);
                                }

                                int currentCommandCount = aiInterface->getMyUnit(currentProducerIndex)->getCommandSize();
                                if (currentCommandCount == 1 && aiInterface->getMyUnit(currentProducerIndex)->getCurrCommand()->getCommandType()->getClass() ==
                            ...
                            if (bestIndex >= 0) {
                                ...
                                if (ai->outputAIBehaviourToConsole())
                                    printf("mega #1 produceSpecific giveCommand to unit [%s] "
                                           "commandType [%s]\n",
                                           aiInterface->getMyUnit(bestIndex)->getType()->getName().c_str(),
                ...
                if (ai->outputAIBehaviourToConsole())
                    printf("produceSpecific giveCommand to unit [%s] commandType [%s]\n", aiInterface->getMyUnit(producerIndex)->getType()->getName().c_str(),
                           (defCt != NULL ? defCt->getName().c_str() : "n/a"));
                if (aiInterface->isLogLevelEnabled(4) == true) {
                    char szBuf[8096] = "";
                    snprintf(szBuf, 8096, "produceSpecific giveCommand to unit [%s] commandType [%s]",
                             aiInterface->getMyUnit(producerIndex)->getType()->getName().c_str(), (defCt != NULL ? defCt->getName().c_str() : "(null)"));
                    aiInterface->printLog(4, szBuf);
                }
...
AiRuleBuild::AiRuleBuild(Ai *ai) : AiRule(ai) {
    buildTask = NULL;
}
                ...
                if (ai->getCountOfType(priorityUnit.first) < priorityUnit.second &&
                    ...
                    if (aiInterface->isLogLevelEnabled(4) == true) {
                        ...
                        snprintf(szBuf, 8096,
                                 ...
                                 "aibcResourceProducerUnits = " MG_SIZE_T_SPECIFIER " priorityUnit.first: [%s]\n",
...
void AiRuleBuild::buildSpecific(const BuildTask *bt) {
    ...
    if (aiInterface->isLogLevelEnabled(4) == true) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, "== START: buildSpecific for resource type [%s] bt->getUnitType() [%s]",
                 (bt->getResourceType() != NULL ? bt->getResourceType()->getName().c_str() : "null"),
                 (bt->getUnitType() != NULL ? bt->getUnitType()->getName(false).c_str() : "null"));
        aiInterface->printLog(4, szBuf);
    }
    ...
    if (aiInterface->reqsOk(bt->getUnitType())) {
        ...
        if (builders.empty() == false) {
            ...
            if (bt->getForcePos() == false) {
                ...
                vector<Unit *> enemies;
                ai->getAiInterface()->getWorld()->getUnitUpdater()->findEnemiesForCell(searchPos, bt->getUnitType()->getSize(), enemySightDistanceToAvoid,
                        ...
                        for (int j1 = 0; j1 < 25 && enemies.empty() == false; ++j1) {
                            ...
                            if (ai->getAiInterface()->isFreeCells(tryPos - Vec2i(spacing), bt->getUnitType()->getSize() + spacing * 2, fLand)) {
                                ...
                                ai->getAiInterface()->getWorld()->getUnitUpdater()->findEnemiesForCell(
                                    tryPos, bt->getUnitType()->getSize(), enemySightDistanceToAvoid, ai->getAiInterface()->getMyFaction(), enemies, true);
...
AiRuleUpgrade::AiRuleUpgrade(Ai *ai) : AiRule(ai) {
    upgradeTask = NULL;
}
...
AiRuleExpand::AiRuleExpand(Ai *ai) : AiRule(ai) {
    storeType = NULL;
}

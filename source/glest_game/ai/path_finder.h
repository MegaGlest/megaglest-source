...
class PathFinder {
 public:
  class BadUnitNodeList {
   public:
    ...
    inline bool isPosBad(const Vec2i &pos1, const Vec2i &pos2) {
      ...
      std::map<Vec2i, std::map<Vec2i, bool> >::iterator iterFind =
          badPosList.find(pos1);
  ...
  class FactionStateManager {
   ...
   public:
    FactionStateManager() { init(); }
    ~FactionStateManager() { clear(); }
    ...
    void clear() {
      for (unsigned int index = 0; index < (unsigned int)factions.size();
           ++index) {
    ...
    int size() { return (int)factions.size(); }
  };
 ...
 private:
  ...
  TravelState aStar(Unit *unit, const Vec2i &finalPos, bool inBailout,
                    int frameIndex, int maxNodeCount = -1,
                    uint32 *searched_node_count = NULL);
  ...
  inline static Node *minHeuristicFastLookup(FactionState &faction) {
    ...
    faction.openNodesList.begin()->second.erase(
        faction.openNodesList.begin()->second.begin());
  ...
  inline bool processNode(Unit *unit, Node *node, const Vec2i finalPos, int x,
                          int y, bool &nodeLimitReached, int maxNodeCount) {
    bool result = false;
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
      ...
      snprintf(szBuf, 8096,
               ...
               allowUnitMoveSoon, maxNodeCount, node->pos.getString().c_str(),
               finalPos.getString().c_str(), sucPos.getString().c_str(),
               faction.openPosList.size(), faction.closedNodesList.size());
    ...
    if (foundOpenPosForPos == false && allowUnitMoveSoon) {
      ...
      if (sucNode != NULL) {
        ...
        sucNode->prev = node;
        sucNode->next = NULL;
        sucNode->exploredCell = map->getSurfaceCell(Map::toSurfCoords(sucPos))
  ...
  inline bool canUnitMoveSoon(Unit *unit, const Vec2i &pos1,
                              const Vec2i &pos2) {
    bool result = map->aproxCanMoveSoon(unit, pos1, pos2);
    return result;
  }

  inline void doAStarPathSearch(
      bool &nodeLimitReached, int &whileLoopCount, int &unitFactionIndex,
      bool &pathFound, Node *&node, const Vec2i &finalPos,
      const std::map<Vec2i, bool> &closedNodes,
      const std::map<Vec2i, Vec2i> &cameFrom,
      const std::map<std::pair<Vec2i, Vec2i>, bool> &canAddNode, Unit *&unit,
    ...
    if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
      ...
      char szBuf[8096] = "";
      snprintf(szBuf, 8096,
               "In doAStarPathSearch() nodeLimitReached %d whileLoopCount %d "
               "unitFactionIndex %d pathFound %d maxNodeCount %d",
    ...
    while (nodeLimitReached == false) {
      ...
      if (faction.openNodesList.empty() == true) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
          ...
          char szBuf[8096] = "";
          snprintf(szBuf, 8096,
                   "In doAStarPathSearch() nodeLimitReached %d whileLoopCount "
                   "%d unitFactionIndex %d pathFound %d maxNodeCount %d",
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
        ...
        snprintf(szBuf, 8096,
                 "In doAStarPathSearch() nodeLimitReached %d whileLoopCount %d "
                 "unitFactionIndex %d pathFound %d maxNodeCount %d node->pos = "
                 "%s finalPos = %s node->exploredCell = %d",
                 nodeLimitReached, whileLoopCount, unitFactionIndex, pathFound,
                 maxNodeCount, node->pos.getString().c_str(),
      ...
      // Invalid access to FactionState random from outside main thread current
      // id = " +
      ...
      if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch)
          ...
          SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynchMax)
                  .enabled == true) {
        char szBuf[8096] = "";
        snprintf(szBuf, 8096, "In doAStarPathSearch() tryDirection %d",
                 tryDirection);
      ...
      if (tryDirection == 4) {
        for (int i = 1; i >= -1 && nodeLimitReached == false; --i) {
          for (int j = -1; j <= 1 && nodeLimitReached == false; ++j) {
            if (processNode(unit, node, finalPos, i, j, nodeLimitReached,
                            maxNodeCount) == false) {
              failureCount++;
            }
            cellCount++;

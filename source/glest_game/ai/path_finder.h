...
class PathFinder {
  ...
  private:
    ...
    inline bool processNode(Unit *unit, Node *node, const Vec2i finalPos, int x, int y, bool &nodeLimitReached, int maxNodeCount) {
        bool result = false;
    ...
    inline void doAStarPathSearch(bool &nodeLimitReached, int &whileLoopCount, int &unitFactionIndex, bool &pathFound, Node *&node, const Vec2i &finalPos,
                                  const std::map<Vec2i, bool> &closedNodes, const std::map<Vec2i, Vec2i> &cameFrom,
                                  const std::map<std::pair<Vec2i, Vec2i>, bool> &canAddNode, Unit *&unit, int &maxNodeCount, int curFrameIndex) {
        if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true &&
            SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynchMax).enabled == true) {
            char szBuf[8096] = "";
            snprintf(szBuf, 8096,
        ...
        while (nodeLimitReached == false) {
            ...
            if (SystemFlags::getSystemSettingType(SystemFlags::debugWorldSynch).enabled == true &&
                ...
                snprintf(szBuf, 8096,
                         ...
                         nodeLimitReached, whileLoopCount, unitFactionIndex, pathFound, maxNodeCount, node->pos.getString().c_str(),

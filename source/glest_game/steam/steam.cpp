...
struct SteamPrivate {
    ...
    void updateAchievement(const char *name, bool isSet) { achievements[name] = isSet; }
    ...
    bool initialized() {
        return !userName.empty() && !lang.empty() && (int)stats.size() >= EnumParser<SteamStatName>::getCount() &&

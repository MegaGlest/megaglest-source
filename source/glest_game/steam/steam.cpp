...
std::map<std::string, SteamStatType> Steam::SteamStatNameTypes =
    Steam::create_map();
...
struct SteamPrivate {
  ...
  static void setDebugEnabled(bool value) { debugEnabled = value; }

  void refreshAllStats() {
    achievements.clear();
    stats.clear();

    for (int index = 0; index < EnumParser<SteamStatName>::getCount();
         ++index) {
    ...
    for (int index = 0; index < EnumParser<SteamAchievementName>::getCount();
         ++index) {
  ...
  bool isAchievementSet(const char *name) { return achievements[name]; }

  void updateStat(const char *name, double value) { stats[name] = value; }
    ...
    while ((e = STEAMSHIM_pump()) != 0) {
      ...
      switch (e->type) {
        case SHIMEVENT_GETACHIEVEMENT:
          if (debugEnabled)
            printf(
                "\nGot Shim event SHIMEVENT_GETACHIEVEMENT name [%s] value "
                "[%d] isOk = %d\n",
        ...
        case SHIMEVENT_SETACHIEVEMENT:
          if (debugEnabled)
            printf(
                "\nGot Shim event SHIMEVENT_SETACHIEVEMENT for name [%s] value "
                "[%d] isOk = %d\n",
                e->name, e->ivalue, e->okay);
          break;
        case SHIMEVENT_GETPERSONANAME:
          if (debugEnabled)
            printf(
                "\nGot Shim event SHIMEVENT_GETPERSONANAME isOk = %d value "
                "[%s]\n",
                e->okay, e->name);
          userName = e->name;
          break;
        ...
        case SHIMEVENT_GETSTATI:
          if (debugEnabled)
            printf(
                "\nGot Shim event SHIMEVENT_GETSTATI for stat [%s] value [%d] "
                "isOk = %d\n",
                e->name, e->ivalue, e->okay);
          if (e->okay) {
        ...
        case SHIMEVENT_GETSTATF:
          if (debugEnabled)
            printf(
                "\nGot Shim event SHIMEVENT_GETSTATF for stat [%s] value [%f] "
                "isOk = %d\n",
        ...
        case SHIMEVENT_SETSTATI:
          if (debugEnabled)
            printf(
                "\nGot Shim event SHIMEVENT_SETSTATI for stat [%s] value [%d] "
                "isOk = %d\n",
                e->name, e->ivalue, e->okay);
          break;
        case SHIMEVENT_SETSTATF:
          if (debugEnabled)
            printf(
                "\nGot Shim event SHIMEVENT_SETSTATF for stat [%s] value [%f] "
                "isOk = %d\n",
                e->name, e->fvalue, e->okay);
          break;
        default:
  ...
  bool initialized() {
    return !userName.empty() && !lang.empty() &&
           (int)stats.size() >= EnumParser<SteamStatName>::getCount() &&
           (int)achievements.size() >=
               EnumParser<SteamAchievementName>::getCount();
  }
};
...
Steam::~Steam() { delete p; }

const std::string &Steam::userName() const { return p->userName; }

const std::string &Steam::lang() const { return p->lang; }
...
void Steam::unlock(const char *name) { p->setAchievement(name, true); }

void Steam::lock(const char *name) { p->setAchievement(name, false); }

bool Steam::isUnlocked(const char *name) { return p->isAchievementSet(name); }
...
SteamLocal::~SteamLocal() { delete p; }
...
void SteamLocal::storeStats() const { p->save(saveFilePlayerLocalStats); }

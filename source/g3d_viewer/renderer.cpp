...
Model *Renderer::newModel(
    ...
    string *sourceLoader) {
  return modelManager->newModel(path, deletePixMapAfterLoad, loadedFileList,
                                sourceLoader);
}
...
void Renderer::toggleNormals() { normals = normals ? false : true; }

void Renderer::toggleWireframe() { wireframe = wireframe ? false : true; }

void Renderer::toggleGrid() { grid = grid ? false : true; }
...
void Renderer::updateParticleManager() { particleManager->update(); }

bool Renderer::hasActiveParticleSystem(
...
Texture2D *Renderer::getPlayerColorTexture(PlayerColor playerColor) {
  ...
  case pcMagenta:
    ...
    break;
  default:
    throw megaglest_runtime_error("Unknown playercolor: " +
                                  intToStr(playerColor));
    break;
  }
...
void Renderer::initTextureManager() { textureManager->init(); }

void Renderer::initModelManager() { modelManager->init(); }
...
void Renderer::saveScreen(const string &path,
  ...
  // if(overrideSize != NULL && overrideSize->first > 0 && overrideSize->second
  // > 0) {

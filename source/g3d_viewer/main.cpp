...
void printParameterHelp(const char *argv0, bool foundInvalidArgs) {
  ...
  printf("\nPress R to restart particles, this also reloads all files if they "
         "are changed.");
  ...
  printf("\n\n                     \tare loading.");
  printf("\n\n                     \tWhere x is a comma-delimited list of one "
         "or more");
  ...
  printf("\n\n                     \tAuto load the projectile particle "
         "specified in");
  printf("\n\n                     \tpath/filename x.");
  printf("\n\n                     \tWhere x is a Projectile Particle "
         "Definition XML");
  printf("\n\n                     \t    filename to load.");
  printf("\n\n                     \texample:");
  printf("\n\n                     \t%s %s=techs/megapack/",
...
MainWindow::MainWindow(std::pair<string, vector<string>> unitToLoad,
    ...
    : wxFrame(NULL, -1, ToUnicode(winHeader),
      ...
      glCanvas(NULL), renderer(NULL), timer(NULL), menu(NULL), fileDialog(NULL),
      colorPicker(NULL), model(NULL), initTextureManager(true),
          ...
          for (unsigned int i = 0;
            ...
            if (skillXmlName == lookipForSkillName) {
              ...
              if (sn->hasChild("splash") == true) {
                ...
                // == "true" && 	j < particlesSplashNode->getChildCount();
                // ++j) {
    ...
    if (this->particlePathList.empty() == false) {
      ...
      for (unsigned int idx = 0; idx < this->particlePathList.size(); idx++) {
        ...
        if (fileExists(unitXML) == true) {
          ...
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] unit XML NOT FOUND [%s] using default "
                   "position values\n",
                   extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                   __LINE__, unitXML.c_str());
...
void MainWindow::loadProjectileParticle(string path) {
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("In [%s::%s Line: %d] about to load [%s] "
           ...
           extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
           __LINE__, path.c_str(), this->particleProjectilePathList.size());
...
void MainWindow::loadSplashParticle(
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("In [%s::%s Line: %d] about to load [%s] "
           "particleSplashPathList.size() = " MG_SIZE_T_SPECIFIER "\n",
           extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
           __LINE__, path.c_str(), this->particleSplashPathList.size());
  ...
  if (timer)
    timer->Start(100);
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf("In [%s::%s Line: %d] after load [%s] particleSplashPathList.size() "
           "= " MG_SIZE_T_SPECIFIER "\n",
           extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
           __LINE__, path.c_str(), this->particleSplashPathList.size());
}

...
void printParameterHelp(const char *argv0, bool foundInvalidArgs) {
  ...
  printf("\n%s %s, [Using %s]\n", extractFileFromDirectoryPath(argv0).c_str(),
         g3dviewerVersionString.c_str(),
         (const char *)wxConvCurrent->cWX2MB(wxVERSION_STRING));

  printf(
      "\nDisplays glest 3D-models and unit/projectile/splash particle "
      "systems.\n");
  printf(
      "\nRotate with left mouse button. Zoom with right mouse button or "
      "mousewheel.");
  printf("\nUse ctrl to load more than one particle system.");
  printf(
      "\nPress R to restart particles, this also reloads all files if they are "
      "changed.");
  ...
  printf("\n\n%s  \t\tDisplays this help text.",
         (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_HELP]));

  //         "================================================================================"
  printf("\n\n%s=x  \t\tAuto load the unit / skill information specified",
         (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_LOAD_UNIT]));
  printf("\n\n                     \tin path/filename x.");
  printf(
      "\n\n                     \tWhere x is a g3d filename to load separated "
      "with a");
  printf(
      "\n\n                     \tcomma and one or more skill names to try "
      "loading.");
  printf("\n\n                     \texample:");
  printf("\n\n                     \t%s %s=techs/megapack/factions/",
         extractFileFromDirectoryPath(argv0).c_str(),
         (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_LOAD_UNIT]));
  printf(
      "\n\n                     "
  ...
  printf("\n\n%s=x  \tAuto load the model specified in path/filename x.",
         (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_LOAD_MODEL]));
  ...
  printf("\n\n%s=x  ", (const char *)wxConvCurrent->cWX2MB(
                           GAME_ARGS[GAME_ARG_LOAD_MODEL_ANIMATION_VALUE]));
  ...
  printf("\n\n                     \t%s %s=0.5",
         extractFileFromDirectoryPath(argv0).c_str(),
         (const char *)wxConvCurrent->cWX2MB(
             GAME_ARGS[GAME_ARG_LOAD_MODEL_ANIMATION_VALUE]));

  //         "================================================================================"
  printf(
      "\n\n%s=x  \tAutomatically takes a screenshot of the items you",
      (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_AUTO_SCREENSHOT]));
  printf("\n\n                     \tare loading.");
  printf(
      "\n\n                     \tWhere x is a comma-delimited list of one or "
      "more");
  printf("\n\n                     \t    of the optional settings:");
  printf(
      "\n\n                     \t    transparent, enable_grid, "
      "enable_wireframe,");
  printf(
      "\n\n                     \t    enable_normals, disable_grid, "
      "disable_wireframe,");
  printf(
  ...
  printf("\n\n                     \t%s=techs/megapack/factions/tech/units/",
         (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_LOAD_MODEL]));
  printf(
      "\n\n                     "
  ...
  //         "================================================================================"
  printf(
      "\n\n%s=x  \tAuto load the particle specified in path/filename x.",
      (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_LOAD_PARTICLE]));
  printf(
  ...
  printf("\n\n%s=x  ", (const char *)wxConvCurrent->cWX2MB(
                           GAME_ARGS[GAME_ARG_LOAD_PARTICLE_PROJECTILE]));
  printf(
      "\n\n                     \tAuto load the projectile particle specified "
      "in");
  printf("\n\n                     \tpath/filename x.");
  printf(
      "\n\n                     \tWhere x is a Projectile Particle Definition "
      "XML");
  printf("\n\n                     \t    filename to load.");
  printf("\n\n                     \texample:");
  printf("\n\n                     \t%s %s=techs/megapack/",
  ...
  printf("\n\n%s=x  ", (const char *)wxConvCurrent->cWX2MB(
                           GAME_ARGS[GAME_ARG_LOAD_PARTICLE_SPLASH]));
  printf(
  ...
  printf("\n\n%s=x  ", (const char *)wxConvCurrent->cWX2MB(
                           GAME_ARGS[GAME_ARG_LOAD_PARTICLE_LOOP_VALUE]));
  printf(
      "\n\n                     \tParticle loop value when loading one or "
      "more");
  printf("\n\n                     \tparticles.");
  printf(
      "\n\n                     \tWhere x is an integer value from 1 to "
      "particle count.");
  printf("\n\n                     \texample:");
  printf("\n\n                     \t%s %s=25",
         extractFileFromDirectoryPath(argv0).c_str(),
  ...
  printf("\n\n%s=x  \tZoom value when loading a model.",
         (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_ZOOM_VALUE]));
  printf(
  ...
  printf(
      "\n\n%s=x  \tX Coordinate Rotation value when loading a model.",
      (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_ROTATE_X_VALUE]));
  printf(
      "\n\n                     \tWhere x is a decimal value from -10.0 to "
      "10.0");
  printf("\n\n                     \texample:");
  printf(
      "\n\n                     \t%s %s=2.2",
  ...
  printf(
      "\n\n%s=x  \tY Coordinate Rotation value when loading a model.",
      (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_ROTATE_Y_VALUE]));
  printf(
      "\n\n                     \tWhere x is a decimal value from -10.0 to "
      "10.0");
  printf("\n\n                     \texample:");
  ...
  printf("\n\n%s=x  \tSpecify which image format to use for screenshots.",
         (const char *)wxConvCurrent->cWX2MB(
             GAME_ARGS[GAME_ARG_SCREENSHOT_FORMAT]));
  printf(
      "\n\n                     \tWhere x is one of the following supported "
      "formats:");
  printf("\n\n                     \t    png,jpg,tga,bmp");
  printf(
      "\n\n                     \t*NOTE: png is the default (and supports "
      "transparency)");
  printf("\n\n                     \texample:");
  printf("\n\n                     \t%s %s=jpg",
         extractFileFromDirectoryPath(argv0).c_str(),
...
MainWindow::MainWindow(std::pair<string, vector<string> > unitToLoad,
                       ...
                       const string splashParticlePath, float defaultAnimation,
                       int defaultParticleLoopStart, float defaultZoom,
                       float defaultXRot, float defaultYRot, string appPath)
...

{
  ...
  // menu
  menuFile = new wxMenu();
  menuFile->Append(miFileLoad, wxT("&Load G3d model\tCtrl+L"),
                   wxT("Load 3D model"));
  menuFile->Append(miFileLoadParticleXML, wxT("Load &Particle XML\tCtrl+P"),
                   wxT("Press ctrl before menu for keeping current particles"));
  menuFile->Append(
      miFileLoadProjectileParticleXML,
      wxT("Load Pro&jectile Particle XML\tCtrl+J"),
      wxT("Press ctrl before menu for keeping current projectile particles"));
  menuFile->Append(
      miFileLoadSplashParticleXML, wxT("Load &Splash Particle XML\tCtrl+S"),
      wxT("Press ctrl before menu for keeping current splash particles"));
  menuFile->Append(miFileClearAll, wxT("&Clear All\tCtrl+C"));
  menuFile->AppendCheckItem(miFileToggleScreenshotTransparent,
                            wxT("&Transparent Screenshots\tCtrl+T"));
  ...
  menuSpeed->Append(miRestart, wxT("&Restart particles\tR"),
                    wxT("Restart particle animations, this also reloads model "
                        "and particle files if they are changed"));
  menu->Append(menuSpeed, wxT("&Speed"));

  // custom color
  menuCustomColor = new wxMenu();
  menuCustomColor->Append(miChangeBackgroundColor,
  ...
  menuCustomColor->AppendCheckItem(
      miColorMagenta,
      wxT("&Pink\t7"));  // it is called Pink everywhere else so...
  ...
  HKEY keyHandle;
  DWORD dwDisposition;
  RegCreateKeyEx(HKEY_CURRENT_USER, subKey.c_str(), 0, NULL, 0, KEY_ALL_ACCESS,
                 NULL, &keyHandle, &dwDisposition);
  // Set the value.
...
void MainWindow::onPaintNoEvent() {
  ...
  string statusTextValue =
      statusbarText + " animation speed: " + floatToStr(speed * 1000.0) +
      " anim value: " + floatToStr(anim) + " zoom: " + floatToStr(zoom) +
      " rotX: " + floatToStr(rotX) + " rotY: " + floatToStr(rotY);
  ...
  bool haveLoadedParticles = (particleProjectilePathList.empty() == false ||
                              particleSplashPathList.empty() == false);
  ...
  if ((modelPathList.empty() == false) && resetAnimation &&
      haveLoadedParticles) {
    ...
    if (renderer->hasActiveParticleSystem(
            ParticleSystem::pst_ProjectileParticleSystem) == false &&
        renderer->hasActiveParticleSystem(
            ParticleSystem::pst_SplashParticleSystem) == false) {
...
void MainWindow::onMouseWheelDown(wxMouseEvent &event) {
  try {
    ...
    wxMessageDialog(NULL, ToUnicode(e.what()), ToUnicode("Error"),
                    wxOK | wxICON_ERROR)
        .ShowModal();
  }
}
  ...
  try {
    ...
    if (isControlKeyPressed == true) {
      fileDialog->SetMessage(
          wxT("Adding Mega-Glest particle to current view."));
    } else {
      fileDialog->SetMessage(
          wxT("Selecting Mega-Glest particle for current view."));
    }
...
void MainWindow::onMenuFileLoadProjectileParticleXML(wxCommandEvent &event) {
  try {
    ...
    if (isControlKeyPressed == true) {
      fileDialog->SetMessage(
          wxT("Adding Mega-Glest projectile particle to current view."));
    } else {
      fileDialog->SetMessage(
          wxT("Selecting Mega-Glest projectile particle for current view."));
    }
...
void MainWindow::onMenuFileLoadSplashParticleXML(wxCommandEvent &event) {
  try {
    ...
    if (isControlKeyPressed == true) {
      fileDialog->SetMessage(
          wxT("Adding Mega-Glest splash particle to current view."));
    } else {
      fileDialog->SetMessage(
          wxT("Selecting Mega-Glest splash particle for current view."));
    }
...
}  // is it possible to join loadParticle(), loadProjectileParticle() and
   // loadSplashParticle() to one method?
...
void MainWindow::OnChangeColor(wxCommandEvent &event) {
  try {
    ...
    if (dialog.ShowModal() == wxID_OK) {
      ...
      renderer->setBackgroundColor(col.Red() / 255.0f, col.Green() / 255.0f,
                                   col.Blue() / 255.0f);
    }
  } catch (std::runtime_error &e) {
    std::cout << e.what() << std::endl;
...
void MainWindow::saveScreenshot() {
  try {
    ...
    if (autoSaveScreenshotIndex >= 0) {
      ...
      // string screenShotsPath = extractDirectoryPathFromFile(appPath) +
      // string("screens/");
  ...
  try {
    if (this->unitPath.first != "") {
      ...
      if (fileFound == true) {
        ...
        for (unsigned int skillIdx = 0;
             foundSkillName == false && skillIdx < this->unitPath.second.size();
             ++skillIdx) {
          string lookipForSkillName = this->unitPath.second[skillIdx];

          const XmlNode *skillsNode = unitNode->getChild("skills");
          for (unsigned int i = 0;
               foundSkillName == false && i < skillsNode->getChildCount();
               ++i) {
            ...
            string skillXmlName =
                nameNode->getAttribute("value")->getRestrictedValue();
            if (skillXmlName == lookipForSkillName) {
              printf("Found skill [%s]\n", lookipForSkillName.c_str());
              foundSkillName = true;

              if (sn->getChild("animation") != NULL) {
                skillModelFile = sn->getChild("animation")
                                     ->getAttribute("path")
                                     ->getRestrictedValue(unitPath + '/');
              ...
              if (sn->hasChild("particles") == true) {
                ...
                // for(int j = 0; particlesNode != NULL &&
                // particlesNode->getAttribute("value")->getRestrictedValue() ==
                // "true" && 	j < particlesNode->getChildCount(); ++j) {
                if (particlesNode != NULL &&
                    particlesNode->getAttribute("value")
                            ->getRestrictedValue() == "true" &&
                    particlesNode->hasChild("particle-file") == true) {
                  const XmlNode *pf = particlesNode->getChild("particle-file");
                  if (pf != NULL) {
                    skillParticleFile =
                        unitPath + '/' +
                        pf->getAttribute("path")->getRestrictedValue();
              ...
              if (sn->hasChild("projectile") == true) {
                const XmlNode *particlesProjectileNode =
                    sn->getChild("projectile");
                // for(int j = 0; particlesProjectileNode != NULL &&
                // particlesProjectileNode->getAttribute("value")->getRestrictedValue()
                // == "true" && 	j <
                // particlesProjectileNode->getChildCount();
                //++j) {
                if (particlesProjectileNode != NULL &&
                    particlesProjectileNode->getAttribute("value")
                            ->getRestrictedValue() == "true" &&
                    particlesProjectileNode->hasChild("particle") == true) {
                  const XmlNode *pf =
                      particlesProjectileNode->getChild("particle");
                  if (pf != NULL &&
                      pf->getAttribute("value")->getRestrictedValue() ==
                          "true") {
                    skillParticleProjectileFile =
                        unitPath + '/' +
                        pf->getAttribute("path")->getRestrictedValue();
              ...
              if (sn->hasChild("splash") == true) {
                ...
                // for(int j = 0; particlesSplashNode != NULL &&
                // particlesSplashNode->getAttribute("value")->getRestrictedValue()
                // == "true" && 	j <
                // particlesSplashNode->getChildCount(); ++j) {
                if (particlesSplashNode != NULL &&
                    particlesSplashNode->getAttribute("value")
                            ->getRestrictedValue() == "true" &&
        ...
        if (skillParticleProjectileFile != "") {
          this->particleProjectilePathList.push_back(
              skillParticleProjectileFile);
    ...
    wxMessageDialog(NULL, ToUnicode(e.what()),
                    ToUnicode("Not a Mega-Glest particle XML file, or broken"),
                    wxOK | wxICON_ERROR)
        .ShowModal();
  }
}
...
void MainWindow::loadModel(string path) {
  try {
    ...
    for (unsigned int idx = 0; idx < this->modelPathList.size(); idx++) {
      ...
      if (timer) timer->Start(100);
      titlestring =
          extractFileFromDirectoryPath(modelPath) + " - " + titlestring;
    }
    SetTitle(ToUnicode(titlestring));
  } catch (std::runtime_error &e) {
    std::cout << e.what() << std::endl;
  ...
  try {
    if (this->particlePathList.empty() == false) {
      ...
      for (unsigned int idx = 0; idx < this->particlePathList.size(); idx++) {
        ...
        std::string unitXML =
            dir + folderDelimiter + extractFileFromDirectoryPath(dir) + ".xml";

        if (SystemFlags::VERBOSE_MODE_ENABLED)
          printf("In [%s::%s Line: %d] looking for unit XML [%s]\n",
                 extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                 __LINE__, unitXML.c_str());
        ...
        if (fileExists(unitXML) == true) {
          {
            ...
            // size
            size = parametersNode->getChild("size")
                       ->getAttribute("value")
                       ->getIntValue();
            // height
            height = parametersNode->getChild("height")
                         ->getAttribute("value")
                         ->getIntValue();
          }

          // std::cout << "About to load [" << particlePath << "] from [" << dir
          // << "] unit [" << unitXML << "]" << std::endl;
        } else {
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf(
                "In [%s::%s Line: %d] unit XML NOT FOUND [%s] using default "
                "position values\n",
                extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                __LINE__, unitXML.c_str());
        ...
        unitParticleSystemType->load(
            NULL, dir,
            dir + folderDelimiter +
                particlePath,  // ### if we knew which particle it was, we could
                               // be more accurate
            renderer, loadedFileList, "g3dviewer", "");
        unitParticleSystemTypes.push_back(unitParticleSystemType);

        for (std::vector<UnitParticleSystemType *>::const_iterator it =
          ...
          ups->setFactionColor(renderer->getPlayerColorTexture(playerColor)
                                   ->getPixmap()
                                   ->getPixel3f(0, 0));
...
void MainWindow::loadProjectileParticle(string path) {
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf(
        "In [%s::%s Line: %d] about to load [%s] "
        "particleProjectilePathList.size() = " MG_SIZE_T_SPECIFIER "\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        path.c_str(), this->particleProjectilePathList.size());
  ...
  try {
    if (this->particleProjectilePathList.empty() == false) {
      if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("this->particleProjectilePathList.size() = " MG_SIZE_T_SPECIFIER
               "\n",
               this->particleProjectilePathList.size());

      string titlestring = winHeader;
      for (unsigned int idx = 0; idx < this->particleProjectilePathList.size();
           idx++) {
        ...
        string particleFile = dir + folderDelimiter + particlePath;
        {
          if (SystemFlags::VERBOSE_MODE_ENABLED)
            printf("In [%s::%s Line: %d] loading [%s] idx = %u\n",
                   extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__,
                   __LINE__, particleFile.c_str(), idx);
          XmlTree xmlTree;
        ...
        projectileParticleSystemType->load(
            NULL,
            dir,  // ### we don't know if there are overrides in the unit XML
            particleFile, renderer, loadedFileList, "g3dviewer", "");

        // std::cout << "Values loaded, about to read..." << std::endl;

        projectileParticleSystemTypes.push_back(projectileParticleSystemType);

        for (std::vector<ParticleSystemTypeProjectile *>::const_iterator it =
                 projectileParticleSystemTypes.begin();
          ...
          ps->setFactionColor(renderer->getPlayerColorTexture(playerColor)
                                  ->getPixmap()
                                  ->getPixel3f(0, 0));
    ...
    wxMessageDialog(
        ...
        ToUnicode("Not a Mega-Glest projectile particle XML file, or broken"),
        wxOK | wxICON_ERROR)
        .ShowModal();
  }
  if (timer) timer->Start(100);
...
void MainWindow::loadSplashParticle(
    string path) {  // uses ParticleSystemTypeSplash::load  (and own list...)
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf(
        "In [%s::%s Line: %d] about to load [%s] particleSplashPathList.size() "
        "= " MG_SIZE_T_SPECIFIER "\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        path.c_str(), this->particleSplashPathList.size());
  ...
  try {
    if (this->particleSplashPathList.empty() == false) {
      string titlestring = winHeader;
      for (unsigned int idx = 0; idx < this->particleSplashPathList.size();
           idx++) {
        ...
        if (fileExists(unitXML) == true) {
          ...
          // height
          // height=
          // parametersNode->getChild("height")->getAttribute("value")->getIntValue();
        }
...

        {
          ...
          // const XmlNode *particleSystemNode= xmlTree.getRootNode();
          //  std::cout << "Loaded successfully, loading values..." <<
          //  std::endl;
        }

        std::map<string, vector<pair<string, string> > > loadedFileList;
        ParticleSystemTypeSplash *splashParticleSystemType =
            new ParticleSystemTypeSplash();
        splashParticleSystemType->load(
            NULL, dir, dir + folderDelimiter + particlePath,
            renderer,  // ### we don't know if there are overrides in the unit
                       // XML
            loadedFileList, "g3dviewer",
            "");  // <---- only that must be splash...
        ...
        for (std::vector<ParticleSystemTypeSplash *>::const_iterator it =
                 splashParticleSystemTypes.begin();
             it != splashParticleSystemTypes.end(); ++it) {
          ...
          if (size > 0) {
            ...
            // Vec3f vec2 = Vec3f(size * 2.f, height * 2.f, height * 2.f);   //
            // <------- removed relative projectile ps->setPath(vec, vec2); //
            // <------- removed relative projectile
          }
          ps->setFactionColor(renderer->getPlayerColorTexture(playerColor)
                                  ->getPixmap()
                                  ->getPixel3f(0, 0));
  ...
  if (SystemFlags::VERBOSE_MODE_ENABLED)
    printf(
        "In [%s::%s Line: %d] after load [%s] particleSplashPathList.size() "
        "= " MG_SIZE_T_SPECIFIER "\n",
        extractFileFromDirectoryPath(__FILE__).c_str(), __FUNCTION__, __LINE__,
        path.c_str(), this->particleSplashPathList.size());
}
...
void MainWindow::onKeyDown(wxKeyEvent &e) {
  try {
    // std::cout << "e.ControlDown() = " << e.ControlDown() << " e.GetKeyCode()
    // = " << e.GetKeyCode() << " isCtrl = " << (e.GetKeyCode() == WXK_CONTROL)
    // << std::endl;

    // Note: This ctrl-key handling is buggy since it never resets when ctrl is
    // released later, so I reset it at end of loadcommands for now.
    ...
    // std::cout << "isControlKeyPressed = " << isControlKeyPressed <<
    // std::endl;
    ...
    if (e.GetKeyCode() == 388) {
      ...
      glClearColor(0.6f, 0.6f, 0.6f,
                   1.0f);  // w  key //backgroundcolor constant 0.3 -> 0.6
    ...
    /*
            ...
            gridBrightness += 0.1f; if (gridBrightness >1.0) gridBrightness
    =1.0;
    }
...
EVT_MENU(miFileLoadProjectileParticleXML,
         MainWindow::onMenuFileLoadProjectileParticleXML)
EVT_MENU(miFileLoadSplashParticleXML,
         MainWindow::onMenuFileLoadSplashParticleXML)
EVT_MENU(miFileClearAll, MainWindow::onMenuFileClearAll)
EVT_MENU(miFileToggleScreenshotTransparent,
         MainWindow::onMenumFileToggleScreenshotTransparent)
...
#if wxCHECK_VERSION(2, 9, 1)
    : wxGLCanvas(mainWindow, wxID_ANY, args, wxDefaultPosition,
                 mainWindow->GetClientSize(), wxFULL_REPAINT_ON_RESIZE,
                 wxT("GLCanvas")) {
  this->context = new wxGLContext(this);
#else
    : wxGLCanvas(mainWindow, -1, wxDefaultPosition, wxDefaultSize, 0,
                 wxT("GLCanvas"), args) {
  this->context = NULL;
#endif
...
bool App::OnInit() {
  ...
  for (int idx = 1; idx < argc; ++idx) {
    ...
    const wxWX2MBbuf tmp_buf = wxConvCurrent->cWX2MB(argv[idx]);
#endif
    if (hasCommandArgument(knownArgCount, (wxChar **)&GAME_ARGS[0],
                           (const char *)tmp_buf, NULL, 0, true) == false &&
        argv[idx][0] == '-') {
  ...
  if (foundInvalidArgs == true ||
      hasCommandArgument(argc, argv,
                         (const char *)wxConvCurrent->cWX2MB(
                             GAME_ARGS[GAME_ARG_HELP])) == true) {
    printParameterHelp(static_cast<const char *>(WX2CHR(argv[0])),
                       foundInvalidArgs);
    return false;
  }
  ...
  if (hasCommandArgument(argc, argv,
                         (const char *)wxConvCurrent->cWX2MB(
                             GAME_ARGS[GAME_ARG_VERBOSE])) == true) {
    SystemFlags::VERBOSE_MODE_ENABLED = true;
  }

  if (hasCommandArgument(argc, argv,
                         (const char *)wxConvCurrent->cWX2MB(
                             GAME_ARGS[GAME_ARG_AUTO_SCREENSHOT])) == true) {
    autoScreenShotAndExit = true;

    const wxWX2MBbuf param = (const char *)wxConvCurrent->cWX2MB(
        GAME_ARGS[GAME_ARG_AUTO_SCREENSHOT]);
    // printf("param = [%s]\n",(const char*)param);

    int foundParamIndIndex = -1;
    hasCommandArgument(argc, argv, string((const char *)param) + string("="),
                       &foundParamIndIndex);
...
#else
    string options =
        (const char *)wxConvCurrent->cWX2MB(argv[foundParamIndIndex]);
#endif
    ...
    if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
      ...
      for (unsigned int i = 0; i < autoScreenShotParams.size(); ++i) {
#ifdef WIN32
        auto_ptr<wchar_t> wstr(
            Ansi2WideString(autoScreenShotParams[i].c_str()));
  ...
  if (hasCommandArgument(argc, argv,
                         (const char *)wxConvCurrent->cWX2MB(
                             GAME_ARGS[GAME_ARG_LOAD_UNIT])) == true) {
    const wxWX2MBbuf param =
        (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_LOAD_UNIT]);
    // printf("param = [%s]\n",(const char*)param);
    ...
    // printf("foundParamIndIndex = %d\n",foundParamIndIndex);
    string customPath =
        static_cast<const char *>(WX2CHR(argv[foundParamIndIndex]));
    vector<string> paramPartTokens;
  ...
  if (hasCommandArgument(argc, argv,
                         (const char *)wxConvCurrent->cWX2MB(
                             GAME_ARGS[GAME_ARG_LOAD_MODEL])) == true &&
      hasCommandArgument(argc, argv,
                         (const char *)wxConvCurrent->cWX2MB(
                             GAME_ARGS[GAME_ARG_LOAD_MODEL_ANIMATION_VALUE])) ==
          false) {
    ...
    if (paramPartTokens.size() >= 2 && paramPartTokens[1].length() > 0) {
      ...
      printf("\nInvalid path specified on commandline [%s] value [%s]\n\n",
             (const char *)WX2CHR(argv[foundParamIndIndex]),
             (paramPartTokens.size() >= 2 ? paramPartTokens[1].c_str() : NULL));
      printParameterHelp(WX2CHR(argv[0]), false);
      return false;
    }
  ...
  if (hasCommandArgument(argc, argv,
                         (const char *)wxConvCurrent->cWX2MB(
                             GAME_ARGS[GAME_ARG_LOAD_PARTICLE])) == true &&
      hasCommandArgument(argc, argv,
                         (const char *)wxConvCurrent->cWX2MB(
                             GAME_ARGS[GAME_ARG_LOAD_PARTICLE_LOOP_VALUE])) ==
          false) {
  ...
  if (hasCommandArgument(argc, argv,
                         (const char *)wxConvCurrent->cWX2MB(
                             GAME_ARGS[GAME_ARG_ZOOM_VALUE])) == true) {
    const wxWX2MBbuf param =
        (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_ZOOM_VALUE]);
  ...
  if (hasCommandArgument(argc, argv,
                         (const char *)wxConvCurrent->cWX2MB(
                             GAME_ARGS[GAME_ARG_ROTATE_X_VALUE])) == true) {
    const wxWX2MBbuf param =
        (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_ROTATE_X_VALUE]);
  ...
  if (hasCommandArgument(argc, argv,
                         (const char *)wxConvCurrent->cWX2MB(
                             GAME_ARGS[GAME_ARG_ROTATE_Y_VALUE])) == true) {
    const wxWX2MBbuf param =
        (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_ROTATE_Y_VALUE]);
  ...
  if (hasCommandArgument(argc, argv,
                         (const char *)wxConvCurrent->cWX2MB(
                             GAME_ARGS[GAME_ARG_SCREENSHOT_FORMAT])) == true) {
    const wxWX2MBbuf param = (const char *)wxConvCurrent->cWX2MB(
        GAME_ARGS[GAME_ARG_SCREENSHOT_FORMAT]);
  ...
  // wxConvCurrent->cWX2MB(wxFNCONV(argv[0])); 		appPath = tmp_buf; #else
  // appPath = wxFNCONV(argv[0]); #endif 	printf("appPath
  // [%s]\n",argv[0]);
  ...
  mainWindow = new MainWindow(unitToLoad, modelPath, particlePath,
                              projectileParticlePath, splashParticlePath,
                              newAnimValue, newParticleLoopValue, newZoomValue,
                              newXRotValue, newYRotValue, appPath);
...
int App::MainLoop() {
  try {
    ...
    wxMessageDialog(NULL, ToUnicode(e.what()), ToUnicode("Exception"),
                    wxOK | wxICON_ERROR)
        .ShowModal();
    return 1;
  }

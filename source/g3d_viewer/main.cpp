...
bool hasCommandArgument(int argc, wxChar **argv, const string &argName, int *foundIndex = NULL, int startLookupIndex = 1, bool useArgParamLen = false) {
    bool result = false;
...
void printParameterHelp(const char *argv0, bool foundInvalidArgs) {
    ...
    //         "================================================================================"
    printf("\n%s %s, [Using %s]\n", extractFileFromDirectoryPath(argv0).c_str(), g3dviewerVersionString.c_str(),
    ...
    printf("\n\n                     \tcomma and one or more skill names to try "
           "loading.");
    printf("\n\n                     \texample:");
    printf("\n\n                     \t%s %s=techs/megapack/factions/", extractFileFromDirectoryPath(argv0).c_str(),
    ...
    printf("\n\n                     \texample:");
    printf("\n\n                     \t%s %s=0.5", extractFileFromDirectoryPath(argv0).c_str(),
           (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_LOAD_MODEL_ANIMATION_VALUE]));
    ...
    printf("\n\n                     \t    disable_normals, saveas-<filename>, "
           "resize-wxh");
    printf("\n\n                     \texample:");
    printf("\n\n                     \t%s %s=transparent,", extractFileFromDirectoryPath(argv0).c_str(),
    ...
    printf("\n\n                     \t    filename to load.");
    printf("\n\n                     \texample:");
    printf("\n\n                     \t%s %s=techs/megapack/", extractFileFromDirectoryPath(argv0).c_str(),
           (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_LOAD_PARTICLE_PROJECTILE]));
    printf("\n\n                     "
    ...
    printf("\n\n                     \texample:");
    printf("\n\n                     \t%s %s=techs/megapack/", extractFileFromDirectoryPath(argv0).c_str(),
           (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_LOAD_PARTICLE_SPLASH]));
    printf("\n\n                     "
    ...
    printf("\n\n                     \tWhere x is an integer value from 1 to "
           "particle count.");
    printf("\n\n                     \texample:");
    printf("\n\n                     \t%s %s=25", extractFileFromDirectoryPath(argv0).c_str(),
           (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_LOAD_PARTICLE_LOOP_VALUE]));
    ...
    printf("\n\n                     \texample:");
    printf("\n\n                     \t%s %s=4.2", extractFileFromDirectoryPath(argv0).c_str(),
    ...
    printf("\n\n                     \tWhere x is a decimal value from -10.0 to "
           "10.0");
    printf("\n\n                     \texample:");
    printf("\n\n                     \t%s %s=2.2", extractFileFromDirectoryPath(argv0).c_str(),
    ...
    printf("\n\n                     \t*NOTE: png is the default (and supports "
           "transparency)");
    printf("\n\n                     \texample:");
    printf("\n\n                     \t%s %s=jpg", extractFileFromDirectoryPath(argv0).c_str(),
           (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_SCREENSHOT_FORMAT]));
...
MainWindow::MainWindow(std::pair<string, vector<string>> unitToLoad, const string modelPath, const string particlePath, const string projectileParticlePath,
                       const string splashParticlePath, float defaultAnimation, int defaultParticleLoopStart, float defaultZoom, float defaultXRot,
                       float defaultYRot, string appPath)
    : wxFrame(NULL, -1, ToUnicode(winHeader), wxPoint(Renderer::windowX, Renderer::windowY), wxSize(Renderer::windowW, Renderer::windowH)), glCanvas(NULL),
      renderer(NULL), timer(NULL), menu(NULL), fileDialog(NULL), colorPicker(NULL), model(NULL), initTextureManager(true), startupSettingsInited(false)

{
    ...
    menuFile->Append(miFileLoadProjectileParticleXML, wxT("Load Pro&jectile Particle XML\tCtrl+J"),
    ...
    menuCustomColor->AppendCheckItem(miColorMagenta,
...
void MainWindow::onPaintNoEvent() {
    ...
    string statusTextValue = statusbarText + " animation speed: " + floatToStr(speed * 1000.0) + " anim value: " + floatToStr(anim) +
                             " zoom: " + floatToStr(zoom) + " rotX: " + floatToStr(rotX) + " rotY: " + floatToStr(rotY);
    ...
    try {
        if (this->particlePathList.empty() == false) {
            ...
            for (unsigned int idx = 0; idx < this->particlePathList.size(); idx++) {
                ...
                unitParticleSystemType->load(NULL, dir,
                                             dir + folderDelimiter + particlePath, // ### if we knew which particle it was, we could
                                                                                   // be more accurate
    ...
    try {
        if (this->particleProjectilePathList.empty() == false) {
            ...
            for (unsigned int idx = 0; idx < this->particleProjectilePathList.size(); idx++) {
                ...
                projectileParticleSystemType->load(NULL,
...
void MainWindow::loadSplashParticle(string path) { // uses ParticleSystemTypeSplash::load  (and own list...)
    if (SystemFlags::VERBOSE_MODE_ENABLED)
        printf("In [%s::%s Line: %d] about to load [%s] particleSplashPathList.size() "
               "= " MG_SIZE_T_SPECIFIER "\n",
    ...
    try {
        if (this->particleSplashPathList.empty() == false) {
            ...
            for (unsigned int idx = 0; idx < this->particleSplashPathList.size(); idx++) {
                ...
                splashParticleSystemType->load(NULL, dir, dir + folderDelimiter + particlePath,
                ...
                for (std::vector<ParticleSystemTypeSplash *>::const_iterator it = splashParticleSystemTypes.begin(); it != splashParticleSystemTypes.end();
                     ++it) {
...
bool App::OnInit() {
    ...
    if (foundInvalidArgs == true || hasCommandArgument(argc, argv, (const char *)wxConvCurrent->cWX2MB(GAME_ARGS[GAME_ARG_HELP])) == true) {
